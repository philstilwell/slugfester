import { renderAssessmentProcessGuide } from "./data/assessment-process-guide.js?v=6a9b7e854492c801";
import { topicCategoryDefinitions } from "./data/topics.js?v=6a9b7e854492c801";
import { assessmentGuide, debateSectionAnchor, relatedDebates } from "./data/reader-guides.js?v=6a9b7e854492c801";
import { debateSummaries } from "./data/debate-summaries.js?v=6a9b7e854492c801";
import { avatarsForSpeakerText } from "./data/interlocutors.js?v=6a9b7e854492c801";
import { getReferenceDefinition, referenceFromUrl } from "./data/references.js?v=6a9b7e854492c801";
import {
  DEFAULT_IMAGE_ALT,
  ...
  topicsPath,
  topicsSeo
} from "./seo.js?v=6a9b7e854492c801";
...
async function loadDebateAnalytics() {
  if (!debateAnalyticsPromise) {
    debateAnalyticsPromise = import("./data/debate-analytics.js?v=6a9b7e854492c801")
...
async function loadSectionScoreExtremes() {
  if (!sectionScoreExtremesPromise) {
    sectionScoreExtremesPromise = import("./data/section-score-extremes.js?v=6a9b7e854492c801")
...
async function loadDebateDetail(id) {
  if (!debateDetailPromises.has(id)) {
    const promise = import(`./data/debate-details/${id}.js?v=6a9b7e854492c801`)
      .then(({ debate }) => debate)
      .catch((error) => {
...
async function loadReferenceAppearances(type, slug) {
  ...
  if (!referenceAppearancePromises.has(key)) {
    const promise = import(`./data/reference-appearances/${type}-${slug}.js?v=6a9b7e854492c801`)
      ...
      <section class="search-tool" aria-label="Search filters">
        ...
        <details class="filter-section filter-accordion" ${state.people.length ? "open" : ""}>
          <summary class="filter-heading">
            <span class="filter-photo-preview" aria-hidden="true">
              <img src="/assets/mascots/logo-rival-brown.jpg" alt="" width="52" height="52" />
              <img src="/assets/mascots/logo-rival-white.jpg" alt="" width="52" height="52" />
            </span>
...
function renderSourceContext(debate) {
  if (!debate.assessmentScopeDisclosure) return "";
  return `
    <section class="scoring-note source-context" aria-label="Source and assessment scope">
      <strong>Source and assessment scope</strong>
      <span>${escapeHtml(debate.assessmentScopeDisclosure)}</span>
    </section>
  `;
}
...
function hasInitialRouteContent() {
  return !window.location.hash.startsWith("#/") &&
    app.querySelector("[data-initial-path]")?.dataset.initialPath === window.location.pathname;
}

function renderRouteLoading() {
  if (hasInitialRouteContent()) return;
  app.innerHTML = renderShell(`
...
async function route({ focusMain = false } = {}) {
  ...
  if (interlocutorMatch && !biographies) {
    biographiesPromise ||= import("./data/interlocutor-bios.js?v=6a9b7e854492c801")
  ...
  if (insightsMatch && window.location.pathname.includes("/data-and-methods") && !insightsMethodsContent) {
    insightsMethodsPromise ||= import("./data/insights-methods.js?v=6a9b7e854492c801")
  ...
  if (insightsMatch && !insightsContent) {
    insightsPromise ||= import("./data/insights.js?v=6a9b7e854492c801")
  ...
  if (loaders.length) {
    ...
    try {
      ...
      if (hasInitialRouteContent()) {
        const main = app.querySelector("main");
        main.querySelector(".initial-load-status")?.remove();
        main.insertAdjacentHTML("afterbegin", `<p class="initial-load-status" role="status">Interactive data could not load. The published summary remains available below. <a href="${escapeHtml(window.location.href)}">Retry loading the interactive page</a>.</p>`);
        return;
      }
      app.innerHTML = renderShell(`
        <main class="not-found">
