...
const PATH = 'docs/assessment-production/post-canary-continuation-v1/batch-07/audio-verification/correction-2-execution-activation.json';
...
if (shouldWrite) {
  assert(activation.status === 'frozen-batch-07-audio-validation-correction-2-prepared-not-active', 'inactive activation unavailable');
  activation.status = 'frozen-batch-07-audio-validation-correction-2-authorized-under-standing-authorization';
  ...
  activation.nextAuthorizedAction = 'execute-exactly-one-batch-07-audio-validation-correction-2-pass';
  await writeFile(PATH, `${JSON.stringify(activation, null, 2)}\n`);
  console.log(`activated ${PATH}`);
} else {
  assert(activation.status === 'frozen-batch-07-audio-validation-correction-2-authorized-under-standing-authorization', 'correction activation is not active');
  ...
  console.log('Batch 7 audio validation correction-2 activation is valid.');
}
