...
const ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-01/discovery";
...
const SCRIPT =
  "scripts/run-assessment-production-post-canary-batch-01-discovery.mjs";
...
const EXPECTED_PREPARATION_STATUS =
  "frozen-thirty-one-post-canary-batch-01-discovery-contexts-prepared-not-authorized";
const EXPECTED_ACTIVATION_STATUS =
  "frozen-thirty-one-post-canary-batch-01-discovery-contexts-authorized";
  ...
  assertV4(
    preparation.status === EXPECTED_PREPARATION_STATUS &&
      preparation.protocolId ===
        "assessment-production-post-canary-batch-01-discovery" &&
      ...
      preparation.executionPolicy.copiedInputBytesMaximum === 82445 &&
    ...
    "Batch 1 frozen discovery preparation is invalid"
  );
...
async function preflight() {
  ...
  assertV4(
    preparationValidation.status ===
      "batch-01-discovery-execution-manifest-validation-passed-frozen-awaiting-separate-model-execution-approval" &&
    ...
    "Batch 1 preparation validation is invalid"
  );
...
async function activate() {
  ...
  const activation = {
    ...
    userAuthorization: {
      instruction:
        "I approve execution of exactly the frozen 31 Batch 1 score-blind discovery contexts using 5.6 Sol with low reasoning effort through my ChatGPT subscription, with a direct incremental cost cap of $0. Stop after discovery validation, analysis, commit, and push. Do not run inventory models, judgment models, paid audio or transcription, score derivation, publication reconstruction, or production mutation.",
    ...
    nextRequiredAction:
      "execute-exactly-thirty-one-frozen-batch-01-discovery-contexts-once",
  };
  if (shouldWrite) {
...
async function loadAndValidateActivation() {
  ...
  assertV4(
    activation.status === EXPECTED_ACTIVATION_STATUS &&
      ...
      activation.executionPolicy.copiedInputBytesMaximum === 82445 &&
    ...
    "Batch 1 discovery execution is unauthorized"
  );
  ...
  assertV4(
    ...
    "frozen Batch 1 context count drifted"
  );
...
async function execute() {
  ...
  async function executeContext(context, contextIndex) {
    ...
    const sourceDirectory = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-post-canary-batch-01-discovery-${context.debateNumber}-${context.chunkId}-`
      )
    );
    const isolatedCodexHome = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-post-canary-batch-01-discovery-home-${context.debateNumber}-${context.chunkId}-`
      )
    );
    ...
    process.stdout.write(
      `[post-canary-batch-01-discovery] Debate ${context.debateNumber} ${context.chunkId} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
    );
    return record;
  }
  ...
  const execution = {
    ...
    status:
      validContexts === preparation.contexts.length
        ...
        : "post-canary-batch-01-discovery-complete-with-failure",
    productionContinuation: true,
    developmentValidationOnly: false,
    productionCanary: false,
    stagingOnly: true,
...
async function analyze() {
  ...
  assertV4(
    ...
    "all Batch 1 discovery contexts must pass without retry before analysis"
  );
  if (shouldWrite) {
    assertV4(
  ...
  const analysis = {
    ...
    status: candidateMinimumPassed
      ...
      : "post-canary-batch-01-discovery-failed-candidate-minimum-stop-no-retry",
    productionContinuation: true,
    developmentValidationOnly: false,
    productionCanary: false,
    stagingOnly: true,
...
if (command === "preflight") await preflight();
  ...
  throw new Error(
    "usage: run-assessment-production-post-canary-batch-01-discovery.mjs <preflight|activate|run|analyze|validate>"
  );
}
