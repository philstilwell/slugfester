...
const MANIFEST =
  "docs/assessment-production/production-checkpoint-v2.2-1/discovery/execution-preparation-manifest.json";
...
assert.equal(manifest.contexts.filter((context) => context.sourceChainOverlayApplied).length, 3);
...
assert.equal(manifest.executionPolicy.copiedInputBytesMaximum, 82270);
assert.equal(preparation.totals.maximumCopiedInputBytes, 82270);
assert.equal(manifest.copiedInputBoundary.partitionContextBytesMaximum, 70000);
assert.equal(
  manifest.copiedInputBoundary.historicalValidationCopiedInputBytesMaximum,
  70000
);
assert.equal(
  manifest.copiedInputBoundary.frozenObservedCopiedInputBytesMaximum,
  82270
);
assert.equal(
  manifest.copiedInputBoundary.executionCopiedInputBytesMaximum,
  82270
);
assert.deepEqual(manifest.copiedInputBoundary.maximumContext, {
  debateNumber: "167",
  chunkId: "chunk-002",
});
...
assert.deepEqual(manifest.costEstimate.expectedParallelWallMinutes, [18, 35]);
...
assert.equal(
  manifest.isolation
    .pluginsAppsMemoriesSkillsBrowsingComputerUseAndMultiAgentUnavailable,
  true
);
