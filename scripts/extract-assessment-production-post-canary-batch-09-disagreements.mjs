...
import {
  ...
  ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_09_DISAGREEMENT_ROOT,
  extractAssessmentProductionPostCanaryBatch09Disagreements
} from "./lib/assessment-production-post-canary-batch-09-disagreement.mjs";
import {
...
const USER_AUTHORIZATION = Object.freeze({
  ...
  standingAuthorizationPath: POST_CANARY_BATCH_09_STANDING_AUTHORIZATION,
...
const TOOL_SOURCES = [
  "scripts/extract-assessment-production-post-canary-batch-09-disagreements.mjs",
  "scripts/lib/assessment-production-post-canary-batch-09-disagreement.mjs",
  ...
  "scripts/test-assessment-production-post-canary-batch-09-disagreements.mjs"
];
...
assertV4(
  judgmentAnalysis.status ===
    ...
    judgmentAnalysis.nextAuthorizedAction ===
      "extract-freeze-and-analyze-batch-09-disagreements-under-standing-authorization",
  "Batch 9 accepted-judgment gate or prior stop rule changed"
);
assertV4(
  JSON.stringify(judgmentAnalysis.pairs.map((pair) => pair.debateNumber)) ===
    JSON.stringify(EXPECTED_DEBATES),
  "Batch 9 independent-judgment population changed"
);
assertV4(
  ...
  "Batch 9 does not contain ten accepted, independent A/B judgment pairs"
);
assertV4(
  ...
  "Batch 9 twenty-context accepted execution contract changed"
);
assertV4(
  ...
  "Batch 9 model, authentication, score-blindness, or tie contract changed"
);
assertV4(
  executionActivation.executionPolicy.attemptsPerContext === 1 &&
    ...
    JSON.stringify(executionActivation.executionPolicy.schedulerRamp) ===
      JSON.stringify([1, 2]),
  "Batch 9 one-attempt or frozen 1-to-2 scheduler stop rule changed"
);
assertV4(
...
assertV4(
  ...
  "Batch 9 source-compatibility evidence changed"
);
assertV4(
  ...
  "Batch 9 accepted execution evidence changed"
);
...
const sourceHashes = {
  ...
  [POST_CANARY_BATCH_09_STANDING_AUTHORIZATION]: standingAuthorization.sha256
};
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const disagreementPath =
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_09_DISAGREEMENT_ROOT}/disagreements/debate-${debateNumber}.json`;
...
assertV4(totalMoves === 180, "Batch 9 extraction must compare 180 moves");
assertV4(
  JSON.stringify(extractedAudioQueue.map(queueKey).sort()) ===
    JSON.stringify(independentGateQueue.map(queueKey).sort()),
  "Batch 9 frozen audio queue was not reproduced"
);

const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-09-disagreement-extraction",
  protocolId:
    ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_09_DISAGREEMENT_PROTOCOL_ID,
  status:
  ...
  standingAuthorization: {
    path: POST_CANARY_BATCH_09_STANDING_AUTHORIZATION,
  ...
  nextAuthorizedAction:
    "prepare-freeze-and-analyze-batch-09-local-audio-source-work-items-under-standing-authorization"
};

if (shouldWrite) {
  await mkdir(ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_09_DISAGREEMENT_ROOT, {
    recursive: true
  });
  await writeFile(
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_09_DISAGREEMENT_ROOT}/analysis.json`,
