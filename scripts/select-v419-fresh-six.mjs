...
const v417SamplePath = "docs/calibration/v4.1.7/fresh-six-gate/source-only-sample.json";
const v418SamplePath = "docs/calibration/v4.1.8/source-integrity-fresh-six-gate/source-only-sample.json";
const workflowPath = "docs/assessment-workflow-v4.1.9.md";
const scriptPath = "scripts/select-v419-fresh-six.mjs";
const outputPath = "docs/calibration/v4.1.9/schema-bounded-fresh-six-gate/source-only-sample.json";
const salt = "slugfester-v4.1.9-schema-bounded-fresh-six-source-only";
...
const [poolText, v417Text, v418Text] = await Promise.all([poolPath, v417SamplePath, v418SamplePath].map((file) => readFile(path.resolve(root, file), "utf8")));
const pool = JSON.parse(poolText);
const priorSamples = [JSON.parse(v417Text), JSON.parse(v418Text)];
...
const v417Ids = priorSamples[0].debates.map((debate) => debate.debateId);
const v418Ids = priorSamples[1].debates.map((debate) => debate.debateId);
const exclusions = new Set([...pool.retiredDebateIds, ...v416DevelopmentIds, ...v417Ids, ...v418Ids]);
...
if (new Set(selected.map((candidate) => candidate.debateId)).size !== 6 || !selected.some((candidate) => candidate.durationSeconds < 5400) || !selected.some((candidate) => candidate.durationSeconds > 7200)) throw new Error("v4.1.9 fresh-six selection invariants failed");
if (selected.some((candidate) => v417Ids.includes(candidate.debateId) || v418Ids.includes(candidate.debateId))) throw new Error("v4.1.9 sample overlaps a prior fresh gate");

const sourceHashes = {
  [poolPath]: sha256(poolText),
  [v417SamplePath]: sha256(v417Text),
  [v418SamplePath]: sha256(v418Text),
...
const artifact = {
  schemaVersion: "4.1.9-schema-bounded-fresh-six-sample",
  ...
  exclusions: { priorCalibrationIdsFromPool: pool.retiredDebateIds.length, v416DevelopmentDebates: ["55", "103", "161"], v417DiagnosticDebates: priorSamples[0].debates.map((debate) => debate.number), v418DiagnosticDebates: priorSamples[1].debates.map((debate) => debate.number), excludedDebateIds: [...exclusions].sort() },
  ...
  audit: { debates: selected.length, distinctTopicFamilies: new Set(selected.map((debate) => debate.family)).size, distinctDebateIds: new Set(selected.map((debate) => debate.debateId)).size, v417Overlap: selected.filter((debate) => v417Ids.includes(debate.debateId)).length, v418Overlap: selected.filter((debate) => v418Ids.includes(debate.debateId)).length, minimumDurationSeconds: Math.min(...selected.map((debate) => debate.durationSeconds)), maximumDurationSeconds: Math.max(...selected.map((debate) => debate.durationSeconds)), localTranscriptChainsPresent: selected.filter((debate) => debate.transcriptChainPresentAtSelection).length, legacyAssessmentFields: 0 },
...
console.log(JSON.stringify({ status: shouldWrite ? "frozen" : "preview", debates: selected.map((debate) => ({ number: debate.number, debateId: debate.debateId, family: debate.family, durationMinutes: Number((debate.durationSeconds / 60).toFixed(1)) })), aggregateFamilyRank: best.aggregateRank, minimumDurationMinutes: Number((artifact.audit.minimumDurationSeconds / 60).toFixed(1)), maximumDurationMinutes: Number((artifact.audit.maximumDurationSeconds / 60).toFixed(1)), v417Overlap: artifact.audit.v417Overlap, v418Overlap: artifact.audit.v418Overlap, legacyAssessmentContentAccessed: false, sourcePacketPreparationAuthorized: true }, null, 2));
