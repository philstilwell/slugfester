...
import {
  ...
  POST_CANARY_BATCH_05_RENDERING_MODEL,
  POST_CANARY_BATCH_05_RENDERING_ORDER,
  POST_CANARY_BATCH_05_RENDERING_PORT,
  ...
  buildPostCanaryBatch05RenderingPacket
...
const preparationPath = `${POST_CANARY_BATCH_05_RENDERING_ROOT}/preparation-manifest.json`;
...
const FINALIZATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-05/publication-finalization";
...
const PACKETS_ROOT = `${POST_CANARY_BATCH_05_RENDERING_ROOT}/packets`;
...
const IMAGE_ANALYZER = POST_CANARY_BATCH_05_RENDERING_IMAGE_CONTRACT.analyzerPath;
...
if (shouldWrite) {
  assertV4(
    ...
    "Batch 5 rendering preparation already exists; freeze is immutable"
  );
}
...
assertV4(
  finalizationPreparation.status ===
    ...
    finalizationActivation.status ===
      "frozen-post-canary-batch-05-publication-finalization-authorized" &&
    finalizationExecution.status ===
      "ten-debate-batch-05-publication-finalization-passed" &&
    ...
    compatibility.status === "batch-05-production-compatibility-boundary-recorded" &&
    ...
    finalizationAudit.totals.sections === 49 &&
    finalizationAudit.totals.moves === 187 &&
  ...
  "passing Batch 5 publication finalization required"
);
assertV4(
...
for (const debateNumber of POST_CANARY_BATCH_05_RENDERING_ORDER) {
  ...
  const packet = buildPostCanaryBatch05RenderingPacket({
    auditRow,
...
const directInputPaths = [
  ...
  "scripts/prepare-assessment-production-post-canary-batch-05-rendering-verification.mjs",
  "scripts/activate-assessment-production-post-canary-batch-05-rendering-verification.mjs",
  "scripts/finalize-assessment-production-post-canary-batch-05-rendering-verification.mjs",
  "scripts/test-assessment-production-post-canary-batch-05-rendering-verification-preparation.mjs",
...
for (const file of [...futureOutputPathsExcludedFromSourceHashes, EVIDENCE_ROOT]) {
  assertV4(!(await exists(file)), `future Batch 5 rendering output exists: ${file}`);
}

const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-05-rendering-verification-preparation",
  protocolId: POST_CANARY_BATCH_05_RENDERING_PROTOCOL_ID,
  status: "frozen-post-canary-batch-05-rendering-verification-prepared-not-authorized",
  frozenAt,
  checkpointCommit,
  ...
  userAuthorization: {
    ...
    scopeInterpretation:
      "Prepare, validate, freeze, commit, and push the Batch 5 rendering-verification plan without opening a candidate page or taking screenshots.",
  ...
  viewports: structuredClone(POST_CANARY_BATCH_05_RENDERING_VIEWPORTS),
  ...
  browserPlan: {
    ...
    localServer: {
      command: [
        ...
        "-m",
        "http.server",
        String(POST_CANARY_BATCH_05_RENDERING_PORT),
        "--bind",
        "127.0.0.1"
      ],
      workingDirectory: ".",
      port: POST_CANARY_BATCH_05_RENDERING_PORT,
      baseUrl: `http://127.0.0.1:${POST_CANARY_BATCH_05_RENDERING_PORT}`,
    ...
    imageContract: structuredClone(POST_CANARY_BATCH_05_RENDERING_IMAGE_CONTRACT),
  ...
  nextAuthorizedAction:
    "activate-and-execute-frozen-batch-05-rendering-verification-under-standing-authorization"
};
