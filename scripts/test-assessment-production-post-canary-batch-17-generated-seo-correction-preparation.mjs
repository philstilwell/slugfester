...
const analysisPath = `${POST_CANARY_BATCH_17_GENERATED_SEO_CORRECTION_ROOT}/preparation-analysis.json`;
const [plan, analysis] = await Promise.all([readJson(planPath), readJson(analysisPath)]);

assert.equal(plan.status, "frozen-batch-17-generated-seo-derivative-correction-plan-prepared");
...
assert.equal(plan.proposedWrites.length, 6);
assert.equal(plan.inventory.filter((record) => !record.changed).length, 374);
...
assert.equal(plan.executionContract.writeOnlyProposedPaths.length, 6);
...
assert.equal(analysis.status, "batch-17-generated-seo-correction-plan-freeze-passed");
...
console.log(serializedJson({ status: "passed", generatedOutputs: 380, proposedWrites: 6, generatedProductionFilesWritten: 0 }));
