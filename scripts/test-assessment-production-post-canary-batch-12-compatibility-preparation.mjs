...
const preparationPath =
  `${POST_CANARY_BATCH_12_COMPATIBILITY_ROOT}/preparation-manifest.json`;
const analysisPath =
  `${POST_CANARY_BATCH_12_COMPATIBILITY_ROOT}/analysis.json`;
...
assert.equal(
  ...
  "batch-12-site-ledger-adapter-and-validator-route"
);
assert.equal(
...
assert.equal(preparation.totals.sections, 54);
assert.equal(preparation.totals.moves, 204);
assert.equal(preparation.totals.oneSidedDisplayRows, 10);
assert.equal(preparation.totals.overallBlunders, 63);
assert.equal(preparation.totals.emptyReferenceLinks, 59);
assert.equal(preparation.totals.suppliedReferenceLinks, 4);
...
assert.equal(
  await exists(`${POST_CANARY_BATCH_12_COMPATIBILITY_ROOT}/execution.json`),
  false
);
assert.equal(
  await exists(
...
assert.deepEqual(packetNumbers, POST_CANARY_BATCH_12_COMPATIBILITY_ORDER);
for (const packetRecord of preparation.artifacts.packets) {
  ...
  assert.equal(packet.batchNumber, 12);
  ...
  const positive = validatePostCanaryBatch12SiteLedgerAdapter({
