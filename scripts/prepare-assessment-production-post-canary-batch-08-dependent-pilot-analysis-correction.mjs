...
import {
  BATCH_08_DEPENDENT_PILOT_CORRECTION_PROTOCOL_ID,
  BATCH_08_DEPENDENT_PILOT_CORRECTION_ROOT,
  BATCH_08_DEPENDENT_PILOT_OUTPUT_PATHS,
  jsonLeafChanges,
  runIsolatedBatch08DependentPilotAnalysis
} from "./lib/assessment-production-post-canary-batch-08-dependent-pilot-analysis-correction.mjs";
import {
...
const diagnosisPath = `${POST_CANARY_BATCH_08_GENERATED_SEO_CORRECTION_ROOT}/validation-failure-diagnosis.json`;
const seoPlanPath = `${POST_CANARY_BATCH_08_GENERATED_SEO_CORRECTION_ROOT}/correction-plan.json`;
const seoActivationPath = `${POST_CANARY_BATCH_08_GENERATED_SEO_CORRECTION_ROOT}/execution-activation.json`;
const planPath = `${BATCH_08_DEPENDENT_PILOT_CORRECTION_ROOT}/correction-plan.json`;
const analysisPath = `${BATCH_08_DEPENDENT_PILOT_CORRECTION_ROOT}/preparation-analysis.json`;
const activationPath = `${BATCH_08_DEPENDENT_PILOT_CORRECTION_ROOT}/execution-activation.json`;
const executionPath = `${BATCH_08_DEPENDENT_PILOT_CORRECTION_ROOT}/execution.json`;
const executionAnalysisPath = `${BATCH_08_DEPENDENT_PILOT_CORRECTION_ROOT}/analysis.json`;
...
assertV4(
  diagnosis.status === "frozen-batch-08-generated-seo-full-check-dependent-pilot-analysis-staleness-diagnosed" &&
    diagnosis.recoveryBoundary.writablePathsMaximum === 2 &&
    seoActivation.status === "frozen-batch-08-generated-seo-correction-pass-activated" &&
    seoActivation.plan.sha256 === sha256(await readBytes(seoPlanPath)),
  "accepted Batch 8 generated SEO validation-failure diagnosis required"
);
...
const baselineOutputs = await Promise.all(BATCH_08_DEPENDENT_PILOT_OUTPUT_PATHS.map(lockFile));
const isolated = await runIsolatedBatch08DependentPilotAnalysis(root);
...
const affectedNumbers = new Set(["156"]);
...
assertV4(
  ...
  "pilot JSON changed outside aggregate legacy comparison and Debate 156 row"
);

const toolPaths = [
  "scripts/lib/assessment-production-post-canary-batch-08-dependent-pilot-analysis-correction.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-08-dependent-pilot-analysis-correction.mjs",
  "scripts/test-assessment-production-post-canary-batch-08-dependent-pilot-analysis-correction-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-08-dependent-pilot-analysis-correction.mjs",
  "scripts/test-assessment-production-post-canary-batch-08-dependent-pilot-analysis-correction-activation.mjs",
  "scripts/run-assessment-production-post-canary-batch-08-dependent-pilot-analysis-correction.mjs"
];
const plan = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-08-dependent-pilot-analysis-correction-plan",
  protocolId: BATCH_08_DEPENDENT_PILOT_CORRECTION_PROTOCOL_ID,
  status: "frozen-batch-08-dependent-pilot-analysis-two-file-correction-plan-prepared",
  frozenAt,
  batchNumber: 8,
  ...
  protectedProduction: {
    ...
    generatedOutputs: await Promise.all(seoPlan.inventory.map(async (record) => {
      ...
      return { ...actual, role: seoPlan.executionContract.writeOnlyProposedPaths.includes(record.path) ? "accepted-batch-08-derivative" : "preserved-unrelated-derivative" };
    }))
  },
  ...
  correctionContract: {
    ...
    writablePaths: BATCH_08_DEPENDENT_PILOT_OUTPUT_PATHS,
  ...
  nextAuthorizedAction: "activate-and-execute-one-frozen-batch-08-dependent-pilot-analysis-correction"
};
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-08-dependent-pilot-analysis-correction-preparation-analysis",
  protocolId: BATCH_08_DEPENDENT_PILOT_CORRECTION_PROTOCOL_ID,
  status: "batch-08-dependent-pilot-analysis-correction-plan-freeze-passed",
  analyzedAt: frozenAt,
...
if (shouldWrite) {
  ...
  await mkdir(resolve(BATCH_08_DEPENDENT_PILOT_CORRECTION_ROOT), { recursive: true });
