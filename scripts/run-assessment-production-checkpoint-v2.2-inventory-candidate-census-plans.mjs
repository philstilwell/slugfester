...
assertV4(
  activation.status ===
    ...
    activation.gateDisposition?.checkpointDiscoveryRetried === false &&
    activation.gateDisposition
  ...
  "production checkpoint v2.2 candidate-census plan execution is unauthorized"
);
...
async function executeContext(context, contextIndex) {
  ...
  const sourceDirectory = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-production-v22-candidate-plan-${context.debateNumber}-`
    )
  );
  const isolatedCodexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-production-v22-candidate-plan-home-${context.debateNumber}-`
    )
  );
  ...
  try {
    ...
    const prompt = `Read inventory-source-packet.json, candidate-census.json, chronology-fallback-inventory-guide.md, inventory-manual.md, and plan-schema.json completely; read nothing else. Act only as the isolated score-blind candidate-census inventory planner for production checkpoint v2.2 Debate ${context.debateNumber}. Review every candidate in the complete census. Author only one burden route per side and four to six weighted issue sections totaling exactly 100 percent. Candidate selection and candidate evidence excerpts are deferred to separate fresh side contexts. Ratings, response topology, scores, winners, legacy assessments, other debates, other judgments, execution metadata, tags, Overall Commentary, AI Extension, and publication prose are prohibited. Return exactly one plan-schema-conforming JSON object.`;
    process.stdout.write(
      `[production-v2.2-candidate-plan] starting ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
  ...
  process.stdout.write(
    `[production-v2.2-candidate-plan] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  ...
  status:
    validContexts === preparation.contexts.length
      ...
      : "production-checkpoint-v2.2-candidate-census-plan-gate-complete-with-failure",
  ...
  nextRequiredAction:
    validContexts === preparation.contexts.length
      ? "analyze-and-replay-ten-production-checkpoint-v2.2-candidate-census-plans-model-free-only"
      : "stop-preserve-failed-production-checkpoint-v2.2-candidate-census-plan-gate",
};
