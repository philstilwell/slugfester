...
import { POST_CANARY_BATCH_11_PUBLICATION_FINALIZATION_PROTOCOL_ID,
  POST_CANARY_BATCH_11_PUBLICATION_FINALIZATION_ROOT } from
...
assertV4(p.status === "frozen-post-canary-batch-11-publication-finalization-prepared" &&
  p.protocolId === POST_CANARY_BATCH_11_PUBLICATION_FINALIZATION_PROTOCOL_ID && p.contexts?.length === 10 &&
...
"Batch 11 finalization is not prepared");
...
const activation = { schemaVersion: "1.0-assessment-production-post-canary-batch-11-publication-finalization-activation",
  protocolId: p.protocolId, status: "frozen-post-canary-batch-11-publication-finalization-authorized",
  ...
  nextAuthorizedAction: "execute-one-frozen-batch-11-deterministic-publication-finalization-pass" };
