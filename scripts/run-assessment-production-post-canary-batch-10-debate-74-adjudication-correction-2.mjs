...
import { validatePostCanaryBatch10DisputeAdjudicationOutput } from "./lib/assessment-production-post-canary-batch-10-dispute-adjudication.mjs";
import { assertV4 } from "./lib/v4-lean-production.mjs";

const RECOVERY_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-10/dispute-only-adjudication/failure-recovery/correction-2";
...
assertV4(
  manifest.status ===
      "frozen-two-batch-10-debate-74-field-disjoint-adjudication-correction-2-contexts-authorized" &&
  ...
  "Batch 10 dispute-only adjudication execution is unauthorized"
);
...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-post-canary-batch-10-debate-74-adjudication-correction-${context.debateNumber}-`
    )
  );
  const codexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-post-canary-batch-10-debate-74-adjudication-correction-home-${context.debateNumber}-`
    )
  );
  ...
  try {
    ...
    const burdenInstruction = context.burdenAdjustmentSides.includes("con")
      ? "IMPORTANT: packet.json contains exactly one burdenAdjustmentDisputes entry for side con. You must return burdenAdjustmentDecisions as an array containing exactly one side-con entry with choice 1 or 2 and a rationale of at least 40 characters; do not leave that array empty."
      : "This shard contains no burden-adjustment dispute, so burdenAdjustmentDecisions must be an empty array.";
    ...
    }; read nothing else. Act only as the isolated, score-blind, disputed-fields-only adjudicator for post-canary Batch 10 Debate ${
      context.debateNumber
    }, correction-2 shard ${context.shardId}. Decide every required anonymous candidate pair and scoring field in this field-disjoint shard exactly once from the locked evidence. ${burdenInstruction} Candidate ordering may reverse independently for every field. Select only candidate 1 or candidate 2. Never mix, average, interpolate, repair, rewrite, or invent a candidate. Use any supplied diarized transcript only for its associated move. Never infer either initial pass identity or rationale. Never calculate a move, section, side, or debate score. Never produce a winner, legacy assessment, Overall Commentary, AI Extension, or publication prose. Before returning, verify that moveDecisions has exactly ${context.disputedMoves} entries and burdenAdjustmentDecisions has exactly ${context.burdenAdjustmentDisputes} entries. Return exactly one schema-conforming JSON object.`;
    process.stdout.write(
      `[batch-10-debate-74-correction-2] starting index ${context.contextIndex} ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
    ...
    if (
      ...
      try {
        validation = validatePostCanaryBatch10DisputeAdjudicationOutput(
  ...
  process.stdout.write(
    `[batch-10-debate-74-correction-2] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-10-dispute-only-adjudication-model-execution",
  protocolId: manifest.protocolId,
  status: passed
    ? "two-batch-10-debate-74-field-disjoint-adjudication-correction-2-contexts-passed"
    : "batch-10-debate-74-adjudication-correction-2-gate-complete-with-failure",
