...
const PRIOR_SELECTION = "docs/assessment-production/post-canary-continuation-v1/batch-11/selection.json";
const PRIOR_SELECTION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-11/selection-analysis.json";
const PRIOR_MUTATION_MANIFEST = "docs/assessment-production/post-canary-continuation-v1/batch-11/production-publication/mutation-manifest.json";
const PRIOR_COMPLETION_EXECUTION = "docs/assessment-production/post-canary-continuation-v1/batch-11/production-publication/generated-seo-correction/execution.json";
const PRIOR_COMPLETION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-11/production-publication/generated-seo-correction/analysis.json";
const PRIOR_COMPLETION_COMMIT = "defff0c14dc7bcaec481a58d207e6b9f445402a4";
const SCRIPT = "scripts/select-assessment-production-post-canary-batch-12.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-12-selection.mjs";
...
assertV4(!(await exists(FUTURE_SOURCE_PREPARATION)), "source preparation already exists before Batch 12 selection");
...
assertV4(
  ...
  "Batch 12 selection is not compatible with the frozen continuation policy"
);
...
assertV4(
  priorSelection.status === "eleventh-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    priorSelection.protocolId === "assessment-production-post-canary-batch-11" &&
    priorSelection.productionSequenceOrdinal === 12 &&
    priorSelection.selected.length === 10 &&
    priorSelectionAnalysis.status === "eleventh-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization" &&
    priorSelectionAnalysis.selection.sha256 === sha256(controlBytes[PRIOR_SELECTION]) &&
    priorMutationManifest.status === "frozen-batch-11-production-publication-mutation-manifest-prepared" &&
    JSON.stringify(priorMutationManifest.explicitOrder) === JSON.stringify(priorSelectedNumbers) &&
    priorCompletionExecution.status === "passed-complete-batch-11-transaction-ready-for-atomic-commit" &&
    priorCompletionExecution.totals.isolatedGeneratorRuns === 1 &&
    priorCompletionExecution.totals.repositoryValidationRuns === 2 &&
    priorCompletionExecution.totals.generatedDerivativeWrites === 12 &&
    priorCompletionExecution.totals.dependentCorrectionWrites === 4 &&
    ...
    priorCompletionExecution.productionTransaction.generatorByteIdentical === true &&
    priorCompletionExecution.dependentRecovery.scoringFixtureFiles === 2 &&
    priorCompletionExecution.dependentRecovery.calibrationAnalysisFiles === 2 &&
    priorCompletionExecution.dependentRecovery.scoreChanges === 0 &&
    priorCompletionExecution.dependentRecovery.ledgerChanges === 0 &&
    priorCompletionAnalysis.status === "batch-11-production-publication-accepted-for-atomic-commit-after-generated-seo-correction" &&
    ...
    priorCompletionAnalysis.decision.batch11ProductionPublicationGatePassed === true &&
    priorCompletionAnalysis.decision.boundedRepositoryDependentCorrectionPassed === true &&
    ...
    priorCompletionAnalysis.decision.seoGeneratorWriteRerunPerformed === false &&
  ...
  "Batch 11 has not satisfied the frozen successful-publication prerequisite"
);
assertV4(git("rev-parse", "--abbrev-ref", "HEAD") === "main", "Batch 12 selection must run on main");
assertV4(git("rev-parse", "HEAD") === PRIOR_COMPLETION_COMMIT, "Batch 12 selection checkpoint is not the exact Batch 11 publication commit");
assertV4(git("rev-parse", "origin/main") === PRIOR_COMPLETION_COMMIT, "Batch 11 publication commit is not the pushed origin/main checkpoint");
...
assertV4(
  JSON.stringify(priorSelectedNumbers) === JSON.stringify(ranked.slice(100, 110).map((item) => item.debateNumber)),
  "Batch 11 is not the immutable eleventh rank slice"
);

const selected = [];
let totalEvents = 0;
let totalDurationMs = 0;
for (const [offset, rankedItem] of ranked.slice(110, 120).entries()) {
...
assertV4(selected.length === 10 && new Set(selected.map((item) => item.debateNumber)).size === 10, "Batch 12 did not select ten unique debates");

const checkpointCommit = git("rev-parse", "HEAD");
const selection = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-selection",
  protocolId: "assessment-production-post-canary-batch-12",
  status: "twelfth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed",
  frozenAt,
  checkpointCommit,
  branch: "main",
  ...
  stagingOnly: true,
  batchNumber: 12,
  productionSequenceOrdinal: 13,
  userAuthorization: {
    instruction: "Continue the SLUGFESTER reassessment campaign with Batch 12 under the complete-workflow standing authorization supplied on 2026-08-26.",
    scopeInterpretation: "Materialize and source-validate only the deterministic Batch 12 selection under the frozen continuation policy after authenticating successful Batch 11 publication. This selection pass prepares no source packets, executes no models, derives no scores, and performs no production mutation.",
  ...
  eligibility: {
    ...
    previouslyPublishedAndRemovedCount: 110,
    remainingBeforeBatchCount: ranked.length - 110,
    remainingBeforeBatchRankedSha256: sha256(serializedJson(ranked.slice(110))),
    selectedCount: selected.length,
    remainingUnselectedCount: ranked.length - 120,
    dyadicOnly: true,
  ...
  nextAuthorizedAction: "prepare-batch-12-standing-authorization-and-source-packets-under-user-authorization"
};
...
const analysis = {
  ...
  status: "twelfth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization",
  analyzedAt: frozenAt,
  ...
  finding: "Successful Batch 11 production publication was authenticated at the pushed atomic commit. The immutable campaign rank then selected ordinals 111 through 120 as ten unique pending dyadic debates, and all thirty effective source files and ten canonical event chains passed without replacement.",
  decision: {
