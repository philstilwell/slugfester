...
import {
  POST_CANARY_BATCH_03_COMPATIBILITY_ORDER,
  POST_CANARY_BATCH_03_COMPATIBILITY_PROTOCOL_ID,
  POST_CANARY_BATCH_03_COMPATIBILITY_ROOT,
  POST_CANARY_BATCH_03_SITE_LEDGER_ADAPTER_VERSION,
  buildPostCanaryBatch03SiteLedgerAdapter,
  serializedJson,
  sha256,
...
const paths = {
  preparation: `${POST_CANARY_BATCH_03_COMPATIBILITY_ROOT}/preparation-manifest.json`,
  analysis: `${POST_CANARY_BATCH_03_COMPATIBILITY_ROOT}/analysis.json`,
  activation: `${POST_CANARY_BATCH_03_COMPATIBILITY_ROOT}/execution-activation.json`,
  execution: `${POST_CANARY_BATCH_03_COMPATIBILITY_ROOT}/execution.json`,
  stagedLedgerRoot:
    `${POST_CANARY_BATCH_03_COMPATIBILITY_ROOT}/output-bundle/staged-ledgers`,
  compatibility:
    "docs/assessment-production/post-canary-continuation-v1/batch-03/publication-finalization/compatibility-analysis.json",
  finalizationAudit:
    "docs/assessment-production/post-canary-continuation-v1/batch-03/publication-finalization/output-bundle/finalization-audit.json",
  finalLedger:
  ...
  renderingAnalysis:
    "docs/assessment-production/post-canary-continuation-v1/batch-03/rendering-verification/analysis.json",
  renderingAudit:
    "docs/assessment-production/post-canary-continuation-v1/batch-03/rendering-verification/rendering-audit.json",
  ...
  preparationScript:
    "scripts/prepare-assessment-production-post-canary-batch-03-compatibility.mjs",
  preparationTest:
    "scripts/test-assessment-production-post-canary-batch-03-compatibility-preparation.mjs",
...
assertV4(
  ...
  "a stable --frozen-at ISO timestamp is required for the first Batch 3 compatibility preparation write"
);
...
assertV4(
  renderingAnalysis.status ===
      "ten-debate-batch-03-rendering-verification-passed" &&
    ...
    renderingAnalysis.nextAuthorizedAction ===
      "prepare-batch-03-production-compatibility-under-standing-authorization",
  "Batch 3 rendering gate does not authorize compatibility planning only"
);
assertV4(
  renderingAudit.status ===
      "passed-ten-debate-batch-03-rendering-verification" &&
  ...
  "passed Batch 3 rendering evidence audit required"
);
assertV4(
  compatibility.status ===
      "batch-03-production-compatibility-boundary-recorded" &&
    canonicalJson(compatibility.findings.map((finding) => finding.id)) ===
      canonicalJson(["batch-03-site-ledger-adapter-and-validator-route"]) &&
  ...
  "frozen Batch 3 publication compatibility blocker required"
);
assertV4(
  finalizationAudit.status ===
      "passed-ten-debate-batch-03-publication-finalization" &&
  ...
  "passed one-pass Batch 3 staging-only finalization audit required"
);
assertV4(
  scores.status ===
      "post-canary-batch-03-single-score-pass-stability-gate-passed" &&
  ...
  "accepted one-pass Batch 3 score ledger required"
);
validatePostCanaryBatch03FinalLedger(
  finalLedger,
...
const packetRecords = [];
for (const debateNumber of POST_CANARY_BATCH_03_COMPATIBILITY_ORDER) {
  ...
  const packetPath =
    `${POST_CANARY_BATCH_03_COMPATIBILITY_ROOT}/packets/debate-${debateNumber}.json`;
  ...
  const adapter = buildPostCanaryBatch03SiteLedgerAdapter({
    finalLedgerDebate,
    scoreDebate,
    candidate,
  ...
  const packet = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-03-compatibility-plan-packet",
    protocolId: POST_CANARY_BATCH_03_COMPATIBILITY_PROTOCOL_ID,
    status: "frozen-post-canary-batch-03-compatibility-plan-packet",
...
assertV4(
  totals.debates === 10 &&
    ...
    totals.suppliedReferenceLinks === 11,
  "Batch 3 compatibility population changed"
);

const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-compatibility-preparation-manifest",
  protocolId: POST_CANARY_BATCH_03_COMPATIBILITY_PROTOCOL_ID,
  status: "post-canary-batch-03-compatibility-plan-prepared-and-frozen",
  frozenAt,
  productionCanary: false,
  batchNumber: 3,
  planningOnly: true,
  ...
  userAuthorization: {
    instruction:
      "Use the frozen Batch 3 standing authorization for deterministic compatibility staging.",
    scopeInterpretation:
      "Prepare, validate, freeze, commit, and push the Batch 3 production-compatibility plan; later activation and one staging pass are already covered by the standing authorization. Do not publish production ledgers, mutate production, run models, use paid services, or select Batch 4."
  },
  scope: {
    finding: "batch-03-site-ledger-adapter-and-validator-route",
    description:
      "Freeze the exact Batch 3 ledger adapters and schema-specific validator route without applying them to the active validator or production data."
  },
  invariants: {
    ...
    scoreProtocolId:
      "assessment-production-post-canary-batch-03-single-deterministic-score-pass",
  ...
  proposedValidatorRoute: {
    ...
    newSchemaVersion: POST_CANARY_BATCH_03_SITE_LEDGER_ADAPTER_VERSION,
    existingCheckpointSchemaVersion:
      "1.0-production-checkpoint-v2.2-site-ledger-adapter",
    exactRoute:
      "When a production ledger uses the Batch 3 adapter schema, load the Batch 3 packet and activation from the frozen Batch 3 compatibility root, verify their hashes, and replay the adapter with the Batch 3 validator. Keep the checkpoint, Batch 1, and Batch 2 adapters on their existing roots, credentials, and validators, and keep legacy Rubric v2 and v2.1 ledgers on their existing calculators.",
    oneSidedDisplayRows:
    ...
    activationRequirements: [
      "activation status is post-canary-batch-03-compatibility-execution-authorized-and-frozen",
  ...
  stagedExecutionPlan: [
    ...
    },
    {
      ...
      allowedWrites: [
        paths.activeValidator,
        `${POST_CANARY_BATCH_03_COMPATIBILITY_ROOT}/output-bundle/**`,
        paths.execution,
        paths.analysis
      ],
      actions: [
        "wire the schema-specific Batch 3 adapter route",
  ...
  mandatoryExecutionTests: {
    positive: [
      "all ten Batch 3 adapters reproduce all 200 locked move scores, 53 section scores per side, and 20 overall scores",
      "all ten final candidates match ledgerMoveId, section order, model, rubric, and score protocol",
      "all 10 one-sided display rows pass through the authenticated Batch 3 adapter route",
      "all 44 empty Overall Commentary reference arrays pass without synthesized links",
      "all 11 supplied reference links retain full validation",
    ...
    negative: [
      ...
      "a Batch 3 adapter routed through the checkpoint, Batch 1, or Batch 2 root fails",
  ...
  nextAuthorizedAction:
    "activate-and-execute-one-batch-03-production-compatibility-pass-under-standing-authorization"
};
const preparationBytes = serializedJson(preparation);
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-compatibility-preparation-analysis",
  protocolId: POST_CANARY_BATCH_03_COMPATIBILITY_PROTOCOL_ID,
  status: "post-canary-batch-03-compatibility-plan-freeze-passed",
  productionCanary: false,
  batchNumber: 3,
  planningOnly: true,
  preparation: {
...
if (write) {
  assertV4(
    ...
    "Batch 3 compatibility execution artifact already exists; planning write blocked"
  );
  await mkdir(
    resolve(`${POST_CANARY_BATCH_03_COMPATIBILITY_ROOT}/packets`),
    { recursive: true }
  );
  ...
  assertV4(
    ...
    "stored Batch 3 compatibility preparation differs from deterministic replay"
  );
  const storedAnalysis = await readJson(paths.analysis);
  assertV4(
    canonicalJson(storedAnalysis) === canonicalJson(analysis),
    "stored Batch 3 compatibility analysis differs from deterministic replay"
  );
  for (const record of packetRecords) {
    const storedPacket = await readFile(resolve(record.packetPath), "utf8");
    assertV4(
      storedPacket === record.packetBytes,
      `${record.debateNumber}: stored Batch 3 compatibility packet differs from deterministic replay`
    );
  }
}
