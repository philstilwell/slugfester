...
import {
  ...
  evaluatePostCanaryBatch01ScoreStability
...
const accepted = evaluatePostCanaryBatch01ScoreStability(passing);
...
assert.equal(
  evaluatePostCanaryBatch01ScoreStability(winnerMutation).acceptancePassed,
  false
);
const distanceMutation = structuredClone(passing);
distanceMutation[2].final = scores(95, 80);
assert.equal(
  evaluatePostCanaryBatch01ScoreStability(distanceMutation).acceptancePassed,
  false
);
assert.throws(() =>
  evaluatePostCanaryBatch01ScoreStability(passing, {
    ...POST_CANARY_BATCH_01_SCORE_STABILITY_THRESHOLDS,
