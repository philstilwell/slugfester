...
if (!(await exists(prepPath))) {
  const sourcePreparation = JSON.parse(
    await readFile(
      "docs/assessment-production/score-stability-v2.1.3-validation-cohort/disagreement-extraction/audio-source-preparation.json",
      "utf8"
    )
  );
  assert.equal(sourcePreparation.status, "prepared-five-v2.1.3-local-audio-clips");
  assert.equal(sourcePreparation.clips.length, 5);
  assert.equal(sourcePreparation.authorization.audioVerificationManifestPreparation, true);
  assert.equal(sourcePreparation.authorization.paidTranscriptionExecution, false);
  console.log(JSON.stringify({ status: "passed-prefreeze", clips: 5, paidCalls: 0, scoresDerived: 0 }, null, 2));
  process.exit(0);
}
...
assert.equal(
  preparation.status,
  "prepared-five-paid-known-speaker-diarizations-pending-explicit-user-approval"
);
...
assert.equal(preparation.costEstimate.clipMinutes, 7.3645);
assert.equal(preparation.costEstimate.expectedCostUsd, 0.0442);
...
if (!(await exists(manifestPath))) {
  ...
  console.log(JSON.stringify({
    ...
    maximumAuthorizedCostUsd: preparation.costEstimate.maximumAuthorizedCostUsd,
    retries: 0,
...
assert.equal(manifest.status, "frozen-five-paid-known-speaker-diarizations-authorized");
...
if (!(await exists(executionPath))) {
  ...
  console.log(JSON.stringify({ status: "passed-frozen", calls: 5, expectedCostUsd: 0.0442, retries: 0, scoresDerived: 0 }, null, 2));
  process.exit(0);
}
...
if (analysis.gate.passed) {
  assert.equal(analysis.status, "passed-all-five-v2.1.3-confidence-moves-audio-verified");
  assert.equal(analysis.gate.verified, 5);
  assert.equal(analysis.gate.unresolved, 0);
  assert.equal(analysis.authorization.adjudicationPacketPreparation, true);
} else {
  assert.equal(analysis.status, "v2.1.3-audio-verification-unresolved");
