...
const priorRecoveryRoot = `${root}/audio-source-official-ytdlp-recovery-7`;
...
const userInstruction = "I authorize that.";
...
const sourcePaths = [
  "scripts/prepare-assessment-production-post-canary-batch-10-audio-source-bestaudio-recovery-8.mjs",
  "scripts/run-assessment-production-post-canary-batch-10-audio-source-bestaudio-recovery-8.mjs",
  "scripts/test-assessment-production-post-canary-batch-10-audio-source-bestaudio-recovery-8-preparation.mjs",
  "scripts/test-assessment-production-post-canary-batch-10-audio-source-bestaudio-recovery-8-cohort.mjs",
...
assertV4(
  priorExecution.status ===
    ...
    priorExecution.state?.officialYtDlpNativeInvocations === 1 &&
    ...
    priorExecution.failure?.category ===
      "source-or-validation-failure-during-authorized-official-ytdlp-recovery-7" &&
    priorExecution.failure?.stopRequired === true,
  "Batch 10 official yt-dlp recovery-7 failure changed"
);
assertV4(
  priorDiagnosis.status ===
      "preserved-batch-10-official-ytdlp-recovery-7-format-139-unavailable-stop-diagnosed" &&
    priorDiagnosis.diagnosis?.category ===
      "frozen-format-139-not-available-in-fresh-official-ytdlp-extraction" &&
    priorDiagnosis.diagnosis?.mediaBytesReceived === 0 &&
    priorDiagnosis.diagnosis?.alternateFormatInventoryRequested === false &&
    priorDiagnosis.diagnosis?.alternateFormatSelected === false &&
    priorDiagnosis.disposition?.recovery7Consumed === true &&
    priorDiagnosis.disposition?.paidTranscriptionMayBegin === false,
  "Batch 10 official yt-dlp recovery-7 diagnosis changed"
);
assertV4(
  work.moves.length === 9 &&
...
const bestAudioSources = priorPlan.officialYtDlpSources.map((source) => ({
  debateNumber: source.debateNumber,
  sourceVideoId: source.sourceVideoId,
  mode: "current-bestaudio-once",
  maximumRequiredEndMs: source.maximumRequiredEndMs,
  moveIds: structuredClone(source.moveIds),
  videoUrl: source.videoUrl,
  priorUnavailableFormatId: source.formatId,
  formatSelector: "bestaudio",
  outputTemplate: "source.bestaudio8.%(ext)s",
  audioOnlyRequired: true,
  positiveByteCountRequired: true,
  formatInventoryInvocationAuthorized: false,
  officialToolPath,
  ...
  concurrentFragmentsMaximum: 1
}));
const bestAudioVideoIds = new Set(bestAudioSources.map((source) => source.sourceVideoId));
const exactCohort = structuredClone(priorPlan.exactCohort);
exactCohort.sources = exactCohort.sources.map((source) => bestAudioVideoIds.has(source.sourceVideoId)
  ? { ...source, mode: "current-bestaudio-once" }
  : source
);
assertV4(
  JSON.stringify(bestAudioSources.map((item) => item.sourceVideoId)) ===
    JSON.stringify(["8k9A7d2Wnjk", "h-I_9e5qxnc", "0IpKHdVLZb4"]),
  "Batch 10 exact best-audio source cohort changed"
);
...
const plan = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-10-audio-source-bestaudio-recovery-8-plan",
  status:
  ...
  userAuthorization: {
    ...
    currentBestAudioSelectionAuthorized: true,
    runtimeFormatSelectionAuthorized: true,
    separateFormatInventoryAuthorized: false,
    sourceDownloadsMustBeSequential: true,
    ...
    freshDebate123AttemptAfterFormat139StopAuthorized: true,
    automaticRetriesAuthorized: false,
  ...
  executionPolicy: {
    ...
    runtimeFormatSelectionsMaximum: 3,
    separateFormatInventoryInvocationsMaximum: 0,
...
if (shouldActivate) {
  ...
  assertV4(
    ...
    "frozen best-audio correction plan changed"
  );
  assertV4(!(await exists(activationPath)), "recovery activation already exists");
  const activation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-10-audio-source-bestaudio-recovery-8-activation",
    status:
...
console.log(JSON.stringify({
  ...
  nextAction: "validate-and-commit-the-frozen-current-bestaudio-recovery-plan"
}, null, 2));
