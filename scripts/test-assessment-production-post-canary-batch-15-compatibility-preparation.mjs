...
const preparationPath =
  `${POST_CANARY_BATCH_15_COMPATIBILITY_ROOT}/preparation-manifest.json`;
const analysisPath =
  `${POST_CANARY_BATCH_15_COMPATIBILITY_ROOT}/analysis.json`;
...
assert.equal(
  ...
  "batch-15-site-ledger-adapter-and-validator-route"
);
assert.equal(
...
assert.equal(preparation.totals.moves, 191);
assert.equal(preparation.totals.oneSidedDisplayRows, 13);
assert.equal(preparation.totals.overallBlunders, 60);
assert.equal(preparation.totals.emptyReferenceLinks, 55);
assert.equal(preparation.totals.suppliedReferenceLinks, 5);
...
assert.equal(
  await exists(`${POST_CANARY_BATCH_15_COMPATIBILITY_ROOT}/execution.json`),
  false
);
assert.equal(
  await exists(
...
assert.deepEqual(packetNumbers, POST_CANARY_BATCH_15_COMPATIBILITY_ORDER);
for (const packetRecord of preparation.artifacts.packets) {
  ...
  assert.equal(packet.batchNumber, 15);
  ...
  const positive = validatePostCanaryBatch15SiteLedgerAdapter({
