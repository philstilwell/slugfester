...
assert(documents.activation.calls.length === 10, "exactly ten calls required");
assert(documents.execution.callsAttempted <= 10, "attempt count exceeds authorization");
...
assert(documents.analysis.gate.requiredMoves === 10, "ten required moves changed");
assert(documents.activation.costEstimate.maximumAuthorizedCostUsd === 0.4, "approved cap changed");
...
const executionComplete = documents.execution.callsCompleted === 10;
...
const analysis = {
  ...
  workflowDisposition: {
    ...
    userReviewRequired: approvedCapExceeded
  },
  authorization: {
