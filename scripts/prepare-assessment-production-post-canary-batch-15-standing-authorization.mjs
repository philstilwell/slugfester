...
import {
  ...
  POST_CANARY_BATCH_15_STANDING_AUTHORIZATION_INSTRUCTION_PATH,
  POST_CANARY_BATCH_15_STANDING_AUTHORIZATION_PROTOCOL_ID,
  POST_CANARY_BATCH_15_STANDING_AUTHORIZATION_STATUS,
  validatePostCanaryBatch15StandingAuthorization
...
const SELECTION_FREEZE_COMMIT = "02a06c30c8c6d399a36d855814c9c50c26806ed3";
const SOURCE_PACKET_COMMIT = "3a1592340080674f17fa9b2d7a49dfd919b68a5b";
...
const SOURCE_FILES = [
  ...
  "scripts/prepare-assessment-production-post-canary-batch-15-standing-authorization.mjs",
  "scripts/test-assessment-production-post-canary-batch-15-standing-authorization.mjs"
];
...
assertV4(
  !(await exists(POST_CANARY_BATCH_15_STANDING_AUTHORIZATION)),
  `${POST_CANARY_BATCH_15_STANDING_AUTHORIZATION} already exists`
);
...
assertV4(
  selection.status ===
    ...
    selection.batchNumber === 15 &&
    selection.checkpointCommit ===
      "cbacd97e1366e2a42ff462b3e623bcff80bfa3c3" &&
    ...
    analysis.status ===
      "fifteenth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization" &&
  ...
  "the frozen Batch 15 selection or source-packet boundary changed"
);
assertV4(
  execFileSync("git", ["show", `${SELECTION_FREEZE_COMMIT}:${SELECTION}`]).equals(selectionBytes),
  "Batch 15 selection differs from the user-authorized selection commit"
);
...
assertV4(
  ...
  "Batch 15 standing authorization must freeze at the pushed source-packet commit"
);
...
const record = {
  ...
  protocolId: POST_CANARY_BATCH_15_STANDING_AUTHORIZATION_PROTOCOL_ID,
  status: POST_CANARY_BATCH_15_STANDING_AUTHORIZATION_STATUS,
  authorizedAt,
  ...
  userAuthorization: {
    instruction: POST_CANARY_BATCH_15_STANDING_AUTHORIZATION_INSTRUCTION,
    ...
    scopeInterpretation: "Create and use a complete-workflow Batch 15 standing authorization for the ten frozen selections, beginning with the separately frozen score-blind source packets, including bounded diagnosed recovery and production publication, while preserving the established campaign controls and stopping before Batch 16 selection.",
  ...
  stopRules: {
    ...
    outsideBatchFifteenDebateBlocks: true,
    ...
    batchSixteenSelectionBlocks: true,
  ...
  nextAuthorizedAction:
    "prepare-validate-freeze-commit-and-push-batch-15-discovery-execution-manifest"
};
validatePostCanaryBatch15StandingAuthorization(record);
if (shouldWrite) {
  await writeFile(
