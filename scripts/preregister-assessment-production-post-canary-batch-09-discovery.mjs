...
const BATCH_ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-09";
...
const PROTOCOL_ID = "assessment-production-post-canary-batch-09-discovery";
const WORKFLOW = "docs/assessment-production-canary-discovery-workflow.md";
const SCRIPT = "scripts/preregister-assessment-production-post-canary-batch-09-discovery.mjs";
...
assertV4(
  preparation.status === "post-canary-batch-09-ten-complete-score-blind-source-packets-prepared-awaiting-validation" &&
    preparationValidation.status === "post-canary-batch-09-score-blind-source-packet-validation-passed-frozen-under-standing-authorization" &&
    ...
    preparation.totals.discoveryContexts === 31 &&
    preparation.totals.maximumCopiedInputBytes === 65975 &&
  ...
  "validated Batch 9 source preparation is not frozen at the required authorization boundary"
);
assertV4(
  ...
  "Batch 9 standing authorization does not cover exact discovery execution"
);
assertV4(
  JSON.stringify(preparation.contexts.map((debate) => debate.debateNumber)) === JSON.stringify(REQUIRED_ORDER),
  "Batch 9 debate order drifted"
);
assertV4(
  ...
  for (const chunk of debate.chunks) {
    ...
    assertV4(
      sha256(chunkBytes) === chunk.chunkLedgerSha256 &&
        ...
        buildBatch09TokenCountedChunkLedger(chunkBytes).equals(tokenBytes) &&
...
assertV4(contexts.length === 31, "Batch 9 discovery must contain exactly 31 contexts");
...
assertV4(maximumContext.debateNumber === "176" && maximumContext.chunkId === "chunk-002", "maximum copied-input context drifted");
...
const manifest = {
  ...
  status: shouldWrite ? "frozen-thirty-one-post-canary-batch-09-discovery-contexts-prepared-not-authorized" : "preview",
  ...
  AIOnly: true,
  userAuthorization: {
    scope: "standing-authorized Batch 9 discovery preparation and execution",
  ...
  costEstimate: {
    ...
    expectedAggregateModelMinutes: [32, 62],
    expectedAggregateComputeHours: [0.53, 1.03],
    absoluteGateTimeoutMinutes: 120,
    estimateBasis: "The exact 31-context Batch 9 input set is hash-locked with an observed maximum of 65,975 copied bytes; no input is truncated or repartitioned. The frozen 1→2→4 scheduler uses the subscription-backed route with a $0 direct incremental cost boundary.",
  },
  ...
  nextAuthorizedAction: "prepare-and-freeze-batch-09-discovery-execution-activation-under-standing-authorization",
};
