...
assert.equal(work.batchNumber, 17);
assert.equal(work.stagingOnly, true);
assert.equal(preparation.userAuthorization.workItemsAuthorized, 1);
assert.equal(
...
assert.equal(preparation.totals.moves, 1);
assert.equal(work.moves.length, 1);
...
assert.equal(preparation.validation.exactAuthorizedMoveCount, 1);
assert.equal(
  preparation.validation.canonicalLocalTextAndMetadataChainsVerified,
  1
);
assert.equal(preparation.validation.repositoryRenderedLockedExcerpts, 1);
assert.equal(preparation.validation.repositoryRenderedTimestampWindows, 1);
assert.equal(preparation.validation.expectedSpeakersLocked, 1);
assert.equal(
