...
const root = POST_CANARY_BATCH_03_COMPATIBILITY_ROOT;
...
const validator = await import(
  `./validate-debates.mjs?batch03Compatibility=${activation.validator.proposedSha256}`
);
...
for (const lock of activation.packetHashes) {
  ...
  const routeLock =
    validator.validatePostCanaryBatch03LedgerAdapterRouteLocks({
      debate: candidate,
      ledgerText,
  ...
  const route = validator.validatePostCanaryBatch03LedgerAdapterRoute({
    debate: candidate,
    ledger,
    ledgerText
  });
  assert.equal(route.repositoryScoreReplayPassed, true);
  const adapter = validatePostCanaryBatch03SiteLedgerAdapter({
    adapter: ledger,
    candidate,
...
assert.equal(suppliedReferenceLinks, 11);
assert.equal(
  ...
  JSON.stringify(
    {
      ...
      positiveRoutes,
      negativeControls,
      existingAdapterRoutesPreserved: 3,
      modelContexts: 0,
      paidServiceCalls: 0,
