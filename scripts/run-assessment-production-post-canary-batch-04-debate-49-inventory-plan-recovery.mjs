...
const RECOVERY = `${ROOT}/plan-recovery-1/debate-49`;
...
const MERGED_PLAN = `${ROOT}/plans/debate-49.json`;
...
const ORIGINAL_SCHEMA = `${ROOT}/schemas/plans/debate-49.schema.json`;
const PREPARER =
  "scripts/prepare-assessment-production-post-canary-batch-04-debate-49-inventory-plan-recovery.mjs";
const RUNNER =
  "scripts/run-assessment-production-post-canary-batch-04-debate-49-inventory-plan-recovery.mjs";
const PROTOCOL_ID =
...
const PREPARATION_STATUS =
  "two-frozen-debate-49-inventory-plan-recovery-shards-prepared-not-activated";
const ACTIVATION_STATUS =
  "two-frozen-debate-49-inventory-plan-recovery-shards-authorized";
const EXECUTION_STATUS =
  "two-debate-49-inventory-plan-recovery-shards-passed-and-merged";
...
async function loadPreparation({ requireFutureAbsent = false } = {}) {
  ...
  assertV4(
    ...
    "Debate 49 recovery preparation controls drifted"
  );
...
async function preflight() {
  ...
  assertV4(
    validation.status ===
      "passed-frozen-debate-49-inventory-plan-recovery-preparation" &&
      validation.contexts === 2 &&
      validation.modelContextsExecuted === 0 &&
      validation.directIncrementalCostUsd === 0,
    "Debate 49 recovery preparation validator failed"
  );
  ...
  console.log(
    JSON.stringify(
      {
        status: "passed-debate-49-recovery-model-free-preflight",
        contexts: 2,
        shards: SHARDS,
...
async function activate() {
  ...
  const activation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-04-debate-49-inventory-plan-recovery-execution-activation",
...
async function loadActivation({ requireFutureAbsent = false } = {}) {
  ...
  assertV4(
    ...
    "Debate 49 recovery activation controls drifted"
  );
...
async function execute() {
  ...
  async function executeContext(context) {
    ...
    const sourceDirectory = await mkdtemp(
      path.join(os.tmpdir(), `slugfester-batch-04-debate-49-${context.shardId}-`)
    );
    const isolatedCodexHome = await mkdtemp(
      path.join(os.tmpdir(), `slugfester-batch-04-debate-49-home-${context.shardId}-`)
    );
    ...
    process.stdout.write(
      `[batch-04-debate-49-plan-recovery] ${context.shardId} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
    );
    return record;
  }
  ...
  if (results.length === 2 && results.every((result) => result.accepted)) {
    try {
      ...
      const sourceContext = sourcePreparation.contexts.find(
        (context) => context.debateNumber === "49"
      );
  ...
  const execution = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-04-debate-49-inventory-plan-recovery-model-execution",
    protocolId: PROTOCOL_ID,
    status: passed
      ? EXECUTION_STATUS
      : "debate-49-inventory-plan-recovery-complete-with-failure",
    ...
    nextRequiredAction: passed
      ...
      : "stop-and-request-new-user-approval-after-failed-bounded-recovery",
  };
...
async function analyze() {
  ...
  assertV4(
    ...
    "Debate 49 bounded recovery did not pass"
  );
  assertV4(
  ...
  const recoveryAnalysis = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-04-debate-49-inventory-plan-recovery-analysis",
    protocolId: PROTOCOL_ID,
    status:
      "debate-49-bounded-first-recovery-passed-complete-ten-plan-cohort-replay-passed",
    activation: ACTIVATION,
    activationSha256: sha256(activationBytes),
    execution: EXECUTION,
...
async function validate() {
  ...
  if (!(await exists(EXECUTION))) {
    ...
    console.log(
      JSON.stringify(
        {
          status: "passed-debate-49-recovery-activation-awaiting-model-execution",
          contexts: 2,
          shards: SHARDS,
          model: activation.model,
          attemptsPerContext: 1,
  ...
  if (execution.status !== EXECUTION_STATUS) {
    ...
    console.log(
      JSON.stringify(
        {
          status: "passed-recorded-failed-debate-49-bounded-recovery",
          contextsAttempted: execution.contextsAttempted,
          invalidContexts: execution.invalidContexts,
          secondFailureStopRequired: true,
          retries: 0,
          timeoutExtensions: 0,
          directIncrementalCostUsd: 0,
        },
  ...
  assertV4(
    ...
    "passing Debate 49 recovery record drifted"
  );
  if (!(await exists(COHORT_ANALYSIS))) {
    ...
    console.log(
      JSON.stringify(
        {
          status: "passed-debate-49-recovery-awaiting-cohort-analysis",
          contexts: 2,
  ...
  console.log(
    JSON.stringify(
      {
        status: "passed-debate-49-recovery-and-complete-ten-plan-cohort-replay",
        acceptedPlans: 10,
        recoveredDebate: "49",
        recoveryShards: 2,
        originalFailedPartialOutputReused: false,
