...
import { MODEL, PACKET_VERSION, PROTOCOL_ID, ROOT, makeSchema, sha256 } from "./lib/assessment-production-post-canary-batch-15-audio-attribution.mjs";
...
const base = "docs/assessment-production/post-canary-continuation-v1/batch-15";
...
const [instructionBytes, workflowBytes, manualBytes, standingBytes, selectionBytes, workItemsBytes, auditBytes, analysisBytes, costBytes] = await Promise.all([
  readFile(paths.instruction), readFile(paths.workflow), readFile(paths.manual), readFile(paths.standing), readFile(paths.selection), readFile(paths.workItems), readFile(paths.originalAudit), readFile(paths.originalAnalysis), readFile(paths.originalCost),
]);
...
assert.equal(instructionBytes.toString("utf8").trim(), standing.userAuthorization.instruction);
assert.equal(standing.batchNumber, 15);
...
assert.equal(analysis.gate.verified, 1);
assert.equal(analysis.gate.unresolved, 2);
assert.equal(audit.totals.paidDiarizationCallsCompleted, 3);
assert.equal(audit.totals.retries, 0);
assert.equal(cost.costControl.approvedCapExceeded, false);
assert.equal(cost.costControl.usageDerivedEstimatedCostUsd, 0.1002925);
...
for (const debateNumber of ["39", "98"]) {
...
assert.equal(contexts.reduce((sum, item) => sum + item.moveIds.length, 0), 2);

const head = execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim();
const authorization = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-15-audio-attribution-successor-authorization",
  protocolId: PROTOCOL_ID,
  status: "frozen-active-batch-15-audio-attribution-recovery-level-1-successor-authorization",
  ...
  userAuthorization: {
    ...
    scopeInterpretation: "Resume the blocked Batch 15 workflow using the minimum two fresh debate-isolated, zero-cost, evidence-limited contexts that can resolve the two preserved audio-attribution decisions, then continue automatically through the already authorized complete workflow.",
  },
  preservedPriorGate: {
    verified: 1,
    unresolved: 2,
    paidCallsCompleted: 3,
    paidRetries: 0,
    usageDerivedEstimatedCostUsd: 0.1002925,
  ...
  sourceHashes,
  nextAuthorizedAction: "prepare-validate-commit-and-push-two-frozen-batch-15-audio-attribution-recovery-contexts",
};
const publicContexts = contexts.map(({ packetBytes, schemaBytes, ...context }) => context);
const preparation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-15-audio-attribution-preparation",
  protocolId: PROTOCOL_ID,
  status: "prepared-two-batch-15-audio-attribution-recovery-contexts-not-active",
  preparedAt,
  checkpointCommit: head,
  productionCanary: false,
  batchNumber: 15,
  ...
  executionPolicy: { concurrency: 1, contexts: 2, attemptsPerContext: 1, retriesMaximum: 0, perInvocationTimeoutMs: 900000, APIKeysRemoved: true, paidTranscriptionCalls: 0, directIncrementalCostUsdMaximum: 0 },
...
console.log(JSON.stringify({ status: shouldWrite ? preparation.status : "preview", contexts: 2, decisions: 2, model: `${MODEL.label}/low`, authentication: MODEL.authentication, attemptsMaximum: 1, retriesMaximum: 0, paidTranscriptionCalls: 0, directIncrementalCostUsdMaximum: 0, scoresDerived: 0 }, null, 2));
