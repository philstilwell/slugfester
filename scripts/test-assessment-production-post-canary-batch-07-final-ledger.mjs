...
const scoreArtifactCandidates = [
  `${POST_CANARY_BATCH_07_FINAL_LEDGER_ROOT}/calculated-scores.json`,
  `${POST_CANARY_BATCH_07_FINAL_LEDGER_ROOT}/score-analysis.json`,
  `${POST_CANARY_BATCH_07_FINAL_LEDGER_ROOT}/score-pass-manifest.json`
];
...
const rebuiltLedger = buildPostCanaryBatch07FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
...
assert.deepEqual(
  ...
  ["193", "80", "121", "100", "78", "113", "180", "02", "182", "56"]
);
assert.equal(storedLedger.audit.finalMoves, 187);
assert.equal(storedLedger.audit.disputedMoves, 180);
assert.equal(storedLedger.audit.pairSelections, 204);
assert.equal(storedLedger.audit.scoringFieldSelections, 289);
assert.equal(storedLedger.audit.burdenAdjustmentSelections, 18);
assert.equal(storedLedger.audit.candidateSelections, 511);
assert.equal(storedLedger.audit.meanMerges, 403);
assert.equal(storedLedger.audit.dependencyMeanMergesSuppressed, 0);
assert.equal(storedLedger.audit.audioVerifiedMoves, 5);
...
assert.throws(() =>
  buildPostCanaryBatch07FinalLedger(mutatedInputs, inputs.sourceHashes)
);
...
assert.equal(analysis.totals.finalMoves, 187);
assert.equal(analysis.totals.disputedMoves, 180);
assert.equal(analysis.totals.candidateSelections, 511);
