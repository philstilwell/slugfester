...
import {
  ...
  evaluatePostCanaryBatch05ScoreStability
...
const accepted = evaluatePostCanaryBatch05ScoreStability(passing);
...
assert.equal(
  evaluatePostCanaryBatch05ScoreStability(winnerMutation).acceptancePassed,
  false
);
const distanceMutation = structuredClone(passing);
distanceMutation[2].final = scores(95, 80);
assert.equal(
  evaluatePostCanaryBatch05ScoreStability(distanceMutation).acceptancePassed,
  false
);
assert.throws(() =>
  evaluatePostCanaryBatch05ScoreStability(passing, {
    ...POST_CANARY_BATCH_05_SCORE_STABILITY_THRESHOLDS,
