...
const PROTOCOL_ID = "assessment-production-post-canary-batch-03-discovery";
const WORKFLOW = "docs/assessment-production-canary-discovery-workflow.md";
const SCRIPT = "scripts/preregister-assessment-production-post-canary-batch-03-discovery.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-03-discovery-manifest.mjs";
...
assertV4(
  preparation.status === "post-canary-batch-03-ten-complete-score-blind-source-packets-prepared-awaiting-validation" &&
    preparationValidation.status === "post-canary-batch-03-score-blind-source-packet-validation-passed-frozen-standing-authorization-active" &&
    ...
    preparation.totals.maximumCopiedInputBytes === 82227 &&
  ...
  "validated Batch 3 source preparation is not frozen at the required authorization boundary"
);
assertV4(
  ...
  "Batch 3 standing authorization does not cover exact discovery execution"
);
assertV4(
  JSON.stringify(preparation.contexts.map((debate) => debate.debateNumber)) === JSON.stringify(REQUIRED_ORDER),
  "Batch 3 debate order drifted"
);
assertV4(
  ...
  for (const chunk of debate.chunks) {
    ...
    assertV4(
      sha256(chunkBytes) === chunk.chunkLedgerSha256 &&
        ...
        buildBatch03TokenCountedChunkLedger(chunkBytes).equals(tokenBytes) &&
...
assertV4(contexts.length === 38, "Batch 3 discovery must contain exactly 38 contexts");
...
assertV4(maximumContext.debateNumber === "150" && maximumContext.chunkId === "chunk-001", "maximum copied-input context drifted");
...
const manifest = {
  ...
  status: shouldWrite ? "frozen-thirty-eight-post-canary-batch-03-discovery-contexts-prepared-not-authorized" : "preview",
  ...
  AIOnly: true,
  userAuthorization: {
    scope: "standing-authorized Batch 3 discovery preparation and execution",
  ...
  costEstimate: {
    ...
    estimateBasis: "The exact 38-context Batch 3 input set is hash-locked with an observed maximum of 82,227 copied bytes; no input is truncated or repartitioned. The frozen 1→2→4 scheduler uses the subscription-backed route with a $0 direct incremental cost boundary.",
  },
  ...
  nextAuthorizedAction: "prepare-and-freeze-batch-03-discovery-execution-activation-under-standing-authorization",
};
