...
const ORIGINAL_PACKET = `${ROOT}/packets/plans/debate-49.json`;
...
function shardSchema(fullSchema, field) {
  return {
    ...
    $id: `${fullSchema.$id}-debate-49-recovery-1-${field}`,
    title: `Slugfester Batch 4 Debate 49 ${field} recovery shard`,
    type: "object",
...
function mergeRule() {
  return {
    ruleId: "batch-04-debate-49-plan-recovery-1-field-disjoint-merge-v1",
...
async function buildArtifacts({ frozenAt, checkpointCommit }) {
  ...
  assertV4(
    ...
    "preserved Debate 49 planner timeout boundary drifted"
  );
  assertV4(
    standingAuthorization.record.recoveryControls.boundedFirstRecoveryAuthorized ===
      ...
      standingAuthorization.record.stopRules.secondFailureOfCorrectedContextBlocks ===
        true,
    "Batch 4 first-recovery authority drifted"
  );
  ...
  for (const field of SHARDS) {
    const schemaPath = `${RECOVERY}/schemas/debate-49-${field}.schema.json`;
  ...
  const packets = {};
  for (const field of SHARDS) {
    const packetPath = `${RECOVERY}/packets/debate-49-${field}.json`;
    const output = `${RECOVERY}/outputs/debate-49-${field}.json`;
    ...
    const packet = {
      schemaVersion:
        "1.0-assessment-production-post-canary-batch-04-debate-49-inventory-plan-recovery-shard-packet",
      protocolId: PROTOCOL_ID,
      recoveryId: "debate-49-plan-recovery-1",
  ...
  const diagnosis = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-04-debate-49-inventory-plan-timeout-diagnosis",
    protocolId: PROTOCOL_ID,
    status:
      "preserved-debate-49-candidate-census-plan-timeout-diagnosed-no-output-accepted",
    frozenAt,
    checkpointCommit,
    branch: "main",
    debateNumber: DEBATE_NUMBER,
    ...
    nextAuthorizedAction:
      "freeze-two-minimum-field-disjoint-debate-49-replacement-contexts",
  };
  ...
  const correction = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-04-debate-49-inventory-plan-recovery-correction-plan",
    protocolId: PROTOCOL_ID,
    status:
      "two-minimum-field-disjoint-debate-49-inventory-plan-recovery-contexts-frozen-not-activated",
    frozenAt,
    checkpointCommit,
    branch: "main",
    diagnosis: DIAGNOSIS,
  ...
  const preparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-04-debate-49-inventory-plan-recovery-execution-preparation-manifest",
    protocolId: PROTOCOL_ID,
    status:
    ...
    meteredApiCostUsdMaximum: 0,
    nextRequiredAction:
      "separately-activate-two-frozen-debate-49-inventory-plan-recovery-shards",
  };
...
async function validate() {
  ...
  assertV4(
    ...
    "frozen Debate 49 recovery preparation controls drifted"
  );
  ...
  console.log(
    JSON.stringify(
      {
        status: "passed-frozen-debate-49-inventory-plan-recovery-preparation",
        contexts: 2,
        shards: SHARDS,
        protectedAcceptedPlans: 9,
