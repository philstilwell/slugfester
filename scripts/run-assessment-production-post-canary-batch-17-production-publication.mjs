...
const manifestPath = `${POST_CANARY_BATCH_17_PRODUCTION_PUBLICATION_ROOT}/mutation-manifest.json`;
const activationPath = `${POST_CANARY_BATCH_17_PRODUCTION_PUBLICATION_ROOT}/execution-activation.json`;
const executionPath = `${POST_CANARY_BATCH_17_PRODUCTION_PUBLICATION_ROOT}/execution.json`;
const analysisPath = `${POST_CANARY_BATCH_17_PRODUCTION_PUBLICATION_ROOT}/analysis.json`;
const startedAt = new Date().toISOString();
const [manifest, activation] = await Promise.all([readJson(manifestPath), readJson(activationPath)]);

assertV4(
  manifest.protocolId === POST_CANARY_BATCH_17_PRODUCTION_PUBLICATION_PROTOCOL_ID &&
    activation.status === "frozen-batch-17-production-publication-mutation-pass-activated" &&
    activation.executionDiscipline.attempts === 1 &&
    activation.executionDiscipline.retries === 0 &&
    activation.executionDiscipline.reruns === 0,
  "frozen Batch 17 production publication activation required"
);
...
for (const lock of [manifest.validator, manifest.productionBaseline.references,
  manifest.compatibilityAcceptance.analysis, manifest.compatibilityAcceptance.execution,
  manifest.scorePolicy.promotion, manifest.scorePolicy.activeControl,
...
assertV4(changedRecords === 4, "exactly four production records must change");
for (const debate of manifest.debates) {
...
const batchLedgerPaths = new Set(
  manifest.debates.map((debate) => debate.productionLedgerPath)
);
for (const lock of manifest.productionBaseline.existingProductionLedgers.files) {
  if (!batchLedgerPaths.has(lock.path)) {
    assertV4(
      sha256(await readBytes(lock.path)) === lock.sha256,
      `${lock.path}: unrelated production ledger changed during mutation`
    );
  }
}
...
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-17-production-publication-execution",
  protocolId: POST_CANARY_BATCH_17_PRODUCTION_PUBLICATION_PROTOCOL_ID,
  status: "passed-batch-17-production-publication-mutation-awaiting-generated-derivative-correction",
  startedAt,
  completedAt,
  ...
  protected: {
    ...
    unrelatedPreexistingProductionLedgersByteIdentical: 170,
    preexistingBatchProductionLedgersReplaced: 0,
    newBatchProductionLedgersPublished: 4
  },
  validation: {
  ...
  totals: {
    ...
    productionLedgerFiles: 4,
    changedDebateRecords: 4,
    scorePasses: 0,
    modelContexts: 0,
    paidServiceCalls: 0,
  ...
  nextAuthorizedAction: "prepare-isolated-batch-17-generated-seo-derivative-correction-under-standing-authorization"
};
await writeFile(resolve(executionPath), serializedJson(execution));
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-17-production-publication-analysis",
  protocolId: POST_CANARY_BATCH_17_PRODUCTION_PUBLICATION_PROTOCOL_ID,
  status: "batch-17-production-publication-mutation-passed-pending-generated-derivative-correction",
  analyzedAt: completedAt,
  execution: await lockFile(executionPath),
  decision: {
    exactMutationAuthenticated: true,
    fourRecordsReplaced: true,
    fourLedgersPublishedByteForByte: true,
...
console.log(serializedJson({ status: execution.status, debatesPublished: 4, ledgersPublished: 4, changedRecords, routeValidationPassed: true, directIncrementalCostUsd: 0 }));
