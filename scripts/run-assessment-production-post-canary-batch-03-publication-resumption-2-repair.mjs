...
assertV4(
  activation.status ===
      "frozen-eight-isolated-sixteen-field-batch-03-debate-157-publication-resumption-2-repair-contexts-authorized-under-failure-recovery-standing-authorization" &&
  ...
  "the Debate 157 repair execution is not authorized"
);
assertV4(
  ...
  "the Debate 157 repair execution controls changed"
);
...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), `batch-03-debate-157-repair-${context.contextIndex}-`)
  );
  const codexHome = await mkdtemp(
    path.join(os.tmpdir(), `batch-03-debate-157-repair-home-${context.contextIndex}-`)
  );
  ...
  try {
    ...
    const prompt = [
      ...
      `Act only as isolated Debate 157 bounded publication repair editor ${context.contextIndex}.`,
    ...
    process.stdout.write(
      `[batch-03-debate-157-publication-repair] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort}\n`
    );
    ...
    if (
      ...
      try {
        validationSummary = validateDebate157RepairOutput(
      ...
      const validation = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-03-publication-resumption-2-debate-157-repair-validation",
      ...
      const provenance = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-03-publication-resumption-2-debate-157-repair-provenance",
        protocolId: activation.protocolId,
        debateNumber: "157",
        packetIndex: context.packetIndex,
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-publication-resumption-2-debate-157-repair-model-execution",
  protocolId: activation.protocolId,
  status:
    validContexts === 8
      ? "batch-03-debate-157-eight-context-publication-resumption-2-repair-gate-passed"
      : "batch-03-debate-157-publication-resumption-2-repair-gate-complete-with-failure",
  gateStartedAt,
  ...
  authorization: {
    ...
    deterministicMergeAndCompleteValidation: validContexts === 8,
    retry: false,
