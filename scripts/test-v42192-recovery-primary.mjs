...
assert.equal(manifest.status, "frozen-three-recovery-primary-contexts-authorized");
...
assert.equal(manifest.stopRules.correctionAuthorized, false);
assert.equal(manifest.deterministicCompilation.repositoryDerivedResponseClass, true);
for (const context of manifest.contexts) {
  assert.equal(classifyV4219PrimaryRoute(context).route, "direct");
  assert.equal(context.routeEvidence.durationUsedForRouting, false);
}
