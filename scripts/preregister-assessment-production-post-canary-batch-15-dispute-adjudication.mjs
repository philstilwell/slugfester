...
const preparationPath = `${POST_CANARY_BATCH_15_DISPUTE_ADJ_ROOT}/preparation-manifest.json`;
const manifestPath = `${POST_CANARY_BATCH_15_DISPUTE_ADJ_ROOT}/execution-preparation-manifest.json`;
const activationPath = `${POST_CANARY_BATCH_15_DISPUTE_ADJ_ROOT}/execution-activation.json`;
const executionPath = `${POST_CANARY_BATCH_15_DISPUTE_ADJ_ROOT}/model-execution.json`;
...
assertV4(
  preparation.status ===
    ...
    preparation.totals.disputedMoves === 186 &&
    preparation.totals.candidateSelections === 561 &&
  ...
  "Batch 15 adjudication packets are not ready for execution preparation"
);
assertV4(
  ...
  "Batch 15 accepted source or cost boundary changed"
);
assertV4(
  ...
  "Batch 15 adjudication model or score-blind boundary changed"
);
assertV4(
  preparation.totals.maximumCopiedInputBytes <= 400000,
  "Batch 15 context exceeds the frozen transport ceiling"
);
assertV4(
