...
const stageRoot = rootIndex >= 0 ? process.argv[rootIndex + 1] : V429_ROOT;
const manifest = JSON.parse(await readFile(`${stageRoot}/execution-manifest.json`, "utf8"));
const codex = "/Applications/ChatGPT.app/Contents/Resources/codex";
const authSource = path.join(os.homedir(), ".codex", "auth.json");
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
assertV4(manifest.authorization.twoProposalContexts, "partition execution unauthorized");
...
for (const chunk of manifest.chunks) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-partition-${chunk.chunkId}-`));
  const home = await mkdtemp(path.join(os.tmpdir(), `slugfester-partition-home-${chunk.chunkId}-`));
  ...
  try {
    ...
    process.stdout.write(`[${manifest.stageLabel ?? "v4.2.9-partition"}] starting ${manifest.model.label}/${manifest.model.reasoningEffort} ${chunk.chunkId}\n`);
    ...
    if (invocation.timedOut || invocation.code !== 0 || invocation.signal !== null || !resultExists) {
      ...
      const validation = await run(process.execPath, ["scripts/validate-v429-proposal.mjs", chunk.rawOutput, `${stageRoot}/preparation-manifest.json`, chunk.chunkId], { cwd: root, env: process.env }, 120000);
