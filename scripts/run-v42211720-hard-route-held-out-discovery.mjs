...
assertV4(
  manifest.status === "frozen-twenty-hard-route-held-out-discovery-contexts-authorized"
    ...
    && manifest.operationalCanary.status === "retired-transport-canary-passed",
...
async function executeContext(context, contextIndex) {
  ...
  try {
    ...
    process.stdout.write(`[v4.2.21.17.20] starting ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber} ${context.chunkId}\n`);
  ...
  process.stdout.write(`[v4.2.21.17.20] Debate ${context.debateNumber} ${context.chunkId} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
...
let rampPassed = await runFixedIndexes([0], 1, "ramp-one");
...
const execution = {
  schemaVersion: "4.2.21.17.20-hard-route-held-out-discovery-model-execution",
  protocolId: manifest.protocolId,
  status: validContexts === manifest.contexts.length
    ? "twenty-hard-route-held-out-discovery-contexts-passed"
    : "hard-route-held-out-discovery-complete-with-failure",
  heldOut: true,
  gateStartedAt,
