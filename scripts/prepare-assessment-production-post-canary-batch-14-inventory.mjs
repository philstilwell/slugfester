...
import {
  POST_CANARY_BATCH_14_STANDING_AUTHORIZATION,
  loadAndValidatePostCanaryBatch14StandingAuthorization,
} from "./lib/assessment-production-post-canary-batch-14-standing-authorization.mjs";
...
const SCRIPT =
  "scripts/prepare-assessment-production-post-canary-batch-14-inventory.mjs";
const PROTOCOL_ID =
  "assessment-production-post-canary-batch-14-candidate-sharded-inventory";
const SOURCE_PACKET_VERSION =
  "1.0-assessment-production-post-canary-batch-14-candidate-sharded-inventory-source-packet";
...
function buildInventorySourcePacket(sourcePacket) {
  assertV4(
    sourcePacket?.schemaVersion ===
      "1.0-assessment-production-post-canary-batch-14-score-blind-source-packet" &&
      sourcePacket.protocolId ===
        "assessment-production-post-canary-batch-14-source-preparation" &&
    ...
    "Batch 14 source packet is unavailable or not dyadic"
  );
  return {
...
function makePlanPacket({
  ...
  return {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-14-candidate-census-plan-packet",
    protocolId: PROTOCOL_ID,
...
async function buildFrozenArtifacts(frozenAt) {
  const standingAuthorization =
    await loadAndValidatePostCanaryBatch14StandingAuthorization();
  const [
  ...
  assertV4(
    ...
    "Batch 14 standing authorization does not cover exact inventory preparation and execution"
  );

  assertV4(
    discoveryAnalysis.status ===
      "post-canary-batch-14-discovery-passed-standing-authorization-active-for-inventory-preparation" &&
      ...
      discoveryAnalysis.audit?.validContexts === 33 &&
      ...
      discoveryAnalysis.totals?.modelContextsExecuted === 33 &&
      ...
      discoveryAnalysis.nextAuthorizedAction ===
        "prepare-freeze-and-activate-batch-14-candidate-census-planner-contexts-under-standing-authorization",
    "passed Batch 14 discovery boundary drifted"
  );
  assertV4(
    discoveryExecution.status ===
      "thirty-three-post-canary-batch-14-discovery-contexts-passed" &&
      discoveryExecution.contextsAttempted === 33 &&
      discoveryExecution.validContexts === 33 &&
    ...
    "Batch 14 discovery execution drifted"
  );
  assertV4(
      sourcePreparation.status ===
      "post-canary-batch-14-ten-complete-score-blind-source-packets-prepared-awaiting-validation" &&
      ...
      sourcePreparation.tokenLedgerCompatibility?.occurrences?.length === 0 &&
      allBooleanLeavesTrue(sourcePreparation.stopRules),
    "Batch 14 source preparation or model boundary drifted"
  );
  assertV4(
    sourcePreparationValidation.status ===
      "post-canary-batch-14-score-blind-source-packet-validation-passed-frozen-under-standing-authorization" &&
    ...
    "Batch 14 source validation drifted"
  );
  assertV4(
    selection.status ===
      "fourteenth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    ...
    "Batch 14 frozen selection drifted"
  );
  assertV4(
  ...
  for (const discovered of discoveryAnalysis.debates) {
    ...
    assertV4(
      ...
      `${discovered.debateNumber}: Batch 14 identity drifted`
    );
    const [
  ...
  assertV4(
    ...
    "Batch 14 totals or packet state drifted"
  );
  ...
  const sourceFiles = [
    ...
    "scripts/lib/assessment-production-post-canary-batch-14-standing-authorization.mjs",
    POST_CANARY_BATCH_14_STANDING_AUTHORIZATION,
    SCRIPT,
  ...
  const preparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-14-candidate-sharded-inventory-preparation",
    protocolId: PROTOCOL_ID,
    sideSelectionProtocolId: CHRONOLOGY_FALLBACK_INVENTORY.protocolId,
    status:
      "post-canary-batch-14-candidate-sharded-source-assets-and-ten-planner-packets-frozen",
    preparedAt: frozenAt,
    ...
    userAuthorization: {
      scope:
        "prepare, freeze, activate, and execute the Batch 14 candidate-census and candidate-sharded side-selection workflow under the frozen standing authorization",
      standingAuthorization: POST_CANARY_BATCH_14_STANDING_AUTHORIZATION,
    ...
    nextAuthorizedAction:
      "freeze-and-activate-batch-14-candidate-census-plan-execution-under-standing-authorization",
  };
  ...
  const executionPreparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-14-candidate-census-plan-execution-preparation-manifest",
    protocolId: PROTOCOL_ID,
    status:
      "frozen-ten-post-canary-batch-14-candidate-census-plan-contexts-prepared-not-authorized",
    frozenAt,
    ...
    costEstimate: {
      ...
      estimateBasis:
        `The promoted ten-context candidate-census planner used this contract at concurrency two. Batch 14 freezes the same ten-context shape with an observed maximum of ${maximumCopiedInputBytes.toLocaleString("en-US")} copied bytes, below the proven ${V212_CANDIDATE_SHARDED_INVENTORY.maximumCopiedInputBytes.toLocaleString("en-US")}-byte ceiling. ChatGPT-subscription execution has no direct incremental API charge.`,
    },
  ...
  const validation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-14-candidate-sharded-inventory-preparation-validation",
    protocolId: PROTOCOL_ID,
    status:
      "batch-14-candidate-sharded-inventory-packets-and-plan-execution-manifest-validation-passed-frozen-standing-authorization-active",
...
async function validateFrozen() {
  ...
  assertV4(
    ...
    "frozen Batch 14 inventory preparation is invalid"
  );
  ...
  assertV4(
    executionPreparation.status ===
      ...
      executionPreparation.sourceCompatibility
        .minimumCandidateLexicalTokensChanged === false &&
      executionPreparation.sourceCompatibility.occurrences.length === 0 &&
...
if (command === "freeze") await freeze();
  ...
  throw new Error(
    "usage: prepare-assessment-production-post-canary-batch-14-inventory.mjs <freeze|validate>"
  );
}
