...
assertV4(
  preparation.protocolId === POST_CANARY_BATCH_01_RESUMPTION_REPAIR_PROTOCOL_ID &&
    preparation.status ===
    ...
    preparation.contexts?.length === 3 &&
    JSON.stringify(
      preparation.contexts.map(({ debateNumber, writableFieldCount }) => ({
        debateNumber,
        writableFieldCount
      }))
    ) ===
      JSON.stringify([
        { debateNumber: "91", writableFieldCount: 1 },
        { debateNumber: "13", writableFieldCount: 2 },
        { debateNumber: "13", writableFieldCount: 1 }
      ]) &&
    ...
    preparation.authorization?.deterministicMergeAndCompleteValidation ===
      false &&
  ...
  "the publication-resumption repair execution is not prepared"
);
assertV4(
...
const activation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-01-publication-resumption-repair-execution-activation",
  ...
  userAuthorization: {
    instruction:
      "I approve activation and execution of exactly the three frozen Batch 1 score-locked publication-resumption repair contexts using 5.6 Sol with low reasoning effort through my ChatGPT subscription, with a direct incremental cost cap of $0. Use the frozen 1→2 scheduler, one attempt per context, and no retries. Stop after deterministic repair-output validation, merge, complete Debate 91 and Debate 13 publication validation, complete ten-debate cohort replay, analysis, committing, and pushing. Do not execute publication models, prepare or execute further repair packets, use paid services, compile or finalize publication, mutate production, or select the next batch.",
    directIncrementalCostUsdMaximum: 0,
    repairModelContexts: 3,
    writableFields: 4,
  ...
  authorization: {
    ...
    deterministicCohortReplay: true,
  ...
  nextRequiredAction:
    "execute-the-three-frozen-batch-01-publication-resumption-repair-contexts-once"
};
