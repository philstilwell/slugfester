...
const MANIFEST = `${POST_CANARY_BATCH_03_DEBATE_157_REPAIR_ROOT}/execution-preparation-manifest.json`;
...
assert.equal(manifest.protocolId, POST_CANARY_BATCH_03_DEBATE_157_REPAIR_PROTOCOL_ID);
...
assert.deepEqual(manifest.costEstimate.expectedParallelWallMinutes, [4, 16]);
...
assert.equal(manifest.totals.contexts, 8);
assert.equal(manifest.totals.writableFields, 16);
...
for (const context of manifest.contexts) {
  ...
  assert.equal(packet.debateNumber, "157");
  ...
  assert.deepEqual(schema, buildDebate157RepairSchema(packet));
  ...
  assert.equal(validateDebate157RepairOutput(repair, packet).status, "passed");
...
const merged = mergeAndValidateDebate157Repairs({
  baseOutput,
  repairs: syntheticRepairs,
  repairPackets,
  publicationPacket
});
assert.equal(merged.transformations.length, 16);
