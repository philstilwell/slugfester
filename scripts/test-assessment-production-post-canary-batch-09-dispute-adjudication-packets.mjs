...
import {
  POST_CANARY_BATCH_09_DISPUTE_ADJ_ISOLATION,
  POST_CANARY_BATCH_09_DISPUTE_ADJ_OUTPUT_VERSION,
...
const EXPECTED_DEBATES = ["170", "134", "19", "114", "166", "89", "176", "183", "112", "17"];
...
const preparation = JSON.parse(
  await readFile(`${POST_CANARY_BATCH_09_DISPUTE_ADJ_ROOT}/preparation-manifest.json`, "utf8")
);
...
assert.equal(preparation.totals.disputedMoves, 173);
assert.equal(preparation.totals.candidateSelections, 459);
...
assert.equal(preparation.acceptedSourceBoundary.allFourAudioMovesVerified, true);
...
for (const context of preparation.contexts) {
  ...
  const output = {
    schemaVersion: POST_CANARY_BATCH_09_DISPUTE_ADJ_OUTPUT_VERSION,
    ...
    isolation: structuredClone(POST_CANARY_BATCH_09_DISPUTE_ADJ_ISOLATION),
  ...
  const validation = validatePostCanaryBatch09DisputeAdjudicationOutput(output, packet);
...
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_09_DISPUTE_ADJ_ROOT}/packets`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_09_DISPUTE_ADJ_ROOT}/provenance`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
