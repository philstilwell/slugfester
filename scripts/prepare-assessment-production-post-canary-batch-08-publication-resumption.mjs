...
const REPAIRED_DEBATE_88 = `${ROOT}/repair-1/merged/debate-88.json`;
const DEBATE_88_PACKET = `${ROOT}/packets/debate-88.json`;
...
const repairedDebate88 = value(REPAIRED_DEBATE_88);
const debate88Packet = value(DEBATE_88_PACKET);
...
assertV4(
  originalPreparation.status ===
    ...
    originalPreparation.totals?.moves === 182 &&
    ...
    originalActivation.status ===
      "frozen-ten-post-canary-batch-08-publication-contexts-authorized" &&
    originalActivation.contexts?.length === 10,
  "the original Batch 8 publication preparation changed"
);
assertV4(
  originalExecution.status ===
      "post-canary-batch-08-publication-gate-complete-with-failure" &&
    ...
    originalAnalysis.status ===
      "post-canary-batch-08-publication-output-gate-failed" &&
    diagnosis.status ===
      "frozen-diagnosed-batch-08-debate-88-two-field-publication-validation-failure" &&
    diagnosis.executionBoundary?.unattemptedContextIndexes?.length === 9 &&
    canonicalJson(diagnosis.executionBoundary.unattemptedContextIndexes) ===
      canonicalJson([1, 2, 3, 4, 5, 6, 7, 8, 9]) &&
    canonicalJson(
      originalPreparation.contexts
        .slice(1)
        .map((context) => context.debateNumber)
    ) === canonicalJson(POST_CANARY_BATCH_08_PUBLICATION_RESUMPTION_DEBATES),
  "the preserved nine-context publication failure boundary changed"
);
assertV4(
  repairPreparation.status ===
      "frozen-one-context-batch-08-publication-repair-prepared-not-activated" &&
    repairActivation.status ===
      "frozen-one-context-batch-08-publication-repair-authorized" &&
    repairExecution.status ===
      "batch-08-one-context-publication-repair-gate-passed" &&
    ...
    repairAnalysis.status ===
      "batch-08-debate-88-publication-repair-and-validation-passed-awaiting-nine-context-resumption" &&
    ...
    repairValidation.repairedDebates?.[0]?.validation?.moves === 20 &&
  ...
  "the accepted Debate 88 repair boundary changed"
);
assertV4(
  validatePostCanaryBatch08PublicationOutput(
      repairedDebate88,
  ...
  "the repaired Debate 88 staging output failed replay"
);
...
for (
  ...
  resumptionIndex < POST_CANARY_BATCH_08_PUBLICATION_RESUMPTION_DEBATES.length;
  resumptionIndex += 1
) {
  const debateNumber =
    POST_CANARY_BATCH_08_PUBLICATION_RESUMPTION_DEBATES[resumptionIndex];
...
assertV4(
  contexts.length === 9 &&
    ...
    resumptionAudioVerifiedMoves === 6,
...
const newStaticSources = [
  ...
  REPAIRED_DEBATE_88,
  DEBATE_88_PACKET,
  "scripts/lib/assessment-production-post-canary-batch-08-publication-resumption.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-08-publication-resumption.mjs",
  "scripts/test-assessment-production-post-canary-batch-08-publication-resumption-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-08-publication-resumption.mjs",
  "scripts/run-assessment-production-post-canary-batch-08-publication-resumption.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-08-publication-resumption.mjs"
];
...
const stopRules = {
  acceptedDebate88RepairFailureBlocks: true,
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-08-publication-resumption-execution-preparation-manifest",
  protocolId: POST_CANARY_BATCH_08_PUBLICATION_RESUMPTION_PROTOCOL_ID,
  status:
  ...
  userAuthorization: {
    instruction:
      "The Batch 8 standing authorization permits preparation, activation, execution, validation, analysis, committing, and pushing of the nine untouched score-locked publication-resumption contexts at direct incremental cost $0.",
  ...
  inputs: {
    ...
    repairedDebate88: REPAIRED_DEBATE_88,
    debate88Packet: DEBATE_88_PACKET
  },
  ...
  acceptedDebate88: {
    ...
    packet: DEBATE_88_PACKET,
    output: REPAIRED_DEBATE_88,
  ...
  isolation: {
    ...
    repairedDebate88OutputUnavailableToResumptionModels: true,
    failedOriginalDebate88OutputUnavailable: true,
  ...
  deterministicValidation: {
    ...
    acceptedDebate88RepairReplayedAtFreeze: true,
  ...
  acceptanceContract: {
    ...
    resumptionMovesRequired: 162,
    cohortMovesRequired: 182,
    resumptionCritiquesRequired: 162,
    cohortCritiquesRequired: 182,
  ...
  nextAuthorizedAction:
    "activate-and-execute-exactly-nine-frozen-batch-08-publication-resumption-contexts-under-standing-authorization"
};
