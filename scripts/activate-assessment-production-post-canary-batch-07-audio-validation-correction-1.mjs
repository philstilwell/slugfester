...
const PATH = 'docs/assessment-production/post-canary-continuation-v1/batch-07/audio-verification/correction-1-execution-activation.json';
...
if (shouldWrite) {
  assert(activation.status === 'frozen-batch-07-audio-validation-correction-1-prepared-not-active', 'inactive activation unavailable');
  activation.status = 'frozen-batch-07-audio-validation-correction-1-authorized-under-standing-authorization';
  ...
  activation.nextAuthorizedAction = 'execute-exactly-one-batch-07-audio-validation-correction-1-pass';
  await writeFile(PATH, `${JSON.stringify(activation, null, 2)}\n`);
  console.log(`activated ${PATH}`);
} else {
  assert(activation.status === 'frozen-batch-07-audio-validation-correction-1-authorized-under-standing-authorization', 'correction activation is not active');
  ...
  console.log('Batch 7 audio validation correction-1 activation is valid.');
}
