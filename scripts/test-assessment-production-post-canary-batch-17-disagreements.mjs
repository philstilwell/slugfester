...
import { extractAssessmentProductionPostCanaryBatch17Disagreements } from "./lib/assessment-production-post-canary-batch-17-disagreement.mjs";
...
assert.equal(analysis.batchNumber, 17);
...
assert.equal(analysis.adjudicationWorkload.totalMoves, 79);
assert.equal(
...
assert.equal(analysis.sourceAcceptance.acceptedContexts, 8);
assert.equal(analysis.sourceAcceptance.rejectedContexts, 0);
assert.equal(analysis.sourceAcceptance.acceptedOutputHashReplays, 8);
assert.equal(analysis.sourceAcceptance.unchangedV4220ValidatorPasses, 8);
  ...
  JSON.stringify(
    {
      ...
      frozenArtifactHashesVerified: 4,
      deterministicDisagreementReplays: 4,
