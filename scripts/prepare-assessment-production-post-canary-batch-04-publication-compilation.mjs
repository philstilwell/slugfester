...
import {
  POST_CANARY_BATCH_04_PUBLICATION_COMPILATION_ORDER,
  POST_CANARY_BATCH_04_PUBLICATION_COMPILATION_PROTOCOL_ID,
  POST_CANARY_BATCH_04_PUBLICATION_COMPILATION_ROOT
} from "./lib/assessment-production-post-canary-batch-04-publication-compilation.mjs";
...
const ROOT = POST_CANARY_BATCH_04_PUBLICATION_COMPILATION_ROOT;
const PUBLICATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-04/publication-reconstruction";
const FINAL_REPAIR_ROOT = `${PUBLICATION_ROOT}/resumption-2/repair-1`;
...
const ACCEPTED_OUTPUTS = Object.freeze({
  "127": `${PUBLICATION_ROOT}/repair-1/merged/debate-127.json`,
  "67": `${PUBLICATION_ROOT}/resumption-1/outputs/debate-67.json`,
  "85": `${PUBLICATION_ROOT}/resumption-1/outputs/debate-85.json`,
  "49": `${PUBLICATION_ROOT}/resumption-1/repair-1/merged/debate-49.json`,
  "186": `${PUBLICATION_ROOT}/resumption-2/outputs/debate-186.json`,
  "81": `${PUBLICATION_ROOT}/resumption-2/outputs/debate-81.json`,
  "148": `${PUBLICATION_ROOT}/resumption-2/outputs/debate-148.json`,
  "47": `${PUBLICATION_ROOT}/resumption-2/outputs/debate-47.json`,
  "03": `${PUBLICATION_ROOT}/resumption-2/repair-1/merged/debate-03.json`,
  "185": `${PUBLICATION_ROOT}/resumption-2/repair-1/merged/debate-185.json`
});
const PUBLICATION_PACKETS = Object.freeze(
  Object.fromEntries(
    POST_CANARY_BATCH_04_PUBLICATION_COMPILATION_ORDER.map((debateNumber) => [
      debateNumber,
...
assertV4(
  canonicalJson(Object.keys(ACCEPTED_OUTPUTS).sort()) ===
    canonicalJson([...POST_CANARY_BATCH_04_PUBLICATION_COMPILATION_ORDER].sort()) &&
    canonicalJson(Object.keys(PUBLICATION_PACKETS).sort()) ===
      canonicalJson([...POST_CANARY_BATCH_04_PUBLICATION_COMPILATION_ORDER].sort()),
  "Batch 4 compilation inputs do not match the frozen cohort order"
);

const finalRepairAnalysis = await parse(FINAL_REPAIR_ANALYSIS);
assertV4(
  finalRepairAnalysis.status ===
      "batch-04-complete-ten-debate-publication-output-gate-passed-after-two-single-field-repairs" &&
    finalRepairAnalysis.gate?.completeDebate03ValidationPassed === true &&
    finalRepairAnalysis.gate?.completeDebate185ValidationPassed === true &&
    finalRepairAnalysis.gate?.completeTenDebateCohortReplayPassed === true &&
    finalRepairAnalysis.gate?.cohortDebates === 10 &&
    finalRepairAnalysis.gate?.cohortMoves === 203 &&
    finalRepairAnalysis.gate?.cohortCritiques === 203 &&
    finalRepairAnalysis.gate?.modelAuthoredScores === 0 &&
    finalRepairAnalysis.gate?.immutableFieldsChanged === 0 &&
    finalRepairAnalysis.nextAuthorizedAction ===
      "prepare-batch-04-publication-compilation-under-standing-authorization",
  "the passing Batch 4 publication-reconstruction checkpoint changed"
);
...
const identityRows = [];
const contexts = [];
for (const debateNumber of POST_CANARY_BATCH_04_PUBLICATION_COMPILATION_ORDER) {
  ...
  const validation = validatePostCanaryBatch04PublicationOutput(output, packet);
  assertV4(
...
assertV4(
  contexts.length === 10 &&
    contexts.reduce((sum, context) => sum + context.expectedMoves, 0) === 203,
  "the complete Batch 4 compilation cohort changed"
);

const identitySnapshot = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-publication-compilation-identity-snapshot",
  protocolId: POST_CANARY_BATCH_04_PUBLICATION_COMPILATION_PROTOCOL_ID,
...
const sourceFiles = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-04-publication-compilation.mjs",
  ...
  "scripts/prepare-assessment-production-post-canary-batch-04-publication-compilation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-04-publication-compilation.mjs",
  "scripts/run-assessment-production-post-canary-batch-04-publication-compilation.mjs",
  "scripts/test-assessment-production-post-canary-batch-04-publication-compilation-preparation.mjs",
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-deterministic-publication-compilation-preparation",
  protocolId: POST_CANARY_BATCH_04_PUBLICATION_COMPILATION_PROTOCOL_ID,
  status:
    "frozen-post-canary-batch-04-deterministic-publication-compilation-prepared-not-authorized",
  frozenAt,
  ...
  AIOnly: true,
  userAuthorization: {
    instruction: POST_CANARY_BATCH_04_STANDING_AUTHORIZATION_INSTRUCTION,
    scopeReference:
      "the Batch 4 standing authorization for the remaining publication workflow",
  ...
  explicitOrder: POST_CANARY_BATCH_04_PUBLICATION_COMPILATION_ORDER,
  contexts,
  executionPolicy: {
  ...
  nextAuthorizedAction:
    "activate-and-execute-one-frozen-batch-04-deterministic-publication-compilation-pass-under-standing-authorization"
};
  ...
  JSON.stringify(
    {
      status: shouldWrite
        ? manifest.status
        : "post-canary-batch-04-publication-compilation-preparation-preview",
