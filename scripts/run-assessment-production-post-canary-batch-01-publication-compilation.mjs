...
import {
  ...
  compilePostCanaryBatch01PublicationStagingRecord,
...
assertV4(
  ...
  "Batch 1 deterministic publication compilation is not authorized"
);
...
try {
  ...
  for (const debateNumber of POST_CANARY_BATCH_01_PUBLICATION_COMPILATION_ORDER) {
    ...
    const compiled = compilePostCanaryBatch01PublicationStagingRecord({
      output,
      packet,
      identity
    });
    const validation = validatePostCanaryBatch01CompiledStagingRecord({
      compiled,
      output,
      packet,
      identity
  ...
  const audit = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-01-deterministic-publication-compilation-audit",
    protocolId: activation.protocolId,
    status: "passed-ten-debate-batch-01-deterministic-publication-compilation",
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-01-deterministic-publication-compilation-execution",
  protocolId: activation.protocolId,
  status:
    status === "passed"
      ? "ten-debate-batch-01-deterministic-publication-compilation-passed"
      : "ten-debate-batch-01-deterministic-publication-compilation-failed",
  startedAt,
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-01-deterministic-publication-compilation-analysis",
  protocolId: activation.protocolId,
  ...
  nextAuthorizedAction:
    status === "passed"
      ? "user-approval-required-before-batch-01-publication-finalization-preparation-only"
