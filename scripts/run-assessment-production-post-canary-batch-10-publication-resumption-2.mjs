...
assertV4(
  activation.status === "frozen-six-original-unattempted-batch-10-publication-contexts-activated" &&
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-10-publication-resumption-2-${context.debateNumber}-`));
  const isolatedCodexHome = await mkdtemp(path.join(os.tmpdir(), `batch-10-publication-resumption-2-home-${context.debateNumber}-`));
  ...
  try {
    ...
    const prompt = [
      ...
      "Never infer, emit, recalculate, or suggest changing a score; never change identity, structure, move selection, or source evidence; never consult legacy assessment material, any prior Batch 10 publication output, other debates, rankings, winners, or another model context; never attribute AI material to a participant.",
    ...
    process.stdout.write(`[batch-10-publication-resumption-2] starting original index ${context.originalContextIndex} Debate ${context.debateNumber}\n`);
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 ||
      ...
      try {
        validationSummary = validatePostCanaryBatch10PublicationOutput(
      ...
      const validation = {
        schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-resumption-validation",
      ...
      const provenance = {
        schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-resumption-provenance",
        protocolId: activation.protocolId,
        originalContextIndex: context.originalContextIndex,
        debateNumber: context.debateNumber,
        debateId: context.debateId,
  ...
  process.stdout.write(`[batch-10-publication-resumption-2] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
...
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-six-original-unattempted-context-resumption-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "six-original-unattempted-batch-10-publication-contexts-passed"
    : "six-context-publication-resumption-completed-with-failure",
  gateStartedAt,
