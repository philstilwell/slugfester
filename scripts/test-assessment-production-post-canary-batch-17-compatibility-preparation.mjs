...
const preparationPath =
  `${POST_CANARY_BATCH_17_COMPATIBILITY_ROOT}/preparation-manifest.json`;
const analysisPath =
  `${POST_CANARY_BATCH_17_COMPATIBILITY_ROOT}/analysis.json`;
...
assert.equal(
  ...
  "batch-17-site-ledger-adapter-and-validator-route"
);
assert.equal(
...
assert.equal(preparation.totals.debates, 4);
assert.equal(preparation.totals.planPackets, 4);
assert.equal(preparation.totals.sections, 21);
assert.equal(preparation.totals.moves, 79);
assert.equal(preparation.totals.oneSidedDisplayRows, 5);
assert.equal(preparation.totals.overallBlunders, 20);
assert.equal(preparation.totals.emptyReferenceLinks, 14);
assert.equal(preparation.totals.suppliedReferenceLinks, 6);
assert.equal(preparation.totals.repositoryScoreReplays, 4);
...
assert.equal(
  await exists(`${POST_CANARY_BATCH_17_COMPATIBILITY_ROOT}/execution.json`),
  false
);
assert.equal(
  await exists(
...
assert.deepEqual(packetNumbers, POST_CANARY_BATCH_17_COMPATIBILITY_ORDER);
for (const packetRecord of preparation.artifacts.packets) {
  ...
  const positive = validatePostCanaryBatch17SiteLedgerAdapter({
...
assert.equal(analysis.checks.batch02RoutePreservationSpecified, true);
assert.equal(analysis.checks.batch03RoutePreservationSpecified, true);
assert.equal(analysis.checks.batch04RoutePreservationSpecified, true);
assert.equal(analysis.checks.batch05RoutePreservationSpecified, true);
assert.equal(analysis.checks.batch06RoutePreservationSpecified, true);
assert.equal(analysis.checks.batch07RoutePreservationSpecified, true);
assert.equal(analysis.checks.batch08RoutePreservationSpecified, true);
assert.equal(analysis.checks.batch09RoutePreservationSpecified, true);
assert.equal(analysis.checks.batch10RoutePreservationSpecified, true);
assert.equal(analysis.checks.batch11RoutePreservationSpecified, true);
assert.equal(analysis.checks.batch12RoutePreservationSpecified, true);
assert.equal(analysis.checks.batch13RoutePreservationSpecified, true);
assert.equal(analysis.checks.batch14RoutePreservationSpecified, true);
assert.equal(analysis.checks.batch15RoutePreservationSpecified, true);
assert.equal(analysis.checks.batch16RoutePreservationSpecified, true);
