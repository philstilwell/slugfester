...
import {
  ...
  evaluatePostCanaryBatch09ScoreStability
...
const accepted = evaluatePostCanaryBatch09ScoreStability(passing);
...
assert.equal(
  evaluatePostCanaryBatch09ScoreStability(winnerMutation).acceptancePassed,
  false
);
const distanceMutation = structuredClone(passing);
distanceMutation[2].final = scores(95, 80);
assert.equal(
  evaluatePostCanaryBatch09ScoreStability(distanceMutation).acceptancePassed,
  false
);
assert.throws(() =>
  evaluatePostCanaryBatch09ScoreStability(passing, {
    ...POST_CANARY_BATCH_09_SCORE_STABILITY_THRESHOLDS,
