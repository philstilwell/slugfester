...
import { POST_CANARY_BATCH_08_RENDERING_ORDER, POST_CANARY_BATCH_08_RENDERING_PROTOCOL_ID,
  POST_CANARY_BATCH_08_RENDERING_ROOT, validatePostCanaryBatch08RenderingPacket } from
...
assertV4(m.status === "frozen-post-canary-batch-08-rendering-verification-prepared" &&
  m.protocolId === POST_CANARY_BATCH_08_RENDERING_PROTOCOL_ID && m.packets?.length === 10 &&
  canonicalJson(m.explicitOrder) === canonicalJson(POST_CANARY_BATCH_08_RENDERING_ORDER) &&
  m.gateExpectations?.sections === 51 && m.gateExpectations?.moves === 182 &&
...
"Batch 8 rendering preparation changed");
...
for (const row of m.packets) {
  ...
  validatePostCanaryBatch08RenderingPacket(JSON.parse(bytes));
}
for (const file of m.futureOutputPathsExcludedFromSourceHashes) assertV4(!(await exists(file)), `${file} exists`);
console.log(JSON.stringify({ status: "passed", debates: 10, sections: 51, moves: 182,
