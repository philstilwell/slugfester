...
import { POST_CANARY_BATCH_12_PUBLICATION_FINALIZATION_PROTOCOL_ID,
  POST_CANARY_BATCH_12_PUBLICATION_FINALIZATION_ROOT } from
...
assertV4(p.status === "frozen-post-canary-batch-12-publication-finalization-prepared" &&
  p.protocolId === POST_CANARY_BATCH_12_PUBLICATION_FINALIZATION_PROTOCOL_ID && p.contexts?.length === 10 &&
...
"Batch 12 finalization is not prepared");
...
const activation = { schemaVersion: "1.0-assessment-production-post-canary-batch-12-publication-finalization-activation",
  protocolId: p.protocolId, status: "frozen-post-canary-batch-12-publication-finalization-authorized",
  ...
  nextAuthorizedAction: "execute-one-frozen-batch-12-deterministic-publication-finalization-pass" };
