...
import { POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_ROOT } from
  "./lib/assessment-production-post-canary-batch-11-publication-compilation.mjs";
import { POST_CANARY_BATCH_11_PUBLICATION_FINALIZATION_ORDER,
  POST_CANARY_BATCH_11_PUBLICATION_FINALIZATION_PROTOCOL_ID,
  POST_CANARY_BATCH_11_PUBLICATION_FINALIZATION_ROOT,
  buildPostCanaryBatch11PublicationFinalization,
  buildPostCanaryBatch11PublicationStagingPreviewHtml,
  validatePostCanaryBatch11PublicationFinalCandidate } from
  "./lib/assessment-production-post-canary-batch-11-publication-finalization.mjs";
...
const ROOT = POST_CANARY_BATCH_11_PUBLICATION_FINALIZATION_ROOT;
const COMPILATION_ANALYSIS = `${POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_ROOT}/analysis.json`;
const COMPILATION_MANIFEST = `${POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_ROOT}/preparation-manifest.json`;
const COMPILATION_AUDIT = `${POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_ROOT}/compilation-audit.json`;
const IDENTITY = `${POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_ROOT}/production-identity-snapshot.json`;
...
assertV4(analysis.status === "ten-debate-batch-11-deterministic-publication-compilation-passed" &&
  ...
  canonicalJson(compilation.explicitOrder) === canonicalJson(POST_CANARY_BATCH_11_PUBLICATION_FINALIZATION_ORDER),
"the accepted Batch 11 compilation changed");
const contexts = [];
for (const debateNumber of POST_CANARY_BATCH_11_PUBLICATION_FINALIZATION_ORDER) {
  ...
  const built = buildPostCanaryBatch11PublicationFinalization({ compiled,
    compiledPath: source.plannedCompiledOutput, compiledSha256: sha256(compiledBytes),
    output, packet, identity: identityRow });
  const validation = validatePostCanaryBatch11PublicationFinalCandidate({
...
"the Batch 11 finalization cohort changed");
const previewBytes = Buffer.from(buildPostCanaryBatch11PublicationStagingPreviewHtml());
const sources = [COMPILATION_ANALYSIS, COMPILATION_MANIFEST, COMPILATION_AUDIT, IDENTITY,
  POST_CANARY_BATCH_11_STANDING_AUTHORIZATION, "src/app.js", "src/styles.css",
  ...
  "scripts/prepare-assessment-production-post-canary-batch-11-publication-finalization.mjs",
  "scripts/test-assessment-production-post-canary-batch-11-publication-finalization-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-11-publication-finalization.mjs",
  "scripts/run-assessment-production-post-canary-batch-11-publication-finalization.mjs",
...
const manifest = { schemaVersion: "1.0-assessment-production-post-canary-batch-11-publication-finalization-preparation",
  protocolId: POST_CANARY_BATCH_11_PUBLICATION_FINALIZATION_PROTOCOL_ID,
  status: "frozen-post-canary-batch-11-publication-finalization-prepared",
  ...
  explicitOrder: POST_CANARY_BATCH_11_PUBLICATION_FINALIZATION_ORDER, contexts,
  ...
  nextAuthorizedAction: "activate-one-batch-11-deterministic-publication-finalization-pass" };
