...
const cohortRoot = "docs/assessment-production/post-canary-continuation-v1/batch-06";
...
const mediaRoot = "output/transcribe/assessment-production-post-canary-batch-06-audio-verification";
...
const TOOL_SOURCES = [
  ...
  "scripts/prepare-assessment-production-post-canary-batch-06-audio-verification.mjs",
  "scripts/test-assessment-production-post-canary-batch-06-audio-verification-preparation.mjs",
  "scripts/assessment-production-post-canary-batch-06-audio-verification-stage.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-06-audio-sources.mjs",
  "scripts/test-assessment-production-post-canary-batch-06-audio-sources.mjs"
];
...
assertV4(
  sourcePreparation.status ===
      "prepared-two-post-canary-batch-06-local-audio-clips-standing-authorization-active-for-audio-verification-preparation" &&
    sourcePreparation.authorization.audioVerificationManifestPreparation &&
    sourcePreparation.nextAuthorizedAction ===
      "prepare-freeze-validate-and-push-batch-06-audio-verification-manifest-and-cost-estimate-under-standing-authorization",
  "Batch 6 audio-verification preparation is not authorized"
);
assertV4(sourcePreparation.clips.length === 2 && work.moves.length === 2, "two-clip cohort changed");
assertV4(
...
assertV4(
  workPreparation.sourceCompatibility.occurrences.length === 1 &&
    workPreparation.sourceCompatibility.occurrences[0].debateNumber === "141" &&
...
assertV4(calls.length === 2 && references.length === 2, "two calls and two references required");
...
const sourceFiles = [
  ...
  batch02PreparationPath, batch02CostPath, POST_CANARY_BATCH_06_STANDING_AUTHORIZATION,
  ...TOOL_SOURCES,
...
const manifest = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-06-audio-verification-execution-preparation-manifest",
  protocolId: "assessment-production-post-canary-batch-06-decomposed-consensus",
  status: shouldWrite
    ? "prepared-two-post-canary-batch-06-paid-known-speaker-diarizations-standing-authorization-conditional-activation-ready"
    : "preview-two-post-canary-batch-06-paid-known-speaker-diarizations-standing-authorization-conditional-activation-ready",
  ...
  costEstimate: {
    ...
    durationOnlyPlanningEstimateUsd: Number((clipSeconds / 60 * 0.006).toFixed(4)),
    primaryExpectedFutureExecutionCostUsd: Number(expectedCostUsd.toFixed(7)),
    primaryEstimateBasis: "Batch 2 returned-token usage rates applied to Batch 6 exact clip seconds and two-call count",
    ...
    officialPricingCheckedAt: "2026-08-22",
  ...
  sourceHashes,
  nextAuthorizedAction: "activate-and-execute-two-batch-06-paid-audio-verification-calls-under-standing-authorization"
};
