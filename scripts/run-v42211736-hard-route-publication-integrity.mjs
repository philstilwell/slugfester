...
const manifest = JSON.parse(await readFile(path.resolve(`${V42211736_ROOT}/execution-manifest.json`), "utf8"));
...
async function runContext(context, contextIndex) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `v42211736-publication-${context.debateNumber}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `v42211736-publication-home-${context.debateNumber}-`));
  ...
  try {
    ...
    if (invocation.timedOut || invocation.code !== 0 || invocation.signal !== null || !resultExists) {
      ...
      try { validation = validateV42211736PublicationOutput(JSON.parse(await readFile(path.resolve(context.output), "utf8")), JSON.parse(await readFile(path.resolve(context.packet), "utf8"))); }
  ...
  process.stdout.write(`[v4.2.21.17.36-publication-integrity] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
