...
import {
  POST_CANARY_BATCH_15_FINAL_LEDGER_PROTOCOL_ID,
  POST_CANARY_BATCH_15_FINAL_LEDGER_ROOT,
  buildPostCanaryBatch15FinalLedger,
...
const manifestPath =
  `${POST_CANARY_BATCH_15_FINAL_LEDGER_ROOT}/final-ledger-manifest.json`;
const ledgerPath = `${POST_CANARY_BATCH_15_FINAL_LEDGER_ROOT}/final-ledger.json`;
const analysisPath = `${POST_CANARY_BATCH_15_FINAL_LEDGER_ROOT}/analysis.json`;
if (shouldWrite) {
...
const preview = buildPostCanaryBatch15FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
const validation = validatePostCanaryBatch15FinalLedger(
  preview,
  inputs.debateInputs,
  inputs.sourceHashes
);
const expected = {
  ...
  pairSelections: 221,
  scoringFieldSelections: 320,
  burdenAdjustmentSelections: 20,
  roundedMeanPopulation: 409,
...
assertV4(
  ...
  "post-canary Batch 15 final-ledger preview population mismatch"
);
...
const sourceFiles = [
  ...
  "scripts/preregister-assessment-production-post-canary-batch-15-final-ledger.mjs",
  "scripts/assemble-assessment-production-post-canary-batch-15-final-ledger.mjs",
  "scripts/validate-assessment-production-post-canary-batch-15-final-ledger.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-15-final-ledger.mjs",
  "scripts/test-assessment-production-post-canary-batch-15-final-ledger.mjs"
];
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-15-final-ledger-assembly-manifest",
  ...
  userAuthorization: {
    instruction:
      "The frozen Batch 15 standing authorization permits deterministic final-ledger assembly and the single repository score pass while every gate passes.",
    scopeReference:
      "docs/assessment-production/post-canary-continuation-v1/batch-15/standing-authorization.json",
  ...
  nextAuthorizedAction:
    "assemble-and-validate-ten-post-canary-batch-15-final-ledgers-deterministically"
};
if (shouldWrite) {
  await mkdir(path.resolve(POST_CANARY_BATCH_15_FINAL_LEDGER_ROOT), {
    recursive: true
  });
  await writeFile(
