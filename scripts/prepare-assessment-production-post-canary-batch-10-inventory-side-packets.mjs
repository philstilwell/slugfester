...
const BATCH_SOURCE_PREPARATION =
  "docs/assessment-production/post-canary-continuation-v1/batch-10/source-preparation/preparation-manifest.json";
...
const SCRIPT =
  "scripts/prepare-assessment-production-post-canary-batch-10-inventory-side-packets.mjs";
const PLAN_RUNNER =
...
function makeSidePacket({
  ...
  return {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-10-chronology-fallback-side-selector-packet",
...
async function buildArtifacts({ frozenAt, checkpointCommit }) {
  ...
  assertV4(
    sourcePreparation.schemaVersion ===
      ...
      sourcePreparation.totals.conCandidates === 170 &&
    ...
    "Batch 10 source preparation boundary drifted"
  );
  ...
  assertV4(
      ...
      planAnalysis.audit.recoveredDebate === "177" &&
    ...
    "accepted Batch 10 planner gate drifted"
  );
  assertV4(
    isDeepStrictEqual(
    ...
    "Batch 10 source compatibility boundary drifted"
  );
  ...
  const standingScope =
    "prepare, freeze, activate, and execute exactly twenty Batch 10 score-blind side-selector contexts under the frozen standing authorization";
  const sidePreparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-10-chronology-fallback-side-packet-preparation",
    protocolId: PROTOCOL_ID,
    sideSelectionProtocolId: CHRONOLOGY_FALLBACK_INVENTORY.protocolId,
    status:
      "twenty-exact-post-canary-batch-10-side-selector-packets-frozen-not-authorized",
    frozenAt,
    checkpointCommit,
    branch: "main",
    ...
    nextRequiredAction:
      "freeze-batch-10-side-selector-execution-preparation-manifest-model-free-only",
  };
  ...
  const executionPreparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-10-side-selector-execution-preparation-manifest",
    protocolId: PROTOCOL_ID,
    sideSelectionProtocolId: CHRONOLOGY_FALLBACK_INVENTORY.protocolId,
    status:
      "frozen-twenty-post-canary-batch-10-side-selector-contexts-prepared-not-authorized",
    frozenAt,
    checkpointCommit,
    branch: "main",
    productionContinuation: true,
    ...
    costEstimate: {
      ...
      estimateBasis:
        "The promoted twenty-context side-selector gate completed in 10.80 wall minutes and 19.74 aggregate model minutes at concurrency two. Batch 10 preserves the same one-side isolated context shape; this manifest records the exact observed packet sizes against the proven 115,000-byte input ceiling. ChatGPT-subscription execution has no direct incremental API charge.",
    },
    ...
    nextAuthorizedAction:
      "freeze-and-activate-batch-10-side-selector-execution-under-standing-authorization",
  };
...
if (command === "prepare") await prepare();
  ...
  throw new Error(
    "usage: prepare-assessment-production-post-canary-batch-10-inventory-side-packets.mjs <prepare|validate>"
  );
}
