...
const scoreArtifactCandidates = [
  `${POST_CANARY_BATCH_12_FINAL_LEDGER_ROOT}/calculated-scores.json`,
  `${POST_CANARY_BATCH_12_FINAL_LEDGER_ROOT}/score-analysis.json`,
  `${POST_CANARY_BATCH_12_FINAL_LEDGER_ROOT}/score-pass-manifest.json`
];
...
const rebuiltLedger = buildPostCanaryBatch12FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
...
assert.deepEqual(
  ...
  ["152", "28", "119", "116", "131", "07", "33", "15", "29", "11"]
);
assert.equal(storedLedger.audit.finalMoves, 204);
assert.equal(storedLedger.audit.disputedMoves, 196);
assert.equal(storedLedger.audit.pairSelections, 199);
assert.equal(storedLedger.audit.scoringFieldSelections, 311);
assert.equal(storedLedger.audit.burdenAdjustmentSelections, 18);
assert.equal(storedLedger.audit.candidateSelections, 528);
assert.equal(storedLedger.audit.meanMerges, 442);
...
assert.throws(() =>
  buildPostCanaryBatch12FinalLedger(mutatedInputs, inputs.sourceHashes)
);
...
assert.equal(analysis.totals.finalMoves, 204);
assert.equal(analysis.totals.disputedMoves, 196);
assert.equal(analysis.totals.candidateSelections, 528);
