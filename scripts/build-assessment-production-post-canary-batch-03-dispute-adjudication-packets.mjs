...
import {
  POST_CANARY_BATCH_03_DISPUTE_ADJ_PROTOCOL_ID,
  POST_CANARY_BATCH_03_DISPUTE_ADJ_ROOT,
  buildPostCanaryBatch03DisputeAdjudicationPacket,
  makePostCanaryBatch03DisputeAdjudicationSchema
} from "./lib/assessment-production-post-canary-batch-03-dispute-adjudication.mjs";
...
const USER_AUTHORIZATION = Object.freeze({
  instruction:
    "The frozen Batch 3 standing authorization permits automatic preparation, validation, freezing, activation, execution, analysis, committing, and pushing of score-blind dispute-only adjudication checkpoints for exactly the ten selected debates while every frozen gate passes, with subscription-backed and local direct incremental cost capped at $0.",
...
const inputPaths = {
  ...
  schema: `${POST_CANARY_BATCH_03_DISPUTE_ADJ_ROOT}/adjudication.schema.json`
};
const TOOL_SOURCES = [
  ...
  "scripts/build-assessment-production-post-canary-batch-03-dispute-adjudication-packets.mjs",
  "scripts/test-assessment-production-post-canary-batch-03-dispute-adjudication-packets.mjs",
  "scripts/validate-assessment-production-post-canary-batch-03-dispute-adjudication-output.mjs",
  "scripts/preregister-assessment-production-post-canary-batch-03-dispute-adjudication.mjs",
  "scripts/test-assessment-production-post-canary-batch-03-dispute-adjudication-manifest.mjs"
];
...
assertV4(
  sourceAnalysis.status ===
      "post-canary-batch-03-deterministic-disagreements-extracted-standing-authorization-active-for-audio-work" &&
    JSON.stringify(sourceAnalysis.debates.map((item) => item.debateNumber)) ===
      JSON.stringify(EXPECTED_DEBATES) &&
    sourceAnalysis.adjudicationWorkload.disputedMoves === 190 &&
    sourceAnalysis.adjudicationWorkload.candidateSelections === 586 &&
    sourceAnalysis.adjudicationWorkload.packetsPrepared === false &&
    sourceAnalysis.adjudicationWorkload.modelContextsExecuted === 0,
  "Batch 3 deterministic disagreement workload changed"
);
assertV4(
  judgmentExecution.status ===
      "twenty-post-canary-batch-03-independent-judgment-contexts-passed" &&
    ...
    judgmentAnalysis.status ===
      "twenty-post-canary-batch-03-independent-judgments-passed-standing-authorization-active-for-disagreement-extraction" &&
    judgmentAnalysis.totals.contexts === 20,
  "Batch 3 accepted independent-judgment boundary changed"
);
assertV4(
  ...
  "Batch 3 independent-judgment configuration changed"
);
...
assertV4(
  ...
  "Batch 3 standing authorization changed"
);
assertV4(
  audioAudit.status ===
  ...
  "Batch 3 accepted audio-verification boundary changed"
);
assertV4(
  ...
  "Batch 3 packet-preparation authorization changed"
);
...
assertV4(audioByDebateMove.size === 8, "Batch 3 verified audio population changed");

const schema = makePostCanaryBatch03DisputeAdjudicationSchema();
...
if (shouldWrite) {
  const immutableOutputs = [
    inputPaths.schema,
    `${POST_CANARY_BATCH_03_DISPUTE_ADJ_ROOT}/preparation-manifest.json`,
    `${POST_CANARY_BATCH_03_DISPUTE_ADJ_ROOT}/execution-preparation-manifest.json`,
    ...EXPECTED_DEBATES.flatMap((debateNumber) => [
      `${POST_CANARY_BATCH_03_DISPUTE_ADJ_ROOT}/packets/debate-${debateNumber}.json`,
      `${POST_CANARY_BATCH_03_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`
    ])
  ];
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const built = buildPostCanaryBatch03DisputeAdjudicationPacket(
  ...
  const provenancePath =
    `${POST_CANARY_BATCH_03_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`;
  const outputPath =
    `${POST_CANARY_BATCH_03_DISPUTE_ADJ_ROOT}/outputs/debate-${debateNumber}.json`;
  const packetBytes = pretty(built.packet);
  const provenanceBytes = pretty({
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-03-adjudication-candidate-provenance",
...
const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-dispute-only-adjudication-preparation",
  protocolId: POST_CANARY_BATCH_03_DISPUTE_ADJ_PROTOCOL_ID,
  status: shouldWrite
    ? "prepared-ten-isolated-post-canary-batch-03-dispute-only-adjudication-contexts"
    : "preview",
  frozenAt,
  ...
  acceptedSourceBoundary: {
    ...
    audioUsageDerivedEstimatedCostUsd:
      audioAudit.totals.preservedUsageDerivedEstimatedCostUsd,
  ...
  nextAuthorizedAction:
    "freeze-post-canary-batch-03-dispute-only-adjudication-execution-preparation-manifest-model-free-only"
};
assertV4(
  contexts.length === 10 &&
    disputedMoves === 190 &&
  ...
  "Batch 3 adjudication packet workload differs from frozen inputs"
);
assertV4(
  preparation.totals.maximumCopiedInputBytes <= 360000,
  `Batch 3 context ${preparation.totals.maximumCopiedInputBytes} bytes exceeds the 360 KB ceiling`
);
if (shouldWrite) {
  await mkdir(POST_CANARY_BATCH_03_DISPUTE_ADJ_ROOT, { recursive: true });
