...
import {
  POST_CANARY_BATCH_07_PUBLICATION_COMPILATION_ORDER,
  POST_CANARY_BATCH_07_PUBLICATION_COMPILATION_PROTOCOL_ID,
  POST_CANARY_BATCH_07_PUBLICATION_COMPILATION_ROOT,
  compilePostCanaryBatch07PublicationStagingRecord,
  validatePostCanaryBatch07CompiledStagingRecord
} from "./lib/assessment-production-post-canary-batch-07-publication-compilation.mjs";
...
const ROOT = POST_CANARY_BATCH_07_PUBLICATION_COMPILATION_ROOT;
const COHORT = `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/resumption-4/analysis.json`;
...
const ACCEPTED_OUTPUTS = Object.freeze({
  "193": `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/repair-1/merged/debate-193.json`,
  "80": `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/resumption-1/repair-1/merged/debate-80.json`,
  "121": `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/resumption-2/outputs/debate-121.json`,
  "100": `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/resumption-2/repair-1/merged/debate-100.json`,
  "78": `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/resumption-2/repair-1/merged/debate-78.json`,
  "113": `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/resumption-3/outputs/debate-113.json`,
  "180": `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/resumption-3/outputs/debate-180.json`,
  "02": `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/resumption-3/repair-1/merged/debate-02.json`,
  "182": `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/resumption-4/outputs/debate-182.json`,
  "56": `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/resumption-4/outputs/debate-56.json`
});
const acceptedOutput = (debateNumber) => ACCEPTED_OUTPUTS[debateNumber];
const packetPath = (debateNumber) =>
  `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/packets/debate-${debateNumber}.json`;
...
assertV4(cohort.status === "batch-07-complete-ten-debate-publication-reconstruction-cohort-passed" &&
  cohort.cohortValidation?.status === "passed" && cohort.cohortValidation?.debates === 10 &&
  cohort.cohortValidation?.moves === 187 && cohort.cohortValidation?.critiques === 187 &&
  cohort.cohortValidation?.exactSourceQuotes === 20,
"the accepted Batch 7 publication cohort changed");

const productionByNumber = new Map(debates.map((debate) => [debate.number, debate]));
const identities = [];
const contexts = [];
for (const debateNumber of POST_CANARY_BATCH_07_PUBLICATION_COMPILATION_ORDER) {
  ...
  const validation = validatePostCanaryBatch07PublicationOutput(output, packet);
  ...
  const compiled = compilePostCanaryBatch07PublicationStagingRecord({ output, packet, identity });
  const compiledValidation = validatePostCanaryBatch07CompiledStagingRecord({ compiled, output, packet, identity });
...
assertV4(contexts.length === 10 && contexts.reduce((sum, row) => sum + row.expectedMoves, 0) === 187,
"the Batch 7 compilation cohort changed");
const identitySnapshot = { schemaVersion: "1.0-assessment-production-post-canary-batch-07-publication-compilation-identity-snapshot",
  protocolId: POST_CANARY_BATCH_07_PUBLICATION_COMPILATION_PROTOCOL_ID,
...
const sources = [COHORT, POST_CANARY_BATCH_07_STANDING_AUTHORIZATION,
  ...
  "scripts/lib/assessment-production-post-canary-batch-07-publication-compilation.mjs",
  "scripts/lib/assessment-production-post-canary-batch-07-standing-authorization.mjs",
  "scripts/lib/v4-lean-production.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-07-publication-compilation.mjs",
  "scripts/test-assessment-production-post-canary-batch-07-publication-compilation-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-07-publication-compilation.mjs",
  "scripts/run-assessment-production-post-canary-batch-07-publication-compilation.mjs",
...
const manifest = { schemaVersion: "1.0-assessment-production-post-canary-batch-07-deterministic-publication-compilation-preparation",
  protocolId: POST_CANARY_BATCH_07_PUBLICATION_COMPILATION_PROTOCOL_ID,
  status: "frozen-post-canary-batch-07-deterministic-publication-compilation-prepared",
  ...
  explicitOrder: POST_CANARY_BATCH_07_PUBLICATION_COMPILATION_ORDER, contexts,
  ...
  nextAuthorizedAction: "activate-one-batch-07-deterministic-publication-compilation-pass" };
...
console.log(JSON.stringify({ status: manifest.status, debates: 10, moves: 187,
