...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-10/discovery";
...
assert.equal(manifest.status, "frozen-thirty-six-post-canary-batch-10-discovery-contexts-prepared-not-authorized");
...
assert.equal(manifest.executionPolicy.copiedInputBytesMaximum, 82238);
assert.equal(preparation.totals.maximumCopiedInputBytes, 82238);
...
assert.equal(manifest.copiedInputBoundary.frozenObservedCopiedInputBytesMaximum, 82238);
assert.equal(manifest.copiedInputBoundary.executionCopiedInputBytesMaximum, 82238);
assert.deepEqual(manifest.copiedInputBoundary.maximumContext, { debateNumber: "130", chunkId: "chunk-001" });
...
for (const context of manifest.contexts) {
  ...
  assert(buildBatch10TokenCountedChunkLedger(chunkBytes).equals(tokenBytes));
...
assert.equal(manifest.nextAuthorizedAction, "prepare-and-freeze-batch-10-discovery-execution-activation-under-standing-authorization");

const validation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-discovery-execution-preparation-validation",
  protocolId: manifest.protocolId,
  status: "batch-10-discovery-execution-manifest-validation-passed-frozen-standing-authorization-active",
