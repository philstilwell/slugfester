...
assertV4(
  preparation.protocolId === CHECKPOINT_V22_RESUMPTION_PROTOCOL_ID &&
    preparation.status === "nine-untouched-production-checkpoint-v2.2-publication-contexts-prepared-and-frozen" &&
    preparation.contexts.length === 9 &&
    preparation.totals.moves === 169 &&
    preparation.policy.attemptsPerContext === 1 &&
    preparation.policy.retriesMaximum === 0 &&
    preparation.totals.modelAuthoredScores === 0,
  "resumption preparation mismatch"
);
...
const sourceFiles = [
  ...
  preparation.acceptedDebate50.packet,
  ...
  "scripts/prepare-assessment-production-checkpoint-v2.2-publication-resumption.mjs",
  "scripts/preregister-assessment-production-checkpoint-v2.2-publication-resumption.mjs",
  "scripts/run-assessment-production-checkpoint-v2.2-publication-resumption.mjs",
  "scripts/analyze-assessment-production-checkpoint-v2.2-publication-resumption.mjs",
  "scripts/test-assessment-production-checkpoint-v2.2-publication-resumption.mjs"
];
...
const execution = `${CHECKPOINT_V22_RESUMPTION_ROOT}/model-execution.json`;
const analysis = `${CHECKPOINT_V22_RESUMPTION_ROOT}/analysis.json`;
const futureOutputPathsExcludedFromSourceHashes = [
  ...preparation.contexts.flatMap((context) => [context.rawOutput, context.validation, context.provenance]),
  execution,
  analysis
];
...
const activation = {
  schemaVersion: "1.0-production-checkpoint-v2.2-publication-resumption-execution-activation",
  protocolId: preparation.protocolId,
  status: "frozen-nine-untouched-production-checkpoint-v2.2-publication-contexts-authorized",
  frozenAt,
  ...
  acceptedDebate50: preparation.acceptedDebate50,
  contexts: preparation.contexts,
  executionPolicy: preparation.policy,
  acceptanceContract: preparation.acceptanceContract,
  isolation: {
    oneDebatePerContext: true,
    freshContextPerDebate: true,
  ...
  costEstimate: {
    ...
    expectedWallMinutes: [20, 45],
    maximumWallMinutes: 120
  },
  authorization: {
    modelContexts: true,
...
console.log(JSON.stringify({
  ...
  contexts: 9,
  debates: activation.contexts.map(({ debateNumber }) => debateNumber),
