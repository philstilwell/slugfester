...
assert.deepEqual(preparation.scope.debates, ["108", "144", "76", "92"]);
assert.equal(preparation.scope.frozenTargetClips, 10);
assert.equal(preparation.scope.verificationCalls, 10);
assert.equal(preparation.scope.sameDebateSpeakerSupportReferences, 8);
assert.deepEqual(preparation.scope.sourceCompatibility.occurrences, []);
assert.equal(preparation.referenceContract.references.length, 8);
...
assert.equal(preparation.costEstimate.clipSeconds, 1308.743);
assert.equal(preparation.costEstimate.clipMinutes, 21.8124);
assert.equal(preparation.costEstimate.durationOnlyPlanningEstimateUsd, 0.1308743);
assert.equal(preparation.costEstimate.durationOnlyEstimateAuthorizedByUser, true);
assert.equal(preparation.costEstimate.usageDerivedPlanningEstimateUsd, 0.476735);
assert.equal(preparation.costEstimate.primaryExpectedFutureExecutionCostUsd, 0.476735);
...
assert.deepEqual(preparation.costEstimate.projectedUsage, {
  ...
  textInputTokens: 1140,
  outputTokens: 41292,
  totalInputTokens: 25526
});
...
assert.equal(preparation.executionPolicy.callsMaximum, 10);
...
assert.equal(
  ...
  "activate-and-execute-ten-batch-16-paid-audio-verification-calls-under-standing-authorization"
);
