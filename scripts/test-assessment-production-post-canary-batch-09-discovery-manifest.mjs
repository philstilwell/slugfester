...
assert.equal(manifest.executionPolicy.copiedInputBytesMaximum, 65975);
assert.equal(preparation.totals.maximumCopiedInputBytes, 65975);
...
assert.equal(manifest.copiedInputBoundary.frozenObservedCopiedInputBytesMaximum, 65975);
assert.equal(manifest.copiedInputBoundary.executionCopiedInputBytesMaximum, 65975);
assert.deepEqual(manifest.copiedInputBoundary.maximumContext, { debateNumber: "176", chunkId: "chunk-002" });
...
for (const context of manifest.contexts) {
  ...
  assert(buildBatch09TokenCountedChunkLedger(chunkBytes).equals(tokenBytes));
...
assert.equal(manifest.nextAuthorizedAction, "prepare-and-freeze-batch-09-discovery-execution-activation-under-standing-authorization");

const validation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-discovery-execution-preparation-validation",
  protocolId: manifest.protocolId,
  status: "batch-09-discovery-execution-manifest-validation-passed-frozen-standing-authorization-active",
