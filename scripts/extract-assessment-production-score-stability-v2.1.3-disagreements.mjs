...
assertV4(
  judgmentAnalysis.status ===
    "twenty-v2.1.3-independent-judgments-passed-disagreement-extraction-authorized" &&
    judgmentAnalysis.authorization.disagreementExtraction &&
    judgmentAnalysis.nextAuthorizedAction ===
      "extract-v2.1.3-independent-judgment-disagreements-deterministically-only",
  "v2.1.3 disagreement extraction is not authorized"
);
assertV4(
  JSON.stringify(judgmentAnalysis.pairs.map((pair) => pair.debateNumber)) ===
    JSON.stringify(EXPECTED_DEBATES),
  "v2.1.3 independent-judgment population changed"
);
assertV4(
  executionPreparation.contexts.length === 20 &&
    executionActivation.executionPolicy.contexts === 20,
  "v2.1.3 twenty-context execution contract changed"
);
assertV4(
  ...
  "v2.1.3 model, authentication, or score-blindness contract changed"
);
assertV4(
  executionActivation.executionPolicy.retriesMaximum === 0 &&
    executionActivation.executionPolicy.timeoutExtensionsMaximum === 0,
  "v2.1.3 retry or timeout-extension stop rule changed"
);
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const disagreementPath =
    `${ASSESSMENT_PRODUCTION_SCORE_STABILITY_V213_DISAGREEMENT_ROOT}/disagreements/debate-${debateNumber}.json`;
  if (shouldWrite) {
...
assertV4(totalMoves === 189, "v2.1.3 extraction must compare 189 moves");
assertV4(
...
const analysis = {
  schemaVersion: "1.0-score-stability-v2.1.3-disagreement-extraction",
  protocolId:
    ASSESSMENT_PRODUCTION_SCORE_STABILITY_V213_DISAGREEMENT_PROTOCOL_ID,
  status:
...
if (shouldWrite) {
  await mkdir(ASSESSMENT_PRODUCTION_SCORE_STABILITY_V213_DISAGREEMENT_ROOT, {
    recursive: true
  });
  await writeFile(
    `${ASSESSMENT_PRODUCTION_SCORE_STABILITY_V213_DISAGREEMENT_ROOT}/analysis.json`,
