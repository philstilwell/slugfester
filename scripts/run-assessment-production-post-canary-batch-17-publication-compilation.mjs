...
assertV4(a.status === "frozen-post-canary-batch-17-deterministic-publication-compilation-authorized" &&
  ...
  canonicalJson(a.explicitOrder) === canonicalJson(POST_CANARY_BATCH_17_PUBLICATION_COMPILATION_ORDER),
"Batch 17 compilation is not authorized");
...
try {
  ...
  for (const debateNumber of POST_CANARY_BATCH_17_PUBLICATION_COMPILATION_ORDER) {
    ...
    const compiled = compilePostCanaryBatch17PublicationStagingRecord({ output, packet, identity: id });
    const validation = validatePostCanaryBatch17CompiledStagingRecord({ compiled, output, packet, identity: id });
...
const passed = rows.length === 4 && !failureMessage;
if (passed) await writeFile(path.resolve(a.artifacts.compilationAudit), `${JSON.stringify({
  schemaVersion: "1.0-assessment-production-post-canary-batch-17-publication-compilation-audit",
  protocolId: a.protocolId, status: "passed", explicitOrder: a.explicitOrder, rows,
  totals: { debates: 4, sections: rows.reduce((sum, row) => sum + row.validation.sections, 0),
    moves: 79, deterministicCompilationPasses: 1, reruns: 0, scorePasses: 0,
    modelContexts: 0, modelAuthoredScores: 0, directIncrementalCostUsd: 0 },
  productionMutationPerformed: false }, null, 2)}\n`);
const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-17-publication-compilation-execution",
  protocolId: a.protocolId, status: passed ? "four-debate-batch-17-deterministic-publication-compilation-passed" :
    "four-debate-batch-17-deterministic-publication-compilation-failed",
...
const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-17-publication-compilation-analysis",
  protocolId: a.protocolId, status: execution.status, productionCanary: false, batchNumber: 17,
  gate: { compiledRecordsPassed: rows.length, expectedCompiledRecords: 4,
  ...
  nextAuthorizedAction: passed ? "prepare-batch-17-publication-finalization" : "diagnose-compilation-failure" };
