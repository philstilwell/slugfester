...
const passed =
  ...
  validation.finalMoves === 177 &&
  validation.disputedMoves === 169 &&
  validation.candidateSelections === 461 &&
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-01-final-ledger-analysis",
  protocolId: ledger.protocolId,
  status: passed
    ? "post-canary-batch-01-deterministic-final-ledger-gate-passed"
    : "post-canary-batch-01-deterministic-final-ledger-gate-failed",
  analyzedAt,
  ...
  integrity: {
    ...
    correctedDebate195AdjudicationReplayedExactly: true,
  ...
  nextAuthorizedAction: passed
    ...
    : "user-approval-required-before-batch-01-final-ledger-failure-diagnosis-only"
};
if (shouldWrite) {
  await writeFile(
