...
import { V3811_PERFORMANCE_DEBATES, V3811_PERFORMANCE_PASSES, V3811_PERFORMANCE_ROOT, assertV3811, containsProhibitedDerivedField, readJson } from "./lib/v3811-performance-judgment.mjs";
...
const manifestPath = `${V3811_PERFORMANCE_ROOT}/initial-execution-manifest.json`;
if (shouldWrite) {
  try {
...
const preparation = await readJson(`${V3811_PERFORMANCE_ROOT}/preparation-manifest.json`);
const fixture = await readJson(`${V3811_PERFORMANCE_ROOT}/dry-fixture.json`);
const preflight = await readJson(`${V3811_PERFORMANCE_ROOT}/schema-preflight/model-execution.json`);
assertV3811(preparation.status === "prepared-score-blind-no-model-execution" && preparation.totals.moves === 81 && preparation.totals.pendingAudioVerifications === 0, "performance preparation invalid");
assertV3811(fixture.status === "passed" && fixture.totals.contexts === 6 && fixture.totals.judgments === 162 && fixture.totals.calculatedTotals === 0, "performance dry fixture invalid");
assertV3811(preflight.status === "endpoint-preflight-passed" && preflight.validSyntheticContexts === 1 && preflight.syntheticMoves === 7 && preflight.responseClassesExercised === 7 && preflight.debateJudgments === 0 && preflight.retries === 0, "expanded exact-schema endpoint preflight did not pass cleanly");

const schemaPath = `${V3811_PERFORMANCE_ROOT}/performance-judgment-schema.json`;
const contexts = [];
for (const pass of V3811_PERFORMANCE_PASSES) for (const debateNumber of V3811_PERFORMANCE_DEBATES) {
  const packetPath = `${V3811_PERFORMANCE_ROOT}/packets/debate-${debateNumber}.json`;
  const packet = await readJson(packetPath);
  assertV3811(packet.moves.length > 0 && !containsProhibitedDerivedField(packet), `${pass}.${debateNumber}: packet contains prohibited derived field`);
  contexts.push({
    pass,
    debateNumber,
    ...
    output: `${V3811_PERFORMANCE_ROOT}/initial-outputs/debate-${debateNumber}-pass-${pass.toLowerCase()}.json`
  });
}
const sourceFiles = [
  ...
  `${V3811_PERFORMANCE_ROOT}/preparation-assessment.md`,
  `${V3811_PERFORMANCE_ROOT}/preparation-manifest.json`,
  `${V3811_PERFORMANCE_ROOT}/dry-fixture.json`,
  `${V3811_PERFORMANCE_ROOT}/schema-preflight/execution-manifest.json`,
  `${V3811_PERFORMANCE_ROOT}/schema-preflight/model-execution.json`,
  `${V3811_PERFORMANCE_ROOT}/schema-preflight/output.json`,
  schemaPath,
  "scripts/lib/reassessment-scoring.mjs",
  "scripts/lib/v3811-performance-judgment.mjs",
  "scripts/build-v3811-performance-judgment-packets.mjs",
  "scripts/test-v3811-performance-judgment-tooling.mjs",
  "scripts/validate-v3811-performance-judgment-preparation.mjs",
  "scripts/validate-v3811-performance-judgment-output.mjs",
  "scripts/preregister-v3811-performance-judgment-execution.mjs",
  "scripts/validate-v3811-performance-judgment-execution-lock.mjs",
  "scripts/run-v3811-performance-judgment-initial.mjs",
...
const executionOutput = `${V3811_PERFORMANCE_ROOT}/initial-model-execution.json`;
const manifest = {
  schemaVersion: "3.8.11-performance-judgment-initial-execution-manifest",
