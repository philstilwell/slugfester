...
import {
  POST_CANARY_BATCH_15_PUBLICATION_COMPILATION_ORDER,
  POST_CANARY_BATCH_15_PUBLICATION_COMPILATION_PROTOCOL_ID,
  POST_CANARY_BATCH_15_PUBLICATION_COMPILATION_ROOT,
  compilePostCanaryBatch15PublicationStagingRecord,
  validatePostCanaryBatch15CompiledStagingRecord
} from "./lib/assessment-production-post-canary-batch-15-publication-compilation.mjs";
import { POST_CANARY_BATCH_15_PUBLICATION_ROOT } from
  "./lib/assessment-production-post-canary-batch-15-publication.mjs";
import { validatePostCanaryBatch15PublicationOutput } from
  "./lib/assessment-production-post-canary-batch-15-publication-validation.mjs";
import { POST_CANARY_BATCH_15_STANDING_AUTHORIZATION,
  loadAndValidatePostCanaryBatch15StandingAuthorization } from
  "./lib/assessment-production-post-canary-batch-15-standing-authorization.mjs";
...
const ROOT = POST_CANARY_BATCH_15_PUBLICATION_COMPILATION_ROOT;
const COHORT = `${POST_CANARY_BATCH_15_PUBLICATION_ROOT}/failure-recovery/finalized/complete-publication-overlay.json`;
...
const ACCEPTED_OUTPUTS = Object.freeze(Object.fromEntries(
  POST_CANARY_BATCH_15_PUBLICATION_COMPILATION_ORDER.map((debateNumber) => [
    debateNumber,
    `${POST_CANARY_BATCH_15_PUBLICATION_ROOT}/outputs/debate-${debateNumber}.json`
  ])
));
const acceptedOutput = (debateNumber) => ACCEPTED_OUTPUTS[debateNumber];
const packetPath = (debateNumber) =>
  `${POST_CANARY_BATCH_15_PUBLICATION_ROOT}/packets/debate-${debateNumber}.json`;
...
assertV4(
  cohort.status ===
    "complete-ten-debate-batch-15-publication-cohort-valid-after-bounded-recovery" &&
  cohort.debates?.length === 10 &&
  cohort.debates.reduce((sum, row) => sum + row.validation.moves, 0) === 191 &&
  cohort.debates.reduce((sum, row) => sum + row.validation.critiques, 0) === 191 &&
...
"the accepted Batch 15 publication cohort changed");

const productionByNumber = new Map(debates.map((debate) => [debate.number, debate]));
const identities = [];
const contexts = [];
for (const debateNumber of POST_CANARY_BATCH_15_PUBLICATION_COMPILATION_ORDER) {
  ...
  const compiled = compilePostCanaryBatch15PublicationStagingRecord({ output, packet, identity });
  const compiledValidation = validatePostCanaryBatch15CompiledStagingRecord({ compiled, output, packet, identity });
...
assertV4(contexts.length === 10 && contexts.reduce((sum, row) => sum + row.expectedMoves, 0) === 191,
"the Batch 15 compilation cohort changed");
const identitySnapshot = { schemaVersion: "1.0-assessment-production-post-canary-batch-15-publication-compilation-identity-snapshot",
  protocolId: POST_CANARY_BATCH_15_PUBLICATION_COMPILATION_PROTOCOL_ID,
...
const sources = [COHORT, POST_CANARY_BATCH_15_STANDING_AUTHORIZATION,
  ...
  "scripts/lib/assessment-production-post-canary-batch-15-publication-compilation.mjs",
  "scripts/lib/assessment-production-post-canary-batch-15-standing-authorization.mjs",
  "scripts/lib/v4-lean-production.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-15-publication-compilation.mjs",
  "scripts/test-assessment-production-post-canary-batch-15-publication-compilation-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-15-publication-compilation.mjs",
  "scripts/run-assessment-production-post-canary-batch-15-publication-compilation.mjs",
...
const manifest = { schemaVersion: "1.0-assessment-production-post-canary-batch-15-deterministic-publication-compilation-preparation",
  protocolId: POST_CANARY_BATCH_15_PUBLICATION_COMPILATION_PROTOCOL_ID,
  status: "frozen-post-canary-batch-15-deterministic-publication-compilation-prepared",
  ...
  userAuthorization: { standingAuthorization: POST_CANARY_BATCH_15_STANDING_AUTHORIZATION,
  ...
  explicitOrder: POST_CANARY_BATCH_15_PUBLICATION_COMPILATION_ORDER, contexts,
  ...
  nextAuthorizedAction: "activate-one-batch-15-deterministic-publication-compilation-pass" };
...
console.log(JSON.stringify({ status: manifest.status, debates: 10, moves: 191,
