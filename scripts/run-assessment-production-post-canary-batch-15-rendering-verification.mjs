...
"valid Batch 15 rendering activation required");
...
for (const debateNumber of POST_CANARY_BATCH_15_RENDERING_ORDER) {
  ...
  packetRows.push({ row, packet: validatePostCanaryBatch15RenderingPacket(
...
assertV4(plans.length === 20, "twenty Batch 15 rendering viewport plans required");
...
try {
  ...
  throw new Error(`Playwright CLI Batch 15 rendering execution failed: ${error.stdout || error.message}`);
}
...
if (browserResult.status !== "passed" || browserResult.results?.length !== 20) {
  await Promise.all(plans.flatMap((plan) => [plan.collapsedScreenshot, plan.openScreenshot])
    .map((file) => rm(file, { force: true })));
  throw new Error(`Batch 15 rendering stopped on its first failing viewport: ${JSON.stringify(browserResult).slice(-5000)}`);
}
...
try {
  for (const result of browserResult.results) {
    ...
    const evidence = {
      schemaVersion: "1.0-assessment-production-post-canary-batch-15-rendering-evidence",
    ...
    validatePostCanaryBatch15RenderingEvidence({ packet,
      viewportName: result.viewportName,
...
console.log(JSON.stringify({ status: "ten-debate-batch-15-rendering-browser-pass-complete",
  ...
  directIncrementalCostUsd: 0,
  nextAuthorizedAction: "validate-and-finalize-batch-15-rendering-verification" }, null, 2));
