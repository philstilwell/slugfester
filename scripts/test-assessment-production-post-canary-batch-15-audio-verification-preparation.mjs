...
assert.deepEqual(preparation.scope.debates, ["39", "159", "98"]);
assert.equal(preparation.scope.frozenTargetClips, 3);
assert.equal(preparation.scope.verificationCalls, 3);
...
assert.equal(preparation.costEstimate.clipSeconds, 270.76);
assert.equal(preparation.costEstimate.clipMinutes, 4.5127);
assert.equal(preparation.costEstimate.durationOnlyPlanningEstimateUsd, 0.027076);
assert.equal(preparation.costEstimate.durationOnlyEstimateAuthorizedByUser, true);
assert.equal(preparation.costEstimate.usageDerivedPlanningEstimateUsd, 0.0988975);
assert.equal(preparation.costEstimate.primaryExpectedFutureExecutionCostUsd, 0.0988975);
...
assert.deepEqual(preparation.costEstimate.projectedUsage, {
  audioInputTokens: 5045,
  textInputTokens: 342,
  outputTokens: 8543,
  totalInputTokens: 5387
});
...
assert.equal(preparation.executionPolicy.callsMaximum, 3);
...
assert.equal(
  ...
  "activate-and-execute-three-batch-15-paid-audio-verification-calls-under-standing-authorization"
);
