...
assert.equal(
  ...
  sidePreparation.sideSelectionProtocolId
);
assert.equal(
  manifest.status,
...
assert.deepEqual(manifest.costEstimate.expectedParallelWallMinutes, [10, 25]);
assert.deepEqual(manifest.costEstimate.expectedAggregateModelMinutes, [18, 40]);
...
for (const [index, context] of manifest.contexts.entries()) {
  ...
  assert.equal(packet.isolation.failedProductionCanaryOutputsUnavailable, true);
...
assert.deepEqual(manifest.executionPolicy, {
  ...
  observedMaximumCopiedInputBytes: 59832,
...
assert.equal(
  manifest.acceptancePolicy.preferredMoveKindRequiredInEveryNomination,
  true
);
assert.equal(
  manifest.acceptancePolicy.constructiveOrphanFallbackRequiredInEveryNomination,
  true
);
assert.equal(manifest.acceptancePolicy.fallbackConditionRepositoryOwned, true);
assert.equal(
  manifest.acceptancePolicy.fallbackAppliedOnlyToRetainedOrphanReply,
  true
);
assert.equal(
