...
assert.equal(selection.status, "first-post-canary-ten-debate-batch-selection-frozen-source-gate-passed");
assert.equal(analysis.status, "first-post-canary-batch-selection-analysis-passed-awaiting-source-packet-preparation-decision");
assert.equal(analysis.selection.sha256, sha256(selectionBytes));
assert.equal(selection.batchNumber, 1);
assert.equal(selection.productionSequenceOrdinal, 2);
...
assert.equal(selection.eligibility.censusSize, 164);
assert.equal(selection.eligibility.rankedCensusSize, 164);
assert.equal(selection.eligibility.selectedCount, 10);
assert.equal(selection.eligibility.remainingUnselectedCount, 154);
...
assert.deepEqual(selection.selected.map((item) => item.debateNumber), ranked.slice(0, 10).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.rankSha256), ranked.slice(0, 10).map((item) => item.rankSha256));
...
assert.equal(selection.deterministicOrdering.firstTenSelectedWithoutReplacement, true);
...
assert.equal(selection.nextAuthorizedAction, "user-decision-on-first-post-canary-batch-source-packet-preparation");

console.log(JSON.stringify({ status: "passed", selectedDebates: selection.selected.map((item) => item.debateNumber), sourceFiles: 30, replacements: 0, sourcePacketsPrepared: 0, modelContexts: 0, directCostUsd: 0, nextAuthorizedAction: selection.nextAuthorizedAction }, null, 2));
