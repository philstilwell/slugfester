...
import {
  POST_CANARY_BATCH_13_DISPUTE_ADJ_ISOLATION,
...
const EXPECTED_DEBATES = ["26", "190", "87", "20", "70", "30", "37", "117", "111", "34"];
...
const preparation = JSON.parse(
  await readFile(`${POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT}/preparation-manifest.json`, "utf8")
);
...
assert.equal(preparation.totals.disputedMoves, 191);
assert.equal(preparation.totals.candidateSelections, 541);
...
assert.equal(preparation.acceptedSourceBoundary.allEightAudioMovesVerified, true);
assert.equal(preparation.acceptedSourceBoundary.audioValidationOverlaysPreserved, 4);
assert.equal(preparation.acceptedSourceBoundary.audioAttributionRecoveryContextsPreserved, 2);
...
for (const context of preparation.contexts) {
  ...
  const output = {
    schemaVersion: POST_CANARY_BATCH_13_DISPUTE_ADJ_OUTPUT_VERSION,
    ...
    isolation: structuredClone(POST_CANARY_BATCH_13_DISPUTE_ADJ_ISOLATION),
  ...
  const validation = validatePostCanaryBatch13DisputeAdjudicationOutput(output, packet);
...
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT}/packets`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT}/provenance`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
