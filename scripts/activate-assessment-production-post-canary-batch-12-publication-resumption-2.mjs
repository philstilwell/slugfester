...
assertV4(!(await exists(ACTIVATION)), "eight-context resumption activation already exists");
const preparationBytes = await readFile(path.resolve(PREPARATION));
const preparation = JSON.parse(preparationBytes);
assertV4(preparation.protocolId === PROTOCOL_ID &&
  preparation.status === "frozen-eight-original-unattempted-batch-12-publication-contexts-prepared-not-activated" &&
  preparation.contexts?.length === 8 && preparation.totals?.moves === 157 &&
...
"eight original unattempted contexts are not prepared");
...
"eight-context model or execution controls changed");
...
const activation = {
  ...preparation,
  schemaVersion: "1.0-assessment-production-post-canary-batch-12-publication-eight-original-unattempted-context-resumption-activation",
  status: "frozen-eight-original-unattempted-batch-12-publication-contexts-activated",
  activatedAt,
  ...
  nextAuthorizedAction: "execute-only-eight-frozen-original-unattempted-publication-contexts"
};
