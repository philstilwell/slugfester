...
assert.equal(selection.status, "fourteenth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed");
assert.equal(analysis.status, "fourteenth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization");
assert.equal(analysis.selection.sha256, sha256(selectionBytes));
assert.equal(selection.batchNumber, 14);
assert.equal(selection.productionSequenceOrdinal, 15);
...
assert.equal(selection.eligibility.previouslyPublishedAndRemovedCount, 130);
assert.equal(selection.eligibility.remainingBeforeBatchCount, 34);
assert.equal(selection.eligibility.selectedCount, 10);
assert.equal(selection.eligibility.remainingUnselectedCount, 24);
...
assert.equal(priorCompletionExecution.status, "passed-complete-batch-13-production-transaction-after-dependent-pilot-analysis-correction");
...
assert.equal(priorCompletionExecution.acceptedTransaction.validatorByteIdentical, true);
assert.equal(priorCompletionExecution.acceptedTransaction.seoGeneratorByteIdentical, true);
assert.equal(priorCompletionAnalysis.status, "batch-13-production-publication-accepted-after-dependent-pilot-analysis-correction");
...
assert.equal(priorCompletionAnalysis.decision.productionTransactionPreserved, true);
...
assert.equal(sha256(serializedJson(ranked.slice(130))), selection.eligibility.remainingBeforeBatchRankedSha256);
assert.deepEqual(priorSelection.selected.map((item) => item.debateNumber), ranked.slice(120, 130).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.debateNumber), ranked.slice(130, 140).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.rankOrdinal), [131, 132, 133, 134, 135, 136, 137, 138, 139, 140]);
assert.deepEqual(selection.selected.map((item) => item.rankSha256), ranked.slice(130, 140).map((item) => item.rankSha256));
...
assert.deepEqual(selection.deterministicOrdering.selectedRankOrdinals, [131, 140]);
...
assert.equal(selection.nextAuthorizedAction, "prepare-batch-14-standing-authorization-and-source-packets-under-user-authorization");

console.log(JSON.stringify({ status: "passed", selectedDebates: selection.selected.map((item) => item.debateNumber), selectedRankOrdinals: [131, 140], sourceFiles: 30, replacements: 0, sourcePacketsPrepared: 0, modelContexts: 0, paidServiceCalls: 0, directCostUsd: 0, nextAuthorizedAction: selection.nextAuthorizedAction }, null, 2));
