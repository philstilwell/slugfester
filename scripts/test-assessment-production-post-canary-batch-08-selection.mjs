...
assert.equal(selection.status, "eighth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed");
assert.equal(analysis.status, "eighth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization");
assert.equal(analysis.selection.sha256, sha256(selectionBytes));
assert.equal(selection.batchNumber, 8);
assert.equal(selection.productionSequenceOrdinal, 9);
...
assert.equal(selection.eligibility.previouslyPublishedAndRemovedCount, 70);
assert.equal(selection.eligibility.remainingBeforeBatchCount, 94);
assert.equal(selection.eligibility.selectedCount, 10);
assert.equal(selection.eligibility.remainingUnselectedCount, 84);
...
assert.equal(priorCompletionExecution.status, "passed-complete-batch-07-transaction-ready-for-atomic-commit");
...
assert.equal(priorCompletionAnalysis.status, "batch-07-production-publication-accepted-for-atomic-commit-after-generated-seo-correction");
...
assert.equal(sha256(serializedJson(ranked.slice(70))), selection.eligibility.remainingBeforeBatchRankedSha256);
assert.deepEqual(priorSelection.selected.map((item) => item.debateNumber), ranked.slice(60, 70).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.debateNumber), ranked.slice(70, 80).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.rankOrdinal), [71, 72, 73, 74, 75, 76, 77, 78, 79, 80]);
assert.deepEqual(selection.selected.map((item) => item.rankSha256), ranked.slice(70, 80).map((item) => item.rankSha256));
...
assert.deepEqual(selection.deterministicOrdering.selectedRankOrdinals, [71, 80]);
...
assert.equal(selection.nextAuthorizedAction, "prepare-batch-08-standing-authorization-and-source-packets-under-user-authorization");

console.log(JSON.stringify({ status: "passed", selectedDebates: selection.selected.map((item) => item.debateNumber), selectedRankOrdinals: [71, 80], sourceFiles: 30, replacements: 0, sourcePacketsPrepared: 0, modelContexts: 0, paidServiceCalls: 0, directCostUsd: 0, nextAuthorizedAction: selection.nextAuthorizedAction }, null, 2));
