...
assertV4(activation.status ===
  "frozen-four-unattempted-batch-05-publication-resumption-2-contexts-authorized" &&
  activation.contexts?.length === 4 && activation.authorization?.publicationModelExecution === true &&
  activation.authorization?.deterministicCohortReplay === true &&
  activation.authorization?.retry === false && activation.authorization?.repairPacketPreparation === false &&
...
"resumption-2 execution is not authorized");
for (const [file, digest] of Object.entries(activation.sourceHashes)) assertV4(
  sha256(await readFile(path.resolve(file))) === digest, `resumption-2 source mismatch: ${file}`);
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-05-publication-resumption-2-${context.debateNumber}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `batch-05-publication-resumption-2-home-${context.debateNumber}-`));
  ...
  try {
    ...
    const prompt = ["Read production-workflow.md, readiness-workflow.md, output-contract.md, manual.md, reference-catalog.json, packet.json, and schema.json completely and no other files.",
      ...
      "Use no CJK, Hangul, Kana, replacement characters, or prohibited rational-invulnerability language. Return exactly one schema-conforming JSON object and nothing else."].join(" ");
    process.stdout.write(`[batch-05-publication-resumption-2] starting index ${context.contextIndex} Debate ${context.debateNumber} ${activation.model.label}/${activation.model.reasoningEffort}\n`);
    ...
    const base = { contextIndex: context.contextIndex,
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 ||
      ...
      const validation = { schemaVersion: "1.0-assessment-production-post-canary-batch-05-publication-resumption-2-validation",
      ...
      const provenance = { schemaVersion: "1.0-assessment-production-post-canary-batch-05-publication-resumption-2-provenance",
        ...
        correctionContextCount: 0, apiKeysRemoved: true,
    ...
    record = { contextIndex: context.contextIndex, originalContextIndex: context.originalContextIndex,
      debateNumber: context.debateNumber, debateId: context.debateId, attemptCount: 1,
      retryCount: 0, timeoutExtensionCount: 0, correctionContextCount: 0,
  ...
  } finally { activeContexts -= 1; await rm(temporary, { recursive: true, force: true });
    await rm(codexHome, { recursive: true, force: true }); }
  process.stdout.write(`[batch-05-publication-resumption-2] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
async function runPool(indexes, maximumConcurrency) { const queue = [...indexes]; const completed = [];
...
for (const phase of activation.executionPolicy.rampPhases) {
  ...
  results.push(...phaseResults); const valid = phaseResults.filter((r) => r.gateAcceptancePassed)
    .map((r) => r.contextIndex); const passed = valid.length === phase.contextIndexes.length;
  rampPhases.push({ ...phase, attemptedContextIndexes: phaseResults.map((r) => r.contextIndex),
    validContextIndexes: valid, passed, skippedBecausePriorRampFailed: false });
...
const unattemptedContextIndexes = activation.contexts.map((r) => r.contextIndex)
  .filter((index) => !results.some((r) => r.contextIndex === index));
const wallElapsedMs = Date.now() - gateStarted;
const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-05-publication-resumption-2-execution",
  protocolId: activation.protocolId, status: results.length === 4 && validContexts === 4
    ? "four-batch-05-publication-resumption-2-contexts-passed"
    : "batch-05-publication-resumption-2-complete-with-failure",
...
console.log(JSON.stringify({ status: execution.status, contextsAttempted: execution.contextsAttempted,
  ...
  correctionContexts: 0, meteredApiCostUsd: 0,
