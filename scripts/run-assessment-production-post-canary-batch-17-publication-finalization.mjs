...
const identity = JSON.parse(await readFile(path.resolve(
  "docs/assessment-production/post-canary-continuation-v1/batch-17/deterministic-publication-compilation/production-identity-snapshot.json"), "utf8"));
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const exists = (file) => access(path.resolve(file)).then(() => true, () => false);
assertV4(a.status === "frozen-post-canary-batch-17-publication-finalization-authorized" &&
  ...
  canonicalJson(a.explicitOrder) === canonicalJson(POST_CANARY_BATCH_17_PUBLICATION_FINALIZATION_ORDER),
"Batch 17 finalization is not authorized");
...
try {
  ...
  assertV4(candidates.length === 4 && candidates.reduce((sum, row) => sum + row.validation.moves, 0) === 79,
    "complete finalization validation failed");
} catch (error) { failureMessage = (error.stack ?? error.message).slice(-10000); }
const passed = candidates.length === 4 && !failureMessage;
const rows = [];
if (passed) {
  ...
  const preview = Buffer.from(buildPostCanaryBatch17PublicationStagingPreviewHtml());
  ...
  await writeFile(path.resolve(a.artifacts.audit), `${JSON.stringify({
    schemaVersion: "1.0-assessment-production-post-canary-batch-17-publication-finalization-audit",
    protocolId: a.protocolId, status: "passed", rows,
    totals: { debates: 4, moves: 79, deterministicFinalizationPasses: 1,
...
const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-17-publication-finalization-execution",
  protocolId: a.protocolId, status: passed ? "four-debate-batch-17-publication-finalization-passed" :
    "four-debate-batch-17-publication-finalization-failed", startedAt,
...
const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-17-publication-finalization-analysis",
  protocolId: a.protocolId, status: execution.status, productionCanary: false, batchNumber: 17,
  gate: { finalCandidatesPassed: rows.length, expectedFinalCandidates: 4,
  ...
  nextAuthorizedAction: passed ? "prepare-batch-17-rendering-verification" : "diagnose-finalization-failure" };
