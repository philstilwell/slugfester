...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-17/independent-judgments";
const PROTOCOL_ID =
  "assessment-production-post-canary-batch-17-independent-judgments";
const INVENTORY_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-17/inventory-candidate-sharded/inventory-analysis.json";
const INVENTORY_PREPARATION = "docs/assessment-production/post-canary-continuation-v1/batch-17/inventory-candidate-sharded/preparation-manifest.json";
const SOURCE_PREPARATION = "docs/assessment-production/post-canary-continuation-v1/batch-17/source-preparation/preparation-manifest.json";
const SOURCE_VALIDATION = "docs/assessment-production/post-canary-continuation-v1/batch-17/source-preparation/validation.json";
...
const SOURCE_FILES = [
  ...
  POST_CANARY_BATCH_17_STANDING_AUTHORIZATION,
  "scripts/prepare-assessment-production-post-canary-batch-17-independent-judgments.mjs",
  "scripts/test-assessment-production-post-canary-batch-17-independent-judgment-preparation.mjs"
];
...
assertV4(
  inventoryAnalysis.status ===
      "post-canary-batch-17-inventory-gate-passed-standing-authorization-active-for-independent-judgment-packet-preparation" &&
    ...
    inventoryAnalysis.totals?.debates === 4 &&
    inventoryAnalysis.totals?.moves === 79 &&
    ...
    inventoryAnalysis.nextAuthorizedAction ===
      "prepare-freeze-and-activate-batch-17-independent-judgment-contexts-under-standing-authorization",
  "the Batch 17 inventory gate is not ready for the separately authorized independent-judgment packet preparation"
);
assertV4(
  inventoryPreparation.status ===
    ...
    inventoryPreparation.contexts?.length === 4,
  "the Batch 17 inventory preparation is unavailable"
);
assertV4(
  ...
  "the Batch 17 source preparation is unavailable"
);
assertV4(
  sourceValidation.status ===
    ...
    sourceValidation.totals?.debates === 4 &&
    ...
    sourceValidation.checks?.exactSourceRowsInjectedOmittedOrRewritten ===
      false,
  "the Batch 17 source validation is unavailable"
);
assertV4(
...
assertV4(
  ...
  "the frozen Batch 17 cohort boundary changed"
);
assertV4(
  inventoryAnalysis.debates.map((debate) => debate.debateNumber).join(",") ===
    EXPECTED_DEBATES.join(","),
  "the passed Batch 17 inventory debate order changed"
);
...
assertV4(contexts.length === 8, "independent-judgment preparation must contain eight contexts");
assertV4(
  contexts.reduce((sum, context) => sum + context.moves, 0) === 158,
...
const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-17-independent-judgment-preparation",
  ...
  userAuthorization: {
    scope:
      "prepare, freeze, activate, and execute exactly eight Batch 17 score-blind independent-judgment contexts under the frozen standing authorization",
  ...
  nextAuthorizedAction:
    "prepare-post-canary-batch-17-independent-judgment-execution-preparation-manifest-model-free-only",
};
