...
const DISCOVERY_EXECUTION = `${BATCH_ROOT}/discovery/recovery-1/debate-77-chunk-002/cohort-execution-overlay.json`;
...
const SCRIPT =
  "scripts/prepare-assessment-production-post-canary-batch-17-inventory.mjs";
const PROTOCOL_ID =
  "assessment-production-post-canary-batch-17-candidate-sharded-inventory";
const SOURCE_PACKET_VERSION =
  "1.0-assessment-production-post-canary-batch-17-candidate-sharded-inventory-source-packet";
...
function buildInventorySourcePacket(sourcePacket) {
  assertV4(
    sourcePacket?.schemaVersion ===
      "1.0-assessment-production-post-canary-batch-17-score-blind-source-packet" &&
      sourcePacket.protocolId ===
        "assessment-production-post-canary-batch-17-source-preparation" &&
    ...
    "Batch 17 source packet is unavailable or not dyadic"
  );
  return {
...
function makePlanPacket({
  ...
  return {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-17-candidate-census-plan-packet",
    protocolId: PROTOCOL_ID,
...
async function buildFrozenArtifacts(frozenAt) {
  ...
  assertV4(
    ...
    "Batch 17 standing authorization does not cover exact inventory preparation and execution"
  );
  ...
  assertV4(
    discoveryAnalysis.status ===
      ...
      discoveryAnalysis.audit?.validContexts === 17 &&
      ...
      discoveryAnalysis.totals?.modelContextsExecuted === 18 &&
    ...
    "passed Batch 17 discovery boundary drifted"
  );
  assertV4(
    discoveryExecution.status ===
      "seventeen-post-canary-batch-17-discovery-contexts-passed" &&
      discoveryExecution.contextsAttempted === 17 &&
      discoveryExecution.validContexts === 17 &&
    ...
    "Batch 17 discovery execution drifted"
  );
  assertV4(
    ...
    "Batch 17 source preparation or model boundary drifted"
  );
  assertV4(
    sourcePreparationValidation.status ===
      ...
      sourcePreparationValidation.checks?.allTwelveOriginalSourceHashesReplayed ===
        true &&
    ...
    "Batch 17 source validation drifted"
  );
  assertV4(
    selection.status ===
    ...
    "Batch 17 frozen selection drifted"
  );
  assertV4(
  ...
  for (const discovered of discoveryAnalysis.debates) {
    ...
    assertV4(
      ...
      `${discovered.debateNumber}: Batch 17 identity drifted`
    );
    const [
  ...
  assertV4(
    ...
    "Batch 17 totals or packet state drifted"
  );
  ...
  const preparation = {
    ...
    userAuthorization: {
      scope:
        "prepare, freeze, activate, and execute the Batch 17 candidate-census and candidate-sharded side-selection workflow under the frozen standing authorization",
    ...
    stageDesign: {
      ...
      contextsPerDebate: 3,
      totalContextsPlanned: 12,
      exactPlannerPacketsFrozen: 4,
      completeSideTransportsFrozen: 8,
      maximumPlanChronologyFallbackSchemaPrototypesFrozen: 8,
    ...
    totals: {
      ...
      plannedModelContexts: 12,
      exactPlannerPacketsFrozen: 4,
      completeSideTransportsFrozen: 8,
      sideSchemaPrototypesFrozen: 8,
      exactSidePacketsFrozen: 0,
    ...
    nextAuthorizedAction:
      "freeze-and-activate-batch-17-candidate-census-plan-execution-under-standing-authorization",
  };
  ...
  const executionPreparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-17-candidate-census-plan-execution-preparation-manifest",
    protocolId: PROTOCOL_ID,
    status:
      "frozen-four-post-canary-batch-17-candidate-census-plan-contexts-prepared-not-authorized",
    frozenAt,
    ...
    costEstimate: {
      ...
      estimateBasis:
        `The promoted candidate-census planner used this contract at concurrency two. Batch 17 freezes four planner contexts with an observed maximum of ${maximumCopiedInputBytes.toLocaleString("en-US")} copied bytes, below the proven ${V212_CANDIDATE_SHARDED_INVENTORY.maximumCopiedInputBytes.toLocaleString("en-US")}-byte ceiling. ChatGPT-subscription execution has no direct incremental API charge.`,
    },
    ...
    executionPolicy: {
      ...
      allFourPlansMustPassBeforeSidePacketPreparation: true,
    ...
    stopRules: {
      ...
      sidePacketFreezeBeforeFourAcceptedPlansBlocks: true,
      retryBlocks: true,
      timeoutExtensionBlocks: true,
    },
    totals: {
  ...
  const validation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-17-candidate-sharded-inventory-preparation-validation",
    protocolId: PROTOCOL_ID,
    status:
      "batch-17-candidate-sharded-inventory-packets-and-plan-execution-manifest-validation-passed-frozen-standing-authorization-active",
    ...
    JSON.stringify(
      {
        ...
        exactPlannerPacketsFrozen: 4,
        completeSideTransportsFrozen: 8,
        sidePrototypeSchemasFrozenNonExecutable: 8,
        exactSidePacketsDeferred: 8,
        modelExecutionAuthorized: false,
        paidServicesAuthorized: false,
        directIncrementalCostUsd: 0,
        scoresDerived: 0,
...
async function validateFrozen() {
  ...
  assertV4(
    preparation.status ===
      ...
      preparation.totals.exactPlannerPacketsFrozen === 4 &&
      preparation.totals.completeSideTransportsFrozen === 8 &&
      preparation.totals.sideSchemaPrototypesFrozen === 8 &&
    ...
    "frozen Batch 17 inventory preparation is invalid"
  );
  ...
  assertV4(
    executionPreparation.status ===
      ...
      executionPreparation.contexts.length === 4 &&
      ...
      executionPreparation.executionPolicy.contexts === 4 &&
      ...
      executionPreparation.executionPolicy.directIncrementalCostUsdMaximum ===
        0 &&
      executionPreparation.totals.planContextsPrepared === 4 &&
...
if (command === "freeze") await freeze();
  ...
  throw new Error(
    "usage: prepare-assessment-production-post-canary-batch-17-inventory.mjs <freeze|validate>"
  );
}
