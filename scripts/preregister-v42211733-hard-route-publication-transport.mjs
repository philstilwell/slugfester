...
const manifestPath = `${V42211733_ROOT}/execution-manifest.json`;
const executionPath = `${V42211733_ROOT}/model-execution.json`;
const analysisPath = `${V42211733_ROOT}/analysis.json`;
...
const preparation = JSON.parse(await readFile(path.resolve(`${V42211733_ROOT}/preparation-manifest.json`), "utf8"));
assertV4(preparation.status === "prepared-five-isolated-hard-route-publication-transport-contexts" && preparation.authorization.executionManifest && preparation.totals.modelContextsExecuted === 0 && preparation.totals.modelAuthoredScores === 0, "publication transport execution manifest is not authorized");
assertV4(preparation.totals.maximumCopiedInputBytes <= 400000, "publication context exceeds frozen transport budget");
assertV4(preparation.repair.allSchemaPreflightsPassed && preparation.repair.repositoryUniquenessValidationRetained && !preparation.repair.priorGateTreatedAsRetry, "publication transport repair boundary mismatch");
...
const sourceFiles = [
  ...
  `${V42211733_ROOT}/preparation-manifest.json`,
  ...
  "scripts/prepare-v42211733-hard-route-publication-transport.mjs",
  "scripts/preregister-v42211733-hard-route-publication-transport.mjs",
  "scripts/run-v42211733-hard-route-publication-transport.mjs",
  "scripts/analyze-v42211733-hard-route-publication-transport.mjs",
  "scripts/test-v42211733-hard-route-publication-transport-gate.mjs"
];
...
const manifest = {
  schemaVersion: "4.2.21.17.33-hard-route-publication-transport-execution-manifest",
