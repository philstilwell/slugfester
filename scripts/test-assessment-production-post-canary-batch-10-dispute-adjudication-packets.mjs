...
import {
  POST_CANARY_BATCH_10_DISPUTE_ADJ_ISOLATION,
  POST_CANARY_BATCH_10_DISPUTE_ADJ_OUTPUT_VERSION,
...
const EXPECTED_DEBATES = ["21", "74", "107", "142", "123", "177", "68", "147", "61", "130"];
...
const preparation = JSON.parse(
  await readFile(`${POST_CANARY_BATCH_10_DISPUTE_ADJ_ROOT}/preparation-manifest.json`, "utf8")
);
...
assert.equal(preparation.batchNumber, 10);
...
assert.equal(preparation.totals.disputedMoves, 177);
assert.equal(preparation.totals.candidateSelections, 483);
assert.equal(preparation.totals.audioVerifiedMoves, 9);
...
assert.equal(preparation.acceptedSourceBoundary.allNineAudioMovesVerified, true);
assert.equal(preparation.acceptedSourceBoundary.audioValidationOverlaysPreserved, 0);
...
for (const context of preparation.contexts) {
  ...
  assert.equal(packet.batchNumber, 10);
  ...
  const output = {
    schemaVersion: POST_CANARY_BATCH_10_DISPUTE_ADJ_OUTPUT_VERSION,
    ...
    isolation: structuredClone(POST_CANARY_BATCH_10_DISPUTE_ADJ_ISOLATION),
  ...
  const validation = validatePostCanaryBatch10DisputeAdjudicationOutput(output, packet);
...
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_10_DISPUTE_ADJ_ROOT}/packets`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_10_DISPUTE_ADJ_ROOT}/provenance`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
