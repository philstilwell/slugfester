...
assert.equal(manifest.protocolId, POST_CANARY_BATCH_06_PUBLICATION_PROTOCOL_ID);
assert.equal(manifest.productionCanary, false);
assert.equal(manifest.batchNumber, 6);
...
assert.equal(manifest.totals.moves, 200);
assert.equal(manifest.totals.sections, 52);
assert.equal(manifest.totals.audioVerifiedMoves, 2);
...
for (const context of manifest.contexts) {
  ...
  assert.equal(packet.protocolId, POST_CANARY_BATCH_06_PUBLICATION_PROTOCOL_ID);
  ...
  assert.deepEqual(schema, buildPostCanaryBatch06PublicationSchema(packet));
  assert.equal(schema.properties.schemaVersion.const, POST_CANARY_BATCH_06_PUBLICATION_OUTPUT_VERSION);
  assert.equal(schema.properties.protocolId.const, POST_CANARY_BATCH_06_PUBLICATION_PROTOCOL_ID);
