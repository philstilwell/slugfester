...
import {
  POST_CANARY_BATCH_06_DISPUTE_ADJ_ISOLATION,
...
const EXPECTED_DEBATES = ["73", "36", "38", "97", "141", "06", "168", "135", "143", "169"];
...
const preparation = JSON.parse(
  await readFile(`${POST_CANARY_BATCH_06_DISPUTE_ADJ_ROOT}/preparation-manifest.json`, "utf8")
);
...
for (const context of preparation.contexts) {
  ...
  const output = {
    schemaVersion: POST_CANARY_BATCH_06_DISPUTE_ADJ_OUTPUT_VERSION,
    ...
    isolation: structuredClone(POST_CANARY_BATCH_06_DISPUTE_ADJ_ISOLATION),
  ...
  const validation = validatePostCanaryBatch06DisputeAdjudicationOutput(output, packet);
...
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_06_DISPUTE_ADJ_ROOT}/packets`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_06_DISPUTE_ADJ_ROOT}/provenance`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
