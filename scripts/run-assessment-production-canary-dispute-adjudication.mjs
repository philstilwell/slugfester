...
assertV4(
  ...
  "production-canary dispute adjudication execution unauthorized"
);
...
async function runContext(context, contextIndex) {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), `production-canary-adjudication-${context.debateNumber}-`)
  );
  const codexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `production-canary-adjudication-home-${context.debateNumber}-`
    )
  );
  ...
  try {
    ...
    const prompt = `Read rubric-base.md, rubric-derived.md, rubric-bounded.md, workflow.md, manual.md, packet.json, schema.json${
      audioFiles.length ? `, and ${audioFiles.join(", ")}` : ""
    }; read nothing else. Act only as the isolated disputed-fields-only production-canary adjudicator for Debate ${
    ...
    process.stdout.write(
      `[production-canary-adjudication] starting index ${contextIndex} ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
    ...
    if (
      ...
      try {
        validation = validateProductionCanaryDisputeAdjudicationOutput(
  ...
  process.stdout.write(
    `[production-canary-adjudication] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-production-canary-dispute-only-adjudication-model-execution",
  protocolId: manifest.protocolId,
  status: passed
    ? "ten-isolated-production-canary-dispute-only-adjudication-contexts-passed"
    : "production-canary-dispute-only-adjudication-gate-complete-with-failure",
