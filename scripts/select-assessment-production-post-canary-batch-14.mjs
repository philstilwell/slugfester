...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-14";
...
const PRIOR_SELECTION = "docs/assessment-production/post-canary-continuation-v1/batch-13/selection.json";
const PRIOR_SELECTION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-13/selection-analysis.json";
const PRIOR_MUTATION_MANIFEST = "docs/assessment-production/post-canary-continuation-v1/batch-13/production-publication/mutation-manifest.json";
const PRIOR_COMPLETION_EXECUTION = "docs/assessment-production/post-canary-continuation-v1/batch-13/production-publication/generated-seo-correction/dependent-pilot-analysis-correction/execution.json";
const PRIOR_COMPLETION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-13/production-publication/generated-seo-correction/dependent-pilot-analysis-correction/analysis.json";
const PRIOR_COMPLETION_COMMIT = "59dfc4f036ed22499783da758f23f4c66b298a0f";
const SCRIPT = "scripts/select-assessment-production-post-canary-batch-14.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-14-selection.mjs";
...
assertV4(!(await exists(FUTURE_SOURCE_PREPARATION)), "source preparation already exists before Batch 14 selection");
...
assertV4(
  ...
  "Batch 14 selection is not compatible with the frozen continuation policy"
);
...
assertV4(
  priorSelection.status === "thirteenth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    priorSelection.protocolId === "assessment-production-post-canary-batch-13" &&
    priorSelection.productionSequenceOrdinal === 14 &&
    priorSelection.selected.length === 10 &&
    priorSelectionAnalysis.status === "thirteenth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization" &&
    priorSelectionAnalysis.selection.sha256 === sha256(controlBytes[PRIOR_SELECTION]) &&
    priorMutationManifest.status === "frozen-batch-13-production-publication-mutation-manifest-prepared" &&
    JSON.stringify(priorMutationManifest.explicitOrder) === JSON.stringify(priorSelectedNumbers) &&
    priorCompletionExecution.status === "passed-complete-batch-13-production-transaction-after-dependent-pilot-analysis-correction" &&
    ...
    priorCompletionAnalysis.status === "batch-13-production-publication-accepted-after-dependent-pilot-analysis-correction" &&
  ...
  "Batch 13 has not satisfied the frozen successful-publication prerequisite"
);
assertV4(git("rev-parse", "--abbrev-ref", "HEAD") === "main", "Batch 14 selection must run on main");
assertV4(git("rev-parse", "HEAD") === PRIOR_COMPLETION_COMMIT, "Batch 14 selection checkpoint is not the exact Batch 13 publication commit");
assertV4(git("rev-parse", "origin/main") === PRIOR_COMPLETION_COMMIT, "Batch 13 publication commit is not the pushed origin/main checkpoint");
...
assertV4(
  JSON.stringify(priorSelectedNumbers) === JSON.stringify(ranked.slice(120, 130).map((item) => item.debateNumber)),
  "Batch 13 is not the immutable thirteenth rank slice"
);

const selected = [];
let totalEvents = 0;
let totalDurationMs = 0;
for (const [offset, rankedItem] of ranked.slice(130, 140).entries()) {
...
assertV4(selected.length === 10 && new Set(selected.map((item) => item.debateNumber)).size === 10, "Batch 14 did not select ten unique debates");

const checkpointCommit = git("rev-parse", "HEAD");
const selection = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-selection",
  protocolId: "assessment-production-post-canary-batch-14",
  status: "fourteenth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed",
  frozenAt,
  checkpointCommit,
  branch: "main",
  ...
  stagingOnly: true,
  batchNumber: 14,
  productionSequenceOrdinal: 15,
  userAuthorization: {
    instruction: "Continue Batch 14 at your discretion.",
    scopeInterpretation: "Materialize and source-validate only the deterministic Batch 14 selection under the frozen continuation policy after authenticating successful Batch 13 publication. This selection pass prepares no source packets, executes no models, derives no scores, and performs no production mutation.",
  ...
  eligibility: {
    ...
    previouslyPublishedAndRemovedCount: 130,
    remainingBeforeBatchCount: ranked.length - 130,
    remainingBeforeBatchRankedSha256: sha256(serializedJson(ranked.slice(130))),
    selectedCount: selected.length,
    remainingUnselectedCount: ranked.length - 140,
    dyadicOnly: true,
  ...
  nextAuthorizedAction: "prepare-batch-14-standing-authorization-and-source-packets-under-user-authorization"
};
...
const analysis = {
  ...
  status: "fourteenth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization",
  analyzedAt: frozenAt,
  ...
  finding: "Successful Batch 13 production publication was authenticated at the pushed atomic commit. The immutable campaign rank then selected ordinals 131 through 140 as ten unique pending dyadic debates, and all thirty effective source files and ten canonical event chains passed without replacement.",
  decision: {
