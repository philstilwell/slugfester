...
assertV4(
  activation.status === "frozen-eight-context-fifty-two-field-batch-10-publication-repair-activated" &&
    activation.authorization?.modelExecution === true && activation.contexts?.length === 8 &&
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-10-field-repair-${context.contextIndex}-`));
  const isolatedCodexHome = await mkdtemp(path.join(os.tmpdir(), `batch-10-field-repair-home-${context.contextIndex}-`));
  ...
  try {
    ...
    process.stdout.write(`[batch-10-field-repair] starting context ${context.contextIndex} ${context.shardId} (${context.writableFieldCount} fields)\n`);
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 ||
      ...
      const validation = {
        schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-field-disjoint-repair-validation",
      ...
      const provenance = {
        schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-field-disjoint-repair-provenance",
        protocolId: activation.protocolId,
        contextIndex: context.contextIndex,
        shardId: context.shardId,
        debateNumber: context.debateNumber,
  ...
  process.stdout.write(`[batch-10-field-repair] ${context.shardId} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
...
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-field-disjoint-repair-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "eight-field-disjoint-batch-10-publication-repair-contexts-passed"
    : "eight-field-disjoint-batch-10-publication-repair-contexts-completed-with-failure",
  gateStartedAt,
  ...
  results,
  fieldDisjointAcrossContexts: new Set(results.flatMap((result) => result.writableFields)).size === 52,
  hostAwakeGuardAppliedToEveryAttempt: results.every((result) => result.hostAwakeGuardApplied),
  meteredApiCostUsd: 0,
  paidServiceCallsThisStage: 0,
  modelAuthoredScores: 0,
  nextRequiredAction: passed
    ? "deterministically-merge-and-validate-five-debates-then-replay-complete-cohort"
