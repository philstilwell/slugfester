...
import {
  ...
  POST_CANARY_BATCH_07_PUBLICATION_REPAIR_OUTPUT_VERSION,
  POST_CANARY_BATCH_07_PUBLICATION_REPAIR_PACKET_VERSION,
...
const DIAGNOSIS = `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/failure-diagnosis.json`;
...
const STATIC = [DIAGNOSIS, PRODUCTION_WORKFLOW, READINESS_WORKFLOW, OUTPUT_CONTRACT,
  MANUAL, POST_CANARY_BATCH_07_STANDING_AUTHORIZATION,
  `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/execution-preparation-manifest.json`,
  `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/execution-activation.json`,
  `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/model-execution.json`,
  `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/analysis.json`,
  ...
  "scripts/lib/assessment-production-post-canary-batch-07-publication-repair.mjs",
  "scripts/lib/assessment-production-post-canary-batch-07-standing-authorization.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-07-publication-repair.mjs",
  "scripts/test-assessment-production-post-canary-batch-07-publication-repair-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-07-publication-repair.mjs",
  "scripts/run-assessment-production-post-canary-batch-07-publication-repair.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-07-publication-repair.mjs"];
for (const debateNumber of [...REJECTED, ...ACCEPTED]) {
  STATIC.push(`${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/outputs/debate-${debateNumber}.json`,
...
assertV4(
  diagnosis.status === "frozen-diagnosed-batch-07-debate-193-two-field-publication-validation-failure" &&
    diagnosis.failedFields?.length === 2 &&
    diagnosis.failedFields.filter((field) => field.type === "critique-word-boundary").length === 1 &&
    diagnosis.failedFields.filter((field) => field.type === "representative-quote-exact-source-substring").length === 1 &&
  ...
  "the frozen Batch 7 publication diagnosis or standing authorization changed"
);
...
const baseOutputs = Object.fromEntries(await Promise.all(REJECTED.map(async (debateNumber) =>
  [debateNumber, JSON.parse(await readFile(path.resolve(
    `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/outputs/debate-${debateNumber}.json`), "utf8"))])));
const publicationPackets = Object.fromEntries(await Promise.all(REJECTED.map(async (debateNumber) =>
  [debateNumber, JSON.parse(await readFile(path.resolve(
    `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/packets/debate-${debateNumber}.json`), "utf8"))])));
const diagnosedByPath = new Map(
  diagnosis.failedFields.map((field) => [`193:${field.path}`, field])
);
const plannedContexts = [{
  contextIndex: 0,
  packetId: "debate-193-repair-0",
  debateNumber: "193",
...
for (const planned of plannedContexts) {
  ...
  const basePath = `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/outputs/debate-${debateNumber}.json`;
  const publicationPacketPath = `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/packets/debate-${debateNumber}.json`;
  const packet = { schemaVersion: POST_CANARY_BATCH_07_PUBLICATION_REPAIR_PACKET_VERSION,
    protocolId: POST_CANARY_BATCH_07_PUBLICATION_REPAIR_PROTOCOL_ID,
    contextIndex, packetId, productionCanary: false, batchNumber: 7, stagingOnly: true,
    ...
    corrections };
  const schema = buildBatch07PublicationRepairSchema(packet);
  ...
  const synthetic = { schemaVersion: POST_CANARY_BATCH_07_PUBLICATION_REPAIR_OUTPUT_VERSION,
    protocolId: POST_CANARY_BATCH_07_PUBLICATION_REPAIR_PROTOCOL_ID,
  ...
  validateBatch07PublicationRepairOutput(synthetic, packet);
...
for (const debateNumber of REJECTED) {
  ...
  const merge = mergeAndValidateBatch07PublicationRepairs({ baseOutput: baseOutputs[debateNumber],
...
const manifest = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-07-publication-repair-preparation",
  protocolId: POST_CANARY_BATCH_07_PUBLICATION_REPAIR_PROTOCOL_ID,
  status: "frozen-one-context-batch-07-publication-repair-prepared-not-activated",
  ...
  userAuthorization: { source: POST_CANARY_BATCH_07_STANDING_AUTHORIZATION,
  ...
  diagnosis: { path: DIAGNOSIS, sha256: sha256(diagnosisBytes), failedFields: 2,
    failedCritiques: 1, failedQuotes: 1, rejectedDebates: REJECTED },
  model: POST_CANARY_BATCH_07_PUBLICATION_MODEL,
  contexts,
  ...
  nextAuthorizedAction: "activate-exactly-one-frozen-batch-07-two-field-publication-repair-context"
};
