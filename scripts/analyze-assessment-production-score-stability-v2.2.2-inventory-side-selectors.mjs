...
assertV4(
  execution.status === "twenty-v2.2.2-side-selector-contexts-passed" &&
...
for (const prepared of sourcePreparation.contexts) {
  ...
  const inventoryProposalOutput =
    `${ROOT}/inventory-proposals/debate-${prepared.debateNumber}.json`;
  const lockedInventoryOutput =
    `${ROOT}/locked-inventories/debate-${prepared.debateNumber}.json`;
  const validationOutput =
    `${ROOT}/validations/debate-${prepared.debateNumber}.json`;
  const provenanceOutput =
    `${ROOT}/provenance/debate-${prepared.debateNumber}.json`;
  ...
  const [
    ...
    readFile(prepared.composedPlanOutput),
    ...
    readFile(source.validatorCandidateEvidenceBundle, "utf8").then(JSON.parse),
    readFile(source.originalEvents, "utf8").then(JSON.parse),
  ]);
  ...
  for (const context of contexts) {
    ...
    const [selectionBytes, sideTransport] = await Promise.all([
      ...
      readFile(
        source.sideAssets.find((asset) => asset.side === context.side)
          .transport,
        "utf8"
      ).then(JSON.parse),
    ]);
  ...
  const summary = {
    schemaVersion:
      "1.0-score-stability-v2.2.2-chronology-fallback-inventory-validation",
  ...
  await Promise.all([
    writeFile(inventoryProposalOutput, proposalBytes),
    writeFile(lockedInventoryOutput, lockedBytes),
    writeFile(validationOutput, validationBytes),
    writeFile(provenanceOutput, provenanceBytes),
  ]);
  debates.push({
    debateNumber: prepared.debateNumber,
    debateId: prepared.debateId,
    plan: prepared.composedPlanOutput,
    ...
    sideSelectionSha256: sideSelectionHashes,
    inventoryProposal: inventoryProposalOutput,
    inventoryProposalSha256: sha256(proposalBytes),
    lockedInventory: lockedInventoryOutput,
...
const analysis = {
  schemaVersion:
    "1.0-score-stability-v2.2.2-chronology-fallback-inventory-analysis",
  protocolId: activation.protocolId,
  status:
  ...
  totals: {
    ...
    belowHighAttributionMoves: debates.reduce(
