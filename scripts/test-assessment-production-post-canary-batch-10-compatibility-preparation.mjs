...
const preparationPath =
  `${POST_CANARY_BATCH_10_COMPATIBILITY_ROOT}/preparation-manifest.json`;
const analysisPath =
  `${POST_CANARY_BATCH_10_COMPATIBILITY_ROOT}/analysis.json`;
...
assert.equal(
  ...
  "batch-10-site-ledger-adapter-and-validator-route"
);
assert.equal(
...
assert.equal(preparation.totals.moves, 182);
assert.equal(preparation.totals.oneSidedDisplayRows, 8);
...
assert.equal(
  await exists(`${POST_CANARY_BATCH_10_COMPATIBILITY_ROOT}/execution.json`),
  false
);
assert.equal(
  await exists(
...
assert.deepEqual(packetNumbers, POST_CANARY_BATCH_10_COMPATIBILITY_ORDER);
for (const packetRecord of preparation.artifacts.packets) {
  ...
  const positive = validatePostCanaryBatch10SiteLedgerAdapter({
