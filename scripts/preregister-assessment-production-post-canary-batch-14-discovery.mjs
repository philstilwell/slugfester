...
const BATCH_ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-14";
...
const PROTOCOL_ID = "assessment-production-post-canary-batch-14-discovery";
const WORKFLOW = "docs/assessment-production-canary-discovery-workflow.md";
const SCRIPT = "scripts/preregister-assessment-production-post-canary-batch-14-discovery.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-14-discovery-manifest.mjs";
...
assertV4(
  preparation.status === "post-canary-batch-14-ten-complete-score-blind-source-packets-prepared-awaiting-validation" &&
    preparationValidation.status === "post-canary-batch-14-score-blind-source-packet-validation-passed-frozen-under-standing-authorization" &&
    ...
    preparation.totals.discoveryContexts === 33 &&
    preparation.totals.maximumCopiedInputBytes === 65890 &&
  ...
  "validated Batch 14 source preparation is not frozen at the required authorization boundary"
);
assertV4(
  ...
  "Batch 14 standing authorization does not cover exact discovery execution"
);
assertV4(
  JSON.stringify(preparation.contexts.map((debate) => debate.debateNumber)) === JSON.stringify(REQUIRED_ORDER),
  "Batch 14 debate order drifted"
);
assertV4(
  ...
  for (const chunk of debate.chunks) {
    ...
    assertV4(
      sha256(chunkBytes) === chunk.chunkLedgerSha256 &&
        ...
        buildBatch14TokenCountedChunkLedger(chunkBytes).equals(tokenBytes) &&
...
assertV4(contexts.length === 33, "Batch 14 discovery must contain exactly 33 contexts");
...
assertV4(maximumContext.debateNumber === "57" && maximumContext.chunkId === "chunk-002", "maximum copied-input context drifted");
...
const sourceFiles = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-14-source-preparation.mjs",
...
const manifest = {
  ...
  status: shouldWrite ? "frozen-thirty-three-post-canary-batch-14-discovery-contexts-prepared-not-authorized" : "preview",
  ...
  AIOnly: true,
  userAuthorization: {
    scope: "standing-authorized Batch 14 discovery preparation and execution",
  ...
  costEstimate: {
    ...
    expectedAggregateModelMinutes: [35, 68],
    expectedAggregateComputeHours: [0.58, 1.13],
    absoluteGateTimeoutMinutes: 120,
    estimateBasis: "The exact 33-context Batch 14 input set is hash-locked with an observed maximum of 65,890 copied bytes; no input is truncated or repartitioned. The frozen 1→2→4 scheduler uses the subscription-backed route with a $0 direct incremental cost boundary.",
  },
  ...
  nextAuthorizedAction: "prepare-and-freeze-batch-14-discovery-execution-activation-under-standing-authorization",
};
