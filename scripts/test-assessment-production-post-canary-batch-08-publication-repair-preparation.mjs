...
assertV4(manifest.protocolId === POST_CANARY_BATCH_08_PUBLICATION_REPAIR_PROTOCOL_ID &&
  manifest.status === "frozen-one-context-batch-08-publication-repair-prepared-not-activated" &&
...
"Batch 8 repair preparation boundary changed");
...
for (const context of manifest.contexts) {
  ...
  assertV4(sha256(packetBytes) === context.packetSha256 &&
    ...
    canonicalJson(schema) === canonicalJson(buildBatch08PublicationRepairSchema(packet)) &&
...
console.log(JSON.stringify({ status: "passed-batch-08-publication-repair-preparation",
