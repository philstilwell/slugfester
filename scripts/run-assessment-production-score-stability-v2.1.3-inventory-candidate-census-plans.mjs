...
assertV4(
  ...
  "v2.1.3 candidate-census plan execution is unauthorized"
);
...
async function executeContext(context, contextIndex) {
  ...
  const sourceDirectory = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-v213-candidate-plan-${context.debateNumber}-`
    )
  );
  const isolatedCodexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-v213-candidate-plan-home-${context.debateNumber}-`
    )
  );
  ...
  process.stdout.write(
    `[v2.1.3-candidate-plan] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-score-stability-v2.1.3-candidate-census-plan-model-execution",
  protocolId: activation.protocolId,
  status:
    validContexts === preparation.contexts.length
      ? "ten-v2.1.3-candidate-census-plan-contexts-passed"
      : "v2.1.3-candidate-census-plan-gate-complete-with-failure",
  ...
  nextRequiredAction:
    validContexts === preparation.contexts.length
      ? "analyze-and-replay-ten-v2.1.3-candidate-census-plans-model-free-only"
      : "stop-preserve-failed-v2.1.3-candidate-census-plan-gate",
};
