...
const PRIOR_SELECTION = "docs/assessment-production/post-canary-continuation-v1/batch-10/selection.json";
const PRIOR_SELECTION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-10/selection-analysis.json";
const PRIOR_MUTATION_MANIFEST = "docs/assessment-production/post-canary-continuation-v1/batch-10/production-publication/mutation-manifest.json";
const PRIOR_COMPLETION_EXECUTION = "docs/assessment-production/post-canary-continuation-v1/batch-10/production-publication/generated-seo-correction/dependent-pilot-analysis-correction/execution.json";
const PRIOR_COMPLETION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-10/production-publication/generated-seo-correction/dependent-pilot-analysis-correction/analysis.json";
const PRIOR_COMPLETION_COMMIT = "2a38b1d70b49d51d3356b57a5d25e1b676bbdc5b";
const SCRIPT = "scripts/select-assessment-production-post-canary-batch-11.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-11-selection.mjs";
...
assertV4(!(await exists(FUTURE_SOURCE_PREPARATION)), "source preparation already exists before Batch 11 selection");
...
assertV4(
  ...
  "Batch 11 selection is not compatible with the frozen continuation policy"
);
...
assertV4(
  priorSelection.status === "tenth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    priorSelection.protocolId === "assessment-production-post-canary-batch-10" &&
    priorSelection.productionSequenceOrdinal === 11 &&
    priorSelection.selected.length === 10 &&
    priorSelectionAnalysis.status === "tenth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization" &&
    priorSelectionAnalysis.selection.sha256 === sha256(controlBytes[PRIOR_SELECTION]) &&
    priorMutationManifest.status === "frozen-batch-10-production-publication-mutation-manifest-prepared" &&
    JSON.stringify(priorMutationManifest.explicitOrder) === JSON.stringify(priorSelectedNumbers) &&
    priorCompletionExecution.status === "passed-complete-batch-10-production-transaction-after-dependent-pilot-analysis-correction" &&
    priorCompletionExecution.totals.originalSeoPasses === 1 &&
    priorCompletionExecution.totals.deterministicCorrectionPasses === 1 &&
    priorCompletionExecution.totals.repositoryValidationRunsInCorrection === 1 &&
    ...
    priorCompletionExecution.acceptedTransaction.productionDebates === 10 &&
    priorCompletionExecution.acceptedTransaction.productionLedgers === 10 &&
    priorCompletionExecution.acceptedTransaction.referencesByteIdentical === true &&
    priorCompletionExecution.acceptedTransaction.validatorByteIdentical === true &&
    priorCompletionExecution.acceptedTransaction.seoGeneratorByteIdentical === true &&
    priorCompletionExecution.acceptedTransaction.generatedSeoWrites === 12 &&
    priorCompletionExecution.acceptedTransaction.unchangedGeneratedSeoOutputs === 368 &&
    priorCompletionExecution.acceptedTransaction.dependentPilotAnalysisWrites === 2 &&
    priorCompletionAnalysis.status === "batch-10-production-publication-accepted-after-dependent-pilot-analysis-correction" &&
    priorCompletionAnalysis.execution.sha256 === sha256(controlBytes[PRIOR_COMPLETION_EXECUTION]) &&
    priorCompletionAnalysis.decision.boundedCorrectionPassed === true &&
    priorCompletionAnalysis.decision.exactTwoDependentWritesPassed === true &&
    priorCompletionAnalysis.decision.productionTransactionPreserved === true &&
    priorCompletionAnalysis.decision.all380GeneratedOutputsPreserved === true &&
  ...
  "Batch 10 has not satisfied the frozen successful-publication prerequisite"
);
assertV4(git("rev-parse", "--abbrev-ref", "HEAD") === "main", "Batch 11 selection must run on main");
assertV4(git("rev-parse", "HEAD") === PRIOR_COMPLETION_COMMIT, "Batch 11 selection checkpoint is not the exact Batch 10 publication commit");
assertV4(git("rev-parse", "origin/main") === PRIOR_COMPLETION_COMMIT, "Batch 10 publication commit is not the pushed origin/main checkpoint");
...
assertV4(
  JSON.stringify(priorSelectedNumbers) === JSON.stringify(ranked.slice(90, 100).map((item) => item.debateNumber)),
  "Batch 10 is not the immutable tenth rank slice"
);

const selected = [];
let totalEvents = 0;
let totalDurationMs = 0;
for (const [offset, rankedItem] of ranked.slice(100, 110).entries()) {
...
assertV4(selected.length === 10 && new Set(selected.map((item) => item.debateNumber)).size === 10, "Batch 11 did not select ten unique debates");

const checkpointCommit = git("rev-parse", "HEAD");
const selection = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-selection",
  protocolId: "assessment-production-post-canary-batch-11",
  status: "eleventh-post-canary-ten-debate-batch-selection-frozen-source-gate-passed",
  frozenAt,
  checkpointCommit,
  branch: "main",
  ...
  stagingOnly: true,
  batchNumber: 11,
  productionSequenceOrdinal: 12,
  userAuthorization: {
    instruction: "Continue the SLUGFESTER reassessment campaign with Batch 11 under the complete-workflow standing authorization supplied on 2026-08-26.",
    scopeInterpretation: "Materialize and source-validate only the deterministic Batch 11 selection under the frozen continuation policy after authenticating successful Batch 10 publication. This selection pass prepares no source packets, executes no models, derives no scores, and performs no production mutation.",
  ...
  eligibility: {
    ...
    previouslyPublishedAndRemovedCount: 100,
    remainingBeforeBatchCount: ranked.length - 100,
    remainingBeforeBatchRankedSha256: sha256(serializedJson(ranked.slice(100))),
    selectedCount: selected.length,
    remainingUnselectedCount: ranked.length - 110,
    dyadicOnly: true,
  ...
  nextAuthorizedAction: "prepare-batch-11-standing-authorization-and-source-packets-under-user-authorization"
};
...
const analysis = {
  ...
  status: "eleventh-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization",
  analyzedAt: frozenAt,
  ...
  finding: "Successful Batch 10 production publication was authenticated at the pushed atomic commit. The immutable campaign rank then selected ordinals 101 through 110 as ten unique pending dyadic debates, and all thirty effective source files and ten canonical event chains passed without replacement.",
  decision: {
