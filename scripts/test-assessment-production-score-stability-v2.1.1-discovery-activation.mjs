...
assert.equal(activation.failedGateDisposition.v21AcceptedAsPassed, false);
...
assert.equal(activation.executionPolicy.contexts, 42);
...
assert.equal(activation.schemaHardening.repositoryMaterializedEndEvent, true);
assert.equal(activation.schemaHardening.modelAuthoredEndEventProhibited, true);
assert.equal(activation.schemaHardening.minimumRequestedLexicalTokens, 12);
...
assert.equal(execution.contextsPlanned, 42);
assert(execution.contextsAttempted >= 1 && execution.contextsAttempted <= 42);
assert.equal(execution.contextsUnattempted, 42 - execution.contextsAttempted);
...
assert.equal(execution.modelAuthoredEndEvents, false);
for (const result of execution.results) {
  ...
  if (result.accepted) {
    assert.equal(
      result.validationSummary.repositoryMaterializedSourceWindows,
      true
    );
    assert.equal(result.validationSummary.modelAuthoredEndEvents, false);
  }
}

if (
  execution.status !==
  "forty-two-v2.1.1-validation-discovery-contexts-passed"
) {
...
assert.equal(execution.contextsAttempted, 42);
assert.equal(execution.contextsUnattempted, 0);
assert.equal(execution.validContexts, 42);
...
assert.equal(execution.repositoryMaterializedSourceWindows, true);
...
assert.equal(
  analysis.status,
  "v2.1.1-validation-discovery-passed-candidate-sharded-inventory-preparation-authorized"
);
assert.equal(analysis.debates.length, 10);
assert.equal(analysis.audit.validContexts, 42);
...
assert.equal(analysis.audit.repositoryMaterializedEndEvent, true);
assert.equal(analysis.audit.modelAuthoredEndEventProhibited, true);
assert.equal(analysis.audit.minimumRequestedLexicalTokens, 12);
...
for (const debate of analysis.debates) {
  ...
  assert(debate.con >= 4);
  assert.equal(
    debate.repositoryMaterializedSourceWindows,
    debate.candidates
  );
  assert.equal(debate.modelAuthoredEndEvents, false);
...
assert.equal(analysis.totals.modelContextsExecuted, 42);
assert.equal(
  analysis.totals.repositoryMaterializedSourceWindows,
  analysis.totals.candidates
);
assert.equal(analysis.totals.modelAuthoredEndEvents, 0);
