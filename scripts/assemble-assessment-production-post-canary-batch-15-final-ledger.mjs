...
const finalLedgerPath =
  `${POST_CANARY_BATCH_15_FINAL_LEDGER_ROOT}/final-ledger.json`;
...
assertV4(
  ...
  "post-canary Batch 15 final-ledger manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen Batch 15 final-ledger source hash mismatch`
  );
}
const inputs = await loadPostCanaryBatch15FinalLedgerInputs();
const ledger = buildPostCanaryBatch15FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
