...
assertV4(
  ...
  "the Batch 2 publication resumption is not authorized"
);
assertV4(
  activation.model?.label === "5.6 Sol" &&
    ...
    activation.executionPolicy?.contexts === 2 &&
  ...
  "the Batch 2 publication resumption controls changed"
);
...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), `batch-02-publication-resumption-4-${context.debateNumber}-`)
  );
  const codexHome = await mkdtemp(
    path.join(os.tmpdir(), `batch-02-publication-resumption-4-home-${context.debateNumber}-`)
  );
  ...
  try {
    ...
    process.stdout.write(
      `[batch-02-publication-resumption-4] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
    ...
    if (
      ...
      try {
        validationSummary = validatePostCanaryBatch02PublicationOutput(
      ...
      const validationRecord = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-02-publication-resumption-4-validation",
      ...
      const provenanceRecord = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-02-publication-resumption-4-provenance",
        protocolId: activation.protocolId,
        contextIndex: context.contextIndex,
        originalContextIndex: context.originalContextIndex,
        debateNumber: context.debateNumber,
  ...
  process.stdout.write(
    `[batch-02-publication-resumption-4] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
async function runPool(indexes, maximumConcurrency) {
  ...
  async function worker() {
    while (queue.length) {
      completed.push(await runContext(activation.contexts[queue.shift()]));
    }
  }
  await Promise.all(
    Array.from(
...
const unattemptedContextIndexes = activation.contexts
  ...
  .filter(
    (contextIndex) =>
      !results.some((result) => result.contextIndex === contextIndex)
  );
const passed = results.length === 2 && validContexts === 2;
const wallElapsedMs = Date.now() - gateStarted;
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-publication-resumption-4-model-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "two-post-canary-batch-02-publication-resumption-4-contexts-passed"
    : "post-canary-batch-02-publication-resumption-4-complete-with-failure",
  gateStartedAt,
  ...
  participantJudgmentWasScoreBlind: true,
  ownDebateScoresImmutable: true,
  acceptedDebate103RepairContexts: 9,
  acceptedDebate103RepairedFields: 17,
  acceptedDebate172RepairContexts: 1,
  acceptedDebate172RepairedFields: 2,
  acceptedDebate04RepairContexts: 0,
  acceptedDebate136RepairContexts: 5,
  acceptedDebate136RepairedFields: 9,
  acceptedDebate83RepairContexts: 1,
  acceptedDebate83RepairedFields: 1,
  acceptedDebate66RepairContexts: 0,
  acceptedDebate126RepairContexts: 0,
  acceptedDebate99RepairContexts: 5,
  acceptedDebate99RepairedFields: 9,
  meteredApiCostUsd: 0,
  paidServiceCallsThisStage: 0,
  modelAuthoredScores: 0,
  scorePassesExecutedThisStage: 0,
