...
const MANIFEST =
  `${POST_CANARY_BATCH_07_PUBLICATION_RESUMPTION_2_ROOT}/execution-preparation-manifest.json`;
...
assert.equal(manifest.batchNumber, 7);
...
assert.equal(manifest.totals.acceptedMoves, 38);
assert.equal(manifest.totals.resumptionContexts, 8);
assert.equal(manifest.totals.resumptionMoves, 149);
...
for (const context of manifest.contexts) {
  ...
  assert.deepEqual(schema, buildPostCanaryBatch07PublicationSchema(packet));
...
assert.equal(moves, 149);
assert.equal(sections, 41);
assert.equal(audioVerifiedMoves, 4);

const [repairedBytes, debate193PacketBytes, completeValidation] =
  await Promise.all([
    readFile(manifest.acceptedDebate193.output),
    readFile(manifest.acceptedDebate193.packet),
    readFile(manifest.acceptedDebate193.validation, "utf8").then(JSON.parse)
  ]);
const repairedValidation = validatePostCanaryBatch07PublicationOutput(
  JSON.parse(repairedBytes),
  JSON.parse(debate193PacketBytes)
);
assert.equal(repairedValidation.status, "passed");
assert.equal(repairedValidation.moves, 19);
assert.equal(repairedValidation.critiques, 19);
...
const [repaired80Bytes, debate80PacketBytes, complete80Validation] =
  await Promise.all([
    readFile(manifest.acceptedDebate80.output),
    readFile(manifest.acceptedDebate80.packet),
    readFile(manifest.acceptedDebate80.validation, "utf8").then(JSON.parse)
  ]);
const repaired80Validation = validatePostCanaryBatch07PublicationOutput(
  JSON.parse(repaired80Bytes),
  JSON.parse(debate80PacketBytes)
);
assert.equal(repaired80Validation.status, "passed");
assert.equal(repaired80Validation.moves, 19);
assert.equal(repaired80Validation.critiques, 19);
assert.equal(repaired80Validation.lockedScoresUnchanged, true);
assert.equal(complete80Validation.authorizedFieldsChanged, 5);
assert.equal(complete80Validation.immutableFieldsChanged, 0);
