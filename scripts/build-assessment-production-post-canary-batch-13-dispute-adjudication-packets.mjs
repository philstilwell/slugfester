...
import {
  ...
  POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT,
  buildPostCanaryBatch13DisputeAdjudicationPacket,
  makePostCanaryBatch13DisputeAdjudicationSchema
} from "./lib/assessment-production-post-canary-batch-13-dispute-adjudication.mjs";
...
const USER_AUTHORIZATION = Object.freeze({
  instruction:
    "The frozen Batch 13 standing authorization permits automatic preparation, validation, freezing, activation, execution, analysis, committing, and pushing of score-blind dispute-only adjudication checkpoints for exactly the ten selected debates while every frozen gate passes, with subscription-backed and local direct incremental cost capped at $0.",
...
const inputPaths = {
  ...
  schema: `${POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT}/adjudication.schema.json`
};
const TOOL_SOURCES = [
  "scripts/lib/v4-lean-production.mjs",
  "scripts/lib/v4221175-decomposed-adjudication.mjs",
  "scripts/lib/v42211728-hard-route-adjudication.mjs",
  "scripts/lib/assessment-production-post-canary-batch-13-dispute-adjudication.mjs",
  "scripts/build-assessment-production-post-canary-batch-13-dispute-adjudication-packets.mjs",
  "scripts/test-assessment-production-post-canary-batch-13-dispute-adjudication-packets.mjs",
  "scripts/validate-assessment-production-post-canary-batch-13-dispute-adjudication-output.mjs",
  "scripts/preregister-assessment-production-post-canary-batch-13-dispute-adjudication.mjs",
  "scripts/test-assessment-production-post-canary-batch-13-dispute-adjudication-manifest.mjs"
];
...
const audioRecoveryDebate37Path = `${audioRecoveryRoot}/outputs/debate-37.json`;
const audioRecoveryDebate111Path = `${audioRecoveryRoot}/outputs/debate-111.json`;
...
const sourcePaths = [
  ...
  audioRecoveryDebate37Path,
  audioRecoveryDebate111Path,
...
const [
  ...
  audioRecoveryDebate37,
  audioRecoveryDebate111,
  judgmentPreparation,
  judgmentExecution,
  judgmentAnalysis,
...
assertV4(
  sourceAnalysis.status ===
      "post-canary-batch-13-deterministic-disagreements-extracted-standing-authorization-active-for-audio-work" &&
    JSON.stringify(sourceAnalysis.debates.map((item) => item.debateNumber)) ===
      JSON.stringify(EXPECTED_DEBATES) &&
    sourceAnalysis.adjudicationWorkload.disputedMoves === 191 &&
    sourceAnalysis.adjudicationWorkload.candidateSelections === 541 &&
    sourceAnalysis.adjudicationWorkload.packetsPrepared === false &&
    sourceAnalysis.adjudicationWorkload.modelContextsExecuted === 0,
  "Batch 13 deterministic disagreement workload changed"
);
assertV4(
  ...
  "Batch 13 accepted independent-judgment boundary changed"
);
assertV4(
  ...
  "Batch 13 independent-judgment configuration changed"
);
...
assertV4(
  standingAuthorization.status ===
      "frozen-active-batch-13-complete-remaining-workflow-standing-authorization" &&
  ...
  "Batch 13 standing authorization changed"
);
assertV4(
  audioAudit.status ===
      "post-canary-batch-13-audio-verification-unresolved-after-empty-segment-overlays" &&
    audioAudit.totals.requiredMoves === 8 &&
    audioAudit.totals.verified === 2 &&
    audioAudit.totals.unresolved === 6 &&
    audioAudit.totals.retries === 0 &&
    audioAudit.totals.corrections === 4 &&
    ...
    audioCost.costControl.usageDerivedEstimatedCostUsd === 0.21423 &&
    audioCost.costControl.approvedMaximumCostUsd === 1 &&
    audioCost.costControl.directIncrementalCostCapControlPassed === true &&
    audioExecutionManifest.calls.length === 8 &&
    audioModelExecution.callsAttempted === 8 &&
    audioModelExecution.callsCompleted === 8 &&
    audioModelExecution.retries === 0 &&
    audioRecoveryCombined.status === "post-canary-batch-13-combined-audio-verification-passed" &&
    audioRecoveryCombined.totals.requiredMoves === 8 &&
    audioRecoveryCombined.totals.verified === 8 &&
    audioRecoveryCombined.totals.unresolved === 0 &&
    audioRecoveryAnalysis.status === "batch-13-audio-attribution-recovery-passed" &&
    audioRecoveryAnalysis.validation.verified === 6 &&
    audioRecoveryAnalysis.validation.unresolved === 0,
  "Batch 13 accepted audio-verification boundary changed"
);
assertV4(
  ...
  "Batch 13 packet-preparation authorization changed"
);
...
const recoveredDecisionByKey = new Map(
  [audioRecoveryDebate37, audioRecoveryDebate111]
...
assertV4(audioByDebateMove.size === 8, "Batch 13 verified audio population changed");

const schema = makePostCanaryBatch13DisputeAdjudicationSchema();
...
if (shouldWrite) {
  const immutableOutputs = [
    inputPaths.schema,
    `${POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT}/preparation-manifest.json`,
    `${POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT}/execution-preparation-manifest.json`,
    ...EXPECTED_DEBATES.flatMap((debateNumber) => [
      `${POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT}/packets/debate-${debateNumber}.json`,
      `${POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`
    ])
  ];
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const built = buildPostCanaryBatch13DisputeAdjudicationPacket(
  ...
  const provenancePath =
    `${POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`;
  const outputPath =
    `${POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT}/outputs/debate-${debateNumber}.json`;
  const packetBytes = pretty(built.packet);
  const provenanceBytes = pretty({
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-13-adjudication-candidate-provenance",
...
const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-13-dispute-only-adjudication-preparation",
  protocolId: POST_CANARY_BATCH_13_DISPUTE_ADJ_PROTOCOL_ID,
  status: shouldWrite
    ? "prepared-ten-isolated-post-canary-batch-13-dispute-only-adjudication-contexts"
    : "preview",
  frozenAt,
  ...
  acceptedSourceBoundary: {
    ...
    allEightAudioMovesVerified: true,
    audioValidationOverlaysPreserved: 4,
    audioAttributionRecoveryContextsPreserved: 2,
    audioCorrectionCallsPreserved: 0,
    audioUsageDerivedEstimatedCostUsd: 0.21423,
  ...
  nextAuthorizedAction:
    "freeze-post-canary-batch-13-dispute-only-adjudication-execution-preparation-manifest-model-free-only"
};
assertV4(
  contexts.length === 10 &&
    disputedMoves === 191 &&
    disputedMoves === sourceAnalysis.adjudicationWorkload.disputedMoves &&
    candidateSelections === 541 &&
    candidateSelections === sourceAnalysis.adjudicationWorkload.candidateSelections &&
    audioVerifiedMoves === 8,
  `Batch 13 adjudication packet workload differs from frozen inputs: ${JSON.stringify({ contexts: contexts.length, disputedMoves, candidateSelections, audioVerifiedMoves, sourceDisputedMoves: sourceAnalysis.adjudicationWorkload.disputedMoves, sourceCandidateSelections: sourceAnalysis.adjudicationWorkload.candidateSelections })}`
);
assertV4(
  preparation.totals.maximumCopiedInputBytes <= 400000,
  `Batch 13 context ${preparation.totals.maximumCopiedInputBytes} bytes exceeds the frozen 400 KB correction ceiling`
);
if (shouldWrite) {
  await mkdir(POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT, { recursive: true });
