...
const diagnosisPath =
  `${root}/audio-source-transport-recovery-1/failure-diagnosis.json`;
...
const finalSourcePath = `${localRoot}/debate-170/audio/source.mp3`;
...
const sourcePaths = [
  "scripts/prepare-assessment-production-post-canary-batch-09-audio-transport-recovery-2.mjs",
  "scripts/run-assessment-production-post-canary-batch-09-audio-transport-recovery-2.mjs",
  "scripts/test-assessment-production-post-canary-batch-09-audio-transport-recovery-2-preparation.mjs",
  "scripts/test-assessment-production-post-canary-batch-09-audio-source-recovery-2-cohort.mjs",
...
assertV4(
  diagnosis.status ===
    "preserved-batch-09-debate-170-hls-format-unavailable-recovery-failure-diagnosed" &&
    diagnosis.localEvidence.preservedInvalidOriginalAttempt.sha256 ===
...
assertV4(!(await exists(finalSourcePath)), "Debate 170 final source unexpectedly exists");
...
const plan = {
  ...
  debate170Recovery: {
    failedFinalSourcePath: finalSourcePath,
    preservedInvalidEvidencePath: preservedInvalidPath,
    downloadTemplate:
      `${localRoot}/debate-170/audio/source.recovery-2.progressive.%(ext)s`,
    normalizedTemporaryPath:
      `${localRoot}/debate-170/audio/source.recovery-2.normalized.mp3`,
    finalSourcePath,
    videoUrl: "https://www.youtube.com/watch?v=HoTILnpd3q8",
    transport: "tv-android-vr-web-progressive-webm-http-chunk-transport",
    ytDlpArguments: [
      "-m", "yt_dlp", "--no-playlist", "--quiet", "--no-warnings",
      "--no-continue", "--retries", "0", "--fragment-retries", "0",
      "--extractor-retries", "0", "--file-access-retries", "0",
      "--concurrent-fragments", "1", "--http-chunk-size", "10485760",
      "--force-ipv4", "--extractor-args",
...
if (shouldActivate) {
  ...
  const activation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-09-audio-source-transport-recovery-2-activation",
    status:
