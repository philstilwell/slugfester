...
const MANIFEST =
  `${POST_CANARY_BATCH_01_PUBLICATION_ROOT}/execution-preparation-manifest.json`;
...
assert.equal(manifest.protocolId, POST_CANARY_BATCH_01_PUBLICATION_PROTOCOL_ID);
...
assert.equal(manifest.totals.moves, 177);
...
for (const context of manifest.contexts) {
  ...
  assert.equal(packet.protocolId, POST_CANARY_BATCH_01_PUBLICATION_PROTOCOL_ID);
  ...
  assert.equal(schema.properties.schemaVersion.const, POST_CANARY_BATCH_01_PUBLICATION_OUTPUT_VERSION);
  assert.equal(schema.properties.protocolId.const, POST_CANARY_BATCH_01_PUBLICATION_PROTOCOL_ID);
