...
import { POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_ORDER,
  ...
  compilePostCanaryBatch16PublicationStagingRecord } from
...
assertV4(m.status === "frozen-post-canary-batch-16-deterministic-publication-compilation-prepared" &&
  canonicalJson(m.explicitOrder) === canonicalJson(POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_ORDER) &&
  m.contexts?.length === 10 && m.contexts.reduce((sum, row) => sum + row.expectedMoves, 0) === 194 &&
...
"Batch 16 compilation preparation changed");
let moves = 0;
for (const context of m.contexts) {
  ...
  const compiled = compilePostCanaryBatch16PublicationStagingRecord({ output, packet, identity: row });
  const validation = validatePostCanaryBatch16CompiledStagingRecord({ compiled, output, packet, identity: row });
  moves += validation.moves;
}
assertV4(moves === 194, "compiled move replay changed");
