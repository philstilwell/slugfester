...
import { DEBATES, ORIGINAL_CONTEXT_INDEXES, PROTOCOL_ID, ROOT } from
  "./lib/assessment-production-post-canary-batch-13-publication-resumption-2.mjs";
...
const RESUMPTION_1_ROOT = `${PUBLICATION_ROOT}/original-unattempted-context-resumption-1`;
...
const DEBATE_20_REPAIR_ANALYSIS = `${RESUMPTION_1_ROOT}/debate-20-field-disjoint-repair-1/analysis.json`;
const ACCEPTED_DEBATES = Object.freeze(["26", "190", "87", "20"]);
...
const [originalPreparationBytes, originalActivationBytes, originalExecutionBytes,
  originalAnalysisBytes, debate87AnalysisBytes, resumption1ExecutionBytes,
  resumption1AnalysisBytes, debate20RepairAnalysisBytes] =
  await Promise.all([
    ORIGINAL_PREPARATION, ORIGINAL_ACTIVATION, ORIGINAL_EXECUTION,
    ORIGINAL_ANALYSIS, DEBATE_87_ANALYSIS, RESUMPTION_1_EXECUTION,
    RESUMPTION_1_ANALYSIS, DEBATE_20_REPAIR_ANALYSIS
...
assertV4(
  resumption1Execution.status === "seven-context-publication-resumption-stopped-with-failure" &&
    ...
    resumption1Execution.results?.[0]?.originalContextIndex === 3 &&
    resumption1Execution.results?.[0]?.debateNumber === "20" &&
    resumption1Execution.results?.[0]?.status === "output-validation-failed" &&
    ...
    resumption1Analysis.status === "seven-context-publication-resumption-failed",
...
assertV4(
  contexts.reduce((sum, context) => sum + context.moves, 0) === 120 &&
    contexts.reduce((sum, context) => sum + context.sections, 0) === 32 &&
...
const sourceScripts = [
  "scripts/lib/assessment-production-post-canary-batch-13-publication-resumption-2.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-13-publication-resumption-2.mjs",
  "scripts/activate-assessment-production-post-canary-batch-13-publication-resumption-2.mjs",
  "scripts/run-assessment-production-post-canary-batch-13-publication-resumption-2.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-13-publication-resumption-2.mjs",
...
const sourceFiles = [
  ...
  DEBATE_87_ANALYSIS, RESUMPTION_1_EXECUTION, RESUMPTION_1_ANALYSIS,
  DEBATE_20_REPAIR_ANALYSIS,
...
const manifest = {
  ...
  status: "frozen-six-original-unattempted-batch-13-publication-contexts-prepared-not-activated",
  frozenAt,
  ...
  userAuthorization: {
    ...
    resolvedScope: "resume only the six original publication contexts still unattempted after Debate 20 failed the first resumption ramp and passed its one-shard, two-field bounded repair",
  ...
  acceptedPriorDebates: acceptedOutputs.map((item) => ({
    ...
    source: item.debateNumber === "87" ? "exceptional-third-level-atomic-shard-recovery" : item.debateNumber === "20" ? "one-shard-two-field-publication-repair" : "original-publication-first-attempt",
  ...
  previousResumptionBoundary: {
    ...
    attemptedOriginalContextIndexes: [3],
