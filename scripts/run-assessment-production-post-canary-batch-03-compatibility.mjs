...
assertV4(write, "Batch 3 compatibility runner requires --write");
...
const paths = {
  root: POST_CANARY_BATCH_03_COMPATIBILITY_ROOT,
...
assertV4(
  ...
  "Batch 3 compatibility pass was already attempted; rerun forbidden"
);
...
assertV4(
  ...
  "frozen Batch 3 compatibility activation is unavailable"
);
...
tests.push(
  runCommand(
    "batch-03-route-positive-negative-controls",
    process.execPath,
...
if (tests.at(-1).passed) {
  tests.push(
    runCommand("checkpoint-batch-01-batch-02-legacy-reference-regression", process.execPath, [
...
const nextAuthorizedAction = compatibilityAcceptancePassed
  ? "prepare-batch-03-production-publication-mutation-manifest-under-standing-authorization"
  : "diagnose-batch-03-compatibility-failure-under-failure-recovery-standing-authorization";
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-compatibility-execution",
  protocolId: preparation.protocolId,
  status: compatibilityAcceptancePassed
    ? "batch-03-route-score-checkpoint-batch-01-batch-02-legacy-reference-validation-passed"
    : "failed-closed-batch-03-compatibility-staging-pass",
  startedAt,
  completedAt,
  activation: {
  ...
  regressions: {
    batch03RouteControlsPassed: tests[0]?.passed === true,
    checkpointBatch01Batch02LegacyReferencePassed: tests[1]?.passed === true,
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-compatibility-execution-analysis",
  protocolId: preparation.protocolId,
  status: compatibilityAcceptancePassed
    ? "batch-03-production-compatibility-passed"
    : "batch-03-production-compatibility-failed-closed",
  ...
  checks: {
    ...
    routeAndScoreControlsPassed: execution.regressions.batch03RouteControlsPassed,
    checkpointBatch01Batch02LegacyReferencePassed:
      execution.regressions.checkpointBatch01Batch02LegacyReferencePassed,
