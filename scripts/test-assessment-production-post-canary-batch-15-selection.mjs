...
assert.equal(selection.status, "fifteenth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed");
assert.equal(analysis.status, "fifteenth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization");
assert.equal(analysis.selection.sha256, sha256(selectionBytes));
assert.equal(selection.batchNumber, 15);
assert.equal(selection.productionSequenceOrdinal, 16);
...
assert.equal(selection.eligibility.previouslyPublishedAndRemovedCount, 140);
assert.equal(selection.eligibility.remainingBeforeBatchCount, 24);
assert.equal(selection.eligibility.selectedCount, 10);
assert.equal(selection.eligibility.remainingUnselectedCount, 14);
...
assert.equal(priorCompletionExecution.status, "passed-complete-batch-14-transaction-ready-for-atomic-commit");
...
assert.equal(priorCompletionAnalysis.status, "batch-14-production-publication-accepted-for-atomic-commit-after-generated-seo-correction");
assert.equal(priorCompletionAnalysis.decision.generatedSeoCorrectionGatePassed, true);
assert.equal(priorCompletionAnalysis.decision.batch14ProductionPublicationGatePassed, true);
...
assert.equal(sha256(serializedJson(ranked.slice(140))), selection.eligibility.remainingBeforeBatchRankedSha256);
assert.deepEqual(priorSelection.selected.map((item) => item.debateNumber), ranked.slice(130, 140).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.debateNumber), ranked.slice(140, 150).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.rankOrdinal), [141, 142, 143, 144, 145, 146, 147, 148, 149, 150]);
assert.deepEqual(selection.selected.map((item) => item.rankSha256), ranked.slice(140, 150).map((item) => item.rankSha256));
...
assert.deepEqual(selection.deterministicOrdering.selectedRankOrdinals, [141, 150]);
...
assert.equal(selection.nextAuthorizedAction, "prepare-batch-15-standing-authorization-and-source-packets-under-user-authorization");

console.log(JSON.stringify({ status: "passed", selectedDebates: selection.selected.map((item) => item.debateNumber), selectedRankOrdinals: [141, 150], sourceFiles: 30, replacements: 0, sourcePacketsPrepared: 0, modelContexts: 0, paidServiceCalls: 0, directCostUsd: 0, nextAuthorizedAction: selection.nextAuthorizedAction }, null, 2));
