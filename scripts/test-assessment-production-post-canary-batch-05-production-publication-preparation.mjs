...
const analysisPath = `${POST_CANARY_BATCH_05_PRODUCTION_PUBLICATION_ROOT}/preparation-analysis.json`;
const [manifest, analysis] = await Promise.all([readJson(manifestPath), readJson(analysisPath)]);

assert.equal(manifest.protocolId, POST_CANARY_BATCH_05_PRODUCTION_PUBLICATION_PROTOCOL_ID);
assert.equal(manifest.status, "frozen-batch-05-production-publication-mutation-manifest-prepared");
...
assert.deepEqual(manifest.explicitOrder, POST_CANARY_BATCH_05_PRODUCTION_PUBLICATION_ORDER);
...
assert.equal(manifest.productionBaseline.existingProductionLedgers.count, 51);
...
assert.equal(analysis.status, "batch-05-production-publication-mutation-manifest-freeze-passed");
...
assert.equal(await exists(`${POST_CANARY_BATCH_05_PRODUCTION_PUBLICATION_ROOT}/execution-activation.json`), false);
assert.equal(await exists(`${POST_CANARY_BATCH_05_PRODUCTION_PUBLICATION_ROOT}/execution.json`), false);
