...
const ROOT =
  "docs/assessment-production/score-stability-v2.1.3-validation-cohort/source-preparation";
...
const inventoryAnalysis = JSON.parse(
  await readFile(preparation.inputs.inventorySuccessorAnalysis, "utf8")
);
const discoveryAnalysis = JSON.parse(
  await readFile(preparation.inputs.discoverySuccessorAnalysis, "utf8")
);
...
assert.equal(
  preparation.failedGateDisposition
    .v212FailedOutputsUsedAsFreshSuccessorModelInput,
  false
);
...
assert.equal(
  preparation.inventorySuccessorContract.scoreFieldsAvailable,
  false
);
assert.deepEqual(
  preparation.discoverySuccessorContract.sourceSelectionShape,
  ["startEvent", "endEvent"]
);
assert.equal(
...
assert.equal(
  preparation.discoverySuccessorContract.repositoryDerivedLexicalTokenCount,
  true
);
assert.equal(
...
for (const [key, value] of Object.entries(preparation.authorization)) {
  if (!["deterministicValidation", "discoveryExecutionManifestPreparation"].includes(key)) {
...
function assertNoProhibitedKeys(value, location = "packet") {
  ...
  for (const [key, child] of Object.entries(value)) {
    assert(!prohibitedPacketKeys.has(key), `${location}.${key}: prohibited`);
...
for (const context of preparation.contexts) {
  ...
  for (const chunk of context.chunks) {
    ...
    assert.equal(sourceWindow.properties.endEvent.minimum, chunk.coreStartEvent);
