...
import { POST_CANARY_BATCH_09_PUBLICATION_COMPILATION_ORDER,
  ...
  compilePostCanaryBatch09PublicationStagingRecord } from
...
assertV4(m.status === "frozen-post-canary-batch-09-deterministic-publication-compilation-prepared" &&
  canonicalJson(m.explicitOrder) === canonicalJson(POST_CANARY_BATCH_09_PUBLICATION_COMPILATION_ORDER) &&
  m.contexts?.length === 10 && m.contexts.reduce((sum, row) => sum + row.expectedMoves, 0) === 180 &&
...
"Batch 9 compilation preparation changed");
let moves = 0;
for (const context of m.contexts) {
  ...
  const compiled = compilePostCanaryBatch09PublicationStagingRecord({ output, packet, identity: row });
  const validation = validatePostCanaryBatch09CompiledStagingRecord({ compiled, output, packet, identity: row });
  moves += validation.moves;
}
assertV4(moves === 180, "compiled move replay changed");
