...
if (!shouldExecute) {
  console.log(
    JSON.stringify(
      {
        status: "passed-activated-batch-10-audio-source-manual-redirect-recovery-5-preflight",
...
const installSource = async (source) => {
  ...
  const rangeSource = plan.manualRedirectSources.find(
    (item) => item.sourceVideoId === source.sourceVideoId
  );
  assertV4(
  ...
  normalizedTemporaryPath = path.join(
    audioDirectory,
    "source.manual-redirect5-format139.normalized.mp3"
  );
  const rawSourcePath = path.join(
    audioDirectory,
    "source.manual-redirect5-format139.m4a"
  );
  assertV4(
    ...
    for (
      ...
      for (let requestIndex = 0; requestIndex <= 3; requestIndex += 1) {
        const responseBodyPath = path.join(
          audioDirectory,
          `source.manual-redirect5-format139.range-${String(index).padStart(3, "0")}.hop-${requestIndex}.body`
        );
        const headersPath = path.join(
          audioDirectory,
          `source.manual-redirect5-format139.range-${String(index).padStart(3, "0")}.hop-${requestIndex}.headers.txt`
        );
        assertV4(
  ...
  acquisitionMode =
    "downloaded-public-source-format139-manual-https-googlevideo-redirect-chain";
  state.sourcesInstalled += 1;
  publicSourceAttemptAudit.push({
    debateNumber,
    videoId: source.sourceVideoId,
...
try {
  ...
  const preparation = {
    ...
    recovery: {
      ...
      manualRedirectFormat139Downloads: 3,
    ...
    acquisitionPolicy: {
      ...
      acquisitionFormat:
        "frozen-youtube-audio-only-format139-manual-https-googlevideo-redirect-chain",
  ...
  executionFailure = {
    category: "source-or-validation-failure-during-authorized-manual-redirect-recovery-5",
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-10-audio-source-manual-redirect-recovery-5-execution",
  status: executionFailure
    ? "failed-one-shot-batch-10-audio-source-manual-redirect-recovery-5-stop-required"
    : "completed-one-shot-batch-10-audio-source-manual-redirect-recovery-5-and-cohort",
  batchNumber: 10,
  planSha256: sha256(planBytes),
  activationSha256: sha256(activationBytes),
  state,
