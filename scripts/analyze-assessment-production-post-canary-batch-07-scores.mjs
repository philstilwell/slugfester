...
import {
  POST_CANARY_BATCH_07_CALCULATED_SCORES_VERSION,
...
const validation = validatePostCanaryBatch07Scores(
  scores,
  ledger,
  inputs.debateInputs,
...
assertV4(
  scores.schemaVersion === POST_CANARY_BATCH_07_CALCULATED_SCORES_VERSION &&
    !scores.productionCanary &&
    scores.batchNumber === 7 &&
    scores.stagingOnly &&
    !scores.developmentValidationOnly &&
    scores.formulaBoundary.scoringPasses === 1 &&
    !scores.formulaBoundary.modelCalculatedScores,
  "post-canary Batch 7 score artifact identity mismatch"
);
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-07-score-analysis",
  protocolId: scores.protocolId,
  status: passed
    ? "post-canary-batch-07-score-stability-gate-passed"
    : "post-canary-batch-07-score-stability-gate-failed",
  ...
  nextAuthorizedAction: passed
    ...
    : "standing-authorization-permits-batch-07-score-stability-failure-diagnosis"
};
if (shouldWrite) {
  await writeFile(
