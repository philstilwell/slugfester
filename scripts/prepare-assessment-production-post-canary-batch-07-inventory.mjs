...
const SCRIPT =
  "scripts/prepare-assessment-production-post-canary-batch-07-inventory.mjs";
const PROTOCOL_ID =
  "assessment-production-post-canary-batch-07-candidate-sharded-inventory";
const SOURCE_PACKET_VERSION =
  "1.0-assessment-production-post-canary-batch-07-candidate-sharded-inventory-source-packet";
...
const EXPECTED = {
  ...
  proCandidates: 160,
  conCandidates: 162,
  belowHighAttributionCandidates: 3,
};
...
function buildInventorySourcePacket(sourcePacket) {
  assertV4(
    sourcePacket?.schemaVersion ===
      "1.0-assessment-production-post-canary-batch-07-score-blind-source-packet" &&
      sourcePacket.protocolId ===
        "assessment-production-post-canary-batch-07-source-preparation" &&
    ...
    "Batch 7 source packet is unavailable or not dyadic"
  );
  return {
...
function makePlanPacket({
  ...
  return {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-07-candidate-census-plan-packet",
    protocolId: PROTOCOL_ID,
...
async function buildFrozenArtifacts(frozenAt) {
  ...
  assertV4(
    ...
    "Batch 7 standing authorization does not cover exact inventory preparation and execution"
  );

  assertV4(
    discoveryAnalysis.status ===
      "post-canary-batch-07-discovery-passed-standing-authorization-active-for-inventory-preparation" &&
      ...
      discoveryAnalysis.audit?.validContexts === 35 &&
      ...
      discoveryAnalysis.totals?.modelContextsExecuted === 35 &&
      ...
      discoveryAnalysis.nextAuthorizedAction ===
        "prepare-freeze-and-activate-batch-07-candidate-census-planner-contexts-under-standing-authorization",
    "passed Batch 7 discovery boundary drifted"
  );
  assertV4(
    discoveryExecution.status ===
      "thirty-five-post-canary-batch-07-discovery-contexts-passed" &&
      discoveryExecution.contextsAttempted === 35 &&
      discoveryExecution.validContexts === 35 &&
    ...
    "Batch 7 discovery execution drifted"
  );
  assertV4(
      sourcePreparation.status ===
      "post-canary-batch-07-ten-complete-score-blind-source-packets-prepared-awaiting-validation" &&
    ...
    "Batch 7 source preparation or model boundary drifted"
  );
  assertV4(
    sourcePreparationValidation.status ===
      "post-canary-batch-07-score-blind-source-packet-validation-passed-frozen-under-standing-authorization" &&
    ...
    "Batch 7 source validation drifted"
  );
  assertV4(
    selection.status ===
    ...
    "Batch 7 frozen selection drifted"
  );
  assertV4(
  ...
  for (const discovered of discoveryAnalysis.debates) {
    ...
    assertV4(
      ...
      `${discovered.debateNumber}: Batch 7 identity drifted`
    );
    const [
  ...
  assertV4(
    ...
    "Batch 7 totals or packet state drifted"
  );
  ...
  const preparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-07-candidate-sharded-inventory-preparation",
    protocolId: PROTOCOL_ID,
    sideSelectionProtocolId: CHRONOLOGY_FALLBACK_INVENTORY.protocolId,
    status:
      "post-canary-batch-07-candidate-sharded-source-assets-and-ten-planner-packets-frozen",
    preparedAt: frozenAt,
    ...
    userAuthorization: {
      scope:
        "prepare, freeze, activate, and execute the Batch 7 candidate-census and candidate-sharded side-selection workflow under the frozen standing authorization",
      standingAuthorization: POST_CANARY_BATCH_07_STANDING_AUTHORIZATION,
    ...
    nextAuthorizedAction:
      "freeze-and-activate-batch-07-candidate-census-plan-execution-under-standing-authorization",
  };
  ...
  const executionPreparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-07-candidate-census-plan-execution-preparation-manifest",
    protocolId: PROTOCOL_ID,
    status:
      "frozen-ten-post-canary-batch-07-candidate-census-plan-contexts-prepared-not-authorized",
    frozenAt,
    ...
    costEstimate: {
      ...
      estimateBasis:
        `The promoted ten-context candidate-census planner used this contract at concurrency two. Batch 7 freezes the same ten-context shape with an observed maximum of ${maximumCopiedInputBytes.toLocaleString("en-US")} copied bytes, below the proven ${V212_CANDIDATE_SHARDED_INVENTORY.maximumCopiedInputBytes.toLocaleString("en-US")}-byte ceiling. ChatGPT-subscription execution has no direct incremental API charge.`,
    },
  ...
  const validation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-07-candidate-sharded-inventory-preparation-validation",
    protocolId: PROTOCOL_ID,
    status:
      "batch-07-candidate-sharded-inventory-packets-and-plan-execution-manifest-validation-passed-frozen-standing-authorization-active",
...
async function validateFrozen() {
  ...
  assertV4(
    ...
    "frozen Batch 7 inventory preparation is invalid"
  );
...
if (command === "freeze") await freeze();
  ...
  throw new Error(
    "usage: prepare-assessment-production-post-canary-batch-07-inventory.mjs <freeze|validate>"
  );
}
