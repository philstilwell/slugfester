...
import {
  ...
  buildPostCanaryBatch03ValidatorSource,
  sha256,
  validatePostCanaryBatch03ValidatorSource
} from "./lib/assessment-production-post-canary-batch-03-compatibility-execution.mjs";
...
const paths = {
  ...
  preparationAnalysis: `${POST_CANARY_BATCH_03_COMPATIBILITY_ROOT}/analysis.json`,
  ...
  executionLibrary:
    "scripts/lib/assessment-production-post-canary-batch-03-compatibility-execution.mjs",
  activationScript:
    "scripts/activate-assessment-production-post-canary-batch-03-compatibility.mjs",
  activationTest:
    "scripts/test-assessment-production-post-canary-batch-03-compatibility-activation.mjs",
  executionScript:
    "scripts/run-assessment-production-post-canary-batch-03-compatibility.mjs",
  executionTest:
    "scripts/test-assessment-production-post-canary-batch-03-compatibility-execution.mjs"
};
...
assertV4(
  preparation.status ===
      "post-canary-batch-03-compatibility-plan-prepared-and-frozen" &&
  ...
  "frozen Batch 3 compatibility preparation is unavailable"
);
assertV4(
  preparationAnalysis.status ===
      "post-canary-batch-03-compatibility-plan-freeze-passed" &&
    preparationAnalysis.preparation.sha256 === sha256(preparationBytes) &&
    preparationAnalysis.finding.executed === false,
  "frozen Batch 3 compatibility preparation analysis is unavailable"
);
...
assertV4(
  ...
  "a stable --activated-at ISO timestamp is required for the first Batch 3 compatibility activation write"
);
assertV4(
  !(await exists(paths.execution)) && !(await exists(paths.stagedLedgerRoot)),
  "Batch 3 compatibility execution already exists; activation blocked"
);
...
assertV4(
  canonicalJson(packetHashes.map((item) => item.debateNumber)) ===
    canonicalJson(POST_CANARY_BATCH_03_COMPATIBILITY_ORDER),
  "Batch 3 compatibility activation packet order changed"
);
...
assertV4(
  ...
  "active validator differs from the frozen Batch 3 baseline"
);
const proposedValidator = buildPostCanaryBatch03ValidatorSource(
  baselineValidator
);
const proposedValidatorValidation =
  validatePostCanaryBatch03ValidatorSource(proposedValidator);
...
const activation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-compatibility-execution-activation",
  protocolId: preparation.protocolId,
  status: POST_CANARY_BATCH_03_COMPATIBILITY_ACTIVATION_STATUS,
  activatedAt,
  ...
  userAuthorization: {
    instruction:
      "The Batch 3 standing authorization covers activation and exactly one deterministic compatibility staging pass.",
    scopeInterpretation:
      "Activate the frozen Batch 3 compatibility manifest and execute exactly one validator-and-staged-ledger compatibility pass with no reruns, models, paid services, production ledger publication, production data mutation, or Batch 4 selection."
  },
  preparation: {
  ...
  validator: {
    ...
    routeCredentialStatus: POST_CANARY_BATCH_03_COMPATIBILITY_ACTIVATION_STATUS
  },
  packetHashes,
  executionToolHashes,
  protectedProduction: {
  ...
  requiredRegressions: [
    "Batch 3 route positive and negative controls",
  ...
  nextAuthorizedAction:
    "execute-one-frozen-batch-03-production-compatibility-staging-pass-no-reruns"
};
const activationBytes = serializedJson(activation);

if (write) {
  assertV4(!existingActivation, "Batch 3 compatibility is already activated");
  await writeFile(resolve(paths.activation), activationBytes);
} else {
  assertV4(
    existingActivation &&
      canonicalJson(existingActivation) === canonicalJson(activation),
    "stored Batch 3 compatibility activation differs from deterministic replay"
  );
}
