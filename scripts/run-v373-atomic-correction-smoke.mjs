...
const manifestText = await readFile(path.resolve(root, V373_EXECUTION_MANIFEST), "utf8");
...
assert(manifest.status === "frozen-before-model-execution" && manifest.correctionSmokeExecutionAuthorized, "execution manifest does not authorize this smoke");
assert(manifest.executionPolicy.modelOutputRetriesMaximum === 0 && manifest.executionPolicy.attemptsPerContext === 1, "execution retry lock invalid");
for (const [file, digest] of Object.entries(manifest.sourceHashes)) {
  assert(sha256(await readFile(path.resolve(root, file), "utf8")) === digest, `source hash mismatch: ${file}`);
}
await access(codex);
...
async function modelTask(context) {
  const { debateNumber, reviewerPass, packet, schema, output, bundleCount } = context;
  const temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-v373-${reviewerPass}-${debateNumber}-`));
  const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), `slugfester-v373-home-${reviewerPass}-${debateNumber}-`));
  ...
  try {
    ...
    process.stdout.write(`\n[v3.7.3] starting ${manifest.model.label} Debate ${debateNumber} ${reviewerPass}\n`);
    const invocation = await run(codex, [
      ...
      "-c", `model_reasoning_effort=\"${manifest.model.reasoningEffort}\"`, "--disable", "plugins", "--disable", "remote_plugin", "--disable", "skill_search",
      ...
      "Read workflow.md, rubric.md, manual.md, schema.json, and packet.json completely and no other files. Return exactly one schema-conforming atomic-bundle output. Process every bundle once and in order. Do not emit scores or commentary outside the JSON object."
    ...
    const base = {
      ...
      sameRequestStreamRecoveries: [...log.matchAll(streamPattern)].length,
    ...
    try {
      ...
      const validation = await run(process.execPath, ["scripts/validate-v373-atomic-output.mjs", output, packet, schema], { cwd: root, env: process.env });
      const valid = validation.code === 0;
      const scoringFieldCount = containsScoreField(parsed) ? 1 : 0;
      process.stdout.write(`[v3.7.3] Debate ${debateNumber} ${reviewerPass} ${valid ? "completed-valid" : "output-validation-failed"}\n`);
      return {
        ...base,
        ...
        invalidBundleCount: valid ? 0 : bundleCount,
        scoringFieldCount,
        outputBundleCount: Array.isArray(parsed.bundles) ? parsed.bundles.length : 0,
        validationExitCode: validation.code,
        validationMessage: valid ? null : validation.stderr.trim().slice(-1000)
      };
    } catch (error) {
      return { ...base, status: "output-validation-failed", outputWritten: false, deterministicValidationPassed: false, invalidBundleCount: bundleCount, scoringFieldCount: 0, outputError: error.message };
    }
  } finally {
...
function executionRecord(schemaVersion, startedAt, contexts, results) {
  return {
    ...
    invalidBundleCount: results.reduce((sum, item) => sum + item.invalidBundleCount, 0),
...
const initialContexts = ["pass-a", "pass-b"].flatMap((reviewerPass) =>
  manifest.debateNumbers.map((debateNumber) => manifest.initialContexts[reviewerPass][debateNumber])
);
const initialStartedAt = new Date().toISOString();
const initialResults = await pool(initialContexts.map((context) => () => modelTask(context)));
const initialExecution = executionRecord("3.7.3-initial-atomic-execution", initialStartedAt, initialContexts, initialResults);
await writeFile(path.resolve(root, manifest.artifacts.initialExecution), `${JSON.stringify(initialExecution, null, 2)}\n`);

const extraction = await run(process.execPath, ["scripts/extract-v373-atomic-disagreements.mjs", "--write"], { cwd: root, env: process.env });
assert(extraction.code === 0, `initial disagreement extraction failed: ${extraction.stderr}`);
const disagreement = JSON.parse(await readFile(path.resolve(root, manifest.artifacts.initialDisagreements), "utf8"));
let adjudicationExecution;
if (!disagreement.allInitialValid) {
  adjudicationExecution = executionRecord("3.7.3-adjudication-atomic-execution", new Date().toISOString(), [], []);
} else {
  const adjudicationStartedAt = new Date().toISOString();
  const adjudicationResults = await pool(disagreement.adjudicationContexts.map((context) => () => modelTask(context)));
  adjudicationExecution = executionRecord("3.7.3-adjudication-atomic-execution", adjudicationStartedAt, disagreement.adjudicationContexts, adjudicationResults);
}
await writeFile(path.resolve(root, manifest.artifacts.adjudicationExecution), `${JSON.stringify(adjudicationExecution, null, 2)}\n`);

const analysis = await run(process.execPath, ["scripts/analyze-v373-atomic-correction-smoke.mjs", "--write"], { cwd: root, env: process.env });
assert(analysis.code === 0, `correction-smoke analysis failed: ${analysis.stderr}`);
const parsedAnalysis = JSON.parse(await readFile(path.resolve(root, manifest.artifacts.analysis), "utf8"));
console.log(JSON.stringify({
  status: "correction-smoke-execution-recorded",
  passed: parsedAnalysis.passed,
  initialValidContexts: initialExecution.validOutputContexts,
  initialAgreements: disagreement.counts.agreements,
  initialDisagreements: disagreement.counts.disagreements,
  adjudicationContexts: adjudicationExecution.contextsPlanned,
  adjudicationValidContexts: adjudicationExecution.validOutputContexts,
  retries: initialExecution.totalRetries + adjudicationExecution.totalRetries,
  meteredApiCostUsd: 0
}, null, 2));
