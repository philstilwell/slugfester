...
import { POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_ORDER,
  ...
  compilePostCanaryBatch11PublicationStagingRecord } from
...
assertV4(m.status === "frozen-post-canary-batch-11-deterministic-publication-compilation-prepared" &&
  canonicalJson(m.explicitOrder) === canonicalJson(POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_ORDER) &&
  m.contexts?.length === 10 && m.contexts.reduce((sum, row) => sum + row.expectedMoves, 0) === 190 &&
...
"Batch 11 compilation preparation changed");
let moves = 0;
for (const context of m.contexts) {
  ...
  const compiled = compilePostCanaryBatch11PublicationStagingRecord({ output, packet, identity: row });
  const validation = validatePostCanaryBatch11CompiledStagingRecord({ compiled, output, packet, identity: row });
  moves += validation.moves;
}
assertV4(moves === 190, "compiled move replay changed");
