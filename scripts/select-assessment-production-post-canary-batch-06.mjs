...
const PRIOR_SELECTION = "docs/assessment-production/post-canary-continuation-v1/batch-05/selection.json";
const PRIOR_SELECTION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-05/selection-analysis.json";
const PRIOR_MUTATION_MANIFEST = "docs/assessment-production/post-canary-continuation-v1/batch-05/production-publication/mutation-manifest.json";
const PRIOR_COMPLETION_EXECUTION = "docs/assessment-production/post-canary-continuation-v1/batch-05/production-publication/generated-seo-correction/dependent-pilot-analysis-correction/execution.json";
const PRIOR_COMPLETION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-05/production-publication/generated-seo-correction/dependent-pilot-analysis-correction/analysis.json";
const PRIOR_COMPLETION_COMMIT = "e280e9c7240d10f50b70c7881a977c39765dcf41";
const SCRIPT = "scripts/select-assessment-production-post-canary-batch-06.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-06-selection.mjs";
...
assertV4(!(await exists(FUTURE_SOURCE_PREPARATION)), "source preparation already exists before Batch 6 selection");
...
assertV4(
  ...
  "Batch 6 selection is not compatible with the frozen continuation policy"
);
...
assertV4(
  priorSelection.status === "fifth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    priorSelection.batchNumber === 5 &&
    priorSelection.selected.length === 10 &&
    priorSelectionAnalysis.status === "fifth-post-canary-batch-selection-analysis-passed-awaiting-source-packet-preparation-decision" &&
    priorSelectionAnalysis.selection.sha256 === sha256(controlBytes[PRIOR_SELECTION]) &&
    priorMutationManifest.status === "frozen-batch-05-production-publication-mutation-manifest-prepared" &&
    JSON.stringify(priorMutationManifest.explicitOrder) === JSON.stringify(priorSelectedNumbers) &&
    priorCompletionExecution.status === "passed-complete-batch-05-production-transaction-after-dependent-pilot-analysis-correction" &&
    ...
    priorCompletionAnalysis.status === "batch-05-production-publication-accepted-after-dependent-pilot-analysis-correction" &&
  ...
  "Batch 5 has not satisfied the frozen successful-publication prerequisite"
);
assertV4(git("rev-parse", "--abbrev-ref", "HEAD") === "main", "Batch 6 selection must run on main");
assertV4(git("rev-parse", "HEAD") === PRIOR_COMPLETION_COMMIT, "Batch 6 selection checkpoint is not the exact Batch 5 publication commit");
assertV4(git("rev-parse", "origin/main") === PRIOR_COMPLETION_COMMIT, "Batch 5 publication commit is not the pushed origin/main checkpoint");
...
assertV4(
  JSON.stringify(priorSelectedNumbers) === JSON.stringify(ranked.slice(40, 50).map((item) => item.debateNumber)),
  "Batch 5 is not the immutable fifth rank slice"
);

const selected = [];
let totalEvents = 0;
let totalDurationMs = 0;
for (const [offset, rankedItem] of ranked.slice(50, 60).entries()) {
...
assertV4(selected.length === 10 && new Set(selected.map((item) => item.debateNumber)).size === 10, "Batch 6 did not select ten unique debates");

const checkpointCommit = git("rev-parse", "HEAD");
const selection = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-selection",
  protocolId: "assessment-production-post-canary-batch-06",
  status: "sixth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed",
  frozenAt,
  checkpointCommit,
  branch: "main",
  ...
  stagingOnly: true,
  batchNumber: 5,
  productionSequenceOrdinal: 7,
  userAuthorization: {
    instruction: "I authorize creation and use of a Batch 6 standing authorization covering selection and the complete Batch 6 SLUGFESTER reassessment and publication workflow.",
    scopeInterpretation: "Materialize and source-validate Batch 6 under the frozen continuation policy after authenticating successful Batch 5 publication, as the first stage of the separately recorded standing-authorized workflow. This selection pass itself prepares no source packets, executes no models, derives no scores, and performs no production mutation.",
  ...
  eligibility: {
    ...
    previouslyPublishedAndRemovedCount: 50,
    remainingBeforeBatchCount: ranked.length - 50,
    remainingBeforeBatchRankedSha256: sha256(serializedJson(ranked.slice(50))),
    selectedCount: selected.length,
    remainingUnselectedCount: ranked.length - 60,
    dyadicOnly: true,
  ...
  nextAuthorizedAction: "prepare-batch-06-standing-authorization-and-source-packets-under-user-authorization"
};
...
const analysis = {
  ...
  status: "sixth-post-canary-batch-selection-analysis-passed-under-standing-authorization",
  analyzedAt: frozenAt,
  ...
  finding: "Successful Batch 5 production publication was authenticated at the pushed atomic commit. The immutable campaign rank then selected ordinals 51 through 60 as ten unique pending dyadic debates, and all thirty effective source files and ten canonical event chains passed without replacement.",
  decision: {
