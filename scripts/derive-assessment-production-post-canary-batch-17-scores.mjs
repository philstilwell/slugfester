...
import {
  ...
  derivePostCanaryBatch17Scores
} from "./lib/assessment-production-post-canary-batch-17-score-gate.mjs";
import { assertV4 } from "./lib/v4-lean-production.mjs";

const manifestPath = `${POST_CANARY_BATCH_17_SCORE_ROOT}/score-pass-manifest.json`;
...
assertV4(
  ...
  "post-canary Batch 17 score manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen Batch 17 score source hash mismatch`
  );
}
...
assertV4(productionReferences.length === 4, "four diagnostic references required");
const calculated = derivePostCanaryBatch17Scores(
  ledger,
  inputs.debateInputs,
