...
const finalLedgerPath =
  `${POST_CANARY_BATCH_14_FINAL_LEDGER_ROOT}/final-ledger.json`;
...
assertV4(
  ...
  "post-canary Batch 14 final-ledger manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen Batch 14 final-ledger source hash mismatch`
  );
}
const inputs = await loadPostCanaryBatch14FinalLedgerInputs();
const ledger = buildPostCanaryBatch14FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
