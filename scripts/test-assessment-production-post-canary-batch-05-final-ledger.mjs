...
import {
  ...
  buildPostCanaryBatch05FinalLedger,
...
const manifestPath =
  `${POST_CANARY_BATCH_05_FINAL_LEDGER_ROOT}/final-ledger-manifest.json`;
const ledgerPath = `${POST_CANARY_BATCH_05_FINAL_LEDGER_ROOT}/final-ledger.json`;
const analysisPath = `${POST_CANARY_BATCH_05_FINAL_LEDGER_ROOT}/analysis.json`;
const scoreArtifactCandidates = [
  `${POST_CANARY_BATCH_05_FINAL_LEDGER_ROOT}/calculated-scores.json`,
  `${POST_CANARY_BATCH_05_FINAL_LEDGER_ROOT}/score-analysis.json`,
  `${POST_CANARY_BATCH_05_FINAL_LEDGER_ROOT}/score-pass-manifest.json`
];
...
assert.equal(
  ...
  "The frozen Batch 5 standing authorization permits deterministic final-ledger assembly and the single repository score pass while every gate passes."
);
...
const rebuiltLedger = buildPostCanaryBatch05FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
assert.deepEqual(storedLedger, rebuiltLedger);
const validation = validatePostCanaryBatch05FinalLedger(
  storedLedger,
  inputs.debateInputs,
  inputs.sourceHashes
);
...
assert.deepEqual(
  ...
  ["158", "46", "64", "132", "189", "109", "179", "05", "42", "59"]
);
assert.equal(storedLedger.audit.finalMoves, 187);
assert.equal(storedLedger.audit.disputedMoves, 177);
assert.equal(storedLedger.audit.pairSelections, 189);
assert.equal(storedLedger.audit.scoringFieldSelections, 324);
assert.equal(storedLedger.audit.burdenAdjustmentSelections, 14);
assert.equal(storedLedger.audit.candidateSelections, 527);
assert.equal(storedLedger.audit.meanMerges, 411);
assert.equal(storedLedger.audit.dependencyMeanMergesSuppressed, 0);
assert.equal(storedLedger.audit.audioVerifiedMoves, 6);
...
assert.throws(() =>
  buildPostCanaryBatch05FinalLedger(mutatedInputs, inputs.sourceHashes)
);
...
assert.equal(analysis.totals.disputedMoves, 177);
assert.equal(analysis.totals.candidateSelections, 527);
