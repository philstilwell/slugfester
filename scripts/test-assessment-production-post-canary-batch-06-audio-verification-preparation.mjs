...
assert.deepEqual(preparation.scope.debates, ["97"]);
...
assert.equal(preparation.scope.sourceCompatibility.occurrences.length, 1);
assert.equal(preparation.scope.sourceCompatibility.occurrences[0].debateNumber, "141");
...
assert.equal(preparation.costEstimate.clipSeconds, 306.250938);
assert.equal(preparation.costEstimate.clipMinutes, 5.1042);
...
assert.deepEqual(preparation.costEstimate.projectedUsage, {
  audioInputTokens: 5707,
  textInputTokens: 228,
  outputTokens: 9663,
  totalInputTokens: 5935
});
...
assert.equal(
  ...
  "activate-and-execute-two-batch-06-paid-audio-verification-calls-under-standing-authorization"
);
