...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-03/source-preparation";
...
assert.equal(preparation.status, "post-canary-batch-03-ten-complete-score-blind-source-packets-prepared-awaiting-validation");
...
for (const context of preparation.contexts) {
  ...
  assert(frozen, `${context.debateNumber}: absent from frozen Batch 3 selection`);
  ...
  assert.equal(packet.schemaVersion, "1.0-assessment-production-post-canary-batch-03-score-blind-source-packet");
  ...
  for (const chunk of context.chunks) {
    ...
    assert.deepEqual(
      ...
      findBatch03ZeroLexicalTokenRows(chunkBytes).map(({ eventIndex, startMs, durationMs, text }) => ({
        eventIndex,
        startMs,
        durationMs,
...
const validation = {
  ...
  status: "post-canary-batch-03-score-blind-source-packet-validation-passed-frozen-standing-authorization-active",
  ...
  nextAuthorizedAction: "prepare-freeze-and-activate-batch-03-discovery-execution-under-standing-authorization",
};
