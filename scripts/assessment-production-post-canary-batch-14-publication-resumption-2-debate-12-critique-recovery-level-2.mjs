...
const root = `${resumptionRoot}/critique-repair-level-2`;
const levelOneRoot = `${resumptionRoot}/critique-repair`;
const protocolId = "assessment-production-post-canary-batch-14-publication-resumption-2-debate-12-critique-field-disjoint-recovery-level-2";
...
if (mode === "--prepare") {
  ...
  assert.equal(levelOneExecution.validContexts, 7);
  ...
  assert.deepEqual(invalidIds, [
    "con-propositions-versus-reality",
    "con-reality-identity-through-time",
    "con-thinker-capable-of-error"
  ]);
  const contexts = [];
  for (let offset = 0; offset < invalidIds.length; offset += 1) {
    ...
    const repairPacket = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-resumption-2-debate-12-critique-recovery-level-2-packet", protocolId, contextIndex, debateNumber: "12", debateId: packet.debateId, assessmentModel: model.label, writableFields: moveIds.map((moveId) => `moveProse.${moveId}.critique`), writableFieldCount: moveIds.length, targets, allOtherFieldsUnavailableAndImmutable: true, participantJudgmentClosed: true, publicationScoreLocked: true };
    ...
    const properties = { schemaVersion: { type: "string", const: "1.0-assessment-production-post-canary-batch-14-publication-resumption-2-debate-12-critique-recovery-level-2-output" }, protocolId: { type: "string", const: protocolId }, contextIndex: { type: "integer", const: contextIndex }, debateNumber: { type: "string", const: "12" }, debateId: { type: "string", const: packet.debateId }, assessmentModel: { type: "string", const: model.label }, completedAt: { type: "string", minLength: 1 }, corrections: { type: "array", minItems: moveIds.length, maxItems: moveIds.length, items: { anyOf: itemSchemas } } };
    const schema = { $schema: "https://json-schema.org/draft/2020-12/schema", $id: `slugfester-b14-debate12-critique-repair-level-2-${contextIndex}`, type: "object", additionalProperties: false, required: Object.keys(properties), properties };
  ...
  assert.equal(contexts.length, 3); assert.equal(contexts.reduce((sum, item) => sum + item.writableFieldCount, 0), 3); assert(contexts.every((item) => item.writableFieldCount === 1));
  ...
  const manifest = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-critique-recovery-level-2-preparation", protocolId, status: "frozen-three-atomic-critique-batch-14-publication-recovery-level-2-prepared-not-activated", frozenAt: at, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(), productionCanary: false, batchNumber: 14, stagingOnly: true, recoveryLevel: 2, recoveryLevelsMaximum: 2, model, contexts, executionPolicy: { contexts: 3, maximumParallelContexts: 2, attemptsPerContext: 1, retriesMaximum: 0, timeoutExtensionsMaximum: 0, timeoutMsPerContext: 600000, writableFieldsMaximumPerPacket: 1, directIncrementalCostUsdMaximum: 0, paidServices: false, separateActivationRequired: true, APIKeysRemoved: true, removedEnvironmentVariables: ["OPENAI_API_KEY", "OPENAI_ORG_ID", "OPENAI_PROJECT_ID", "OPENAI_BASE_URL", "AZURE_OPENAI_API_KEY", "CODEX_API_KEY"] }, diagnosis: { path: files.levelOneExecution, sha256: sourceHashes[files.levelOneExecution], failedLevelOneShards: 2, unavailableCritiques: 3, acceptedLevelOneCritiques: 14, allOtherFieldsStructurallyValid: true }, sourceHashes, futureOutputs, authorization: { preparation: true, activation: true, modelExecution: false, deterministicMergeAndValidation: false, retries: false, furtherRecoveryLevel: false, scorePass: false, productionMutation: false, nextBatchSelection: false }, nextAuthorizedAction: "activate-three-atomic-debate-12-critique-recovery-level-2-contexts" };
  if (shouldWrite) await writeFile(files.preparation, pretty(manifest));
  console.log(pretty({ status: shouldWrite ? manifest.status : "preview", contexts: 3, writableCritiques: 3, maximumWritableFieldsPerContext: 1, attemptsMaximum: 3, retriesMaximum: 0, recoveryLevel: 2, recoveryLevelsMaximum: 2, directIncrementalCostUsdMaximum: 0, modelExecutionAuthorized: false })); process.exit(0);
}

if (mode === "--activate") {
  const bytes = await readFile(files.preparation); const preparation = JSON.parse(bytes);
  assert.equal(preparation.status, "frozen-three-atomic-critique-batch-14-publication-recovery-level-2-prepared-not-activated"); assert(!(await exists(files.activation)), "activation exists");
  ...
  const activation = { ...preparation, schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-critique-recovery-level-2-activation", status: "frozen-three-atomic-critique-batch-14-publication-recovery-level-2-authorized", activatedAt: at, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(), preparation: { path: files.preparation, sha256: sha256(bytes) }, authorization: { ...preparation.authorization, modelExecution: true, deterministicMergeAndValidation: true }, nextAuthorizedAction: "execute-three-atomic-debate-12-critique-recovery-level-2-contexts-once" };
  if (shouldWrite) await writeFile(files.activation, pretty(activation));
  console.log(pretty({ status: shouldWrite ? activation.status : "preview", contexts: 3, writableCritiques: 3, recoveryLevel: 2, attemptsMaximum: 3, retriesMaximum: 0, directIncrementalCostUsdMaximum: 0, modelExecutionAuthorized: shouldWrite })); process.exit(0);
}

if (mode === "--run") {
  const activation = await readJson(files.activation); assert.equal(activation.status, "frozen-three-atomic-critique-batch-14-publication-recovery-level-2-authorized");
  ...
  const passed = results.every((item) => item.gateAcceptancePassed); const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-critique-recovery-level-2-execution", protocolId, status: passed ? "three-atomic-critique-batch-14-publication-recovery-level-2-execution-passed" : "batch-14-publication-critique-recovery-level-2-execution-failed", startedAt, completedAt: new Date().toISOString(), elapsedMs: Date.now() - started, contextsAttempted: 3, validContexts: results.filter((item) => item.gateAcceptancePassed).length, invalidContexts: results.filter((item) => !item.gateAcceptancePassed).length, attempts: 3, retries: 0, timeoutExtensions: 0, directIncrementalCostUsd: 0, paidServiceCalls: 0, modelAuthoredScores: 0, results, authorization: { analysis: passed, retries: false, furtherRecoveryLevel: false, paidServices: false, productionMutation: false, nextBatchSelection: false } };
  await writeFile(files.execution, pretty(execution)); console.log(pretty({ status: execution.status, contextsAttempted: 3, validContexts: execution.validContexts, invalidContexts: execution.invalidContexts, elapsedMinutes: Number((execution.elapsedMs / 60000).toFixed(2)), attempts: 3, retries: 0, recoveryLevel: 2, directIncrementalCostUsd: 0 })); process.exit(0);
}
...
if (mode === "--analyze") {
  ...
  assert.equal(execution.status, "three-atomic-critique-batch-14-publication-recovery-level-2-execution-passed");
  ...
  const corrections = [...levelOneOutputs, ...levelTwoOutputs].flatMap((output) => output.corrections); assert.equal(corrections.length, 17); assert.equal(new Set(corrections.map((item) => item.moveId)).size, 17);
  ...
  const provenance = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-debate-12-recovery-provenance", protocolId, debateNumber: "12", originalFailure: { status: "output-validation-failed", originalOutputPreserved: true }, recoveryLevel1: { contexts: 9, acceptedContexts: [0, 1, 3, 4, 5, 6, 7], acceptedCritiques: 14, failedContextIndexes: [2, 8] }, recoveryLevel2: { contexts: 3, acceptedCritiques: 3, eachUnavailableFieldAcceptedExactlyOnce: true }, merge: { acceptedCompanionFieldsChanged: false, scoresChanged: false, scorePassRerun: false, modelAuthoredScores: 0, completeValidationPassed: true } };
  const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-resumption-2-debate-12-critique-recovery-level-2-analysis", protocolId, status: "debate-12-two-level-field-disjoint-publication-recovery-passed-awaiting-five-context-resumption", recoveryLevel: 2, recoveryLevelsMaximum: 2, debateNumber: "12", moves: 18, critiques: 18, contextsThisLevel: 3, attemptsThisLevel: 3, attemptsAcrossRecoveryLevels: 12, retries: 0, timeoutExtensions: 0, directIncrementalCostUsd: 0, paidServiceCalls: 0, modelAuthoredScores: 0, validation, output: { path: files.mergedOutput, sha256: sha256(mergedText) }, authorization: { fiveOriginalUnattemptedContextResumption: true, furtherRecoveryLevel: false, deterministicCompilation: false, productionMutation: false, nextBatchSelection: false }, nextAuthorizedAction: "resume-five-original-unattempted-publication-contexts" };
  ...
  console.log(pretty({ status: analysis.status, debateNumber: "12", moves: validation.moves, critiques: validation.critiques, minimumCritiqueCharacters: validation.minimumCritiqueCharacters, exactSourceQuotes: validation.quoteExactSourceMatches, contextsThisLevel: 3, attemptsThisLevel: 3, attemptsAcrossRecoveryLevels: 12, retries: 0, directIncrementalCostUsd: 0, nextAuthorizedAction: analysis.nextAuthorizedAction }));
}
