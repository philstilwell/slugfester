...
const protocolId = "assessment-production-post-canary-batch-17-publication-initial-context-continuation";
...
if (mode === "--prepare") {
  const [originalActivation, originalExecution, originalAnalysis] = await Promise.all([
    files.originalActivation, files.originalExecution, files.originalAnalysis
  ].map(readJson));
  assert.equal(originalExecution.status, "post-canary-batch-17-publication-gate-complete-with-failure");
  assert.equal(originalAnalysis.status, "post-canary-batch-17-publication-output-gate-failed");
  assert.deepEqual(originalExecution.unattemptedContextIndexes, [3]);
  const contexts = originalActivation.contexts.filter((item) => item.contextIndex === 3);
  assert.deepEqual(contexts.map((item) => item.contextIndex), [3]);
  ...
  const sourceFiles = [
    ...
    "scripts/assessment-production-post-canary-batch-17-publication-continuation.mjs"
  ];
  const sourceHashes = {};
  ...
  const manifest = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-17-publication-continuation-preparation",
    protocolId,
    status: "frozen-one-unattempted-batch-17-publication-context-prepared-not-activated",
    frozenAt: at,
    checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(),
    productionCanary: false,
    batchNumber: 17,
    stagingOnly: true,
    preservedInitialGate: { validDebates: ["77", "44"], failedDebate: "171", unattemptedDebate: "62", originalEvidenceChanged: false },
    ...
    executionPolicy: {
      ...
      schedulerRamp: [1],
      rampPhases: [
        { phase: "single-original-attempt", maximumParallelContexts: 1, contextIndexes: [3], expansionRequiresAllValid: true }
      ],
    ...
    nextAuthorizedAction: "activate-one-previously-unattempted-batch-17-publication-context"
  };
  if (shouldWrite) await writeFile(path.resolve(files.preparation), pretty(manifest));
  console.log(pretty({ status: shouldWrite ? manifest.status : "preview", debates: contexts.map((item) => item.debateNumber), contexts: 1, attemptsMaximum: 1, retriesMaximum: 0, maximumParallelContexts: 1, directIncrementalCostUsdMaximum: 0, modelExecutionAuthorized: false }));
  process.exit(0);
}
...
if (mode === "--activate") {
  ...
  assert.equal(preparation.status, "frozen-one-unattempted-batch-17-publication-context-prepared-not-activated");
  ...
  const activation = { ...preparation, schemaVersion: "1.0-assessment-production-post-canary-batch-17-publication-continuation-activation", status: "frozen-one-unattempted-batch-17-publication-context-authorized", activatedAt: at, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(), preparation: { path: files.preparation, sha256: sha256(preparationBytes) }, authorization: { ...preparation.authorization, modelExecution: true, deterministicAnalysis: true }, nextAuthorizedAction: "execute-one-previously-unattempted-batch-17-publication-context-once" };
  if (shouldWrite) await writeFile(path.resolve(files.activation), pretty(activation));
  console.log(pretty({ status: shouldWrite ? activation.status : "preview", debates: activation.contexts.map((item) => item.debateNumber), contexts: 1, attemptsMaximum: 1, retriesMaximum: 0, directIncrementalCostUsdMaximum: 0, modelExecutionAuthorized: shouldWrite }));
  process.exit(0);
}

if (mode === "--run") {
  const activation = await readJson(files.activation);
  assert.equal(activation.status, "frozen-one-unattempted-batch-17-publication-context-authorized");
  ...
  const execute = async (context) => {
    const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-17-publication-continuation-${context.debateNumber}-`));
    const temporaryHome = await mkdtemp(path.join(os.tmpdir(), `batch-17-publication-continuation-home-${context.debateNumber}-`));
    ...
    try {
      ...
      console.error(`[batch-17-publication-continuation] starting index ${context.contextIndex} Debate ${context.debateNumber}`);
      ...
      try { validationSummary = validatePostCanaryBatch17PublicationOutput(JSON.parse(resultBytes), JSON.parse(await readFile(path.resolve(context.packet), "utf8"))); } catch (error) { validationMessage = String(error?.stack ?? error).slice(-10000); }
      const accepted = validationSummary?.status === "passed";
      const validationRecord = { schemaVersion: "1.0-assessment-production-post-canary-batch-17-publication-validation", protocolId: "assessment-production-post-canary-batch-17-publication-reconstruction", status: accepted ? "passed" : "failed", debateNumber: context.debateNumber, debateId: context.debateId, outputSha256: sha256(resultBytes), validationSummary, validationMessage, modelAuthoredScores: 0, lockedScoresUnchanged: accepted ? true : null };
      const provenance = { schemaVersion: "1.0-assessment-production-post-canary-batch-17-publication-continuation-provenance", protocolId, debateNumber: context.debateNumber, debateId: context.debateId, model: activation.model, authentication: "ChatGPT subscription", reasoningEffort: activation.model.reasoningEffort, attemptCount: 1, retryCount: 0, timeoutExtensionCount: 0, correctionContextCount: 0, apiKeysRemoved: true, isolatedTemporaryCodexHome: true, isolatedTemporaryWorkingDirectory: true, participantJudgmentWasScoreBlind: true, ownDebateScoresImmutable: true, outputSha256: sha256(resultBytes), modelAuthoredScores: 0, scorePassesExecutedThisStage: 0, meteredApiCostUsd: 0, paidServiceCallsThisStage: 0 };
  ...
  const runPool = async (indexes) => {
    ...
    await Promise.all(Array.from({ length: Math.min(1, indexes.length) }, worker));
  ...
  const passed = results.length === 1 && validContexts === 1;
  const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-17-publication-continuation-execution", protocolId, status: passed ? "one-unattempted-batch-17-publication-context-passed" : "batch-17-publication-continuation-complete-with-failure", startedAt, completedAt: new Date().toISOString(), elapsedMs: Date.now() - started, contextsPlanned: 1, contextsAttempted: results.length, contextsUnattempted: unattemptedContextIndexes.length, unattemptedContextIndexes, validContexts, invalidContexts: results.length - validContexts, attempts: results.length, retries: 0, timeoutExtensions: 0, correctionContexts: 0, maximumObservedConcurrency, schedulerRamp: [1], rampPhases, results, participantJudgmentWasScoreBlind: true, ownDebateScoresImmutable: true, meteredApiCostUsd: 0, paidServiceCallsThisStage: 0, modelAuthoredScores: 0, scorePassesExecutedThisStage: 0, authorization: { deterministicAnalysis: true, recoveryPreparation: !passed, retry: false, timeoutExtension: false, scorePass: false, publicationFinalization: false, productionMutation: false, nextBatchSelection: false } };
...
if (mode === "--analyze") {
  ...
  for (const result of execution.results) {
    ...
    const validation = validatePostCanaryBatch17PublicationOutput(await readJson(context.rawOutput), await readJson(context.packet));
  ...
  const passed = execution.status === "one-unattempted-batch-17-publication-context-passed" && valid.length === 1;
  const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-17-publication-continuation-analysis", protocolId, status: passed ? "batch-17-publication-continuation-gate-passed" : "batch-17-publication-continuation-gate-failed", batchNumber: 17, productionCanary: false, stagingOnly: true, execution: { contextsPlanned: 1, contextsAttempted: execution.contextsAttempted, contextsUnattempted: execution.contextsUnattempted, validContexts: execution.validContexts, invalidContexts: execution.invalidContexts, attempts: execution.attempts, retries: 0, timeoutExtensions: 0, maximumObservedConcurrency: execution.maximumObservedConcurrency }, validationReplay, totals: { debates: 1, critiques: valid.reduce((sum, item) => sum + item.validation.critiques, 0), exactSourceQuotes: valid.reduce((sum, item) => sum + item.validation.quoteExactSourceMatches, 0), overallCommentarySides: valid.reduce((sum, item) => sum + item.validation.overallCommentarySides, 0), aiExtensionSides: valid.reduce((sum, item) => sum + item.validation.aiExtensionSides, 0), modelContexts: execution.contextsAttempted, modelAuthoredScores: 0, scorePassesExecutedThisStage: 0, paidServiceCalls: 0, directIncrementalCostUsd: 0 }, integrity: { preservedInitialGateRetained: true, scoresRemainedImmutable: true, everyAcceptedOutputReplayedDeterministically: valid.every((item) => item.validationReplayed), retriesPerformed: false, timeoutExtensionsPerformed: false, productionMutated: false }, authorization: { failureDiagnosis: true, recoveryPacketPreparation: true, deterministicCohortFinalization: false, scorePass: false, publicationFinalization: false, productionMutation: false, nextBatchSelection: false }, nextAuthorizedAction: passed ? "recover-only-the-one-failed-batch-17-publication-critique-field" : "diagnose-and-recover-only-the-failed-or-unattempted-publication-fields" };
