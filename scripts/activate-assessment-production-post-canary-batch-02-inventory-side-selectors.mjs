...
const SCRIPT =
  "scripts/activate-assessment-production-post-canary-batch-02-inventory-side-selectors.mjs";
const RUNNER =
  "scripts/run-assessment-production-post-canary-batch-02-inventory-side-selectors.mjs";
const ANALYZER =
  "scripts/analyze-assessment-production-post-canary-batch-02-inventory-side-selectors.mjs";
...
assertV4(
  sidePacketPreparation.status ===
    ...
    JSON.stringify(sidePacketPreparation.sourceCompatibility) ===
      JSON.stringify(preparation.sourceCompatibility),
  "Batch 2 side-packet preparation drifted"
);
...
const activation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-side-selector-execution-activation",
  ...
  userAuthorization: {
    ...
    approvedScope:
      "Activate and execute exactly the twenty frozen Batch 2 score-blind side-selector contexts using 5.6 Sol with low reasoning effort through the ChatGPT subscription, with a direct incremental cost cap of $0. Use the frozen 1→2 scheduler, one attempt per context, no retries, and no timeout extensions, then stop after deterministic side validation, inventory compilation, analysis, commit, and push. Do not run judgment models, use paid services, perform audio work, derive scores, reconstruct publication, mutate production, or select the next batch.",
  ...
  nextRequiredAction:
    "execute-exactly-twenty-frozen-post-canary-batch-02-side-selector-contexts-once",
};
