...
import {
  POST_CANARY_BATCH_07_DISPUTE_ADJ_ISOLATION,
  POST_CANARY_BATCH_07_DISPUTE_ADJ_OUTPUT_VERSION,
...
const EXPECTED_DEBATES = ["193", "80", "121", "100", "78", "113", "180", "02", "182", "56"];
...
const preparation = JSON.parse(
  await readFile(`${POST_CANARY_BATCH_07_DISPUTE_ADJ_ROOT}/preparation-manifest.json`, "utf8")
);
...
assert.equal(preparation.batchNumber, 7);
...
assert.equal(preparation.totals.disputedMoves, 180);
assert.equal(preparation.totals.candidateSelections, 511);
...
assert.equal(preparation.acceptedSourceBoundary.allFiveAudioMovesVerified, true);
...
for (const context of preparation.contexts) {
  ...
  assert.equal(packet.batchNumber, 7);
  ...
  const output = {
    schemaVersion: POST_CANARY_BATCH_07_DISPUTE_ADJ_OUTPUT_VERSION,
    ...
    isolation: structuredClone(POST_CANARY_BATCH_07_DISPUTE_ADJ_ISOLATION),
  ...
  const validation = validatePostCanaryBatch07DisputeAdjudicationOutput(output, packet);
...
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_07_DISPUTE_ADJ_ROOT}/packets`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_07_DISPUTE_ADJ_ROOT}/provenance`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
