...
const root = POST_CANARY_BATCH_07_COMPATIBILITY_ROOT;
...
const validator = await import(
  `./validate-debates.mjs?batch07Compatibility=${activation.validator.proposedSha256}`
);
...
for (const lock of activation.packetHashes) {
  ...
  const routeLock =
    validator.validatePostCanaryBatch07LedgerAdapterRouteLocks({
      debate: candidate,
      ledgerText,
  ...
  const route = validator.validatePostCanaryBatch07LedgerAdapterRoute({
    debate: candidate,
    ledger,
    ledgerText
  });
  assert.equal(route.repositoryScoreReplayPassed, true);
  const adapter = validatePostCanaryBatch07SiteLedgerAdapter({
    adapter: ledger,
    candidate,
...
assert.equal(emptyReferenceLinks, 53);
