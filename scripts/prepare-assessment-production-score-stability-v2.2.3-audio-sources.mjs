...
const planRoot =
  "docs/assessment-production/score-stability-v2.2.3-validation-cohort/disagreement-extraction";
const localRoot =
...
const TOOL_SOURCES = [
  "scripts/prepare-assessment-production-score-stability-v2.2.3-audio-sources.mjs",
  "scripts/test-assessment-production-score-stability-v2.2.3-audio-sources.mjs"
];
...
assertV4(
  ...
  "v2.2.3 local audio-source preparation is not authorized"
);
assertV4(
  work.status === "prepared-four-v2.2.3-local-audio-source-work-items" &&
    work.moves.length === 4 &&
    work.authorization.localAudioSourcePreparation,
  "v2.2.3 audio work-item population changed"
);
assertV4(
  sha256(workBytes) === workPreparation.workArtifact.sha256,
  "v2.2.3 audio work-item hash changed"
);
...
for (const file of TOOL_SOURCES) toolSourceHashes[file] = sha256(await readFile(file));
...
assertV4(grouped.size === 2, "expected exactly two v2.2.3 audio sources");
...
if (!shouldWrite) {
  console.log(
    JSON.stringify(
      {
        status: "preview-two-v2.2.3-local-audio-sources-four-clips",
...
const preparation = {
  schemaVersion: "1.0-score-stability-v2.2.3-audio-source-preparation",
  protocolId: work.protocolId,
  status: "prepared-four-v2.2.3-local-audio-clips",
