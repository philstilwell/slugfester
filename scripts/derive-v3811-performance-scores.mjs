...
import { V3811_PERFORMANCE_DEBATES, V3811_PERFORMANCE_ROOT, assertV3811, validateV3811PerformanceOutput } from "./lib/v3811-performance-judgment.mjs";

const root = process.cwd();
const initialOutputsRoot = `${V3811_PERFORMANCE_ROOT}/initial-outputs`;
const outputPath = `${V3811_PERFORMANCE_ROOT}/calculated-scores.json`;
...
function moveScore(ratings) {
  const calibrationCharity = combineCalibrationCharity({ epistemicCalibration: ratings.epistemicCalibration, representationalCharity: ratings.representationalCharity }, "v3.8.11 calibration/charity");
  const score = scoreDimensions({ logicalCoherence: ratings.logicalCoherence, evidenceWarrant: ratings.evidenceWarrant, responsiveness: ratings.responsiveness, relevanceBurden: ratings.relevanceBurden, precisionClarity: ratings.precisionClarity, calibrationCharity }, "v3.8.11 final move dimensions");
...
function weightedSectionScore(moveRecords, scoreKey) {
  ...
  assertV3811(moveRecords.length > 0 && importanceTotal > 0, "every side requires at least one move per section");
...
const finalLedger = await readJson(`${V3811_PERFORMANCE_ROOT}/final-ledger.json`);
const ledgerValidation = await readJson(`${V3811_PERFORMANCE_ROOT}/final-ledger-validation.json`);
const finalLedgerBytes = await readFile(path.resolve(root, `${V3811_PERFORMANCE_ROOT}/final-ledger.json`));
assertV3811(finalLedger.status === "assembled-pending-independent-ledger-validation" && finalLedger.authorization.independentLedgerValidation && !finalLedger.authorization.scoreDerivation, "final ledger boundary invalid");
assertV3811(ledgerValidation.status === "passed" && ledgerValidation.finalLedgerValidated && ledgerValidation.authorization.scoreDerivation && !ledgerValidation.authorization.assessmentProse && ledgerValidation.ledgerSha256 === createHash("sha256").update(finalLedgerBytes).digest("hex"), "independent final-ledger validation missing or stale");
const debates = [];
const passATotals = [];
const passBTotals = [];
...
for (const debateNumber of V3811_PERFORMANCE_DEBATES) {
  ...
  const packet = await readJson(`${V3811_PERFORMANCE_ROOT}/packets/debate-${debateNumber}.json`);
  const passA = await readJson(`${initialOutputsRoot}/debate-${debateNumber}-pass-a.json`);
  const passB = await readJson(`${initialOutputsRoot}/debate-${debateNumber}-pass-b.json`);
...
const output = {
  schemaVersion: "3.8.11-performance-calculated-scores",
