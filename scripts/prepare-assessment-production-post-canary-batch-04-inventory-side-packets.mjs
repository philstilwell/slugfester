...
const BATCH_SOURCE_PREPARATION =
  "docs/assessment-production/post-canary-continuation-v1/batch-04/source-preparation/preparation-manifest.json";
...
const PLAN_RECOVERY_ANALYSIS = `${ROOT}/plan-recovery-1/debate-49/analysis.json`;
...
const SCRIPT =
  "scripts/prepare-assessment-production-post-canary-batch-04-inventory-side-packets.mjs";
const PLAN_RUNNER =
...
function makeSidePacket({
  ...
  return {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-04-chronology-fallback-side-selector-packet",
...
async function buildArtifacts({ frozenAt, checkpointCommit }) {
  ...
  assertV4(
    sourcePreparation.schemaVersion ===
      ...
      sourcePreparation.totals.proCandidates === 193 &&
      sourcePreparation.totals.conCandidates === 211 &&
    ...
    "Batch 4 source preparation boundary drifted"
  );
  ...
  assertV4(
      ...
      planExecution.results.find((result) => result.debateNumber === "49")
        ?.status === "timed-out" &&
      planExecution.results.find((result) => result.debateNumber === "49")
        ?.accepted === false &&
      planExecution.results.find((result) => result.debateNumber === "49")
      ...
      planAnalysis.audit.recoveredDebate === "49" &&
      ...
      planAnalysis.totals.originalPlanContextsAttempted === 10 &&
      planAnalysis.totals.recoveryShardContextsAttempted === 2 &&
      planAnalysis.totals.acceptedPlans === 10 &&
      planAnalysis.totals.modelContextsExecuted === 12 &&
      planAnalysis.totals.retries === 0 &&
      planAnalysis.totals.timeoutExtensions === 0 &&
      planAnalysis.totals.directIncrementalCostUsd === 0 &&
    ...
    "accepted Batch 4 planner gate drifted"
  );
  assertV4(
    isDeepStrictEqual(
    ...
    "Batch 4 source compatibility boundary drifted"
  );
  ...
  const standingScope =
    "prepare, freeze, activate, and execute exactly twenty Batch 4 score-blind side-selector contexts under the frozen standing authorization";
  const sidePreparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-04-chronology-fallback-side-packet-preparation",
    protocolId: PROTOCOL_ID,
    sideSelectionProtocolId: CHRONOLOGY_FALLBACK_INVENTORY.protocolId,
    status:
      "twenty-exact-post-canary-batch-04-side-selector-packets-frozen-not-authorized",
    frozenAt,
    checkpointCommit,
    branch: "main",
    ...
    plannerGate: {
      ...
      originalAcceptedPlans: 9,
      recoveredPlans: 1,
      recoveredDebate: "49",
      recoveryAnalysis: PLAN_RECOVERY_ANALYSIS,
      recoveryAnalysisSha256: sha256(await readFile(PLAN_RECOVERY_ANALYSIS)),
      minimumFieldDisjointRecoveryShards: 2,
    ...
    nextRequiredAction:
      "freeze-batch-04-side-selector-execution-preparation-manifest-model-free-only",
  };
  ...
  const executionPreparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-04-side-selector-execution-preparation-manifest",
    protocolId: PROTOCOL_ID,
    sideSelectionProtocolId: CHRONOLOGY_FALLBACK_INVENTORY.protocolId,
    status:
      "frozen-twenty-post-canary-batch-04-side-selector-contexts-prepared-not-authorized",
    frozenAt,
    checkpointCommit,
    branch: "main",
    productionContinuation: true,
    ...
    costEstimate: {
      ...
      estimateBasis:
        "The promoted twenty-context side-selector gate completed in 10.80 wall minutes and 19.74 aggregate model minutes at concurrency two. Batch 4 preserves the same one-side isolated context shape; this manifest records the exact observed packet sizes against the proven 115,000-byte input ceiling. ChatGPT-subscription execution has no direct incremental API charge.",
    },
    ...
    nextAuthorizedAction:
      "freeze-and-activate-batch-04-side-selector-execution-under-standing-authorization",
  };
...
if (command === "prepare") await prepare();
  ...
  throw new Error(
    "usage: prepare-assessment-production-post-canary-batch-04-inventory-side-packets.mjs <prepare|validate>"
  );
}
