...
const ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-10/independent-judgments";
...
assertV4(
  activation.status ===
      "frozen-twenty-post-canary-batch-10-independent-judgment-contexts-authorized" &&
  ...
  "Batch 10 independent-judgment execution is unauthorized"
);
assertV4(
...
async function executeContext(context, contextIndex) {
  ...
  assertV4(
    remainingGateMs > 0,
    "absolute Batch 10 independent-judgment gate timeout reached before launch"
  );
  ...
  const temporary = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-batch-10-judgment-${context.debateNumber}-${context.reviewerPass.toLowerCase()}-`
    )
  );
  const isolatedCodexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-batch-10-judgment-home-${context.debateNumber}-${context.reviewerPass.toLowerCase()}-`
    )
  );
  ...
  try {
    ...
    if (
      ...
      const validation = await run(
        process.execPath,
        [
          "scripts/validate-assessment-production-post-canary-batch-10-independent-judgment.mjs",
  ...
  process.stdout.write(
    `[batch-10-judgment] Debate ${context.debateNumber} Pass ${context.reviewerPass} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-10-independent-judgment-model-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "twenty-post-canary-batch-10-independent-judgment-contexts-passed"
    : "post-canary-batch-10-independent-judgment-gate-complete-with-failure",
