...
import { POST_CANARY_BATCH_02_SCORE_ROOT } from "./lib/assessment-production-post-canary-batch-02-score-gate.mjs";
...
assertV4(
  ...
  "post-canary Batch 2 score-pass activation is not prepared"
);
...
const manifest = {
  ...preparation,
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-single-score-pass-execution-manifest",
  status:
  ...
  userExecutionAuthorization: {
    instruction:
      "I approve activation and execution of the one frozen Batch 2 deterministic score pass, with a direct incremental cost cap of $0. Use exactly one repository score pass and no reruns. Stop after deterministic score validation, stability analysis, committing, and pushing. Do not execute models, use paid services, prepare publication packets, reconstruct publication, mutate production, or select the next batch.",
  ...
  nextAuthorizedAction:
    "execute-single-deterministic-post-canary-batch-02-score-pass-once"
};
if (shouldWrite) {
  await writeFile(
