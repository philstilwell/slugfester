...
assert.equal(preparation.protocolId, POST_CANARY_BATCH_03_RENDERING_PROTOCOL_ID);
assert.equal(
  preparation.status,
...
assert.deepEqual(preparation.explicitOrder, [...POST_CANARY_BATCH_03_RENDERING_ORDER]);
...
assert.equal(preparation.preservedControls.integerRoundedTiePolicyPreserved, true);
...
assert.equal(
  ...
  canonicalJson(POST_CANARY_BATCH_03_RENDERING_VIEWPORTS)
);
assert.equal(
...
assert.equal(preparation.browserPlan.localServer.port, POST_CANARY_BATCH_03_RENDERING_PORT);
...
assert.equal(preparation.gateExpectations.sections, 53);
assert.equal(preparation.gateExpectations.moves, 200);
...
assert.equal(
  preparation.toolHashes[POST_CANARY_BATCH_03_RENDERING_IMAGE_CONTRACT.analyzerPath],
...
for (const row of preparation.packets) {
  ...
  const packet = validatePostCanaryBatch03RenderingPacket(JSON.parse(bytes));
  ...
  assert.equal(packet.preview.url.includes(`127.0.0.1:${POST_CANARY_BATCH_03_RENDERING_PORT}`), true);
...
assert.equal(sectionTotal, 53);
assert.equal(moveTotal, 200);

const previewHtml = await readFile(path.resolve(preparation.inputs.localPreview), "utf8");
assert.match(previewHtml, /validated post-canary Batch 3 candidate only/);
...
console.log(
  JSON.stringify(
    {
      status: "post-canary-batch-03-rendering-preparation-passed",
