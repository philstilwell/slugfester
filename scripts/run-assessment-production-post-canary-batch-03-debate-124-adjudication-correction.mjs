...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-post-canary-batch-03-debate-124-adjudication-correction-${context.debateNumber}-`
    )
  );
  const codexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-post-canary-batch-03-debate-124-adjudication-correction-home-${context.debateNumber}-`
    )
  );
  ...
  try {
    ...
    process.stdout.write(
      `[batch-03-debate-124-correction] starting index ${context.contextIndex} ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
  ...
  process.stdout.write(
    `[batch-03-debate-124-correction] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
