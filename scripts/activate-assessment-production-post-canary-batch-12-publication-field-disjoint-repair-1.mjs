...
assertV4(
  preparation.protocolId === PROTOCOL_ID &&
    preparation.status === "frozen-two-context-three-field-batch-12-publication-repair-prepared-not-activated" &&
    preparation.contexts?.length === 2 && preparation.totals?.writableFields === 3 &&
    preparation.diagnosis?.totals?.invalidCritiques === 3 &&
    preparation.diagnosis?.totals?.invalidQuotes === 0 &&
    preparation.contexts.every((context) => context.writableFieldCount <= 2) &&
    new Set(preparation.contexts.flatMap((context) => context.writableFields)).size === 3,
  "two-shard repair is not prepared"
);
assertV4(
  ...
  "two-shard model or execution controls changed"
);
...
const activation = {
  ...
  status: "frozen-two-context-three-field-batch-12-publication-repair-activated",
  activatedAt,
  ...
  nextAuthorizedAction: "execute-only-two-frozen-field-disjoint-repair-contexts"
};
