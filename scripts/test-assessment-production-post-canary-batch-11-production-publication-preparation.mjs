...
const analysisPath = `${POST_CANARY_BATCH_11_PRODUCTION_PUBLICATION_ROOT}/preparation-analysis.json`;
const [manifest, analysis] = await Promise.all([readJson(manifestPath), readJson(analysisPath)]);

assert.equal(manifest.protocolId, POST_CANARY_BATCH_11_PRODUCTION_PUBLICATION_PROTOCOL_ID);
assert.equal(manifest.status, "frozen-batch-11-production-publication-mutation-manifest-prepared");
...
assert.deepEqual(manifest.explicitOrder, POST_CANARY_BATCH_11_PRODUCTION_PUBLICATION_ORDER);
...
assert.equal(manifest.productionBaseline.existingProductionLedgers.count, 111);
assert.equal(manifest.productionBaseline.batchProductionLedgersAbsent, 9);
assert.equal(manifest.productionBaseline.batchProductionLedgersPresent, 1);
...
assert.equal(analysis.status, "batch-11-production-publication-mutation-manifest-freeze-passed");
...
assert.equal(await exists(`${POST_CANARY_BATCH_11_PRODUCTION_PUBLICATION_ROOT}/execution-activation.json`), false);
assert.equal(await exists(`${POST_CANARY_BATCH_11_PRODUCTION_PUBLICATION_ROOT}/execution.json`), false);
