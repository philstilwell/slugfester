...
assertV4(
  ...
  "publication execution is not authorized"
);
assertV4(
  ...
  "publication execution controls changed"
);
...
assertV4(
  activation.executionEnvironment?.codexCliVersion ===
    (await new Promise((resolve, reject) => {
      const child = spawn(activation.executionEnvironment.codexPath, ["--version"], {
...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), `checkpoint-v22-publication-${context.debateNumber}-`)
  );
  const codexHome = await mkdtemp(
    path.join(os.tmpdir(), `checkpoint-v22-publication-home-${context.debateNumber}-`)
  );
  ...
  try {
    ...
    process.stdout.write(
      `[checkpoint-v2.2-publication] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
    ...
    if (invocation.timedOut || invocation.code !== 0 || invocation.signal !== null || !resultExists) {
      ...
      const validationRecord = {
        schemaVersion: "1.0-production-checkpoint-v2.2-publication-validation",
      ...
      const provenanceRecord = {
        schemaVersion: "1.0-production-checkpoint-v2.2-publication-provenance",
        protocolId: activation.protocolId,
        debateNumber: context.debateNumber,
        debateId: context.debateId,
  ...
  process.stdout.write(
    `[checkpoint-v2.2-publication] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion: "1.0-production-checkpoint-v2.2-publication-model-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "ten-production-checkpoint-v2.2-publication-contexts-passed"
