...
assertV4(write, "Batch 16 compatibility runner requires --write");
...
const paths = {
  root: POST_CANARY_BATCH_16_COMPATIBILITY_ROOT,
...
assertV4(
  ...
  "Batch 16 compatibility pass was already attempted; rerun forbidden"
);
...
assertV4(
  ...
  "frozen Batch 16 compatibility activation is unavailable"
);
...
tests.push(
  runCommand(
    "batch-16-route-positive-negative-controls",
    process.execPath,
...
if (tests.at(-1).passed) {
  tests.push(
    runCommand("checkpoint-batch-01-through-batch-15-legacy-reference-regression", process.execPath, [
...
const nextAuthorizedAction = compatibilityAcceptancePassed
  ? "prepare-batch-16-production-publication-mutation-manifest-under-standing-authorization"
  : "diagnose-batch-16-compatibility-failure-under-failure-recovery-standing-authorization";
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-16-compatibility-execution",
  protocolId: preparation.protocolId,
  status: compatibilityAcceptancePassed
    ? "batch-16-route-score-checkpoint-batch-01-through-batch-15-legacy-reference-validation-passed"
    : "failed-closed-batch-16-compatibility-staging-pass",
  startedAt,
  completedAt,
  activation: {
  ...
  regressions: {
    batch16RouteControlsPassed: tests[0]?.passed === true,
    checkpointBatch01ThroughBatch15LegacyReferencePassed:
      tests[1]?.passed === true,
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-16-compatibility-execution-analysis",
  protocolId: preparation.protocolId,
  status: compatibilityAcceptancePassed
    ? "batch-16-production-compatibility-passed"
    : "batch-16-production-compatibility-failed-closed",
  ...
  checks: {
    ...
    routeAndScoreControlsPassed: execution.regressions.batch16RouteControlsPassed,
    checkpointBatch01ThroughBatch15LegacyReferencePassed:
      execution.regressions.checkpointBatch01ThroughBatch15LegacyReferencePassed,
