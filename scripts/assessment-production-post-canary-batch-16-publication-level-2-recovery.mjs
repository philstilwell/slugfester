...
const protocolId = "assessment-production-post-canary-batch-16-publication-level-2-one-field-recovery";
...
if (mode === "--prepare") {
  ...
  assert.equal(levelOneAnalysis.status, "batch-16-publication-level-1-recovery-gate-failed");
  ...
  assert.deepEqual(failedMoveIds, [
  ...
  const contexts = failedMoveIds.map((moveId, contextIndex) => {
    ...
    const shardId = `debate-16-${moveId}-critique-recovery-2`;
    const repairPacket = {
    ...
    return { contextIndex, shardId, debateNumber: "16", debateId: packet.debateId, moveId, writableField: `moveProse.${moveId}.critique`, packet: packetPath, packetSha256: sha256(pretty(repairPacket)), schema: schemaPath, schemaSha256: sha256(pretty(schema)), output: outputPath, repairPacket, schemaDocument: schema };
  });
  assert.equal(new Set(contexts.map((item) => item.writableField)).size, 2);
  ...
  assert.equal(successfulLevelOneOutputs.length, 7);
  const sourceFiles = [
    ...
    "scripts/assessment-production-post-canary-batch-16-publication-level-2-recovery.mjs",
  ...
  const manifest = {
    ...
    protocolId,
    status: "frozen-two-one-field-batch-16-publication-recovery-level-2-prepared-not-activated",
    frozenAt: at,
    ...
    nextAuthorizedAction: "activate-two-fresh-one-field-batch-16-publication-recovery-level-2-contexts"
  };
  if (shouldWrite) await writeFile(path.resolve(files.preparation), pretty(manifest));
  console.log(pretty({ status: shouldWrite ? manifest.status : "preview", contexts: 2, writableFields: 2, attemptsMaximum: 2, retriesMaximum: 0, recoveryLevel: 2, directIncrementalCostUsdMaximum: 0, modelExecutionAuthorized: false }));
  process.exit(0);
}
...
if (mode === "--activate") {
  ...
  assert.equal(preparation.status, "frozen-two-one-field-batch-16-publication-recovery-level-2-prepared-not-activated");
  ...
  const activation = { ...preparation, schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-level-2-recovery-activation", status: "frozen-two-one-field-batch-16-publication-recovery-level-2-authorized", activatedAt: at, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(), preparation: { path: files.preparation, sha256: sha256(preparationBytes) }, authorization: { ...preparation.authorization, modelExecution: true, deterministicMergeAndValidation: true }, nextAuthorizedAction: "execute-two-fresh-one-field-batch-16-publication-recovery-level-2-contexts-once" };
  if (shouldWrite) await writeFile(path.resolve(files.activation), pretty(activation));
  console.log(pretty({ status: shouldWrite ? activation.status : "preview", contexts: 2, attemptsMaximum: 2, retriesMaximum: 0, recoveryLevel: 2, directIncrementalCostUsdMaximum: 0, modelExecutionAuthorized: shouldWrite }));
  process.exit(0);
}

if (mode === "--run") {
  const activation = await readJson(files.activation);
  assert.equal(activation.status, "frozen-two-one-field-batch-16-publication-recovery-level-2-authorized");
  ...
  const startedAt = new Date().toISOString();
  const started = Date.now();
  console.error(`[batch-16-publication-recovery-2] starting ${activation.contexts.map((item) => item.moveId).join(", ")}`);
  const results = await Promise.all(activation.contexts.map(execute));
  for (const result of results) console.error(`[batch-16-publication-recovery-2] ${result.moveId} ${result.status} ${(result.elapsedMs / 60000).toFixed(2)}m`);
  const passed = results.every((item) => item.gateAcceptancePassed);
  const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-level-2-recovery-execution", protocolId, status: passed ? "two-one-field-batch-16-publication-recovery-level-2-execution-passed" : "batch-16-publication-recovery-level-2-execution-failed", startedAt, completedAt: new Date().toISOString(), elapsedMs: Date.now() - started, contextsAttempted: 2, validContexts: results.filter((item) => item.gateAcceptancePassed).length, invalidContexts: results.filter((item) => !item.gateAcceptancePassed).length, attempts: 2, retries: 0, timeoutExtensions: 0, directIncrementalCostUsd: 0, paidServiceCalls: 0, modelAuthoredScores: 0, results, authorization: { analysis: true, retries: false, furtherOrdinaryRecoveryLevel: false, exceptionalThirdLevelForAtomicCompanionLossOnly: false, paidServices: false, scorePass: false, productionMutation: false, nextBatchSelection: false } };
  await writeFile(path.resolve(files.execution), pretty(execution));
  console.log(pretty({ status: execution.status, contextsAttempted: 2, validContexts: execution.validContexts, invalidContexts: execution.invalidContexts, elapsedMinutes: Number((execution.elapsedMs / 60000).toFixed(2)), attempts: 2, retries: 0, recoveryLevel: 2, directIncrementalCostUsd: 0 }));
  process.exit(0);
}
...
if (mode === "--analyze") {
  ...
  const targetMoveIds = [...levelOneActivation.contexts.map((item) => item.moveId)];
  const replay = structuredClone(merged);
  for (const moveId of targetMoveIds) replay.moveProse[moveId].critique = failedOutput.moveProse[moveId].critique;
  ...
  const validationRecord = { schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-validation", protocolId: merged.protocolId, status: "passed", debateNumber: "16", debateId: packet.debateId, outputSha256: sha256(mergedBytes), validationSummary: validation, validationMessage: null, modelAuthoredScores: 0, lockedScoresUnchanged: true };
  const provenance = { schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-two-level-recovery-provenance", protocolId, debateNumber: "16", originalFailure: { status: "output-validation-failed", outputSha256: sha256(failedOutputBytes), preserved: true }, levelOne: { contexts: 9, validFields: 7, invalidFields: 2, attempts: 9, retries: 0 }, levelTwo: { contexts: 2, validFields: 2, invalidFields: 0, attempts: 2, retries: 0, rejectedPriorStringsUnavailable: true }, merge: { targetFieldsChanged: 9, acceptedCompanionFieldsChanged: false, scoresChanged: false, scorePassRerun: false, modelAuthoredScores: 0, completeValidationPassed: true }, outputSha256: sha256(mergedBytes) };
  const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-level-2-recovery-analysis", protocolId, status: "batch-16-publication-two-level-one-field-recovery-passed", recoveryLevel: 2, recoveryLevelsMaximum: 2, originalFailurePreserved: true, levelOneContexts: 9, levelTwoContexts: 2, totalAttempts: 11, retries: 0, repairedValidationFields: 9, recoveredDebate: "16", completeValidation: validation, directIncrementalCostUsd: 0, paidServiceCalls: 0, modelAuthoredScores: 0, authorization: { continueUnattemptedInitialPublicationContexts: true, furtherRecoveryLevel: false, scorePass: false, productionMutation: false, nextBatchSelection: false }, nextAuthorizedAction: "freeze-and-execute-the-nine-previously-unattempted-batch-16-publication-contexts" };
  if (shouldWrite) {
  ...
  console.log(pretty({ status: analysis.status, recoveredDebate: "16", levelOneContexts: 9, levelTwoContexts: 2, totalAttempts: 11, retries: 0, repairedValidationFields: 9, acceptedCompanionFieldsChanged: false, directIncrementalCostUsd: 0, nextAuthorizedAction: analysis.nextAuthorizedAction }));
}
