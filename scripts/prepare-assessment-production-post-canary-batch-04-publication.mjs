...
import {
  validatePostCanaryBatch04Scores
} from "./lib/assessment-production-post-canary-batch-04-score-gate.mjs";
import {
  buildPostCanaryBatch04PublicationPacket,
  buildPostCanaryBatch04PublicationSchema,
  postCanaryBatch04ReferenceCatalog,
  POST_CANARY_BATCH_04_PUBLICATION_DEBATES,
...
const SELECTION =
  "docs/assessment-production/post-canary-continuation-v1/batch-04/selection.json";
const FINAL_LEDGER =
...
const FINAL_LEDGER_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-04/final-ledger/final-ledger-manifest.json";
const FINAL_LEDGER_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-04/final-ledger/analysis.json";
const SCORES =
  "docs/assessment-production/post-canary-continuation-v1/batch-04/score-pass/calculated-scores.json";
const SCORE_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-04/score-pass/score-pass-manifest.json";
const SCORE_PREPARATION =
  "docs/assessment-production/post-canary-continuation-v1/batch-04/score-pass/score-pass-preparation-manifest.json";
const SCORE_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-04/score-pass/analysis.json";
const AUDIO_AUDIT =
  "docs/assessment-production/post-canary-continuation-v1/batch-04/audio-verification/audio-verification.json";
const SOURCE_PACKET_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-04/independent-judgments/source-packets";
...
const STATIC_SOURCE_FILES = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-04-score-gate.mjs",
  ...
  "scripts/prepare-assessment-production-post-canary-batch-04-publication.mjs",
  "scripts/test-assessment-production-post-canary-batch-04-publication-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-04-publication.mjs",
  "scripts/run-assessment-production-post-canary-batch-04-publication.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-04-publication.mjs"
];
...
assertV4(
  scoreAnalysis.status === "post-canary-batch-04-score-stability-gate-passed" &&
    scoreAnalysis.productionCanary === false &&
    scoreAnalysis.batchNumber === 4 &&
    ...
    scoreAnalysis.nextAuthorizedAction ===
      "standing-authorization-permits-batch-04-publication-packet-preparation",
  "the accepted Batch 4 score gate is unavailable or its stop boundary changed"
);
assertV4(
  scores.status === "post-canary-batch-04-single-score-pass-stability-gate-passed" &&
  ...
  "the locked Batch 4 score artifact is unavailable"
);
assertV4(
  scoreManifest.status ===
      "frozen-post-canary-batch-04-single-deterministic-score-pass-authorized" &&
    ...
    scorePreparation.status ===
      "frozen-post-canary-batch-04-single-deterministic-score-pass-prepared-not-activated",
  "the frozen Batch 4 single-pass score controls changed"
);
assertV4(
  ledger.status ===
      "passed-post-canary-batch-04-deterministic-final-ledger-assembly" &&
    ledger.debates?.length === 10 &&
    ledger.audit?.finalMoves === 203 &&
  ...
  "the locked Batch 4 final ledger changed"
);
assertV4(
  audioAudit.status ===
      "passed-all-four-post-canary-batch-04-confidence-moves-audio-verified" &&
    audioAudit.totals?.requiredMoves === 4 &&
    audioAudit.totals?.verified === 4 &&
    audioAudit.totals?.unresolved === 0,
  "the ten required Batch 4 audio checks are incomplete"
);
assertV4(
  productionManifest.model?.label === POST_CANARY_BATCH_04_PUBLICATION_MODEL.label &&
    productionManifest.model?.slug === POST_CANARY_BATCH_04_PUBLICATION_MODEL.slug &&
    productionManifest.model?.reasoningEffort ===
      POST_CANARY_BATCH_04_PUBLICATION_MODEL.reasoningEffort &&
    productionManifest.model?.authentication ===
      POST_CANARY_BATCH_04_PUBLICATION_MODEL.authentication,
  "the frozen production model or authentication changed"
);
assertV4(
  selection.selected?.map((item) => item.debateNumber).join(",") ===
      POST_CANARY_BATCH_04_PUBLICATION_DEBATES.join(",") &&
    selection.selected.every((item) => item.speakerCount === 2) &&
    selection.authorization?.productionMutation === false,
  "the frozen Batch 4 selection changed"
);
assertV4(
  ledger.debates.map((debate) => debate.debateNumber).join(",") ===
    ...
    scores.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_04_PUBLICATION_DEBATES.join(","),
  "the Batch 4 ledger or score order changed"
);
assertV4(
...
const finalLedgerInputs = await loadPostCanaryBatch04FinalLedgerInputs();
validatePostCanaryBatch04FinalLedger(
  ledger,
...
const catalog = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-local-reference-catalog",
  references: postCanaryBatch04ReferenceCatalog()
};
...
for (const debateNumber of POST_CANARY_BATCH_04_PUBLICATION_DEBATES) {
  ...
  const packet = buildPostCanaryBatch04PublicationPacket({
    ledgerDebate,
    scoreDebate,
    sourcePacket,
  ...
  const schema = buildPostCanaryBatch04PublicationSchema(packet);
...
assertV4(
  contexts.reduce((sum, context) => sum + context.moves, 0) === 203,
  "Batch 4 publication move coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.sections, 0) === 53,
  "Batch 4 publication section coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.audioVerifiedMoves, 0) === 4,
  "Batch 4 audio-verification coverage changed"
);
assertV4(
  contexts.every(
    (context) =>
  ...
  "Batch 4 publication section or quote eligibility is invalid"
);
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-publication-execution-preparation-manifest",
  ...
  userAuthorization: {
    instruction:
      "The frozen Batch 4 standing authorization permits preparation, activation, and execution of exactly ten score-locked publication-reconstruction contexts while every gate passes, with direct incremental cost capped at $0.",
  ...
  acceptanceContract: {
    ...
    movesAuthoredRequired: 203,
    critiquesRequired: 203,
  ...
  nextAuthorizedAction:
    "standing-authorization-permits-activation-and-execution-of-the-ten-frozen-batch-04-publication-contexts"
};
