...
import {
  ...
  POST_CANARY_BATCH_05_STANDING_AUTHORIZATION_STATUS,
  validatePostCanaryBatch05StandingAuthorization
...
const SELECTION_FREEZE_COMMIT = "cd13bce04689bc6e3cbf4efad5e0a35d9b40c099";
const SOURCE_FILES = [
  ...
  "scripts/prepare-assessment-production-post-canary-batch-05-standing-authorization.mjs",
  "scripts/test-assessment-production-post-canary-batch-05-standing-authorization.mjs"
];
...
assertV4(
  !(await exists(POST_CANARY_BATCH_05_STANDING_AUTHORIZATION)),
  `${POST_CANARY_BATCH_05_STANDING_AUTHORIZATION} already exists`
);
...
assertV4(
  selection.status ===
    ...
    selection.checkpointCommit ===
      "7effc324449805108ff1d23f47873939a3058ca1" &&
    ...
    analysis.status ===
      "fifth-post-canary-batch-selection-analysis-passed-awaiting-source-packet-preparation-decision" &&
  ...
  "the frozen Batch 5 selection or source-packet boundary changed"
);
assertV4(
  execFileSync("git", ["show", `${SELECTION_FREEZE_COMMIT}:${SELECTION}`]).equals(selectionBytes),
  "Batch 5 selection differs from the user-authorized selection commit"
);
...
assertV4(
  execFileSync("git", ["rev-parse", "--abbrev-ref", "HEAD"], {
    ...
    execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim() ===
      "c8fa77873bbd4bae8fd2b0c3310c23f2a6da9206" &&
    execFileSync("git", ["rev-parse", "origin/main"], {
      encoding: "utf8"
    }).trim() === "c8fa77873bbd4bae8fd2b0c3310c23f2a6da9206",
  "Batch 5 standing authorization must freeze at the pushed source-packet commit"
);
...
const record = {
  ...
  protocolId: POST_CANARY_BATCH_05_STANDING_AUTHORIZATION_PROTOCOL_ID,
  status: POST_CANARY_BATCH_05_STANDING_AUTHORIZATION_STATUS,
  authorizedAt,
  ...
  authorizedSequence: [
    "forty-one-frozen-score-blind-discovery-contexts",
  ...
  stopRules: {
    ...
    outsideBatchFiveDebateBlocks: true,
  ...
  nextAuthorizedAction:
    "prepare-validate-freeze-commit-and-push-batch-05-discovery-execution-manifest"
};
validatePostCanaryBatch05StandingAuthorization(record);
if (shouldWrite) {
  await writeFile(
