...
assertV3810(manifest.status === "frozen-one-synthetic-context-authorized" && manifest.authorization.syntheticSchemaPreflightModelExecution && !manifest.authorization.debatePerformanceModelExecution, "schema preflight unauthorized");
...
const temporary = await mkdtemp(path.join(os.tmpdir(), "slugfester-v3810-schema-preflight-"));
const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), "slugfester-v3810-home-schema-preflight-"));
...
try {
  ...
  const prompt = `This is a synthetic structured-output schema and validator preflight. Do not assess any real debate or person. Return exactly one schema-conforming JSON object with debateNumber "schema-preflight", debateId "schema-preflight-no-debate-content", pass "A", and exactly seven move judgments in the packet order below. Copy every identity and sourceSpan exactly, use null lockedBurdenContact, high confidence, and give every rationale enough detail to exceed its minimum length.
...
For every move use 75 for logicalCoherence, evidenceWarrant, precisionClarity, and epistemicCalibration; use relevanceBurden 54 because null contact permits 0 through 54. Use the specified responsiveness and charity values. Use zero burden adjustments for both sides with all eligibility booleans false, empty affected and related arrays, nonempty alreadyCapturedBy, and all audit affirmations true with moveCount 7. Emit no commentary.`;
  ...
  if (invocation.code !== 0 || invocation.signal !== null || invocation.timedOut) result = { ...base, status: "endpoint-preflight-failed", outputWritten: false, packetAwareValidationPassed: false };
    ...
    const validation = await run(process.execPath, ["scripts/validate-v3810-performance-judgment-output.mjs", manifest.artifacts.output, manifest.input.packet, "A"], { cwd: root, env: process.env });
