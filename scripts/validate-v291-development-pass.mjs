...
import {
  BURDEN_ADJUSTMENTS, BURDEN_TIERS, DEFECT_TYPES, SCOPE_RELATIONS,
  deriveDiagnostic, deriveReframe, equal, validateSpan,
} from "./lib/v291-semantics.mjs";

const passArgument = process.argv[2];
if (!passArgument) throw new Error("Usage: node scripts/validate-v291-development-pass.mjs <pass-or-key.json>");
...
const workflowPath = "docs/assessment-workflow-v2.9.1.md";
const rubricPath = "docs/reassessment-rubric-v2.9.1.md";
...
assert(pass.schemaVersion === "2.9.1-development-challenge-pass" && pass.workflowVersion === input.workflowVersion && pass.rubricVersion === input.rubricVersion && labels.includes(pass.pass) && pass.model === "5.6 Sol" && pass.calibrationOnly === true, "pass identity invalid");
const expectedMethod = pass.pass === "KEY" ? "fresh-isolated-v2.9.1-key-adjudication" : pass.pass.startsWith("KEY-") ? "fresh-isolated-v2.9.1-key-candidate" : "fresh-isolated-v2.9.1-development-challenge";
...
const annotationKeys = ["caseId", "moveId", "originalTargetContact", "targetEvidence", "connectedExample", "connectionEvidence", "scopeRelation", "scopeEvidence", "burdenAdjustment", "burdenEvidence", "componentContacts", "relevantContraryMaterial", "contraryEvidence", "defectType", "defectCue", "consequenceStated", "consequenceCue", "malformedDemandExplained", "malformedDemandCue", "replacementDemandStated", "replacementDemandCue", "burdenContact", "rationale"];
const seen = new Set();
for (const [index, annotation] of pass.annotations.entries()) {
  ...
  if (!annotation.originalTargetContact) assert(annotation.componentContacts.every((item) => !item.contacted), `${label}: component contact without original-target contact`);
...
const completionFloors = { originalTargetContacts:17, connectedExamples:3, componentContacts:20, defectCandidates:6, diagnosticPositives:3, reframePositives:3, burdenContacts:10, uniqueRationales:22 };
