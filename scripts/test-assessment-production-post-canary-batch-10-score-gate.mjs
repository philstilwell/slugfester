...
import {
  ...
  evaluatePostCanaryBatch10ScoreStability
...
const accepted = evaluatePostCanaryBatch10ScoreStability(passing);
...
assert.equal(
  evaluatePostCanaryBatch10ScoreStability(winnerMutation).acceptancePassed,
  false
);
const distanceMutation = structuredClone(passing);
distanceMutation[2].final = scores(95, 80);
assert.equal(
  evaluatePostCanaryBatch10ScoreStability(distanceMutation).acceptancePassed,
  false
);
assert.throws(() =>
  evaluatePostCanaryBatch10ScoreStability(passing, {
    ...POST_CANARY_BATCH_10_SCORE_STABILITY_THRESHOLDS,
