...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-17/source-preparation";
...
assert.equal(preparation.status, "post-canary-batch-17-four-complete-score-blind-source-packets-prepared-awaiting-validation");
...
assert.equal(preparation.contexts.length, 4);
assert.equal(preparation.totals.debates, 4);
assert.equal(preparation.totals.sides, 8);
...
for (const context of preparation.contexts) {
  ...
  assert(frozen, `${context.debateNumber}: absent from frozen Batch 17 selection`);
  ...
  assert.equal(packet.schemaVersion, "1.0-assessment-production-post-canary-batch-17-score-blind-source-packet");
  ...
  for (const chunk of context.chunks) {
    ...
    assert.deepEqual(
      ...
      findBatch17ZeroLexicalTokenRows(chunkBytes).map(({ eventIndex, startMs, durationMs, text }) => ({
        eventIndex,
        startMs,
        durationMs,
...
const validation = {
  ...
  status: "post-canary-batch-17-score-blind-source-packet-validation-passed-frozen-under-standing-authorization",
  ...
  checks: {
    ...
    allTwelveOriginalSourceHashesReplayed: true,
    allFourLocalManifestChainsReplayed: true,
  ...
  nextAuthorizedAction: "prepare-freeze-and-activate-batch-17-discovery-execution-under-standing-authorization",
};
