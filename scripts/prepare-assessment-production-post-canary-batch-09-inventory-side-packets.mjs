...
const BATCH_SOURCE_PREPARATION =
  "docs/assessment-production/post-canary-continuation-v1/batch-09/source-preparation/preparation-manifest.json";
...
const SCRIPT =
  "scripts/prepare-assessment-production-post-canary-batch-09-inventory-side-packets.mjs";
const PLAN_RUNNER =
...
function makeSidePacket({
  ...
  return {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-09-chronology-fallback-side-selector-packet",
...
async function buildArtifacts({ frozenAt, checkpointCommit }) {
  ...
  assertV4(
    sourcePreparation.schemaVersion ===
      ...
      sourcePreparation.totals.proCandidates === 137 &&
      sourcePreparation.totals.conCandidates === 152 &&
    ...
    "Batch 9 source preparation boundary drifted"
  );
  ...
  assertV4(
    ...
    "accepted Batch 9 planner gate drifted"
  );
  assertV4(
    isDeepStrictEqual(
    ...
    "Batch 9 source compatibility boundary drifted"
  );
  ...
  const standingScope =
    "prepare, freeze, activate, and execute exactly twenty Batch 9 score-blind side-selector contexts under the frozen standing authorization";
  const sidePreparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-09-chronology-fallback-side-packet-preparation",
    protocolId: PROTOCOL_ID,
    sideSelectionProtocolId: CHRONOLOGY_FALLBACK_INVENTORY.protocolId,
    status:
      "twenty-exact-post-canary-batch-09-side-selector-packets-frozen-not-authorized",
    frozenAt,
    checkpointCommit,
    branch: "main",
    ...
    nextRequiredAction:
      "freeze-batch-09-side-selector-execution-preparation-manifest-model-free-only",
  };
  ...
  const executionPreparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-09-side-selector-execution-preparation-manifest",
    protocolId: PROTOCOL_ID,
    sideSelectionProtocolId: CHRONOLOGY_FALLBACK_INVENTORY.protocolId,
    status:
      "frozen-twenty-post-canary-batch-09-side-selector-contexts-prepared-not-authorized",
    frozenAt,
    checkpointCommit,
    branch: "main",
    productionContinuation: true,
    ...
    costEstimate: {
      ...
      estimateBasis:
        "The promoted twenty-context side-selector gate completed in 10.80 wall minutes and 19.74 aggregate model minutes at concurrency two. Batch 9 preserves the same one-side isolated context shape; this manifest records the exact observed packet sizes against the proven 115,000-byte input ceiling. ChatGPT-subscription execution has no direct incremental API charge.",
    },
    ...
    nextAuthorizedAction:
      "freeze-and-activate-batch-09-side-selector-execution-under-standing-authorization",
  };
...
if (command === "prepare") await prepare();
  ...
  throw new Error(
    "usage: prepare-assessment-production-post-canary-batch-09-inventory-side-packets.mjs <prepare|validate>"
  );
}
