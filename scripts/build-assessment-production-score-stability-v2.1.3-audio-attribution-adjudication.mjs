...
assertV4(
  ...
  "v2.1.3 audio-attribution packet preparation is unauthorized"
);
...
const packet = {
  schemaVersion: V213_AUDIO_ADJ_PACKET_VERSION,
...
const schemaPath = `${V213_AUDIO_ADJ_ROOT}/schema.json`;
...
const preparation = {
  schemaVersion:
    "1.0-score-stability-v2.1.3-audio-attribution-adjudication-preparation",
  protocolId: V213_AUDIO_ADJ_PROTOCOL_ID,
  status: shouldWrite
  ...
  inputs: {
    workflow:
      "docs/assessment-production-score-stability-v2.1.3-audio-attribution-adjudication-workflow.md",
    manual: `${V213_AUDIO_ADJ_ROOT}/manual.md`,
    schema: schemaPath,
    packet: packetPath,
  ...
  nextAuthorizedAction:
    "freeze-one-v2.1.3-audio-attribution-adjudication-execution-manifest"
};
if (shouldWrite) {
  await mkdir(V213_AUDIO_ADJ_ROOT, { recursive: true });
  await writeFile(packetPath, `${JSON.stringify(packet, null, 2)}\n`);
  await writeFile(
    schemaPath,
    `${JSON.stringify(makeV213AudioAttributionAdjudicationSchema(), null, 2)}\n`
  );
