...
import {
  validatePostCanaryBatch10Scores
} from "./lib/assessment-production-post-canary-batch-10-score-gate.mjs";
import {
  buildPostCanaryBatch10PublicationPacket,
  buildPostCanaryBatch10PublicationSchema,
  postCanaryBatch10ReferenceCatalog,
  POST_CANARY_BATCH_10_PUBLICATION_DEBATES,
...
const ROOT = POST_CANARY_BATCH_10_PUBLICATION_ROOT;
...
const SELECTION =
  "docs/assessment-production/post-canary-continuation-v1/batch-10/selection.json";
const FINAL_LEDGER =
...
const FINAL_LEDGER_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-10/final-ledger/final-ledger-manifest.json";
const FINAL_LEDGER_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-10/final-ledger/analysis.json";
const SCORES =
  "docs/assessment-production/post-canary-continuation-v1/batch-10/score-pass/calculated-scores.json";
const SCORE_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-10/score-pass/score-pass-manifest.json";
const SCORE_PREPARATION =
  "docs/assessment-production/post-canary-continuation-v1/batch-10/score-pass/score-pass-preparation-manifest.json";
const SCORE_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-10/score-pass/analysis.json";
const SOURCE_PACKET_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-10/independent-judgments/source-packets";
...
const STATIC_SOURCE_FILES = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-10-score-gate.mjs",
  ...
  "scripts/prepare-assessment-production-post-canary-batch-10-publication.mjs",
  "scripts/test-assessment-production-post-canary-batch-10-publication-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-10-publication.mjs",
  "scripts/run-assessment-production-post-canary-batch-10-publication.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-10-publication.mjs"
];
...
assertV4(
  scoreAnalysis.status === "post-canary-batch-10-score-stability-gate-passed" &&
    scoreAnalysis.productionCanary === false &&
    scoreAnalysis.batchNumber === 10 &&
    ...
    scoreAnalysis.nextAuthorizedAction ===
      "standing-authorization-permits-batch-10-publication-packet-preparation",
  "the accepted Batch 10 score gate is unavailable or its stop boundary changed"
);
assertV4(
  scores.status === "post-canary-batch-10-single-score-pass-stability-gate-passed" &&
  ...
  "the locked Batch 10 score artifact is unavailable"
);
assertV4(
  scoreManifest.status ===
    ...
    scorePreparation.status ===
      "frozen-post-canary-batch-10-single-deterministic-score-pass-prepared-not-activated",
  "the frozen Batch 10 single-pass score controls changed"
);
assertV4(
  ledger.status ===
      "passed-post-canary-batch-10-deterministic-final-ledger-assembly" &&
    ledger.debates?.length === 10 &&
    ledger.audit?.finalMoves === 182 &&
    ledger.audit?.audioVerifiedMoves === 9 &&
    ...
    ledgerAnalysis.status ===
      "post-canary-batch-10-deterministic-final-ledger-gate-passed",
  "the locked Batch 10 final ledger changed"
);
assertV4(
  productionManifest.model?.label === POST_CANARY_BATCH_10_PUBLICATION_MODEL.label &&
    productionManifest.model?.slug === POST_CANARY_BATCH_10_PUBLICATION_MODEL.slug &&
    productionManifest.model?.reasoningEffort ===
      POST_CANARY_BATCH_10_PUBLICATION_MODEL.reasoningEffort &&
    productionManifest.model?.authentication ===
      POST_CANARY_BATCH_10_PUBLICATION_MODEL.authentication,
  "the frozen production model or authentication changed"
);
assertV4(
  selection.selected?.map((item) => item.debateNumber).join(",") ===
      POST_CANARY_BATCH_10_PUBLICATION_DEBATES.join(",") &&
    selection.selected.every((item) => item.speakerCount === 2) &&
    selection.authorization?.productionMutation === false,
  "the frozen Batch 10 selection changed"
);
assertV4(
  ledger.debates.map((debate) => debate.debateNumber).join(",") ===
    ...
    scores.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_10_PUBLICATION_DEBATES.join(","),
  "the Batch 10 ledger or score order changed"
);
assertV4(
...
const finalLedgerInputs = await loadPostCanaryBatch10FinalLedgerInputs();
validatePostCanaryBatch10FinalLedger(
  ledger,
...
const catalog = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-10-local-reference-catalog",
  references: postCanaryBatch10ReferenceCatalog()
};
...
for (const debateNumber of POST_CANARY_BATCH_10_PUBLICATION_DEBATES) {
  ...
  const packet = buildPostCanaryBatch10PublicationPacket({
    ledgerDebate,
    scoreDebate,
    sourcePacket,
  ...
  const schema = buildPostCanaryBatch10PublicationSchema(packet);
...
assertV4(
  contexts.reduce((sum, context) => sum + context.moves, 0) === 182,
  "Batch 10 publication move coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.sections, 0) === 51,
  "Batch 10 publication section coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.audioVerifiedMoves, 0) === 9,
  "Batch 10 audio-verification coverage changed"
);
assertV4(
  contexts.every(
    (context) =>
  ...
  "Batch 10 publication section or quote eligibility is invalid"
);
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-10-publication-execution-preparation-manifest",
  protocolId: POST_CANARY_BATCH_10_PUBLICATION_PROTOCOL_ID,
  status:
    "frozen-ten-post-canary-batch-10-score-locked-publication-contexts-prepared-not-activated",
  frozenAt,
  ...
  userAuthorization: {
    instruction:
      "The frozen Batch 10 standing authorization permits preparation, activation, and execution of exactly ten score-locked publication-reconstruction contexts while every gate passes, with direct incremental cost capped at $0.",
  ...
  model: structuredClone(POST_CANARY_BATCH_10_PUBLICATION_MODEL),
  costEstimate: {
  ...
  acceptanceContract: {
    ...
    movesAuthoredRequired: 182,
    critiquesRequired: 182,
  ...
  nextAuthorizedAction:
    "standing-authorization-permits-activation-and-execution-of-the-ten-frozen-batch-10-publication-contexts"
};
