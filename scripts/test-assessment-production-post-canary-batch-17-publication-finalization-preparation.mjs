...
import { buildPostCanaryBatch17PublicationFinalization,
  validatePostCanaryBatch17PublicationFinalCandidate,
  POST_CANARY_BATCH_17_PUBLICATION_FINALIZATION_ROOT } from
...
assertV4(m.status === "frozen-post-canary-batch-17-publication-finalization-prepared" &&
  m.contexts?.length === 4 && m.executionPolicy?.deterministicFinalizationPassesMaximum === 1 &&
  m.executionPolicy?.rerunsMaximum === 0 && m.finalizationPolicy?.displayFieldsChangedMaximum === 0,
"Batch 17 finalization preparation changed");
let moves = 0;
...
assertV4(moves === 79, "finalization move count changed");
...
console.log(JSON.stringify({ status: "passed", debates: 4, moves,
