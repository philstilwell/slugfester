...
const PROTOCOL_ID =
  "assessment-production-score-stability-v2-fresh-validation-source-preparation";
const CACHE_VERSION = "assessment-production-score-stability-v2-fresh-validation";
...
const MASTER_MANIFEST_PATH = `${VALIDATION_ROOT}/validation-manifest.json`;
const SELECTION_PATH = `${VALIDATION_ROOT}/selection.json`;
...
const SCRIPT_PATH =
  "scripts/prepare-assessment-production-score-stability-v2-source-packets.mjs";
const TEST_PATH =
  "scripts/test-assessment-production-score-stability-v2-source-packets.mjs";
const SOURCE_FILES = [
...
async function mustNotExist(target) {
  await access(target).then(
    () => {
      throw new Error(`${target} already exists; packet preparation is immutable`);
    },
    () => true
  );
}
...
const [masterBytes, selectionBytes, productionManifestBytes, manualBytes] =
  await Promise.all([
    readFile(path.resolve(MASTER_MANIFEST_PATH)),
    readFile(path.resolve(SELECTION_PATH)),
    readFile(path.resolve(PRODUCTION_MANIFEST_PATH)),
    readFile(path.resolve(DISCOVERY_MANUAL_PATH)),
  ]);
...
assertV4(
  master.status ===
    ...
    master.authorization?.sourcePreparationPacketPreparation === true &&
  ...
  "the frozen v2 validation manifest does not authorize source packet preparation"
);
assertV4(
  ...
  "the frozen model, effort, or subscription lock changed"
);
assertV4(
  master.currentCanaryDisposition?.status ===
  ...
  "the failed canary or unpromoted-policy disposition changed"
);
assertV4(
  selection.status === "fresh-disjoint-ten-debate-cohort-source-gate-passed" &&
    selection.selected?.length === 10 &&
    selection.authorization?.freshValidationManifestPreparation === true &&
    selection.authorization?.inventoryModelExecution === false &&
    selection.authorization?.paidTranscription === false,
  "the frozen cohort selection is not the expected source-gated ten"
);
assertV4(
  ...
  "the production corpus manifest or model lock changed"
);
assertV4(
  ...
  "master-manifest cohort identity drifted from frozen selection"
);

for (const [file, expectedHash] of Object.entries(master.sourceHashes ?? {})) {
  await verifiedBytes(file, expectedHash, `master source ${file}`);
}
if (shouldWrite) {
  await Promise.all([
    mustNotExist(ROOT),
...
for (const selected of selection.selected) {
  ...
  assertV4(
    ...
    `${selected.debateNumber}: source preparation requires two frozen interlocutors`
  );
  ...
  assertV4(
    ...
    `${selected.debateNumber}: frozen source event count changed`
  );
  ...
  packet.schemaVersion =
    "1.0-score-stability-v2-validation-score-blind-source-packet";
  ...
  for (const chunk of plan.chunks) {
    ...
    const sourceSpan =
      schema.properties.candidates.items.properties.sourceSpan.properties;
    ...
    assertV4(
      ...
      `${selected.debateNumber}/${chunk.chunkId}: chunk ownership schema mismatch`
    );
    assertV4(
      speakerAllowlist.length === 2 &&
        speakerAllowlist.every((speaker) => speakers.includes(speaker)),
      `${selected.debateNumber}/${chunk.chunkId}: frozen speaker allowlist mismatch`
    );
    ...
    chunks.push({
      ...
      copiedInputBytes:
        manualBytes.length + packetBytes.length + schemaBytes.length + chunkBytes.length,
      rawOutput: `${ROOT}/discovery-outputs/debate-${selected.debateNumber}-${chunk.chunkId}.json`,
    });
  }
...
const preparation = {
  schemaVersion:
    "1.0-score-stability-v2-fresh-validation-source-preparation",
  ...
  inputs: {
    validationManifest: MASTER_MANIFEST_PATH,
