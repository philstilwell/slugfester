...
const [manifest, execution, preparation] = await Promise.all(["execution-manifest.json", "model-execution.json", "preparation-manifest.json"].map((file) => readFile(path.resolve(`${V42211735_ROOT}/${file}`), "utf8").then(JSON.parse)));
