...
import {
  POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_ORDER,
  POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_PROTOCOL_ID,
  POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_ROOT,
  extractProductionDebateRecords,
  inventoryDigest,
  serializedJson,
  sha256
} from "./lib/assessment-production-post-canary-batch-16-production-publication.mjs";
...
const paths = {
  manifest: `${POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_ROOT}/mutation-manifest.json`,
  preparationAnalysis: `${POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_ROOT}/preparation-analysis.json`,
  activation: `${POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_ROOT}/execution-activation.json`,
  execution: `${POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_ROOT}/execution.json`,
  compatibilityRoot: "docs/assessment-production/post-canary-continuation-v1/batch-16/production-compatibility",
  finalizationRoot: "docs/assessment-production/post-canary-continuation-v1/batch-16/publication-finalization/output-bundle/final-candidates",
  ...
  scoreStabilityException: "docs/assessment-production/post-canary-continuation-v1/batch-16/score-pass/exceptional-publication-authorization.json",
  standingAuthorization: "docs/assessment-production/post-canary-continuation-v1/batch-16/standing-authorization.json",
  preparationScript: "scripts/prepare-assessment-production-post-canary-batch-16-production-publication.mjs",
  preparationTest: "scripts/test-assessment-production-post-canary-batch-16-production-publication-preparation.mjs",
  library: "scripts/lib/assessment-production-post-canary-batch-16-production-publication.mjs",
  activationScript: "scripts/activate-assessment-production-post-canary-batch-16-production-publication.mjs",
  activationTest: "scripts/test-assessment-production-post-canary-batch-16-production-publication-activation.mjs",
  executionScript: "scripts/run-assessment-production-post-canary-batch-16-production-publication.mjs"
};
...
const scoreStabilityException = await readJson(paths.scoreStabilityException);
assertV4(
  compatibilityAnalysis.status === "batch-16-production-compatibility-passed" &&
    ...
    compatibilityAnalysis.nextAuthorizedAction === "prepare-batch-16-production-publication-mutation-manifest-under-standing-authorization",
  "accepted Batch 16 compatibility analysis required"
);
assertV4(
  compatibilityExecution.status === "batch-16-route-score-checkpoint-batch-01-through-batch-15-legacy-reference-validation-passed" &&
    compatibilityExecution.regressions?.fullRepositoryPassed === true &&
    compatibilityExecution.totals?.compatibilityPassesCompleted === 1,
  "accepted one-pass Batch 16 compatibility execution required"
);
assertV4(
  compatibilityActivation.status === "post-canary-batch-16-compatibility-execution-authorized-and-frozen" &&
    compatibilityActivation.packetHashes?.length === 10,
  "frozen Batch 16 compatibility activation required"
);
assertV4(
  scoreStabilityException.status === "frozen-active-batch-16-debate-144-score-stability-publication-exception" &&
    canonicalJson(scoreStabilityException.exception?.debates) === canonicalJson(["144"]) &&
    scoreStabilityException.exception?.productionPublication === true &&
  ...
  "frozen Debate 144 production-publication exception required"
);
...
assertV4(ledgerInventory.length === 160, "expected 160 existing production ledgers before Batch 16 publication");

const packetLocks = new Map(compatibilityActivation.packetHashes.map((record) => [record.debateNumber, record]));
const debates = [];
for (const debateNumber of POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_ORDER) {
...
assertV4(
  debates.filter((debate) => debate.productionLedgerBaseline.exists).length === 0 &&
    debates.filter((debate) => !debate.productionLedgerBaseline.exists).length === 10,
  "expected ten absent Batch 16 production ledgers"
);
...
assertV4(validatorLock.sha256 === compatibilityExecution.validator.appliedSha256, "accepted Batch 16 validator changed");
...
const manifest = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-16-production-publication-mutation-manifest",
  protocolId: POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_PROTOCOL_ID,
  status: "frozen-batch-16-production-publication-mutation-manifest-prepared",
  frozenAt,
  baselineCommit,
  ...
  explicitOrder: [...POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_ORDER],
  invariants: {
  ...
  scorePolicy: {
    ...
    exception: await lockFile(paths.scoreStabilityException)
  },
  ...
  executionContract: {
    ...
    requiredOrder: [
      ...
      "confirm the ten absent Batch 16 production-ledger baselines and all 160 existing ledger hashes",
      "publish ten production ledgers byte-for-byte from the staged ledgers",
      "replace exactly ten Batch 16 records in src/data/debates.js with the finalized candidates",
      "keep src/data/references.js byte-identical",
      "run deterministic Batch 16 route and score validation",
  ...
  nextAuthorizedAction: "activate-and-execute-one-frozen-batch-16-production-publication-mutation-pass-under-standing-authorization"
};
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-16-production-publication-preparation-analysis",
  protocolId: POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_PROTOCOL_ID,
  status: "batch-16-production-publication-mutation-manifest-freeze-passed",
  analyzedAt: frozenAt,
  ...
  checks: {
    ...
    candidatesLocked: 10,
    stagedLedgersLocked: 10,
    exactProductionRecordsLocked: 10,
    existingProductionLedgersLocked: 160,
...
if (write) {
  await mkdir(resolve(POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_ROOT), { recursive: true });
