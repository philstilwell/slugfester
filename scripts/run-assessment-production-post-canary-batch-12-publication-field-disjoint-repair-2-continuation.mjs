...
assertV4(
  activation.status === "frozen-fifteen-context-twenty-nine-field-batch-12-publication-repair-level-2-continuation-activated" &&
    activation.authorization?.modelExecution === true && activation.contexts?.length === 15 &&
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-12-field-repair-${context.contextIndex}-`));
  const isolatedCodexHome = await mkdtemp(path.join(os.tmpdir(), `batch-12-field-repair-home-${context.contextIndex}-`));
  ...
  try {
    ...
    const prompt = [
      ...
      "For a novelty target, write at least eight words accurately explaining how the supplied classification and source-move mapping relates the AI Extension item to the transcript record.",
    ...
    process.stdout.write(`[batch-12-field-repair] starting context ${context.contextIndex} ${context.shardId} (${context.writableFieldCount} fields)\n`);
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 ||
      ...
      const validation = {
        schemaVersion: "1.0-assessment-production-post-canary-batch-12-publication-field-disjoint-repair-validation",
      ...
      const provenance = {
        schemaVersion: "1.0-assessment-production-post-canary-batch-12-publication-field-disjoint-repair-provenance",
        protocolId: activation.protocolId,
        contextIndex: context.contextIndex,
        shardId: context.shardId,
        debateNumber: context.debateNumber,
  ...
  process.stdout.write(`[batch-12-field-repair] ${context.shardId} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
...
const passed = results.length === 15 && validContexts === 15;
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-12-publication-field-disjoint-repair-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "fifteen-field-disjoint-batch-12-publication-repair-level-2-continuation-contexts-passed"
    : "fifteen-field-disjoint-batch-12-publication-repair-level-2-continuation-contexts-failed",
  gateStartedAt,
  ...
  nextRequiredAction: passed
    ? "deterministically-merge-and-validate-three-debates-then-replay-complete-cohort"
