...
assertV4(
  ...
  "Batch 14 dispute-only adjudication execution is unauthorized"
);
...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-post-canary-batch-14-adjudication-${context.debateNumber}-`
    )
  );
  const codexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-post-canary-batch-14-adjudication-home-${context.debateNumber}-`
    )
  );
  ...
  try {
    ...
    }; read nothing else. Act only as the isolated, score-blind, disputed-fields-only adjudicator for post-canary Batch 14 Debate ${
    ...
    process.stdout.write(
      `[batch-14-adjudication] starting index ${context.contextIndex} ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
    ...
    if (
      ...
      try {
        validation = validatePostCanaryBatch14DisputeAdjudicationOutput(
  ...
  process.stdout.write(
    `[batch-14-adjudication] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-14-dispute-only-adjudication-model-execution",
  protocolId: manifest.protocolId,
  status: passed
    ? "ten-post-canary-batch-14-dispute-only-adjudication-contexts-passed"
    : "post-canary-batch-14-dispute-only-adjudication-gate-complete-with-failure",
