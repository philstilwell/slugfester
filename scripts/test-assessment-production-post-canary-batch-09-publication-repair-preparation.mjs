...
const MANIFEST = `${POST_CANARY_BATCH_09_DEBATE_170_REPAIR_ROOT}/execution-preparation-manifest.json`;
...
assert.equal(manifest.protocolId, POST_CANARY_BATCH_09_DEBATE_170_REPAIR_PROTOCOL_ID);
...
for (const context of manifest.contexts) {
  ...
  assert.equal(packet.debateNumber, "170");
  ...
  assert.deepEqual(schema, buildDebate170RepairSchema(packet));
  ...
  assert.equal(validateDebate170RepairOutput(repair, packet).status, "passed");
...
const merged = mergeAndValidateDebate170Repairs({
  baseOutput,
...
assert.equal(merged.fullValidation.moves, 20);
