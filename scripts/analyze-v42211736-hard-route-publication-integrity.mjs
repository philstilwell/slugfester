...
const [manifest, execution, preparation] = await Promise.all(["execution-manifest.json", "model-execution.json", "preparation-manifest.json"].map((file) => readFile(path.resolve(`${V42211736_ROOT}/${file}`), "utf8").then(JSON.parse)));
...
for (const context of manifest.contexts) {
  ...
  if (result.gateAcceptancePassed) {
    replay = validateV42211736PublicationOutput(JSON.parse(await readFile(path.resolve(context.output), "utf8")), JSON.parse(await readFile(path.resolve(context.packet), "utf8")));
