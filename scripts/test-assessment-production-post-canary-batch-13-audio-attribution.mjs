...
import { AUDIT, ISOLATION, MODEL, OUTPUT_VERSION, PROTOCOL_ID, ROOT, validateOutput } from "./lib/assessment-production-post-canary-batch-13-audio-attribution.mjs";

const preparation = JSON.parse(await readFile(`${ROOT}/preparation-manifest.json`, "utf8"));
const authorization = JSON.parse(await readFile("docs/assessment-production/post-canary-continuation-v1/batch-13/audio-verification/audio-attribution-successor-authorization.json", "utf8"));
...
console.log(JSON.stringify({ status: "passed-prepared", contexts: 2, decisions: 6, model: "5.6 Sol/low", authentication: "ChatGPT subscription", attemptsMaximum: 2, retriesMaximum: 0, paidTranscriptionCalls: 0, scoresDerived: 0 }, null, 2));
