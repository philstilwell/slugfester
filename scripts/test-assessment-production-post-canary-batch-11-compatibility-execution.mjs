...
const root = POST_CANARY_BATCH_11_COMPATIBILITY_ROOT;
...
const validator = await import(
  `./validate-debates.mjs?batch11Compatibility=${activation.validator.proposedSha256}`
);
...
for (const lock of activation.packetHashes) {
  ...
  const routeLock =
    validator.validatePostCanaryBatch11LedgerAdapterRouteLocks({
      debate: candidate,
      ledgerText,
  ...
  const route = validator.validatePostCanaryBatch11LedgerAdapterRoute({
    debate: candidate,
    ledger,
    ledgerText
  });
  assert.equal(route.repositoryScoreReplayPassed, true);
  const adapter = validatePostCanaryBatch11SiteLedgerAdapter({
    adapter: ledger,
    candidate,
...
assert.equal(oneSidedDisplayRows, 6);
assert.equal(emptyReferenceLinks, 54);
assert.equal(suppliedReferenceLinks, 0);
assert.equal(
  ...
  JSON.stringify(
    {
      ...
      positiveRoutes,
      negativeControls,
      existingAdapterRoutesPreserved: 11,
      modelContexts: 0,
      paidServiceCalls: 0,
