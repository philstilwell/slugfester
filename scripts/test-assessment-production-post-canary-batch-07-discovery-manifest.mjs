...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-07/discovery";
...
assert.equal(manifest.status, "frozen-thirty-five-post-canary-batch-07-discovery-contexts-prepared-not-authorized");
...
assert.equal(manifest.contexts.length, 35);
assert.deepEqual(manifest.contexts.map((context) => context.contextIndex), Array.from({ length: 35 }, (_, index) => index));
...
assert.equal(manifest.executionPolicy.contexts, 35);
...
assert.equal(manifest.executionPolicy.copiedInputBytesMaximum, 82224);
assert.equal(preparation.totals.maximumCopiedInputBytes, 82224);
...
assert.equal(manifest.copiedInputBoundary.frozenObservedCopiedInputBytesMaximum, 82224);
assert.equal(manifest.copiedInputBoundary.executionCopiedInputBytesMaximum, 82224);
assert.deepEqual(manifest.copiedInputBoundary.maximumContext, { debateNumber: "193", chunkId: "chunk-001" });
...
assert.equal(manifest.costEstimate.contexts, 35);
...
assert.equal(preparedByContext.size, 35);
for (const context of manifest.contexts) {
  ...
  assert(buildBatch07TokenCountedChunkLedger(chunkBytes).equals(tokenBytes));
...
assert.equal(manifest.nextAuthorizedAction, "prepare-and-freeze-batch-07-discovery-execution-activation-under-standing-authorization");

const validation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-discovery-execution-preparation-validation",
  protocolId: manifest.protocolId,
  status: "batch-07-discovery-execution-manifest-validation-passed-frozen-standing-authorization-active",
