...
assertV4(
  ...
  "Batch 1 adjudication execution is unavailable or crossed its boundary"
);
for (const [file, digest] of Object.entries(activation.sourceHashes)) {
  assertV4(
    sha256(await readFile(file)) === digest,
    `source hash mismatch during Batch 1 adjudication analysis: ${file}`
  );
}
...
for (const context of activation.contexts) {
  ...
  if (result.gateAcceptancePassed) {
    ...
    replay = validatePostCanaryBatch01DisputeAdjudicationOutput(
...
const semanticPass =
  ...
  valid.reduce((sum, context) => sum + context.candidateSelections, 0) ===
    461 &&
  valid.reduce((sum, context) => sum + context.disputedMoves, 0) === 169;
const timingPass =
  semanticPass &&
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-01-dispute-only-adjudication-analysis",
  protocolId: activation.protocolId,
  status: passed
    ? "post-canary-batch-01-dispute-only-adjudication-gate-passed-awaiting-separate-final-ledger-assembly-approval"
    : semanticPass
      ? timingPass
        ? scoreBlindPass
          ? "post-canary-batch-01-dispute-only-adjudication-gate-failed-isolation"
          : "post-canary-batch-01-dispute-only-adjudication-gate-failed-scoreblindness"
        : "post-canary-batch-01-dispute-only-adjudication-gate-failed-timing"
  ...
  priorCostControl: structuredClone(activation.priorCostControl),
  totals: {
  ...
  nextAuthorizedAction: passed
    ...
    : "user-approval-required-before-any-batch-01-adjudication-failure-diagnosis-or-downstream-work"
};
if (shouldWrite) {
