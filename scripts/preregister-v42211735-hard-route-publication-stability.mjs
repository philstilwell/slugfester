...
const manifestPath = `${V42211735_ROOT}/execution-manifest.json`;
const executionPath = `${V42211735_ROOT}/model-execution.json`;
const analysisPath = `${V42211735_ROOT}/analysis.json`;
...
assertV4(preparation.status === "prepared-five-isolated-hard-route-publication-stability-contexts" && preparation.authorization.executionManifest && preparation.totals.modelContextsExecuted === 0 && preparation.totals.modelAuthoredScores === 0, "publication stability execution manifest is not authorized");
assertV4(preparation.totals.maximumCopiedInputBytes <= 400000, "publication context exceeds frozen transport budget");
assertV4(preparation.repair.counterfactualOnlyCritiqueReplacementPassed && preparation.repair.structuredCritiqueCharacters[0] === 880 && preparation.repair.structuredCritiqueCharacters[1] === 1020 && !preparation.repair.priorGateTreatedAsRetry, "publication stability repair boundary mismatch");
...
const sourceFiles = [
  ...
  `${V42211735_ROOT}/preparation-manifest.json`,
  ...
  "scripts/prepare-v42211735-hard-route-publication-stability.mjs",
  "scripts/preregister-v42211735-hard-route-publication-stability.mjs",
  "scripts/run-v42211735-hard-route-publication-stability.mjs",
  "scripts/analyze-v42211735-hard-route-publication-stability.mjs",
  "scripts/test-v42211735-hard-route-publication-stability-gate.mjs"
];
...
const manifest = {
  schemaVersion: "4.2.21.17.35-hard-route-publication-stability-execution-manifest",
  ...
  deterministicValidation: { modelCritiqueTargetWords: preparation.repair.modelCritiqueTargetWords, repositoryCritiqueAcceptanceWords: preparation.repair.repositoryCritiqueAcceptanceWords, structuredCritiqueCharacters: preparation.repair.structuredCritiqueCharacters, modelSummaryTargetWords: [18, 28], repositorySummaryAcceptanceWords: [8, 35], openAIStructuredOutputSubsetPreflight: true, repositoryUniquenessValidationRetained: true, everyLockedMoveAuthoredExactlyOnce: true, exactQuoteSubstringRequired: true, critiqueWordAndSentenceContract: true, localReferenceCatalogOnly: true, overallCommentaryMinimums: true, aiExtensionNoveltyMapComplete: true, introducedArgumentPerSideRequired: true, exactAccordionDisplayContract: true, prohibitedLanguageAbsent: true, modelAuthoredScores: 0 },
  priorGate: { protocolId: "v4.2.21.17.34-hard-route-publication-prompt-alignment", status: "four-of-five-valid-plus-timing-outlier", treatedAsRetry: false },
