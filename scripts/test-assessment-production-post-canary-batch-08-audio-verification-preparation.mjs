...
assert.deepEqual(preparation.scope.debates, ["137", "156", "120"]);
...
assert.equal(preparation.costEstimate.clipSeconds, 425.622938);
assert.equal(preparation.costEstimate.clipMinutes, 7.0937);
...
assert.deepEqual(preparation.costEstimate.projectedUsage, {
  audioInputTokens: 7931,
  textInputTokens: 684,
  outputTokens: 13429,
  totalInputTokens: 8615
});
...
assert.equal(
  ...
  "activate-and-execute-six-batch-08-paid-audio-verification-calls-under-standing-authorization"
);
