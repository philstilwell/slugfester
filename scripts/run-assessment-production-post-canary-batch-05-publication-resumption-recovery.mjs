...
assertV4(
  activation.status === "frozen-six-context-batch-05-publication-resumption-recovery-authorized" &&
    activation.batchNumber === 5 && activation.contexts?.length === 6 &&
    activation.authorization?.recoveryModelExecution === true &&
  ...
  "the six-context recovery execution is not authorized"
);
for (const [file, digest] of Object.entries(activation.sourceHashes)) {
  assertV4(sha256(await readFile(path.resolve(file))) === digest,
    `recovery source hash mismatch: ${file}`);
}
for (const future of activation.futureOutputPathsExcludedFromSourceHashes) {
  assertV4(!(await exists(future)), `future recovery output exists: ${future}`);
}
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(),
    `batch-05-publication-recovery-${context.contextIndex}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(),
    `batch-05-publication-recovery-home-${context.contextIndex}-`));
  ...
  try {
    const copies = [
      ...
      [activation.modelInputs.recoveryManual, "recovery-manual.md"],
    ...
    const task = context.contextType === "debate-189-two-field-repair"
      ? [
        `Act only as isolated Debate 189 bounded critique repair editor ${context.packetIndex}.`,
        "Rewrite exactly the two correctedCritiques named by the packet and schema.",
        "Preserve the supplied adjudicated substance and locked score band; every other publication field is immutable and unavailable.",
        "For each critique write exactly four ordered labeled sentences, target 112–118 words, remain within 105–130 words and at least 880 characters, and end every sentence with terminal punctuation. Count words before returning."
      ]
      : [
        `Act only as isolated Debate 109 ${context.side}-side score-locked publication resumption editor.`,
        `Author exactly the thirteen original content fields listed in the packet for ${context.shardId}.`,
        "Do not author the other shard, fixed publication fields, or any numerical score. Each original content field must be accepted exactly once.",
        "Apply the full publication manual: exact source quote, every supplied side move exactly once, exact four-sentence critique contract, Overall Commentary, and proportionate AI Extension."
      ];
    const prompt = [
      "Read production-workflow.md, readiness-workflow.md, output-contract.md, publication-manual.md, recovery-manual.md, packet.json, and schema.json completely and no other files.",
      ...task,
      "Participant judgment is closed. Scores are repository-owned and immutable; do not infer, emit, recalculate, or suggest changing a score.",
      "Use no external source, failed partial output, legacy assessment, other context, or other debate.",
      "Return exactly one schema-conforming JSON object and nothing else."
    ].join(" ");
    process.stdout.write(`[batch-05-publication-recovery] starting index ${context.contextIndex} ${context.debateNumber}/${context.contextType} ${activation.model.label}/${activation.model.reasoningEffort}\n`);
    ...
    const base = {
      contextIndex: context.contextIndex, contextType: context.contextType,
      debateNumber: context.debateNumber, debateId: context.debateId,
      packetIndex: context.packetIndex ?? null, shardId: context.shardId ?? null,
      side: context.side ?? null, model: activation.model.label,
      reasoningEffort: activation.model.reasoningEffort,
      attemptCount: 1, retryCount: 0, timeoutExtensionCount: 0,
      recursiveCorrectionContextCount: 0, startedAt,
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 ||
      ...
      try {
        ...
        validationSummary = context.debateNumber === "189"
      ...
      const validation = {
        schemaVersion: "1.0-assessment-production-post-canary-batch-05-publication-resumption-recovery-validation",
        protocolId: activation.protocolId, status: accepted ? "passed" : "failed",
        contextIndex: context.contextIndex, contextType: context.contextType,
        debateNumber: context.debateNumber, packetIndex: context.packetIndex ?? null,
        shardId: context.shardId ?? null, outputSha256: sha256(resultBytes),
        validationSummary, validationMessage, modelAuthoredScores: 0
      };
      const provenance = {
        schemaVersion: "1.0-assessment-production-post-canary-batch-05-publication-resumption-recovery-provenance",
        protocolId: activation.protocolId, contextIndex: context.contextIndex,
        contextType: context.contextType, debateNumber: context.debateNumber,
        packetIndex: context.packetIndex ?? null, shardId: context.shardId ?? null,
      ...
      record = { ...base, status: accepted ? "completed-valid" : "output-validation-failed",
        ...
        outputSha256: sha256(resultBytes), validationWritten: true,
        validationSha256: sha256(validationBytes), provenanceWritten: true,
        provenanceSha256: sha256(provenanceBytes), validationSummary, validationMessage };
    }
  } catch (error) {
    record = { contextIndex: context.contextIndex, contextType: context.contextType,
      debateNumber: context.debateNumber, debateId: context.debateId,
      model: "5.6 Sol", reasoningEffort: "low", attemptCount: 1,
      retryCount: 0, timeoutExtensionCount: 0, recursiveCorrectionContextCount: 0,
  ...
  process.stdout.write(`[batch-05-publication-recovery] index ${context.contextIndex} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
...
for (const phase of activation.executionPolicy.rampPhases) {
  if (!priorRampPassed) {
    phaseRecords.push({ ...phase, attemptedContextIndexes: [], validContextIndexes: [],
      passed: false, skippedBecausePriorRampFailed: true });
    continue;
  }
  const phaseContexts = phase.contextIndexes.map((index) => activation.contexts[index]);
  const phaseResults = await runPool(phaseContexts, phase.maximumParallelContexts);
  results.push(...phaseResults);
  const allValid = phaseResults.every((result) => result.gateAcceptancePassed);
  phaseRecords.push({ ...phase,
    attemptedContextIndexes: phaseResults.map((result) => result.contextIndex),
    validContextIndexes: phaseResults.filter((result) => result.gateAcceptancePassed)
...
const unattemptedContextIndexes = activation.contexts.map((context) => context.contextIndex)
...
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-05-publication-resumption-recovery-model-execution",
  protocolId: activation.protocolId,
  status: validContexts === 6
    ? "batch-05-six-context-publication-resumption-recovery-gate-passed"
    : "batch-05-publication-resumption-recovery-gate-complete-with-failure",
  gateStartedAt, gateCompletedAt: new Date().toISOString(), contextsPlanned: 6,
  ...
  recursiveCorrectionContexts: 0, maximumObservedConcurrency,
  schedulerRamp: [1, 2], wallElapsedMs,
  aggregateModelElapsedMs: results.reduce((sum, result) => sum + result.elapsedMs, 0),
  rampPhases: phaseRecords, results, originalFailedDebate189OutputPreserved: true,
  originalFailedDebate109PartialOutputReused: false,
  participantJudgmentWasScoreBlind: true, publicationWasScoreLocked: true,
  scoresImmutable: true, meteredApiCostUsd: 0, paidServiceCallsThisStage: 0,
  modelAuthoredScores: 0, scorePassesExecutedThisStage: 0,
  authorization: { deterministicAnalysis: true,
    deterministicMergeAndCompleteValidation: validContexts === 6,
    fourContextResumptionPreparation: validContexts === 6,
    retry: false, timeoutExtension: false, recursiveCorrectionModelExecution: false,
    paidServices: false, publicationCompilation: false, productionMutation: false,
