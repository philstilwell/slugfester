...
import {
  validatePostCanaryBatch06Scores
} from "./lib/assessment-production-post-canary-batch-06-score-gate.mjs";
import {
  buildPostCanaryBatch06PublicationPacket,
  buildPostCanaryBatch06PublicationSchema,
  postCanaryBatch06ReferenceCatalog,
  POST_CANARY_BATCH_06_PUBLICATION_DEBATES,
  POST_CANARY_BATCH_06_PUBLICATION_MODEL,
  POST_CANARY_BATCH_06_PUBLICATION_PROTOCOL_ID,
...
const SELECTION =
  "docs/assessment-production/post-canary-continuation-v1/batch-06/selection.json";
const FINAL_LEDGER =
...
const FINAL_LEDGER_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-06/final-ledger/final-ledger-manifest.json";
const FINAL_LEDGER_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-06/final-ledger/analysis.json";
const SCORES =
  "docs/assessment-production/post-canary-continuation-v1/batch-06/score-pass/calculated-scores.json";
const SCORE_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-06/score-pass/score-pass-manifest.json";
const SCORE_PREPARATION =
  "docs/assessment-production/post-canary-continuation-v1/batch-06/score-pass/score-pass-preparation-manifest.json";
const SCORE_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-06/score-pass/analysis.json";
const AUDIO_AUDIT =
  "docs/assessment-production/post-canary-continuation-v1/batch-06/audio-verification/audio-verification.json";
const SOURCE_PACKET_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-06/independent-judgments/source-packets";
...
const STATIC_SOURCE_FILES = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-06-score-gate.mjs",
  "scripts/lib/assessment-production-post-canary-batch-06-publication.mjs",
  "scripts/lib/assessment-production-post-canary-batch-06-publication-validation.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-06-publication.mjs",
  "scripts/test-assessment-production-post-canary-batch-06-publication-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-06-publication.mjs",
  "scripts/run-assessment-production-post-canary-batch-06-publication.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-06-publication.mjs"
];
...
assertV4(
  scoreAnalysis.status === "post-canary-batch-06-score-stability-gate-passed" &&
    scoreAnalysis.productionCanary === false &&
    scoreAnalysis.batchNumber === 6 &&
    ...
    scoreAnalysis.nextAuthorizedAction ===
      "standing-authorization-permits-batch-06-publication-packet-preparation",
  "the accepted Batch 6 score gate is unavailable or its stop boundary changed"
);
assertV4(
  scores.status === "post-canary-batch-06-single-score-pass-stability-gate-passed" &&
  ...
  "the locked Batch 6 score artifact is unavailable"
);
assertV4(
  scoreManifest.status ===
    ...
    scorePreparation.status ===
      "frozen-post-canary-batch-06-single-deterministic-score-pass-prepared-not-activated",
  "the frozen Batch 6 single-pass score controls changed"
);
assertV4(
  ledger.status ===
      "passed-post-canary-batch-06-deterministic-final-ledger-assembly" &&
    ledger.debates?.length === 10 &&
    ledger.audit?.finalMoves === 200 &&
    ...
    ledgerAnalysis.status ===
      "post-canary-batch-06-deterministic-final-ledger-gate-passed",
  "the locked Batch 6 final ledger changed"
);
assertV4(
  audioAudit.status ===
  ...
  "the two required Batch 6 audio checks are incomplete"
);
assertV4(
  productionManifest.model?.label === POST_CANARY_BATCH_06_PUBLICATION_MODEL.label &&
    productionManifest.model?.slug === POST_CANARY_BATCH_06_PUBLICATION_MODEL.slug &&
    productionManifest.model?.reasoningEffort ===
      POST_CANARY_BATCH_06_PUBLICATION_MODEL.reasoningEffort &&
    productionManifest.model?.authentication ===
      POST_CANARY_BATCH_06_PUBLICATION_MODEL.authentication,
  "the frozen production model or authentication changed"
);
assertV4(
  selection.selected?.map((item) => item.debateNumber).join(",") ===
      POST_CANARY_BATCH_06_PUBLICATION_DEBATES.join(",") &&
    selection.selected.every((item) => item.speakerCount === 2) &&
    selection.authorization?.productionMutation === false,
  "the frozen Batch 6 selection changed"
);
assertV4(
  ledger.debates.map((debate) => debate.debateNumber).join(",") ===
    ...
    scores.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_06_PUBLICATION_DEBATES.join(","),
  "the Batch 6 ledger or score order changed"
);
assertV4(
...
const finalLedgerInputs = await loadPostCanaryBatch06FinalLedgerInputs();
validatePostCanaryBatch06FinalLedger(
  ledger,
...
const catalog = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-06-local-reference-catalog",
  references: postCanaryBatch06ReferenceCatalog()
};
...
for (const debateNumber of POST_CANARY_BATCH_06_PUBLICATION_DEBATES) {
  ...
  const packet = buildPostCanaryBatch06PublicationPacket({
    ledgerDebate,
    scoreDebate,
    sourcePacket,
  ...
  const schema = buildPostCanaryBatch06PublicationSchema(packet);
...
assertV4(
  contexts.reduce((sum, context) => sum + context.moves, 0) === 200,
  "Batch 6 publication move coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.sections, 0) === 52,
  "Batch 6 publication section coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.audioVerifiedMoves, 0) === 2,
  "Batch 6 audio-verification coverage changed"
);
assertV4(
  contexts.every(
    (context) =>
  ...
  "Batch 6 publication section or quote eligibility is invalid"
);
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-06-publication-execution-preparation-manifest",
  protocolId: POST_CANARY_BATCH_06_PUBLICATION_PROTOCOL_ID,
  status:
    "frozen-ten-post-canary-batch-06-score-locked-publication-contexts-prepared-not-activated",
  frozenAt,
  ...
  userAuthorization: {
    instruction:
      "The frozen Batch 6 standing authorization permits preparation, activation, and execution of exactly ten score-locked publication-reconstruction contexts while every gate passes, with direct incremental cost capped at $0.",
  ...
  model: structuredClone(POST_CANARY_BATCH_06_PUBLICATION_MODEL),
  costEstimate: {
  ...
  acceptanceContract: {
    ...
    movesAuthoredRequired: 200,
    critiquesRequired: 200,
  ...
  nextAuthorizedAction:
    "standing-authorization-permits-activation-and-execution-of-the-ten-frozen-batch-06-publication-contexts"
};
