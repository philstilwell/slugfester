...
assertV4(
  ...
  "Batch 4 publication analysis boundary changed"
);
...
const passed =
  execution.status === "ten-post-canary-batch-04-publication-contexts-passed" &&
  execution.contextsAttempted === 10 &&
  execution.contextsUnattempted === 0 &&
  execution.validContexts === 10 &&
  execution.invalidContexts === 0 &&
  valid.length === 10 &&
  valid.reduce((sum, item) => sum + item.validation.critiques, 0) === 203 &&
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-publication-analysis",
  protocolId: activation.protocolId,
  status: passed
    ? "post-canary-batch-04-publication-output-gate-passed"
  ...
  nextAuthorizedAction: passed
    ? "standing-authorization-permits-batch-04-publication-compilation-preparation"
