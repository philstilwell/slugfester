...
assert.equal(manifest.protocolId, POST_CANARY_BATCH_07_PUBLICATION_PROTOCOL_ID);
...
assert.equal(manifest.totals.moves, 187);
...
for (const context of manifest.contexts) {
  ...
  assert.equal(packet.protocolId, POST_CANARY_BATCH_07_PUBLICATION_PROTOCOL_ID);
  ...
  assert.equal(schema.properties.schemaVersion.const, POST_CANARY_BATCH_07_PUBLICATION_OUTPUT_VERSION);
  assert.equal(schema.properties.protocolId.const, POST_CANARY_BATCH_07_PUBLICATION_PROTOCOL_ID);
