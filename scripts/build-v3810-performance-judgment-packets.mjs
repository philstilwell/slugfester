...
const workflowPath = "docs/assessment-workflow-v3.8.10.md";
const rubricPath = "docs/reassessment-rubric-v3.8.10.md";
const manualPath = `${V3810_PERFORMANCE_ROOT}/manual.md`;
const preregistrationPath = `${V3810_PERFORMANCE_ROOT}/preregistration.md`;
...
assertV3810(coverage.schemaVersion === "3.8.8-final-coverage-inventory" && coverage.selectedMoveCount === 81, "inherited coverage lock invalid");
assertV3810(sectionLock.schemaVersion === "3.8.8-locked-section-weight-plans" && sectionLock.assignedMoveCount === 81, "inherited section lock invalid");
assertV3810(contactLock.schemaVersion === "3.8.8-locked-burden-contact-ledger" && contactLock.moveCount === 81 && contactLock.unresolvedMoves === 0, "inherited burden-contact lock invalid");
assertV3810(contactAnalysis.passed === true && contactAnalysis.decision.performanceJudgmentPreregistrationAuthorized === true, "burden-contact checkpoint did not authorize performance preregistration");
assertV3810(contactAnalysis.decision.numericalParticipantScoringAuthorized === false, "scores unexpectedly authorized");
assertV3810(sourceAudit.totals.pendingAudioVerifications === 0, "medium-confidence source move remains unverified");
...
for (const debateNumber of V3810_PERFORMANCE_DEBATES) {
  ...
  assertV3810(coverageDebate && sectionDebate && contactDebate && sourceChain, `${debateNumber}: incomplete locked source set`);
  assertV3810(coverageDebate.debateId === sectionDebate.debateId && coverageDebate.debateId === contactDebate.debateId, `${debateNumber}: debate identity mismatch`);
  ...
  assertV3810(assignments.size === coverageDebate.moves.length && contactByMove.size === coverageDebate.moves.length, `${debateNumber}: move lock cardinality mismatch`);
  ...
  const moves = coverageDebate.moves.map((move) => {
    ...
    assertV3810(assignment && contact, `${move.moveId}: missing section or contact lock`);
    assertV3810(canonicalJson(move.sourceSpan) === canonicalJson(contact.sourceSpan), `${move.moveId}: source span changed between locks`);
    assertV3810(move.attributionConfidence === "high" && move.audioVerification === null, `${move.moveId}: audio verification boundary not satisfied`);
    const responseTargets = move.respondsToRefs.map((targetId) => {
      const target = coverageByMove.get(targetId);
      assertV3810(target, `${move.moveId}: unavailable response target ${targetId}`);
      return {
  ...
  assertV3810(sha256(transcriptBytes) === sourceChain.transcriptSha256, `${debateNumber}: transcript hash mismatch`);
  assertV3810(sha256(eventBytes) === sourceChain.eventsSha256, `${debateNumber}: event hash mismatch`);
  assertV3810(sha256(manifestBytes) === sourceChain.localManifestSha256, `${debateNumber}: caption manifest hash mismatch`);
  const packet = {
    schemaVersion: "3.8.10-performance-judgment-packet",
    ...
    modelInputBoundary: {
      ...
      sharedSchemaPath: `${V3810_PERFORMANCE_ROOT}/performance-judgment-schema.json`,
      ...
      priorV388AndV389JudgmentsScoresAndProseUnavailable: true
    }
  };
  packets.push(packet);
  if (shouldWrite) {
    const outputPath = path.resolve(root, `${V3810_PERFORMANCE_ROOT}/packets/debate-${debateNumber}.json`);
...
const manifest = {
  schemaVersion: "3.8.10-performance-judgment-preparation",
  protocolId: "v3.8.10-performance-judgment-consensus",
...
if (shouldWrite) {
  await mkdir(path.resolve(root, V3810_PERFORMANCE_ROOT), { recursive: true });
  await writeFile(path.resolve(root, `${V3810_PERFORMANCE_ROOT}/performance-judgment-schema.json`), `${JSON.stringify(schema, null, 2)}\n`);
  await writeFile(path.resolve(root, `${V3810_PERFORMANCE_ROOT}/preparation-manifest.json`), `${JSON.stringify(manifest, null, 2)}\n`);
}
