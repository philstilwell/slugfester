...
import { V223_AUDIO_ADJ_ROOT } from "./lib/assessment-production-score-stability-v2.2.3-audio-attribution-adjudication.mjs";
import {
...
const manifest = JSON.parse(
  await readFile(`${V223_AUDIO_ADJ_ROOT}/execution-manifest.json`, "utf8")
);
...
assertV4(
  manifest.status ===
      "frozen-one-v2.2.3-audio-attribution-context-authorized" &&
    ...
    manifest.context.debateNumber === "17" &&
    manifest.context.disputedMoves.length === 1 &&
    manifest.context.disputedMoves[0] ===
      "pro-cumulative-moral-christian-case",
  "v2.2.3 audio-attribution execution unauthorized"
);
...
const temporary = await mkdtemp(
  path.join(os.tmpdir(), "slugfester-v223-audio-adj-")
);
const temporaryCodexHome = await mkdtemp(
  path.join(os.tmpdir(), "slugfester-v223-audio-home-")
);
...
try {
  ...
  const prompt =
    "Read workflow.md, manual.md, schema.json, packet.json, audio-transcript.json, and diagnosis.json completely and no other files. Act only as the isolated 5.6 Sol/low audio-attribution adjudicator for Debate 17. Decide only whether the expected speaker authored the locked core proposition and cite exact diarized segment indexes. Ratings, scores, legacy data, other debates, winners, and publication prose are unavailable. Return exactly one schema-conforming JSON object and no commentary.";
  ...
  const transportClassification = classifyTransportEventCount(
    events.length,
    2,
    8
  );
  ...
  if (
    ...
    const valid =
      validation.code === 0 && transportClassification !== "invalid";
    record = {
      ...base,
      status: valid
...
const execution = {
  schemaVersion:
    "1.0-score-stability-v2.2.3-audio-attribution-adjudication-model-execution",
  protocolId: manifest.protocolId,
  status: record.gateAcceptancePassed
    ? "v2.2.3-audio-attribution-adjudication-execution-passed"
    : "v2.2.3-audio-attribution-adjudication-execution-failed",
  contexts: 1,
  attempts: 1,
  retries: 0,
  meteredApiCostUsd: 0,
...
assertV4(
  ...
  "v2.2.3 audio-attribution adjudication failed; downstream work is blocked"
);
