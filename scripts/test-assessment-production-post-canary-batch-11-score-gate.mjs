...
import {
  ...
  evaluatePostCanaryBatch11ScoreStability
...
const accepted = evaluatePostCanaryBatch11ScoreStability(passing);
...
assert.equal(
  evaluatePostCanaryBatch11ScoreStability(winnerMutation).acceptancePassed,
  false
);
const distanceMutation = structuredClone(passing);
distanceMutation[2].final = scores(95, 80);
assert.equal(
  evaluatePostCanaryBatch11ScoreStability(distanceMutation).acceptancePassed,
  false
);
assert.throws(() =>
  evaluatePostCanaryBatch11ScoreStability(passing, {
    ...POST_CANARY_BATCH_11_SCORE_STABILITY_THRESHOLDS,
