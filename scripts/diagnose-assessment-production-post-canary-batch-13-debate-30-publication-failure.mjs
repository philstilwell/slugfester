...
const resumptionRoot = `${root}/original-unattempted-context-resumption-3`;
const outputPath = `${root}/outputs/debate-30.json`;
const packetPath = `${root}/packets/debate-30.json`;
const executionPath = `${resumptionRoot}/model-execution.json`;
const validationPath = `${root}/validations/debate-30.json`;
const analysisPath = `${resumptionRoot}/debate-30-failure-analysis.json`;
...
const failed = execution.results.find((item) => item.debateNumber === "30");
assertV4(execution.status === "five-context-publication-resumption-stopped-with-failure" && execution.contextsAttempted === 1 && execution.validContexts === 0 && execution.contextsUnattempted === 4 && failed?.status === "output-validation-failed" && failed.outputSha256 === sha256(outputBytes) && failed.retryCount === 0, "Debate 30 failure boundary changed");
assertV4(validationRecord.status === "failed" && validationRecord.outputSha256 === sha256(outputBytes), "Debate 30 failed validation record changed");
...
assertV4(structuralStandIn, "Debate 30 has no validation-clean critique stand-in");
...
assertV4(nonTargetValidation.status === "passed", "Debate 30 non-target fields do not validate");
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-13-debate-30-publication-failure-analysis",
  protocolId: "assessment-production-post-canary-batch-13-debate-30-publication-field-disjoint-repair-1",
  status: "debate-30-publication-failure-diagnosed-awaiting-field-disjoint-repair-1",
  debateNumber: "30",
  ...
  authorization: { fieldDisjointRepairLevel1Preparation: true, attemptsPerShard: 1, retries: false, writableFieldsMaximumPerShard: 2, resumeFourOriginalUnattemptedContextsAfterPassingRepair: true, scorePass: false, productionMutation: false, nextBatchSelection: false },
  nextAuthorizedAction: "prepare-minimum-field-disjoint-debate-30-publication-repair-shards"
};
