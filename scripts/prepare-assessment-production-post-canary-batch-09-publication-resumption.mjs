...
import {
  validatePostCanaryBatch09PublicationOutput
} from "./lib/assessment-production-post-canary-batch-09-publication-validation.mjs";
import {
  POST_CANARY_BATCH_09_PUBLICATION_RESUMPTION_DEBATES,
...
const RESUMPTION_ROOT = POST_CANARY_BATCH_09_PUBLICATION_RESUMPTION_ROOT;
...
const REPAIRED_DEBATE_170 = `${ROOT}/repair-1/merged/debate-170.json`;
const DEBATE_170_PACKET = `${ROOT}/packets/debate-170.json`;
const PREPARATION_TEST_FAILURE_DIAGNOSIS =
  `${RESUMPTION_ROOT}/preparation-test-failure-diagnosis.json`;
const PREPARATION_TEST_CORRECTION_PLAN =
  `${RESUMPTION_ROOT}/preparation-test-correction-1-plan.json`;
...
const repairedDebate170 = value(REPAIRED_DEBATE_170);
const debate170Packet = value(DEBATE_170_PACKET);
...
assertV4(
  originalPreparation.status ===
    ...
    originalPreparation.totals?.moves === 180 &&
  ...
  "the original Batch 9 publication preparation changed"
);
assertV4(
  originalExecution.status ===
    ...
    diagnosis.failedContext?.contextIndex === 0 &&
    diagnosis.failedContext?.debateNumber === "170" &&
    canonicalJson(
      originalPreparation.contexts
        .slice(1)
        .map((context) => context.debateNumber)
    ) === canonicalJson(POST_CANARY_BATCH_09_PUBLICATION_RESUMPTION_DEBATES),
...
assertV4(
  repairPreparation.status ===
    ...
    repairAnalysis.status ===
      "batch-09-debate-170-bounded-repair-and-complete-publication-validation-passed" &&
    ...
    repairValidation.validationSummary?.moves === 20 &&
  ...
  "the accepted Debate 170 repair boundary changed"
);
assertV4(
  validatePostCanaryBatch09PublicationOutput(
      repairedDebate170,
  ...
  "the repaired Debate 170 staging output failed replay"
);
...
for (
  ...
  resumptionIndex < POST_CANARY_BATCH_09_PUBLICATION_RESUMPTION_DEBATES.length;
  resumptionIndex += 1
) {
  const debateNumber =
    POST_CANARY_BATCH_09_PUBLICATION_RESUMPTION_DEBATES[resumptionIndex];
...
const newStaticSources = [
  ...
  REPAIRED_DEBATE_170,
  DEBATE_170_PACKET,
  PREPARATION_TEST_FAILURE_DIAGNOSIS,
  PREPARATION_TEST_CORRECTION_PLAN,
  "scripts/lib/assessment-production-post-canary-batch-09-publication-resumption.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-09-publication-resumption.mjs",
  "scripts/test-assessment-production-post-canary-batch-09-publication-resumption-preparation.mjs",
...
const stopRules = {
  acceptedDebate170RepairFailureBlocks: true,
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-09-publication-resumption-execution-preparation-manifest",
  protocolId: POST_CANARY_BATCH_09_PUBLICATION_RESUMPTION_PROTOCOL_ID,
  status:
  ...
  userAuthorization: {
    instruction:
      "The Batch 9 standing authorization permits preparation, activation, execution, validation, analysis, committing, and pushing of the nine untouched score-locked publication-resumption contexts at direct incremental cost $0.",
  ...
  inputs: {
    ...
    preparationTestFailureDiagnosis: PREPARATION_TEST_FAILURE_DIAGNOSIS,
    preparationTestCorrectionPlan: PREPARATION_TEST_CORRECTION_PLAN,
    repairedDebate170: REPAIRED_DEBATE_170,
    debate170Packet: DEBATE_170_PACKET
  },
  ...
  acceptedDebate170: {
    ...
    packet: DEBATE_170_PACKET,
    output: REPAIRED_DEBATE_170,
  ...
  isolation: {
    ...
    repairedDebate170OutputUnavailableToResumptionModels: true,
    failedOriginalDebate170OutputUnavailable: true,
  ...
  deterministicValidation: {
    ...
    acceptedDebate170RepairReplayedAtFreeze: true,
  ...
  acceptanceContract: {
    ...
    resumptionMovesRequired: 160,
    cohortMovesRequired: 180,
    resumptionCritiquesRequired: 160,
    cohortCritiquesRequired: 180,
