...
assertV4(
  ...
  "v2.1.3 adjudication execution unavailable or crossed its boundary"
);
for (const [file, digest] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    sha256(await readFile(file)) === digest,
    `source hash mismatch during v2.1.3 adjudication analysis: ${file}`
  );
}
...
for (const context of manifest.contexts) {
  ...
  if (result.gateAcceptancePassed) {
    replay = validateV213DisputeAdjudicationOutput(
    ...
    assertV4(
      ...
      `${context.debateNumber}: v2.1.3 adjudication replay mismatch`
    );
  }
  contexts.push({
    debateNumber: context.debateNumber,
...
const analysis = {
  schemaVersion:
    "1.0-score-stability-v2.1.3-dispute-only-adjudication-analysis",
  protocolId: manifest.protocolId,
  status: passed
    ? "v2.1.3-dispute-only-adjudication-gate-passed"
    : semanticPass
      ? timingPass
        ? "v2.1.3-adjudication-gate-failed-scoreblindness"
        : "v2.1.3-adjudication-gate-failed-timing"
      : "v2.1.3-adjudication-gate-failed-validation",
  ...
  nextAuthorizedAction: passed
    ? "assemble-v2.1.3-final-ledgers-deterministically"
    : "diagnose-v2.1.3-dispute-adjudication-failure-only"
};
if (shouldWrite) {
  await writeFile(
