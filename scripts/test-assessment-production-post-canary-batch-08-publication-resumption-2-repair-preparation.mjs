...
import { POST_CANARY_BATCH_08_DEBATE_120_REPAIR_PROTOCOL_ID,
  POST_CANARY_BATCH_08_DEBATE_120_REPAIR_ROOT, buildDebate120RepairSchema } from
  "./lib/assessment-production-post-canary-batch-08-publication-resumption-2-repair.mjs";
const MANIFEST = `${POST_CANARY_BATCH_08_DEBATE_120_REPAIR_ROOT}/execution-preparation-manifest.json`;
const exists = (file) => access(path.resolve(file)).then(() => true, () => false);
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
if (!(await exists(MANIFEST))) { console.log(JSON.stringify({ status: "batch-08-debate-120-repair-test-ready" })); process.exit(0); }
const bytes = await readFile(path.resolve(MANIFEST)); const manifest = JSON.parse(bytes);
assert.equal(manifest.protocolId, POST_CANARY_BATCH_08_DEBATE_120_REPAIR_PROTOCOL_ID);
assert.equal(manifest.contexts.length, 2); assert.deepEqual(manifest.contexts.map((row) => row.writableFieldCount), [2, 1]);
assert.equal(new Set(manifest.contexts.flatMap((row) => row.writableFields)).size, 3);
...
for (const context of manifest.contexts) { const packetBytes = await readFile(path.resolve(context.packet));
  ...
  assert.deepEqual(JSON.parse(schemaBytes), buildDebate120RepairSchema(JSON.parse(packetBytes))); }
console.log(JSON.stringify({ status: "batch-08-debate-120-repair-test-passed",
  manifestSha256: sha256(bytes), contexts: 2, writableFields: 3,
