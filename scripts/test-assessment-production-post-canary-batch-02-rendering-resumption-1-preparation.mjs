...
const failureBytes = await readFile(
  path.resolve(POST_CANARY_BATCH_02_RENDERING_RESUMPTION_1_FAILURE)
);
...
assert.equal(preparation.userAuthorization.instruction, "Continue.");
...
assert.equal(
  failure.status,
  "preserved-batch-02-bootstrap-only-transport-readiness-failure"
);
assert.equal(failure.affectedViewport.debateNumber, "103");
...
assert.equal((await loadPostCanaryBatch02RenderingResumption1Packets(preparation)).size, 10);
...
console.log(
  JSON.stringify(
    {
      status: "post-canary-batch-02-rendering-resumption-1-preparation-passed",
