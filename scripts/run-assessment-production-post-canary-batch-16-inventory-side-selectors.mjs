...
assertV4(
  activation.status ===
      "frozen-twenty-post-canary-batch-16-side-selector-contexts-authorized" &&
  ...
  "post-canary Batch 16 side-selector execution is unauthorized"
);
...
async function executeContext(context, contextIndex) {
  ...
  const sourceDirectory = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-post-canary-batch-16-side-${context.debateNumber}-${context.side}-`
    )
  );
  const isolatedCodexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-post-canary-batch-16-side-home-${context.debateNumber}-${context.side}-`
    )
  );
  ...
  process.stdout.write(
    `[post-canary-batch-16-side-selector] Debate ${context.debateNumber} ${context.side} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-16-side-selector-model-execution",
  protocolId: activation.protocolId,
  status:
    validContexts === preparation.contexts.length
      ? "twenty-post-canary-batch-16-side-selector-contexts-passed"
      : "post-canary-batch-16-side-selector-gate-complete-with-failure",
  ...
  nextRequiredAction:
    validContexts === preparation.contexts.length
      ? "analyze-compile-and-lock-post-canary-batch-16-inventories-model-free-only"
      : "stop-preserve-failed-post-canary-batch-16-side-selector-gate",
};
