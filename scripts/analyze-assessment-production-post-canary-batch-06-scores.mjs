...
import {
  POST_CANARY_BATCH_06_CALCULATED_SCORES_VERSION,
...
const manifest = JSON.parse(
  await readFile(
    path.resolve(`${POST_CANARY_BATCH_06_SCORE_ROOT}/score-pass-manifest.json`),
    "utf8"
  )
);
...
const validation = validatePostCanaryBatch06Scores(
  scores,
  ledger,
  inputs.debateInputs,
...
assertV4(
  scores.schemaVersion === POST_CANARY_BATCH_06_CALCULATED_SCORES_VERSION &&
    !scores.productionCanary &&
    scores.batchNumber === 6 &&
    scores.stagingOnly &&
    !scores.developmentValidationOnly &&
    scores.formulaBoundary.scoringPasses === 1 &&
    !scores.formulaBoundary.modelCalculatedScores,
  "post-canary Batch 6 score artifact identity mismatch"
);
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-06-score-analysis",
  protocolId: scores.protocolId,
  status: passed
    ? "post-canary-batch-06-score-stability-gate-passed"
    : "post-canary-batch-06-score-stability-gate-failed",
  ...
  nextAuthorizedAction: passed
    ...
    : "standing-authorization-permits-batch-06-score-stability-failure-diagnosis"
};
if (shouldWrite) {
  await writeFile(
