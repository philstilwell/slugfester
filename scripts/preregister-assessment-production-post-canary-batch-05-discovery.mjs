...
const BATCH_ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-05";
...
const PROTOCOL_ID = "assessment-production-post-canary-batch-05-discovery";
const WORKFLOW = "docs/assessment-production-canary-discovery-workflow.md";
const SCRIPT = "scripts/preregister-assessment-production-post-canary-batch-05-discovery.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-05-discovery-manifest.mjs";
const CODEX_PATH = "/Applications/ChatGPT.app/Contents/Resources/codex";
const REQUIRED_ORDER = ["158", "46", "64", "132", "189", "109", "179", "05", "42", "59"];
...
assertV4(
  preparation.status === "post-canary-batch-05-ten-complete-score-blind-source-packets-prepared-awaiting-validation" &&
    preparationValidation.status === "post-canary-batch-05-score-blind-source-packet-validation-passed-frozen-awaiting-user-approval-before-any-model-execution" &&
    ...
    preparation.totals.maximumCopiedInputBytes === 66284 &&
  ...
  "validated Batch 5 source preparation is not frozen at the required authorization boundary"
);
assertV4(
  ...
  "Batch 5 standing authorization does not cover exact discovery execution"
);
assertV4(
  JSON.stringify(preparation.contexts.map((debate) => debate.debateNumber)) === JSON.stringify(REQUIRED_ORDER),
  "Batch 5 debate order drifted"
);
assertV4(
  ...
  for (const chunk of debate.chunks) {
    ...
    assertV4(
      sha256(chunkBytes) === chunk.chunkLedgerSha256 &&
        ...
        buildBatch05TokenCountedChunkLedger(chunkBytes).equals(tokenBytes) &&
...
assertV4(contexts.length === 41, "Batch 5 discovery must contain exactly 41 contexts");
...
assertV4(maximumContext.debateNumber === "109" && maximumContext.chunkId === "chunk-004", "maximum copied-input context drifted");
...
const sourceFiles = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-05-source-preparation.mjs",
...
const manifest = {
  ...
  status: shouldWrite ? "frozen-forty-one-post-canary-batch-05-discovery-contexts-prepared-not-authorized" : "preview",
  ...
  AIOnly: true,
  userAuthorization: {
    scope: "standing-authorized Batch 5 discovery preparation and execution",
  ...
  costEstimate: {
    ...
    estimateBasis: "The exact 41-context Batch 5 input set is hash-locked with an observed maximum of 66,284 copied bytes; no input is truncated or repartitioned. The frozen 1→2→4 scheduler uses the subscription-backed route with a $0 direct incremental cost boundary.",
  },
  ...
  nextAuthorizedAction: "prepare-and-freeze-batch-05-discovery-execution-activation-under-standing-authorization",
};
