...
const [selection, manifest] = await Promise.all(
  [selectionPath, manifestPath].map((file) =>
...
for (const [file, expected] of Object.entries(selection.sourceHashes)) {
  assert.equal(
    ...
    expected,
    `${file}: production checkpoint selection source hash mismatch`
  );
}
...
const policyHash = selection.activePolicy.normativeTextSha256;
const promotionHash = selection.activePolicy.promotionSha256;
const expectedRank = manifest.items
  ...
  .map((item) => ({
    ...
    rankSha256: sha256(
      Buffer.from(
        `${policyHash}|${promotionHash}|${item.debateNumber}|${item.debateId}`
      )
    ),
  }))
  .sort(
...
assert.equal(selection.historicalDisposition.failedV1CanaryReclassified, false);
