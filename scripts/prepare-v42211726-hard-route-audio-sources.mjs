...
const planRoot = "docs/calibration/v4.2.21.17.26/hard-route-disagreement-audio-prep";
const localRoot = "output/transcribe/v42211726-audio-verification/debate-153";
...
assertV4(analysis.status === "hard-route-deterministic-disagreements-extracted-audio-source-preparation-authorized" && analysis.authorization.audioSourcePreparation, "hard-route audio source preparation is not authorized");
assertV4(work.moves.length === 3 && work.moves.every((move) => move.debateNumber === "153"), "expected exactly three Debate 153 audio work items");
...
if (!(await exists(sourceAudio))) {
  ...
  assertV4(downloaded.length === 1, "expected one downloaded Debate 153 media file");
...
assertV4(Number.isFinite(sourceDurationSeconds) && sourceDurationSeconds * 1000 >= Math.max(...work.moves.map((move) => move.clipWindow.endMs)), "normalized Debate 153 source audio is too short");
...
const preparation = {
  schemaVersion: "4.2.21.17.26-hard-route-audio-source-preparation",
  protocolId: analysis.protocolId,
  status: "prepared-three-local-hard-route-audio-clips",
  ...
  authorization: { paidTranscriptionManifest: true, paidTranscriptionExecution: false, audioVerificationExecution: false, adjudicationPacketPreparation: false, adjudicationModelExecution: false, scoreDerivation: false, all195Debates: false },
};
