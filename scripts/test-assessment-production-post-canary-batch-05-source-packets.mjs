...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-05/source-preparation";
...
assert.equal(preparation.status, "post-canary-batch-05-ten-complete-score-blind-source-packets-prepared-awaiting-validation");
...
assert.equal(preparation.totals.zeroLexicalTokenRowOccurrences, 0);
...
for (const context of preparation.contexts) {
  ...
  assert(frozen, `${context.debateNumber}: absent from frozen Batch 5 selection`);
  ...
  assert.equal(packet.schemaVersion, "1.0-assessment-production-post-canary-batch-05-score-blind-source-packet");
  ...
  for (const chunk of context.chunks) {
    ...
    assert.deepEqual(
      ...
      findBatch05ZeroLexicalTokenRows(chunkBytes).map(({ eventIndex, startMs, durationMs, text }) => ({
        eventIndex,
        startMs,
        durationMs,
...
const validation = {
  ...
  status: "post-canary-batch-05-score-blind-source-packet-validation-passed-frozen-awaiting-user-approval-before-any-model-execution",
