...
import {
  ...
  derivePostCanaryBatch05Scores
} from "./lib/assessment-production-post-canary-batch-05-score-gate.mjs";
import { assertV4 } from "./lib/v4-lean-production.mjs";

const manifestPath = `${POST_CANARY_BATCH_05_SCORE_ROOT}/score-pass-manifest.json`;
...
assertV4(
  ...
  "post-canary Batch 5 score manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen Batch 5 score source hash mismatch`
  );
}
...
const calculated = derivePostCanaryBatch05Scores(
  ledger,
  inputs.debateInputs,
