...
const planPath = `${root}/audio-source-range-recovery-3/correction-plan.json`;
const activationPath = `${root}/audio-source-range-recovery-3/execution-activation.json`;
...
assert.equal(plan.userAuthorization.rangeVerifiedDownloadsAuthorized, 3);
...
assert.equal(plan.rangeVerifiedSources.length, 3);
assert.deepEqual(
  plan.rangeVerifiedSources.map((item) => item.sourceVideoId),
  ["8k9A7d2Wnjk", "h-I_9e5qxnc", "0IpKHdVLZb4"]
);
assert.deepEqual(
  plan.rangeVerifiedSources.map((item) => item.declaredBytes),
  [37050541, 24192081, 54678961]
);
assert(plan.rangeVerifiedSources.every((item) => item.formatId === "139"));
assert(plan.rangeVerifiedSources.every((item) => item.protocol === "https"));
assert(plan.rangeVerifiedSources.every((item) => item.audioCodec === "mp4a.40.5"));
assert.deepEqual(
  plan.rangeVerifiedSources.map((item) => item.rangeHttpGetInvocationsMaximum),
  [5, 3, 7]
);
assert(plan.rangeVerifiedSources.every((item) => item.rangeChunkBytes === 8388608));
assert(plan.rangeVerifiedSources.every((item) => item.curlRetriesMaximum === 0));
assert.equal(plan.transportDiagnosis.failedFormatId, "18");
assert.equal(plan.transportDiagnosis.selectedFallbackFormatId, "139");
...
console.log(JSON.stringify({
  ...
  sources: 5,
  clips: 9,
  rangeVerifiedDownloads: 3,
  protectedInvalidFiles: 2,
