...
assert.equal(selection.status, "seventh-post-canary-ten-debate-batch-selection-frozen-source-gate-passed");
assert.equal(analysis.status, "seventh-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization");
assert.equal(analysis.selection.sha256, sha256(selectionBytes));
assert.equal(selection.batchNumber, 7);
assert.equal(selection.productionSequenceOrdinal, 8);
...
assert.equal(selection.eligibility.previouslyPublishedAndRemovedCount, 60);
assert.equal(selection.eligibility.remainingBeforeBatchCount, 104);
assert.equal(selection.eligibility.selectedCount, 10);
assert.equal(selection.eligibility.remainingUnselectedCount, 94);
...
assert.equal(priorCompletionExecution.status, "passed-complete-batch-06-transaction-ready-for-atomic-commit");
...
assert.equal(priorCompletionExecution.totals.automaticRepairs, 0);
assert.equal(priorCompletionExecution.totals.rollbacks, 0);
...
assert.equal(priorCompletionAnalysis.status, "batch-06-production-publication-accepted-for-atomic-commit-after-generated-seo-correction");
assert.equal(priorCompletionAnalysis.decision.generatedSeoCorrectionGatePassed, true);
assert.equal(priorCompletionAnalysis.decision.batch06ProductionPublicationGatePassed, true);
...
assert.equal(sha256(serializedJson(ranked.slice(60))), selection.eligibility.remainingBeforeBatchRankedSha256);
assert.deepEqual(priorSelection.selected.map((item) => item.debateNumber), ranked.slice(50, 60).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.debateNumber), ranked.slice(60, 70).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.rankOrdinal), [61, 62, 63, 64, 65, 66, 67, 68, 69, 70]);
assert.deepEqual(selection.selected.map((item) => item.rankSha256), ranked.slice(60, 70).map((item) => item.rankSha256));
...
assert.deepEqual(selection.deterministicOrdering.selectedRankOrdinals, [61, 70]);
...
assert.equal(selection.nextAuthorizedAction, "prepare-batch-07-standing-authorization-and-source-packets-under-user-authorization");

console.log(JSON.stringify({ status: "passed", selectedDebates: selection.selected.map((item) => item.debateNumber), selectedRankOrdinals: [61, 70], sourceFiles: 30, replacements: 0, sourcePacketsPrepared: 0, modelContexts: 0, paidServiceCalls: 0, directCostUsd: 0, nextAuthorizedAction: selection.nextAuthorizedAction }, null, 2));
