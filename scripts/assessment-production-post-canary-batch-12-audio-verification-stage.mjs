...
const stageRoot = "docs/assessment-production/post-canary-continuation-v1/batch-12/audio-verification";
const paths = {
...
const expectedDebates = ["152", "07", "15"];
const expectedMoves = [
  "pro-evidence-experience-epistemology",
  "pro-doubled-kyrios-veiled-claim",
  "pro-synoptic-disclosure-early-christology",
  "con-natural-inquiry-falsifiability"
];
const expectedCalls = 4;
const durationOnlyPlanningEstimateUsd = 0.0470241;
const usageDerivedPlanningEstimateUsd = 0.171415;
const toolPath = "scripts/assessment-production-post-canary-batch-12-audio-verification-stage.mjs";
...
async function activate() {
  ...
  const activation = {
    ...preparation,
    schemaVersion: "1.0-assessment-production-post-canary-batch-12-audio-verification-execution-manifest",
    status: "frozen-four-post-canary-batch-12-paid-known-speaker-diarizations-authorized-under-standing-authorization",
    activatedAt,
    ...
    nextAuthorizedAction: "execute-exactly-four-post-canary-batch-12-paid-audio-verification-calls-once-sequentially"
  };
...
async function validateActivation(manifest) {
  assert.equal(
    manifest.status,
    "frozen-four-post-canary-batch-12-paid-known-speaker-diarizations-authorized-under-standing-authorization"
  );
...
async function run() {
  ...
  for (const [callIndex, call] of manifest.calls.entries()) {
    ...
    process.stdout.write(`[batch-12-audio] ${callIndex + 1}/${expectedCalls} starting ${call.debateNumber}:${call.moveId}\n`);
    ...
    results.push(result);
    process.stdout.write(`[batch-12-audio] ${callIndex + 1}/${expectedCalls} ${result.status}; cumulative $${result.cumulativeUsageDerivedEstimatedCostUsd.toFixed(7)}\n`);
  }
  ...
  const execution = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-12-audio-verification-model-execution",
    protocolId: manifest.protocolId,
    status: completed.length === expectedCalls
      ? "four-post-canary-batch-12-paid-known-speaker-diarizations-completed"
      : "post-canary-batch-12-paid-diarization-incomplete",
    provider: "OpenAI Transcription API",
...
async function analyze() {
  ...
  const status = passed
    ...
    : executionComplete
      ? "post-canary-batch-12-audio-verification-unresolved"
      : "post-canary-batch-12-audio-verification-incomplete";
  const nextAuthorizedAction = execution.costCapReachedOrExceeded || !execution.directIncrementalCostCapControlPassed
    ? "user-review-required-before-any-batch-12-downstream-work-after-audio-cost-cap-event"
    : passed
      ? "prepare-freeze-and-push-batch-12-dispute-only-adjudication-packets-under-standing-authorization"
      : "standing-authorization-stop-new-approval-required-before-batch-12-audio-verification-failure-diagnosis";
  const authorization = {
  ...
  const audit = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-12-audio-verification-audit",
    protocolId: manifest.protocolId,
    status,
  ...
  const analysis = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-12-audio-verification-analysis",
    protocolId: manifest.protocolId,
    status,
    productionCanary: false,
    batchNumber: 12,
...
async function cost() {
  ...
  const result = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-12-audio-cost-control-analysis",
