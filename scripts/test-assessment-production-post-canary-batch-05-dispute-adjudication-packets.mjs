...
import {
  POST_CANARY_BATCH_05_DISPUTE_ADJ_ISOLATION,
  POST_CANARY_BATCH_05_DISPUTE_ADJ_OUTPUT_VERSION,
...
const EXPECTED_DEBATES = ["158", "46", "64", "132", "189", "109", "179", "05", "42", "59"];
...
const preparation = JSON.parse(
  await readFile(`${POST_CANARY_BATCH_05_DISPUTE_ADJ_ROOT}/preparation-manifest.json`, "utf8")
);
...
assert.equal(preparation.totals.candidateSelections, 527);
...
assert.equal(preparation.acceptedSourceBoundary.audioValidationOverlaysPreserved, 3);
...
for (const context of preparation.contexts) {
  ...
  assert.equal(packet.batchNumber, 5);
  ...
  const output = {
    schemaVersion: POST_CANARY_BATCH_05_DISPUTE_ADJ_OUTPUT_VERSION,
    ...
    isolation: structuredClone(POST_CANARY_BATCH_05_DISPUTE_ADJ_ISOLATION),
  ...
  const validation = validatePostCanaryBatch05DisputeAdjudicationOutput(output, packet);
...
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_05_DISPUTE_ADJ_ROOT}/packets`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_05_DISPUTE_ADJ_ROOT}/provenance`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
