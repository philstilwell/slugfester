...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-01/discovery";
...
assert.equal(manifest.status, "frozen-thirty-one-post-canary-batch-01-discovery-contexts-prepared-not-authorized");
...
assert.equal(manifest.executionPolicy.copiedInputBytesMaximum, 82445);
assert.equal(preparation.totals.maximumCopiedInputBytes, 82445);
...
assert.equal(manifest.copiedInputBoundary.frozenObservedCopiedInputBytesMaximum, 82445);
assert.equal(manifest.copiedInputBoundary.executionCopiedInputBytesMaximum, 82445);
assert.deepEqual(manifest.copiedInputBoundary.maximumContext, { debateNumber: "13", chunkId: "chunk-001" });
...
assert.equal(manifest.costEstimate.contexts, 31);
...
assert.deepEqual(manifest.costEstimate.expectedParallelWallMinutes, [16, 32]);
assert.deepEqual(manifest.costEstimate.expectedAggregateModelMinutes, [54, 95]);
...
assert.equal(preparedByContext.size, 31);
...
assert.equal(manifest.nextAuthorizedAction, "user-approval-required-before-batch-01-discovery-execution-activation-or-any-model-execution");

const validation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-discovery-execution-preparation-validation",
  protocolId: manifest.protocolId,
  status: "batch-01-discovery-execution-manifest-validation-passed-frozen-awaiting-separate-model-execution-approval",
