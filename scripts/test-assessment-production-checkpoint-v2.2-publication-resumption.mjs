...
import { CHECKPOINT_V22_RESUMPTION_DEBATES, CHECKPOINT_V22_RESUMPTION_ROOT } from "./lib/assessment-production-checkpoint-v2.2-publication-resumption.mjs";

const readJson = (file) => readFile(path.resolve(file), "utf8").then(JSON.parse);
const exists = (file) => access(path.resolve(file)).then(() => true, () => false);
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const preparation = await readJson(`${CHECKPOINT_V22_RESUMPTION_ROOT}/preparation-manifest.json`);
assert.equal(preparation.status, "nine-untouched-production-checkpoint-v2.2-publication-contexts-prepared-and-frozen");
assert.deepEqual(preparation.contexts.map(({ debateNumber }) => debateNumber), CHECKPOINT_V22_RESUMPTION_DEBATES);
assert.deepEqual(preparation.contexts.map(({ originalContextIndex }) => originalContextIndex), [1, 2, 3, 4, 5, 6, 7, 8, 9]);
assert.equal(preparation.contexts.length, 9);
assert.equal(preparation.totals.moves, 169);
...
assert.equal(preparation.authorization.publicationModelExecution, false);
assert.equal(preparation.authorization.productionMutation, false);
assert.equal((await readJson(preparation.acceptedDebate50.validation)).status, "passed");

const activationPath = `${CHECKPOINT_V22_RESUMPTION_ROOT}/execution-activation.json`;
if (await exists(activationPath)) {
  const activation = await readJson(activationPath);
  assert.equal(activation.authorization.publicationModelExecution, true);
  assert.equal(activation.authorization.retry, false);
  assert.equal(activation.authorization.correctionModelExecution, false);
  ...
  if (await exists(activation.artifacts.execution)) {
    ...
    assert.equal(new Set(execution.results.map(({ debateNumber }) => debateNumber)).size, execution.results.length);
    if (await exists(activation.artifacts.analysis)) {
      ...
      assert.equal(analysis.gate.correctionContexts, 0);
      assert.equal(analysis.gate.modelAuthoredScores, 0);
      assert.equal(analysis.authorization.productionMutation, false);
      if (analysis.status === "production-checkpoint-v2.2-publication-resumed-model-gate-passed") {
        assert.equal(analysis.gate.cohortSemanticPass, true);
        assert.equal(analysis.gate.timingPass, true);
        assert.equal(analysis.gate.cohortValidDebates, 10);
        assert.equal(analysis.gate.cohortMoves, 188);
        assert.equal(analysis.gate.cohortCritiques, 188);
        assert.equal(analysis.gate.cohortExactSourceQuotes, 20);
        assert.equal(analysis.gate.cohortOverallCommentarySides, 20);
        assert.equal(analysis.gate.cohortAIExtensionSides, 20);
...
console.log(JSON.stringify({
  ...
  untouchedOriginalContextIndexes: [1, 2, 3, 4, 5, 6, 7, 8, 9],
