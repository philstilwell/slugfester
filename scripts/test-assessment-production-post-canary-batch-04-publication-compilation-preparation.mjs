...
assert.equal(manifest.aggregateExpectations.moves, 203);
assert.equal(manifest.aggregateExpectations.critiques, 203);
...
for (const context of manifest.contexts) {
  assert.equal(
    ...
    manifest.inputs.acceptedPublicationOutputs[context.debateNumber]
  );
...
assert.equal(replayedMoves, 203);
