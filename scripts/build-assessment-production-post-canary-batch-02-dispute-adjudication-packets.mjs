...
import {
  ...
  makePostCanaryBatch02DisputeAdjudicationSchema
...
const USER_AUTHORIZATION = Object.freeze({
  instruction:
    "I approve preparation, validation, freezing, committing, and pushing of exactly ten Batch 2 score-blind dispute-only adjudication packets and their execution-preparation manifest only, with a direct incremental cost cap of $0. Use only the accepted independent judgments, frozen disagreements, and accepted audio-verification results. Do not execute adjudication or judgment models, use paid services, assemble final ledgers, derive scores, reconstruct publication, mutate production, or select the next batch.",
...
const inputPaths = {
  ...
  schema: `${POST_CANARY_BATCH_02_DISPUTE_ADJ_ROOT}/adjudication.schema.json`
};
const TOOL_SOURCES = [
  ...
  "scripts/build-assessment-production-post-canary-batch-02-dispute-adjudication-packets.mjs",
  "scripts/test-assessment-production-post-canary-batch-02-dispute-adjudication-packets.mjs",
  "scripts/validate-assessment-production-post-canary-batch-02-dispute-adjudication-output.mjs",
  "scripts/preregister-assessment-production-post-canary-batch-02-dispute-adjudication.mjs",
  "scripts/test-assessment-production-post-canary-batch-02-dispute-adjudication-manifest.mjs"
];
...
assertV4(
  sourceAnalysis.status ===
      "post-canary-batch-02-deterministic-disagreements-extracted-awaiting-separate-audio-work-approval" &&
    JSON.stringify(sourceAnalysis.debates.map((item) => item.debateNumber)) ===
      JSON.stringify(EXPECTED_DEBATES) &&
    sourceAnalysis.adjudicationWorkload.disputedMoves === 182 &&
    sourceAnalysis.adjudicationWorkload.candidateSelections === 535 &&
    sourceAnalysis.adjudicationWorkload.packetsPrepared === false &&
    sourceAnalysis.adjudicationWorkload.modelContextsExecuted === 0,
  "Batch 2 deterministic disagreement workload changed"
);
assertV4(
  judgmentExecution.status ===
      "twenty-post-canary-batch-02-independent-judgment-contexts-passed" &&
    ...
    judgmentAnalysis.status ===
      "twenty-post-canary-batch-02-independent-judgments-passed-awaiting-separate-disagreement-extraction-approval" &&
    judgmentAnalysis.totals.contexts === 20,
  "Batch 2 accepted independent-judgment boundary changed"
);
assertV4(
  ...
  "Batch 2 independent-judgment configuration changed"
);
...
assertV4(
  ...
  "Batch 2 accepted audio-verification boundary changed"
);
assertV4(
  ...
  "Batch 2 packet-preparation authorization changed"
);
...
assertV4(audioByDebateMove.size === 10, "Batch 2 verified audio population changed");

const schema = makePostCanaryBatch02DisputeAdjudicationSchema();
...
if (shouldWrite) {
  const immutableOutputs = [
    inputPaths.schema,
    `${POST_CANARY_BATCH_02_DISPUTE_ADJ_ROOT}/preparation-manifest.json`,
    `${POST_CANARY_BATCH_02_DISPUTE_ADJ_ROOT}/execution-preparation-manifest.json`,
    ...EXPECTED_DEBATES.flatMap((debateNumber) => [
      `${POST_CANARY_BATCH_02_DISPUTE_ADJ_ROOT}/packets/debate-${debateNumber}.json`,
      `${POST_CANARY_BATCH_02_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`
    ])
  ];
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const built = buildPostCanaryBatch02DisputeAdjudicationPacket(
  ...
  const provenancePath =
    `${POST_CANARY_BATCH_02_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`;
  const outputPath =
    `${POST_CANARY_BATCH_02_DISPUTE_ADJ_ROOT}/outputs/debate-${debateNumber}.json`;
  const packetBytes = pretty(built.packet);
  const provenanceBytes = pretty({
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-02-adjudication-candidate-provenance",
...
const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-dispute-only-adjudication-preparation",
  ...
  acceptedSourceBoundary: {
    ...
    allTenAudioMovesVerified: true,
    audioValidationOverlaysPreserved: 2,
    audioUsageDerivedEstimatedCostUsd:
      audioAudit.totals.usageDerivedEstimatedCostUsd,
  ...
  nextAuthorizedAction:
    "freeze-post-canary-batch-02-dispute-only-adjudication-execution-preparation-manifest-model-free-only"
};
assertV4(
  contexts.length === 10 &&
    disputedMoves === 182 &&
  ...
  "Batch 2 adjudication packet workload differs from frozen inputs"
);
assertV4(
  preparation.totals.maximumCopiedInputBytes <= 350000,
  `Batch 2 context ${preparation.totals.maximumCopiedInputBytes} bytes exceeds the 350 KB ceiling`
);
if (shouldWrite) {
  await mkdir(POST_CANARY_BATCH_02_DISPUTE_ADJ_ROOT, { recursive: true });
