...
import {
  ...
  evaluatePostCanaryBatch12ScoreStability
...
const accepted = evaluatePostCanaryBatch12ScoreStability(passing);
...
assert.equal(
  evaluatePostCanaryBatch12ScoreStability(winnerMutation).acceptancePassed,
  false
);
const distanceMutation = structuredClone(passing);
distanceMutation[2].final = scores(95, 80);
assert.equal(
  evaluatePostCanaryBatch12ScoreStability(distanceMutation).acceptancePassed,
  false
);
assert.throws(() =>
  evaluatePostCanaryBatch12ScoreStability(passing, {
    ...POST_CANARY_BATCH_12_SCORE_STABILITY_THRESHOLDS,
