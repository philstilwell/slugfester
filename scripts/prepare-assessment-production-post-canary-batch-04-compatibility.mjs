...
import {
  POST_CANARY_BATCH_04_COMPATIBILITY_ORDER,
  POST_CANARY_BATCH_04_COMPATIBILITY_PROTOCOL_ID,
  POST_CANARY_BATCH_04_COMPATIBILITY_ROOT,
  POST_CANARY_BATCH_04_SITE_LEDGER_ADAPTER_VERSION,
  buildPostCanaryBatch04SiteLedgerAdapter,
  serializedJson,
  sha256,
...
const paths = {
  preparation: `${POST_CANARY_BATCH_04_COMPATIBILITY_ROOT}/preparation-manifest.json`,
  analysis: `${POST_CANARY_BATCH_04_COMPATIBILITY_ROOT}/analysis.json`,
  activation: `${POST_CANARY_BATCH_04_COMPATIBILITY_ROOT}/execution-activation.json`,
  execution: `${POST_CANARY_BATCH_04_COMPATIBILITY_ROOT}/execution.json`,
  stagedLedgerRoot:
    `${POST_CANARY_BATCH_04_COMPATIBILITY_ROOT}/output-bundle/staged-ledgers`,
  compatibility:
    "docs/assessment-production/post-canary-continuation-v1/batch-04/publication-finalization/compatibility-analysis.json",
  finalizationAudit:
    "docs/assessment-production/post-canary-continuation-v1/batch-04/publication-finalization/output-bundle/finalization-audit.json",
  finalLedger:
    "docs/assessment-production/post-canary-continuation-v1/batch-04/final-ledger/final-ledger.json",
  scores:
    "docs/assessment-production/post-canary-continuation-v1/batch-04/score-pass/calculated-scores.json",
  renderingAnalysis:
    "docs/assessment-production/post-canary-continuation-v1/batch-04/rendering-verification/analysis.json",
  renderingAudit:
    "docs/assessment-production/post-canary-continuation-v1/batch-04/rendering-verification/rendering-audit.json",
  ...
  preparationScript:
    "scripts/prepare-assessment-production-post-canary-batch-04-compatibility.mjs",
  preparationTest:
    "scripts/test-assessment-production-post-canary-batch-04-compatibility-preparation.mjs",
...
assertV4(
  ...
  "a stable --frozen-at ISO timestamp is required for the first Batch 4 compatibility preparation write"
);
...
assertV4(
  renderingAnalysis.status ===
      "ten-debate-batch-04-rendering-verification-passed" &&
    ...
    renderingAnalysis.compatibilityBoundary?.productionMutationBlocked ===
      true &&
    renderingAnalysis.authorization?.compatibilityRemedyPlanPreparation === true &&
    ...
    renderingAnalysis.nextAuthorizedAction ===
      "prepare-batch-04-production-compatibility-under-standing-authorization",
  "Batch 4 rendering gate does not authorize compatibility planning only"
);
assertV4(
  renderingAudit.status ===
      "passed-ten-debate-batch-04-rendering-verification" &&
    renderingAudit.totals?.debates === 10 &&
    renderingAudit.totals?.viewportResults === 20 &&
    renderingAudit.totals?.failedRequests === 0 &&
    renderingAudit.productionMutationPerformed === false,
  "passed Batch 4 rendering evidence audit required"
);
assertV4(
  compatibility.status ===
      "batch-04-production-compatibility-boundary-recorded" &&
    canonicalJson(compatibility.findings.map((finding) => finding.id)) ===
      canonicalJson(["batch-04-site-ledger-adapter-and-validator-route"]) &&
  ...
  "frozen Batch 4 publication compatibility blocker required"
);
assertV4(
  finalizationAudit.status ===
      "passed-ten-debate-batch-04-publication-finalization" &&
  ...
  "passed one-pass Batch 4 staging-only finalization audit required"
);
assertV4(
  scores.status ===
      "post-canary-batch-04-single-score-pass-stability-gate-passed" &&
  ...
  "accepted one-pass Batch 4 score ledger required"
);
validatePostCanaryBatch04FinalLedger(
  finalLedger,
...
const packetRecords = [];
for (const debateNumber of POST_CANARY_BATCH_04_COMPATIBILITY_ORDER) {
  ...
  const packetPath =
    `${POST_CANARY_BATCH_04_COMPATIBILITY_ROOT}/packets/debate-${debateNumber}.json`;
  ...
  const adapter = buildPostCanaryBatch04SiteLedgerAdapter({
    finalLedgerDebate,
    scoreDebate,
    candidate,
  ...
  const packet = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-04-compatibility-plan-packet",
    protocolId: POST_CANARY_BATCH_04_COMPATIBILITY_PROTOCOL_ID,
    status: "frozen-post-canary-batch-04-compatibility-plan-packet",
...
assertV4(
  totals.debates === 10 &&
    totals.sections === 53 &&
    totals.moves === 203 &&
    totals.oneSidedDisplayRows === 7 &&
    totals.overallBlunders === 55 &&
    totals.emptyReferenceLinks === 49 &&
    totals.suppliedReferenceLinks === 6,
  "Batch 4 compatibility population changed"
);

const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-compatibility-preparation-manifest",
  protocolId: POST_CANARY_BATCH_04_COMPATIBILITY_PROTOCOL_ID,
  status: "post-canary-batch-04-compatibility-plan-prepared-and-frozen",
  frozenAt,
  productionCanary: false,
  batchNumber: 4,
  planningOnly: true,
  ...
  userAuthorization: {
    instruction:
      "Use the frozen Batch 4 standing authorization for deterministic compatibility staging.",
    scopeInterpretation:
      "Prepare, validate, freeze, commit, and push the Batch 4 production-compatibility plan; later activation and one staging pass are already covered by the standing authorization. Do not publish production ledgers, mutate production, run models, use paid services, or select Batch 5."
  },
  scope: {
    finding: "batch-04-site-ledger-adapter-and-validator-route",
    description:
      "Freeze the exact Batch 4 ledger adapters and schema-specific validator route without applying them to the active validator or production data."
  },
  invariants: {
    ...
    scoreProtocolId:
      "assessment-production-post-canary-batch-04-single-deterministic-score-pass",
  ...
  proposedValidatorRoute: {
    ...
    newSchemaVersion: POST_CANARY_BATCH_04_SITE_LEDGER_ADAPTER_VERSION,
    existingCheckpointSchemaVersion:
      "1.0-production-checkpoint-v2.2-site-ledger-adapter",
    exactRoute:
      "When a production ledger uses the Batch 4 adapter schema, load the Batch 4 packet and activation from the frozen Batch 4 compatibility root, verify their hashes, and replay the adapter with the Batch 4 validator. Keep the checkpoint and Batch 1–3 adapters on their existing roots, credentials, and validators, and keep legacy Rubric v2 and v2.1 ledgers on their existing calculators.",
    oneSidedDisplayRows:
      "Treat all five authenticated adjudicated-consensus adapter schemas as requiring ledgerMoveId and permitting a section exchange with only one side populated.",
    activationRequirements: [
      "activation status is post-canary-batch-04-compatibility-execution-authorized-and-frozen",
  ...
  stagedExecutionPlan: [
    ...
    },
    {
      ...
      allowedWrites: [
        paths.activeValidator,
        `${POST_CANARY_BATCH_04_COMPATIBILITY_ROOT}/output-bundle/**`,
        paths.execution,
        paths.analysis
      ],
      actions: [
        "wire the schema-specific Batch 4 adapter route",
  ...
  mandatoryExecutionTests: {
    positive: [
      "all ten Batch 4 adapters reproduce all 203 locked move scores, 53 section scores per side, and 20 overall scores",
      "all ten final candidates match ledgerMoveId, section order, model, rubric, and score protocol",
      "all 7 one-sided display rows pass through the authenticated Batch 4 adapter route",
    ...
    negative: [
      ...
      "a Batch 4 adapter routed through the checkpoint or Batch 1–3 roots fails",
  ...
  nextAuthorizedAction:
    "activate-and-execute-one-batch-04-production-compatibility-pass-under-standing-authorization"
};
const preparationBytes = serializedJson(preparation);
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-compatibility-preparation-analysis",
  protocolId: POST_CANARY_BATCH_04_COMPATIBILITY_PROTOCOL_ID,
  status: "post-canary-batch-04-compatibility-plan-freeze-passed",
  productionCanary: false,
  batchNumber: 4,
  planningOnly: true,
  preparation: {
...
if (write) {
  assertV4(
    ...
    "Batch 4 compatibility execution artifact already exists; planning write blocked"
  );
  await mkdir(
    resolve(`${POST_CANARY_BATCH_04_COMPATIBILITY_ROOT}/packets`),
    { recursive: true }
  );
  ...
  assertV4(
    ...
    "stored Batch 4 compatibility preparation differs from deterministic replay"
  );
  const storedAnalysis = await readJson(paths.analysis);
  assertV4(
    canonicalJson(storedAnalysis) === canonicalJson(analysis),
    "stored Batch 4 compatibility analysis differs from deterministic replay"
  );
  for (const record of packetRecords) {
    const storedPacket = await readFile(resolve(record.packetPath), "utf8");
    assertV4(
      storedPacket === record.packetBytes,
      `${record.debateNumber}: stored Batch 4 compatibility packet differs from deterministic replay`
    );
  }
}
