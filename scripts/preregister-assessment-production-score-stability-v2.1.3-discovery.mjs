...
const PROTOCOL_ID =
  "assessment-production-score-stability-v2.1.3-fresh-validation-discovery";
const WORKFLOW = "docs/assessment-production-canary-discovery-workflow.md";
const SCRIPT =
  "scripts/preregister-assessment-production-score-stability-v2.1.3-discovery.mjs";
const TEST =
  "scripts/test-assessment-production-score-stability-v2.1.3-discovery-manifest.mjs";
...
assertV4(
  preparation.status ===
    ...
    preparation.totals.boundedEndSchemas === 36 &&
    preparation.totals.modelEndEventRequiringSchemas === 36 &&
    preparation.totals.requestedTokenProhibitingSchemas === 36 &&
    preparation.totals.deterministicTokenMinimumContexts === 36 &&
    ...
    preparation.discoverySuccessorContract.minimumLexicalTokens ===
      V212_DISCOVERY_MINIMUM_LEXICAL_TOKENS &&
    preparation.discoverySuccessorContract.requestedLexicalTokensRemoved ===
      true &&
  ...
  "v2.1.3 source preparation does not authorize discovery-manifest preparation"
);
assertV4(
...
const manifest = {
  schemaVersion:
    "1.0-score-stability-v2.1.3-discovery-execution-preparation-manifest",
  ...
  costEstimate: {
    ...
    expectedAggregateModelMinutes: [44, 85],
    expectedAggregateComputeHours: [0.73, 1.42],
    absoluteGateTimeoutMinutes: 120,
    estimateBasis:
      "Scaled from the realized v2.1.2 33-context run (14.02 wall minutes, 45.76 aggregate model minutes) and this cohort's 36-context copied-input distribution; subscription use has no direct incremental API charge.",
  },
  ...
  nextAuthorizedAction:
    "prepare-separate-v2.1.3-discovery-execution-activation-only",
};
