...
const PREPARATION = `${ROOT}/audio-source-partial-execution-1.json`;
const EXECUTION = `${ROOT}/audio-source-recovery-1-debate-15.json`;
const LOCAL_ROOT =
...
const FAILED_SOURCE = `${LOCAL_ROOT}/source.failed-attempt-1.mp3`;
const PARTIAL_DOWNLOAD = `${LOCAL_ROOT}/source.recovery-attempt-1.partial.mp4`;
const VIDEO_ID = "5OXPlUCGScY";
const FORMAT_SELECTOR = "18";
...
async function validatePreparation() {
  ...
  assertV4(
    preparation.status ===
      ...
      preparation.recoveryPlan.level === 1 &&
      ...
      preparation.failure.failedArtifactSha256 ===
        "87965bd54bc175f5645aa773bd5a0a98c396524a0dd0c4e94463828c11ed2754",
    "Batch 12 Debate 15 recovery preparation changed"
  );
  for (const item of [...preparation.successfulSources, ...preparation.successfulClips]) {
    const file = item.sourceAudio ?? item.clipPath;
    const digest = item.sourceAudioSha256 ?? item.clipSha256;
    assertV4(await hashFile(file) === digest, `${file}: preserved success hash changed`);
  }
  return preparation;
}
...
async function applyRecovery() {
  ...
  assertV4(
    await hashFile(SOURCE) === preparation.failure.failedArtifactSha256,
  ...
  const execution = {
    ...
    status: "debate-15-audio-source-recovery-level-1-passed",
...
async function validateRecovery() {
  ...
  assertV4(
    execution.status === "debate-15-audio-source-recovery-level-1-passed" &&
      ...
      execution.failedArtifactSha256 === preparation.failure.failedArtifactSha256 &&
...
if (command === "preview") {
  ...
  console.log(JSON.stringify({
    status: "preview-one-debate-15-audio-source-recovery-shard",
  ...
  console.log(JSON.stringify({
    status: "passed-batch-12-debate-15-audio-source-recovery",
