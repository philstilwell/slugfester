...
const canary = JSON.parse(await readFile(preparation.inputs.canary, "utf8"));

assert.equal(preparation.status, "ten-debate-production-canary-source-and-discovery-packets-prepared");
...
assert.equal(preparation.totals.directSizedSources, 3);
assert.equal(preparation.totals.partitionMediumSources, 5);
assert.equal(preparation.totals.partitionHeavySources, 2);
...
for (const [file, expectedHash] of Object.entries(preparation.sourceHashes)) {
  assert.equal(sha256(await readFile(file)), expectedHash, `${file}: preparation source hash changed`);
}

const canaryByNumber = new Map(canary.debates.map((debate) => [debate.debateNumber, debate]));
...
function assertNoProhibitedKeys(value, location = "packet") {
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) => assertNoProhibitedKeys(item, `${location}[${index}]`));
    return;
  }
  for (const [key, child] of Object.entries(value)) {
    assert(!prohibitedPacketKeys.has(key), `${location}.${key}: prohibited assessment field`);
    assertNoProhibitedKeys(child, `${location}.${key}`);
  }
}

for (const context of preparation.contexts) {
  const frozen = canaryByNumber.get(context.debateNumber);
  assert(frozen, `${context.debateNumber}: missing from frozen canary`);
  const [packetBytes, planBytes, fullLedgerBytes, transcriptBytes, eventsBytes, manifestBytes] =
    await Promise.all([
  ...
  assert(rebuilt.sourceLedgerBytes.equals(fullLedgerBytes), `${context.debateNumber}: compact ledger replay failed`);
  ...
  for (const chunk of context.chunks) {
    ...
    assert.deepEqual(schema.properties.candidates.items.properties.speaker.enum, expectedSpeakers);
    assert.equal(schema.properties.calibrationOnly.const, true);
    assert.equal(schema.properties.completeCoreReviewed.const, true);
  }
}

assert.equal(preparation.futureOutputPathsExcludedFromSourceHashes.length, preparation.totals.discoveryContexts);
