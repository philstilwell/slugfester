...
import { V421_PROTOCOL_ID, V421_ROOT } from "./lib/v421-compact-fresh.mjs";
...
const manifestPath = `${V421_ROOT}/primary-execution-manifest.json`;
const executionPath = `${V421_ROOT}/primary-model-execution.json`;
const analysisPath = `${V421_ROOT}/primary-analysis.json`;
...
const [preparation, sample] = await Promise.all([
  readFile(path.resolve(root, `${V421_ROOT}/preparation-manifest.json`), "utf8").then(JSON.parse),
  readFile(path.resolve(root, `${V421_ROOT}/source-only-sample.json`), "utf8").then(JSON.parse)
]);
assertV4(preparation.status === "prepared-six-compact-source-only-contexts" && preparation.totals.debates === 6 && preparation.totals.compactSourceLedgers === 6 && preparation.authorization.deterministicFixtures && !preparation.authorization.primaryModelExecution, "v4.2.1 preparation invalid");
assertV4(sample.status === "frozen-before-legacy-score-access" && sample.audit.priorFreshGateOverlap === 0 && !sample.selectionBoundary.legacyAssessmentContentAccessed, "v4.2.1 sample blindness invalid");
...
for (const item of preparation.debates) {
  ...
  assertV4(packet.modelInputBoundary.plainTranscriptDeliveredToModel === false && packet.modelInputBoundary.originalEventsFileDeliveredToModel === false && packet.transportChain.replayExactToOriginalEvents, `${item.debateNumber}: compact transport boundary invalid`);
  contexts.push({
    ...
    compactCopiedInputBytes: item.compactCopiedInputBytes,
    packet: item.packet,
...
const sourceFiles = [
  "docs/assessment-workflow-v4.2.1.md",
  ...Object.values(preparation.inputs),
  `${V421_ROOT}/source-only-sample.json`,
  `${V421_ROOT}/preparation-manifest.json`,
  ...
  "scripts/build-v421-compact-fresh-packets.mjs",
  "scripts/test-v421-compact-fresh.mjs",
  "scripts/test-v421-primary-tooling.mjs",
  "scripts/validate-v421-primary-output.mjs",
  "scripts/preregister-v421-primary-execution.mjs",
  "scripts/run-v421-primary-execution.mjs",
  "scripts/analyze-v421-primary.mjs",
...
const manifest = {
  schemaVersion: "4.2.1-compact-fresh-primary-execution-manifest",
  protocolId: V421_PROTOCOL_ID,
  stage: "six-source-blind-compact-transport-primary-judgments",
  status: "frozen-six-compact-context-primary-authorized",
  frozenAt,
...
console.log(JSON.stringify({ status: shouldWrite ? "frozen" : "preview", contexts: 6, attemptsPerContext: 1, retriesMaximum: 0, timeoutMinutesPerContext: 30, primaryReasoningEffort: manifest.model.reasoningEffort, comparatorHidden: true, compactLedgerOnly: true, originalTranscriptCopiedToModel: false, originalEventsCopiedToModel: false, meanCompactCopiedInputBytes: preparation.totals.meanCompactCopiedInputBytes, excerptMaximumCharacters: 600, repositoryOwnedSourceTimes: true, meteredApiCostUsdMaximum: 0, transcriptionCostUsdMaximum: 0 }, null, 2));
