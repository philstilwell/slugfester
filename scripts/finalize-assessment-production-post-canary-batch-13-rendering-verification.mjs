...
import {
  ...
  hashFile,
  sha256,
  validatePostCanaryBatch13RenderingEvidence,
...
const ROOT = POST_CANARY_BATCH_13_RENDERING_ROOT;
...
assertV4(activation.status ===
  "frozen-post-canary-batch-13-rendering-verification-authorized" &&
...
"Batch 13 rendering activation changed");
...
for (const debateNumber of POST_CANARY_BATCH_13_RENDERING_ORDER) {
  ...
  const packet = validatePostCanaryBatch13RenderingPacket(JSON.parse(packetBytes));
  for (const viewportName of ["desktop", "mobile"]) {
    ...
    validatePostCanaryBatch13RenderingEvidence({ packet, viewportName,
...
"complete passing Batch 13 rendering evidence required");
...
const audit = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-13-rendering-audit",
  protocolId: activation.protocolId,
  status: "passed-ten-debate-batch-13-rendering-verification",
...
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-13-rendering-execution",
  protocolId: activation.protocolId,
  status: "ten-debate-batch-13-rendering-verification-passed",
  completedAt,
  attempts: 20,
  retries: 0,
  timeoutExtensions: 0,
...
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-13-rendering-analysis",
  protocolId: activation.protocolId,
  ...
  nextAuthorizedAction: "prepare-batch-13-production-compatibility-staging"
};
...
console.log(JSON.stringify({ status: analysis.status, debates: 10,
  sections: 53, moves: 199, viewportResults: 20,
