...
assertV4(
  ...
  "the Batch 1 publication finalization is not prepared"
);
for (const [file, digest] of Object.entries(preparation.sourceHashes)) {
  assertV4(
    sha256(await readFile(path.resolve(file))) === digest,
    `Batch 1 finalization preparation source hash mismatch: ${file}`
  );
}
...
const activation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-01-publication-finalization-execution-activation",
  protocolId: POST_CANARY_BATCH_01_PUBLICATION_FINALIZATION_PROTOCOL_ID,
  status: "frozen-post-canary-batch-01-publication-finalization-authorized",
  activatedAt,
  ...
  userAuthorization: {
    instruction:
      "I approve activation and execution of the one frozen Batch 1 deterministic publication-finalization pass, with a direct incremental cost cap of $0. Use exactly one repository finalization pass and no reruns. Stop after deterministic final-candidate validation, analysis, committing, and pushing. Do not execute models, use paid services, render or verify rendering, publish production ledgers, mutate production, or select the next batch.",
  ...
  nextRequiredAction:
    "execute-one-frozen-batch-01-deterministic-publication-finalization-pass"
};
