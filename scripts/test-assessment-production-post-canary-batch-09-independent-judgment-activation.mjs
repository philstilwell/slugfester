...
if (analysis.acceptance.passed) {
  ...
  assert.equal(analysis.totals.uniqueMoves, 180);
  assert.equal(analysis.totals.movesJudgedAcrossPasses, 360);
  assert.equal(
  ...
  assert.equal(
    analysis.status,
    "post-canary-batch-09-independent-judgment-gate-failed-analysis-only"
  );
}
console.log(
  JSON.stringify(
