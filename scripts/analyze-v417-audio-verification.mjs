...
import { V417_AUDIO_PROTOCOL_ID, V417_AUDIO_SCHEMA_VERSION, V417_AUDIO_THRESHOLDS, evaluateAttributionTranscript } from "./lib/v417-audio-verification.mjs";
...
const [plan, execution, passBAnalysis] = await Promise.all([readJson(planPath), readJson(`${V417_PASS_B_ROOT}/audio-model-execution.json`), readJson(`${V417_PASS_B_ROOT}/analysis.json`)]);
...
for (const debate of plan.debates) {
  ...
  for (const move of debate.moves) {
    ...
    const evaluation = evaluateAttributionTranscript(transcript, move, V417_AUDIO_THRESHOLDS);
