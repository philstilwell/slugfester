...
const protocolId = "assessment-production-post-canary-batch-16-publication-level-1-one-field-recovery";
...
const files = {
  ...
  packet: `${publicationRoot}/packets/debate-16.json`,
  failedOutput: `${publicationRoot}/outputs/debate-16.json`,
  failedValidation: `${publicationRoot}/validations/debate-16.json`,
  failedProvenance: `${publicationRoot}/provenance/debate-16.json`,
...
if (mode === "--prepare") {
  const [diagnosis, originalExecution, packet, failedOutput] = await Promise.all([
    files.diagnosis, files.originalExecution, files.packet, files.failedOutput
  ].map(readJson));
  assert.equal(diagnosis.status, "batch-16-publication-canary-nine-critique-fields-failed");
  assert.equal(diagnosis.totals.invalidFields, 9);
  assert.equal(originalExecution.contextsAttempted, 1);
  assert.deepEqual(originalExecution.unattemptedContextIndexes, [1, 2, 3, 4, 5, 6, 7, 8, 9]);
  const contexts = diagnosis.failedCritiques.map((failure, contextIndex) => {
    const move = packet.moves.find((item) => item.moveId === failure.moveId);
    assert(move, `${failure.moveId}: locked move missing`);
    const immutable = failedOutput.moveProse[failure.moveId];
    const shardId = `debate-16-${failure.moveId}-critique-recovery-1`;
    const repairPacket = {
      ...
      recoveryLevel: 1,
      writableField: `moveProse.${failure.moveId}.critique`,
      target: {
        moveId: failure.moveId,
        lockedMove: move,
        immutableCompanionFields: { role: immutable.role, words: immutable.words, tags: immutable.tags },
        rejectedPriorStringAvailable: false
      },
    ...
    const properties = {
      ...
      debateNumber: { type: "string", const: "16" },
      ...
      correction: {
        ...
        properties: {
          moveId: { type: "string", const: failure.moveId },
          critique: { type: "string", minLength: 880, maxLength: 1800 }
        }
      }
    };
    const schema = {
      $schema: "https://json-schema.org/draft/2020-12/schema",
      $id: `slugfester-batch-16-${shardId}`,
      type: "object",
      additionalProperties: false,
    ...
    return {
      ...
      writableField: `moveProse.${failure.moveId}.critique`,
      packet: packetPath,
      packetSha256: sha256(pretty(repairPacket)),
      schema: schemaPath,
      schemaSha256: sha256(pretty(schema)),
      output: outputPath,
      repairPacket,
      schemaDocument: schema
    };
  });
  assert.equal(new Set(contexts.map((item) => item.writableField)).size, 9);
  const sourceFiles = [
    files.diagnosis, files.originalExecution, files.originalAnalysis, files.packet,
    files.failedOutput, files.failedValidation, files.failedProvenance,
    files.workflow, files.outputContract, files.manual,
    "scripts/assessment-production-post-canary-batch-16-publication-level-1-recovery.mjs",
  ...
  const manifest = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-level-1-recovery-preparation",
    protocolId,
    status: "frozen-nine-one-field-batch-16-publication-recovery-level-1-prepared-not-activated",
    frozenAt: at,
    ...
    authorization: {
      ...
      deterministicMergeAndValidation: false,
      retries: false,
    ...
    nextAuthorizedAction: "activate-nine-fresh-one-field-batch-16-publication-recovery-contexts"
  };
  if (shouldWrite) await writeFile(path.resolve(files.preparation), pretty(manifest));
  console.log(pretty({ status: shouldWrite ? manifest.status : "preview", contexts: 9, writableFields: 9, attemptsMaximum: 9, retriesMaximum: 0, recoveryLevel: 1, directIncrementalCostUsdMaximum: 0, modelExecutionAuthorized: false }));
  process.exit(0);
}
...
if (mode === "--activate") {
  ...
  assert.equal(preparation.status, "frozen-nine-one-field-batch-16-publication-recovery-level-1-prepared-not-activated");
  assert(!(await exists(files.activation)), "activation exists");
  for (const [file, digest] of Object.entries(preparation.sourceHashes)) {
    assert.equal(sha256(await readFile(path.resolve(file))), digest, `source changed: ${file}`);
  }
  const activation = {
    ...preparation,
    schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-level-1-recovery-activation",
    status: "frozen-nine-one-field-batch-16-publication-recovery-level-1-authorized",
    activatedAt: at,
    checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(),
    preparation: { path: files.preparation, sha256: sha256(preparationBytes) },
    authorization: { ...preparation.authorization, modelExecution: true, deterministicMergeAndValidation: true },
    nextAuthorizedAction: "execute-nine-fresh-one-field-batch-16-publication-recovery-contexts-once"
  };
  if (shouldWrite) await writeFile(path.resolve(files.activation), pretty(activation));
  console.log(pretty({ status: shouldWrite ? activation.status : "preview", contexts: 9, attemptsMaximum: 9, retriesMaximum: 0, recoveryLevel: 1, directIncrementalCostUsdMaximum: 0, modelExecutionAuthorized: shouldWrite }));
  process.exit(0);
}

if (mode === "--run") {
  const activation = await readJson(files.activation);
  assert.equal(activation.status, "frozen-nine-one-field-batch-16-publication-recovery-level-1-authorized");
  ...
  const execute = async (context) => {
    const temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-b16-pub-r1-${context.contextIndex}-`));
    const temporaryHome = await mkdtemp(path.join(os.tmpdir(), `slugfester-b16-pub-r1-home-${context.contextIndex}-`));
    ...
    try {
      ...
      const prompt = "Read workflow.md, output-contract.md, manual.md, packet.json, and schema.json completely and no other files. Act only as the fresh isolated 5.6 Sol/low Batch 16 one-field publication recovery editor. Author exactly the single writable critique identified by the packet. The rejected prior string is unavailable. Return exactly one schema-conforming JSON object and no commentary.";
      const invocation = await spawnRun(codex, [
        "exec", "--ephemeral", "--skip-git-repo-check", "--ignore-user-config", "--ignore-rules",
        "--model", model.slug, "-c", `model_reasoning_effort=\"${model.reasoningEffort}\"`,
      ...
      ], { cwd: temporary, env: environment }, activation.executionPolicy.timeoutMsPerContext);
      ...
      const base = {
        ...
        writableField: context.writableField,
        attemptCount: 1,
        retryCount: 0,
      ...
      if (invocation.error || invocation.timedOut || invocation.code !== 0 || invocation.signal !== null) {
        return { ...base, status: invocation.timedOut ? "timed-out" : "transport-failed", outputWritten: false, gateAcceptancePassed: false, failureMessage: String(invocation.error ?? invocation.stderr).slice(-6000) };
      }
  ...
  for (let offset = 0; offset < activation.contexts.length; offset += 2) {
    ...
    console.error(`[batch-16-publication-recovery-1] starting ${pair.map((item) => item.moveId).join(", ")}`);
    const pairResults = await Promise.all(pair.map(execute));
    results.push(...pairResults);
    for (const result of pairResults) console.error(`[batch-16-publication-recovery-1] ${result.moveId} ${result.status} ${(result.elapsedMs / 60000).toFixed(2)}m`);
  }
  const passed = results.every((item) => item.gateAcceptancePassed);
  const execution = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-level-1-recovery-execution",
    protocolId,
    status: passed ? "nine-one-field-batch-16-publication-recovery-level-1-execution-passed" : "batch-16-publication-recovery-level-1-execution-failed",
    startedAt,
    completedAt: new Date().toISOString(),
    elapsedMs: Date.now() - started,
    ...
    results,
    authorization: { analysis: true, retries: false, furtherRecoveryLevel: !passed, paidServices: false, scorePass: false, productionMutation: false, nextBatchSelection: false }
  };
...
if (mode === "--analyze") {
  const [activation, execution, diagnosis, packet, failedOutput, failedValidationBytes, failedProvenanceBytes, failedOutputBytes] = await Promise.all([
    readJson(files.activation), readJson(files.execution), readJson(files.diagnosis), readJson(files.packet), readJson(files.failedOutput),
    readFile(path.resolve(files.failedValidation)), readFile(path.resolve(files.failedProvenance)), readFile(path.resolve(files.failedOutput))
  ]);
  assert.equal(execution.contextsAttempted, 9);
  const failed = execution.results.filter((item) => !item.gateAcceptancePassed);
  if (failed.length) {
    const analysis = {
      schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-level-1-recovery-analysis",
      protocolId,
      status: "batch-16-publication-level-1-recovery-gate-failed",
      recoveryLevel: 1,
      recoveryLevelsMaximum: 2,
      ...
      invalidContexts: failed.length,
      failedFields: failed.map((item) => item.writableField),
      directIncrementalCostUsd: 0,
      authorization: { recoveryLevel2Preparation: true, scorePass: false, productionMutation: false, nextBatchSelection: false },
      nextAuthorizedAction: "prepare-minimum-fresh-field-disjoint-batch-16-publication-recovery-level-2-shards"
    };
    if (shouldWrite) await writeFile(path.resolve(files.analysis), pretty(analysis));
    console.log(pretty(analysis));
    process.exit(0);
  }
  assert.equal(execution.status, "nine-one-field-batch-16-publication-recovery-level-1-execution-passed");
  const outputByContext = new Map(await Promise.all(activation.contexts.map(async (context) => [context.contextIndex, await readJson(context.output)])));
  const merged = structuredClone(failedOutput);
  for (const context of activation.contexts) {
    const correction = outputByContext.get(context.contextIndex).correction;
    assert.equal(correction.moveId, context.moveId);
  ...
  for (const failure of diagnosis.failedCritiques) {
    replay.moveProse[failure.moveId].critique = failedOutput.moveProse[failure.moveId].critique;
  }
  assert.deepEqual(replay, failedOutput, "a non-target companion field changed during recovery merge");
  ...
  const validationRecord = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-validation",
    protocolId: merged.protocolId,
    status: "passed",
    debateNumber: "16",
    debateId: packet.debateId,
    outputSha256: sha256(mergedBytes),
  ...
  const provenance = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-level-1-recovery-provenance",
    protocolId,
    debateNumber: "16",
    originalFailure: { status: "output-validation-failed", outputSha256: sha256(failedOutputBytes), preserved: true },
    recoveryLevel1: {
      contexts: 9,
      acceptedFields: activation.contexts.map((item) => item.writableField),
      oneWritableFieldPerFreshContext: true,
      oneAttemptPerContext: true,
      retries: 0,
      rejectedPriorStringsUnavailable: true
    },
    merge: { acceptedCompanionFieldsChanged: false, scoresChanged: false, scorePassRerun: false, modelAuthoredScores: 0, completeValidationPassed: true },
    outputSha256: sha256(mergedBytes)
  };
  const analysis = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-level-1-recovery-analysis",
    protocolId,
    status: "batch-16-publication-level-1-one-field-recovery-passed",
    recoveryLevel: 1,
    recoveryLevelsMaximum: 2,
    originalFailurePreserved: true,
    contexts: 9,
    attempts: 9,
    retries: 0,
    repairedValidationFields: 9,
    recoveredDebate: "16",
    completeValidation: validation,
    directIncrementalCostUsd: 0,
    paidServiceCalls: 0,
    modelAuthoredScores: 0,
    authorization: { continueUnattemptedInitialPublicationContexts: true, furtherRecoveryLevel: false, scorePass: false, productionMutation: false, nextBatchSelection: false },
    nextAuthorizedAction: "freeze-and-execute-the-nine-previously-unattempted-batch-16-publication-contexts"
  };
  if (shouldWrite) {
    const preservedRoot = `${root}/preserved`;
    await mkdir(path.resolve(preservedRoot), { recursive: true });
    await writeFile(path.resolve(`${preservedRoot}/debate-16-original-output.json`), failedOutputBytes);
    await writeFile(path.resolve(`${preservedRoot}/debate-16-original-validation.json`), failedValidationBytes);
    await writeFile(path.resolve(`${preservedRoot}/debate-16-original-provenance.json`), failedProvenanceBytes);
  ...
  console.log(pretty({ status: analysis.status, recoveredDebate: "16", contexts: 9, attempts: 9, retries: 0, repairedValidationFields: 9, acceptedCompanionFieldsChanged: false, directIncrementalCostUsd: 0, nextAuthorizedAction: analysis.nextAuthorizedAction }));
}
