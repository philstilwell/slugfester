...
const preparationPath =
  `${POST_CANARY_BATCH_01_COMPATIBILITY_ROOT}/preparation-manifest.json`;
const analysisPath =
  `${POST_CANARY_BATCH_01_COMPATIBILITY_ROOT}/analysis.json`;
...
assert.equal(
  analysis.status,
  "post-canary-batch-01-compatibility-plan-freeze-passed"
);
assert.equal(
...
assert.equal(preparation.totals.moves, 177);
assert.equal(preparation.totals.oneSidedDisplayRows, 19);
assert.equal(preparation.totals.overallBlunders, 53);
assert.equal(preparation.totals.emptyReferenceLinks, 52);
assert.equal(preparation.totals.suppliedReferenceLinks, 1);
...
assert.equal(
  await exists(`${POST_CANARY_BATCH_01_COMPATIBILITY_ROOT}/execution.json`),
  false
);
assert.equal(
  await exists(
...
assert.deepEqual(packetNumbers, POST_CANARY_BATCH_01_COMPATIBILITY_ORDER);
for (const packetRecord of preparation.artifacts.packets) {
  ...
  const positive = validatePostCanaryBatch01SiteLedgerAdapter({
