...
import { V416_AUDIO_PROTOCOL_ID, V416_AUDIO_SCHEMA_VERSION } from "./lib/v416-audio-verification.mjs";
...
assertV4(plan.status === "frozen-eight-move-known-speaker-diarization-plan" && plan.authorization.paidDiarization, "paid diarization is not authorized");
assertV4(plan.cost.estimatedCostUsd <= plan.cost.maximumAuthorizedCostUsd && plan.cost.maximumAuthorizedCostUsd === 0.15, "audio cost cap invalid");
...
function runOne(debate, move) {
  const args = [
    plan.executionPolicy.helperPath,
    path.resolve(root, move.clipPath),
    "--model", plan.model.id,
    "--response-format", plan.model.responseFormat,
    "--chunking-strategy", plan.model.chunkingStrategy,
    "--language", plan.model.language,
    "--out", path.resolve(root, move.transcriptPath)
  ];
...
const execution = {
  schemaVersion: "4.1.6-pass-b-audio-model-execution",
  protocolId: V416_AUDIO_PROTOCOL_ID,
  status: passed ? "passed-eight-one-attempt-diarization-calls" : "failed-fast-no-retry",
  plan: planPath,
  model: plan.model,
  ...
  results,
  failure,
  authorization: { deterministicAudioAnalysis: passed, furtherPaidRetry: false, disagreementExtraction: false, adjudicationModelExecution: false, scoreDerivation: false, productionMutation: false }
};
