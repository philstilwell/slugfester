...
assert(activation.transcriptLocks.length === 2 && activation.executionPolicy.completeTwoTranscriptReplayRequired, 'two-transcript replay boundary changed');
...
if (checkOnly) {
  assert(['frozen-batch-06-audio-validation-correction-1-prepared-not-active','frozen-batch-06-audio-validation-correction-1-authorized-under-standing-authorization'].includes(activation.status), 'activation status invalid');
  console.log('Batch 6 audio validation correction-1 runner preflight passed.');
  process.exit(0);
}
assert(activation.status === 'frozen-batch-06-audio-validation-correction-1-authorized-under-standing-authorization', 'correction is not activated');
...
assert(request.calls.length === 2 && priorExecution.results.length === 2 && priorExecution.retries === 0 && !priorExecution.requestFailure, 'preserved execution changed');
...
try {
  for (const [callIndex, call] of request.calls.entries()) {
    ...
    if (call.moveId === activation.target.moveId) {
      ...
      assert(JSON.stringify(frozenIndexes) === JSON.stringify([52, 110]), 'omission indexes changed');
...
const replayComplete = validationFailure === null && moves.length === 2;
const passed = replayComplete && verified === 2;
const status = passed ? 'passed-all-two-batch-06-audio-attributions-after-empty-segment-overlay' : replayComplete ? 'batch-06-audio-verification-unresolved-after-empty-segment-overlay' : 'batch-06-audio-validation-correction-1-failed';
const debates = replayComplete ? [{ debateNumber: '97', debateId: request.calls[0].debateId, moves }] : [];
const totals = {
  requiredMoves: 2, verified, unresolved: replayComplete ? unresolved : null,
...
const executionRecord = {
  schemaVersion: '1.0-assessment-production-post-canary-batch-06-audio-validation-correction-1-execution', protocolId: request.protocolId, status,
  startedAt, completedAt: new Date().toISOString(), elapsedMs: Date.now() - start, productionCanary: false, batchNumber: 6, correctionNumber: 1,
  deterministicPassesAttempted: 1, deterministicPassesCompleted: replayComplete ? 1 : 0, retries: 0, reruns: 0, automaticRepairs: 0,
  completedValidations: moves.length, requiredValidations: 2, verified, unresolved: replayComplete ? unresolved : null, overlayApplications,
  ...
  nextAuthorizedAction: passed ? 'resume-batch-06-standing-authorization-with-dispute-only-adjudication-preparation' : 'preserve-and-diagnose-batch-06-audio-correction-result',
};
await writeFile(activation.outputs.execution, `${JSON.stringify(executionRecord, null, 2)}\n`);
if (replayComplete) {
  const audit = {
    schemaVersion: '1.0-assessment-production-post-canary-batch-06-audio-verification-audit', protocolId: request.protocolId, status, productionCanary: false, batchNumber: 6, stagingOnly: true,
    correction: { number: 1, operation: activation.correction.name, targetMoveId: activation.target.moveId, omittedSegmentIndexes: [52, 110], transientOverlayApplications: 1, persistentTranscriptWrites: 0, allOriginalTranscriptsUnchanged: true },
  ...
  const analysis = {
    schemaVersion: '1.0-assessment-production-post-canary-batch-06-audio-verification-analysis', protocolId: request.protocolId, status, productionCanary: false, batchNumber: 6, stagingOnly: true,
    gate: { passed, executionComplete: true, deterministicReplayComplete: true, requiredMoves: 2, verified, unresolved, deterministicThresholdsApplied: true, exactValidatorPreserved: true, originalTranscriptsPreserved: true, transientCorrectionOverlayApplications: 1, persistentTranscriptWrites: 0 },
    ...
    standingAuthorization: { path: 'docs/assessment-production/post-canary-continuation-v1/batch-06/standing-authorization.json', sha256: activation.sourceHashes['docs/assessment-production/post-canary-continuation-v1/batch-06/standing-authorization.json'], automaticContinuationWhileGatesPass: true },
  ...
  const cost = {
    schemaVersion: '1.0-assessment-production-post-canary-batch-06-audio-cost-control-analysis', protocolId: request.protocolId,
    ...
    analyzedAt: new Date().toISOString(), productionCanary: false, batchNumber: 6, stagingOnly: true,
