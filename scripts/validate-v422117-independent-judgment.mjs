...
const mediumConfidenceMoves = compiled.rawOutput.moves.filter((move) => move.assessmentConfidence === "medium").map((move) => move.moveId);
const summary = { schemaVersion: "4.2.21.17-independent-judgment-validation", protocolId: preparation.protocolId, status: "passed", debateNumber, reviewerPass, moves: compiled.rawOutput.moves.length, lockedInventorySha256: packet.lockedInventorySha256, unchangedV4220ValidatorPassed: true, semanticRepairPerformed: compiled.provenance.semanticRepairPerformed, mediumConfidenceMoves, audioVerificationRequiredBeforeAdjudication: mediumConfidenceMoves.length > 0, scoresDerived: 0 };
if (writeFlag === "--write") {
