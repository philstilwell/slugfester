...
import { MODEL, PROTOCOL_ID, ROOT, sha256 } from "./lib/assessment-production-post-canary-batch-14-audio-attribution.mjs";
...
const authorizationPath = "docs/assessment-production/post-canary-continuation-v1/batch-14/audio-verification/audio-attribution-successor-authorization.json";
...
assert.equal(preparation.status, "prepared-one-batch-14-audio-attribution-recovery-context-not-active");
assert.equal(authorization.status, "frozen-active-batch-14-audio-attribution-recovery-level-2-successor-authorization");
assert.equal(preparation.authorization.sha256, sha256(authorizationBytes));
assert.deepEqual(preparation.contexts.map((item) => [item.debateNumber, item.moveIds.length]), [["12", 1]]);
...
const sourceFiles = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-14-audio-attribution.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-14-audio-attribution.mjs",
  "scripts/test-assessment-production-post-canary-batch-14-audio-attribution.mjs",
  "scripts/activate-assessment-production-post-canary-batch-14-audio-attribution.mjs",
  "scripts/run-assessment-production-post-canary-batch-14-audio-attribution.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-14-audio-attribution.mjs",
];
const sourceHashes = {};
...
const manifest = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-14-audio-attribution-execution-manifest",
  protocolId: PROTOCOL_ID,
  status: "frozen-one-batch-14-audio-attribution-recovery-context-active",
  activatedAt,
