...
assert.equal(preparation.calls.length, 12);
assert.deepEqual(preparation.calls.map((call) => `${call.debateNumber}:${call.moveId}`), expectedMoves);
assert.deepEqual(preparation.scope.debates, ["53", "110", "133", "12", "187", "106", "57"]);
assert.equal(preparation.scope.frozenTargetClips, 12);
assert.equal(preparation.scope.verificationCalls, 12);
assert.equal(preparation.scope.sameDebateSpeakerSupportReferences, 14);
assert.deepEqual(preparation.scope.sourceCompatibility.occurrences, []);
assert.equal(preparation.referenceContract.references.length, 14);
...
assert.equal(preparation.costEstimate.clipSeconds, 1179.75);
assert.equal(preparation.costEstimate.clipMinutes, 19.6625);
assert.equal(preparation.costEstimate.durationOnlyPlanningEstimateUsd, 0.117975);
assert.equal(preparation.costEstimate.durationOnlyEstimateAuthorizedByUser, true);
assert.equal(preparation.costEstimate.usageDerivedPlanningEstimateUsd, 0.430595);
assert.equal(preparation.costEstimate.primaryExpectedFutureExecutionCostUsd, 0.430595);
...
assert.deepEqual(preparation.costEstimate.projectedUsage, {
  audioInputTokens: 21982,
  textInputTokens: 1368,
  outputTokens: 37222,
  totalInputTokens: 23350
});
...
assert.equal(preparation.executionPolicy.callsMaximum, 12);
...
assert.equal(
  ...
  "activate-and-execute-twelve-batch-14-paid-audio-verification-calls-under-standing-authorization"
);
