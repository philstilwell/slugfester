...
assertV4(
  ...
  "Batch 17 publication analysis boundary changed"
);
...
for (const result of execution.results) {
  ...
  const validation = validatePostCanaryBatch17PublicationOutput(
...
const passed =
  execution.status === "four-post-canary-batch-17-publication-contexts-passed" &&
  execution.contextsAttempted === 4 &&
  execution.contextsUnattempted === 0 &&
  execution.validContexts === 4 &&
  execution.invalidContexts === 0 &&
  valid.length === 4 &&
  valid.reduce((sum, item) => sum + item.validation.critiques, 0) === 79 &&
  valid.reduce((sum, item) => sum + item.validation.quoteExactSourceMatches, 0) === 8 &&
  valid.reduce((sum, item) => sum + item.validation.overallCommentarySides, 0) === 8 &&
  valid.reduce((sum, item) => sum + item.validation.aiExtensionSides, 0) === 8;

const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-17-publication-analysis",
  protocolId: activation.protocolId,
  status: passed
    ? "post-canary-batch-17-publication-output-gate-passed"
  ...
  nextAuthorizedAction: passed
    ? "standing-authorization-permits-batch-17-publication-compilation-preparation"
    : "standing-authorization-permits-batch-17-publication-failure-diagnosis"
};
