...
assert.equal(manifest.protocolId, POST_CANARY_BATCH_08_PUBLICATION_PROTOCOL_ID);
...
for (const context of manifest.contexts) {
  ...
  assert.equal(packet.protocolId, POST_CANARY_BATCH_08_PUBLICATION_PROTOCOL_ID);
  ...
  assert.equal(schema.properties.schemaVersion.const, POST_CANARY_BATCH_08_PUBLICATION_OUTPUT_VERSION);
  assert.equal(schema.properties.protocolId.const, POST_CANARY_BATCH_08_PUBLICATION_PROTOCOL_ID);
