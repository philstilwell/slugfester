...
assertV4(
  activation.status ===
    ...
    activation.executionPolicy.contexts === 38 &&
  ...
  "v2.2 discovery execution is unauthorized"
);
...
assertV4(
  preparationManifest.contexts.length === 38 &&
...
async function executeContext(context, contextIndex) {
  ...
  const sourceDirectory = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-v22-discovery-${context.debateNumber}-${context.chunkId}-`
    )
  );
  const isolatedCodexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-v22-discovery-home-${context.debateNumber}-${context.chunkId}-`
    )
  );
  ...
  try {
    ...
    process.stdout.write(
      `[v2.2-discovery] starting ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber} ${context.chunkId}\n`
    );
  ...
  process.stdout.write(
    `[v2.2-discovery] Debate ${context.debateNumber} ${context.chunkId} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-score-stability-v2.2-fresh-validation-discovery-model-execution",
  ...
  status:
    validContexts === preparationManifest.contexts.length
      ...
      : "v2.2-validation-discovery-complete-with-failure",
