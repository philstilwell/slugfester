...
const stageRoot = "docs/assessment-production/post-canary-continuation-v1/batch-10/audio-verification";
const paths = {
...
const expectedDebates = ["21", "74", "123", "147", "130"];
const expectedMoves = [
  "con-religious-motivation-and-sanctity",
  "pro-natural-transcendence-without-supernaturalism",
  "pro-voluntary-risk-arena",
  "con-involuntary-belief-eternal-stakes",
  "pro-continuous-providential-opportunity",
  "pro-suffering-enlarges-receptivity",
  "pro-qualia-concept-unsettled",
  "con-experience-sensitive-religious-belief",
  "pro-missing-body-cumulative-significance"
];
const toolPath = "scripts/assessment-production-post-canary-batch-10-audio-verification-stage.mjs";
...
const standingAuthorization = await loadAndValidatePostCanaryBatch10StandingAuthorization();
...
async function validatePreparation(preparation) {
  assert.equal(
    preparation.status,
    "prepared-nine-post-canary-batch-10-paid-known-speaker-diarizations-standing-authorization-conditional-activation-ready"
  );
  assert.equal(preparation.calls.length, 9);
  ...
  assert.equal(preparation.costEstimate.durationOnlyPlanningEstimateUsd, 0.1308768);
  assert.equal(preparation.costEstimate.primaryExpectedFutureExecutionCostUsd, 0.47646);
...
async function activate() {
  ...
  const activation = {
    ...preparation,
    schemaVersion: "1.0-assessment-production-post-canary-batch-10-audio-verification-execution-manifest",
    status: "frozen-nine-post-canary-batch-10-paid-known-speaker-diarizations-authorized-under-standing-authorization",
    activatedAt,
    ...
    userExecutionAuthorization: {
      ...
      authorizedDurationOnlyPlanningEstimateUsd: 0.1308768,
      frozenUsageDerivedEstimateUsd: 0.47646,
      verificationCallsAuthorized: 9,
    ...
    canonicalSourceGate: {
      ...
      transcriptHashesVerified: 5,
      eventHashesVerified: 5,
      manifestHashesVerified: 5,
      dyadicDebatesVerified: 5
    },
    executionToolHashes,
    costEstimate: {
    ...
    nextAuthorizedAction: "execute-exactly-nine-post-canary-batch-10-paid-audio-verification-calls-once-sequentially"
  };
  ...
  console.log(JSON.stringify({
    ...
    activatedAt,
    calls: 9,
    durationOnlyPlanningEstimateUsd: 0.1308768,
    expectedCostUsd: 0.47646,
    maximumAuthorizedCostUsd: 1,
    canonicalSourceHashesVerified: 15,
    sequentialExecution: true,
    attemptsPerCall: 1,
    retriesMaximum: 0,
...
async function validateActivation(manifest) {
  assert.equal(
    manifest.status,
    "frozen-nine-post-canary-batch-10-paid-known-speaker-diarizations-authorized-under-standing-authorization"
  );
  assert.equal(manifest.calls.length, 9);
  ...
  assert.equal(manifest.costEstimate.durationOnlyPlanningEstimateUsd, 0.1308768);
  assert.equal(manifest.costEstimate.primaryExpectedFutureExecutionCostUsd, 0.47646);
...
async function run() {
  ...
  for (const [callIndex, call] of manifest.calls.entries()) {
    ...
    process.stdout.write(`[batch-10-audio] ${callIndex + 1}/9 starting ${call.debateNumber}:${call.moveId}\n`);
    ...
    results.push(result);
    process.stdout.write(`[batch-10-audio] ${callIndex + 1}/9 ${result.status}; cumulative $${result.cumulativeUsageDerivedEstimatedCostUsd.toFixed(7)}\n`);
  }
  ...
  const execution = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-10-audio-verification-model-execution",
    protocolId: manifest.protocolId,
    status: completed.length === 9
      ? "nine-post-canary-batch-10-paid-known-speaker-diarizations-completed"
      : "post-canary-batch-10-paid-diarization-incomplete",
    provider: "OpenAI Transcription API",
...
async function analyze() {
  ...
  const executionComplete = execution.callsCompleted === 9;
  const passed = executionComplete && verified === 9;
  const status = passed
    ? "passed-all-nine-post-canary-batch-10-confidence-moves-audio-verified"
    : executionComplete
      ? "post-canary-batch-10-audio-verification-unresolved"
      : "post-canary-batch-10-audio-verification-incomplete";
  const nextAuthorizedAction = execution.costCapReachedOrExceeded || !execution.directIncrementalCostCapControlPassed
    ? "user-review-required-before-any-batch-10-downstream-work-after-audio-cost-cap-event"
    : passed
      ? "prepare-freeze-and-push-batch-10-dispute-only-adjudication-packets-under-standing-authorization"
      : "standing-authorization-stop-new-approval-required-before-batch-10-audio-verification-failure-diagnosis";
  const authorization = {
  ...
  const audit = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-10-audio-verification-audit",
    protocolId: manifest.protocolId,
    status,
  ...
  const analysis = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-10-audio-verification-analysis",
    protocolId: manifest.protocolId,
    status,
    productionCanary: false,
    batchNumber: 10,
...
async function cost() {
  ...
  const result = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-10-audio-cost-control-analysis",
    ...
    costControl: {
      ...
      authorizedDurationOnlyPlanningEstimateUsd: 0.1308768,
      usageDerivedPlanningEstimateUsd: 0.47646,
      approvedMaximumCostUsd: 1,
      usageDerivedEstimatedCostUsd: totals.usageDerivedEstimatedCostUsd,
      estimateDifferenceUsd: totals.usageDerivedEstimatedCostUsd - 0.47646,
...
async function test() {
  ...
  if (!(await exists(paths.activation))) {
    console.log(JSON.stringify({ status: "passed-preactivation", calls: 9, paidCalls: 0 }, null, 2));
    return;
  }
  ...
  if (!(await exists(paths.execution))) {
    ...
    console.log(JSON.stringify({ status: "passed-activated", calls: 9, retries: 0, maximumAuthorizedCostUsd: 1 }, null, 2));
    return;
  }
  const execution = await readJson(paths.execution);
  assert.equal(execution.callsPlanned, 9);
  assert.equal(execution.results.length, 9);
  ...
  assert(execution.callsAttempted <= 9);
  assert.equal(execution.retries, 0);
  ...
  if (!execution.requestFailure && !execution.costCapReachedOrExceeded) {
    assert.equal(execution.callsAttempted, 9);
    assert.equal(execution.callsCompleted, 9);
  ...
  assert.equal(audit.totals.requiredMoves, 9);
  assert.equal(audit.totals.verified + audit.totals.unresolved, 9);
  assert.equal(audit.totals.retries, 0);
  assert.equal(analysis.gate.requiredMoves, 9);
  assert.equal(analysis.gate.verified + analysis.gate.unresolved, 9);
  ...
  assert.equal(costResult.costControl.authorizedDurationOnlyPlanningEstimateUsd, 0.1308768);
  assert.equal(costResult.costControl.usageDerivedPlanningEstimateUsd, 0.47646);
