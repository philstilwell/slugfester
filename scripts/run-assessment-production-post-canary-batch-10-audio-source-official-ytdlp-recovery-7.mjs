...
assertV4(
  plan.officialYtDlpSources.length === 3 &&
    plan.officialYtDlpSources.every((source) =>
...
if (!shouldExecute) {
  console.log(
    JSON.stringify(
      {
        status: "passed-activated-batch-10-audio-source-official-ytdlp-recovery-7-preflight",
...
const installSource = async (source) => {
  ...
  const nativeSource = plan.officialYtDlpSources.find(
    (item) => item.sourceVideoId === source.sourceVideoId
  );
  assertV4(
    source.mode === "range-verified-format139-once" && nativeSource,
    `${debateNumber}: official yt-dlp format-139 source plan is missing`
  );
  assertV4(!(await exists(finalSourcePath)), `${debateNumber}: source already exists`);
  normalizedTemporaryPath = path.join(
    audioDirectory,
    "source.official-ytdlp7-format139.normalized.mp3"
  );
  const rawSourcePath = path.join(
    audioDirectory,
    "source.official-ytdlp7-format139.m4a"
  );
  assertV4(
    !(await exists(rawSourcePath)),
    `${debateNumber}: official yt-dlp transfer evidence already exists`
  );
  ...
  assertV4(
    rawStat.size === nativeSource.declaredBytes,
    `${debateNumber}: official yt-dlp source byte count failed`
  );
  ...
  assertV4(
    ...
    `${debateNumber}: official yt-dlp format-139 source validation failed`
  );
  ...
  sourceRecords.push({
    ...
    publicSourceAcquisitionAttempts: 1,
    publicSourceAttemptOutcome: "success",
    sourceFormatId: nativeSource.formatId,
    ...
    declaredSourceBytes: nativeSource.declaredBytes,
...
try {
  ...
  const preparation = {
    ...
    recovery: {
      ...
      officialYtDlpFormat139Downloads: 3,
  ...
  executionFailure = {
    category: "source-or-validation-failure-during-authorized-official-ytdlp-recovery-7",
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-10-audio-source-official-ytdlp-recovery-7-execution",
  status: executionFailure
    ? "failed-one-shot-batch-10-audio-source-official-ytdlp-recovery-7-stop-required"
