...
import {
  ...
  evaluatePostCanaryBatch06ScoreStability
...
const accepted = evaluatePostCanaryBatch06ScoreStability(passing);
...
assert.equal(
  evaluatePostCanaryBatch06ScoreStability(winnerMutation).acceptancePassed,
  false
);
const distanceMutation = structuredClone(passing);
distanceMutation[2].final = scores(95, 80);
assert.equal(
  evaluatePostCanaryBatch06ScoreStability(distanceMutation).acceptancePassed,
  false
);
assert.throws(() =>
  evaluatePostCanaryBatch06ScoreStability(passing, {
    ...POST_CANARY_BATCH_06_SCORE_STABILITY_THRESHOLDS,
