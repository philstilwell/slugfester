...
const ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-05/inventory-candidate-sharded";
...
const SCRIPT =
  "scripts/run-assessment-production-post-canary-batch-05-inventory-plans.mjs";
const PREPARER =
  "scripts/prepare-assessment-production-post-canary-batch-05-inventory.mjs";
const CONTEXTS = 10;
const DEBATES = ["158", "46", "64", "132", "189", "109", "179", "05", "42", "59"];
...
const ACTIVATION_STATUS =
  "frozen-ten-post-canary-batch-05-candidate-census-plan-contexts-authorized";
const EXECUTION_STATUS =
  "ten-post-canary-batch-05-candidate-census-plan-contexts-passed";
const PROTOCOL_ID =
...
async function loadAndValidatePreparation({ requireFutureAbsent = false } = {}) {
  ...
  assertV4(
    ...
    "Batch 5 candidate-census planner preparation is invalid"
  );
  assertV4(
    ...
    "Batch 5 source preparation drifted"
  );
...
async function preflight() {
  ...
  assertV4(
    ...
    "frozen Batch 5 preparation validator failed"
  );
  console.log(
    JSON.stringify(
      {
...
async function activate() {
  ...
  const activation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-05-candidate-census-plan-execution-activation",
    ...
    userAuthorization: {
      scope:
        "activate and execute exactly the ten frozen Batch 5 candidate-census planner contexts under the frozen standing authorization",
    ...
    nextRequiredAction:
      "execute-exactly-ten-frozen-batch-05-candidate-census-plan-contexts-once",
  };
...
async function loadAndValidateActivation({ requireFutureAbsent = false } = {}) {
  ...
  assertV4(
    ...
    "Batch 5 candidate-census planner execution is unauthorized"
  );
...
async function execute() {
  ...
  async function executeContext(context, contextIndex) {
    ...
    const sourceDirectory = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-post-canary-batch-05-candidate-plan-${context.debateNumber}-`
      )
    );
    const isolatedCodexHome = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-post-canary-batch-05-candidate-plan-home-${context.debateNumber}-`
      )
    );
    ...
    process.stdout.write(
      `[post-canary-batch-05-candidate-plan] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
    );
    return record;
  }
  ...
  const execution = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-05-candidate-census-plan-model-execution",
    protocolId: PROTOCOL_ID,
    status:
      validContexts === CONTEXTS
        ? EXECUTION_STATUS
        : "post-canary-batch-05-candidate-census-plan-gate-complete-with-failure",
    productionContinuation: true,
    developmentValidationOnly: false,
    productionCanary: false,
    stagingOnly: true,
    ...
    nextRequiredAction:
      validContexts === CONTEXTS
        ? "analyze-and-replay-ten-batch-05-candidate-census-plans-model-free-only"
        : "stop-preserve-failed-batch-05-candidate-census-plan-gate",
  };
...
async function analyze() {
  ...
  const analysis = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-05-candidate-census-plan-analysis",
    protocolId: PROTOCOL_ID,
    status:
      "post-canary-batch-05-candidate-census-plan-gate-passed-standing-authorization-active-for-side-packet-preparation",
    productionContinuation: true,
    developmentValidationOnly: false,
    productionCanary: false,
    stagingOnly: true,
    ...
    nextAuthorizedAction:
      "prepare-freeze-and-activate-twenty-exact-batch-05-side-selector-packets-under-standing-authorization",
  };
...
if (command === "preflight") await preflight();
  ...
  throw new Error(
    "usage: run-assessment-production-post-canary-batch-05-inventory-plans.mjs <preflight|activate|run|analyze|validate>"
  );
}
