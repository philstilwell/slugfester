...
const EXPECTED_AUDIO = [
  "17:pro-cumulative-moral-christian-case",
  "17:pro-gospel-priors-testimony",
  "75:pro-immaterial-concrete-foundation",
  "75:pro-weaker-senses-of-necessity"
];
const sha256 = (value) =>
...
assert.equal(
  preparation.status,
  "prepared-and-frozen-four-v2.2.3-local-audio-source-work-items"
);
assert.equal(work.status, "prepared-four-v2.2.3-local-audio-source-work-items");
...
assert.equal(preparation.totals.moves, 4);
assert.equal(work.moves.length, 4);
...
assert.equal(preparation.validation.canonicalLocalTranscriptChainsVerified, 2);
assert.equal(preparation.validation.repositoryRenderedLockedExcerpts, 4);
assert.equal(preparation.validation.repositoryRenderedTimestampWindows, 4);
assert.equal(preparation.validation.expectedSpeakersLocked, 4);
