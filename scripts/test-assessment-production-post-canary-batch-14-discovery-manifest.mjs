...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-14/discovery";
...
assert.equal(manifest.status, "frozen-thirty-three-post-canary-batch-14-discovery-contexts-prepared-not-authorized");
...
assert.deepEqual(manifest.contexts.map((context) => context.contextIndex), Array.from({ length: 33 }, (_, index) => index));
...
assert.equal(manifest.executionPolicy.copiedInputBytesMaximum, 65890);
assert.equal(preparation.totals.maximumCopiedInputBytes, 65890);
...
assert.equal(manifest.copiedInputBoundary.frozenObservedCopiedInputBytesMaximum, 65890);
assert.equal(manifest.copiedInputBoundary.executionCopiedInputBytesMaximum, 65890);
assert.deepEqual(manifest.copiedInputBoundary.maximumContext, { debateNumber: "57", chunkId: "chunk-002" });
...
for (const context of manifest.contexts) {
  ...
  assert(buildBatch14TokenCountedChunkLedger(chunkBytes).equals(tokenBytes));
...
assert.equal(manifest.nextAuthorizedAction, "prepare-and-freeze-batch-14-discovery-execution-activation-under-standing-authorization");

const validation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-discovery-execution-preparation-validation",
  protocolId: manifest.protocolId,
  status: "batch-14-discovery-execution-manifest-validation-passed-frozen-standing-authorization-active",
