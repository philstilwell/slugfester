...
import {
  buildAssessmentProductionPostCanaryBatch02AudioWorkItems
} from "./lib/assessment-production-post-canary-batch-02-audio-work-items.mjs";
import {
  ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_02_DISAGREEMENT_PROTOCOL_ID,
  ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_02_DISAGREEMENT_ROOT
} from "./lib/assessment-production-post-canary-batch-02-disagreement.mjs";
...
const USER_AUTHORIZATION = Object.freeze({
  instruction:
    "I approve preparation, validation, freezing, committing, and pushing of exactly ten Batch 2 local audio-source work items, with a direct incremental cost cap of $0. Do not access, download, or play audio; execute models; use paid services; adjudicate; derive scores; reconstruct publication; mutate production; or select the next batch.",
  directIncrementalCostUsdMaximum: 0,
  workItemsAuthorized: 10,
  audioAccessAuthorized: false,
  audioDownloadAuthorized: false,
...
const TOOL_SOURCES = [
  "scripts/prepare-assessment-production-post-canary-batch-02-audio-work-items.mjs",
  "scripts/test-assessment-production-post-canary-batch-02-audio-work-items.mjs",
  "scripts/lib/assessment-production-post-canary-batch-02-audio-work-items.mjs",
...
if (shouldWrite) {
  assertV4(
    ...
    "frozen Batch 2 audio work-item artifacts already exist"
  );
}
...
assertV4(
  analysis.status ===
    ...
    analysis.authorization.audioWorkPreparation === false &&
    analysis.nextAuthorizedAction ===
      "user-approval-required-before-batch-02-local-audio-source-work-item-preparation",
  "Batch 2 disagreement gate or prior audio-work stop rule changed"
);
assertV4(
  analysis.audioWorkload.moves === 10 &&
  ...
  "Batch 2 ten-move audio queue or preparation boundary changed"
);
assertV4(
  analysis.sourceCompatibility?.status ===
    ...
    analysis.sourceCompatibility.minimumCandidateLexicalTokensChanged ===
      false &&
    analysis.sourceCompatibility.occurrences?.length === 1 &&
    analysis.sourceCompatibility.occurrences[0]?.debateNumber === "99",
  "Batch 2 source-compatibility evidence changed"
);
assertV4(
  ...
  "Batch 2 execution-preparation population changed"
);
assertV4(
...
assertV4(
  ...
  "Batch 2 exact ten-move audio queue was not reproduced"
);
assertV4(
  audioWorkItems.length === 10 &&
    sourceSummaries.length === 4 &&
  ...
  "Batch 2 work-item trigger, count, or metadata-only boundary changed"
);

const workDocument = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-audio-work-items",
  protocolId:
    ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_02_DISAGREEMENT_PROTOCOL_ID,
  status:
    "prepared-ten-post-canary-batch-02-local-audio-source-work-items-awaiting-separate-audio-access-approval",
  ...
  scoresDerived: 0,
  authorization: {
    localAudioSourcePreparation: false,
    audioAccess: false,
    audioDownload: false,
    audioPlayback: false,
...
const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-audio-work-item-preparation",
  protocolId:
    ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_02_DISAGREEMENT_PROTOCOL_ID,
  status:
    "prepared-and-frozen-ten-post-canary-batch-02-local-audio-source-work-items-awaiting-separate-audio-access-approval",
  developmentValidationOnly: false,
  productionCanary: false,
  batchNumber: 2,
  stagingOnly: true,
  ...
  nextAuthorizedAction:
    "user-approval-required-before-batch-02-local-audio-source-and-clip-preparation"
};
