...
import {
  buildBatch03TokenCountedChunkLedger,
  findBatch03ZeroLexicalTokenRows,
} from "./lib/assessment-production-post-canary-batch-03-source-preparation.mjs";
import {
...
const BATCH_ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-03";
const ROOT = `${BATCH_ROOT}/source-preparation`;
const PROTOCOL_ID = "assessment-production-post-canary-batch-03-source-preparation";
const CACHE_VERSION = "post-canary-continuation-v1-batch-03";
...
const STANDING_AUTHORIZATION = POST_CANARY_BATCH_03_STANDING_AUTHORIZATION;
...
const SCRIPT = "scripts/prepare-assessment-production-post-canary-batch-03-source-packets.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-03-source-packets.mjs";
...
const CONTROL_FILES = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-03-source-preparation.mjs",
...
const { record: standingAuthorization, sha256: standingAuthorizationSha256 } =
  await loadAndValidatePostCanaryBatch03StandingAuthorization();

assertV4(
  selection.status === "third-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    selectionAnalysis.status === "third-post-canary-batch-selection-analysis-passed-awaiting-source-packet-preparation-decision" &&
    selectionAnalysis.selection.sha256 === sha256(selectionBytes) &&
    selection.nextAuthorizedAction === "user-decision-on-third-post-canary-batch-source-packet-preparation" &&
    JSON.stringify(selection.selected.map((item) => item.debateNumber)) === JSON.stringify(REQUIRED_ORDER),
  "Batch 3 frozen selection is not ready for its standing-authorized source-packet step"
);
assertV4(
  standingAuthorization.authorization.sourcePacketPreparation === true &&
    standingAuthorization.selectedDebates.join(",") === REQUIRED_ORDER.join(",") &&
    standingAuthorization.costBoundary.subscriptionAndLocalDirectIncrementalCostUsdMaximum === 0 &&
    sha256(controlBytes[STANDING_AUTHORIZATION]) === standingAuthorizationSha256,
  "Batch 3 standing authorization does not authorize source-packet preparation"
);
assertV4(
  ...
  "Batch 3 model boundary or stop rules drifted"
);
assertV4(
...
for (const selected of selection.selected) {
  ...
  packet.schemaVersion = "1.0-assessment-production-post-canary-batch-03-score-blind-source-packet";
  ...
  for (const chunk of plan.chunks) {
    ...
    const tokenLedgerBytes = buildBatch03TokenCountedChunkLedger(chunkBytes);
    const zeroLexicalTokenRows = findBatch03ZeroLexicalTokenRows(chunkBytes);
...
const preparation = {
  ...
  status: shouldWrite ? "post-canary-batch-03-ten-complete-score-blind-source-packets-prepared-awaiting-validation" : "preview",
  ...
  AIOnly: true,
  userAuthorization: {
    scope: "Batch 3 standing-authorized score-blind source-packet preparation and deterministic validation only",
  ...
  tokenLedgerCompatibility: {
    status: "exact-source-zero-lexical-token-row-preserved-with-zero-count",
    reason: `${zeroLexicalTokenOccurrences.length} nonempty canonical caption rows contain zero repository lexical tokens; every occurrence is preserved byte-for-byte with count zero.`,
    policy: "Preserve the exact canonical row and text, record lexicalTokenCount as 0, and continue to enforce the unchanged 12-token minimum on every inclusive candidate window.",
  ...
  nextAuthorizedAction: "run-batch-03-deterministic-source-packet-validation-then-prepare-discovery-execution-under-standing-authorization",
};
