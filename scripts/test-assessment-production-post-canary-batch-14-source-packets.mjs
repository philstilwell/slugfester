...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-14/source-preparation";
...
assert.equal(preparation.status, "post-canary-batch-14-ten-complete-score-blind-source-packets-prepared-awaiting-validation");
...
for (const context of preparation.contexts) {
  ...
  assert(frozen, `${context.debateNumber}: absent from frozen Batch 14 selection`);
  ...
  assert.equal(packet.schemaVersion, "1.0-assessment-production-post-canary-batch-14-score-blind-source-packet");
  ...
  for (const chunk of context.chunks) {
    ...
    assert.deepEqual(
      ...
      findBatch14ZeroLexicalTokenRows(chunkBytes).map(({ eventIndex, startMs, durationMs, text }) => ({
        eventIndex,
        startMs,
        durationMs,
...
const validation = {
  ...
  status: "post-canary-batch-14-score-blind-source-packet-validation-passed-frozen-under-standing-authorization",
  ...
  nextAuthorizedAction: "prepare-freeze-and-activate-batch-14-discovery-execution-under-standing-authorization",
};
