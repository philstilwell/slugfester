...
const root = POST_CANARY_BATCH_05_COMPATIBILITY_ROOT;
...
const validator = await import(
  `./validate-debates.mjs?batch05Compatibility=${activation.validator.proposedSha256}`
);
...
for (const lock of activation.packetHashes) {
  ...
  const routeLock =
    validator.validatePostCanaryBatch05LedgerAdapterRouteLocks({
      debate: candidate,
      ledgerText,
  ...
  const route = validator.validatePostCanaryBatch05LedgerAdapterRoute({
    debate: candidate,
    ledger,
    ledgerText
  });
  assert.equal(route.repositoryScoreReplayPassed, true);
  const adapter = validatePostCanaryBatch05SiteLedgerAdapter({
    adapter: ledger,
    candidate,
...
assert.equal(oneSidedDisplayRows, 7);
assert.equal(emptyReferenceLinks, 59);
assert.equal(suppliedReferenceLinks, 5);
assert.equal(
  ...
  JSON.stringify(
    {
      ...
      positiveRoutes,
      negativeControls,
      existingAdapterRoutesPreserved: 4,
      modelContexts: 0,
      paidServiceCalls: 0,
