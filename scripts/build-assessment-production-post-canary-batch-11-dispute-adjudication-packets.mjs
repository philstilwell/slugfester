...
import {
  POST_CANARY_BATCH_11_DISPUTE_ADJ_PROTOCOL_ID,
  POST_CANARY_BATCH_11_DISPUTE_ADJ_ROOT,
  buildPostCanaryBatch11DisputeAdjudicationPacket,
  makePostCanaryBatch11DisputeAdjudicationSchema
} from "./lib/assessment-production-post-canary-batch-11-dispute-adjudication.mjs";
...
const USER_AUTHORIZATION = Object.freeze({
  instruction:
    "The frozen Batch 11 standing authorization permits automatic preparation, validation, freezing, activation, execution, analysis, committing, and pushing of score-blind dispute-only adjudication checkpoints for exactly the ten selected debates while every frozen gate passes, with subscription-backed and local direct incremental cost capped at $0.",
...
const inputPaths = {
  ...
  schema: `${POST_CANARY_BATCH_11_DISPUTE_ADJ_ROOT}/adjudication.schema.json`
};
const TOOL_SOURCES = [
  "scripts/lib/v4-lean-production.mjs",
  "scripts/lib/v4221175-decomposed-adjudication.mjs",
  "scripts/lib/v42211728-hard-route-adjudication.mjs",
  "scripts/lib/assessment-production-post-canary-batch-11-dispute-adjudication.mjs",
  "scripts/build-assessment-production-post-canary-batch-11-dispute-adjudication-packets.mjs",
  "scripts/test-assessment-production-post-canary-batch-11-dispute-adjudication-packets.mjs",
  "scripts/validate-assessment-production-post-canary-batch-11-dispute-adjudication-output.mjs",
  "scripts/preregister-assessment-production-post-canary-batch-11-dispute-adjudication.mjs",
  "scripts/test-assessment-production-post-canary-batch-11-dispute-adjudication-manifest.mjs"
];
...
const judgmentExecutionPath =
  `${JUDGMENT_ROOT}/recovery-1/debate-60-pass-b/cohort-execution-overlay.json`;
...
assertV4(
  sourceAnalysis.status ===
      "post-canary-batch-11-deterministic-disagreements-extracted-standing-authorization-active-for-audio-work" &&
    JSON.stringify(sourceAnalysis.debates.map((item) => item.debateNumber)) ===
      JSON.stringify(EXPECTED_DEBATES) &&
    sourceAnalysis.adjudicationWorkload.disputedMoves === 184 &&
    sourceAnalysis.adjudicationWorkload.candidateSelections === 507 &&
    sourceAnalysis.adjudicationWorkload.packetsPrepared === false &&
    sourceAnalysis.adjudicationWorkload.modelContextsExecuted === 0,
  "Batch 11 deterministic disagreement workload changed"
);
assertV4(
  judgmentExecution.status ===
    ...
    judgmentAnalysis.status ===
      "twenty-post-canary-batch-11-independent-judgments-passed-standing-authorization-active-for-disagreement-extraction" &&
    judgmentAnalysis.totals.contexts === 20,
  "Batch 11 accepted independent-judgment boundary changed"
);
assertV4(
  ...
  "Batch 11 independent-judgment configuration changed"
);
...
assertV4(
  ...
  "Batch 11 standing authorization changed"
);
assertV4(
  audioAudit.status ===
      "passed-all-two-post-canary-batch-11-confidence-moves-audio-verified" &&
    ...
    audioCost.costControl.usageDerivedEstimatedCostUsd === 0.06726 &&
    ...
    audioModelExecution.callsAttempted === 2 &&
    audioModelExecution.callsCompleted === 2 &&
    audioModelExecution.retries === 0,
  "Batch 11 accepted audio-verification boundary changed"
);
assertV4(
  ...
  "Batch 11 packet-preparation authorization changed"
);
...
assertV4(audioByDebateMove.size === 2, "Batch 11 verified audio population changed");

const schema = makePostCanaryBatch11DisputeAdjudicationSchema();
...
if (shouldWrite) {
  const immutableOutputs = [
    inputPaths.schema,
    `${POST_CANARY_BATCH_11_DISPUTE_ADJ_ROOT}/preparation-manifest.json`,
    `${POST_CANARY_BATCH_11_DISPUTE_ADJ_ROOT}/execution-preparation-manifest.json`,
    ...EXPECTED_DEBATES.flatMap((debateNumber) => [
      `${POST_CANARY_BATCH_11_DISPUTE_ADJ_ROOT}/packets/debate-${debateNumber}.json`,
      `${POST_CANARY_BATCH_11_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`
    ])
  ];
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const built = buildPostCanaryBatch11DisputeAdjudicationPacket(
  ...
  const provenancePath =
    `${POST_CANARY_BATCH_11_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`;
  const outputPath =
    `${POST_CANARY_BATCH_11_DISPUTE_ADJ_ROOT}/outputs/debate-${debateNumber}.json`;
  const packetBytes = pretty(built.packet);
  const provenanceBytes = pretty({
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-11-adjudication-candidate-provenance",
...
const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-11-dispute-only-adjudication-preparation",
  protocolId: POST_CANARY_BATCH_11_DISPUTE_ADJ_PROTOCOL_ID,
  status: shouldWrite
    ? "prepared-ten-isolated-post-canary-batch-11-dispute-only-adjudication-contexts"
    : "preview",
  frozenAt,
  ...
  acceptedSourceBoundary: {
    ...
    audioUsageDerivedEstimatedCostUsd: 0.06726,
  ...
  nextAuthorizedAction:
    "freeze-post-canary-batch-11-dispute-only-adjudication-execution-preparation-manifest-model-free-only"
};
assertV4(
  contexts.length === 10 &&
    disputedMoves === 184 &&
  ...
  "Batch 11 adjudication packet workload differs from frozen inputs"
);
assertV4(
  preparation.totals.maximumCopiedInputBytes <= 400000,
  `Batch 11 context ${preparation.totals.maximumCopiedInputBytes} bytes exceeds the frozen 400 KB correction ceiling`
);
if (shouldWrite) {
  await mkdir(POST_CANARY_BATCH_11_DISPUTE_ADJ_ROOT, { recursive: true });
