...
const V211_SELECTION =
  "docs/assessment-production/score-stability-v2.1.1-validation-cohort/selection.json";
const SCRIPT =
  "scripts/select-assessment-production-score-stability-v2.1.2-cohort.mjs";
const TEST =
  "scripts/test-assessment-production-score-stability-v2.1.2-cohort.mjs";
...
const v211Selection = JSON.parse(sourceBytes[V211_SELECTION]);
...
assertV4(
  productionManifest.schemaVersion ===
    ...
    v211Selection.status ===
      "fresh-disjoint-v2.1.1-ten-debate-cohort-source-gate-passed" &&
    v211Selection.selected?.length === 10,
  "v2.1.2 cohort selection is unauthorized or its source boundary drifted"
);
...
const failedV211ValidationDebates = new Set(
  v211Selection.selected.map((item) => item.debateNumber)
);
...
assertV4(
  productionCanaryDebates.size === 10 &&
    validationCohorts.every((cohort) => cohort.size === 10) &&
    validationCohorts.every((cohort, index) =>
      [...cohort].every(
        (number) =>
          !productionCanaryDebates.has(number) &&
          validationCohorts.every(
            (other, otherIndex) =>
...
assertV4(
  ...
  "v2.1.2 cohort is not fresh, disjoint, dyadic, and source-gated"
);

const selection = {
  schemaVersion: "1.0-score-stability-v2.1.2-validation-cohort-selection",
  protocolId:
    "assessment-production-score-stability-v2.1.2-fresh-validation-selection",
  ...
  successorProtocol: {
    ...
    version: "v2.1.2-bounded-end-discovery",
    sourceSelectionShape: ["startEvent", "endEvent"],
    minimumLexicalTokens: 12,
  ...
  selectionPolicy: {
    ...
    failedV211ValidationCohortExcluded: true,
    ...
    failedV211ValidationDebateNumbers: [...failedV211ValidationDebates].sort(),
    observedDebateNumbers: [...observedDebates].sort(),
    eligibleCandidateCount: eligible.length,
    deterministicRank:
      "SHA-256(v2.1 policy hash | v2.1.2 successor analysis hash | debate number | debate id), ascending",
  ...
  nextAuthorizedAction:
    "prepare-v2.1.2-source-packets-token-ledgers-and-schemas-model-free-only",
};
