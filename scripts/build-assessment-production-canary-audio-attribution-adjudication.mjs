...
import {
  ...
  PRODUCTION_CANARY_AUDIO_ADJ_PACKET_VERSION,
...
assertV4(
  ...
  "audio-attribution adjudication packet preparation is unauthorized"
);
...
const packet = {
  schemaVersion: PRODUCTION_CANARY_AUDIO_ADJ_PACKET_VERSION,
...
const schemaPath = `${PRODUCTION_CANARY_AUDIO_ADJ_ROOT}/schema.json`;
...
const preparation = {
  schemaVersion: "1.0-production-canary-audio-attribution-adjudication-preparation",
  protocolId: PRODUCTION_CANARY_AUDIO_ADJ_PROTOCOL_ID,
  status: shouldWrite
  ...
  inputs: {
    workflow:
      "docs/assessment-production-canary-audio-attribution-adjudication-workflow.md",
    manual: `${PRODUCTION_CANARY_AUDIO_ADJ_ROOT}/manual.md`,
    schema: schemaPath,
    packet: packetPath,
...
if (shouldWrite) {
  await mkdir(PRODUCTION_CANARY_AUDIO_ADJ_ROOT, { recursive: true });
