...
const analysisPath = `${BATCH_08_DEPENDENT_PILOT_CORRECTION_ROOT}/analysis.json`;
const startedAt = new Date().toISOString();
const [plan, activation] = await Promise.all([readJson(planPath), readJson(activationPath)]);
assertV4(
  plan.protocolId === BATCH_08_DEPENDENT_PILOT_CORRECTION_PROTOCOL_ID &&
    activation.status === "frozen-batch-08-dependent-pilot-analysis-correction-activated" &&
  ...
  "frozen one-shot Batch 8 dependent pilot correction activation required"
);
...
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-08-dependent-pilot-analysis-correction-execution",
  protocolId: BATCH_08_DEPENDENT_PILOT_CORRECTION_PROTOCOL_ID,
  status: "passed-complete-batch-08-production-transaction-after-dependent-pilot-analysis-correction",
  startedAt,
  completedAt,
...
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-08-dependent-pilot-analysis-correction-analysis",
  protocolId: BATCH_08_DEPENDENT_PILOT_CORRECTION_PROTOCOL_ID,
  status: "batch-08-production-publication-accepted-after-dependent-pilot-analysis-correction",
