...
const ROOT =
  "docs/assessment-production/production-checkpoint-v2.2-1/source-preparation";
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const preparation = JSON.parse(
  await readFile(`${ROOT}/preparation-manifest.json`, "utf8")
);
const master = JSON.parse(
  await readFile(preparation.inputs.masterManifest, "utf8")
);
...
assert.equal(preparation.totals.sourceChainOverlays, 1);
assert.equal(preparation.totals.partitionedDiscoveryDebates, 10);
assert.equal(preparation.totals.sourceEvents, master.cohort.totalEventCount);
for (const field of [
...
for (const [key, value] of Object.entries(preparation.authorization)) {
  if (
    ![
      ...
      "discoveryExecutionManifestPreparation",
    ].includes(key)
  ) {
...
const selectedByNumber = new Map(
  master.cohort.debates.map((debate) => [debate.debateNumber, debate])
);
...
for (const context of preparation.contexts) {
  ...
  assert(frozen, `${context.debateNumber}: not in frozen checkpoint`);
  const [
    packetBytes,
    planBytes,
    fullLedgerBytes,
  ...
  assert.equal(packet.modelInputBoundary.productionCanary, true);
  ...
  JSON.stringify(
    {
      ...
      sides: preparation.totals.sides,
