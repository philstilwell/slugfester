...
import { POST_CANARY_BATCH_02_PUBLICATION_ROOT } from "./lib/assessment-production-post-canary-batch-02-publication.mjs";
import {
...
const paths = {
  ...
  output: `${ROOT}/outputs/debate-103.json`,
  packet: `${ROOT}/packets/debate-103.json`,
  validation: `${ROOT}/validations/debate-103.json`,
  provenance: `${ROOT}/provenance/debate-103.json`,
...
assertV4(
  ...
  "the failed Batch 2 publication gate does not authorize diagnosis"
);
...
assertV4(
  result.contextIndex === 0 &&
    result.debateNumber === "103" &&
    result.debateId === "woodford-edwards-rational-belief-god-2023" &&
  ...
  "the failed Debate 103 publication context changed"
);
...
const expected = [
  ["pro-rationality-evidential-standard", 131, 1],
  ["pro-gratuitous-evil-divine-love", 138, 8],
  ["pro-salvation-confusion-divine-love", 134, 4],
  ["con-reason-designed-cognition", 134, 4],
  ["con-life-permitting-fine-tuning", 134, 4],
  ["con-objective-morality-grounded-in-god", 136, 6],
  ["pro-evolution-cognitive-reliability", 135, 5],
  ["con-truth-directed-designed-intelligence", 138, 8],
  ["con-suffering-counterevidence-not-defeater", 133, 3],
  ["pro-creator-evidence-classical-god-gap", 139, 9],
  ["pro-evolution-moral-intuitions", 137, 7],
  ["pro-divine-command-dilemma", 136, 6],
  ["con-biblical-revelation-and-human-dignity", 131, 1],
  ["con-euthyphro-divine-nature-resolution", 135, 5],
  ["pro-history-does-not-establish-miracles", 134, 4],
  ["con-guided-evolution-and-human-purpose", 133, 3],
  ["pro-natural-selection-explains-complexity", 135, 5]
];
assertV4(
  JSON.stringify(
  ...
  "Debate 103 failure is not the expected seventeen-field word-boundary failure"
);
assertV4(
  critiqueRows.every(
    (row) =>
...
assertV4(
  originalValidationMessage ===
    "pro-rationality-evidential-standard: critique outside 105–130 words",
  "the original deterministic failure changed"
);

// This copy is never written. Removing only surplus whitespace-delimited words
// proves that the seventeen listed critique fields are the complete failure boundary.
...
assertV4(
  hypotheticalWordsRemoved === 83,
  "Debate 103 diagnostic word-removal boundary changed"
);
...
assertV4(
  hypotheticalReplay.status === "passed" &&
    hypotheticalReplay.moves === 17 &&
    hypotheticalReplay.critiques === 17 &&
  ...
  "diagnostic-only substitutions did not isolate the Debate 103 failure"
);
...
assertV4(
  proposedRepairPartition.length === 9 &&
    proposedRepairPartition.every(
      (fields) => fields.length >= 1 && fields.length <= 2
    ) &&
    proposedRepairPartition.flat().length === 17,
  "prospective Debate 103 repair partition changed"
);

const diagnosis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-publication-failure-diagnosis",
  ...
  diagnosticReplay: {
    ...
    purpose:
      "prove-all-other-output-fields-pass-after-bounded-in-memory-removal-of-eighty-three-excess-words",
  ...
  prospectiveRecoveryOnly: {
    ...
    requiredAfterRepair: [
      "revalidate-the-complete-debate-103-publication-output",
      "freeze-a-separate-nine-context-resumption-manifest-only-if-debate-103-passes",
  ...
  nextRequiredAction:
    "prepare-nine-bounded-debate-103-publication-repair-packets-under-standing-authorization"
};
...
if (shouldWrite) {
  ...
  assertV4(
    ...
    "the frozen Debate 103 publication failure diagnosis changed"
  );
}
console.log(
  JSON.stringify(
