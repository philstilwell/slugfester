...
const RECOVERY_ROOT = `${CORRECTION_ROOT}/activation-harness-correction-1`;
...
const PREPARE_SCRIPT = "scripts/prepare-assessment-production-post-canary-batch-05-compatibility-preparation-correction-2-activation-harness-correction-1.mjs";
const ACTIVATE_SCRIPT = "scripts/activate-assessment-production-post-canary-batch-05-compatibility-preparation-correction-2-activation-harness-correction-1.mjs";
const RUN_SCRIPT = "scripts/run-assessment-production-post-canary-batch-05-compatibility-preparation-correction-2-activation-harness-correction-1.mjs";
...
if (await exists(RECOVERY_ROOT)) throw new Error("activation harness correction-1 already exists");
if (await exists(CORRECTION_ACTIVATION)) throw new Error("correction-2 activation unexpectedly exists");

const [correctionPlanBytes, targetScriptBytes] = await Promise.all([
  readFile(CORRECTION_PLAN),
  readFile(TARGET_SCRIPT)
]);
const correctionPlan = JSON.parse(correctionPlanBytes);
const before = "plan.sourceHashes[plan.correctionScope.target] !== plan.correctionScope.oldSha256";
...
if (targetScriptText.split(before).length !== 2) throw new Error("faulty activation preflight comparison is not unique");
if (
  correctionPlan.status !== "frozen-batch-05-compatibility-preparation-correction-2-prepared" ||
  correctionPlan.sourceHashes?.[TARGET_SCRIPT] !== sha256(targetScriptBytes)
) throw new Error("frozen correction-2 plan does not authenticate the failed activation harness");
...
const diagnosis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-05-compatibility-preparation-correction-2-activation-harness-diagnosis",
  status: "frozen-batch-05-correction-2-activation-preflight-file-versus-field-hash-error-diagnosed",
  diagnosedAt: frozenAt,
  checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(),
  failedCommand: "node scripts/activate-assessment-production-post-canary-batch-05-compatibility-preparation-correction-2.mjs --activated-at 2026-08-22T15:24:11.000Z",
  failure: {
    category: "deterministic-activation-preflight-file-hash-versus-json-field-hash-comparison",
    exitCode: 1,
    message: "correction-2 preimage or proposed hash is unauthenticated",
    invalidComparison: "SHA-256 of the complete analysis.json file compared with the old preparation.sha256 field value",
    requiredComparison: "analysis.json preparation.sha256 field compared with the frozen old field value"
  },
...
const plan = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-05-compatibility-preparation-correction-2-activation-harness-correction-1-plan",
  status: "frozen-batch-05-correction-2-activation-harness-correction-1-prepared",
  frozenAt,
  ...
  sourceHashes,
  executionPolicy: { attemptsMaximum: 1, retriesMaximum: 0, rerunsMaximum: 0, modelContexts: 0, paidServiceCalls: 0 },
  artifacts: { activation: ACTIVATION, execution: EXECUTION, analysis: ANALYSIS },
  nextAuthorizedAction: "activate-one-batch-05-correction-2-activation-harness-correction-1-pass"
};
await mkdir(RECOVERY_ROOT, { recursive: true });
await Promise.all([
  writeFile(DIAGNOSIS, jsonBytes(diagnosis)),
  writeFile(PROPOSED_SCRIPT, proposedScriptBytes),
  writeFile(PROPOSED_PLAN, proposedCorrectionPlanBytes),
  writeFile(PLAN, jsonBytes(plan))
]);
console.log(JSON.stringify({ status: plan.status, writesBeforeFailure: 0, writableHarnessExpressions: 1, directIncrementalCostUsd: 0, nextAuthorizedAction: plan.nextAuthorizedAction }, null, 2));
