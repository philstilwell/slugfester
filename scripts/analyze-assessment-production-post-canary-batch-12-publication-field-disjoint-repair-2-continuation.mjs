...
import { DEBATE_PLANS, ROOT, validateFieldDisjointRepairOutput } from
  "./lib/assessment-production-post-canary-batch-12-publication-field-disjoint-repair-2-continuation.mjs";
...
assertV4(
  execution.contextsPlanned === 15 &&
    [1, 15].includes(execution.contextsAttempted) &&
  ...
  "fifteen-shard level-2 continuation execution boundary changed"
);
      ...
      for (const context of debateContexts) {
        ...
        for (const target of data.packet.targets) {
          ...
          if (target.kind === "critique") {
            ...
            const extension = corrected.aiExtension[target.side];
            const candidates = [
              extension.thesis,
              ...extension.premises,
            ...
            const item = candidates.find((candidate) => candidate.id === target.itemId);
            assertV4(item, `${target.field}: AI Extension item changed`);
            originalValue = item.novelty.explanation;
            assertV4(sha256(originalValue) === target.originalValueSha256,
              `${target.field}: original novelty explanation changed`);
...
if (allRepaired) {
  const allDebates = ["152", "28", "119", "116", "131", "07", "33", "15", "29", "11"];
  ...
  for (const debateNumber of allDebates) {
    ...
    validationReplay.push({
      ...
      source: correctedData
        ? "accepted-field-disjoint-repair-level-2-continuation-merge"
        : debateNumber === "152"
          ? "accepted-field-disjoint-repair-level-1-merge"
          : debateNumber === "28"
            ? "accepted-field-disjoint-repair-level-2-merge"
            : "accepted-original-first-attempt",
      validation
    });
  }
  const completePassed = validationReplay.length === 10 &&
    validationReplay.every((item) => item.validation.status === "passed") &&
    validationReplay.reduce((sum, item) => sum + item.validation.moves, 0) === 204 &&
    validationReplay.reduce((sum, item) => sum + item.validation.critiques, 0) === 204 &&
  ...
  completeCohort = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-12-complete-publication-cohort-analysis-after-two-recovery-levels",
    protocolId: activation.protocolId,
    status: "post-canary-batch-12-complete-ten-debate-publication-output-gate-passed-after-two-recovery-levels",
    ...
    totals: {
      ...
      recoveryLevel1Contexts: 2,
      recoveryLevel2Contexts: 16,
...
const analysis = {
  ...
  protocolId: activation.protocolId,
  status: allRepaired
    ? "three-debate-field-disjoint-repair-level-2-continuation-passed-and-complete-cohort-validated"
    : repairedDebates > 0
  ...
  totals: {
    ...
    debatesFailed: 3 - repairedDebates,
    critiqueFieldsTargeted: 28,
    quoteFieldsTargeted: 0,
    noveltyExplanationFieldsTargeted: 1,
    writableFieldsTargeted: 29,
  ...
  nextAuthorizedAction: allRepaired
    ? "prepare-batch-12-deterministic-publication-compilation"
    : "stop-final-recovery-level-exhausted"
};
