...
const planPath = `${root}/audio-source-transport-recovery-1/correction-plan.json`;
...
assert.equal(plan.userAuthorization.firstBoundedTransportRecoveryAuthorized, true);
assert.equal(plan.userAuthorization.debate123AdditionalPublicSourceAttemptsAuthorized, 1);
assert.equal(plan.userAuthorization.debate147OriginalPublicSourceAttemptsAuthorized, 1);
assert.equal(plan.userAuthorization.debate130OriginalPublicSourceAttemptsAuthorized, 1);
...
assert.equal(await exists(plan.protectedInvalidEvidence.preservedPath), false);
...
assert.equal(plan.debate123Recovery.maximumAdditionalCliInvocations, 1);
assert.equal(plan.debate123Recovery.retriesMaximum, 0);
assert.equal(
  plan.debate123Recovery.transport,
  "web-safari-hls-manifest-native-segment-transport"
);
assert(plan.debate123Recovery.ytDlpArguments.includes("--no-continue"));
assert(plan.debate123Recovery.ytDlpArguments.includes("m3u8:native"));
