...
const PREPARATION = `${POST_CANARY_BATCH_02_RENDERING_ROOT}/preparation-manifest.json`;
...
assert.equal(preparation.protocolId, POST_CANARY_BATCH_02_RENDERING_PROTOCOL_ID);
assert.equal(
  preparation.status,
...
assert.deepEqual(preparation.explicitOrder, [...POST_CANARY_BATCH_02_RENDERING_ORDER]);
...
assert.equal(preparation.preservedControls.debate172IntegerRoundedTiePreserved, true);
...
assert.equal(
  ...
  canonicalJson(POST_CANARY_BATCH_02_RENDERING_VIEWPORTS)
);
assert.equal(
...
assert.equal(preparation.browserPlan.localServer.port, POST_CANARY_BATCH_02_RENDERING_PORT);
...
assert.equal(preparation.gateExpectations.moves, 190);
...
assert.equal(
  preparation.toolHashes[POST_CANARY_BATCH_02_RENDERING_IMAGE_CONTRACT.analyzerPath],
...
for (const row of preparation.packets) {
  ...
  assert.equal(packet.preview.url.includes(`127.0.0.1:${POST_CANARY_BATCH_02_RENDERING_PORT}`), true);
...
assert.equal(sectionTotal, 51);
assert.equal(moveTotal, 190);

const previewHtml = await readFile(path.resolve(preparation.inputs.localPreview), "utf8");
assert.match(previewHtml, /validated post-canary Batch 2 candidate only/);
...
console.log(
  JSON.stringify(
    {
      status: "post-canary-batch-02-rendering-preparation-passed",
