...
const preparationPath =
  `${POST_CANARY_BATCH_13_COMPATIBILITY_ROOT}/preparation-manifest.json`;
const analysisPath =
  `${POST_CANARY_BATCH_13_COMPATIBILITY_ROOT}/analysis.json`;
...
assert.equal(
  ...
  "batch-13-site-ledger-adapter-and-validator-route"
);
assert.equal(
...
assert.equal(preparation.totals.moves, 199);
assert.equal(preparation.totals.oneSidedDisplayRows, 13);
assert.equal(preparation.totals.overallBlunders, 52);
assert.equal(preparation.totals.emptyReferenceLinks, 44);
assert.equal(preparation.totals.suppliedReferenceLinks, 8);
...
assert.equal(
  await exists(`${POST_CANARY_BATCH_13_COMPATIBILITY_ROOT}/execution.json`),
  false
);
assert.equal(
  await exists(
...
assert.deepEqual(packetNumbers, POST_CANARY_BATCH_13_COMPATIBILITY_ORDER);
for (const packetRecord of preparation.artifacts.packets) {
  ...
  assert.equal(packet.batchNumber, 13);
  ...
  const positive = validatePostCanaryBatch13SiteLedgerAdapter({
