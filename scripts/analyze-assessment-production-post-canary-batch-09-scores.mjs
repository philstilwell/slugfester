...
import {
  POST_CANARY_BATCH_09_CALCULATED_SCORES_VERSION,
...
const manifest = JSON.parse(
  await readFile(
    path.resolve(`${POST_CANARY_BATCH_09_SCORE_ROOT}/score-pass-manifest.json`),
    "utf8"
  )
);
...
const validation = validatePostCanaryBatch09Scores(
  scores,
  ledger,
  inputs.debateInputs,
...
assertV4(
  scores.schemaVersion === POST_CANARY_BATCH_09_CALCULATED_SCORES_VERSION &&
  ...
  "post-canary Batch 9 score artifact identity mismatch"
);
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-09-score-analysis",
  protocolId: scores.protocolId,
  status: passed
    ? "post-canary-batch-09-score-stability-gate-passed"
    : "post-canary-batch-09-score-stability-gate-failed",
  ...
  nextAuthorizedAction: passed
    ...
    : "standing-authorization-permits-batch-09-score-stability-failure-diagnosis"
};
if (shouldWrite) {
  await writeFile(
