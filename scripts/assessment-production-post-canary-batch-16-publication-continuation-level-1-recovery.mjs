...
const recoveryRoot = `${publicationRoot}/continuation-failure-recovery`;
const root = `${recoveryRoot}/level-1`;
const protocolId = "assessment-production-post-canary-batch-16-publication-continuation-level-1-one-field-recovery";
...
if (mode === "--prepare") {
  ...
  assert.equal(diagnosis.status, "batch-16-publication-continuation-six-field-failures-diagnosed");
  assert.equal(diagnosis.totals.invalidFields, 6);
  assert.equal(continuationExecution.contextsAttempted, 2);
  ...
  const contexts = diagnosis.failures.map((failure, contextIndex) => {
    ...
    const repairPacket = { schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-continuation-one-field-repair-packet", protocolId, contextIndex, shardId, debateNumber: failure.debateNumber, debateId: source.packet.debateId, assessmentModel: model.label, recoveryLevel: 1, writableField: failure.field, target, allOtherFieldsUnavailableAndImmutable: true, participantJudgmentClosed: true, publicationScoreLocked: true };
    const properties = {
      schemaVersion: { type: "string", const: "1.0-assessment-production-post-canary-batch-16-publication-continuation-one-field-repair-output" },
...
if (mode === "--activate") {
  ...
  assert.equal(preparation.status, "frozen-six-one-field-batch-16-publication-continuation-recovery-level-1-prepared-not-activated");
  ...
  const activation = { ...preparation, schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-continuation-level-1-recovery-activation", status: "frozen-six-one-field-batch-16-publication-continuation-recovery-level-1-authorized", activatedAt: at, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(), preparation: { path: files.preparation, sha256: sha256(preparationBytes) }, authorization: { ...preparation.authorization, modelExecution: true, deterministicMergeAndValidation: true }, nextAuthorizedAction: "execute-six-fresh-one-field-batch-16-publication-continuation-recovery-level-1-contexts-once" };
  if (shouldWrite) await writeFile(path.resolve(files.activation), pretty(activation));
  console.log(pretty({ status: shouldWrite ? activation.status : "preview", contexts: 6, attemptsMaximum: 6, retriesMaximum: 0, recoveryLevel: 1, directIncrementalCostUsdMaximum: 0, modelExecutionAuthorized: shouldWrite }));
  process.exit(0);
}

if (mode === "--run") {
  const activation = await readJson(files.activation);
  assert.equal(activation.status, "frozen-six-one-field-batch-16-publication-continuation-recovery-level-1-authorized");
  ...
  for (let offset = 0; offset < activation.contexts.length; offset += 2) {
    const pair = activation.contexts.slice(offset, offset + 2); console.error(`[batch-16-publication-continuation-recovery-1] starting ${pair.map((item) => `${item.debateNumber}:${item.field}`).join(", ")}`);
    const pairResults = await Promise.all(pair.map(execute)); results.push(...pairResults); for (const result of pairResults) console.error(`[batch-16-publication-continuation-recovery-1] ${result.debateNumber}:${result.field} ${result.status} ${(result.elapsedMs / 60000).toFixed(2)}m`);
  }
  const passed = results.every((item) => item.gateAcceptancePassed);
  const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-continuation-level-1-recovery-execution", protocolId, status: passed ? "six-one-field-batch-16-publication-continuation-recovery-level-1-execution-passed" : "batch-16-publication-continuation-recovery-level-1-execution-failed", startedAt, completedAt: new Date().toISOString(), elapsedMs: Date.now() - started, contextsAttempted: 6, validContexts: results.filter((item) => item.gateAcceptancePassed).length, invalidContexts: results.filter((item) => !item.gateAcceptancePassed).length, attempts: 6, retries: 0, timeoutExtensions: 0, directIncrementalCostUsd: 0, paidServiceCalls: 0, modelAuthoredScores: 0, results, authorization: { analysis: true, retries: false, furtherRecoveryLevel: !passed, paidServices: false, scorePass: false, productionMutation: false, nextBatchSelection: false } };
  await writeFile(path.resolve(files.execution), pretty(execution));
  console.log(pretty({ status: execution.status, contextsAttempted: 6, validContexts: execution.validContexts, invalidContexts: execution.invalidContexts, elapsedMinutes: Number((execution.elapsedMs / 60000).toFixed(2)), attempts: 6, retries: 0, recoveryLevel: 1, directIncrementalCostUsd: 0 })); process.exit(0);
}
...
if (mode === "--analyze") {
  ...
  if (invalid.length) {
    const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-continuation-level-1-recovery-analysis", protocolId, status: "batch-16-publication-continuation-recovery-level-1-gate-failed", recoveryLevel: 1, recoveryLevelsMaximum: 2, originalFailuresPreserved: true, contexts: 6, attempts: 6, retries: 0, validContexts: execution.validContexts, invalidContexts: invalid.length, failedFields: invalid.map((item) => `${item.debateNumber}:${item.field}`), rejectedRawOutputsPreserved: invalid.every((item) => item.outputWritten), directIncrementalCostUsd: 0, authorization: { recoveryLevel2Preparation: true, scorePass: false, productionMutation: false, nextBatchSelection: false }, nextAuthorizedAction: "prepare-minimum-fresh-field-disjoint-batch-16-publication-continuation-recovery-level-2-shards" };
    if (shouldWrite) await writeFile(path.resolve(files.analysis), pretty(analysis)); console.log(pretty(analysis)); process.exit(0);
  }
  assert.equal(execution.status, "six-one-field-batch-16-publication-continuation-recovery-level-1-execution-passed");
  ...
  const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-continuation-level-1-recovery-analysis", protocolId, status: "batch-16-publication-continuation-level-1-one-field-recovery-passed", recoveryLevel: 1, recoveryLevelsMaximum: 2, originalFailuresPreserved: true, contexts: 6, attempts: 6, retries: 0, repairedValidationFields: 6, recoveredDebates: diagnosis.failedDebates, validationResults, directIncrementalCostUsd: 0, paidServiceCalls: 0, modelAuthoredScores: 0, authorization: { continueUnattemptedInitialPublicationContexts: true, furtherRecoveryLevel: false, scorePass: false, productionMutation: false, nextBatchSelection: false }, nextAuthorizedAction: "freeze-and-execute-the-seven-previously-unattempted-batch-16-publication-contexts" };
  if (shouldWrite) await writeFile(path.resolve(files.analysis), pretty(analysis)); console.log(pretty({ status: analysis.status, recoveredDebates: analysis.recoveredDebates, contexts: 6, attempts: 6, retries: 0, repairedValidationFields: 6, acceptedCompanionFieldsChanged: false, directIncrementalCostUsd: 0, nextAuthorizedAction: analysis.nextAuthorizedAction }));
}
