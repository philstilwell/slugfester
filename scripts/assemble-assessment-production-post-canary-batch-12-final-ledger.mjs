...
const finalLedgerPath =
  `${POST_CANARY_BATCH_12_FINAL_LEDGER_ROOT}/final-ledger.json`;
...
assertV4(
  ...
  "post-canary Batch 12 final-ledger manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen Batch 12 final-ledger source hash mismatch`
  );
}
const inputs = await loadPostCanaryBatch12FinalLedgerInputs();
const ledger = buildPostCanaryBatch12FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
