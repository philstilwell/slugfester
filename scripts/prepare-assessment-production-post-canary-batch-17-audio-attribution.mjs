...
import { MODEL, PACKET_VERSION, PROTOCOL_ID, ROOT, makeSchema, sha256 } from "./lib/assessment-production-post-canary-batch-17-audio-attribution.mjs";
...
assert.equal(standing.batchNumber, 17);
...
assert.equal(analysis.gate.verified, 0);
assert.equal(analysis.gate.unresolved, 1);
assert.equal(audit.totals.paidDiarizationCallsCompleted, 1);
assert.equal(audit.totals.retries, 0);
assert.equal(cost.costControl.approvedCapExceeded, false);
assert.equal(cost.costControl.usageDerivedEstimatedCostUsd, 0.0590775);
...
const authorization = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-17-audio-attribution-successor-authorization",
  protocolId: PROTOCOL_ID,
  status: "frozen-active-batch-17-audio-attribution-recovery-level-1-successor-authorization",
  ...
  userAuthorization: {
    ...
    scopeInterpretation: "Resume the blocked Batch 17 workflow using the minimum one fresh debate-isolated, zero-cost, evidence-limited context that can resolve the one preserved audio-attribution decision, then continue automatically through the already authorized complete workflow.",
  },
  preservedPriorGate: {
    verified: 0,
    unresolved: 1,
    paidCallsCompleted: 1,
    paidRetries: 0,
    usageDerivedEstimatedCostUsd: 0.0590775,
  ...
  sourceHashes,
  nextAuthorizedAction: "prepare-validate-commit-and-push-one-frozen-batch-17-audio-attribution-recovery-context",
};
const publicContexts = contexts.map(({ packetBytes, schemaBytes, ...context }) => context);
const preparation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-17-audio-attribution-preparation",
  protocolId: PROTOCOL_ID,
  status: "prepared-one-batch-17-audio-attribution-recovery-context-not-active",
  preparedAt,
  checkpointCommit: head,
  productionCanary: false,
  batchNumber: 17,
