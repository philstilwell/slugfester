...
const PROTOCOL_ID = "assessment-production-post-canary-batch-10-discovery";
const WORKFLOW = "docs/assessment-production-canary-discovery-workflow.md";
const SCRIPT = "scripts/preregister-assessment-production-post-canary-batch-10-discovery.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-10-discovery-manifest.mjs";
...
assertV4(
  preparation.status === "post-canary-batch-10-ten-complete-score-blind-source-packets-prepared-awaiting-validation" &&
    preparationValidation.status === "post-canary-batch-10-score-blind-source-packet-validation-passed-frozen-under-standing-authorization" &&
    ...
    preparation.totals.maximumCopiedInputBytes === 82238 &&
  ...
  "validated Batch 10 source preparation is not frozen at the required authorization boundary"
);
assertV4(
  ...
  "Batch 10 standing authorization does not cover exact discovery execution"
);
assertV4(
  JSON.stringify(preparation.contexts.map((debate) => debate.debateNumber)) === JSON.stringify(REQUIRED_ORDER),
  "Batch 10 debate order drifted"
);
assertV4(
  ...
  for (const chunk of debate.chunks) {
    ...
    assertV4(
      sha256(chunkBytes) === chunk.chunkLedgerSha256 &&
        ...
        buildBatch10TokenCountedChunkLedger(chunkBytes).equals(tokenBytes) &&
...
assertV4(contexts.length === 36, "Batch 10 discovery must contain exactly 36 contexts");
...
assertV4(maximumContext.debateNumber === "130" && maximumContext.chunkId === "chunk-001", "maximum copied-input context drifted");
...
const manifest = {
  ...
  status: shouldWrite ? "frozen-thirty-six-post-canary-batch-10-discovery-contexts-prepared-not-authorized" : "preview",
  ...
  AIOnly: true,
  userAuthorization: {
    scope: "standing-authorized Batch 10 discovery preparation and execution",
  ...
  costEstimate: {
    ...
    estimateBasis: "The exact 36-context Batch 10 input set is hash-locked with an observed maximum of 82,238 copied bytes; no input is truncated or repartitioned. The frozen 1→2→4 scheduler uses the subscription-backed route with a $0 direct incremental cost boundary.",
  },
  ...
  nextAuthorizedAction: "prepare-and-freeze-batch-10-discovery-execution-activation-under-standing-authorization",
};
