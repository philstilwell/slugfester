...
import { POST_CANARY_BATCH_10_PUBLICATION_TIMEOUT_RECOVERY_ROOT as ROOT, validatePublicationTimeoutRecoveryShardOutput } from "./lib/assessment-production-post-canary-batch-10-publication-resumption-timeout-recovery.mjs";
...
assertV4(activation.status === "frozen-two-context-batch-10-debate-21-publication-timeout-recovery-activated" && activation.contexts?.length === 2 && activation.authorization?.modelExecution === true && activation.model?.slug === "gpt-5.6-sol" && activation.model?.reasoningEffort === "low" && activation.model?.authentication === "ChatGPT subscription", "recovery execution is not activated");
...
function invoke(codexArgs, options, timeoutMs) {
  return new Promise((resolve) => {
    const child = spawn(caffeinate, [...activation.executionEnvironment.hostAwakeGuard.args, codex, ...codexArgs], { ...options, detached: true, stdio: ["ignore", "pipe", "pipe"] });
    let stdout = ""; let stderr = ""; let timedOut = false; let forceTimer;
    child.stdout.on("data", (chunk) => { stdout += chunk; });
    child.stderr.on("data", (chunk) => { stderr += chunk; });
    const terminateGroup = (signal) => { try { process.kill(-child.pid, signal); } catch { child.kill(signal); } };
    const timer = setTimeout(() => { timedOut = true; terminateGroup("SIGTERM"); forceTimer = setTimeout(() => terminateGroup("SIGKILL"), 5000); }, timeoutMs);
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-10-publication-timeout-recovery-${context.contextIndex}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `batch-10-publication-timeout-recovery-home-${context.contextIndex}-`));
  ...
  try {
    ...
    process.stdout.write(`[batch-10-publication-timeout-recovery] starting ${context.contextIndex} Debate ${context.debateNumber} ${context.side}\n`);
  ...
  process.stdout.write(`[batch-10-publication-timeout-recovery] ${context.contextIndex} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
...
if (validContexts !== 2) process.exitCode = 2;
