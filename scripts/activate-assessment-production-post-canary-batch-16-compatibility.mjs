...
import {
  ...
  buildPostCanaryBatch16ValidatorSource,
  sha256,
  validatePostCanaryBatch16ValidatorSource
} from "./lib/assessment-production-post-canary-batch-16-compatibility-execution.mjs";
...
const paths = {
  ...
  preparationAnalysis: `${POST_CANARY_BATCH_16_COMPATIBILITY_ROOT}/analysis.json`,
  ...
  executionLibrary:
    "scripts/lib/assessment-production-post-canary-batch-16-compatibility-execution.mjs",
  activationScript:
    "scripts/activate-assessment-production-post-canary-batch-16-compatibility.mjs",
  activationTest:
    "scripts/test-assessment-production-post-canary-batch-16-compatibility-activation.mjs",
  executionScript:
    "scripts/run-assessment-production-post-canary-batch-16-compatibility.mjs",
  executionTest:
    "scripts/test-assessment-production-post-canary-batch-16-compatibility-execution.mjs"
};
...
assertV4(
  preparation.status ===
      "post-canary-batch-16-compatibility-plan-prepared-and-frozen" &&
  ...
  "frozen Batch 16 compatibility preparation is unavailable"
);
assertV4(
  preparationAnalysis.status ===
      "post-canary-batch-16-compatibility-plan-freeze-passed" &&
    preparationAnalysis.preparation.sha256 === sha256(preparationBytes) &&
    preparationAnalysis.finding.executed === false,
  "frozen Batch 16 compatibility preparation analysis is unavailable"
);
...
assertV4(
  ...
  "a stable --activated-at ISO timestamp is required for the first Batch 16 compatibility activation write"
);
assertV4(
  !(await exists(paths.execution)) && !(await exists(paths.stagedLedgerRoot)),
  "Batch 16 compatibility execution already exists; activation blocked"
);
...
assertV4(
  canonicalJson(packetHashes.map((item) => item.debateNumber)) ===
    canonicalJson(POST_CANARY_BATCH_16_COMPATIBILITY_ORDER),
  "Batch 16 compatibility activation packet order changed"
);
...
assertV4(
  ...
  "active validator differs from the frozen Batch 16 baseline"
);
const proposedValidator = buildPostCanaryBatch16ValidatorSource(
  baselineValidator
);
const proposedValidatorValidation =
  validatePostCanaryBatch16ValidatorSource(proposedValidator);
...
const activation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-16-compatibility-execution-activation",
  protocolId: preparation.protocolId,
  status: POST_CANARY_BATCH_16_COMPATIBILITY_ACTIVATION_STATUS,
  activatedAt,
  ...
  userAuthorization: {
    instruction:
      "The Batch 16 standing authorization covers activation and exactly one deterministic compatibility staging pass.",
    scopeInterpretation:
      "Activate the frozen Batch 16 compatibility manifest and execute exactly one validator-and-staged-ledger compatibility pass with no reruns, models, paid services, production ledger publication, production data mutation, or Batch 17 selection."
  },
  preparation: {
  ...
  validator: {
    ...
    routeCredentialStatus: POST_CANARY_BATCH_16_COMPATIBILITY_ACTIVATION_STATUS
  },
  packetHashes,
  executionToolHashes,
  protectedProduction: {
  ...
  requiredRegressions: [
    "Batch 16 route positive and negative controls",
  ...
  nextAuthorizedAction:
    "execute-one-frozen-batch-16-production-compatibility-staging-pass-no-reruns"
};
const activationBytes = serializedJson(activation);

if (write) {
  assertV4(!existingActivation, "Batch 16 compatibility is already activated");
  await writeFile(resolve(paths.activation), activationBytes);
} else {
  assertV4(
    existingActivation &&
      canonicalJson(existingActivation) === canonicalJson(activation),
    "stored Batch 16 compatibility activation differs from deterministic replay"
  );
}
