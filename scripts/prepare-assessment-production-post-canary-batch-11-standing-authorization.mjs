...
import {
  ...
  POST_CANARY_BATCH_11_STANDING_AUTHORIZATION_INSTRUCTION,
  POST_CANARY_BATCH_11_STANDING_AUTHORIZATION_INSTRUCTION_PATH,
  POST_CANARY_BATCH_11_STANDING_AUTHORIZATION_PROTOCOL_ID,
  POST_CANARY_BATCH_11_STANDING_AUTHORIZATION_STATUS,
  validatePostCanaryBatch11StandingAuthorization
...
const SELECTION_FREEZE_COMMIT = "9f4724173d772deea42609bfb767644872b0916f";
const SOURCE_PACKET_COMMIT = "d446c055aec2a0e4170209501d63ddcc20683f3f";
const SOURCE_FILES = [
  ...
  "scripts/prepare-assessment-production-post-canary-batch-11-standing-authorization.mjs",
  "scripts/test-assessment-production-post-canary-batch-11-standing-authorization.mjs"
];
const REQUIRED_ORDER = ["54", "01", "82", "191", "151", "188", "60", "79", "43", "24"];
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const exists = (file) => access(path.resolve(file)).then(() => true, () => false);

assertV4(
  !(await exists(POST_CANARY_BATCH_11_STANDING_AUTHORIZATION)),
  `${POST_CANARY_BATCH_11_STANDING_AUTHORIZATION} already exists`
);
...
assertV4(
  selection.status ===
    ...
    selection.batchNumber === 11 &&
    selection.checkpointCommit ===
      "2a38b1d70b49d51d3356b57a5d25e1b676bbdc5b" &&
    ...
    analysis.status ===
      "eleventh-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization" &&
  ...
  "the frozen Batch 11 selection or source-packet boundary changed"
);
assertV4(
  execFileSync("git", ["show", `${SELECTION_FREEZE_COMMIT}:${SELECTION}`]).equals(selectionBytes),
  "Batch 11 selection differs from the user-authorized selection commit"
);
...
assertV4(
  ...
  "Batch 11 standing authorization must freeze at the pushed source-packet commit"
);
...
const record = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-11-standing-authorization",
  protocolId: POST_CANARY_BATCH_11_STANDING_AUTHORIZATION_PROTOCOL_ID,
  status: POST_CANARY_BATCH_11_STANDING_AUTHORIZATION_STATUS,
  authorizedAt,
  ...
  userAuthorization: {
    instruction: POST_CANARY_BATCH_11_STANDING_AUTHORIZATION_INSTRUCTION,
    resolvedAntecedent: "The attached Batch 11 request authorizes deterministic selection and the complete reassessment, recovery, publication, validation, commit, and push workflow.",
    scopeInterpretation: "Create and use a complete-workflow Batch 11 standing authorization for the ten frozen selections, beginning with the separately frozen score-blind source packets, including bounded diagnosed recovery and production publication, while preserving the established campaign controls and stopping before Batch 12 selection.",
  ...
  stopRules: {
    ...
    outsideBatchElevenDebateBlocks: true,
    ...
    batchTwelveSelectionBlocks: true,
  ...
  nextAuthorizedAction:
    "prepare-validate-freeze-commit-and-push-batch-11-discovery-execution-manifest"
};
validatePostCanaryBatch11StandingAuthorization(record);
if (shouldWrite) {
  await writeFile(
    path.resolve(POST_CANARY_BATCH_11_STANDING_AUTHORIZATION),
