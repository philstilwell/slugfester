...
const resumptionRoot = `${root}/original-unattempted-context-resumption-2`;
const outputPath = `${root}/outputs/debate-70.json`;
const packetPath = `${root}/packets/debate-70.json`;
const executionPath = `${resumptionRoot}/model-execution.json`;
const validationPath = `${root}/validations/debate-70.json`;
const analysisPath = `${resumptionRoot}/debate-70-failure-analysis.json`;
...
const failed = execution.results.find((item) => item.debateNumber === "70");
assertV4(execution.status === "six-context-publication-resumption-stopped-with-failure" && execution.contextsAttempted === 1 && execution.validContexts === 0 && execution.contextsUnattempted === 5 && failed?.status === "output-validation-failed" && failed.outputSha256 === sha256(outputBytes) && failed.retryCount === 0, "Debate 70 failure boundary changed");
assertV4(validationRecord.status === "failed" && validationRecord.outputSha256 === sha256(outputBytes), "Debate 70 failed validation record changed");
...
assertV4(structuralStandIn, "Debate 70 has no validation-clean critique stand-in");
...
assertV4(nonTargetValidation.status === "passed", "Debate 70 non-target fields do not validate");
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-13-debate-70-publication-failure-analysis",
  protocolId: "assessment-production-post-canary-batch-13-debate-70-publication-field-disjoint-repair-1",
  status: "debate-70-publication-failure-diagnosed-awaiting-field-disjoint-repair-1",
  debateNumber: "70",
  ...
  nextAuthorizedAction: "prepare-minimum-field-disjoint-debate-70-publication-repair-shards"
};
