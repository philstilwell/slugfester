...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-04";
...
const PRIOR_SELECTION = "docs/assessment-production/post-canary-continuation-v1/batch-03/selection.json";
const PRIOR_SELECTION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-03/selection-analysis.json";
const PRIOR_MUTATION_MANIFEST = "docs/assessment-production/post-canary-continuation-v1/batch-03/production-publication/mutation-manifest.json";
const PRIOR_COMPLETION_EXECUTION = "docs/assessment-production/post-canary-continuation-v1/batch-03/production-publication/generated-seo-correction/execution.json";
const PRIOR_COMPLETION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-03/production-publication/generated-seo-correction/analysis.json";
const PRIOR_COMPLETION_COMMIT = "1a23cf5749c991903265fcbbc79d015ae6ee0337";
const SCRIPT = "scripts/select-assessment-production-post-canary-batch-04.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-04-selection.mjs";
...
assertV4(!(await exists(FUTURE_SOURCE_PREPARATION)), "source preparation already exists before Batch 4 selection");
...
assertV4(
  ...
  "Batch 4 selection is not compatible with the frozen continuation policy"
);
...
assertV4(
  priorSelection.status === "third-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    priorSelection.batchNumber === 3 &&
    priorSelection.selected.length === 10 &&
    priorSelectionAnalysis.status === "third-post-canary-batch-selection-analysis-passed-awaiting-source-packet-preparation-decision" &&
    priorSelectionAnalysis.selection.sha256 === sha256(controlBytes[PRIOR_SELECTION]) &&
    priorMutationManifest.status === "frozen-batch-03-production-publication-mutation-manifest-prepared" &&
    JSON.stringify(priorMutationManifest.explicitOrder) === JSON.stringify(priorSelectedNumbers) &&
    priorCompletionExecution.status === "passed-complete-batch-03-transaction-ready-for-atomic-commit" &&
    ...
    priorCompletionAnalysis.status === "batch-03-production-publication-accepted-for-atomic-commit-after-generated-seo-correction" &&
    priorCompletionAnalysis.execution.sha256 === sha256(controlBytes[PRIOR_COMPLETION_EXECUTION]) &&
    priorCompletionAnalysis.decision.batch03ProductionPublicationGatePassed === true &&
    priorCompletionAnalysis.decision.completeTransactionAccepted === true &&
    priorCompletionAnalysis.result.repositoryValidationExitCode === 0 &&
    priorCompletionAnalysis.result.fullRepositoryRegressionPassed === true,
  "Batch 3 has not satisfied the frozen successful-publication prerequisite"
);
assertV4(git("rev-parse", "--abbrev-ref", "HEAD") === "main", "Batch 4 selection must run on main");
assertV4(git("rev-parse", "HEAD") === PRIOR_COMPLETION_COMMIT, "Batch 4 selection checkpoint is not the exact Batch 3 publication commit");
assertV4(git("rev-parse", "origin/main") === PRIOR_COMPLETION_COMMIT, "Batch 3 publication commit is not the pushed origin/main checkpoint");
...
assertV4(
  JSON.stringify(priorSelectedNumbers) === JSON.stringify(ranked.slice(20, 30).map((item) => item.debateNumber)),
  "Batch 3 is not the immutable third rank slice"
);

const selected = [];
let totalEvents = 0;
let totalDurationMs = 0;
for (const [offset, rankedItem] of ranked.slice(30, 40).entries()) {
...
assertV4(selected.length === 10 && new Set(selected.map((item) => item.debateNumber)).size === 10, "Batch 4 did not select ten unique debates");

const checkpointCommit = git("rev-parse", "HEAD");
const selection = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-selection",
  protocolId: "assessment-production-post-canary-batch-04",
  status: "fourth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed",
  frozenAt,
  checkpointCommit,
  branch: "main",
  ...
  stagingOnly: true,
  batchNumber: 4,
  productionSequenceOrdinal: 5,
  userAuthorization: {
    instruction: "Continue.",
    scopeInterpretation: "Materialize and source-validate only Batch 4 under the frozen continuation policy after authenticating successful Batch 3 publication. Do not prepare source or model packets, execute models, derive scores, reconstruct publication prose, mutate production, or select Batch 5.",
  ...
  eligibility: {
    ...
    previouslyPublishedAndRemovedCount: 30,
    remainingBeforeBatchCount: ranked.length - 30,
    remainingBeforeBatchRankedSha256: sha256(serializedJson(ranked.slice(30))),
    selectedCount: selected.length,
    remainingUnselectedCount: ranked.length - 40,
    dyadicOnly: true,
  ...
  nextAuthorizedAction: "user-decision-on-fourth-post-canary-batch-source-packet-preparation"
};
...
const analysis = {
  ...
  status: "fourth-post-canary-batch-selection-analysis-passed-awaiting-source-packet-preparation-decision",
  analyzedAt: frozenAt,
  ...
  finding: "Successful Batch 3 production publication was authenticated at the pushed atomic commit. The immutable campaign rank then selected ordinals 31 through 40 as ten unique pending dyadic debates, and all thirty effective source files and ten canonical event chains passed without replacement.",
  decision: {
