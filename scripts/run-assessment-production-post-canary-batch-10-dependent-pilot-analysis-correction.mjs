...
const analysisPath = `${BATCH_10_DEPENDENT_PILOT_CORRECTION_ROOT}/analysis.json`;
const startedAt = new Date().toISOString();
const [plan, activation] = await Promise.all([readJson(planPath), readJson(activationPath)]);
assertV4(
  plan.protocolId === BATCH_10_DEPENDENT_PILOT_CORRECTION_PROTOCOL_ID &&
    activation.status === "frozen-batch-10-dependent-pilot-analysis-correction-activated" &&
  ...
  "frozen one-shot Batch 10 dependent pilot correction activation required"
);
...
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-10-dependent-pilot-analysis-correction-execution",
  protocolId: BATCH_10_DEPENDENT_PILOT_CORRECTION_PROTOCOL_ID,
  status: "passed-complete-batch-10-production-transaction-after-dependent-pilot-analysis-correction",
  startedAt,
  completedAt,
  ...
  nextAuthorizedAction: "atomic-commit-and-push-complete-validated-batch-10-production-transaction-then-stop-before-batch-11-selection"
};
await writeFile(resolve(executionPath), serializedJson(execution));
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-10-dependent-pilot-analysis-correction-analysis",
  protocolId: BATCH_10_DEPENDENT_PILOT_CORRECTION_PROTOCOL_ID,
  status: "batch-10-production-publication-accepted-after-dependent-pilot-analysis-correction",
