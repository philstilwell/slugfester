...
const manifestPath = `${V42211734_ROOT}/execution-manifest.json`;
const executionPath = `${V42211734_ROOT}/model-execution.json`;
const analysisPath = `${V42211734_ROOT}/analysis.json`;
...
const preparation = JSON.parse(await readFile(path.resolve(`${V42211734_ROOT}/preparation-manifest.json`), "utf8"));
assertV4(preparation.status === "prepared-five-isolated-hard-route-publication-prompt-contexts" && preparation.authorization.executionManifest && preparation.totals.modelContextsExecuted === 0 && preparation.totals.modelAuthoredScores === 0, "publication prompt execution manifest is not authorized");
assertV4(preparation.totals.maximumCopiedInputBytes <= 400000, "publication context exceeds frozen transport budget");
assertV4(preparation.repair.counterfactualOnlySummaryReplacementPassed && preparation.repair.structuredOutputRepairRetained && preparation.repair.repositoryUniquenessValidationRetained && !preparation.repair.priorGateTreatedAsRetry, "publication prompt repair boundary mismatch");
...
const sourceFiles = [
  ...
  `${V42211734_ROOT}/preparation-manifest.json`,
  ...
  "scripts/prepare-v42211734-hard-route-publication-prompt.mjs",
  "scripts/preregister-v42211734-hard-route-publication-prompt.mjs",
  "scripts/run-v42211734-hard-route-publication-prompt.mjs",
  "scripts/analyze-v42211734-hard-route-publication-prompt.mjs",
  "scripts/test-v42211734-hard-route-publication-prompt-gate.mjs"
];
...
const manifest = {
  schemaVersion: "4.2.21.17.34-hard-route-publication-prompt-execution-manifest",
  ...
  costEstimate: { authentication: "ChatGPT subscription", meteredApiCostUsdMaximum: 0, transcriptionCostUsdMaximum: 0, expectedWallMinutes: [12, 22], expectedAggregateModelMinutes: [20, 32], absoluteGateTimeoutMinutes: 60 },
  ...
  deterministicValidation: { modelSummaryTargetWords: preparation.repair.modelSummaryTargetWords, repositorySummaryAcceptanceWords: preparation.repair.repositorySummaryAcceptanceWords, openAIStructuredOutputSubsetPreflight: true, repositoryUniquenessValidationRetained: true, everyLockedMoveAuthoredExactlyOnce: true, exactQuoteSubstringRequired: true, critiqueWordAndSentenceContract: true, localReferenceCatalogOnly: true, overallCommentaryMinimums: true, aiExtensionNoveltyMapComplete: true, introducedArgumentPerSideRequired: true, exactAccordionDisplayContract: true, prohibitedLanguageAbsent: true, modelAuthoredScores: 0 },
  priorGate: { protocolId: "v4.2.21.17.33-hard-route-publication-transport-repair", status: "failed-summary-contract", treatedAsRetry: false },
