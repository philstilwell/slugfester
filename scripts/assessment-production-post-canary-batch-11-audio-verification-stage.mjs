...
const expectedDebates = ["01", "188"];
const expectedMoves = [
  "con-suffering-christian-specificity",
  "con-nonspatiotemporal-meditative-experience"
];
const expectedCalls = 2;
const durationOnlyPlanningEstimateUsd = 0.01888;
const usageDerivedPlanningEstimateUsd = 0.068935;
const toolPath = "scripts/assessment-production-post-canary-batch-11-audio-verification-stage.mjs";
...
const standingAuthorization = await loadAndValidatePostCanaryBatch11StandingAuthorization();
...
async function activate() {
  ...
  const activation = {
    ...preparation,
    schemaVersion: "1.0-assessment-production-post-canary-batch-11-audio-verification-execution-manifest",
    status: "frozen-two-post-canary-batch-11-paid-known-speaker-diarizations-authorized-under-standing-authorization",
    activatedAt,
    ...
    nextAuthorizedAction: "execute-exactly-two-post-canary-batch-11-paid-audio-verification-calls-once-sequentially"
  };
...
async function validateActivation(manifest) {
  assert.equal(
    manifest.status,
    "frozen-two-post-canary-batch-11-paid-known-speaker-diarizations-authorized-under-standing-authorization"
  );
...
async function run() {
  ...
  for (const [callIndex, call] of manifest.calls.entries()) {
    ...
    process.stdout.write(`[batch-11-audio] ${callIndex + 1}/${expectedCalls} starting ${call.debateNumber}:${call.moveId}\n`);
    ...
    results.push(result);
    process.stdout.write(`[batch-11-audio] ${callIndex + 1}/${expectedCalls} ${result.status}; cumulative $${result.cumulativeUsageDerivedEstimatedCostUsd.toFixed(7)}\n`);
  }
  ...
  const execution = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-11-audio-verification-model-execution",
    protocolId: manifest.protocolId,
    status: completed.length === expectedCalls
      ? "two-post-canary-batch-11-paid-known-speaker-diarizations-completed"
      : "post-canary-batch-11-paid-diarization-incomplete",
    provider: "OpenAI Transcription API",
...
async function analyze() {
  ...
  const status = passed
    ...
    : executionComplete
      ? "post-canary-batch-11-audio-verification-unresolved"
      : "post-canary-batch-11-audio-verification-incomplete";
  const nextAuthorizedAction = execution.costCapReachedOrExceeded || !execution.directIncrementalCostCapControlPassed
    ? "user-review-required-before-any-batch-11-downstream-work-after-audio-cost-cap-event"
    : passed
      ? "prepare-freeze-and-push-batch-11-dispute-only-adjudication-packets-under-standing-authorization"
      : "standing-authorization-stop-new-approval-required-before-batch-11-audio-verification-failure-diagnosis";
  const authorization = {
  ...
  const audit = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-11-audio-verification-audit",
    protocolId: manifest.protocolId,
    status,
  ...
  const analysis = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-11-audio-verification-analysis",
    protocolId: manifest.protocolId,
    status,
    productionCanary: false,
    batchNumber: 11,
...
async function cost() {
  ...
  const result = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-11-audio-cost-control-analysis",
