...
import { V223_DISPUTE_ADJ_ROOT } from "./lib/assessment-production-score-stability-v2.2.3-dispute-adjudication.mjs";
...
const manifestPath = `${V223_DISPUTE_ADJ_ROOT}/execution-manifest.json`;
const executionPath = `${V223_DISPUTE_ADJ_ROOT}/model-execution.json`;
const analysisPath = `${V223_DISPUTE_ADJ_ROOT}/analysis.json`;
const exists = (file) =>
  access(file).then(
    () => true,
    () => false
...
assertV4(
  preparation.status ===
    ...
    preparation.totals.disputedMoves === 185 &&
    preparation.totals.candidateSelections === 490 &&
    preparation.totals.audioVerifiedMoves === 4,
  "v2.2.3 adjudication preparation does not authorize an execution manifest"
);
assertV4(
  ...
  "v2.2.3 adjudication model/authentication boundary changed"
);
assertV4(
  preparation.totals.maximumCopiedInputBytes <= 350000,
  "v2.2.3 adjudication context exceeds the frozen transport budget"
);
assertV4(
...
const sourceFiles = [
  ...
  "scripts/build-assessment-production-score-stability-v2.2.3-dispute-adjudication-packets.mjs",
  "scripts/test-assessment-production-score-stability-v2.2.3-dispute-adjudication-packets.mjs",
  "scripts/validate-assessment-production-score-stability-v2.2.3-dispute-adjudication-output.mjs",
  "scripts/preregister-assessment-production-score-stability-v2.2.3-dispute-adjudication.mjs",
  "scripts/run-assessment-production-score-stability-v2.2.3-dispute-adjudication.mjs",
  "scripts/analyze-assessment-production-score-stability-v2.2.3-dispute-adjudication.mjs",
  "scripts/test-assessment-production-score-stability-v2.2.3-dispute-adjudication-gate.mjs",
...
const manifest = {
  schemaVersion:
    "1.0-score-stability-v2.2.3-dispute-only-adjudication-execution-manifest",
  ...
  nextAuthorizedAction:
    "execute-ramped-ten-v2.2.3-dispute-only-adjudication-contexts-once"
};
if (shouldWrite) {
