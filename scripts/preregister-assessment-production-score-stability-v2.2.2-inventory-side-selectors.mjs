...
const SCRIPT =
  "scripts/preregister-assessment-production-score-stability-v2.2.2-inventory-side-selectors.mjs";
const TEST =
  "scripts/test-assessment-production-score-stability-v2.2.2-inventory-side-selector-manifest.mjs";
...
assertV4(
  sidePreparation.status ===
    ...
    sidePreparation.totals?.candidatesTransported === 361 &&
    sidePreparation.totals?.modelContextsExecuted === 0 &&
    sidePreparation.failedGateDisposition?.v221PlanningGatePreservedFailed ===
      true &&
    sidePreparation.failedGateDisposition
      ?.v221ValidPartialPlansReusableForSuccessorAcceptance === false &&
    sidePreparation.failedGateDisposition?.v221Debate75Retried === false &&
    sidePreparation.failedGateDisposition?.v221TimeoutExtended === false &&
    sidePreparation.inventorySuccessorContract?.planAndSideIsolationPreserved ===
      true &&
    sidePreparation.inventorySuccessorContract
      ?.fallbackAppliedOnlyToRetainedOrphanReply === true &&
    sidePreparation.inventorySuccessorContract?.scoreFieldsAvailable === false &&
    sidePreparation.proposedPolicy?.promoted === false &&
    sidePreparation.authorization
      ?.sideSelectorExecutionManifestPreparation === true &&
    sidePreparation.authorization?.sideSelectorExecutionActivation === false &&
...
const futureOutputs =
  sidePreparation.futureOutputPathsExcludedFromSourceHashes.filter(
    (file) => file !== MANIFEST
  );
assertV4(
...
const manifest = {
  schemaVersion:
    "1.0-score-stability-v2.2.2-side-selector-execution-preparation-manifest",
  ...
  costEstimate: {
    ...
    estimateBasis:
      "The earlier twenty-context side-selector gate completed in 10.80 wall minutes and 19.74 aggregate model minutes at concurrency two. These exact contexts are 42-65 KB and retain the same side-selection contract. Completion is not guaranteed. ChatGPT-subscription execution has no direct incremental API charge.",
  },
  ...
  nextAuthorizedAction:
    "prepare-separate-v2.2.2-side-selector-execution-activation-only",
};
