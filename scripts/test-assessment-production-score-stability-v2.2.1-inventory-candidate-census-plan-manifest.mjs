...
assert.equal(
  manifest.failedGateDisposition.sourceV22DiscoveryGatePreservedFailed,
  true
);
assert.equal(manifest.failedGateDisposition.sourceV22DiscoveryGateRetried, false);
assert.equal(
  manifest.failedGateDisposition.sourceV22DiscoveryExecutionReclassified,
  false
);
assert.equal(
  manifest.failedGateDisposition.v221MechanicalRecoveryChangedCandidateFields,
  false
);
assert.equal(
  manifest.failedGateDisposition.predecessorV213ScoreGatePreservedFailed,
  true
);
assert.equal(
...
assert.deepEqual(manifest.executionPolicy, {
  ...
  observedMaximumCopiedInputBytes: 57707,
...
for (const [index, context] of manifest.contexts.entries()) {
  ...
  assert.equal(packet.isolation.v221MechanicalRecoveryMetadataUnavailable, true);
