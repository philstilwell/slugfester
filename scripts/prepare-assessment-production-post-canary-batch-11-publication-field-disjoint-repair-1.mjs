...
} from "./lib/assessment-production-post-canary-batch-11-publication-field-disjoint-repair-1.mjs";
...
const ALL_DEBATES = ["54", "01", "82", "191", "151", "188", "60", "79", "43", "24"];
...
assertV4(
  originalExecution.status === "post-canary-batch-11-publication-gate-complete-with-failure" &&
    originalExecution.contextsAttempted === 10 && originalExecution.validContexts === 6 &&
    originalExecution.invalidContexts === 4 && originalExecution.attempts === 10 &&
    ...
    originalAnalysis.status === "post-canary-batch-11-publication-output-gate-failed",
  "original Batch 11 publication failure boundary changed"
);
...
const debate54 = allArtifacts.get("54");
const debate54Validation = validatePostCanaryBatch11PublicationOutput(
  debate54.outputJson, debate54.packetJson
);
assertV4(debate54Validation.status === "passed" && debate54Validation.moves === 20,
  "accepted Debate 54 output no longer validates");
const structuralStandIn = debate54.outputJson.moveProse[
  debate54.packetJson.moves[0].moveId
].critique;

const expectedCritiqueCounts = { "191": 1, "151": 1, "60": 19, "43": 11 };
const diagnostics = [];
...
for (const plan of DEBATE_PLANS) {
  ...
  const nonTargetAudit = validatePostCanaryBatch11PublicationOutput(
...
assertV4(diagnostics.reduce((sum, item) => sum + item.invalidCritiques.length, 0) === 32 &&
  diagnostics.reduce((sum, item) => sum + item.invalidQuotes.length, 0) === 0,
"diagnosed 32-field repair scope changed");

const manualText = `# Batch 11 field-disjoint publication repair\n\n` +
...
for (const plan of DEBATE_PLANS) {
  ...
  for (let shardIndex = 0; shardIndex < plan.critiqueShardSizes.length; shardIndex += 1) {
    ...
    const packet = {
      schemaVersion: "1.0-assessment-production-post-canary-batch-11-publication-field-disjoint-repair-packet",
...
assertV4(contexts.length === 18 &&
  contexts.every((context) => context.writableFieldCount >= 1 && context.writableFieldCount <= 2) &&
  contexts.reduce((sum, context) => sum + context.writableFieldCount, 0) === 32 &&
  new Set(contexts.flatMap((context) => context.writableFields)).size === 32,
"eighteen-shard, two-field-maximum coverage changed");

const sourceScripts = [
  "scripts/lib/assessment-production-post-canary-batch-11-publication-field-disjoint-repair-1.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-11-publication-field-disjoint-repair-1.mjs",
  "scripts/activate-assessment-production-post-canary-batch-11-publication-field-disjoint-repair-1.mjs",
  "scripts/run-assessment-production-post-canary-batch-11-publication-field-disjoint-repair-1.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-11-publication-field-disjoint-repair-1.mjs",
...
const manifest = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-11-publication-field-disjoint-repair-preparation",
  protocolId: PROTOCOL_ID,
  status: "frozen-eighteen-context-thirty-two-field-batch-11-publication-repair-prepared-not-activated",
  frozenAt,
  ...
  stagingOnly: true,
  userAuthorization: {
    source: "Batch 11 complete-workflow standing authorization",
    resolvedScope: "eighteen fresh debate-isolated field-disjoint shards correcting exactly 32 rejected critiques while retaining all six accepted debates and every validation-clean field",
  ...
  diagnosis: {
    status: "deterministically-confirmed-thirty-two-critique-failures",
    debates: diagnostics,
    totals: { debates: 4, invalidCritiques: 32, invalidQuotes: 0, writableFields: 32 },
    everyNonTargetFieldStructurallyValidated: true,
    temporaryStandInsWrittenOrRetained: false
  },
  retainedAcceptedDebate54: {
    output: debate54.output,
    outputSha256: sha256(debate54.outputBytes),
    validation: debate54Validation
  },
  modelInputs: {
  ...
  retentionContract: {
    acceptedOriginalDebatesRetained: 6,
    ...
    completeTenDebateCohortReplayRequiredAfterAllFourPass: true
  },
  ...
  totals: {
    debates: 4, contexts: 18, critiqueFields: 32, quoteFields: 0, writableFields: 32,
  ...
  nextAuthorizedAction: "commit-and-push-frozen-eighteen-shard-repair-then-activate"
};
