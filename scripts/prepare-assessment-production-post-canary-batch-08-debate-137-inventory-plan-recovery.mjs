...
const ORIGINAL_PACKET = `${ROOT}/packets/plans/debate-137.json`;
...
function shardSchema(fullSchema, field) {
  return {
    ...
    $id: `${fullSchema.$id}-debate-137-recovery-1-${field}`,
    title: `Slugfester Batch 8 Debate 137 ${field} recovery shard`,
    type: "object",
...
function mergeRule() {
  return {
    ruleId: "batch-08-debate-137-plan-recovery-1-field-disjoint-merge-v1",
...
async function buildArtifacts({ frozenAt, checkpointCommit }) {
  ...
  assertV4(
    ...
    "preserved Debate 137 planner timeout boundary drifted"
  );
  assertV4(
    standingAuthorization.record.recoveryControls.boundedFirstRecoveryAuthorized ===
      ...
      standingAuthorization.record.stopRules.thirdFailureOfSameUnderlyingProblemBlocks ===
        true,
    "Batch 8 first-recovery authority drifted"
  );
  ...
  for (const field of SHARDS) {
    const schemaPath = `${RECOVERY}/schemas/debate-137-${field}.schema.json`;
  ...
  const packets = {};
  for (const field of SHARDS) {
    const packetPath = `${RECOVERY}/packets/debate-137-${field}.json`;
    const output = `${RECOVERY}/outputs/debate-137-${field}.json`;
    ...
    const packet = {
      schemaVersion:
        "1.0-assessment-production-post-canary-batch-08-debate-137-inventory-plan-recovery-shard-packet",
      protocolId: PROTOCOL_ID,
      recoveryId: "debate-137-plan-recovery-1",
  ...
  const diagnosis = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-08-debate-137-inventory-plan-timeout-diagnosis",
    protocolId: PROTOCOL_ID,
    status:
      "preserved-debate-137-candidate-census-plan-timeout-diagnosed-no-output-accepted",
    frozenAt,
    checkpointCommit,
    branch: "main",
    debateNumber: DEBATE_NUMBER,
    ...
    nextAuthorizedAction:
      "freeze-two-minimum-field-disjoint-debate-137-replacement-contexts",
  };
  ...
  const correction = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-08-debate-137-inventory-plan-recovery-correction-plan",
    protocolId: PROTOCOL_ID,
    status:
      "two-minimum-field-disjoint-debate-137-inventory-plan-recovery-contexts-frozen-not-activated",
    frozenAt,
    checkpointCommit,
    branch: "main",
    diagnosis: DIAGNOSIS,
  ...
  const preparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-08-debate-137-inventory-plan-recovery-execution-preparation-manifest",
    protocolId: PROTOCOL_ID,
    status:
    ...
    meteredApiCostUsdMaximum: 0,
    nextRequiredAction:
      "separately-activate-two-frozen-debate-137-inventory-plan-recovery-shards",
  };
...
async function validate() {
  ...
  assertV4(
    ...
    "frozen Debate 137 recovery preparation controls drifted"
  );
  ...
  console.log(
    JSON.stringify(
      {
        status: "passed-frozen-debate-137-inventory-plan-recovery-preparation",
        contexts: 2,
        shards: SHARDS,
        protectedAcceptedPlans: 9,
