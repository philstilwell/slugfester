...
const identity = JSON.parse(await readFile(path.resolve(
  "docs/assessment-production/post-canary-continuation-v1/batch-07/deterministic-publication-compilation/production-identity-snapshot.json"), "utf8"));
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const exists = (file) => access(path.resolve(file)).then(() => true, () => false);
assertV4(a.status === "frozen-post-canary-batch-07-publication-finalization-authorized" &&
  ...
  canonicalJson(a.explicitOrder) === canonicalJson(POST_CANARY_BATCH_07_PUBLICATION_FINALIZATION_ORDER),
"Batch 7 finalization is not authorized");
...
try {
  ...
  assertV4(candidates.length === 10 && candidates.reduce((sum, row) => sum + row.validation.moves, 0) === 187,
...
if (passed) {
  ...
  const preview = Buffer.from(buildPostCanaryBatch07PublicationStagingPreviewHtml());
  ...
  await writeFile(path.resolve(a.artifacts.audit), `${JSON.stringify({
    schemaVersion: "1.0-assessment-production-post-canary-batch-07-publication-finalization-audit",
    protocolId: a.protocolId, status: "passed", rows,
    totals: { debates: 10, moves: 187, deterministicFinalizationPasses: 1,
...
const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-07-publication-finalization-execution",
  protocolId: a.protocolId, status: passed ? "ten-debate-batch-07-publication-finalization-passed" :
    "ten-debate-batch-07-publication-finalization-failed", startedAt,
...
const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-07-publication-finalization-analysis",
  protocolId: a.protocolId, status: execution.status, productionCanary: false, batchNumber: 7,
  ...
  nextAuthorizedAction: passed ? "prepare-batch-07-rendering-verification" : "diagnose-finalization-failure" };
