...
import {
  ...
  POST_CANARY_BATCH_13_STANDING_AUTHORIZATION_INSTRUCTION_PATH,
  POST_CANARY_BATCH_13_STANDING_AUTHORIZATION_PROTOCOL_ID,
  POST_CANARY_BATCH_13_STANDING_AUTHORIZATION_STATUS,
  validatePostCanaryBatch13StandingAuthorization
...
const SELECTION_FREEZE_COMMIT = "4801afd8a290da34f191bd865b5359b6a75b713c";
const SOURCE_PACKET_COMMIT = "cb8cee8014d3aafc51e3b77548474891f8cd8161";
const SOURCE_FILES = [
  ...
  "scripts/prepare-assessment-production-post-canary-batch-13-standing-authorization.mjs",
  "scripts/test-assessment-production-post-canary-batch-13-standing-authorization.mjs"
];
...
assertV4(
  !(await exists(POST_CANARY_BATCH_13_STANDING_AUTHORIZATION)),
  `${POST_CANARY_BATCH_13_STANDING_AUTHORIZATION} already exists`
);
...
assertV4(
  selection.status ===
    ...
    selection.batchNumber === 13 &&
    selection.checkpointCommit ===
      "8faec58c73e82077648c1caf654b00abfc7f1236" &&
    ...
    analysis.status ===
      "thirteenth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization" &&
    ...
    sourcePreparation.totals.discoveryContexts === 41 &&
    ...
    sourceValidation.totals.discoveryContexts === 41 &&
    sourceValidation.totals.modelContextsExecuted === 0 &&
    sourceValidation.directIncrementalCostUsd === 0,
  "the frozen Batch 13 selection or source-packet boundary changed"
);
assertV4(
  execFileSync("git", ["show", `${SELECTION_FREEZE_COMMIT}:${SELECTION}`]).equals(selectionBytes),
  "Batch 13 selection differs from the user-authorized selection commit"
);
...
assertV4(
  ...
  "Batch 13 standing authorization must freeze at the pushed source-packet commit"
);
...
const record = {
  ...
  protocolId: POST_CANARY_BATCH_13_STANDING_AUTHORIZATION_PROTOCOL_ID,
  status: POST_CANARY_BATCH_13_STANDING_AUTHORIZATION_STATUS,
  authorizedAt,
  ...
  userAuthorization: {
    instruction: POST_CANARY_BATCH_13_STANDING_AUTHORIZATION_INSTRUCTION,
    resolvedAntecedent: "The user's instruction to continue Batch 13 at discretion authorizes the complete reassessment, bounded recovery, publication, validation, commit, and push workflow under the current frozen campaign controls.",
    scopeInterpretation: "Create and use a complete-workflow Batch 13 standing authorization for the ten frozen selections, beginning with the separately frozen score-blind source packets, including bounded diagnosed recovery and production publication, while preserving the established campaign controls and stopping before Batch 14 selection.",
  ...
  nextAuthorizedAction:
    "prepare-validate-freeze-commit-and-push-batch-13-discovery-execution-manifest"
};
validatePostCanaryBatch13StandingAuthorization(record);
if (shouldWrite) {
  await writeFile(
