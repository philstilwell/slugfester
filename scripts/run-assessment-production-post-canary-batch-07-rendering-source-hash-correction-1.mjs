...
const EXECUTION = `${CORRECTION}/execution.json`;
const ANALYSIS = `${CORRECTION}/analysis.json`;
...
const [planBytes, preparationBytes, activationBytes, proposedPreparationBytes,
  proposedActivationBytes, targetBytes, prepareHarnessBytes, runHarnessBytes] = await Promise.all([
  readFile(path.resolve(PLAN)), readFile(path.resolve(PREPARATION)),
  readFile(path.resolve(ACTIVATION)), readFile(path.resolve(PROPOSED_PREPARATION)),
  readFile(path.resolve(PROPOSED_ACTIVATION)), readFile(path.resolve(TARGET)),
  readFile(path.resolve(PREPARE_HARNESS)), readFile(path.resolve(RUN_HARNESS))
]);
const plan = JSON.parse(planBytes);
assertV4(
  plan.status === "frozen-batch-07-rendering-source-hash-correction-1-prepared" &&
  ...
  sha256(prepareHarnessBytes) === plan.authenticatedHarnesses[PREPARE_HARNESS] &&
  sha256(runHarnessBytes) === plan.authenticatedHarnesses[RUN_HARNESS],
  "correction authentication failed"
);
for (const [target, bytes] of [
  [PREPARATION, proposedPreparationBytes],
  [ACTIVATION, proposedActivationBytes]
]) {
...
const passed =
  preparation.sourceHashes[TARGET] === targetSha256 &&
  activation.sourceHashes[TARGET] === targetSha256 &&
...
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-07-rendering-source-hash-correction-1-execution",
  status: "passed",
  attempts: 1,
  retries: 0,
  timeoutExtensions: 0,
  correctedFields: [
    "preparation.sourceHashes.src/data/debates.js",
    "activation.sourceHashes.src/data/debates.js",
    "activation.preparation.sha256",
    "activation.executionNavigation.input.preparationSha256",
    "activation.executionNavigation.token"
  ],
  directIncrementalCostUsd: 0
};
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-07-rendering-source-hash-correction-1-analysis",
  status: "batch-07-rendering-source-hash-correction-1-passed",
