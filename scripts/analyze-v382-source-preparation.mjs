...
import { V381_ROOT, V382_DEBATE_NUMBERS, V382_ROOT, V38_SOURCE_AUDIT, assert } from "./lib/v382-source-preparation.mjs";
...
const [audit, disagreements, maps] = await Promise.all([
  readJson(V38_SOURCE_AUDIT),
  readJson(`${V382_ROOT}/initial-disagreements.json`),
  readJson(`${V382_ROOT}/adjudication-option-map.json`)
]);
const audioPath = `${V382_ROOT}/audio-verification.json`;
const audioRequiredPath = `${V382_ROOT}/audio-verification-required.json`;
const audio = await exists(audioPath) ? await readJson(audioPath) : { schemaVersion: "3.8.2-source-audio-verification", debates: {} };
const reports = [];
const finalDebates = [];
const pendingAudioItems = [];
let pendingAudio = 0;
...
for (const debateNumber of V382_DEBATE_NUMBERS) {
  ...
  const reviewPacket = await readJson(`${V382_ROOT}/review/packets/debate-${debateNumber}.json`);
  const review = await readJson(`${V382_ROOT}/review/outputs/debate-${debateNumber}.json`);
  ...
  if (dispute.disagreementCount > 0) {
    const [adjudicationPacket, adjudicationSchema, output] = await Promise.all([
      readJson(dispute.adjudicationPacket), readJson(dispute.adjudicationSchema), readJson(dispute.adjudicationOutput)
    ]);
  ...
  for (const move of resolved.moves.filter((item) => item.audioVerificationRequired)) {
    ...
    if (!record) {
      pendingAudio += 1;
      debatePendingAudio += 1;
      pendingAudioItems.push({
        debateNumber,
        debateId: proposal.debateId,
        videoId: audit.debateSources[debateNumber].videoId,
        sourceUrl: audit.debateSources[debateNumber].sourceUrl,
        moveId: move.moveId,
        sourceSpan: move.sourceSpan,
        proposedSpeaker: move.speaker,
        proposedSide: move.side,
        allowedSpeakersBySide: packet.sides,
  ...
  if (shouldWrite) {
    const resolvedPath = `${V382_ROOT}/resolved/debate-${debateNumber}.json`;
  ...
  reports.push({
    debateNumber,
    debateId: proposal.debateId,
    comparisonFields: resolvedFields.length,
    initialAgreements: resolvedFields.filter((item) => item.agreed).length,
    initialDisagreements: resolvedFields.filter((item) => !item.agreed).length,
    finalTwoVoteFields: resolvedFields.filter((item) => item.finalVotes >= 2).length,
    unresolvedFields: resolvedFields.filter((item) => item.finalVotes < 2).length,
    proposedMoves: resolved.moves.length,
    acceptedMovesBeforeSelection: resolved.moves.filter((item) => item.accepted).length,
    requiredAudio: resolved.moves.filter((item) => item.audioVerification !== null || item.audioVerificationRequired).length,
    pendingAudio: debatePendingAudio,
    eligibleMovesAfterAudio: selection.eligibleMoveCount,
    validSelectionCombinations: selection.combinationCount,
    selectedMoves: selectedMoveIds.length
  });
}
...
const inventory = {
  schemaVersion: "3.8.2-heldout-source-inventory",
  inheritedSemanticSchema: "3.8.1",
  status: sourcePreparationPassed ? "locked-source-inventory" : "source-inventory-incomplete",
  warning: "Provisional burden-contact values are AI source-preparation aids hidden from classifiers; they are not truth keys.",
  debateCount: finalDebates.length,
  selectedMoveCount: selected.length,
  debates: finalDebates
};
const analysis = {
  schemaVersion: "3.8.2-heldout-source-preparation-analysis",
  ...
  totals: {
    debateCount: reports.length,
    comparisonFields: reports.reduce((sum, item) => sum + item.comparisonFields, 0),
    initialAgreements: reports.reduce((sum, item) => sum + item.initialAgreements, 0),
    initialDisagreements: reports.reduce((sum, item) => sum + item.initialDisagreements, 0),
    finalTwoVoteFields: reports.reduce((sum, item) => sum + item.finalTwoVoteFields, 0),
    unresolvedFields: reports.reduce((sum, item) => sum + item.unresolvedFields, 0),
    requiredAudioVerifications: reports.reduce((sum, item) => sum + item.requiredAudio, 0),
    pendingAudioVerifications: pendingAudio,
  ...
  decision: {
    classificationPacketConstructionPreregistrationAuthorized: sourcePreparationPassed,
    burdenContactClassificationModelExecutionAuthorized: false,
  ...
  artifacts: {
    ...
    audioVerificationRequired: audioRequiredPath
  }
};
if (shouldWrite) {
  await writeFile(path.resolve(root, analysis.artifacts.finalInventory), `${JSON.stringify(inventory, null, 2)}\n`);
  await writeFile(path.resolve(root, `${V382_ROOT}/source-preparation-analysis.json`), `${JSON.stringify(analysis, null, 2)}\n`);
  await writeFile(path.resolve(root, audioRequiredPath), `${JSON.stringify({ schemaVersion: "3.8.2-source-audio-verification-required", pendingCount: pendingAudioItems.length, items: pendingAudioItems }, null, 2)}\n`);
}
