...
const manifest = JSON.parse(await readFile(path.resolve(
  `${POST_CANARY_BATCH_10_RENDERING_ROOT}/preparation-manifest.json`), "utf8"));
const exists = (file) => access(path.resolve(file)).then(() => true, () => false);
assertV4(manifest.status ===
  "frozen-post-canary-batch-10-rendering-verification-prepared" &&
  manifest.protocolId === POST_CANARY_BATCH_10_RENDERING_PROTOCOL_ID &&
  manifest.packets?.length === 10 &&
  canonicalJson(manifest.explicitOrder) ===
    canonicalJson(POST_CANARY_BATCH_10_RENDERING_ORDER) &&
  manifest.gateExpectations?.sections === 51 &&
  manifest.gateExpectations?.moves === 182 &&
...
"Batch 10 rendering preparation changed");
...
for (const row of manifest.packets) {
  ...
  validatePostCanaryBatch10RenderingPacket(JSON.parse(packetBytes));
}
...
console.log(JSON.stringify({ status: "passed", debates: 10,
  sections: 51, moves: 182, packets: 10,
