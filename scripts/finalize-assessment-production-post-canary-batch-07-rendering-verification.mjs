...
import { POST_CANARY_BATCH_07_RENDERING_ORDER, POST_CANARY_BATCH_07_RENDERING_ROOT,
  validatePostCanaryBatch07RenderingPacket,
  validatePostCanaryBatch07RenderingViewportEvidence } from
...
assertV4(a.status === "frozen-post-canary-batch-07-rendering-verification-authorized" &&
...
"Batch 7 rendering activation changed");
const results = [];
for (const debateNumber of POST_CANARY_BATCH_07_RENDERING_ORDER) {
  ...
  const packet = validatePostCanaryBatch07RenderingPacket(JSON.parse(packetBytes));
  for (const viewportName of ["desktop", "mobile"]) {
    const evidence = JSON.parse(await readFile(path.resolve(packet.viewports[viewportName].evidence.result), "utf8"));
    validatePostCanaryBatch07RenderingViewportEvidence({ packet, viewportName,
...
assertV4(results.length === 20 && results.every((row) => row.evidence.status === "passed-rendering-viewport"),
  "complete Batch 7 rendering evidence failed");
const totals = { debates: 10, sections: 51, moves: 187, viewportResults: 20,
...
const audit = { schemaVersion: "1.0-assessment-production-post-canary-batch-07-rendering-audit",
  protocolId: a.protocolId, status: "passed-ten-debate-batch-07-rendering-verification",
...
const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-07-rendering-execution",
  protocolId: a.protocolId, status: "ten-debate-batch-07-rendering-verification-passed",
...
const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-07-rendering-analysis",
  ...
  nextAuthorizedAction: "prepare-batch-07-production-compatibility-staging" };
...
console.log(JSON.stringify({ status: analysis.status, debates: 10, sections: 51, moves: 187,
