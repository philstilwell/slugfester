...
assertV4(
  ...
  "frozen remedy-v3 preparation required"
);
execFileSync(
  process.execPath,
  [
...
for (const [file, digest] of Object.entries(preparation.sourceHashes)) {
  assertV4(
    ...
    `source hash mismatch before remedy-v3 activation: ${file}`
  );
}
for (const row of preparation.packets) {
  ...
  assertV4(
    ...
    `${row.debateNumber}: remedy-v3 packet hash mismatch`
  );
  validateCheckpointV22RenderingRemedyV3Packet(JSON.parse(bytes));
}
...
const activation = {
  schemaVersion:
    "1.0-production-checkpoint-v2.2-rendering-remedy-v3-execution-activation",
  protocolId: CHECKPOINT_V22_RENDERING_REMEDY_V3_PROTOCOL_ID,
  status:
  ...
  nextAuthorizedAction:
    "single-remedy-v3-rendering-execution-under-frozen-stop-rules"
};

if (shouldWrite) {
  assertV4(!(await exists(activationPath)), "remedy-v3 activation already exists");
