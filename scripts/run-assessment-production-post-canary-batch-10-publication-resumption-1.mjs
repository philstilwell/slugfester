...
assertV4(
  activation.status === "frozen-nine-original-unattempted-batch-10-publication-contexts-activated" &&
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-10-publication-resumption-${context.debateNumber}-`));
  const isolatedCodexHome = await mkdtemp(path.join(os.tmpdir(), `batch-10-publication-resumption-home-${context.debateNumber}-`));
  ...
  try {
    ...
    const prompt = [
      ...
      "Never infer, emit, recalculate, or suggest changing a score; never change identity, structure, move selection, or source evidence; never consult legacy assessment material, Debate 21 output, other debates, rankings, winners, or another model context; never attribute AI material to a participant.",
    ...
    process.stdout.write(`[batch-10-publication-resumption] starting original index ${context.originalContextIndex} Debate ${context.debateNumber}\n`);
  ...
  process.stdout.write(`[batch-10-publication-resumption] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
...
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-original-unattempted-context-resumption-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "nine-original-unattempted-batch-10-publication-contexts-passed"
