...
const [preparationBytes, activationBytes, failureBytes] = await Promise.all([
  readFile(path.resolve(POST_CANARY_BATCH_02_RENDERING_RESUMPTION_1_PREPARATION)),
  readFile(path.resolve(POST_CANARY_BATCH_02_RENDERING_RESUMPTION_1_ACTIVATION)),
  readFile(path.resolve(POST_CANARY_BATCH_02_RENDERING_RESUMPTION_1_FAILURE))
]);
...
assertV4(
  preparation.protocolId ===
    ...
    activation.protocolId ===
      POST_CANARY_BATCH_02_RENDERING_RESUMPTION_1_PROTOCOL_ID &&
    activation.status === "frozen-batch-02-rendering-resumption-1-authorized" &&
  ...
  "valid frozen Batch 2 rendering resumption-1 activation required"
);
validatePostCanaryBatch02RenderingResumption1Plan(activation.viewportPlan);
if (shouldWrite) {
  for (const output of [
    POST_CANARY_BATCH_02_RENDERING_RESUMPTION_1_EXECUTION,
    POST_CANARY_BATCH_02_RENDERING_RESUMPTION_1_ANALYSIS,
...
assertV4(
  evidenceAudit.status ===
      "ten-debate-batch-02-rendering-resumption-1-evidence-passed" &&
    evidenceAudit.debates === 10 &&
    evidenceAudit.sections === 51 &&
    evidenceAudit.moves === 190 &&
  ...
  "passing Batch 2 rendering resumption-1 evidence required"
);

const packets = await loadPostCanaryBatch02RenderingResumption1Packets(preparation);
const rows = [];
...
assertV4(rows.length === 10, "ten Batch 2 rendering audit rows required");

const renderingAudit = {
  ...evidenceAudit,
  status: "passed-ten-debate-batch-02-rendering-resumption-1",
  completedAt,
  failureDiagnosis: {
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-rendering-resumption-1-execution",
  protocolId: POST_CANARY_BATCH_02_RENDERING_RESUMPTION_1_PROTOCOL_ID,
  status: "ten-debate-batch-02-rendering-resumption-1-passed",
  completedAt,
  preparation: {
  ...
  activation: {
    path: POST_CANARY_BATCH_02_RENDERING_RESUMPTION_1_ACTIVATION,
    sha256: sha256(activationBytes)
  },
  renderingAudit: {
    path: POST_CANARY_BATCH_02_RENDERING_RESUMPTION_1_AUDIT,
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-rendering-resumption-1-analysis",
  protocolId: POST_CANARY_BATCH_02_RENDERING_RESUMPTION_1_PROTOCOL_ID,
  status: "ten-debate-batch-02-rendering-resumption-1-passed",
  analyzedAt: completedAt,
  execution: {
    path: POST_CANARY_BATCH_02_RENDERING_RESUMPTION_1_EXECUTION,
  ...
  authorization: {
    compatibilityPlanPreparation: false,
...
if (shouldWrite) {
  for (const [file, bytes] of [
    [POST_CANARY_BATCH_02_RENDERING_RESUMPTION_1_AUDIT, auditBytes],
    [POST_CANARY_BATCH_02_RENDERING_RESUMPTION_1_EXECUTION, executionBytes],
    [
      POST_CANARY_BATCH_02_RENDERING_RESUMPTION_1_ANALYSIS,
