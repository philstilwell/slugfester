...
const DISCOVERY_EXECUTION = `${BATCH_ROOT}/discovery/recovery-1/debate-11-chunk-001/cohort-execution-overlay.json`;
...
const SCRIPT =
  "scripts/prepare-assessment-production-post-canary-batch-12-inventory.mjs";
const PROTOCOL_ID =
  "assessment-production-post-canary-batch-12-candidate-sharded-inventory";
const SOURCE_PACKET_VERSION =
  "1.0-assessment-production-post-canary-batch-12-candidate-sharded-inventory-source-packet";
...
function buildInventorySourcePacket(sourcePacket) {
  assertV4(
    sourcePacket?.schemaVersion ===
      "1.0-assessment-production-post-canary-batch-12-score-blind-source-packet" &&
      sourcePacket.protocolId ===
        "assessment-production-post-canary-batch-12-source-preparation" &&
    ...
    "Batch 12 source packet is unavailable or not dyadic"
  );
  return {
...
function makePlanPacket({
  ...
  return {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-12-candidate-census-plan-packet",
    protocolId: PROTOCOL_ID,
...
async function buildFrozenArtifacts(frozenAt) {
  ...
  assertV4(
    ...
    "Batch 12 standing authorization does not cover exact inventory preparation and execution"
  );
  ...
  assertV4(
    discoveryAnalysis.status ===
      ...
      discoveryAnalysis.audit?.validContexts === 44 &&
      ...
      discoveryAnalysis.totals?.modelContextsExecuted === 46 &&
      discoveryAnalysis.totals?.effectiveDiscoveryContexts === 44 &&
      discoveryAnalysis.totals?.recoveryShardContexts === 2 &&
    ...
    "passed Batch 12 discovery boundary drifted"
  );
  assertV4(
    discoveryExecution.status ===
      "forty-four-post-canary-batch-12-discovery-contexts-passed-after-bounded-recovery" &&
      discoveryExecution.contextsAttempted === 44 &&
      discoveryExecution.validContexts === 44 &&
      discoveryExecution.invalidContexts === 0 &&
      discoveryExecution.recovery?.recoveryLevel === 1 &&
      discoveryExecution.recovery?.fieldDisjointShardContextsPassed === 2 &&
      discoveryExecution.recovery?.originalAcceptedOutputsPreservedByteIdentical === 43 &&
    ...
    "Batch 12 discovery execution drifted"
  );
  assertV4(
    ...
    "Batch 12 source preparation or model boundary drifted"
  );
  assertV4(
    ...
    "Batch 12 source validation drifted"
  );
  assertV4(
    selection.status ===
    ...
    "Batch 12 frozen selection drifted"
  );
  assertV4(
  ...
  for (const discovered of discoveryAnalysis.debates) {
    ...
    assertV4(
      ...
      `${discovered.debateNumber}: Batch 12 identity drifted`
    );
    const [
  ...
  assertV4(
    ...
    "Batch 12 totals or packet state drifted"
  );
  ...
  const preparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-12-candidate-sharded-inventory-preparation",
    protocolId: PROTOCOL_ID,
    sideSelectionProtocolId: CHRONOLOGY_FALLBACK_INVENTORY.protocolId,
    status:
      "post-canary-batch-12-candidate-sharded-source-assets-and-ten-planner-packets-frozen",
    preparedAt: frozenAt,
    ...
    userAuthorization: {
      scope:
        "prepare, freeze, activate, and execute the Batch 12 candidate-census and candidate-sharded side-selection workflow under the frozen standing authorization",
    ...
    nextAuthorizedAction:
      "freeze-and-activate-batch-12-candidate-census-plan-execution-under-standing-authorization",
  };
  ...
  const executionPreparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-12-candidate-census-plan-execution-preparation-manifest",
    protocolId: PROTOCOL_ID,
    status:
      "frozen-ten-post-canary-batch-12-candidate-census-plan-contexts-prepared-not-authorized",
    frozenAt,
    ...
    costEstimate: {
      ...
      estimateBasis:
        `The promoted ten-context candidate-census planner used this contract at concurrency two. Batch 12 freezes the same ten-context shape with an observed maximum of ${maximumCopiedInputBytes.toLocaleString("en-US")} copied bytes, below the proven ${V212_CANDIDATE_SHARDED_INVENTORY.maximumCopiedInputBytes.toLocaleString("en-US")}-byte ceiling. ChatGPT-subscription execution has no direct incremental API charge.`,
    },
  ...
  const validation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-12-candidate-sharded-inventory-preparation-validation",
    protocolId: PROTOCOL_ID,
    status:
      "batch-12-candidate-sharded-inventory-packets-and-plan-execution-manifest-validation-passed-frozen-standing-authorization-active",
...
async function validateFrozen() {
  ...
  assertV4(
    ...
    "frozen Batch 12 inventory preparation is invalid"
  );
...
if (command === "freeze") await freeze();
  ...
  throw new Error(
    "usage: prepare-assessment-production-post-canary-batch-12-inventory.mjs <freeze|validate>"
  );
}
