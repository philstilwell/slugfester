...
assertV4(a.status === "frozen-post-canary-batch-09-deterministic-publication-compilation-authorized" &&
  ...
  canonicalJson(a.explicitOrder) === canonicalJson(POST_CANARY_BATCH_09_PUBLICATION_COMPILATION_ORDER),
"Batch 9 compilation is not authorized");
...
try {
  ...
  for (const debateNumber of POST_CANARY_BATCH_09_PUBLICATION_COMPILATION_ORDER) {
    ...
    const compiled = compilePostCanaryBatch09PublicationStagingRecord({ output, packet, identity: id });
    const validation = validatePostCanaryBatch09CompiledStagingRecord({ compiled, output, packet, identity: id });
...
if (passed) await writeFile(path.resolve(a.artifacts.compilationAudit), `${JSON.stringify({
  schemaVersion: "1.0-assessment-production-post-canary-batch-09-publication-compilation-audit",
  ...
  totals: { debates: 10, sections: rows.reduce((sum, row) => sum + row.validation.sections, 0),
    moves: 180, deterministicCompilationPasses: 1, reruns: 0, scorePasses: 0,
    modelContexts: 0, modelAuthoredScores: 0, directIncrementalCostUsd: 0 },
  productionMutationPerformed: false }, null, 2)}\n`);
const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-09-publication-compilation-execution",
  protocolId: a.protocolId, status: passed ? "ten-debate-batch-09-deterministic-publication-compilation-passed" :
    "ten-debate-batch-09-deterministic-publication-compilation-failed",
...
const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-09-publication-compilation-analysis",
  ...
  nextAuthorizedAction: passed ? "prepare-batch-09-publication-finalization" : "diagnose-compilation-failure" };
