...
import { POST_CANARY_BATCH_05_PUBLICATION_MODEL,
  POST_CANARY_BATCH_05_PUBLICATION_ROOT } from
...
import { POST_CANARY_BATCH_05_DEBATE_42_REPAIR_OUTPUT_VERSION,
  POST_CANARY_BATCH_05_DEBATE_42_REPAIR_PACKET_VERSION,
  POST_CANARY_BATCH_05_DEBATE_42_REPAIR_PROTOCOL_ID,
  POST_CANARY_BATCH_05_DEBATE_42_REPAIR_ROOT,
  buildDebate42RepairSchema, debate42RepairMoveId,
  mergeAndValidateDebate42Repair, validateDebate42RepairOutput } from
...
const ROOT = POST_CANARY_BATCH_05_DEBATE_42_REPAIR_ROOT;
...
const BASE_OUTPUT = `${RESUMPTION_ROOT}/outputs/debate-42.json`;
const PUBLICATION_PACKET = `${POST_CANARY_BATCH_05_PUBLICATION_ROOT}/packets/debate-42.json`;
...
const STATIC = [DIAGNOSIS, BASE_OUTPUT, PUBLICATION_PACKET, PRODUCTION_WORKFLOW,
  READINESS_WORKFLOW, OUTPUT_CONTRACT, MANUAL, POST_CANARY_BATCH_05_STANDING_AUTHORIZATION,
  ...
  `${RESUMPTION_ROOT}/analysis.json`, `${RESUMPTION_ROOT}/outputs/debate-179.json`,
  `${RESUMPTION_ROOT}/outputs/debate-05.json`,
  ...
  "scripts/lib/assessment-production-post-canary-batch-05-publication-resumption-2-repair.mjs",
  "scripts/lib/assessment-production-post-canary-batch-05-standing-authorization.mjs",
  "scripts/diagnose-assessment-production-post-canary-batch-05-publication-resumption-2-debate-42.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-05-publication-resumption-2-repair.mjs",
  "scripts/test-assessment-production-post-canary-batch-05-publication-resumption-2-repair-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-05-publication-resumption-2-repair.mjs",
  "scripts/run-assessment-production-post-canary-batch-05-publication-resumption-2-repair.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-05-publication-resumption-2-repair.mjs"];
...
assertV4(diagnosis.status === "frozen-diagnosed-batch-05-debate-42-four-critique-word-overruns" &&
  diagnosis.failedFieldCount === 4 && diagnosis.excessWordsTotal === 8 &&
  diagnosis.diagnosticReplay?.result?.status === "passed" &&
  diagnosis.minimumBoundedRepair?.packetCount === 2 &&
  diagnosis.preservedFields?.debate59Unattempted === true,
"the frozen Debate 42 diagnosis changed");
...
for (let packetIndex = 0; packetIndex < 2; packetIndex += 1) {
  ...
  const corrections = writableFields.map((field) => {
    const moveId = debate42RepairMoveId(field);
  ...
  const packet = { schemaVersion: POST_CANARY_BATCH_05_DEBATE_42_REPAIR_PACKET_VERSION,
    protocolId: POST_CANARY_BATCH_05_DEBATE_42_REPAIR_PROTOCOL_ID,
    contextIndex: packetIndex, packetIndex, productionCanary: false, batchNumber: 5,
    stagingOnly: true, debateNumber: "42", debateId: publicationPacket.debateId,
    ...
    constraints: { writableFields, writableFieldCount: 2, maximumWritableFields: 2,
      ...
      recursiveRecoveryMaximum: 0 }, corrections };
  const schema = buildDebate42RepairSchema(packet);
  ...
  contexts.push({ contextIndex: packetIndex, packetIndex, debateNumber: "42",
  ...
  const outputRecord = { schemaVersion: POST_CANARY_BATCH_05_DEBATE_42_REPAIR_OUTPUT_VERSION,
    protocolId: POST_CANARY_BATCH_05_DEBATE_42_REPAIR_PROTOCOL_ID,
    packetIndex, debateNumber: "42", debateId: packet.debateId,
    assessmentModel: POST_CANARY_BATCH_05_PUBLICATION_MODEL.label,
    completedAt: frozenAt, correctedCritiques };
  validateDebate42RepairOutput(outputRecord, packet); synthetic.push(outputRecord);
}
assertV4(contexts.length === 2 && new Set(contexts.flatMap((row) => row.writableFields)).size === 4,
  "the two-packet repair boundary changed");
const syntheticMerge = mergeAndValidateDebate42Repair({ baseOutput,
  repairOutputs: synthetic, repairPackets: packets, publicationPacket });
assertV4(syntheticMerge.fullValidation.status === "passed" &&
  syntheticMerge.fullValidation.moves === 15, "synthetic complete Debate 42 replay failed");
const futureOutputs = [`${ROOT}/execution-activation.json`, `${ROOT}/model-execution.json`,
  ...
  `${ROOT}/merged/debate-42.json`, `${ROOT}/complete-debate-validation.json`, `${ROOT}/merge-audit.json`];
...
const manifest = { schemaVersion: "1.0-assessment-production-post-canary-batch-05-debate-42-repair-preparation",
  protocolId: POST_CANARY_BATCH_05_DEBATE_42_REPAIR_PROTOCOL_ID,
  status: "frozen-two-context-batch-05-debate-42-publication-repair-prepared-and-authorized",
  ...
  costEstimate: { authentication: "ChatGPT subscription", contexts: 2,
    ...
    expectedWallMinutes: [2, 10], absoluteGateTimeoutMinutes: 20 },
  ...
  isolation: { freshTemporaryWorkingDirectoryPerContext: true,
    ...
    APIKeysRemoved: true },
  repairContract: { contexts: 2, writableFields: 4, maximumWritableFieldsPerPacket: 2,
  ...
  executionPolicy: { contexts: 2, attemptsPerContext: 1, retriesMaximum: 0,
    ...
    { phase: "repair-second", maximumParallelContexts: 1,
  ...
  authorization: { executionActivationPreparation: true, repairModelExecution: false,
    deterministicMergeAndValidation: false, debate59ResumptionPreparation: false,
    paidServices: false, productionMutation: false, nextBatchSelection: false },
  nextAuthorizedAction: "activate-and-execute-exactly-two-frozen-debate-42-repair-contexts" };
...
console.log(JSON.stringify({ status: manifest.status, contextsPrepared: 2,
  writableFieldsPrepared: 4, completeSyntheticReplay: "passed",
