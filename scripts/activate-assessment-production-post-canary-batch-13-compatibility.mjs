...
import {
  ...
  buildPostCanaryBatch13ValidatorSource,
  sha256,
  validatePostCanaryBatch13ValidatorSource
} from "./lib/assessment-production-post-canary-batch-13-compatibility-execution.mjs";
...
const paths = {
  ...
  preparationAnalysis: `${POST_CANARY_BATCH_13_COMPATIBILITY_ROOT}/analysis.json`,
  ...
  executionLibrary:
    "scripts/lib/assessment-production-post-canary-batch-13-compatibility-execution.mjs",
  activationScript:
    "scripts/activate-assessment-production-post-canary-batch-13-compatibility.mjs",
  activationTest:
    "scripts/test-assessment-production-post-canary-batch-13-compatibility-activation.mjs",
  executionScript:
    "scripts/run-assessment-production-post-canary-batch-13-compatibility.mjs",
  executionTest:
    "scripts/test-assessment-production-post-canary-batch-13-compatibility-execution.mjs"
};
...
assertV4(
  preparation.status ===
      "post-canary-batch-13-compatibility-plan-prepared-and-frozen" &&
  ...
  "frozen Batch 13 compatibility preparation is unavailable"
);
assertV4(
  preparationAnalysis.status ===
      "post-canary-batch-13-compatibility-plan-freeze-passed" &&
    preparationAnalysis.preparation.sha256 === sha256(preparationBytes) &&
    preparationAnalysis.finding.executed === false,
  "frozen Batch 13 compatibility preparation analysis is unavailable"
);
...
assertV4(
  ...
  "a stable --activated-at ISO timestamp is required for the first Batch 13 compatibility activation write"
);
assertV4(
  !(await exists(paths.execution)) && !(await exists(paths.stagedLedgerRoot)),
  "Batch 13 compatibility execution already exists; activation blocked"
);
...
assertV4(
  canonicalJson(packetHashes.map((item) => item.debateNumber)) ===
    canonicalJson(POST_CANARY_BATCH_13_COMPATIBILITY_ORDER),
  "Batch 13 compatibility activation packet order changed"
);
...
assertV4(
  ...
  "active validator differs from the frozen Batch 13 baseline"
);
const proposedValidator = buildPostCanaryBatch13ValidatorSource(
  baselineValidator
);
const proposedValidatorValidation =
  validatePostCanaryBatch13ValidatorSource(proposedValidator);
...
const activation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-13-compatibility-execution-activation",
  protocolId: preparation.protocolId,
  status: POST_CANARY_BATCH_13_COMPATIBILITY_ACTIVATION_STATUS,
  activatedAt,
  ...
  userAuthorization: {
    instruction:
      "The Batch 13 standing authorization covers activation and exactly one deterministic compatibility staging pass.",
    scopeInterpretation:
      "Activate the frozen Batch 13 compatibility manifest and execute exactly one validator-and-staged-ledger compatibility pass with no reruns, models, paid services, production ledger publication, production data mutation, or Batch 14 selection."
  },
  preparation: {
  ...
  validator: {
    ...
    routeCredentialStatus: POST_CANARY_BATCH_13_COMPATIBILITY_ACTIVATION_STATUS
  },
  packetHashes,
  executionToolHashes,
  protectedProduction: {
  ...
  requiredRegressions: [
    "Batch 13 route positive and negative controls",
  ...
  nextAuthorizedAction:
    "execute-one-frozen-batch-13-production-compatibility-staging-pass-no-reruns"
};
const activationBytes = serializedJson(activation);

if (write) {
  assertV4(!existingActivation, "Batch 13 compatibility is already activated");
  await writeFile(resolve(paths.activation), activationBytes);
} else {
  assertV4(
    existingActivation &&
      canonicalJson(existingActivation) === canonicalJson(activation),
    "stored Batch 13 compatibility activation differs from deterministic replay"
  );
}
