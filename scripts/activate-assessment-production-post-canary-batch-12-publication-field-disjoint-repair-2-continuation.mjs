...
assertV4(
  preparation.protocolId === PROTOCOL_ID &&
    preparation.status === "frozen-fifteen-context-twenty-nine-field-batch-12-publication-repair-level-2-continuation-prepared-not-activated" &&
    preparation.contexts?.length === 15 && preparation.totals?.writableFields === 29 &&
    preparation.diagnosis?.totals?.invalidCritiques === 28 &&
    preparation.diagnosis?.totals?.invalidNoveltyExplanations === 1 &&
    preparation.diagnosis?.totals?.invalidQuotes === 0 &&
    preparation.contexts.every((context) => context.writableFieldCount <= 2) &&
    new Set(preparation.contexts.flatMap((context) => context.writableFields)).size === 29,
  "fifteen-shard level-2 continuation is not prepared"
);
assertV4(
  ...
  "fifteen-shard level-2 continuation model or execution controls changed"
);
...
const activation = {
  ...
  status: "frozen-fifteen-context-twenty-nine-field-batch-12-publication-repair-level-2-continuation-activated",
  activatedAt,
  ...
  nextAuthorizedAction: "execute-only-fifteen-frozen-level-2-continuation-shards"
};
