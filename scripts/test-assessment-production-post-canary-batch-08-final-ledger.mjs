...
const scoreArtifactCandidates = [
  `${POST_CANARY_BATCH_08_FINAL_LEDGER_ROOT}/calculated-scores.json`,
  `${POST_CANARY_BATCH_08_FINAL_LEDGER_ROOT}/score-analysis.json`,
  `${POST_CANARY_BATCH_08_FINAL_LEDGER_ROOT}/score-pass-manifest.json`
];
...
const rebuiltLedger = buildPostCanaryBatch08FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
...
assert.deepEqual(
  ...
  ["88", "194", "137", "08", "65", "140", "156", "120", "118", "145"]
);
assert.equal(storedLedger.audit.finalMoves, 182);
assert.equal(storedLedger.audit.disputedMoves, 174);
assert.equal(storedLedger.audit.pairSelections, 192);
assert.equal(storedLedger.audit.scoringFieldSelections, 321);
assert.equal(storedLedger.audit.burdenAdjustmentSelections, 20);
assert.equal(storedLedger.audit.candidateSelections, 533);
assert.equal(storedLedger.audit.meanMerges, 373);
...
assert.throws(() =>
  buildPostCanaryBatch08FinalLedger(mutatedInputs, inputs.sourceHashes)
);
...
assert.equal(analysis.totals.disputedMoves, 174);
assert.equal(analysis.totals.candidateSelections, 533);
