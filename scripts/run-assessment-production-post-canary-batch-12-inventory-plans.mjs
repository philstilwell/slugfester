...
const SCRIPT =
  "scripts/run-assessment-production-post-canary-batch-12-inventory-plans.mjs";
const PREPARER =
...
async function loadAndValidatePreparation({ requireFutureAbsent = false } = {}) {
  ...
  assertV4(
    ...
    "Batch 12 candidate-census planner preparation is invalid"
  );
  assertV4(
    ...
    "Batch 12 source preparation drifted"
  );
...
async function preflight() {
  ...
  assertV4(
    ...
    "frozen Batch 12 preparation validator failed"
  );
  console.log(
    JSON.stringify(
      {
...
async function activate() {
  ...
  const activation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-12-candidate-census-plan-execution-activation",
    ...
    userAuthorization: {
      scope:
        "activate and execute exactly the ten frozen Batch 12 candidate-census planner contexts under the frozen standing authorization",
    ...
    nextRequiredAction:
      "execute-exactly-ten-frozen-batch-12-candidate-census-plan-contexts-once",
  };
...
async function loadAndValidateActivation({ requireFutureAbsent = false } = {}) {
  ...
  assertV4(
    ...
    "Batch 12 candidate-census planner execution is unauthorized"
  );
...
async function execute() {
  ...
  async function executeContext(context, contextIndex) {
    ...
    const sourceDirectory = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-post-canary-batch-12-candidate-plan-${context.debateNumber}-`
      )
    );
    const isolatedCodexHome = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-post-canary-batch-12-candidate-plan-home-${context.debateNumber}-`
      )
    );
    ...
    try {
      ...
      process.stdout.write(
        `[post-canary-batch-12-candidate-plan] starting ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber}\n`
      );
    ...
    process.stdout.write(
      `[post-canary-batch-12-candidate-plan] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
    );
    return record;
  }
  ...
  const execution = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-12-candidate-census-plan-model-execution",
    protocolId: PROTOCOL_ID,
    status:
    ...
    nextRequiredAction:
      validContexts === CONTEXTS
        ? "analyze-and-replay-ten-batch-12-candidate-census-plans-model-free-only"
        : "stop-preserve-failed-batch-12-candidate-census-plan-gate",
  };
...
if (command === "preflight") await preflight();
  ...
  throw new Error(
    "usage: run-assessment-production-post-canary-batch-12-inventory-plans.mjs <preflight|activate|run|analyze|validate>"
  );
}
