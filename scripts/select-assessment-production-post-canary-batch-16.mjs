...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-16";
...
const PRIOR_SELECTION = "docs/assessment-production/post-canary-continuation-v1/batch-15/selection.json";
const PRIOR_SELECTION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-15/selection-analysis.json";
const PRIOR_MUTATION_MANIFEST = "docs/assessment-production/post-canary-continuation-v1/batch-15/production-publication/mutation-manifest.json";
const PRIOR_COMPLETION_EXECUTION = "docs/assessment-production/post-canary-continuation-v1/batch-15/production-publication/generated-seo-correction/execution.json";
const PRIOR_COMPLETION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-15/production-publication/generated-seo-correction/analysis.json";
const PRIOR_COMPLETION_COMMIT = "a6f27248d55f8dea4856ac4234d4b31dcc0c528c";
const SCRIPT = "scripts/select-assessment-production-post-canary-batch-16.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-16-selection.mjs";
...
assertV4(!(await exists(FUTURE_SOURCE_PREPARATION)), "source preparation already exists before Batch 16 selection");
...
assertV4(
  ...
  "Batch 16 selection is not compatible with the frozen continuation policy"
);
...
assertV4(
  priorSelection.status === "fifteenth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    priorSelection.protocolId === "assessment-production-post-canary-batch-15" &&
    priorSelection.productionSequenceOrdinal === 16 &&
    priorSelection.selected.length === 10 &&
    priorSelectionAnalysis.status === "fifteenth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization" &&
    priorSelectionAnalysis.selection.sha256 === sha256(controlBytes[PRIOR_SELECTION]) &&
    priorMutationManifest.status === "frozen-batch-15-production-publication-mutation-manifest-prepared" &&
    JSON.stringify(priorMutationManifest.explicitOrder) === JSON.stringify(priorSelectedNumbers) &&
    priorCompletionExecution.status === "passed-complete-batch-15-transaction-ready-for-atomic-commit" &&
    ...
    priorCompletionAnalysis.status === "batch-15-production-publication-accepted-for-atomic-commit-after-generated-seo-correction" &&
    priorCompletionAnalysis.decision.generatedSeoCorrectionGatePassed === true &&
    priorCompletionAnalysis.decision.batch15ProductionPublicationGatePassed === true &&
  ...
  "Batch 15 has not satisfied the frozen successful-publication prerequisite"
);
assertV4(git("rev-parse", "--abbrev-ref", "HEAD") === "main", "Batch 16 selection must run on main");
assertV4(git("rev-parse", "HEAD") === PRIOR_COMPLETION_COMMIT, "Batch 16 selection checkpoint is not the exact Batch 15 publication commit");
assertV4(git("rev-parse", "origin/main") === PRIOR_COMPLETION_COMMIT, "Batch 15 publication commit is not the pushed origin/main checkpoint");
...
assertV4(
  JSON.stringify(priorSelectedNumbers) === JSON.stringify(ranked.slice(140, 150).map((item) => item.debateNumber)),
  "Batch 15 is not the immutable fifteenth rank slice"
);

const selected = [];
let totalEvents = 0;
let totalDurationMs = 0;
for (const [offset, rankedItem] of ranked.slice(150, 160).entries()) {
...
assertV4(selected.length === 10 && new Set(selected.map((item) => item.debateNumber)).size === 10, "Batch 16 did not select ten unique debates");

const checkpointCommit = git("rev-parse", "HEAD");
const selection = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-selection",
  protocolId: "assessment-production-post-canary-batch-16",
  status: "sixteenth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed",
  frozenAt,
  checkpointCommit,
  branch: "main",
  ...
  stagingOnly: true,
  batchNumber: 16,
  productionSequenceOrdinal: 17,
  userAuthorization: {
    instruction: "Continue the SLUGFESTER reassessment campaign with Batch 16 in /Users/philstilwell/Documents/SLUGFESTER.com.",
    scopeInterpretation: "Materialize and source-validate only the deterministic Batch 16 selection under the frozen continuation policy after authenticating successful Batch 15 publication. This selection pass prepares no source packets, executes no models, derives no scores, and performs no production mutation.",
  ...
  eligibility: {
    ...
    previouslyPublishedAndRemovedCount: 150,
    remainingBeforeBatchCount: ranked.length - 150,
    remainingBeforeBatchRankedSha256: sha256(serializedJson(ranked.slice(150))),
    selectedCount: selected.length,
    remainingUnselectedCount: ranked.length - 160,
    dyadicOnly: true,
  ...
  nextAuthorizedAction: "prepare-batch-16-standing-authorization-and-source-packets-under-user-authorization"
};
...
const analysis = {
  ...
  status: "sixteenth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization",
  analyzedAt: frozenAt,
  ...
  finding: "Successful Batch 15 production publication was authenticated at the pushed atomic commit. The immutable campaign rank then selected ordinals 151 through 160 as ten unique pending dyadic debates, and all thirty effective source files and ten canonical event chains passed without replacement.",
  decision: {
