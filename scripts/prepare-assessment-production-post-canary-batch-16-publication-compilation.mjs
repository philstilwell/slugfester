...
import {
  POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_ORDER,
  POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_PROTOCOL_ID,
  POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_ROOT,
  compilePostCanaryBatch16PublicationStagingRecord,
  validatePostCanaryBatch16CompiledStagingRecord
} from "./lib/assessment-production-post-canary-batch-16-publication-compilation.mjs";
import { POST_CANARY_BATCH_16_PUBLICATION_ROOT } from
  "./lib/assessment-production-post-canary-batch-16-publication.mjs";
...
const COHORT = `${POST_CANARY_BATCH_16_PUBLICATION_ROOT}/failure-recovery/finalized/complete-publication-overlay.json`;
...
const ACCEPTED_OUTPUTS = Object.freeze(Object.fromEntries(
  POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_ORDER.map((debateNumber) => [
    debateNumber,
    `${POST_CANARY_BATCH_16_PUBLICATION_ROOT}/outputs/debate-${debateNumber}.json`
  ])
));
const acceptedOutput = (debateNumber) => ACCEPTED_OUTPUTS[debateNumber];
const packetPath = (debateNumber) =>
  `${POST_CANARY_BATCH_16_PUBLICATION_ROOT}/packets/debate-${debateNumber}.json`;
...
assertV4(
  cohort.status ===
    "complete-ten-debate-batch-16-publication-cohort-valid-after-bounded-recovery" &&
  cohort.debates?.length === 10 &&
  cohort.debates.reduce((sum, row) => sum + row.validation.moves, 0) === 194 &&
  cohort.debates.reduce((sum, row) => sum + row.validation.critiques, 0) === 194 &&
...
"the accepted Batch 16 publication cohort changed");
...
for (const debateNumber of POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_ORDER) {
  ...
  const validation = validatePostCanaryBatch16PublicationOutput(output, packet);
  ...
  const compiled = compilePostCanaryBatch16PublicationStagingRecord({ output, packet, identity });
  const compiledValidation = validatePostCanaryBatch16CompiledStagingRecord({ compiled, output, packet, identity });
...
assertV4(contexts.length === 10 && contexts.reduce((sum, row) => sum + row.expectedMoves, 0) === 194,
"the Batch 16 compilation cohort changed");
const identitySnapshot = { schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-compilation-identity-snapshot",
  protocolId: POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_PROTOCOL_ID,
...
const sources = [COHORT, POST_CANARY_BATCH_16_STANDING_AUTHORIZATION,
  ...
  "scripts/prepare-assessment-production-post-canary-batch-16-publication-compilation.mjs",
  "scripts/test-assessment-production-post-canary-batch-16-publication-compilation-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-16-publication-compilation.mjs",
  "scripts/run-assessment-production-post-canary-batch-16-publication-compilation.mjs",
...
const manifest = { schemaVersion: "1.0-assessment-production-post-canary-batch-16-deterministic-publication-compilation-preparation",
  protocolId: POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_PROTOCOL_ID,
  status: "frozen-post-canary-batch-16-deterministic-publication-compilation-prepared",
  ...
  explicitOrder: POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_ORDER, contexts,
  ...
  nextAuthorizedAction: "activate-one-batch-16-deterministic-publication-compilation-pass" };
