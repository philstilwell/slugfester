...
const preparationPath =
  `${POST_CANARY_BATCH_07_COMPATIBILITY_ROOT}/preparation-manifest.json`;
const analysisPath =
  `${POST_CANARY_BATCH_07_COMPATIBILITY_ROOT}/analysis.json`;
...
assert.equal(
  ...
  "batch-07-site-ledger-adapter-and-validator-route"
);
assert.equal(
...
assert.equal(preparation.totals.overallBlunders, 56);
...
assert.equal(
  await exists(`${POST_CANARY_BATCH_07_COMPATIBILITY_ROOT}/execution.json`),
  false
);
assert.equal(
  await exists(
...
assert.deepEqual(packetNumbers, POST_CANARY_BATCH_07_COMPATIBILITY_ORDER);
for (const packetRecord of preparation.artifacts.packets) {
  ...
  const positive = validatePostCanaryBatch07SiteLedgerAdapter({
