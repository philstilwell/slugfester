...
const scoreArtifactCandidates = [
  `${POST_CANARY_BATCH_14_FINAL_LEDGER_ROOT}/calculated-scores.json`,
  `${POST_CANARY_BATCH_14_FINAL_LEDGER_ROOT}/score-analysis.json`,
  `${POST_CANARY_BATCH_14_FINAL_LEDGER_ROOT}/score-pass-manifest.json`
];
...
const rebuiltLedger = buildPostCanaryBatch14FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
...
assert.deepEqual(
  ...
  ["53", "69", "110", "133", "12", "187", "160", "55", "106", "57"]
);
assert.equal(storedLedger.audit.finalMoves, 190);
assert.equal(storedLedger.audit.disputedMoves, 181);
assert.equal(storedLedger.audit.pairSelections, 183);
assert.equal(storedLedger.audit.scoringFieldSelections, 338);
assert.equal(storedLedger.audit.burdenAdjustmentSelections, 18);
assert.equal(storedLedger.audit.candidateSelections, 539);
assert.equal(storedLedger.audit.meanMerges, 394);
assert.equal(storedLedger.audit.dependencyMeanMergesSuppressed, 0);
assert.equal(storedLedger.audit.audioVerifiedMoves, 12);
...
assert.throws(() =>
  buildPostCanaryBatch14FinalLedger(mutatedInputs, inputs.sourceHashes)
);
...
assert.equal(analysis.totals.finalMoves, 190);
assert.equal(analysis.totals.disputedMoves, 181);
assert.equal(analysis.totals.candidateSelections, 539);
