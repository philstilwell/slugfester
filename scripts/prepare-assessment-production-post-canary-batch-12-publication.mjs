...
import {
  buildPostCanaryBatch12PublicationPacket,
  buildPostCanaryBatch12PublicationSchema,
  postCanaryBatch12ReferenceCatalog,
  POST_CANARY_BATCH_12_PUBLICATION_DEBATES,
  POST_CANARY_BATCH_12_PUBLICATION_MODEL,
...
const ROOT = POST_CANARY_BATCH_12_PUBLICATION_ROOT;
...
const SELECTION =
  "docs/assessment-production/post-canary-continuation-v1/batch-12/selection.json";
const FINAL_LEDGER =
...
const FINAL_LEDGER_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-12/final-ledger/final-ledger-manifest.json";
const FINAL_LEDGER_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-12/final-ledger/analysis.json";
const SCORES =
  "docs/assessment-production/post-canary-continuation-v1/batch-12/score-pass/calculated-scores.json";
const SCORE_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-12/score-pass/score-pass-manifest.json";
const SCORE_PREPARATION =
  "docs/assessment-production/post-canary-continuation-v1/batch-12/score-pass/score-pass-preparation-manifest.json";
const SCORE_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-12/score-pass/analysis.json";
const SOURCE_PACKET_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-12/independent-judgments/source-packets";
...
const STATIC_SOURCE_FILES = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-12-score-gate.mjs",
  "scripts/lib/assessment-production-post-canary-batch-12-publication.mjs",
  "scripts/lib/assessment-production-post-canary-batch-12-publication-validation.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-12-publication.mjs",
  "scripts/test-assessment-production-post-canary-batch-12-publication-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-12-publication.mjs",
  "scripts/run-assessment-production-post-canary-batch-12-publication.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-12-publication.mjs"
];
...
assertV4(
  scoreAnalysis.status === "post-canary-batch-12-score-stability-gate-passed" &&
    scoreAnalysis.productionCanary === false &&
    scoreAnalysis.batchNumber === 12 &&
    ...
    scoreAnalysis.nextAuthorizedAction ===
      "standing-authorization-permits-batch-12-publication-packet-preparation",
  "the accepted Batch 12 score gate is unavailable or its stop boundary changed"
);
assertV4(
  scores.status === "post-canary-batch-12-single-score-pass-stability-gate-passed" &&
  ...
  "the locked Batch 12 score artifact is unavailable"
);
assertV4(
  scoreManifest.status ===
      "frozen-post-canary-batch-12-single-deterministic-score-pass-authorized" &&
    ...
    scorePreparation.status ===
      "frozen-post-canary-batch-12-single-deterministic-score-pass-prepared-not-activated",
  "the frozen Batch 12 single-pass score controls changed"
);
assertV4(
  ledger.status ===
      "passed-post-canary-batch-12-deterministic-final-ledger-assembly" &&
    ledger.debates?.length === 10 &&
    ledger.audit?.finalMoves === 204 &&
    ...
    ledgerManifest.status ===
      "frozen-ten-debate-post-canary-batch-12-deterministic-final-ledger-assembly" &&
    ledgerAnalysis.status ===
      "post-canary-batch-12-deterministic-final-ledger-gate-passed",
  "the locked Batch 12 final ledger changed"
);
assertV4(
  productionManifest.model?.label === POST_CANARY_BATCH_12_PUBLICATION_MODEL.label &&
    productionManifest.model?.slug === POST_CANARY_BATCH_12_PUBLICATION_MODEL.slug &&
    productionManifest.model?.reasoningEffort ===
      POST_CANARY_BATCH_12_PUBLICATION_MODEL.reasoningEffort &&
    productionManifest.model?.authentication ===
      POST_CANARY_BATCH_12_PUBLICATION_MODEL.authentication,
  "the frozen production model or authentication changed"
);
assertV4(
  selection.selected?.map((item) => item.debateNumber).join(",") ===
      POST_CANARY_BATCH_12_PUBLICATION_DEBATES.join(",") &&
    selection.selected.every((item) => item.speakerCount === 2) &&
    selection.authorization?.productionMutation === false,
  "the frozen Batch 12 selection changed"
);
assertV4(
  ledger.debates.map((debate) => debate.debateNumber).join(",") ===
    ...
    scores.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_12_PUBLICATION_DEBATES.join(","),
  "the Batch 12 ledger or score order changed"
);
assertV4(
...
const finalLedgerInputs = await loadPostCanaryBatch12FinalLedgerInputs();
validatePostCanaryBatch12FinalLedger(
  ledger,
...
const catalog = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-12-local-reference-catalog",
  references: postCanaryBatch12ReferenceCatalog()
};
...
for (const debateNumber of POST_CANARY_BATCH_12_PUBLICATION_DEBATES) {
  ...
  const packet = buildPostCanaryBatch12PublicationPacket({
    ledgerDebate,
    scoreDebate,
    sourcePacket,
  ...
  const schema = buildPostCanaryBatch12PublicationSchema(packet);
...
assertV4(
  contexts.reduce((sum, context) => sum + context.moves, 0) === 204,
  "Batch 12 publication move coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.sections, 0) === 54,
  "Batch 12 publication section coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.audioVerifiedMoves, 0) === 3,
  "Batch 12 audio-verification coverage changed"
);
assertV4(
  contexts.every(
    (context) =>
  ...
  "Batch 12 publication section or quote eligibility is invalid"
);
...
const manifest = {
  ...
  userAuthorization: {
    instruction:
      "The frozen Batch 12 standing authorization permits preparation, activation, and execution of exactly ten score-locked publication-reconstruction contexts while every gate passes, with direct incremental cost capped at $0.",
  ...
  model: structuredClone(POST_CANARY_BATCH_12_PUBLICATION_MODEL),
  costEstimate: {
  ...
  acceptanceContract: {
    ...
    movesAuthoredRequired: 204,
    critiquesRequired: 204,
