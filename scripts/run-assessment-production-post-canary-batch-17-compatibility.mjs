...
assertV4(write, "Batch 17 compatibility runner requires --write");
...
const paths = {
  root: POST_CANARY_BATCH_17_COMPATIBILITY_ROOT,
...
assertV4(
  ...
  "Batch 17 compatibility pass was already attempted; rerun forbidden"
);
...
assertV4(
  ...
  "frozen Batch 17 compatibility activation is unavailable"
);
...
tests.push(
  runCommand(
    "batch-17-route-positive-negative-controls",
    process.execPath,
...
if (tests.at(-1).passed) {
  tests.push(
    runCommand("checkpoint-batch-01-through-batch-16-legacy-reference-regression", process.execPath, [
...
const nextAuthorizedAction = compatibilityAcceptancePassed
  ? "prepare-batch-17-production-publication-mutation-manifest-under-standing-authorization"
  : "diagnose-batch-17-compatibility-failure-under-failure-recovery-standing-authorization";
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-17-compatibility-execution",
  protocolId: preparation.protocolId,
  status: compatibilityAcceptancePassed
    ? "batch-17-route-score-checkpoint-batch-01-through-batch-16-legacy-reference-validation-passed"
    : "failed-closed-batch-17-compatibility-staging-pass",
  startedAt,
  completedAt,
  activation: {
  ...
  regressions: {
    batch17RouteControlsPassed: tests[0]?.passed === true,
    checkpointBatch01ThroughBatch16LegacyReferencePassed:
      tests[1]?.passed === true,
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-17-compatibility-execution-analysis",
  protocolId: preparation.protocolId,
  status: compatibilityAcceptancePassed
    ? "batch-17-production-compatibility-passed"
    : "batch-17-production-compatibility-failed-closed",
  ...
  checks: {
    ...
    routeAndScoreControlsPassed: execution.regressions.batch17RouteControlsPassed,
    checkpointBatch01ThroughBatch16LegacyReferencePassed:
      execution.regressions.checkpointBatch01ThroughBatch16LegacyReferencePassed,
