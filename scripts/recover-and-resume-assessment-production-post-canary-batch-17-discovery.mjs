...
if (command === "prepare") await prepareRecovery();
else if (command === "activate") await activateRecovery();
else if (command === "run") await executeRecovery();
else if (command === "analyze") await analyzeRecovery();
else if (command === "validate") await validateRecovery();
else {
  throw new Error(
    "usage: recover-and-resume-assessment-production-post-canary-batch-17-discovery.mjs <prepare|activate|run|analyze|validate>"
  );
}
