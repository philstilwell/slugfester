...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-03/discovery";
...
assert.equal(manifest.status, "frozen-thirty-eight-post-canary-batch-03-discovery-contexts-prepared-not-authorized");
...
assert.deepEqual(manifest.contexts.map((context) => context.contextIndex), Array.from({ length: 38 }, (_, index) => index));
...
assert.equal(manifest.executionPolicy.copiedInputBytesMaximum, 82227);
assert.equal(preparation.totals.maximumCopiedInputBytes, 82227);
...
assert.equal(manifest.copiedInputBoundary.frozenObservedCopiedInputBytesMaximum, 82227);
assert.equal(manifest.copiedInputBoundary.executionCopiedInputBytesMaximum, 82227);
assert.deepEqual(manifest.copiedInputBoundary.maximumContext, { debateNumber: "150", chunkId: "chunk-001" });
...
for (const context of manifest.contexts) {
  ...
  assert(buildBatch03TokenCountedChunkLedger(chunkBytes).equals(tokenBytes));
...
assert.equal(manifest.nextAuthorizedAction, "prepare-and-freeze-batch-03-discovery-execution-activation-under-standing-authorization");

const validation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-discovery-execution-preparation-validation",
  protocolId: manifest.protocolId,
  status: "batch-03-discovery-execution-manifest-validation-passed-frozen-standing-authorization-active",
