...
const SCRIPT =
  "scripts/run-assessment-production-post-canary-batch-01-inventory-plans.mjs";
const PREPARER =
  "scripts/prepare-assessment-production-post-canary-batch-01-inventory.mjs";
const CONTEXTS = 10;
const DEBATES = ["31", "94", "52", "146", "91", "175", "75", "72", "13", "195"];
...
const ACTIVATION_STATUS =
  "frozen-ten-post-canary-batch-01-candidate-census-plan-contexts-authorized";
const EXECUTION_STATUS =
  "ten-post-canary-batch-01-candidate-census-plan-contexts-passed";
const PROTOCOL_ID =
...
async function loadAndValidatePreparation({ requireFutureAbsent = false } = {}) {
  ...
  assertV4(
    ...
    "Batch 1 candidate-census planner preparation is invalid"
  );
  assertV4(
...
async function preflight() {
  ...
  assertV4(
    ...
    "frozen Batch 1 preparation validator failed"
  );
  console.log(
    JSON.stringify(
      {
...
async function activate() {
  ...
  const activation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-01-candidate-census-plan-execution-activation",
    ...
    userAuthorization: {
      instruction:
        "I approve activation and execution of exactly the ten frozen Batch 1 candidate-census planner contexts using 5.6 Sol with low reasoning effort through my ChatGPT subscription, with a direct incremental cost cap of $0. Use the frozen 1→2 scheduler and one attempt per context, then stop after deterministic plan validation, analysis, commit, and push. Do not prepare or execute side-selector packets or models, judgment models, paid services, audio work, score derivation, publication reconstruction, or production mutation.",
    ...
    nextRequiredAction:
      "execute-exactly-ten-frozen-batch-01-candidate-census-plan-contexts-once",
  };
...
async function loadAndValidateActivation({ requireFutureAbsent = false } = {}) {
  ...
  assertV4(
    ...
    "Batch 1 candidate-census planner execution is unauthorized"
  );
...
async function execute() {
  ...
  async function executeContext(context, contextIndex) {
    ...
    const sourceDirectory = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-post-canary-batch-01-candidate-plan-${context.debateNumber}-`
      )
    );
    const isolatedCodexHome = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-post-canary-batch-01-candidate-plan-home-${context.debateNumber}-`
      )
    );
    ...
    process.stdout.write(
      `[post-canary-batch-01-candidate-plan] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
    );
    return record;
  }
  ...
  const execution = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-01-candidate-census-plan-model-execution",
    protocolId: PROTOCOL_ID,
    status:
      validContexts === CONTEXTS
        ? EXECUTION_STATUS
        : "post-canary-batch-01-candidate-census-plan-gate-complete-with-failure",
    productionContinuation: true,
    developmentValidationOnly: false,
    productionCanary: false,
    stagingOnly: true,
    ...
    nextRequiredAction:
      validContexts === CONTEXTS
        ? "analyze-and-replay-ten-batch-01-candidate-census-plans-model-free-only"
        : "stop-preserve-failed-batch-01-candidate-census-plan-gate",
  };
...
async function analyze() {
  ...
  const analysis = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-01-candidate-census-plan-analysis",
    protocolId: PROTOCOL_ID,
    status:
      "post-canary-batch-01-candidate-census-plan-gate-passed-awaiting-separate-side-packet-preparation-approval",
    productionContinuation: true,
    developmentValidationOnly: false,
    productionCanary: false,
    stagingOnly: true,
    ...
    nextAuthorizedAction:
      "user-approval-required-before-preparing-and-freezing-twenty-exact-batch-01-side-selector-packets-or-any-further-model-execution",
  };
...
if (command === "preflight") await preflight();
  ...
  throw new Error(
    "usage: run-assessment-production-post-canary-batch-01-inventory-plans.mjs <preflight|activate|run|analyze|validate>"
  );
}
