...
const discovery = JSON.parse(
  await readFile(preparation.inputs.discoveryAnalysis, "utf8")
);
assert.equal(
  preparation.status,
...
assert.equal(
  preparation.transport.semanticCandidateDownselectionPerformed,
  false
);
assert.equal(
...
assert.equal(preparation.transport.parsedSemanticIdentityVerified, true);
assert.equal(
  preparation.transport.compactSerializationChosenBeforeInventoryModelExecution,
  true
);
assert.deepEqual(preparation.transport.prettyCeilingFailureDebates, ["158"]);
assert.equal(preparation.transport.maximumPrettyCopiedInputBytes, 115076);
assert.equal(preparation.transport.maximumCopiedInputBytes, 98839);
assert(
...
assert.equal(
  sha256(await readFile(preparation.inputs.discoveryAnalysis)),
  preparation.inputs.discoveryAnalysisSha256
);
assert.equal(
  sha256(await readFile(preparation.inputs.sourcePreparation)),
  preparation.inputs.sourcePreparationSha256
);
assert.equal(
  sha256(await readFile(preparation.inputs.manual)),
  preparation.inputs.manualSha256
);
for (const [file, digest] of Object.entries(preparation.sourceHashes)) {
  assert.equal(sha256(await readFile(file)), digest, `${file}: source drift`);
}
const discoveryByNumber = new Map(
  discovery.debates.map((debate) => [debate.debateNumber, debate])
);
...
for (const context of preparation.contexts) {
  ...
  assert.equal(
    validateV4221162InventoryCandidateTransport(transport, fullEvidence).status,
    "passed"
  );
  assert.deepEqual(
    fullEvidence,
    buildV422115EvidenceBundle(candidateBundle, eventsDocument)
  );
  assert.deepEqual(
    transport,
  ...
  assert(transportBytes.equals(Buffer.from(`${JSON.stringify(transport)}\n`)));
  assert(schemaBytes.equals(Buffer.from(`${JSON.stringify(schema)}\n`)));
  assert.equal(transport.candidateCount, context.candidates);
  assert.equal(transport.candidates.length, context.candidates);
  assert.equal(
    new Set(
      transport.candidates.map((candidate) => candidate.qualifiedCandidateId)
    ).size,
    context.candidates
  );
  assert(
    transport.candidates.every(
      (candidate) => candidate.candidateEvidence.sourceExact === true
    )
  );
  assert(
    transport.candidates.every(
      (candidate) => !Object.hasOwn(candidate, "attributionConfidence")
    )
  );
  assert.equal(
  ...
  assert.equal(
    context.prettyCopiedInputBytes - context.copiedInputBytes,
    context.compactSerializationSavingsBytes
  );
...
assert.equal(candidateTotal, 406);
  ...
  JSON.stringify(
    {
      ...
      maximumPrettyCopiedInputBytes:
        preparation.transport.maximumPrettyCopiedInputBytes,
      maximumCompactCopiedInputBytes:
        preparation.transport.maximumCopiedInputBytes,
      everyCandidateRetained: true,
      exactEvidenceReplay: true,
      compactSemanticIdentity: true,
