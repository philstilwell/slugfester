...
assert.equal(diagnosis.failure.debateNumber, "189");
assert.equal(diagnosis.failure.sourceVideoId, "3DHvNRK452c");
assert.equal(
...
assert.equal(diagnosis.preservedFiles.length, 5);
...
assert.equal(diagnosis.authorization.correctionPreparation, false);
  ...
  JSON.stringify(
    {
      ...
      publicSourceAttemptsConsumed: 2,
      retries: 0,
