...
import { V421_ROOT, compileV421PrimaryOutput, validateV421SourceLedger } from "./lib/v421-compact-fresh.mjs";

const root = process.cwd();
const manifest = JSON.parse(await readFile(path.resolve(root, `${V421_ROOT}/primary-execution-manifest.json`), "utf8"));
const codex = "/Applications/ChatGPT.app/Contents/Resources/codex";
const authSource = path.join(os.homedir(), ".codex", "auth.json");
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
assertV4(manifest.status === "frozen-six-compact-context-primary-authorized" && manifest.authorization.primaryModelExecution && !manifest.authorization.passBModelExecution, "v4.2.1 primary execution unauthorized");
...
for (const context of manifest.contexts) {
  const [packet, eventsBytes, ledgerBytes] = await Promise.all([readFile(path.resolve(root, context.packet), "utf8").then(JSON.parse), readFile(path.resolve(root, context.originalEvents)), readFile(path.resolve(root, context.sourceLedger))]);
  validateV421SourceLedger(ledgerBytes, JSON.parse(eventsBytes), packet.transportChain.sourceLedgerSha256);
}
...
const results = [];
for (const context of manifest.contexts) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-v421-primary-${context.debateNumber}-`));
  const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), `slugfester-v421-home-primary-${context.debateNumber}-`));
  ...
  try {
    ...
    const prompt = `Read rubric-base.md, rubric-derived.md, rubric-bounded.md, manual.md, packet.json, schema.json, and every line of source-ledger.jsonl completely; read no other files. The manual governs operational conflicts. Act only as the fresh isolated compact-transport primary judge for Debate ${context.debateNumber}. The JSONL ledger is the complete timestamped transcript. Build four to six genuinely contested sections and the minimum eight-to-twenty-four moves needed to preserve every load-bearing route and decisive exchange, with one or two actual moves per side per section. For every sourceSpan, return only startEvent, endEvent, and an exact or near-exact excerpt copied from that inclusive ledger range in source order. Every excerpt must contain 12 to 100 lexical tokens and no more than 600 characters; shorten it before submission if either limit may be exceeded. Never return startMs or endMs. Verify every excerpt against source-ledger.jsonl. Compression must never raise a rating and selection must never depend on likely score. Do not supply sequence numbers; chronology and times are repository-derived. Silently run the source-span, excerpt-size, response, burden-reference, rating-band, charity, adjustment-exclusion, and duplicate-exclusion consistency checklist before submission. Use medium or low attribution confidence whenever speaker identity is not secure, and never claim audio verification occurred. Do not calculate totals, identify a winner, write publication prose, or supply precision/clarity or epistemic-calibration scalar values. Control selection, legacy assessments, prior scores, winners, other debates, all earlier primary outputs, and high-effort references are unavailable. Return exactly one schema-conforming JSON object and no commentary.`;
    process.stdout.write(`[v4.2.1-primary] starting ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber}\n`);
    ...
    if (invocation.timedOut || invocation.code !== 0 || invocation.signal !== null || !resultExists) {
      record = { ...base, status: invocation.timedOut ? "timed-out" : !resultExists ? "result-missing" : "transport-failed", rawOutputWritten: false, compiledOutputWritten: false, deterministicValidationPassed: false, gateAcceptancePassed: false };
    } else {
      ...
      const validation = await run(process.execPath, ["scripts/validate-v421-primary-output.mjs", context.rawOutput, context.packet], { cwd: root, env: process.env }, 120000);
      ...
      if (valid) {
        const [rawOutput, packet, eventsBytes] = await Promise.all([readFile(path.resolve(root, context.rawOutput), "utf8").then(JSON.parse), readFile(path.resolve(root, context.packet), "utf8").then(JSON.parse), readFile(path.resolve(root, context.originalEvents))]);
        const compiled = compileV421PrimaryOutput(rawOutput, packet, JSON.parse(eventsBytes));
  ...
  results.push(record);
  process.stdout.write(`[v4.2.1-primary] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
