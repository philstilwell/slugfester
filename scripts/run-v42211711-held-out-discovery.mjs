...
assertV4(
  manifest.status === "frozen-seventeen-held-out-discovery-contexts-authorized"
...
async function executeContext(context, contextIndex) {
  ...
  try {
    ...
    process.stdout.write(`[v4.2.21.17.11] starting ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber} ${context.chunkId}\n`);
  ...
  process.stdout.write(`[v4.2.21.17.11] Debate ${context.debateNumber} ${context.chunkId} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
...
const execution = {
  schemaVersion: "4.2.21.17.11-held-out-discovery-model-execution",
  protocolId: manifest.protocolId,
  status: validContexts === manifest.contexts.length
    ? "seventeen-held-out-discovery-contexts-passed"
    : "held-out-discovery-complete-with-failure",
  heldOut: true,
  gateStartedAt,
