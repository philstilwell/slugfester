...
assert(manifest.status === "frozen-one-context-authorized" && manifest.authorization.conditionalAdjudicationModelExecution && manifest.authorization.modelContexts === 1, "conditional execution not authorized");
assert(!manifest.authorization.scoringModelExecution && !manifest.authorization.assessmentProse && !manifest.stopRules.furtherAutomaticRetryAuthorized, "conditional boundary invalid");
...
const temporary = await mkdtemp(path.join(os.tmpdir(), "slugfester-v388-conditional-"));
const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), "slugfester-v388-home-conditional-"));
...
try {
  ...
  const prompt = "Read workflow.md, rubric.md, manual.md, packet.json, and schema.json completely and no other files. Act only as a fresh isolated conditional-field adjudicator. Decide the single proposition field and select exactly one supplied anonymous option. Do not infer pass identity, revisit candidate validity, merge options, add a third value, emit progress commentary, or emit provisional JSON. Return exactly one final schema-conforming JSON object. Do not classify burden contact, assign sections, weights, or importance, score participants, infer a winner, reconstruct prose, write Overall Commentary, or write an AI Extension.";
  process.stdout.write("\n[v3.8.8-coverage-conditional] starting 5.6 Sol Debate 55 (1 field)\n");
  ...
  if (invocation.timedOut || invocation.code !== 0 || invocation.signal !== null) record = { ...base, status: invocation.timedOut ? "timed-out" : "transport-failed", outputWritten: false, deterministicValidationPassed: false, gateAcceptancePassed: false };
    ...
    const validation = await run(process.execPath, ["scripts/validate-v388-coverage-conditional-adjudication.mjs", manifest.artifacts.output, manifest.modelInputs.packet, manifest.modelInputs.schema], { cwd: root, env: process.env });
...
const execution = { schemaVersion: "3.8.8-coverage-conditional-adjudication-model-execution", protocolId: manifest.protocolId, stage: manifest.stage, startedAt, completedAt: new Date().toISOString(), contextsPlanned: 1, validOutputContexts: record.gateAcceptancePassed ? 1 : 0, totalAttempts: 1, totalRetries: 0, meteredApiCostUsd: 0, transcriptionCostUsd: 0, results: [record] };
await writeFile(path.resolve(root, manifest.artifacts.execution), `${JSON.stringify(execution, null, 2)}\n`);
assert(record.gateAcceptancePassed, "conditional adjudication failed; consensus merge remains blocked");
console.log(JSON.stringify({ status: "v3.8.8-coverage-conditional-adjudication-passed", validOutputContexts: 1, disputedFields: 1, transportClassification: record.transportClassification, meteredApiCostUsd: 0, deterministicConsensusMergeAuthorized: true, scoringAuthorized: false }, null, 2));
