...
assertV4(
  activation.status === "frozen-one-context-three-field-debate-107-publication-correction-activated" &&
    activation.authorization?.modelExecution === true &&
    activation.userAuthorization?.explicitThreeWritableFieldException === true &&
    canonicalJson(activation.context?.writableFields) ===
      canonicalJson(TARGETS.map((target) => target.field)),
  "Debate 107 correction execution is not activated"
);
assertV4(
  ...
  "correction execution policy changed"
);
...
const temporary = await mkdtemp(path.join(os.tmpdir(), "batch-10-debate-107-three-field-correction-"));
const isolatedCodexHome = await mkdtemp(path.join(os.tmpdir(), "batch-10-debate-107-three-field-correction-home-"));
...
try {
  ...
  const prompt = [
    ...
    "Act only as the isolated Debate 107 three-field AI-extension novelty-explanation correction editor.",
    "The user explicitly authorized one context for all three listed fields as a narrow exception to the normal two-field repair-packet limit.",
    "Author exactly the three novelty.explanation strings in the supplied fixed order; item text, classification, sourceMoveIds, and every other field are unavailable and immutable.",
    ...
    "The rejected explanations and failed publication output are unavailable. Do not infer or recreate them.",
  ...
  process.stdout.write("[batch-10-debate-107-correction] starting one fresh three-field context\n");
  ...
  if (invocation.error || invocation.timedOut || invocation.code !== 0 || invocation.signal !== null || !resultExists) {
    execution = {
      schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-debate-107-correction-execution",
    ...
    const validation = {
      schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-debate-107-correction-validation",
      protocolId: activation.protocolId,
    ...
    const provenance = {
      schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-debate-107-correction-provenance",
      protocolId: activation.protocolId,
      contextIndex: 0,
      debateNumber: "107",
