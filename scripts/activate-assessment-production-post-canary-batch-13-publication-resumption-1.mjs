...
assertV4(!(await exists(ACTIVATION)), "seven-context resumption activation already exists");
const preparationBytes = await readFile(path.resolve(PREPARATION));
const preparation = JSON.parse(preparationBytes);
assertV4(
  preparation.protocolId === PROTOCOL_ID &&
    preparation.status === "frozen-seven-original-unattempted-batch-13-publication-contexts-prepared-not-activated" &&
    preparation.contexts?.length === 7 && preparation.totals?.moves === 141 &&
  ...
  "seven original unattempted contexts are not prepared"
);
assertV4(
  ...
  "seven-context model or execution controls changed"
);
assertV4(
...
const activation = {
  ...preparation,
  schemaVersion: "1.0-assessment-production-post-canary-batch-13-publication-original-unattempted-context-resumption-activation",
  status: "frozen-seven-original-unattempted-batch-13-publication-contexts-activated",
  activatedAt,
  ...
  nextAuthorizedAction: "execute-only-seven-frozen-original-unattempted-publication-contexts"
};
