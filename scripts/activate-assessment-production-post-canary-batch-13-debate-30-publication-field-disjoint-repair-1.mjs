...
assertV4(
  preparation.protocolId === PROTOCOL_ID &&
    preparation.status === "frozen-nine-context-seventeen-field-batch-13-debate-30-publication-repair-prepared-not-activated" &&
    preparation.contexts?.length === 9 && preparation.totals?.writableFields === 17 &&
    preparation.diagnosis?.totals?.invalidCritiques === 16 &&
    preparation.diagnosis?.totals?.invalidQuotes === 1 &&
    preparation.contexts.every((context) => context.writableFieldCount <= 2) &&
    new Set(preparation.contexts.flatMap((context) => context.writableFields)).size === 17,
  "nine-shard Debate 30 repair is not prepared"
);
assertV4(
  ...
  "nine-shard model or execution controls changed"
);
...
const activation = {
  ...
  status: "frozen-nine-context-seventeen-field-batch-13-debate-30-publication-repair-activated",
  activatedAt,
  ...
  nextAuthorizedAction: "execute-only-nine-frozen-debate-30-field-disjoint-repair-contexts"
};
