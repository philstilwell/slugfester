...
import { V3810_PERFORMANCE_ROOT, assertV3810 } from "./lib/v3810-performance-judgment.mjs";

const root = process.cwd();
const manifest = JSON.parse(await readFile(path.resolve(root, `${V3810_PERFORMANCE_ROOT}/initial-execution-manifest.json`), "utf8"));
const codex = "/Applications/ChatGPT.app/Contents/Resources/codex";
const authSource = path.join(os.homedir(), ".codex", "auth.json");
const bytes = (relativePath) => readFile(path.resolve(root, relativePath));
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
assertV3810(manifest.status === "frozen-six-context-authorized" && manifest.authorization.initialPerformanceModelExecution && !manifest.authorization.scoreDerivation && !manifest.stopRules.furtherAutomaticRetryAuthorized, "initial performance execution unauthorized");
for (const [relativePath, digest] of Object.entries(manifest.sourceHashes)) assertV3810(sha256(await bytes(relativePath)) === digest, `source hash mismatch: ${relativePath}`);
for (const output of manifest.futureOutputPathsExcludedFromSourceHashes) {
  try {
    await access(path.resolve(root, output));
    throw new Error(`future output already exists: ${output}`);
  } catch (error) {
...
for (const context of manifest.contexts) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-v3810-performance-${context.debateNumber}-${context.pass}-`));
  const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), `slugfester-v3810-home-performance-${context.debateNumber}-${context.pass}-`));
  ...
  try {
    ...
    const prompt = `Read workflow.md, rubric.md, manual.md, packet.json, schema.json, transcript.txt, and events.json completely and no other files. Act only as the fresh isolated Pass ${context.pass} performance judge for Debate ${context.debateNumber}. Judge every locked move exactly once and in packet order. Apply the response component tests, locked burden band, seven dimension anchors, explicit charity-tested rule, and burden-adjustment exclusion rule literally. Return exactly one final schema-conforming JSON object with no progress commentary or provisional JSON. Copy locked fields exactly. Do not calculate or emit move, section, overall, or participant totals; do not name a winner; and do not write participant critiques, Overall Commentary, AI Extension, or publication prose. The other pass and all legacy v3.8.8 and v3.8.9 judgments, scores, prose, and gate conclusions are unavailable.`;
    process.stdout.write(`\n[v3.8.10-performance] starting ${manifest.model.label} Debate ${context.debateNumber} Pass ${context.pass}\n`);
    ...
    if (invocation.timedOut || invocation.code !== 0 || invocation.signal !== null) record = { ...base, status: invocation.timedOut ? "timed-out" : "transport-failed", outputWritten: false, deterministicValidationPassed: false, gateAcceptancePassed: false };
      ...
      const validation = await run(process.execPath, ["scripts/validate-v3810-performance-judgment-output.mjs", context.output, context.packet, context.pass], { cwd: root, env: process.env });
...
const execution = { schemaVersion: "3.8.10-performance-judgment-initial-model-execution", protocolId: manifest.protocolId, stage: manifest.stage, startedAt: results[0]?.startedAt ?? null, completedAt: new Date().toISOString(), contextsPlanned: 6, validOutputContexts: results.filter((item) => item.gateAcceptancePassed).length, moveJudgmentsAcrossPasses: results.reduce((sum, item) => sum + (item.gateAcceptancePassed ? item.moveCount : 0), 0), totalAttempts: results.length, totalRetries: 0, meteredApiCostUsd: 0, transcriptionCostUsd: 0, results };
await writeFile(path.resolve(root, manifest.artifacts.execution), `${JSON.stringify(execution, null, 2)}\n`);
assertV3810(execution.validOutputContexts === 6 && execution.moveJudgmentsAcrossPasses === 162, "initial performance execution failed; disagreement extraction remains blocked");
console.log(JSON.stringify({ status: "v3.8.10-performance-initial-passed", validOutputContexts: 6, debates: 3, moves: 81, moveJudgmentsAcrossPasses: 162, totalRetries: 0, meteredApiCostUsd: 0, disagreementExtractionAuthorized: true, scoreDerivationAuthorized: false }, null, 2));
