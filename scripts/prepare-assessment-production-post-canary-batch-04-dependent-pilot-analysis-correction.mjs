...
import {
  ...
  BATCH_04_DEPENDENT_PILOT_OUTPUT_PATHS,
  jsonLeafChanges,
...
const diagnosisPath = `${POST_CANARY_BATCH_04_GENERATED_SEO_CORRECTION_ROOT}/validation-failure-diagnosis.json`;
const seoPlanPath = `${POST_CANARY_BATCH_04_GENERATED_SEO_CORRECTION_ROOT}/correction-plan.json`;
const seoActivationPath = `${POST_CANARY_BATCH_04_GENERATED_SEO_CORRECTION_ROOT}/execution-activation.json`;
...
const executionAnalysisPath = `${BATCH_04_DEPENDENT_PILOT_CORRECTION_ROOT}/analysis.json`;
...
assertV4(
  diagnosis.status === "frozen-batch-04-generated-seo-full-check-dependent-pilot-analysis-staleness-diagnosed" &&
    diagnosis.recoveryBoundary.writablePathsMaximum === 2 &&
    seoActivation.status === "frozen-batch-04-generated-seo-correction-pass-activated" &&
    seoActivation.plan.sha256 === sha256(await readBytes(seoPlanPath)),
  "accepted Batch 4 generated SEO validation-failure diagnosis required"
);
...
const baselineOutputs = await Promise.all(BATCH_04_DEPENDENT_PILOT_OUTPUT_PATHS.map(lockFile));
...
const affectedNumbers = new Set(["03", "81"]);
...
assertV4(
  ...
  "pilot JSON changed outside aggregate legacy comparison and Debates 03/81 rows"
);

const toolPaths = [
  "scripts/lib/assessment-production-post-canary-batch-04-dependent-pilot-analysis-correction.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-04-dependent-pilot-analysis-correction.mjs",
  "scripts/test-assessment-production-post-canary-batch-04-dependent-pilot-analysis-correction-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-04-dependent-pilot-analysis-correction.mjs",
  "scripts/test-assessment-production-post-canary-batch-04-dependent-pilot-analysis-correction-activation.mjs",
  "scripts/run-assessment-production-post-canary-batch-04-dependent-pilot-analysis-correction.mjs"
];
const plan = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-04-dependent-pilot-analysis-correction-plan",
  protocolId: BATCH_04_DEPENDENT_PILOT_CORRECTION_PROTOCOL_ID,
  status: "frozen-batch-04-dependent-pilot-analysis-two-file-correction-plan-prepared",
  frozenAt,
  batchNumber: 4,
  ...
  protectedProduction: {
    ...
    generatedOutputs: await Promise.all(seoPlan.inventory.map(async (record) => {
      ...
      return { ...actual, role: seoPlan.executionContract.writeOnlyProposedPaths.includes(record.path) ? "accepted-batch-04-derivative" : "preserved-unrelated-derivative" };
    }))
  },
  ...
  correctionContract: {
    ...
    writablePaths: BATCH_04_DEPENDENT_PILOT_OUTPUT_PATHS,
  ...
  nextAuthorizedAction: "activate-and-execute-one-frozen-batch-04-dependent-pilot-analysis-correction"
};
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-04-dependent-pilot-analysis-correction-preparation-analysis",
  protocolId: BATCH_04_DEPENDENT_PILOT_CORRECTION_PROTOCOL_ID,
  status: "batch-04-dependent-pilot-analysis-correction-plan-freeze-passed",
  analyzedAt: frozenAt,
...
if (shouldWrite) {
  ...
  await mkdir(resolve(BATCH_04_DEPENDENT_PILOT_CORRECTION_ROOT), { recursive: true });
