...
import {
  POST_CANARY_BATCH_14_PUBLICATION_COMPILATION_ORDER,
  POST_CANARY_BATCH_14_PUBLICATION_COMPILATION_PROTOCOL_ID,
  POST_CANARY_BATCH_14_PUBLICATION_COMPILATION_ROOT,
  compilePostCanaryBatch14PublicationStagingRecord,
  validatePostCanaryBatch14CompiledStagingRecord
} from "./lib/assessment-production-post-canary-batch-14-publication-compilation.mjs";
import { POST_CANARY_BATCH_14_PUBLICATION_ROOT } from
  "./lib/assessment-production-post-canary-batch-14-publication.mjs";
...
const ROOT = POST_CANARY_BATCH_14_PUBLICATION_COMPILATION_ROOT;
const COHORT = `${POST_CANARY_BATCH_14_PUBLICATION_ROOT}/failure-recovery/original-unattempted-context-resumption-4/complete-cohort-analysis.json`;
...
const ACCEPTED_OUTPUTS = Object.freeze(Object.fromEntries(
  POST_CANARY_BATCH_14_PUBLICATION_COMPILATION_ORDER.map((debateNumber) => [
    debateNumber,
    `${POST_CANARY_BATCH_14_PUBLICATION_ROOT}/outputs/debate-${debateNumber}.json`
  ])
));
const acceptedOutput = (debateNumber) => ACCEPTED_OUTPUTS[debateNumber];
const packetPath = (debateNumber) =>
  `${POST_CANARY_BATCH_14_PUBLICATION_ROOT}/packets/debate-${debateNumber}.json`;
...
assertV4(
  cohort.status ===
    "post-canary-batch-14-complete-ten-debate-publication-output-gate-passed-after-recovery" &&
  cohort.totals?.debates === 10 && cohort.totals?.lockedMoves === 190 &&
  cohort.totals?.critiques === 190 && cohort.totals?.exactSourceQuotes === 20 &&
  cohort.totals?.scorePasses === 1 && cohort.totals?.scorePassReruns === 0 &&
  cohort.validationReplay?.length === 10 &&
  cohort.validationReplay.every((row) => row.validation?.status === "passed"),
"the accepted Batch 14 publication cohort changed");

const productionByNumber = new Map(debates.map((debate) => [debate.number, debate]));
const identities = [];
const contexts = [];
for (const debateNumber of POST_CANARY_BATCH_14_PUBLICATION_COMPILATION_ORDER) {
  ...
  const validation = validatePostCanaryBatch14PublicationOutput(output, packet);
  ...
  const compiled = compilePostCanaryBatch14PublicationStagingRecord({ output, packet, identity });
  const compiledValidation = validatePostCanaryBatch14CompiledStagingRecord({ compiled, output, packet, identity });
...
"the Batch 14 compilation cohort changed");
const identitySnapshot = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-compilation-identity-snapshot",
  protocolId: POST_CANARY_BATCH_14_PUBLICATION_COMPILATION_PROTOCOL_ID,
...
const sources = [COHORT, POST_CANARY_BATCH_14_STANDING_AUTHORIZATION,
  ...
  "scripts/prepare-assessment-production-post-canary-batch-14-publication-compilation.mjs",
  "scripts/test-assessment-production-post-canary-batch-14-publication-compilation-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-14-publication-compilation.mjs",
  "scripts/run-assessment-production-post-canary-batch-14-publication-compilation.mjs",
...
const manifest = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-deterministic-publication-compilation-preparation",
  protocolId: POST_CANARY_BATCH_14_PUBLICATION_COMPILATION_PROTOCOL_ID,
  status: "frozen-post-canary-batch-14-deterministic-publication-compilation-prepared",
  ...
  explicitOrder: POST_CANARY_BATCH_14_PUBLICATION_COMPILATION_ORDER, contexts,
  ...
  nextAuthorizedAction: "activate-one-batch-14-deterministic-publication-compilation-pass" };
