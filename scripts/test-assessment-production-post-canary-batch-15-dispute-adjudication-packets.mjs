...
import {
  POST_CANARY_BATCH_15_DISPUTE_ADJ_ISOLATION,
...
const preparation = JSON.parse(
  await readFile(`${POST_CANARY_BATCH_15_DISPUTE_ADJ_ROOT}/preparation-manifest.json`, "utf8")
);
...
assert.equal(preparation.acceptedSourceBoundary.allThreeAudioMovesVerified, true);
...
for (const context of preparation.contexts) {
  ...
  const output = {
    schemaVersion: POST_CANARY_BATCH_15_DISPUTE_ADJ_OUTPUT_VERSION,
    ...
    isolation: structuredClone(POST_CANARY_BATCH_15_DISPUTE_ADJ_ISOLATION),
  ...
  const validation = validatePostCanaryBatch15DisputeAdjudicationOutput(output, packet);
...
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_15_DISPUTE_ADJ_ROOT}/packets`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_15_DISPUTE_ADJ_ROOT}/provenance`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
