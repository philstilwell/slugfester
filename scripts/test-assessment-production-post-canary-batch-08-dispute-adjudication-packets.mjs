...
import {
  POST_CANARY_BATCH_08_DISPUTE_ADJ_ISOLATION,
  POST_CANARY_BATCH_08_DISPUTE_ADJ_OUTPUT_VERSION,
...
const EXPECTED_DEBATES = ["88", "194", "137", "08", "65", "140", "156", "120", "118", "145"];
...
const preparation = JSON.parse(
  await readFile(`${POST_CANARY_BATCH_08_DISPUTE_ADJ_ROOT}/preparation-manifest.json`, "utf8")
);
...
assert.equal(preparation.totals.disputedMoves, 174);
assert.equal(preparation.totals.candidateSelections, 533);
assert.equal(preparation.totals.audioVerifiedMoves, 6);
...
assert.equal(preparation.acceptedSourceBoundary.allSixAudioMovesVerified, true);
assert.equal(preparation.acceptedSourceBoundary.audioValidationOverlaysPreserved, 4);
...
for (const context of preparation.contexts) {
  ...
  const output = {
    schemaVersion: POST_CANARY_BATCH_08_DISPUTE_ADJ_OUTPUT_VERSION,
    ...
    isolation: structuredClone(POST_CANARY_BATCH_08_DISPUTE_ADJ_ISOLATION),
  ...
  const validation = validatePostCanaryBatch08DisputeAdjudicationOutput(output, packet);
...
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_08_DISPUTE_ADJ_ROOT}/packets`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_08_DISPUTE_ADJ_ROOT}/provenance`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
