...
import { V42211736_ROOT, validateV42211736PublicationOutput } from "./lib/v42211736-hard-route-publication-integrity.mjs";

const manifestPath = `${V42211736_ROOT}/execution-manifest.json`, executionPath = `${V42211736_ROOT}/model-execution.json`, analysisPath = `${V42211736_ROOT}/analysis.json`;
...
assert.equal(preparation.status, "prepared-five-isolated-hard-route-publication-integrity-contexts");
...
assert.equal(preparation.repair.priorFindingCount, 3);
assert.deepEqual(preparation.repair.repositoryCritiqueAcceptanceWords, [105, 130]);
assert.equal(preparation.repair.critiqueCharacterMinimum, 880);
assert.equal(preparation.repair.critiqueCharacterMaximum, null);
assert.equal(preparation.repair.terminalPunctuationRequired, true);
assert.equal(preparation.repair.unexpectedCJKAndHangulRejected, true);
...
assert.match(await readFile(path.resolve(preparation.inputs.workflow), "utf8"), /There is no structured maximum character length/);
assert.match(await readFile(path.resolve("scripts/run-v42211736-hard-route-publication-integrity.mjs"), "utf8"), /never truncate to fit a character maximum/);
...
for (const debateNumber of ["51", "63"]) {
  const context = preparation.contexts.find((item) => item.debateNumber === debateNumber);
  const priorOutput = JSON.parse(await readFile(path.resolve(`docs/calibration/v4.2.21.17.35/hard-route-publication-stability/outputs/debate-${debateNumber}.json`), "utf8"));
  const packet = JSON.parse(await readFile(path.resolve(context.packet), "utf8"));
  assert.throws(() => validateV42211736PublicationOutput(priorOutput, packet), /critique lacks terminal punctuation|unexpected non-Latin script/);
}
{
  const context = preparation.contexts.find((item) => item.debateNumber === "90");
  const priorOutput = JSON.parse(await readFile(path.resolve("docs/calibration/v4.2.21.17.35/hard-route-publication-stability/outputs/debate-90.json"), "utf8"));
  const packet = JSON.parse(await readFile(path.resolve(context.packet), "utf8"));
  assert.equal(validateV42211736PublicationOutput(priorOutput, packet).status, "passed");
}
