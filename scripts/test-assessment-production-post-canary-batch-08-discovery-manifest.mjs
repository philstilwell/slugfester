...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-08/discovery";
...
assert.equal(manifest.status, "frozen-thirty-nine-post-canary-batch-08-discovery-contexts-prepared-not-authorized");
...
assert.deepEqual(manifest.copiedInputBoundary.maximumContext, { debateNumber: "137", chunkId: "chunk-004" });
...
for (const context of manifest.contexts) {
  ...
  assert(buildBatch08TokenCountedChunkLedger(chunkBytes).equals(tokenBytes));
...
assert.equal(manifest.nextAuthorizedAction, "prepare-and-freeze-batch-08-discovery-execution-activation-under-standing-authorization");

const validation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-discovery-execution-preparation-validation",
  protocolId: manifest.protocolId,
  status: "batch-08-discovery-execution-manifest-validation-passed-frozen-standing-authorization-active",
