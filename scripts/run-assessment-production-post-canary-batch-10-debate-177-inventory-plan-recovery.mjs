...
const ACTIVATION_STATUS =
  "two-frozen-debate-177-inventory-plan-recovery-shards-authorized";
const EXECUTION_STATUS =
  "two-debate-177-inventory-plan-recovery-shards-passed-and-merged";
...
async function loadPreparation({ requireFutureAbsent = false } = {}) {
  ...
  assertV4(
    ...
    "Debate 177 recovery preparation controls drifted"
  );
...
async function preflight() {
  ...
  assertV4(
    validation.status ===
      "passed-frozen-debate-177-inventory-plan-recovery-preparation" &&
      validation.contexts === 2 &&
      validation.modelContextsExecuted === 0 &&
      validation.directIncrementalCostUsd === 0,
    "Debate 177 recovery preparation validator failed"
  );
  ...
  console.log(
    JSON.stringify(
      {
        status: "passed-debate-177-recovery-model-free-preflight",
        contexts: 2,
        shards: SHARDS,
...
async function activate() {
  ...
  const activation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-10-debate-177-inventory-plan-recovery-execution-activation",
...
async function loadActivation({ requireFutureAbsent = false } = {}) {
  ...
  assertV4(
    ...
    "Debate 177 recovery activation controls drifted"
  );
...
async function execute() {
  ...
  async function executeContext(context) {
    ...
    const sourceDirectory = await mkdtemp(
      path.join(os.tmpdir(), `slugfester-batch-10-debate-177-${context.shardId}-`)
    );
    const isolatedCodexHome = await mkdtemp(
      path.join(os.tmpdir(), `slugfester-batch-10-debate-177-home-${context.shardId}-`)
    );
    ...
    process.stdout.write(
      `[batch-10-debate-177-plan-recovery] ${context.shardId} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
    );
    return record;
  }
  ...
  if (results.length === 2 && results.every((result) => result.accepted)) {
    try {
      ...
      const sourceContext = sourcePreparation.contexts.find(
        (context) => context.debateNumber === "177"
      );
  ...
  const execution = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-10-debate-177-inventory-plan-recovery-model-execution",
    protocolId: PROTOCOL_ID,
    status: passed
      ? EXECUTION_STATUS
      : "debate-177-inventory-plan-recovery-complete-with-failure",
...
async function analyze() {
  ...
  assertV4(
    ...
    "Debate 177 bounded recovery did not pass"
  );
  assertV4(
  ...
  for (const [contextIndex, context] of originalPreparation.contexts.entries()) {
    ...
    if (context.debateNumber !== "177") {
      ...
      assertV4(
        ...
        "Debate 177 merged plan drifted"
      );
    }
  ...
  const recoveryAnalysis = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-10-debate-177-inventory-plan-recovery-analysis",
    protocolId: PROTOCOL_ID,
    status:
      "debate-177-bounded-first-recovery-passed-complete-ten-plan-cohort-replay-passed",
    activation: ACTIVATION,
    activationSha256: sha256(activationBytes),
    execution: EXECUTION,
...
async function validate() {
  ...
  if (!(await exists(EXECUTION))) {
    ...
    console.log(
      JSON.stringify(
        {
          status: "passed-debate-177-recovery-activation-awaiting-model-execution",
          contexts: 2,
          shards: SHARDS,
          model: activation.model,
          attemptsPerContext: 1,
  ...
  if (execution.status !== EXECUTION_STATUS) {
    ...
    console.log(
      JSON.stringify(
        {
          status: "passed-recorded-failed-debate-177-bounded-recovery",
          contextsAttempted: execution.contextsAttempted,
          invalidContexts: execution.invalidContexts,
  ...
  assertV4(
    ...
    "passing Debate 177 recovery record drifted"
  );
  if (!(await exists(COHORT_ANALYSIS))) {
    ...
    console.log(
      JSON.stringify(
        {
          status: "passed-debate-177-recovery-awaiting-cohort-analysis",
          contexts: 2,
  ...
  console.log(
    JSON.stringify(
      {
        status: "passed-debate-177-recovery-and-complete-ten-plan-cohort-replay",
        acceptedPlans: 10,
        recoveredDebate: "177",
        recoveryShards: 2,
        originalFailedPartialOutputReused: false,
