...
import {
  ...
  makeV213DisputeAdjudicationSchema
...
assertV4(
  ...
  "v2.1.3 dispute adjudication preparation is unauthorized"
);
...
const originalUnresolved = audioAudit.debates
  ...
  .find(
    (move) =>
      move.debateNumber === "78" && move.moveId === audioDecision.moveId
  );
assertV4(
  audioDecision.status === "verified" && originalUnresolved,
  "Debate 78 combined audio resolution unavailable"
);
combinedAudioByDebateMove.set(
  `78:${audioDecision.moveId}`,
  {
...
const schema = makeV213DisputeAdjudicationSchema();
...
const inputPaths = {
  ...
  manual: `${V213_DISPUTE_ADJ_ROOT}/manual.md`,
  schema: `${V213_DISPUTE_ADJ_ROOT}/adjudication.schema.json`
};
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const built = buildV213DisputeAdjudicationPacket(
    disagreements,
    lockedInventory,
    events,
    audioByMoveId
  ...
  const packetPath =
    `${V213_DISPUTE_ADJ_ROOT}/packets/debate-${debateNumber}.json`;
  const provenancePath =
    `${V213_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`;
  const outputPath =
    `${V213_DISPUTE_ADJ_ROOT}/outputs/debate-${debateNumber}.json`;
  ...
  if (shouldWrite) {
    ...
    await writeFile(
      ...
      `${JSON.stringify(
        {
          schemaVersion:
            "1.0-score-stability-v2.1.3-adjudication-candidate-provenance",
...
const preparation = {
  schemaVersion:
    "1.0-score-stability-v2.1.3-dispute-only-adjudication-preparation",
  ...
  nextAuthorizedAction:
    "freeze-ten-v2.1.3-dispute-only-adjudication-execution-contexts"
};
assertV4(
  contexts.length === 10 &&
  ...
  "v2.1.3 adjudication workload differs from deterministic extraction"
);
assertV4(
...
assertV4(
  ...
  `v2.1.3 adjudication context ${preparation.totals.maximumCopiedInputBytes} bytes exceeds the promoted 350 KB ceiling`
);
if (shouldWrite) {
  await mkdir(V213_DISPUTE_ADJ_ROOT, { recursive: true });
  await writeFile(
    `${V213_DISPUTE_ADJ_ROOT}/adjudication.schema.json`,
    schemaBytes
  );
  await writeFile(
