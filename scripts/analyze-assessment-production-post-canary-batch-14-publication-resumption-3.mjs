...
assertV4(
  execution.contextsPlanned === 5 && execution.retries === 0 &&
  ...
  "five-context resumption execution boundary changed"
);
...
const fiveValid = replayed.filter((item) => item.gateAcceptancePassed);
const resumptionPassed =
  execution.status === "five-original-unattempted-batch-14-publication-contexts-passed" &&
  execution.contextsAttempted === 5 && execution.contextsUnattempted === 0 &&
  execution.validContexts === 5 && execution.invalidContexts === 0 &&
  ...
  fiveValid.length === 5;
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-original-unattempted-context-resumption-analysis",
  protocolId: activation.protocolId,
  status: resumptionPassed
    ? "five-original-unattempted-batch-14-publication-contexts-passed"
    : "five-context-publication-resumption-failed",
  ...
  totals: {
    debates: 5,
    lockedMoves: 94,
    critiques: fiveValid.reduce((sum, item) => sum + item.validation.critiques, 0),
    exactSourceQuotes: fiveValid.reduce((sum, item) => sum + item.validation.quoteExactSourceMatches, 0),
    overallCommentarySides: fiveValid.reduce((sum, item) => sum + item.validation.overallCommentarySides, 0),
    aiExtensionSides: fiveValid.reduce((sum, item) => sum + item.validation.aiExtensionSides, 0),
...
if (resumptionPassed) {
  const allDebates = ["53", "69", "110", "133", "12", ...DEBATES];
  ...
  for (const debateNumber of allDebates) {
    ...
    completeReplay.push({
      ...
      source: debateNumber === "53"
        ? "accepted-two-level-eleven-critique-recovery"
        : ["110", "133"].includes(debateNumber)
          ? "accepted-two-level-nineteen-critique-recovery"
        : debateNumber === "12"
          ? "accepted-two-level-seventeen-critique-recovery"
        : debateNumber === "69"
          ? "accepted-original-first-attempt-resumption-1"
          : "original-first-attempt-five-context-resumption",
      validation
    });
  }
  const completePassed =
    ...
    completeReplay.reduce((sum, item) => sum + item.validation.moves, 0) === 190 &&
    completeReplay.reduce((sum, item) => sum + item.validation.critiques, 0) === 190 &&
  ...
  completeCohort = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-14-complete-publication-cohort-analysis-after-recovery",
    protocolId: activation.protocolId,
    status: "post-canary-batch-14-complete-ten-debate-publication-output-gate-passed-after-recovery",
    productionCanary: false,
    batchNumber: 14,
    stagingOnly: true,
    debates: allDebates,
    ...
    totals: {
      ...
      debate53CritiqueRecoveryContexts: 10,
      debate53CorrectedCritiques: 11,
      debates110And133CritiqueRecoveryContexts: 14,
      debates110And133CorrectedCritiques: 19,
      debate12CritiqueRecoveryLevel1Contexts: 9,
      debate12CritiqueRecoveryLevel2Contexts: 3,
      debate12CorrectedCritiques: 17,
      retries: 0,
      timeoutExtensions: 0,
      modelAuthoredScores: 0,
    ...
    integrity: {
      ...
      fiveResumedContextsWereOriginalFirstAttempts: true,
      failedPublicationOutputsPreservedAsFailureEvidence: true,
      acceptedRecoveredFieldsRetainedDeterministically: true,
    ...
    nextAuthorizedAction: "prepare-batch-14-deterministic-publication-compilation"
  };
}
