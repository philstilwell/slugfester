...
assertV4(
  preparation.schemaVersion ===
    ...
    canonicalJson(preparation.viewports) ===
      canonicalJson(CHECKPOINT_V22_RENDERING_REMEDY_V2_VIEWPORTS) &&
  ...
  "invalid frozen remedy-v2 preparation"
);
...
for (const row of preparation.packets) {
  ...
  assertV4(
    ...
    `${row.debateNumber}: model/authentication boundary changed`
  );
}

const calibration = JSON.parse(
  await readFile(path.resolve(preparation.inputs.viewportCalibration), "utf8")
);
const preflight = JSON.parse(
  await readFile(path.resolve(preparation.inputs.syntheticPreflightResult), "utf8")
);
const diagnosis = JSON.parse(
  await readFile(path.resolve(preparation.inputs.controllerDiagnosis), "utf8")
);
assertV4(
  ...
  "synthetic remedy-v2 evidence is not passing"
);
...
assertV4(
  ...
  "remedy-v2 preparation replay mismatch"
);

console.log(JSON.stringify({
  status: "remedy-v2-rendering-preparation-passed",
