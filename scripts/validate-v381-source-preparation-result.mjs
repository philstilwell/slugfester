...
import { V381_EXECUTION_MANIFEST, V381_ROOT, assert } from "./lib/v381-source-preparation.mjs";
...
for (const record of [proposal, review, adjudication]) {
  ...
  assert(record.results.every((item) => item.sameRequestStreamRecoveries <= manifest.executionPolicy.sameRequestStreamRecoveriesMaximumPerContext), `${record.stage}: stream recovery bound exceeded`);
}
assert(proposal.results.every((item) => item.outputSha256 && item.enrichedOutputSha256 && item.enrichedOutputWritten), "raw/enriched proposal hashes incomplete");
for (const lock of [reviewLock, adjudicationLock]) {
  assert(lock.frozenBeforeStageModelExecution && lock.futureOutputPathsExcluded, `${lock.stage}: phase lock invalid`);
...
assert(map.schemaVersion === "3.8.1-source-adjudication-option-map-set", "adjudication map invalid");
...
if (analysis.sourcePreparationPassed) assert(inventory.status === "locked-source-inventory" && inventory.selectedMoveCount === 12 && analysis.totals.pendingAudioVerifications === 0, "passed correction inventory invalid");
else assert(["awaiting-required-audio-verification", "source-preparation-failed"].includes(analysis.status), "incomplete correction status invalid");
assert(!(await exists(`${V381_ROOT}/classification`)), "classification directory must not exist");
