...
import {
  ...
  POST_CANARY_BATCH_03_PUBLICATION_FINALIZATION_PROTOCOL_ID,
  POST_CANARY_BATCH_03_PUBLICATION_FINALIZATION_ROOT,
  buildPostCanaryBatch03PublicationFinalization,
  validatePostCanaryBatch03PublicationFinalCandidate
} from "./lib/assessment-production-post-canary-batch-03-publication-finalization.mjs";
import {
...
const ROOT = POST_CANARY_BATCH_03_PUBLICATION_FINALIZATION_ROOT;
...
assertV4(
  compilationAnalysis.status ===
    ...
    compilationAudit.status ===
      "passed-ten-debate-batch-03-deterministic-publication-compilation" &&
    canonicalJson(compilationAudit.explicitOrder) ===
      canonicalJson(POST_CANARY_BATCH_03_PUBLICATION_FINALIZATION_ORDER) &&
    compilationAudit.rows.length === 10 &&
    compilationAudit.totals.sections === 53 &&
    compilationAudit.totals.moves === 200 &&
  ...
  "passing Batch 3 deterministic compilation evidence required"
);

const contexts = [];
const compatibilityRows = [];
const sourceChainFiles = [];
for (const debateNumber of POST_CANARY_BATCH_03_PUBLICATION_FINALIZATION_ORDER) {
  ...
  assertV4(
    ...
    `${debateNumber}: Batch 3 finalization source context missing`
  );
  ...
  assertV4(
    ...
    `${debateNumber}: Batch 3 compiled input hash changed`
  );
  const compiled = JSON.parse(compiledBytes);
  const built = buildPostCanaryBatch03PublicationFinalization({
    compiled,
  ...
  const validation = validatePostCanaryBatch03PublicationFinalCandidate({
...
const compatibility = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-publication-finalization-compatibility-analysis",
  protocolId: POST_CANARY_BATCH_03_PUBLICATION_FINALIZATION_PROTOCOL_ID,
  status: "batch-03-production-compatibility-boundary-recorded",
  analyzedAt: frozenAt,
  ...
  findings: [
    {
      id: "batch-03-site-ledger-adapter-and-validator-route",
      description:
        "The live site validator recognizes adjudicated-consensus ledgers only through the earlier checkpoint-specific adapter schema, activation, and packet root; it cannot authenticate or replay Batch 3 ledgers without a separately frozen Batch 3 adapter route.",
      evidence: {
        debates: 10,
        sections: 53,
        moves: 200,
      ...
      requiredRemedy:
        "A separately authorized production-compatibility plan must define, hash-lock, and test a Batch 3 site-ledger adapter and validator route without changing scores, prose, attribution, or the accepted optional-reference behavior."
    }
  ],
  authorization: {
...
const sourceFiles = [
  ...
  "scripts/prepare-assessment-production-post-canary-batch-03-publication-finalization.mjs",
  "scripts/activate-assessment-production-post-canary-batch-03-publication-finalization.mjs",
  "scripts/run-assessment-production-post-canary-batch-03-publication-finalization.mjs",
  "scripts/test-assessment-production-post-canary-batch-03-publication-finalization-preparation.mjs",
...
for (const file of futureOutputPaths) {
  assertV4(!(await exists(file)), `future Batch 3 finalization output exists: ${file}`);
}

const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-publication-finalization-preparation",
  protocolId: POST_CANARY_BATCH_03_PUBLICATION_FINALIZATION_PROTOCOL_ID,
  status: "frozen-post-canary-batch-03-publication-finalization-prepared-not-authorized",
  frozenAt,
  ...
  userAuthorization: {
    ...
    scopeInterpretation:
      "Prepare, validate, freeze, activate, and execute the deterministic Batch 3 publication-finalization pass under the standing authorization.",
  ...
  explicitOrder: POST_CANARY_BATCH_03_PUBLICATION_FINALIZATION_ORDER,
  contexts,
  executionPolicy: {
  ...
  finalizationPolicy: {
    ...
    previewUsesPostCanaryBatch03StagingBanner: true,
    previewNoindex: true,
  ...
  nextAuthorizedAction:
    "activate-and-execute-one-frozen-batch-03-deterministic-publication-finalization-pass-under-standing-authorization"
};
  ...
  JSON.stringify(
    {
      status: shouldWrite
        ? manifest.status
        : "post-canary-batch-03-publication-finalization-preparation-preview",
