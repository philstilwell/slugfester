...
const BATCH_ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-04";
...
const PROTOCOL_ID = "assessment-production-post-canary-batch-04-discovery";
const WORKFLOW = "docs/assessment-production-canary-discovery-workflow.md";
const SCRIPT = "scripts/preregister-assessment-production-post-canary-batch-04-discovery.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-04-discovery-manifest.mjs";
const CODEX_PATH = "/Applications/ChatGPT.app/Contents/Resources/codex";
const REQUIRED_ORDER = ["127", "67", "85", "49", "186", "81", "148", "47", "03", "185"];
...
assertV4(
  preparation.status === "post-canary-batch-04-ten-complete-score-blind-source-packets-prepared-awaiting-validation" &&
    preparationValidation.status === "post-canary-batch-04-score-blind-source-packet-validation-passed-frozen-awaiting-user-approval-before-any-model-execution" &&
    ...
    preparation.totals.discoveryContexts === 44 &&
    preparation.totals.maximumCopiedInputBytes === 65995 &&
  ...
  "validated Batch 4 source preparation is not frozen at the required authorization boundary"
);
assertV4(
  ...
  "Batch 4 standing authorization does not cover exact discovery execution"
);
assertV4(
  ...
  for (const chunk of debate.chunks) {
    ...
    assertV4(
      sha256(chunkBytes) === chunk.chunkLedgerSha256 &&
        ...
        buildBatch04TokenCountedChunkLedger(chunkBytes).equals(tokenBytes) &&
...
assertV4(contexts.length === 44, "Batch 4 discovery must contain exactly 44 contexts");
...
assertV4(maximumContext.debateNumber === "81" && maximumContext.chunkId === "chunk-003", "maximum copied-input context drifted");
...
const sourceFiles = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-04-source-preparation.mjs",
...
const manifest = {
  ...
  status: shouldWrite ? "frozen-forty-four-post-canary-batch-04-discovery-contexts-prepared-not-authorized" : "preview",
  ...
  AIOnly: true,
  userAuthorization: {
    scope: "standing-authorized Batch 4 discovery preparation and execution",
  ...
  costEstimate: {
    ...
    estimateBasis: "The exact 44-context Batch 4 input set is hash-locked with an observed maximum of 65,995 copied bytes; no input is truncated or repartitioned. The frozen 1→2→4 scheduler uses the subscription-backed route with a $0 direct incremental cost boundary.",
  },
  ...
  nextAuthorizedAction: "prepare-and-freeze-batch-04-discovery-execution-activation-under-standing-authorization",
};
