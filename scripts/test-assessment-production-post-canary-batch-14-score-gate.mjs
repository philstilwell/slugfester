...
import {
  ...
  evaluatePostCanaryBatch14ScoreStability
...
const accepted = evaluatePostCanaryBatch14ScoreStability(passing);
...
assert.equal(
  evaluatePostCanaryBatch14ScoreStability(winnerMutation).acceptancePassed,
  false
);
const distanceMutation = structuredClone(passing);
distanceMutation[2].final = scores(95, 80);
assert.equal(
  evaluatePostCanaryBatch14ScoreStability(distanceMutation).acceptancePassed,
  false
);
assert.throws(() =>
  evaluatePostCanaryBatch14ScoreStability(passing, {
    ...POST_CANARY_BATCH_14_SCORE_STABILITY_THRESHOLDS,
