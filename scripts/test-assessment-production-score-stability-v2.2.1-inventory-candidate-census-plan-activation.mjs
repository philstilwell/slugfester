...
const RUNNER =
  "scripts/run-assessment-production-score-stability-v2.2.1-inventory-candidate-census-plans.mjs";
...
assert.equal(
  ...
  "1.0-score-stability-v2.2.1-candidate-census-plan-execution-activation"
);
...
assert.equal(
  activation.failedGateDisposition.currentV212InventoryGatePreservedFailed,
  true
);
assert.equal(
  activation.failedGateDisposition.v212FailedOutputsUsedAsFreshSuccessorModelInput,
  false
);
assert.equal(
  activation.failedGateDisposition.sourceV22DiscoveryGatePreservedFailed,
  true
);
assert.equal(activation.failedGateDisposition.sourceV22DiscoveryGateRetried, false);
assert.equal(
  activation.failedGateDisposition.sourceV22DiscoveryExecutionReclassified,
  false
);
assert.equal(
  activation.failedGateDisposition.v221MechanicalRecoveryChangedCandidateFields,
  false
);
assert.equal(
  activation.failedGateDisposition.predecessorV213ScoreGatePreservedFailed,
  true
);
assert.equal(
...
assert.equal(
  activation.inventorySuccessorContract.fallbackAppliedOnlyToRetainedOrphanReply,
  true
);
...
assert.equal(
  ...
  "execute-frozen-v2.2.1-candidate-census-plan-gate-once"
);
