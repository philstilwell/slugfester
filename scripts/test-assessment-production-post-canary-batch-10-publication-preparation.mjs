...
const MANIFEST =
  `${POST_CANARY_BATCH_10_PUBLICATION_ROOT}/execution-preparation-manifest.json`;
...
assert.equal(manifest.protocolId, POST_CANARY_BATCH_10_PUBLICATION_PROTOCOL_ID);
assert.equal(manifest.productionCanary, false);
assert.equal(manifest.batchNumber, 10);
...
assert.equal(manifest.totals.audioVerifiedMoves, 9);
...
for (const context of manifest.contexts) {
  ...
  assert.equal(packet.protocolId, POST_CANARY_BATCH_10_PUBLICATION_PROTOCOL_ID);
  ...
  assert.deepEqual(schema, buildPostCanaryBatch10PublicationSchema(packet));
  assert.equal(schema.properties.schemaVersion.const, POST_CANARY_BATCH_10_PUBLICATION_OUTPUT_VERSION);
  assert.equal(schema.properties.protocolId.const, POST_CANARY_BATCH_10_PUBLICATION_PROTOCOL_ID);
...
assert.equal(audioVerifiedMoves, 9);
assert.equal(
