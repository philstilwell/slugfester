...
import {
  POST_CANARY_BATCH_04_DEBATE_127_BASE_OUTPUT,
  POST_CANARY_BATCH_04_DEBATE_127_PUBLICATION_PACKET,
  POST_CANARY_BATCH_04_DEBATE_127_REPAIR_FIELDS,
  POST_CANARY_BATCH_04_DEBATE_127_REPAIR_PACKET_VERSION,
  POST_CANARY_BATCH_04_DEBATE_127_REPAIR_PARTITIONS,
  POST_CANARY_BATCH_04_DEBATE_127_REPAIR_PROTOCOL_ID,
  POST_CANARY_BATCH_04_DEBATE_127_REPAIR_ROOT,
  buildDebate127RepairSchema,
  debate127RepairMoveId
} from "./lib/assessment-production-post-canary-batch-04-publication-repair.mjs";
import {
...
const ROOT = POST_CANARY_BATCH_04_DEBATE_127_REPAIR_ROOT;
const MANIFEST = `${ROOT}/execution-preparation-manifest.json`;
const DIAGNOSIS = `${POST_CANARY_BATCH_04_PUBLICATION_ROOT}/failure-diagnosis.json`;
const FAILED_EXECUTION = `${POST_CANARY_BATCH_04_PUBLICATION_ROOT}/model-execution.json`;
const FAILED_ANALYSIS = `${POST_CANARY_BATCH_04_PUBLICATION_ROOT}/analysis.json`;
...
const STATIC_SOURCE_FILES = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-04-publication-repair.mjs",
  "scripts/lib/assessment-production-post-canary-batch-04-standing-authorization.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-04-publication-repair.mjs",
  "scripts/test-assessment-production-post-canary-batch-04-publication-repair-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-04-publication-repair.mjs",
  "scripts/run-assessment-production-post-canary-batch-04-publication-repair.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-04-publication-repair.mjs"
];
...
const baseOutput = parsed(POST_CANARY_BATCH_04_DEBATE_127_BASE_OUTPUT);
const publicationPacket = parsed(POST_CANARY_BATCH_04_DEBATE_127_PUBLICATION_PACKET);
...
assertV4(
  diagnosis.status ===
    ...
    diagnosis.failedContext?.debateNumber === "127" &&
    diagnosis.failedContext?.debateId === publicationPacket.debateId &&
    diagnosis.failureBoundary?.failedFieldCount === 3 &&
    diagnosis.failureBoundary?.excessWordsTotal === 4 &&
  ...
  "the frozen Debate 127 failure diagnosis changed"
);
assertV4(
...
assertV4(
  execution.contextsPlanned === 10 &&
    ...
    execution.results?.[0]?.debateNumber === "127" &&
...
assertV4(
  sha256(bytesByFile[POST_CANARY_BATCH_04_DEBATE_127_BASE_OUTPUT]) ===
      diagnosis.failedContext.outputSha256 &&
    sha256(bytesByFile[POST_CANARY_BATCH_04_DEBATE_127_BASE_OUTPUT]) ===
      diagnosis.artifacts.output.sha256 &&
    sha256(bytesByFile[POST_CANARY_BATCH_04_DEBATE_127_PUBLICATION_PACKET]) ===
  ...
  "a diagnosed Debate 127 source artifact drifted"
);
assertV4(
  canonicalJson(diagnosis.prospectiveRecoveryOnly.proposedRepairPartition) ===
      canonicalJson(POST_CANARY_BATCH_04_DEBATE_127_REPAIR_PARTITIONS) &&
    canonicalJson(diagnosis.diagnosticReplay.hypotheticalWritableFields) ===
      canonicalJson(POST_CANARY_BATCH_04_DEBATE_127_REPAIR_FIELDS),
...
for (let packetIndex = 0; packetIndex < 2; packetIndex += 1) {
  const writableFields = POST_CANARY_BATCH_04_DEBATE_127_REPAIR_PARTITIONS[packetIndex];
  const corrections = writableFields.map((field) => {
    const moveId = debate127RepairMoveId(field);
  ...
  const packet = {
    schemaVersion: POST_CANARY_BATCH_04_DEBATE_127_REPAIR_PACKET_VERSION,
    protocolId: POST_CANARY_BATCH_04_DEBATE_127_REPAIR_PROTOCOL_ID,
    packetIndex,
    ...
    immutableBaseOutput: POST_CANARY_BATCH_04_DEBATE_127_BASE_OUTPUT,
    publicationPacket: POST_CANARY_BATCH_04_DEBATE_127_PUBLICATION_PACKET,
  ...
  const schema = buildDebate127RepairSchema(packet);
...
assertV4(
  contexts.length === 2 &&
    ...
    new Set(contexts.flatMap((context) => context.writableFields)).size === 3,
  "exactly two disjoint one-or-two-field repair contexts are required"
);

const ACTIVATION = `${ROOT}/execution-activation.json`;
const EXECUTION = `${ROOT}/model-execution.json`;
const ANALYSIS = `${ROOT}/analysis.json`;
const MERGED_OUTPUT = `${ROOT}/merged/debate-127.json`;
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-debate-127-publication-repair-execution-preparation-manifest",
  protocolId: POST_CANARY_BATCH_04_DEBATE_127_REPAIR_PROTOCOL_ID,
  status:
  ...
  userAuthorization: {
    ...
    resolvedScope:
      "prepare, validate, freeze, activate, and execute exactly two bounded Debate 127 publication-repair packets exposing three diagnosed critique fields",
    standingAuthorizationApplied: true,
    standingAuthorizationSha256: standingAuthorization.sha256,
    directIncrementalCostUsdMaximum: 0,
    contextsPrepared: 2,
    writableFieldsPrepared: 3,
  ...
  costEstimate: {
    ...
    expectedAggregateModelMinutes: [2, 10],
    absoluteGateTimeoutMinutes: 20,
    estimateBasis: {
      source:
      ...
      scalingRule: "two-sevenths-of-observed-seven-context-repair-with-contingency"
    }
  },
  ...
  repairContract: {
    repairType: "critique-word-boundary",
    writableFields: POST_CANARY_BATCH_04_DEBATE_127_REPAIR_FIELDS,
  ...
  deterministicValidation: {
    ...
    twoBoundedFieldSchemasReproducedAtFreeze: true,
  ...
  artifacts: {
    ...
    execution: EXECUTION,
    analysis: ANALYSIS,
    originalFailedOutput: POST_CANARY_BATCH_04_DEBATE_127_BASE_OUTPUT,
  ...
  nextAuthorizedAction:
    "activate-and-execute-exactly-two-frozen-debate-127-publication-repair-contexts-under-standing-authorization"
};
