...
const finalLedgerPath =
  `${POST_CANARY_BATCH_06_FINAL_LEDGER_ROOT}/final-ledger.json`;
...
assertV4(
  ...
  "post-canary Batch 6 final-ledger manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen Batch 6 final-ledger source hash mismatch`
  );
}
const inputs = await loadPostCanaryBatch06FinalLedgerInputs();
const ledger = buildPostCanaryBatch06FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
