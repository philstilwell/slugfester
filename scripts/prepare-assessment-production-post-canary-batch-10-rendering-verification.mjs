...
import {
  ...
  POST_CANARY_BATCH_10_RENDERING_PORT,
  POST_CANARY_BATCH_10_RENDERING_PROTOCOL_ID,
  POST_CANARY_BATCH_10_RENDERING_REQUIRED_CHECKS,
  POST_CANARY_BATCH_10_RENDERING_ROOT,
  POST_CANARY_BATCH_10_RENDERING_VIEWPORTS,
  buildPostCanaryBatch10RenderingPacket,
  hashFile,
  sha256
...
const METADATA_CORRECTION =
  "docs/assessment-production/post-canary-continuation-v1/batch-10/deterministic-publication-compilation/metadata-correction/analysis.json";
const COMPATIBILITY_BOUNDARY = `${FINALIZATION}/compatibility-analysis.json`;
const PREPARATION = `${POST_CANARY_BATCH_10_RENDERING_ROOT}/preparation-manifest.json`;
const ACTIVATION = `${POST_CANARY_BATCH_10_RENDERING_ROOT}/execution-activation.json`;
const EXECUTION = `${POST_CANARY_BATCH_10_RENDERING_ROOT}/execution.json`;
const ANALYSIS = `${POST_CANARY_BATCH_10_RENDERING_ROOT}/analysis.json`;
const AUDIT = `${POST_CANARY_BATCH_10_RENDERING_ROOT}/rendering-audit.json`;
const PACKETS = `${POST_CANARY_BATCH_10_RENDERING_ROOT}/packets`;
...
"Batch 10 standing rendering authorization required");
...
const [finalPreparation, finalActivation, finalExecution, finalAnalysis, finalAudit,
  correction, originalPreviewBytes] = await Promise.all([
  parse(FINALIZATION_PREPARATION), parse(FINALIZATION_ACTIVATION),
  parse(FINALIZATION_EXECUTION), parse(FINALIZATION_ANALYSIS),
  parse(FINALIZATION_AUDIT), parse(METADATA_CORRECTION),
  readFile(path.resolve(ORIGINAL_PREVIEW))
]);
assertV4(finalPreparation.status ===
  "frozen-post-canary-batch-10-publication-finalization-prepared" &&
  finalActivation.status ===
  "frozen-post-canary-batch-10-publication-finalization-authorized" &&
  finalExecution.status === "ten-debate-batch-10-publication-finalization-passed" &&
  finalAnalysis.status === "ten-debate-batch-10-publication-finalization-passed" &&
  finalAudit.status === "passed" && finalAudit.rows?.length === 10 &&
  finalAudit.totals?.moves === 182 &&
  correction.status === "batch-10-compilation-metadata-correction-passed" &&
  correction.displayCandidatesChanged === 0 && correction.participantScoresChanged === 0,
"passing corrected Batch 10 finalization required");

let previewText = originalPreviewBytes.toString("utf8");
previewText = previewText.replace(
  "renderPublicationStagingDebate(await response.json());",
  `renderPublicationStagingDebate(await response.json());\n        const banner = document.querySelector("[data-publication-staging-preview] .calibration-preview-note");\n        if (banner) banner.innerHTML = "<strong>Publication staging preview:</strong> validated post-canary Batch 10 candidate only. This scorecard remains excluded from production data and rankings pending rendering and mutation authorization.";`
);
assertV4(previewText !== originalPreviewBytes.toString("utf8"),
  "Batch 10 rendering preview adaptation failed");
const previewBytes = Buffer.from(previewText);

const compatibility = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-finalization-compatibility-analysis",
  protocolId: "assessment-production-post-canary-batch-10-publication-finalization",
  status: "batch-10-production-compatibility-boundary-recorded",
  analyzedAt: frozenAt,
  productionCanary: false,
  batchNumber: 10,
  stagingOnly: true,
  findings: [{
    id: "batch-10-site-ledger-adapter-and-validator-route",
    description: "The live validator requires a separately frozen authenticated Batch 10 adapter route before production mutation.",
...
for (const debateNumber of POST_CANARY_BATCH_10_RENDERING_ORDER) {
  ...
  const packet = buildPostCanaryBatch10RenderingPacket({ auditRow: row,
...
const sources = [FINALIZATION_PREPARATION, FINALIZATION_ACTIVATION,
  ...
  METADATA_CORRECTION, ORIGINAL_PREVIEW, POST_CANARY_BATCH_10_STANDING_AUTHORIZATION,
  "src/app.js", "src/styles.css",
  "scripts/lib/assessment-production-post-canary-batch-10-rendering-verification.mjs",
  "scripts/lib/assessment-production-post-canary-batch-10-publication-finalization.mjs",
  "scripts/lib/assessment-production-post-canary-batch-10-standing-authorization.mjs",
  "scripts/lib/v4-lean-production.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-10-rendering-verification.mjs",
  "scripts/test-assessment-production-post-canary-batch-10-rendering-verification-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-10-rendering-verification.mjs",
  "scripts/run-assessment-production-post-canary-batch-10-rendering-verification.mjs",
  "scripts/finalize-assessment-production-post-canary-batch-10-rendering-verification.mjs",
...
const manifest = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-10-rendering-preparation",
  protocolId: POST_CANARY_BATCH_10_RENDERING_PROTOCOL_ID,
  status: "frozen-post-canary-batch-10-rendering-verification-prepared",
  frozenAt,
  ...
  explicitOrder: POST_CANARY_BATCH_10_RENDERING_ORDER,
  viewports: POST_CANARY_BATCH_10_RENDERING_VIEWPORTS,
  requiredChecks: POST_CANARY_BATCH_10_RENDERING_REQUIRED_CHECKS,
  browserPlan: {
    controller: "playwright-cli",
    browserFamily: "Chromium",
    sessionName: "batch10-rendering",
    port: POST_CANARY_BATCH_10_RENDERING_PORT,
  ...
  artifacts: {
    ...
    evidenceRoot: `${POST_CANARY_BATCH_10_RENDERING_ROOT}/evidence`,
    screenshotRoot: "output/playwright/batch-10-rendering",
  ...
  nextAuthorizedAction: "activate-batch-10-rendering-verification"
};
...
console.log(JSON.stringify({ status: manifest.status, debates: 10,
  sections: 51, moves: 182, viewportResultsPlanned: 20,
