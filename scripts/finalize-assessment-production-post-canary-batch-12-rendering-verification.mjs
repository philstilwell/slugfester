...
import {
  ...
  hashFile,
  sha256,
  validatePostCanaryBatch12RenderingEvidence,
  validatePostCanaryBatch12RenderingPacket
...
const ROOT = POST_CANARY_BATCH_12_RENDERING_ROOT;
...
assertV4(activation.status ===
  "frozen-post-canary-batch-12-rendering-verification-authorized" &&
...
"Batch 12 rendering activation changed");
...
for (const debateNumber of POST_CANARY_BATCH_12_RENDERING_ORDER) {
  ...
  const packet = validatePostCanaryBatch12RenderingPacket(JSON.parse(packetBytes));
  for (const viewportName of ["desktop", "mobile"]) {
    ...
    validatePostCanaryBatch12RenderingEvidence({ packet, viewportName,
...
"complete passing Batch 12 rendering evidence required");
...
const audit = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-12-rendering-audit",
  protocolId: activation.protocolId,
  status: "passed-ten-debate-batch-12-rendering-verification",
...
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-12-rendering-execution",
  protocolId: activation.protocolId,
  status: "ten-debate-batch-12-rendering-verification-passed",
  completedAt,
  attempts: 20,
  retries: 0,
  timeoutExtensions: 0,
...
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-12-rendering-analysis",
  protocolId: activation.protocolId,
  ...
  nextAuthorizedAction: "prepare-batch-12-production-compatibility-staging"
};
...
console.log(JSON.stringify({ status: analysis.status, debates: 10,
  sections: 54, moves: 204, viewportResults: 20,
