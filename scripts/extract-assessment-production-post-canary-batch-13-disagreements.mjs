...
const TOOL_SOURCES = [
  "scripts/extract-assessment-production-post-canary-batch-13-disagreements.mjs",
  ...
  "scripts/test-assessment-production-post-canary-batch-13-disagreements.mjs"
];
...
assertV4(
  ...
  "Batch 13 accepted-judgment gate or prior stop rule changed"
);
assertV4(
  JSON.stringify(judgmentAnalysis.pairs.map((pair) => pair.debateNumber)) ===
    JSON.stringify(EXPECTED_DEBATES),
  "Batch 13 independent-judgment population changed"
);
assertV4(
  ...
  "Batch 13 does not contain ten accepted, independent A/B judgment pairs"
);
assertV4(
  ...
  "Batch 13 twenty-context accepted execution contract changed"
);
assertV4(
  ...
  "Batch 13 model, authentication, score-blindness, or tie contract changed"
);
assertV4(
  executionActivation.executionPolicy.attemptsPerContext === 1 &&
    ...
    JSON.stringify(executionActivation.executionPolicy.schedulerRamp) ===
      JSON.stringify([1, 2]),
  "Batch 13 one-attempt or frozen 1-to-2 scheduler stop rule changed"
);
assertV4(
...
assertV4(
  ...
  "Batch 13 accepted execution evidence changed"
);
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const disagreementPath =
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_13_DISAGREEMENT_ROOT}/disagreements/debate-${debateNumber}.json`;
...
assertV4(totalMoves === 199, "Batch 13 extraction must compare 199 moves");
assertV4(
  JSON.stringify(extractedAudioQueue.map(queueKey).sort()) ===
    JSON.stringify(independentGateQueue.map(queueKey).sort()),
  "Batch 13 frozen audio queue was not reproduced"
);

const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-13-disagreement-extraction",
  protocolId:
    ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_13_DISAGREEMENT_PROTOCOL_ID,
  status:
  ...
  standingAuthorization: {
    path: POST_CANARY_BATCH_13_STANDING_AUTHORIZATION,
...
if (shouldWrite) {
  await mkdir(ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_13_DISAGREEMENT_ROOT, {
    recursive: true
  });
  await writeFile(
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_13_DISAGREEMENT_ROOT}/analysis.json`,
