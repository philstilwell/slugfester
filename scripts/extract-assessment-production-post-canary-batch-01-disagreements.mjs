...
import {
  ...
  ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_01_DISAGREEMENT_ROOT,
  extractAssessmentProductionPostCanaryBatch01Disagreements
} from "./lib/assessment-production-post-canary-batch-01-disagreement.mjs";
...
const USER_AUTHORIZATION = Object.freeze({
  instruction:
    "I approve deterministic extraction, validation, freezing, analysis, committing, and pushing of the Batch 1 disagreements from the twenty accepted independent-judgment outputs only, with a direct incremental cost cap of $0. Do not execute models, paid services, audio work, adjudication, score derivation, publication reconstruction, production mutation, or next-batch selection.",
...
const TOOL_SOURCES = [
  "scripts/extract-assessment-production-post-canary-batch-01-disagreements.mjs",
  ...
  "scripts/test-assessment-production-post-canary-batch-01-disagreements.mjs"
];
...
assertV4(
  judgmentAnalysis.status ===
    "twenty-post-canary-batch-01-independent-judgments-passed-awaiting-separate-disagreement-extraction-approval" &&
    judgmentAnalysis.authorization.disagreementExtraction === false &&
    judgmentAnalysis.nextAuthorizedAction ===
      "user-approval-required-before-batch-01-deterministic-disagreement-extraction",
  "Batch 1 accepted-judgment gate or prior stop rule changed"
);
assertV4(
  JSON.stringify(judgmentAnalysis.pairs.map((pair) => pair.debateNumber)) ===
    JSON.stringify(EXPECTED_DEBATES),
  "Batch 1 independent-judgment population changed"
);
assertV4(
  ...
  "Batch 1 does not contain ten accepted, independent A/B judgment pairs"
);
assertV4(
  ...
  "Batch 1 twenty-context accepted execution contract changed"
);
assertV4(
  ...
  "Batch 1 model, authentication, score-blindness, or tie contract changed"
);
assertV4(
  executionActivation.executionPolicy.attemptsPerContext === 1 &&
    ...
    JSON.stringify(executionActivation.executionPolicy.schedulerRamp) ===
      JSON.stringify([1, 2]),
  "Batch 1 one-attempt or frozen 1-to-2 scheduler stop rule changed"
);
assertV4(
...
assertV4(
  ...
  "Batch 1 accepted execution evidence changed"
);
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const disagreementPath =
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_01_DISAGREEMENT_ROOT}/disagreements/debate-${debateNumber}.json`;
...
assertV4(totalMoves === 177, "Batch 1 extraction must compare 177 moves");
assertV4(
  JSON.stringify(extractedAudioQueue.map(queueKey).sort()) ===
    JSON.stringify(independentGateQueue.map(queueKey).sort()),
  "Batch 1 three-move audio queue was not reproduced"
);

const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-01-disagreement-extraction",
  protocolId:
    ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_01_DISAGREEMENT_PROTOCOL_ID,
  status:
  ...
  nextAuthorizedAction:
    "user-approval-required-before-batch-01-local-audio-source-work-item-preparation"
};

if (shouldWrite) {
  await mkdir(ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_01_DISAGREEMENT_ROOT, {
    recursive: true
  });
  await writeFile(
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_01_DISAGREEMENT_ROOT}/analysis.json`,
