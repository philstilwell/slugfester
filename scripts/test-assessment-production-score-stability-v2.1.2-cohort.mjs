...
assert.deepEqual(selection.successorProtocol.sourceSelectionShape, [
  "startEvent",
  "endEvent",
]);
assert.equal(selection.successorProtocol.minimumLexicalTokens, 12);
assert.equal(
  selection.successorProtocol.repositoryDerivedLexicalTokenCount,
  true
);
assert.equal(
...
assert.equal(selection.successorProtocol.requestedLexicalTokensRemoved, true);
...
assert.equal(selection.selectionPolicy.failedV211ValidationCohortExcluded, true);
assert.equal(
