...
const RESUMPTION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-10/dispute-only-adjudication/failure-recovery/resumption";

const activationPath =
  `${RESUMPTION_ROOT}/execution-activation.json`;
...
assertV4(
  manifest.status ===
      "frozen-seven-post-canary-batch-10-dispute-only-adjudication-resumption-contexts-authorized" &&
...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-post-canary-batch-10-adjudication-resumption-${context.debateNumber}-`
    )
  );
  const codexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-post-canary-batch-10-adjudication-resumption-home-${context.debateNumber}-`
    )
  );
  ...
  try {
    ...
    process.stdout.write(
      `[batch-10-adjudication-resumption] starting index ${context.contextIndex} ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
  ...
  process.stdout.write(
    `[batch-10-adjudication-resumption] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  ...
  status: passed
    ...
    : "post-canary-batch-10-dispute-only-adjudication-resumption-gate-complete-with-failure",
