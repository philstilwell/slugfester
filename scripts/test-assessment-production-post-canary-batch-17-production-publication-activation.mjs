...
assert.equal(activation.status, "frozen-batch-17-production-publication-mutation-pass-activated");
assert.equal(activation.directIncrementalCostCapUsd, 0);
assert.deepEqual(activation.frozenOutput.changedDebateNumbers, POST_CANARY_BATCH_17_PRODUCTION_PUBLICATION_ORDER);
assert.equal(activation.frozenOutput.productionLedgerOutputs.length, 4);
...
console.log(JSON.stringify({ status: "passed", activatedMutationPasses: 1, exactProductionWrites: 5 }, null, 2));
