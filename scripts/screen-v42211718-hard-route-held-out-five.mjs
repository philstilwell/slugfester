...
if (sample.status !== "frozen-pending-route-metadata-screening" || !sample.authorization.routeMetadataScreening) throw new Error("hard-route sample screening unauthorized");
...
const passed = decisions.length === 5
  && decisions.filter((item) => item.route === "direct").length === 0
  && decisions.filter((item) => item.route === "partition").length === 5
  && new Set(decisions.map((item) => item.assignedFamily)).size >= 5
...
const screening = {
  schemaVersion: "4.2.21.17.18-hard-route-held-out-five-screening",
  protocolId: sample.protocolId,
  status: passed ? "hard-route-held-out-five-screened-source-preparation-authorized" : "hard-route-held-out-five-screening-failed",
  ...
  sourceBoundary: {
    ...
    noUnusedDirectControlsRemaining: true,
    hardRouteOnlyLimitationRecorded: true,
  },
  decisions,
  audit: {
