...
const root = process.cwd(), gateRoot = "docs/calibration/v3.6.3/schema-smoke-correction";
const manifestText = await readFile(path.resolve(root, gateRoot, "gate-manifest.json"), "utf8"), manifest = JSON.parse(manifestText);
...
assert(manifest.status === "frozen-before-correction-smoke" && manifest.model.slug === "gpt-5.6-terra" && manifest.model.reasoningEffort === "high", "manifest/model lock invalid");
...
async function pool(tasks) {
  let next = 0;
  const results = new Array(tasks.length), workers = Array.from({ length: Math.min(concurrency, tasks.length) }, async () => {
...
function countScoreFields(value) {
  ...
  return Object.entries(value).reduce((sum, [key, child]) => sum + (/^(score|scores|moveScore|sectionScore|overall|winner)$/i.test(key) ? 1 : 0) + countScoreFields(child), 0);
}
...
async function modelTask(family) {
  const definition = manifest.families[family], temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-v363-${family}-`));
  const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), `slugfester-v363-home-${family}-`)), startedAt = new Date().toISOString();
  try {
    const mappings = [
      ["docs/assessment-workflow-v3.6.3.md", "workflow.md"], ["docs/reassessment-rubric-v3.6.3.md", "rubric.md"],
      [`${gateRoot}/correction-manual.md`, "manual.md"], [definition.schema, "schema.json"], [definition.packet, "packet.json"]
    ];
    ...
    process.stdout.write(`\n[v3.6.3] starting isolated ${family} correction smoke\n`);
    ...
    const base = {
      ...
      preInferenceSchemaRejected: invocation.code !== 0 && schemaRejectionPattern.test(combinedLog),
      sameRequestStreamRecoveries: [...combinedLog.matchAll(streamRecoveryPattern)].length, spawnError: invocation.spawnError
    };
    ...
    try {
      ...
      const outputText = await readFile(outputPath, "utf8"), scoringFieldCount = countScoreFields(JSON.parse(outputText));
      const validation = await run(process.execPath, ["scripts/validate-v363-smoke-output.mjs", manifest.outputs[family], definition.packet, family], { cwd: root, env: process.env });
      const valid = validation.code === 0;
      process.stdout.write(`[v3.6.3] ${family} ${valid ? "completed-valid" : "output-validation-failed"}\n`);
      return { ...base, status: valid ? "completed-valid" : "output-validation-failed", outputWritten: true, outputSha256: sha256(outputText), deterministicValidationPassed: valid, scoringFieldCount, validationExitCode: validation.code };
    } catch (error) {
...
const families = Object.keys(manifest.families), results = await pool(families.map((family) => () => modelTask(family)));
const execution = {
  schemaVersion: "3.6.3-schema-smoke-execution", gateId: manifest.gateId, completedAt: new Date().toISOString(),
  authentication: "ChatGPT subscription auth copied into isolated temporary CODEX_HOME; API keys removed", model: manifest.model,
  contextsPlanned: families.length, contextsCompleted: results.filter((item) => item.commandExitCode === 0).length,
  contextsFailed: results.filter((item) => item.commandExitCode !== 0).length, validOutputCount: results.filter((item) => item.status === "completed-valid").length,
  ...
  scoringFieldCount: results.reduce((sum, item) => sum + item.scoringFieldCount, 0), meteredApiCostUsd: 0, results
};
