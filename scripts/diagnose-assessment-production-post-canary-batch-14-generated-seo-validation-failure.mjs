...
const diagnosisPath = `${POST_CANARY_BATCH_14_GENERATED_SEO_CORRECTION_ROOT}/validation-failure-diagnosis.json`;
const [plan, activation] = await Promise.all([readJson(planPath), readJson(activationPath)]);
assertV4(
  plan.status === "frozen-batch-14-generated-seo-derivative-correction-plan-prepared" &&
    activation.status === "frozen-batch-14-generated-seo-correction-pass-activated" &&
    activation.plan.sha256 === sha256(await readBytes(planPath)),
  "spent Batch 14 generated SEO activation required"
);
...
assertV4(
  affectedCalibrationDebates.length === 1 &&
    affectedCalibrationDebates[0].debateNumber === "34",
  "expected exactly Batch 14 Debate 34 to feed the legacy pilot analysis"
);
...
const diagnosis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-14-generated-seo-validation-failure-diagnosis",
  protocolId: "assessment-production-post-canary-batch-14-generated-seo-dependent-pilot-analysis-recovery",
  status: "frozen-batch-14-generated-seo-full-check-dependent-pilot-analysis-staleness-diagnosed",
  diagnosedAt,
  batchNumber: 14,
  directIncrementalCostUsd: 0,
  failedExecution: {
    command: "node scripts/run-assessment-production-post-canary-batch-14-generated-seo-correction.mjs",
  ...
  rootCause: {
    finding: "The complete repository check recomputes the legacy v2.1 pilot report from current production debate scores. Batch 14 replaces Debate 34, which is sampled by that report, so its two checked-in deterministic outputs became stale even though the frozen SEO output inventory passed.",
  ...
  nextAuthorizedAction: "prepare-and-freeze-one-bounded-batch-14-dependent-pilot-analysis-correction"
};
