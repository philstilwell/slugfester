...
assertV4(
  ...
  "Batch 13 independent-judgment execution is unauthorized"
);
assertV4(
...
async function executeContext(context, contextIndex) {
  ...
  assertV4(
    remainingGateMs > 0,
    "absolute Batch 13 independent-judgment gate timeout reached before launch"
  );
  ...
  const temporary = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-batch-13-judgment-${context.debateNumber}-${context.reviewerPass.toLowerCase()}-`
    )
  );
  const isolatedCodexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-batch-13-judgment-home-${context.debateNumber}-${context.reviewerPass.toLowerCase()}-`
    )
  );
  ...
  try {
    ...
    if (
      ...
      const validation = await run(
        process.execPath,
        [
          "scripts/validate-assessment-production-post-canary-batch-13-independent-judgment.mjs",
  ...
  process.stdout.write(
    `[batch-13-judgment] Debate ${context.debateNumber} Pass ${context.reviewerPass} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-13-independent-judgment-model-execution",
