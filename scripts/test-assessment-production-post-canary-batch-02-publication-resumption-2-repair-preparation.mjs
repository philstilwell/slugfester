...
import {
  ...
  mergeAndValidateResumption2Repairs,
  validateResumption2RepairOutput
} from "./lib/assessment-production-post-canary-batch-02-publication-resumption-2-repair.mjs";

const MANIFEST = `${POST_CANARY_BATCH_02_RESUMPTION_2_REPAIR_ROOT}/execution-preparation-manifest.json`;
...
assert.equal(manifest.protocolId, POST_CANARY_BATCH_02_RESUMPTION_2_REPAIR_PROTOCOL_ID);
...
assert.deepEqual(
  ...
  POST_CANARY_BATCH_02_RESUMPTION_2_REPAIR_PARTITIONS.map(
...
assert.deepEqual(manifest.costEstimate.expectedParallelWallMinutes, [5, 14]);
...
assert.equal(manifest.totals.contexts, 6);
assert.equal(manifest.totals.writableFields, 10);
...
for (const context of manifest.contexts) {
  ...
  assert(["136", "83"].includes(packet.debateNumber));
  ...
  assert.deepEqual(schema, buildResumption2RepairSchema(packet));
  ...
  assert.equal(validateResumption2RepairOutput(repair, packet).status, "passed");
...
const merged = mergeAndValidateResumption2Repairs({
  baseOutputs,
  repairs: syntheticRepairs,
  repairPackets,
  publicationPackets
});
assert.equal(merged.transformations.length, 10);
assert.equal(merged.fullValidations["136"].status, "passed");
assert.equal(merged.fullValidations["136"].moves, 18);
assert.equal(merged.fullValidations["83"].status, "passed");
assert.equal(merged.fullValidations["83"].moves, 20);
assert(
