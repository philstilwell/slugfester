...
assertV4(
  ...
  "the Debate 31 repair execution is not authorized"
);
assertV4(
  ...
  "the Debate 31 repair execution controls changed"
);
...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), `batch-01-debate-31-repair-${context.contextIndex}-`)
  );
  const codexHome = await mkdtemp(
    path.join(os.tmpdir(), `batch-01-debate-31-repair-home-${context.contextIndex}-`)
  );
  ...
  try {
    ...
    const prompt = [
      ...
      `Act only as isolated Debate 31 two-field publication repair editor ${context.contextIndex}.`,
    ...
    process.stdout.write(
      `[batch-01-debate-31-publication-repair] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort}\n`
    );
    ...
    if (
      ...
      try {
        validationSummary = validateDebate31RepairOutput(
      ...
      const validation = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-01-debate-31-publication-repair-validation",
      ...
      const provenance = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-01-debate-31-publication-repair-provenance",
        protocolId: activation.protocolId,
        debateNumber: "31",
        packetIndex: context.packetIndex,
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-01-debate-31-publication-repair-model-execution",
  protocolId: activation.protocolId,
  status:
    validContexts === 7
      ? "batch-01-debate-31-seven-context-publication-repair-gate-passed"
      : "batch-01-debate-31-publication-repair-gate-complete-with-failure",
  gateStartedAt,
