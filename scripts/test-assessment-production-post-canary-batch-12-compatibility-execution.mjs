...
const root = POST_CANARY_BATCH_12_COMPATIBILITY_ROOT;
...
const validator = await import(
  `./validate-debates.mjs?batch12Compatibility=${activation.validator.proposedSha256}`
);
...
for (const lock of activation.packetHashes) {
  ...
  const routeLock =
    validator.validatePostCanaryBatch12LedgerAdapterRouteLocks({
      debate: candidate,
      ledgerText,
  ...
  const route = validator.validatePostCanaryBatch12LedgerAdapterRoute({
    debate: candidate,
    ledger,
    ledgerText
  });
  assert.equal(route.repositoryScoreReplayPassed, true);
  const adapter = validatePostCanaryBatch12SiteLedgerAdapter({
    adapter: ledger,
    candidate,
  ...
  JSON.stringify(
    {
      ...
      positiveRoutes,
      negativeControls,
      existingAdapterRoutesPreserved: 13,
      modelContexts: 0,
      paidServiceCalls: 0,
