...
import { POST_CANARY_BATCH_08_PUBLICATION_RESUMPTION_3_ROOT } from
  "./lib/assessment-production-post-canary-batch-08-publication-resumption-3.mjs";
import { assertV4, canonicalJson } from "./lib/v4-lean-production.mjs";
const ROOT = POST_CANARY_BATCH_08_PUBLICATION_RESUMPTION_3_ROOT;
...
assertV4(activation.status ===
  "frozen-one-untouched-post-canary-batch-08-publication-resumption-3-context-authorized" &&
  ...
  activation.executionPolicy?.contexts === 1 && activation.executionPolicy?.attemptsPerContext === 1 &&
...
"the one-context Batch 8 publication resumption is not authorized");
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-08-publication-resumption-3-${context.debateNumber}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `batch-08-publication-resumption-3-home-${context.debateNumber}-`));
  ...
  try {
    ...
    process.stdout.write(`[batch-08-publication-resumption-3] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber}\n`);
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 ||
      ...
      try { validationSummary = validatePostCanaryBatch08PublicationOutput(JSON.parse(outputBytes),
      ...
      const validationRecord = { schemaVersion:
          "1.0-assessment-production-post-canary-batch-08-publication-resumption-3-validation",
      ...
      const provenanceRecord = { schemaVersion:
          "1.0-assessment-production-post-canary-batch-08-publication-resumption-3-provenance",
  ...
  process.stdout.write(`[batch-08-publication-resumption-3] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
...
const execution = { schemaVersion:
    "1.0-assessment-production-post-canary-batch-08-publication-resumption-3-model-execution",
  protocolId: activation.protocolId, status: passed
    ? "one-post-canary-batch-08-publication-resumption-3-context-passed"
    : "post-canary-batch-08-publication-resumption-3-complete-with-failure",
  ...
  maximumObservedConcurrency, schedulerRamp: [1], wallElapsedMs,
  ...
  ownDebateScoresImmutable: true, acceptedPriorDebates: 9,
