  ...
  assertV4(
    preparation.schemaVersion ===
      ...
      preparation.executionPolicy.allowAlreadyRunningIndependentContextToFinish ===
        true &&
      preparation.executionPolicy.allFourPlansMustPassBeforeSidePacketPreparation ===
        true &&
    ...
    "Batch 17 candidate-census planner preparation is invalid"
  );
  assertV4(
    ...
    "Batch 17 source preparation drifted"
  );
...
async function preflight() {
  ...
  assertV4(
    ...
    "frozen Batch 17 preparation validator failed"
  );
  console.log(
    JSON.stringify(
      {
...
async function activate() {
  ...
  const activation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-17-candidate-census-plan-execution-activation",
    ...
    userAuthorization: {
      scope:
        "activate and execute exactly the four frozen Batch 17 candidate-census planner contexts under the frozen standing authorization",
    ...
    nextRequiredAction:
      "execute-exactly-four-frozen-batch-17-candidate-census-plan-contexts-once",
  };
...
async function loadAndValidateActivation({ requireFutureAbsent = false } = {}) {
  ...
  assertV4(
    ...
    "Batch 17 candidate-census planner execution is unauthorized"
  );
...
async function execute() {
  ...
  async function executeContext(context, contextIndex) {
    ...
    const sourceDirectory = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-post-canary-batch-17-candidate-plan-${context.debateNumber}-`
      )
    );
    const isolatedCodexHome = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-post-canary-batch-17-candidate-plan-home-${context.debateNumber}-`
      )
    );
    ...
    process.stdout.write(
      `[post-canary-batch-17-candidate-plan] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
    );
    return record;
  }
  ...
  const execution = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-17-candidate-census-plan-model-execution",
    protocolId: PROTOCOL_ID,
    status:
      validContexts === CONTEXTS
        ? EXECUTION_STATUS
        : "post-canary-batch-17-candidate-census-plan-gate-complete-with-failure",
    productionContinuation: true,
    developmentValidationOnly: false,
    productionCanary: false,
    stagingOnly: true,
    ...
    nextRequiredAction:
      validContexts === CONTEXTS
        ? "analyze-and-replay-four-batch-17-candidate-census-plans-model-free-only"
        : "stop-preserve-failed-batch-17-candidate-census-plan-gate",
  };
...
async function analyze() {
  ...
  const analysis = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-17-candidate-census-plan-analysis",
    protocolId: PROTOCOL_ID,
    status:
...
if (command === "preflight") await preflight();
  ...
  throw new Error(
    "usage: run-assessment-production-post-canary-batch-17-inventory-plans.mjs <preflight|activate|run|analyze|validate>"
  );
}
