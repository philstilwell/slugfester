...
assertV388(manifest.status === "frozen-six-context-recovery-authorized" && manifest.authorization.schemaCompatibilityRecoveryModelExecution && !manifest.authorization.scoreDerivation && !manifest.authorization.furtherAutomaticRetry, "performance schema recovery unauthorized");
...
for (const output of manifest.futureOutputPathsExcludedFromSourceHashes) {
  try { await access(path.resolve(root, output)); throw new Error(`future recovery output already exists: ${output}`); }
...
for (const context of manifest.contexts) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-v388-performance-recovery-${context.debateNumber}-${context.pass}-`));
  const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), `slugfester-v388-home-performance-recovery-${context.debateNumber}-${context.pass}-`));
  ...
  try {
    ...
    const prompt = `Read workflow.md, rubric.md, manual.md, packet.json, schema.json, transcript.txt, and events.json completely and no other files. Act only as the fresh isolated Pass ${context.pass} performance judge for Debate ${context.debateNumber}. This is a schema-compatibility recovery after the original endpoint rejected the schema before inference; no prior judgment exists. Judge every locked move exactly once and in packet order. Apply the response component tests, locked burden band, seven dimension anchors, explicit charity-tested rule, and burden-adjustment exclusion rule literally. Return exactly one final schema-conforming JSON object with no progress commentary or provisional JSON. Copy locked fields exactly. Do not calculate or emit move, section, overall, or participant totals; do not name a winner; and do not write participant critiques, Overall Commentary, AI Extension, or publication prose. The other pass and all legacy assessments are unavailable.`;
    process.stdout.write(`\n[v3.8.8-performance-recovery] starting ${manifest.model.label} Debate ${context.debateNumber} Pass ${context.pass}\n`);
    ...
    if (invocation.timedOut || invocation.code !== 0 || invocation.signal !== null) record = { ...base, status: invocation.timedOut ? "timed-out" : "transport-failed", outputWritten: false, deterministicValidationPassed: false, gateAcceptancePassed: false };
      ...
      const validation = await run(process.execPath, ["scripts/validate-v388-performance-judgment-output.mjs", context.output, context.packet, context.pass], { cwd: root, env: process.env });
...
const execution = { schemaVersion: "3.8.8-performance-schema-recovery-model-execution", protocolId: manifest.protocolId, stage: manifest.stage, startedAt: results[0]?.startedAt ?? null, completedAt: new Date().toISOString(), contextsPlanned: 6, validOutputContexts: results.filter((item) => item.gateAcceptancePassed).length, moveJudgmentsAcrossPasses: results.reduce((sum, item) => sum + (item.gateAcceptancePassed ? item.moveCount : 0), 0), recoveryAttempts: results.length, retriesWithinRecovery: 0, originalPreInferenceAttempts: manifest.defectBoundary.failedAttempts, totalRecordedAttempts: manifest.defectBoundary.failedAttempts + results.length, meteredApiCostUsd: 0, transcriptionCostUsd: 0, results };
await writeFile(path.resolve(root, manifest.artifacts.execution), `${JSON.stringify(execution, null, 2)}\n`);
assertV388(execution.validOutputContexts === 6 && execution.moveJudgmentsAcrossPasses === 162, "performance schema recovery failed; disagreement extraction remains blocked");
console.log(JSON.stringify({ status: "v3.8.8-performance-schema-recovery-passed", validOutputContexts: 6, moves: 81, moveJudgmentsAcrossPasses: 162, recoveryRetries: 0, meteredApiCostUsd: 0, disagreementExtractionAuthorized: true, scoreDerivationAuthorized: false }, null, 2));
