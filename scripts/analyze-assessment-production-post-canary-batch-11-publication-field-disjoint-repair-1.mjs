...
import { DEBATE_PLANS, ROOT, validateFieldDisjointRepairOutput } from
  "./lib/assessment-production-post-canary-batch-11-publication-field-disjoint-repair-1.mjs";
...
assertV4(
  execution.contextsPlanned === 18 &&
    [1, 18].includes(execution.contextsAttempted) &&
  ...
  "eighteen-shard repair execution boundary changed"
);
...
for (const plan of DEBATE_PLANS) {
  ...
  if (acceptedContexts.length === debateContexts.length) {
    try {
      ...
      completeValidation = validatePostCanaryBatch11PublicationOutput(
...
if (allRepaired) {
  ...
  for (const debateNumber of ALL_DEBATES) {
    ...
    const correctedData = correctedByDebate.get(debateNumber);
    const outputPath = `${PUBLICATION_ROOT}/outputs/debate-${debateNumber}.json`;
    const outputBytes = correctedData?.correctedBytes ?? await readFile(path.resolve(outputPath));
    ...
    validationReplay.push({
      ...
      source: correctedData
        ? "accepted-field-disjoint-repair-merge"
        : "accepted-original-batch-11-publication-output",
      validation
    });
  }
  const completePassed = validationReplay.length === 10 &&
    validationReplay.every((item) => item.validation.status === "passed") &&
    validationReplay.reduce((sum, item) => sum + item.validation.moves, 0) === 190 &&
    validationReplay.reduce((sum, item) => sum + item.validation.critiques, 0) === 190 &&
  ...
  completeCohort = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-11-complete-publication-cohort-analysis-after-field-disjoint-repair",
    protocolId: activation.protocolId,
    status: "post-canary-batch-11-complete-ten-debate-publication-output-gate-passed-after-field-disjoint-repair",
    ...
    totals: {
      ...
      fieldDisjointRepairContexts: 18,
      totalCodexInvocations: 28,
      invocationsProducingNoFinalModelOutput: 0,
      retries: 0,
      timeoutExtensions: 0,
    ...
    integrity: {
      ...
      failedPublicationStringsRetained: false,
      allValidationCleanFieldsRetainedDeterministically: true,
      everyRepairFieldReplacedExactlyOnce: true,
    ...
    nextAuthorizedAction: "prepare-batch-11-deterministic-publication-compilation"
  };
}

const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-11-publication-field-disjoint-repair-analysis",
  protocolId: activation.protocolId,
  status: allRepaired
    ? "four-debate-field-disjoint-repair-passed-and-complete-cohort-validated"
    : repairedDebates > 0
  ...
  totals: {
    ...
    debatesFailed: 4 - repairedDebates,
    critiqueFieldsTargeted: 32,
    quoteFieldsTargeted: 0,
    writableFieldsTargeted: 32,
  ...
  authorization: {
    deterministicCompilationPreparation: allRepaired,
    publicationFinalization: false,
    renderingVerification: false,
    paidServices: false,
    productionMutation: false,
  ...
  nextAuthorizedAction: allRepaired
    ? "prepare-batch-11-deterministic-publication-compilation"
...
if (shouldWrite) {
  ...
  for (const [debateNumber, data] of correctedByDebate) {
    ...
    const acceptedValidation = {
      schemaVersion: "1.0-assessment-production-post-canary-batch-11-publication-validation-after-field-disjoint-repair",
      protocolId: activation.protocolId,
      status: "passed",
      debateNumber,
    ...
    const acceptedProvenance = {
      schemaVersion: "1.0-assessment-production-post-canary-batch-11-publication-provenance-after-field-disjoint-repair",
      protocolId: activation.protocolId,
      debateNumber,
      debateId: data.corrected.debateId,
    ...
    await writeFile(path.resolve(completeValidationPath), `${JSON.stringify({
      schemaVersion: "1.0-assessment-production-post-canary-batch-11-field-disjoint-repair-complete-validation",
      protocolId: activation.protocolId,
      status: "passed",
      debateNumber,
      correctedFields: data.replacements.map((item) => item.field),
    ...
    await writeFile(path.resolve(mergeAuditPath), `${JSON.stringify({
      schemaVersion: "1.0-assessment-production-post-canary-batch-11-field-disjoint-repair-merge-audit",
      protocolId: activation.protocolId,
      status: "passed",
      debateNumber,
      sourceFailedOutput: {
