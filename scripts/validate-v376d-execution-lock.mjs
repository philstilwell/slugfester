...
assert(manifest.status === "frozen-before-model-execution" && manifest.disjointTestExecutionAuthorized && manifest.retiredCases && manifest.caseDisjointFromV376Development && manifest.dyadicOnly, "execution manifest identity invalid");
...
assert(sha256(dryText) === manifest.executionDryFixture.sha256 && dry.passed && dry.modelContextsExecuted === 0 && dry.syntheticAdjudicationContexts === 3 && dry.twoVoteResolutions === 3 && dry.noContactDisputeVerified && dry.polarityDisputeVerified && dry.tierDisputeVerified, "execution dry fixture invalid");
execFileSync(process.execPath, ["scripts/test-v376d-execution-tooling.mjs"], { cwd: root, stdio: "ignore" });
...
assert(manifest.thresholds.compositeCases === 12 && manifest.thresholds.initialCompositeAgreementsMinimum === 11 && manifest.thresholds.initialDisagreementsMaximum === 1 && manifest.thresholds.finalTwoVoteBundlesRequired === 12 && manifest.thresholds.scoringFieldsMaximum === 0, "semantic threshold invalid");
assert(manifest.audioPolicy.mediumConfidenceMovesRequireAudioVerification && manifest.audioPolicy.requiredAudioVerifications === manifest.audioPolicy.completedAudioVerifications, "audio-verification lock invalid");
for (const [reviewerPass, debates] of Object.entries(manifest.initialContexts)) for (const [debateNumber, context] of Object.entries(debates)) {
  ...
  assert(packet.reviewerPass === reviewerPass && packet.debateNumber === debateNumber && !canonicalJson(packet).includes("matchesProvisionalReference") && !canonicalJson(packet).includes("semanticTuple"), `${reviewerPass}.${debateNumber}: packet leak or identity invalid`);
}
...
console.log(JSON.stringify({ status: "passed", artifactIntegrityPassed: true, executionToolingPassed: true, modelExecutionAuthorizedByNarrowManifest: true, initialContexts: 6, adjudicationContextsMaximum: 3, compositeCases: 12, minimumInitialAgreement: 11, meteredApiCostUsdMaximum: 0, manifestSha256: sha256(manifestText) }, null, 2));
