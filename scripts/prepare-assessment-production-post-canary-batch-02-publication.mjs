...
import {
  validatePostCanaryBatch02Scores
} from "./lib/assessment-production-post-canary-batch-02-score-gate.mjs";
import {
  buildPostCanaryBatch02PublicationPacket,
  buildPostCanaryBatch02PublicationSchema,
  postCanaryBatch02ReferenceCatalog,
  POST_CANARY_BATCH_02_PUBLICATION_DEBATES,
  POST_CANARY_BATCH_02_PUBLICATION_MODEL,
  POST_CANARY_BATCH_02_PUBLICATION_PROTOCOL_ID,
...
const FINAL_LEDGER_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-02/final-ledger/final-ledger-manifest.json";
const FINAL_LEDGER_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-02/final-ledger/analysis.json";
const SCORES =
...
const SCORE_PREPARATION =
  "docs/assessment-production/post-canary-continuation-v1/batch-02/score-pass/score-pass-preparation-manifest.json";
const SCORE_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-02/score-pass/analysis.json";
const AUDIO_AUDIT =
  "docs/assessment-production/post-canary-continuation-v1/batch-02/audio-verification/audio-verification.json";
const SOURCE_PACKET_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-02/independent-judgments/source-packets";
...
const STATIC_SOURCE_FILES = [
  ...
  "scripts/prepare-assessment-production-post-canary-batch-02-publication.mjs",
  "scripts/test-assessment-production-post-canary-batch-02-publication-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-02-publication.mjs",
  "scripts/run-assessment-production-post-canary-batch-02-publication.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-02-publication.mjs"
];
...
assertV4(
  scoreAnalysis.status === "post-canary-batch-02-score-stability-gate-passed" &&
    scoreAnalysis.productionCanary === false &&
    scoreAnalysis.batchNumber === 2 &&
    ...
    scoreAnalysis.nextAuthorizedAction ===
      "user-approval-required-before-batch-02-publication-packet-preparation",
  "the accepted Batch 2 score gate is unavailable or its stop boundary changed"
);
assertV4(
  scores.status === "post-canary-batch-02-single-score-pass-stability-gate-passed" &&
  ...
  "the locked Batch 2 score artifact is unavailable"
);
assertV4(
  scoreManifest.status ===
      "frozen-post-canary-batch-02-single-deterministic-score-pass-authorized" &&
  ...
  "the frozen Batch 2 single-pass score controls changed"
);
assertV4(
  ledger.status ===
      "passed-post-canary-batch-02-deterministic-final-ledger-assembly" &&
  ...
  "the locked Batch 2 final ledger changed"
);
assertV4(
  audioAudit.status ===
      "passed-all-ten-post-canary-batch-02-confidence-moves-audio-verified-after-correction-2-reference-overlay" &&
    audioAudit.totals?.requiredMoves === 10 &&
    audioAudit.totals?.verified === 10 &&
    audioAudit.totals?.unresolved === 0,
  "the ten required Batch 2 audio checks are incomplete"
);
assertV4(
  productionManifest.model?.label === POST_CANARY_BATCH_02_PUBLICATION_MODEL.label &&
    productionManifest.model?.slug === POST_CANARY_BATCH_02_PUBLICATION_MODEL.slug &&
    productionManifest.model?.reasoningEffort ===
      POST_CANARY_BATCH_02_PUBLICATION_MODEL.reasoningEffort &&
    productionManifest.model?.authentication ===
      POST_CANARY_BATCH_02_PUBLICATION_MODEL.authentication,
  "the frozen production model or authentication changed"
);
assertV4(
  selection.selected?.map((item) => item.debateNumber).join(",") ===
      POST_CANARY_BATCH_02_PUBLICATION_DEBATES.join(",") &&
    selection.selected.every((item) => item.speakerCount === 2) &&
    selection.authorization?.productionMutation === false,
  "the frozen Batch 2 selection changed"
);
assertV4(
  ledger.debates.map((debate) => debate.debateNumber).join(",") ===
    ...
    scores.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_02_PUBLICATION_DEBATES.join(","),
  "the Batch 2 ledger or score order changed"
);
assertV4(
...
const finalLedgerInputs = await loadPostCanaryBatch02FinalLedgerInputs();
validatePostCanaryBatch02FinalLedger(
  ledger,
...
const catalog = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-local-reference-catalog",
  references: postCanaryBatch02ReferenceCatalog()
};
...
for (const debateNumber of POST_CANARY_BATCH_02_PUBLICATION_DEBATES) {
  ...
  const packet = buildPostCanaryBatch02PublicationPacket({
    ledgerDebate,
    scoreDebate,
    sourcePacket,
  ...
  const schema = buildPostCanaryBatch02PublicationSchema(packet);
...
assertV4(
  ...
  "Batch 2 publication move coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.sections, 0) === 51,
  "Batch 2 publication section coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.audioVerifiedMoves, 0) === 10,
  "Batch 2 audio-verification coverage changed"
);
assertV4(
  contexts.every(
    (context) =>
  ...
  "Batch 2 publication section or quote eligibility is invalid"
);
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-publication-execution-preparation-manifest",
  protocolId: POST_CANARY_BATCH_02_PUBLICATION_PROTOCOL_ID,
  status:
  ...
  nextAuthorizedAction:
    "user-approval-required-before-activation-and-execution-of-the-ten-frozen-batch-02-publication-contexts"
};
