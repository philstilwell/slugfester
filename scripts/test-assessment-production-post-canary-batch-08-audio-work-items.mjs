...
assert.equal(preparation.userAuthorization.workItemsAuthorized, 6);
assert.equal(
...
assert.equal(preparation.totals.moves, 6);
assert.equal(work.moves.length, 6);
...
assert.equal(preparation.validation.exactAuthorizedMoveCount, 6);
assert.equal(
  preparation.validation.canonicalLocalTextAndMetadataChainsVerified,
  3
);
assert.equal(preparation.validation.repositoryRenderedLockedExcerpts, 6);
assert.equal(preparation.validation.repositoryRenderedTimestampWindows, 6);
assert.equal(preparation.validation.expectedSpeakersLocked, 6);
assert.equal(
