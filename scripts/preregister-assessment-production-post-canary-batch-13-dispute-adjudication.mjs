...
import { POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT } from "./lib/assessment-production-post-canary-batch-13-dispute-adjudication.mjs";
...
const preparationPath = `${POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT}/preparation-manifest.json`;
const manifestPath = `${POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT}/execution-preparation-manifest.json`;
const activationPath = `${POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT}/execution-activation.json`;
const executionPath = `${POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT}/model-execution.json`;
const analysisPath = `${POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT}/analysis.json`;
...
assertV4(
  preparation.status ===
    ...
    preparation.totals.disputedMoves === 191 &&
    preparation.totals.candidateSelections === 541 &&
  ...
  "Batch 13 adjudication packets are not ready for execution preparation"
);
assertV4(
  ...
  "Batch 13 accepted source or cost boundary changed"
);
assertV4(
  ...
  "Batch 13 adjudication model or score-blind boundary changed"
);
assertV4(
  preparation.totals.maximumCopiedInputBytes <= 400000,
  "Batch 13 context exceeds the frozen transport ceiling"
);
assertV4(
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-13-dispute-only-adjudication-execution-preparation-manifest",
  protocolId: preparation.protocolId,
  status:
    "frozen-ten-post-canary-batch-13-dispute-only-adjudication-contexts-prepared-not-authorized",
  frozenAt,
  activatedAt: null,
  ...
  nextAuthorizedAction:
    "standing-authorization-permits-batch-13-dispute-only-adjudication-activation-after-frozen-gate-passes"
};
