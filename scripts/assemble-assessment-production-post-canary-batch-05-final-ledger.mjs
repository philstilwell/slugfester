...
const finalLedgerPath =
  `${POST_CANARY_BATCH_05_FINAL_LEDGER_ROOT}/final-ledger.json`;
...
assertV4(
  ...
  "post-canary Batch 5 final-ledger manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen Batch 5 final-ledger source hash mismatch`
  );
}
const inputs = await loadPostCanaryBatch05FinalLedgerInputs();
const ledger = buildPostCanaryBatch05FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
