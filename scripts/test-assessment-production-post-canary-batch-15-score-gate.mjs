...
import {
  ...
  evaluatePostCanaryBatch15ScoreStability
...
const accepted = evaluatePostCanaryBatch15ScoreStability(passing);
...
assert.equal(
  evaluatePostCanaryBatch15ScoreStability(winnerMutation).acceptancePassed,
  false
);
const distanceMutation = structuredClone(passing);
distanceMutation[2].final = scores(95, 80);
assert.equal(
  evaluatePostCanaryBatch15ScoreStability(distanceMutation).acceptancePassed,
  false
);
assert.throws(() =>
  evaluatePostCanaryBatch15ScoreStability(passing, {
    ...POST_CANARY_BATCH_15_SCORE_STABILITY_THRESHOLDS,
