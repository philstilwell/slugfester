...
assertV4(
  ...
  "frozen remedy-v5 preparation required"
);
execFileSync(
  process.execPath,
...
for (const [file, digest] of Object.entries(preparation.sourceHashes)) {
  assertV4(
    ...
    `source hash mismatch before remedy-v5 activation: ${file}`
  );
}
for (const row of preparation.packets) {
  ...
  assertV4(
    ...
    `${row.debateNumber}: remedy-v5 packet hash mismatch`
  );
  validateCheckpointV22RenderingRemedyV5Packet(JSON.parse(bytes));
}
...
const activation = {
  schemaVersion:
    "1.0-production-checkpoint-v2.2-rendering-remedy-v5-execution-activation",
  protocolId: CHECKPOINT_V22_RENDERING_REMEDY_V5_PROTOCOL_ID,
  status:
  ...
  nextAuthorizedAction:
    "single-remedy-v5-rendering-execution-under-frozen-stop-rules"
};

if (shouldWrite) {
  assertV4(!(await exists(activationPath)), "remedy-v5 activation already exists");
