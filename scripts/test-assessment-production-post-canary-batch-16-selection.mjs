...
assert.equal(selection.status, "sixteenth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed");
assert.equal(analysis.status, "sixteenth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization");
assert.equal(analysis.selection.sha256, sha256(selectionBytes));
assert.equal(selection.batchNumber, 16);
assert.equal(selection.productionSequenceOrdinal, 17);
...
assert.equal(selection.eligibility.previouslyPublishedAndRemovedCount, 150);
assert.equal(selection.eligibility.remainingBeforeBatchCount, 14);
assert.equal(selection.eligibility.selectedCount, 10);
assert.equal(selection.eligibility.remainingUnselectedCount, 4);
...
assert.equal(priorCompletionExecution.status, "passed-complete-batch-15-transaction-ready-for-atomic-commit");
...
assert.equal(priorCompletionAnalysis.status, "batch-15-production-publication-accepted-for-atomic-commit-after-generated-seo-correction");
assert.equal(priorCompletionAnalysis.decision.generatedSeoCorrectionGatePassed, true);
assert.equal(priorCompletionAnalysis.decision.batch15ProductionPublicationGatePassed, true);
...
assert.equal(sha256(serializedJson(ranked.slice(150))), selection.eligibility.remainingBeforeBatchRankedSha256);
assert.deepEqual(priorSelection.selected.map((item) => item.debateNumber), ranked.slice(140, 150).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.debateNumber), ranked.slice(150, 160).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.rankOrdinal), [151, 152, 153, 154, 155, 156, 157, 158, 159, 160]);
assert.deepEqual(selection.selected.map((item) => item.rankSha256), ranked.slice(150, 160).map((item) => item.rankSha256));
...
assert.deepEqual(selection.deterministicOrdering.selectedRankOrdinals, [151, 160]);
...
assert.equal(selection.nextAuthorizedAction, "prepare-batch-16-standing-authorization-and-source-packets-under-user-authorization");

console.log(JSON.stringify({ status: "passed", selectedDebates: selection.selected.map((item) => item.debateNumber), selectedRankOrdinals: [151, 160], sourceFiles: 30, replacements: 0, sourcePacketsPrepared: 0, modelContexts: 0, paidServiceCalls: 0, directCostUsd: 0, nextAuthorizedAction: selection.nextAuthorizedAction }, null, 2));
