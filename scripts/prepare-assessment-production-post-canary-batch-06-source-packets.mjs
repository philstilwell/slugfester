...
import {
  buildBatch06TokenCountedChunkLedger,
  findBatch06ZeroLexicalTokenRows,
} from "./lib/assessment-production-post-canary-batch-06-source-preparation.mjs";
import {
...
const PROTOCOL_ID = "assessment-production-post-canary-batch-06-source-preparation";
const CACHE_VERSION = "post-canary-continuation-v1-batch-06";
...
const SCRIPT = "scripts/prepare-assessment-production-post-canary-batch-06-source-packets.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-06-source-packets.mjs";
...
assertV4(
  selection.status === "sixth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    ...
    selection.nextAuthorizedAction === "prepare-batch-06-standing-authorization-and-source-packets-under-user-authorization" &&
    JSON.stringify(selection.selected.map((item) => item.debateNumber)) === JSON.stringify(REQUIRED_ORDER),
  "Batch 6 frozen selection is not ready for its separately authorized source-packet step"
);
assertV4(
  ...
  "Batch 6 model boundary or stop rules drifted"
);
assertV4(
...
for (const selected of selection.selected) {
  ...
  packet.schemaVersion = "1.0-assessment-production-post-canary-batch-06-score-blind-source-packet";
  ...
  for (const chunk of plan.chunks) {
    ...
    const tokenLedgerBytes = buildBatch06TokenCountedChunkLedger(chunkBytes);
    const zeroLexicalTokenRows = findBatch06ZeroLexicalTokenRows(chunkBytes);
...
const preparation = {
  ...
  status: shouldWrite ? "post-canary-batch-06-ten-complete-score-blind-source-packets-prepared-awaiting-validation" : "preview",
  ...
  AIOnly: true,
  userAuthorization: {
    scope: "prepare, validate, freeze, commit, and push score-blind source packets for Batch 6 under the complete-workflow standing authorization",
  ...
  nextAuthorizedAction: "run-batch-06-deterministic-source-packet-validation-model-free-only",
};
