...
import { DEBATES, ORIGINAL_CONTEXT_INDEXES, ROOT } from
  "./lib/assessment-production-post-canary-batch-13-publication-resumption-1.mjs";
...
assertV4(
  execution.contextsPlanned === 7 && execution.retries === 0 &&
  ...
  "seven-context resumption execution boundary changed"
);
...
const sevenValid = replayed.filter((item) => item.gateAcceptancePassed);
const resumptionPassed =
  execution.status === "seven-original-unattempted-batch-13-publication-contexts-passed" &&
  execution.contextsAttempted === 7 && execution.contextsUnattempted === 0 &&
  execution.validContexts === 7 && execution.invalidContexts === 0 &&
...
const analysis = {
  ...
  status: resumptionPassed
    ? "seven-original-unattempted-batch-13-publication-contexts-passed"
    : "seven-context-publication-resumption-failed",
  ...
  totals: {
    debates: 7,
    lockedMoves: 141,
    critiques: sevenValid.reduce((sum, item) => sum + item.validation.critiques, 0),
    exactSourceQuotes: sevenValid.reduce((sum, item) => sum + item.validation.quoteExactSourceMatches, 0),
    overallCommentarySides: sevenValid.reduce((sum, item) => sum + item.validation.overallCommentarySides, 0),
    aiExtensionSides: sevenValid.reduce((sum, item) => sum + item.validation.aiExtensionSides, 0),
...
if (resumptionPassed) {
  const allDebates = ["26", "190", "87", ...DEBATES];
    ...
    completeReplay.push({
      ...
      source: debateNumber === "87"
        ...
        : ["26", "190"].includes(debateNumber)
          ...
          : "original-first-attempt-seven-context-resumption",
      validation
    });
  }
  ...
  completeCohort = {
    ...
    integrity: {
      ...
      sevenResumedContextsWereOriginalFirstAttempts: true,
