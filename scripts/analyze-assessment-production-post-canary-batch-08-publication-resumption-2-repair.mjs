...
import { POST_CANARY_BATCH_08_DEBATE_120_REPAIR_ROOT, mergeAndValidateDebate120Repair } from
  "./lib/assessment-production-post-canary-batch-08-publication-resumption-2-repair.mjs";
import { assertV4 } from "./lib/v4-lean-production.mjs";
const shouldWrite = process.argv.includes("--write"); const ROOT = POST_CANARY_BATCH_08_DEBATE_120_REPAIR_ROOT;
...
if (e.contextsAttempted === 2 && e.validContexts === 2 && e.invalidContexts === 0 &&
  ...
  try { const baseBytes = await readFile(path.resolve(p.inputs.immutableRejectedOutput));
    ...
    merge = mergeAndValidateDebate120Repair({ baseOutput: JSON.parse(baseBytes),
...
const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-08-debate-120-repair-analysis",
  protocolId: a.protocolId, status: passed
    ? "batch-08-debate-120-two-packet-repair-and-complete-validation-passed"
    : "batch-08-debate-120-repair-or-complete-validation-failed",
  ...
  gate: { contextsPlanned: 2, contextsAttempted: e.contextsAttempted,
    ...
    correctedFields: 3, completeValidationPassed: passed,
  ...
  authorization: { oneContextResumptionManifestPreparation: passed,
    oneContextModelExecution: false, retry: false, timeoutExtension: false,
    recursiveCorrection: false, paidServices: false, productionMutation: false,
    nextBatchSelection: false },
  nextAuthorizedAction: passed ? "prepare-and-resume-only-unattempted-debate-145-publication-context"
    : "stop-after-failed-debate-120-repair" };
if (shouldWrite && passed) { const mergedBytes = Buffer.from(`${JSON.stringify(merge.merged, null, 2)}\n`);
  const validation = { schemaVersion: "1.0-assessment-production-post-canary-batch-08-debate-120-complete-validation",
    protocolId: a.protocolId, status: "passed", debateNumber: "120",
    mergedOutputSha256: sha256(mergedBytes), validationSummary: merge.fullValidation,
    authorizedFieldsChanged: 3, immutableFieldsChanged: 0,
    modelAuthoredScores: 0, lockedScoresUnchanged: true };
  const audit = { schemaVersion: "1.0-assessment-production-post-canary-batch-08-debate-120-repair-merge-audit",
    protocolId: a.protocolId, status: "passed", debateNumber: "120",
    ...
    authorizedTransformations: merge.transformations, authorizedFieldsChanged: 3,
...
console.log(JSON.stringify({ status: analysis.status, contextsAttempted: e.contextsAttempted,
  ...
  correctedFields: 3, movesValidated: analysis.gate.movesValidated,
