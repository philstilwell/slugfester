...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-17/discovery";
...
assert.equal(manifest.status, "frozen-seventeen-post-canary-batch-17-discovery-contexts-prepared-not-authorized");
...
assert.equal(manifest.contexts.length, 17);
assert.deepEqual(manifest.contexts.map((context) => context.contextIndex), Array.from({ length: 17 }, (_, index) => index));
...
assert.equal(manifest.executionPolicy.contexts, 17);
...
assert.equal(manifest.executionPolicy.copiedInputBytesMaximum, 66203);
assert.equal(preparation.totals.maximumCopiedInputBytes, 66203);
...
assert.equal(manifest.copiedInputBoundary.frozenObservedCopiedInputBytesMaximum, 66203);
assert.equal(manifest.copiedInputBoundary.executionCopiedInputBytesMaximum, 66203);
assert.deepEqual(manifest.copiedInputBoundary.maximumContext, { debateNumber: "62", chunkId: "chunk-004" });
...
assert.equal(manifest.costEstimate.contexts, 17);
...
assert.deepEqual(manifest.costEstimate.expectedParallelWallMinutes, [6, 14]);
assert.deepEqual(manifest.costEstimate.expectedAggregateModelMinutes, [17, 33]);
...
assert.equal(preparedByContext.size, 17);
for (const context of manifest.contexts) {
  ...
  assert(buildBatch17TokenCountedChunkLedger(chunkBytes).equals(tokenBytes));
...
assert.equal(manifest.nextAuthorizedAction, "prepare-and-freeze-batch-17-discovery-execution-activation-under-standing-authorization");

const validation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-discovery-execution-preparation-validation",
  protocolId: manifest.protocolId,
  status: "batch-17-discovery-execution-manifest-validation-passed-frozen-standing-authorization-active",
