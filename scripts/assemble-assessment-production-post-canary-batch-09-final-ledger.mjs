...
const finalLedgerPath =
  `${POST_CANARY_BATCH_09_FINAL_LEDGER_ROOT}/final-ledger.json`;
...
assertV4(
  ...
  "post-canary Batch 9 final-ledger manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen Batch 9 final-ledger source hash mismatch`
  );
}
const inputs = await loadPostCanaryBatch09FinalLedgerInputs();
const ledger = buildPostCanaryBatch09FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
