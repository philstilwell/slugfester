...
import { buildBatch11TokenCountedChunkLedger } from "./lib/assessment-production-post-canary-batch-11-source-preparation.mjs";
import {
...
const ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-11/discovery";
...
const SCRIPT =
  "scripts/run-assessment-production-post-canary-batch-11-discovery.mjs";
...
const EXPECTED_PREPARATION_STATUS =
  "frozen-thirty-nine-post-canary-batch-11-discovery-contexts-prepared-not-authorized";
const EXPECTED_ACTIVATION_STATUS =
  "frozen-thirty-nine-post-canary-batch-11-discovery-contexts-authorized";
  ...
  assertV4(
    preparation.status === EXPECTED_PREPARATION_STATUS &&
      preparation.protocolId ===
        "assessment-production-post-canary-batch-11-discovery" &&
      ...
      preparation.executionPolicy.copiedInputBytesMaximum === 69824 &&
    ...
    "Batch 11 frozen discovery preparation is invalid"
  );
...
async function validateContextInputs(preparation, context) {
  ...
  assertV4(
    buildBatch11TokenCountedChunkLedger(validationChunkBytes).equals(
      modelLedgerBytes
...
async function preflight() {
  ...
  assertV4(
    preparationValidation.status ===
      "batch-11-discovery-execution-manifest-validation-passed-frozen-standing-authorization-active" &&
    ...
    "Batch 11 preparation validation is invalid"
  );
...
async function activate() {
  ...
  const activation = {
    ...
    userAuthorization: {
      scope:
        "activate and execute exactly the 39 frozen Batch 11 score-blind discovery contexts under the frozen standing authorization",
    ...
    nextRequiredAction:
      "execute-exactly-thirty-nine-frozen-batch-11-discovery-contexts-once",
  };
  if (shouldWrite) {
...
async function loadAndValidateActivation() {
  ...
  assertV4(
    activation.status === EXPECTED_ACTIVATION_STATUS &&
      ...
      activation.executionPolicy.copiedInputBytesMaximum === 69824 &&
    ...
    "Batch 11 discovery execution is unauthorized"
  );
  ...
  assertV4(
    ...
    "frozen Batch 11 context count drifted"
  );
...
async function execute() {
  ...
  async function executeContext(context, contextIndex) {
    ...
    const sourceDirectory = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-post-canary-batch-11-discovery-${context.debateNumber}-${context.chunkId}-`
      )
    );
    const isolatedCodexHome = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-post-canary-batch-11-discovery-home-${context.debateNumber}-${context.chunkId}-`
      )
    );
    ...
    process.stdout.write(
      `[post-canary-batch-11-discovery] Debate ${context.debateNumber} ${context.chunkId} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
    );
    return record;
  }
  ...
  const execution = {
    ...
    status:
      validContexts === preparation.contexts.length
        ...
        : "post-canary-batch-11-discovery-complete-with-failure",
    productionContinuation: true,
    developmentValidationOnly: false,
    productionCanary: false,
    stagingOnly: true,
...
async function analyze() {
  ...
  assertV4(
    ...
    "all Batch 11 discovery contexts must pass without retry before analysis"
  );
  if (shouldWrite) {
    assertV4(
  ...
  const analysis = {
    ...
    status: candidateMinimumPassed
      ...
      : "post-canary-batch-11-discovery-failed-candidate-minimum-stop-no-retry",
    productionContinuation: true,
    developmentValidationOnly: false,
    productionCanary: false,
    stagingOnly: true,
...
if (command === "preflight") await preflight();
  ...
  throw new Error(
    "usage: run-assessment-production-post-canary-batch-11-discovery.mjs <preflight|activate|run|analyze|validate>"
  );
}
