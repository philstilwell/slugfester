...
import { V421_ROOT, compileV421PrimaryOutput, deriveV421PrimaryScores, evaluateV421Escalation, evaluateV421PrimaryTiming, validateV421PrimaryOutput } from "./lib/v421-compact-fresh.mjs";

const shouldWrite = process.argv.includes("--write");
const [preparation, execution] = await Promise.all([
  readFile(path.resolve(V421_ROOT, "preparation-manifest.json"), "utf8").then(JSON.parse),
  readFile(path.resolve(V421_ROOT, "primary-model-execution.json"), "utf8").then(JSON.parse)
]);
assertV4(execution.status === "primary-execution-passed" && execution.validContexts === 6 && execution.compiledOutputs === 6 && execution.retries === 0 && execution.authorization.primaryAnalysis, "clean v4.2.1 primary execution unavailable");

const debates = [];
for (const item of preparation.debates) {
  const packet = JSON.parse(await readFile(item.packet, "utf8"));
  const [rawPrimary, compiledPrimary, eventsBytes, ledgerBytes] = await Promise.all([
  ...
  const validation = validateV421PrimaryOutput(rawPrimary, packet, eventsDocument, eventsBytes, ledgerBytes);
  const replayCompiled = compileV421PrimaryOutput(rawPrimary, packet, eventsDocument);
  assertV4(canonicalJson(replayCompiled) === canonicalJson(compiledPrimary), `${item.debateNumber}: compiled primary replay mismatch`);
  const triggerScores = deriveV421PrimaryScores(compiledPrimary);
  ...
  const provisionalEscalation = evaluateV421Escalation({ primary: compiledPrimary, scores: triggerScores, controlSampleSelected: item.controlSampleSelected, unresolvedAudioMoveIds: [] });
  debates.push({
    ...
    escalation: { ...provisionalEscalation, audioComplete: audioVerificationMoveIds.length === 0, publicationBlocked: audioVerificationMoveIds.length > 0 || provisionalEscalation.publicationBlocked, triggerScoresComputedOnlyInMemory: true, triggerScoresWithheldFromArtifacts: true },
...
const runtime = evaluateV421PrimaryTiming(execution.results);
...
const analysis = {
  schemaVersion: "4.2.1-compact-fresh-primary-analysis",
  ...
  sourceIntegrityBoundary: { originalEventFileHashesVerified: 6, compactLedgerHashesVerified: 6, compactLedgersReplayedExactly: 6, eventAwareValidatedContexts: 6, schemaBoundedExcerptContexts: 6, deterministicTimeCompilationsReplayed: 6, modelSuppliedMillisecondsAccepted: false },
  debates,
  runtime,
