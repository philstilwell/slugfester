...
assertV4(
  ...
  "frozen remedy-v4 preparation required"
);
execFileSync(
  process.execPath,
...
for (const [file, digest] of Object.entries(preparation.sourceHashes)) {
  assertV4(
    ...
    `source hash mismatch before remedy-v4 activation: ${file}`
  );
}
for (const row of preparation.packets) {
  ...
  assertV4(
    ...
    `${row.debateNumber}: remedy-v4 packet hash mismatch`
  );
  validateCheckpointV22RenderingRemedyV4Packet(JSON.parse(bytes));
}
...
const activation = {
  schemaVersion:
    "1.0-production-checkpoint-v2.2-rendering-remedy-v4-execution-activation",
  protocolId: CHECKPOINT_V22_RENDERING_REMEDY_V4_PROTOCOL_ID,
  status:
  ...
  nextAuthorizedAction:
    "single-remedy-v4-rendering-execution-under-frozen-stop-rules"
};

if (shouldWrite) {
  assertV4(!(await exists(activationPath)), "remedy-v4 activation already exists");
