...
import {
  POST_CANARY_BATCH_14_DISPUTE_ADJ_ISOLATION,
  POST_CANARY_BATCH_14_DISPUTE_ADJ_OUTPUT_VERSION,
...
const EXPECTED_DEBATES = ["53", "69", "110", "133", "12", "187", "160", "55", "106", "57"];
...
const preparation = JSON.parse(
  await readFile(`${POST_CANARY_BATCH_14_DISPUTE_ADJ_ROOT}/preparation-manifest.json`, "utf8")
);
...
assert.equal(preparation.totals.disputedMoves, 181);
assert.equal(preparation.totals.candidateSelections, 539);
assert.equal(preparation.totals.audioVerifiedMoves, 12);
...
assert.equal(preparation.acceptedSourceBoundary.allTwelveAudioMovesVerified, true);
assert.equal(preparation.acceptedSourceBoundary.audioValidationOverlaysPreserved, 1);
assert.equal(preparation.acceptedSourceBoundary.audioAttributionRecoveryContextsPreserved, 1);
...
for (const context of preparation.contexts) {
  ...
  const output = {
    schemaVersion: POST_CANARY_BATCH_14_DISPUTE_ADJ_OUTPUT_VERSION,
    ...
    isolation: structuredClone(POST_CANARY_BATCH_14_DISPUTE_ADJ_ISOLATION),
  ...
  const validation = validatePostCanaryBatch14DisputeAdjudicationOutput(output, packet);
...
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_14_DISPUTE_ADJ_ROOT}/packets`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_14_DISPUTE_ADJ_ROOT}/provenance`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
