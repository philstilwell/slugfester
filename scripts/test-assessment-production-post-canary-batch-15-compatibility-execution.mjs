...
const root = POST_CANARY_BATCH_15_COMPATIBILITY_ROOT;
...
const validator = await import(
  `./validate-debates.mjs?batch15Compatibility=${activation.validator.proposedSha256}`
);
...
for (const lock of activation.packetHashes) {
  ...
  const routeLock =
    validator.validatePostCanaryBatch15LedgerAdapterRouteLocks({
      debate: candidate,
      ledgerText,
  ...
  const route = validator.validatePostCanaryBatch15LedgerAdapterRoute({
    debate: candidate,
    ledger,
    ledgerText
  });
  assert.equal(route.repositoryScoreReplayPassed, true);
  const adapter = validatePostCanaryBatch15SiteLedgerAdapter({
    adapter: ledger,
    candidate,
  ...
  JSON.stringify(
    {
      ...
      positiveRoutes,
      negativeControls,
      existingAdapterRoutesPreserved: 15,
      modelContexts: 0,
      paidServiceCalls: 0,
