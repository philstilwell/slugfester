...
assert.equal(selection.status, "thirteenth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed");
assert.equal(analysis.status, "thirteenth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization");
assert.equal(analysis.selection.sha256, sha256(selectionBytes));
assert.equal(selection.batchNumber, 13);
assert.equal(selection.productionSequenceOrdinal, 14);
...
assert.equal(selection.eligibility.previouslyPublishedAndRemovedCount, 120);
assert.equal(selection.eligibility.remainingBeforeBatchCount, 44);
assert.equal(selection.eligibility.selectedCount, 10);
assert.equal(selection.eligibility.remainingUnselectedCount, 34);
...
assert.equal(priorCompletionExecution.status, "passed-complete-batch-12-transaction-ready-for-atomic-commit");
...
assert.equal(priorCompletionExecution.dependentRecovery, undefined);
assert.equal(priorCompletionAnalysis.status, "batch-12-production-publication-accepted-for-atomic-commit-after-generated-seo-correction");
assert.equal(priorCompletionAnalysis.decision.generatedSeoCorrectionGatePassed, true);
assert.equal(priorCompletionAnalysis.decision.batch12ProductionPublicationGatePassed, true);
assert.equal(priorCompletionAnalysis.decision.singleCorrectionPassAccepted, true);
...
assert.equal(sha256(serializedJson(ranked.slice(120))), selection.eligibility.remainingBeforeBatchRankedSha256);
assert.deepEqual(priorSelection.selected.map((item) => item.debateNumber), ranked.slice(110, 120).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.debateNumber), ranked.slice(120, 130).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.rankOrdinal), [121, 122, 123, 124, 125, 126, 127, 128, 129, 130]);
assert.deepEqual(selection.selected.map((item) => item.rankSha256), ranked.slice(120, 130).map((item) => item.rankSha256));
...
assert.deepEqual(selection.deterministicOrdering.selectedRankOrdinals, [121, 130]);
...
assert.equal(selection.nextAuthorizedAction, "prepare-batch-13-standing-authorization-and-source-packets-under-user-authorization");

console.log(JSON.stringify({ status: "passed", selectedDebates: selection.selected.map((item) => item.debateNumber), selectedRankOrdinals: [121, 130], sourceFiles: 30, replacements: 0, sourcePacketsPrepared: 0, modelContexts: 0, paidServiceCalls: 0, directCostUsd: 0, nextAuthorizedAction: selection.nextAuthorizedAction }, null, 2));
