...
const passed =
  ...
  validation.finalMoves === 182 &&
  validation.disputedMoves === 177 &&
  validation.candidateSelections === 483 &&
  roundedMeanPopulation === 381 &&
  validation.audioVerifiedMoves === 9 &&
  validation.acceptedAdjudicationOutputsWithoutCorrection === 9 &&
  validation.calculatedScores === 0 &&
  validation.scoreDerivationAuthorized === false;
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-10-final-ledger-analysis",
  protocolId: ledger.protocolId,
  status: passed
    ? "post-canary-batch-10-deterministic-final-ledger-gate-passed"
    : "post-canary-batch-10-deterministic-final-ledger-gate-failed",
  analyzedAt,
  ...
  nextAuthorizedAction: passed
    ...
    : "standing-authorization-permits-batch-10-final-ledger-diagnosis"
};
if (shouldWrite) {
  await writeFile(
