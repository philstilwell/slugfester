...
const RECOVERY_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-10/dispute-only-adjudication/failure-recovery";

const activationPath =
  `${RECOVERY_ROOT}/correction-execution-activation.json`;
...
assertV4(
  manifest.status ===
      "frozen-two-batch-10-debate-74-field-disjoint-adjudication-correction-contexts-authorized" &&
...
async function runContext(context) {
  ...
  process.stdout.write(
    `[batch-10-debate-74-correction] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  ...
  status: passed
    ? "two-batch-10-debate-74-field-disjoint-adjudication-correction-contexts-passed"
