...
import {
  ...
  ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_14_DISAGREEMENT_ROOT,
  extractAssessmentProductionPostCanaryBatch14Disagreements
} from "./lib/assessment-production-post-canary-batch-14-disagreement.mjs";
import {
...
const USER_AUTHORIZATION = Object.freeze({
  ...
  standingAuthorizationPath: POST_CANARY_BATCH_14_STANDING_AUTHORIZATION,
...
const TOOL_SOURCES = [
  "scripts/extract-assessment-production-post-canary-batch-14-disagreements.mjs",
  "scripts/lib/assessment-production-post-canary-batch-14-disagreement.mjs",
  ...
  "scripts/test-assessment-production-post-canary-batch-14-disagreements.mjs"
];
...
assertV4(
  judgmentAnalysis.status ===
    ...
    judgmentAnalysis.nextAuthorizedAction ===
      "extract-freeze-and-analyze-batch-14-disagreements-under-standing-authorization",
  "Batch 14 accepted-judgment gate or prior stop rule changed"
);
assertV4(
  JSON.stringify(judgmentAnalysis.pairs.map((pair) => pair.debateNumber)) ===
    JSON.stringify(EXPECTED_DEBATES),
  "Batch 14 independent-judgment population changed"
);
assertV4(
  ...
  "Batch 14 does not contain ten accepted, independent A/B judgment pairs"
);
assertV4(
  ...
  "Batch 14 twenty-context accepted execution contract changed"
);
assertV4(
  ...
  "Batch 14 model, authentication, score-blindness, or tie contract changed"
);
assertV4(
  executionActivation.executionPolicy.attemptsPerContext === 1 &&
    ...
    JSON.stringify(executionActivation.executionPolicy.schedulerRamp) ===
      JSON.stringify([1, 2]),
  "Batch 14 one-attempt or frozen 1-to-2 scheduler stop rule changed"
);
assertV4(
...
assertV4(
  judgmentAnalysis.sourceCompatibility?.status ===
    ...
    judgmentAnalysis.sourceCompatibility.minimumCandidateLexicalTokensChanged ===
      false &&
    judgmentAnalysis.sourceCompatibility.occurrences?.length === 0,
  "Batch 14 source-compatibility evidence changed"
);
assertV4(
  modelExecution.results.length === 20 &&
    modelExecution.results.every(
      (result) =>
        ...
        [
          "completed-valid",
          "completed-valid-after-bounded-field-disjoint-recovery",
  ...
  "Batch 14 accepted execution evidence changed"
);
...
const sourceHashes = {
  ...
  [POST_CANARY_BATCH_14_STANDING_AUTHORIZATION]: standingAuthorization.sha256
};
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const disagreementPath =
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_14_DISAGREEMENT_ROOT}/disagreements/debate-${debateNumber}.json`;
...
assertV4(totalMoves === 190, "Batch 14 extraction must compare 190 moves");
assertV4(
  JSON.stringify(extractedAudioQueue.map(queueKey).sort()) ===
    JSON.stringify(independentGateQueue.map(queueKey).sort()),
  "Batch 14 frozen audio queue was not reproduced"
);

const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-14-disagreement-extraction",
  protocolId:
    ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_14_DISAGREEMENT_PROTOCOL_ID,
  status:
  ...
  standingAuthorization: {
    path: POST_CANARY_BATCH_14_STANDING_AUTHORIZATION,
  ...
  nextAuthorizedAction:
    "prepare-freeze-and-analyze-batch-14-local-audio-source-work-items-under-standing-authorization"
};

if (shouldWrite) {
  await mkdir(ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_14_DISAGREEMENT_ROOT, {
    recursive: true
  });
  await writeFile(
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_14_DISAGREEMENT_ROOT}/analysis.json`,
