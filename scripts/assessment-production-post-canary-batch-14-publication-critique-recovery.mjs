...
const protocolId = "assessment-production-post-canary-batch-14-publication-critique-field-disjoint-recovery";
...
if (mode === "--prepare") {
  ...
  assert.equal(invalidIds.length, 11);
  const contexts = [];
  for (let offset = 0; offset < invalidIds.length; offset += 2) {
    ...
    const schema = { $schema: "https://json-schema.org/draft/2020-12/schema", $id: `slugfester-b14-debate53-critique-repair-${contextIndex}`, type: "object", additionalProperties: false, required: Object.keys(properties), properties };
  ...
  assert.equal(contexts.length, 6); assert.equal(contexts.reduce((sum, item) => sum + item.writableFieldCount, 0), 11); assert(contexts.every((item) => item.writableFieldCount <= 2));
  ...
  const manifest = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-critique-recovery-preparation", protocolId, status: "frozen-six-context-eleven-critique-batch-14-publication-recovery-level-1-prepared-not-activated", frozenAt: at, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(), productionCanary: false, batchNumber: 14, stagingOnly: true, recoveryLevel: 1, recoveryLevelsMaximum: 2, model, contexts, executionPolicy: { contexts: 6, maximumParallelContexts: 2, attemptsPerContext: 1, retriesMaximum: 0, timeoutExtensionsMaximum: 0, timeoutMsPerContext: 600000, writableFieldsMaximumPerPacket: 2, directIncrementalCostUsdMaximum: 0, paidServices: false, separateActivationRequired: true, APIKeysRemoved: true, removedEnvironmentVariables: ["OPENAI_API_KEY", "OPENAI_ORG_ID", "OPENAI_PROJECT_ID", "OPENAI_BASE_URL", "AZURE_OPENAI_API_KEY", "CODEX_API_KEY"] }, diagnosis: { path: files.diagnosis, sha256: sourceHashes[files.diagnosis], invalidCritiques: 11, allOtherFieldsStructurallyValid: true }, sourceHashes, futureOutputs, authorization: { preparation: true, activation: true, modelExecution: false, deterministicMergeAndValidation: false, retries: false, furtherRecoveryLevel: false, scorePass: false, productionMutation: false, nextBatchSelection: false }, nextAuthorizedAction: "activate-six-field-disjoint-debate-53-critique-recovery-contexts" };
  if (shouldWrite) await writeFile(files.preparation, pretty(manifest));
  console.log(pretty({ status: shouldWrite ? manifest.status : "preview", contexts: 6, writableCritiques: 11, maximumWritableFieldsPerContext: 2, attemptsMaximum: 6, retriesMaximum: 0, recoveryLevel: 1, recoveryLevelsMaximum: 2, directIncrementalCostUsdMaximum: 0, modelExecutionAuthorized: false })); process.exit(0);
}

if (mode === "--activate") {
  const bytes = await readFile(files.preparation); const preparation = JSON.parse(bytes);
  assert.equal(preparation.status, "frozen-six-context-eleven-critique-batch-14-publication-recovery-level-1-prepared-not-activated"); assert(!(await exists(files.activation)), "activation exists");
  ...
  const activation = { ...preparation, schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-critique-recovery-activation", status: "frozen-six-context-eleven-critique-batch-14-publication-recovery-level-1-authorized", activatedAt: at, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(), preparation: { path: files.preparation, sha256: sha256(bytes) }, authorization: { ...preparation.authorization, modelExecution: true, deterministicMergeAndValidation: true }, nextAuthorizedAction: "execute-six-field-disjoint-debate-53-critique-recovery-contexts-once" };
  if (shouldWrite) await writeFile(files.activation, pretty(activation));
  console.log(pretty({ status: shouldWrite ? activation.status : "preview", contexts: 6, writableCritiques: 11, recoveryLevel: 1, attemptsMaximum: 6, retriesMaximum: 0, directIncrementalCostUsdMaximum: 0, modelExecutionAuthorized: shouldWrite })); process.exit(0);
}

if (mode === "--run") {
  const activation = await readJson(files.activation); assert.equal(activation.status, "frozen-six-context-eleven-critique-batch-14-publication-recovery-level-1-authorized");
  ...
  async function execute(context) {
    ...
    try {
      ...
      const prompt = `Read workflow.md, output-contract.md, manual.md, packet.json, and schema.json completely and no other files. Act only as the isolated 5.6 Sol/low Debate 53 critique-recovery editor for context ${context.contextIndex}. Author exactly the ${context.moveIds.length} required critique string${context.moveIds.length === 1 ? "" : "s"} in packet order. Return exactly one schema-conforming JSON object and no commentary.`;
  ...
  const passed = results.every((item) => item.gateAcceptancePassed); const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-critique-recovery-execution", protocolId, status: passed ? "six-context-eleven-critique-batch-14-publication-recovery-level-1-execution-passed" : "batch-14-publication-critique-recovery-level-1-execution-failed", startedAt, completedAt: new Date().toISOString(), elapsedMs: Date.now() - started, contextsAttempted: 6, validContexts: results.filter((item) => item.gateAcceptancePassed).length, invalidContexts: results.filter((item) => !item.gateAcceptancePassed).length, attempts: 6, retries: 0, timeoutExtensions: 0, directIncrementalCostUsd: 0, paidServiceCalls: 0, modelAuthoredScores: 0, results, authorization: { analysis: passed, retries: false, furtherRecoveryLevel: !passed, paidServices: false, productionMutation: false, nextBatchSelection: false } };
  await writeFile(files.execution, pretty(execution)); console.log(pretty({ status: execution.status, contextsAttempted: 6, validContexts: execution.validContexts, invalidContexts: execution.invalidContexts, elapsedMinutes: Number((execution.elapsedMs / 60000).toFixed(2)), attempts: 6, retries: 0, recoveryLevel: 1, directIncrementalCostUsd: 0 })); process.exit(0);
}
...
if (mode === "--analyze") {
  ...
  assert.equal(execution.status, "six-context-eleven-critique-batch-14-publication-recovery-level-1-execution-passed"); const outputs = await Promise.all(activation.contexts.map((item) => readJson(item.output))); const corrections = outputs.flatMap((output) => output.corrections); assert.equal(corrections.length, 11); assert.equal(new Set(corrections.map((item) => item.moveId)).size, 11);
  ...
  const provenance = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-debate-53-recovery-provenance", protocolId, debateNumber: "53", originalFailure: { status: "output-validation-failed", originalOutputPreserved: true }, recoveryLevel1: { contexts: 6, acceptedCritiques: 11, acceptedCompanionFieldsReused: true, eachTargetFieldAcceptedExactlyOnce: true }, merge: { acceptedFieldsChanged: false, scoresChanged: false, scorePassRerun: false, modelAuthoredScores: 0, completeValidationPassed: true } };
  const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-critique-recovery-analysis", protocolId, status: "debate-53-level-1-field-disjoint-publication-recovery-passed-awaiting-nine-context-resumption", recoveryLevel: 1, recoveryLevelsMaximum: 2, debateNumber: "53", moves: 20, critiques: 20, contextsThisLevel: 6, attemptsThisLevel: 6, retries: 0, timeoutExtensions: 0, directIncrementalCostUsd: 0, paidServiceCalls: 0, modelAuthoredScores: 0, validation, output: { path: files.mergedOutput, sha256: sha256(mergedText) }, authorization: { nineOriginalUnattemptedContextResumption: true, furtherRecoveryLevel: false, deterministicCompilation: false, productionMutation: false, nextBatchSelection: false }, nextAuthorizedAction: "resume-nine-original-unattempted-publication-contexts" };
  ...
  console.log(pretty({ status: analysis.status, debateNumber: "53", moves: validation.moves, critiques: validation.critiques, minimumCritiqueCharacters: validation.minimumCritiqueCharacters, exactSourceQuotes: validation.quoteExactSourceMatches, contextsThisLevel: 6, attemptsThisLevel: 6, retries: 0, directIncrementalCostUsd: 0, nextAuthorizedAction: analysis.nextAuthorizedAction }));
}
