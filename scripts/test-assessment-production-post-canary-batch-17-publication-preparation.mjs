...
import {
  ...
  POST_CANARY_BATCH_17_PUBLICATION_OUTPUT_VERSION,
...
const MANIFEST =
  `${POST_CANARY_BATCH_17_PUBLICATION_ROOT}/execution-preparation-manifest.json`;
...
assert.equal(manifest.protocolId, POST_CANARY_BATCH_17_PUBLICATION_PROTOCOL_ID);
assert.equal(manifest.productionCanary, false);
assert.equal(manifest.batchNumber, 17);
...
assert.equal(manifest.userAuthorization.contextsPrepared, 4);
...
assert.equal(manifest.totals.debates, 4);
assert.equal(manifest.totals.contexts, 4);
assert.equal(manifest.totals.moves, 79);
assert.equal(manifest.totals.sections, 21);
assert.equal(manifest.totals.audioVerifiedMoves, 1);
...
assert.equal(manifest.costEstimate.contexts, 4);
...
assert.deepEqual(manifest.costEstimate.expectedParallelWallMinutes, [10, 20]);
assert.deepEqual(manifest.costEstimate.expectedAggregateModelMinutes, [17, 28]);
...
assert.equal(manifest.executionPolicy.contexts, 4);
...
for (const context of manifest.contexts) {
  ...
  assert.equal(packet.protocolId, POST_CANARY_BATCH_17_PUBLICATION_PROTOCOL_ID);
  ...
  assert.deepEqual(schema, buildPostCanaryBatch17PublicationSchema(packet));
  assert.equal(schema.properties.schemaVersion.const, POST_CANARY_BATCH_17_PUBLICATION_OUTPUT_VERSION);
  assert.equal(schema.properties.protocolId.const, POST_CANARY_BATCH_17_PUBLICATION_PROTOCOL_ID);
...
assert.equal(moves, 79);
assert.equal(sections, 21);
assert.equal(quoteEligibleMoves, manifest.totals.quoteEligibleMoves);
assert.equal(audioVerifiedMoves, 1);
assert.equal(
