...
const MANIFEST = `${POST_CANARY_BATCH_08_PUBLICATION_RESUMPTION_2_ROOT}/execution-preparation-manifest.json`;
const exists = (file) => access(path.resolve(file)).then(() => true, () => false);
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
if (!(await exists(MANIFEST))) {
  console.log(JSON.stringify({ status: "batch-08-publication-resumption-2-test-ready" }));
  process.exit(0);
}
const manifest = JSON.parse(await readFile(path.resolve(MANIFEST), "utf8"));
assert.equal(manifest.protocolId, POST_CANARY_BATCH_08_PUBLICATION_RESUMPTION_2_PROTOCOL_ID);
assert.equal(manifest.status,
  "frozen-six-untouched-post-canary-batch-08-publication-resumption-2-contexts-prepared-not-authorized");
assert.deepEqual(manifest.contexts.map((row) => row.debateNumber),
  [...POST_CANARY_BATCH_08_PUBLICATION_RESUMPTION_2_DEBATES]);
assert.deepEqual(manifest.contexts.map((row) => row.originalContextIndex), [4, 5, 6, 7, 8, 9]);
assert.equal(manifest.contexts.reduce((sum, row) => sum + row.moves, 0), 106);
assert.equal(manifest.acceptedDebates.reduce((sum, row) => sum + row.moves, 0), 76);
...
console.log(JSON.stringify({ status: "batch-08-publication-resumption-2-test-passed",
  contexts: 6, debates: manifest.contexts.map((row) => row.debateNumber),
