...
const finalLedgerPath =
  `${POST_CANARY_BATCH_11_FINAL_LEDGER_ROOT}/final-ledger.json`;
...
assertV4(
  ...
  "post-canary Batch 11 final-ledger manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen Batch 11 final-ledger source hash mismatch`
  );
}
const inputs = await loadPostCanaryBatch11FinalLedgerInputs();
const ledger = buildPostCanaryBatch11FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
