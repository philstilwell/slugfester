...
const preparationPath =
  `${POST_CANARY_BATCH_08_COMPATIBILITY_ROOT}/preparation-manifest.json`;
const analysisPath =
  `${POST_CANARY_BATCH_08_COMPATIBILITY_ROOT}/analysis.json`;
...
assert.equal(preparation.batchNumber, 8);
...
assert.equal(
  ...
  "batch-08-site-ledger-adapter-and-validator-route"
);
assert.equal(
...
assert.equal(preparation.totals.overallBlunders, 59);
...
assert.equal(
  await exists(`${POST_CANARY_BATCH_08_COMPATIBILITY_ROOT}/execution.json`),
  false
);
assert.equal(
  await exists(
...
assert.deepEqual(packetNumbers, POST_CANARY_BATCH_08_COMPATIBILITY_ORDER);
for (const packetRecord of preparation.artifacts.packets) {
  ...
  assert.equal(packet.batchNumber, 8);
  ...
  const positive = validatePostCanaryBatch08SiteLedgerAdapter({
