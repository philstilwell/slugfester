...
assert(manifest.status === "frozen-before-fixture-build" && manifest.workflowVersion === V36_WORKFLOW && manifest.rubricVersion === V36_RUBRIC, "manifest identity invalid");
assert(manifest.calibrationOnly && manifest.retiredGoldUsedForValidatorFixtures && !manifest.independentModelAccuracyTest, "development-fixture status invalid");
assert(manifest.modelContextsExecuted === 0 && manifest.meteredApiCostUsd === 0 && manifest.transcriptionCostUsd === 0, "cost/execution scope invalid");
...
for (const debate of manifest.sample.debates) for (const source of Object.values(debate.sources)) assert(sha256(await read(source.path)) === source.sha256, `${debate.debateId}: fixture source hash mismatch`);
execFileSync(process.execPath, ["scripts/build-v36-retired-decision-fixtures.mjs"], { cwd: root, stdio: "ignore" });
execFileSync(process.execPath, ["scripts/test-v36-decision-cards.mjs"], { cwd: root, stdio: "ignore" });
const retiredText = await read(manifest.outputs.retiredFixtures), analysisText = await read(manifest.outputs.fixtureAnalysis), analysis = JSON.parse(analysisText);
assert(analysis.sources.manifestSha256 === sha256(manifestText) && analysis.sources.retiredFixturesSha256 === sha256(retiredText), "analysis provenance invalid");
