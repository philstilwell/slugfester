...
const ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-12/inventory-candidate-sharded";
const RECOVERY = `${ROOT}/plan-recovery-1/debate-28`;
...
const MERGED_PLAN = `${ROOT}/plans/debate-28.json`;
...
const ORIGINAL_SCHEMA = `${ROOT}/schemas/plans/debate-28.schema.json`;
const PREPARER =
  "scripts/prepare-assessment-production-post-canary-batch-12-debate-28-inventory-plan-recovery.mjs";
const RUNNER =
  "scripts/run-assessment-production-post-canary-batch-12-debate-28-inventory-plan-recovery.mjs";
const PROTOCOL_ID =
  "assessment-production-post-canary-batch-12-candidate-sharded-inventory";
const DEBATES = ["152", "28", "119", "116", "131", "07", "33", "15", "29", "11"];
const SHARDS = ["routes", "sections"];
const PREPARATION_STATUS =
  "two-frozen-debate-28-inventory-plan-recovery-shards-prepared-not-activated";
const ACTIVATION_STATUS =
  "two-frozen-debate-28-inventory-plan-recovery-shards-authorized";
const EXECUTION_STATUS =
  "two-debate-28-inventory-plan-recovery-shards-passed-and-merged";
...
async function loadPreparation({ requireFutureAbsent = false } = {}) {
  ...
  assertV4(
    ...
    "Debate 28 recovery preparation controls drifted"
  );
...
async function preflight() {
  ...
  assertV4(
    validation.status ===
      "passed-frozen-debate-28-inventory-plan-recovery-preparation" &&
      validation.contexts === 2 &&
      validation.modelContextsExecuted === 0 &&
      validation.directIncrementalCostUsd === 0,
    "Debate 28 recovery preparation validator failed"
  );
  ...
  console.log(
    JSON.stringify(
      {
        status: "passed-debate-28-recovery-model-free-preflight",
        contexts: 2,
        shards: SHARDS,
...
async function activate() {
  ...
  const activation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-12-debate-28-inventory-plan-recovery-execution-activation",
...
async function loadActivation({ requireFutureAbsent = false } = {}) {
  ...
  assertV4(
    ...
    "Debate 28 recovery activation controls drifted"
  );
...
async function execute() {
  ...
  async function executeContext(context) {
    ...
    const sourceDirectory = await mkdtemp(
      path.join(os.tmpdir(), `slugfester-batch-12-debate-28-${context.shardId}-`)
    );
    const isolatedCodexHome = await mkdtemp(
      path.join(os.tmpdir(), `slugfester-batch-12-debate-28-home-${context.shardId}-`)
    );
    ...
    process.stdout.write(
      `[batch-12-debate-28-plan-recovery] ${context.shardId} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
    );
    return record;
  }
  ...
  if (results.length === 2 && results.every((result) => result.accepted)) {
    try {
      ...
      const sourceContext = sourcePreparation.contexts.find(
        (context) => context.debateNumber === "28"
      );
  ...
  const execution = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-12-debate-28-inventory-plan-recovery-model-execution",
    protocolId: PROTOCOL_ID,
    status: passed
      ? EXECUTION_STATUS
      : "debate-28-inventory-plan-recovery-complete-with-failure",
...
async function analyze() {
  ...
  assertV4(
    ...
    "Debate 28 bounded recovery did not pass"
  );
  assertV4(
  ...
  const recoveryAnalysis = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-12-debate-28-inventory-plan-recovery-analysis",
    protocolId: PROTOCOL_ID,
    status:
      "debate-28-bounded-first-recovery-passed-complete-ten-plan-cohort-replay-passed",
    activation: ACTIVATION,
    activationSha256: sha256(activationBytes),
    execution: EXECUTION,
    ...
    nextAuthorizedAction:
      "prepare-freeze-and-activate-twenty-exact-batch-12-side-selector-packets-under-standing-authorization",
  };
  ...
  const analysis = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-12-candidate-census-plan-analysis",
    protocolId: PROTOCOL_ID,
    status:
      "post-canary-batch-12-candidate-census-plan-gate-passed-standing-authorization-active-for-side-packet-preparation",
...
async function validate() {
  ...
  if (!(await exists(EXECUTION))) {
    ...
    console.log(
      JSON.stringify(
        {
          status: "passed-debate-28-recovery-activation-awaiting-model-execution",
          contexts: 2,
          shards: SHARDS,
          model: activation.model,
          attemptsPerContext: 1,
  ...
  if (execution.status !== EXECUTION_STATUS) {
    ...
    console.log(
      JSON.stringify(
        {
          status: "passed-recorded-failed-debate-28-bounded-recovery",
          contextsAttempted: execution.contextsAttempted,
          invalidContexts: execution.invalidContexts,
  ...
  assertV4(
    ...
    "passing Debate 28 recovery record drifted"
  );
  if (!(await exists(COHORT_ANALYSIS))) {
    ...
    console.log(
      JSON.stringify(
        {
          status: "passed-debate-28-recovery-awaiting-cohort-analysis",
          contexts: 2,
  ...
  console.log(
    JSON.stringify(
      {
        status: "passed-debate-28-recovery-and-complete-ten-plan-cohort-replay",
        acceptedPlans: 10,
        recoveredDebate: "28",
        recoveryShards: 2,
        originalFailedPartialOutputReused: false,
