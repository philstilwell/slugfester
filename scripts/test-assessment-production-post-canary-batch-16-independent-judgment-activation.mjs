...
if (analysis.acceptance.passed) {
  ...
  assert.equal(analysis.totals.uniqueMoves, 194);
  assert.equal(analysis.totals.movesJudgedAcrossPasses, 388);
  assert.equal(
  ...
  assert.equal(
    analysis.status,
    "post-canary-batch-16-independent-judgment-gate-failed-analysis-only"
  );
}
console.log(
  JSON.stringify(
