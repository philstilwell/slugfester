...
assertV4(
  ...
  "v2.1.3 dispute adjudication execution unauthorized"
);
...
async function runContext(context, contextIndex) {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), `slugfester-v213-adjudication-${context.debateNumber}-`)
  );
  const codexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-v213-adjudication-home-${context.debateNumber}-`
    )
  );
  ...
  try {
    ...
    }; read nothing else. Act only as the isolated disputed-fields-only v2.1.3 adjudicator for Debate ${
    ...
    process.stdout.write(
      `[v2.1.3-adjudication] starting index ${contextIndex} ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
    ...
    if (
      ...
      try {
        validation = validateV213DisputeAdjudicationOutput(
  ...
  process.stdout.write(
    `[v2.1.3-adjudication] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-score-stability-v2.1.3-dispute-only-adjudication-model-execution",
  protocolId: manifest.protocolId,
  status: passed
    ? "ten-isolated-v2.1.3-dispute-only-adjudication-contexts-passed"
    : "v2.1.3-dispute-only-adjudication-gate-complete-with-failure",
