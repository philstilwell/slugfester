...
assertV4(
  activation.status ===
    ...
    activation.authorization?.remainingEightContextExecution === false &&
  ...
  "the Debate 172 repair execution is not authorized"
);
assertV4(
  ...
  "the Debate 172 repair execution controls changed"
);
...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), `batch-02-debate-172-repair-${context.contextIndex}-`)
  );
  const codexHome = await mkdtemp(
    path.join(os.tmpdir(), `batch-02-debate-172-repair-home-${context.contextIndex}-`)
  );
  ...
  try {
    ...
    const prompt = [
      ...
      `Act only as isolated Debate 172 two-field publication repair editor ${context.contextIndex}.`,
    ...
    process.stdout.write(
      `[batch-02-debate-172-publication-repair] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort}\n`
    );
    ...
    if (
      ...
      try {
        validationSummary = validateDebate172RepairOutput(
      ...
      const validation = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-02-debate-172-publication-repair-validation",
      ...
      const provenance = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-02-debate-172-publication-repair-provenance",
        protocolId: activation.protocolId,
        debateNumber: "172",
        packetIndex: context.packetIndex,
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-debate-172-publication-repair-model-execution",
  protocolId: activation.protocolId,
  status:
    validContexts === 1
      ? "batch-02-debate-172-one-context-publication-repair-gate-passed"
      : "batch-02-debate-172-publication-repair-gate-complete-with-failure",
  gateStartedAt,
  ...
  authorization: {
    ...
    deterministicMergeAndCompleteValidation: validContexts === 1,
    retry: false,
