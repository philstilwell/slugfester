...
const samplePath = "docs/calibration/v4.2.21.17.18/hard-route-held-out-five/source-only-sample.json";
const screeningPath = "docs/calibration/v4.2.21.17.18/hard-route-held-out-five/sample-screening.json";
...
const [sample, screening, manualBytes] = await Promise.all([
  readFile(samplePath, "utf8").then(JSON.parse),
  readFile(screeningPath, "utf8").then(JSON.parse),
  readFile(manualPath),
]);
assertV4(screening.status === "hard-route-held-out-five-screened-source-preparation-authorized" && screening.authorization.sourcePacketPreparation, "hard-route held-out source preparation is unauthorized");
...
for (const debate of sample.debates) {
  ...
  const fullLedgerPath = `.assessment-cache/compact-ledgers/v4.2.21.17.19/debate-${debate.number}.jsonl`;
  ...
  packet.schemaVersion = "4.2.21.17.19-hard-route-held-out-source-packet";
  packet.protocolId = "v4.2.21.17.19-hard-route-held-out-source-preparation";
  ...
  packet.modelInputBoundary = {
    ...
    legacyAssessmentsPriorJudgmentsScoresWinnersAndPublicationProseUnavailable: true,
  };
  ...
  for (const chunk of plan.chunks) {
    ...
    const chunkLedgerPath = `.assessment-cache/partition-ledgers/v4.2.21.17.19/debate-${debate.number}/${chunk.chunkId}.jsonl`;
    ...
    assertV4(span.startEvent.minimum === chunk.coreStartEvent && span.startEvent.maximum === chunk.coreEndEvent, `${debate.number}/${chunk.chunkId}: hardened start-event schema unavailable`);
    assertV4(span.endEvent.minimum === chunk.contextStartEvent && span.endEvent.maximum === chunk.contextEndEvent, `${debate.number}/${chunk.chunkId}: hardened end-event schema unavailable`);
    assertV4(allowedSpeakers.length === 2 && allowedSpeakers.every((speaker) => [...packet.sides.pro.speakers, ...packet.sides.con.speakers].includes(speaker)), `${debate.number}/${chunk.chunkId}: hardened speaker allowlist unavailable`);
    ...
    chunks.push({
      ...
      rawOutput: `${root}/discovery-outputs/debate-${debate.number}-${chunk.chunkId}.json`,
    });
  }
  if (shouldWrite) {
...
const preparation = {
  schemaVersion: "4.2.21.17.19-hard-route-held-out-source-preparation",
  protocolId: "v4.2.21.17.19-hard-route-held-out-source-preparation",
  status: shouldWrite ? "five-hard-route-held-out-source-and-hardened-discovery-contexts-prepared" : "preview",
  ...
  inputs: { sample: samplePath, screening: screeningPath, discoveryManual: manualPath },
  contexts,
  totals: {
    debates: 5,
    direct: contexts.filter((context) => context.frozenRoute === "direct").length,
    partition: contexts.filter((context) => context.frozenRoute === "partition").length,
...
console.log(JSON.stringify({
  ...
  debates: contexts.map((context) => ({
    ...
    route: context.frozenRoute,
    severity: context.partitionSeverity,
  ...
  hardenedOwnershipSchemas: allChunks.length,
