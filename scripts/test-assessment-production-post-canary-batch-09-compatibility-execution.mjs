...
const root = POST_CANARY_BATCH_09_COMPATIBILITY_ROOT;
...
const validator = await import(
  `./validate-debates.mjs?batch09Compatibility=${activation.validator.proposedSha256}`
);
...
for (const lock of activation.packetHashes) {
  ...
  const routeLock =
    validator.validatePostCanaryBatch09LedgerAdapterRouteLocks({
      debate: candidate,
      ledgerText,
  ...
  const route = validator.validatePostCanaryBatch09LedgerAdapterRoute({
    debate: candidate,
    ledger,
    ledgerText
  });
  assert.equal(route.repositoryScoreReplayPassed, true);
  const adapter = validatePostCanaryBatch09SiteLedgerAdapter({
    adapter: ledger,
    candidate,
...
assert.equal(oneSidedDisplayRows, 10);
assert.equal(emptyReferenceLinks, 57);
assert.equal(suppliedReferenceLinks, 3);
assert.equal(
  ...
  JSON.stringify(
    {
      ...
      positiveRoutes,
      negativeControls,
      existingAdapterRoutesPreserved: 9,
      modelContexts: 0,
      paidServiceCalls: 0,
