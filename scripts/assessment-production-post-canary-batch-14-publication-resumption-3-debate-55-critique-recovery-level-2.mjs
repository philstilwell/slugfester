...
const recoveryRoot =
  `${publicationRoot}/failure-recovery/original-unattempted-context-resumption-3/debate-55-timeout-recovery`;
const root = `${recoveryRoot}/critique-repair-level-2`;
const protocolId =
  "assessment-production-post-canary-batch-14-publication-resumption-3-debate-55-critique-field-disjoint-recovery-level-2";
...
const files = {
  diagnosis: `${recoveryRoot}/analysis.json`, executionLevel1: `${recoveryRoot}/model-execution.json`, packet: `${publicationRoot}/packets/debate-55.json`, fullSchema: `${publicationRoot}/schemas/debate-55.schema.json`, moveShard: `${recoveryRoot}/outputs/context-0.json`, quoteShard: `${recoveryRoot}/outputs/context-1.json`, extensionShard: `${recoveryRoot}/outputs/context-2.json`, workflow: "docs/assessment-production-workflow.md", outputContract: "/Users/philstilwell/.codex/skills/reassess-slugfester-debates/references/output-contract.md", manual: `${root}/manual.md`, preparation: `${root}/execution-preparation-manifest.json`, activation: `${root}/execution-activation.json`, execution: `${root}/model-execution.json`, analysis: `${root}/analysis.json`, mergedOutput: `${publicationRoot}/outputs/debate-55.json`, mergedValidation: `${publicationRoot}/validations/debate-55.json`, mergedProvenance: `${publicationRoot}/provenance/debate-55.json`,
};
...
if (mode === "--prepare") {
  ...
  assert.equal(diagnosis.status, "debate-55-three-shard-recovery-complete-merged-validation-failed-twelve-critiques");
  assert.equal(diagnosis.diagnosis.invalidCritiqueCount, 12);
  assert.equal(diagnosis.diagnosis.allOtherFieldsStructurallyValid, true);
  assert.equal(executionLevel1.status, "three-context-debate-55-publication-timeout-recovery-execution-passed");
  ...
  assert.equal(invalidIds.length, 12);
  assert.equal(new Set(invalidIds).size, 12);
  assert(invalidIds.every((moveId) => packet.moves.some((move) => move.moveId === moveId)));
  const contexts = [];
  for (let offset = 0; offset < invalidIds.length; offset += 2) {
    ...
    const repairPacket = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-resumption-3-debate-55-critique-recovery-level-2-packet", protocolId, contextIndex, debateNumber: "55", debateId: packet.debateId, assessmentModel: model.label, writableFields: moveIds.map((moveId) => `moveProse.${moveId}.critique`), writableFieldCount: moveIds.length, targets, allOtherFieldsUnavailableAndImmutable: true, participantJudgmentClosed: true, publicationScoreLocked: true };
    ...
    const properties = { schemaVersion: { type: "string", const: "1.0-assessment-production-post-canary-batch-14-publication-resumption-3-debate-55-critique-recovery-level-2-output" }, protocolId: { type: "string", const: protocolId }, contextIndex: { type: "integer", const: contextIndex }, debateNumber: { type: "string", const: "55" }, debateId: { type: "string", const: packet.debateId }, assessmentModel: { type: "string", const: model.label }, completedAt: { type: "string", minLength: 1 }, corrections: { type: "array", minItems: moveIds.length, maxItems: moveIds.length, items: { anyOf: itemSchemas } } };
    const schema = { $schema: "https://json-schema.org/draft/2020-12/schema", $id: `slugfester-b14-debate55-critique-repair-level-2-${contextIndex}`, type: "object", additionalProperties: false, required: Object.keys(properties), properties };
    ...
    contexts.push({ contextIndex, debateNumber: "55", moveIds, writableFields: repairPacket.writableFields, writableFieldCount: moveIds.length, packet: packetPath, packetSha256: sha256(packetText), schema: schemaPath, schemaSha256: sha256(schemaText), output: outputPath });
  ...
  assert.equal(contexts.length, 6); assert.equal(contexts.reduce((sum, item) => sum + item.writableFieldCount, 0), 12); assert(contexts.every((item) => item.writableFieldCount <= 2));
  ...
  const manifest = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-critique-recovery-preparation", protocolId, status: "frozen-six-context-twelve-critique-batch-14-publication-recovery-level-2-prepared-not-activated", frozenAt: at, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(), productionCanary: false, batchNumber: 14, stagingOnly: true, recoveryLevel: 2, recoveryLevelsMaximum: 2, model, contexts, executionPolicy: { contexts: 6, maximumParallelContexts: 2, attemptsPerContext: 1, retriesMaximum: 0, timeoutExtensionsMaximum: 0, timeoutMsPerContext: 600000, writableFieldsMaximumPerPacket: 2, directIncrementalCostUsdMaximum: 0, paidServices: false, separateActivationRequired: true, APIKeysRemoved: true, removedEnvironmentVariables: ["OPENAI_API_KEY", "OPENAI_ORG_ID", "OPENAI_PROJECT_ID", "OPENAI_BASE_URL", "AZURE_OPENAI_API_KEY", "CODEX_API_KEY"] }, diagnosis: { path: files.diagnosis, sha256: sourceHashes[files.diagnosis], invalidCritiques: 12, allOtherFieldsStructurallyValid: true }, sourceHashes, futureOutputs, authorization: { preparation: true, activation: true, modelExecution: false, deterministicMergeAndValidation: false, retries: false, furtherRecoveryLevel: false, scorePass: false, productionMutation: false, nextBatchSelection: false }, nextAuthorizedAction: "activate-final-six-field-disjoint-debate-55-critique-recovery-contexts" };
  if (shouldWrite) await writeFile(files.preparation, pretty(manifest));
  console.log(pretty({ status: shouldWrite ? manifest.status : "preview", contexts: 6, writableCritiques: 12, maximumWritableFieldsPerContext: 2, attemptsMaximum: 6, retriesMaximum: 0, recoveryLevel: 2, recoveryLevelsMaximum: 2, directIncrementalCostUsdMaximum: 0, modelExecutionAuthorized: false })); process.exit(0);
}

if (mode === "--activate") {
  const bytes = await readFile(files.preparation); const preparation = JSON.parse(bytes);
  assert.equal(preparation.status, "frozen-six-context-twelve-critique-batch-14-publication-recovery-level-2-prepared-not-activated"); assert(!(await exists(files.activation)), "activation exists");
  ...
  const activation = { ...preparation, schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-critique-recovery-activation", status: "frozen-six-context-twelve-critique-batch-14-publication-recovery-level-2-authorized", activatedAt: at, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(), preparation: { path: files.preparation, sha256: sha256(bytes) }, authorization: { ...preparation.authorization, modelExecution: true, deterministicMergeAndValidation: true }, nextAuthorizedAction: "execute-final-six-field-disjoint-debate-55-critique-recovery-contexts-once" };
  if (shouldWrite) await writeFile(files.activation, pretty(activation));
  console.log(pretty({ status: shouldWrite ? activation.status : "preview", contexts: 6, writableCritiques: 12, recoveryLevel: 2, attemptsMaximum: 6, retriesMaximum: 0, directIncrementalCostUsdMaximum: 0, modelExecutionAuthorized: shouldWrite })); process.exit(0);
}

if (mode === "--run") {
  const activation = await readJson(files.activation); assert.equal(activation.status, "frozen-six-context-twelve-critique-batch-14-publication-recovery-level-2-authorized");
  ...
  async function execute(context) {
    const temp = await mkdtemp(path.join(os.tmpdir(), `slugfester-b14-pub55-c-${context.contextIndex}-`)); const home = await mkdtemp(path.join(os.tmpdir(), `slugfester-b14-pub55-ch-${context.contextIndex}-`)); const startedAt = new Date().toISOString(); const started = Date.now();
    try {
      ...
      const prompt = `Read workflow.md, output-contract.md, manual.md, packet.json, and schema.json completely and no other files. Act only as the isolated 5.6 Sol/low Debate 55 critique-recovery editor for context ${context.contextIndex}. Author exactly the ${context.moveIds.length} required critique string${context.moveIds.length === 1 ? "" : "s"} in packet order. Return exactly one schema-conforming JSON object and no commentary.`;
  ...
  const passed = results.every((item) => item.gateAcceptancePassed); const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-critique-recovery-execution", protocolId, status: passed ? "six-context-twelve-critique-batch-14-publication-recovery-level-2-execution-passed" : "batch-14-publication-critique-recovery-level-2-execution-failed", startedAt, completedAt: new Date().toISOString(), elapsedMs: Date.now() - started, contextsAttempted: 6, validContexts: results.filter((item) => item.gateAcceptancePassed).length, invalidContexts: results.filter((item) => !item.gateAcceptancePassed).length, attempts: 6, retries: 0, timeoutExtensions: 0, directIncrementalCostUsd: 0, paidServiceCalls: 0, modelAuthoredScores: 0, results, authorization: { analysis: passed, retries: false, furtherRecoveryLevel: false, paidServices: false, productionMutation: false, nextBatchSelection: false } };
  await writeFile(files.execution, pretty(execution)); console.log(pretty({ status: execution.status, contextsAttempted: 6, validContexts: execution.validContexts, invalidContexts: execution.invalidContexts, elapsedMinutes: Number((execution.elapsedMs / 60000).toFixed(2)), attempts: 6, retries: 0, recoveryLevel: 2, directIncrementalCostUsd: 0 })); assert(passed, "final Debate 55 critique recovery failed"); process.exit(0);
}
...
if (mode === "--analyze") {
  ...
  assert.equal(execution.status, "six-context-twelve-critique-batch-14-publication-recovery-level-2-execution-passed"); const outputs = await Promise.all(activation.contexts.map((item) => readJson(item.output))); const corrections = outputs.flatMap((output) => output.corrections); assert.equal(corrections.length, 12); assert.equal(new Set(corrections.map((item) => item.moveId)).size, 12);
  ...
  const provenance = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-debate-55-recovery-provenance", protocolId, debateNumber: "55", outputSha256: sha256(mergedText), originalFailure: { status: "timed-out", partialOutputReused: false }, recoveryLevel1: { contexts: 3, acceptedCritiques: 5, acceptedFields: ["moveProse.role", "moveProse.words", "moveProse.tags", "summary", "representativeQuotes", "overallCommentary", "aiExtension"] }, recoveryLevel2: { contexts: 6, acceptedCritiques: 12, eachTargetCritiqueAcceptedExactlyOnce: true }, merge: { acceptedFieldsChanged: false, scoresChanged: false, scorePassRerun: false, modelAuthoredScores: 0, completeValidationPassed: true } };
  const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-critique-recovery-analysis", protocolId, status: "debate-55-two-level-field-disjoint-publication-recovery-passed-awaiting-two-context-resumption", recoveryLevel: 2, recoveryLevelsMaximum: 2, debateNumber: "55", moves: 17, critiques: 17, contextsThisLevel: 6, attemptsThisLevel: 6, attemptsAcrossRecoveryLevels: 9, retries: 0, timeoutExtensions: 0, directIncrementalCostUsd: 0, paidServiceCalls: 0, modelAuthoredScores: 0, validation, output: { path: files.mergedOutput, sha256: sha256(mergedText) }, authorization: { twoOriginalUnattemptedContextResumption: true, furtherRecoveryLevel: false, deterministicCompilation: false, productionMutation: false, nextBatchSelection: false }, nextAuthorizedAction: "resume-two-original-unattempted-publication-contexts" };
  ...
  console.log(pretty({ status: analysis.status, debateNumber: "55", moves: validation.moves, critiques: validation.critiques, minimumCritiqueCharacters: validation.minimumCritiqueCharacters, exactSourceQuotes: validation.quoteExactSourceMatches, contextsThisLevel: 6, attemptsThisLevel: 6, retries: 0, directIncrementalCostUsd: 0, nextAuthorizedAction: analysis.nextAuthorizedAction }));
}
