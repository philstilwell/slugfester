...
import {
  ...
  hashFile,
  sha256,
  validatePostCanaryBatch15RenderingEvidence,
  validatePostCanaryBatch15RenderingPacket
...
const ROOT = POST_CANARY_BATCH_15_RENDERING_ROOT;
...
assertV4(activation.status ===
  "frozen-post-canary-batch-15-rendering-verification-authorized" &&
...
"Batch 15 rendering activation changed");
...
for (const debateNumber of POST_CANARY_BATCH_15_RENDERING_ORDER) {
  ...
  const packet = validatePostCanaryBatch15RenderingPacket(JSON.parse(packetBytes));
  for (const viewportName of ["desktop", "mobile"]) {
    ...
    validatePostCanaryBatch15RenderingEvidence({ packet, viewportName,
...
"complete passing Batch 15 rendering evidence required");
...
const audit = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-15-rendering-audit",
  protocolId: activation.protocolId,
  status: "passed-ten-debate-batch-15-rendering-verification",
...
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-15-rendering-execution",
  protocolId: activation.protocolId,
  status: "ten-debate-batch-15-rendering-verification-passed",
  completedAt,
  attempts: 20,
  retries: 0,
  timeoutExtensions: 0,
...
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-15-rendering-analysis",
  protocolId: activation.protocolId,
  ...
  nextAuthorizedAction: "prepare-batch-15-production-compatibility-staging"
};
...
console.log(JSON.stringify({ status: analysis.status, debates: 10,
  sections: 51, moves: 191, viewportResults: 20,
