...
const BATCH_ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-11";
...
const PROTOCOL_ID = "assessment-production-post-canary-batch-11-discovery";
const WORKFLOW = "docs/assessment-production-canary-discovery-workflow.md";
const SCRIPT = "scripts/preregister-assessment-production-post-canary-batch-11-discovery.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-11-discovery-manifest.mjs";
...
assertV4(
  preparation.status === "post-canary-batch-11-ten-complete-score-blind-source-packets-prepared-awaiting-validation" &&
    preparationValidation.status === "post-canary-batch-11-score-blind-source-packet-validation-passed-frozen-under-standing-authorization" &&
    ...
    preparation.totals.discoveryContexts === 39 &&
    preparation.totals.maximumCopiedInputBytes === 69824 &&
  ...
  "validated Batch 11 source preparation is not frozen at the required authorization boundary"
);
assertV4(
  ...
  "Batch 11 standing authorization does not cover exact discovery execution"
);
assertV4(
  JSON.stringify(preparation.contexts.map((debate) => debate.debateNumber)) === JSON.stringify(REQUIRED_ORDER),
  "Batch 11 debate order drifted"
);
assertV4(
  ...
  for (const chunk of debate.chunks) {
    ...
    assertV4(
      sha256(chunkBytes) === chunk.chunkLedgerSha256 &&
        ...
        buildBatch11TokenCountedChunkLedger(chunkBytes).equals(tokenBytes) &&
...
assertV4(contexts.length === 39, "Batch 11 discovery must contain exactly 39 contexts");
...
assertV4(maximumContext.debateNumber === "191" && maximumContext.chunkId === "chunk-002", "maximum copied-input context drifted");
...
const sourceFiles = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-11-source-preparation.mjs",
...
const manifest = {
  ...
  status: shouldWrite ? "frozen-thirty-nine-post-canary-batch-11-discovery-contexts-prepared-not-authorized" : "preview",
  ...
  AIOnly: true,
  userAuthorization: {
    scope: "standing-authorized Batch 11 discovery preparation and execution",
  ...
  costEstimate: {
    ...
    expectedAggregateModelMinutes: [38, 75],
    expectedAggregateComputeHours: [0.63, 1.25],
    absoluteGateTimeoutMinutes: 120,
    estimateBasis: "The exact 39-context Batch 11 input set is hash-locked with an observed maximum of 69,824 copied bytes; no input is truncated or repartitioned. The frozen 1→2→4 scheduler uses the subscription-backed route with a $0 direct incremental cost boundary.",
  },
  ...
  nextAuthorizedAction: "prepare-and-freeze-batch-11-discovery-execution-activation-under-standing-authorization",
};
