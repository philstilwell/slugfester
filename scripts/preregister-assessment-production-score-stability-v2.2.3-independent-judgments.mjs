...
const SOURCE_PREPARATION =
  "docs/assessment-production/score-stability-v2.2.2-validation-cohort/inventory-route-section-plan-successor/section-packet-preparation-manifest.json";
const UPSTREAM_EXECUTION_PREPARATION =
  "docs/assessment-production/score-stability-v2.2.2-validation-cohort/inventory-route-section-plan-successor/side-execution-preparation-manifest.json";
...
const SCRIPT =
  "scripts/preregister-assessment-production-score-stability-v2.2.3-independent-judgments.mjs";
const TEST =
  "scripts/test-assessment-production-score-stability-v2.2.3-independent-judgment-manifest.mjs";
...
const [
  ...
  await Promise.all([
    ...
    readFile(UPSTREAM_EXECUTION_PREPARATION, "utf8").then(JSON.parse),
...
assertV4(
  preparation.status ===
    ...
    preparation.totals.uniqueMoves === 197 &&
    preparation.totals.movesJudgedAcrossPasses === 394 &&
  ...
  "v2.2.3 preparation does not authorize an execution-preparation manifest"
);
assertV4(
...
assertV4(
  ...
  "the unpromoted v2.2 score-stability proposal drifted"
);
assertV4(
  sourcePreparation.status ===
      "ten-exact-v2.2.2-section-packets-frozen-not-authorized" &&
    upstreamExecutionPreparation.status ===
      "frozen-twenty-v2.2.2-side-selector-contexts-prepared-not-authorized" &&
    Object.values(upstreamExecutionPreparation.stopRules).every(Boolean),
  "the accepted upstream preparation or its stop rules are unavailable"
);
assertV4(
...
const sourceFiles = [
  ...
  SOURCE_PREPARATION,
  UPSTREAM_EXECUTION_PREPARATION,
  PREPARATION,
  CANARY_EXECUTION,
  CANARY_ANALYSIS,
  ...
  "scripts/prepare-assessment-production-score-stability-v2.2.3-independent-judgments.mjs",
  "scripts/test-assessment-production-score-stability-v2.2.3-independent-judgment-preparation.mjs",
  SCRIPT,
  TEST,
...
const stopRules = {
  ...structuredClone(upstreamExecutionPreparation.stopRules),
...
const manifest = {
  schemaVersion:
    "1.0-score-stability-v2.2.3-independent-judgment-execution-preparation-manifest",
  protocolId: preparation.protocolId,
  status:
    "frozen-twenty-v2.2.3-independent-judgment-contexts-prepared-not-authorized",
  frozenAt,
  ...
  costEstimate: {
    ...
    contexts: 20,
    expectedParallelWallMinutes: [48, 75],
    expectedAggregateModelMinutes: [85, 120],
    expectedAggregateComputeHours: [1.42, 2],
  ...
  nextAuthorizedAction:
    "prepare-separate-v2.2.3-independent-judgment-execution-activation-only",
};
