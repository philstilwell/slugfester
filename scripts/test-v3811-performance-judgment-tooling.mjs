...
function fixtureOutput(packet, pass) {
  return {
    schemaVersion: "3.8.11-performance-judgment-output",
    ...
    moveJudgments: packet.moves.map((move) => {
      ...
      return {
        ...
        ratings: {
          ...
          relevanceBurden: rating(V3811_BURDEN_RANGES[tier][0]),
...
if (canonicalJson(schema) !== canonicalJson(makeV3811PerformanceSchema())) throw new Error("stored shared schema differs from generator");
const reports = [];
for (const debateNumber of V3811_PERFORMANCE_DEBATES) {
  ...
  for (const pass of V3811_PERFORMANCE_PASSES) reports.push(validateV3811PerformanceOutput(fixtureOutput(packet, pass), packet, pass));
}

const samplePacket = await readJson(`${V3811_PERFORMANCE_ROOT}/packets/debate-55.json`);
...
const dispute = extractV3811MoveDisagreement(baseMove, variantMove);
...
try {
  validateV3811PerformanceOutput(invalidDuplicate, samplePacket, "A");
} catch (error) {
...
try {
  validateV3811PerformanceOutput(invalidCharity, samplePacket, "A");
} catch (error) {
...
validateV3811PerformanceOutput(lexicalRegressionDiagnostic, samplePacket, "A");
...
validateV3811PerformanceOutput(lexicalRegressionCharity, samplePacket, "A");

const fixture = {
  schemaVersion: "3.8.11-performance-judgment-tooling-dry-fixture",
  status: "passed",
  reports,
...
if (shouldWrite) {
  await mkdir(path.resolve(V3811_PERFORMANCE_ROOT), { recursive: true });
  await writeFile(path.resolve(`${V3811_PERFORMANCE_ROOT}/dry-fixture.json`), `${JSON.stringify(fixture, null, 2)}\n`);
}
