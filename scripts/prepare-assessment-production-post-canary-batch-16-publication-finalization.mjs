...
import { POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_ROOT } from
  "./lib/assessment-production-post-canary-batch-16-publication-compilation.mjs";
import { POST_CANARY_BATCH_16_PUBLICATION_FINALIZATION_ORDER,
  POST_CANARY_BATCH_16_PUBLICATION_FINALIZATION_PROTOCOL_ID,
  POST_CANARY_BATCH_16_PUBLICATION_FINALIZATION_ROOT,
  buildPostCanaryBatch16PublicationFinalization,
  buildPostCanaryBatch16PublicationStagingPreviewHtml,
  validatePostCanaryBatch16PublicationFinalCandidate } from
  "./lib/assessment-production-post-canary-batch-16-publication-finalization.mjs";
import { POST_CANARY_BATCH_16_STANDING_AUTHORIZATION,
  loadAndValidatePostCanaryBatch16StandingAuthorization } from
  "./lib/assessment-production-post-canary-batch-16-standing-authorization.mjs";
...
const ROOT = POST_CANARY_BATCH_16_PUBLICATION_FINALIZATION_ROOT;
const COMPILATION_ANALYSIS = `${POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_ROOT}/analysis.json`;
const COMPILATION_MANIFEST = `${POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_ROOT}/preparation-manifest.json`;
const COMPILATION_AUDIT = `${POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_ROOT}/compilation-audit.json`;
const IDENTITY = `${POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_ROOT}/production-identity-snapshot.json`;
...
assertV4(analysis.status === "ten-debate-batch-16-deterministic-publication-compilation-passed" &&
  analysis.gate?.compiledRecordsPassed === 10 && analysis.gate?.moves === 194 &&
  compilationAudit.status === "passed" && compilationAudit.totals?.moves === 194 &&
  canonicalJson(compilation.explicitOrder) === canonicalJson(POST_CANARY_BATCH_16_PUBLICATION_FINALIZATION_ORDER),
"the accepted Batch 16 compilation changed");
const contexts = [];
for (const debateNumber of POST_CANARY_BATCH_16_PUBLICATION_FINALIZATION_ORDER) {
  ...
  const built = buildPostCanaryBatch16PublicationFinalization({ compiled,
    compiledPath: source.plannedCompiledOutput, compiledSha256: sha256(compiledBytes),
    output, packet, identity: identityRow });
  const validation = validatePostCanaryBatch16PublicationFinalCandidate({
...
assertV4(contexts.length === 10 && contexts.reduce((sum, row) => sum + row.syntheticValidation.moves, 0) === 194,
"the Batch 16 finalization cohort changed");
const previewBytes = Buffer.from(buildPostCanaryBatch16PublicationStagingPreviewHtml());
const sources = [COMPILATION_ANALYSIS, COMPILATION_MANIFEST, COMPILATION_AUDIT, IDENTITY,
  POST_CANARY_BATCH_16_STANDING_AUTHORIZATION, "src/app.js", "src/styles.css",
  "scripts/lib/assessment-production-post-canary-batch-16-publication-finalization.mjs",
  "scripts/lib/assessment-production-post-canary-batch-16-publication-compilation.mjs",
  ...
  "scripts/prepare-assessment-production-post-canary-batch-16-publication-finalization.mjs",
  "scripts/test-assessment-production-post-canary-batch-16-publication-finalization-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-16-publication-finalization.mjs",
  "scripts/run-assessment-production-post-canary-batch-16-publication-finalization.mjs",
...
const manifest = { schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-finalization-preparation",
  protocolId: POST_CANARY_BATCH_16_PUBLICATION_FINALIZATION_PROTOCOL_ID,
  status: "frozen-post-canary-batch-16-publication-finalization-prepared",
  frozenAt, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(),
  productionCanary: false, batchNumber: 16, stagingOnly: true,
  userAuthorization: { standingAuthorization: POST_CANARY_BATCH_16_STANDING_AUTHORIZATION,
    standingAuthorizationSha256: standing.sha256, directIncrementalCostUsdMaximum: 0 },
  explicitOrder: POST_CANARY_BATCH_16_PUBLICATION_FINALIZATION_ORDER, contexts,
  ...
  nextAuthorizedAction: "activate-one-batch-16-deterministic-publication-finalization-pass" };
...
console.log(JSON.stringify({ status: manifest.status, debates: 10, moves: 194,
