...
const preparationPath =
  `${POST_CANARY_BATCH_11_COMPATIBILITY_ROOT}/preparation-manifest.json`;
const analysisPath =
  `${POST_CANARY_BATCH_11_COMPATIBILITY_ROOT}/analysis.json`;
...
assert.equal(
  ...
  "batch-11-site-ledger-adapter-and-validator-route"
);
assert.equal(
...
assert.equal(preparation.totals.sections, 50);
assert.equal(preparation.totals.moves, 190);
assert.equal(preparation.totals.oneSidedDisplayRows, 6);
assert.equal(preparation.totals.overallBlunders, 54);
assert.equal(preparation.totals.emptyReferenceLinks, 54);
assert.equal(preparation.totals.suppliedReferenceLinks, 0);
...
assert.equal(
  await exists(`${POST_CANARY_BATCH_11_COMPATIBILITY_ROOT}/execution.json`),
  false
);
assert.equal(
  await exists(
...
assert.deepEqual(packetNumbers, POST_CANARY_BATCH_11_COMPATIBILITY_ORDER);
for (const packetRecord of preparation.artifacts.packets) {
  ...
  assert.equal(packet.batchNumber, 11);
  ...
  const positive = validatePostCanaryBatch11SiteLedgerAdapter({
