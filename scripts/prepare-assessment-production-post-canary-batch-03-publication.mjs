...
const SELECTION =
  "docs/assessment-production/post-canary-continuation-v1/batch-03/selection.json";
const FINAL_LEDGER =
...
const FINAL_LEDGER_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-03/final-ledger/final-ledger-manifest.json";
const FINAL_LEDGER_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-03/final-ledger/analysis.json";
const SCORES =
...
const SCORE_PREPARATION =
  "docs/assessment-production/post-canary-continuation-v1/batch-03/score-pass/score-pass-preparation-manifest.json";
const SCORE_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-03/score-pass/analysis.json";
const AUDIO_AUDIT =
  "docs/assessment-production/post-canary-continuation-v1/batch-03/audio-verification/correction-audio-verification.json";
const SOURCE_PACKET_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-03/independent-judgments/source-packets";
...
const STATIC_SOURCE_FILES = [
  ...
  "scripts/prepare-assessment-production-post-canary-batch-03-publication.mjs",
  "scripts/test-assessment-production-post-canary-batch-03-publication-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-03-publication.mjs",
  "scripts/run-assessment-production-post-canary-batch-03-publication.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-03-publication.mjs"
];
...
assertV4(
  scoreAnalysis.status === "post-canary-batch-03-score-stability-gate-passed" &&
    scoreAnalysis.productionCanary === false &&
    scoreAnalysis.batchNumber === 3 &&
    ...
    scoreAnalysis.nextAuthorizedAction ===
      "standing-authorization-permits-batch-03-publication-packet-preparation",
  "the accepted Batch 3 score gate is unavailable or its stop boundary changed"
);
assertV4(
  scores.status === "post-canary-batch-03-single-score-pass-stability-gate-passed" &&
  ...
  "the locked Batch 3 score artifact is unavailable"
);
assertV4(
  scoreManifest.status ===
      "frozen-post-canary-batch-03-single-deterministic-score-pass-authorized" &&
    ...
    scorePreparation.status ===
      "frozen-post-canary-batch-03-single-deterministic-score-pass-prepared-not-activated",
  "the frozen Batch 3 single-pass score controls changed"
);
assertV4(
  ledger.status ===
      "passed-post-canary-batch-03-deterministic-final-ledger-assembly" &&
    ...
    ledgerAnalysis.status ===
      "post-canary-batch-03-deterministic-final-ledger-gate-passed",
  "the locked Batch 3 final ledger changed"
);
assertV4(
  audioAudit.status ===
      "passed-all-eight-batch-03-audio-attributions-and-cost-overlay-after-correction" &&
    audioAudit.totals?.requiredMoves === 8 &&
    audioAudit.totals?.verified === 8 &&
    audioAudit.totals?.unresolved === 0,
  "the ten required Batch 3 audio checks are incomplete"
);
assertV4(
  productionManifest.model?.label === POST_CANARY_BATCH_03_PUBLICATION_MODEL.label &&
    productionManifest.model?.slug === POST_CANARY_BATCH_03_PUBLICATION_MODEL.slug &&
    productionManifest.model?.reasoningEffort ===
      POST_CANARY_BATCH_03_PUBLICATION_MODEL.reasoningEffort &&
    productionManifest.model?.authentication ===
      POST_CANARY_BATCH_03_PUBLICATION_MODEL.authentication,
  "the frozen production model or authentication changed"
);
assertV4(
  selection.selected?.map((item) => item.debateNumber).join(",") ===
      POST_CANARY_BATCH_03_PUBLICATION_DEBATES.join(",") &&
    selection.selected.every((item) => item.speakerCount === 2) &&
    selection.authorization?.productionMutation === false,
  "the frozen Batch 3 selection changed"
);
assertV4(
  ledger.debates.map((debate) => debate.debateNumber).join(",") ===
    ...
    scores.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_03_PUBLICATION_DEBATES.join(","),
  "the Batch 3 ledger or score order changed"
);
assertV4(
...
const finalLedgerInputs = await loadPostCanaryBatch03FinalLedgerInputs();
validatePostCanaryBatch03FinalLedger(
  ledger,
...
const catalog = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-local-reference-catalog",
  references: postCanaryBatch03ReferenceCatalog()
};
...
for (const debateNumber of POST_CANARY_BATCH_03_PUBLICATION_DEBATES) {
  ...
  const packet = buildPostCanaryBatch03PublicationPacket({
    ledgerDebate,
    scoreDebate,
    sourcePacket,
  ...
  const schema = buildPostCanaryBatch03PublicationSchema(packet);
...
assertV4(
  ...
  "Batch 3 publication move coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.sections, 0) === 53,
  "Batch 3 publication section coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.audioVerifiedMoves, 0) === 8,
  "Batch 3 audio-verification coverage changed"
);
assertV4(
  contexts.every(
    (context) =>
  ...
  "Batch 3 publication section or quote eligibility is invalid"
);
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-publication-execution-preparation-manifest",
  ...
  userAuthorization: {
    instruction:
      "The frozen Batch 3 standing authorizations permit preparation, activation, and execution of exactly ten score-locked publication-reconstruction contexts while every gate passes, with direct incremental cost capped at $0.",
  ...
  nextAuthorizedAction:
    "standing-authorization-permits-activation-and-execution-of-the-ten-frozen-batch-03-publication-contexts"
};
