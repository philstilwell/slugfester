...
import { POST_CANARY_BATCH_14_PUBLICATION_COMPILATION_ORDER,
  ...
  compilePostCanaryBatch14PublicationStagingRecord } from
...
assertV4(m.status === "frozen-post-canary-batch-14-deterministic-publication-compilation-prepared" &&
  canonicalJson(m.explicitOrder) === canonicalJson(POST_CANARY_BATCH_14_PUBLICATION_COMPILATION_ORDER) &&
...
"Batch 14 compilation preparation changed");
let moves = 0;
for (const context of m.contexts) {
  ...
  const compiled = compilePostCanaryBatch14PublicationStagingRecord({ output, packet, identity: row });
  const validation = validatePostCanaryBatch14CompiledStagingRecord({ compiled, output, packet, identity: row });
