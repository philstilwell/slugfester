...
const BATCH_ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-17";
...
const PROTOCOL_ID = "assessment-production-post-canary-batch-17-discovery";
const WORKFLOW = "docs/assessment-production-canary-discovery-workflow.md";
const SCRIPT = "scripts/preregister-assessment-production-post-canary-batch-17-discovery.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-17-discovery-manifest.mjs";
...
assertV4(
  preparation.status === "post-canary-batch-17-four-complete-score-blind-source-packets-prepared-awaiting-validation" &&
    preparationValidation.status === "post-canary-batch-17-score-blind-source-packet-validation-passed-frozen-under-standing-authorization" &&
    ...
    preparation.totals.discoveryContexts === 17 &&
    preparation.totals.maximumCopiedInputBytes === 66203 &&
  ...
  "validated Batch 17 source preparation is not frozen at the required authorization boundary"
);
assertV4(
  ...
  "Batch 17 standing authorization does not cover exact discovery execution"
);
assertV4(
  JSON.stringify(preparation.contexts.map((debate) => debate.debateNumber)) === JSON.stringify(REQUIRED_ORDER),
  "Batch 17 debate order drifted"
);
assertV4(
  ...
  for (const chunk of debate.chunks) {
    ...
    assertV4(
      sha256(chunkBytes) === chunk.chunkLedgerSha256 &&
        ...
        buildBatch17TokenCountedChunkLedger(chunkBytes).equals(tokenBytes) &&
...
assertV4(contexts.length === 17, "Batch 17 discovery must contain exactly 17 contexts");
...
assertV4(maximumContext.debateNumber === "62" && maximumContext.chunkId === "chunk-004", "maximum copied-input context drifted");
...
const sourceFiles = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-17-source-preparation.mjs",
...
const manifest = {
  ...
  status: shouldWrite ? "frozen-seventeen-post-canary-batch-17-discovery-contexts-prepared-not-authorized" : "preview",
  ...
  AIOnly: true,
  userAuthorization: {
    scope: "standing-authorized Batch 17 discovery preparation and execution",
  ...
  costEstimate: {
    ...
    expectedAggregateModelMinutes: [17, 33],
    expectedAggregateComputeHours: [0.28, 0.55],
    absoluteGateTimeoutMinutes: 120,
    estimateBasis: "The exact 17-context final-short Batch 17 input set is hash-locked with an observed maximum of 66,203 copied bytes; no input is truncated or repartitioned. The frozen 1→2→4 scheduler uses the subscription-backed route with a $0 direct incremental cost boundary.",
  },
  ...
  nextAuthorizedAction: "prepare-and-freeze-batch-17-discovery-execution-activation-under-standing-authorization",
};
