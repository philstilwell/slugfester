...
const COMPILATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-04/deterministic-publication-compilation";
...
assertV4(
  compilationAnalysis.status ===
      "ten-debate-batch-04-deterministic-publication-compilation-passed" &&
    ...
    compilationAudit.status ===
      "passed-ten-debate-batch-04-deterministic-publication-compilation" &&
    ...
    compilationAudit.totals.moves === 203 &&
  ...
  "passing Batch 4 deterministic compilation evidence required"
);
...
for (const debateNumber of POST_CANARY_BATCH_04_PUBLICATION_FINALIZATION_ORDER) {
  ...
  assertV4(
    ...
    `${debateNumber}: Batch 4 finalization source context missing`
  );
  ...
  assertV4(
    ...
    `${debateNumber}: Batch 4 compiled input hash changed`
  );
...
const compatibility = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-publication-finalization-compatibility-analysis",
  protocolId: POST_CANARY_BATCH_04_PUBLICATION_FINALIZATION_PROTOCOL_ID,
  status: "batch-04-production-compatibility-boundary-recorded",
  analyzedAt: frozenAt,
  ...
  findings: [
    {
      id: "batch-04-site-ledger-adapter-and-validator-route",
      description:
        "The live site validator recognizes adjudicated-consensus ledgers only through the earlier checkpoint-specific adapter schema, activation, and packet root; it cannot authenticate or replay Batch 4 ledgers without a separately frozen Batch 4 adapter route.",
      evidence: {
        debates: 10,
        sections: 53,
        moves: 203,
      ...
      requiredRemedy:
        "A separately authorized production-compatibility plan must define, hash-lock, and test a Batch 4 site-ledger adapter and validator route without changing scores, prose, attribution, or the accepted optional-reference behavior."
    }
  ],
  authorization: {
...
const sourceFiles = [
  ...
  "scripts/prepare-assessment-production-post-canary-batch-04-publication-finalization.mjs",
  "scripts/activate-assessment-production-post-canary-batch-04-publication-finalization.mjs",
  "scripts/run-assessment-production-post-canary-batch-04-publication-finalization.mjs",
  "scripts/test-assessment-production-post-canary-batch-04-publication-finalization-preparation.mjs",
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-publication-finalization-preparation",
  protocolId: POST_CANARY_BATCH_04_PUBLICATION_FINALIZATION_PROTOCOL_ID,
  status: "frozen-post-canary-batch-04-publication-finalization-prepared-not-authorized",
  frozenAt,
  ...
  userAuthorization: {
    ...
    scopeInterpretation:
      "Prepare, validate, freeze, activate, and execute the deterministic Batch 4 publication-finalization pass under the standing authorization.",
  ...
  finalizationPolicy: {
    ...
    previewUsesPostCanaryBatch04StagingBanner: true,
    previewNoindex: true,
  ...
  nextAuthorizedAction:
    "activate-and-execute-one-frozen-batch-04-deterministic-publication-finalization-pass-under-standing-authorization"
};
  ...
  JSON.stringify(
    {
      status: shouldWrite
        ? manifest.status
        : "post-canary-batch-04-publication-finalization-preparation-preview",
