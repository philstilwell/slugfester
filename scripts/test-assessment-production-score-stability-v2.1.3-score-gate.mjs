...
import { evaluateV213ScoreStability } from "./lib/assessment-production-score-stability-v2.1.3-score-gate.mjs";
...
const accepted = evaluateV213ScoreStability(passing);
...
const tieAccepted = evaluateV213ScoreStability(tieCollapse);
...
assert.equal(
  evaluateV213ScoreStability(winnerMutation).acceptancePassed,
  false
);
const distanceMutation = structuredClone(passing);
distanceMutation[2].final = scores(90, 76);
assert.equal(
  evaluateV213ScoreStability(distanceMutation).acceptancePassed,
  false
);
