...
const SOURCE_PACKET_VERSION =
  "1.0-assessment-production-post-canary-batch-04-candidate-sharded-inventory-source-packet";
...
function buildInventorySourcePacket(sourcePacket) {
  assertV4(
    sourcePacket?.schemaVersion ===
      "1.0-assessment-production-post-canary-batch-04-score-blind-source-packet" &&
      sourcePacket.protocolId ===
        "assessment-production-post-canary-batch-04-source-preparation" &&
    ...
    "Batch 4 source packet is unavailable or not dyadic"
  );
  return {
...
function makePlanPacket({
  ...
  return {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-04-candidate-census-plan-packet",
    protocolId: PROTOCOL_ID,
...
async function buildFrozenArtifacts(frozenAt) {
  ...
  assertV4(
    ...
    "Batch 4 standing authorization does not cover exact inventory preparation and execution"
  );

  assertV4(
    discoveryAnalysis.status ===
      "post-canary-batch-04-discovery-passed-standing-authorization-active-for-inventory-preparation" &&
      ...
      discoveryAnalysis.totals?.modelContextsExecuted === 44 &&
      ...
      discoveryAnalysis.nextAuthorizedAction ===
        "prepare-freeze-and-activate-batch-04-candidate-census-planner-contexts-under-standing-authorization",
    "passed Batch 4 discovery boundary drifted"
  );
  assertV4(
    discoveryExecution.status ===
      "forty-four-post-canary-batch-04-discovery-contexts-passed" &&
    ...
    "Batch 4 discovery execution drifted"
  );
  assertV4(
      sourcePreparation.status ===
      "post-canary-batch-04-ten-complete-score-blind-source-packets-prepared-awaiting-validation" &&
    ...
    "Batch 4 source preparation or model boundary drifted"
  );
  assertV4(
    sourcePreparationValidation.status ===
      "post-canary-batch-04-score-blind-source-packet-validation-passed-frozen-awaiting-user-approval-before-any-model-execution" &&
    ...
    "Batch 4 source validation drifted"
  );
  assertV4(
    selection.status ===
      "fourth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    ...
    "Batch 4 frozen selection drifted"
  );
  assertV4(
  ...
  for (const discovered of discoveryAnalysis.debates) {
    ...
    assertV4(
      ...
      `${discovered.debateNumber}: Batch 4 identity drifted`
    );
    const [
  ...
  assertV4(
    ...
    "Batch 4 totals or packet state drifted"
  );
  ...
  const preparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-04-candidate-sharded-inventory-preparation",
    protocolId: PROTOCOL_ID,
    ...
    userAuthorization: {
      scope:
        "prepare, freeze, activate, and execute the Batch 4 candidate-census and candidate-sharded side-selection workflow under the frozen standing authorization",
    ...
    nextAuthorizedAction:
      "freeze-and-activate-batch-04-candidate-census-plan-execution-under-standing-authorization",
  };
  ...
  const executionPreparation = {
    ...
    costEstimate: {
      ...
      estimateBasis:
        `The promoted ten-context candidate-census planner used this contract at concurrency two. Batch 4 freezes the same ten-context shape with an observed maximum of ${maximumCopiedInputBytes.toLocaleString("en-US")} copied bytes, below the proven ${V212_CANDIDATE_SHARDED_INVENTORY.maximumCopiedInputBytes.toLocaleString("en-US")}-byte ceiling. ChatGPT-subscription execution has no direct incremental API charge.`,
    },
  ...
  const validation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-04-candidate-sharded-inventory-preparation-validation",
    protocolId: PROTOCOL_ID,
    status:
      "batch-04-candidate-sharded-inventory-packets-and-plan-execution-manifest-validation-passed-frozen-standing-authorization-active",
...
async function validateFrozen() {
  ...
  assertV4(
    ...
    "frozen Batch 4 inventory preparation is invalid"
  );
...
if (command === "freeze") await freeze();
  ...
  throw new Error(
    "usage: prepare-assessment-production-post-canary-batch-04-inventory.mjs <freeze|validate>"
  );
}
