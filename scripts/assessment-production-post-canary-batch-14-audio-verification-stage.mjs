...
const stageRoot = "docs/assessment-production/post-canary-continuation-v1/batch-14/audio-verification";
const paths = {
...
const expectedDebates = ["53", "110", "133", "12", "187", "106", "57"];
const expectedMoves = [
  "pro-ancient-historical-reporting-standard",
  "pro-prebiotic-sugar-pathway-limit",
  "con-noncausal-compatibilist-basis",
  "con-perceptual-categorization-pressure",
  "con-reality-identity-through-time",
  "con-hard-problem-vitalism-analogy",
  "pro-objective-value-beyond-catalogue",
  "con-mortality-vulnerability-personhood",
  "pro-encoding-insufficient-for-survival",
  "pro-highest-value-functions-as-god",
  "con-atheist-conduct-not-implicit-religion",
  "con-corrigible-wellbeing-moral-circle"
];
const expectedCalls = 12;
const durationOnlyPlanningEstimateUsd = 0.117975;
const usageDerivedPlanningEstimateUsd = 0.430595;
const toolPath = "scripts/assessment-production-post-canary-batch-14-audio-verification-stage.mjs";
...
const standingAuthorization = await loadAndValidatePostCanaryBatch14StandingAuthorization();
...
async function validatePreparation(preparation) {
  assert.equal(
    preparation.status,
    "prepared-twelve-post-canary-batch-14-paid-known-speaker-diarizations-standing-authorization-conditional-activation-ready"
  );
...
async function activate() {
  ...
  const activation = {
    ...preparation,
    schemaVersion: "1.0-assessment-production-post-canary-batch-14-audio-verification-execution-manifest",
    status: "frozen-twelve-post-canary-batch-14-paid-known-speaker-diarizations-authorized-under-standing-authorization",
    activatedAt,
    ...
    canonicalSourceGate: {
      ...
      transcriptHashesVerified: 7,
      eventHashesVerified: 7,
      manifestHashesVerified: 7,
      dyadicDebatesVerified: 7
    },
    executionToolHashes,
    costEstimate: {
    ...
    nextAuthorizedAction: "execute-exactly-twelve-post-canary-batch-14-paid-audio-verification-calls-once-sequentially"
  };
  ...
  console.log(JSON.stringify({
    ...
    maximumAuthorizedCostUsd: 1,
    canonicalSourceHashesVerified: 21,
    sequentialExecution: true,
    attemptsPerCall: 1,
    retriesMaximum: 0,
...
async function run() {
  ...
  for (const [callIndex, call] of manifest.calls.entries()) {
    ...
    process.stdout.write(`[batch-14-audio] ${callIndex + 1}/${expectedCalls} starting ${call.debateNumber}:${call.moveId}\n`);
    ...
    results.push(result);
    process.stdout.write(`[batch-14-audio] ${callIndex + 1}/${expectedCalls} ${result.status}; cumulative $${result.cumulativeUsageDerivedEstimatedCostUsd.toFixed(7)}\n`);
  }
  ...
  const execution = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-14-audio-verification-model-execution",
    protocolId: manifest.protocolId,
    status: completed.length === expectedCalls
      ? "twelve-post-canary-batch-14-paid-known-speaker-diarizations-completed"
      : "post-canary-batch-14-paid-diarization-incomplete",
    provider: "OpenAI Transcription API",
...
async function analyze() {
  ...
  const status = passed
    ? "passed-all-twelve-post-canary-batch-14-confidence-moves-audio-verified"
    : executionComplete
      ? "post-canary-batch-14-audio-verification-unresolved"
      : "post-canary-batch-14-audio-verification-incomplete";
  const nextAuthorizedAction = execution.costCapReachedOrExceeded || !execution.directIncrementalCostCapControlPassed
    ? "user-review-required-before-any-batch-14-downstream-work-after-audio-cost-cap-event"
    : passed
...
async function cost() {
  ...
  const result = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-14-audio-cost-control-analysis",
