...
const [manifest, execution, preparation] = await Promise.all(["execution-manifest.json", "model-execution.json", "preparation-manifest.json"].map((file) => readFile(path.resolve(`${V42211737_ROOT}/${file}`), "utf8").then(JSON.parse)));
...
for (const context of manifest.contexts) {
  ...
  if (result.gateAcceptancePassed) {
    const [rawOutput, output, packet] = await Promise.all([context.rawOutput, context.output, context.packet].map((file) => readFile(path.resolve(file), "utf8").then(JSON.parse)));
    const normalizedReplay = normalizeV42211737PublicationOutput(rawOutput, packet);
    assertV4(JSON.stringify(normalizedReplay.output) === JSON.stringify(output) && JSON.stringify(normalizedReplay.transformations) === JSON.stringify(result.deterministicQuoteNormalizations), `${context.debateNumber}: deterministic quote normalization replay mismatch`);
    replay = validateV42211736PublicationOutput(output, packet);
