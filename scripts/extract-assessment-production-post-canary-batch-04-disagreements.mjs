...
import {
  ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_04_DISAGREEMENT_PROTOCOL_ID,
  ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_04_DISAGREEMENT_ROOT,
  extractAssessmentProductionPostCanaryBatch04Disagreements
} from "./lib/assessment-production-post-canary-batch-04-disagreement.mjs";
import {
...
const USER_AUTHORIZATION = Object.freeze({
  ...
  standingAuthorizationPath: POST_CANARY_BATCH_04_STANDING_AUTHORIZATION,
...
const TOOL_SOURCES = [
  "scripts/extract-assessment-production-post-canary-batch-04-disagreements.mjs",
  ...
  "scripts/test-assessment-production-post-canary-batch-04-disagreements.mjs"
];
...
assertV4(
  judgmentAnalysis.status ===
    ...
    judgmentAnalysis.nextAuthorizedAction ===
      "extract-freeze-and-analyze-batch-04-disagreements-under-standing-authorization",
  "Batch 4 accepted-judgment gate or prior stop rule changed"
);
assertV4(
  JSON.stringify(judgmentAnalysis.pairs.map((pair) => pair.debateNumber)) ===
    JSON.stringify(EXPECTED_DEBATES),
  "Batch 4 independent-judgment population changed"
);
assertV4(
  ...
  "Batch 4 does not contain ten accepted, independent A/B judgment pairs"
);
assertV4(
  ...
  "Batch 4 twenty-context accepted execution contract changed"
);
assertV4(
  ...
  "Batch 4 model, authentication, score-blindness, or tie contract changed"
);
assertV4(
  executionActivation.executionPolicy.attemptsPerContext === 1 &&
    ...
    JSON.stringify(executionActivation.executionPolicy.schedulerRamp) ===
      JSON.stringify([1, 2]),
  "Batch 4 one-attempt or frozen 1-to-2 scheduler stop rule changed"
);
assertV4(
...
assertV4(
  ...
  "Batch 4 source-compatibility evidence changed"
);
assertV4(
  ...
  "Batch 4 accepted execution evidence changed"
);
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const disagreementPath =
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_04_DISAGREEMENT_ROOT}/disagreements/debate-${debateNumber}.json`;
...
assertV4(totalMoves === 203, "Batch 4 extraction must compare 203 moves");
assertV4(
  JSON.stringify(extractedAudioQueue.map(queueKey).sort()) ===
    JSON.stringify(independentGateQueue.map(queueKey).sort()),
  "Batch 4 frozen audio queue was not reproduced"
);

const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-disagreement-extraction",
  protocolId:
    ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_04_DISAGREEMENT_PROTOCOL_ID,
  status:
...
if (shouldWrite) {
  await mkdir(ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_04_DISAGREEMENT_ROOT, {
    recursive: true
  });
  await writeFile(
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_04_DISAGREEMENT_ROOT}/analysis.json`,
