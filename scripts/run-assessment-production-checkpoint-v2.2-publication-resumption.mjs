...
const activation = JSON.parse(await readFile(path.resolve(`${CHECKPOINT_V22_RESUMPTION_ROOT}/execution-activation.json`), "utf8"));
...
assertV4(
  ...
  "publication resumption is not authorized or its controls changed"
);
...
assertV4(currentVersion === activation.executionEnvironment.codexCliVersion, "the frozen Codex CLI version changed");
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `checkpoint-v22-resumption-${context.debateNumber}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `checkpoint-v22-resumption-home-${context.debateNumber}-`));
  ...
  try {
    ...
    const prompt = [
      ...
      "Participant judgment, adjudication, move selection, and every score are closed and repository-owned; the participant judgment was score-blind.",
    ...
    process.stdout.write(`[checkpoint-v2.2-resumption] starting index ${context.contextIndex} 5.6 Sol/low Debate ${context.debateNumber}\n`);
    ...
    if (invocation.timedOut || invocation.code !== 0 || invocation.signal !== null || !resultExists) {
      ...
      try {
        validationSummary = validateCheckpointV22PublicationOutput(
      ...
      const validation = {
        schemaVersion: "1.0-production-checkpoint-v2.2-publication-resumption-validation",
      ...
      const provenance = {
        schemaVersion: "1.0-production-checkpoint-v2.2-publication-resumption-provenance",
        protocolId: activation.protocolId,
        debateNumber: context.debateNumber,
        debateId: context.debateId,
  ...
  process.stdout.write(`[checkpoint-v2.2-resumption] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
...
const execution = {
  schemaVersion: "1.0-production-checkpoint-v2.2-publication-resumption-model-execution",
  protocolId: activation.protocolId,
  status: passed ? "nine-untouched-production-checkpoint-v2.2-publication-contexts-passed" : "production-checkpoint-v2.2-publication-resumption-complete-with-failure",
  gateStartedAt,
