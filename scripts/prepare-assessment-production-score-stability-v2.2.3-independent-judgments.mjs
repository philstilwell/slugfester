...
const PROTOCOL_ID = "assessment-production-score-stability-v2.2.3-fresh-validation-independent-judgments";
...
const PREDECESSOR_FAILURE =
  "docs/assessment-production/score-stability-v2.2.2-validation-cohort/independent-judgments/preparation-failure.json";
...
const predecessorFailure = JSON.parse(predecessorFailureBytes);
...
assertV4(
  ...
  "the inventory gate does not authorize v2.2.3 independent-judgment packet preparation"
);
assertV4(
  ...
  "the v2.2.3 inventory preparation is unavailable"
);
assertV4(
  ...
  "the v2.2.3 source preparation is unavailable"
);
assertV4(
  predecessorFailure.status ===
      "v2.2.2-independent-judgment-packet-preparation-failed-transport-ceiling" &&
    predecessorFailure.disposition?.failedPreparationPreserved === true &&
    predecessorFailure.disposition?.partialPacketFreezeAccepted === false &&
    predecessorFailure.disposition?.transportCeilingRaised === false &&
    predecessorFailure.disposition?.independentJudgmentModelExecutionAuthorized ===
      false &&
    predecessorFailure.nextAuthorizedAction ===
      "develop-v2.2.3-semantics-preserving-compact-judgment-schema-model-free-only",
  "the v2.2.2 failed packet checkpoint does not authorize this successor"
);
assertV4(
...
const preparation = {
  schemaVersion:
    "1.0-score-stability-v2.2.3-independent-judgment-preparation",
  ...
  predecessorPacketFailure: {
    path: PREDECESSOR_FAILURE,
    sha256: sha256(predecessorFailureBytes),
    preservedFailed: true,
    partialPacketFreezeAccepted: false,
    transportCeilingRaised: false,
  },
  proposedPolicy: {
  ...
  inputs: {
    ...
    predecessorFailure: PREDECESSOR_FAILURE,
    predecessorFailureSha256: sha256(predecessorFailureBytes),
