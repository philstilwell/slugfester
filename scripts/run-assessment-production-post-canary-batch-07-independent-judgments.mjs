...
assertV4(
  ...
  "Batch 7 independent-judgment execution is unauthorized"
);
assertV4(
...
async function executeContext(context, contextIndex) {
  ...
  assertV4(
    remainingGateMs > 0,
    "absolute Batch 7 independent-judgment gate timeout reached before launch"
  );
  ...
  const temporary = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-batch-07-judgment-${context.debateNumber}-${context.reviewerPass.toLowerCase()}-`
    )
  );
  const isolatedCodexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-batch-07-judgment-home-${context.debateNumber}-${context.reviewerPass.toLowerCase()}-`
    )
  );
  ...
  process.stdout.write(
    `[batch-07-judgment] Debate ${context.debateNumber} Pass ${context.reviewerPass} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-07-independent-judgment-model-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "twenty-post-canary-batch-07-independent-judgment-contexts-passed"
    : "post-canary-batch-07-independent-judgment-gate-complete-with-failure",
