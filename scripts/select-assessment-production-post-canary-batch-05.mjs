...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-05";
...
const PRIOR_SELECTION = "docs/assessment-production/post-canary-continuation-v1/batch-04/selection.json";
const PRIOR_SELECTION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-04/selection-analysis.json";
const PRIOR_MUTATION_MANIFEST = "docs/assessment-production/post-canary-continuation-v1/batch-04/production-publication/mutation-manifest.json";
const PRIOR_COMPLETION_EXECUTION = "docs/assessment-production/post-canary-continuation-v1/batch-04/production-publication/generated-seo-correction/dependent-pilot-analysis-correction/execution.json";
const PRIOR_COMPLETION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-04/production-publication/generated-seo-correction/dependent-pilot-analysis-correction/analysis.json";
const PRIOR_COMPLETION_COMMIT = "7effc324449805108ff1d23f47873939a3058ca1";
const SCRIPT = "scripts/select-assessment-production-post-canary-batch-05.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-05-selection.mjs";
...
assertV4(!(await exists(FUTURE_SOURCE_PREPARATION)), "source preparation already exists before Batch 5 selection");
...
assertV4(
  ...
  "Batch 5 selection is not compatible with the frozen continuation policy"
);
...
assertV4(
  priorSelection.status === "fourth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    priorSelection.batchNumber === 4 &&
    priorSelection.selected.length === 10 &&
    priorSelectionAnalysis.status === "fourth-post-canary-batch-selection-analysis-passed-awaiting-source-packet-preparation-decision" &&
    priorSelectionAnalysis.selection.sha256 === sha256(controlBytes[PRIOR_SELECTION]) &&
    priorMutationManifest.status === "frozen-batch-04-production-publication-mutation-manifest-prepared" &&
    JSON.stringify(priorMutationManifest.explicitOrder) === JSON.stringify(priorSelectedNumbers) &&
    priorCompletionExecution.status === "passed-complete-batch-04-production-transaction-after-dependent-pilot-analysis-correction" &&
    ...
    priorCompletionAnalysis.status === "batch-04-production-publication-accepted-after-dependent-pilot-analysis-correction" &&
  ...
  "Batch 4 has not satisfied the frozen successful-publication prerequisite"
);
assertV4(git("rev-parse", "--abbrev-ref", "HEAD") === "main", "Batch 5 selection must run on main");
assertV4(git("rev-parse", "HEAD") === PRIOR_COMPLETION_COMMIT, "Batch 5 selection checkpoint is not the exact Batch 4 publication commit");
assertV4(git("rev-parse", "origin/main") === PRIOR_COMPLETION_COMMIT, "Batch 4 publication commit is not the pushed origin/main checkpoint");
...
assertV4(
  JSON.stringify(priorSelectedNumbers) === JSON.stringify(ranked.slice(30, 40).map((item) => item.debateNumber)),
  "Batch 4 is not the immutable fourth rank slice"
);

const selected = [];
let totalEvents = 0;
let totalDurationMs = 0;
for (const [offset, rankedItem] of ranked.slice(40, 50).entries()) {
...
assertV4(selected.length === 10 && new Set(selected.map((item) => item.debateNumber)).size === 10, "Batch 5 did not select ten unique debates");

const checkpointCommit = git("rev-parse", "HEAD");
const selection = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-selection",
  protocolId: "assessment-production-post-canary-batch-05",
  status: "fifth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed",
  frozenAt,
  checkpointCommit,
  branch: "main",
  ...
  stagingOnly: true,
  batchNumber: 5,
  productionSequenceOrdinal: 6,
  userAuthorization: {
    instruction: "Continue.",
    scopeInterpretation: "Materialize and source-validate only Batch 5 under the frozen continuation policy after authenticating successful Batch 4 publication. Do not prepare source or model packets, execute models, derive scores, reconstruct publication prose, mutate production, or select Batch 6.",
  ...
  eligibility: {
    ...
    previouslyPublishedAndRemovedCount: 40,
    remainingBeforeBatchCount: ranked.length - 40,
    remainingBeforeBatchRankedSha256: sha256(serializedJson(ranked.slice(40))),
    selectedCount: selected.length,
    remainingUnselectedCount: ranked.length - 50,
    dyadicOnly: true,
  ...
  nextAuthorizedAction: "user-decision-on-fifth-post-canary-batch-source-packet-preparation"
};
...
const analysis = {
  ...
  status: "fifth-post-canary-batch-selection-analysis-passed-awaiting-source-packet-preparation-decision",
  analyzedAt: frozenAt,
  ...
  finding: "Successful Batch 4 production publication was authenticated at the pushed atomic commit. The immutable campaign rank then selected ordinals 41 through 50 as ten unique pending dyadic debates, and all thirty effective source files and ten canonical event chains passed without replacement.",
  decision: {
