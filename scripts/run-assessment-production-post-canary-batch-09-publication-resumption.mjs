...
assertV4(
  ...
  "the Batch 9 publication resumption is not authorized"
);
assertV4(
...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), `batch-09-publication-resumption-${context.debateNumber}-`)
  );
  const codexHome = await mkdtemp(
    path.join(os.tmpdir(), `batch-09-publication-resumption-home-${context.debateNumber}-`)
  );
  ...
  try {
    ...
    process.stdout.write(
      `[batch-09-publication-resumption] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
    ...
    if (
      ...
      try {
        validationSummary = validatePostCanaryBatch09PublicationOutput(
      ...
      const validationRecord = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-09-publication-resumption-validation",
      ...
      const provenanceRecord = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-09-publication-resumption-provenance",
        protocolId: activation.protocolId,
        contextIndex: context.contextIndex,
        originalContextIndex: context.originalContextIndex,
        debateNumber: context.debateNumber,
  ...
  process.stdout.write(
    `[batch-09-publication-resumption] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-09-publication-resumption-model-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "nine-post-canary-batch-09-publication-resumption-contexts-passed"
    : "post-canary-batch-09-publication-resumption-complete-with-failure",
  gateStartedAt,
  ...
  participantJudgmentWasScoreBlind: true,
  ownDebateScoresImmutable: true,
  acceptedDebate170RepairContexts: 7,
  acceptedDebate170RepairedFields: 14,
  meteredApiCostUsd: 0,
  paidServiceCallsThisStage: 0,
  modelAuthoredScores: 0,
  scorePassesExecutedThisStage: 0,
