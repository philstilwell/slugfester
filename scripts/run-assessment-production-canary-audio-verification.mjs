...
assertV4(
  ...
  "production-canary paid audio execution unauthorized"
);
...
for (const call of manifest.calls) {
  ...
  process.stdout.write(
    `[production-canary-audio] starting ${call.debateNumber}:${call.moveId}\n`
  );
  ...
  const result = {
    ...
    estimatedExposureUsd: Number(
      (
        ...
        manifest.costEstimate.planningPricePerMinuteUsd
      ).toFixed(6)
    )
  };
  results.push(result);
  process.stdout.write(
    `[production-canary-audio] ${call.debateNumber}:${call.moveId} ${result.status}\n`
  );
}
...
const execution = {
  schemaVersion: "1.0-production-canary-audio-verification-model-execution",
  protocolId: manifest.protocolId,
  status:
    completed.length === manifest.calls.length
      ? "four-paid-known-speaker-diarizations-completed"
      : "production-canary-paid-diarization-incomplete",
