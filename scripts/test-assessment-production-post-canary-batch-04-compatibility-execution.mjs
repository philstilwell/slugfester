...
const root = POST_CANARY_BATCH_04_COMPATIBILITY_ROOT;
...
const validator = await import(
  `./validate-debates.mjs?batch04Compatibility=${activation.validator.proposedSha256}`
);
...
for (const lock of activation.packetHashes) {
  ...
  const routeLock =
    validator.validatePostCanaryBatch04LedgerAdapterRouteLocks({
      debate: candidate,
      ledgerText,
  ...
  const route = validator.validatePostCanaryBatch04LedgerAdapterRoute({
    debate: candidate,
    ledger,
    ledgerText
  });
  assert.equal(route.repositoryScoreReplayPassed, true);
  const adapter = validatePostCanaryBatch04SiteLedgerAdapter({
    adapter: ledger,
    candidate,
...
assert.equal(emptyReferenceLinks, 49);
