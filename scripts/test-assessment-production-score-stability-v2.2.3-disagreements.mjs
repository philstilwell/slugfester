...
import { extractAssessmentProductionScoreStabilityV223Disagreements } from "./lib/assessment-production-score-stability-v2.2.3-disagreement.mjs";
...
assert.equal(analysis.adjudicationWorkload.totalMoves, 197);
assert.equal(
...
assert.equal(
  analysis.proposedPolicy.agreedWinningSideMayCollapseToIntegerRoundedTie,
  true
);
assert.equal(
  analysis.proposedPolicy.agreedInitialTieImposesNoDirectionConstraint,
  true
);
assert.equal(analysis.proposedPolicy.numericalThresholdsChanged, false);
...
for (const debate of analysis.debates) {
  ...
  const [primaryA, primaryB, lockedInventory] = await Promise.all([
    ...
    readFile(
      `docs/assessment-production/score-stability-v2.2.2-validation-cohort/inventory-route-section-plan-successor/locked-inventories/debate-${debate.debateNumber}.json`,
      "utf8"
    ).then(JSON.parse)
  ]);
  const replayed =
