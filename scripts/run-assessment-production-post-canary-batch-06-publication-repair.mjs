...
assertV4(activation.status === "frozen-twenty-five-context-batch-06-publication-repair-authorized" &&
  activation.contexts?.length === 25 && activation.authorization?.repairModelExecution === true &&
  ...
  new Set(activation.contexts.flatMap((row) => row.writableFields)).size === 49,
"the Batch 6 publication repair execution is not authorized");
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-06-publication-repair-${context.contextIndex}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `batch-06-publication-repair-home-${context.contextIndex}-`));
  ...
  try {
    ...
    const prompt = [
      ...
      `Act only as isolated score-locked Batch 6 publication repair editor ${context.packetId}.`,
    ...
    process.stdout.write(`[batch-06-publication-repair] starting index ${context.contextIndex} ${context.packetId} ${activation.model.label}/${activation.model.reasoningEffort}\n`);
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 ||
      ...
      try {
        validationSummary = validateBatch06PublicationRepairOutput(JSON.parse(outputBytes),
      ...
      const validation = { schemaVersion: "1.0-assessment-production-post-canary-batch-06-publication-repair-validation",
      ...
      const provenance = { schemaVersion: "1.0-assessment-production-post-canary-batch-06-publication-repair-provenance",
  ...
  process.stdout.write(`[batch-06-publication-repair] index ${context.contextIndex} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
...
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-06-publication-repair-execution",
  protocolId: activation.protocolId,
  status: validContexts === 25
    ? "batch-06-twenty-five-context-publication-repair-gate-passed"
    : "batch-06-publication-repair-gate-complete-with-failure",
  gateStartedAt, gateCompletedAt: new Date().toISOString(), contextsPlanned: 25,
  contextsAttempted: results.length, contextsUnattempted: 25 - results.length,
  ...
  maximumObservedConcurrency: 2, schedulerRamp: [1, 2], wallElapsedMs: Date.now() - started,
