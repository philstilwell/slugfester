...
import {
  POST_CANARY_BATCH_02_COMPATIBILITY_ACTIVATION_STATUS,
  buildPostCanaryBatch02ValidatorSource,
  sha256,
  validatePostCanaryBatch02ValidatorSource
} from "./lib/assessment-production-post-canary-batch-02-compatibility-execution.mjs";
...
const paths = {
  ...
  preparationAnalysis: `${POST_CANARY_BATCH_02_COMPATIBILITY_ROOT}/analysis.json`,
  ...
  executionLibrary:
    "scripts/lib/assessment-production-post-canary-batch-02-compatibility-execution.mjs",
  activationScript:
    "scripts/activate-assessment-production-post-canary-batch-02-compatibility.mjs",
  activationTest:
    "scripts/test-assessment-production-post-canary-batch-02-compatibility-activation.mjs",
  executionScript:
    "scripts/run-assessment-production-post-canary-batch-02-compatibility.mjs",
  executionTest:
    "scripts/test-assessment-production-post-canary-batch-02-compatibility-execution.mjs"
};
...
assertV4(
  ...
  "frozen Batch 2 compatibility preparation is unavailable"
);
assertV4(
  ...
  "frozen Batch 2 compatibility preparation analysis is unavailable"
);
...
assertV4(
  ...
  "a stable --activated-at ISO timestamp is required for the first Batch 2 compatibility activation write"
);
assertV4(
  !(await exists(paths.execution)) && !(await exists(paths.stagedLedgerRoot)),
  "Batch 2 compatibility execution already exists; activation blocked"
);
...
assertV4(
  canonicalJson(packetHashes.map((item) => item.debateNumber)) ===
    canonicalJson(POST_CANARY_BATCH_02_COMPATIBILITY_ORDER),
  "Batch 2 compatibility activation packet order changed"
);
...
assertV4(
  ...
  "active validator differs from the frozen Batch 2 baseline"
);
const proposedValidator = buildPostCanaryBatch02ValidatorSource(
  baselineValidator
);
const proposedValidatorValidation =
  validatePostCanaryBatch02ValidatorSource(proposedValidator);
...
const activation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-compatibility-execution-activation",
  protocolId: preparation.protocolId,
  status: POST_CANARY_BATCH_02_COMPATIBILITY_ACTIVATION_STATUS,
  activatedAt,
  ...
  userAuthorization: {
    instruction:
      "I approve the activation and exactly one deterministic Batch 2 compatibility staging pass.",
    scopeInterpretation:
      "Activate the frozen Batch 2 compatibility manifest and execute exactly one validator-and-staged-ledger compatibility pass with no reruns, models, paid services, production ledger publication, production data mutation, or Batch 3 selection."
  },
  preparation: {
  ...
  validator: {
    ...
    routeCredentialStatus: POST_CANARY_BATCH_02_COMPATIBILITY_ACTIVATION_STATUS
  },
  packetHashes,
  executionToolHashes,
  protectedProduction: {
  ...
  requiredRegressions: [
    "Batch 2 route positive and negative controls",
  ...
  nextAuthorizedAction:
    "execute-one-frozen-batch-02-production-compatibility-staging-pass-no-reruns"
};
const activationBytes = serializedJson(activation);

if (write) {
  assertV4(!existingActivation, "Batch 2 compatibility is already activated");
  await writeFile(resolve(paths.activation), activationBytes);
} else {
  assertV4(
    existingActivation &&
      canonicalJson(existingActivation) === canonicalJson(activation),
    "stored Batch 2 compatibility activation differs from deterministic replay"
  );
}
