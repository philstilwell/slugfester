...
const manifest = JSON.parse(await readFile(path.resolve(`${V42211735_ROOT}/execution-manifest.json`), "utf8"));
...
async function runContext(context, contextIndex) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `v42211735-publication-${context.debateNumber}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `v42211735-publication-home-${context.debateNumber}-`));
  ...
  process.stdout.write(`[v4.2.21.17.35-publication-stability] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
