...
assertV4(
  ...
  "the Batch 3 publication resumption is not authorized"
);
assertV4(
  activation.model?.label === "5.6 Sol" &&
    ...
    activation.executionPolicy?.contexts === 6 &&
  ...
  "the Batch 3 publication resumption controls changed"
);
...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), `batch-03-publication-resumption-2-${context.debateNumber}-`)
  );
  const codexHome = await mkdtemp(
    path.join(os.tmpdir(), `batch-03-publication-resumption-2-home-${context.debateNumber}-`)
  );
  ...
  try {
    ...
    process.stdout.write(
      `[batch-03-publication-resumption-2] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
    ...
    if (
      ...
      try {
        validationSummary = validatePostCanaryBatch03PublicationOutput(
      ...
      const validationRecord = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-03-publication-resumption-2-validation",
      ...
      const provenanceRecord = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-03-publication-resumption-2-provenance",
        protocolId: activation.protocolId,
        contextIndex: context.contextIndex,
        originalContextIndex: context.originalContextIndex,
        debateNumber: context.debateNumber,
  ...
  process.stdout.write(
    `[batch-03-publication-resumption-2] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-publication-resumption-2-model-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "six-post-canary-batch-03-publication-resumption-2-contexts-passed"
  ...
  participantJudgmentWasScoreBlind: true,
  ownDebateScoresImmutable: true,
  acceptedDebatesBeforeResumption: 4,
  acceptedMovesBeforeResumption: 80,
  acceptedRepairContextsBeforeResumption: 5,
  acceptedRepairedFieldsBeforeResumption: 8,
  meteredApiCostUsd: 0,
  paidServiceCallsThisStage: 0,
  modelAuthoredScores: 0,
  scorePassesExecutedThisStage: 0,
