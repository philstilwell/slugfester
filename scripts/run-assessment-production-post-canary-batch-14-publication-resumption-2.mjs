...
assertV4(
  activation.status === "frozen-six-original-unattempted-batch-14-publication-contexts-activated" &&
...
async function runContext(context) {
  ...
  try {
    ...
    const prompt = [
      ...
      "Never infer, emit, recalculate, or suggest changing a score; never change identity, structure, move selection, or source evidence; never consult legacy assessment material, any accepted prior Batch 14 output, other debates, rankings, winners, or another model context; never attribute AI material to a participant.",
