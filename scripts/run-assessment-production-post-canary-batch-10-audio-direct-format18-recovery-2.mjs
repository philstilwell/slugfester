...
if (!shouldExecute) {
  console.log(
    JSON.stringify(
      {
        status: "passed-activated-batch-10-audio-direct-format18-recovery-2-preflight",
...
const installSource = async (source) => {
  ...
  const directSource = plan.directFormat18Sources.find(
    (item) => item.sourceVideoId === source.sourceVideoId
  );
  assertV4(
    source.mode === "direct-format18-once" && directSource,
    `${debateNumber}: direct format-18 source plan is missing`
  );
  assertV4(!(await exists(finalSourcePath)), `${debateNumber}: source already exists`);
  normalizedTemporaryPath = path.join(
    audioDirectory,
    "source.direct-format18.normalized.mp3"
  );
  const directUrl = execFileSync(
    ...
    [
      ...
      "youtube:player_client=android,web", "-f", directSource.formatId,
      "-g", directSource.videoUrl
    ],
    { encoding: "utf8" }
  ).trim();
  ...
  assertV4(
    ...
    `${debateNumber}: direct format-18 URL resolution failed`
  );
  state.ffmpegStreamingInvocations += 1;
  normalize(
    directUrl,
    normalizedTemporaryPath,
    directSource.ffmpegStreamingTimeoutMs
  );
  ...
  acquisitionMode = "streamed-public-source-direct-format18";
  state.sourcesInstalled += 1;
  publicSourceAttemptAudit.push({
    debateNumber,
    videoId: source.sourceVideoId,
...
try {
  ...
  const preparation = {
    ...
    acquisitionPolicy: {
      ...
      acquisitionFormat: "frozen-youtube-format18-direct-url-stream",
  ...
  executionFailure = {
    category: "source-or-validation-failure-during-authorized-direct-format18-recovery-2",
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-10-audio-source-direct-format18-recovery-2-execution",
  status: executionFailure
    ? "failed-one-shot-batch-10-audio-source-direct-format18-recovery-2-stop-required"
    : "completed-one-shot-batch-10-audio-source-direct-format18-recovery-2-and-cohort",
  batchNumber: 10,
  planSha256: sha256(planBytes),
  activationSha256: sha256(activationBytes),
  state,
