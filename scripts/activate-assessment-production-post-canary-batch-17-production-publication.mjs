...
const preparationAnalysisPath = `${POST_CANARY_BATCH_17_PRODUCTION_PUBLICATION_ROOT}/preparation-analysis.json`;
...
assertV4(
  manifest.status === "frozen-batch-17-production-publication-mutation-manifest-prepared" &&
    manifest.protocolId === POST_CANARY_BATCH_17_PRODUCTION_PUBLICATION_PROTOCOL_ID &&
    manifest.debates?.length === 4 &&
  ...
  "frozen Batch 17 production publication manifest required"
);
assertV4(
  preparationAnalysis.status === "batch-17-production-publication-mutation-manifest-freeze-passed" &&
    preparationAnalysis.manifest.sha256 === sha256(serializedJson(manifest)),
  "accepted Batch 17 production publication preparation required"
);
...
assertV4(
  ...
  "proposed production source changes outside Batch 17"
);

const executionTools = await Promise.all([
  "scripts/lib/assessment-production-post-canary-batch-17-production-publication.mjs",
  "scripts/run-assessment-production-post-canary-batch-17-production-publication.mjs",
  "scripts/test-assessment-production-post-canary-batch-17-production-publication-activation.mjs",
  "scripts/validate-debates.mjs",
  "scripts/test-assessment-production-score-stability-policy-active.mjs"
].map(lockFile));
const activation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-17-production-publication-execution-activation",
  protocolId: POST_CANARY_BATCH_17_PRODUCTION_PUBLICATION_PROTOCOL_ID,
  status: "frozen-batch-17-production-publication-mutation-pass-activated",
  activatedAt,
  ...
  nextAuthorizedAction: "execute-one-frozen-batch-17-production-publication-mutation-pass-no-reruns"
};
if (write) {
...
console.log(serializedJson({ status: activation.status, write, proposedDebatesSha256: activation.frozenOutput.proposedSha256, productionLedgerOutputs: 4 }));
