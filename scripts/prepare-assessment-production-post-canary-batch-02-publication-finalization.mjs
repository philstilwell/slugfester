...
import {
  ...
  buildPostCanaryBatch02PublicationFinalization,
  validatePostCanaryBatch02PublicationFinalCandidate
...
const COMPILATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-02/deterministic-publication-compilation";
...
assertV4(
  compilationAnalysis.status ===
      "ten-debate-batch-02-deterministic-publication-compilation-passed" &&
    ...
    compilationAudit.status ===
      "passed-ten-debate-batch-02-deterministic-publication-compilation" &&
    ...
    compilationAudit.totals.sections === 51 &&
    compilationAudit.totals.moves === 190 &&
  ...
  "passing Batch 2 deterministic compilation evidence required"
);

const contexts = [];
const compatibilityRows = [];
const sourceChainFiles = [];
for (const debateNumber of POST_CANARY_BATCH_02_PUBLICATION_FINALIZATION_ORDER) {
  ...
  assertV4(
    ...
    `${debateNumber}: Batch 2 finalization source context missing`
  );
  ...
  assertV4(
    ...
    `${debateNumber}: Batch 2 compiled input hash changed`
  );
  const compiled = JSON.parse(compiledBytes);
  const built = buildPostCanaryBatch02PublicationFinalization({
    compiled,
  ...
  const validation = validatePostCanaryBatch02PublicationFinalCandidate({
...
const compatibility = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-publication-finalization-compatibility-analysis",
  protocolId: POST_CANARY_BATCH_02_PUBLICATION_FINALIZATION_PROTOCOL_ID,
  status: "batch-02-production-compatibility-boundary-recorded",
  analyzedAt: frozenAt,
  ...
  findings: [
    {
      id: "batch-02-site-ledger-adapter-and-validator-route",
      description:
        "The live site validator recognizes adjudicated-consensus ledgers only through the earlier checkpoint-specific adapter schema, activation, and packet root; it cannot authenticate or replay Batch 2 ledgers without a separately frozen Batch 2 adapter route.",
      evidence: {
        debates: 10,
        sections: 51,
        moves: 190,
      ...
      requiredRemedy:
        "A separately authorized production-compatibility plan must define, hash-lock, and test a Batch 2 site-ledger adapter and validator route without changing scores, prose, attribution, or the accepted optional-reference behavior."
    }
  ],
  authorization: {
...
const sourceFiles = [
  ...
  "scripts/prepare-assessment-production-post-canary-batch-02-publication-finalization.mjs",
  "scripts/activate-assessment-production-post-canary-batch-02-publication-finalization.mjs",
  "scripts/run-assessment-production-post-canary-batch-02-publication-finalization.mjs",
  "scripts/test-assessment-production-post-canary-batch-02-publication-finalization-preparation.mjs",
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-publication-finalization-preparation",
  protocolId: POST_CANARY_BATCH_02_PUBLICATION_FINALIZATION_PROTOCOL_ID,
  status: "frozen-post-canary-batch-02-publication-finalization-prepared-not-authorized",
  frozenAt,
  ...
  userAuthorization: {
    ...
    scopeInterpretation:
      "Prepare, validate, freeze, activate, and execute the deterministic Batch 2 publication-finalization pass under the standing authorization.",
  ...
  explicitOrder: POST_CANARY_BATCH_02_PUBLICATION_FINALIZATION_ORDER,
  contexts,
  executionPolicy: {
  ...
  finalizationPolicy: {
    ...
    previewUsesPostCanaryBatch02StagingBanner: true,
    previewNoindex: true,
  ...
  nextAuthorizedAction:
    "activate-and-execute-one-frozen-batch-02-deterministic-publication-finalization-pass-under-standing-authorization"
};
  ...
  JSON.stringify(
    {
      status: shouldWrite
        ? manifest.status
        : "post-canary-batch-02-publication-finalization-preparation-preview",
