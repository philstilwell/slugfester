...
const expectedDebates = ["158", "189", "05"];
const expectedMoves = [
  "pro-case-specific-extraordinary-testimony-standard",
  "con-unverified-resurrection-prior",
  "con-no-presented-extrabiblical-support",
  "con-simple-laws-beneath-cell-complexity",
  "con-logical-grounding-burden",
  "pro-logic-reflects-gods-thinking"
];
const toolPath = "scripts/assessment-production-post-canary-batch-05-audio-verification-stage.mjs";
...
async function activate() {
  ...
  const activation = {
    ...preparation,
    schemaVersion: "1.0-assessment-production-post-canary-batch-05-audio-verification-execution-manifest",
    status: "frozen-six-post-canary-batch-05-paid-known-speaker-diarizations-authorized-under-standing-authorization",
    activatedAt,
    ...
    userExecutionAuthorization: {
      ...
      frozenUsageDerivedEstimateUsd: 0.1780675,
    ...
    nextAuthorizedAction: "execute-exactly-six-post-canary-batch-05-paid-audio-verification-calls-once-sequentially"
  };
...
async function validateActivation(manifest) {
  ...
  assert.equal(manifest.costEstimate.primaryExpectedFutureExecutionCostUsd, 0.1780675);
...
async function run() {
  ...
  for (const [callIndex, call] of manifest.calls.entries()) {
    ...
    process.stdout.write(`[batch-05-audio] ${callIndex + 1}/6 starting ${call.debateNumber}:${call.moveId}\n`);
    ...
    results.push(result);
    process.stdout.write(`[batch-05-audio] ${callIndex + 1}/6 ${result.status}; cumulative $${result.cumulativeUsageDerivedEstimatedCostUsd.toFixed(7)}\n`);
  }
  ...
  const execution = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-05-audio-verification-model-execution",
    protocolId: manifest.protocolId,
    status: completed.length === 6
      ? "six-post-canary-batch-05-paid-known-speaker-diarizations-completed"
      : "post-canary-batch-05-paid-diarization-incomplete",
    provider: "OpenAI Transcription API",
...
async function analyze() {
  ...
  const status = passed
    ? "passed-all-six-post-canary-batch-05-confidence-moves-audio-verified"
    : executionComplete
      ? "post-canary-batch-05-audio-verification-unresolved"
      : "post-canary-batch-05-audio-verification-incomplete";
  const nextAuthorizedAction = execution.costCapReachedOrExceeded || !execution.directIncrementalCostCapControlPassed
    ? "user-review-required-before-any-batch-05-downstream-work-after-audio-cost-cap-event"
    : passed
      ? "prepare-freeze-and-push-batch-05-dispute-only-adjudication-packets-under-standing-authorization"
      : "standing-authorization-stop-new-approval-required-before-batch-05-audio-verification-failure-diagnosis";
  const authorization = {
  ...
  const audit = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-05-audio-verification-audit",
    protocolId: manifest.protocolId,
    status,
  ...
  const analysis = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-05-audio-verification-analysis",
    protocolId: manifest.protocolId,
    status,
    productionCanary: false,
    batchNumber: 5,
...
async function cost() {
  ...
  const result = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-05-audio-cost-control-analysis",
    ...
    costControl: {
      ...
      usageDerivedPlanningEstimateUsd: 0.1780675,
      approvedMaximumCostUsd: 1,
      usageDerivedEstimatedCostUsd: totals.usageDerivedEstimatedCostUsd,
      estimateDifferenceUsd: totals.usageDerivedEstimatedCostUsd - 0.1780675,
...
async function test() {
  ...
  assert.equal(costResult.costControl.usageDerivedPlanningEstimateUsd, 0.1780675);
