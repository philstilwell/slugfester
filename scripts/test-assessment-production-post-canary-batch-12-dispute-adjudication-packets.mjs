...
import {
  POST_CANARY_BATCH_12_DISPUTE_ADJ_ISOLATION,
  POST_CANARY_BATCH_12_DISPUTE_ADJ_OUTPUT_VERSION,
...
const EXPECTED_DEBATES = ["152", "28", "119", "116", "131", "07", "33", "15", "29", "11"];
...
const preparation = JSON.parse(
  await readFile(`${POST_CANARY_BATCH_12_DISPUTE_ADJ_ROOT}/preparation-manifest.json`, "utf8")
);
...
assert.equal(preparation.totals.disputedMoves, 196);
assert.equal(preparation.totals.candidateSelections, 528);
...
for (const context of preparation.contexts) {
  ...
  const output = {
    schemaVersion: POST_CANARY_BATCH_12_DISPUTE_ADJ_OUTPUT_VERSION,
    ...
    isolation: structuredClone(POST_CANARY_BATCH_12_DISPUTE_ADJ_ISOLATION),
  ...
  const validation = validatePostCanaryBatch12DisputeAdjudicationOutput(output, packet);
...
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_12_DISPUTE_ADJ_ROOT}/packets`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_12_DISPUTE_ADJ_ROOT}/provenance`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
