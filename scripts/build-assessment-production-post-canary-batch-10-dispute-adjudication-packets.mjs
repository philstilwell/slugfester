...
import {
  POST_CANARY_BATCH_10_DISPUTE_ADJ_PROTOCOL_ID,
  POST_CANARY_BATCH_10_DISPUTE_ADJ_ROOT,
  buildPostCanaryBatch10DisputeAdjudicationPacket,
  makePostCanaryBatch10DisputeAdjudicationSchema
} from "./lib/assessment-production-post-canary-batch-10-dispute-adjudication.mjs";
...
const USER_AUTHORIZATION = Object.freeze({
  instruction:
    "The frozen Batch 10 standing authorization permits automatic preparation, validation, freezing, activation, execution, analysis, committing, and pushing of score-blind dispute-only adjudication checkpoints for exactly the ten selected debates while every frozen gate passes, with subscription-backed and local direct incremental cost capped at $0.",
...
const inputPaths = {
  ...
  schema: `${POST_CANARY_BATCH_10_DISPUTE_ADJ_ROOT}/adjudication.schema.json`
};
const TOOL_SOURCES = [
  ...
  "scripts/build-assessment-production-post-canary-batch-10-dispute-adjudication-packets.mjs",
  "scripts/test-assessment-production-post-canary-batch-10-dispute-adjudication-packets.mjs",
  "scripts/validate-assessment-production-post-canary-batch-10-dispute-adjudication-output.mjs",
  "scripts/preregister-assessment-production-post-canary-batch-10-dispute-adjudication.mjs",
  "scripts/test-assessment-production-post-canary-batch-10-dispute-adjudication-manifest.mjs"
];
...
assertV4(
  sourceAnalysis.status ===
      "post-canary-batch-10-deterministic-disagreements-extracted-standing-authorization-active-for-audio-work" &&
    JSON.stringify(sourceAnalysis.debates.map((item) => item.debateNumber)) ===
      JSON.stringify(EXPECTED_DEBATES) &&
    sourceAnalysis.adjudicationWorkload.disputedMoves === 177 &&
    sourceAnalysis.adjudicationWorkload.candidateSelections === 483 &&
    sourceAnalysis.adjudicationWorkload.packetsPrepared === false &&
    sourceAnalysis.adjudicationWorkload.modelContextsExecuted === 0,
  "Batch 10 deterministic disagreement workload changed"
);
assertV4(
  judgmentExecution.status ===
      "twenty-post-canary-batch-10-independent-judgment-contexts-passed" &&
    ...
    judgmentAnalysis.status ===
      "twenty-post-canary-batch-10-independent-judgments-passed-standing-authorization-active-for-disagreement-extraction" &&
    judgmentAnalysis.totals.contexts === 20,
  "Batch 10 accepted independent-judgment boundary changed"
);
assertV4(
  ...
  "Batch 10 independent-judgment configuration changed"
);
...
assertV4(
  ...
  "Batch 10 standing authorization changed"
);
assertV4(
  audioAudit.status ===
      "passed-all-nine-post-canary-batch-10-confidence-moves-audio-verified" &&
    audioAudit.totals.requiredMoves === 9 &&
    audioAudit.totals.verified === 9 &&
    ...
    audioCost.costControl.usageDerivedEstimatedCostUsd === 0.45836250000000006 &&
    audioCost.costControl.approvedMaximumCostUsd === 1 &&
    audioCost.costControl.directIncrementalCostCapControlPassed === true &&
    audioExecutionManifest.calls.length === 9 &&
    audioModelExecution.callsAttempted === 9 &&
    audioModelExecution.callsCompleted === 9 &&
    audioModelExecution.retries === 0,
  "Batch 10 accepted audio-verification boundary changed"
);
assertV4(
  ...
  "Batch 10 packet-preparation authorization changed"
);
...
assertV4(audioByDebateMove.size === 9, "Batch 10 verified audio population changed");

const schema = makePostCanaryBatch10DisputeAdjudicationSchema();
...
if (shouldWrite) {
  const immutableOutputs = [
    inputPaths.schema,
    `${POST_CANARY_BATCH_10_DISPUTE_ADJ_ROOT}/preparation-manifest.json`,
    `${POST_CANARY_BATCH_10_DISPUTE_ADJ_ROOT}/execution-preparation-manifest.json`,
    ...EXPECTED_DEBATES.flatMap((debateNumber) => [
      `${POST_CANARY_BATCH_10_DISPUTE_ADJ_ROOT}/packets/debate-${debateNumber}.json`,
      `${POST_CANARY_BATCH_10_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`
    ])
  ];
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const built = buildPostCanaryBatch10DisputeAdjudicationPacket(
  ...
  const provenancePath =
    `${POST_CANARY_BATCH_10_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`;
  const outputPath =
    `${POST_CANARY_BATCH_10_DISPUTE_ADJ_ROOT}/outputs/debate-${debateNumber}.json`;
  const packetBytes = pretty(built.packet);
  const provenanceBytes = pretty({
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-10-adjudication-candidate-provenance",
...
const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-10-dispute-only-adjudication-preparation",
  protocolId: POST_CANARY_BATCH_10_DISPUTE_ADJ_PROTOCOL_ID,
  status: shouldWrite
    ? "prepared-ten-isolated-post-canary-batch-10-dispute-only-adjudication-contexts"
    : "preview",
  frozenAt,
  ...
  acceptedSourceBoundary: {
    ...
    allNineAudioMovesVerified: true,
    audioValidationOverlaysPreserved: 0,
    audioCorrectionCallsPreserved: 0,
    audioUsageDerivedEstimatedCostUsd: 0.4583625,
  ...
  nextAuthorizedAction:
    "freeze-post-canary-batch-10-dispute-only-adjudication-execution-preparation-manifest-model-free-only"
};
assertV4(
  contexts.length === 10 &&
    disputedMoves === 177 &&
    ...
    audioVerifiedMoves === 9,
  "Batch 10 adjudication packet workload differs from frozen inputs"
);
assertV4(
  preparation.totals.maximumCopiedInputBytes <= 400000,
  `Batch 10 context ${preparation.totals.maximumCopiedInputBytes} bytes exceeds the frozen 400 KB correction ceiling`
);
if (shouldWrite) {
  await mkdir(POST_CANARY_BATCH_10_DISPUTE_ADJ_ROOT, { recursive: true });
