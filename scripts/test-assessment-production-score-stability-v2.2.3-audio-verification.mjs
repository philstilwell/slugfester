...
if (!(await exists(prepPath))) {
  const sourcePreparation = JSON.parse(
    await readFile(
      "docs/assessment-production/score-stability-v2.2.3-validation-cohort/disagreement-extraction/audio-source-preparation.json",
      "utf8"
    )
  );
  assert.equal(sourcePreparation.status, "prepared-four-v2.2.3-local-audio-clips");
  assert.equal(sourcePreparation.clips.length, 4);
  ...
  console.log(JSON.stringify({ status: "passed-prefreeze", clips: 4, paidCalls: 0, scoresDerived: 0 }, null, 2));
  process.exit(0);
}
...
assert.equal(preparation.costEstimate.clipMinutes, 11.9948);
assert.equal(preparation.costEstimate.expectedCostUsd, 0.072);
...
assert.equal(manifest.status, "frozen-four-paid-known-speaker-diarizations-authorized");
...
if (!(await exists(executionPath))) {
  ...
  console.log(JSON.stringify({ status: "passed-frozen", calls: 4, expectedCostUsd: 0.072, retries: 0, scoresDerived: 0 }, null, 2));
  process.exit(0);
}
...
if (analysis.gate.passed) {
  assert.equal(analysis.status, "passed-all-four-v2.2.3-confidence-moves-audio-verified");
  ...
  assert.equal(analysis.status, "v2.2.3-audio-verification-unresolved");
