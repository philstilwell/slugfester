...
assert(activation.schemaVersion === '1.0-assessment-production-post-canary-batch-07-audio-validation-correction-2-execution-activation', 'activation schema changed');
...
assert(!(await exists(activation.outputs.execution)), `future output already exists: ${activation.outputs.execution}`);
for (const output of [activation.outputs.audit, activation.outputs.analysis, activation.outputs.cost]) {
  assert((await hashFile(output)) === activation.sourceHashes[output], `preserved correction-1 output changed: ${output}`);
}

if (checkOnly) {
  assert(['frozen-batch-07-audio-validation-correction-2-prepared-not-active','frozen-batch-07-audio-validation-correction-2-authorized-under-standing-authorization'].includes(activation.status), 'activation status invalid');
  console.log('Batch 7 audio validation correction-2 runner preflight passed.');
  process.exit(0);
}
assert(activation.status === 'frozen-batch-07-audio-validation-correction-2-authorized-under-standing-authorization', 'correction is not activated');
...
try {
  for (const [callIndex, call] of request.calls.entries()) {
    ...
    if (call.moveId === activation.target.moveId) {
      const emptyOverlay = activation.correction.emptySegmentOverlay;
      const originalTopLevel = Object.fromEntries(Object.entries(transcript).filter(([key]) => key !== 'segments'));
      assert(transcript.segments.length === emptyOverlay.originalSegmentCount, 'target segment count changed');
      const frozenIndexes = emptyOverlay.omittedSegments.map((item) => item.index);
      assert(JSON.stringify(frozenIndexes) === JSON.stringify([10]), 'omission indexes changed');
      for (const frozen of emptyOverlay.omittedSegments) {
      ...
      assert(overlay.segments.length === emptyOverlay.transientSegmentCount, 'overlay segment count changed');
      ...
      validationOverlay = { operation: 'preserve-correction-1-empty-segment-overlay', omittedSegmentIndexes: frozenIndexes, originalSegmentCount: emptyOverlay.originalSegmentCount, validationSegmentCount: emptyOverlay.transientSegmentCount, persistentTranscriptWritten: false };
    }
    const referenceOverlay = activation.correction.referenceOverlays.find((item) => item.callIndex === callIndex);
    if (referenceOverlay) {
      assert(referenceOverlay.debateNumber === call.debateNumber && referenceOverlay.moveId === call.moveId && referenceOverlay.expectedSpeaker === call.expectedSpeaker, `${call.moveId}: reference overlay identity changed`);
      assert(call.verificationExcerpt.includes(referenceOverlay.replacement), `${call.moveId}: reference overlay left frozen excerpt`);
      assert(referenceOverlay.replacement.trim().split(/\s+/).length === referenceOverlay.tokenCount, `${call.moveId}: reference overlay token count changed`);
      referenceOverlayApplications += 1;
    }
    const verificationExcerpt = referenceOverlay?.replacement ?? call.verificationExcerpt;
    const deterministicEvidence = evaluateAttributionTranscript(transcript, { moveId: call.moveId, expectedSpeaker: call.expectedSpeaker, verificationExcerpt }, activation.exactThresholds);
    moves.push({
      ...
      validationOverlay,
      verificationReferenceOverlay: referenceOverlay ? { originalExcerptPreserved: true, replacement: referenceOverlay.replacement, replacementTokenCount: referenceOverlay.tokenCount, persistentManifestWrite: false } : null,
      deterministicEvidence,
    });
  }
  assert(overlayApplications === 1, 'exactly one transcript overlay required');
  assert(referenceOverlayApplications === 2, 'exactly two reference overlays required');
...
const status = passed ? 'passed-all-five-batch-07-audio-attributions-after-recursive-reference-overlay' : replayComplete ? 'batch-07-audio-verification-unresolved-after-recursive-reference-overlay' : 'batch-07-audio-validation-correction-2-failed';
...
const totals = {
  ...
  retries: 0, corrections: 2, deterministicReplayPasses: replayComplete ? 1 : 0, clipMinutes: request.costEstimate.clipMinutes,
...
const executionRecord = {
  schemaVersion: '1.0-assessment-production-post-canary-batch-07-audio-validation-correction-2-execution', protocolId: request.protocolId, status,
  startedAt, completedAt: new Date().toISOString(), elapsedMs: Date.now() - start, productionCanary: false, batchNumber: 7, correctionNumber: 2,
  deterministicPassesAttempted: 1, deterministicPassesCompleted: replayComplete ? 1 : 0, retries: 0, reruns: 0, automaticRepairs: 0,
  completedValidations: moves.length, requiredValidations: 5, verified, unresolved: replayComplete ? unresolved : null, overlayApplications, referenceOverlayApplications,
...
if (passed) {
  const audit = {
    ...
    correction: { number: 2, operation: activation.correction.name, targetMoveId: activation.target.moveId, omittedSegmentIndexes: [10], transientOverlayApplications: 1, referenceOverlayApplications: 2, referenceOverlayMoveIds: activation.correction.referenceOverlays.map((item) => item.moveId), persistentTranscriptWrites: 0, allOriginalTranscriptsUnchanged: true },
  ...
  const analysis = {
    ...
    gate: { passed, executionComplete: true, deterministicReplayComplete: true, requiredMoves: 5, verified, unresolved, deterministicThresholdsApplied: true, exactValidatorPreserved: true, originalTranscriptsPreserved: true, transientCorrectionOverlayApplications: 1, transientReferenceOverlayApplications: 2, persistentTranscriptWrites: 0 },
...
console.log(JSON.stringify({ status, replayComplete, verified, unresolved: replayComplete ? unresolved : null, overlayApplications, referenceOverlayApplications, persistentTranscriptWrites: 0, modelOrApiCalls: 0, paidServiceCalls: 0, directIncrementalCostUsd: 0 }, null, 2));
