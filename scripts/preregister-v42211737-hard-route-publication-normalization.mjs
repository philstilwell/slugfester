...
const manifestPath = `${V42211737_ROOT}/execution-manifest.json`;
const executionPath = `${V42211737_ROOT}/model-execution.json`;
const analysisPath = `${V42211737_ROOT}/analysis.json`;
...
assertV4(preparation.status === "prepared-five-isolated-hard-route-publication-normalization-contexts" && preparation.authorization.executionManifest && preparation.totals.modelContextsExecuted === 0 && preparation.totals.modelAuthoredScores === 0, "publication normalization execution manifest is not authorized");
assertV4(preparation.totals.maximumCopiedInputBytes <= 400000, "publication context exceeds frozen transport budget");
assertV4(preparation.normalization.v17_36ReplayPassed && preparation.normalization.v17_36ReplayTransformations === 1 && preparation.normalization.rawAndNormalizedOutputsSeparated && preparation.normalization.allOtherFieldsImmutable && !preparation.normalization.priorGateTreatedAsRetry, "publication normalization boundary mismatch");
...
const sourceFiles = [
  "docs/assessment-workflow-v4.2.21.17.37.md",
  ...
  `${V42211737_ROOT}/preparation-manifest.json`,
  ...
  "docs/calibration/v4.2.21.17.36/hard-route-publication-integrity/model-execution.json",
  "docs/calibration/v4.2.21.17.36/hard-route-publication-integrity/analysis.json",
  "docs/calibration/v4.2.21.17.36/hard-route-publication-integrity/outputs/debate-51.json",
  ...
  "scripts/prepare-v42211737-hard-route-publication-normalization.mjs",
  "scripts/preregister-v42211737-hard-route-publication-normalization.mjs",
  "scripts/run-v42211737-hard-route-publication-normalization.mjs",
  "scripts/analyze-v42211737-hard-route-publication-normalization.mjs",
  "scripts/test-v42211737-hard-route-publication-normalization-gate.mjs"
];
...
const manifest = {
  schemaVersion: "4.2.21.17.37-hard-route-publication-normalization-execution-manifest",
  ...
  deterministicValidation: { quoteGenerationTargetWords: preparation.normalization.representativeQuoteGenerationTargetWords, repositoryQuoteAcceptanceWords: preparation.normalization.repositoryQuoteAcceptanceWords, overlongExactQuoteOperation: preparation.normalization.overlongExactQuoteOperation, rawAndNormalizedOutputsSeparated: true, allNonQuoteFieldsImmutableDuringNormalization: true, modelCritiqueTargetWords: [112, 122], repositoryCritiqueAcceptanceWords: [105, 130], structuredCritiqueCharacterMinimum: 880, structuredCritiqueCharacterMaximum: null, terminalPunctuationRequired: true, unexpectedCJKAndHangulRejected: true, modelSummaryTargetWords: [18, 28], repositorySummaryAcceptanceWords: [8, 35], openAIStructuredOutputSubsetPreflight: true, repositoryUniquenessValidationRetained: true, everyLockedMoveAuthoredExactlyOnce: true, exactQuoteSubstringRequired: true, critiqueWordAndSentenceContract: true, localReferenceCatalogOnly: true, overallCommentaryMinimums: true, aiExtensionNoveltyMapComplete: true, introducedArgumentPerSideRequired: true, exactAccordionDisplayContract: true, prohibitedLanguageAbsent: true, modelAuthoredScores: 0 },
  priorGate: { protocolId: "v4.2.21.17.36-hard-route-publication-integrity", status: "failed-one-word-quote-boundary", treatedAsRetry: false },
  ...
  artifacts: { execution: executionPath, analysis: analysisPath, rawOutputs: preparation.contexts.map((context) => context.rawOutput), outputs: preparation.contexts.map((context) => context.output) },
  futureOutputPathsExcludedFromSourceHashes: [...preparation.contexts.flatMap((context) => [context.rawOutput, context.output]), executionPath, analysisPath],
  sourceHashes
};
