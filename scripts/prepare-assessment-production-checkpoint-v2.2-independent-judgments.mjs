...
const ROOT = "docs/assessment-production/production-checkpoint-v2.2-1/independent-judgments";
const PROTOCOL_ID =
  "assessment-production-checkpoint-v2.2-1-independent-judgments";
const INVENTORY_ANALYSIS = "docs/assessment-production/production-checkpoint-v2.2-1/inventory-chronology-fallback/inventory-analysis.json";
const INVENTORY_PREPARATION = "docs/assessment-production/production-checkpoint-v2.2-1/inventory-chronology-fallback/preparation-manifest.json";
const SOURCE_PREPARATION = "docs/assessment-production/production-checkpoint-v2.2-1/source-preparation/preparation-manifest.json";
...
const SOURCE_FILES = [
  ...
  "scripts/prepare-assessment-production-checkpoint-v2.2-independent-judgments.mjs",
  "scripts/test-assessment-production-checkpoint-v2.2-independent-judgment-preparation.mjs"
];
...
assertV4(
  inventoryAnalysis.status ===
    ...
    inventoryAnalysis.gateDisposition
      ?.failedProductionCanaryV1PreservedFailed === true &&
    inventoryAnalysis.gateDisposition
      ?.failedProductionCanaryV1OutputsUsedAsModelInput === false &&
    inventoryAnalysis.gateDisposition?.priorValidationCohortsReclassified ===
      false &&
    ...
    inventoryAnalysis.totals?.moves === 188 &&
  ...
  "the inventory gate does not authorize production checkpoint v2.2 independent-judgment packet preparation"
);
assertV4(
  ...
  "the production checkpoint v2.2 inventory preparation is unavailable"
);
assertV4(
  ...
  "the production checkpoint v2.2 source preparation is unavailable"
);
assertV4(
...
assertV4(
  cohortSelection.status ===
    ...
    cohortSelection.productionCanary === true &&
  ...
  "the frozen production checkpoint v2.2 cohort boundary changed"
);
assertV4(
...
assertV4(
  contexts.reduce((sum, context) => sum + context.moves, 0) === 376,
...
const preparation = {
  schemaVersion:
    "1.0-production-checkpoint-v2.2-independent-judgment-preparation",
  ...
  stagingOnly: true,
  AIOnly: true,
  gateDisposition: structuredClone(inventoryAnalysis.gateDisposition),
  ...
  nextAuthorizedAction:
    "prepare-production-checkpoint-v2.2-independent-judgment-execution-manifest-model-free-only",
};
