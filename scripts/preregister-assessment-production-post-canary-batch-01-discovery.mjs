...
const BATCH_ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-01";
...
const PROTOCOL_ID = "assessment-production-post-canary-batch-01-discovery";
const WORKFLOW = "docs/assessment-production-canary-discovery-workflow.md";
const SCRIPT = "scripts/preregister-assessment-production-post-canary-batch-01-discovery.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-01-discovery-manifest.mjs";
const CODEX_PATH = "/Applications/ChatGPT.app/Contents/Resources/codex";
const REQUIRED_ORDER = ["31", "94", "52", "146", "91", "175", "75", "72", "13", "195"];
...
assertV4(
  preparation.status === "post-canary-batch-01-ten-complete-score-blind-source-packets-prepared-awaiting-validation" &&
    preparationValidation.status === "post-canary-batch-01-score-blind-source-packet-validation-passed-frozen-awaiting-user-approval-before-any-model-execution" &&
    ...
    preparation.totals.maximumCopiedInputBytes === 82445 &&
  ...
  "validated Batch 1 source preparation is not frozen at the required authorization boundary"
);
assertV4(
  JSON.stringify(preparation.contexts.map((debate) => debate.debateNumber)) === JSON.stringify(REQUIRED_ORDER),
  "Batch 1 debate order drifted"
);
assertV4(
...
assertV4(contexts.length === 31, "Batch 1 discovery must contain exactly 31 contexts");
...
assertV4(maximumContext.debateNumber === "13" && maximumContext.chunkId === "chunk-001", "maximum copied-input context drifted");
...
const manifest = {
  ...
  status: shouldWrite ? "frozen-thirty-one-post-canary-batch-01-discovery-contexts-prepared-not-authorized" : "preview",
  ...
  AIOnly: true,
  userAuthorization: {
    scope: "prepare, validate, freeze, commit, and push the Batch 1 discovery-execution manifest only",
  ...
  costEstimate: {
    ...
    expectedAggregateModelMinutes: [54, 95],
    expectedAggregateComputeHours: [0.9, 1.58],
    absoluteGateTimeoutMinutes: 120,
    estimateBasis: "Context-count scaling from the promoted 36-context production canary discovery estimate, rounded outward; Batch 1 has 31 contexts and a nearly identical maximum copied-input size. Subscription use has no direct incremental API charge.",
  },
  ...
  nextAuthorizedAction: "user-approval-required-before-batch-01-discovery-execution-activation-or-any-model-execution",
};
