...
const ROOT =
  "docs/assessment-production/score-stability-v2.1.1-validation-cohort/source-preparation";
...
assert.equal(preparation.discoveryProtocolId, V211_DISCOVERY_PROTOCOL_ID);
...
for (const field of [
  ...
  "tokenMinimumSchemas",
  "modelEndEventProhibitingSchemas",
...
assert.equal(preparation.failedGateDisposition.v21AcceptedAsPassed, false);
...
for (const context of preparation.contexts) {
  ...
  assert.equal(packet.modelInputBoundary.modelAuthoredEndEventProhibited, true);
  assert.equal(
  ...
  for (const chunk of context.chunks) {
    ...
    const rebuiltSchema = makeV211DiscoverySchema({
      packet,
      chunk,
      eventsDocument,
