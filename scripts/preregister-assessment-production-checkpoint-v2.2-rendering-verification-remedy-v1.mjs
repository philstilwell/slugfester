...
const preparationPath = `${CHECKPOINT_V22_RENDERING_REMEDY_V1_ROOT}/preparation-manifest.json`;
...
assertV4(
  preparation.status ===
    ...
    preparation.protocolId === CHECKPOINT_V22_RENDERING_REMEDY_V1_PROTOCOL_ID &&
    ...
    preparation.syntheticPreflight.status ===
      "passed-synthetic-browser-runner-preflight" &&
    preparation.authorization.replacementRenderingVerification === false &&
    preparation.authorization.productionMutation === false,
  "frozen replacement rendering preparation changed"
);
...
for (const item of preparation.packets) {
  ...
  assertV4(
    ...
    `${item.debateNumber}: replacement packet hash changed`
  );
  validateCheckpointV22RenderingRemedyV1Packet(JSON.parse(packetBytes));
}
...
const activation = {
  schemaVersion:
    "1.0-production-checkpoint-v2.2-rendering-remedy-v1-execution-activation",
  protocolId: CHECKPOINT_V22_RENDERING_REMEDY_V1_PROTOCOL_ID,
  status: "replacement-rendering-verification-execution-authorized-and-frozen",
  frozenAt,
  ...
  sourceHashes: {
    ...
    ...Object.fromEntries(preparation.packets.map((item) => [item.path, item.sha256]))
  },
  authorization: {
    browserControl: true,
    screenshotCapture: true,
    replacementRenderingVerification: true,
...
if (shouldWrite) {
  await writeFile(path.resolve(activationPath), `${JSON.stringify(activation, null, 2)}\n`);
}
console.log(JSON.stringify({
  status: shouldWrite
    ? activation.status
    : "replacement-rendering-verification-activation-preview",
  ...
  modelContexts: 0,
  directCostUsd: 0,
  replacementRenderingVerificationAuthorized: true,
  renderingRepair: false,
  retry: false,
  productionMutation: false
}, null, 2));
