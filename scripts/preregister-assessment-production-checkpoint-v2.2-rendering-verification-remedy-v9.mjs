...
assertV4(
  preparation.protocolId === CHECKPOINT_V22_RENDERING_REMEDY_V9_PROTOCOL_ID &&
    ...
    preparation.authorization.remedyV9PlanPreparation === true &&
  ...
  "frozen remedy-v9 preparation required"
);
execFileSync(
  process.execPath,
...
for (const [file, digest] of Object.entries(preparation.sourceHashes)) {
  assertV4(
    ...
    `source hash mismatch before remedy-v9 activation: ${file}`
  );
}
for (const row of preparation.packets) {
  ...
  assertV4(
    ...
    `${row.debateNumber}: remedy-v9 packet hash mismatch`
  );
  validateCheckpointV22RenderingRemedyV9Packet(JSON.parse(bytes));
}
...
const activation = {
  schemaVersion:
    "1.0-production-checkpoint-v2.2-rendering-remedy-v9-execution-activation",
  protocolId: CHECKPOINT_V22_RENDERING_REMEDY_V9_PROTOCOL_ID,
  status:
  ...
  nextAuthorizedAction:
    "single-remedy-v9-rendering-execution-under-frozen-stop-rules"
};

if (shouldWrite) {
  assertV4(!(await exists(activationPath)), "remedy-v9 activation already exists");
