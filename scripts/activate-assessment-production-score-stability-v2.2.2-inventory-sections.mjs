...
const PREPARATION_MANIFEST = `${ROOT}/section-execution-preparation-manifest.json`;
...
const SCRIPT =
  "scripts/activate-assessment-production-score-stability-v2.2.2-inventory-sections.mjs";
const RUNNER =
  "scripts/run-assessment-production-score-stability-v2.2.2-inventory-sections.mjs";
const ANALYZER =
  "scripts/analyze-assessment-production-score-stability-v2.2.2-inventory-sections.mjs";
const TEST =
  "scripts/test-assessment-production-score-stability-v2.2.2-inventory-section-activation.mjs";
...
assertV4(
  preparation.status ===
    ...
    preparation.executionPolicy?.immutableRouteHashRequired === true &&
    preparation.executionPolicy?.allTenSectionsMustPassBeforePlanComposition ===
      true &&
    preparation.authorization?.executionActivationPreparation === true &&
    preparation.authorization?.sectionModelContexts === false &&
    preparation.authorization?.planComposition === false &&
  ...
  "section execution activation is not authorized"
);
assertV4(
  sourcePreparation.status ===
      "ten-exact-v2.2.2-section-packets-frozen-not-authorized" &&
    sourcePreparation.contexts?.length === 10 &&
    sha256(sourcePreparationBytes) === preparation.sectionPacketPreparationSha256,
  "section packet preparation drifted"
);
...
const sourceFiles = [
  ...
  ...sourcePreparation.contexts.flatMap((context) => [
    context.sectionPacket,
    context.sectionSchema,
    context.routeOutput,
...
const activation = {
  ...
  status: "frozen-ten-v2.2.2-section-contexts-authorized",
  activatedAt,
  ...
  nextRequiredAction: "execute-frozen-v2.2.2-section-gate-once",
};
  ...
  JSON.stringify(
    {
      ...
      sectionModelContextsAuthorized: true,
