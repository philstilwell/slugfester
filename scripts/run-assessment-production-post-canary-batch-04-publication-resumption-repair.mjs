...
assertV4(
  activation.status ===
      "frozen-eleven-isolated-twenty-two-field-batch-04-debate-49-publication-resumption-repair-contexts-authorized-under-standing-authorization" &&
    activation.batchNumber === 4 && activation.contexts?.length === 11 &&
    ...
    activation.authorization?.remainingSixContextExecution === false &&
  ...
  "the Debate 49 repair execution is not authorized"
);
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(),
    `batch-04-debate-49-resumption-repair-${context.contextIndex}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(),
    `batch-04-debate-49-resumption-repair-home-${context.contextIndex}-`));
  ...
  try {
    ...
    const prompt = [
      ...
      `Act only as isolated Debate 49 bounded publication repair editor ${context.contextIndex}.`,
      ...
      "For each critique, write exactly four complete ordered labeled sentences, target 112–118 words, remain within 105–130 words, preferably exceed 900 characters and never fall below 880 characters, and end every sentence with terminal punctuation. Count the words before returning.",
    ...
    process.stdout.write(`[batch-04-debate-49-publication-resumption-repair] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort}\n`);
    ...
    const base = {
      ...
      debateNumber: "49", debateId: context.debateId,
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 ||
      ...
      try {
        validationSummary = validateDebate49RepairOutput(
      ...
      const validation = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-04-debate-49-publication-resumption-repair-validation",
        protocolId: activation.protocolId, status: accepted ? "passed" : "failed",
        debateNumber: "49", packetIndex: context.packetIndex,
        repairOutputSha256: sha256(resultBytes), validationSummary, validationMessage,
        modelAuthoredScores: 0
      };
      const provenance = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-04-debate-49-publication-resumption-repair-provenance",
        protocolId: activation.protocolId, debateNumber: "49",
        packetIndex: context.packetIndex, model: activation.model,
        ...
        participantJudgmentWasScoreBlind: true, scoresImmutable: true,
    ...
    record = { contextIndex: context.contextIndex, packetIndex: context.packetIndex,
      debateNumber: "49", debateId: context.debateId, model: "5.6 Sol",
      ...
      participantJudgmentWasScoreBlind: true, scoresImmutable: true,
      writableFields: context.writableFields, meteredApiCostUsd: 0,
      paidServiceCallsThisStage: 0, modelAuthoredScores: 0,
  ...
  process.stdout.write(`[batch-04-debate-49-publication-resumption-repair] index ${context.contextIndex} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}

async function runPool(contexts, maximumConcurrency) {
  const results = []; let next = 0;
  async function worker() { while (next < contexts.length) {
    const index = next; next += 1; results[index] = await runContext(contexts[index]);
  } }
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-debate-49-publication-resumption-repair-model-execution",
  protocolId: activation.protocolId,
  status: validContexts === 11 ?
    "batch-04-debate-49-eleven-context-publication-resumption-repair-gate-passed" :
    "batch-04-debate-49-publication-resumption-repair-gate-complete-with-failure",
  gateStartedAt, gateCompletedAt: new Date().toISOString(), contextsPlanned: 11,
  ...
  rampPhases: phaseRecords, results, originalFailedOutputPreserved: true,
  participantJudgmentWasScoreBlind: true, scoresImmutable: true,
  meteredApiCostUsd: 0, paidServiceCallsThisStage: 0,
  modelAuthoredScores: 0, scorePassesExecutedThisStage: 0,
  authorization: { deterministicAnalysis: true,
    deterministicMergeAndCompleteValidation: validContexts === 11,
...
console.log(JSON.stringify({ status: execution.status,
  ...
  elapsedMinutes: Number((wallElapsedMs / 60000).toFixed(2)), attempts: execution.attempts,
  retries: 0, timeoutExtensions: 0, recursiveCorrectionContexts: 0,
