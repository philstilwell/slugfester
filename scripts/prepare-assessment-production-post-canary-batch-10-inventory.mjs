...
const SCRIPT =
  "scripts/prepare-assessment-production-post-canary-batch-10-inventory.mjs";
const PROTOCOL_ID =
  "assessment-production-post-canary-batch-10-candidate-sharded-inventory";
const SOURCE_PACKET_VERSION =
  "1.0-assessment-production-post-canary-batch-10-candidate-sharded-inventory-source-packet";
...
const EXPECTED = {
  ...
  proCandidates: 158,
  conCandidates: 170,
  belowHighAttributionCandidates: 2,
};
...
function buildInventorySourcePacket(sourcePacket) {
  assertV4(
    sourcePacket?.schemaVersion ===
      "1.0-assessment-production-post-canary-batch-10-score-blind-source-packet" &&
      sourcePacket.protocolId ===
        "assessment-production-post-canary-batch-10-source-preparation" &&
    ...
    "Batch 10 source packet is unavailable or not dyadic"
  );
  return {
...
function makePlanPacket({
  ...
  return {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-10-candidate-census-plan-packet",
    protocolId: PROTOCOL_ID,
...
async function buildFrozenArtifacts(frozenAt) {
  ...
  assertV4(
    ...
    "Batch 10 standing authorization does not cover exact inventory preparation and execution"
  );

  assertV4(
    discoveryAnalysis.status ===
      "post-canary-batch-10-discovery-passed-standing-authorization-active-for-inventory-preparation" &&
      ...
      discoveryAnalysis.nextAuthorizedAction ===
        "prepare-freeze-and-activate-batch-10-candidate-census-planner-contexts-under-standing-authorization",
    "passed Batch 10 discovery boundary drifted"
  );
  assertV4(
    discoveryExecution.status ===
      "thirty-six-post-canary-batch-10-discovery-contexts-passed" &&
    ...
    "Batch 10 discovery execution drifted"
  );
  assertV4(
    ...
    "Batch 10 source preparation or model boundary drifted"
  );
  assertV4(
    ...
    "Batch 10 source validation drifted"
  );
  assertV4(
    selection.status ===
    ...
    "Batch 10 frozen selection drifted"
  );
  assertV4(
  ...
  for (const discovered of discoveryAnalysis.debates) {
    ...
    assertV4(
      ...
      `${discovered.debateNumber}: Batch 10 identity drifted`
    );
    const [
  ...
  assertV4(
    ...
    "Batch 10 totals or packet state drifted"
  );
  ...
  const preparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-10-candidate-sharded-inventory-preparation",
    protocolId: PROTOCOL_ID,
    ...
    userAuthorization: {
      scope:
        "prepare, freeze, activate, and execute the Batch 10 candidate-census and candidate-sharded side-selection workflow under the frozen standing authorization",
    ...
    nextAuthorizedAction:
      "freeze-and-activate-batch-10-candidate-census-plan-execution-under-standing-authorization",
  };
  ...
  const executionPreparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-10-candidate-census-plan-execution-preparation-manifest",
    protocolId: PROTOCOL_ID,
    status:
    ...
    costEstimate: {
      ...
      estimateBasis:
        `The promoted ten-context candidate-census planner used this contract at concurrency two. Batch 10 freezes the same ten-context shape with an observed maximum of ${maximumCopiedInputBytes.toLocaleString("en-US")} copied bytes, below the proven ${V212_CANDIDATE_SHARDED_INVENTORY.maximumCopiedInputBytes.toLocaleString("en-US")}-byte ceiling. ChatGPT-subscription execution has no direct incremental API charge.`,
    },
  ...
  const validation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-10-candidate-sharded-inventory-preparation-validation",
    protocolId: PROTOCOL_ID,
    status:
      "batch-10-candidate-sharded-inventory-packets-and-plan-execution-manifest-validation-passed-frozen-standing-authorization-active",
...
async function validateFrozen() {
  ...
  assertV4(
    ...
    "frozen Batch 10 inventory preparation is invalid"
  );
...
if (command === "freeze") await freeze();
  ...
  throw new Error(
    "usage: prepare-assessment-production-post-canary-batch-10-inventory.mjs <freeze|validate>"
  );
}
