...
const scoreArtifactCandidates = [
  `${POST_CANARY_BATCH_10_FINAL_LEDGER_ROOT}/calculated-scores.json`,
  `${POST_CANARY_BATCH_10_FINAL_LEDGER_ROOT}/score-analysis.json`,
  `${POST_CANARY_BATCH_10_FINAL_LEDGER_ROOT}/score-pass-manifest.json`
];
...
const rebuiltLedger = buildPostCanaryBatch10FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
...
assert.deepEqual(
  ...
  ["21", "74", "107", "142", "123", "177", "68", "147", "61", "130"]
);
assert.equal(storedLedger.audit.finalMoves, 182);
assert.equal(storedLedger.audit.disputedMoves, 177);
assert.equal(storedLedger.audit.pairSelections, 200);
assert.equal(storedLedger.audit.scoringFieldSelections, 265);
assert.equal(storedLedger.audit.burdenAdjustmentSelections, 18);
assert.equal(storedLedger.audit.candidateSelections, 483);
assert.equal(storedLedger.audit.meanMerges, 381);
assert.equal(storedLedger.audit.dependencyMeanMergesSuppressed, 0);
assert.equal(storedLedger.audit.audioVerifiedMoves, 9);
...
assert.throws(() =>
  buildPostCanaryBatch10FinalLedger(mutatedInputs, inputs.sourceHashes)
);
...
assert.equal(analysis.totals.finalMoves, 182);
assert.equal(analysis.totals.disputedMoves, 177);
assert.equal(analysis.totals.candidateSelections, 483);
