...
} from "./lib/assessment-production-post-canary-batch-13-debate-70-publication-field-disjoint-repair-1.mjs";
...
const RESUMPTION_ROOT = `${PUBLICATION_ROOT}/original-unattempted-context-resumption-2`;
...
const FAILURE_ANALYSIS = `${RESUMPTION_ROOT}/debate-70-failure-analysis.json`;
...
assertV4(
  originalExecution.status === "six-context-publication-resumption-stopped-with-failure" &&
    originalExecution.contextsAttempted === 1 && originalExecution.contextsUnattempted === 5 &&
    ...
    originalAnalysis.status === "six-context-publication-resumption-failed" &&
    failureAnalysis.status === "debate-70-publication-failure-diagnosed-awaiting-field-disjoint-repair-1" &&
    failureAnalysis.diagnosis?.invalidCritiqueCount === 5 &&
    failureAnalysis.diagnosis?.invalidQuoteCount === 0 &&
    failureAnalysis.diagnosis?.allOtherFieldsStructurallyValid === true,
  "Debate 70 publication resumption failure boundary changed"
);
...
const debate70 = allArtifacts.get("70");
const structuralStandIn = Object.values(debate70.outputJson.moveProse)
  .map((entry) => entry.critique)
  .find((critique) => {
    const words = wordCount(critique);
    return words >= 105 && words <= 130 && critique.length >= 880;
  });
assertV4(structuralStandIn, "Debate 70 has no validation-clean diagnostic stand-in");

const expectedCritiqueCounts = { "70": 5 };
const diagnostics = [];
...
assertV4(diagnostics.reduce((sum, item) => sum + item.invalidCritiques.length, 0) === 5 &&
  diagnostics.reduce((sum, item) => sum + item.invalidQuotes.length, 0) === 0,
"diagnosed five-field repair scope changed");
...
assertV4(contexts.length === 3 &&
  contexts.every((context) => context.writableFieldCount >= 1 && context.writableFieldCount <= 2) &&
  contexts.reduce((sum, context) => sum + context.writableFieldCount, 0) === 5 &&
  new Set(contexts.flatMap((context) => context.writableFields)).size === 5,
"three-shard, five-field coverage changed");

const sourceScripts = [
  "scripts/lib/assessment-production-post-canary-batch-13-debate-70-publication-field-disjoint-repair-1.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-13-debate-70-publication-field-disjoint-repair-1.mjs",
  "scripts/activate-assessment-production-post-canary-batch-13-debate-70-publication-field-disjoint-repair-1.mjs",
  "scripts/run-assessment-production-post-canary-batch-13-debate-70-publication-field-disjoint-repair-1.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-13-debate-70-publication-field-disjoint-repair-1.mjs",
...
const manifest = {
  ...
  protocolId: PROTOCOL_ID,
  status: "frozen-three-context-five-field-batch-13-debate-70-publication-repair-prepared-not-activated",
  frozenAt,
  ...
  userAuthorization: {
    ...
    resolvedScope: "three fresh isolated field-disjoint shards correcting exactly five rejected Debate 70 critiques while retaining every validation-clean field",
  ...
  diagnosis: {
    status: "deterministically-confirmed-five-critique-failures",
    debates: diagnostics,
    totals: { debates: 1, invalidCritiques: 5, invalidQuotes: 0, writableFields: 5 },
    everyNonTargetFieldStructurallyValidated: true,
    temporaryStandInsWrittenOrRetained: false
  },
  diagnosticStandIn: {
    source: "validation-clean-critique-from-the-same-preserved-Debate-70-output",
  ...
  retentionContract: {
    ...
    completeTenDebateCohortReplayDeferredUntilFiveUnattemptedContextsComplete: true
  },
  ...
  totals: {
    debates: 1, contexts: 3, critiqueFields: 5, quoteFields: 0, writableFields: 5,
  ...
  nextAuthorizedAction: "commit-and-push-frozen-three-shard-debate-70-repair-then-activate"
};
