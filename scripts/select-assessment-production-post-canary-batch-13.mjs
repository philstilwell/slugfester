...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-13";
...
const PRIOR_SELECTION = "docs/assessment-production/post-canary-continuation-v1/batch-12/selection.json";
const PRIOR_SELECTION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-12/selection-analysis.json";
const PRIOR_MUTATION_MANIFEST = "docs/assessment-production/post-canary-continuation-v1/batch-12/production-publication/mutation-manifest.json";
const PRIOR_COMPLETION_EXECUTION = "docs/assessment-production/post-canary-continuation-v1/batch-12/production-publication/generated-seo-correction/execution.json";
const PRIOR_COMPLETION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-12/production-publication/generated-seo-correction/analysis.json";
const PRIOR_COMPLETION_COMMIT = "8faec58c73e82077648c1caf654b00abfc7f1236";
const SCRIPT = "scripts/select-assessment-production-post-canary-batch-13.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-13-selection.mjs";
...
assertV4(!(await exists(FUTURE_SOURCE_PREPARATION)), "source preparation already exists before Batch 13 selection");
...
assertV4(
  ...
  "Batch 13 selection is not compatible with the frozen continuation policy"
);
...
assertV4(
  priorSelection.status === "twelfth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    priorSelection.protocolId === "assessment-production-post-canary-batch-12" &&
    priorSelection.productionSequenceOrdinal === 13 &&
    priorSelection.selected.length === 10 &&
    priorSelectionAnalysis.status === "twelfth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization" &&
    priorSelectionAnalysis.selection.sha256 === sha256(controlBytes[PRIOR_SELECTION]) &&
    priorMutationManifest.status === "frozen-batch-12-production-publication-mutation-manifest-prepared" &&
    JSON.stringify(priorMutationManifest.explicitOrder) === JSON.stringify(priorSelectedNumbers) &&
    priorCompletionExecution.status === "passed-complete-batch-12-transaction-ready-for-atomic-commit" &&
    ...
    priorCompletionExecution.dependentRecovery === undefined &&
    priorCompletionAnalysis.status === "batch-12-production-publication-accepted-for-atomic-commit-after-generated-seo-correction" &&
    ...
    priorCompletionAnalysis.decision.batch12ProductionPublicationGatePassed === true &&
  ...
  "Batch 12 has not satisfied the frozen successful-publication prerequisite"
);
assertV4(git("rev-parse", "--abbrev-ref", "HEAD") === "main", "Batch 13 selection must run on main");
assertV4(git("rev-parse", "HEAD") === PRIOR_COMPLETION_COMMIT, "Batch 13 selection checkpoint is not the exact Batch 12 publication commit");
assertV4(git("rev-parse", "origin/main") === PRIOR_COMPLETION_COMMIT, "Batch 12 publication commit is not the pushed origin/main checkpoint");
...
assertV4(
  JSON.stringify(priorSelectedNumbers) === JSON.stringify(ranked.slice(110, 120).map((item) => item.debateNumber)),
  "Batch 12 is not the immutable twelfth rank slice"
);

const selected = [];
let totalEvents = 0;
let totalDurationMs = 0;
for (const [offset, rankedItem] of ranked.slice(120, 130).entries()) {
...
assertV4(selected.length === 10 && new Set(selected.map((item) => item.debateNumber)).size === 10, "Batch 13 did not select ten unique debates");

const checkpointCommit = git("rev-parse", "HEAD");
const selection = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-selection",
  protocolId: "assessment-production-post-canary-batch-13",
  status: "thirteenth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed",
  frozenAt,
  checkpointCommit,
  branch: "main",
  ...
  stagingOnly: true,
  batchNumber: 13,
  productionSequenceOrdinal: 14,
  userAuthorization: {
    instruction: "Continue Batch 13 at your discretion.",
    scopeInterpretation: "Materialize and source-validate only the deterministic Batch 13 selection under the frozen continuation policy after authenticating successful Batch 12 publication. This selection pass prepares no source packets, executes no models, derives no scores, and performs no production mutation.",
  ...
  eligibility: {
    ...
    previouslyPublishedAndRemovedCount: 120,
    remainingBeforeBatchCount: ranked.length - 120,
    remainingBeforeBatchRankedSha256: sha256(serializedJson(ranked.slice(120))),
    selectedCount: selected.length,
    remainingUnselectedCount: ranked.length - 130,
    dyadicOnly: true,
  ...
  nextAuthorizedAction: "prepare-batch-13-standing-authorization-and-source-packets-under-user-authorization"
};
...
const analysis = {
  ...
  status: "thirteenth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization",
  analyzedAt: frozenAt,
  ...
  finding: "Successful Batch 12 production publication was authenticated at the pushed atomic commit. The immutable campaign rank then selected ordinals 121 through 130 as ten unique pending dyadic debates, and all thirty effective source files and ten canonical event chains passed without replacement.",
  decision: {
