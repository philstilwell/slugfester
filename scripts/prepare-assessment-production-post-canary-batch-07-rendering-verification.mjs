...
import { POST_CANARY_BATCH_07_RENDERING_IMAGE_CONTRACT,
  POST_CANARY_BATCH_07_RENDERING_MODEL, POST_CANARY_BATCH_07_RENDERING_ORDER,
  POST_CANARY_BATCH_07_RENDERING_PORT, POST_CANARY_BATCH_07_RENDERING_PROTOCOL_ID,
  POST_CANARY_BATCH_07_RENDERING_REQUIRED_BOOLEAN_CHECKS,
  POST_CANARY_BATCH_07_RENDERING_ROOT, POST_CANARY_BATCH_07_RENDERING_VIEWPORTS,
  buildPostCanaryBatch07RenderingPacket } from
...
const FINALIZATION_ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-07/publication-finalization";
...
const PACKETS_ROOT = `${POST_CANARY_BATCH_07_RENDERING_ROOT}/packets`;
const ACTIVATION = `${POST_CANARY_BATCH_07_RENDERING_ROOT}/execution-activation.json`;
const EXECUTION = `${POST_CANARY_BATCH_07_RENDERING_ROOT}/execution.json`;
const ANALYSIS = `${POST_CANARY_BATCH_07_RENDERING_ROOT}/analysis.json`;
const RENDERING_AUDIT = `${POST_CANARY_BATCH_07_RENDERING_ROOT}/rendering-audit.json`;
...
assertV4(finalPreparation.status === "frozen-post-canary-batch-07-publication-finalization-prepared" &&
  finalActivation.status === "frozen-post-canary-batch-07-publication-finalization-authorized" &&
  finalExecution.status === "ten-debate-batch-07-publication-finalization-passed" &&
  finalExecution.deterministicFinalizationPasses === 1 && finalExecution.reruns === 0 &&
  finalAnalysis.status === "ten-debate-batch-07-publication-finalization-passed" &&
  finalAnalysis.gate?.finalCandidatesPassed === 10 && finalAnalysis.gate?.moves === 187 &&
  finalAudit.status === "passed" && finalAudit.rows?.length === 10 &&
  finalAudit.totals?.moves === 187 && finalAudit.totals?.deterministicFinalizationPasses === 1,
"passing Batch 7 finalization required");
...
for (const [name, viewport] of Object.entries(POST_CANARY_BATCH_07_RENDERING_VIEWPORTS)) assertV4(
  canonicalJson(provenNavigation.viewports[name]) === canonicalJson(viewport), `${name}: viewport contract changed`);
const compatibility = { schemaVersion: "1.0-assessment-production-post-canary-batch-07-publication-finalization-compatibility-analysis",
  protocolId: "assessment-production-post-canary-batch-07-publication-finalization",
  status: "batch-07-production-compatibility-boundary-recorded", analyzedAt: frozenAt,
  productionCanary: false, batchNumber: 7, stagingOnly: true,
  findings: [{ id: "batch-07-site-ledger-adapter-and-validator-route",
    description: "The live validator requires a separately frozen authenticated Batch 7 adapter route before production mutation.",
...
previewText = previewText.replace("renderPublicationStagingDebate(await response.json());",
  `renderPublicationStagingDebate(await response.json());\n        const banner = document.querySelector("[data-publication-staging-preview] .calibration-preview-note");\n        if (banner) banner.innerHTML = "<strong>Publication staging preview:</strong> validated post-canary Batch 7 candidate only. This scorecard remains excluded from production data and rankings pending rendering and mutation authorization.";`);
...
const packets = [];
for (const debateNumber of POST_CANARY_BATCH_07_RENDERING_ORDER) {
  ...
  const packet = buildPostCanaryBatch07RenderingPacket({ auditRow,
...
const sources = [FINALIZATION_PREPARATION, FINALIZATION_ACTIVATION, FINALIZATION_EXECUTION,
  ...
  PROVEN_NAVIGATION, PROVEN_PREFLIGHT, POST_CANARY_BATCH_07_STANDING_AUTHORIZATION,
  ...
  "scripts/lib/assessment-production-post-canary-batch-07-rendering-verification.mjs",
  "scripts/lib/assessment-production-post-canary-batch-07-rendering-browser-runner.mjs",
  "scripts/lib/assessment-production-post-canary-batch-07-publication-finalization.mjs",
  "scripts/lib/assessment-production-post-canary-batch-07-standing-authorization.mjs",
  "scripts/lib/v4-lean-production.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-07-rendering-verification.mjs",
  "scripts/test-assessment-production-post-canary-batch-07-rendering-verification-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-07-rendering-verification.mjs",
  "scripts/validate-assessment-production-post-canary-batch-07-rendering-evidence.mjs",
  "scripts/finalize-assessment-production-post-canary-batch-07-rendering-verification.mjs",
...
const futureOutputPathsExcludedFromSourceHashes = [ACTIVATION, EXECUTION, ANALYSIS, RENDERING_AUDIT,
  ...packets.flatMap((row) => ["desktop", "mobile"].flatMap((name) => {
    const root = `${POST_CANARY_BATCH_07_RENDERING_ROOT}/evidence/debate-${row.debateNumber}`;
...
const manifest = { schemaVersion: "1.0-assessment-production-post-canary-batch-07-rendering-verification-preparation",
  protocolId: POST_CANARY_BATCH_07_RENDERING_PROTOCOL_ID,
  status: "frozen-post-canary-batch-07-rendering-verification-prepared",
  frozenAt, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(),
  productionCanary: false, batchNumber: 7, stagingOnly: true,
  userAuthorization: { standingAuthorization: POST_CANARY_BATCH_07_STANDING_AUTHORIZATION,
    standingAuthorizationSha256: standing.sha256, directIncrementalCostUsdMaximum: 0 },
  packets: packets.map(({ serialized, ...row }) => row), explicitOrder: POST_CANARY_BATCH_07_RENDERING_ORDER,
  viewports: POST_CANARY_BATCH_07_RENDERING_VIEWPORTS,
  requiredBooleanChecks: POST_CANARY_BATCH_07_RENDERING_REQUIRED_BOOLEAN_CHECKS,
  browserPlan: { controller: "split-Chromium-fresh-keyboard-location-assign-transport",
    ...
    port: POST_CANARY_BATCH_07_RENDERING_PORT, explicitViewportOrder: ["desktop", "mobile"],
    oneAttemptPerViewport: true, retriesMaximum: 0, timeoutExtensionsMaximum: 0,
    timeoutMsPerViewport: 15000 },
  gateExpectations: { debates: 10, sections: 51, moves: 187, viewportResults: 20,
  ...
  model: { ...POST_CANARY_BATCH_07_RENDERING_MODEL, contextsPlannedThisStage: 0 },
  ...
  artifacts: { preparation: PREPARATION, activation: ACTIVATION, execution: EXECUTION,
    ...
    evidenceRoot: `${POST_CANARY_BATCH_07_RENDERING_ROOT}/evidence`,
  ...
  authorization: { renderingVerificationPreparation: true, executionActivation: true,
    ...
    productionMutation: false, nextBatchSelection: false },
  nextAuthorizedAction: "activate-batch-07-rendering-verification" };
if (shouldWrite) {
...
console.log(JSON.stringify({ status: manifest.status, debates: 10, sections: 51, moves: 187,
