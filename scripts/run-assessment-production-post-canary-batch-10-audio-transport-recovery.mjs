...
if (!shouldExecute) {
  console.log(
    JSON.stringify(
      {
        status: "passed-activated-batch-10-audio-transport-recovery-preflight",
        sources: plan.exactCohort.sourceCount,
        clips: plan.exactCohort.clipCount,
        additionalDebate123DownloadAttempts: 1,
...
const state = {
  attempts: 1,
  debate123AdditionalDownloadCliInvocations: 0,
  debate147DownloadCliInvocations: 0,
  debate130DownloadCliInvocations: 0,
...
const installSource = async (source) => {
  ...
  if (source.mode === "recover-once") {
    assertV4(
      source.sourceVideoId === "8k9A7d2Wnjk" &&
        !(await exists(plan.protectedInvalidEvidence.preservedPath)),
      "Debate 123 recovery preimage changed"
    );
    await rename(
      plan.protectedInvalidEvidence.path,
      plan.protectedInvalidEvidence.preservedPath
    );
    downloadTemplate = path.resolve(repositoryRoot, plan.debate123Recovery.downloadTemplate);
    normalizedTemporaryPath = path.resolve(
      repositoryRoot,
      plan.debate123Recovery.normalizedTemporaryPath
    );
    downloadArgs = [
      ...plan.debate123Recovery.ytDlpArguments,
      "-o", downloadTemplate,
      plan.debate123Recovery.videoUrl
    ];
    state.debate123AdditionalDownloadCliInvocations += 1;
    ...
    if (debateNumber === "147") state.debate147DownloadCliInvocations += 1;
    if (debateNumber === "130") state.debate130DownloadCliInvocations += 1;
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-10-audio-source-transport-recovery-execution",
