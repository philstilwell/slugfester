...
assert.equal(manifest.aggregateExpectations.sections, 50);
...
assert.equal(manifest.preservedControls.debate91IntegerRoundedTiePreserved, true);
...
assert.deepEqual(manifest.compatibilityBoundary.blockers, [
  "batch-01-site-ledger-adapter-and-validator-route"
]);
assert.equal(
...
const preview = buildPostCanaryBatch01PublicationStagingPreviewHtml();
assert.match(preview, /noindex,nofollow/);
assert.match(preview, /validated post-canary Batch 1 candidate only/);
