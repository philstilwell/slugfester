...
const executionPath =
  `${POST_CANARY_BATCH_02_DISPUTE_ADJ_ROOT}/model-execution.json`;
...
assertV4(
  ...
  "Batch 2 adjudication execution is unavailable or crossed its boundary"
);
for (const [file, digest] of Object.entries(activation.sourceHashes)) {
  assertV4(
    sha256(await readFile(file)) === digest,
    `source hash mismatch during Batch 2 adjudication analysis: ${file}`
  );
}
...
for (const context of activation.contexts) {
  ...
  if (result.gateAcceptancePassed) {
    ...
    replay = validatePostCanaryBatch02DisputeAdjudicationOutput(
...
const semanticPass =
  ...
  valid.reduce((sum, context) => sum + context.candidateSelections, 0) ===
    535 &&
  valid.reduce((sum, context) => sum + context.disputedMoves, 0) === 182;
const timingPass =
  semanticPass &&
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-dispute-only-adjudication-analysis",
  ...
  status: passed
    ...
    : semanticPass
      ? timingPass
        ? scoreBlindPass
          ? "post-canary-batch-02-dispute-only-adjudication-gate-failed-isolation"
          : "post-canary-batch-02-dispute-only-adjudication-gate-failed-scoreblindness"
        : "post-canary-batch-02-dispute-only-adjudication-gate-failed-timing"
      : "post-canary-batch-02-dispute-only-adjudication-gate-failed-validation",
  ...
  nextAuthorizedAction: passed
    ? "user-approval-required-before-batch-02-deterministic-final-ledger-assembly"
    : "user-approval-required-before-any-batch-02-adjudication-failure-diagnosis-or-downstream-work"
};
if (shouldWrite) {
