...
assertV4(
  preparation.protocolId === CHECKPOINT_V22_RENDERING_REMEDY_V8_PROTOCOL_ID &&
    preparation.status ===
      "eighth-replacement-rendering-verification-plan-prepared-and-frozen" &&
    preparation.authorization.remedyV8PlanPreparation === true &&
  ...
  "frozen remedy-v8 preparation required"
);
execFileSync(
  process.execPath,
...
for (const [file, digest] of Object.entries(preparation.sourceHashes)) {
  assertV4(
    ...
    `source hash mismatch before remedy-v8 activation: ${file}`
  );
}
for (const row of preparation.packets) {
  ...
  assertV4(
    ...
    `${row.debateNumber}: remedy-v8 packet hash mismatch`
  );
  validateCheckpointV22RenderingRemedyV8Packet(JSON.parse(bytes));
}
...
const activation = {
  schemaVersion:
    "1.0-production-checkpoint-v2.2-rendering-remedy-v8-execution-activation",
  protocolId: CHECKPOINT_V22_RENDERING_REMEDY_V8_PROTOCOL_ID,
  status:
  ...
  nextAuthorizedAction:
    "single-remedy-v8-rendering-execution-under-frozen-stop-rules"
};

if (shouldWrite) {
  assertV4(!(await exists(activationPath)), "remedy-v8 activation already exists");
