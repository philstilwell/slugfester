...
assertV4(
  ...
  "usage: validate-assessment-production-canary-independent-judgment.mjs JUDGMENT PREPARATION DEBATE PASS [--write]"
);
...
assertV4(
  ...
  "production-canary independent-judgment preparation unavailable"
);
const context = preparation.contexts.find(
  (item) => item.debateNumber === debateNumber && item.reviewerPass === reviewerPass
);
...
assertV4(sha256(originalEventsBytes) === context.originalEventsSha256, `${debateNumber}: original event hash drifted`);
assertV4(sha256(sourceLedgerBytes) === context.fullLedgerSha256, `${debateNumber}: source-ledger hash drifted`);
assertV4(sourcePacket.sourceChain.eventsSha256 === context.originalEventsSha256, `${debateNumber}: source-packet event hash drifted`);
assertV4(sourcePacket.transportChain.sourceLedgerSha256 === context.fullLedgerSha256, `${debateNumber}: source-packet ledger hash drifted`);
assertV4(sourceLedgerBytes.toString("utf8").endsWith("\n"), `${debateNumber}: source ledger lacks terminal newline`);

const rows = sourceLedgerBytes.toString("utf8").slice(0, -1).split("\n");
const canonicalEvents = rows.map((line, index) => {
  const row = JSON.parse(line);
  assertV4(Array.isArray(row) && row.length === 4 && row[0] === index, `${debateNumber}: invalid source-ledger row ${index}`);
...
assertV4(canonicalJson(canonicalEvents) === canonicalJson(originalProjection), `${debateNumber}: canonical event projection drifted`);
...
const confidenceMoves = (level) => compiled.rawOutput.moves
...
const summary = {
  schemaVersion: "1.0-production-canary-independent-judgment-validation",
...
if (writeFlag === "--write") {
  for (const output of [context.rawOutput, context.validationOutput, context.provenanceOutput]) {
