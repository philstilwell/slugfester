...
import {
  POST_CANARY_BATCH_09_PRODUCTION_PUBLICATION_ORDER,
...
const paths = {
  manifest: `${POST_CANARY_BATCH_09_PRODUCTION_PUBLICATION_ROOT}/mutation-manifest.json`,
  preparationAnalysis: `${POST_CANARY_BATCH_09_PRODUCTION_PUBLICATION_ROOT}/preparation-analysis.json`,
  activation: `${POST_CANARY_BATCH_09_PRODUCTION_PUBLICATION_ROOT}/execution-activation.json`,
  execution: `${POST_CANARY_BATCH_09_PRODUCTION_PUBLICATION_ROOT}/execution.json`,
  compatibilityRoot: "docs/assessment-production/post-canary-continuation-v1/batch-09/production-compatibility",
  finalizationRoot: "docs/assessment-production/post-canary-continuation-v1/batch-09/publication-finalization/output-bundle/final-candidates",
  ...
  standingAuthorization: "docs/assessment-production/post-canary-continuation-v1/batch-09/standing-authorization.json",
  preparationScript: "scripts/prepare-assessment-production-post-canary-batch-09-production-publication.mjs",
  preparationTest: "scripts/test-assessment-production-post-canary-batch-09-production-publication-preparation.mjs",
  library: "scripts/lib/assessment-production-post-canary-batch-09-production-publication.mjs",
  activationScript: "scripts/activate-assessment-production-post-canary-batch-09-production-publication.mjs",
  activationTest: "scripts/test-assessment-production-post-canary-batch-09-production-publication-activation.mjs",
  executionScript: "scripts/run-assessment-production-post-canary-batch-09-production-publication.mjs"
};
...
assertV4(
  compatibilityAnalysis.status === "batch-09-production-compatibility-passed" &&
    ...
    compatibilityAnalysis.nextAuthorizedAction === "prepare-batch-09-production-publication-mutation-manifest-under-standing-authorization",
  "accepted Batch 9 compatibility analysis required"
);
assertV4(
  compatibilityExecution.status === "batch-09-route-score-checkpoint-batch-01-batch-02-batch-03-batch-04-batch-05-batch-06-batch-07-batch-08-legacy-reference-validation-passed" &&
    compatibilityExecution.regressions?.fullRepositoryPassed === true &&
    compatibilityExecution.totals?.compatibilityPassesCompleted === 1,
  "accepted one-pass Batch 9 compatibility execution required"
);
assertV4(
  compatibilityActivation.status === "post-canary-batch-09-compatibility-execution-authorized-and-frozen" &&
    compatibilityActivation.packetHashes?.length === 10,
  "frozen Batch 9 compatibility activation required"
);
...
assertV4(ledgerInventory.length === 91, "expected 91 existing production ledgers before Batch 9 publication");

const packetLocks = new Map(compatibilityActivation.packetHashes.map((record) => [record.debateNumber, record]));
const debates = [];
for (const debateNumber of POST_CANARY_BATCH_09_PRODUCTION_PUBLICATION_ORDER) {
...
assertV4(validatorLock.sha256 === compatibilityExecution.validator.appliedSha256, "accepted Batch 9 validator changed");
...
const manifest = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-09-production-publication-mutation-manifest",
  protocolId: POST_CANARY_BATCH_09_PRODUCTION_PUBLICATION_PROTOCOL_ID,
  status: "frozen-batch-09-production-publication-mutation-manifest-prepared",
  frozenAt,
  baselineCommit,
  ...
  explicitOrder: [...POST_CANARY_BATCH_09_PRODUCTION_PUBLICATION_ORDER],
  invariants: {
  ...
  executionContract: {
    ...
    requiredOrder: [
      ...
      "confirm all ten production-ledger paths remain absent and all 91 existing ledgers remain byte-identical",
      "publish ten production ledgers byte-for-byte from the staged ledgers",
      "replace exactly ten Batch 9 records in src/data/debates.js with the finalized candidates",
      "keep src/data/references.js byte-identical",
      "run deterministic Batch 9 route and score validation",
  ...
  nextAuthorizedAction: "activate-and-execute-one-frozen-batch-09-production-publication-mutation-pass-under-standing-authorization"
};
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-09-production-publication-preparation-analysis",
  protocolId: POST_CANARY_BATCH_09_PRODUCTION_PUBLICATION_PROTOCOL_ID,
  status: "batch-09-production-publication-mutation-manifest-freeze-passed",
  analyzedAt: frozenAt,
  ...
  checks: {
    ...
    existingProductionLedgersLocked: 91,
...
if (write) {
  await mkdir(resolve(POST_CANARY_BATCH_09_PRODUCTION_PUBLICATION_ROOT), { recursive: true });
