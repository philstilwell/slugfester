...
assertV4(
  ...
  "frozen Batch 4 rendering preparation required"
);
execFileSync(
  process.execPath,
  [
...
for (const [file, digest] of Object.entries(preparation.sourceHashes)) {
  assertV4(
    ...
    `source hash mismatch before Batch 4 rendering activation: ${file}`
  );
}
for (const [file, digest] of Object.entries(preparation.toolHashes)) {
  assertV4(
    sha256(await readFile(path.resolve(file))) === digest,
    `tool hash mismatch before Batch 4 rendering activation: ${file}`
  );
}
for (const row of preparation.packets) {
  ...
  assertV4(
    ...
    `${row.debateNumber}: Batch 4 rendering packet hash mismatch`
  );
  validatePostCanaryBatch04RenderingPacket(JSON.parse(bytes));
}
...
assertV4(
  ...
  "Batch 4 rendering evidence already exists"
);
...
const activation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-rendering-verification-execution-activation",
  protocolId: POST_CANARY_BATCH_04_RENDERING_PROTOCOL_ID,
  status: "frozen-post-canary-batch-04-rendering-verification-authorized",
  authorizedAt,
  activationCommit,
  ...
  nextAuthorizedAction:
    "single-batch-04-rendering-verification-execution-under-standing-authorization-and-frozen-stop-rules"
};

if (shouldWrite) {
  assertV4(!(await exists(ACTIVATION)), "Batch 4 rendering activation already exists");
