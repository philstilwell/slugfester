...
const BATCH_ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-07";
...
const PROTOCOL_ID = "assessment-production-post-canary-batch-07-discovery";
const WORKFLOW = "docs/assessment-production-canary-discovery-workflow.md";
const SCRIPT = "scripts/preregister-assessment-production-post-canary-batch-07-discovery.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-07-discovery-manifest.mjs";
...
assertV4(
  preparation.status === "post-canary-batch-07-ten-complete-score-blind-source-packets-prepared-awaiting-validation" &&
    preparationValidation.status === "post-canary-batch-07-score-blind-source-packet-validation-passed-frozen-under-standing-authorization" &&
    ...
    preparation.totals.discoveryContexts === 35 &&
    preparation.totals.maximumCopiedInputBytes === 82224 &&
  ...
  "validated Batch 7 source preparation is not frozen at the required authorization boundary"
);
assertV4(
  ...
  "Batch 7 standing authorization does not cover exact discovery execution"
);
assertV4(
  JSON.stringify(preparation.contexts.map((debate) => debate.debateNumber)) === JSON.stringify(REQUIRED_ORDER),
  "Batch 7 debate order drifted"
);
assertV4(
  ...
  for (const chunk of debate.chunks) {
    ...
    assertV4(
      sha256(chunkBytes) === chunk.chunkLedgerSha256 &&
        ...
        buildBatch07TokenCountedChunkLedger(chunkBytes).equals(tokenBytes) &&
...
assertV4(contexts.length === 35, "Batch 7 discovery must contain exactly 35 contexts");
...
assertV4(maximumContext.debateNumber === "193" && maximumContext.chunkId === "chunk-001", "maximum copied-input context drifted");
...
const sourceFiles = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-07-source-preparation.mjs",
...
const manifest = {
  ...
  status: shouldWrite ? "frozen-thirty-five-post-canary-batch-07-discovery-contexts-prepared-not-authorized" : "preview",
  ...
  AIOnly: true,
  userAuthorization: {
    scope: "standing-authorized Batch 7 discovery preparation and execution",
  ...
  costEstimate: {
    ...
    estimateBasis: "The exact 35-context Batch 7 input set is hash-locked with an observed maximum of 82,224 copied bytes; no input is truncated or repartitioned. The frozen 1→2→4 scheduler uses the subscription-backed route with a $0 direct incremental cost boundary.",
  },
  ...
  nextAuthorizedAction: "prepare-and-freeze-batch-07-discovery-execution-activation-under-standing-authorization",
};
