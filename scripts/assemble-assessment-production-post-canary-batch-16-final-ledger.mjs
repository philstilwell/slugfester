...
assertV4(
  ...
  "post-canary Batch 16 final-ledger manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen Batch 16 final-ledger source hash mismatch`
  );
}
const inputs = await loadPostCanaryBatch16FinalLedgerInputs();
const ledger = buildPostCanaryBatch16FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
