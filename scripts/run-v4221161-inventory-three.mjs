...
assertV4(manifest.status === "frozen-retired-partition-three-inventory-gate-authorized" && manifest.authorization.modelContexts, "score-blind inventory gate unauthorized");
...
for (const context of manifest.contexts) {
  ...
  let record;
  try {
    for (const [source, target] of [[manifest.modelInputs.manual, "manual.md"], [context.packet, "packet.json"], [context.candidateEvidenceBundle, "candidate-evidence-bundle.json"], [context.schema, "schema.json"]]) await copyFile(source, path.join(temporary, target));
    ...
    const prompt = `Read manual.md, packet.json, candidate-evidence-bundle.json, and schema.json; read nothing else. Act only as the fresh score-blind inventory curator for Debate ${context.debateNumber}. The candidate-evidence bundle is complete and unreduced. Produce four to six weighted issue sections totaling exactly 100%, with one or two pro and one or two con selections in every section and no candidate used twice. Define one route per side and its burden bridges. Author only each selected candidate ID, a unique move ID, a global constructive-or-reply classification, and a source-faithful proposition. A reply must have an earlier selected opposing move in source chronology, but do not name targets. Ratings, response topology, burden contact, scores, winners, Overall Commentary, AI Extension, and publication prose are prohibited. Return exactly one schema-conforming JSON object.`;
    process.stdout.write(`[v4.2.21.16.1-inventory] starting ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber}\n`);
    ...
    if (invocation.timedOut || invocation.code !== 0 || invocation.signal !== null || !resultExists) record = { ...base, status: invocation.timedOut ? "timed-out" : !resultExists ? "result-missing" : "transport-failed", accepted: false, proposalWritten: false };
      ...
      const validation = await run(process.execPath, ["scripts/validate-v422116-inventory.mjs", context.proposalOutput, manifest.preparation, context.debateNumber, "--write"], { cwd: process.cwd(), env: process.env }, 180000);
  ...
  results.push(record);
  process.stdout.write(`[v4.2.21.16.1-inventory] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
}
const validContexts = results.filter((result) => result.accepted).length;
const execution = { schemaVersion: "4.2.21.16.1-score-blind-inventory-model-execution", protocolId: manifest.protocolId, status: validContexts === 3 ? "retired-partition-three-inventory-gate-passed" : "retired-partition-three-inventory-gate-complete-with-failure", contextsPlanned: 3, contextsAttempted: results.length, validContexts, invalidContexts: results.length - validContexts, attempts: results.length, retries: 0, totalElapsedMs: results.reduce((sum, result) => sum + result.elapsedMs, 0), results, meteredApiCostUsd: 0, transcriptionCostUsd: 0, scoresDerived: 0, authorization: { deterministicAnalysis: true, retry: false, semanticCorrection: false, independentJudgmentPreparation: validContexts === 3, independentJudgmentExecution: false, scoreDerivation: false } };
