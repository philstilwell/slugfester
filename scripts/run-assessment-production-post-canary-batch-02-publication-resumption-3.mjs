...
assertV4(
  activation.model?.label === "5.6 Sol" &&
    ...
    activation.executionPolicy?.contexts === 5 &&
...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), `batch-02-publication-resumption-3-${context.debateNumber}-`)
  );
  const codexHome = await mkdtemp(
    path.join(os.tmpdir(), `batch-02-publication-resumption-3-home-${context.debateNumber}-`)
  );
  ...
  try {
    ...
    process.stdout.write(
      `[batch-02-publication-resumption-3] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
    ...
    if (
      ...
      const validationRecord = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-02-publication-resumption-3-validation",
      ...
      const provenanceRecord = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-02-publication-resumption-3-provenance",
        protocolId: activation.protocolId,
        contextIndex: context.contextIndex,
        originalContextIndex: context.originalContextIndex,
        debateNumber: context.debateNumber,
  ...
  process.stdout.write(
    `[batch-02-publication-resumption-3] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-publication-resumption-3-model-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "five-post-canary-batch-02-publication-resumption-3-contexts-passed"
