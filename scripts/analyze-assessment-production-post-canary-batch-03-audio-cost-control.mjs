...
assert(documents.activation.calls.length === 8, "exactly eight calls required");
assert(documents.execution.callsAttempted <= 8, "attempt count exceeds authorization");
...
assert(documents.analysis.gate.requiredMoves === 8, "eight required moves changed");
...
const executionComplete = documents.execution.callsCompleted === 8;
...
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-03-audio-cost-control-analysis",
