...
import {
  POST_CANARY_BATCH_05_DISPUTE_ADJ_PROTOCOL_ID,
  POST_CANARY_BATCH_05_DISPUTE_ADJ_ROOT,
  buildPostCanaryBatch05DisputeAdjudicationPacket,
  makePostCanaryBatch05DisputeAdjudicationSchema
} from "./lib/assessment-production-post-canary-batch-05-dispute-adjudication.mjs";
...
const USER_AUTHORIZATION = Object.freeze({
  instruction:
    "The frozen Batch 5 standing authorization permits automatic preparation, validation, freezing, activation, execution, analysis, committing, and pushing of score-blind dispute-only adjudication checkpoints for exactly the ten selected debates while every frozen gate passes, with subscription-backed and local direct incremental cost capped at $0.",
...
const inputPaths = {
  ...
  schema: `${POST_CANARY_BATCH_05_DISPUTE_ADJ_ROOT}/adjudication.schema.json`
};
const TOOL_SOURCES = [
  "scripts/lib/v4-lean-production.mjs",
  "scripts/lib/v4221175-decomposed-adjudication.mjs",
  "scripts/lib/v42211728-hard-route-adjudication.mjs",
  "scripts/lib/assessment-production-post-canary-batch-05-dispute-adjudication.mjs",
  "scripts/build-assessment-production-post-canary-batch-05-dispute-adjudication-packets.mjs",
  "scripts/test-assessment-production-post-canary-batch-05-dispute-adjudication-packets.mjs",
  "scripts/validate-assessment-production-post-canary-batch-05-dispute-adjudication-output.mjs",
  "scripts/preregister-assessment-production-post-canary-batch-05-dispute-adjudication.mjs",
  "scripts/test-assessment-production-post-canary-batch-05-dispute-adjudication-manifest.mjs"
];
...
const acceptedAudioRoot =
  `${AUDIO_ROOT}/debate-189-timeline-alignment-recovery`;
...
assertV4(
  ...
  "Batch 5 accepted independent-judgment boundary changed"
);
assertV4(
  ...
  "Batch 5 independent-judgment configuration changed"
);
...
assertV4(
  audioAudit.status ===
    ...
    audioAnalysis.gate.originalReferencesPreserved === true &&
    audioCost.status ===
      "additional-usage-derived-cost-within-approved-cap" &&
    audioCost.actualAdditionalUsageDerivedCostUsd === 0.07585 &&
    audioCost.cumulativeUsageDerivedCostUsd <= 1 &&
    audioCost.additionalCostCapPassed === true &&
    audioExecutionManifest.status ===
      "frozen-six-post-canary-batch-05-paid-known-speaker-diarizations-authorized-under-standing-authorization" &&
    audioExecutionManifest.calls.length === 6,
  "Batch 5 accepted audio-verification boundary changed"
);
assertV4(
  ...
  "Batch 5 packet-preparation authorization changed"
);
...
assertV4(audioByDebateMove.size === 6, "Batch 5 verified audio population changed");

const schema = makePostCanaryBatch05DisputeAdjudicationSchema();
...
if (shouldWrite) {
  const immutableOutputs = [
    inputPaths.schema,
    `${POST_CANARY_BATCH_05_DISPUTE_ADJ_ROOT}/preparation-manifest.json`,
    `${POST_CANARY_BATCH_05_DISPUTE_ADJ_ROOT}/execution-preparation-manifest.json`,
    ...EXPECTED_DEBATES.flatMap((debateNumber) => [
      `${POST_CANARY_BATCH_05_DISPUTE_ADJ_ROOT}/packets/debate-${debateNumber}.json`,
      `${POST_CANARY_BATCH_05_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`
    ])
  ];
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const built = buildPostCanaryBatch05DisputeAdjudicationPacket(
  ...
  const provenancePath =
    `${POST_CANARY_BATCH_05_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`;
  const outputPath =
    `${POST_CANARY_BATCH_05_DISPUTE_ADJ_ROOT}/outputs/debate-${debateNumber}.json`;
  const packetBytes = pretty(built.packet);
  const provenanceBytes = pretty({
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-05-adjudication-candidate-provenance",
...
const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-05-dispute-only-adjudication-preparation",
  protocolId: POST_CANARY_BATCH_05_DISPUTE_ADJ_PROTOCOL_ID,
  status: shouldWrite
    ? "prepared-ten-isolated-post-canary-batch-05-dispute-only-adjudication-contexts"
    : "preview",
  frozenAt,
  ...
  acceptedSourceBoundary: {
    ...
    audioValidationOverlaysPreserved: 3,
    audioCorrectionCallsPreserved: 1,
    audioUsageDerivedEstimatedCostUsd:
      audioCost.cumulativeUsageDerivedCostUsd,
  ...
  nextAuthorizedAction:
    "freeze-post-canary-batch-05-dispute-only-adjudication-execution-preparation-manifest-model-free-only"
};
assertV4(
  contexts.length === 10 &&
    disputedMoves === 177 &&
    disputedMoves === sourceAnalysis.adjudicationWorkload.disputedMoves &&
    candidateSelections === 527 &&
    candidateSelections === sourceAnalysis.adjudicationWorkload.candidateSelections &&
    audioVerifiedMoves === 6,
  "Batch 5 adjudication packet workload differs from frozen inputs"
);
assertV4(
  preparation.totals.maximumCopiedInputBytes <= 360000,
  `Batch 5 context ${preparation.totals.maximumCopiedInputBytes} bytes exceeds the 360 KB ceiling`
);
if (shouldWrite) {
  await mkdir(POST_CANARY_BATCH_05_DISPUTE_ADJ_ROOT, { recursive: true });
