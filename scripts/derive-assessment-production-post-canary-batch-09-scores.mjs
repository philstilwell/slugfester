...
import {
  ...
  derivePostCanaryBatch09Scores
} from "./lib/assessment-production-post-canary-batch-09-score-gate.mjs";
import { assertV4 } from "./lib/v4-lean-production.mjs";

const manifestPath = `${POST_CANARY_BATCH_09_SCORE_ROOT}/score-pass-manifest.json`;
...
assertV4(
  ...
  "post-canary Batch 9 score manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen Batch 9 score source hash mismatch`
  );
}
...
const calculated = derivePostCanaryBatch09Scores(
  ledger,
  inputs.debateInputs,
