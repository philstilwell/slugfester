...
import {
  buildBatch05TokenCountedChunkLedger,
  findBatch05ZeroLexicalTokenRows,
} from "./lib/assessment-production-post-canary-batch-05-source-preparation.mjs";
import {
...
const PROTOCOL_ID = "assessment-production-post-canary-batch-05-source-preparation";
const CACHE_VERSION = "post-canary-continuation-v1-batch-05";
...
const SCRIPT = "scripts/prepare-assessment-production-post-canary-batch-05-source-packets.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-05-source-packets.mjs";
...
assertV4(
  selection.status === "fifth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    selectionAnalysis.status === "fifth-post-canary-batch-selection-analysis-passed-awaiting-source-packet-preparation-decision" &&
    selectionAnalysis.selection.sha256 === sha256(selectionBytes) &&
    selection.nextAuthorizedAction === "user-decision-on-fifth-post-canary-batch-source-packet-preparation" &&
    JSON.stringify(selection.selected.map((item) => item.debateNumber)) === JSON.stringify(REQUIRED_ORDER),
  "Batch 5 frozen selection is not ready for its separately authorized source-packet step"
);
assertV4(
  ...
  "Batch 5 model boundary or stop rules drifted"
);
assertV4(
...
for (const selected of selection.selected) {
  ...
  packet.schemaVersion = "1.0-assessment-production-post-canary-batch-05-score-blind-source-packet";
  ...
  for (const chunk of plan.chunks) {
    ...
    const tokenLedgerBytes = buildBatch05TokenCountedChunkLedger(chunkBytes);
    const zeroLexicalTokenRows = findBatch05ZeroLexicalTokenRows(chunkBytes);
...
const preparation = {
  ...
  status: shouldWrite ? "post-canary-batch-05-ten-complete-score-blind-source-packets-prepared-awaiting-validation" : "preview",
  ...
  AIOnly: true,
  userAuthorization: {
    scope: "prepare, validate, freeze, commit, and push score-blind source packets for Batch 5 only",
  ...
  nextAuthorizedAction: "run-batch-05-deterministic-source-packet-validation-model-free-only",
};
