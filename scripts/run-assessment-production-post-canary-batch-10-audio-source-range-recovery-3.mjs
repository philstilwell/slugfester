...
if (!shouldExecute) {
  console.log(
    JSON.stringify(
      {
        status: "passed-activated-batch-10-audio-source-range-recovery-3-preflight",
...
const installSource = async (source) => {
  ...
  const rangeSource = plan.rangeVerifiedSources.find(
    (item) => item.sourceVideoId === source.sourceVideoId
  );
  assertV4(
  ...
  normalizedTemporaryPath = path.join(
    audioDirectory,
    "source.range-format139.normalized.mp3"
  );
  const rawSourcePath = path.join(audioDirectory, "source.range-format139.m4a");
  assertV4(
  ...
  try {
    for (
      ...
      const partPath = path.join(
        audioDirectory,
        `source.range-format139.part-${String(index).padStart(3, "0")}.m4a`
      );
      const headersPath = path.join(
        audioDirectory,
        `source.range-format139.part-${String(index).padStart(3, "0")}.headers.txt`
      );
      assertV4(
      ...
      assertV4(
        partBytes.length === rangeEnd - rangeStart + 1 &&
          /HTTP\/(?:1\.[01]|2) 206/i.test(responseHeaders) &&
  ...
  acquisitionMode = "downloaded-public-source-format139-full-range-verified";
  state.sourcesInstalled += 1;
  publicSourceAttemptAudit.push({
    debateNumber,
    videoId: source.sourceVideoId,
...
try {
  ...
  const preparation = {
    ...
    recovery: {
      ...
      rangeVerifiedFormat139Downloads: 3
    },
    acquisitionPolicy: {
      ...
      acquisitionFormat: "frozen-youtube-audio-only-format139-range-verified-chunked-http-get",
  ...
  executionFailure = {
    category: "source-or-validation-failure-during-authorized-range-recovery-3",
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-10-audio-source-range-recovery-3-execution",
