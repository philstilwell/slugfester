...
assertV4(preparation.status ===
  ...
  preparation.protocolId === POST_CANARY_BATCH_17_RENDERING_PROTOCOL_ID &&
  preparation.packets?.length === 4 &&
  preparation.authorization?.executionActivation === true &&
  preparation.authorization?.renderingVerification === false,
"Batch 17 rendering verification is not prepared");
...
const activation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-17-rendering-activation",
  protocolId: preparation.protocolId,
  status: "frozen-post-canary-batch-17-rendering-verification-authorized",
  authorizedAt,
  activationCommit,
  ...
  nextAuthorizedAction: "execute-frozen-batch-17-rendering-verification"
};
...
console.log(JSON.stringify({ status: activation.status, debates: 4,
  viewportResultsAuthorized: 8, screenshotsAuthorized: 16,
