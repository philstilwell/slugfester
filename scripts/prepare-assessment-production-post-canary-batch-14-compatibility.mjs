...
import {
  POST_CANARY_BATCH_14_COMPATIBILITY_ORDER,
  POST_CANARY_BATCH_14_COMPATIBILITY_PROTOCOL_ID,
  POST_CANARY_BATCH_14_COMPATIBILITY_ROOT,
  POST_CANARY_BATCH_14_SITE_LEDGER_ADAPTER_VERSION,
  buildPostCanaryBatch14SiteLedgerAdapter,
  serializedJson,
  sha256,
...
const paths = {
  preparation: `${POST_CANARY_BATCH_14_COMPATIBILITY_ROOT}/preparation-manifest.json`,
  analysis: `${POST_CANARY_BATCH_14_COMPATIBILITY_ROOT}/analysis.json`,
  activation: `${POST_CANARY_BATCH_14_COMPATIBILITY_ROOT}/execution-activation.json`,
  execution: `${POST_CANARY_BATCH_14_COMPATIBILITY_ROOT}/execution.json`,
  stagedLedgerRoot:
    `${POST_CANARY_BATCH_14_COMPATIBILITY_ROOT}/output-bundle/staged-ledgers`,
  compatibility:
    "docs/assessment-production/post-canary-continuation-v1/batch-14/publication-finalization/compatibility-analysis.json",
  finalizationAudit:
    "docs/assessment-production/post-canary-continuation-v1/batch-14/publication-finalization/finalization-audit.json",
  finalLedger:
    "docs/assessment-production/post-canary-continuation-v1/batch-14/final-ledger/final-ledger.json",
  scores:
    "docs/assessment-production/post-canary-continuation-v1/batch-14/score-pass/calculated-scores.json",
  renderingAnalysis:
    "docs/assessment-production/post-canary-continuation-v1/batch-14/rendering-verification/analysis.json",
  renderingAudit:
    "docs/assessment-production/post-canary-continuation-v1/batch-14/rendering-verification/rendering-audit.json",
  ...
  standingAuthorization:
    "docs/assessment-production/post-canary-continuation-v1/batch-14/standing-authorization.json",
  batchFinalLedgerLibrary:
    "scripts/lib/assessment-production-post-canary-batch-14-final-ledger.mjs",
  ...
  preparationScript:
    "scripts/prepare-assessment-production-post-canary-batch-14-compatibility.mjs",
  preparationTest:
    "scripts/test-assessment-production-post-canary-batch-14-compatibility-preparation.mjs",
...
assertV4(
  ...
  "a stable --frozen-at ISO timestamp is required for the first Batch 14 compatibility preparation write"
);
...
assertV4(
  renderingAnalysis.status ===
      "ten-debate-batch-14-rendering-verification-passed" &&
    ...
    renderingAnalysis.nextAuthorizedAction ===
      "prepare-batch-14-production-compatibility-staging",
  "Batch 14 rendering gate does not authorize compatibility planning only"
);
assertV4(
  renderingAudit.status ===
      "passed-ten-debate-batch-14-rendering-verification" &&
  ...
  "passed Batch 14 rendering evidence audit required"
);
assertV4(
  compatibility.status ===
      "batch-14-production-compatibility-boundary-recorded" &&
    canonicalJson(compatibility.findings.map((finding) => finding.id)) ===
      canonicalJson(["batch-14-site-ledger-adapter-and-validator-route"]) &&
  ...
  "frozen Batch 14 publication compatibility blocker required"
);
assertV4(
  ...
  "passed one-pass Batch 14 staging-only finalization audit required"
);
assertV4(
  scores.status ===
      "post-canary-batch-14-single-score-pass-stability-gate-passed" &&
  ...
  "accepted one-pass Batch 14 score ledger required"
);
validatePostCanaryBatch14FinalLedger(
  finalLedger,
...
const renderingResultOrderValid =
  renderingAudit.results.length === POST_CANARY_BATCH_14_COMPATIBILITY_ORDER.length * 2 &&
  renderingAudit.results.every((row, index) =>
    row.debateNumber === POST_CANARY_BATCH_14_COMPATIBILITY_ORDER[Math.floor(index / 2)] &&
    row.viewportName === (index % 2 === 0 ? "desktop" : "mobile")
  );
assertV4(
  finalLedger.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_14_COMPATIBILITY_ORDER.join(",") &&
    scores.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_14_COMPATIBILITY_ORDER.join(",") &&
    renderingResultOrderValid &&
    renderedDebateOrder.join(",") ===
      POST_CANARY_BATCH_14_COMPATIBILITY_ORDER.join(","),
  "Batch 14 compatibility debate order changed"
);
...
const packetRecords = [];
for (const debateNumber of POST_CANARY_BATCH_14_COMPATIBILITY_ORDER) {
  ...
  const candidatePath =
    `docs/assessment-production/post-canary-continuation-v1/batch-14/` +
    `publication-finalization/output-bundle/final-candidates/debate-${debateNumber}.json`;
  const packetPath =
    `${POST_CANARY_BATCH_14_COMPATIBILITY_ROOT}/packets/debate-${debateNumber}.json`;
  ...
  const adapter = buildPostCanaryBatch14SiteLedgerAdapter({
    finalLedgerDebate,
    scoreDebate,
    candidate,
  ...
  const packet = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-14-compatibility-plan-packet",
    protocolId: POST_CANARY_BATCH_14_COMPATIBILITY_PROTOCOL_ID,
    status: "frozen-post-canary-batch-14-compatibility-plan-packet",
...
assertV4(
  totals.debates === 10 &&
    ...
    totals.overallBlunders === 55 &&
    totals.emptyReferenceLinks === 45 &&
    totals.suppliedReferenceLinks === 10,
  "Batch 14 compatibility population changed"
);

const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-14-compatibility-preparation-manifest",
  protocolId: POST_CANARY_BATCH_14_COMPATIBILITY_PROTOCOL_ID,
  status: "post-canary-batch-14-compatibility-plan-prepared-and-frozen",
  frozenAt,
  productionCanary: false,
  batchNumber: 14,
  planningOnly: true,
  ...
  userAuthorization: {
    instruction:
      "Use the frozen Batch 14 standing authorization for deterministic compatibility staging.",
    scopeInterpretation:
      "Prepare, validate, freeze, commit, and push the Batch 14 production-compatibility plan; later activation and one staging pass are already covered by the standing authorization. Do not publish production ledgers, mutate production, run models, use paid services, or select Batch 14."
  },
  scope: {
    finding: "batch-14-site-ledger-adapter-and-validator-route",
    description:
      "Freeze the exact Batch 14 ledger adapters and schema-specific validator route without applying them to the active validator or production data."
  },
  invariants: {
    ...
    scoreProtocolId:
      "assessment-production-post-canary-batch-14-single-deterministic-score-pass",
  ...
  proposedValidatorRoute: {
    ...
    newSchemaVersion: POST_CANARY_BATCH_14_SITE_LEDGER_ADAPTER_VERSION,
    existingCheckpointSchemaVersion:
      "1.0-production-checkpoint-v2.2-site-ledger-adapter",
    exactRoute:
      "When a production ledger uses the Batch 14 adapter schema, load the Batch 14 packet and activation from the frozen Batch 14 compatibility root, verify their hashes, and replay the adapter with the Batch 14 validator. Keep the checkpoint and Batch 1–12 adapters on their existing roots, credentials, and validators, and keep legacy Rubric v2 and v2.1 ledgers on their existing calculators.",
    oneSidedDisplayRows:
      "Treat all thirteen authenticated adjudicated-consensus adapter schemas as requiring ledgerMoveId and permitting a section exchange with only one side populated.",
    activationRequirements: [
      "activation status is post-canary-batch-14-compatibility-execution-authorized-and-frozen",
  ...
  stagedExecutionPlan: [
    ...
    },
    {
      ...
      allowedWrites: [
        paths.activeValidator,
        `${POST_CANARY_BATCH_14_COMPATIBILITY_ROOT}/output-bundle/**`,
        paths.execution,
        paths.analysis
      ],
      actions: [
        "wire the schema-specific Batch 14 adapter route",
  ...
  mandatoryExecutionTests: {
    positive: [
      "all ten Batch 14 adapters reproduce all 190 locked move scores, 52 section scores per side, and 20 overall scores",
      "all ten final candidates match ledgerMoveId, section order, model, rubric, and score protocol",
      "all 13 one-sided display rows pass through the authenticated Batch 14 adapter route",
      "all 44 empty Overall Commentary reference arrays pass without synthesized links",
      "all 8 supplied reference links retain their exact validation behavior",
    ...
    negative: [
      ...
      "a Batch 14 adapter routed through the checkpoint or Batch 1–12 roots fails",
  ...
  nextAuthorizedAction:
    "activate-and-execute-one-batch-14-production-compatibility-pass-under-standing-authorization"
};
const preparationBytes = serializedJson(preparation);
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-14-compatibility-preparation-analysis",
  protocolId: POST_CANARY_BATCH_14_COMPATIBILITY_PROTOCOL_ID,
  status: "post-canary-batch-14-compatibility-plan-freeze-passed",
  productionCanary: false,
  batchNumber: 14,
  planningOnly: true,
  preparation: {
...
if (write) {
  assertV4(
    ...
    "Batch 14 compatibility execution artifact already exists; planning write blocked"
  );
  await mkdir(
    resolve(`${POST_CANARY_BATCH_14_COMPATIBILITY_ROOT}/packets`),
    { recursive: true }
  );
  ...
  assertV4(
    ...
    "stored Batch 14 compatibility preparation differs from deterministic replay"
  );
  const storedAnalysis = await readJson(paths.analysis);
  assertV4(
    canonicalJson(storedAnalysis) === canonicalJson(analysis),
    "stored Batch 14 compatibility analysis differs from deterministic replay"
  );
  for (const record of packetRecords) {
    const storedPacket = await readFile(resolve(record.packetPath), "utf8");
    assertV4(
      storedPacket === record.packetBytes,
      `${record.debateNumber}: stored Batch 14 compatibility packet differs from deterministic replay`
    );
  }
}
