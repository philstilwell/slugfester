...
import {
  POST_CANARY_BATCH_17_COMPATIBILITY_ORDER,
  POST_CANARY_BATCH_17_COMPATIBILITY_PROTOCOL_ID,
  POST_CANARY_BATCH_17_COMPATIBILITY_ROOT,
  POST_CANARY_BATCH_17_SITE_LEDGER_ADAPTER_VERSION,
  buildPostCanaryBatch17SiteLedgerAdapter,
  serializedJson,
  sha256,
...
const paths = {
  preparation: `${POST_CANARY_BATCH_17_COMPATIBILITY_ROOT}/preparation-manifest.json`,
  analysis: `${POST_CANARY_BATCH_17_COMPATIBILITY_ROOT}/analysis.json`,
  activation: `${POST_CANARY_BATCH_17_COMPATIBILITY_ROOT}/execution-activation.json`,
  execution: `${POST_CANARY_BATCH_17_COMPATIBILITY_ROOT}/execution.json`,
  stagedLedgerRoot:
    `${POST_CANARY_BATCH_17_COMPATIBILITY_ROOT}/output-bundle/staged-ledgers`,
  compatibility:
    "docs/assessment-production/post-canary-continuation-v1/batch-17/publication-finalization/compatibility-analysis.json",
  finalizationAudit:
    "docs/assessment-production/post-canary-continuation-v1/batch-17/publication-finalization/finalization-audit.json",
  finalLedger:
  ...
  renderingAnalysis:
    "docs/assessment-production/post-canary-continuation-v1/batch-17/rendering-verification/analysis.json",
  renderingAudit:
    "docs/assessment-production/post-canary-continuation-v1/batch-17/rendering-verification/rendering-audit.json",
  ...
  batch16CompatibilityLibrary:
    "scripts/lib/assessment-production-post-canary-batch-16-compatibility.mjs",
  batch16CompatibilityActivation:
    "docs/assessment-production/post-canary-continuation-v1/batch-16/production-compatibility/execution-activation.json",
  standingAuthorization:
    "docs/assessment-production/post-canary-continuation-v1/batch-17/standing-authorization.json",
  batchFinalLedgerLibrary:
    "scripts/lib/assessment-production-post-canary-batch-17-final-ledger.mjs",
  compatibilityLibrary:
    "scripts/lib/assessment-production-post-canary-batch-17-compatibility.mjs",
  ...
  preparationScript:
    "scripts/prepare-assessment-production-post-canary-batch-17-compatibility.mjs",
  preparationTest:
    "scripts/test-assessment-production-post-canary-batch-17-compatibility-preparation.mjs",
...
assertV4(
  ...
  "a stable --frozen-at ISO timestamp is required for the first Batch 17 compatibility preparation write"
);
...
assertV4(
  renderingAnalysis.status ===
      "four-debate-batch-17-rendering-verification-passed" &&
    renderingAnalysis.decision?.renderingGatePassed === true &&
    renderingAnalysis.decision.fourDebatesPassed === true &&
    ...
    renderingAnalysis.nextAuthorizedAction ===
      "prepare-batch-17-production-compatibility-staging",
  "Batch 17 rendering gate does not authorize compatibility planning only"
);
assertV4(
  renderingAudit.status ===
      "passed-four-debate-batch-17-rendering-verification" &&
    renderingAudit.totals?.debates === 4 &&
    renderingAudit.totals?.viewportResults === 8 &&
    renderingAudit.totals?.runtimeFailures === 0 &&
    renderingAudit.productionMutationPerformed === false,
  "passed Batch 17 rendering evidence audit required"
);
assertV4(
  compatibility.status ===
      "batch-17-production-compatibility-boundary-recorded" &&
    canonicalJson(compatibility.findings.map((finding) => finding.id)) ===
      canonicalJson(["batch-17-site-ledger-adapter-and-validator-route"]) &&
  ...
  "frozen Batch 17 publication compatibility blocker required"
);
assertV4(
  ...
  "passed one-pass Batch 17 staging-only finalization audit required"
);
assertV4(
  scores.status ===
      "post-canary-batch-17-single-score-pass-stability-gate-passed" &&
  ...
  "accepted one-pass Batch 17 score ledger required"
);
validatePostCanaryBatch17FinalLedger(
  finalLedger,
...
const renderingResultOrderValid =
  renderingAudit.results.length === POST_CANARY_BATCH_17_COMPATIBILITY_ORDER.length * 2 &&
  renderingAudit.results.every((row, index) =>
    row.debateNumber === POST_CANARY_BATCH_17_COMPATIBILITY_ORDER[Math.floor(index / 2)] &&
    row.viewportName === (index % 2 === 0 ? "desktop" : "mobile")
  );
assertV4(
  finalLedger.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_17_COMPATIBILITY_ORDER.join(",") &&
    scores.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_17_COMPATIBILITY_ORDER.join(",") &&
    renderingResultOrderValid &&
    renderedDebateOrder.join(",") ===
      POST_CANARY_BATCH_17_COMPATIBILITY_ORDER.join(","),
  "Batch 17 compatibility debate order changed"
);
...
const packetRecords = [];
for (const debateNumber of POST_CANARY_BATCH_17_COMPATIBILITY_ORDER) {
  ...
  const packetPath =
    `${POST_CANARY_BATCH_17_COMPATIBILITY_ROOT}/packets/debate-${debateNumber}.json`;
  ...
  const adapter = buildPostCanaryBatch17SiteLedgerAdapter({
    finalLedgerDebate,
    scoreDebate,
    candidate,
  ...
  const packet = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-17-compatibility-plan-packet",
    protocolId: POST_CANARY_BATCH_17_COMPATIBILITY_PROTOCOL_ID,
    status: "frozen-post-canary-batch-17-compatibility-plan-packet",
...
const staticSourcePaths = [
  ...
  paths.batch16CompatibilityLibrary,
  paths.batch16CompatibilityActivation,
...
assertV4(
  totals.debates === 4 &&
    totals.sections === 21 &&
    totals.moves === 79 &&
    totals.oneSidedDisplayRows === 5 &&
    totals.overallBlunders === 20 &&
    totals.emptyReferenceLinks === 14 &&
    totals.suppliedReferenceLinks === 6,
  "Batch 17 compatibility population changed"
);

const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-17-compatibility-preparation-manifest",
  protocolId: POST_CANARY_BATCH_17_COMPATIBILITY_PROTOCOL_ID,
  status: "post-canary-batch-17-compatibility-plan-prepared-and-frozen",
  frozenAt,
  productionCanary: false,
  batchNumber: 17,
  planningOnly: true,
  ...
  userAuthorization: {
    instruction:
      "Use the frozen Batch 17 standing authorization for deterministic compatibility staging.",
    scopeInterpretation:
      "Prepare, validate, freeze, commit, and push the Batch 17 production-compatibility plan; later activation and one staging pass are already covered by the standing authorization. Do not publish production ledgers, mutate production, run models, use paid services, or select Batch 18."
  },
  scope: {
    finding: "batch-17-site-ledger-adapter-and-validator-route",
    description:
      "Freeze the exact Batch 17 ledger adapters and schema-specific validator route without applying them to the active validator or production data."
  },
  invariants: {
    ...
    scoreProtocolId:
      "assessment-production-post-canary-batch-17-single-deterministic-score-pass",
  ...
  proposedValidatorRoute: {
    ...
    newSchemaVersion: POST_CANARY_BATCH_17_SITE_LEDGER_ADAPTER_VERSION,
    existingCheckpointSchemaVersion:
      "1.0-production-checkpoint-v2.2-site-ledger-adapter",
    exactRoute:
      "When a production ledger uses the Batch 17 adapter schema, load the Batch 17 packet and activation from the frozen Batch 17 compatibility root, verify their hashes, and replay the adapter with the Batch 17 validator. Keep the checkpoint and Batch 1–15 adapters on their existing roots, credentials, and validators, and keep legacy Rubric v2 and v2.1 ledgers on their existing calculators.",
    oneSidedDisplayRows:
    ...
    activationRequirements: [
      "activation status is post-canary-batch-17-compatibility-execution-authorized-and-frozen",
    ...
    unchangedBehavior: [
      ...
      "the Batch 16 adapter route, credential, packets, and hashes remain valid",
  ...
  stagedExecutionPlan: [
    ...
    },
    {
      ...
      allowedWrites: [
        paths.activeValidator,
        `${POST_CANARY_BATCH_17_COMPATIBILITY_ROOT}/output-bundle/**`,
        paths.execution,
        paths.analysis
      ],
      actions: [
        "wire the schema-specific Batch 17 adapter route",
        "materialize the four exact staged adapters from the frozen packets",
  ...
  mandatoryExecutionTests: {
    positive: [
      "all four Batch 17 adapters reproduce all 79 locked move scores, 21 section scores per side, and 8 overall scores",
      "all four final candidates match ledgerMoveId, section order, model, rubric, and score protocol",
      "all 5 one-sided display rows pass through the authenticated Batch 17 adapter route",
      "all 14 empty Overall Commentary reference arrays pass without synthesized links",
      "all 6 supplied reference links retain their exact validation behavior",
      ...
      "all ten Batch 16 adapters retain their existing activation, packet, and score replays",
    ...
    negative: [
      ...
      "a Batch 17 adapter routed through the checkpoint or Batch 1–15 roots fails",
  ...
  stopRules: {
    ...
    batch16RouteRegressionBlocks: true,
  ...
  nextAuthorizedAction:
    "activate-and-execute-one-batch-17-production-compatibility-pass-under-standing-authorization"
};
const preparationBytes = serializedJson(preparation);
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-17-compatibility-preparation-analysis",
  protocolId: POST_CANARY_BATCH_17_COMPATIBILITY_PROTOCOL_ID,
  status: "post-canary-batch-17-compatibility-plan-freeze-passed",
  productionCanary: false,
  batchNumber: 17,
  planningOnly: true,
  preparation: {
  ...
  checks: {
    ...
    batch16RoutePreservationSpecified: true,
...
if (write) {
  assertV4(
    ...
    "Batch 17 compatibility execution artifact already exists; planning write blocked"
  );
  await mkdir(
    resolve(`${POST_CANARY_BATCH_17_COMPATIBILITY_ROOT}/packets`),
    { recursive: true }
  );
  ...
  assertV4(
    ...
    "stored Batch 17 compatibility preparation differs from deterministic replay"
  );
  const storedAnalysis = await readJson(paths.analysis);
  assertV4(
    canonicalJson(storedAnalysis) === canonicalJson(analysis),
    "stored Batch 17 compatibility analysis differs from deterministic replay"
  );
  for (const record of packetRecords) {
    const storedPacket = await readFile(resolve(record.packetPath), "utf8");
    assertV4(
      storedPacket === record.packetBytes,
      `${record.debateNumber}: stored Batch 17 compatibility packet differs from deterministic replay`
    );
  }
}
