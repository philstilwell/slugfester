...
assertV4(!(await exists(ACTIVATION)), "six-context resumption activation already exists");
const preparationBytes = await readFile(path.resolve(PREPARATION));
const preparation = JSON.parse(preparationBytes);
assertV4(preparation.protocolId === PROTOCOL_ID &&
  preparation.status === "frozen-six-original-unattempted-batch-10-publication-contexts-prepared-not-activated" &&
  preparation.contexts?.length === 6 && preparation.totals?.moves === 107 &&
  canonicalJson(preparation.contexts.map((context) => context.originalContextIndex)) ===
    canonicalJson(ORIGINAL_CONTEXT_INDEXES) &&
  canonicalJson(preparation.contexts.map((context) => context.debateNumber)) === canonicalJson(DEBATES),
"six original unattempted contexts are not prepared");
assertV4(preparation.model?.slug === "gpt-5.6-sol" &&
...
"six-context model or execution controls changed");
...
const activation = {
  ...preparation,
  schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-six-original-unattempted-context-resumption-activation",
  status: "frozen-six-original-unattempted-batch-10-publication-contexts-activated",
  activatedAt,
  ...
  nextAuthorizedAction: "execute-only-six-frozen-original-unattempted-publication-contexts"
};
