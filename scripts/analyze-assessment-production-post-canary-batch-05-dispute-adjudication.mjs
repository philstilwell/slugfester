...
const executionPath =
  `${POST_CANARY_BATCH_05_DISPUTE_ADJ_ROOT}/model-execution.json`;
...
assertV4(
  ...
  "Batch 5 adjudication execution is unavailable or crossed its boundary"
);
for (const [file, digest] of Object.entries(activation.sourceHashes)) {
  assertV4(
    sha256(await readFile(file)) === digest,
    `source hash mismatch during Batch 5 adjudication analysis: ${file}`
  );
}
...
for (const context of activation.contexts) {
  ...
  if (result.gateAcceptancePassed) {
    ...
    replay = validatePostCanaryBatch05DisputeAdjudicationOutput(
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-05-dispute-only-adjudication-analysis",
  ...
  status: passed
    ...
    : semanticPass
      ? timingPass
        ? scoreBlindPass
          ? "post-canary-batch-05-dispute-only-adjudication-gate-failed-isolation"
          : "post-canary-batch-05-dispute-only-adjudication-gate-failed-scoreblindness"
        : "post-canary-batch-05-dispute-only-adjudication-gate-failed-timing"
      : "post-canary-batch-05-dispute-only-adjudication-gate-failed-validation",
  ...
  gate: {
    ...
    requiredCandidateSelections: 527,
  ...
  nextAuthorizedAction: passed
    ? "standing-authorization-permits-batch-05-deterministic-final-ledger-assembly"
    : "new-user-approval-required-after-batch-05-adjudication-failure-before-downstream-work"
};
if (shouldWrite) {
