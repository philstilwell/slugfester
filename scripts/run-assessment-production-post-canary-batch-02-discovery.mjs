...
import { buildBatch02TokenCountedChunkLedger } from "./lib/assessment-production-post-canary-batch-02-source-preparation.mjs";
import {
  parseV42219Ledger,
  serializeV42219Rows,
...
const ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-02/discovery";
...
const SCRIPT =
  "scripts/run-assessment-production-post-canary-batch-02-discovery.mjs";
...
const EXPECTED_PREPARATION_STATUS =
  "frozen-thirty-six-post-canary-batch-02-discovery-contexts-prepared-not-authorized";
const EXPECTED_ACTIVATION_STATUS =
  "frozen-thirty-six-post-canary-batch-02-discovery-contexts-authorized";
  ...
  assertV4(
    preparation.status === EXPECTED_PREPARATION_STATUS &&
      preparation.protocolId ===
        "assessment-production-post-canary-batch-02-discovery" &&
      ...
      preparation.tokenLedgerCompatibility.minimumCandidateLexicalTokensChanged ===
        false &&
      preparation.tokenLedgerCompatibility.occurrences.length === 1 &&
      preparation.tokenLedgerCompatibility.occurrences[0].debateNumber === "99" &&
      preparation.tokenLedgerCompatibility.occurrences[0].eventIndex === 3272 &&
      ...
      preparation.executionPolicy.copiedInputBytesMaximum === 68003 &&
    ...
    "Batch 2 frozen discovery preparation is invalid"
  );
...
async function validateContextInputs(preparation, context) {
  ...
  assertV4(
    buildBatch02TokenCountedChunkLedger(validationChunkBytes).equals(
      modelLedgerBytes
...
async function preflight() {
  ...
  assertV4(
    preparationValidation.status ===
      "batch-02-discovery-execution-manifest-validation-passed-frozen-awaiting-separate-model-execution-approval" &&
    ...
    "Batch 2 preparation validation is invalid"
  );
...
async function activate() {
  ...
  const { preparation, bytes: preparationBytes } =
  ...
  const activation = {
    ...
    userAuthorization: {
      instruction:
        "I give explicit approval to activate and execute exactly these 36 discovery contexts.",
    ...
    nextRequiredAction:
      "execute-exactly-thirty-six-frozen-batch-02-discovery-contexts-once",
  };
  if (shouldWrite) {
...
async function loadAndValidateActivation() {
  ...
  assertV4(
    activation.status === EXPECTED_ACTIVATION_STATUS &&
      ...
      activation.tokenLedgerCompatibility.minimumCandidateLexicalTokensChanged ===
        false &&
      activation.tokenLedgerCompatibility.occurrences.length === 1 &&
      ...
      activation.executionPolicy.copiedInputBytesMaximum === 68003 &&
    ...
    "Batch 2 discovery execution is unauthorized"
  );
  ...
  assertV4(
    ...
    "frozen Batch 2 context count drifted"
  );
...
async function execute() {
  ...
  async function executeContext(context, contextIndex) {
    ...
    const sourceDirectory = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-post-canary-batch-02-discovery-${context.debateNumber}-${context.chunkId}-`
      )
    );
    const isolatedCodexHome = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-post-canary-batch-02-discovery-home-${context.debateNumber}-${context.chunkId}-`
      )
    );
    ...
    process.stdout.write(
      `[post-canary-batch-02-discovery] Debate ${context.debateNumber} ${context.chunkId} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
    );
    return record;
  }
  ...
  const execution = {
    ...
    status:
      validContexts === preparation.contexts.length
        ...
        : "post-canary-batch-02-discovery-complete-with-failure",
    productionContinuation: true,
    developmentValidationOnly: false,
    productionCanary: false,
    stagingOnly: true,
...
async function analyze() {
  ...
  assertV4(
    ...
    "all Batch 2 discovery contexts must pass without retry before analysis"
  );
  if (shouldWrite) {
    assertV4(
  ...
  const analysis = {
    ...
    status: candidateMinimumPassed
      ...
      : "post-canary-batch-02-discovery-failed-candidate-minimum-stop-no-retry",
    productionContinuation: true,
    developmentValidationOnly: false,
    productionCanary: false,
    stagingOnly: true,
...
if (command === "preflight") await preflight();
  ...
  throw new Error(
    "usage: run-assessment-production-post-canary-batch-02-discovery.mjs <preflight|activate|run|analyze|validate>"
  );
}
