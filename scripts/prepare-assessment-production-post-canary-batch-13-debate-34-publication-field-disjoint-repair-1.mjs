...
} from "./lib/assessment-production-post-canary-batch-13-debate-34-publication-field-disjoint-repair-1.mjs";
...
const RESUMPTION_ROOT = `${PUBLICATION_ROOT}/original-unattempted-context-resumption-4`;
...
const FAILURE_ANALYSIS = `${RESUMPTION_ROOT}/debate-34-failure-analysis.json`;
...
assertV4(
  originalExecution.status === "four-context-publication-resumption-stopped-with-failure" &&
    originalExecution.contextsAttempted === 4 && originalExecution.contextsUnattempted === 0 &&
    originalExecution.validContexts === 3 && originalExecution.invalidContexts === 1 &&
    originalExecution.attempts === 4 &&
    ...
    originalAnalysis.status === "four-context-publication-resumption-failed" &&
    failureAnalysis.status === "debate-34-publication-failure-diagnosed-awaiting-field-disjoint-repair-1" &&
    failureAnalysis.diagnosis?.invalidCritiqueCount === 9 &&
    failureAnalysis.diagnosis?.invalidQuoteCount === 0 &&
    failureAnalysis.diagnosis?.allOtherFieldsStructurallyValid === true,
  "Debate 34 publication resumption failure boundary changed"
);
...
const debate34 = allArtifacts.get("34");
const structuralStandIn = Object.values(debate34.outputJson.moveProse)
  .map((entry) => entry.critique)
  .find((critique) => {
    const words = wordCount(critique);
    return words >= 105 && words <= 130 && critique.length >= 880;
  });
assertV4(structuralStandIn, "Debate 34 has no validation-clean diagnostic stand-in");

const expectedCritiqueCounts = { "34": 9 };
const diagnostics = [];
...
assertV4(diagnostics.reduce((sum, item) => sum + item.invalidCritiques.length, 0) === 9 &&
  diagnostics.reduce((sum, item) => sum + item.invalidQuotes.length, 0) === 0,
"diagnosed nine-field repair scope changed");
...
assertV4(contexts.length === 5 &&
  contexts.every((context) => context.writableFieldCount >= 1 && context.writableFieldCount <= 2) &&
  contexts.reduce((sum, context) => sum + context.writableFieldCount, 0) === 9 &&
  new Set(contexts.flatMap((context) => context.writableFields)).size === 9,
"five-shard, nine-field coverage changed");

const sourceScripts = [
  "scripts/lib/assessment-production-post-canary-batch-13-debate-34-publication-field-disjoint-repair-1.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-13-debate-34-publication-field-disjoint-repair-1.mjs",
  "scripts/activate-assessment-production-post-canary-batch-13-debate-34-publication-field-disjoint-repair-1.mjs",
  "scripts/run-assessment-production-post-canary-batch-13-debate-34-publication-field-disjoint-repair-1.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-13-debate-34-publication-field-disjoint-repair-1.mjs",
...
const manifest = {
  ...
  protocolId: PROTOCOL_ID,
  status: "frozen-five-context-nine-field-batch-13-debate-34-publication-repair-prepared-not-activated",
  frozenAt,
  ...
  userAuthorization: {
    ...
    resolvedScope: "five fresh isolated field-disjoint shards correcting exactly nine rejected Debate 34 critiques while retaining every validation-clean field",
  ...
  diagnosis: {
    status: "deterministically-confirmed-nine-critique-failures",
    debates: diagnostics,
    totals: { debates: 1, invalidCritiques: 9, invalidQuotes: 0, writableFields: 9 },
    everyNonTargetFieldStructurallyValidated: true,
    temporaryStandInsWrittenOrRetained: false
  },
  diagnosticStandIn: {
    source: "validation-clean-critique-from-the-same-preserved-Debate-34-output",
  ...
  retentionContract: {
    ...
    completeTenDebateCohortReplayRequiredImmediatelyAfterRepair: true
  },
  ...
  totals: {
    debates: 1, contexts: 5, critiqueFields: 9, quoteFields: 0, writableFields: 9,
  ...
  nextAuthorizedAction: "commit-and-push-frozen-five-shard-debate-34-repair-then-activate"
};
