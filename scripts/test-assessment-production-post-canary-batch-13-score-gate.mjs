...
import {
  ...
  evaluatePostCanaryBatch13ScoreStability
...
const accepted = evaluatePostCanaryBatch13ScoreStability(passing);
...
assert.equal(
  evaluatePostCanaryBatch13ScoreStability(winnerMutation).acceptancePassed,
  false
);
const distanceMutation = structuredClone(passing);
distanceMutation[2].final = scores(95, 80);
assert.equal(
  evaluatePostCanaryBatch13ScoreStability(distanceMutation).acceptancePassed,
  false
);
assert.throws(() =>
  evaluatePostCanaryBatch13ScoreStability(passing, {
    ...POST_CANARY_BATCH_13_SCORE_STABILITY_THRESHOLDS,
