...
import {
  validatePostCanaryBatch15Scores
} from "./lib/assessment-production-post-canary-batch-15-score-gate.mjs";
import {
  buildPostCanaryBatch15PublicationPacket,
  buildPostCanaryBatch15PublicationSchema,
  postCanaryBatch15ReferenceCatalog,
  POST_CANARY_BATCH_15_PUBLICATION_DEBATES,
  POST_CANARY_BATCH_15_PUBLICATION_MODEL,
  POST_CANARY_BATCH_15_PUBLICATION_PROTOCOL_ID,
  POST_CANARY_BATCH_15_PUBLICATION_ROOT
} from "./lib/assessment-production-post-canary-batch-15-publication.mjs";
...
const ROOT = POST_CANARY_BATCH_15_PUBLICATION_ROOT;
...
const SELECTION =
  "docs/assessment-production/post-canary-continuation-v1/batch-15/selection.json";
const FINAL_LEDGER =
...
const FINAL_LEDGER_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-15/final-ledger/final-ledger-manifest.json";
const FINAL_LEDGER_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-15/final-ledger/analysis.json";
const SCORES =
  "docs/assessment-production/post-canary-continuation-v1/batch-15/score-pass/calculated-scores.json";
const SCORE_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-15/score-pass/score-pass-manifest.json";
const SCORE_PREPARATION =
  "docs/assessment-production/post-canary-continuation-v1/batch-15/score-pass/score-pass-preparation-manifest.json";
const SCORE_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-15/score-pass/analysis.json";
const SOURCE_PACKET_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-15/independent-judgments/source-packets";
...
const STATIC_SOURCE_FILES = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-15-score-gate.mjs",
  "scripts/lib/assessment-production-post-canary-batch-15-publication.mjs",
  "scripts/lib/assessment-production-post-canary-batch-15-publication-validation.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-15-publication.mjs",
  "scripts/test-assessment-production-post-canary-batch-15-publication-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-15-publication.mjs",
  "scripts/run-assessment-production-post-canary-batch-15-publication.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-15-publication.mjs"
];
...
assertV4(
  scoreAnalysis.status === "post-canary-batch-15-score-stability-gate-passed" &&
    scoreAnalysis.productionCanary === false &&
    scoreAnalysis.batchNumber === 15 &&
    ...
    scoreAnalysis.nextAuthorizedAction ===
      "standing-authorization-permits-batch-15-publication-packet-preparation",
  "the accepted Batch 15 score gate is unavailable or its stop boundary changed"
);
assertV4(
  scores.status === "post-canary-batch-15-single-score-pass-stability-gate-passed" &&
  ...
  "the locked Batch 15 score artifact is unavailable"
);
assertV4(
  scoreManifest.status ===
      "frozen-post-canary-batch-15-single-deterministic-score-pass-authorized" &&
    ...
    scorePreparation.status ===
      "frozen-post-canary-batch-15-single-deterministic-score-pass-prepared-not-activated",
  "the frozen Batch 15 single-pass score controls changed"
);
assertV4(
  ledger.status ===
      "passed-post-canary-batch-15-deterministic-final-ledger-assembly" &&
    ledger.debates?.length === 10 &&
    ledger.audit?.finalMoves === 191 &&
    ...
    ledgerManifest.status ===
      "frozen-ten-debate-post-canary-batch-15-deterministic-final-ledger-assembly" &&
    ledgerAnalysis.status ===
      "post-canary-batch-15-deterministic-final-ledger-gate-passed",
  "the locked Batch 15 final ledger changed"
);
assertV4(
  productionManifest.model?.label === POST_CANARY_BATCH_15_PUBLICATION_MODEL.label &&
    productionManifest.model?.slug === POST_CANARY_BATCH_15_PUBLICATION_MODEL.slug &&
    productionManifest.model?.reasoningEffort ===
      POST_CANARY_BATCH_15_PUBLICATION_MODEL.reasoningEffort &&
    productionManifest.model?.authentication ===
      POST_CANARY_BATCH_15_PUBLICATION_MODEL.authentication,
  "the frozen production model or authentication changed"
);
assertV4(
  selection.selected?.map((item) => item.debateNumber).join(",") ===
      POST_CANARY_BATCH_15_PUBLICATION_DEBATES.join(",") &&
    selection.selected.every((item) => item.speakerCount === 2) &&
    selection.authorization?.productionMutation === false,
  "the frozen Batch 15 selection changed"
);
assertV4(
  ledger.debates.map((debate) => debate.debateNumber).join(",") ===
    ...
    scores.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_15_PUBLICATION_DEBATES.join(","),
  "the Batch 15 ledger or score order changed"
);
assertV4(
...
const finalLedgerInputs = await loadPostCanaryBatch15FinalLedgerInputs();
validatePostCanaryBatch15FinalLedger(
  ledger,
...
const catalog = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-15-local-reference-catalog",
  references: postCanaryBatch15ReferenceCatalog()
};
...
for (const debateNumber of POST_CANARY_BATCH_15_PUBLICATION_DEBATES) {
  ...
  const packet = buildPostCanaryBatch15PublicationPacket({
    ledgerDebate,
    scoreDebate,
    sourcePacket,
  ...
  const schema = buildPostCanaryBatch15PublicationSchema(packet);
...
assertV4(
  contexts.reduce((sum, context) => sum + context.moves, 0) === 191,
  "Batch 15 publication move coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.sections, 0) === 51,
  "Batch 15 publication section coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.audioVerifiedMoves, 0) === 3,
  "Batch 15 audio-verification coverage changed"
);
assertV4(
  contexts.every(
    (context) =>
  ...
  "Batch 15 publication section or quote eligibility is invalid"
);
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-15-publication-execution-preparation-manifest",
  protocolId: POST_CANARY_BATCH_15_PUBLICATION_PROTOCOL_ID,
  status:
    "frozen-ten-post-canary-batch-15-score-locked-publication-contexts-prepared-not-activated",
  frozenAt,
  ...
  userAuthorization: {
    instruction:
      "The frozen Batch 15 standing authorization permits preparation, activation, and execution of exactly ten score-locked publication-reconstruction contexts while every gate passes, with direct incremental cost capped at $0.",
  ...
  model: structuredClone(POST_CANARY_BATCH_15_PUBLICATION_MODEL),
  costEstimate: {
  ...
  acceptanceContract: {
    ...
    movesAuthoredRequired: 191,
    critiquesRequired: 191,
  ...
  nextAuthorizedAction:
    "standing-authorization-permits-activation-and-execution-of-the-ten-frozen-batch-15-publication-contexts"
};
