...
assertV4(
  plan.headerParitySources.length === 3 &&
    plan.headerParitySources.every((source) =>
      JSON.stringify(Object.keys(source.requestHeaders)) === JSON.stringify([
        "User-Agent", "Accept", "Accept-Language", "Sec-Fetch-Mode"
      ])
    ),
  "Batch 10 header-parity source policy changed"
);
...
if (!shouldExecute) {
  console.log(
    JSON.stringify(
      {
        status: "passed-activated-batch-10-audio-source-header-parity-recovery-6-preflight",
        ...
        manualHttpRequestsMaximum: plan.executionPolicy.manualHttpRequestsMaximum,
        finalHttp206ResponsesRequired:
          plan.executionPolicy.finalHttp206ResponsesRequired,
        redirectsMaximum: plan.executionPolicy.redirectsMaximum,
        redirectsPerRangeMaximum: plan.executionPolicy.redirectsPerRangeMaximum,
...
const state = {
  ...
  manualHttpRequests: 0,
  finalHttp206Responses: 0,
  redirectsFollowed: 0,
  headerParityHttpRequests: 0,
  requestHeaderFieldsPerHttpRequest: 4,
...
const sanitizeErrorMessage = (value) => String(value)
  ...
  .slice(0, 6000);
const isGoogleVideoHostname = (hostname) =>
...
const installSource = async (source) => {
  ...
  const rangeSource = plan.headerParitySources.find(
    (item) => item.sourceVideoId === source.sourceVideoId
  );
  assertV4(
  ...
  normalizedTemporaryPath = path.join(
    audioDirectory,
    "source.header-parity6-format139.normalized.mp3"
  );
  const rawSourcePath = path.join(
    audioDirectory,
    "source.header-parity6-format139.m4a"
  );
  assertV4(
  ...
  const resolvedUrl = new URL(directUrl);
  assertV4(
    resolvedUrl.protocol === "https:" &&
      isGoogleVideoHostname(resolvedUrl.hostname) &&
      !directUrl.includes("\n"),
  ...
  try {
    for (
      ...
      let completedPartBytes = null;
      let redirectsForRange = 0;
      const hopRecords = [];
      for (let requestIndex = 0; requestIndex <= 3; requestIndex += 1) {
        const responseBodyPath = path.join(
          audioDirectory,
          `source.header-parity6-format139.range-${String(index).padStart(3, "0")}.hop-${requestIndex}.body`
        );
        const headersPath = path.join(
          audioDirectory,
          `source.header-parity6-format139.range-${String(index).padStart(3, "0")}.hop-${requestIndex}.headers.txt`
        );
        assertV4(
          !(await exists(responseBodyPath)) && !(await exists(headersPath)),
          `${debateNumber}: range ${index} hop ${requestIndex} evidence exists`
        );
        state.manualHttpRequests += 1;
        state.headerParityHttpRequests += 1;
        const requestHeaderArguments = Object.entries(
          rangeSource.requestHeaders
        ...
        const responseBody = await readFile(responseBodyPath);
        const responseHeaders = await readFile(headersPath, "utf8");
        const statusMatches = [
          ...responseHeaders.matchAll(/^HTTP\/\S+\s+(\d{3})/gm)
        ];
        assertV4(
          statusMatches.length === 1,
          `${debateNumber}: range ${index} hop ${requestIndex} status invalid`
        );
        const status = Number(statusMatches[0][1]);
        const sanitizedResponseHeaders = responseHeaders.replace(
          /^location:\s*\S+\s*$/gim,
          "location: [REDACTED_SIGNED_URL]"
        );
        await writeFile(headersPath, sanitizedResponseHeaders);
        const baseHopRecord = {
          requestIndex,
          status,
          ...
          responseHeadersSha256: sha256(sanitizedResponseHeaders)
        };
        if (status === 302) {
          assertV4(
            redirectsForRange < rangeSource.redirectsPerRangeMaximum,
            `${debateNumber}: range ${index} redirect maximum exceeded`
          );
          const locations = [
            ...responseHeaders.matchAll(/^location:\s*(\S+)\s*$/gim)
          ];
          assertV4(
            locations.length === 1,
            `${debateNumber}: range ${index} redirect location invalid`
          );
          const nextUrl = new URL(locations[0][1]);
          assertV4(
            nextUrl.protocol === "https:" && isGoogleVideoHostname(nextUrl.hostname),
            `${debateNumber}: range ${index} redirect destination rejected`
          );
          redirectsForRange += 1;
          state.redirectsFollowed += 1;
          hopRecords.push({
            ...baseHopRecord,
            redirectDestinationProtocol: "https:",
            redirectDestinationHostClass: "googlevideo.com"
          });
          currentUrl = nextUrl.href;
          await unlink(responseBodyPath);
          continue;
        }
        assertV4(
          status === 206 &&
            responseBody.length === rangeEnd - rangeStart + 1 &&
            responseHeaders.toLowerCase().includes(
              `content-range: bytes ${rangeStart}-${rangeEnd}/${rangeSource.declaredBytes}`
            ),
          `${debateNumber}: range ${index} final media response invalid`
        );
        state.finalHttp206Responses += 1;
        completedPartBytes = responseBody;
        hopRecords.push(baseHopRecord);
        await rawHandle.write(responseBody);
        await unlink(responseBodyPath);
        break;
      }
      assertV4(
        completedPartBytes !== null,
        `${debateNumber}: range ${index} final HTTP 206 absent`
      );
      rangeRecords.push({
        index,
        start: rangeStart,
        end: rangeEnd,
        bytes: completedPartBytes.length,
        redirectCount: redirectsForRange,
        sha256: sha256(completedPartBytes),
        hops: hopRecords
      });
    }
  } finally {
  ...
  assertV4(
    rawStat.size === rangeSource.declaredBytes &&
      ...
      rangeRecords.every((range) =>
        range.redirectCount <= rangeSource.redirectsPerRangeMaximum &&
        range.hops.at(-1)?.status === 206
      ),
  ...
  acquisitionMode =
    "downloaded-public-source-format139-yt-dlp-header-parity-manual-googlevideo-redirect-chain";
  state.sourcesInstalled += 1;
  publicSourceAttemptAudit.push({
    debateNumber,
    videoId: source.sourceVideoId,
  ...
  sourceRecords.push({
    ...
    requestHeaderParityApplied: true,
    requestHeaderFieldNames: Object.keys(rangeSource.requestHeaders),
...
try {
  ...
  assertV4(
    state.directUrlResolutionCliInvocations ===
        plan.executionPolicy.directUrlResolutionInvocationsMaximum &&
      state.rangeHttpGetInvocations ===
        plan.executionPolicy.rangeHttpGetInvocationsMaximum &&
      state.manualHttpRequests <= plan.executionPolicy.manualHttpRequestsMaximum &&
      state.headerParityHttpRequests === state.manualHttpRequests &&
      state.requestHeaderFieldsPerHttpRequest === 4 &&
      state.finalHttp206Responses ===
        plan.executionPolicy.finalHttp206ResponsesRequired &&
      state.redirectsFollowed <= plan.executionPolicy.redirectsMaximum &&
      state.repeatedByteRanges === plan.executionPolicy.repeatedByteRangesMaximum &&
      state.publicSourceDownloads ===
        plan.executionPolicy.publicSourceDownloadsMaximum,
    "Batch 10 manual redirect-chain execution totals changed"
  );
  ...
  const preparation = {
    ...
    recovery: {
      ...
      headerParityFormat139Downloads: 3,
      requestHeaderFieldsPerHttpRequest: 4,
      headerParityHttpRequests: state.headerParityHttpRequests,
      logicalRangeRequests: state.rangeHttpGetInvocations,
      manualHttpRequests: state.manualHttpRequests,
      finalHttp206Responses: state.finalHttp206Responses,
      redirectsFollowed: state.redirectsFollowed,
      repeatedByteRanges: state.repeatedByteRanges
    },
    acquisitionPolicy: {
      ...
      acquisitionFormat:
        "frozen-youtube-audio-only-format139-yt-dlp-header-parity-manual-googlevideo-redirect-chain",
      requestHeaderParity: {
        package: "yt-dlp",
        installedVersion: "2026.03.17",
        fieldNames: Object.keys(plan.headerParitySources[0].requestHeaders),
        exactValuesAuthenticatedByRecoveryPlan: true
      },
  ...
  executionFailure = {
    category: "source-or-validation-failure-during-authorized-header-parity-recovery-6",
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-10-audio-source-header-parity-recovery-6-execution",
  status: executionFailure
    ? "failed-one-shot-batch-10-audio-source-header-parity-recovery-6-stop-required"
    : "completed-one-shot-batch-10-audio-source-header-parity-recovery-6-and-cohort",
  batchNumber: 10,
  planSha256: sha256(planBytes),
  activationSha256: sha256(activationBytes),
  state,
