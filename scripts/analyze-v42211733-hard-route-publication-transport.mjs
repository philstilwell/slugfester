...
import { V42211733_ROOT } from "./lib/v42211733-hard-route-publication-transport.mjs";

const shouldWrite = process.argv.includes("--write");
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const [manifest, execution, preparation] = await Promise.all(["execution-manifest.json", "model-execution.json", "preparation-manifest.json"].map((file) => readFile(path.resolve(`${V42211733_ROOT}/${file}`), "utf8").then(JSON.parse)));
