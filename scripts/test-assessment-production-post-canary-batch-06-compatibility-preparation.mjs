...
const preparationPath =
  `${POST_CANARY_BATCH_06_COMPATIBILITY_ROOT}/preparation-manifest.json`;
const analysisPath =
  `${POST_CANARY_BATCH_06_COMPATIBILITY_ROOT}/analysis.json`;
...
assert.equal(preparation.batchNumber, 6);
...
assert.equal(
  ...
  "batch-06-site-ledger-adapter-and-validator-route"
);
assert.equal(
...
assert.equal(
  await exists(`${POST_CANARY_BATCH_06_COMPATIBILITY_ROOT}/execution.json`),
  false
);
assert.equal(
  await exists(
...
assert.deepEqual(packetNumbers, POST_CANARY_BATCH_06_COMPATIBILITY_ORDER);
for (const packetRecord of preparation.artifacts.packets) {
  ...
  assert.equal(packet.batchNumber, 6);
  ...
  const positive = validatePostCanaryBatch06SiteLedgerAdapter({
