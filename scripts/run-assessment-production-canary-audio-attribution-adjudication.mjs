...
import { PRODUCTION_CANARY_AUDIO_ADJ_ROOT } from "./lib/assessment-production-canary-audio-attribution-adjudication.mjs";
import { assertV4 } from "./lib/v4-lean-production.mjs";

const root = process.cwd();
const manifest = JSON.parse(
  await readFile(`${PRODUCTION_CANARY_AUDIO_ADJ_ROOT}/execution-manifest.json`, "utf8")
);
...
assertV4(
  ...
  "production-canary audio-attribution execution unauthorized"
);
...
const temporary = await mkdtemp(
  path.join(os.tmpdir(), "slugfester-production-canary-audio-adj-")
);
const temporaryCodexHome = await mkdtemp(
  path.join(os.tmpdir(), "slugfester-production-canary-audio-home-")
);
...
try {
  ...
  const prompt =
    "Read workflow.md, manual.md, schema.json, packet.json, and audio-transcript.json completely and no other files. Act only as the isolated 5.6 Sol/low audio-attribution adjudicator for Debate 05. Decide only whether the expected speaker authored the locked core proposition and cite exact diarized segment indexes. Ratings, scores, legacy data, other debates, winners, and publication prose are unavailable. Return exactly one schema-conforming JSON object and no commentary.";
...
const execution = {
  schemaVersion:
    "1.0-production-canary-audio-attribution-adjudication-model-execution",
  protocolId: manifest.protocolId,
  status: record.gateAcceptancePassed
    ? "production-canary-audio-attribution-adjudication-execution-passed"
    : "production-canary-audio-attribution-adjudication-execution-failed",
  contexts: 1,
  attempts: 1,
  retries: 0,
  meteredApiCostUsd: 0,
...
assertV4(
  ...
  "production-canary audio-attribution adjudication failed; downstream work is blocked"
);
