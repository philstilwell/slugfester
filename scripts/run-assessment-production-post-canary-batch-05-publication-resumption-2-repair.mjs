...
import { POST_CANARY_BATCH_05_DEBATE_42_REPAIR_ROOT, validateDebate42RepairOutput } from
...
assertV4(activation.status === "frozen-two-context-batch-05-debate-42-publication-repair-authorized" &&
...
"the Debate 42 repair execution is not authorized");
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-05-debate-42-repair-${context.contextIndex}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `batch-05-debate-42-repair-home-${context.contextIndex}-`));
  ...
  try { const copies = [[activation.modelInputs.productionWorkflow, "production-workflow.md"],
    ...
    const prompt = ["Read production-workflow.md, readiness-workflow.md, output-contract.md, manual.md, packet.json, and schema.json completely and no other files.",
      `Act only as isolated Debate 42 bounded repair editor ${context.packetIndex}.`,
      "Rewrite exactly the two correctedCritiques named by the packet and schema, preserving adjudicated substance and the locked score band.",
    ...
    process.stdout.write(`[batch-05-debate-42-repair] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort}\n`);
    ...
    const base = { contextIndex: context.contextIndex, packetIndex: context.packetIndex,
      debateNumber: "42", debateId: context.debateId, model: activation.model.label,
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 || invocation.signal !== null || !resultExists) {
      ...
      try { validationSummary = validateDebate42RepairOutput(JSON.parse(outputBytes),
      ...
      const validation = { schemaVersion: "1.0-assessment-production-post-canary-batch-05-debate-42-repair-validation",
        ...
        debateNumber: "42", outputSha256: sha256(outputBytes), validationSummary,
        validationMessage, modelAuthoredScores: 0 };
      const provenance = { schemaVersion: "1.0-assessment-production-post-canary-batch-05-debate-42-repair-provenance",
        protocolId: activation.protocolId, contextIndex: context.contextIndex,
        packetIndex: context.packetIndex, debateNumber: "42", model: activation.model,
    ...
    debateNumber: "42", attemptCount: 1, retryCount: 0, timeoutExtensionCount: 0,
  ...
  process.stdout.write(`[batch-05-debate-42-repair] index ${context.contextIndex} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`); return record;
}
...
for (const phase of activation.executionPolicy.rampPhases) { if (!proceed) { phases.push({ ...phase,
  ...
  const result = await runContext(activation.contexts[phase.contextIndexes[0]]); results.push(result);
  const passed = result.gateAcceptancePassed; phases.push({ ...phase, attemptedContextIndexes: [result.contextIndex],
    validContextIndexes: passed ? [result.contextIndex] : [], passed, skippedBecausePriorRampFailed: false });
  if (!passed) proceed = false; }
const validContexts = results.filter((row) => row.gateAcceptancePassed).length;
const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-05-debate-42-repair-execution",
  protocolId: activation.protocolId, status: validContexts === 2
    ? "batch-05-debate-42-two-context-repair-gate-passed"
    : "batch-05-debate-42-repair-gate-complete-with-failure",
