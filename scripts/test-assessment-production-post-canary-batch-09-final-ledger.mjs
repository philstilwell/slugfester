...
const scoreArtifactCandidates = [
  `${POST_CANARY_BATCH_09_FINAL_LEDGER_ROOT}/calculated-scores.json`,
  `${POST_CANARY_BATCH_09_FINAL_LEDGER_ROOT}/score-analysis.json`,
  `${POST_CANARY_BATCH_09_FINAL_LEDGER_ROOT}/score-pass-manifest.json`
];
...
const rebuiltLedger = buildPostCanaryBatch09FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
...
assert.deepEqual(
  ...
  ["170", "134", "19", "114", "166", "89", "176", "183", "112", "17"]
);
assert.equal(storedLedger.audit.finalMoves, 180);
assert.equal(storedLedger.audit.disputedMoves, 173);
assert.equal(storedLedger.audit.pairSelections, 166);
assert.equal(storedLedger.audit.scoringFieldSelections, 275);
assert.equal(storedLedger.audit.burdenAdjustmentSelections, 18);
assert.equal(storedLedger.audit.candidateSelections, 459);
assert.equal(storedLedger.audit.meanMerges, 391);
...
assert.throws(() =>
  buildPostCanaryBatch09FinalLedger(mutatedInputs, inputs.sourceHashes)
);
...
assert.equal(analysis.totals.finalMoves, 180);
assert.equal(analysis.totals.disputedMoves, 173);
assert.equal(analysis.totals.candidateSelections, 459);
