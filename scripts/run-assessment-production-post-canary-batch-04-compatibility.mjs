...
assertV4(write, "Batch 4 compatibility runner requires --write");
...
const paths = {
  root: POST_CANARY_BATCH_04_COMPATIBILITY_ROOT,
...
assertV4(
  ...
  "Batch 4 compatibility pass was already attempted; rerun forbidden"
);
...
assertV4(
  ...
  "frozen Batch 4 compatibility activation is unavailable"
);
...
tests.push(
  runCommand(
    "batch-04-route-positive-negative-controls",
    process.execPath,
...
if (tests.at(-1).passed) {
  tests.push(
    runCommand("checkpoint-batch-01-batch-02-batch-03-legacy-reference-regression", process.execPath, [
...
const nextAuthorizedAction = compatibilityAcceptancePassed
  ? "prepare-batch-04-production-publication-mutation-manifest-under-standing-authorization"
  : "diagnose-batch-04-compatibility-failure-under-failure-recovery-standing-authorization";
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-compatibility-execution",
  protocolId: preparation.protocolId,
  status: compatibilityAcceptancePassed
    ? "batch-04-route-score-checkpoint-batch-01-batch-02-batch-03-legacy-reference-validation-passed"
    : "failed-closed-batch-04-compatibility-staging-pass",
  startedAt,
  completedAt,
  activation: {
  ...
  regressions: {
    batch04RouteControlsPassed: tests[0]?.passed === true,
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-compatibility-execution-analysis",
  protocolId: preparation.protocolId,
  status: compatibilityAcceptancePassed
    ? "batch-04-production-compatibility-passed"
    : "batch-04-production-compatibility-failed-closed",
  ...
  checks: {
    ...
    routeAndScoreControlsPassed: execution.regressions.batch04RouteControlsPassed,
    checkpointBatch01Batch02Batch03LegacyReferencePassed:
