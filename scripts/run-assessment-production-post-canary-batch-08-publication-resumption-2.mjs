...
assertV4(activation.status ===
  "frozen-six-untouched-post-canary-batch-08-publication-resumption-2-contexts-authorized" &&
  ...
  activation.executionPolicy?.contexts === 6 && activation.executionPolicy?.attemptsPerContext === 1 &&
...
"the six-context Batch 8 publication resumption is not authorized");
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-08-publication-resumption-2-${context.debateNumber}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `batch-08-publication-resumption-2-home-${context.debateNumber}-`));
  ...
  try {
    ...
    process.stdout.write(`[batch-08-publication-resumption-2] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber}\n`);
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 ||
      ...
      const validationRecord = { schemaVersion:
          "1.0-assessment-production-post-canary-batch-08-publication-resumption-2-validation",
      ...
      const provenanceRecord = { schemaVersion:
          "1.0-assessment-production-post-canary-batch-08-publication-resumption-2-provenance",
  ...
  process.stdout.write(`[batch-08-publication-resumption-2] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
...
const execution = { schemaVersion:
    "1.0-assessment-production-post-canary-batch-08-publication-resumption-2-model-execution",
  protocolId: activation.protocolId, status: passed
    ? "six-post-canary-batch-08-publication-resumption-2-contexts-passed"
    : "post-canary-batch-08-publication-resumption-2-complete-with-failure",
  ...
  ownDebateScoresImmutable: true, acceptedPriorDebates: 4,
