...
import {
  POST_CANARY_BATCH_11_DISPUTE_ADJ_ISOLATION,
...
const EXPECTED_DEBATES = ["54", "01", "82", "191", "151", "188", "60", "79", "43", "24"];
...
const preparation = JSON.parse(
  await readFile(`${POST_CANARY_BATCH_11_DISPUTE_ADJ_ROOT}/preparation-manifest.json`, "utf8")
);
...
assert.equal(preparation.totals.disputedMoves, 184);
...
assert.equal(preparation.acceptedSourceBoundary.allTwoAudioMovesVerified, true);
...
for (const context of preparation.contexts) {
  ...
  const output = {
    schemaVersion: POST_CANARY_BATCH_11_DISPUTE_ADJ_OUTPUT_VERSION,
    ...
    isolation: structuredClone(POST_CANARY_BATCH_11_DISPUTE_ADJ_ISOLATION),
  ...
  const validation = validatePostCanaryBatch11DisputeAdjudicationOutput(output, packet);
...
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_11_DISPUTE_ADJ_ROOT}/packets`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_11_DISPUTE_ADJ_ROOT}/provenance`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
