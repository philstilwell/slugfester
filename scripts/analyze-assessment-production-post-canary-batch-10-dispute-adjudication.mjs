...
const activationPath =
  `${POST_CANARY_BATCH_10_DISPUTE_ADJ_ROOT}/execution-activation.json`;
const executionPath =
  `${POST_CANARY_BATCH_10_DISPUTE_ADJ_ROOT}/model-execution.json`;
...
assertV4(
  activation.status ===
      "frozen-ten-post-canary-batch-10-dispute-only-adjudication-contexts-authorized" &&
  ...
  "Batch 10 adjudication execution is unavailable or crossed its boundary"
);
for (const [file, digest] of Object.entries(activation.sourceHashes)) {
  assertV4(
    sha256(await readFile(file)) === digest,
    `source hash mismatch during Batch 10 adjudication analysis: ${file}`
  );
}
...
for (const context of activation.contexts) {
  ...
  if (result.gateAcceptancePassed) {
    ...
    replay = validatePostCanaryBatch10DisputeAdjudicationOutput(
...
const semanticPass =
  ...
  valid.reduce((sum, context) => sum + context.candidateSelections, 0) ===
    483 &&
  valid.reduce((sum, context) => sum + context.disputedMoves, 0) === 177;
const timingPass =
  semanticPass &&
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-10-dispute-only-adjudication-analysis",
  protocolId: activation.protocolId,
  status: passed
    ? "post-canary-batch-10-dispute-only-adjudication-gate-passed-standing-authorization-active-for-final-ledger-assembly"
    : semanticPass
      ? timingPass
        ? scoreBlindPass
          ? "post-canary-batch-10-dispute-only-adjudication-gate-failed-isolation"
          : "post-canary-batch-10-dispute-only-adjudication-gate-failed-scoreblindness"
        : "post-canary-batch-10-dispute-only-adjudication-gate-failed-timing"
      : "post-canary-batch-10-dispute-only-adjudication-gate-failed-validation",
  ...
  nextAuthorizedAction: passed
    ? "standing-authorization-permits-batch-10-deterministic-final-ledger-assembly"
    : "new-user-approval-required-after-batch-10-adjudication-failure-before-downstream-work"
};
if (shouldWrite) {
