...
const paths = {
  ...
  compatibility:
    "docs/assessment-production/post-canary-continuation-v1/batch-07/publication-finalization/compatibility-analysis.json",
  finalizationAudit:
    "docs/assessment-production/post-canary-continuation-v1/batch-07/publication-finalization/finalization-audit.json",
  finalLedger:
    "docs/assessment-production/post-canary-continuation-v1/batch-07/final-ledger/final-ledger.json",
  scores:
    "docs/assessment-production/post-canary-continuation-v1/batch-07/score-pass/calculated-scores.json",
  renderingAnalysis:
    "docs/assessment-production/post-canary-continuation-v1/batch-07/rendering-verification/analysis.json",
  renderingAudit:
    "docs/assessment-production/post-canary-continuation-v1/batch-07/rendering-verification/rendering-audit.json",
  ...
  standingAuthorization:
    "docs/assessment-production/post-canary-continuation-v1/batch-07/standing-authorization.json",
  batchFinalLedgerLibrary:
    "scripts/lib/assessment-production-post-canary-batch-07-final-ledger.mjs",
  ...
  preparationScript:
    "scripts/prepare-assessment-production-post-canary-batch-07-compatibility.mjs",
  preparationTest:
    "scripts/test-assessment-production-post-canary-batch-07-compatibility-preparation.mjs",
...
assertV4(
  ...
  "a stable --frozen-at ISO timestamp is required for the first Batch 7 compatibility preparation write"
);
...
assertV4(
  renderingAnalysis.status ===
      "ten-debate-batch-07-rendering-verification-passed" &&
    ...
    renderingAnalysis.nextAuthorizedAction ===
      "prepare-batch-07-production-compatibility-staging",
  "Batch 7 rendering gate does not authorize compatibility planning only"
);
assertV4(
  renderingAudit.status ===
      "passed-ten-debate-batch-07-rendering-verification" &&
  ...
  "passed Batch 7 rendering evidence audit required"
);
assertV4(
  compatibility.status ===
      "batch-07-production-compatibility-boundary-recorded" &&
    canonicalJson(compatibility.findings.map((finding) => finding.id)) ===
      canonicalJson(["batch-07-site-ledger-adapter-and-validator-route"]) &&
  ...
  "frozen Batch 7 publication compatibility blocker required"
);
assertV4(
  ...
  "passed one-pass Batch 7 staging-only finalization audit required"
);
assertV4(
  scores.status ===
      "post-canary-batch-07-single-score-pass-stability-gate-passed" &&
  ...
  "accepted one-pass Batch 7 score ledger required"
);
validatePostCanaryBatch07FinalLedger(
  finalLedger,
...
const renderingResultOrderValid =
  renderingAudit.results.length === POST_CANARY_BATCH_07_COMPATIBILITY_ORDER.length * 2 &&
  renderingAudit.results.every((row, index) =>
    row.debateNumber === POST_CANARY_BATCH_07_COMPATIBILITY_ORDER[Math.floor(index / 2)] &&
    row.viewportName === (index % 2 === 0 ? "desktop" : "mobile")
  );
assertV4(
  finalLedger.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_07_COMPATIBILITY_ORDER.join(",") &&
    scores.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_07_COMPATIBILITY_ORDER.join(",") &&
    renderingResultOrderValid &&
    renderedDebateOrder.join(",") ===
      POST_CANARY_BATCH_07_COMPATIBILITY_ORDER.join(","),
  "Batch 7 compatibility debate order changed"
);
...
const packetRecords = [];
for (const debateNumber of POST_CANARY_BATCH_07_COMPATIBILITY_ORDER) {
  ...
  const packetPath =
    `${POST_CANARY_BATCH_07_COMPATIBILITY_ROOT}/packets/debate-${debateNumber}.json`;
  ...
  const adapter = buildPostCanaryBatch07SiteLedgerAdapter({
    finalLedgerDebate,
    scoreDebate,
    candidate,
  ...
  const packet = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-07-compatibility-plan-packet",
    protocolId: POST_CANARY_BATCH_07_COMPATIBILITY_PROTOCOL_ID,
    status: "frozen-post-canary-batch-07-compatibility-plan-packet",
...
assertV4(
  totals.debates === 10 &&
    ...
    totals.overallBlunders === 56 &&
    totals.emptyReferenceLinks === 53 &&
    totals.suppliedReferenceLinks === 3,
  "Batch 7 compatibility population changed"
);

const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-07-compatibility-preparation-manifest",
  protocolId: POST_CANARY_BATCH_07_COMPATIBILITY_PROTOCOL_ID,
  status: "post-canary-batch-07-compatibility-plan-prepared-and-frozen",
  frozenAt,
  productionCanary: false,
  batchNumber: 7,
  planningOnly: true,
  ...
  userAuthorization: {
    instruction:
      "Use the frozen Batch 7 standing authorization for deterministic compatibility staging.",
    scopeInterpretation:
      "Prepare, validate, freeze, commit, and push the Batch 7 production-compatibility plan; later activation and one staging pass are already covered by the standing authorization. Do not publish production ledgers, mutate production, run models, use paid services, or select Batch 8."
  },
  scope: {
    finding: "batch-07-site-ledger-adapter-and-validator-route",
    description:
      "Freeze the exact Batch 7 ledger adapters and schema-specific validator route without applying them to the active validator or production data."
  },
  invariants: {
    ...
    scoreProtocolId:
      "assessment-production-post-canary-batch-07-single-deterministic-score-pass",
  ...
  proposedValidatorRoute: {
    ...
    newSchemaVersion: POST_CANARY_BATCH_07_SITE_LEDGER_ADAPTER_VERSION,
    existingCheckpointSchemaVersion:
      "1.0-production-checkpoint-v2.2-site-ledger-adapter",
    exactRoute:
      "When a production ledger uses the Batch 7 adapter schema, load the Batch 7 packet and activation from the frozen Batch 7 compatibility root, verify their hashes, and replay the adapter with the Batch 7 validator. Keep the checkpoint and Batch 1–6 adapters on their existing roots, credentials, and validators, and keep legacy Rubric v2 and v2.1 ledgers on their existing calculators.",
    oneSidedDisplayRows:
      "Treat all eight authenticated adjudicated-consensus adapter schemas as requiring ledgerMoveId and permitting a section exchange with only one side populated.",
    activationRequirements: [
      "activation status is post-canary-batch-07-compatibility-execution-authorized-and-frozen",
  ...
  stagedExecutionPlan: [
    ...
    },
    {
      ...
      allowedWrites: [
        paths.activeValidator,
        `${POST_CANARY_BATCH_07_COMPATIBILITY_ROOT}/output-bundle/**`,
        paths.execution,
        paths.analysis
      ],
      actions: [
        "wire the schema-specific Batch 7 adapter route",
  ...
  mandatoryExecutionTests: {
    positive: [
      "all ten Batch 7 adapters reproduce all 187 locked move scores, 51 section scores per side, and 20 overall scores",
      "all ten final candidates match ledgerMoveId, section order, model, rubric, and score protocol",
      "all 13 one-sided display rows pass through the authenticated Batch 7 adapter route",
      "all 53 empty Overall Commentary reference arrays pass without synthesized links",
    ...
    negative: [
      ...
      "a Batch 7 adapter routed through the checkpoint or Batch 1–6 roots fails",
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-07-compatibility-preparation-analysis",
  protocolId: POST_CANARY_BATCH_07_COMPATIBILITY_PROTOCOL_ID,
  status: "post-canary-batch-07-compatibility-plan-freeze-passed",
  productionCanary: false,
  batchNumber: 7,
  planningOnly: true,
  preparation: {
...
if (write) {
  assertV4(
    ...
    "Batch 7 compatibility execution artifact already exists; planning write blocked"
  );
  await mkdir(
    resolve(`${POST_CANARY_BATCH_07_COMPATIBILITY_ROOT}/packets`),
    { recursive: true }
  );
  ...
  assertV4(
    ...
    "stored Batch 7 compatibility preparation differs from deterministic replay"
  );
  const storedAnalysis = await readJson(paths.analysis);
  assertV4(
    canonicalJson(storedAnalysis) === canonicalJson(analysis),
    "stored Batch 7 compatibility analysis differs from deterministic replay"
  );
  for (const record of packetRecords) {
    const storedPacket = await readFile(resolve(record.packetPath), "utf8");
    assertV4(
      storedPacket === record.packetBytes,
      `${record.debateNumber}: stored Batch 7 compatibility packet differs from deterministic replay`
    );
  }
}
