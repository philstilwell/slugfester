...
const finalLedgerPath =
  `${POST_CANARY_BATCH_01_FINAL_LEDGER_ROOT}/final-ledger.json`;
...
assertV4(
  ...
  "post-canary Batch 1 final-ledger manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen Batch 1 final-ledger source hash mismatch`
  );
}
const inputs = await loadPostCanaryBatch01FinalLedgerInputs();
const ledger = buildPostCanaryBatch01FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
