...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-07/rendering-verification";
...
const PREPARE_HARNESS = "scripts/prepare-assessment-production-post-canary-batch-07-rendering-source-hash-correction-1.mjs";
const RUN_HARNESS = "scripts/run-assessment-production-post-canary-batch-07-rendering-source-hash-correction-1.mjs";
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const exists = (file) => access(path.resolve(file)).then(() => true, () => false);

for (const file of [PLAN, PROPOSED_PREPARATION, PROPOSED_ACTIVATION]) {
  assertV4(!(await exists(file)), `${file} exists`);
}
const [preparationBytes, activationBytes, targetBytes, prepareHarnessBytes,
  runHarnessBytes] = await Promise.all([
  readFile(path.resolve(PREPARATION)),
  readFile(path.resolve(ACTIVATION)),
  readFile(path.resolve(TARGET)),
  readFile(path.resolve(PREPARE_HARNESS)),
  readFile(path.resolve(RUN_HARNESS))
]);
const preparation = JSON.parse(preparationBytes);
const activation = JSON.parse(activationBytes);
assertV4(
  preparation.status === "frozen-post-canary-batch-07-rendering-verification-prepared" &&
  activation.status === "frozen-post-canary-batch-07-rendering-verification-authorized" &&
...
proposedPreparation.sourceHashes[TARGET] = targetSha256;
...
proposedActivation.sourceHashes[TARGET] = targetSha256;
...
const plan = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-07-rendering-source-hash-correction-1-plan",
  status: "frozen-batch-07-rendering-source-hash-correction-1-prepared",
  classification: "execution-runner-required-production-baseline-hash-omitted-from-preparation-and-activation",
  diagnosis: {
    category: "known-inherited-rendering-preflight-source-hash-omission",
    meaning: "The frozen viewport runner authenticates src/data/debates.js, but the preparation and activation records omitted its baseline hash; every viewport would otherwise report a false production-data mutation.",
    viewportAttemptsConsumed: 0
  },
  original: {
  ...
  correctionScope: {
    addedSourceHashField: TARGET,
    addedSourceHashValue: targetSha256,
    dependentPreparationHashUpdated: true,
    dependentNavigationTokenUpdated: true,
    otherFieldsChanged: false
  },
  proposed: {
    preparation: PROPOSED_PREPARATION,
    preparationSha256: sha256(proposedPreparationBytes),
    activation: PROPOSED_ACTIVATION,
    activationSha256: sha256(proposedActivationBytes)
  },
  authenticatedHarnesses: {
    [PREPARE_HARNESS]: sha256(prepareHarnessBytes),
    [RUN_HARNESS]: sha256(runHarnessBytes)
  },
  executionPolicy: { attemptsMaximum: 1, retriesMaximum: 0, timeoutExtensionsMaximum: 0 },
  directIncrementalCostUsd: 0,
  nextAuthorizedAction: "execute-one-batch-07-rendering-source-hash-correction-1-pass"
};
if (shouldWrite) {
...
console.log(JSON.stringify({
  status: plan.status,
  addedSourceHash: TARGET,
  dependentHashesRegenerated: 2,
  viewportAttemptsConsumed: 0,
  attemptsMaximum: 1,
  directIncrementalCostUsd: 0,
