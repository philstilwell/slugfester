...
const manifest = JSON.parse(await readFile(path.resolve(root, `${V3811_PERFORMANCE_ROOT}/initial-execution-manifest.json`), "utf8"));
const codex = "/Applications/ChatGPT.app/Contents/Resources/codex";
const authSource = path.join(os.homedir(), ".codex", "auth.json");
const bytes = (relativePath) => readFile(path.resolve(root, relativePath));
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
assertV3811(manifest.status === "frozen-six-context-authorized" && manifest.authorization.initialPerformanceModelExecution && !manifest.authorization.scoreDerivation && !manifest.stopRules.furtherAutomaticRetryAuthorized, "initial performance execution unauthorized");
...
for (const context of manifest.contexts) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-v3811-performance-${context.debateNumber}-${context.pass}-`));
  const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), `slugfester-v3811-home-performance-${context.debateNumber}-${context.pass}-`));
  ...
  try {
    ...
    if (invocation.timedOut || invocation.code !== 0 || invocation.signal !== null) record = { ...base, status: invocation.timedOut ? "timed-out" : "transport-failed", outputWritten: false, deterministicValidationPassed: false, gateAcceptancePassed: false };
      ...
      const validation = await run(process.execPath, ["scripts/validate-v3811-performance-judgment-output.mjs", context.output, context.packet, context.pass], { cwd: root, env: process.env });
