...
import { assert, canonicalJson } from "./lib/v375-correction.mjs";
...
execFileSync(process.execPath, ["scripts/validate-v375-correction-execution-lock.mjs"], { cwd: root, stdio: "ignore" });
...
for (const [reviewerPass, debates] of Object.entries(manifest.initialContexts)) for (const [debateNumber, context] of Object.entries(debates)) {
  ...
  if (result.status === "completed-valid") {
    ...
    execFileSync(process.execPath, ["scripts/validate-v375-correction-output.mjs", context.output, context.packet, context.schema], { cwd: root, stdio: "ignore" });
  }
}
...
for (const context of disagreement.adjudicationContexts) {
  ...
  assert(!canonicalJson(packet).includes("semanticTuple") && !canonicalJson(packet).includes("matchesDevelopmentReference"), `pass-c.${context.debateNumber}: sealed reference leaked`);
  if (result.status === "completed-valid") {
    const outputText = await read(context.output);
...
assert(analysis.sources.executionManifestSha256 === sha256(manifestText) && analysis.sources.initialExecutionSha256 === sha256(initialText) && analysis.sources.initialDisagreementsSha256 === sha256(disagreementText), "analysis initial provenance invalid");
...
assert(analysis.passed === expectedPassed && analysis.decision.correctedDisjointRepeatabilityTestPreregistrationAuthorized === expectedPassed, "analysis pass formula invalid");
...
console.log(JSON.stringify({ status: "passed", artifactIntegrityPassed: true, correctionSmokePassed: analysis.passed, initial: analysis.results.initial, final: { resolved: analysis.results.final.resolved, matchesDevelopmentReference: analysis.results.final.matchesDevelopmentReference, differsFromDevelopmentReference: analysis.results.final.differsFromDevelopmentReference }, decision: analysis.decision, analysisSha256: sha256(analysisText) }, null, 2));
