...
import {
  POST_CANARY_BATCH_07_PUBLICATION_MODEL,
  POST_CANARY_BATCH_07_PUBLICATION_ROOT
} from "./lib/assessment-production-post-canary-batch-07-publication.mjs";
import {
  validatePostCanaryBatch07PublicationOutput
} from "./lib/assessment-production-post-canary-batch-07-publication-validation.mjs";
import {
  POST_CANARY_BATCH_07_PUBLICATION_RESUMPTION_2_DEBATES,
...
const RESUMPTION_ROOT = POST_CANARY_BATCH_07_PUBLICATION_RESUMPTION_2_ROOT;
...
const REPAIRED_DEBATE_193 = `${ROOT}/repair-1/merged/debate-193.json`;
const DEBATE_193_PACKET = `${ROOT}/packets/debate-193.json`;
...
const DEBATE_80_DIAGNOSIS = `${RESUMPTION_1_ROOT}/repair-1/failure-diagnosis.json`;
const DEBATE_80_REPAIR_PREPARATION = `${RESUMPTION_1_ROOT}/repair-1/execution-preparation-manifest.json`;
const DEBATE_80_REPAIR_ACTIVATION = `${RESUMPTION_1_ROOT}/repair-1/execution-activation.json`;
const DEBATE_80_REPAIR_EXECUTION = `${RESUMPTION_1_ROOT}/repair-1/model-execution.json`;
const DEBATE_80_REPAIR_ANALYSIS = `${RESUMPTION_1_ROOT}/repair-1/analysis.json`;
const DEBATE_80_REPAIR_VALIDATION = `${RESUMPTION_1_ROOT}/repair-1/complete-debate-validation.json`;
const DEBATE_80_REPAIR_MERGE_AUDIT = `${RESUMPTION_1_ROOT}/repair-1/merge-audit.json`;
const REPAIRED_DEBATE_80 = `${RESUMPTION_1_ROOT}/repair-1/merged/debate-80.json`;
const DEBATE_80_PACKET = `${ROOT}/packets/debate-80.json`;
...
const loaded = Object.fromEntries(
  await Promise.all(
    [
      ...
      DEBATE_80_DIAGNOSIS,
      DEBATE_80_REPAIR_PREPARATION,
      DEBATE_80_REPAIR_ACTIVATION,
      DEBATE_80_REPAIR_EXECUTION,
      DEBATE_80_REPAIR_ANALYSIS,
      DEBATE_80_REPAIR_VALIDATION,
      DEBATE_80_REPAIR_MERGE_AUDIT,
      REPAIRED_DEBATE_80,
      DEBATE_80_PACKET
...
const repairedDebate193 = value(REPAIRED_DEBATE_193);
const debate193Packet = value(DEBATE_193_PACKET);
...
const debate80Diagnosis = value(DEBATE_80_DIAGNOSIS);
const debate80RepairPreparation = value(DEBATE_80_REPAIR_PREPARATION);
const debate80RepairActivation = value(DEBATE_80_REPAIR_ACTIVATION);
const debate80RepairExecution = value(DEBATE_80_REPAIR_EXECUTION);
const debate80RepairAnalysis = value(DEBATE_80_REPAIR_ANALYSIS);
const debate80RepairValidation = value(DEBATE_80_REPAIR_VALIDATION);
const debate80RepairMergeAudit = value(DEBATE_80_REPAIR_MERGE_AUDIT);
const repairedDebate80 = value(REPAIRED_DEBATE_80);
const debate80Packet = value(DEBATE_80_PACKET);
...
assertV4(
  originalPreparation.status ===
    ...
    originalActivation.status ===
      "frozen-ten-post-canary-batch-07-publication-contexts-authorized" &&
    originalActivation.contexts?.length === 10,
  "the original Batch 7 publication preparation changed"
);
assertV4(
  originalExecution.status ===
      "post-canary-batch-07-publication-gate-complete-with-failure" &&
    ...
    originalAnalysis.status ===
      "post-canary-batch-07-publication-output-gate-failed" &&
    diagnosis.status ===
      "frozen-diagnosed-batch-07-debate-193-two-field-publication-validation-failure" &&
    diagnosis.preservedExecution?.contextsUnattempted === 9,
  "the preserved original publication failure boundary changed"
);
assertV4(
  repairPreparation.status ===
      "frozen-one-context-batch-07-publication-repair-prepared-not-activated" &&
    repairActivation.status ===
      "frozen-one-context-batch-07-publication-repair-authorized" &&
    ...
    repairAnalysis.authorization?.nineContextResumptionPreparation === true &&
    repairAnalysis.authorization?.nineContextResumptionExecution === false &&
    repairValidation.status === "passed" &&
    repairValidation.repairedDebates?.[0]?.validation?.moves === 19 &&
  ...
  "the accepted Debate 193 repair boundary changed"
);
assertV4(
  validatePostCanaryBatch07PublicationOutput(
      repairedDebate193,
  ...
  "the repaired Debate 193 staging output failed replay"
);
assertV4(
  resumption1Preparation.status ===
      "frozen-nine-untouched-post-canary-batch-07-publication-resumption-contexts-prepared-not-authorized" &&
    resumption1Activation.status ===
      "frozen-nine-untouched-post-canary-batch-07-publication-resumption-contexts-authorized" &&
    resumption1Execution.status ===
      "post-canary-batch-07-publication-resumption-complete-with-failure" &&
    ...
    canonicalJson(resumption1Execution.unattemptedContextIndexes) ===
      canonicalJson([1, 2, 3, 4, 5, 6, 7, 8]) &&
    debate80Diagnosis.status ===
      "frozen-diagnosed-batch-07-debate-80-five-critique-word-overruns" &&
    debate80RepairPreparation.status ===
      "frozen-three-context-batch-07-debate-80-publication-repair-prepared-not-activated" &&
    debate80RepairActivation.status ===
      "frozen-three-context-batch-07-debate-80-publication-repair-authorized" &&
    debate80RepairExecution.status ===
      "batch-07-debate-80-three-context-repair-gate-passed" &&
    debate80RepairAnalysis.status ===
      "batch-07-debate-80-three-packet-repair-and-complete-validation-passed" &&
    debate80RepairValidation.status === "passed" &&
    debate80RepairValidation.authorizedFieldsChanged === 5 &&
    debate80RepairValidation.immutableFieldsChanged === 0 &&
    debate80RepairMergeAudit.status === "passed" &&
    validatePostCanaryBatch07PublicationOutput(repairedDebate80, debate80Packet).status === "passed",
  "the accepted Debate 80 repair or eight-context resumption boundary changed"
);
...
for (
  ...
  resumptionIndex < POST_CANARY_BATCH_07_PUBLICATION_RESUMPTION_2_DEBATES.length;
  resumptionIndex += 1
) {
  const debateNumber =
    POST_CANARY_BATCH_07_PUBLICATION_RESUMPTION_2_DEBATES[resumptionIndex];
  const source = originalPreparation.contexts.find(
    (context) => context.debateNumber === debateNumber
  );
  assertV4(
    source && source.contextIndex === resumptionIndex + 2,
...
assertV4(
  contexts.length === 8 &&
    ...
    resumptionAudioVerifiedMoves === 4,
  "the eight-context resumption coverage changed"
);
...
const newStaticSources = [
  ...
  "scripts/prepare-assessment-production-post-canary-batch-07-publication-resumption-2.mjs",
  "scripts/test-assessment-production-post-canary-batch-07-publication-resumption-2-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-07-publication-resumption-2.mjs",
  "scripts/run-assessment-production-post-canary-batch-07-publication-resumption-2.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-07-publication-resumption-2.mjs"
];
...
const stopRules = {
  acceptedDebate193RepairFailureBlocks: true,
  acceptedDebate80RepairFailureBlocks: true,
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-07-publication-resumption-2-execution-preparation-manifest",
  protocolId: POST_CANARY_BATCH_07_PUBLICATION_RESUMPTION_2_PROTOCOL_ID,
  status:
  ...
  userAuthorization: {
    instruction:
      "The Batch 7 standing authorization permits preparation, activation, execution, validation, analysis, committing, and pushing of the eight untouched score-locked publication-resumption contexts at direct incremental cost $0.",
  ...
  costEstimate: {
    ...
    expectedAggregateModelMinutes: [34, 56],
    expectedAggregateComputeHours: [0.57, 0.93],
  ...
  inputs: {
    ...
    repairedDebate193: REPAIRED_DEBATE_193,
    debate193Packet: DEBATE_193_PACKET,
    ...
    debate80Diagnosis: DEBATE_80_DIAGNOSIS,
    debate80RepairPreparation: DEBATE_80_REPAIR_PREPARATION,
    debate80RepairActivation: DEBATE_80_REPAIR_ACTIVATION,
    debate80RepairExecution: DEBATE_80_REPAIR_EXECUTION,
    debate80RepairAnalysis: DEBATE_80_REPAIR_ANALYSIS,
    repairedDebate80: REPAIRED_DEBATE_80,
    debate80Packet: DEBATE_80_PACKET
  },
  ...
  acceptedDebate193: {
    ...
    packet: DEBATE_193_PACKET,
    output: REPAIRED_DEBATE_193,
  ...
  acceptedDebate80: {
    ...
    packet: DEBATE_80_PACKET,
    output: REPAIRED_DEBATE_80,
    validation: DEBATE_80_REPAIR_VALIDATION,
    mergeAudit: DEBATE_80_REPAIR_MERGE_AUDIT,
    moves: 19,
    critiques: 19,
    exactSourceQuotes: 2,
    overallCommentarySides: 2,
  ...
  isolation: {
    ...
    repairedDebate193OutputUnavailableToResumptionModels: true,
    repairedDebate80OutputUnavailableToResumptionModels: true,
    failedOriginalDebate193OutputUnavailable: true,
  ...
  deterministicValidation: {
    ...
    acceptedDebate193RepairReplayedAtFreeze: true,
    acceptedDebate80RepairReplayedAtFreeze: true,
    eightOriginalPacketsAndSchemasReusedByteForByte: true,
    eightLocalCanonicalSourceChainsReplayedAtFreeze: true,
    originalEightOutputValidationAndProvenancePathsAbsentAtFreeze: true,
  ...
  acceptanceContract: {
    ...
    resumptionMovesRequired: 149,
    cohortMovesRequired: 187,
    resumptionCritiquesRequired: 149,
