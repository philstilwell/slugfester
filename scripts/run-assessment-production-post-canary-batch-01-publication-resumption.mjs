...
const ROOT = POST_CANARY_BATCH_01_PUBLICATION_RESUMPTION_ROOT;
...
assertV4(
  ...
  "the Batch 1 publication resumption is not authorized"
);
assertV4(
  ...
  "the Batch 1 publication resumption controls changed"
);
...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), `batch-01-publication-resumption-${context.debateNumber}-`)
  );
  const codexHome = await mkdtemp(
    path.join(os.tmpdir(), `batch-01-publication-resumption-home-${context.debateNumber}-`)
  );
  ...
  try {
    ...
    process.stdout.write(
      `[batch-01-publication-resumption] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
    ...
    if (
      ...
      try {
        validationSummary = validatePostCanaryBatch01PublicationOutput(
      ...
      const validationRecord = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-01-publication-resumption-validation",
      ...
      const provenanceRecord = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-01-publication-resumption-provenance",
        protocolId: activation.protocolId,
        contextIndex: context.contextIndex,
        originalContextIndex: context.originalContextIndex,
        debateNumber: context.debateNumber,
  ...
  process.stdout.write(
    `[batch-01-publication-resumption] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-01-publication-resumption-model-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "nine-post-canary-batch-01-publication-resumption-contexts-passed"
  ...
  participantJudgmentWasScoreBlind: true,
  ownDebateScoresImmutable: true,
  acceptedDebate31RepairContexts: 7,
  acceptedDebate31RepairedFields: 14,
  meteredApiCostUsd: 0,
  paidServiceCallsThisStage: 0,
  modelAuthoredScores: 0,
  scorePassesExecutedThisStage: 0,
