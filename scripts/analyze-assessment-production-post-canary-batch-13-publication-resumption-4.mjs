...
assertV4(
  execution.contextsPlanned === 4 && execution.retries === 0 &&
  ...
  "four-context resumption execution boundary changed"
);
...
const fourValid = replayed.filter((item) => item.gateAcceptancePassed);
const resumptionPassed =
  execution.status === "four-original-unattempted-batch-13-publication-contexts-passed" &&
  execution.contextsAttempted === 4 && execution.contextsUnattempted === 0 &&
  execution.validContexts === 4 && execution.invalidContexts === 0 &&
  ...
  fourValid.length === 4;
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-13-publication-original-unattempted-context-resumption-analysis",
  protocolId: activation.protocolId,
  status: resumptionPassed
    ? "four-original-unattempted-batch-13-publication-contexts-passed"
    : "four-context-publication-resumption-failed",
  ...
  totals: {
    debates: 4,
    lockedMoves: 79,
    critiques: fourValid.reduce((sum, item) => sum + item.validation.critiques, 0),
    exactSourceQuotes: fourValid.reduce((sum, item) => sum + item.validation.quoteExactSourceMatches, 0),
    overallCommentarySides: fourValid.reduce((sum, item) => sum + item.validation.overallCommentarySides, 0),
    aiExtensionSides: fourValid.reduce((sum, item) => sum + item.validation.aiExtensionSides, 0),
...
if (resumptionPassed) {
  const allDebates = ["26", "190", "87", "20", "70", "30", ...DEBATES];
  ...
  for (const debateNumber of allDebates) {
    ...
    completeReplay.push({
      ...
      source: debateNumber === "87"
        ? "accepted-exceptional-third-level-atomic-shard-recovery"
        : debateNumber === "20"
          ? "accepted-one-shard-two-field-publication-repair"
        : debateNumber === "70"
          ? "accepted-three-shard-five-field-publication-repair"
        : debateNumber === "30"
          ? "accepted-nine-shard-seventeen-field-publication-repair"
        : ["26", "190"].includes(debateNumber)
          ? "accepted-original-first-attempt-before-timeout"
          : "original-first-attempt-four-context-resumption",
      validation
    });
  }
  const completePassed =
    ...
    completeReplay.reduce((sum, item) => sum + item.validation.moves, 0) === 199 &&
    completeReplay.reduce((sum, item) => sum + item.validation.critiques, 0) === 199 &&
  ...
  completeCohort = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-13-complete-publication-cohort-analysis-after-recovery",
    protocolId: activation.protocolId,
    ...
    totals: {
      ...
      debate87TimeoutRecoveryContexts: 3,
      debate87Level2CritiqueRecoveryContexts: 10,
      debate87ExceptionalAtomicRecoveryContexts: 2,
      debate87ExceptionalRecoveredCritiques: 4,
      debate20FieldDisjointRepairContexts: 1,
      debate20CorrectedCritiques: 2,
      debate70FieldDisjointRepairContexts: 3,
      debate70CorrectedCritiques: 5,
      debate30FieldDisjointRepairContexts: 9,
      debate30CorrectedCritiques: 16,
      debate30CorrectedQuotes: 1,
      retries: 0,
      timeoutExtensions: 0,
      modelAuthoredScores: 0,
    ...
    integrity: {
      ...
      fourResumedContextsWereOriginalFirstAttempts: true,
      failedDebate87AtomicShardOutputsPreservedAsFailureEvidence: true,
      acceptedDebate87FieldsRetainedDeterministically: true,
