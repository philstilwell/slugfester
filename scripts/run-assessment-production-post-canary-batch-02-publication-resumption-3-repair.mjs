...
assertV4(
    ...
    activation.authorization?.remainingTwoContextExecution === false &&
  ...
  "the resumption-3 repair execution is not authorized"
);
assertV4(
  ...
  "the resumption-3 repair execution controls changed"
);
...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), `batch-02-resumption-3-repair-${context.contextIndex}-`)
  );
  const codexHome = await mkdtemp(
    path.join(os.tmpdir(), `batch-02-resumption-3-repair-home-${context.contextIndex}-`)
  );
  ...
  try {
    ...
    process.stdout.write(
      `[batch-02-resumption-3-publication-repair] starting index ${context.contextIndex} Debate ${context.debateNumber} ${activation.model.label}/${activation.model.reasoningEffort}\n`
    );
    ...
    if (
      ...
      try {
        validationSummary = validateResumption3RepairOutput(
      ...
      const validation = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-02-publication-resumption-3-repair-validation",
      ...
      const provenance = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-02-publication-resumption-3-repair-provenance",
        protocolId: activation.protocolId,
        debateNumber: context.debateNumber,
        packetIndex: context.packetIndex,
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-publication-resumption-3-repair-model-execution",
  protocolId: activation.protocolId,
  status:
    validContexts === 5
      ? "batch-02-resumption-3-five-context-publication-repair-gate-passed"
      : "batch-02-resumption-3-publication-repair-gate-complete-with-failure",
  gateStartedAt,
  ...
  authorization: {
    ...
    deterministicMergeAndCompleteValidation: validContexts === 5,
    retry: false,
