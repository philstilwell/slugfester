...
const SCRIPT =
  "scripts/preregister-assessment-production-checkpoint-v2.2-inventory-candidate-census-plans.mjs";
const TEST =
  "scripts/test-assessment-production-checkpoint-v2.2-inventory-candidate-census-plan-manifest.mjs";
...
assertV4(
  preparation.schemaVersion ===
    ...
    preparation.totals?.candidates === 332 &&
    preparation.totals?.proCandidates === 175 &&
    preparation.totals?.conCandidates === 157 &&
    ...
    preparation.gateDisposition?.checkpointDiscoveryPassed === true &&
  ...
  "inventory preparation does not authorize plan-execution manifest preparation"
);
assertV4(
...
assertV4(
  JSON.stringify(preparation.scheduling?.inventoryConcurrencyRamp) ===
      JSON.stringify([1, 2]) &&
...
const manifest = {
  ...
  costEstimate: {
    ...
    estimateBasis:
      "The prior validated ten-context candidate-census planner completed within this range at concurrency two. These production checkpoint inputs use the same frozen contract and are no larger than 53,118 copied bytes. ChatGPT-subscription execution has no direct incremental API charge.",
  },
