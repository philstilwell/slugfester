...
const BATCH_SOURCE_PREPARATION =
  "docs/assessment-production/post-canary-continuation-v1/batch-02/source-preparation/preparation-manifest.json";
...
const SCRIPT =
  "scripts/prepare-assessment-production-post-canary-batch-02-inventory-side-packets.mjs";
const PLAN_RUNNER =
...
function makeSidePacket({
  ...
  return {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-02-chronology-fallback-side-selector-packet",
...
async function buildArtifacts({ frozenAt, checkpointCommit }) {
  ...
  assertV4(
    sourcePreparation.schemaVersion ===
      ...
      sourcePreparation.totals.proCandidates === 162 &&
      sourcePreparation.totals.conCandidates === 177 &&
      ...
      sourcePreparation.sourceCompatibility.occurrences[0].debateNumber === "99" &&
    ...
    "Batch 2 source preparation boundary drifted"
  );
  ...
  assertV4(
    ...
    "accepted Batch 2 planner gate drifted"
  );
  assertV4(
    isDeepStrictEqual(
    ...
    "Batch 2 source compatibility boundary drifted"
  );
  ...
  const userInstruction = "I approve.";
  const approvedScope =
    "Prepare, validate, freeze, commit, and push exactly twenty Batch 2 score-blind side-selector packets and their execution-preparation manifest only, with a direct incremental cost cap of $0. Do not execute side-selector or later models, use paid services, perform audio work, derive scores, reconstruct publication, mutate production, or select the next batch.";
  const sidePreparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-02-chronology-fallback-side-packet-preparation",
    protocolId: PROTOCOL_ID,
    sideSelectionProtocolId: CHRONOLOGY_FALLBACK_INVENTORY.protocolId,
    status:
      "twenty-exact-post-canary-batch-02-side-selector-packets-frozen-not-authorized",
    frozenAt,
    checkpointCommit,
    branch: "main",
    ...
    nextRequiredAction:
      "freeze-batch-02-side-selector-execution-preparation-manifest-model-free-only",
  };
  ...
  const executionPreparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-02-side-selector-execution-preparation-manifest",
    protocolId: PROTOCOL_ID,
    sideSelectionProtocolId: CHRONOLOGY_FALLBACK_INVENTORY.protocolId,
    status:
      "frozen-twenty-post-canary-batch-02-side-selector-contexts-prepared-not-authorized",
    frozenAt,
    checkpointCommit,
    branch: "main",
    productionContinuation: true,
    ...
    costEstimate: {
      ...
      estimateBasis:
        "The promoted twenty-context side-selector gate completed in 10.80 wall minutes and 19.74 aggregate model minutes at concurrency two. Batch 2 preserves the same one-side isolated context shape; this manifest records the exact observed packet sizes against the proven 115,000-byte input ceiling. ChatGPT-subscription execution has no direct incremental API charge.",
    },
    ...
    nextAuthorizedAction:
      "user-approval-required-before-batch-02-side-selector-execution-activation-or-any-side-selector-model-execution",
  };
    ...
    JSON.stringify(
      {
        ...
        debates: 10,
        contexts: 20,
        exactSideSchemasFrozen: shouldWrite ? 20 : 0,
        exactSidePacketsFrozen: shouldWrite ? 20 : 0,
        candidatesTransported: EXPECTED_CANDIDATES,
        copiedInputBytes: {
          minimum:
  ...
  assertV4(
    sidePreparation.contexts.length === 20 &&
      ...
      sidePreparation.sourceCompatibility.minimumCandidateLexicalTokensChanged ===
        false &&
      sidePreparation.sourceCompatibility.occurrences.length === 1 &&
      sidePreparation.sourceCompatibility.occurrences[0].debateNumber === "99" &&
      isDeepStrictEqual(
...
if (command === "prepare") await prepare();
  ...
  throw new Error(
    "usage: prepare-assessment-production-post-canary-batch-02-inventory-side-packets.mjs <prepare|validate>"
  );
}
