...
import { V419_ROOT, compileV419PrimaryOutput } from "./lib/v419-schema-bounded-source.mjs";

const root = process.cwd();
const manifest = JSON.parse(await readFile(path.resolve(root, `${V419_ROOT}/primary-execution-manifest.json`), "utf8"));
const codex = "/Applications/ChatGPT.app/Contents/Resources/codex";
const authSource = path.join(os.homedir(), ".codex", "auth.json");
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
assertV4(manifest.status === "frozen-six-context-primary-authorized" && manifest.authorization.primaryModelExecution && !manifest.authorization.passBModelExecution, "v4.1.9 primary execution unauthorized");
...
for (const context of manifest.contexts) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-v419-primary-${context.debateNumber}-`));
  const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), `slugfester-v419-home-primary-${context.debateNumber}-`));
  ...
  try {
    ...
    if (invocation.timedOut || invocation.code !== 0 || invocation.signal !== null) {
      ...
      const validation = await run(process.execPath, ["scripts/validate-v419-primary-output.mjs", context.rawOutput, context.packet], { cwd: root, env: process.env }, 120000);
      ...
      if (valid) {
        const [rawOutput, packet, eventsBytes] = await Promise.all([readFile(path.resolve(root, context.rawOutput), "utf8").then(JSON.parse), readFile(path.resolve(root, context.packet), "utf8").then(JSON.parse), readFile(path.resolve(root, context.events))]);
        const compiled = compileV419PrimaryOutput(rawOutput, packet, JSON.parse(eventsBytes));
  ...
  results.push(record);
  process.stdout.write(`[v4.1.9-primary] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
