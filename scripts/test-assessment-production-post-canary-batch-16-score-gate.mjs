...
import {
  ...
  evaluatePostCanaryBatch16ScoreStability
...
const accepted = evaluatePostCanaryBatch16ScoreStability(passing);
...
assert.equal(
  evaluatePostCanaryBatch16ScoreStability(winnerMutation).acceptancePassed,
  false
);
const distanceMutation = structuredClone(passing);
distanceMutation[2].final = scores(95, 80);
assert.equal(
  evaluatePostCanaryBatch16ScoreStability(distanceMutation).acceptancePassed,
  false
);
assert.throws(() =>
  evaluatePostCanaryBatch16ScoreStability(passing, {
    ...POST_CANARY_BATCH_16_SCORE_STABILITY_THRESHOLDS,
