...
assertV4(
  preparation.protocolId === PROTOCOL_ID &&
    preparation.status === "frozen-eighteen-context-thirty-two-field-batch-11-publication-repair-prepared-not-activated" &&
    preparation.contexts?.length === 18 && preparation.totals?.writableFields === 32 &&
    preparation.diagnosis?.totals?.invalidCritiques === 32 &&
    preparation.diagnosis?.totals?.invalidQuotes === 0 &&
    preparation.contexts.every((context) => context.writableFieldCount <= 2) &&
    new Set(preparation.contexts.flatMap((context) => context.writableFields)).size === 32,
  "eighteen-shard repair is not prepared"
);
assertV4(
  ...
  "eighteen-shard model or execution controls changed"
);
...
const activation = {
  ...preparation,
  schemaVersion: "1.0-assessment-production-post-canary-batch-11-publication-field-disjoint-repair-activation",
  status: "frozen-eighteen-context-thirty-two-field-batch-11-publication-repair-activated",
  activatedAt,
  ...
  nextAuthorizedAction: "execute-only-eighteen-frozen-field-disjoint-repair-contexts"
};
