...
assertV4(a.status === "frozen-post-canary-batch-14-deterministic-publication-compilation-authorized" &&
  ...
  canonicalJson(a.explicitOrder) === canonicalJson(POST_CANARY_BATCH_14_PUBLICATION_COMPILATION_ORDER),
"Batch 14 compilation is not authorized");
...
try {
  ...
  for (const debateNumber of POST_CANARY_BATCH_14_PUBLICATION_COMPILATION_ORDER) {
    ...
    const compiled = compilePostCanaryBatch14PublicationStagingRecord({ output, packet, identity: id });
    const validation = validatePostCanaryBatch14CompiledStagingRecord({ compiled, output, packet, identity: id });
...
if (passed) await writeFile(path.resolve(a.artifacts.compilationAudit), `${JSON.stringify({
  schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-compilation-audit",
...
const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-compilation-execution",
  protocolId: a.protocolId, status: passed ? "ten-debate-batch-14-deterministic-publication-compilation-passed" :
    "ten-debate-batch-14-deterministic-publication-compilation-failed",
...
const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-compilation-analysis",
  protocolId: a.protocolId, status: execution.status, productionCanary: false, batchNumber: 14,
  ...
  nextAuthorizedAction: passed ? "prepare-batch-14-publication-finalization" : "diagnose-compilation-failure" };
