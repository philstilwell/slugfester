...
import {
  buildBatch02TokenCountedChunkLedger,
  findBatch02ZeroLexicalTokenRows,
} from "./lib/assessment-production-post-canary-batch-02-source-preparation.mjs";
import {
...
const BATCH_ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-02";
const ROOT = `${BATCH_ROOT}/source-preparation`;
const PROTOCOL_ID = "assessment-production-post-canary-batch-02-source-preparation";
const CACHE_VERSION = "post-canary-continuation-v1-batch-02";
...
const SCRIPT = "scripts/prepare-assessment-production-post-canary-batch-02-source-packets.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-02-source-packets.mjs";
const REQUIRED_ORDER = ["103", "172", "04", "136", "83", "66", "126", "99", "93", "101"];
const CONTROL_FILES = [
  SELECTION,
  SELECTION_ANALYSIS,
  CONTINUATION_POLICY,
  ...
  "scripts/lib/assessment-production-post-canary-batch-02-source-preparation.mjs",
  SCRIPT,
  TEST,
];
...
assertV4(
  selection.status === "second-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    selectionAnalysis.status === "second-post-canary-batch-selection-analysis-passed-awaiting-source-packet-preparation-decision" &&
    selectionAnalysis.selection.sha256 === sha256(selectionBytes) &&
    selection.nextAuthorizedAction === "user-decision-on-second-post-canary-batch-source-packet-preparation" &&
    JSON.stringify(selection.selected.map((item) => item.debateNumber)) === JSON.stringify(REQUIRED_ORDER),
  "Batch 2 frozen selection is not ready for its separately authorized source-packet step"
);
assertV4(
  ...
  "Batch 2 model boundary or stop rules drifted"
);
assertV4(
...
for (const selected of selection.selected) {
  ...
  packet.schemaVersion = "1.0-assessment-production-post-canary-batch-02-score-blind-source-packet";
  ...
  for (const chunk of plan.chunks) {
    ...
    const tokenLedgerBytes = buildBatch02TokenCountedChunkLedger(chunkBytes);
    const zeroLexicalTokenRows = findBatch02ZeroLexicalTokenRows(chunkBytes);
...
const preparation = {
  ...
  status: shouldWrite ? "post-canary-batch-02-ten-complete-score-blind-source-packets-prepared-awaiting-validation" : "preview",
  ...
  AIOnly: true,
  userAuthorization: {
    scope: "prepare, validate, freeze, commit, and push score-blind source packets for Batch 2 only",
  ...
  tokenLedgerCompatibility: {
    ...
    reason: "One nonempty canonical caption row contains only a redaction marker and therefore has zero repository lexical tokens.",
  ...
  nextAuthorizedAction: "run-batch-02-deterministic-source-packet-validation-model-free-only",
};
