...
const manifest = JSON.parse(await readFile(path.resolve(
  `${POST_CANARY_BATCH_15_RENDERING_ROOT}/preparation-manifest.json`), "utf8"));
const exists = (file) => access(path.resolve(file)).then(() => true, () => false);
assertV4(manifest.status ===
  "frozen-post-canary-batch-15-rendering-verification-prepared" &&
  manifest.protocolId === POST_CANARY_BATCH_15_RENDERING_PROTOCOL_ID &&
  manifest.packets?.length === 10 &&
  canonicalJson(manifest.explicitOrder) ===
    canonicalJson(POST_CANARY_BATCH_15_RENDERING_ORDER) &&
  manifest.gateExpectations?.sections === 51 &&
  manifest.gateExpectations?.moves === 191 &&
...
"Batch 15 rendering preparation changed");
...
for (const row of manifest.packets) {
  ...
  validatePostCanaryBatch15RenderingPacket(JSON.parse(packetBytes));
}
...
console.log(JSON.stringify({ status: "passed", debates: 10,
  sections: 51, moves: 191, packets: 10,
