...
assert.equal(
  ...
  "1.0-assessment-production-post-canary-batch-17-independent-judgment-execution-activation"
);
assert.equal(
  activation.status,
...
assert.equal(activation.batchNumber, 17);
...
assert.equal(activation.contexts.length, 8);
assert.deepEqual(
...
assert.equal(activation.executionPolicy.contexts, 8);
...
assert.equal(activation.acceptanceContract.validContextsRequired, 8);
...
if (!(await exists(activation.artifacts.execution))) {
  ...
  assert.equal(
    ...
    "execute-frozen-post-canary-batch-17-independent-judgment-gate-once"
  );
  console.log(
    JSON.stringify(
      {
...
assert.equal(execution.contextsPlanned, 8);
assert(execution.contextsAttempted >= 1 && execution.contextsAttempted <= 8);
assert.equal(execution.contextsUnattempted, 8 - execution.contextsAttempted);
...
if (
  execution.status ===
  "eight-post-canary-batch-17-independent-judgment-contexts-passed"
) {
  assert.equal(execution.contextsAttempted, 8);
  assert.equal(execution.contextsUnattempted, 0);
  assert.equal(execution.validContexts, 8);
  ...
  assert.equal(
    execution.status,
    "post-canary-batch-17-independent-judgment-gate-complete-with-failure"
  );
  assert(
...
assert.equal(analysis.pairs.length, 4);
...
if (analysis.acceptance.passed) {
  ...
  assert.equal(analysis.acceptance.preparedSourceHashReplays, 8);
  assert.equal(analysis.acceptance.unchangedV4220ValidatorPasses, 8);
  assert.equal(analysis.acceptance.canonicalEventProjectionReplays, 8);
  ...
  assert.equal(analysis.totals.uniqueMoves, 79);
  assert.equal(analysis.totals.movesJudgedAcrossPasses, 158);
  assert.equal(
  ...
  assert.equal(
    analysis.status,
    "post-canary-batch-17-independent-judgment-gate-failed-analysis-only"
  );
}
console.log(
  JSON.stringify(
