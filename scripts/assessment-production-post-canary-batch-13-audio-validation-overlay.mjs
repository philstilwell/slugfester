...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-13/audio-verification";
const paths = {
...
const SCRIPT = "scripts/assessment-production-post-canary-batch-13-audio-validation-overlay.mjs";
const VALIDATOR = "scripts/lib/v416-audio-verification.mjs";
const EXPECTED_CALLS = 8;
const EXPECTED_OVERLAYS = 4;
const EXPECTED_FAILURE = "con-shared-liability-neutrality: segment 2 text invalid";
...
async function loadFrozenRun() {
  ...
  assert.equal(execution.status, "eight-post-canary-batch-13-paid-known-speaker-diarizations-completed");
...
async function inspectTranscripts(activation, execution) {
  ...
  assert.equal(targets.length, EXPECTED_OVERLAYS, "exactly four empty provider segments are required");
...
async function prepare() {
  ...
  const failure = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-13-audio-validation-failure",
    protocolId: activation.protocolId,
    status: "batch-13-audio-validation-failure-preserved-bounded-overlay-recovery-level-1-authorized",
    recordedAt: new Date().toISOString(),
    batchNumber: 13,
    failure: {
      analyzer: "scripts/assessment-production-post-canary-batch-13-audio-verification-stage.mjs",
  ...
  const preparation = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-13-audio-validation-overlay-preparation",
    protocolId: activation.protocolId,
    status: "frozen-four-empty-segment-transient-validation-overlays-ready-for-one-bounded-replay",
    ...
    contract: {
      ...
      paidCalls: 0,
      paidRetries: 0,
      exactEightTranscriptReplayRequired: true,
    },
    authorization: {
    ...
    nextAuthorizedAction: "execute-one-batch-13-eight-transcript-deterministic-overlay-replay",
  };
  if (shouldWrite) {
    await Promise.all([
...
async function run() {
  ...
  assert.equal(preparation.status, "frozen-four-empty-segment-transient-validation-overlays-ready-for-one-bounded-replay");
  ...
  const status = passed
    ? "passed-all-eight-post-canary-batch-13-confidence-moves-audio-verified-after-empty-segment-overlays"
  ...
  const overlayExecution = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-13-audio-validation-overlay-execution",
    protocolId: activation.protocolId,
    status,
