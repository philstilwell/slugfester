...
import {
  buildBatch12TokenCountedChunkLedger,
  findBatch12ZeroLexicalTokenRows,
} from "./lib/assessment-production-post-canary-batch-12-source-preparation.mjs";
import {
...
const PROTOCOL_ID = "assessment-production-post-canary-batch-12-source-preparation";
const CACHE_VERSION = "post-canary-continuation-v1-batch-12";
...
const SCRIPT = "scripts/prepare-assessment-production-post-canary-batch-12-source-packets.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-12-source-packets.mjs";
...
assertV4(
  selection.status === "twelfth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    selectionAnalysis.status === "twelfth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization" &&
    selectionAnalysis.selection.sha256 === sha256(selectionBytes) &&
    selection.nextAuthorizedAction === "prepare-batch-12-standing-authorization-and-source-packets-under-user-authorization" &&
    JSON.stringify(selection.selected.map((item) => item.debateNumber)) === JSON.stringify(REQUIRED_ORDER),
  "Batch 12 frozen selection is not ready for its separately authorized source-packet step"
);
assertV4(
  ...
  "Batch 12 model boundary or stop rules drifted"
);
assertV4(
...
for (const selected of selection.selected) {
  ...
  packet.schemaVersion = "1.0-assessment-production-post-canary-batch-12-score-blind-source-packet";
  ...
  for (const chunk of plan.chunks) {
    ...
    const tokenLedgerBytes = buildBatch12TokenCountedChunkLedger(chunkBytes);
    const zeroLexicalTokenRows = findBatch12ZeroLexicalTokenRows(chunkBytes);
...
const preparation = {
  ...
  status: shouldWrite ? "post-canary-batch-12-ten-complete-score-blind-source-packets-prepared-awaiting-validation" : "preview",
  ...
  AIOnly: true,
  userAuthorization: {
    scope: "prepare, validate, freeze, commit, and push score-blind source packets for Batch 12 under the complete-workflow standing authorization",
  ...
  nextAuthorizedAction: "run-batch-12-deterministic-source-packet-validation-model-free-only",
};
