...
assert.equal(activation.batchNumber, 4);
...
if (analysis.acceptance.passed) {
  ...
  assert.equal(analysis.totals.uniqueMoves, 203);
  assert.equal(analysis.totals.movesJudgedAcrossPasses, 406);
  assert.equal(
  ...
  assert.equal(
    analysis.status,
    "post-canary-batch-04-independent-judgment-gate-failed-analysis-only"
  );
}
console.log(
  JSON.stringify(
