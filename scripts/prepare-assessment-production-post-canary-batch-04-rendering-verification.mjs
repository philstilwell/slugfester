...
import {
  ...
  POST_CANARY_BATCH_04_RENDERING_MODEL,
  POST_CANARY_BATCH_04_RENDERING_ORDER,
  POST_CANARY_BATCH_04_RENDERING_PORT,
  ...
  buildPostCanaryBatch04RenderingPacket
...
const preparationPath = `${POST_CANARY_BATCH_04_RENDERING_ROOT}/preparation-manifest.json`;
...
const FINALIZATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-04/publication-finalization";
...
const PACKETS_ROOT = `${POST_CANARY_BATCH_04_RENDERING_ROOT}/packets`;
const EVIDENCE_ROOT = `${POST_CANARY_BATCH_04_RENDERING_ROOT}/evidence`;
const ACTIVATION = `${POST_CANARY_BATCH_04_RENDERING_ROOT}/execution-activation.json`;
const EXECUTION = `${POST_CANARY_BATCH_04_RENDERING_ROOT}/execution.json`;
const ANALYSIS = `${POST_CANARY_BATCH_04_RENDERING_ROOT}/analysis.json`;
const RENDERING_AUDIT = `${POST_CANARY_BATCH_04_RENDERING_ROOT}/rendering-audit.json`;
const IMAGE_ANALYZER = POST_CANARY_BATCH_04_RENDERING_IMAGE_CONTRACT.analyzerPath;
...
if (shouldWrite) {
  assertV4(
    ...
    "Batch 4 rendering preparation already exists; freeze is immutable"
  );
}
...
assertV4(
  finalizationPreparation.status ===
      "frozen-post-canary-batch-04-publication-finalization-prepared-not-authorized" &&
    ...
    compatibility.status === "batch-04-production-compatibility-boundary-recorded" &&
  ...
  "passing Batch 4 publication finalization required"
);
assertV4(
...
for (const debateNumber of POST_CANARY_BATCH_04_RENDERING_ORDER) {
  ...
  const packet = buildPostCanaryBatch04RenderingPacket({
    auditRow,
...
const directInputPaths = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-04-publication-finalization.mjs",
  "scripts/lib/assessment-production-post-canary-batch-04-standing-authorization.mjs",
  "scripts/lib/assessment-production-post-canary-batch-04-rendering-verification.mjs",
  "scripts/lib/assessment-production-post-canary-batch-04-rendering-browser-runner.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-04-rendering-verification.mjs",
  "scripts/activate-assessment-production-post-canary-batch-04-rendering-verification.mjs",
  "scripts/finalize-assessment-production-post-canary-batch-04-rendering-verification.mjs",
  "scripts/test-assessment-production-post-canary-batch-04-rendering-verification-preparation.mjs",
  "scripts/validate-assessment-production-post-canary-batch-04-rendering-evidence.mjs",
...
for (const file of [...futureOutputPathsExcludedFromSourceHashes, EVIDENCE_ROOT]) {
  assertV4(!(await exists(file)), `future Batch 4 rendering output exists: ${file}`);
}

const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-rendering-verification-preparation",
  protocolId: POST_CANARY_BATCH_04_RENDERING_PROTOCOL_ID,
  status: "frozen-post-canary-batch-04-rendering-verification-prepared-not-authorized",
  frozenAt,
  checkpointCommit,
  ...
  userAuthorization: {
    ...
    scopeInterpretation:
      "Prepare, validate, freeze, commit, and push the Batch 4 rendering-verification plan without opening a candidate page or taking screenshots.",
  ...
  explicitOrder: [...POST_CANARY_BATCH_04_RENDERING_ORDER],
  viewports: structuredClone(POST_CANARY_BATCH_04_RENDERING_VIEWPORTS),
  ...
  browserPlan: {
    ...
    localServer: {
      command: [
        ...
        "-m",
        "http.server",
        String(POST_CANARY_BATCH_04_RENDERING_PORT),
        "--bind",
        "127.0.0.1"
      ],
      workingDirectory: ".",
      port: POST_CANARY_BATCH_04_RENDERING_PORT,
      baseUrl: `http://127.0.0.1:${POST_CANARY_BATCH_04_RENDERING_PORT}`,
    ...
    imageContract: structuredClone(POST_CANARY_BATCH_04_RENDERING_IMAGE_CONTRACT),
  ...
  nextAuthorizedAction:
    "activate-and-execute-frozen-batch-04-rendering-verification-under-standing-authorization"
};
