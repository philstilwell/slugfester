...
const activationPath =
  `${POST_CANARY_BATCH_09_DISPUTE_ADJ_ROOT}/execution-activation.json`;
const executionPath =
  `${POST_CANARY_BATCH_09_DISPUTE_ADJ_ROOT}/model-execution.json`;
...
assertV4(
  activation.status ===
      "frozen-ten-post-canary-batch-09-dispute-only-adjudication-contexts-authorized" &&
  ...
  "Batch 9 adjudication execution is unavailable or crossed its boundary"
);
for (const [file, digest] of Object.entries(activation.sourceHashes)) {
  assertV4(
    sha256(await readFile(file)) === digest,
    `source hash mismatch during Batch 9 adjudication analysis: ${file}`
  );
}
...
for (const context of activation.contexts) {
  ...
  if (result.gateAcceptancePassed) {
    ...
    replay = validatePostCanaryBatch09DisputeAdjudicationOutput(
...
const semanticPass =
  ...
  valid.reduce((sum, context) => sum + context.candidateSelections, 0) ===
    459 &&
  valid.reduce((sum, context) => sum + context.disputedMoves, 0) === 173;
const timingPass =
  semanticPass &&
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-09-dispute-only-adjudication-analysis",
  ...
  status: passed
    ...
    : semanticPass
      ? timingPass
        ? scoreBlindPass
          ? "post-canary-batch-09-dispute-only-adjudication-gate-failed-isolation"
          : "post-canary-batch-09-dispute-only-adjudication-gate-failed-scoreblindness"
        : "post-canary-batch-09-dispute-only-adjudication-gate-failed-timing"
      : "post-canary-batch-09-dispute-only-adjudication-gate-failed-validation",
  ...
  nextAuthorizedAction: passed
    ? "standing-authorization-permits-batch-09-deterministic-final-ledger-assembly"
    : "new-user-approval-required-after-batch-09-adjudication-failure-before-downstream-work"
};
if (shouldWrite) {
