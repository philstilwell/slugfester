...
const scoreArtifactCandidates = [
  `${POST_CANARY_BATCH_02_FINAL_LEDGER_ROOT}/calculated-scores.json`,
  `${POST_CANARY_BATCH_02_FINAL_LEDGER_ROOT}/score-analysis.json`,
  `${POST_CANARY_BATCH_02_FINAL_LEDGER_ROOT}/score-pass-manifest.json`
];
...
const rebuiltLedger = buildPostCanaryBatch02FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
...
assert.deepEqual(
  ...
  ["103", "172", "04", "136", "83", "66", "126", "99", "93", "101"]
);
assert.equal(storedLedger.audit.finalMoves, 190);
assert.equal(storedLedger.audit.disputedMoves, 182);
assert.equal(storedLedger.audit.pairSelections, 207);
assert.equal(storedLedger.audit.scoringFieldSelections, 314);
assert.equal(storedLedger.audit.burdenAdjustmentSelections, 14);
assert.equal(storedLedger.audit.candidateSelections, 535);
assert.equal(storedLedger.audit.meanMerges, 393);
...
assert.throws(() =>
  buildPostCanaryBatch02FinalLedger(mutatedInputs, inputs.sourceHashes)
);
...
assert.equal(analysis.totals.disputedMoves, 182);
assert.equal(analysis.totals.candidateSelections, 535);
