...
const resumptionRoot = `${root}/original-unattempted-context-resumption-1`;
const outputPath = `${root}/outputs/debate-20.json`;
const packetPath = `${root}/packets/debate-20.json`;
const executionPath = `${resumptionRoot}/model-execution.json`;
const validationPath = `${root}/validations/debate-20.json`;
const analysisPath = `${resumptionRoot}/debate-20-failure-analysis.json`;
...
const failed = execution.results.find((item) => item.debateNumber === "20");
assertV4(execution.status === "seven-context-publication-resumption-stopped-with-failure" && execution.contextsAttempted === 1 && execution.validContexts === 0 && execution.contextsUnattempted === 6 && failed?.status === "output-validation-failed" && failed.outputSha256 === sha256(outputBytes) && failed.retryCount === 0, "Debate 20 failure boundary changed");
assertV4(validationRecord.status === "failed" && validationRecord.outputSha256 === sha256(outputBytes), "Debate 20 failed validation record changed");
...
assertV4(structuralStandIn, "Debate 20 has no validation-clean critique stand-in");
...
assertV4(nonTargetValidation.status === "passed", "Debate 20 non-target fields do not validate");
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-13-debate-20-publication-failure-analysis",
  protocolId: "assessment-production-post-canary-batch-13-debate-20-publication-field-disjoint-repair-1",
  status: "debate-20-publication-failure-diagnosed-awaiting-field-disjoint-repair-1",
  debateNumber: "20",
  ...
  authorization: { fieldDisjointRepairLevel1Preparation: true, attemptsPerShard: 1, retries: false, writableFieldsMaximumPerShard: 2, resumeSixOriginalUnattemptedContextsAfterPassingRepair: true, scorePass: false, productionMutation: false, nextBatchSelection: false },
  nextAuthorizedAction: "prepare-minimum-field-disjoint-debate-20-publication-repair-shards"
};
