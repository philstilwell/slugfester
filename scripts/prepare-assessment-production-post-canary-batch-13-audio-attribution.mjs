...
import { MODEL, PACKET_VERSION, PROTOCOL_ID, ROOT, makeSchema, sha256 } from "./lib/assessment-production-post-canary-batch-13-audio-attribution.mjs";
...
const base = "docs/assessment-production/post-canary-continuation-v1/batch-13";
const audioRoot = `${base}/audio-verification`;
const paths = {
  instruction: `${audioRoot}/audio-attribution-recovery-instruction.txt`,
...
assert.equal(instructionBytes.toString("utf8").trim(), "I authorize/approve of all actions necessary to get the job done.");
assert.equal(standing.batchNumber, 13);
assert.equal(analysis.gate.passed, false);
assert.equal(analysis.gate.verified, 2);
assert.equal(analysis.gate.unresolved, 6);
assert.equal(audit.totals.paidDiarizationCallsCompleted, 8);
assert.equal(audit.totals.retries, 0);
assert.equal(cost.costControl.approvedCapExceeded, false);
assert.equal(cost.costControl.usageDerivedEstimatedCostUsd, 0.21423);
...
for (const debateNumber of ["37", "111"]) {
...
assert.equal(contexts.reduce((sum, item) => sum + item.moveIds.length, 0), 6);

const head = execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim();
const authorization = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-13-audio-attribution-successor-authorization",
  protocolId: PROTOCOL_ID,
  status: "frozen-active-batch-13-audio-attribution-recovery-level-2-successor-authorization",
  ...
  userAuthorization: {
    ...
    scopeInterpretation: "Resume the blocked Batch 13 workflow using the smallest zero-cost, evidence-limited recovery that can resolve the six preserved audio-attribution decisions, then continue automatically through the already authorized complete workflow.",
  },
  preservedPriorGate: {
    verified: 2,
    unresolved: 6,
    paidCallsCompleted: 8,
    paidRetries: 0,
    usageDerivedEstimatedCostUsd: 0.21423,
  ...
  sourceHashes,
  nextAuthorizedAction: "prepare-validate-commit-and-push-two-frozen-batch-13-audio-attribution-recovery-contexts",
};
const publicContexts = contexts.map(({ packetBytes, schemaBytes, ...context }) => context);
const preparation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-13-audio-attribution-preparation",
  protocolId: PROTOCOL_ID,
  status: "prepared-two-batch-13-audio-attribution-recovery-contexts-not-active",
  preparedAt,
  checkpointCommit: head,
  productionCanary: false,
  batchNumber: 13,
  ...
  executionPolicy: { concurrency: 2, contexts: 2, attemptsPerContext: 1, retriesMaximum: 0, perInvocationTimeoutMs: 900000, APIKeysRemoved: true, paidTranscriptionCalls: 0, directIncrementalCostUsdMaximum: 0 },
...
console.log(JSON.stringify({ status: shouldWrite ? preparation.status : "preview", contexts: 2, decisions: 6, model: `${MODEL.label}/low`, authentication: MODEL.authentication, attemptsMaximum: 2, retriesMaximum: 0, paidTranscriptionCalls: 0, directIncrementalCostUsdMaximum: 0, scoresDerived: 0 }, null, 2));
