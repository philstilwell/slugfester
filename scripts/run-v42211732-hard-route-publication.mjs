...
import { validateV42211732PublicationOutput, V42211732_ROOT } from "./lib/v42211732-hard-route-publication.mjs";

const manifest = JSON.parse(await readFile(path.resolve(`${V42211732_ROOT}/execution-manifest.json`), "utf8"));
...
async function runContext(context, contextIndex) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `v42211732-publication-${context.debateNumber}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `v42211732-publication-home-${context.debateNumber}-`));
  ...
  try {
    ...
    process.stdout.write(`[v4.2.21.17.32-publication] starting index ${contextIndex} ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber}\n`);
    ...
    if (invocation.timedOut || invocation.code !== 0 || invocation.signal !== null || !resultExists) record = { ...base, status: invocation.timedOut ? "timed-out" : !resultExists ? "result-missing" : "transport-failed", gateAcceptancePassed: false, outputWritten: false, failureMessage: `${invocation.stdout}\n${invocation.stderr}`.trim().slice(-10000) };
    else {
  ...
  process.stdout.write(`[v4.2.21.17.32-publication] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
