...
import { MODEL, ROOT, sha256, validateOutput } from "./lib/assessment-production-post-canary-batch-14-audio-attribution.mjs";

const root = process.cwd();
const manifest = JSON.parse(await readFile(`${ROOT}/execution-manifest.json`, "utf8"));
assert.equal(manifest.status, "frozen-one-batch-14-audio-attribution-recovery-context-active");
...
async function executeContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-b14-audio-adj-${context.debateNumber}-`));
  const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), `slugfester-b14-audio-home-${context.debateNumber}-`));
...
const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-audio-attribution-model-execution", protocolId: manifest.protocolId, status: passed ? "batch-14-audio-attribution-recovery-execution-passed" : "batch-14-audio-attribution-recovery-execution-failed", startedAt, completedAt: new Date().toISOString(), elapsedMs: Date.now() - started, contexts: 1, attempts: 1, retries: 0, directIncrementalCostUsd: 0, paidTranscriptionCalls: 0, scoresDerived: 0, results, authorization: { analysis: passed, furtherRetry: false, paidTranscription: false, disputeAdjudicationPreparation: false, scoreDerivation: false, productionMutation: false, nextBatchSelection: false } };
await writeFile(manifest.artifacts.execution, `${JSON.stringify(execution, null, 2)}\n`);
console.log(JSON.stringify({ status: execution.status, elapsedMinutes: Number((execution.elapsedMs / 60000).toFixed(2)), contexts: 1, attempts: 1, retries: 0, valid: results.filter((item) => item.gateAcceptancePassed).length, directIncrementalCostUsd: 0, paidTranscriptionCalls: 0, scoresDerived: 0 }, null, 2));
assert(passed, "Batch 14 audio-attribution recovery execution failed; downstream work is blocked");
