...
const scoreArtifactCandidates = [
  `${POST_CANARY_BATCH_17_FINAL_LEDGER_ROOT}/calculated-scores.json`,
  `${POST_CANARY_BATCH_17_FINAL_LEDGER_ROOT}/score-analysis.json`,
  `${POST_CANARY_BATCH_17_FINAL_LEDGER_ROOT}/score-pass-manifest.json`
];
...
const rebuiltLedger = buildPostCanaryBatch17FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
...
assert.equal(storedLedger.audit.finalMoves, 79);
assert.equal(storedLedger.audit.disputedMoves, 77);
assert.equal(storedLedger.audit.pairSelections, 80);
assert.equal(storedLedger.audit.scoringFieldSelections, 142);
assert.equal(storedLedger.audit.burdenAdjustmentSelections, 8);
assert.equal(storedLedger.audit.candidateSelections, 230);
assert.equal(storedLedger.audit.meanMerges, 158);
assert.equal(storedLedger.audit.dependencyMeanMergesSuppressed, 0);
assert.equal(storedLedger.audit.audioVerifiedMoves, 1);
assert.equal(storedLedger.audit.acceptedAdjudicationOutputsWithoutCorrection, 4);
...
assert.throws(() =>
  buildPostCanaryBatch17FinalLedger(mutatedInputs, inputs.sourceHashes)
);
