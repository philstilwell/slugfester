...
const identity = JSON.parse(await readFile(path.resolve(
  "docs/assessment-production/post-canary-continuation-v1/batch-09/deterministic-publication-compilation/production-identity-snapshot.json"), "utf8"));
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const exists = (file) => access(path.resolve(file)).then(() => true, () => false);
assertV4(a.status === "frozen-post-canary-batch-09-publication-finalization-authorized" &&
  ...
  canonicalJson(a.explicitOrder) === canonicalJson(POST_CANARY_BATCH_09_PUBLICATION_FINALIZATION_ORDER),
"Batch 9 finalization is not authorized");
...
try {
  ...
  assertV4(candidates.length === 10 && candidates.reduce((sum, row) => sum + row.validation.moves, 0) === 180,
...
if (passed) {
  ...
  const preview = Buffer.from(buildPostCanaryBatch09PublicationStagingPreviewHtml());
  ...
  await writeFile(path.resolve(a.artifacts.audit), `${JSON.stringify({
    schemaVersion: "1.0-assessment-production-post-canary-batch-09-publication-finalization-audit",
    protocolId: a.protocolId, status: "passed", rows,
    totals: { debates: 10, moves: 180, deterministicFinalizationPasses: 1,
...
const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-09-publication-finalization-execution",
  protocolId: a.protocolId, status: passed ? "ten-debate-batch-09-publication-finalization-passed" :
    "ten-debate-batch-09-publication-finalization-failed", startedAt,
...
const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-09-publication-finalization-analysis",
  ...
  nextAuthorizedAction: passed ? "prepare-batch-09-rendering-verification" : "diagnose-finalization-failure" };
