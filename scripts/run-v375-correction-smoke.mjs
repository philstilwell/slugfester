...
import { assert } from "./lib/v375-correction.mjs";
import { V375_EXECUTION_MANIFEST } from "./lib/v375-execution.mjs";

const root = process.cwd(), manifestText = await readFile(path.resolve(root, V375_EXECUTION_MANIFEST), "utf8"), manifest = JSON.parse(manifestText);
...
async function modelTask(context) {
  ...
  const temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-v375-${reviewerPass}-${debateNumber}-`));
  const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), `slugfester-v375-home-${reviewerPass}-${debateNumber}-`));
  ...
  try {
    ...
    process.stdout.write(`\n[v3.7.5] starting ${manifest.model.label} Debate ${debateNumber} ${reviewerPass}\n`);
    const invocation = await run(codex, [
      ...
      "Read workflow.md, rubric.md, manual.md, schema.json, and packet.json completely and no other files. Return exactly one schema-conforming correction output. Process every bundle once and in order. Do not emit scores or commentary outside the JSON object."
    ...
    try {
      ...
      const validation = await run(process.execPath, ["scripts/validate-v375-correction-output.mjs", output, packet, schema], { cwd: root, env: process.env });
      const valid = validation.code === 0;
      process.stdout.write(`[v3.7.5] Debate ${debateNumber} ${reviewerPass} ${valid ? "completed-valid" : "output-validation-failed"}\n`);
...
const initialContexts = ["pass-a", "pass-b"].flatMap((reviewerPass) => manifest.debateNumbers.map((debateNumber) => manifest.initialContexts[reviewerPass][debateNumber]));
const initialStartedAt = new Date().toISOString(), initialResults = await pool(initialContexts.map((context) => () => modelTask(context)));
const initialExecution = executionRecord("3.7.5-initial-correction-execution", initialStartedAt, initialContexts, initialResults);
await writeFile(path.resolve(root, manifest.artifacts.initialExecution), `${JSON.stringify(initialExecution, null, 2)}\n`);
const extraction = await run(process.execPath, ["scripts/extract-v375-correction-disagreements.mjs", "--write"], { cwd: root, env: process.env });
...
if (!disagreement.allInitialValid) adjudicationExecution = executionRecord("3.7.5-adjudication-correction-execution", new Date().toISOString(), [], []);
else {
  const adjudicationStartedAt = new Date().toISOString(), adjudicationResults = await pool(disagreement.adjudicationContexts.map((context) => () => modelTask(context)));
  adjudicationExecution = executionRecord("3.7.5-adjudication-correction-execution", adjudicationStartedAt, disagreement.adjudicationContexts, adjudicationResults);
}
await writeFile(path.resolve(root, manifest.artifacts.adjudicationExecution), `${JSON.stringify(adjudicationExecution, null, 2)}\n`);
const analysisRun = await run(process.execPath, ["scripts/analyze-v375-correction-smoke.mjs", "--write"], { cwd: root, env: process.env });
assert(analysisRun.code === 0, `analysis failed: ${analysisRun.stderr}`);
const analysis = JSON.parse(await readFile(path.resolve(root, manifest.artifacts.analysis), "utf8"));
console.log(JSON.stringify({ status: "correction-smoke-execution-recorded", passed: analysis.passed, initialValidContexts: initialExecution.validOutputContexts, initialAgreements: disagreement.counts.agreements, initialDisagreements: disagreement.counts.disagreements, adjudicationContexts: adjudicationExecution.contextsPlanned, adjudicationValidContexts: adjudicationExecution.validOutputContexts, retries: initialExecution.totalRetries + adjudicationExecution.totalRetries, meteredApiCostUsd: 0 }, null, 2));
