...
const ROOT = POST_CANARY_BATCH_07_PUBLICATION_RESUMPTION_3_ROOT;
...
assertV4(activation.status ===
  "frozen-five-untouched-post-canary-batch-07-publication-resumption-3-contexts-authorized" &&
  ...
  activation.executionPolicy?.contexts === 5 && activation.executionPolicy?.attemptsPerContext === 1 &&
...
"the five-context Batch 7 publication resumption is not authorized");
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-07-publication-resumption-3-${context.debateNumber}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `batch-07-publication-resumption-3-home-${context.debateNumber}-`));
  ...
  try {
    ...
    process.stdout.write(`[batch-07-publication-resumption-3] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber}\n`);
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 ||
      ...
      const validationRecord = { schemaVersion:
          "1.0-assessment-production-post-canary-batch-07-publication-resumption-3-validation",
      ...
      const provenanceRecord = { schemaVersion:
          "1.0-assessment-production-post-canary-batch-07-publication-resumption-3-provenance",
  ...
  process.stdout.write(`[batch-07-publication-resumption-3] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
...
const execution = { schemaVersion:
    "1.0-assessment-production-post-canary-batch-07-publication-resumption-3-model-execution",
  protocolId: activation.protocolId, status: passed
    ? "five-post-canary-batch-07-publication-resumption-3-contexts-passed"
    : "post-canary-batch-07-publication-resumption-3-complete-with-failure",
  gateStartedAt, gateCompletedAt: new Date().toISOString(), contextsPlanned: 5,
  ...
  ownDebateScoresImmutable: true, acceptedPriorDebates: 5,
