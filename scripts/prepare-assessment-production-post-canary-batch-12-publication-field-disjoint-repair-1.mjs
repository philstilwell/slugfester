...
} from "./lib/assessment-production-post-canary-batch-12-publication-field-disjoint-repair-1.mjs";
...
assertV4(
  originalExecution.status === "post-canary-batch-12-publication-gate-complete-with-failure" &&
    originalExecution.contextsAttempted === 1 && originalExecution.contextsUnattempted === 9 &&
  ...
  "original Batch 12 publication failure boundary changed"
);
...
const debate152 = allArtifacts.get("152");
const structuralStandIn = Object.values(debate152.outputJson.moveProse)
  .map((entry) => entry.critique)
  .find((critique) => {
    const words = wordCount(critique);
    return words >= 105 && words <= 130 && critique.length >= 880;
  });
assertV4(structuralStandIn, "Debate 152 has no validation-clean diagnostic stand-in");

const expectedCritiqueCounts = { "152": 3 };
const diagnostics = [];
...
assertV4(diagnostics.reduce((sum, item) => sum + item.invalidCritiques.length, 0) === 3 &&
  diagnostics.reduce((sum, item) => sum + item.invalidQuotes.length, 0) === 0,
"diagnosed three-field repair scope changed");
...
assertV4(contexts.length === 2 &&
  contexts.every((context) => context.writableFieldCount >= 1 && context.writableFieldCount <= 2) &&
  contexts.reduce((sum, context) => sum + context.writableFieldCount, 0) === 3 &&
  new Set(contexts.flatMap((context) => context.writableFields)).size === 3,
"two-shard, three-field coverage changed");

const sourceScripts = [
  "scripts/lib/assessment-production-post-canary-batch-12-publication-field-disjoint-repair-1.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-12-publication-field-disjoint-repair-1.mjs",
  "scripts/activate-assessment-production-post-canary-batch-12-publication-field-disjoint-repair-1.mjs",
  "scripts/run-assessment-production-post-canary-batch-12-publication-field-disjoint-repair-1.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-12-publication-field-disjoint-repair-1.mjs",
...
const manifest = {
  ...
  protocolId: PROTOCOL_ID,
  status: "frozen-two-context-three-field-batch-12-publication-repair-prepared-not-activated",
  frozenAt,
  ...
  userAuthorization: {
    ...
    resolvedScope: "two fresh field-disjoint shards correcting exactly three rejected Debate 152 critiques while retaining every validation-clean field",
  ...
  diagnosis: {
    status: "deterministically-confirmed-three-critique-failures",
    debates: diagnostics,
    totals: { debates: 1, invalidCritiques: 3, invalidQuotes: 0, writableFields: 3 },
    everyNonTargetFieldStructurallyValidated: true,
    temporaryStandInsWrittenOrRetained: false
  },
  diagnosticStandIn: {
    source: "validation-clean-critique-from-the-same-preserved-Debate-152-output",
  ...
  retentionContract: {
    ...
    completeTenDebateCohortReplayDeferredUntilNineUnattemptedContextsComplete: true
  },
  ...
  totals: {
    debates: 1, contexts: 2, critiqueFields: 3, quoteFields: 0, writableFields: 3,
  ...
  nextAuthorizedAction: "commit-and-push-frozen-two-shard-repair-then-activate"
};
