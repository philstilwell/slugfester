...
const preflightRoot = `${V388_ADJUDICATION_ROOT}/schema-preflight`;
...
assertV388(!(await exists(preflightRoot)), `${preflightRoot} already exists`);
const preparation = JSON.parse(await bytes(preparationPath));
assertV388(preparation.status === "passed-dispute-only-adjudication-preparation" && preparation.authorization.freezeAdjudicationExecutionManifest && !preparation.authorization.adjudicationModelExecution, "adjudication preparation invalid");
await access(codex);
...
const temporary = await mkdtemp(path.join(os.tmpdir(), "slugfester-v388-adjudication-preflight-"));
const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), "slugfester-v388-home-adjudication-preflight-"));
let execution;
try {
  ...
  const prompt = "Return one synthetic adjudication JSON object. Use debateNumber 55 and debateId synthetic-schema-preflight. Include exactly one move decision for synthetic-move-1: responseTupleChoice 1, charityPairChoice null, one logicalCoherence rating choice selecting 1, and a rationale of at least 40 characters. Include one pro burden-adjustment decision selecting 1 with a rationale of at least 40 characters. Affirm every required isolation and audit field exactly. Do not include any other fields.";
  ...
  assertV388(invocation.code === 0 && invocation.signal === null, "adjudication schema endpoint preflight failed");
  ...
  const validation = validateV388AdjudicationOutput(output, syntheticPacket);
  execution = {
    schemaVersion: "3.8.8-performance-adjudication-schema-preflight-execution",
    protocolId: "v3.8.8-performance-judgment-consensus",
