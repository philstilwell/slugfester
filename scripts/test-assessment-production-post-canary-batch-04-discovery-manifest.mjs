...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-04/discovery";
...
assert.equal(manifest.status, "frozen-forty-four-post-canary-batch-04-discovery-contexts-prepared-not-authorized");
...
assert.equal(manifest.executionPolicy.copiedInputBytesMaximum, 65995);
assert.equal(preparation.totals.maximumCopiedInputBytes, 65995);
...
assert.equal(manifest.copiedInputBoundary.frozenObservedCopiedInputBytesMaximum, 65995);
assert.equal(manifest.copiedInputBoundary.executionCopiedInputBytesMaximum, 65995);
assert.deepEqual(manifest.copiedInputBoundary.maximumContext, { debateNumber: "81", chunkId: "chunk-003" });
...
for (const context of manifest.contexts) {
  ...
  assert(buildBatch04TokenCountedChunkLedger(chunkBytes).equals(tokenBytes));
...
assert.equal(manifest.nextAuthorizedAction, "prepare-and-freeze-batch-04-discovery-execution-activation-under-standing-authorization");

const validation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-discovery-execution-preparation-validation",
  protocolId: manifest.protocolId,
  status: "batch-04-discovery-execution-manifest-validation-passed-frozen-standing-authorization-active",
