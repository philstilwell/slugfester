...
assert.equal(record.sourcePreparation.frozenDiscoveryContexts, 36);
assert.equal(record.recoveryHistory.sourcePreparationTransportShape.prospectivePatternApplications, 1);
assert.equal(record.recoveryHistory.sourcePreparationTransportShape.failureCorrectionsUsed, 0);
assert.equal(record.recoveryHistory.sourcePreparationTransportShape.recursiveCorrectionsRemaining, 1);
...
assert.equal(
  sha256(await readFile(POST_CANARY_BATCH_10_STANDING_AUTHORIZATION)),
  sha256(bytes)
);
...
console.log(
  JSON.stringify(
    {
      status: "passed-active-batch-10-standing-authorization",
