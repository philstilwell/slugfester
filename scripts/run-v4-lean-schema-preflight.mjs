...
try {
  ...
  const prompt = "Read workflow.md, rubric.md, manual.md, packet.json, schema.json, transcript.txt, and events.json completely and no other files. Act only as the synthetic isolated v4.0 primary judge. Use exactly four sections, select all eight substantive events as eight chronological moves, include one move per side in each section, and exercise the structural contracts without calculating totals or writing publication prose. Return exactly one schema-conforming JSON object and no commentary.";
