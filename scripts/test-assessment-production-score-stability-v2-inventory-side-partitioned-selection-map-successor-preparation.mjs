...
assert.equal(
  preparation.failedGateDisposition.uniqueSelectionSuccessorGatePreservedFailed,
  true
);
assert.equal(
...
assert.equal(preparation.selectionTopology.repositorySideStructurallyPartitioned, true);
assert.equal(preparation.selectionTopology.wrongSideCandidateKeyRepresentable, false);
assert.equal(preparation.selectionTopology.orderWithinSideModelAuthored, false);
assert.equal(
  preparation.selectionTopology.orderWithinSideRepositoryDerivedFromChronology,
  true
);
assert.equal(preparation.selectionTopology.positionCollisionRepresentable, false);
assert.equal(preparation.selectionTopology.preservedRegressionArtifacts, 13);
assert.equal(preparation.selectionTopology.preservedLockedInventoriesIdentical, 13);
assert.equal(preparation.selectionTopology.failedDebate31DuplicateRejected, true);
assert.equal(
  preparation.selectionTopology.failedDebate31CardinalityRejectedAfterOrderRemoval,
  true
);
...
for (const context of preparation.contexts) {
  const [manual, columnarGuide, sideGuide, packet, transport, schemaBytes] =
    await Promise.all([
      readFile(preparation.inputs.manual),
      readFile(preparation.inputs.columnarTransportGuide),
      readFile(preparation.inputs.sidePartitionedSelectionMapGuide),
  ...
  assert.equal(schemaBytes.includes(Buffer.from('"orderWithinSide"')), false);
  ...
  assert.equal(proKeys.length, context.proCandidates);
  assert.equal(conKeys.length, context.conCandidates);
