...
const manifestPath = `${POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_ROOT}/mutation-manifest.json`;
const preparationAnalysisPath = `${POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_ROOT}/preparation-analysis.json`;
const activationPath = `${POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_ROOT}/execution-activation.json`;
const executionPath = `${POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_ROOT}/execution.json`;
const analysisPath = `${POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_ROOT}/analysis.json`;
...
assertV4(
  manifest.status === "frozen-batch-16-production-publication-mutation-manifest-prepared" &&
    manifest.protocolId === POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_PROTOCOL_ID &&
  ...
  "frozen Batch 16 production publication manifest required"
);
assertV4(
  preparationAnalysis.status === "batch-16-production-publication-mutation-manifest-freeze-passed" &&
    preparationAnalysis.manifest.sha256 === sha256(serializedJson(manifest)),
  "accepted Batch 16 production publication preparation required"
);
...
for (const lock of [
  ...
  manifest.scorePolicy.exception,
...
for (const debate of manifest.debates) {
  const productionLedgerExists = await exists(debate.productionLedgerPath);
  assertV4(
    productionLedgerExists === debate.productionLedgerBaseline.exists,
    `${debate.debateNumber}: production ledger baseline existence changed`
  );
  if (productionLedgerExists) {
    assertV4(
      sha256(await readBytes(debate.productionLedgerPath)) ===
        debate.productionLedgerBaseline.sha256,
      `${debate.debateNumber}: production ledger baseline hash changed`
    );
  }
...
assertV4(
  ...
  "proposed production source changes outside Batch 16"
);

const executionTools = await Promise.all([
  "scripts/lib/assessment-production-post-canary-batch-16-production-publication.mjs",
  "scripts/run-assessment-production-post-canary-batch-16-production-publication.mjs",
  "scripts/test-assessment-production-post-canary-batch-16-production-publication-activation.mjs",
  "scripts/validate-debates.mjs",
  "scripts/test-assessment-production-score-stability-policy-active.mjs"
].map(lockFile));
const activation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-16-production-publication-execution-activation",
  protocolId: POST_CANARY_BATCH_16_PRODUCTION_PUBLICATION_PROTOCOL_ID,
  status: "frozen-batch-16-production-publication-mutation-pass-activated",
  activatedAt,
  ...
  nextAuthorizedAction: "execute-one-frozen-batch-16-production-publication-mutation-pass-no-reruns"
};
if (write) {
