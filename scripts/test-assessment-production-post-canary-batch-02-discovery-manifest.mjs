...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-02/discovery";
...
assert.equal(manifest.status, "frozen-thirty-six-post-canary-batch-02-discovery-contexts-prepared-not-authorized");
...
assert.equal(manifest.tokenLedgerCompatibility.occurrences.length, 1);
...
assert.equal(manifest.contexts.length, 36);
assert.deepEqual(manifest.contexts.map((context) => context.contextIndex), Array.from({ length: 36 }, (_, index) => index));
...
assert.equal(manifest.executionPolicy.contexts, 36);
...
assert.equal(manifest.executionPolicy.copiedInputBytesMaximum, 68003);
assert.equal(preparation.totals.maximumCopiedInputBytes, 68003);
...
assert.equal(manifest.copiedInputBoundary.frozenObservedCopiedInputBytesMaximum, 68003);
assert.equal(manifest.copiedInputBoundary.executionCopiedInputBytesMaximum, 68003);
assert.deepEqual(manifest.copiedInputBoundary.maximumContext, { debateNumber: "136", chunkId: "chunk-002" });
...
assert.equal(manifest.costEstimate.contexts, 36);
...
assert.equal(manifest.stageBoundary.discoveryExecutionActivationPreparation, "not-authorized");
assert.equal(manifest.stageBoundary.discoveryModelExecution, "not-authorized");
...
assert.equal(preparedByContext.size, 36);
for (const context of manifest.contexts) {
  ...
  assert(buildBatch02TokenCountedChunkLedger(chunkBytes).equals(tokenBytes));
...
assert.equal(manifest.nextAuthorizedAction, "user-approval-required-before-batch-02-discovery-execution-activation-or-any-model-execution");

const validation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-discovery-execution-preparation-validation",
  protocolId: manifest.protocolId,
  status: "batch-02-discovery-execution-manifest-validation-passed-frozen-awaiting-separate-model-execution-approval",
