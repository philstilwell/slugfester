...
import { POST_CANARY_BATCH_17_PUBLICATION_COMPILATION_PROTOCOL_ID,
  POST_CANARY_BATCH_17_PUBLICATION_COMPILATION_ROOT } from
...
assertV4(p.status === "frozen-post-canary-batch-17-deterministic-publication-compilation-prepared" &&
  p.protocolId === POST_CANARY_BATCH_17_PUBLICATION_COMPILATION_PROTOCOL_ID && p.contexts?.length === 4 &&
...
"Batch 17 compilation is not prepared");
...
const activation = { schemaVersion: "1.0-assessment-production-post-canary-batch-17-publication-compilation-activation",
  protocolId: p.protocolId, status: "frozen-post-canary-batch-17-deterministic-publication-compilation-authorized",
  ...
  nextAuthorizedAction: "execute-one-frozen-batch-17-deterministic-publication-compilation-pass" };
if (shouldWrite) await writeFile(path.resolve(ACTIVATION), `${JSON.stringify(activation, null, 2)}\n`);
console.log(JSON.stringify({ status: activation.status, debates: 4,
