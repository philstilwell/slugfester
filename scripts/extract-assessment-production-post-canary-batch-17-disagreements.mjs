...
import {
  ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_17_DISAGREEMENT_PROTOCOL_ID,
  ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_17_DISAGREEMENT_ROOT,
  extractAssessmentProductionPostCanaryBatch17Disagreements
} from "./lib/assessment-production-post-canary-batch-17-disagreement.mjs";
import {
...
const TOOL_SOURCES = [
  "scripts/extract-assessment-production-post-canary-batch-17-disagreements.mjs",
  "scripts/lib/assessment-production-post-canary-batch-17-disagreement.mjs",
  ...
  "scripts/test-assessment-production-post-canary-batch-17-disagreements.mjs"
];
...
assertV4(
  judgmentAnalysis.status ===
    "eight-post-canary-batch-17-independent-judgments-passed-standing-authorization-active-for-disagreement-extraction" &&
    judgmentAnalysis.authorization.disagreementExtraction === false &&
    judgmentAnalysis.nextAuthorizedAction ===
      "extract-freeze-and-analyze-batch-17-disagreements-under-standing-authorization",
  "Batch 17 accepted-judgment gate or prior stop rule changed"
);
assertV4(
  JSON.stringify(judgmentAnalysis.pairs.map((pair) => pair.debateNumber)) ===
    JSON.stringify(EXPECTED_DEBATES),
  "Batch 17 independent-judgment population changed"
);
assertV4(
  ...
  "Batch 17 does not contain four accepted, independent A/B judgment pairs"
);
assertV4(
  executionPreparation.contexts.length === 8 &&
    executionActivation.executionPolicy.contexts === 8 &&
    modelExecution.contextsPlanned === 8 &&
    modelExecution.contextsAttempted === 8 &&
    modelExecution.validContexts === 8 &&
    modelExecution.invalidContexts === 0,
  "Batch 17 eight-context accepted execution contract changed"
);
assertV4(
  ...
  "Batch 17 model, authentication, score-blindness, or tie contract changed"
);
assertV4(
  executionActivation.executionPolicy.attemptsPerContext === 1 &&
    ...
    JSON.stringify(executionActivation.executionPolicy.schedulerRamp) ===
      JSON.stringify([1, 2]),
  "Batch 17 one-attempt or frozen 1-to-2 scheduler stop rule changed"
);
assertV4(
...
assertV4(
  ...
  "Batch 17 source-compatibility evidence changed"
);
assertV4(
  modelExecution.results.length === 8 &&
  ...
  "Batch 17 accepted execution evidence changed"
);
...
const sourceHashes = {
  ...
  [POST_CANARY_BATCH_17_STANDING_AUTHORIZATION]: standingAuthorization.sha256
};
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const disagreementPath =
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_17_DISAGREEMENT_ROOT}/disagreements/debate-${debateNumber}.json`;
...
assertV4(totalMoves === 79, "Batch 17 extraction must compare 79 moves");
assertV4(
  JSON.stringify(extractedAudioQueue.map(queueKey).sort()) ===
    JSON.stringify(independentGateQueue.map(queueKey).sort()),
  "Batch 17 frozen audio queue was not reproduced"
);

const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-17-disagreement-extraction",
  protocolId:
    ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_17_DISAGREEMENT_PROTOCOL_ID,
  status:
    "post-canary-batch-17-deterministic-disagreements-extracted-standing-authorization-active-for-audio-work",
  ...
  inputBoundary: {
    substantiveInputs:
      "eight-accepted-independent-judgment-raw-outputs-only",
  ...
  sourceAcceptance: {
    ...
    acceptedContexts: 8,
    rejectedContexts: 0,
    acceptedOutputHashReplays: 8,
    acceptedValidationHashReplays: 8,
    acceptedProvenanceHashReplays: 8,
    unchangedV4220ValidatorPasses: 8,
  ...
  nextAuthorizedAction:
    "prepare-freeze-and-analyze-batch-17-local-audio-source-work-items-under-standing-authorization"
};

if (shouldWrite) {
  await mkdir(ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_17_DISAGREEMENT_ROOT, {
    recursive: true
  });
  await writeFile(
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_17_DISAGREEMENT_ROOT}/analysis.json`,
