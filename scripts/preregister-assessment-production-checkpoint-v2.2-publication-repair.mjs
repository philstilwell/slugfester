...
const preparationPath = `${CHECKPOINT_V22_REPAIR_ROOT}/preparation-manifest.json`;
...
assertV4(
  preparation.status === "one-isolated-two-field-debate-50-publication-repair-prepared-and-frozen" &&
    preparation.protocolId === CHECKPOINT_V22_REPAIR_PROTOCOL_ID &&
    preparation.context.writableFields.length === 2 &&
    preparation.policy.attemptsPerContext === 1 &&
    preparation.policy.retriesMaximum === 0 &&
    preparation.totals.modelAuthoredScores === 0,
  "repair preparation mismatch"
);
...
const sourceFiles = [
  ...
  "scripts/lib/assessment-production-checkpoint-v2.2-publication-repair.mjs",
  "scripts/prepare-assessment-production-checkpoint-v2.2-publication-repair.mjs",
  "scripts/preregister-assessment-production-checkpoint-v2.2-publication-repair.mjs",
  "scripts/run-assessment-production-checkpoint-v2.2-publication-repair.mjs",
  "scripts/analyze-assessment-production-checkpoint-v2.2-publication-repair.mjs",
  "scripts/test-assessment-production-checkpoint-v2.2-publication-repair.mjs"
];
...
const execution = `${CHECKPOINT_V22_REPAIR_ROOT}/model-execution.json`;
const analysis = `${CHECKPOINT_V22_REPAIR_ROOT}/analysis.json`;
const futureOutputPathsExcludedFromSourceHashes = [
  preparation.context.repairOutput,
  preparation.context.mergedOutput,
  preparation.context.validation,
  preparation.context.completeValidation,
  preparation.context.provenance,
  preparation.context.mergeAudit,
  execution,
  analysis
];
...
const activation = {
  schemaVersion: "1.0-production-checkpoint-v2.2-publication-repair-execution-activation",
  protocolId: CHECKPOINT_V22_REPAIR_PROTOCOL_ID,
  status: "frozen-one-isolated-two-field-debate-50-publication-repair-authorized",
  frozenAt,
  ...
  modelInputs: {
    ...
    packet: preparation.context.packet,
    schema: preparation.context.schema
  },
  context: preparation.context,
  executionPolicy: {
    ...preparation.policy,
    removedEnvironmentVariables: ["OPENAI_API_KEY", "OPENAI_ORG_ID", "CODEX_API_KEY"]
  },
  isolation: {
    oneDebatePerContext: true,
    onlyTwoDiagnosedCritiquesWritable: true,
  ...
  authorization: {
    repairModelContext: true,
    deterministicRepairValidation: true,
    deterministicMergeAndCompleteDebateValidationIfRepairPasses: true,
    retry: false,
  ...
  artifacts: {
    ...
    mergedOutput: preparation.context.mergedOutput,
    validation: preparation.context.validation,
    completeValidation: preparation.context.completeValidation,
    provenance: preparation.context.provenance,
    mergeAudit: preparation.context.mergeAudit
  },
...
console.log(JSON.stringify({
  ...
  codexCliVersion,
  expectedWallMinutes: activation.costEstimate.expectedWallMinutes,
  meteredApiCostUsdMaximum: 0,
  productionMutation: false
}, null, 2));
