...
import { POST_CANARY_BATCH_08_DEBATE_08_REPAIR_ROOT, mergeAndValidateDebate08Repair } from
  "./lib/assessment-production-post-canary-batch-08-publication-resumption-repair.mjs";
import { assertV4 } from "./lib/v4-lean-production.mjs";
const shouldWrite = process.argv.includes("--write"); const ROOT = POST_CANARY_BATCH_08_DEBATE_08_REPAIR_ROOT;
...
assertV4(e.contextsPlanned === 1 && e.attempts === e.contextsAttempted && e.retries === 0 &&
...
if (e.contextsAttempted === 1 && e.validContexts === 1 && e.invalidContexts === 0 &&
  ...
  try { const baseBytes = await readFile(path.resolve(p.inputs.immutableRejectedOutput));
    ...
    merge = mergeAndValidateDebate08Repair({ baseOutput: JSON.parse(baseBytes),
...
const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-08-debate-08-repair-analysis",
  protocolId: a.protocolId, status: passed
    ? "batch-08-debate-08-one-packet-repair-and-complete-validation-passed"
    : "batch-08-debate-08-repair-or-complete-validation-failed",
  productionCanary: false, batchNumber: 8, stagingOnly: true,
  gate: { contextsPlanned: 1, contextsAttempted: e.contextsAttempted,
    contextsPassed: e.validContexts, contextsFailed: e.invalidContexts,
    correctedFields: 2, completeValidationPassed: passed,
  ...
  authorization: { sixContextResumptionManifestPreparation: passed,
  ...
  nextAuthorizedAction: passed ? "prepare-and-resume-exactly-six-unattempted-publication-contexts"
    : "stop-after-failed-debate-08-repair" };
if (shouldWrite && passed) { const mergedBytes = Buffer.from(`${JSON.stringify(merge.merged, null, 2)}\n`);
  const validation = { schemaVersion: "1.0-assessment-production-post-canary-batch-08-debate-08-complete-validation",
    protocolId: a.protocolId, status: "passed", debateNumber: "08",
  ...
  const audit = { schemaVersion: "1.0-assessment-production-post-canary-batch-08-debate-08-repair-merge-audit",
    protocolId: a.protocolId, status: "passed", debateNumber: "08",
    ...
    authorizedTransformations: merge.transformations, authorizedFieldsChanged: 2,
...
console.log(JSON.stringify({ status: analysis.status, contextsAttempted: e.contextsAttempted,
  ...
  correctedFields: 2, movesValidated: analysis.gate.movesValidated,
