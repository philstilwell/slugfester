...
assertV4(
  preparation.protocolId === PROTOCOL_ID &&
    preparation.status === "frozen-one-context-two-field-batch-13-debate-20-publication-repair-prepared-not-activated" &&
    preparation.contexts?.length === 1 && preparation.totals?.writableFields === 2 &&
    preparation.diagnosis?.totals?.invalidCritiques === 2 &&
    preparation.diagnosis?.totals?.invalidQuotes === 0 &&
    preparation.contexts.every((context) => context.writableFieldCount <= 2) &&
    new Set(preparation.contexts.flatMap((context) => context.writableFields)).size === 2,
  "one-shard Debate 20 repair is not prepared"
);
assertV4(
  preparation.model?.slug === "gpt-5.6-sol" &&
    ...
    canonicalJson(preparation.executionPolicy?.schedulerRamp) === canonicalJson([1]),
  "one-shard model or execution controls changed"
);
...
const activation = {
  ...
  status: "frozen-one-context-two-field-batch-13-debate-20-publication-repair-activated",
  activatedAt,
  ...
  nextAuthorizedAction: "execute-only-one-frozen-debate-20-field-disjoint-repair-context"
};
