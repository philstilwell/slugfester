...
assert.equal(preparation.protocolId, POST_CANARY_BATCH_04_RENDERING_PROTOCOL_ID);
assert.equal(
  preparation.status,
  "frozen-post-canary-batch-04-rendering-verification-prepared-not-authorized"
);
...
assert.deepEqual(preparation.explicitOrder, [...POST_CANARY_BATCH_04_RENDERING_ORDER]);
...
assert.equal(
  ...
  canonicalJson(POST_CANARY_BATCH_04_RENDERING_VIEWPORTS)
);
assert.equal(
...
assert.equal(preparation.browserPlan.localServer.port, POST_CANARY_BATCH_04_RENDERING_PORT);
...
assert.equal(preparation.gateExpectations.moves, 203);
...
assert.equal(
  preparation.toolHashes[POST_CANARY_BATCH_04_RENDERING_IMAGE_CONTRACT.analyzerPath],
...
for (const row of preparation.packets) {
  ...
  const packet = validatePostCanaryBatch04RenderingPacket(JSON.parse(bytes));
  ...
  assert.equal(packet.preview.url.includes(`127.0.0.1:${POST_CANARY_BATCH_04_RENDERING_PORT}`), true);
...
assert.equal(moveTotal, 203);

const previewHtml = await readFile(path.resolve(preparation.inputs.localPreview), "utf8");
assert.match(previewHtml, /validated post-canary Batch 4 candidate only/);
...
console.log(
  JSON.stringify(
    {
      status: "post-canary-batch-04-rendering-preparation-passed",
