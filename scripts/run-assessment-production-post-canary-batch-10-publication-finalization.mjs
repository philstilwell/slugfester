...
assertV4(a.status === "frozen-post-canary-batch-10-publication-finalization-authorized" &&
  ...
  canonicalJson(a.explicitOrder) === canonicalJson(POST_CANARY_BATCH_10_PUBLICATION_FINALIZATION_ORDER),
"Batch 10 finalization is not authorized");
...
if (passed) {
  ...
  const preview = Buffer.from(buildPostCanaryBatch10PublicationStagingPreviewHtml());
  ...
  await writeFile(path.resolve(a.artifacts.audit), `${JSON.stringify({
    schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-finalization-audit",
...
const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-finalization-execution",
  protocolId: a.protocolId, status: passed ? "ten-debate-batch-10-publication-finalization-passed" :
    "ten-debate-batch-10-publication-finalization-failed", startedAt,
...
const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-finalization-analysis",
  ...
  nextAuthorizedAction: passed ? "prepare-batch-10-rendering-verification" : "diagnose-finalization-failure" };
