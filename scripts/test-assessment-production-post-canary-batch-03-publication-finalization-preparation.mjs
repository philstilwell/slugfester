...
assert.equal(manifest.aggregateExpectations.sections, 53);
assert.equal(manifest.aggregateExpectations.moves, 200);
...
assert.equal(replayedMoves, 200);
...
const preview = buildPostCanaryBatch03PublicationStagingPreviewHtml();
assert.match(preview, /noindex,nofollow/);
assert.match(preview, /validated post-canary Batch 3 candidate only/);
