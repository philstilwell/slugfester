...
import { POST_CANARY_BATCH_07_DEBATE_80_REPAIR_ROOT, validateDebate80RepairOutput } from
...
assertV4(activation.status === "frozen-three-context-batch-07-debate-80-publication-repair-authorized" &&
  activation.contexts?.length === 3 && activation.authorization?.repairModelExecution === true &&
...
"the Debate 80 repair execution is not authorized");
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-07-debate-80-repair-${context.contextIndex}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `batch-07-debate-80-repair-home-${context.contextIndex}-`));
  ...
  try { const copies = [[activation.modelInputs.productionWorkflow, "production-workflow.md"],
    ...
    const prompt = ["Read production-workflow.md, readiness-workflow.md, output-contract.md, manual.md, packet.json, and schema.json completely and no other files.",
      `Act only as isolated Debate 80 bounded repair editor ${context.packetIndex}.`,
    ...
    process.stdout.write(`[batch-07-debate-80-repair] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort}\n`);
    ...
    const base = { contextIndex: context.contextIndex, packetIndex: context.packetIndex,
      debateNumber: "80", debateId: context.debateId, model: activation.model.label,
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 || invocation.signal !== null || !resultExists) {
      ...
      try { validationSummary = validateDebate80RepairOutput(JSON.parse(outputBytes),
      ...
      const validation = { schemaVersion: "1.0-assessment-production-post-canary-batch-07-debate-80-repair-validation",
        ...
        debateNumber: "80", outputSha256: sha256(outputBytes), validationSummary,
        validationMessage, modelAuthoredScores: 0 };
      const provenance = { schemaVersion: "1.0-assessment-production-post-canary-batch-07-debate-80-repair-provenance",
        protocolId: activation.protocolId, contextIndex: context.contextIndex,
        packetIndex: context.packetIndex, debateNumber: "80", model: activation.model,
    ...
    debateNumber: "80", attemptCount: 1, retryCount: 0, timeoutExtensionCount: 0,
  ...
  process.stdout.write(`[batch-07-debate-80-repair] index ${context.contextIndex} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`); return record;
}
...
const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-07-debate-80-repair-execution",
  protocolId: activation.protocolId, status: validContexts === 3
    ? "batch-07-debate-80-three-context-repair-gate-passed"
    : "batch-07-debate-80-repair-gate-complete-with-failure",
  gateStartedAt, gateCompletedAt: new Date().toISOString(), contextsPlanned: 3,
  contextsAttempted: results.length, contextsUnattempted: 3 - results.length,
