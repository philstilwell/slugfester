...
assert.equal(
  execution.state.manualHttpRequests,
  execution.state.rangeHttpGetInvocations + execution.state.redirectsFollowed
);
assert(execution.state.manualHttpRequests >= 15);
assert(execution.state.manualHttpRequests <= 60);
assert.equal(
  execution.state.headerParityHttpRequests,
  execution.state.manualHttpRequests
);
assert.equal(execution.state.requestHeaderFieldsPerHttpRequest, 4);
assert.equal(execution.state.finalHttp206Responses, 15);
assert(execution.state.redirectsFollowed >= 0);
assert(execution.state.redirectsFollowed <= 45);
assert.equal(execution.state.repeatedByteRanges, 0);
...
assert.equal(preparation.recovery.headerParityFormat139Downloads, 3);
assert.equal(preparation.recovery.requestHeaderFieldsPerHttpRequest, 4);
assert.equal(
  preparation.recovery.headerParityHttpRequests,
  execution.state.manualHttpRequests
);
assert.equal(preparation.recovery.logicalRangeRequests, 15);
assert.equal(
  preparation.recovery.manualHttpRequests,
  execution.state.manualHttpRequests
);
assert.equal(preparation.recovery.finalHttp206Responses, 15);
assert.equal(
  preparation.recovery.redirectsFollowed,
  execution.state.redirectsFollowed
);
assert.equal(preparation.recovery.repeatedByteRanges, 0);
...
assert(downloaded.every((source) => source.requestHeaderParityApplied === true));
assert(downloaded.every((source) =>
  JSON.stringify(source.requestHeaderFieldNames) === JSON.stringify([
    "User-Agent", "Accept", "Accept-Language", "Sec-Fetch-Mode"
  ])
));
assert.deepEqual(downloaded.map((source) => source.rangeRequests.length), [5, 3, 7]);
assert(downloaded.every((source) => source.rangeRequests.every(
  (range) => range.redirectCount >= 0 && range.redirectCount <= 3
)));
assert(downloaded.every((source) => source.rangeRequests.every((range) => {
  const finalHop = range.hops.at(-1);
  return finalHop?.status === 206 &&
    range.hops.slice(0, -1).every((hop) =>
      hop.status === 302 &&
      hop.redirectDestinationProtocol === "https:" &&
      hop.redirectDestinationHostClass === "googlevideo.com"
    );
})));
assert(downloaded.every((source) => {
  const keys = source.rangeRequests.map((range) => `${range.start}-${range.end}`);
  return new Set(keys).size === keys.length &&
    source.rangeRequests.every((range, index, ranges) =>
      range.start === (index === 0 ? 0 : ranges[index - 1].end + 1) &&
      range.bytes === range.end - range.start + 1
    );
}));
assert.equal(
  downloaded.flatMap((source) => source.rangeRequests).length,
  15
);
assert.equal(preparation.acquisitionPolicy.redirectsPerRangeMaximum, 3);
assert.equal(
  preparation.acquisitionPolicy.redirectDestinationProtocolRequired,
  "https:"
);
assert.equal(
  preparation.acquisitionPolicy.redirectDestinationHostnameSuffixRequired,
  ".googlevideo.com"
);
assert.deepEqual(
  preparation.acquisitionPolicy.requestHeaderParity.fieldNames,
  ["User-Agent", "Accept", "Accept-Language", "Sec-Fetch-Mode"]
);
assert.equal(
  preparation.acquisitionPolicy.requestHeaderParity.exactValuesAuthenticatedByRecoveryPlan,
  true
);
...
console.log(JSON.stringify({
  ...
  manualRedirectDownloads: downloaded.length,
  retries: 0,
