...
import {
  BATCH_05_DEPENDENT_PILOT_CORRECTION_PROTOCOL_ID,
  BATCH_05_DEPENDENT_PILOT_CORRECTION_ROOT,
  BATCH_05_DEPENDENT_PILOT_OUTPUT_PATHS,
  jsonLeafChanges,
  runIsolatedBatch05DependentPilotAnalysis
} from "./lib/assessment-production-post-canary-batch-05-dependent-pilot-analysis-correction.mjs";
import {
...
import { serializedJson, sha256 } from "./lib/assessment-production-post-canary-batch-05-production-publication.mjs";
...
const seoPlanPath = `${POST_CANARY_BATCH_05_GENERATED_SEO_CORRECTION_ROOT}/correction-plan.json`;
const seoActivationPath = `${POST_CANARY_BATCH_05_GENERATED_SEO_CORRECTION_ROOT}/execution-activation.json`;
const planPath = `${BATCH_05_DEPENDENT_PILOT_CORRECTION_ROOT}/correction-plan.json`;
const analysisPath = `${BATCH_05_DEPENDENT_PILOT_CORRECTION_ROOT}/preparation-analysis.json`;
const activationPath = `${BATCH_05_DEPENDENT_PILOT_CORRECTION_ROOT}/execution-activation.json`;
const executionPath = `${BATCH_05_DEPENDENT_PILOT_CORRECTION_ROOT}/execution.json`;
const executionAnalysisPath = `${BATCH_05_DEPENDENT_PILOT_CORRECTION_ROOT}/analysis.json`;
...
assertV4(
  diagnosis.status === "frozen-batch-05-generated-seo-full-check-dependent-pilot-analysis-staleness-diagnosed" &&
    diagnosis.recoveryBoundary.writablePathsMaximum === 2 &&
    seoActivation.status === "frozen-batch-05-generated-seo-correction-pass-activated" &&
    seoActivation.plan.sha256 === sha256(await readBytes(seoPlanPath)),
  "accepted Batch 5 generated SEO validation-failure diagnosis required"
);
...
const baselineOutputs = await Promise.all(BATCH_05_DEPENDENT_PILOT_OUTPUT_PATHS.map(lockFile));
const isolated = await runIsolatedBatch05DependentPilotAnalysis(root);
...
const affectedNumbers = new Set(["05", "189"]);
...
assertV4(
  ...
  "pilot JSON changed outside aggregate legacy comparison and Debates 05/189 rows"
);

const toolPaths = [
  "scripts/lib/assessment-production-post-canary-batch-05-dependent-pilot-analysis-correction.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-05-dependent-pilot-analysis-correction.mjs",
  "scripts/test-assessment-production-post-canary-batch-05-dependent-pilot-analysis-correction-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-05-dependent-pilot-analysis-correction.mjs",
  "scripts/test-assessment-production-post-canary-batch-05-dependent-pilot-analysis-correction-activation.mjs",
  "scripts/run-assessment-production-post-canary-batch-05-dependent-pilot-analysis-correction.mjs"
];
const plan = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-05-dependent-pilot-analysis-correction-plan",
  protocolId: BATCH_05_DEPENDENT_PILOT_CORRECTION_PROTOCOL_ID,
  status: "frozen-batch-05-dependent-pilot-analysis-two-file-correction-plan-prepared",
  frozenAt,
  batchNumber: 4,
  ...
  protectedProduction: {
    ...
    generatedOutputs: await Promise.all(seoPlan.inventory.map(async (record) => {
      ...
      return { ...actual, role: seoPlan.executionContract.writeOnlyProposedPaths.includes(record.path) ? "accepted-batch-05-derivative" : "preserved-unrelated-derivative" };
    }))
  },
  ...
  correctionContract: {
    ...
    writablePaths: BATCH_05_DEPENDENT_PILOT_OUTPUT_PATHS,
  ...
  nextAuthorizedAction: "activate-and-execute-one-frozen-batch-05-dependent-pilot-analysis-correction"
};
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-05-dependent-pilot-analysis-correction-preparation-analysis",
  protocolId: BATCH_05_DEPENDENT_PILOT_CORRECTION_PROTOCOL_ID,
  status: "batch-05-dependent-pilot-analysis-correction-plan-freeze-passed",
  analyzedAt: frozenAt,
...
if (shouldWrite) {
  ...
  await mkdir(resolve(BATCH_05_DEPENDENT_PILOT_CORRECTION_ROOT), { recursive: true });
