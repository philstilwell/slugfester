...
const root = POST_CANARY_BATCH_02_COMPATIBILITY_ROOT;
...
const validator = await import(
  `./validate-debates.mjs?batch02Compatibility=${activation.validator.proposedSha256}`
);
...
for (const lock of activation.packetHashes) {
  ...
  const routeLock =
    validator.validatePostCanaryBatch02LedgerAdapterRouteLocks({
      debate: candidate,
      ledgerText,
  ...
  const route = validator.validatePostCanaryBatch02LedgerAdapterRoute({
    debate: candidate,
    ledger,
    ledgerText
  });
  assert.equal(route.repositoryScoreReplayPassed, true);
  const adapter = validatePostCanaryBatch02SiteLedgerAdapter({
    adapter: ledger,
    candidate,
  ...
  JSON.stringify(
    {
      ...
      positiveRoutes,
      negativeControls,
      existingAdapterRoutesPreserved: 2,
      modelContexts: 0,
      paidServiceCalls: 0,
