...
const manifestPath = `${POST_CANARY_BATCH_10_PRODUCTION_PUBLICATION_ROOT}/mutation-manifest.json`;
const preparationAnalysisPath = `${POST_CANARY_BATCH_10_PRODUCTION_PUBLICATION_ROOT}/preparation-analysis.json`;
const activationPath = `${POST_CANARY_BATCH_10_PRODUCTION_PUBLICATION_ROOT}/execution-activation.json`;
const executionPath = `${POST_CANARY_BATCH_10_PRODUCTION_PUBLICATION_ROOT}/execution.json`;
const analysisPath = `${POST_CANARY_BATCH_10_PRODUCTION_PUBLICATION_ROOT}/analysis.json`;
...
assertV4(
  manifest.status === "frozen-batch-10-production-publication-mutation-manifest-prepared" &&
    manifest.protocolId === POST_CANARY_BATCH_10_PRODUCTION_PUBLICATION_PROTOCOL_ID &&
  ...
  "frozen Batch 10 production publication manifest required"
);
assertV4(
  preparationAnalysis.status === "batch-10-production-publication-mutation-manifest-freeze-passed" &&
    preparationAnalysis.manifest.sha256 === sha256(serializedJson(manifest)),
  "accepted Batch 10 production publication preparation required"
);
...
assertV4(
  ...
  "proposed production source changes outside Batch 10"
);

const executionTools = await Promise.all([
  "scripts/lib/assessment-production-post-canary-batch-10-production-publication.mjs",
  "scripts/run-assessment-production-post-canary-batch-10-production-publication.mjs",
  "scripts/test-assessment-production-post-canary-batch-10-production-publication-activation.mjs",
  "scripts/validate-debates.mjs",
  "scripts/test-assessment-production-score-stability-policy-active.mjs"
].map(lockFile));
const activation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-10-production-publication-execution-activation",
  protocolId: POST_CANARY_BATCH_10_PRODUCTION_PUBLICATION_PROTOCOL_ID,
  status: "frozen-batch-10-production-publication-mutation-pass-activated",
  activatedAt,
  ...
  nextAuthorizedAction: "execute-one-frozen-batch-10-production-publication-mutation-pass-no-reruns"
};
if (write) {
