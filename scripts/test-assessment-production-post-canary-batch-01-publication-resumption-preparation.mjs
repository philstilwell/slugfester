...
const MANIFEST =
  `${POST_CANARY_BATCH_01_PUBLICATION_RESUMPTION_ROOT}/execution-preparation-manifest.json`;
...
assert.equal(manifest.totals.acceptedMoves, 14);
assert.equal(manifest.totals.resumptionContexts, 9);
assert.equal(manifest.totals.resumptionMoves, 163);
...
assert.equal(manifest.totals.cohortMoves, 177);
...
for (const context of manifest.contexts) {
  ...
  assert.deepEqual(schema, buildPostCanaryBatch01PublicationSchema(packet));
...
assert.equal(moves, 163);
assert.equal(sections, 46);
assert.equal(audioVerifiedMoves, 3);

const [repairedBytes, debate31PacketBytes, completeValidation] =
  await Promise.all([
    readFile(manifest.acceptedDebate31.output),
    readFile(manifest.acceptedDebate31.packet),
    readFile(manifest.acceptedDebate31.validation, "utf8").then(JSON.parse)
  ]);
const repairedValidation = validatePostCanaryBatch01PublicationOutput(
  JSON.parse(repairedBytes),
  JSON.parse(debate31PacketBytes)
);
assert.equal(repairedValidation.status, "passed");
assert.equal(repairedValidation.moves, 14);
assert.equal(repairedValidation.critiques, 14);
