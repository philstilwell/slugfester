...
import {
  POST_CANARY_BATCH_14_DISPUTE_ADJ_PROTOCOL_ID,
  POST_CANARY_BATCH_14_DISPUTE_ADJ_ROOT,
  buildPostCanaryBatch14DisputeAdjudicationPacket,
  makePostCanaryBatch14DisputeAdjudicationSchema
} from "./lib/assessment-production-post-canary-batch-14-dispute-adjudication.mjs";
...
const USER_AUTHORIZATION = Object.freeze({
  instruction:
    "The frozen Batch 14 standing authorization permits automatic preparation, validation, freezing, activation, execution, analysis, committing, and pushing of score-blind dispute-only adjudication checkpoints for exactly the ten selected debates while every frozen gate passes, with subscription-backed and local direct incremental cost capped at $0.",
...
const inputPaths = {
  ...
  schema: `${POST_CANARY_BATCH_14_DISPUTE_ADJ_ROOT}/adjudication.schema.json`
};
const TOOL_SOURCES = [
  "scripts/lib/v4-lean-production.mjs",
  "scripts/lib/v4221175-decomposed-adjudication.mjs",
  "scripts/lib/v42211728-hard-route-adjudication.mjs",
  "scripts/lib/assessment-production-post-canary-batch-14-dispute-adjudication.mjs",
  "scripts/build-assessment-production-post-canary-batch-14-dispute-adjudication-packets.mjs",
  "scripts/test-assessment-production-post-canary-batch-14-dispute-adjudication-packets.mjs",
  "scripts/validate-assessment-production-post-canary-batch-14-dispute-adjudication-output.mjs",
  "scripts/preregister-assessment-production-post-canary-batch-14-dispute-adjudication.mjs",
  "scripts/test-assessment-production-post-canary-batch-14-dispute-adjudication-manifest.mjs"
];
...
const audioRecoveryDebate12Path = `${audioRecoveryRoot}/outputs/debate-12.json`;
const judgmentPreparationPath = `${JUDGMENT_ROOT}/execution-preparation-manifest.json`;
const judgmentExecutionPath =
  `${JUDGMENT_ROOT}/model-execution.json`;
...
assertV4(
  sourceAnalysis.status ===
      "post-canary-batch-14-deterministic-disagreements-extracted-standing-authorization-active-for-audio-work" &&
    JSON.stringify(sourceAnalysis.debates.map((item) => item.debateNumber)) ===
      JSON.stringify(EXPECTED_DEBATES) &&
    sourceAnalysis.adjudicationWorkload.disputedMoves === 181 &&
    sourceAnalysis.adjudicationWorkload.candidateSelections === 539 &&
    sourceAnalysis.adjudicationWorkload.packetsPrepared === false &&
    sourceAnalysis.adjudicationWorkload.modelContextsExecuted === 0,
  "Batch 14 deterministic disagreement workload changed"
);
assertV4(
  judgmentExecution.status ===
      "twenty-post-canary-batch-14-independent-judgment-contexts-passed" &&
    ...
    judgmentAnalysis.status ===
      "twenty-post-canary-batch-14-independent-judgments-passed-standing-authorization-active-for-disagreement-extraction" &&
    judgmentAnalysis.totals.contexts === 20,
  "Batch 14 accepted independent-judgment boundary changed"
);
assertV4(
  ...
  "Batch 14 independent-judgment configuration changed"
);
...
assertV4(
  standingAuthorization.status ===
      "frozen-active-batch-14-complete-remaining-workflow-standing-authorization" &&
...
assertV4(
  audioAudit.status ===
    ...
    audioCost.costControl.usageDerivedEstimatedCostUsd === 0.422005 &&
    audioCost.costControl.approvedMaximumCostUsd === 1 &&
    audioCost.costControl.directIncrementalCostCapControlPassed === true &&
    audioExecutionManifest.calls.length === 12 &&
    audioModelExecution.callsAttempted === 12 &&
    audioModelExecution.callsCompleted === 12 &&
    audioModelExecution.retries === 0 &&
    audioRecoveryCombined.status === "post-canary-batch-14-combined-audio-verification-passed" &&
    audioRecoveryCombined.totals.requiredMoves === 12 &&
    audioRecoveryCombined.totals.verified === 12 &&
    audioRecoveryCombined.totals.unresolved === 0 &&
    audioRecoveryAnalysis.status === "batch-14-audio-attribution-recovery-passed" &&
    audioRecoveryAnalysis.validation.verified === 1 &&
    audioRecoveryAnalysis.validation.unresolved === 0,
  "Batch 14 accepted audio-verification boundary changed"
);
assertV4(
  ...
  "Batch 14 packet-preparation authorization changed"
);
...
assertV4(audioByDebateMove.size === 12, "Batch 14 verified audio population changed");

const schema = makePostCanaryBatch14DisputeAdjudicationSchema();
...
if (shouldWrite) {
  const immutableOutputs = [
    inputPaths.schema,
    `${POST_CANARY_BATCH_14_DISPUTE_ADJ_ROOT}/preparation-manifest.json`,
    `${POST_CANARY_BATCH_14_DISPUTE_ADJ_ROOT}/execution-preparation-manifest.json`,
    ...EXPECTED_DEBATES.flatMap((debateNumber) => [
      `${POST_CANARY_BATCH_14_DISPUTE_ADJ_ROOT}/packets/debate-${debateNumber}.json`,
      `${POST_CANARY_BATCH_14_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`
    ])
  ];
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const built = buildPostCanaryBatch14DisputeAdjudicationPacket(
  ...
  const provenancePath =
    `${POST_CANARY_BATCH_14_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`;
  const outputPath =
    `${POST_CANARY_BATCH_14_DISPUTE_ADJ_ROOT}/outputs/debate-${debateNumber}.json`;
  const packetBytes = pretty(built.packet);
  const provenanceBytes = pretty({
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-14-adjudication-candidate-provenance",
...
const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-14-dispute-only-adjudication-preparation",
  protocolId: POST_CANARY_BATCH_14_DISPUTE_ADJ_PROTOCOL_ID,
  status: shouldWrite
    ? "prepared-ten-isolated-post-canary-batch-14-dispute-only-adjudication-contexts"
    : "preview",
  frozenAt,
  ...
  acceptedSourceBoundary: {
    ...
    allTwelveAudioMovesVerified: true,
    audioValidationOverlaysPreserved: 1,
    audioAttributionRecoveryContextsPreserved: 1,
    audioCorrectionCallsPreserved: 0,
    audioUsageDerivedEstimatedCostUsd: 0.422005,
  ...
  nextAuthorizedAction:
    "freeze-post-canary-batch-14-dispute-only-adjudication-execution-preparation-manifest-model-free-only"
};
assertV4(
  contexts.length === 10 &&
    disputedMoves === 181 &&
    disputedMoves === sourceAnalysis.adjudicationWorkload.disputedMoves &&
    candidateSelections === 539 &&
    candidateSelections === sourceAnalysis.adjudicationWorkload.candidateSelections &&
    audioVerifiedMoves === 12,
  `Batch 14 adjudication packet workload differs from frozen inputs: ${JSON.stringify({ contexts: contexts.length, disputedMoves, candidateSelections, audioVerifiedMoves, sourceDisputedMoves: sourceAnalysis.adjudicationWorkload.disputedMoves, sourceCandidateSelections: sourceAnalysis.adjudicationWorkload.candidateSelections })}`
);
assertV4(
  preparation.totals.maximumCopiedInputBytes <= 400000,
  `Batch 14 context ${preparation.totals.maximumCopiedInputBytes} bytes exceeds the frozen 400 KB correction ceiling`
);
if (shouldWrite) {
  await mkdir(POST_CANARY_BATCH_14_DISPUTE_ADJ_ROOT, { recursive: true });
