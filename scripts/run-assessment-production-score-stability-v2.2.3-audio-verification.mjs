...
assertV4(
  ...
  "v2.2.3 paid audio execution unauthorized"
);
...
for (const call of manifest.calls) {
  ...
  process.stdout.write(`[v2.2.3-audio] starting ${call.debateNumber}:${call.moveId}\n`);
  ...
  results.push(result);
  process.stdout.write(`[v2.2.3-audio] ${call.debateNumber}:${call.moveId} ${result.status}\n`);
}

const attempted = results.filter((result) => result.attemptCount === 1);
const completed = results.filter((result) => result.status === "completed");
const execution = {
  schemaVersion: "1.0-score-stability-v2.2.3-audio-verification-model-execution",
  protocolId: manifest.protocolId,
  status: completed.length === manifest.calls.length
    ? "four-paid-known-speaker-diarizations-completed"
    : "v2.2.3-paid-diarization-incomplete",
