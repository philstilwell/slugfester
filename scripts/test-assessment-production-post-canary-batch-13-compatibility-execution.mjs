...
const root = POST_CANARY_BATCH_13_COMPATIBILITY_ROOT;
...
const validator = await import(
  `./validate-debates.mjs?batch13Compatibility=${activation.validator.proposedSha256}`
);
...
for (const lock of activation.packetHashes) {
  ...
  const routeLock =
    validator.validatePostCanaryBatch13LedgerAdapterRouteLocks({
      debate: candidate,
      ledgerText,
  ...
  const route = validator.validatePostCanaryBatch13LedgerAdapterRoute({
    debate: candidate,
    ledger,
    ledgerText
  });
  assert.equal(route.repositoryScoreReplayPassed, true);
  const adapter = validatePostCanaryBatch13SiteLedgerAdapter({
    adapter: ledger,
    candidate,
...
assert.equal(oneSidedDisplayRows, 13);
assert.equal(emptyReferenceLinks, 44);
assert.equal(suppliedReferenceLinks, 8);
assert.equal(
