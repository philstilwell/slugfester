...
import {
  ...
  evaluatePostCanaryBatch07ScoreStability
...
const accepted = evaluatePostCanaryBatch07ScoreStability(passing);
...
assert.equal(
  evaluatePostCanaryBatch07ScoreStability(winnerMutation).acceptancePassed,
  false
);
const distanceMutation = structuredClone(passing);
distanceMutation[2].final = scores(95, 80);
assert.equal(
  evaluatePostCanaryBatch07ScoreStability(distanceMutation).acceptancePassed,
  false
);
assert.throws(() =>
  evaluatePostCanaryBatch07ScoreStability(passing, {
    ...POST_CANARY_BATCH_07_SCORE_STABILITY_THRESHOLDS,
