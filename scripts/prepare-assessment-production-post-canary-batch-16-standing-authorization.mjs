...
import {
  ...
  POST_CANARY_BATCH_16_STANDING_AUTHORIZATION_INSTRUCTION_PATH,
  POST_CANARY_BATCH_16_STANDING_AUTHORIZATION_PROTOCOL_ID,
  POST_CANARY_BATCH_16_STANDING_AUTHORIZATION_STATUS,
  validatePostCanaryBatch16StandingAuthorization
...
const SELECTION_FREEZE_COMMIT = "055ef8bcb28264a4c22a167a0eeaa83f47a47ab4";
const SOURCE_PACKET_COMMIT = "055ef8bcb28264a4c22a167a0eeaa83f47a47ab4";
...
const SOURCE_FILES = [
  ...
  "scripts/prepare-assessment-production-post-canary-batch-16-standing-authorization.mjs",
  "scripts/test-assessment-production-post-canary-batch-16-standing-authorization.mjs"
];
const REQUIRED_ORDER = ["16", "108", "164", "144", "76", "41", "92", "139", "163", "161"];
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const exists = (file) => access(path.resolve(file)).then(() => true, () => false);

assertV4(
  !(await exists(POST_CANARY_BATCH_16_STANDING_AUTHORIZATION)),
  `${POST_CANARY_BATCH_16_STANDING_AUTHORIZATION} already exists`
);
...
assertV4(
  selection.status ===
    ...
    selection.batchNumber === 16 &&
    selection.checkpointCommit ===
      "a6f27248d55f8dea4856ac4234d4b31dcc0c528c" &&
    ...
    analysis.status ===
      "sixteenth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization" &&
  ...
  "the frozen Batch 16 selection or source-packet boundary changed"
);
assertV4(
  execFileSync("git", ["show", `${SELECTION_FREEZE_COMMIT}:${SELECTION}`]).equals(selectionBytes),
  "Batch 16 selection differs from the user-authorized selection commit"
);
...
assertV4(
  ...
  "Batch 16 standing authorization must freeze at the pushed source-packet commit"
);
...
const record = {
  ...
  protocolId: POST_CANARY_BATCH_16_STANDING_AUTHORIZATION_PROTOCOL_ID,
  status: POST_CANARY_BATCH_16_STANDING_AUTHORIZATION_STATUS,
  authorizedAt,
  ...
  userAuthorization: {
    instruction: POST_CANARY_BATCH_16_STANDING_AUTHORIZATION_INSTRUCTION,
    ...
    scopeInterpretation: "Create and use a complete-workflow Batch 16 standing authorization for the ten frozen selections, beginning with the separately frozen score-blind source packets, including bounded diagnosed recovery and production publication, while preserving the established campaign controls and stopping before Batch 17 selection.",
  ...
  stopRules: {
    ...
    outsideBatchSixteenDebateBlocks: true,
    ...
    batchSeventeenSelectionBlocks: true,
  ...
  nextAuthorizedAction:
    "prepare-validate-freeze-commit-and-push-batch-16-discovery-execution-manifest"
};
validatePostCanaryBatch16StandingAuthorization(record);
if (shouldWrite) {
  await writeFile(
