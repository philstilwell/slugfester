...
assertV4(activation.status ===
  ...
  activation.contexts?.length === 6 && activation.authorization?.publicationModelExecution === true &&
  ...
  activation.authorization?.paidServices === false && activation.model?.slug === "gpt-5.6-sol" &&
...
"the Batch 4 publication resumption-2 is not authorized");
for (const [file, digest] of Object.entries(activation.sourceHashes)) {
  assertV4(sha256(await readFile(path.resolve(file))) === digest,
    `resumption-2 source hash mismatch: ${file}`);
}
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(),
    `batch-04-publication-resumption-2-${context.debateNumber}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(),
    `batch-04-publication-resumption-2-home-${context.debateNumber}-`));
  ...
  try {
    ...
    process.stdout.write(`[batch-04-publication-resumption-2] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber}\n`);
    ...
    const base = { contextIndex: context.contextIndex,
      ...
      resumption1ContextIndex: context.resumption1ContextIndex,
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 ||
      ...
      try { validationSummary = validatePostCanaryBatch04PublicationOutput(
      ...
      const validation = { schemaVersion:
        "1.0-assessment-production-post-canary-batch-04-publication-resumption-2-validation",
      ...
      const provenance = { schemaVersion:
        "1.0-assessment-production-post-canary-batch-04-publication-resumption-2-provenance",
  ...
  process.stdout.write(`[batch-04-publication-resumption-2] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
...
for (const phase of activation.executionPolicy.rampPhases) {
  ...
  const validContextIndexes = phaseResults.filter((result) => result.gateAcceptancePassed)
    .map((result) => result.contextIndex);
  const passed = validContextIndexes.length === phase.contextIndexes.length;
  rampPhases.push({ ...phase, attemptedContextIndexes: phaseResults.map((result) => result.contextIndex),
...
const execution = { schemaVersion:
  "1.0-assessment-production-post-canary-batch-04-publication-resumption-2-model-execution",
  protocolId: activation.protocolId,
  status: results.length === 6 && validContexts === 6 ?
    "six-post-canary-batch-04-publication-resumption-2-contexts-passed" :
    "post-canary-batch-04-publication-resumption-2-complete-with-failure",
  ...
  ownDebateScoresImmutable: true, acceptedCohortDebates: 4,
  meteredApiCostUsd: 0, paidServiceCallsThisStage: 0,
