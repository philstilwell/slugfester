...
import {
  POST_CANARY_BATCH_06_PUBLICATION_COMPILATION_ORDER,
  POST_CANARY_BATCH_06_PUBLICATION_COMPILATION_PROTOCOL_ID,
  POST_CANARY_BATCH_06_PUBLICATION_COMPILATION_ROOT,
  compilePostCanaryBatch06PublicationStagingRecord,
  validatePostCanaryBatch06CompiledStagingRecord
} from "./lib/assessment-production-post-canary-batch-06-publication-compilation.mjs";
import { POST_CANARY_BATCH_06_PUBLICATION_ROOT } from
  "./lib/assessment-production-post-canary-batch-06-publication.mjs";
import { validatePostCanaryBatch06PublicationOutput } from
  "./lib/assessment-production-post-canary-batch-06-publication-validation.mjs";
...
const ROOT = POST_CANARY_BATCH_06_PUBLICATION_COMPILATION_ROOT;
const COHORT = `${POST_CANARY_BATCH_06_PUBLICATION_ROOT}/repair-1/ten-debate-cohort-replay.json`;
...
const REPAIRED = new Set(["141", "168", "135", "143"]);
const acceptedOutput = (debateNumber) => REPAIRED.has(debateNumber)
  ? `${POST_CANARY_BATCH_06_PUBLICATION_ROOT}/repair-1/merged/debate-${debateNumber}.json`
  : `${POST_CANARY_BATCH_06_PUBLICATION_ROOT}/outputs/debate-${debateNumber}.json`;
...
assertV4(cohort.status === "passed" && cohort.debates?.length === 10 &&
  cohort.totals?.moves === 200 && cohort.totals?.critiques === 200 && cohort.totals?.quotes === 20,
"the accepted Batch 6 publication cohort changed");

const productionByNumber = new Map(debates.map((debate) => [debate.number, debate]));
const identities = [];
const contexts = [];
for (const debateNumber of POST_CANARY_BATCH_06_PUBLICATION_COMPILATION_ORDER) {
  ...
  const compiled = compilePostCanaryBatch06PublicationStagingRecord({ output, packet, identity });
  const compiledValidation = validatePostCanaryBatch06CompiledStagingRecord({ compiled, output, packet, identity });
...
assertV4(contexts.length === 10 && contexts.reduce((sum, row) => sum + row.expectedMoves, 0) === 200,
"the Batch 6 compilation cohort changed");
const identitySnapshot = { schemaVersion: "1.0-assessment-production-post-canary-batch-06-publication-compilation-identity-snapshot",
  protocolId: POST_CANARY_BATCH_06_PUBLICATION_COMPILATION_PROTOCOL_ID,
...
const sources = [COHORT, POST_CANARY_BATCH_06_STANDING_AUTHORIZATION,
  ...
  "scripts/lib/assessment-production-post-canary-batch-06-publication-compilation.mjs",
  "scripts/lib/assessment-production-post-canary-batch-06-standing-authorization.mjs",
  "scripts/lib/v4-lean-production.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-06-publication-compilation.mjs",
  "scripts/test-assessment-production-post-canary-batch-06-publication-compilation-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-06-publication-compilation.mjs",
  "scripts/run-assessment-production-post-canary-batch-06-publication-compilation.mjs",
...
const manifest = { schemaVersion: "1.0-assessment-production-post-canary-batch-06-deterministic-publication-compilation-preparation",
  protocolId: POST_CANARY_BATCH_06_PUBLICATION_COMPILATION_PROTOCOL_ID,
  status: "frozen-post-canary-batch-06-deterministic-publication-compilation-prepared",
  ...
  explicitOrder: POST_CANARY_BATCH_06_PUBLICATION_COMPILATION_ORDER, contexts,
  ...
  nextAuthorizedAction: "activate-one-batch-06-deterministic-publication-compilation-pass" };
...
console.log(JSON.stringify({ status: manifest.status, debates: 10, moves: 200,
