...
import { V374_MANIFEST, assert } from "./lib/v374-disjoint-atomic.mjs";

const root = process.cwd();
const manifestText = await readFile(path.resolve(root, V374_MANIFEST), "utf8");
...
assert(manifest.status === "frozen-before-model-execution" && manifest.executionAuthorizedByThisPreregistration, "gate manifest does not authorize this test");
...
async function modelTask(context) {
  ...
  const temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-v374-${reviewerPass}-${debateNumber}-`));
  const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), `slugfester-v374-home-${reviewerPass}-${debateNumber}-`));
  ...
  try {
    ...
    process.stdout.write(`\n[v3.7.4] starting ${manifest.model.label} Debate ${debateNumber} ${reviewerPass}\n`);
    ...
    try {
      ...
      const validation = await run(process.execPath, ["scripts/validate-v374-atomic-output.mjs", output, packet, schema], { cwd: root, env: process.env });
      const valid = validation.code === 0;
      process.stdout.write(`[v3.7.4] Debate ${debateNumber} ${reviewerPass} ${valid ? "completed-valid" : "output-validation-failed"}\n`);
...
const initialExecution = executionRecord("3.7.4-initial-atomic-execution", initialStartedAt, initialContexts, initialResults);
await writeFile(path.resolve(root, manifest.artifacts.initialExecution), `${JSON.stringify(initialExecution, null, 2)}\n`);
const extraction = await run(process.execPath, ["scripts/extract-v374-disagreements.mjs", "--write"], { cwd: root, env: process.env });
...
if (!disagreement.allInitialValid) adjudicationExecution = executionRecord("3.7.4-adjudication-atomic-execution", new Date().toISOString(), [], []);
else {
  ...
  adjudicationExecution = executionRecord("3.7.4-adjudication-atomic-execution", adjudicationStartedAt, disagreement.adjudicationContexts, adjudicationResults);
}
await writeFile(path.resolve(root, manifest.artifacts.adjudicationExecution), `${JSON.stringify(adjudicationExecution, null, 2)}\n`);
const analysisRun = await run(process.execPath, ["scripts/analyze-v374-disjoint-atomic-test.mjs", "--write"], { cwd: root, env: process.env });
...
console.log(JSON.stringify({ status: "disjoint-retired-atomic-execution-recorded", passed: analysis.passed, initialValidContexts: initialExecution.validOutputContexts, initialAgreements: disagreement.counts.agreements, initialDisagreements: disagreement.counts.disagreements, adjudicationContexts: adjudicationExecution.contextsPlanned, adjudicationValidContexts: adjudicationExecution.validOutputContexts, retries: initialExecution.totalRetries + adjudicationExecution.totalRetries, meteredApiCostUsd: 0 }, null, 2));
