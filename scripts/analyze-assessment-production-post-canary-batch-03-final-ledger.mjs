...
const passed =
  ...
  validation.finalMoves === 200 &&
  validation.disputedMoves === 190 &&
  validation.candidateSelections === 586 &&
  roundedMeanPopulation === 419 &&
  validation.audioVerifiedMoves === 8 &&
  validation.acceptedAdjudicationOutputsWithoutCorrection === 8 &&
  validation.calculatedScores === 0 &&
  validation.scoreDerivationAuthorized === false;
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-final-ledger-analysis",
  protocolId: ledger.protocolId,
  status: passed
    ? "post-canary-batch-03-deterministic-final-ledger-gate-passed"
    : "post-canary-batch-03-deterministic-final-ledger-gate-failed",
  analyzedAt,
  ...
  nextAuthorizedAction: passed
    ...
    : "failure-recovery-standing-authorization-permits-batch-03-final-ledger-diagnosis"
};
if (shouldWrite) {
  await writeFile(
