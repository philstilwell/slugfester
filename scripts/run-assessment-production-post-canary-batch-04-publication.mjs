...
} from "./lib/assessment-production-post-canary-batch-04-publication-validation.mjs";
...
assertV4(
  ...
  "Batch 4 publication execution is not authorized"
);
assertV4(
  ...
  "Batch 4 publication execution controls changed"
);
...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), `batch-04-publication-${context.debateNumber}-`)
  );
  const codexHome = await mkdtemp(
    path.join(os.tmpdir(), `batch-04-publication-home-${context.debateNumber}-`)
  );
  ...
  try {
    ...
    process.stdout.write(
      `[batch-04-publication] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
    ...
    if (
      ...
      const validationRecord = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-04-publication-validation",
      ...
      const provenanceRecord = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-04-publication-provenance",
        protocolId: activation.protocolId,
        debateNumber: context.debateNumber,
        debateId: context.debateId,
  ...
  process.stdout.write(
    `[batch-04-publication] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-publication-model-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "ten-post-canary-batch-04-publication-contexts-passed"
