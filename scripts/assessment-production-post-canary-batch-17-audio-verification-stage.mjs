...
const stageRoot = "docs/assessment-production/post-canary-continuation-v1/batch-17/audio-verification";
const paths = {
...
const expectedDebates = ["77"];
const expectedMoves = [
  "con-strongest-naturalist-audience-standard"
];
const expectedCalls = 1;
const durationOnlyPlanningEstimateUsd = 0.016676;
const usageDerivedPlanningEstimateUsd = 0.060675;
const toolPath = "scripts/assessment-production-post-canary-batch-17-audio-verification-stage.mjs";
...
async function activate() {
  ...
  const activation = {
    ...preparation,
    schemaVersion: "1.0-assessment-production-post-canary-batch-17-audio-verification-execution-manifest",
    status: "frozen-one-post-canary-batch-17-paid-known-speaker-diarization-authorized-under-standing-authorization",
    activatedAt,
    ...
    nextAuthorizedAction: "execute-exactly-one-post-canary-batch-17-paid-audio-verification-call-once"
  };
...
async function validateActivation(manifest) {
  assert.equal(
    manifest.status,
    "frozen-one-post-canary-batch-17-paid-known-speaker-diarization-authorized-under-standing-authorization"
  );
...
async function run() {
  ...
  for (const [callIndex, call] of manifest.calls.entries()) {
    ...
    process.stdout.write(`[batch-17-audio] ${callIndex + 1}/${expectedCalls} starting ${call.debateNumber}:${call.moveId}\n`);
    ...
    results.push(result);
    process.stdout.write(`[batch-17-audio] ${callIndex + 1}/${expectedCalls} ${result.status}; cumulative $${result.cumulativeUsageDerivedEstimatedCostUsd.toFixed(7)}\n`);
  }
  ...
  const execution = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-17-audio-verification-model-execution",
    protocolId: manifest.protocolId,
    status: completed.length === expectedCalls
      ? "one-post-canary-batch-17-paid-known-speaker-diarization-completed"
      : "post-canary-batch-17-paid-diarization-incomplete",
    provider: "OpenAI Transcription API",
...
async function analyze() {
  ...
  const status = passed
    ? "passed-all-one-post-canary-batch-17-confidence-move-audio-verified"
    : executionComplete
      ? "post-canary-batch-17-audio-verification-unresolved"
      : "post-canary-batch-17-audio-verification-incomplete";
  const nextAuthorizedAction = execution.costCapReachedOrExceeded || !execution.directIncrementalCostCapControlPassed
    ? "user-review-required-before-any-batch-17-downstream-work-after-audio-cost-cap-event"
    : passed
      ? "prepare-freeze-and-push-batch-17-dispute-only-adjudication-packets-under-standing-authorization"
      : "standing-authorization-stop-new-approval-required-before-batch-17-audio-verification-failure-diagnosis";
  const authorization = {
  ...
  const audit = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-17-audio-verification-audit",
    protocolId: manifest.protocolId,
    status,
  ...
  const analysis = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-17-audio-verification-analysis",
    protocolId: manifest.protocolId,
    status,
    productionCanary: false,
    batchNumber: 17,
...
async function cost() {
  ...
  const result = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-17-audio-cost-control-analysis",
