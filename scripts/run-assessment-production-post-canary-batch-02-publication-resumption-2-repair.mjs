...
assertV4(
  activation.status ===
    ...
    activation.authorization?.remainingFiveContextExecution === false &&
  ...
  "the resumption-2 repair execution is not authorized"
);
assertV4(
  ...
  "the resumption-2 repair execution controls changed"
);
...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), `batch-02-resumption-2-repair-${context.contextIndex}-`)
  );
  const codexHome = await mkdtemp(
    path.join(os.tmpdir(), `batch-02-resumption-2-repair-home-${context.contextIndex}-`)
  );
  ...
  try {
    ...
    const prompt = [
      ...
      `Act only as isolated Debate ${context.debateNumber} bounded publication repair editor ${context.contextIndex}.`,
      "Rewrite exactly the one or two correctedCritiques entries named by the schema, preserving the supplied adjudicated substance and locked score band.",
    ...
    process.stdout.write(
      `[batch-02-resumption-2-publication-repair] starting index ${context.contextIndex} Debate ${context.debateNumber} ${activation.model.label}/${activation.model.reasoningEffort}\n`
    );
    ...
    if (
      ...
      const validation = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-02-publication-resumption-2-repair-validation",
      ...
      const provenance = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-02-publication-resumption-2-repair-provenance",
        protocolId: activation.protocolId,
        debateNumber: context.debateNumber,
        packetIndex: context.packetIndex,
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-publication-resumption-2-repair-model-execution",
  protocolId: activation.protocolId,
  status:
    validContexts === 6
      ? "batch-02-resumption-2-six-context-publication-repair-gate-passed"
      : "batch-02-resumption-2-publication-repair-gate-complete-with-failure",
  gateStartedAt,
