...
assert.equal(activation.batchNumber, 10);
...
if (analysis.acceptance.passed) {
  ...
  assert.equal(analysis.totals.uniqueMoves, 182);
  assert.equal(analysis.totals.movesJudgedAcrossPasses, 364);
  assert.equal(
  ...
  assert.equal(
    analysis.status,
    "post-canary-batch-10-independent-judgment-gate-failed-analysis-only"
  );
}
console.log(
  JSON.stringify(
