...
import { validateDebate157RepairOutput } from "./lib/assessment-production-post-canary-batch-03-publication-resumption-2-repair.mjs";
...
assertV4(
  activation.status === "seven-frozen-unattempted-debate-157-publication-repair-contexts-authorized-for-resumption" &&
    ...
    activation.modelInputs?.correction2OutputUnavailable === true &&
    activation.modelInputs?.failedPacket0OutputUnavailable === true,
  "the seven-context Debate 157 repair resumption is not authorized"
);
assertV4(
  activation.model?.label === "5.6 Sol" && activation.model?.slug === "gpt-5.6-sol" &&
    ...
    activation.executionPolicy?.timeoutExtensionsMaximum === 0 && activation.executionPolicy?.maximumParallelContexts === 2 &&
...
for (const [file, digest] of Object.entries(activation.sourceHashes)) {
  assertV4(sha256(await readFile(path.resolve(file))) === digest, `repair resumption source hash mismatch: ${file}`);
}
for (const future of activation.futureOutputPathsExcludedFromSourceHashes) {
  assertV4(!(await exists(future)), `future repair resumption output exists: ${future}`);
}
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-03-debate-157-repair-resume-${context.packetIndex}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `batch-03-debate-157-repair-resume-home-${context.packetIndex}-`));
  ...
  try {
    ...
    const prompt = [
      ...
      `Act only as isolated Debate 157 bounded publication repair editor for original packet ${context.packetIndex}.`,
      ...
      "Participant judgment is closed and score-blind. Scores are repository-owned and immutable. Do not infer, emit, recalculate, or suggest changing a score.",
      "The correction-2 output, failed packet-0 output, other repair packets, other debates, legacy assessments, winners, and rankings are unavailable.",
      "Return exactly one schema-conforming JSON object and nothing else."
    ].join(" ");
    process.stdout.write(`[batch-03-debate-157-repair-resumption] starting packet ${context.packetIndex} 5.6 Sol/low\n`);
    const invocation = await invoke([
      ...
      "--disable", "plugins", "--disable", "remote_plugin", "--disable", "skill_search", "--disable", "apps",
      "--disable", "memories", "--disable", "multi_agent", "--disable", "browser_use", "--disable", "computer_use",
      "--disable", "workspace_dependencies", "--sandbox", "read-only", "--output-schema", "schema.json",
    ...
    const base = {
      ...
      correction2OutputAvailableToModel: false,
      failedPacket0OutputAvailableToModel: false,
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 || invocation.signal !== null || !resultExists) {
      ...
      try {
        validationSummary = validateDebate157RepairOutput(JSON.parse(resultBytes), JSON.parse(await readFile(path.resolve(context.packet), "utf8")));
      } catch (error) {
      ...
      await writeFile(path.resolve(context.validation), `${JSON.stringify({
        schemaVersion: "1.0-assessment-production-post-canary-batch-03-debate-157-publication-repair-resumption-1-validation",
      ...
      await writeFile(path.resolve(context.provenance), `${JSON.stringify({
        schemaVersion: "1.0-assessment-production-post-canary-batch-03-debate-157-publication-repair-resumption-1-provenance",
        protocolId: activation.protocolId,
        debateNumber: "157",
        packetIndex: context.packetIndex,
    ...
    record = { contextIndex: context.contextIndex, originalContextIndex: context.originalContextIndex, packetIndex: context.packetIndex, debateNumber: "157", model: "5.6 Sol", reasoningEffort: "low", attemptCount: 1, retryCount: 0, timeoutExtensionCount: 0, recursiveCorrectionContextCount: 0, startedAt, completedAt: new Date().toISOString(), elapsedMs: Date.now() - started, authentication: "ChatGPT subscription", apiKeysRemoved: true, correction2OutputAvailableToModel: false, failedPacket0OutputAvailableToModel: false, meteredApiCostUsd: 0, paidServiceCallsThisStage: 0, modelAuthoredScores: 0, status: "runner-error", gateAcceptancePassed: false, failureMessage: (error.stack ?? String(error)).slice(-10000) };
  } finally {
    activeContexts -= 1;
...
for (const phase of activation.executionPolicy.rampPhases) {
  if (!priorRampPassed) {
    phaseRecords.push({ ...phase, attemptedContextIndexes: [], validContextIndexes: [], passed: false, skippedBecausePriorRampFailed: true });
    continue;
  }
...
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-03-debate-157-publication-repair-resumption-1-model-execution",
  protocolId: activation.protocolId,
  status: validContexts === 7 ? "batch-03-debate-157-seven-context-publication-repair-resumption-passed" : "batch-03-debate-157-publication-repair-resumption-failed",
  gateStartedAt,
  ...
  results,
  originalPacket0FailurePreservedAndUnaccepted: true,
  acceptedCorrection2PreservedAndUnavailableToModels: true,
  participantJudgmentWasScoreBlind: true,
  scoresImmutable: true,
  meteredApiCostUsd: 0,
  paidServiceCallsThisStage: 0,
  modelAuthoredScores: 0,
  authorization: { deterministicAnalysis: true, deterministicMergeAndCohortReplay: validContexts === 7, retry: false, timeoutExtension: false, recursiveCorrection: false, paidServices: false, productionMutation: false, nextBatchSelection: false }
};
