...
assert.equal(execution.state.redirectsFollowed, 15);
...
assert.equal(preparation.recovery.redirectRangeFormat139Downloads, 3);
assert.equal(preparation.recovery.redirectsFollowed, 15);
...
assert(downloaded.every((source) => source.rangeRequests.every((range) => range.redirectCount === 1)));
...
console.log(JSON.stringify({
  ...
  redirectRangeDownloads: downloaded.length,
  retries: 0,
