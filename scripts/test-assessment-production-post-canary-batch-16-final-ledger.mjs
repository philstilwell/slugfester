...
const scoreArtifactCandidates = [
  `${POST_CANARY_BATCH_16_FINAL_LEDGER_ROOT}/calculated-scores.json`,
  `${POST_CANARY_BATCH_16_FINAL_LEDGER_ROOT}/score-analysis.json`,
  `${POST_CANARY_BATCH_16_FINAL_LEDGER_ROOT}/score-pass-manifest.json`
];
...
const rebuiltLedger = buildPostCanaryBatch16FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
...
assert.deepEqual(
  ...
  ["16", "108", "164", "144", "76", "41", "92", "139", "163", "161"]
);
assert.equal(storedLedger.audit.finalMoves, 194);
assert.equal(storedLedger.audit.disputedMoves, 185);
assert.equal(storedLedger.audit.pairSelections, 190);
assert.equal(storedLedger.audit.scoringFieldSelections, 366);
assert.equal(storedLedger.audit.burdenAdjustmentSelections, 20);
assert.equal(storedLedger.audit.candidateSelections, 576);
assert.equal(storedLedger.audit.meanMerges, 417);
assert.equal(storedLedger.audit.dependencyMeanMergesSuppressed, 0);
assert.equal(storedLedger.audit.audioVerifiedMoves, 10);
...
assert.throws(() =>
  buildPostCanaryBatch16FinalLedger(mutatedInputs, inputs.sourceHashes)
);
...
assert.equal(analysis.totals.finalMoves, 194);
assert.equal(analysis.totals.disputedMoves, 185);
assert.equal(analysis.totals.candidateSelections, 576);
