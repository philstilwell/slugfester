...
const analysisPath = `${POST_CANARY_BATCH_04_GENERATED_SEO_CORRECTION_ROOT}/preparation-analysis.json`;
const [plan, analysis] = await Promise.all([readJson(planPath), readJson(analysisPath)]);

assert.equal(plan.status, "frozen-batch-04-generated-seo-derivative-correction-plan-prepared");
...
assert.equal(analysis.status, "batch-04-generated-seo-correction-plan-freeze-passed");
