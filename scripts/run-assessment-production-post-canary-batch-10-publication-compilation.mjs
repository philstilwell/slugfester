...
assertV4(a.status === "frozen-post-canary-batch-10-deterministic-publication-compilation-authorized" &&
  ...
  canonicalJson(a.explicitOrder) === canonicalJson(POST_CANARY_BATCH_10_PUBLICATION_COMPILATION_ORDER),
"Batch 10 compilation is not authorized");
...
try {
  ...
  for (const debateNumber of POST_CANARY_BATCH_10_PUBLICATION_COMPILATION_ORDER) {
    ...
    const compiled = compilePostCanaryBatch10PublicationStagingRecord({ output, packet, identity: id });
    const validation = validatePostCanaryBatch10CompiledStagingRecord({ compiled, output, packet, identity: id });
...
if (passed) await writeFile(path.resolve(a.artifacts.compilationAudit), `${JSON.stringify({
  schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-compilation-audit",
  ...
  totals: { debates: 10, sections: rows.reduce((sum, row) => sum + row.validation.sections, 0),
    moves: 182, deterministicCompilationPasses: 1, reruns: 0, scorePasses: 0,
    modelContexts: 0, modelAuthoredScores: 0, directIncrementalCostUsd: 0 },
  productionMutationPerformed: false }, null, 2)}\n`);
const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-compilation-execution",
  protocolId: a.protocolId, status: passed ? "ten-debate-batch-10-deterministic-publication-compilation-passed" :
    "ten-debate-batch-10-deterministic-publication-compilation-failed",
...
const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-compilation-analysis",
  protocolId: a.protocolId, status: execution.status, productionCanary: false, batchNumber: 10,
  ...
  nextAuthorizedAction: passed ? "prepare-batch-10-publication-finalization" : "diagnose-compilation-failure" };
