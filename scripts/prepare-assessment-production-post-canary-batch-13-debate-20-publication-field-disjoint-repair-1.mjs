...
} from "./lib/assessment-production-post-canary-batch-13-debate-20-publication-field-disjoint-repair-1.mjs";
...
const RESUMPTION_ROOT = `${PUBLICATION_ROOT}/original-unattempted-context-resumption-1`;
...
const FAILURE_ANALYSIS = `${RESUMPTION_ROOT}/debate-20-failure-analysis.json`;
...
assertV4(
  originalExecution.status === "seven-context-publication-resumption-stopped-with-failure" &&
    originalExecution.contextsAttempted === 1 && originalExecution.contextsUnattempted === 6 &&
    ...
    originalAnalysis.status === "seven-context-publication-resumption-failed" &&
    failureAnalysis.status === "debate-20-publication-failure-diagnosed-awaiting-field-disjoint-repair-1" &&
    failureAnalysis.diagnosis?.invalidCritiqueCount === 2 &&
    failureAnalysis.diagnosis?.invalidQuoteCount === 0 &&
    failureAnalysis.diagnosis?.allOtherFieldsStructurallyValid === true,
  "Debate 20 publication resumption failure boundary changed"
);
...
const debate20 = allArtifacts.get("20");
const structuralStandIn = Object.values(debate20.outputJson.moveProse)
  .map((entry) => entry.critique)
  .find((critique) => {
    const words = wordCount(critique);
    return words >= 105 && words <= 130 && critique.length >= 880;
  });
assertV4(structuralStandIn, "Debate 20 has no validation-clean diagnostic stand-in");

const expectedCritiqueCounts = { "20": 2 };
const diagnostics = [];
...
assertV4(diagnostics.reduce((sum, item) => sum + item.invalidCritiques.length, 0) === 2 &&
  diagnostics.reduce((sum, item) => sum + item.invalidQuotes.length, 0) === 0,
"diagnosed two-field repair scope changed");
...
assertV4(contexts.length === 1 &&
  contexts.every((context) => context.writableFieldCount >= 1 && context.writableFieldCount <= 2) &&
  contexts.reduce((sum, context) => sum + context.writableFieldCount, 0) === 2 &&
  new Set(contexts.flatMap((context) => context.writableFields)).size === 2,
"one-shard, two-field coverage changed");

const sourceScripts = [
  "scripts/lib/assessment-production-post-canary-batch-13-debate-20-publication-field-disjoint-repair-1.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-13-debate-20-publication-field-disjoint-repair-1.mjs",
  "scripts/activate-assessment-production-post-canary-batch-13-debate-20-publication-field-disjoint-repair-1.mjs",
  "scripts/run-assessment-production-post-canary-batch-13-debate-20-publication-field-disjoint-repair-1.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-13-debate-20-publication-field-disjoint-repair-1.mjs",
...
const manifest = {
  ...
  protocolId: PROTOCOL_ID,
  status: "frozen-one-context-two-field-batch-13-debate-20-publication-repair-prepared-not-activated",
  frozenAt,
  ...
  userAuthorization: {
    ...
    resolvedScope: "one fresh isolated field-disjoint shard correcting exactly two rejected Debate 20 critiques while retaining every validation-clean field",
  ...
  diagnosis: {
    status: "deterministically-confirmed-two-critique-failures",
    debates: diagnostics,
    totals: { debates: 1, invalidCritiques: 2, invalidQuotes: 0, writableFields: 2 },
    everyNonTargetFieldStructurallyValidated: true,
    temporaryStandInsWrittenOrRetained: false
  },
  diagnosticStandIn: {
    source: "validation-clean-critique-from-the-same-preserved-Debate-20-output",
  ...
  retentionContract: {
    ...
    completeTenDebateCohortReplayDeferredUntilSixUnattemptedContextsComplete: true
  },
  ...
  totals: {
    debates: 1, contexts: 1, critiqueFields: 2, quoteFields: 0, writableFields: 2,
  ...
  nextAuthorizedAction: "commit-and-push-frozen-one-shard-debate-20-repair-then-activate"
};
