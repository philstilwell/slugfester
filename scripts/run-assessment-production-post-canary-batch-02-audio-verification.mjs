...
assert(manifest.status === "frozen-ten-post-canary-batch-02-paid-known-speaker-diarizations-authorized", "Batch 2 paid audio execution is not frozen");
assert(manifest.calls.length === 10, "exactly ten calls required");
...
assert(manifest.costEstimate.maximumAuthorizedCostUsd === 0.4, "approved cap changed");
...
for (const [callIndex, call] of manifest.calls.entries()) {
  ...
  process.stdout.write(`[batch-02-audio] ${callIndex + 1}/10 starting ${call.debateNumber}:${call.moveId}\n`);
  ...
  results.push(result);
  process.stdout.write(`[batch-02-audio] ${callIndex + 1}/10 ${call.debateNumber}:${call.moveId} ${result.status}; cumulative usage estimate $${result.cumulativeUsageDerivedEstimatedCostUsd.toFixed(7)}\n`);
}
...
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-02-audio-verification-model-execution",
  protocolId: manifest.protocolId,
  status: completed.length === manifest.calls.length
    ? "ten-post-canary-batch-02-paid-known-speaker-diarizations-completed"
    : "post-canary-batch-02-paid-diarization-incomplete",
