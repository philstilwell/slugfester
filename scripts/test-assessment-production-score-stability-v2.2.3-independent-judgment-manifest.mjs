...
const MANIFEST =
  "docs/assessment-production/score-stability-v2.2.3-validation-cohort/independent-judgments/execution-preparation-manifest.json";
...
assert.equal(
  manifest.failedGateDisposition.v221PlanningGatePreservedFailed,
  true
);
assert.equal(
  manifest.failedGateDisposition.v22DiscoveryGatePreservedFailed,
  true
);
assert.equal(
  manifest.failedGateDisposition.v213ScoreGatePreservedFailed,
  true
);
assert.equal(
...
assert.equal(preparation.totals.maximumCopiedInputBytes, 110026);
