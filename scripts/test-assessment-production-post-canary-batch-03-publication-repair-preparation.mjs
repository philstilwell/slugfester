...
import {
  ...
  mergeAndValidateDebate124Repairs,
  validateDebate124RepairOutput
} from "./lib/assessment-production-post-canary-batch-03-publication-repair.mjs";

const MANIFEST = `${POST_CANARY_BATCH_03_DEBATE_124_REPAIR_ROOT}/execution-preparation-manifest.json`;
...
assert.equal(manifest.protocolId, POST_CANARY_BATCH_03_DEBATE_124_REPAIR_PROTOCOL_ID);
...
for (const context of manifest.contexts) {
  ...
  assert.equal(packet.debateNumber, "124");
  ...
  assert.deepEqual(schema, buildDebate124RepairSchema(packet));
  ...
  assert.equal(validateDebate124RepairOutput(repair, packet).status, "passed");
...
const merged = mergeAndValidateDebate124Repairs({
  baseOutput,
...
assert.equal(merged.fullValidation.moves, 24);
