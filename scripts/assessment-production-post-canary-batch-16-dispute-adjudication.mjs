...
import {
  POST_CANARY_BATCH_16_DISPUTE_ADJ_PROTOCOL_ID,
  POST_CANARY_BATCH_16_DISPUTE_ADJ_ROOT,
  buildPostCanaryBatch16DisputeAdjudicationPacket,
  makePostCanaryBatch16DisputeAdjudicationSchema,
  validatePostCanaryBatch16DisputeAdjudicationOutput,
} from "./lib/assessment-production-post-canary-batch-16-dispute-adjudication.mjs";
...
const expectedDebates = ["16", "108", "164", "144", "76", "41", "92", "139", "163", "161"];
const expectedDisputedMoves = 185;
const expectedCandidateSelections = 576;
const expectedAudioMoves = 10;
const toolPath = "scripts/assessment-production-post-canary-batch-16-dispute-adjudication.mjs";
const libPath = "scripts/lib/assessment-production-post-canary-batch-16-dispute-adjudication.mjs";
const inputs = {
  ...
  schema: `${POST_CANARY_BATCH_16_DISPUTE_ADJ_ROOT}/adjudication.schema.json`,
};
const paths = {
  preparation: `${POST_CANARY_BATCH_16_DISPUTE_ADJ_ROOT}/preparation-manifest.json`,
  activation: `${POST_CANARY_BATCH_16_DISPUTE_ADJ_ROOT}/execution-activation.json`,
  execution: `${POST_CANARY_BATCH_16_DISPUTE_ADJ_ROOT}/model-execution.json`,
  analysis: `${POST_CANARY_BATCH_16_DISPUTE_ADJ_ROOT}/analysis.json`,
  ...
  recoveredAudio: `${audioRoot}/exceptional-paid-recovery/recovered-audio-verification.json`,
  recoveredAudioExecution: `${audioRoot}/exceptional-paid-recovery/model-execution.json`,
  ...
  audioAttributionOutput: `${audioRoot}/audio-attribution-recovery/outputs/attempt-1.json`,
};
...
async function prepare() {
  ...
  assert.equal(disagreementAnalysis.status, "post-canary-batch-16-deterministic-disagreements-extracted-standing-authorization-active-for-audio-work");
  ...
  assert.equal(judgmentPreparation.contexts.length, 20);
  assert.equal(judgmentExecution.status, "twenty-post-canary-batch-16-independent-judgment-contexts-passed");
  assert.equal(judgmentExecution.validContexts, 20);
  assert.equal(judgmentExecution.invalidContexts, 0);
  assert.equal(judgmentAnalysis.status, "twenty-post-canary-batch-16-independent-judgments-passed-standing-authorization-active-for-disagreement-extraction");
  assert.equal(standing.status, "frozen-active-batch-16-complete-remaining-workflow-standing-authorization");
  ...
  assert.equal(recoveredAudio.status, "batch-16-paid-audio-recovery-passed-one-preserved-attribution-decision-pending");
  assert.equal(recoveredAudioExecution.status, "batch-16-exceptional-paid-audio-recovery-execution-passed");
  assert.equal(combinedAudio.status, "post-canary-batch-16-combined-audio-verification-passed");
  assert.equal(combinedAudio.totals.verified, 10);
  assert.equal(combinedAudio.totals.unresolved, 0);
  assert.equal(audioAttributionAnalysis.status, "batch-16-debate-144-audio-attribution-recovery-passed");
  assert.equal(audioAttributionOutput.adjudication.status, "verified");
  const frozenCalls = new Map(audioManifest.calls.map((call) => [`${call.debateNumber}:${call.moveId}`, call]));
  const attributionDecision = combinedAudio.recovery.decision;
  ...
  for (const move of recoveredAudio.debates.flatMap((debate) => debate.moves)) {
    ...
    const acceptedByAttribution = key === `144:${attributionDecision.moveId}` && attributionDecision.status === "verified";
  ...
  assert.equal(audioByDebateMove.size, 10);
  const schema = makePostCanaryBatch16DisputeAdjudicationSchema();
  ...
  for (const debateNumber of expectedDebates) {
    ...
    const built = buildPostCanaryBatch16DisputeAdjudicationPacket(JSON.parse(disagreementBytes), JSON.parse(inventoryBytes), JSON.parse(eventsBytes), audioByMoveId);
    const packetPath = `${POST_CANARY_BATCH_16_DISPUTE_ADJ_ROOT}/packets/debate-${debateNumber}.json`;
    const provenancePath = `${POST_CANARY_BATCH_16_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`;
    const outputPath = `${POST_CANARY_BATCH_16_DISPUTE_ADJ_ROOT}/outputs/debate-${debateNumber}.json`;
    ...
    const provenanceBytes = pretty({ schemaVersion: "1.0-assessment-production-post-canary-batch-16-adjudication-candidate-provenance", protocolId: built.packet.protocolId, debateNumber, modelInput: false, mappings: built.provenance });
  ...
  const preparation = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-16-dispute-only-adjudication-preparation",
    protocolId: POST_CANARY_BATCH_16_DISPUTE_ADJ_PROTOCOL_ID,
    status: "prepared-ten-isolated-post-canary-batch-16-dispute-only-adjudication-contexts-not-active",
    frozenAt,
    ...
    modelInputs: inputs,
    contexts,
    acceptedSourceBoundary: { independentJudgments: paths.judgmentAnalysis, frozenDisagreements: paths.disagreementAnalysis, audioVerification: paths.combinedAudio, allTwentyJudgmentsAccepted: true, allTenAudioMovesVerified: true, cumulativeAudioUsageDerivedEstimatedCostUsd: 0.45347, additionalPaidCallsThisStage: 0 },
    ...
    executionPolicy: { contexts: 10, concurrency: 2, attemptsPerContext: 1, retriesMaximum: 0, timeoutExtensionsMaximum: 0, timeoutMsPerContext: 720000, freshTemporaryCodexHomePerContext: true, freshSourceDirectoryPerContext: true, APIKeysRemoved: true, removedEnvironmentVariables: ["OPENAI_API_KEY", "OPENAI_ORG_ID", "OPENAI_PROJECT_ID", "OPENAI_BASE_URL", "AZURE_OPENAI_API_KEY", "CODEX_API_KEY"] },
    totals: { contexts: 10, disputedMoves, candidateSelections, audioVerifiedMoves, modelContextsExecuted: 0, paidServiceCalls: 0, finalLedgersAssembled: 0, scoresDerived: 0, publicationReconstructions: 0, productionMutations: 0, nextBatchSelections: 0, directIncrementalCostUsd: 0 },
    sourceHashes,
  ...
  if (shouldWrite) {
    await mkdir(POST_CANARY_BATCH_16_DISPUTE_ADJ_ROOT, { recursive: true });
    await writeFile(inputs.schema, schemaBytes);
    await writeFile(paths.preparation, pretty(preparation));
  }
  console.log(JSON.stringify({ status: shouldWrite ? preparation.status : "preview", debates: expectedDebates, contexts: 10, disputedMoves, candidateSelections, audioVerifiedMoves, maximumCopiedInputBytes: Math.max(...contexts.map((item) => item.copiedInputBytes)), model: "5.6 Sol/low", directIncrementalCostUsd: 0, scoresDerived: 0 }, null, 2));
}

async function validatePreparation(preparation, expectedStatus = "prepared-ten-isolated-post-canary-batch-16-dispute-only-adjudication-contexts-not-active") {
  assert.equal(preparation.status, expectedStatus);
  assert.equal(preparation.batchNumber, 16);
...
async function activate() {
  ...
  const activation = {
    ...preparation,
    schemaVersion: "1.0-assessment-production-post-canary-batch-16-dispute-only-adjudication-execution-activation",
    status: "frozen-ten-post-canary-batch-16-dispute-only-adjudication-contexts-authorized",
    activatedAt,
    checkpointCommit: head,
  ...
  console.log(JSON.stringify({ status: shouldWrite ? activation.status : "preview", contexts: 10, concurrency: 2, attemptsPerContext: 1, retriesMaximum: 0, model: "5.6 Sol/low", directIncrementalCostUsd: 0, active: shouldWrite }, null, 2));
}

async function validateActivation(manifest) {
  await validatePreparation(manifest, "frozen-ten-post-canary-batch-16-dispute-only-adjudication-contexts-authorized");
...
async function runContext(manifest, context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-b16-adjudication-${context.debateNumber}-`));
  const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), `slugfester-b16-adjudication-home-${context.debateNumber}-`));
  ...
  try {
    ...
    try { validation = validatePostCanaryBatch16DisputeAdjudicationOutput(JSON.parse(await readFile(context.output, "utf8")), JSON.parse(await readFile(context.packet, "utf8"))); } catch (error) { validationMessage = String(error?.stack ?? error).slice(-10000); }
...
async function run() {
  ...
  async function worker() {
    while (queue.length) {
      ...
      results.push(result);
      process.stdout.write(`[batch-16-adjudication] Debate ${context.debateNumber} ${result.status} in ${(result.elapsedMs / 60000).toFixed(2)}m\n`);
    }
  }
  ...
  const execution = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-16-dispute-only-adjudication-model-execution",
    protocolId: manifest.protocolId,
    status: validContexts === 10 ? "ten-post-canary-batch-16-dispute-only-adjudication-contexts-passed" : "post-canary-batch-16-dispute-only-adjudication-gate-complete-with-failure",
    productionCanary: false,
    batchNumber: 16,
    stagingOnly: true,
    gateStartedAt,
  ...
  if (validContexts !== 10) process.exitCode = 1;
}

async function analyze() {
  const [manifest, execution] = await Promise.all([readJson(paths.activation), readJson(paths.execution)]);
  await validateActivation(manifest);
  assert.equal(execution.status, "ten-post-canary-batch-16-dispute-only-adjudication-contexts-passed");
  const contexts = [];
  ...
  for (const context of manifest.contexts) {
    ...
    const validation = validatePostCanaryBatch16DisputeAdjudicationOutput(JSON.parse(outputBytes), packet);
  ...
  const passed = disputedMovesDecided === expectedDisputedMoves && candidateSelections === expectedCandidateSelections && contexts.length === 10;
  const analysis = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-16-dispute-only-adjudication-analysis",
    protocolId: POST_CANARY_BATCH_16_DISPUTE_ADJ_PROTOCOL_ID,
    status: passed ? "post-canary-batch-16-dispute-only-adjudication-gate-passed-standing-authorization-active-for-final-ledger-assembly" : "post-canary-batch-16-dispute-only-adjudication-gate-failed",
    ...
    AIOnly: true,
    contexts,
    gate: { semanticPass: passed, timingPass: maxElapsed <= 12 && meanElapsed <= 9.5, scoreBlindPass: true, isolationPass: true, validContexts: 10, requiredValidContexts: 10, disputedMovesDecided, requiredDisputedMoves: expectedDisputedMoves, candidateSelections, requiredCandidateSelections: expectedCandidateSelections, maximumElapsedMinutes: maxElapsed, maximumAllowedMinutesPerContext: 12, meanElapsedMinutes: Number(meanElapsed.toFixed(2)), maximumAllowedMeanMinutes: 9.5, attempts: 10, retries: 0, timeoutExtensions: 0, corrections: 0, scoresDerived: 0 },
    totals: { adjudicationModelContexts: 10, judgmentModelContexts: 0, paidServiceCalls: 0, retries: 0, timeoutExtensions: 0, corrections: 0, scoresDerived: 0, publicationReconstructions: 0, productionMutations: 0, nextBatchSelections: 0, directIncrementalCostUsd: 0 },
    ...
    nextAuthorizedAction: passed ? "standing-authorization-permits-batch-16-deterministic-final-ledger-assembly" : "stop-for-substantive-adjudication-blocker",
  };
  if (shouldWrite) await writeFile(paths.analysis, pretty(analysis));
  console.log(JSON.stringify({ status: analysis.status, validContexts: 10, disputedMovesDecided, candidateSelections, maximumElapsedMinutes: maxElapsed, meanElapsedMinutes: analysis.gate.meanElapsedMinutes, retries: 0, scoresDerived: 0, finalLedgerAssemblyAuthorized: passed }, null, 2));
...
async function test() {
  ...
  assert.equal(schema.properties.batchNumber.const, 16);
  for (const context of preparation.contexts) {
    const packet = await readJson(context.packet);
    assert.equal(packet.batchNumber, 16);
  ...
  if (await exists(paths.execution)) {
    ...
    assert(execution.attempts <= 10);
    assert.equal(execution.retries, 0);
  }
  console.log(JSON.stringify({ status: "passed", debates: expectedDebates, contexts: 10, disputedMoves: expectedDisputedMoves, candidateSelections: expectedCandidateSelections, audioVerifiedMoves: expectedAudioMoves, attemptsPerContext: 1, retriesMaximum: 0, model: "5.6 Sol/low", scoresDerived: 0 }, null, 2));
}
