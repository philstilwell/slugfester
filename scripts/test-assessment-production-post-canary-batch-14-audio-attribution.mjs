...
import { AUDIT, ISOLATION, MODEL, OUTPUT_VERSION, PROTOCOL_ID, ROOT, validateOutput } from "./lib/assessment-production-post-canary-batch-14-audio-attribution.mjs";

const preparation = JSON.parse(await readFile(`${ROOT}/preparation-manifest.json`, "utf8"));
const authorization = JSON.parse(await readFile("docs/assessment-production/post-canary-continuation-v1/batch-14/audio-verification/audio-attribution-successor-authorization.json", "utf8"));
...
console.log(JSON.stringify({ status: "passed-prepared", contexts: 1, decisions: 1, model: "5.6 Sol/low", authentication: "ChatGPT subscription", attemptsMaximum: 1, retriesMaximum: 0, paidTranscriptionCalls: 0, scoresDerived: 0 }, null, 2));
