...
import {
  POST_CANARY_BATCH_11_PRODUCTION_PUBLICATION_ORDER,
  POST_CANARY_BATCH_11_PRODUCTION_PUBLICATION_PROTOCOL_ID,
  POST_CANARY_BATCH_11_PRODUCTION_PUBLICATION_ROOT,
...
const paths = {
  manifest: `${POST_CANARY_BATCH_11_PRODUCTION_PUBLICATION_ROOT}/mutation-manifest.json`,
  preparationAnalysis: `${POST_CANARY_BATCH_11_PRODUCTION_PUBLICATION_ROOT}/preparation-analysis.json`,
  activation: `${POST_CANARY_BATCH_11_PRODUCTION_PUBLICATION_ROOT}/execution-activation.json`,
  execution: `${POST_CANARY_BATCH_11_PRODUCTION_PUBLICATION_ROOT}/execution.json`,
  compatibilityRoot: "docs/assessment-production/post-canary-continuation-v1/batch-11/production-compatibility",
  finalizationRoot: "docs/assessment-production/post-canary-continuation-v1/batch-11/publication-finalization/output-bundle/final-candidates",
  ...
  standingAuthorization: "docs/assessment-production/post-canary-continuation-v1/batch-11/standing-authorization.json",
  preparationScript: "scripts/prepare-assessment-production-post-canary-batch-11-production-publication.mjs",
  preparationTest: "scripts/test-assessment-production-post-canary-batch-11-production-publication-preparation.mjs",
  library: "scripts/lib/assessment-production-post-canary-batch-11-production-publication.mjs",
  activationScript: "scripts/activate-assessment-production-post-canary-batch-11-production-publication.mjs",
  activationTest: "scripts/test-assessment-production-post-canary-batch-11-production-publication-activation.mjs",
  executionScript: "scripts/run-assessment-production-post-canary-batch-11-production-publication.mjs"
};
...
assertV4(
  compatibilityAnalysis.status === "batch-11-production-compatibility-passed" &&
    ...
    compatibilityAnalysis.nextAuthorizedAction === "prepare-batch-11-production-publication-mutation-manifest-under-standing-authorization",
  "accepted Batch 11 compatibility analysis required"
);
assertV4(
  compatibilityExecution.status === "batch-11-route-score-checkpoint-batch-01-batch-02-batch-03-batch-04-batch-05-batch-06-batch-07-batch-08-batch-09-batch-10-legacy-reference-validation-passed" &&
    compatibilityExecution.regressions?.fullRepositoryPassed === true &&
    compatibilityExecution.totals?.compatibilityPassesCompleted === 1,
  "accepted one-pass Batch 11 compatibility execution required"
);
assertV4(
  compatibilityActivation.status === "post-canary-batch-11-compatibility-execution-authorized-and-frozen" &&
    compatibilityActivation.packetHashes?.length === 10,
  "frozen Batch 11 compatibility activation required"
);
...
assertV4(ledgerInventory.length === 111, "expected 111 existing production ledgers before Batch 11 publication");

const packetLocks = new Map(compatibilityActivation.packetHashes.map((record) => [record.debateNumber, record]));
const debates = [];
for (const debateNumber of POST_CANARY_BATCH_11_PRODUCTION_PUBLICATION_ORDER) {
...
assertV4(
  debates.filter((debate) => debate.productionLedgerBaseline.exists).length === 1 &&
    debates.filter((debate) => !debate.productionLedgerBaseline.exists).length === 9,
  "expected one existing and nine absent Batch 11 production ledgers"
);
...
assertV4(validatorLock.sha256 === compatibilityExecution.validator.appliedSha256, "accepted Batch 11 validator changed");
...
const manifest = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-11-production-publication-mutation-manifest",
  protocolId: POST_CANARY_BATCH_11_PRODUCTION_PUBLICATION_PROTOCOL_ID,
  status: "frozen-batch-11-production-publication-mutation-manifest-prepared",
  frozenAt,
  baselineCommit,
  ...
  explicitOrder: [...POST_CANARY_BATCH_11_PRODUCTION_PUBLICATION_ORDER],
  invariants: {
  ...
  executionContract: {
    ...
    requiredOrder: [
      ...
      "confirm the nine absent and one present Batch 11 production-ledger baselines and all 111 existing ledger hashes",
      "publish ten production ledgers byte-for-byte from the staged ledgers, replacing the one existing Batch 11 ledger",
      "replace exactly ten Batch 11 records in src/data/debates.js with the finalized candidates",
      "keep src/data/references.js byte-identical",
      "run deterministic Batch 11 route and score validation",
  ...
  nextAuthorizedAction: "activate-and-execute-one-frozen-batch-11-production-publication-mutation-pass-under-standing-authorization"
};
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-11-production-publication-preparation-analysis",
  protocolId: POST_CANARY_BATCH_11_PRODUCTION_PUBLICATION_PROTOCOL_ID,
  status: "batch-11-production-publication-mutation-manifest-freeze-passed",
  analyzedAt: frozenAt,
  ...
  checks: {
    ...
    existingProductionLedgersLocked: 111,
...
if (write) {
  await mkdir(resolve(POST_CANARY_BATCH_11_PRODUCTION_PUBLICATION_ROOT), { recursive: true });
