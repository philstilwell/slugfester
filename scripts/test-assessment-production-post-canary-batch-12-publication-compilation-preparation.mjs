...
import { POST_CANARY_BATCH_12_PUBLICATION_COMPILATION_ORDER,
  ...
  compilePostCanaryBatch12PublicationStagingRecord } from
...
assertV4(m.status === "frozen-post-canary-batch-12-deterministic-publication-compilation-prepared" &&
  canonicalJson(m.explicitOrder) === canonicalJson(POST_CANARY_BATCH_12_PUBLICATION_COMPILATION_ORDER) &&
  m.contexts?.length === 10 && m.contexts.reduce((sum, row) => sum + row.expectedMoves, 0) === 204 &&
...
"Batch 12 compilation preparation changed");
let moves = 0;
for (const context of m.contexts) {
  ...
  const compiled = compilePostCanaryBatch12PublicationStagingRecord({ output, packet, identity: row });
  const validation = validatePostCanaryBatch12CompiledStagingRecord({ compiled, output, packet, identity: row });
  moves += validation.moves;
}
assertV4(moves === 204, "compiled move replay changed");
