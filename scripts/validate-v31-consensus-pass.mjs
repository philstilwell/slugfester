...
import {
  V31_MODEL, V31_PASS_INPUTS, V31_RUBRIC, V31_WORKFLOW, assert, equal,
...
if (!passArgument || !inputArgument) throw new Error("Usage: node scripts/validate-v31-consensus-pass.mjs <pass.json> <input.json>");
...
const [passText, inputText, workflowText, rubricText, manualText, schemaText] = await Promise.all([
  read(passArgument), read(inputArgument), read("docs/assessment-workflow-v3.1.md"), read("docs/reassessment-rubric-v3.1.md"),
  read(`${gateRoot}/annotation-manual.md`), read(`${gateRoot}/consensus-pass-schema.json`)
]);
...
assert(pass.schemaVersion === "3.1-consensus-pass" && pass.workflowVersion === V31_WORKFLOW && pass.rubricVersion === V31_RUBRIC, "pass version mismatch");
assert(["A", "B"].includes(pass.pass) && pass.model === V31_MODEL && pass.calibrationOnly === true && !Number.isNaN(Date.parse(pass.completedAt)), "pass identity invalid");
assert(pass.isolation.method === "fresh-ephemeral-v3.1-consensus-pass" && equal([...pass.isolation.allowedInputs].sort(), [...V31_PASS_INPUTS].sort()), "pass isolation invalid");
