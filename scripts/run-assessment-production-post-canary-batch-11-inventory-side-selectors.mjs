...
import { validateChronologyFallbackSideSelection } from "./lib/assessment-production-score-stability-v2.1.2-inventory-chronology-fallback.mjs";
import {
...
assertV4(
  activation.status ===
      "frozen-twenty-post-canary-batch-11-side-selector-contexts-authorized" &&
    ...
    activation.costBoundary?.directIncrementalCostUsdMaximum === 0 &&
  ...
  "post-canary Batch 11 side-selector execution is unauthorized"
);
...
const [preparationBytes, sourcePreparationBytes, sidePacketPreparationBytes] = await Promise.all([
  ...
  readFile(activation.sidePacketPreparation),
]);
assertV4(
  sha256(preparationBytes) === activation.preparationManifestSha256 &&
    ...
    sha256(sidePacketPreparationBytes) === activation.sidePacketPreparationSha256,
...
assertV4(
  preparation.contexts?.length === 20 &&
    ...
    JSON.stringify(sourcePreparation.sourceCompatibility) ===
      JSON.stringify(activation.sourceCompatibility) &&
    JSON.stringify(sidePacketPreparation.sourceCompatibility) ===
      JSON.stringify(activation.sourceCompatibility),
...
async function executeContext(context, contextIndex) {
  ...
  const sourceDirectory = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-post-canary-batch-11-side-${context.debateNumber}-${context.side}-`
    )
  );
  const isolatedCodexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-post-canary-batch-11-side-home-${context.debateNumber}-${context.side}-`
    )
  );
  ...
  try {
    ...
    const prompt = `Read inventory-source-packet.json, immutable-plan.json, side-candidate-evidence.json, candidate-sharded-inventory-guide.md, inventory-manual.md, and side-schema.json completely; read nothing else. Act only as the isolated score-blind ${context.side}-side candidate-evidence selector for frozen post-canary Batch 11 Debate ${context.debateNumber}. Review every candidate key in the delivered ${context.side}-only transport and map each key to either null or exactly one schema-conforming nomination. Preserve the immutable routes and sections. Every nomination must author both the preferred move kind and a source-grounded constructive orphan fallback with a substantive rationale; the repository applies that fallback only if a retained preferred reply has no earlier retained opposing move. Nominate at least one candidate for every section, using essential, strong, or supporting priority; the repository will retain at most two per section by frozen deterministic reduction. The other side, its output, failed production-canary outputs, validation-cohort outputs, planner execution metadata, ratings, response topology, scores, winners, legacy assessments, other debates, other judgments, tags, Overall Commentary, AI Extension, and publication prose are unavailable and prohibited. Return exactly one side-schema-conforming JSON object.`;
    process.stdout.write(
      `[post-canary-batch-11-side-selector] starting ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber} ${context.side}\n`
    );
    ...
    if (
      ...
      try {
        ...
        validationSummary = validateChronologyFallbackSideSelection({
  ...
  process.stdout.write(
    `[post-canary-batch-11-side-selector] Debate ${context.debateNumber} ${context.side} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-11-side-selector-model-execution",
  protocolId: activation.protocolId,
  status:
    validContexts === preparation.contexts.length
      ? "twenty-post-canary-batch-11-side-selector-contexts-passed"
      : "post-canary-batch-11-side-selector-gate-complete-with-failure",
  ...
  nextRequiredAction:
    validContexts === preparation.contexts.length
      ? "analyze-compile-and-lock-post-canary-batch-11-inventories-model-free-only"
      : "stop-preserve-failed-post-canary-batch-11-side-selector-gate",
};
...
if (validContexts !== preparation.contexts.length) {
  process.exitCode = 1;
}
