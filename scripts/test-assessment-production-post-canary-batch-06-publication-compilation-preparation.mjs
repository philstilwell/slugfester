...
import { POST_CANARY_BATCH_06_PUBLICATION_COMPILATION_ORDER,
  ...
  compilePostCanaryBatch06PublicationStagingRecord } from
...
assertV4(m.status === "frozen-post-canary-batch-06-deterministic-publication-compilation-prepared" &&
  canonicalJson(m.explicitOrder) === canonicalJson(POST_CANARY_BATCH_06_PUBLICATION_COMPILATION_ORDER) &&
  m.contexts?.length === 10 && m.contexts.reduce((sum, row) => sum + row.expectedMoves, 0) === 200 &&
...
"Batch 6 compilation preparation changed");
let moves = 0;
for (const context of m.contexts) {
  ...
  const compiled = compilePostCanaryBatch06PublicationStagingRecord({ output, packet, identity: row });
  const validation = validatePostCanaryBatch06CompiledStagingRecord({ compiled, output, packet, identity: row });
  moves += validation.moves;
}
assertV4(moves === 200, "compiled move replay changed");
