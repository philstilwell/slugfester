...
assertV388Recon(["frozen-three-context-recovered-diagnostic-authorized", "frozen-schema-compatibility-recovery-authorized", "frozen-two-context-post-debate-55-continuation-authorized"].includes(manifest.status) && manifest.authorization.reconstructionModelExecution && manifest.executionPolicy.retriesAuthorized === 0, "reconstruction execution unauthorized");
...
const contextsPlanned = manifest.contexts.length;
const validContexts = results.filter((r) => r.diagnosticAcceptancePassed).length;
const execution = { schemaVersion: "3.8.8-reconstruction-model-execution", protocolId: manifest.protocolId, status: results.length === contextsPlanned && validContexts === contextsPlanned ? `passed-${contextsPlanned}-recovered-diagnostic-reconstructions` : "failed-closed", startedAt: results[0]?.startedAt ?? null, completedAt: new Date().toISOString(), contextsPlanned, contextsAttempted: results.length, validContexts, attempts: results.length, retries: 0, meteredApiCostUsd: 0, additionalTranscriptionEstimatedCostUsd: manifest.cost.additionalTranscriptionEstimatedCostUsd, productionMutationAuthorized: false, results };
await writeFile(path.resolve(root, manifest.artifacts.execution), `${JSON.stringify(execution, null, 2)}\n`);
assertV388Recon(execution.validContexts === contextsPlanned, "reconstruction diagnostic failed; audit and preview remain blocked");
console.log(JSON.stringify({ status: execution.status, validContexts, retries: 0, meteredApiCostUsd: 0, deterministicAuditAuthorized: true, calibrationPreviewAuthorized: false }, null, 2));
