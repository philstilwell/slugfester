...
const passed =
  ...
  validation.audioVerifiedMoves === 1 &&
  validation.acceptedAdjudicationOutputsWithoutCorrection === 4 &&
  validation.calculatedScores === 0 &&
  validation.scoreDerivationAuthorized === false;
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-17-final-ledger-analysis",
  protocolId: ledger.protocolId,
  status: passed
    ? "post-canary-batch-17-deterministic-final-ledger-gate-passed"
    : "post-canary-batch-17-deterministic-final-ledger-gate-failed",
  analyzedAt,
  ...
  nextAuthorizedAction: passed
    ...
    : "standing-authorization-permits-batch-17-final-ledger-diagnosis"
};
if (shouldWrite) {
  await writeFile(
