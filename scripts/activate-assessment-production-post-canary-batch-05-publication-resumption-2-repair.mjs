...
import { POST_CANARY_BATCH_05_DEBATE_42_REPAIR_PROTOCOL_ID,
  POST_CANARY_BATCH_05_DEBATE_42_REPAIR_ROOT } from
...
assertV4(p.protocolId === POST_CANARY_BATCH_05_DEBATE_42_REPAIR_PROTOCOL_ID &&
  p.status === "frozen-two-context-batch-05-debate-42-publication-repair-prepared-and-authorized" &&
  p.contexts?.length === 2 && p.contexts.every((row) => row.writableFieldCount === 2) &&
...
"the Debate 42 repair is not prepared");
...
const activation = { schemaVersion: "1.0-assessment-production-post-canary-batch-05-debate-42-repair-activation",
  protocolId: p.protocolId, status: "frozen-two-context-batch-05-debate-42-publication-repair-authorized",
  ...
  authorization: { repairModelExecution: true, deterministicOutputValidation: true,
    deterministicMergeAndValidation: true, debate59ResumptionPreparation: true,
  ...
  nextAuthorizedAction: "execute-exactly-two-frozen-debate-42-repair-contexts" };
...
console.log(JSON.stringify({ status: activation.status, contextsAuthorized: 2,
  writableFieldsAuthorized: 4, model: activation.model, attemptsPerContext: 1,
