...
import { POST_CANARY_BATCH_09_RENDERING_PROTOCOL_ID, POST_CANARY_BATCH_09_RENDERING_ROOT,
  validatePostCanaryBatch09RenderingPacket } from
...
assertV4(p.status === "frozen-post-canary-batch-09-rendering-verification-prepared" &&
  p.protocolId === POST_CANARY_BATCH_09_RENDERING_PROTOCOL_ID && p.packets?.length === 10 &&
  p.authorization?.executionActivation === true && p.authorization?.renderingVerification === false,
"Batch 9 rendering verification is not prepared");
execFileSync(process.execPath, ["scripts/test-assessment-production-post-canary-batch-09-rendering-verification-preparation.mjs"],
  { cwd: process.cwd(), stdio: "pipe" });
for (const row of p.packets) validatePostCanaryBatch09RenderingPacket(
...
const activation = { schemaVersion: "1.0-assessment-production-post-canary-batch-09-rendering-verification-activation",
  protocolId: p.protocolId, status: "frozen-post-canary-batch-09-rendering-verification-authorized",
  authorizedAt, activationCommit, productionCanary: false, batchNumber: 9, stagingOnly: true,
  ...
  nextAuthorizedAction: "execute-frozen-batch-09-rendering-verification" };
