...
import {
  ...
  evaluatePostCanaryBatch08ScoreStability
...
const accepted = evaluatePostCanaryBatch08ScoreStability(passing);
...
assert.equal(
  evaluatePostCanaryBatch08ScoreStability(winnerMutation).acceptancePassed,
  false
);
const distanceMutation = structuredClone(passing);
distanceMutation[2].final = scores(95, 80);
assert.equal(
  evaluatePostCanaryBatch08ScoreStability(distanceMutation).acceptancePassed,
  false
);
assert.throws(() =>
  evaluatePostCanaryBatch08ScoreStability(passing, {
    ...POST_CANARY_BATCH_08_SCORE_STABILITY_THRESHOLDS,
