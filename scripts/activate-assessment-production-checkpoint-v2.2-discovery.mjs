...
const SCRIPT =
  "scripts/activate-assessment-production-checkpoint-v2.2-discovery.mjs";
const RUNNER =
  "scripts/run-assessment-production-checkpoint-v2.2-discovery.mjs";
const ANALYZER =
  "scripts/analyze-assessment-production-checkpoint-v2.2-discovery.mjs";
const VALIDATOR = "scripts/validate-v212-discovery.mjs";
const TEST =
  "scripts/test-assessment-production-checkpoint-v2.2-discovery-activation.mjs";
...
assertV4(
  preparation.status ===
    ...
    preparation.executionPolicy.contexts === 36 &&
    ...
    preparation.executionPolicy.copiedInputBytesMaximum === 82270 &&
  ...
  "production checkpoint discovery activation is not authorized"
);
...
const sourceFiles = [
  ...
  preparation.sourceHashes["docs/assessment-production/production-checkpoint-v2.2-1/master-manifest.json"]
...
const activation = {
  schemaVersion:
    "1.0-production-checkpoint-v2.2-discovery-execution-activation",
  ...
  historicalDisposition: {
    ...
    priorFailedValidationCohortsPreserved: true,
    historicalFailuresUsedAsProductionModelInputs: false,
  ...
  residualDiscoveryRisks: {
    ...
    mitigation:
      "The model selects an actual bounded final row rather than performing requested-token arithmetic; per-row token counts and the manual disclose the unchanged minimum; deterministic validation rejects reversed or short windows; and the production checkpoint stops on any invalid context without retry.",
  },
  ...
  nextRequiredAction:
    "execute-frozen-production-checkpoint-v2.2-discovery-gate-once",
};
