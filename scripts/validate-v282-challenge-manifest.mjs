...
import { deriveDiagnostic, deriveReframe, equal } from "./lib/v282-semantics.mjs";
...
assert(manifest.schemaVersion === "2.8.2-development-challenge-manifest" && manifest.status === "frozen-before-blind-passes" && manifest.calibrationOnly === true && manifest.heldOutTranscriptsOpened === false && manifest.numericalScoringAuthorized === false, "manifest identity or stop state invalid");
...
console.log(JSON.stringify({ status: "passed", kind: "v2.8.2-challenge-manifest", caseCount: manifest.caseCount, keyFeatureCounts, sourceCount: Object.keys(manifest.sourceHashes).length, manifestSha256: sha256(manifestText) }, null, 2));
