...
import {
  POST_CANARY_BATCH_16_COMPATIBILITY_ORDER,
  POST_CANARY_BATCH_16_COMPATIBILITY_PROTOCOL_ID,
  POST_CANARY_BATCH_16_COMPATIBILITY_ROOT,
  POST_CANARY_BATCH_16_SITE_LEDGER_ADAPTER_VERSION,
  buildPostCanaryBatch16SiteLedgerAdapter,
  serializedJson,
  sha256,
...
const paths = {
  preparation: `${POST_CANARY_BATCH_16_COMPATIBILITY_ROOT}/preparation-manifest.json`,
  analysis: `${POST_CANARY_BATCH_16_COMPATIBILITY_ROOT}/analysis.json`,
  activation: `${POST_CANARY_BATCH_16_COMPATIBILITY_ROOT}/execution-activation.json`,
  execution: `${POST_CANARY_BATCH_16_COMPATIBILITY_ROOT}/execution.json`,
  stagedLedgerRoot:
    `${POST_CANARY_BATCH_16_COMPATIBILITY_ROOT}/output-bundle/staged-ledgers`,
  compatibility:
    "docs/assessment-production/post-canary-continuation-v1/batch-16/publication-finalization/compatibility-analysis.json",
  finalizationAudit:
    "docs/assessment-production/post-canary-continuation-v1/batch-16/publication-finalization/finalization-audit.json",
  finalLedger:
  ...
  renderingAnalysis:
    "docs/assessment-production/post-canary-continuation-v1/batch-16/rendering-verification/analysis.json",
  renderingAudit:
    "docs/assessment-production/post-canary-continuation-v1/batch-16/rendering-verification/rendering-audit.json",
  ...
  batch12CompatibilityLibrary:
    "scripts/lib/assessment-production-post-canary-batch-12-compatibility.mjs",
  batch12CompatibilityActivation:
    "docs/assessment-production/post-canary-continuation-v1/batch-12/production-compatibility/execution-activation.json",
  batch13CompatibilityLibrary:
    "scripts/lib/assessment-production-post-canary-batch-13-compatibility.mjs",
  batch13CompatibilityActivation:
    "docs/assessment-production/post-canary-continuation-v1/batch-13/production-compatibility/execution-activation.json",
  batch14CompatibilityLibrary:
    "scripts/lib/assessment-production-post-canary-batch-14-compatibility.mjs",
  batch14CompatibilityActivation:
    "docs/assessment-production/post-canary-continuation-v1/batch-14/production-compatibility/execution-activation.json",
  batch15CompatibilityLibrary:
    "scripts/lib/assessment-production-post-canary-batch-15-compatibility.mjs",
  batch15CompatibilityActivation:
    "docs/assessment-production/post-canary-continuation-v1/batch-15/production-compatibility/execution-activation.json",
  scoreStabilityException:
  ...
  standingAuthorization:
    "docs/assessment-production/post-canary-continuation-v1/batch-16/standing-authorization.json",
  batchFinalLedgerLibrary:
    "scripts/lib/assessment-production-post-canary-batch-16-final-ledger.mjs",
  compatibilityLibrary:
    "scripts/lib/assessment-production-post-canary-batch-16-compatibility.mjs",
  ...
  preparationScript:
    "scripts/prepare-assessment-production-post-canary-batch-16-compatibility.mjs",
  preparationTest:
    "scripts/test-assessment-production-post-canary-batch-16-compatibility-preparation.mjs",
...
assertV4(
  ...
  "a stable --frozen-at ISO timestamp is required for the first Batch 16 compatibility preparation write"
);
...
const [
  ...
  readJson(paths.scoreStabilityException),
  loadPostCanaryBatch16FinalLedgerInputs()
]);

assertV4(
  renderingAnalysis.status ===
      "ten-debate-batch-16-rendering-verification-passed" &&
    ...
    renderingAnalysis.nextAuthorizedAction ===
      "prepare-batch-16-production-compatibility-staging",
  "Batch 16 rendering gate does not authorize compatibility planning only"
);
assertV4(
  renderingAudit.status ===
      "passed-ten-debate-batch-16-rendering-verification" &&
  ...
  "passed Batch 16 rendering evidence audit required"
);
assertV4(
  compatibility.status ===
      "batch-16-production-compatibility-boundary-recorded" &&
    canonicalJson(compatibility.findings.map((finding) => finding.id)) ===
      canonicalJson(["batch-16-site-ledger-adapter-and-validator-route"]) &&
  ...
  "frozen Batch 16 publication compatibility blocker required"
);
assertV4(
  ...
  "passed one-pass Batch 16 staging-only finalization audit required"
);
assertV4(
  scores.status ===
      "post-canary-batch-16-single-score-pass-stability-gate-failed" &&
    scores.totals.acceptancePassed === false &&
    scores.totals.scoringPasses === 1 &&
    scores.authorization.scoreRerun === false &&
    scores.authorization.productionMutation === false,
  "preserved one-pass Batch 16 score ledger required"
);
assertV4(
  scoreStabilityException.status ===
      "frozen-active-batch-16-debate-144-score-stability-publication-exception" &&
    canonicalJson(scoreStabilityException.exception?.debates) === canonicalJson(["144"]) &&
    scoreStabilityException.exception?.compatibilityStaging === true &&
    scoreStabilityException.immutableBoundaries?.scorePassesCompleted === 1 &&
    scoreStabilityException.immutableBoundaries?.scoreRerunAllowed === false &&
    scoreStabilityException.sourceHashes?.[paths.scores] === await fileSha256(paths.scores) &&
    scoreStabilityException.sourceHashes?.[paths.finalLedger] === await fileSha256(paths.finalLedger),
  "frozen Debate 144 publication exception required"
);
validatePostCanaryBatch16FinalLedger(
  finalLedger,
...
const renderingResultOrderValid =
  renderingAudit.results.length === POST_CANARY_BATCH_16_COMPATIBILITY_ORDER.length * 2 &&
  renderingAudit.results.every((row, index) =>
    row.debateNumber === POST_CANARY_BATCH_16_COMPATIBILITY_ORDER[Math.floor(index / 2)] &&
    row.viewportName === (index % 2 === 0 ? "desktop" : "mobile")
  );
assertV4(
  finalLedger.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_16_COMPATIBILITY_ORDER.join(",") &&
    scores.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_16_COMPATIBILITY_ORDER.join(",") &&
    renderingResultOrderValid &&
    renderedDebateOrder.join(",") ===
      POST_CANARY_BATCH_16_COMPATIBILITY_ORDER.join(","),
  "Batch 16 compatibility debate order changed"
);
...
const packetRecords = [];
for (const debateNumber of POST_CANARY_BATCH_16_COMPATIBILITY_ORDER) {
  ...
  const candidatePath =
    `docs/assessment-production/post-canary-continuation-v1/batch-16/` +
    `publication-finalization/output-bundle/final-candidates/debate-${debateNumber}.json`;
  const packetPath =
    `${POST_CANARY_BATCH_16_COMPATIBILITY_ROOT}/packets/debate-${debateNumber}.json`;
  ...
  const adapter = buildPostCanaryBatch16SiteLedgerAdapter({
    finalLedgerDebate,
    scoreDebate,
    candidate,
  ...
  const packet = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-16-compatibility-plan-packet",
    protocolId: POST_CANARY_BATCH_16_COMPATIBILITY_PROTOCOL_ID,
    status: "frozen-post-canary-batch-16-compatibility-plan-packet",
...
const staticSourcePaths = [
  ...
  paths.batch12CompatibilityLibrary,
  paths.batch12CompatibilityActivation,
  paths.batch13CompatibilityLibrary,
  paths.batch13CompatibilityActivation,
  paths.batch14CompatibilityLibrary,
  paths.batch14CompatibilityActivation,
  paths.batch15CompatibilityLibrary,
  paths.batch15CompatibilityActivation,
  paths.scoreStabilityException,
...
assertV4(
  totals.debates === 10 &&
    totals.sections === 53 &&
    totals.moves === 194 &&
    totals.oneSidedDisplayRows === 12 &&
    totals.overallBlunders === 51 &&
    totals.emptyReferenceLinks === 49 &&
    totals.suppliedReferenceLinks === 2,
  "Batch 16 compatibility population changed"
);

const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-16-compatibility-preparation-manifest",
  protocolId: POST_CANARY_BATCH_16_COMPATIBILITY_PROTOCOL_ID,
  status: "post-canary-batch-16-compatibility-plan-prepared-and-frozen",
  frozenAt,
  productionCanary: false,
  batchNumber: 16,
  planningOnly: true,
  ...
  userAuthorization: {
    instruction:
      "Use the frozen Batch 16 standing authorization for deterministic compatibility staging.",
    scopeInterpretation:
      "Prepare, validate, freeze, commit, and push the Batch 16 production-compatibility plan; later activation and one staging pass are already covered by the standing authorization. Do not publish production ledgers, mutate production, run models, use paid services, or select Batch 17."
  },
  scope: {
    finding: "batch-16-site-ledger-adapter-and-validator-route",
    description:
      "Freeze the exact Batch 16 ledger adapters and schema-specific validator route without applying them to the active validator or production data."
  },
  invariants: {
    ...
    scoreProtocolId:
      "assessment-production-post-canary-batch-16-single-deterministic-score-pass",
  ...
  proposedValidatorRoute: {
    ...
    newSchemaVersion: POST_CANARY_BATCH_16_SITE_LEDGER_ADAPTER_VERSION,
    existingCheckpointSchemaVersion:
      "1.0-production-checkpoint-v2.2-site-ledger-adapter",
    exactRoute:
      "When a production ledger uses the Batch 16 adapter schema, load the Batch 16 packet and activation from the frozen Batch 16 compatibility root, verify their hashes, and replay the adapter with the Batch 16 validator. Keep the checkpoint and Batch 1–15 adapters on their existing roots, credentials, and validators, and keep legacy Rubric v2 and v2.1 ledgers on their existing calculators.",
    oneSidedDisplayRows:
      "Treat all sixteen authenticated adjudicated-consensus adapter schemas as requiring ledgerMoveId and permitting a section exchange with only one side populated.",
    activationRequirements: [
      "activation status is post-canary-batch-16-compatibility-execution-authorized-and-frozen",
    ...
    unchangedBehavior: [
      ...
      "the Batch 12 adapter route, credential, packets, and hashes remain valid",
      "the Batch 13 adapter route, credential, packets, and hashes remain valid",
      "the Batch 14 adapter route, credential, packets, and hashes remain valid",
      "the Batch 15 adapter route, credential, packets, and hashes remain valid",
  ...
  stagedExecutionPlan: [
    ...
    },
    {
      ...
      allowedWrites: [
        paths.activeValidator,
        `${POST_CANARY_BATCH_16_COMPATIBILITY_ROOT}/output-bundle/**`,
        paths.execution,
        paths.analysis
      ],
      actions: [
        "wire the schema-specific Batch 16 adapter route",
  ...
  mandatoryExecutionTests: {
    positive: [
      "all ten Batch 16 adapters reproduce all 194 locked move scores, 53 section scores per side, and 20 overall scores",
      "all ten final candidates match ledgerMoveId, section order, model, rubric, and score protocol",
      "all 12 one-sided display rows pass through the authenticated Batch 16 adapter route",
      "all 49 empty Overall Commentary reference arrays pass without synthesized links",
      "all 2 supplied reference links retain their exact validation behavior",
      ...
      "all ten Batch 12 adapters retain their existing activation, packet, and score replays",
      "all ten Batch 13 adapters retain their existing activation, packet, and score replays",
      "all ten Batch 14 adapters retain their existing activation, packet, and score replays",
      "all ten Batch 15 adapters retain their existing activation, packet, and score replays",
    ...
    negative: [
      ...
      "a Batch 16 adapter routed through the checkpoint or Batch 1–15 roots fails",
  ...
  stopRules: {
    ...
    batch12RouteRegressionBlocks: true,
    batch13RouteRegressionBlocks: true,
    batch14RouteRegressionBlocks: true,
    batch15RouteRegressionBlocks: true,
  ...
  nextAuthorizedAction:
    "activate-and-execute-one-batch-16-production-compatibility-pass-under-standing-authorization"
};
const preparationBytes = serializedJson(preparation);
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-16-compatibility-preparation-analysis",
  protocolId: POST_CANARY_BATCH_16_COMPATIBILITY_PROTOCOL_ID,
  status: "post-canary-batch-16-compatibility-plan-freeze-passed",
  productionCanary: false,
  batchNumber: 16,
  planningOnly: true,
  preparation: {
  ...
  checks: {
    ...
    batch12RoutePreservationSpecified: true,
    batch13RoutePreservationSpecified: true,
    batch14RoutePreservationSpecified: true,
    batch15RoutePreservationSpecified: true,
...
if (write) {
  assertV4(
    ...
    "Batch 16 compatibility execution artifact already exists; planning write blocked"
  );
  await mkdir(
    resolve(`${POST_CANARY_BATCH_16_COMPATIBILITY_ROOT}/packets`),
    { recursive: true }
  );
  ...
  assertV4(
    ...
    "stored Batch 16 compatibility preparation differs from deterministic replay"
  );
  const storedAnalysis = await readJson(paths.analysis);
  assertV4(
    canonicalJson(storedAnalysis) === canonicalJson(analysis),
    "stored Batch 16 compatibility analysis differs from deterministic replay"
  );
  for (const record of packetRecords) {
    const storedPacket = await readFile(resolve(record.packetPath), "utf8");
    assertV4(
      storedPacket === record.packetBytes,
      `${record.debateNumber}: stored Batch 16 compatibility packet differs from deterministic replay`
    );
  }
}
