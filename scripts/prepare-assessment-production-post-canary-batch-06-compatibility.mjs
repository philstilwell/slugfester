...
const paths = {
  preparation: `${POST_CANARY_BATCH_06_COMPATIBILITY_ROOT}/preparation-manifest.json`,
  analysis: `${POST_CANARY_BATCH_06_COMPATIBILITY_ROOT}/analysis.json`,
  activation: `${POST_CANARY_BATCH_06_COMPATIBILITY_ROOT}/execution-activation.json`,
  execution: `${POST_CANARY_BATCH_06_COMPATIBILITY_ROOT}/execution.json`,
  stagedLedgerRoot:
    `${POST_CANARY_BATCH_06_COMPATIBILITY_ROOT}/output-bundle/staged-ledgers`,
  compatibility:
    "docs/assessment-production/post-canary-continuation-v1/batch-06/publication-finalization/compatibility-analysis.json",
  finalizationAudit:
    "docs/assessment-production/post-canary-continuation-v1/batch-06/publication-finalization/finalization-audit.json",
  finalLedger:
    "docs/assessment-production/post-canary-continuation-v1/batch-06/final-ledger/final-ledger.json",
  scores:
    "docs/assessment-production/post-canary-continuation-v1/batch-06/score-pass/calculated-scores.json",
  renderingAnalysis:
    "docs/assessment-production/post-canary-continuation-v1/batch-06/rendering-verification/analysis.json",
  renderingAudit:
    "docs/assessment-production/post-canary-continuation-v1/batch-06/rendering-verification/rendering-audit.json",
  ...
  preparationScript:
    "scripts/prepare-assessment-production-post-canary-batch-06-compatibility.mjs",
  preparationTest:
    "scripts/test-assessment-production-post-canary-batch-06-compatibility-preparation.mjs",
...
assertV4(
  ...
  "a stable --frozen-at ISO timestamp is required for the first Batch 6 compatibility preparation write"
);
...
assertV4(
  renderingAnalysis.status ===
      "ten-debate-batch-06-rendering-verification-passed" &&
    ...
    renderingAnalysis.nextAuthorizedAction ===
      "prepare-batch-06-production-compatibility-staging",
  "Batch 6 rendering gate does not authorize compatibility planning only"
);
assertV4(
  renderingAudit.status ===
      "passed-ten-debate-batch-06-rendering-verification" &&
  ...
  "passed Batch 6 rendering evidence audit required"
);
assertV4(
  compatibility.status ===
      "batch-06-production-compatibility-boundary-recorded" &&
    canonicalJson(compatibility.findings.map((finding) => finding.id)) ===
      canonicalJson(["batch-06-site-ledger-adapter-and-validator-route"]) &&
  ...
  "frozen Batch 6 publication compatibility blocker required"
);
assertV4(
  ...
  "passed one-pass Batch 6 staging-only finalization audit required"
);
assertV4(
  scores.status ===
      "post-canary-batch-06-single-score-pass-stability-gate-passed" &&
  ...
  "accepted one-pass Batch 6 score ledger required"
);
validatePostCanaryBatch06FinalLedger(
  finalLedger,
...
const packetRecords = [];
for (const debateNumber of POST_CANARY_BATCH_06_COMPATIBILITY_ORDER) {
  ...
  const packetPath =
    `${POST_CANARY_BATCH_06_COMPATIBILITY_ROOT}/packets/debate-${debateNumber}.json`;
  ...
  const adapter = buildPostCanaryBatch06SiteLedgerAdapter({
    finalLedgerDebate,
    scoreDebate,
    candidate,
  ...
  const packet = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-06-compatibility-plan-packet",
    protocolId: POST_CANARY_BATCH_06_COMPATIBILITY_PROTOCOL_ID,
    status: "frozen-post-canary-batch-06-compatibility-plan-packet",
...
assertV4(
  totals.debates === 10 &&
    ...
    totals.emptyReferenceLinks === 52 &&
    totals.suppliedReferenceLinks === 3,
  "Batch 6 compatibility population changed"
);

const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-06-compatibility-preparation-manifest",
  protocolId: POST_CANARY_BATCH_06_COMPATIBILITY_PROTOCOL_ID,
  status: "post-canary-batch-06-compatibility-plan-prepared-and-frozen",
  frozenAt,
  productionCanary: false,
  batchNumber: 6,
  planningOnly: true,
  ...
  userAuthorization: {
    instruction:
      "Use the frozen Batch 6 standing authorization for deterministic compatibility staging.",
    scopeInterpretation:
      "Prepare, validate, freeze, commit, and push the Batch 6 production-compatibility plan; later activation and one staging pass are already covered by the standing authorization. Do not publish production ledgers, mutate production, run models, use paid services, or select Batch 7."
  },
  scope: {
    finding: "batch-06-site-ledger-adapter-and-validator-route",
    description:
      "Freeze the exact Batch 6 ledger adapters and schema-specific validator route without applying them to the active validator or production data."
  },
  invariants: {
    ...
    scoreProtocolId:
      "assessment-production-post-canary-batch-06-single-deterministic-score-pass",
  ...
  proposedValidatorRoute: {
    ...
    newSchemaVersion: POST_CANARY_BATCH_06_SITE_LEDGER_ADAPTER_VERSION,
    existingCheckpointSchemaVersion:
      "1.0-production-checkpoint-v2.2-site-ledger-adapter",
    exactRoute:
      "When a production ledger uses the Batch 6 adapter schema, load the Batch 6 packet and activation from the frozen Batch 6 compatibility root, verify their hashes, and replay the adapter with the Batch 6 validator. Keep the checkpoint and Batch 1–5 adapters on their existing roots, credentials, and validators, and keep legacy Rubric v2 and v2.1 ledgers on their existing calculators.",
    oneSidedDisplayRows:
      "Treat all seven authenticated adjudicated-consensus adapter schemas as requiring ledgerMoveId and permitting a section exchange with only one side populated.",
    activationRequirements: [
      "activation status is post-canary-batch-06-compatibility-execution-authorized-and-frozen",
  ...
  stagedExecutionPlan: [
    ...
    },
    {
      ...
      allowedWrites: [
        paths.activeValidator,
        `${POST_CANARY_BATCH_06_COMPATIBILITY_ROOT}/output-bundle/**`,
        paths.execution,
        paths.analysis
      ],
      actions: [
        "wire the schema-specific Batch 6 adapter route",
  ...
  mandatoryExecutionTests: {
    positive: [
      "all ten Batch 6 adapters reproduce all 200 locked move scores, 52 section scores per side, and 20 overall scores",
      "all ten final candidates match ledgerMoveId, section order, model, rubric, and score protocol",
      "all 8 one-sided display rows pass through the authenticated Batch 6 adapter route",
      "all 52 empty Overall Commentary reference arrays pass without synthesized links",
    ...
    negative: [
      ...
      "a Batch 6 adapter routed through the checkpoint or Batch 1–5 roots fails",
  ...
  nextAuthorizedAction:
    "activate-and-execute-one-batch-06-production-compatibility-pass-under-standing-authorization"
};
const preparationBytes = serializedJson(preparation);
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-06-compatibility-preparation-analysis",
  protocolId: POST_CANARY_BATCH_06_COMPATIBILITY_PROTOCOL_ID,
  status: "post-canary-batch-06-compatibility-plan-freeze-passed",
  productionCanary: false,
  batchNumber: 6,
  planningOnly: true,
  preparation: {
...
if (write) {
  assertV4(
    ...
    "Batch 6 compatibility execution artifact already exists; planning write blocked"
  );
  await mkdir(
    resolve(`${POST_CANARY_BATCH_06_COMPATIBILITY_ROOT}/packets`),
    { recursive: true }
  );
  ...
  assertV4(
    ...
    "stored Batch 6 compatibility preparation differs from deterministic replay"
  );
  const storedAnalysis = await readJson(paths.analysis);
  assertV4(
    canonicalJson(storedAnalysis) === canonicalJson(analysis),
    "stored Batch 6 compatibility analysis differs from deterministic replay"
  );
  for (const record of packetRecords) {
    const storedPacket = await readFile(resolve(record.packetPath), "utf8");
    assertV4(
      storedPacket === record.packetBytes,
      `${record.debateNumber}: stored Batch 6 compatibility packet differs from deterministic replay`
    );
  }
}
