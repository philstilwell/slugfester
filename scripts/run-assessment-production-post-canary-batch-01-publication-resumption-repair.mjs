...
assertV4(
  activation.status ===
      "frozen-three-isolated-four-field-batch-01-publication-resumption-repair-contexts-authorized" &&
  ...
  "the publication-resumption repair execution is not authorized"
);
assertV4(
  ...
  "the publication-resumption repair execution controls changed"
);
...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(
      os.tmpdir(),
      `batch-01-publication-resumption-repair-${context.contextIndex}-`
    )
  );
  const codexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `batch-01-publication-resumption-repair-home-${context.contextIndex}-`
    )
  );
  ...
  try {
    ...
    const prompt = [
      ...
      `Act only as isolated Batch 1 publication-resumption repair editor ${context.contextIndex} for Debate ${context.debateNumber}.`,
      "Return exactly the correctedFields entries named by the schema and no other field.",
      context.repairType === "representative-quote-exactness"
        ? "Repair the quotation only: select a 6–14 word target quotation, accepted only if it is a 3–18 word exact substring of the supplied quote-eligible sourceExcerpt. Do not normalize, add, or omit transcript tokens."
        : "Repair only the named critiques: preserve the supplied adjudicated substance and locked score band; use exactly four ordered labeled sentences, target 112–118 words, remain within 105–130 words and at least 880 characters, and end every sentence with terminal punctuation.",
      ...
      "Do not emit tags, commentary, AI Extension material, another move, another repair packet, another debate, or an unlisted field. Use no CJK, Hangul, Kana, replacement characters, or prohibited rational-invulnerability language.",
      "Return exactly one schema-conforming JSON object and nothing else."
    ].join(" ");
    process.stdout.write(
      `[batch-01-publication-resumption-repair] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
    ...
    if (
      ...
      try {
        validationSummary = validateResumptionRepairOutput(
      ...
      const validation = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-01-publication-resumption-repair-validation",
      ...
      const provenance = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-01-publication-resumption-repair-provenance",
        protocolId: activation.protocolId,
        debateNumber: context.debateNumber,
        packetIndex: context.packetIndex,
...
for (const phase of activation.executionPolicy.rampPhases) {
  ...
  phaseRecords.push({
    ...phase,
    attemptedContextIndexes: phaseResults.map(({ contextIndex }) => contextIndex),
    validContextIndexes: phaseResults
      .filter(({ gateAcceptancePassed }) => gateAcceptancePassed)
      .map(({ contextIndex }) => contextIndex),
    passed: allValid,
...
const validContexts = results.filter(
  ({ gateAcceptancePassed }) => gateAcceptancePassed
).length;
const wallElapsedMs = Date.now() - gateStarted;
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-01-publication-resumption-repair-model-execution",
  protocolId: activation.protocolId,
  status:
    validContexts === 3
      ? "batch-01-publication-resumption-three-context-repair-gate-passed"
      : "batch-01-publication-resumption-repair-gate-complete-with-failure",
  gateStartedAt,
  ...
  authorization: {
    ...
    deterministicMergeAndCompleteValidation: validContexts === 3,
    deterministicCohortReplay: validContexts === 3,
    retry: false,
