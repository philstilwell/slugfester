...
import { PROTOCOL_ID, ROOT, validateOutput } from "./lib/assessment-production-post-canary-batch-15-audio-attribution.mjs";

const shouldWrite = process.argv.includes("--write");
const base = "docs/assessment-production/post-canary-continuation-v1/batch-15/audio-verification";
...
assert.equal(execution.status, "batch-15-audio-attribution-recovery-execution-passed");
...
const passed = recoveredVerified === 2 && recoveredUnresolved === 0;
const combined = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-15-combined-audio-verification",
  protocolId: PROTOCOL_ID,
  status: passed ? "post-canary-batch-15-combined-audio-verification-passed" : "post-canary-batch-15-combined-audio-verification-unresolved",
  ...
  totals: { requiredMoves: 3, deterministicallyVerified: 1, attributionAdjudicatedVerified: recoveredVerified, verified: 1 + recoveredVerified, unresolved: recoveredUnresolved, verificationRate: (1 + recoveredVerified) / 3, paidDiarizationCallsCompleted: 3, paidDiarizationRetries: 0, additionalPaidTranscriptionCalls: 0, additionalDirectIncrementalCostUsd: 0, usageDerivedEstimatedPaidDiarizationCostUsd: originalAudit.totals.usageDerivedEstimatedCostUsd, scoresDerived: 0 },
...
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-15-audio-attribution-recovery-analysis",
  protocolId: PROTOCOL_ID,
  status: passed ? "batch-15-audio-attribution-recovery-passed" : "batch-15-audio-attribution-recovery-unresolved",
  productionCanary: false,
  batchNumber: 15,
  stagingOnly: true,
