...
import {
  ...
  POST_CANARY_BATCH_14_PUBLICATION_OUTPUT_VERSION,
...
const MANIFEST =
  `${POST_CANARY_BATCH_14_PUBLICATION_ROOT}/execution-preparation-manifest.json`;
...
assert.equal(manifest.protocolId, POST_CANARY_BATCH_14_PUBLICATION_PROTOCOL_ID);
assert.equal(manifest.productionCanary, false);
assert.equal(manifest.batchNumber, 14);
...
assert.equal(manifest.totals.moves, 190);
assert.equal(manifest.totals.sections, 52);
assert.equal(manifest.totals.audioVerifiedMoves, 12);
...
for (const context of manifest.contexts) {
  ...
  assert.equal(packet.protocolId, POST_CANARY_BATCH_14_PUBLICATION_PROTOCOL_ID);
  ...
  assert.deepEqual(schema, buildPostCanaryBatch14PublicationSchema(packet));
  assert.equal(schema.properties.schemaVersion.const, POST_CANARY_BATCH_14_PUBLICATION_OUTPUT_VERSION);
  assert.equal(schema.properties.protocolId.const, POST_CANARY_BATCH_14_PUBLICATION_PROTOCOL_ID);
...
assert.equal(moves, 190);
assert.equal(sections, 52);
assert.equal(quoteEligibleMoves, manifest.totals.quoteEligibleMoves);
assert.equal(audioVerifiedMoves, 12);
assert.equal(
