...
assertV4(
  activation.status ===
    ...
    activation.productionCanary === true &&
  ...
  "production checkpoint v2.2 side-selector execution is unauthorized"
);
...
async function executeContext(context, contextIndex) {
  ...
  const sourceDirectory = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-pcv22-side-${context.debateNumber}-${context.side}-`
    )
  );
  const isolatedCodexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-pcv22-side-home-${context.debateNumber}-${context.side}-`
    )
  );
  ...
  process.stdout.write(
    `[production-checkpoint-v2.2-side-selector] Debate ${context.debateNumber} ${context.side} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  ...
  status:
    validContexts === preparation.contexts.length
      ...
      : "production-checkpoint-v2.2-side-selector-gate-complete-with-failure",
  ...
  meteredApiCostUsd: 0,
  transcriptionCostUsd: 0,
  failedProductionCanaryV1Reclassified: false,
  ...
  nextRequiredAction:
    validContexts === preparation.contexts.length
      ? "analyze-compile-and-lock-production-checkpoint-v2.2-inventories-model-free-only"
      : "stop-preserve-failed-production-checkpoint-v2.2-side-selector-gate",
};
