...
const artifact = await readJson(`${V3811_PERFORMANCE_ROOT}/initial-disagreements.json`);
assertV3811(artifact.status === "passed-deterministic-disagreement-extraction", "disagreement artifact status invalid");
assertV3811(artifact.evidenceBoundary.originalCleanGatePassed && !artifact.evidenceBoundary.postHocRepresentationRecoveryUsed && artifact.evidenceBoundary.attempts === 6 && artifact.evidenceBoundary.retries === 0 && artifact.evidenceBoundary.rawOutputsPreserved, "disagreement evidence boundary invalid");
assertV3811(artifact.debates.length === 3 && artifact.authorization.prepareDisputeOnlyAdjudicationPackets && !artifact.authorization.adjudicationModelExecution && !artifact.authorization.scoreDerivation, "disagreement authorization invalid");
...
for (const debateNumber of V3811_PERFORMANCE_DEBATES) {
  ...
  assertV3811(record, `${debateNumber}: missing disagreement record`);
  ...
  assertV3811(record.moveCount === packet.moves.length && passA.moveJudgments.length === packet.moves.length && passB.moveJudgments.length === packet.moves.length, `${debateNumber}: move count mismatch`);
  ...
  for (let index = 0; index < packet.moves.length; index += 1) {
    ...
    assertV3811(Boolean(stored) === disagreement.disputed, `${debateNumber}:${judgmentA.moveId}: dispute presence mismatch`);
    ...
    for (const key of V3811_RATING_KEYS) {
      ...
      assertV3811(Boolean(merge) === expected, `${debateNumber}:${judgmentA.moveId}:${key}: nondisputed merge presence mismatch`);
      if (expected) {
        expectedMerges += 1;
  ...
  assertV3811(disputeMap.size === expectedMoveDisputes && mergeMap.size === expectedMerges, `${debateNumber}: unexpected duplicate or extra dispute entries`);
  ...
  assertV3811(record.disputedMoveCount === expectedMoveDisputes && record.nondisputedUnequalScalarFieldCount === expectedMerges && record.burdenAdjustmentDisputeCount === expectedAdjustmentDisputes.length, `${debateNumber}: stored counts mismatch`);
...
assertV3811(totals.uniqueMoves === 81, "validator must cover 81 moves");
