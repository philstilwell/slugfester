...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-02/source-preparation";
...
assert.equal(preparation.status, "post-canary-batch-02-ten-complete-score-blind-source-packets-prepared-awaiting-validation");
...
assert.deepEqual(preparation.tokenLedgerCompatibility.occurrences, [{
  debateNumber: "99",
  eventIndex: 3272,
  startMs: 6837429,
  durationMs: 2851,
  textSha256: "49168d2b71b06d64d686d39414b8b2135e6d12e0c7ec2bc2b78e03eae6b87691",
}]);
assert.equal(preparation.totals.zeroLexicalTokenRowOccurrences, 1);
...
for (const context of preparation.contexts) {
  ...
  assert(frozen, `${context.debateNumber}: absent from frozen Batch 2 selection`);
  ...
  assert.equal(packet.schemaVersion, "1.0-assessment-production-post-canary-batch-02-score-blind-source-packet");
  ...
  for (const chunk of context.chunks) {
    ...
    assert.deepEqual(
      ...
      findBatch02ZeroLexicalTokenRows(chunkBytes).map(({ eventIndex, startMs, durationMs, text }) => ({
        eventIndex,
        startMs,
        durationMs,
...
const validation = {
  ...
  status: "post-canary-batch-02-score-blind-source-packet-validation-passed-frozen-awaiting-user-approval-before-any-model-execution",
  ...
  nextAuthorizedAction: "stop-and-obtain-user-approval-before-any-model-execution-or-unexpected-paid-service",
};
