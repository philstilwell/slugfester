...
assertV4(preparation.contexts.length === 5 && preparation.totals.discoveryContexts === 18, "held-out discovery source count drifted");
assertV4(preparation.totals.ownershipBoundedSchemas === 18, "hardened ownership schema count drifted");
...
if (!(await exists(MANIFEST))) {
  console.log(JSON.stringify({ status: "passed-prefreeze", debates: 5, contexts: 18, ownershipBoundedSchemas: 18, modelContexts: 0, scoresDerived: 0 }, null, 2));
  process.exit(0);
}

const manifest = JSON.parse(await readFile(MANIFEST, "utf8"));
assertV4(manifest.contexts.length === 18, "frozen held-out context count drifted");
assertV4(manifest.schemaHardening.candidateStartOwnedCoreBounds && manifest.schemaHardening.candidateEndAvailableContextBounds, "schema hardening drifted");
...
if (!(await exists(manifest.artifacts.execution))) {
  ...
  console.log(JSON.stringify({ status: "passed-frozen", debates: 5, contexts: 18, maximumParallelContexts: 4, modelContexts: 0, scoresDerived: 0 }, null, 2));
  process.exit(0);
}

const execution = JSON.parse(await readFile(manifest.artifacts.execution, "utf8"));
assertV4(execution.contextsAttempted === 18 && execution.retries === 0, "execution attempt ledger drifted");
assertV4(execution.maximumParallelContextsObserved <= 4, "scheduler exceeded frozen concurrency");
if (execution.status !== "eighteen-replacement-held-out-discovery-contexts-passed") {
...
assertV4(execution.validContexts === 18 && execution.results.every((result) => result.accepted), "valid context ledger drifted");
...
if (!(await exists(manifest.artifacts.analysis))) {
  console.log(JSON.stringify({ status: "passed-execution", validContexts: 18, retries: 0, wallElapsedMinutes: Number((execution.wallElapsedMs / 60000).toFixed(2)), scoresDerived: 0 }, null, 2));
  process.exit(0);
}
