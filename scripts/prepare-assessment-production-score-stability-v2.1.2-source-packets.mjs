...
const CACHE_VERSION =
  "assessment-production-score-stability-v2.1.2-fresh-validation";
...
const SCRIPT =
  "scripts/prepare-assessment-production-score-stability-v2.1.2-source-packets.mjs";
const TEST =
  "scripts/test-assessment-production-score-stability-v2.1.2-source-packets.mjs";
const SOURCE_FILES = [
  SELECTION,
  SUCCESSOR_ANALYSIS,
  DISCOVERY_MANUAL,
...
assertV4(
  ...
  "v2.1.2 selection does not authorize model-free source preparation"
);
assertV4(
  ...
  "frozen v2.1.2 model or isolation boundary drifted"
);
assertV4(
  selection.policy.version === "v2.1-proposal" &&
    ...
    selection.successorProtocol.minimumLexicalTokens ===
      V212_DISCOVERY_MINIMUM_LEXICAL_TOKENS &&
    selection.successorProtocol.repositoryDerivedLexicalTokenCount === true &&
    selection.successorProtocol
      .modelAuthoredEndEventStructurallyBoundedByLockedContext === true &&
    selection.successorProtocol.requestedLexicalTokensRemoved === true &&
    JSON.stringify(selection.successorProtocol.sourceSelectionShape) ===
...
assertV4(
  analysis.status ===
    ...
    analysis.failedGateDisposition.v211DiscoveryPreservedFailed === true &&
    analysis.failedGateDisposition.retiredOutputsAcceptedForSuccessorEvidence ===
      false &&
    analysis.failedGateDisposition.currentCanaryReclassified === false &&
    ...
    analysis.successorContract.automaticTruncation === false &&
...
for (const selected of selection.selected) {
  ...
  for (const chunk of plan.chunks) {
    ...
    assertV4(
      ...
      `${selected.debateNumber}/${chunk.chunkId}: bounded source-window schema mismatch`
    );
...
const preparation = {
  schemaVersion:
    "1.0-score-stability-v2.1.2-fresh-validation-source-preparation",
