...
const RECOVERED_EXECUTION = `${RECOVERY}/cohort-execution-overlay.json`;
...
const [executionBytes, recoveryAnalysis, originalExecutionBytes] =
  await Promise.all([
    readFile(RECOVERED_EXECUTION),
...
assertV4(
  activation.status ===
    ...
    execution.recovery?.failedContextIndex === 13 &&
    execution.recovery?.fieldDisjointShardContextsAttempted === 2 &&
    execution.recovery?.fieldDisjointShardContextsPassed === 2 &&
    execution.recovery?.originalAcceptedJudgmentsPreservedByteIdentical === 19 &&
    ...
    recoveryAnalysis.cohortExecutionOverlaySha256 === sha256(executionBytes) &&
    recoveryAnalysis.protectedAcceptedOutputsByteIdentical === true &&
    recoveryAnalysis.moveFieldsAcceptedExactlyOnce === 20 &&
  ...
  "Batch 11 judgment execution crossed its frozen boundary"
);
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-11-independent-judgment-analysis-after-bounded-recovery",
  protocolId: activation.protocolId,
  status: passed
    ? "twenty-post-canary-batch-11-independent-judgments-passed-standing-authorization-active-for-disagreement-extraction"
    : "post-canary-batch-11-independent-judgment-gate-failed-analysis-only",
  ...
  recoveryAudit: {
    recoveredContextIndex: 13,
    recoveredDebateNumber: "60",
    ...
    postFailureSchedulerDefectRecorded: true,
  ...
  nextAuthorizedAction: passed
    ...
    : "stop-post-canary-batch-11-independent-judgment-gate-for-audit",
};
if (shouldWrite) {
  await writeFile(
