...
import { sha256 } from "./lib/assessment-production-post-canary-batch-17-production-publication.mjs";
...
assert.equal(activation.status, "frozen-batch-17-generated-seo-correction-pass-activated");
assert.equal(activation.directIncrementalCostCapUsd, 0);
assert.equal(activation.frozenEnumeration.outputCount, 380);
assert.equal(activation.frozenEnumeration.proposedWrites.length, 6);
...
assert.equal(activation.authorization.exactSixGeneratedDerivativeWrites, true);
...
console.log(JSON.stringify({ status: "passed", isolatedGeneratorRuns: 1, proposedWrites: 6, completeRepositoryValidationRuns: 1 }, null, 2));
