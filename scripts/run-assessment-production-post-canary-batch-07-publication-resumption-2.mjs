...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), `batch-07-publication-resumption-2-${context.debateNumber}-`)
  );
  const codexHome = await mkdtemp(
    path.join(os.tmpdir(), `batch-07-publication-resumption-2-home-${context.debateNumber}-`)
  );
  ...
  try {
    ...
    if (
      ...
      const validationRecord = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-07-publication-resumption-2-validation",
      ...
      const provenanceRecord = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-07-publication-resumption-2-provenance",
        protocolId: activation.protocolId,
        contextIndex: context.contextIndex,
        originalContextIndex: context.originalContextIndex,
        debateNumber: context.debateNumber,
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-07-publication-resumption-2-model-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "eight-post-canary-batch-07-publication-resumption-2-contexts-passed"
    : "post-canary-batch-07-publication-resumption-2-complete-with-failure",
  gateStartedAt,
  ...
  participantJudgmentWasScoreBlind: true,
  ownDebateScoresImmutable: true,
  acceptedDebate193RepairContexts: 1,
  acceptedDebate193RepairedFields: 2,
  acceptedDebate80RepairContexts: 3,
  acceptedDebate80RepairedFields: 5,
  meteredApiCostUsd: 0,
  paidServiceCallsThisStage: 0,
  modelAuthoredScores: 0,
  scorePassesExecutedThisStage: 0,
