...
import {
  ...
  ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_03_DISAGREEMENT_ROOT,
  extractAssessmentProductionPostCanaryBatch03Disagreements
} from "./lib/assessment-production-post-canary-batch-03-disagreement.mjs";
import {
...
const TOOL_SOURCES = [
  "scripts/extract-assessment-production-post-canary-batch-03-disagreements.mjs",
  ...
  "scripts/test-assessment-production-post-canary-batch-03-disagreements.mjs"
];
...
assertV4(
  judgmentAnalysis.status ===
    ...
    judgmentAnalysis.nextAuthorizedAction ===
      "extract-freeze-and-analyze-batch-03-disagreements-under-standing-authorization",
  "Batch 3 accepted-judgment gate or prior stop rule changed"
);
assertV4(
  JSON.stringify(judgmentAnalysis.pairs.map((pair) => pair.debateNumber)) ===
    JSON.stringify(EXPECTED_DEBATES),
  "Batch 3 independent-judgment population changed"
);
assertV4(
  ...
  "Batch 3 does not contain ten accepted, independent A/B judgment pairs"
);
assertV4(
  ...
  "Batch 3 twenty-context accepted execution contract changed"
);
assertV4(
  ...
  "Batch 3 model, authentication, score-blindness, or tie contract changed"
);
assertV4(
  executionActivation.executionPolicy.attemptsPerContext === 1 &&
    ...
    JSON.stringify(executionActivation.executionPolicy.schedulerRamp) ===
      JSON.stringify([1, 2]),
  "Batch 3 one-attempt or frozen 1-to-2 scheduler stop rule changed"
);
assertV4(
...
assertV4(
  ...
  "Batch 3 accepted execution evidence changed"
);
...
const sourceHashes = {
  ...
  [POST_CANARY_BATCH_03_STANDING_AUTHORIZATION]: standingAuthorization.sha256
};
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const disagreementPath =
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_03_DISAGREEMENT_ROOT}/disagreements/debate-${debateNumber}.json`;
...
assertV4(totalMoves === 200, "Batch 3 extraction must compare 200 moves");
assertV4(
  JSON.stringify(extractedAudioQueue.map(queueKey).sort()) ===
    JSON.stringify(independentGateQueue.map(queueKey).sort()),
  "Batch 3 frozen audio queue was not reproduced"
);

const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-disagreement-extraction",
  protocolId:
    ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_03_DISAGREEMENT_PROTOCOL_ID,
  status:
  ...
  nextAuthorizedAction:
    "prepare-freeze-and-analyze-batch-03-local-audio-source-work-items-under-standing-authorization"
};

if (shouldWrite) {
  await mkdir(ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_03_DISAGREEMENT_ROOT, {
    recursive: true
  });
  await writeFile(
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_03_DISAGREEMENT_ROOT}/analysis.json`,
