...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-07";
...
const PRIOR_SELECTION = "docs/assessment-production/post-canary-continuation-v1/batch-06/selection.json";
const PRIOR_SELECTION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-06/selection-analysis.json";
const PRIOR_MUTATION_MANIFEST = "docs/assessment-production/post-canary-continuation-v1/batch-06/production-publication/mutation-manifest.json";
const PRIOR_COMPLETION_EXECUTION = "docs/assessment-production/post-canary-continuation-v1/batch-06/production-publication/generated-seo-correction/execution.json";
const PRIOR_COMPLETION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-06/production-publication/generated-seo-correction/analysis.json";
const PRIOR_COMPLETION_COMMIT = "6dde76ca52ece31adf4d02c29c937ff2cc53b287";
const SCRIPT = "scripts/select-assessment-production-post-canary-batch-07.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-07-selection.mjs";
...
assertV4(!(await exists(FUTURE_SOURCE_PREPARATION)), "source preparation already exists before Batch 7 selection");
...
assertV4(
  ...
  "Batch 7 selection is not compatible with the frozen continuation policy"
);
...
assertV4(
  priorSelection.status === "sixth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    priorSelection.protocolId === "assessment-production-post-canary-batch-06" &&
    priorSelection.productionSequenceOrdinal === 7 &&
    priorSelection.selected.length === 10 &&
    priorSelectionAnalysis.status === "sixth-post-canary-batch-selection-analysis-passed-under-standing-authorization" &&
    priorSelectionAnalysis.selection.sha256 === sha256(controlBytes[PRIOR_SELECTION]) &&
    priorMutationManifest.status === "frozen-batch-06-production-publication-mutation-manifest-prepared" &&
    JSON.stringify(priorMutationManifest.explicitOrder) === JSON.stringify(priorSelectedNumbers) &&
    priorCompletionExecution.status === "passed-complete-batch-06-transaction-ready-for-atomic-commit" &&
    ...
    priorCompletionAnalysis.status === "batch-06-production-publication-accepted-for-atomic-commit-after-generated-seo-correction" &&
  ...
  "Batch 6 has not satisfied the frozen successful-publication prerequisite"
);
assertV4(git("rev-parse", "--abbrev-ref", "HEAD") === "main", "Batch 7 selection must run on main");
assertV4(git("rev-parse", "HEAD") === PRIOR_COMPLETION_COMMIT, "Batch 7 selection checkpoint is not the exact Batch 6 publication commit");
assertV4(git("rev-parse", "origin/main") === PRIOR_COMPLETION_COMMIT, "Batch 6 publication commit is not the pushed origin/main checkpoint");
...
assertV4(
  JSON.stringify(priorSelectedNumbers) === JSON.stringify(ranked.slice(50, 60).map((item) => item.debateNumber)),
  "Batch 6 is not the immutable sixth rank slice"
);

const selected = [];
let totalEvents = 0;
let totalDurationMs = 0;
for (const [offset, rankedItem] of ranked.slice(60, 70).entries()) {
...
assertV4(selected.length === 10 && new Set(selected.map((item) => item.debateNumber)).size === 10, "Batch 7 did not select ten unique debates");

const checkpointCommit = git("rev-parse", "HEAD");
const selection = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-selection",
  protocolId: "assessment-production-post-canary-batch-07",
  status: "seventh-post-canary-ten-debate-batch-selection-frozen-source-gate-passed",
  frozenAt,
  checkpointCommit,
  branch: "main",
  ...
  stagingOnly: true,
  batchNumber: 7,
  productionSequenceOrdinal: 8,
  userAuthorization: {
    instruction: "Continue.",
    scopeInterpretation: "Materialize and source-validate only the deterministic Batch 7 selection under the frozen continuation policy after authenticating successful Batch 6 publication. This selection pass prepares no source packets, creates no Batch 7 standing authorization, executes no models, derives no scores, and performs no production mutation.",
  ...
  eligibility: {
    ...
    previouslyPublishedAndRemovedCount: 60,
    remainingBeforeBatchCount: ranked.length - 60,
    remainingBeforeBatchRankedSha256: sha256(serializedJson(ranked.slice(60))),
    selectedCount: selected.length,
    remainingUnselectedCount: ranked.length - 70,
    dyadicOnly: true,
  ...
  nextAuthorizedAction: "prepare-batch-07-standing-authorization-and-source-packets-under-user-authorization"
};
...
const analysis = {
  ...
  status: "seventh-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization",
  analyzedAt: frozenAt,
  ...
  finding: "Successful Batch 6 production publication was authenticated at the pushed atomic commit. The immutable campaign rank then selected ordinals 61 through 70 as ten unique pending dyadic debates, and all thirty effective source files and ten canonical event chains passed without replacement.",
  decision: {
