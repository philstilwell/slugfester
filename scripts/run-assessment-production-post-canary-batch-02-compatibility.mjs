...
assertV4(write, "Batch 2 compatibility runner requires --write");
...
const paths = {
  root: POST_CANARY_BATCH_02_COMPATIBILITY_ROOT,
...
assertV4(
  ...
  "Batch 2 compatibility pass was already attempted; rerun forbidden"
);
...
assertV4(
  ...
  "frozen Batch 2 compatibility activation is unavailable"
);
...
tests.push(
  runCommand(
    "batch-02-route-positive-negative-controls",
    process.execPath,
...
if (tests.at(-1).passed) {
  tests.push(
    runCommand("checkpoint-batch-01-legacy-reference-regression", process.execPath, [
...
const nextAuthorizedAction = compatibilityAcceptancePassed
  ? "user-approval-required-before-batch-02-production-publication-mutation-manifest-preparation-only"
  : "user-approval-required-before-batch-02-compatibility-failure-diagnosis-only";
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-compatibility-execution",
  protocolId: preparation.protocolId,
  status: compatibilityAcceptancePassed
    ? "batch-02-route-score-checkpoint-batch-01-legacy-reference-validation-passed"
    : "failed-closed-batch-02-compatibility-staging-pass",
  startedAt,
  completedAt,
  activation: {
  ...
  regressions: {
    batch02RouteControlsPassed: tests[0]?.passed === true,
    checkpointBatch01LegacyReferencePassed: tests[1]?.passed === true,
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-compatibility-execution-analysis",
  protocolId: preparation.protocolId,
  status: compatibilityAcceptancePassed
    ? "batch-02-production-compatibility-passed"
    : "batch-02-production-compatibility-failed-closed",
  ...
  checks: {
    ...
    routeAndScoreControlsPassed: execution.regressions.batch02RouteControlsPassed,
    checkpointBatch01LegacyReferencePassed:
      execution.regressions.checkpointBatch01LegacyReferencePassed,
