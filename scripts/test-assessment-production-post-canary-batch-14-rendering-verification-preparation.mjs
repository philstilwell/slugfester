...
const manifest = JSON.parse(await readFile(path.resolve(
  `${POST_CANARY_BATCH_14_RENDERING_ROOT}/preparation-manifest.json`), "utf8"));
const exists = (file) => access(path.resolve(file)).then(() => true, () => false);
assertV4(manifest.status ===
  "frozen-post-canary-batch-14-rendering-verification-prepared" &&
  manifest.protocolId === POST_CANARY_BATCH_14_RENDERING_PROTOCOL_ID &&
  manifest.packets?.length === 10 &&
  canonicalJson(manifest.explicitOrder) ===
    canonicalJson(POST_CANARY_BATCH_14_RENDERING_ORDER) &&
  manifest.gateExpectations?.sections === 52 &&
...
"Batch 14 rendering preparation changed");
...
for (const row of manifest.packets) {
  ...
  validatePostCanaryBatch14RenderingPacket(JSON.parse(packetBytes));
}
...
console.log(JSON.stringify({ status: "passed", debates: 10,
  sections: 52, moves: 190, packets: 10,
