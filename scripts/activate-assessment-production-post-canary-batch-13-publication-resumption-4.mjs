...
assertV4(!(await exists(ACTIVATION)), "four-context resumption activation already exists");
const preparationBytes = await readFile(path.resolve(PREPARATION));
const preparation = JSON.parse(preparationBytes);
assertV4(
  preparation.protocolId === PROTOCOL_ID &&
    preparation.status === "frozen-four-original-unattempted-batch-13-publication-contexts-prepared-not-activated" &&
    preparation.contexts?.length === 4 && preparation.totals?.moves === 79 &&
  ...
  "four original unattempted contexts are not prepared"
);
assertV4(
  ...
  "four-context model or execution controls changed"
);
assertV4(
...
const activation = {
  ...
  status: "frozen-four-original-unattempted-batch-13-publication-contexts-activated",
  activatedAt,
  ...
  nextAuthorizedAction: "execute-only-four-frozen-original-unattempted-publication-contexts"
};
