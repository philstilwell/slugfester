...
assert.deepEqual(
  manifest.contexts.map((item) => item.debateNumber),
  EXPECTED_DEBATES
);
...
assert.equal(manifest.userAuthorization.audioCostEstimateAcknowledgedUsd, 0.1190425);
assert.equal(
  manifest.userAuthorization.audioCostCapExceedanceAcknowledgedUsd,
  0.0190425
);
assert.equal(manifest.userAuthorization.directIncrementalCostUsdMaximum, 0);
assert.equal(manifest.priorCostControl.additionalPaidCallsThisStage, 0);
...
for (const context of manifest.contexts) {
  ...
  const audioBytes = (
    await Promise.all(
      context.audioTranscriptInputs.map((item) => readFile(item.sourcePath))
    )
  ).reduce((sum, bytes) => sum + bytes.length, 0);
