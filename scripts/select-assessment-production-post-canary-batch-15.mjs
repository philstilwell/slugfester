...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-15";
...
const PRIOR_SELECTION = "docs/assessment-production/post-canary-continuation-v1/batch-14/selection.json";
const PRIOR_SELECTION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-14/selection-analysis.json";
const PRIOR_MUTATION_MANIFEST = "docs/assessment-production/post-canary-continuation-v1/batch-14/production-publication/mutation-manifest.json";
const PRIOR_COMPLETION_EXECUTION = "docs/assessment-production/post-canary-continuation-v1/batch-14/production-publication/generated-seo-correction/execution.json";
const PRIOR_COMPLETION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-14/production-publication/generated-seo-correction/analysis.json";
const PRIOR_COMPLETION_COMMIT = "cbacd97e1366e2a42ff462b3e623bcff80bfa3c3";
const SCRIPT = "scripts/select-assessment-production-post-canary-batch-15.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-15-selection.mjs";
...
assertV4(!(await exists(FUTURE_SOURCE_PREPARATION)), "source preparation already exists before Batch 15 selection");
...
assertV4(
  ...
  "Batch 15 selection is not compatible with the frozen continuation policy"
);
...
assertV4(
  priorSelection.status === "fourteenth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    priorSelection.protocolId === "assessment-production-post-canary-batch-14" &&
    priorSelection.productionSequenceOrdinal === 15 &&
    priorSelection.selected.length === 10 &&
    priorSelectionAnalysis.status === "fourteenth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization" &&
    priorSelectionAnalysis.selection.sha256 === sha256(controlBytes[PRIOR_SELECTION]) &&
    priorMutationManifest.status === "frozen-batch-14-production-publication-mutation-manifest-prepared" &&
    JSON.stringify(priorMutationManifest.explicitOrder) === JSON.stringify(priorSelectedNumbers) &&
    priorCompletionExecution.status === "passed-complete-batch-14-transaction-ready-for-atomic-commit" &&
    ...
    priorCompletionAnalysis.status === "batch-14-production-publication-accepted-for-atomic-commit-after-generated-seo-correction" &&
    priorCompletionAnalysis.decision.generatedSeoCorrectionGatePassed === true &&
    priorCompletionAnalysis.decision.batch14ProductionPublicationGatePassed === true &&
  ...
  "Batch 14 has not satisfied the frozen successful-publication prerequisite"
);
assertV4(git("rev-parse", "--abbrev-ref", "HEAD") === "main", "Batch 15 selection must run on main");
assertV4(git("rev-parse", "HEAD") === PRIOR_COMPLETION_COMMIT, "Batch 15 selection checkpoint is not the exact Batch 14 publication commit");
assertV4(git("rev-parse", "origin/main") === PRIOR_COMPLETION_COMMIT, "Batch 14 publication commit is not the pushed origin/main checkpoint");
...
assertV4(
  JSON.stringify(priorSelectedNumbers) === JSON.stringify(ranked.slice(130, 140).map((item) => item.debateNumber)),
  "Batch 14 is not the immutable fourteenth rank slice"
);

const selected = [];
let totalEvents = 0;
let totalDurationMs = 0;
for (const [offset, rankedItem] of ranked.slice(140, 150).entries()) {
...
assertV4(selected.length === 10 && new Set(selected.map((item) => item.debateNumber)).size === 10, "Batch 15 did not select ten unique debates");

const checkpointCommit = git("rev-parse", "HEAD");
const selection = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-selection",
  protocolId: "assessment-production-post-canary-batch-15",
  status: "fifteenth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed",
  frozenAt,
  checkpointCommit,
  branch: "main",
  ...
  stagingOnly: true,
  batchNumber: 15,
  productionSequenceOrdinal: 16,
  userAuthorization: {
    instruction: "Continue the SLUGFESTER reassessment campaign with Batch 15 in /Users/philstilwell/Documents/SLUGFESTER.com.",
    scopeInterpretation: "Materialize and source-validate only the deterministic Batch 15 selection under the frozen continuation policy after authenticating successful Batch 14 publication. This selection pass prepares no source packets, executes no models, derives no scores, and performs no production mutation.",
  ...
  eligibility: {
    ...
    previouslyPublishedAndRemovedCount: 140,
    remainingBeforeBatchCount: ranked.length - 140,
    remainingBeforeBatchRankedSha256: sha256(serializedJson(ranked.slice(140))),
    selectedCount: selected.length,
    remainingUnselectedCount: ranked.length - 150,
    dyadicOnly: true,
  ...
  nextAuthorizedAction: "prepare-batch-15-standing-authorization-and-source-packets-under-user-authorization"
};
...
const analysis = {
  ...
  status: "fifteenth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization",
  analyzedAt: frozenAt,
  ...
  finding: "Successful Batch 14 production publication was authenticated at the pushed atomic commit. The immutable campaign rank then selected ordinals 141 through 150 as ten unique pending dyadic debates, and all thirty effective source files and ten canonical event chains passed without replacement.",
  decision: {
