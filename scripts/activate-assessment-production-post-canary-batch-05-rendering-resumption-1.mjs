...
assertV4(
  !(await exists(POST_CANARY_BATCH_05_RENDERING_RESUMPTION_1_ACTIVATION)),
  "Batch 5 rendering resumption-1 activation already exists"
);
...
assertV4(
  ...
  "valid frozen Batch 5 rendering resumption-1 preparation required"
);
execFileSync(
  process.execPath,
  [
...
await loadPostCanaryBatch05RenderingResumption1Packets(preparation);
...
for (const output of preparation.futureOutputPathsExcludedFromSourceHashes) {
  if (output !== POST_CANARY_BATCH_05_RENDERING_RESUMPTION_1_ACTIVATION) {
...
const activation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-05-rendering-transport-readiness-resumption-1-activation",
  protocolId: POST_CANARY_BATCH_05_RENDERING_RESUMPTION_1_PROTOCOL_ID,
  status: "frozen-batch-05-rendering-resumption-1-authorized",
  authorizedAt,
  activationCommit,
  ...
  nextRequiredAction:
    "load-frozen-documentation-then-execute-exactly-twenty-batch-05-rendering-resumption-viewports"
};

if (shouldWrite) {
  await writeFile(
    path.resolve(POST_CANARY_BATCH_05_RENDERING_RESUMPTION_1_ACTIVATION),
