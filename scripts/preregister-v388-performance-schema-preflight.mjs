...
const subsetAudit = await readJson(`${V388_PERFORMANCE_ROOT}/schema-keyword-recovery-audit.json`);
assertV388(subsetAudit.status === "passed-semantic-preserving-unsupported-keyword-removal" && subsetAudit.supportedSubsetLint.status === "passed" && subsetAudit.authorization.oneContextEndpointPreflightRequiredBeforeAnyFurtherSixContextRun, "supported-subset audit does not authorize preflight construction");
const syntheticPacket = {
  schemaVersion: "3.8.8-performance-schema-synthetic-preflight-packet",
...
const sourceFiles = [schemaPath, packetPath, `${V388_PERFORMANCE_ROOT}/schema-keyword-recovery-audit.json`, "scripts/lib/v388-performance-judgment.mjs", "scripts/validate-v388-performance-judgment-output.mjs", "scripts/preregister-v388-performance-schema-preflight.mjs", "scripts/validate-v388-performance-schema-preflight-lock.mjs", "scripts/run-v388-performance-schema-preflight.mjs"];
const sourceHashes = {};
...
const manifest = {
  schemaVersion: "3.8.8-performance-schema-endpoint-preflight-manifest",
  ...
  purpose: "Prove endpoint acceptance of the exact shared schema before any further debate contexts.",
  syntheticOnly: true,
