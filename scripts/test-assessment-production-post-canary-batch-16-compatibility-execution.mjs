...
const root = POST_CANARY_BATCH_16_COMPATIBILITY_ROOT;
...
const validator = await import(
  `./validate-debates.mjs?batch16Compatibility=${activation.validator.proposedSha256}`
);
...
for (const lock of activation.packetHashes) {
  ...
  const routeLock =
    validator.validatePostCanaryBatch16LedgerAdapterRouteLocks({
      debate: candidate,
      ledgerText,
  ...
  const route = validator.validatePostCanaryBatch16LedgerAdapterRoute({
    debate: candidate,
    ledger,
    ledgerText
  });
  assert.equal(route.repositoryScoreReplayPassed, true);
  const adapter = validatePostCanaryBatch16SiteLedgerAdapter({
    adapter: ledger,
    candidate,
...
assert.equal(oneSidedDisplayRows, 12);
assert.equal(emptyReferenceLinks, 49);
assert.equal(suppliedReferenceLinks, 2);
assert.equal(
  ...
  JSON.stringify(
    {
      ...
      positiveRoutes,
      negativeControls,
      existingAdapterRoutesPreserved: 16,
      modelContexts: 0,
      paidServiceCalls: 0,
