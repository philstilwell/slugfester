...
const ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-02/inventory-candidate-sharded";
...
const SCRIPT =
  "scripts/run-assessment-production-post-canary-batch-02-inventory-plans.mjs";
const PREPARER =
  "scripts/prepare-assessment-production-post-canary-batch-02-inventory.mjs";
const CONTEXTS = 10;
const DEBATES = ["103", "172", "04", "136", "83", "66", "126", "99", "93", "101"];
const PREPARATION_STATUS =
  "frozen-ten-post-canary-batch-02-candidate-census-plan-contexts-prepared-not-authorized";
const ACTIVATION_STATUS =
  "frozen-ten-post-canary-batch-02-candidate-census-plan-contexts-authorized";
const EXECUTION_STATUS =
  "ten-post-canary-batch-02-candidate-census-plan-contexts-passed";
const PROTOCOL_ID =
  "assessment-production-post-canary-batch-02-candidate-sharded-inventory";
...
async function loadAndValidatePreparation({ requireFutureAbsent = false } = {}) {
  ...
  assertV4(
    preparation.schemaVersion ===
      "1.0-assessment-production-post-canary-batch-02-candidate-census-plan-execution-preparation-manifest" &&
    ...
    "Batch 2 candidate-census planner preparation is invalid"
  );
  assertV4(
    sourcePreparation.protocolId === PROTOCOL_ID &&
      sourcePreparation.status ===
        "post-canary-batch-02-candidate-sharded-source-assets-and-ten-planner-packets-frozen" &&
      ...
      sourcePreparation.sourceCompatibility.minimumCandidateLexicalTokensChanged ===
        false &&
      sourcePreparation.sourceCompatibility.occurrences.length === 1 &&
      sha256(sourcePreparationBytes) === preparation.preparationSha256,
    "Batch 2 source preparation drifted"
  );
...
async function preflight() {
  ...
  assertV4(
    ...
    "frozen Batch 2 preparation validator failed"
  );
  console.log(
    JSON.stringify(
      {
...
async function activate() {
  ...
  const activation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-02-candidate-census-plan-execution-activation",
    ...
    userAuthorization: {
      ...
      approvedScope:
        "Activate and execute exactly the ten frozen Batch 2 candidate-census planner contexts using 5.6 Sol with low reasoning effort through the ChatGPT subscription, with a direct incremental cost cap of $0. Use the frozen 1→2 scheduler and one attempt per context, then stop after deterministic plan validation, analysis, commit, and push. Do not prepare or execute side-selector packets or models, judgment models, paid services, audio work, score derivation, publication reconstruction, or production mutation.",
    ...
    nextRequiredAction:
      "execute-exactly-ten-frozen-batch-02-candidate-census-plan-contexts-once",
  };
  ...
  assertV4(
    activation.schemaVersion ===
      ...
      activation.sourceCompatibility.minimumCandidateLexicalTokensChanged ===
        false &&
      activation.sourceCompatibility.occurrences.length === 1 &&
    ...
    "Batch 2 candidate-census planner execution is unauthorized"
  );
...
async function execute() {
  ...
  async function executeContext(context, contextIndex) {
    ...
    const sourceDirectory = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-post-canary-batch-02-candidate-plan-${context.debateNumber}-`
      )
    );
    const isolatedCodexHome = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-post-canary-batch-02-candidate-plan-home-${context.debateNumber}-`
      )
    );
    ...
    process.stdout.write(
      `[post-canary-batch-02-candidate-plan] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
    );
    return record;
  }
  ...
  const execution = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-02-candidate-census-plan-model-execution",
    protocolId: PROTOCOL_ID,
    status:
      validContexts === CONTEXTS
        ? EXECUTION_STATUS
        : "post-canary-batch-02-candidate-census-plan-gate-complete-with-failure",
    productionContinuation: true,
    developmentValidationOnly: false,
    productionCanary: false,
    stagingOnly: true,
    ...
    nextRequiredAction:
      validContexts === CONTEXTS
        ? "analyze-and-replay-ten-batch-02-candidate-census-plans-model-free-only"
        : "stop-preserve-failed-batch-02-candidate-census-plan-gate",
  };
...
async function analyze() {
  ...
  const analysis = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-02-candidate-census-plan-analysis",
    protocolId: PROTOCOL_ID,
    status:
      "post-canary-batch-02-candidate-census-plan-gate-passed-awaiting-separate-side-packet-preparation-approval",
    productionContinuation: true,
    developmentValidationOnly: false,
    productionCanary: false,
    stagingOnly: true,
    ...
    nextAuthorizedAction:
      "user-approval-required-before-preparing-and-freezing-twenty-exact-batch-02-side-selector-packets-or-any-further-model-execution",
  };
...
if (command === "preflight") await preflight();
  ...
  throw new Error(
    "usage: run-assessment-production-post-canary-batch-02-inventory-plans.mjs <preflight|activate|run|analyze|validate>"
  );
}
