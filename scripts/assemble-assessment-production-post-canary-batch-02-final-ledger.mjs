...
const finalLedgerPath =
  `${POST_CANARY_BATCH_02_FINAL_LEDGER_ROOT}/final-ledger.json`;
...
assertV4(
  ...
  "post-canary Batch 2 final-ledger manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen Batch 2 final-ledger source hash mismatch`
  );
}
const inputs = await loadPostCanaryBatch02FinalLedgerInputs();
const ledger = buildPostCanaryBatch02FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
