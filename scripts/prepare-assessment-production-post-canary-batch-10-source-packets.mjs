...
import {
  buildBatch10TokenCountedChunkLedger,
  findBatch10ZeroLexicalTokenRows,
} from "./lib/assessment-production-post-canary-batch-10-source-preparation.mjs";
import {
...
const BATCH_ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-10";
const ROOT = `${BATCH_ROOT}/source-preparation`;
const PROTOCOL_ID = "assessment-production-post-canary-batch-10-source-preparation";
const CACHE_VERSION = "post-canary-continuation-v1-batch-10";
...
const TRANSPORT_PLAN = `${BATCH_ROOT}/source-preparation-transport/plan.json`;
const BATCH_09_TRANSPORT_DIAGNOSIS = "docs/assessment-production/post-canary-continuation-v1/batch-09/source-preparation-recovery/failure-diagnosis.json";
const BATCH_09_TRANSPORT_PLAN = "docs/assessment-production/post-canary-continuation-v1/batch-09/source-preparation-recovery/correction-plan.json";
const SCRIPT = "scripts/prepare-assessment-production-post-canary-batch-10-source-packets.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-10-source-packets.mjs";
...
const CONTROL_FILES = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-10-source-preparation.mjs",
  TRANSPORT_PLAN,
  BATCH_09_TRANSPORT_DIAGNOSIS,
  BATCH_09_TRANSPORT_PLAN,
  SCRIPT,
  TEST,
];
...
const transportPlan = JSON.parse(controlBytes[TRANSPORT_PLAN]);
const batch09TransportDiagnosis = JSON.parse(controlBytes[BATCH_09_TRANSPORT_DIAGNOSIS]);
const batch09TransportPlan = JSON.parse(controlBytes[BATCH_09_TRANSPORT_PLAN]);

assertV4(
  selection.status === "tenth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    selectionAnalysis.status === "tenth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization" &&
    selectionAnalysis.selection.sha256 === sha256(selectionBytes) &&
    selection.nextAuthorizedAction === "prepare-batch-10-standing-authorization-and-source-packets-under-user-authorization" &&
    JSON.stringify(selection.selected.map((item) => item.debateNumber)) === JSON.stringify(REQUIRED_ORDER),
  "Batch 10 frozen selection is not ready for its separately authorized source-packet step"
);
assertV4(
  ...
  "Batch 10 model boundary or stop rules drifted"
);
assertV4(
...
assertV4(
  batch09TransportDiagnosis.status === "batch-09-source-preparation-preview-failure-diagnosed-single-chunk-transport-shape" &&
    batch09TransportPlan.status === "frozen-batch-09-debate-19-source-partition-correction-prepared" &&
    transportPlan.status === "frozen-batch-10-debate-68-source-partition-transport-plan" &&
    transportPlan.precedent.diagnosisSha256 === sha256(controlBytes[BATCH_09_TRANSPORT_DIAGNOSIS]) &&
    transportPlan.precedent.correctionPlanSha256 === sha256(controlBytes[BATCH_09_TRANSPORT_PLAN]) &&
    transportPlan.correction.debateNumber === "68" &&
    transportPlan.correction.partitionOverrides.contextEventsMaximum === 600 &&
    transportPlan.correction.expectedChunks === 2 &&
    transportPlan.lockedSources[SCRIPT].sha256 === sha256(controlBytes[SCRIPT]) &&
    transportPlan.lockedSources[TEST].sha256 === sha256(controlBytes[TEST]),
  "Batch 10 source-partition transport plan or source authentication failed"
);
...
for (const selected of selection.selected) {
  ...
  const partitionOverrides = selected.debateNumber === transportPlan.correction.debateNumber
    ? structuredClone(transportPlan.correction.partitionOverrides)
    : {};
  const plan = {
  ...
  if (selected.debateNumber === transportPlan.correction.debateNumber) {
    assertV4(plan.chunks.length === transportPlan.correction.expectedChunks, "Debate 68 transport partition must contain exactly two chunks");
  }
  ...
  packet.schemaVersion = "1.0-assessment-production-post-canary-batch-10-score-blind-source-packet";
  ...
  for (const chunk of plan.chunks) {
    ...
    const tokenLedgerBytes = buildBatch10TokenCountedChunkLedger(chunkBytes);
    const zeroLexicalTokenRows = findBatch10ZeroLexicalTokenRows(chunkBytes);
...
const preparation = {
  ...
  status: shouldWrite ? "post-canary-batch-10-ten-complete-score-blind-source-packets-prepared-awaiting-validation" : "preview",
  ...
  AIOnly: true,
  userAuthorization: {
    scope: "prepare, validate, freeze, commit, and push score-blind source packets for Batch 10 under the user's complete-workflow authorization",
  ...
  nextAuthorizedAction: "run-batch-10-deterministic-source-packet-validation-model-free-only",
};
