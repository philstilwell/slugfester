...
const [diagnosisBytes, planBytes, activationBytes, workPreparationBytes, workBytes] =
  await Promise.all([
...
assertV4(
  plan.status === "frozen-one-final-batch-05-debate-189-hls-transport-recovery-ready" &&
    activation.status ===
    ...
    plan.diagnosis.path === diagnosisPath &&
    plan.diagnosis.sha256 === sha256(diagnosisBytes),
  "Batch 5 recovery-2 activation authentication failed"
);
assertV4(
  plan.executionPolicy.attemptsMaximum === 1 &&
    plan.executionPolicy.debate189FinalDownloadAttemptsMaximum === 1 &&
    ...
    plan.executionPolicy.failedRecovery1PartialOutputReuseMaximum === 0 &&
    plan.executionPolicy.stopOnAnySourceOrValidationFailure,
  "Batch 5 recovery-2 execution policy changed"
);
assertV4(
  !(await exists(executionPath)) &&
    !(await exists(analysisPath)) &&
    !(await exists(finalPreparationPath)),
  "Batch 5 recovery-2 cannot be rerun"
);
for (const [file, digest] of Object.entries(plan.authenticatedInputs)) {
  ...
  assertV4(activation.authenticatedInputs[file] === digest, `activation input changed: ${file}`);
}
for (const [file, digest] of Object.entries(plan.sourceHashes)) {
  assertV4(sha256(await readFile(file)) === digest, `recovery-2 source changed: ${file}`);
  assertV4(activation.sourceHashes[file] === digest, `activation source changed: ${file}`);
}
for (const [file, digest] of Object.entries(plan.toolRuntime.files)) {
  assertV4(sha256(await readFile(file)) === digest, `yt-dlp runtime changed: ${file}`);
}
assertV4(
  execFileSync("python3", ["-m", "yt_dlp", "--version"], { encoding: "utf8" }).trim() ===
    plan.toolRuntime.ytDlpVersion,
  "yt-dlp version changed"
);
...
assertV4(await exists(ffmpeg) && await exists(ffprobe), "FFmpeg tools are unavailable");
...
const assertNoPrefix = async (directory, prefix) => {
  if (!(await exists(directory))) return;
  assertV4(
    (await readdir(directory)).every((entry) => !entry.startsWith(prefix)),
    `${directory}: recovery output prefix already exists`
  );
};

const normalizeDownloadedSource = async ({ downloadedPath, temporaryPath, minimumDurationMs }) => {
  assertV4(!(await exists(temporaryPath)), `${temporaryPath}: temporary output already exists`);
  execFileSync(ffmpeg, [
    "-nostdin", "-hide_banner", "-loglevel", "error", "-y", "-i", downloadedPath,
    "-vn", "-ac", String(plan.mediaEncoding.normalizedChannels),
    "-ar", String(plan.mediaEncoding.normalizedSampleRateHz),
    "-b:a", `${plan.mediaEncoding.normalizedBitrateKbps}k`, temporaryPath
...
const downloadOnce = async ({ route, debateNumber, prefix, stateField }) => {
  const directory = path.dirname(route.finalSourcePath);
  await mkdir(directory, { recursive: true });
  await assertNoPrefix(directory, prefix);
  assertV4(!(await exists(route.normalizedTemporaryPath)), "normalized temporary output already exists");
  runState[stateField] += 1;
  execFileSync("python3", [...route.ytDlpArguments, "-o", route.downloadTemplate, route.videoUrl], {
    stdio: "inherit"
  });
  const downloaded = (await readdir(directory)).filter(
    (entry) => entry.startsWith(prefix) && !entry.endsWith(".part") && !entry.includes(".part-Frag")
  );
  assertV4(downloaded.length === 1, `Debate ${debateNumber}: expected one downloaded source`);
  const downloadedPath = path.join(directory, downloaded[0]);
  ...
  await rename(route.normalizedTemporaryPath, route.finalSourcePath);
  assertV4(
    JSON.stringify(probeAudio(route.finalSourcePath)) === JSON.stringify(probe),
...
const createClip = async (move) => {
  ...
  const clipPath = `${clipDirectory}/${move.moveId.replace(/[^A-Za-z0-9_-]+/g, "-")}.mp3`;
  ...
  execFileSync(ffmpeg, [
    ...
    "-ss", (move.clipWindow.startMs / 1000).toFixed(3), "-i", sourcePath,
    "-t", plannedDurationSeconds.toFixed(3), "-vn",
    "-ac", String(plan.mediaEncoding.clipChannels),
    "-ar", String(plan.mediaEncoding.clipSampleRateHz),
    "-b:a", `${plan.mediaEncoding.clipBitrateKbps}k`, clipPath
  ], { stdio: ["ignore", "pipe", "pipe"] });
...
const runState = {
  attempts: 1,
  debate189FinalDownloadCliInvocations: 0,
...
const writeFailure = async (error) => {
  const failure = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-05-audio-source-transport-recovery-2-execution",
    status: "failed-one-final-batch-05-audio-source-transport-recovery-2-stop-required",
    batchNumber: 5,
    ...
    failure: {
      category: "source-or-validation-failure-during-authorized-final-transport-recovery",
      message: error instanceof Error ? error.message : String(error),
      stopRequired: true,
      furtherSourceAttemptsAuthorized: false
    },
    preservedPriorEvidenceHashes: plan.protectedMedia,
    directIncrementalCostUsd: 0
  };
...
try {
  ...
  const debate189Probe = await downloadOnce({
    ...
    debateNumber: "189",
    prefix: "source.recovery-2.hls.",
    stateField: "debate189FinalDownloadCliInvocations"
  });
  runState.debate189DownloadSucceeded = true;
  runState.debate189NormalizedSourceInstalled = true;

  assertV4(!(await exists(plan.debate05OriginalRoute.finalSourcePath)), "Debate 05 source is no longer unattempted");
  const debate05Probe = await downloadOnce({
    route: plan.debate05OriginalRoute,
    debateNumber: "05",
    prefix: "source.download.",
  ...
  await validateMediaHash(plan.protectedMedia.at(-1));

  const sourcePolicies = {
    "158": {
      acquisitionMode: "downloaded-public-source-before-preserved-failures",
      attempts: 1,
      outcome: "success-before-preserved-failures"
    },
    "189": {
      acquisitionMode: "downloaded-public-source-final-hls-transport-recovery",
      attempts: 3,
  ...
  for (const { debateNumber, sourceVideoId } of plan.exactCohort.sources) {
    ...
    const minimumEndMs = Math.max(
      ...work.moves.filter((move) => move.debateNumber === debateNumber)
  ...
  const publicSourceAttemptAudit = plan.exactCohort.sources.map(({ debateNumber, sourceVideoId }) => ({
    debateNumber,
    videoId: sourceVideoId,
    attempts: sourcePolicies[debateNumber].attempts,
    ordinaryAttemptsMaximum: 1,
    authorizedTransportRecoveryAttempts: debateNumber === "189" ? 2 : 0,
    totalAttemptsMaximum: debateNumber === "189" ? 3 : 1,
    outcome: sourcePolicies[debateNumber].outcome
  }));
  const preparation = {
    schemaVersion:
      "1.2-assessment-production-post-canary-batch-05-audio-source-preparation-transport-recovery-2",
    ...
    recovery: {
      ...
      recovery1Execution: {
        path: `${root}/audio-source-transport-recovery-1/execution.json`,
        sha256: plan.authenticatedInputs[`${root}/audio-source-transport-recovery-1/execution.json`]
      },
      diagnosedInvalidSource: {
        originalPath: plan.debate189Recovery.invalidSourcePath,
        preservedEvidencePath: plan.debate189Recovery.preservedInvalidEvidencePath,
        bytes: plan.protectedMedia.at(-1).bytes,
        sha256: plan.protectedMedia.at(-1).sha256
      },
      debate158ProtectedHashesPreserved: 4,
      debate189TransportRecoveryAttempts: 2,
      debate189FinalHlsAttempts: 1,
    ...
    acquisitionPolicy: {
      ...
      debate189AuthorizedTransportRecoveryAttemptsMaximum: 2,
      completeCohortPublicSourceAttemptsMaximum: 5,
      ytDlpRetryControls: { retries: 0, fragmentRetries: 0, extractorRetries: 0, fileAccessRetries: 0 },
      debate189FinalRecoveryTransport: plan.debate189Recovery.transport,
      debate189FinalRecoveryFormat: "best[protocol^=m3u8]",
      debate189FinalRecoveryClients: "web_safari",
      debate189FinalRecoveryDownloader: "m3u8:native",
    ...
    toolVersions: {
      ...
      ytDlp: plan.toolRuntime.ytDlpVersion
    },
    sources,
    clips,
    totals: {
  ...
  const execution = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-05-audio-source-transport-recovery-2-execution",
    status: "completed-one-final-batch-05-audio-source-transport-recovery-2",
    batchNumber: 5,
    diagnosisSha256: sha256(diagnosisBytes),
    planSha256: sha256(planBytes),
    activationSha256: sha256(activationBytes),
    ...
    state: structuredClone(runState),
    outputs: {
      audioSourcePreparation: { path: finalPreparationPath, sha256: sha256(preparationBytes) },
      sources: sources.map(({ debateNumber, sourceAudio, sourceAudioSha256 }) => ({
        debateNumber, path: sourceAudio, sha256: sourceAudioSha256
      })),
      clips: clips.map(({ debateNumber, moveId, clipPath, clipSha256 }) => ({
        debateNumber, moveId, path: clipPath, sha256: clipSha256
      }))
    },
    directIncrementalCostUsd: 0,
  ...
  const analysis = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-05-audio-source-transport-recovery-2-analysis",
    status: "accepted-complete-three-source-six-clip-batch-05-audio-cohort-after-final-hls-recovery",
    batchNumber: 5,
    ...
    result: {
      ...
      debate189FinalPublicSourceAttempts: 1,
