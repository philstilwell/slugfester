...
import { POST_CANARY_BATCH_15_PUBLICATION_COMPILATION_ORDER,
  ...
  compilePostCanaryBatch15PublicationStagingRecord } from
...
assertV4(m.status === "frozen-post-canary-batch-15-deterministic-publication-compilation-prepared" &&
  canonicalJson(m.explicitOrder) === canonicalJson(POST_CANARY_BATCH_15_PUBLICATION_COMPILATION_ORDER) &&
  m.contexts?.length === 10 && m.contexts.reduce((sum, row) => sum + row.expectedMoves, 0) === 191 &&
...
"Batch 15 compilation preparation changed");
let moves = 0;
for (const context of m.contexts) {
  ...
  const compiled = compilePostCanaryBatch15PublicationStagingRecord({ output, packet, identity: row });
  const validation = validatePostCanaryBatch15CompiledStagingRecord({ compiled, output, packet, identity: row });
  moves += validation.moves;
}
assertV4(moves === 191, "compiled move replay changed");
