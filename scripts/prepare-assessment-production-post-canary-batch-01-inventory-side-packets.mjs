...
const BATCH_SOURCE_PREPARATION =
  "docs/assessment-production/post-canary-continuation-v1/batch-01/source-preparation/preparation-manifest.json";
...
const SCRIPT =
  "scripts/prepare-assessment-production-post-canary-batch-01-inventory-side-packets.mjs";
const PLAN_RUNNER =
...
function makeSidePacket({
  ...
  return {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-01-chronology-fallback-side-selector-packet",
...
async function buildArtifacts({ frozenAt, checkpointCommit }) {
  ...
  assertV4(
    sourcePreparation.schemaVersion ===
      ...
      sourcePreparation.totals.proCandidates === 147 &&
      sourcePreparation.totals.conCandidates === 130 &&
    ...
    "Batch 1 source preparation boundary drifted"
  );
  ...
  assertV4(
    ...
    "accepted Batch 1 planner gate drifted"
  );
  ...
  const userInstruction =
    "I approve preparation, validation, freezing, committing, and pushing of the twenty exact Batch 1 side-selector packets and the side-selector execution-preparation manifest only, with a direct incremental cost cap of $0. Do not execute side-selector models, judgment models, paid services, audio work, score derivation, publication reconstruction, or production mutation.";
  const sidePreparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-01-chronology-fallback-side-packet-preparation",
    protocolId: PROTOCOL_ID,
    sideSelectionProtocolId: CHRONOLOGY_FALLBACK_INVENTORY.protocolId,
    status:
    ...
    nextRequiredAction:
      "freeze-batch-01-side-selector-execution-preparation-manifest-model-free-only",
  };
  ...
  const executionPreparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-01-side-selector-execution-preparation-manifest",
    protocolId: PROTOCOL_ID,
    sideSelectionProtocolId: CHRONOLOGY_FALLBACK_INVENTORY.protocolId,
    status:
    ...
    costEstimate: {
      ...
      estimateBasis:
        "The promoted twenty-context side-selector gate completed in 10.80 wall minutes and 19.74 aggregate model minutes at concurrency two. Batch 1 preserves the same one-side isolated context shape and remains below the proven 115,000-byte input ceiling. ChatGPT-subscription execution has no direct incremental API charge.",
    },
...
if (command === "prepare") await prepare();
  ...
  throw new Error(
    "usage: prepare-assessment-production-post-canary-batch-01-inventory-side-packets.mjs <prepare|validate>"
  );
}
