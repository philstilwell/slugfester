...
const VALIDATION_ROOT =
  "docs/assessment-production/score-stability-v2.2.1-validation-cohort";
const ROOT = `${VALIDATION_ROOT}/inventory-chronology-fallback`;
const PREPARATION = `${ROOT}/preparation-manifest.json`;
const GUIDE = `${ROOT}/chronology-fallback-inventory-guide.md`;
const DISCOVERY_ANALYSIS =
  `${VALIDATION_ROOT}/discovery-mechanical-recovery/analysis.json`;
const SOURCE_PREPARATION =
  "docs/assessment-production/score-stability-v2.2-validation-cohort/source-preparation/preparation-manifest.json";
...
const DISCOVERY_SUCCESSOR_DEVELOPMENT =
...
const SCRIPT =
  "scripts/prepare-assessment-production-score-stability-v2.2.1-inventory-chronology-fallback.mjs";
const TEST =
  "scripts/test-assessment-production-score-stability-v2.2.1-inventory-chronology-fallback-preparation.mjs";
const PROTOCOL_ID =
...
function buildInventorySourcePacket(sourcePacket) {
  assertV4(
    sourcePacket?.schemaVersion ===
      "1.0-score-stability-v2.2-validation-score-blind-source-packet" &&
      sourcePacket.protocolId ===
        "assessment-production-score-stability-v2.2-fresh-validation-source-preparation" &&
    ...
    "v2.2 source packet is unavailable"
  );
  return {
...
assertV4(
  discoveryAnalysis.status ===
    ...
    discoveryAnalysis.totals?.candidates === 361 &&
    discoveryAnalysis.totals?.pro === 181 &&
    discoveryAnalysis.totals?.con === 180 &&
    discoveryAnalysis.audit?.allDiscoveredCandidatesTransported === true &&
    discoveryAnalysis.audit?.silentSemanticDeduplication === false &&
    discoveryAnalysis.audit?.predecessorV22DiscoveryGateReclassified === false &&
    discoveryAnalysis.audit?.predecessorV213ScoreGateReclassified === false &&
    discoveryAnalysis.audit?.proposedV22ScorePolicyPromoted === false &&
    discoveryAnalysis.sourceDiscoveryGateDisposition ===
      "v2.2-failed-and-not-retried" &&
    discoveryAnalysis.rawOutputBoundary?.rawOutputsRewritten === false &&
    discoveryAnalysis.rawOutputBoundary?.candidateFieldsModified === false &&
    discoveryAnalysis.rawOutputBoundary?.semanticCorrectionPerformed === false &&
    discoveryAnalysis.rawOutputBoundary?.retryPerformed === false &&
    discoveryAnalysis.rawOutputBoundary?.sourceExecutionReclassified === false &&
    discoveryAnalysis.nextAuthorizedAction ===
      "prepare-v2.2.1-chronology-fallback-inventory-packets-model-free-only",
  "validated v2.2.1 mechanical recovery does not authorize preparation"
);
assertV4(
  ...
  "v2.2 source preparation or model boundary drifted"
);
assertV4(
...
for (const discovered of discoveryAnalysis.debates) {
  ...
  assertV4(
    ...
    `${discovered.debateNumber}: cohort identity or disposition drifted`
  );
  ...
  for (const side of ["pro", "con"]) {
    ...
    const candidateCount = fullTransport.candidateRows.filter(
      (row) => row[fullTransport.columnOrder.indexOf("side")] === side
    ).length;
    ...
    assertV4(
      maximumPlanSchemaAudit.nullableCandidateProperties === candidateCount &&
        maximumCopiedInputBytes <=
          V212_CANDIDATE_SHARDED_INVENTORY.maximumCopiedInputBytes,
      `${discovered.debateNumber}/${side}: schema or bound drifted`
    );
    pendingWrites.push(
...
assertV4(
  ...
  "v2.2.1 cohort totals or packet state drifted"
);
...
const sourceFiles = [
  ...
  DISCOVERY_SUCCESSOR_DEVELOPMENT,
  PRODUCTION_MANIFEST,
  PRODUCTION_WORKFLOW,
  READINESS_WORKFLOW,
  RUBRIC,
...
const preparation = {
  ...
  failedGateDisposition: {
    ...structuredClone(successorDevelopment.failedGateDisposition),
    sourceV22DiscoveryGatePreservedFailed: true,
    sourceV22DiscoveryGateRetried: false,
    sourceV22DiscoveryExecutionReclassified: false,
    v221MechanicalRecoveryUsedForCandidateTransport: true,
    v221MechanicalRecoveryChangedCandidateFields: false,
    predecessorV213ScoreGatePreservedFailed: true,
    v22ScorePolicyPromoted: false,
  },
  proposedPolicy: {
    ...structuredClone(discoveryAnalysis.proposedScorePolicy),
    promoted: false,
  },
  inventorySuccessorContract: structuredClone(
    successorDevelopment.successorContract
  ),
  model: {
  ...
  inputs: {
    ...
    successorDevelopmentSha256: sha256(successorDevelopmentBytes),
