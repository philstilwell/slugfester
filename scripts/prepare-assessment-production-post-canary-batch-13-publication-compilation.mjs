...
import {
  POST_CANARY_BATCH_13_PUBLICATION_COMPILATION_ORDER,
  POST_CANARY_BATCH_13_PUBLICATION_COMPILATION_PROTOCOL_ID,
  POST_CANARY_BATCH_13_PUBLICATION_COMPILATION_ROOT,
  compilePostCanaryBatch13PublicationStagingRecord,
  validatePostCanaryBatch13CompiledStagingRecord
} from "./lib/assessment-production-post-canary-batch-13-publication-compilation.mjs";
import { POST_CANARY_BATCH_13_PUBLICATION_ROOT } from
  "./lib/assessment-production-post-canary-batch-13-publication.mjs";
...
import { POST_CANARY_BATCH_13_STANDING_AUTHORIZATION,
  loadAndValidatePostCanaryBatch13StandingAuthorization } from
  "./lib/assessment-production-post-canary-batch-13-standing-authorization.mjs";
...
const COHORT = `${POST_CANARY_BATCH_13_PUBLICATION_ROOT}/original-unattempted-context-resumption-4/complete-cohort-analysis.json`;
...
const ACCEPTED_OUTPUTS = Object.freeze(Object.fromEntries(
  POST_CANARY_BATCH_13_PUBLICATION_COMPILATION_ORDER.map((debateNumber) => [
    debateNumber,
    `${POST_CANARY_BATCH_13_PUBLICATION_ROOT}/outputs/debate-${debateNumber}.json`
  ])
));
const acceptedOutput = (debateNumber) => ACCEPTED_OUTPUTS[debateNumber];
const packetPath = (debateNumber) =>
  `${POST_CANARY_BATCH_13_PUBLICATION_ROOT}/packets/debate-${debateNumber}.json`;
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const exists = (file) => access(path.resolve(file)).then(() => true, () => false);
const standing = await loadAndValidatePostCanaryBatch13StandingAuthorization();
...
assertV4(
  cohort.status ===
    "post-canary-batch-13-complete-ten-debate-publication-output-gate-passed-after-recovery" &&
  cohort.totals?.debates === 10 && cohort.totals?.lockedMoves === 199 &&
  cohort.totals?.critiques === 199 && cohort.totals?.exactSourceQuotes === 20 &&
  cohort.integrity?.scoresRemainedImmutable === true &&
  cohort.integrity?.everyAcceptedOutputReplayedDeterministically === true,
"the accepted Batch 13 publication cohort changed");

const productionByNumber = new Map(debates.map((debate) => [debate.number, debate]));
const identities = [];
const contexts = [];
for (const debateNumber of POST_CANARY_BATCH_13_PUBLICATION_COMPILATION_ORDER) {
  ...
  const validation = validatePostCanaryBatch13PublicationOutput(output, packet);
  ...
  const compiled = compilePostCanaryBatch13PublicationStagingRecord({ output, packet, identity });
  const compiledValidation = validatePostCanaryBatch13CompiledStagingRecord({ compiled, output, packet, identity });
...
assertV4(contexts.length === 10 && contexts.reduce((sum, row) => sum + row.expectedMoves, 0) === 199,
"the Batch 13 compilation cohort changed");
const identitySnapshot = { schemaVersion: "1.0-assessment-production-post-canary-batch-13-publication-compilation-identity-snapshot",
  protocolId: POST_CANARY_BATCH_13_PUBLICATION_COMPILATION_PROTOCOL_ID,
...
const sources = [COHORT, POST_CANARY_BATCH_13_STANDING_AUTHORIZATION,
  ...
  "scripts/lib/assessment-production-post-canary-batch-13-publication-compilation.mjs",
  "scripts/lib/assessment-production-post-canary-batch-13-standing-authorization.mjs",
  "scripts/lib/v4-lean-production.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-13-publication-compilation.mjs",
  "scripts/test-assessment-production-post-canary-batch-13-publication-compilation-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-13-publication-compilation.mjs",
  "scripts/run-assessment-production-post-canary-batch-13-publication-compilation.mjs",
...
const manifest = { schemaVersion: "1.0-assessment-production-post-canary-batch-13-deterministic-publication-compilation-preparation",
  protocolId: POST_CANARY_BATCH_13_PUBLICATION_COMPILATION_PROTOCOL_ID,
  status: "frozen-post-canary-batch-13-deterministic-publication-compilation-prepared",
  frozenAt, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(),
  productionCanary: false, batchNumber: 12, stagingOnly: true,
  userAuthorization: { standingAuthorization: POST_CANARY_BATCH_13_STANDING_AUTHORIZATION,
  ...
  explicitOrder: POST_CANARY_BATCH_13_PUBLICATION_COMPILATION_ORDER, contexts,
  ...
  nextAuthorizedAction: "activate-one-batch-13-deterministic-publication-compilation-pass" };
...
console.log(JSON.stringify({ status: manifest.status, debates: 10, moves: 199,
