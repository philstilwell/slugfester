...
import { V388_PERFORMANCE_DEBATES, V388_PERFORMANCE_PASSES, V388_PERFORMANCE_ROOT, assertV388, containsProhibitedDerivedField, readJson } from "./lib/v388-performance-judgment.mjs";
...
const manifestPath = `${V388_PERFORMANCE_ROOT}/initial-execution-manifest.json`;
if (shouldWrite) {
  try {
...
assertV388(preparation.status === "prepared-score-blind-no-model-execution" && preparation.totals.moves === 81 && preparation.totals.pendingAudioVerifications === 0, "performance preparation invalid");
assertV388(fixture.status === "passed" && fixture.totals.contexts === 6 && fixture.totals.judgments === 162 && fixture.totals.calculatedTotals === 0, "performance dry fixture invalid");
...
for (const pass of V388_PERFORMANCE_PASSES) for (const debateNumber of V388_PERFORMANCE_DEBATES) {
  ...
  assertV388(packet.moves.length > 0 && !containsProhibitedDerivedField(packet), `${pass}.${debateNumber}: packet contains prohibited derived field`);
  contexts.push({
    pass,
    debateNumber,
    ...
    output: `${V388_PERFORMANCE_ROOT}/initial-outputs/debate-${debateNumber}-pass-${pass.toLowerCase()}.json`
  });
}
const sourceFiles = [
  ...
  `${V388_PERFORMANCE_ROOT}/preparation-assessment.md`,
  `${V388_PERFORMANCE_ROOT}/preparation-manifest.json`,
  `${V388_PERFORMANCE_ROOT}/dry-fixture.json`,
  schemaPath,
  "scripts/lib/reassessment-scoring.mjs",
  "scripts/lib/v388-performance-judgment.mjs",
  "scripts/build-v388-performance-judgment-packets.mjs",
  "scripts/test-v388-performance-judgment-tooling.mjs",
  "scripts/validate-v388-performance-judgment-preparation.mjs",
  "scripts/validate-v388-performance-judgment-output.mjs",
  "scripts/preregister-v388-performance-judgment-execution.mjs",
  "scripts/validate-v388-performance-judgment-execution-lock.mjs",
  "scripts/run-v388-performance-judgment-initial.mjs",
...
const executionOutput = `${V388_PERFORMANCE_ROOT}/initial-model-execution.json`;
const manifest = {
  schemaVersion: "3.8.8-performance-judgment-initial-execution-manifest",
  ...
  judgmentPolicy: { responseTupleCompound: true, responseTupleFields: ["class", "decisiveTargetIds", "contactedComponents", "totalComponents"], explicitContactedAndMissedComponents: true, lockedBurdenTupleCopied: true, responseBandsEnforced: true, burdenBandsEnforced: true, charityTestedExplicit: true, untestedCharityExactly75: true, duplicateBurdenAdjustmentCaptureForcesZero: true, modelCalculatedTotalsProhibited: true, modelAssessmentProseProhibited: true },
