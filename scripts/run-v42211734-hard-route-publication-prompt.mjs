...
const manifest = JSON.parse(await readFile(path.resolve(`${V42211734_ROOT}/execution-manifest.json`), "utf8"));
...
async function runContext(context, contextIndex) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `v42211734-publication-${context.debateNumber}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `v42211734-publication-home-${context.debateNumber}-`));
  ...
  try {
    ...
    process.stdout.write(`[v4.2.21.17.34-publication-prompt] starting index ${contextIndex} ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber}\n`);
  ...
  process.stdout.write(`[v4.2.21.17.34-publication-prompt] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
