...
import {
  POST_CANARY_BATCH_02_RESUMPTION_2_BASE_OUTPUTS,
  POST_CANARY_BATCH_02_RESUMPTION_2_PUBLICATION_PACKETS,
  POST_CANARY_BATCH_02_RESUMPTION_2_REPAIR_FIELDS,
  POST_CANARY_BATCH_02_RESUMPTION_2_REPAIR_PACKET_VERSION,
  POST_CANARY_BATCH_02_RESUMPTION_2_REPAIR_PARTITIONS,
  POST_CANARY_BATCH_02_RESUMPTION_2_REPAIR_PROTOCOL_ID,
  POST_CANARY_BATCH_02_RESUMPTION_2_REPAIR_ROOT,
  buildResumption2RepairSchema,
  resumption2RepairMoveId
} from "./lib/assessment-production-post-canary-batch-02-publication-resumption-2-repair.mjs";
import {
...
const ROOT = POST_CANARY_BATCH_02_RESUMPTION_2_REPAIR_ROOT;
const MANIFEST = `${ROOT}/execution-preparation-manifest.json`;
const DIAGNOSIS = `${POST_CANARY_BATCH_02_PUBLICATION_RESUMPTION_2_ROOT}/failure-diagnosis.json`;
const FAILED_EXECUTION = `${POST_CANARY_BATCH_02_PUBLICATION_RESUMPTION_2_ROOT}/model-execution.json`;
const FAILED_ANALYSIS = `${POST_CANARY_BATCH_02_PUBLICATION_RESUMPTION_2_ROOT}/analysis.json`;
...
const STATIC_SOURCE_FILES = [
  ...
  FAILED_EXECUTION,
  FAILED_ANALYSIS,
  ...Object.values(POST_CANARY_BATCH_02_RESUMPTION_2_BASE_OUTPUTS),
  ...Object.values(POST_CANARY_BATCH_02_RESUMPTION_2_PUBLICATION_PACKETS),
  ...
  "scripts/lib/assessment-production-post-canary-batch-02-publication-resumption-2-repair.mjs",
  "scripts/lib/assessment-production-post-canary-batch-02-standing-authorization.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-02-publication-resumption-2-repair.mjs",
  "scripts/test-assessment-production-post-canary-batch-02-publication-resumption-2-repair-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-02-publication-resumption-2-repair.mjs",
  "scripts/run-assessment-production-post-canary-batch-02-publication-resumption-2-repair.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-02-publication-resumption-2-repair.mjs"
];
...
const baseOutputs = Object.fromEntries(
  Object.entries(POST_CANARY_BATCH_02_RESUMPTION_2_BASE_OUTPUTS).map(
    ([debateNumber, file]) => [debateNumber, parsed(file)]
  )
);
const publicationPackets = Object.fromEntries(
  Object.entries(POST_CANARY_BATCH_02_RESUMPTION_2_PUBLICATION_PACKETS).map(
    ([debateNumber, file]) => [debateNumber, parsed(file)]
  )
);
...
assertV4(
  diagnosis.status ===
    ...
    diagnosis.failureBoundary?.failedDebateCount === 2 &&
    diagnosis.failureBoundary?.failedFieldCount === 10 &&
    diagnosis.failureBoundary?.unexpectedValidationCategories === 0 &&
    diagnosis.diagnosticReplay?.originalOutputsModified === false &&
    diagnosis.diagnosticReplay?.persistedCorrectedOutputs === false &&
    diagnosis.diagnosticReplay?.completeDebatesPassed?.every(
      (item) => item.result?.status === "passed"
    ),
...
assertV4(
  execution.contextsPlanned === 8 &&
    ...
    execution.results?.filter((result) => !result.gateAcceptancePassed)
      .map((result) => result.debateNumber).join(",") === "136,83" &&
    analysis.status ===
      "post-canary-batch-02-publication-resumption-2-failed-validation",
  "the preserved failed publication gate changed"
);
for (const debateDiagnosis of diagnosis.failureBoundary.debateDiagnoses) {
  const debateNumber = debateDiagnosis.debateNumber;
  assertV4(
    sha256(bytesByFile[POST_CANARY_BATCH_02_RESUMPTION_2_BASE_OUTPUTS[debateNumber]]) ===
        debateDiagnosis.artifacts.output.sha256 &&
      sha256(bytesByFile[POST_CANARY_BATCH_02_RESUMPTION_2_PUBLICATION_PACKETS[debateNumber]]) ===
        debateDiagnosis.artifacts.packet.sha256,
    `Debate ${debateNumber}: diagnosed source artifact drifted`
  );
}
assertV4(
...
assertV4(
  canonicalJson(diagnosis.prospectiveRecoveryOnly.proposedRepairPartition) ===
      canonicalJson(POST_CANARY_BATCH_02_RESUMPTION_2_REPAIR_PARTITIONS) &&
    canonicalJson(
      diagnosis.failureBoundary.debateDiagnoses.flatMap((debate) =>
        debate.failedFields.map(({ path: field }) => field)
      )
    ) ===
      canonicalJson(POST_CANARY_BATCH_02_RESUMPTION_2_REPAIR_FIELDS),
  "the diagnosed repair partition changed"
);

const failureByField = new Map(
  diagnosis.failureBoundary.debateDiagnoses.flatMap((debate) =>
    debate.failedFields.map((entry) => [entry.path, entry])
  )
);
const contexts = [];
const generated = [];
for (let packetIndex = 0; packetIndex < 6; packetIndex += 1) {
  const partition =
    POST_CANARY_BATCH_02_RESUMPTION_2_REPAIR_PARTITIONS[packetIndex];
  ...
  const corrections = writableFields.map((field) => {
    ...
    return {
      ...
      wordsBelowAcceptanceMinimum: diagnosed.wordsBelowMinimum,
      wordsAboveAcceptanceMaximum: diagnosed.wordsAboveMaximum,
      lockedMove: move
    };
  });
  const packet = {
    schemaVersion: POST_CANARY_BATCH_02_RESUMPTION_2_REPAIR_PACKET_VERSION,
    protocolId: POST_CANARY_BATCH_02_RESUMPTION_2_REPAIR_PROTOCOL_ID,
    packetIndex,
    ...
    immutableBaseOutput:
      POST_CANARY_BATCH_02_RESUMPTION_2_BASE_OUTPUTS[debateNumber],
    publicationPacket:
      POST_CANARY_BATCH_02_RESUMPTION_2_PUBLICATION_PACKETS[debateNumber],
...
assertV4(
  contexts.length === 6 &&
    ...
    new Set(contexts.flatMap((context) => context.writableFields)).size === 10,
  "exactly six disjoint one-or-two-field repair contexts are required"
);
...
const MERGED_OUTPUTS = {
  "136": `${ROOT}/merged/debate-136.json`,
  "83": `${ROOT}/merged/debate-83.json`
};
const COMPLETE_VALIDATIONS = {
  "136": `${ROOT}/complete-debate-136-validation.json`,
  "83": `${ROOT}/complete-debate-83-validation.json`
};
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-publication-resumption-2-repair-execution-preparation-manifest",
  protocolId: POST_CANARY_BATCH_02_RESUMPTION_2_REPAIR_PROTOCOL_ID,
  status:
  ...
  userAuthorization: {
    ...
    resolvedScope:
      "prepare, validate, freeze, activate, and execute exactly six bounded Debate 136 and Debate 83 publication-repair packets exposing ten critique fields",
    standingAuthorizationApplied: true,
    standingAuthorizationSha256: standingAuthorization.sha256,
    directIncrementalCostUsdMaximum: 0,
    contextsPrepared: 6,
    writableFieldsPrepared: 10,
  ...
  costEstimate: {
    ...
    contexts: 6,
    expectedParallelWallMinutes: [5, 14],
    expectedAggregateModelMinutes: [7, 18],
    absoluteGateTimeoutMinutes: 48,
    estimateBasis: {
      source:
      ...
      scalingRule: "six-sevenths-of-observed-seven-context-repair-with-contingency"
    }
  },
  ...
  inputs: {
    ...
    immutableBaseOutputs: POST_CANARY_BATCH_02_RESUMPTION_2_BASE_OUTPUTS,
    publicationPackets: POST_CANARY_BATCH_02_RESUMPTION_2_PUBLICATION_PACKETS,
  ...
  repairContract: {
    repairType: "critique-word-boundary",
    writableFields: POST_CANARY_BATCH_02_RESUMPTION_2_REPAIR_FIELDS,
  ...
  deterministicValidation: {
    ...
    sixBoundedFieldSchemasReproducedAtFreeze: true,
  ...
  artifacts: {
    ...
    execution: EXECUTION,
    analysis: ANALYSIS,
    originalFailedOutputs: POST_CANARY_BATCH_02_RESUMPTION_2_BASE_OUTPUTS,
  ...
  nextAuthorizedAction:
    "activate-and-execute-exactly-six-frozen-resumption-2-publication-repair-contexts-under-standing-authorization"
};
