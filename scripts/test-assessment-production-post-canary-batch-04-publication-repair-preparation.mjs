...
import {
  ...
  mergeAndValidateDebate127Repairs,
  validateDebate127RepairOutput
} from "./lib/assessment-production-post-canary-batch-04-publication-repair.mjs";

const MANIFEST = `${POST_CANARY_BATCH_04_DEBATE_127_REPAIR_ROOT}/execution-preparation-manifest.json`;
...
assert.equal(manifest.protocolId, POST_CANARY_BATCH_04_DEBATE_127_REPAIR_PROTOCOL_ID);
...
assert.deepEqual(manifest.costEstimate.expectedParallelWallMinutes, [2, 8]);
...
assert.equal(manifest.totals.contexts, 2);
assert.equal(manifest.totals.writableFields, 3);
...
for (const context of manifest.contexts) {
  ...
  assert.equal(packet.debateNumber, "127");
  ...
  assert.deepEqual(schema, buildDebate127RepairSchema(packet));
  ...
  assert.equal(validateDebate127RepairOutput(repair, packet).status, "passed");
...
const merged = mergeAndValidateDebate127Repairs({
  baseOutput,
  repairs: syntheticRepairs,
  repairPackets,
  publicationPacket
});
assert.equal(merged.transformations.length, 3);
assert.equal(merged.fullValidation.status, "passed");
assert.equal(merged.fullValidation.moves, 23);
