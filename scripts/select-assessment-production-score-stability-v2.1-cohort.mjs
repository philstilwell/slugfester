...
const PRIOR_SELECTION =
  "docs/assessment-production/score-stability-v2-validation-cohort/selection.json";
const SCRIPT =
  "scripts/select-assessment-production-score-stability-v2.1-cohort.mjs";
const TEST =
  "scripts/test-assessment-production-score-stability-v2.1-cohort.mjs";

if (shouldWrite) {
  await access(OUTPUT).then(
    () => {
      throw new Error(`${OUTPUT} already exists; selection is immutable`);
    },
    () => true
  );
}
...
const recoveryAnalysis = JSON.parse(sourceBytes[RECOVERY_ANALYSIS]);
const priorSelection = JSON.parse(sourceBytes[PRIOR_SELECTION]);
assertV4(
  productionManifest.schemaVersion ===
    ...
    recoveryAnalysis.authorization.freshDisjointCohortSelection === true &&
    recoveryAnalysis.authorization.successorPreparation === false &&
    recoveryAnalysis.authorization.successorModelExecution === false &&
    recoveryAnalysis.totals.modelContextsExecuted === 0 &&
    priorSelection.status ===
      "fresh-disjoint-ten-debate-cohort-source-gate-passed" &&
    priorSelection.selected.length === 10,
  "v2.1 cohort selection is not authorized or source boundaries drifted"
);
...
assertV4(
  [...failedV2ValidationDebates].every((number) =>
    recoveryAnalysis.schemas.some((record) => record.debateNumber === number)
  ),
  "prior failed v2 cohort is not fully represented in recovery analysis"
);

const policySeedSha256 = sha256(sourceBytes[POLICY]);
const recoverySeedSha256 = sha256(sourceBytes[RECOVERY_ANALYSIS]);
const eligible = productionManifest.items
  ...
  .map((item) => ({
    item,
    rankSha256: sha256(
      `${policySeedSha256}|${recoverySeedSha256}|${item.debateNumber}|${item.debateId}`
    ),
  }))
  .sort(
    (left, right) =>
...
assertV4(
  ...
  "v2.1 cohort is not fresh, disjoint, and dyadic"
);

const selection = {
  schemaVersion: "1.0-score-stability-v2.1-validation-cohort-selection",
  ...
  recoveryProtocol: {
    ...
    sha256: recoverySeedSha256,
    candidateSharded: true,
    priorFailedArtifactsReusableForAcceptance: false,
    priorFailedArtifactsReusableAsFreshModelInput: false,
  },
  selectionPolicy: {
    cohortSize: 10,
    dyadicOnly: true,
    ...
    deterministicRank:
      "SHA-256(v2.1 policy hash | candidate-sharded analysis hash | debate number | debate id), ascending",
  ...
  authorization: {
    candidateShardedPreparation: true,
  ...
  nextAuthorizedAction:
    "prepare-candidate-sharded-source-and-schema-artifacts-model-free-only",
};
