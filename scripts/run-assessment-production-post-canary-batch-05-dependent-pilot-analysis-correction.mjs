...
const analysisPath = `${BATCH_05_DEPENDENT_PILOT_CORRECTION_ROOT}/analysis.json`;
const startedAt = new Date().toISOString();
const [plan, activation] = await Promise.all([readJson(planPath), readJson(activationPath)]);
assertV4(
  plan.protocolId === BATCH_05_DEPENDENT_PILOT_CORRECTION_PROTOCOL_ID &&
    activation.status === "frozen-batch-05-dependent-pilot-analysis-correction-activated" &&
  ...
  "frozen one-shot Batch 5 dependent pilot correction activation required"
);
...
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-05-dependent-pilot-analysis-correction-execution",
  protocolId: BATCH_05_DEPENDENT_PILOT_CORRECTION_PROTOCOL_ID,
  status: "passed-complete-batch-05-production-transaction-after-dependent-pilot-analysis-correction",
  startedAt,
  completedAt,
  ...
  nextAuthorizedAction: "atomic-commit-and-push-complete-validated-batch-05-production-transaction-then-stop-before-batch-6-selection"
};
await writeFile(resolve(executionPath), serializedJson(execution));
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-05-dependent-pilot-analysis-correction-analysis",
  protocolId: BATCH_05_DEPENDENT_PILOT_CORRECTION_PROTOCOL_ID,
  status: "batch-05-production-publication-accepted-after-dependent-pilot-analysis-correction",
