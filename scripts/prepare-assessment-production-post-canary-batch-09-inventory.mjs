...
const SCRIPT =
  "scripts/prepare-assessment-production-post-canary-batch-09-inventory.mjs";
const PROTOCOL_ID =
  "assessment-production-post-canary-batch-09-candidate-sharded-inventory";
const SOURCE_PACKET_VERSION =
  "1.0-assessment-production-post-canary-batch-09-candidate-sharded-inventory-source-packet";
...
function buildInventorySourcePacket(sourcePacket) {
  assertV4(
    sourcePacket?.schemaVersion ===
      "1.0-assessment-production-post-canary-batch-09-score-blind-source-packet" &&
      sourcePacket.protocolId ===
        "assessment-production-post-canary-batch-09-source-preparation" &&
    ...
    "Batch 9 source packet is unavailable or not dyadic"
  );
  return {
...
function makePlanPacket({
  ...
  return {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-09-candidate-census-plan-packet",
    protocolId: PROTOCOL_ID,
...
async function buildFrozenArtifacts(frozenAt) {
  ...
  assertV4(
    ...
    "Batch 9 standing authorization does not cover exact inventory preparation and execution"
  );

  assertV4(
    discoveryAnalysis.status ===
      "post-canary-batch-09-discovery-passed-standing-authorization-active-for-inventory-preparation" &&
      ...
      discoveryAnalysis.nextAuthorizedAction ===
        "prepare-freeze-and-activate-batch-09-candidate-census-planner-contexts-under-standing-authorization",
    "passed Batch 9 discovery boundary drifted"
  );
  assertV4(
    discoveryExecution.status ===
      "thirty-one-post-canary-batch-09-discovery-contexts-passed" &&
    ...
    "Batch 9 discovery execution drifted"
  );
  assertV4(
    ...
    "Batch 9 source preparation or model boundary drifted"
  );
  assertV4(
    ...
    "Batch 9 source validation drifted"
  );
  assertV4(
    selection.status ===
    ...
    "Batch 9 frozen selection drifted"
  );
  assertV4(
  ...
  for (const discovered of discoveryAnalysis.debates) {
    ...
    assertV4(
      ...
      `${discovered.debateNumber}: Batch 9 identity drifted`
    );
    const [
  ...
  assertV4(
    ...
    "Batch 9 totals or packet state drifted"
  );
  ...
  const preparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-09-candidate-sharded-inventory-preparation",
    protocolId: PROTOCOL_ID,
    ...
    userAuthorization: {
      scope:
        "prepare, freeze, activate, and execute the Batch 9 candidate-census and candidate-sharded side-selection workflow under the frozen standing authorization",
    ...
    nextAuthorizedAction:
      "freeze-and-activate-batch-09-candidate-census-plan-execution-under-standing-authorization",
  };
  ...
  const executionPreparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-09-candidate-census-plan-execution-preparation-manifest",
    protocolId: PROTOCOL_ID,
    status:
      "frozen-ten-post-canary-batch-09-candidate-census-plan-contexts-prepared-not-authorized",
    frozenAt,
    ...
    costEstimate: {
      ...
      estimateBasis:
        `The promoted ten-context candidate-census planner used this contract at concurrency two. Batch 9 freezes the same ten-context shape with an observed maximum of ${maximumCopiedInputBytes.toLocaleString("en-US")} copied bytes, below the proven ${V212_CANDIDATE_SHARDED_INVENTORY.maximumCopiedInputBytes.toLocaleString("en-US")}-byte ceiling. ChatGPT-subscription execution has no direct incremental API charge.`,
    },
  ...
  const validation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-09-candidate-sharded-inventory-preparation-validation",
    protocolId: PROTOCOL_ID,
    status:
      "batch-09-candidate-sharded-inventory-packets-and-plan-execution-manifest-validation-passed-frozen-standing-authorization-active",
...
async function validateFrozen() {
  ...
  assertV4(
    ...
    "frozen Batch 9 inventory preparation is invalid"
  );
...
if (command === "freeze") await freeze();
  ...
  throw new Error(
    "usage: prepare-assessment-production-post-canary-batch-09-inventory.mjs <freeze|validate>"
  );
}
