...
const scoreArtifactCandidates = [
  `${POST_CANARY_BATCH_03_FINAL_LEDGER_ROOT}/calculated-scores.json`,
  `${POST_CANARY_BATCH_03_FINAL_LEDGER_ROOT}/score-analysis.json`,
  `${POST_CANARY_BATCH_03_FINAL_LEDGER_ROOT}/score-pass-manifest.json`
];
...
const rebuiltLedger = buildPostCanaryBatch03FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
...
assert.equal(storedLedger.audit.disputedMoves, 190);
assert.equal(storedLedger.audit.pairSelections, 212);
assert.equal(storedLedger.audit.scoringFieldSelections, 356);
assert.equal(storedLedger.audit.burdenAdjustmentSelections, 18);
assert.equal(storedLedger.audit.candidateSelections, 586);
assert.equal(storedLedger.audit.meanMerges, 419);
assert.equal(storedLedger.audit.dependencyMeanMergesSuppressed, 0);
assert.equal(storedLedger.audit.audioVerifiedMoves, 8);
assert.equal(storedLedger.audit.acceptedAdjudicationOutputsWithoutCorrection, 8);
...
assert.throws(() =>
  buildPostCanaryBatch03FinalLedger(mutatedInputs, inputs.sourceHashes)
);
...
assert.equal(analysis.totals.disputedMoves, 190);
assert.equal(analysis.totals.candidateSelections, 586);
