...
assert.deepEqual(preparation.scope.debates, ["21", "74", "123", "147", "130"]);
assert.equal(preparation.scope.frozenTargetClips, 9);
assert.equal(preparation.scope.verificationCalls, 9);
...
assert.equal(preparation.costEstimate.clipSeconds, 1308.771876);
assert.equal(preparation.costEstimate.clipMinutes, 21.8129);
...
assert.equal(preparation.costEstimate.usageDerivedPlanningEstimateUsd, 0.47646);
...
assert.deepEqual(preparation.costEstimate.projectedUsage, {
  audioInputTokens: 24386,
  textInputTokens: 1026,
  outputTokens: 41293,
  totalInputTokens: 25412
});
...
assert.equal(preparation.executionPolicy.callsMaximum, 9);
...
assert.equal(
  ...
  "activate-and-execute-nine-batch-10-paid-audio-verification-calls-under-standing-authorization"
);
