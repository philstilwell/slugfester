...
const PROTOCOL_ID =
  "assessment-production-post-canary-batch-02-independent-judgments";
const INVENTORY_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-02/inventory-candidate-sharded/inventory-analysis.json";
const INVENTORY_PREPARATION = "docs/assessment-production/post-canary-continuation-v1/batch-02/inventory-candidate-sharded/preparation-manifest.json";
const SOURCE_PREPARATION = "docs/assessment-production/post-canary-continuation-v1/batch-02/source-preparation/preparation-manifest.json";
const SOURCE_VALIDATION = "docs/assessment-production/post-canary-continuation-v1/batch-02/source-preparation/validation.json";
...
assertV4(
  inventoryAnalysis.status ===
      "post-canary-batch-02-inventory-gate-passed-awaiting-separate-judgment-packet-preparation-approval" &&
    ...
    inventoryAnalysis.totals?.chronologyFallbacks === 3 &&
    ...
    inventoryAnalysis.sourceCompatibility?.occurrences?.length === 1 &&
    inventoryAnalysis.sourceCompatibility?.occurrences[0]?.debateNumber ===
      "99" &&
    inventoryAnalysis.nextAuthorizedAction ===
      "user-approval-required-before-batch-02-independent-judgment-packet-preparation",
  "the Batch 2 inventory gate is not ready for the separately authorized independent-judgment packet preparation"
);
assertV4(
  ...
  "the Batch 2 inventory preparation is unavailable"
);
assertV4(
  ...
  "the Batch 2 source preparation is unavailable"
);
assertV4(
  sourceValidation.status ===
    ...
    sourceValidation.checks?.exactZeroLexicalTokenRowPreservedWithZeroCount ===
      true &&
    sourceValidation.checks?.exactSourceRowsInjectedOmittedOrRewritten ===
      false,
  "the Batch 2 source validation is unavailable"
);
assertV4(
...
assertV4(
  ...
  "the frozen Batch 2 cohort boundary changed"
);
assertV4(
  inventoryAnalysis.debates.map((debate) => debate.debateNumber).join(",") ===
    EXPECTED_DEBATES.join(","),
  "the passed Batch 2 inventory debate order changed"
);
...
const preparation = {
  ...
  AIOnly: true,
  userAuthorization: {
    instruction: "I approve this step and all similar steps in the future.",
    approvedScope:
      "Prepare, validate, freeze, commit, and push exactly twenty Batch 2 score-blind independent-judgment packets and their execution-preparation manifest only, with a direct incremental cost cap of $0. Do not execute judgment models, use paid services, perform audio work, derive scores, reconstruct publication, mutate production, or select the next batch.",
    standingApprovalScope:
      "Future equivalent deterministic preparation-only checkpoints with a direct incremental cost cap of $0; excludes model execution, paid services, audio, score derivation, publication execution, production mutation, and next-batch selection.",
