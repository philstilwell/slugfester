...
const replaceExactlyOnce = (value, before, after) => {
  assert.equal(value.split(before).length, 2, `expected one frozen preimage occurrence: ${before}`);
...
const frozenTestPreimage = execFileSync(
  "git",
  ["show", `${FROZEN_PREIMAGE_COMMIT}:${TARGET_TEST}`],
  { encoding: null }
);
assert.equal(sha256(frozenTestPreimage), FROZEN_PREIMAGE_SHA256);
assert.equal(preparation.sourceHashes[TARGET_TEST], FROZEN_PREIMAGE_SHA256);
let reconstructedCorrectedTest = frozenTestPreimage.toString("utf8");
reconstructedCorrectedTest = replaceExactlyOnce(
  reconstructedCorrectedTest,
...
const correctedTestBytes = await readFile(TARGET_TEST);
assert.equal(sha256(Buffer.from(reconstructedCorrectedTest)), CORRECTED_TEST_SHA256);
assert.equal(sha256(correctedTestBytes), CORRECTED_TEST_SHA256);
assert.equal(
  preparation.status,
...
assert.equal(preparation.userAuthorization.thisArtifactActivatesModelExecution, false);
assert.equal(
...
for (const [file, digest] of Object.entries(preparation.sourceHashes)) {
  const authenticatedBytes =
    file === TARGET_TEST ? frozenTestPreimage : await readFile(file);
  assert.equal(
    sha256(authenticatedBytes),
    digest,
    `${file}: preparation source hash changed`
  );
}
...
console.log(JSON.stringify({
  ...
  frozenTestPreimageReconstructed: true,
  authorizedAssertionChanges: 2,
  modelContexts: 0,
  audioCalls: 0,
  scoresDerived: 0,
