...
import {
  ...
  evaluatePostCanaryBatch02ScoreStability
...
const accepted = evaluatePostCanaryBatch02ScoreStability(passing);
...
assert.equal(
  evaluatePostCanaryBatch02ScoreStability(winnerMutation).acceptancePassed,
  false
);
const distanceMutation = structuredClone(passing);
distanceMutation[2].final = scores(95, 80);
assert.equal(
  evaluatePostCanaryBatch02ScoreStability(distanceMutation).acceptancePassed,
  false
);
assert.throws(() =>
  evaluatePostCanaryBatch02ScoreStability(passing, {
    ...POST_CANARY_BATCH_02_SCORE_STABILITY_THRESHOLDS,
