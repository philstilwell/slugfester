...
import { POST_CANARY_BATCH_02_DISPUTE_ADJ_ROOT } from "./lib/assessment-production-post-canary-batch-02-dispute-adjudication.mjs";
...
const preparationPath = `${POST_CANARY_BATCH_02_DISPUTE_ADJ_ROOT}/preparation-manifest.json`;
const manifestPath = `${POST_CANARY_BATCH_02_DISPUTE_ADJ_ROOT}/execution-preparation-manifest.json`;
const activationPath = `${POST_CANARY_BATCH_02_DISPUTE_ADJ_ROOT}/execution-activation.json`;
const executionPath = `${POST_CANARY_BATCH_02_DISPUTE_ADJ_ROOT}/model-execution.json`;
const analysisPath = `${POST_CANARY_BATCH_02_DISPUTE_ADJ_ROOT}/analysis.json`;
...
assertV4(
  preparation.status ===
    ...
    preparation.totals.disputedMoves === 182 &&
    preparation.totals.candidateSelections === 535 &&
  ...
  "Batch 2 adjudication packets are not ready for execution preparation"
);
assertV4(
  ...
  "Batch 2 accepted source or cost boundary changed"
);
assertV4(
  ...
  "Batch 2 adjudication model or score-blind boundary changed"
);
assertV4(
  preparation.totals.maximumCopiedInputBytes <= 350000,
  "Batch 2 context exceeds the frozen transport ceiling"
);
assertV4(
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-dispute-only-adjudication-execution-preparation-manifest",
  protocolId: preparation.protocolId,
  status:
    "frozen-ten-post-canary-batch-02-dispute-only-adjudication-contexts-prepared-not-authorized",
  frozenAt,
  activatedAt: null,
  ...
  executionPolicy: {
    ...
    copiedInputBytesMaximum: 350000,
  ...
  nextAuthorizedAction:
    "user-approval-required-before-batch-02-dispute-only-adjudication-activation-or-any-model-execution"
};
