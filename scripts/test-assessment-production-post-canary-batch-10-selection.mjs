...
assert.equal(selection.status, "tenth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed");
assert.equal(analysis.status, "tenth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization");
assert.equal(analysis.selection.sha256, sha256(selectionBytes));
assert.equal(selection.batchNumber, 10);
assert.equal(selection.productionSequenceOrdinal, 11);
...
assert.equal(selection.eligibility.previouslyPublishedAndRemovedCount, 90);
assert.equal(selection.eligibility.remainingBeforeBatchCount, 74);
assert.equal(selection.eligibility.selectedCount, 10);
assert.equal(selection.eligibility.remainingUnselectedCount, 64);
...
assert.equal(priorCompletionExecution.status, "passed-complete-batch-09-transaction-ready-for-atomic-commit");
...
assert.equal(priorCompletionAnalysis.status, "batch-09-production-publication-accepted-for-atomic-commit-after-generated-seo-correction");
assert.equal(priorCompletionAnalysis.decision.generatedSeoCorrectionGatePassed, true);
assert.equal(priorCompletionAnalysis.decision.batch09ProductionPublicationGatePassed, true);
...
assert.equal(sha256(serializedJson(ranked.slice(90))), selection.eligibility.remainingBeforeBatchRankedSha256);
assert.deepEqual(priorSelection.selected.map((item) => item.debateNumber), ranked.slice(80, 90).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.debateNumber), ranked.slice(90, 100).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.rankOrdinal), [91, 92, 93, 94, 95, 96, 97, 98, 99, 100]);
assert.deepEqual(selection.selected.map((item) => item.rankSha256), ranked.slice(90, 100).map((item) => item.rankSha256));
...
assert.deepEqual(selection.deterministicOrdering.selectedRankOrdinals, [91, 100]);
...
assert.equal(selection.nextAuthorizedAction, "prepare-batch-10-standing-authorization-and-source-packets-under-user-authorization");

console.log(JSON.stringify({ status: "passed", selectedDebates: selection.selected.map((item) => item.debateNumber), selectedRankOrdinals: [91, 100], sourceFiles: 30, replacements: 0, sourcePacketsPrepared: 0, modelContexts: 0, paidServiceCalls: 0, directCostUsd: 0, nextAuthorizedAction: selection.nextAuthorizedAction }, null, 2));
