...
assert.equal(work.batchNumber, 14);
assert.equal(work.stagingOnly, true);
assert.equal(preparation.userAuthorization.workItemsAuthorized, 12);
assert.equal(
...
assert.equal(preparation.totals.debates, 7);
assert.equal(preparation.totals.sourceVideoIds, 7);
assert.equal(preparation.totals.moves, 12);
assert.equal(work.moves.length, 12);
...
assert.equal(preparation.validation.exactAuthorizedMoveCount, 12);
assert.equal(
  preparation.validation.canonicalLocalTextAndMetadataChainsVerified,
  7
);
assert.equal(preparation.validation.repositoryRenderedLockedExcerpts, 12);
assert.equal(preparation.validation.repositoryRenderedTimestampWindows, 12);
assert.equal(preparation.validation.expectedSpeakersLocked, 12);
assert.equal(
