...
import { POST_CANARY_BATCH_08_PUBLICATION_COMPILATION_ORDER,
  ...
  compilePostCanaryBatch08PublicationStagingRecord } from
...
assertV4(m.status === "frozen-post-canary-batch-08-deterministic-publication-compilation-prepared" &&
  canonicalJson(m.explicitOrder) === canonicalJson(POST_CANARY_BATCH_08_PUBLICATION_COMPILATION_ORDER) &&
...
"Batch 8 compilation preparation changed");
let moves = 0;
for (const context of m.contexts) {
  ...
  const compiled = compilePostCanaryBatch08PublicationStagingRecord({ output, packet, identity: row });
  const validation = validatePostCanaryBatch08CompiledStagingRecord({ compiled, output, packet, identity: row });
