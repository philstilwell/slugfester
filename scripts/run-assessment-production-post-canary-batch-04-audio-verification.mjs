...
assert(manifest.status === "frozen-four-post-canary-batch-04-paid-known-speaker-diarizations-authorized-under-standing-authorization", "Batch 4 paid audio execution is not frozen");
...
assert(manifest.costEstimate.primaryExpectedFutureExecutionCostUsd === 0.12208, "frozen estimate changed");
...
for (const [callIndex, call] of manifest.calls.entries()) {
  ...
  process.stdout.write(`[batch-04-audio] ${callIndex + 1}/4 starting ${call.debateNumber}:${call.moveId}\n`);
  ...
  results.push(result);
  process.stdout.write(`[batch-04-audio] ${callIndex + 1}/4 ${call.debateNumber}:${call.moveId} ${result.status}; cumulative usage estimate $${result.cumulativeUsageDerivedEstimatedCostUsd.toFixed(7)}\n`);
}
...
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-04-audio-verification-model-execution",
  protocolId: manifest.protocolId,
  status: completed.length === manifest.calls.length
    ? "four-post-canary-batch-04-paid-known-speaker-diarizations-completed"
    : "post-canary-batch-04-paid-diarization-incomplete",
