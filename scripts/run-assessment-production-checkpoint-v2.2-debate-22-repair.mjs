...
const activation = JSON.parse(
  await readFile(path.resolve(`${CHECKPOINT_V22_DEBATE_22_REPAIR_ROOT}/execution-activation.json`), "utf8")
);
...
assertV4(
  activation.status === "frozen-seven-isolated-thirteen-field-debate-22-publication-repair-contexts-authorized" &&
    ...
    activation.authorization.deterministicCompleteCohortValidationIfAllRepairsPass === true &&
  ...
  "Debate 22 repair execution is not authorized or controls changed"
);
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `checkpoint-v22-debate-22-repair-${context.packetIndex}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `checkpoint-v22-debate-22-repair-home-${context.packetIndex}-`));
  ...
  try {
    ...
    const prompt = [
      ...
      `Act only as isolated Debate 22 publication repair editor for packet ${context.packetIndex}.`,
      ...
      "Before returning, count every critique and revise it until it contains 112–118 words; do not aim at the 130-word acceptance ceiling.",
    ...
    process.stdout.write(`[checkpoint-v2.2-debate-22-repair] starting packet ${context.packetIndex} 5.6 Sol/low ${context.correctedFieldCount} field(s)\n`);
    ...
    if (invocation.timedOut || invocation.code !== 0 || invocation.signal !== null || !resultExists) {
      ...
      try {
        validationSummary = validateDebate22RepairOutput(
      ...
      const validation = {
        schemaVersion: "1.0-production-checkpoint-v2.2-debate-22-repair-validation",
        protocolId: activation.protocolId,
        status: accepted ? "passed" : "failed",
        packetIndex: context.packetIndex,
        debateNumber: "22",
        repairOutputSha256: sha256(outputBytes),
        validationSummary,
        validationMessage,
        modelAuthoredScores: 0
      };
      const provenance = {
        schemaVersion: "1.0-production-checkpoint-v2.2-debate-22-repair-provenance",
        protocolId: activation.protocolId,
        packetIndex: context.packetIndex,
        debateNumber: "22",
  ...
  process.stdout.write(`[checkpoint-v2.2-debate-22-repair] packet ${context.packetIndex} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
...
async function runPool(indexes, maximumConcurrency) {
  ...
  async function worker() {
    while (queue.length) completed.push(await runContext(activation.contexts[queue.shift()]));
  }
...
for (const phase of activation.executionPolicy.rampPhases) {
  if (!expansionAuthorized) {
    rampPhases.push({
      ...
      validContextIndexes: [],
      passed: false,
      skippedBecausePriorRepairFailed: true
    });
    continue;
  }
  ...
  rampPhases.push({
    ...
    validContextIndexes,
    passed,
    skippedBecausePriorRepairFailed: false
  });
...
const execution = {
  schemaVersion: "1.0-production-checkpoint-v2.2-debate-22-publication-repair-model-execution",
  protocolId: activation.protocolId,
  status: passed ? "seven-debate-22-publication-repair-contexts-passed" : "debate-22-publication-repair-complete-with-failure",
  gateStartedAt,
  ...
  authorization: {
    ...
    deterministicCompleteCohortValidationIfAllRepairsPass: true,
    retry: false,
