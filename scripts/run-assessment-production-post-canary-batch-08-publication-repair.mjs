...
assertV4(activation.status === "frozen-one-context-batch-08-publication-repair-authorized" &&
...
"the Batch 8 publication repair execution is not authorized");
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-08-publication-repair-${context.contextIndex}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `batch-08-publication-repair-home-${context.contextIndex}-`));
  ...
  try {
    ...
    const prompt = [
      ...
      `Act only as isolated score-locked Batch 8 publication repair editor ${context.packetId}.`,
    ...
    process.stdout.write(`[batch-08-publication-repair] starting index ${context.contextIndex} ${context.packetId} ${activation.model.label}/${activation.model.reasoningEffort}\n`);
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 ||
      ...
      try {
        validationSummary = validateBatch08PublicationRepairOutput(JSON.parse(outputBytes),
      ...
      const validation = { schemaVersion: "1.0-assessment-production-post-canary-batch-08-publication-repair-validation",
      ...
      const provenance = { schemaVersion: "1.0-assessment-production-post-canary-batch-08-publication-repair-provenance",
  ...
  process.stdout.write(`[batch-08-publication-repair] index ${context.contextIndex} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
...
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-08-publication-repair-execution",
  protocolId: activation.protocolId,
  status: validContexts === 1
    ? "batch-08-one-context-publication-repair-gate-passed"
    : "batch-08-publication-repair-gate-complete-with-failure",
