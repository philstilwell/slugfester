...
assert.equal(work.batchNumber, 2);
assert.equal(work.stagingOnly, true);
assert.equal(preparation.userAuthorization.workItemsAuthorized, 10);
assert.equal(
...
assert.equal(preparation.sources.length, 4);
assert.equal(preparation.totals.debates, 4);
assert.equal(preparation.totals.sourceVideoIds, 4);
assert.equal(preparation.totals.moves, 10);
assert.equal(work.moves.length, 10);
...
assert.equal(preparation.validation.exactAuthorizedMoveCount, 10);
assert.equal(
  preparation.validation.canonicalLocalTextAndMetadataChainsVerified,
  4
);
assert.equal(preparation.validation.repositoryRenderedLockedExcerpts, 10);
assert.equal(preparation.validation.repositoryRenderedTimestampWindows, 10);
assert.equal(preparation.validation.expectedSpeakersLocked, 10);
assert.equal(
