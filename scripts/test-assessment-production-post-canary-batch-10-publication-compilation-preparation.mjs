...
import { POST_CANARY_BATCH_10_PUBLICATION_COMPILATION_ORDER,
  ...
  compilePostCanaryBatch10PublicationStagingRecord } from
...
assertV4(m.status === "frozen-post-canary-batch-10-deterministic-publication-compilation-prepared" &&
  canonicalJson(m.explicitOrder) === canonicalJson(POST_CANARY_BATCH_10_PUBLICATION_COMPILATION_ORDER) &&
  m.contexts?.length === 10 && m.contexts.reduce((sum, row) => sum + row.expectedMoves, 0) === 182 &&
...
"Batch 10 compilation preparation changed");
let moves = 0;
for (const context of m.contexts) {
  ...
  const compiled = compilePostCanaryBatch10PublicationStagingRecord({ output, packet, identity: row });
  const validation = validatePostCanaryBatch10CompiledStagingRecord({ compiled, output, packet, identity: row });
  moves += validation.moves;
}
assertV4(moves === 182, "compiled move replay changed");
