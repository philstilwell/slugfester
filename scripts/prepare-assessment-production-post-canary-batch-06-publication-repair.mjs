...
import {
  ...
  POST_CANARY_BATCH_06_PUBLICATION_REPAIR_OUTPUT_VERSION,
  POST_CANARY_BATCH_06_PUBLICATION_REPAIR_PACKET_VERSION,
  POST_CANARY_BATCH_06_PUBLICATION_REPAIR_PROTOCOL_ID,
  POST_CANARY_BATCH_06_PUBLICATION_REPAIR_ROOT,
  validateBatch06PublicationRepairOutput
...
const DIAGNOSIS = `${POST_CANARY_BATCH_06_PUBLICATION_ROOT}/failure-diagnosis.json`;
...
const REJECTED = ["141", "168", "135", "143"];
const ACCEPTED = ["73", "36", "38", "97", "06", "169"];
...
const STATIC = [DIAGNOSIS, PRODUCTION_WORKFLOW, READINESS_WORKFLOW, OUTPUT_CONTRACT,
  MANUAL, POST_CANARY_BATCH_06_STANDING_AUTHORIZATION,
  `${POST_CANARY_BATCH_06_PUBLICATION_ROOT}/execution-preparation-manifest.json`,
  `${POST_CANARY_BATCH_06_PUBLICATION_ROOT}/execution-activation.json`,
  `${POST_CANARY_BATCH_06_PUBLICATION_ROOT}/model-execution.json`,
  `${POST_CANARY_BATCH_06_PUBLICATION_ROOT}/analysis.json`,
  ...
  "scripts/lib/assessment-production-post-canary-batch-06-publication-repair.mjs",
  "scripts/lib/assessment-production-post-canary-batch-06-standing-authorization.mjs",
  "scripts/diagnose-assessment-production-post-canary-batch-06-publication-failures.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-06-publication-repair.mjs",
  "scripts/test-assessment-production-post-canary-batch-06-publication-repair-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-06-publication-repair.mjs",
  "scripts/run-assessment-production-post-canary-batch-06-publication-repair.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-06-publication-repair.mjs"];
for (const debateNumber of [...REJECTED, ...ACCEPTED]) {
  STATIC.push(`${POST_CANARY_BATCH_06_PUBLICATION_ROOT}/outputs/debate-${debateNumber}.json`,
    `${POST_CANARY_BATCH_06_PUBLICATION_ROOT}/packets/debate-${debateNumber}.json`);
}
...
assertV4(
  diagnosis.status === "frozen-diagnosed-batch-06-publication-49-field-formatting-failure" &&
    diagnosis.failedFieldCount === 49 && diagnosis.failedCritiqueCount === 48 &&
    diagnosis.failedQuoteCount === 1 && diagnosis.minimumBoundedRepair?.packetCount === 25 &&
    diagnosis.diagnosticReplayControls?.everyRejectedDebatePassedAfterOnlyDiagnosedTransientSubstitutions === true &&
    standing.record.authorization?.boundedPublicationRepairs === true,
  "the frozen Batch 6 publication diagnosis or standing authorization changed"
);
...
for (const [debateNumber, fields] of Object.entries(diagnosis.failedFieldsByDebate))
  for (const field of fields) diagnosedByPath.set(`${debateNumber}:${field.path}`, field);
...
for (const planned of diagnosis.minimumBoundedRepair.repairContexts) {
  ...
  const publicationPacketPath = `${POST_CANARY_BATCH_06_PUBLICATION_ROOT}/packets/debate-${debateNumber}.json`;
  const packet = { schemaVersion: POST_CANARY_BATCH_06_PUBLICATION_REPAIR_PACKET_VERSION,
    protocolId: POST_CANARY_BATCH_06_PUBLICATION_REPAIR_PROTOCOL_ID,
    contextIndex, packetId, productionCanary: false, batchNumber: 6, stagingOnly: true,
    ...
    corrections };
  const schema = buildBatch06PublicationRepairSchema(packet);
  ...
  const synthetic = { schemaVersion: POST_CANARY_BATCH_06_PUBLICATION_REPAIR_OUTPUT_VERSION,
    protocolId: POST_CANARY_BATCH_06_PUBLICATION_REPAIR_PROTOCOL_ID,
    contextIndex, packetId, debateNumber, debateId: packet.debateId,
    assessmentModel: POST_CANARY_BATCH_06_PUBLICATION_MODEL.label,
    completedAt: frozenAt, correctedFields };
  validateBatch06PublicationRepairOutput(synthetic, packet);
  syntheticOutputs.push(synthetic);
}

assertV4(contexts.length === 25 && contexts.every((row) => row.writableFieldCount <= 2 &&
  row.copiedInputBytes <= 400000) &&
  new Set(contexts.flatMap((row) => row.writableFields)).size === 49,
"the 25-context repair preparation boundary changed");
for (const debateNumber of REJECTED) {
  ...
  const merge = mergeAndValidateBatch06PublicationRepairs({ baseOutput: baseOutputs[debateNumber],
...
const manifest = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-06-publication-repair-preparation",
  protocolId: POST_CANARY_BATCH_06_PUBLICATION_REPAIR_PROTOCOL_ID,
  status: "frozen-twenty-five-context-batch-06-publication-repair-prepared-and-authorized",
  frozenAt, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(),
  productionCanary: false, batchNumber: 6, stagingOnly: true, AIOnly: true,
  userAuthorization: { source: POST_CANARY_BATCH_06_STANDING_AUTHORIZATION,
    sourceSha256: standing.sha256, directIncrementalCostUsdMaximum: 0 },
  diagnosis: { path: DIAGNOSIS, sha256: sha256(diagnosisBytes), failedFields: 49,
    failedCritiques: 48, failedQuotes: 1, rejectedDebates: REJECTED },
  model: POST_CANARY_BATCH_06_PUBLICATION_MODEL,
  contexts,
  ...
  repairContract: { rejectedOutputsAreImmutableBases: true, acceptedOutputsImmutable: true,
    writableFields: 49, writableFieldsMaximumPerPacket: 2, fieldDisjoint: true,
    ...
    completeFourDebateValidationRequired: true, completeTenDebateCohortReplayRequired: true },
  ...
  executionPolicy: { contexts: 25, attemptsPerContext: 1, retriesMaximum: 0,
    ...
    rampPhases: [
      ...
      { phase: "steady-two", maximumParallelContexts: 2,
        contextIndexes: Array.from({ length: 22 }, (_, index) => index + 3),
  ...
  nextAuthorizedAction: "activate-exactly-twenty-five-frozen-batch-06-publication-repair-contexts"
};
...
console.log(JSON.stringify({ status: manifest.status, contextsPrepared: 25,
  writableFields: 49, maximumWritableFieldsPerPacket: 2,
  maximumCopiedInputBytes: Math.max(...contexts.map((row) => row.copiedInputBytes)),
  syntheticCompleteDebateReplaysPassed: 4, model: manifest.model,
