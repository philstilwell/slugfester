...
import { POST_CANARY_BATCH_07_DEBATE_02_PUBLICATION_REPAIR_PROTOCOL_ID,
  POST_CANARY_BATCH_07_DEBATE_02_PUBLICATION_REPAIR_ROOT } from
...
assertV4(p.protocolId === POST_CANARY_BATCH_07_DEBATE_02_PUBLICATION_REPAIR_PROTOCOL_ID &&
  p.status === "frozen-one-context-batch-07-debate-02-publication-repair-prepared-not-activated" &&
  p.context?.debateNumber === "02" && p.context?.writableFieldCount === 1 &&
  p.model?.slug === "gpt-5.6-sol" && p.model?.reasoningEffort === "low" &&
  p.model?.authentication === "ChatGPT subscription" &&
  p.executionPolicy?.contexts === 1 && p.executionPolicy?.attemptsPerContext === 1 &&
  p.executionPolicy?.retriesMaximum === 0 && p.executionPolicy?.timeoutExtensionsMaximum === 0,
"the Debate 02 publication repair is not prepared");
...
const activation = { schemaVersion:
    "1.0-assessment-production-post-canary-batch-07-debate-02-publication-repair-activation",
  protocolId: p.protocolId,
  status: "frozen-one-context-batch-07-debate-02-publication-repair-authorized",
  ...
  model: p.model, context: p.context, modelInputs: p.modelInputs, isolation: p.isolation,
  ...
  authorization: { repairModelExecution: true, deterministicOutputValidation: true,
    deterministicMergeAndValidation: true, twoContextResumption: true,
  ...
  nextAuthorizedAction: "execute-exactly-one-frozen-batch-07-debate-02-publication-repair-context" };
...
console.log(JSON.stringify({ status: activation.status, contextsAuthorized: 1,
  debateNumber: "02", writableFieldsAuthorized: 1, model: activation.model,
