...
assertV4(
  activation.status === "frozen-eighteen-context-thirty-two-field-batch-11-publication-repair-activated" &&
    activation.authorization?.modelExecution === true && activation.contexts?.length === 18 &&
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-11-field-repair-${context.contextIndex}-`));
  const isolatedCodexHome = await mkdtemp(path.join(os.tmpdir(), `batch-11-field-repair-home-${context.contextIndex}-`));
  ...
  try {
    ...
    process.stdout.write(`[batch-11-field-repair] starting context ${context.contextIndex} ${context.shardId} (${context.writableFieldCount} fields)\n`);
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 ||
      ...
      const validation = {
        schemaVersion: "1.0-assessment-production-post-canary-batch-11-publication-field-disjoint-repair-validation",
      ...
      const provenance = {
        schemaVersion: "1.0-assessment-production-post-canary-batch-11-publication-field-disjoint-repair-provenance",
        protocolId: activation.protocolId,
        contextIndex: context.contextIndex,
        shardId: context.shardId,
        debateNumber: context.debateNumber,
  ...
  process.stdout.write(`[batch-11-field-repair] ${context.shardId} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
...
const passed = results.length === 18 && validContexts === 18;
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-11-publication-field-disjoint-repair-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "eighteen-field-disjoint-batch-11-publication-repair-contexts-passed"
    : "eighteen-field-disjoint-batch-11-publication-repair-contexts-completed-with-failure",
  gateStartedAt,
  ...
  nextRequiredAction: passed
    ? "deterministically-merge-and-validate-four-debates-then-replay-complete-cohort"
