...
import {
  POST_CANARY_BATCH_04_DISPUTE_ADJ_PROTOCOL_ID,
  POST_CANARY_BATCH_04_DISPUTE_ADJ_ROOT,
  buildPostCanaryBatch04DisputeAdjudicationPacket,
  makePostCanaryBatch04DisputeAdjudicationSchema
...
const USER_AUTHORIZATION = Object.freeze({
  instruction:
    "The frozen Batch 4 standing authorization permits automatic preparation, validation, freezing, activation, execution, analysis, committing, and pushing of score-blind dispute-only adjudication checkpoints for exactly the ten selected debates while every frozen gate passes, with subscription-backed and local direct incremental cost capped at $0.",
...
const inputPaths = {
  ...
  schema: `${POST_CANARY_BATCH_04_DISPUTE_ADJ_ROOT}/adjudication.schema.json`
};
const TOOL_SOURCES = [
  ...
  "scripts/build-assessment-production-post-canary-batch-04-dispute-adjudication-packets.mjs",
  "scripts/test-assessment-production-post-canary-batch-04-dispute-adjudication-packets.mjs",
  "scripts/validate-assessment-production-post-canary-batch-04-dispute-adjudication-output.mjs",
  "scripts/preregister-assessment-production-post-canary-batch-04-dispute-adjudication.mjs",
  "scripts/test-assessment-production-post-canary-batch-04-dispute-adjudication-manifest.mjs"
];
...
const audioCostCorrectionPath =
  `${AUDIO_ROOT}/cost-decimal-correction-validation.json`;
...
assertV4(
  sourceAnalysis.status ===
    ...
    sourceAnalysis.adjudicationWorkload.candidateSelections === 582 &&
    sourceAnalysis.adjudicationWorkload.packetsPrepared === false &&
    sourceAnalysis.adjudicationWorkload.modelContextsExecuted === 0,
  "Batch 4 deterministic disagreement workload changed"
);
assertV4(
  judgmentExecution.status ===
      "twenty-post-canary-batch-04-independent-judgment-contexts-passed" &&
    ...
    judgmentAnalysis.status ===
      "twenty-post-canary-batch-04-independent-judgments-passed-standing-authorization-active-for-disagreement-extraction" &&
    judgmentAnalysis.totals.contexts === 20,
  "Batch 4 accepted independent-judgment boundary changed"
);
assertV4(
  ...
  "Batch 4 independent-judgment configuration changed"
);
...
assertV4(
  ...
  "Batch 4 standing authorization changed"
);
assertV4(
  audioAudit.status ===
    ...
    audioCostCorrection.status ===
      "passed-exact-integer-unit-and-seven-decimal-cost-overlay" &&
    audioCostCorrection.exactCostUsd === 0.1144125 &&
    audioCostCorrection.approvedCapExceeded === false &&
    audioCostCorrection.persistentProtectedWrites === 0,
  "Batch 4 accepted audio-verification boundary changed"
);
assertV4(
  ...
  "Batch 4 packet-preparation authorization changed"
);
...
assertV4(audioByDebateMove.size === 4, "Batch 4 verified audio population changed");

const schema = makePostCanaryBatch04DisputeAdjudicationSchema();
...
if (shouldWrite) {
  const immutableOutputs = [
    ...
    `${POST_CANARY_BATCH_04_DISPUTE_ADJ_ROOT}/execution-preparation-manifest.json`,
    ...EXPECTED_DEBATES.flatMap((debateNumber) => [
      `${POST_CANARY_BATCH_04_DISPUTE_ADJ_ROOT}/packets/debate-${debateNumber}.json`,
      `${POST_CANARY_BATCH_04_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`
    ])
  ];
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const built = buildPostCanaryBatch04DisputeAdjudicationPacket(
  ...
  const provenancePath =
    `${POST_CANARY_BATCH_04_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`;
  const outputPath =
    `${POST_CANARY_BATCH_04_DISPUTE_ADJ_ROOT}/outputs/debate-${debateNumber}.json`;
  const packetBytes = pretty(built.packet);
  const provenanceBytes = pretty({
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-04-adjudication-candidate-provenance",
...
const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-dispute-only-adjudication-preparation",
  protocolId: POST_CANARY_BATCH_04_DISPUTE_ADJ_PROTOCOL_ID,
  status: shouldWrite
  ...
  acceptedSourceBoundary: {
    ...
    audioUsageDerivedEstimatedCostUsd:
      audioCostCorrection.exactCostUsd,
  ...
  nextAuthorizedAction:
    "freeze-post-canary-batch-04-dispute-only-adjudication-execution-preparation-manifest-model-free-only"
};
assertV4(
  contexts.length === 10 &&
    disputedMoves === 196 &&
  ...
  "Batch 4 adjudication packet workload differs from frozen inputs"
);
assertV4(
  preparation.totals.maximumCopiedInputBytes <= 360000,
  `Batch 4 context ${preparation.totals.maximumCopiedInputBytes} bytes exceeds the 360 KB ceiling`
);
if (shouldWrite) {
  await mkdir(POST_CANARY_BATCH_04_DISPUTE_ADJ_ROOT, { recursive: true });
