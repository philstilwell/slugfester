...
import { extractAssessmentProductionPostCanaryBatch07Disagreements } from "./assessment-production-post-canary-batch-07-disagreement.mjs";
import {
  buildPostCanaryBatch07DisputeAdjudicationPacket,
  validatePostCanaryBatch07DisputeAdjudicationOutput
} from "./assessment-production-post-canary-batch-07-dispute-adjudication.mjs";

export const POST_CANARY_BATCH_07_FINAL_LEDGER_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-07/final-ledger";
export const POST_CANARY_BATCH_07_FINAL_LEDGER_PROTOCOL_ID =
  "assessment-production-post-canary-batch-07-adjudicated-consensus";
export const POST_CANARY_BATCH_07_FINAL_LEDGER_VERSION =
  "1.0-assessment-production-post-canary-batch-07-adjudicated-raw-ledger";
...
function validateInputReplay(input) {
  ...
  const replayedDisagreements =
    extractAssessmentProductionPostCanaryBatch07Disagreements(
      primaryA,
      primaryB,
      lockedInventory
    );
  assertV4(
    ...
    `${primaryA.debateNumber}: stored disagreements differ from deterministic Batch 7 replay`
  );
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication packet differs from deterministic Batch 7 replay`
  );
  assertV4(
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication provenance differs from deterministic Batch 7 replay`
  );
  validatePostCanaryBatch07DisputeAdjudicationOutput(
    adjudicationOutput,
    adjudicationPacket
...
export function compilePostCanaryBatch07FinalLedgerDebate(input) {
  const {
    primaryA,
    primaryB,
    adjudicationPacket,
...
export function buildPostCanaryBatch07FinalLedger(debateInputs, sourceHashes) {
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === EXPECTED_DEBATES.length,
    "ten post-canary Batch 7 debate inputs required"
  );
  const debates = debateInputs.map(compilePostCanaryBatch07FinalLedgerDebate);
  assertV4(
    debates.map((debate) => debate.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "post-canary Batch 7 final-ledger debate order invalid"
  );
  ...
  assertV4(aggregate.finalMoves === 187, "Batch 7 final-move population mismatch");
  assertV4(aggregate.disputedMoves === 180, "Batch 7 disputed-move population mismatch");
  assertV4(candidateSelections === 511, "Batch 7 candidate-selection population mismatch");
  assertV4(
    aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed === 403,
    "Batch 7 rounded-mean population mismatch"
  );
  assertV4(aggregate.audioVerifiedMoves === 5, "Batch 7 audio-verification population mismatch");
  return {
    schemaVersion: POST_CANARY_BATCH_07_FINAL_LEDGER_VERSION,
    protocolId: POST_CANARY_BATCH_07_FINAL_LEDGER_PROTOCOL_ID,
    status: "passed-post-canary-batch-07-deterministic-final-ledger-assembly",
    ...
    AIOnly: true,
    scoringInputSchema:
      "v4.2.20-source-span-primary-with-post-canary-batch-07-adjudicated-provenance",
    ...
    nextAuthorizedAction:
      "standing-authorization-permits-batch-07-single-deterministic-score-pass-preparation"
  };
}

export function validatePostCanaryBatch07FinalLedger(
  ledger,
  debateInputs,
  sourceHashes
) {
  const expected = buildPostCanaryBatch07FinalLedger(debateInputs, sourceHashes);
  assertV4(
    canonicalJson(ledger) === canonicalJson(expected),
    "post-canary Batch 7 final ledger differs from deterministic replay"
  );
  assertV4(
    !containsProhibitedCalculatedField(
      ledger.debates.map((debate) => debate.finalJudgment)
    ),
    "post-canary Batch 7 final ledger contains a prohibited calculated field"
  );
  return {
    status: "passed",
...
export async function loadPostCanaryBatch07FinalLedgerInputs() {
  ...
  assertV4(
    adjudicationAnalysis.status ===
      ...
      adjudicationAnalysis.gate.disputedMovesDecided === 180 &&
      adjudicationAnalysis.gate.candidateSelections === 511 &&
    ...
    "accepted post-canary Batch 7 adjudication gate unavailable"
  );
  assertV4(
    ...
    "complete Batch 7 audio verification unavailable"
  );
  ...
  assertV4(
    debateInputs.map((input) => input.primaryA.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "loaded post-canary Batch 7 debate order invalid"
  );
