...
export const POST_CANARY_BATCH_09_SCORE_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-09/score-pass";
export const POST_CANARY_BATCH_09_CALCULATED_SCORES_VERSION =
  "1.0-assessment-production-post-canary-batch-09-adjudicated-calculated-scores";
export const POST_CANARY_BATCH_09_SCORE_STABILITY_THRESHOLDS =
  ACTIVE_SCORE_STABILITY_THRESHOLDS;
...
export function evaluatePostCanaryBatch09ScoreStability(
  debates,
  thresholds = POST_CANARY_BATCH_09_SCORE_STABILITY_THRESHOLDS
) {
  assertV4(
    Array.isArray(debates) && debates.length === 10,
    "ten scored post-canary Batch 9 debates required"
  );
  assertV4(
    canonicalJson(thresholds) ===
      canonicalJson(POST_CANARY_BATCH_09_SCORE_STABILITY_THRESHOLDS),
    "post-canary Batch 9 active thresholds changed"
  );
...
export function derivePostCanaryBatch09Scores(
  ledger,
  debateInputs,
  ...
  assertV4(
    ...
    "validated score-free post-canary Batch 9 final ledger required for scoring"
  );
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === 10,
    "ten post-canary Batch 9 score inputs required"
  );
  ...
  const debates = debateInputs.map((input) => {
    ...
    assertV4(
      finalLedgerDebate,
      `${debateNumber}: Batch 9 final-ledger debate missing`
    );
  ...
  const stability = evaluatePostCanaryBatch09ScoreStability(debates);
  ...
  return {
    schemaVersion: POST_CANARY_BATCH_09_CALCULATED_SCORES_VERSION,
    ...
    status: stability.acceptancePassed
      ...
      : "post-canary-batch-09-single-score-pass-stability-gate-failed",
...
export function validatePostCanaryBatch09Scores(
  scores,
  ledger,
  debateInputs,
  productionReferences,
  ...
  const expected = derivePostCanaryBatch09Scores(
    ledger,
    debateInputs,
    productionReferences,
    hashes
  ...
  assertV4(
    ...
    "post-canary Batch 9 calculated scores differ from deterministic replay"
  );
  return {
    status: "passed",
