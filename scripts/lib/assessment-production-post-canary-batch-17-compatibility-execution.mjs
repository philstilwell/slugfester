...
export const POST_CANARY_BATCH_17_COMPATIBILITY_ACTIVATION_STATUS =
...
export function buildPostCanaryBatch17ValidatorSource(baselineSource) {
  ...
  source = replaceExactOnce(
    ...
    "Batch 17 adapter-version route"
  );

  const routeFunctions = `export function validatePostCanaryBatch17LedgerAdapterRouteLocks({
  debate,
  ledgerText,
  packetPath,
  packetText,
...
`;
  source = replaceExactOnce(
    ...
    "Batch 17 route functions"
  );

  const batch17Branch = `  if (
...
`;
  source = replaceExactOnce(
    source,
    "  let calculated;\n",
    `${batch17Branch}  let calculated;\n`,
    "Batch 17 ledger branch"
  );

  validatePostCanaryBatch17ValidatorSource(source);
  return source;
}

export function validatePostCanaryBatch17ValidatorSource(source) {
  const required = [
    "POST_CANARY_BATCH_17_COMPATIBILITY_ROOT",
    "POST_CANARY_BATCH_17_SITE_LEDGER_ADAPTER_VERSION",
    "validatePostCanaryBatch17SiteLedgerAdapter",
    ...
    "validatePostCanaryBatch17LedgerAdapterRoute",
    POST_CANARY_BATCH_17_COMPATIBILITY_ACTIVATION_STATUS,
    "Batch 17 production assessments require an AI Extension",
    "Batch 17 ledger validation failed"
  ];
  for (const text of required) {
    if (!source.includes(text)) {
      throw new Error(`proposed Batch 17 validator is missing ${text}`);
    }
  }
  if (
    ...
    !source.includes("POST_CANARY_BATCH_16_SITE_LEDGER_ADAPTER_VERSION") ||
    !source.includes("validatePostCanaryBatch16LedgerAdapterRoute") ||
    ...
    throw new Error("proposed Batch 17 validator weakens an existing route");
  }
  return {
    status: "passed",
