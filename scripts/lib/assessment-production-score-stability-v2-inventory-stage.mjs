...
export const V2_INVENTORY = Object.freeze({
  ...
  root: "docs/assessment-production/score-stability-v2-validation-cohort/inventory",
  preparation:
    "docs/assessment-production/score-stability-v2-validation-cohort/inventory/preparation-manifest.json",
  manifest:
    "docs/assessment-production/score-stability-v2-validation-cohort/inventory/execution-manifest.json",
  execution:
    "docs/assessment-production/score-stability-v2-validation-cohort/inventory/model-execution.json",
  analysis:
    "docs/assessment-production/score-stability-v2-validation-cohort/inventory/analysis.json",
  manifestStatus:
    "frozen-ten-v2-validation-score-blind-inventory-contexts-authorized",
  executionPassedStatus:
    "ten-v2-validation-score-blind-inventory-contexts-passed",
  executionFailedStatus:
    "v2-validation-score-blind-inventory-complete-with-failure",
  analysisStatus:
    "ten-v2-validation-score-blind-inventories-passed-independent-judgment-packet-preparation-authorized",
  contexts: 10,
  candidates: 406,
  proCandidates: 203,
  conCandidates: 203,
...
const PREREGISTER_PATH =
  "scripts/preregister-assessment-production-score-stability-v2-inventory.mjs";
const RUNNER_PATH =
  "scripts/run-assessment-production-score-stability-v2-inventory.mjs";
const ANALYZER_PATH =
  "scripts/analyze-assessment-production-score-stability-v2-inventory.mjs";
const VALIDATOR_PATH =
  "scripts/validate-assessment-production-score-stability-v2-inventory.mjs";
const TEST_PATH =
  "scripts/test-assessment-production-score-stability-v2-inventory-gate.mjs";
...
async function readPreparation() {
  ...
  assertV4(
    preparation.status ===
      ...
      preparation.transport?.modelTransportAndSchemaSerialization ===
        "lossless compact JSON with terminal newline" &&
      preparation.transport?.parsedSemanticIdentityVerified === true &&
...
export async function preregisterV2Inventory({ shouldWrite, frozenAt }) {
  assertV4(
  ...
  const manifest = {
    schemaVersion:
      "1.0-score-stability-v2-fresh-validation-score-blind-inventory-execution-manifest",
    ...
    costEstimate: {
      ...
      estimateBasis:
        "The retired five-context hard-route inventory gate averaged 1.23 model-minutes per context and completed in 3.82 wall-minutes at concurrency two; this range adds headroom for ten varied validation debates and compact transports up to 98,839 bytes.",
    },
...
export async function validateV2InventoryProposal({
  proposalPath,
  preparationPath,
  debateNumber,
  shouldWrite,
  ...
  const summary = {
    schemaVersion:
      "1.0-score-stability-v2-fresh-validation-score-blind-inventory-validation",
    protocolId: preparation.protocolId,
    status: "passed",
    developmentValidationOnly: true,
    productionCanary: false,
...
export async function runV2Inventory() {
  ...
  async function executeContext(context, contextIndex) {
    ...
    const temporary = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-v2-validation-inventory-${context.debateNumber}-`
      )
    );
    const isolatedCodexHome = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-v2-validation-inventory-home-${context.debateNumber}-`
      )
    );
    ...
    try {
      ...
      const prompt = `Read manual.md, packet.json, candidate-evidence-bundle.json, and schema.json completely; read nothing else. Act only as the isolated score-blind inventory curator for development-validation Debate ${context.debateNumber}. All ${context.candidates} discovered candidates remain present with source-exact excerpts. Produce four to six weighted issue sections totaling exactly 100 percent, with one or two pro and one or two con selections in every section and no candidate used twice. Define one route per side and its burden bridges. Author only each selected candidate ID, a unique move ID, a global constructive-or-reply classification, and a source-faithful proposition. A reply must have an earlier selected opposing move in source chronology, but do not name targets. Ratings, response topology, burden contact, adjustments, scores, winners, tags, Overall Commentary, AI Extension, score-policy analysis, and publication prose are prohibited. Return exactly one schema-conforming JSON object.`;
      process.stdout.write(
        `[v2-validation-inventory] starting ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber}\n`
      );
    ...
    process.stdout.write(
      `[v2-validation-inventory] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
    );
    return record;
  }
  ...
  const execution = {
    schemaVersion:
      "1.0-score-stability-v2-fresh-validation-score-blind-inventory-model-execution",
...
export async function analyzeV2Inventory({ shouldWrite }) {
  const manifest = await readAuthorizedManifest();
  ...
  const analysis = {
    schemaVersion:
      "1.0-score-stability-v2-fresh-validation-score-blind-inventory-analysis",
    protocolId: manifest.protocolId,
...
export async function testV2InventoryGate() {
  ...
  assertV4(
    manifest.acceptance?.everyCandidateAvailableDuringSelection === true &&
      ...
      manifest.transport?.maximumCopiedInputBytes === 98839 &&
