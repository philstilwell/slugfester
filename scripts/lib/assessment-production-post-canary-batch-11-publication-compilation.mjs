...
import { validatePostCanaryBatch11PublicationOutput } from
  "./assessment-production-post-canary-batch-11-publication-validation.mjs";
import { assertV4, canonicalJson } from "./v4-lean-production.mjs";

export const POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-11/deterministic-publication-compilation";
export const POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-11-deterministic-publication-compilation";
export const POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_ORDER = Object.freeze(
  [...POST_CANARY_BATCH_11_PUBLICATION_DEBATES]);

export function compilePostCanaryBatch11PublicationStagingRecord({ output, packet, identity }) {
  validatePostCanaryBatch11PublicationOutput(output, packet);
  ...
  compiled.stagingAudit.protocolId = POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_PROTOCOL_ID;
  compiled.stagingAudit.productionCanary = false;
  compiled.stagingAudit.batchNumber = 11;
  return compiled;
}

export function validatePostCanaryBatch11CompiledStagingRecord({ compiled, output, packet, identity }) {
  const expected = compilePostCanaryBatch11PublicationStagingRecord({ output, packet, identity });
