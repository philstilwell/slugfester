...
import { extractAssessmentProductionPostCanaryBatch09Disagreements } from "./assessment-production-post-canary-batch-09-disagreement.mjs";
import {
  buildPostCanaryBatch09DisputeAdjudicationPacket,
  validatePostCanaryBatch09DisputeAdjudicationOutput
} from "./assessment-production-post-canary-batch-09-dispute-adjudication.mjs";

export const POST_CANARY_BATCH_09_FINAL_LEDGER_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-09/final-ledger";
export const POST_CANARY_BATCH_09_FINAL_LEDGER_PROTOCOL_ID =
  "assessment-production-post-canary-batch-09-adjudicated-consensus";
export const POST_CANARY_BATCH_09_FINAL_LEDGER_VERSION =
  "1.0-assessment-production-post-canary-batch-09-adjudicated-raw-ledger";
...
function validateInputReplay(input) {
  ...
  const replayedDisagreements =
    extractAssessmentProductionPostCanaryBatch09Disagreements(
      primaryA,
      primaryB,
      lockedInventory
    );
  assertV4(
    ...
    `${primaryA.debateNumber}: stored disagreements differ from deterministic Batch 9 replay`
  );
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication packet differs from deterministic Batch 9 replay`
  );
  assertV4(
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication provenance differs from deterministic Batch 9 replay`
  );
  validatePostCanaryBatch09DisputeAdjudicationOutput(
    adjudicationOutput,
    adjudicationPacket
...
export function compilePostCanaryBatch09FinalLedgerDebate(input) {
  const {
    primaryA,
    primaryB,
    adjudicationPacket,
...
export function buildPostCanaryBatch09FinalLedger(debateInputs, sourceHashes) {
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === EXPECTED_DEBATES.length,
    "ten post-canary Batch 9 debate inputs required"
  );
  const debates = debateInputs.map(compilePostCanaryBatch09FinalLedgerDebate);
  assertV4(
    debates.map((debate) => debate.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "post-canary Batch 9 final-ledger debate order invalid"
  );
  ...
  assertV4(aggregate.finalMoves === 180, "Batch 9 final-move population mismatch");
  assertV4(aggregate.disputedMoves === 173, "Batch 9 disputed-move population mismatch");
  assertV4(candidateSelections === 459, "Batch 9 candidate-selection population mismatch");
  assertV4(
    aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed === 391,
    "Batch 9 rounded-mean population mismatch"
  );
  assertV4(aggregate.audioVerifiedMoves === 4, "Batch 9 audio-verification population mismatch");
  return {
    schemaVersion: POST_CANARY_BATCH_09_FINAL_LEDGER_VERSION,
    protocolId: POST_CANARY_BATCH_09_FINAL_LEDGER_PROTOCOL_ID,
    status: "passed-post-canary-batch-09-deterministic-final-ledger-assembly",
    ...
    AIOnly: true,
    scoringInputSchema:
      "v4.2.20-source-span-primary-with-post-canary-batch-09-adjudicated-provenance",
    ...
    nextAuthorizedAction:
      "standing-authorization-permits-batch-09-single-deterministic-score-pass-preparation"
  };
}

export function validatePostCanaryBatch09FinalLedger(
  ledger,
  debateInputs,
  sourceHashes
) {
  const expected = buildPostCanaryBatch09FinalLedger(debateInputs, sourceHashes);
  assertV4(
    canonicalJson(ledger) === canonicalJson(expected),
    "post-canary Batch 9 final ledger differs from deterministic replay"
  );
  assertV4(
    !containsProhibitedCalculatedField(
      ledger.debates.map((debate) => debate.finalJudgment)
    ),
    "post-canary Batch 9 final ledger contains a prohibited calculated field"
  );
  return {
    status: "passed",
...
export async function loadPostCanaryBatch09FinalLedgerInputs() {
  ...
  assertV4(
    adjudicationAnalysis.status ===
        "post-canary-batch-09-dispute-only-adjudication-gate-passed-standing-authorization-active-for-final-ledger-assembly" &&
      adjudicationAnalysis.gate.semanticPass === true &&
      adjudicationAnalysis.gate.validContexts === 10 &&
      adjudicationAnalysis.gate.disputedMovesDecided === 173 &&
      adjudicationAnalysis.gate.candidateSelections === 459 &&
      adjudicationAnalysis.gate.scoresDerived === 0 &&
      adjudicationExecution.status ===
        "ten-post-canary-batch-09-dispute-only-adjudication-contexts-passed" &&
      ...
      preparation.acceptedSourceBoundary.allFourAudioMovesVerified === true &&
      preparation.totals.audioVerifiedMoves === 4,
    "accepted post-canary Batch 9 adjudication gate unavailable"
  );
  const sourcePaths = [
  ...
  assertV4(
    debateInputs.map((input) => input.primaryA.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "loaded post-canary Batch 9 debate order invalid"
  );
