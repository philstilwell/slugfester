...
function parseSourceRows(chunkBytes) {
  assertV4(Buffer.isBuffer(chunkBytes), "Batch 11 token-counted chunk source must be a buffer");
  const sourceRows = chunkBytes
    ...
    .filter(Boolean)
    .map(JSON.parse);
  assertV4(sourceRows.length > 0, "Batch 11 token-counted chunk source is empty");
  for (const [index, row] of sourceRows.entries()) {
    assertV4(
      Array.isArray(row) &&
        ...
        typeof row[3] === "string" &&
        row[3].trim().length > 0,
      `Batch 11 token-counted ledger row ${index}: invalid source shape`
    );
  }
  return sourceRows;
}

export function findBatch11ZeroLexicalTokenRows(chunkBytes) {
...
export function buildBatch11TokenCountedChunkLedger(chunkBytes) {
