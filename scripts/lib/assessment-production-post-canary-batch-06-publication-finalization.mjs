import { POST_CANARY_BATCH_06_PUBLICATION_BYLINE,
  POST_CANARY_BATCH_06_PUBLICATION_MODEL } from
  "./assessment-production-post-canary-batch-06-publication.mjs";
import { POST_CANARY_BATCH_06_PUBLICATION_COMPILATION_ORDER,
  POST_CANARY_BATCH_06_PUBLICATION_COMPILATION_PROTOCOL_ID,
  validatePostCanaryBatch06CompiledStagingRecord } from
  "./assessment-production-post-canary-batch-06-publication-compilation.mjs";
import { assertV4, canonicalJson } from "./v4-lean-production.mjs";

export const POST_CANARY_BATCH_06_PUBLICATION_FINALIZATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-06/publication-finalization";
export const POST_CANARY_BATCH_06_PUBLICATION_FINALIZATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-06-publication-finalization";
export const POST_CANARY_BATCH_06_PUBLICATION_FINALIZATION_ORDER =
  POST_CANARY_BATCH_06_PUBLICATION_COMPILATION_ORDER;

export function buildPostCanaryBatch06PublicationFinalization({ compiled, compiledPath,
  compiledSha256, output, packet, identity }) {
  const compilationValidation = validatePostCanaryBatch06CompiledStagingRecord({
    compiled, output, packet, identity });
  assertV4(compiled.stagingAudit?.protocolId === POST_CANARY_BATCH_06_PUBLICATION_COMPILATION_PROTOCOL_ID &&
    compiled.stagingAudit.productionCanary === false && compiled.stagingAudit.batchNumber === 6 &&
    compiled.stagingAudit.stagingOnly === true && compiled.stagingAudit.productionMutationPerformed === false &&
    compiled.stagingAudit.displayContract.byline === POST_CANARY_BATCH_06_PUBLICATION_BYLINE &&
  ...
  const provenance = { schemaVersion: "1.0-assessment-production-post-canary-batch-06-publication-finalization-provenance",
    protocolId: POST_CANARY_BATCH_06_PUBLICATION_FINALIZATION_PROTOCOL_ID,
    status: "staging-only-batch-06-final-candidate-provenance",
    productionCanary: false, batchNumber: 6, debateNumber: compiled.number,
...
export function validatePostCanaryBatch06PublicationFinalCandidate({ candidate, provenance,
  compiled, output, packet, identity }) {
  const expected = buildPostCanaryBatch06PublicationFinalization({ compiled,
...
export function buildPostCanaryBatch06PublicationStagingPreviewHtml() {
  const allowed = JSON.stringify(POST_CANARY_BATCH_06_PUBLICATION_FINALIZATION_ORDER);
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8">
    ...
    <title>Slugfester Batch 6 publication staging preview</title>
  ...
  <body>
    ...
    <script type="module">
      ...
      const debateNumber = params.get("debate") || "73";
      if (!allowedHosts.has(window.location.hostname)) {
        ...
        document.querySelector("#app").innerHTML = "<main><h1>Unknown Batch 6 publication-staging debate</h1></main>";
      } else {
