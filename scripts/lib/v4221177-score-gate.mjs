...
export const V4221177_CALCULATED_SCORES_VERSION = "4.2.21.17.7-adjudicated-calculated-scores";
export const V4221177_STABILITY_THRESHOLDS = Object.freeze({
...
function deriveRawScores(raw, eventsDocument) {
  ...
  return { ...scores, protocolId: "v4.2.21.17.7-decomposed-consensus" };
}
...
export function evaluateV4221177Stability(debates, thresholds = V4221177_STABILITY_THRESHOLDS) {
...
export function deriveV4221177Scores(ledger, debateInputs, productionReferences, { finalLedgerSha256, productionReferenceSha256 }) {
  assertV4(ledger?.status === "passed-deterministic-final-ledger-assembly" && ledger.authorization.scoreDerivation && ledger.authorization.scorePassesMaximum === 1 && ledger.audit.calculatedScores === 0, "validated adjudicated final ledger required for scoring");
  ...
  const debates = debateInputs.map((input) => {
    ...
    assertV4(finalLedgerDebate, `${debateNumber}: final ledger debate missing`);
  ...
  const stability = evaluateV4221177Stability(debates);
  return {
    schemaVersion: V4221177_CALCULATED_SCORES_VERSION,
    protocolId: ledger.protocolId,
    status: stability.acceptancePassed ? "partition-single-score-pass-stability-gate-passed" : "partition-single-score-pass-stability-gate-failed",
    sources: { finalLedgerSha256, productionReferenceSha256 },
    formulaBoundary: { scoringPasses: 1, rawJudgmentsScoredInPass: 9, finalJudgmentsPublishedFromPass: 3, initialPassScoresUsedForPostAdjudicationStabilityDiagnosticsOnly: true, modelCalculatedScores: false, scoresDerivedOnlyAfterAdjudicatedLedgerLock: true, moveFormula: "repository v4.1 deterministic seven-dimension formula", responseFormula: "repository mapping from adjudicated response class and within-class position", sectionFormula: "importance-weighted move mean", overallFormula: "section-weighted mean plus eligible burden-completion adjustment, rounded and bounded 0-100" },
    acceptanceRule: { prospective: true, productionScoresDiagnosticOnly: true, agreedInitialWinnerMustBePreserved: true, ...structuredClone(V4221177_STABILITY_THRESHOLDS), postResultTuningAllowed: false, automaticRerunAllowed: false },
    debates,
    stability,
    totals: { debates: 3, finalSides: 6, scoringPasses: 1, modelContexts: 0, retries: 0, meteredApiCostUsd: 0, transcriptionCostUsd: 0, acceptancePassed: stability.acceptancePassed },
