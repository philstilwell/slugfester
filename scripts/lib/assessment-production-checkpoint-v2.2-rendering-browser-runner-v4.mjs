...
async function analyzePng(bytes, analyzerPath) {
  const temporary = await mkdtemp(path.join(tmpdir(), "slugfester-v4-image-"));
...
function imageChecks(analysis, viewport, target, ratio, contract) {
  return {
    png: analysis.format === contract.format &&
...
export async function runCheckpointV22RenderingRemedyV4Viewport({
  browser,
  packet,
  viewportName,
  ...
  try {
    ...
    const collapsedBytes = await capturePng(diagnostics.cdp);
    const collapsedTransport = await analyzePng(
      collapsedBytes,
    ...
    const openBytes = await capturePng(diagnostics.cdp);
    const openTransport = await analyzePng(
      openBytes,
      packet.runnerPolicy.imageAnalyzerPath
    );
  ...
  const checks = {
    ...
    collapsedScreenshotPng: collapsedImageChecks.png,
    openScreenshotPng: openImageChecks.png,
