...
import { validatePostCanaryBatch15PublicationOutput } from
  "./assessment-production-post-canary-batch-15-publication-validation.mjs";
import { assertV4, canonicalJson } from "./v4-lean-production.mjs";

export const POST_CANARY_BATCH_15_PUBLICATION_COMPILATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-15/deterministic-publication-compilation";
export const POST_CANARY_BATCH_15_PUBLICATION_COMPILATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-15-deterministic-publication-compilation";
export const POST_CANARY_BATCH_15_PUBLICATION_COMPILATION_ORDER = Object.freeze(
  [...POST_CANARY_BATCH_15_PUBLICATION_DEBATES]);

export function compilePostCanaryBatch15PublicationStagingRecord({ output, packet, identity }) {
  validatePostCanaryBatch15PublicationOutput(output, packet);
  ...
  compiled.stagingAudit.protocolId = POST_CANARY_BATCH_15_PUBLICATION_COMPILATION_PROTOCOL_ID;
  compiled.stagingAudit.productionCanary = false;
  compiled.stagingAudit.batchNumber = 15;
  return compiled;
}

export function validatePostCanaryBatch15CompiledStagingRecord({ compiled, output, packet, identity }) {
  const expected = compilePostCanaryBatch15PublicationStagingRecord({ output, packet, identity });
