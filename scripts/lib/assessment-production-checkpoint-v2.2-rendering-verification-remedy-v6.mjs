...
} from "./assessment-production-checkpoint-v2.2-rendering-verification-remedy-v5.mjs";
import {
...
export const CHECKPOINT_V22_RENDERING_REMEDY_V6_ROOT =
  "docs/assessment-production/production-checkpoint-v2.2-1/rendering-verification-remedy-v6";
export const CHECKPOINT_V22_RENDERING_REMEDY_V6_PROTOCOL_ID =
  "assessment-production-checkpoint-v2.2-1-rendering-verification-remedy-v6";
export const CHECKPOINT_V22_RENDERING_REMEDY_V6_ORDER =
  CHECKPOINT_V22_RENDERING_REMEDY_V5_ORDER;
export const CHECKPOINT_V22_RENDERING_REMEDY_V6_MODEL =
  CHECKPOINT_V22_RENDERING_REMEDY_V5_MODEL;
export const CHECKPOINT_V22_RENDERING_REMEDY_V6_VIEWPORTS =
  CHECKPOINT_V22_RENDERING_REMEDY_V5_VIEWPORTS;
export const CHECKPOINT_V22_RENDERING_REMEDY_V6_PORT = 4194;
export const CHECKPOINT_V22_RENDERING_REMEDY_V6_IMAGE_CONTRACT = Object.freeze({
...
export const CHECKPOINT_V22_RENDERING_REMEDY_V6_REQUIRED_BOOLEAN_CHECKS =
  Object.freeze([
    ...CHECKPOINT_V22_RENDERING_REMEDY_V3_REQUIRED_BOOLEAN_CHECKS,
...
function evidencePaths(debateNumber, viewportName) {
  const root =
    `${CHECKPOINT_V22_RENDERING_REMEDY_V6_ROOT}/evidence/debate-${debateNumber}`;
  return {
...
export function buildCheckpointV22RenderingRemedyV6Packet({
  sourcePacket,
  sourcePacketPath,
  sourcePacketSha256,
  failedExecutionPath,
  ...
  assertV4(
    ...
    `${sourcePacket?.debateNumber ?? "unknown"}: invalid remedy-v5 packet hash`
  );
  const packet = {
    ...structuredClone(sourcePacket),
    schemaVersion:
      "1.0-production-checkpoint-v2.2-rendering-remedy-v6-packet",
    protocolId: CHECKPOINT_V22_RENDERING_REMEDY_V6_PROTOCOL_ID,
    status: "frozen-sixth-replacement-rendering-verification-packet",
    preview: {
      ...structuredClone(sourcePacket.preview),
      url:
        `http://127.0.0.1:${CHECKPOINT_V22_RENDERING_REMEDY_V6_PORT}/` +
        `${sourcePacket.preview.path}?debate=${sourcePacket.debateNumber}`
    },
    viewports: Object.fromEntries(
      Object.entries(CHECKPOINT_V22_RENDERING_REMEDY_V6_VIEWPORTS).map(
        ([name, viewport]) => [
          name,
          {
    ...
    interactionOrder: [
      ...
      "extract-contract-derived-12-byte-collapsed-jpeg-signature",
      ...
      "extract-contract-derived-12-byte-open-jpeg-signature",
    ...
    runnerPolicy: {
      ...
      imageAnalysisContractPath,
      imageAnalyzerPath: CHECKPOINT_V22_RENDERING_REMEDY_V6_IMAGE_CONTRACT.analyzerPath,
      imageAnalyzerSha256:
      ...
      signatureByteCountMustEqualFrozenContract: true,
...
export function validateCheckpointV22RenderingRemedyV6Packet(packet) {
  assertV4(
    packet.schemaVersion ===
        "1.0-production-checkpoint-v2.2-rendering-remedy-v6-packet" &&
      packet.protocolId === CHECKPOINT_V22_RENDERING_REMEDY_V6_PROTOCOL_ID &&
      packet.status ===
        "frozen-sixth-replacement-rendering-verification-packet" &&
      packet.productionCanary === true &&
      packet.stagingOnly === true &&
      CHECKPOINT_V22_RENDERING_REMEDY_V6_ORDER.includes(packet.debateNumber) &&
      ...
      packet.runnerPolicy.signatureByteCountMustEqualFrozenContract === true &&
    ...
    `${packet?.debateNumber ?? "unknown"}: invalid remedy-v6 packet`
  );
  for (const [name, expected] of Object.entries(
    ...
    const root =
      `${CHECKPOINT_V22_RENDERING_REMEDY_V6_ROOT}/evidence/debate-${packet.debateNumber}`;
    assertV4(
      ...
      `${packet.debateNumber}: invalid ${name} remedy-v6 viewport`
    );
  }
  return packet;
}
...
export function validateCheckpointV22RenderingRemedyV6ViewportEvidence({
  packet,
  viewportName,
  ...
  assertV4(
    viewport &&
      ...
      evidence.schemaVersion ===
        "1.0-production-checkpoint-v2.2-rendering-remedy-v6-viewport-evidence" &&
      evidence.protocolId === CHECKPOINT_V22_RENDERING_REMEDY_V6_PROTOCOL_ID &&
    ...
    `${packet?.debateNumber ?? "unknown"}: invalid ${viewportName} remedy-v6 evidence`
  );
  return evidence;
}
