...
import { extractAssessmentProductionPostCanaryBatch05Disagreements } from "./assessment-production-post-canary-batch-05-disagreement.mjs";
import {
  buildPostCanaryBatch05DisputeAdjudicationPacket,
  validatePostCanaryBatch05DisputeAdjudicationOutput
} from "./assessment-production-post-canary-batch-05-dispute-adjudication.mjs";

export const POST_CANARY_BATCH_05_FINAL_LEDGER_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-05/final-ledger";
export const POST_CANARY_BATCH_05_FINAL_LEDGER_PROTOCOL_ID =
  "assessment-production-post-canary-batch-05-adjudicated-consensus";
export const POST_CANARY_BATCH_05_FINAL_LEDGER_VERSION =
  "1.0-assessment-production-post-canary-batch-05-adjudicated-raw-ledger";
...
function validateInputReplay(input) {
  ...
  const replayedDisagreements =
    extractAssessmentProductionPostCanaryBatch05Disagreements(
      primaryA,
      primaryB,
      lockedInventory
    );
  assertV4(
    ...
    `${primaryA.debateNumber}: stored disagreements differ from deterministic Batch 5 replay`
  );
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication packet differs from deterministic Batch 5 replay`
  );
  assertV4(
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication provenance differs from deterministic Batch 5 replay`
  );
  validatePostCanaryBatch05DisputeAdjudicationOutput(
    adjudicationOutput,
    adjudicationPacket
...
export function compilePostCanaryBatch05FinalLedgerDebate(input) {
  const {
    primaryA,
    primaryB,
    adjudicationPacket,
...
export function buildPostCanaryBatch05FinalLedger(debateInputs, sourceHashes) {
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === EXPECTED_DEBATES.length,
    "ten post-canary Batch 5 debate inputs required"
  );
  const debates = debateInputs.map(compilePostCanaryBatch05FinalLedgerDebate);
  assertV4(
    debates.map((debate) => debate.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "post-canary Batch 5 final-ledger debate order invalid"
  );
  ...
  assertV4(aggregate.finalMoves === 187, "Batch 5 final-move population mismatch");
  assertV4(aggregate.disputedMoves === 177, "Batch 5 disputed-move population mismatch");
  assertV4(candidateSelections === 527, "Batch 5 candidate-selection population mismatch");
  assertV4(
    aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed === 411,
    "Batch 5 rounded-mean population mismatch"
  );
  assertV4(aggregate.audioVerifiedMoves === 6, "Batch 5 audio-verification population mismatch");
  return {
    schemaVersion: POST_CANARY_BATCH_05_FINAL_LEDGER_VERSION,
    protocolId: POST_CANARY_BATCH_05_FINAL_LEDGER_PROTOCOL_ID,
    status: "passed-post-canary-batch-05-deterministic-final-ledger-assembly",
    ...
    AIOnly: true,
    scoringInputSchema:
      "v4.2.20-source-span-primary-with-post-canary-batch-05-adjudicated-provenance",
    ...
    nextAuthorizedAction:
      "standing-authorization-permits-batch-05-single-deterministic-score-pass-preparation"
  };
}

export function validatePostCanaryBatch05FinalLedger(
  ledger,
  debateInputs,
  sourceHashes
) {
  const expected = buildPostCanaryBatch05FinalLedger(debateInputs, sourceHashes);
  assertV4(
    canonicalJson(ledger) === canonicalJson(expected),
    "post-canary Batch 5 final ledger differs from deterministic replay"
  );
  assertV4(
    !containsProhibitedCalculatedField(
      ledger.debates.map((debate) => debate.finalJudgment)
    ),
    "post-canary Batch 5 final ledger contains a prohibited calculated field"
  );
  return {
    status: "passed",
...
export async function loadPostCanaryBatch05FinalLedgerInputs() {
  ...
  assertV4(
    adjudicationAnalysis.status ===
        "post-canary-batch-05-dispute-only-adjudication-gate-passed-standing-authorization-active-for-final-ledger-assembly" &&
      adjudicationAnalysis.gate.semanticPass === true &&
      adjudicationAnalysis.gate.validContexts === 10 &&
      adjudicationAnalysis.gate.disputedMovesDecided === 177 &&
      adjudicationAnalysis.gate.candidateSelections === 527 &&
      adjudicationAnalysis.gate.scoresDerived === 0 &&
      adjudicationExecution.status ===
        "ten-post-canary-batch-05-dispute-only-adjudication-contexts-passed" &&
      adjudicationExecution.validContexts === 10 &&
      standingAuthorization.authorization.finalLedgerAssembly === true &&
      standingAuthorization.authorization.singleScorePass === true,
    "accepted post-canary Batch 5 adjudication gate unavailable"
  );
  assertV4(
    ...
    "complete Batch 5 audio verification unavailable"
  );
  ...
  assertV4(
    debateInputs.map((input) => input.primaryA.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "loaded post-canary Batch 5 debate order invalid"
  );
