...
export const POST_CANARY_BATCH_04_PUBLICATION_COMPILATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-04/deterministic-publication-compilation";
export const POST_CANARY_BATCH_04_PUBLICATION_COMPILATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-04-deterministic-publication-compilation";
export const POST_CANARY_BATCH_04_PUBLICATION_COMPILATION_ORDER = Object.freeze(
  [...POST_CANARY_BATCH_04_PUBLICATION_DEBATES]
);
...
function referenceUrl(tag) {
  ...
  assertV4(
    ...
    `unknown Batch 4 publication reference ${tag.type}:${tag.slug}`
  );
...
export function compilePostCanaryBatch04PublicationStagingRecord({
  output,
  packet,
  identity
}) {
...
export function validatePostCanaryBatch04CompiledStagingRecord({
  compiled,
  output,
  packet,
  identity
}) {
  const expected = compilePostCanaryBatch04PublicationStagingRecord({
    output,
    packet,
    identity
  });
