...
export const POST_CANARY_BATCH_07_COMPATIBILITY_ACTIVATION_STATUS =
...
export function buildPostCanaryBatch07ValidatorSource(baselineSource) {
  ...
  source = replaceExactOnce(
    ...
    "Batch 7 adapter-version route"
  );

  const routeFunctions = `export function validatePostCanaryBatch07LedgerAdapterRouteLocks({
  debate,
  ledgerText,
  packetPath,
  packetText,
...
`;
  source = replaceExactOnce(
    ...
    "Batch 7 route functions"
  );

  const batch07Branch = `  if (
...
`;
  source = replaceExactOnce(
    source,
    "  let calculated;\n",
    `${batch07Branch}  let calculated;\n`,
    "Batch 7 ledger branch"
  );

  validatePostCanaryBatch07ValidatorSource(source);
  return source;
}

export function validatePostCanaryBatch07ValidatorSource(source) {
  const required = [
    "POST_CANARY_BATCH_07_COMPATIBILITY_ROOT",
    "POST_CANARY_BATCH_07_SITE_LEDGER_ADAPTER_VERSION",
    "validatePostCanaryBatch07SiteLedgerAdapter",
    ...
    "validatePostCanaryBatch07LedgerAdapterRoute",
    POST_CANARY_BATCH_07_COMPATIBILITY_ACTIVATION_STATUS,
    "Batch 7 production assessments require an AI Extension",
    "Batch 7 ledger validation failed"
  ];
  for (const text of required) {
    if (!source.includes(text)) {
      throw new Error(`proposed Batch 7 validator is missing ${text}`);
    }
  }
  if (
    ...
    throw new Error("proposed Batch 7 validator weakens an existing route");
  }
  return {
    status: "passed",
