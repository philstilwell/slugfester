...
import { extractAssessmentProductionPostCanaryBatch08Disagreements } from "./assessment-production-post-canary-batch-08-disagreement.mjs";
import {
  buildPostCanaryBatch08DisputeAdjudicationPacket,
  validatePostCanaryBatch08DisputeAdjudicationOutput
} from "./assessment-production-post-canary-batch-08-dispute-adjudication.mjs";

export const POST_CANARY_BATCH_08_FINAL_LEDGER_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-08/final-ledger";
export const POST_CANARY_BATCH_08_FINAL_LEDGER_PROTOCOL_ID =
  "assessment-production-post-canary-batch-08-adjudicated-consensus";
export const POST_CANARY_BATCH_08_FINAL_LEDGER_VERSION =
  "1.0-assessment-production-post-canary-batch-08-adjudicated-raw-ledger";
...
function audioMap(audioAudit, audioExecutionManifest) {
  return new Map(
    audioAudit.debates.flatMap((debate) =>
      debate.moves.map((move) => {
        ...
        assertV4(
          frozenCall && frozenCall.clipSha256 && frozenCall.durationSeconds,
...
function validateInputReplay(input) {
  ...
  const replayedDisagreements =
    extractAssessmentProductionPostCanaryBatch08Disagreements(
      primaryA,
      primaryB,
      lockedInventory
    );
  assertV4(
    ...
    `${primaryA.debateNumber}: stored disagreements differ from deterministic Batch 8 replay`
  );
  const rebuiltAdjudication =
    buildPostCanaryBatch08DisputeAdjudicationPacket(
      ...
      lockedInventory,
      originalEventsDocument,
      audioMap(audioAudit, audioExecutionManifest)
    );
  assertV4(
    canonicalJson(rebuiltAdjudication.packet) ===
      canonicalJson(adjudicationPacket),
    `${primaryA.debateNumber}: adjudication packet differs from deterministic Batch 8 replay`
  );
  assertV4(
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication provenance differs from deterministic Batch 8 replay`
  );
  validatePostCanaryBatch08DisputeAdjudicationOutput(
    adjudicationOutput,
    adjudicationPacket
...
export function compilePostCanaryBatch08FinalLedgerDebate(input) {
  const {
    primaryA,
    primaryB,
    adjudicationPacket,
...
export function buildPostCanaryBatch08FinalLedger(debateInputs, sourceHashes) {
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === EXPECTED_DEBATES.length,
    "ten post-canary Batch 8 debate inputs required"
  );
  const debates = debateInputs.map(compilePostCanaryBatch08FinalLedgerDebate);
  assertV4(
    debates.map((debate) => debate.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "post-canary Batch 8 final-ledger debate order invalid"
  );
  ...
  assertV4(aggregate.finalMoves === 182, "Batch 8 final-move population mismatch");
  assertV4(aggregate.disputedMoves === 174, "Batch 8 disputed-move population mismatch");
  assertV4(candidateSelections === 533, "Batch 8 candidate-selection population mismatch");
  assertV4(
    aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed === 373,
    "Batch 8 rounded-mean population mismatch"
  );
  assertV4(aggregate.audioVerifiedMoves === 6, "Batch 8 audio-verification population mismatch");
  return {
    schemaVersion: POST_CANARY_BATCH_08_FINAL_LEDGER_VERSION,
    protocolId: POST_CANARY_BATCH_08_FINAL_LEDGER_PROTOCOL_ID,
    status: "passed-post-canary-batch-08-deterministic-final-ledger-assembly",
    ...
    AIOnly: true,
    scoringInputSchema:
      "v4.2.20-source-span-primary-with-post-canary-batch-08-adjudicated-provenance",
    ...
    nextAuthorizedAction:
      "standing-authorization-permits-batch-08-single-deterministic-score-pass-preparation"
  };
}

export function validatePostCanaryBatch08FinalLedger(
  ledger,
  debateInputs,
  sourceHashes
) {
  const expected = buildPostCanaryBatch08FinalLedger(debateInputs, sourceHashes);
  assertV4(
    canonicalJson(ledger) === canonicalJson(expected),
    "post-canary Batch 8 final ledger differs from deterministic replay"
  );
  assertV4(
    !containsProhibitedCalculatedField(
      ledger.debates.map((debate) => debate.finalJudgment)
    ),
    "post-canary Batch 8 final ledger contains a prohibited calculated field"
  );
  return {
    status: "passed",
...
export async function loadPostCanaryBatch08FinalLedgerInputs() {
  ...
  const acceptedAudioRoot = `${AUDIO_ROOT}/resolution-execution`;
  ...
  const standingAuthorizationPath = `${BATCH_ROOT}/continuation-standing-authorization.json`;
  const adjudicationCorrectionAnalysisPath =
    `${ADJUDICATION_ROOT}/gate-correction-2-analysis.json`;
  const [
    ...
    adjudicationCorrectionAnalysis
  ] = await Promise.all(
    [
      ...
      adjudicationCorrectionAnalysisPath
    ].map((file) => readFile(path.resolve(file), "utf8").then(JSON.parse))
  );
  assertV4(
    adjudicationAnalysis.status ===
        "post-canary-batch-08-dispute-only-adjudication-gate-passed-standing-authorization-active-for-final-ledger-assembly" &&
      adjudicationAnalysis.gate.semanticPass === true &&
      adjudicationAnalysis.gate.validContexts === 10 &&
      adjudicationAnalysis.gate.disputedMovesDecided === 174 &&
      adjudicationAnalysis.gate.candidateSelections === 533 &&
      adjudicationAnalysis.gate.scoresDerived === 0 &&
      adjudicationExecution.status ===
        "ten-post-canary-batch-08-dispute-only-adjudication-contexts-passed" &&
      adjudicationExecution.validContexts === 10 &&
      standingAuthorization.executionControls.scorePassesMaximum === 1 &&
      standingAuthorization.automaticContinuation.whileEveryFrozenGatePasses === true &&
      adjudicationCorrectionAnalysis.status ===
        "passed-batch-08-adjudication-gate-after-recursive-harness-correction" &&
      adjudicationCorrectionAnalysis.acceptedGate.validContexts === 10 &&
      adjudicationCorrectionAnalysis.acceptedGate.scoresDerived === 0,
    "accepted post-canary Batch 8 adjudication gate unavailable"
  );
  assertV4(
    audioAnalysis.status ===
        "passed-all-six-batch-08-audio-attributions-after-transient-reference-overlays" &&
      audioAnalysis.gate.verified === 6 &&
      audioAnalysis.gate.unresolved === 0 &&
      audioAudit.status ===
        "passed-all-six-batch-08-audio-attributions-after-transient-reference-overlays" &&
      audioAudit.totals.verified === 6 &&
      audioAudit.totals.unresolved === 0,
    "complete Batch 8 audio verification unavailable"
  );
  ...
  for (const context of preparation.contexts) {
    ...
    const result = adjudicationExecution.results.find(
  ...
  assertV4(
    debateInputs.map((input) => input.primaryA.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "loaded post-canary Batch 8 debate order invalid"
  );
