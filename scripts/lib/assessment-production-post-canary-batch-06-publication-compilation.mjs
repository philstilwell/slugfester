...
import { validatePostCanaryBatch06PublicationOutput } from
  "./assessment-production-post-canary-batch-06-publication-validation.mjs";
import { assertV4, canonicalJson } from "./v4-lean-production.mjs";

export const POST_CANARY_BATCH_06_PUBLICATION_COMPILATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-06/deterministic-publication-compilation";
export const POST_CANARY_BATCH_06_PUBLICATION_COMPILATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-06-deterministic-publication-compilation";
export const POST_CANARY_BATCH_06_PUBLICATION_COMPILATION_ORDER = Object.freeze(
  [...POST_CANARY_BATCH_06_PUBLICATION_DEBATES]);

export function compilePostCanaryBatch06PublicationStagingRecord({ output, packet, identity }) {
  validatePostCanaryBatch06PublicationOutput(output, packet);
  ...
  compiled.stagingAudit.protocolId = POST_CANARY_BATCH_06_PUBLICATION_COMPILATION_PROTOCOL_ID;
  compiled.stagingAudit.productionCanary = false;
  compiled.stagingAudit.batchNumber = 6;
  return compiled;
}

export function validatePostCanaryBatch06CompiledStagingRecord({ compiled, output, packet, identity }) {
  const expected = compilePostCanaryBatch06PublicationStagingRecord({ output, packet, identity });
