...
export const POST_CANARY_BATCH_09_COMPATIBILITY_ACTIVATION_STATUS =
...
export function buildPostCanaryBatch09ValidatorSource(baselineSource) {
  ...
  source = replaceExactOnce(
    ...
    "Batch 9 adapter-version route"
  );

  const routeFunctions = `export function validatePostCanaryBatch09LedgerAdapterRouteLocks({
  debate,
  ledgerText,
  packetPath,
  packetText,
...
`;
  source = replaceExactOnce(
    ...
    "Batch 9 ledger branch"
  );

  validatePostCanaryBatch09ValidatorSource(source);
  return source;
}

export function validatePostCanaryBatch09ValidatorSource(source) {
  const required = [
    "POST_CANARY_BATCH_09_COMPATIBILITY_ROOT",
    "POST_CANARY_BATCH_09_SITE_LEDGER_ADAPTER_VERSION",
    "validatePostCanaryBatch09SiteLedgerAdapter",
    ...
    "validatePostCanaryBatch09LedgerAdapterRoute",
    POST_CANARY_BATCH_09_COMPATIBILITY_ACTIVATION_STATUS,
    "Batch 9 production assessments require an AI Extension",
    "Batch 9 ledger validation failed"
  ];
  for (const text of required) {
    if (!source.includes(text)) {
      throw new Error(`proposed Batch 9 validator is missing ${text}`);
    }
  }
  if (
    ...
    throw new Error("proposed Batch 9 validator weakens an existing route");
  }
  return {
    status: "passed",
