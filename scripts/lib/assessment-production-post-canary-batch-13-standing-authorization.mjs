...
export const POST_CANARY_BATCH_13_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-13";
export const POST_CANARY_BATCH_13_STANDING_AUTHORIZATION =
  `${POST_CANARY_BATCH_13_ROOT}/standing-authorization.json`;
export const POST_CANARY_BATCH_13_STANDING_AUTHORIZATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-13-standing-authorization";
export const POST_CANARY_BATCH_13_STANDING_AUTHORIZATION_STATUS =
  "frozen-active-batch-13-complete-remaining-workflow-standing-authorization";
export const POST_CANARY_BATCH_13_STANDING_AUTHORIZATION_INSTRUCTION_PATH =
  POST_CANARY_BATCH_13_ROOT + "/standing-authorization-instruction.txt";
export const POST_CANARY_BATCH_13_STANDING_AUTHORIZATION_INSTRUCTION = readFileSync(
  path.resolve(POST_CANARY_BATCH_13_STANDING_AUTHORIZATION_INSTRUCTION_PATH),
  "utf8"
).trimEnd();

const sha256 = (value) => createHash("sha256").update(value).digest("hex");

export function validatePostCanaryBatch13StandingAuthorization(record) {
  assertV4(
    record?.schemaVersion ===
        "1.0-assessment-production-post-canary-batch-13-standing-authorization" &&
      record.protocolId ===
        POST_CANARY_BATCH_13_STANDING_AUTHORIZATION_PROTOCOL_ID &&
      record.status === POST_CANARY_BATCH_13_STANDING_AUTHORIZATION_STATUS &&
      record.productionCanary === false &&
      record.batchNumber === 13 &&
      record.sourcePacketCommit === "cb8cee8014d3aafc51e3b77548474891f8cd8161" &&
      record.stagingOnly === true &&
      !Number.isNaN(Date.parse(record.authorizedAt)) &&
      record.userAuthorization?.instruction ===
        POST_CANARY_BATCH_13_STANDING_AUTHORIZATION_INSTRUCTION &&
      record.userAuthorization?.resolvedAntecedent ===
        "The user's instruction to continue Batch 13 at discretion authorizes the complete reassessment, bounded recovery, publication, validation, commit, and push workflow under the current frozen campaign controls." &&
      record.userAuthorization?.scopeInterpretation ===
        "Create and use a complete-workflow Batch 13 standing authorization for the ten frozen selections, beginning with the separately frozen score-blind source packets, including bounded diagnosed recovery and production publication, while preserving the established campaign controls and stopping before Batch 14 selection." &&
    ...
    "Batch 13 standing-authorization identity changed"
  );
  assertV4(
    canonicalJson(record.selectedDebates) ===
      canonicalJson(["26", "190", "87", "20", "70", "30", "37", "117", "111", "34"]),
    "Batch 13 standing-authorization debate boundary changed"
  );
  assertV4(
    ...
    "Batch 13 standing-authorization model changed"
  );
  assertV4(
    ...
    "Batch 13 standing-authorization execution controls changed"
  );
  assertV4(
  ...
  assertV4(
    ...
    "Batch 13 standing-authorization cost boundary changed"
  );
  assertV4(
    ...
    "Batch 13 standing-authorization source-recovery controls changed"
  );
  assertV4(
    Object.values(record.stopRules ?? {}).every(Boolean),
    "Batch 13 standing-authorization stop rule disabled"
  );
  assertV4(
    ...
    "Batch 13 standing-authorization recovery controls changed"
  );
  assertV4(
    ...
    "Batch 13 standing-authorization Git controls changed"
  );
  return record;
}

export async function loadAndValidatePostCanaryBatch13StandingAuthorization() {
  const bytes = await readFile(
    path.resolve(POST_CANARY_BATCH_13_STANDING_AUTHORIZATION)
  );
  const record = validatePostCanaryBatch13StandingAuthorization(JSON.parse(bytes));
