...
export const POST_CANARY_BATCH_07_SCORE_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-07/score-pass";
export const POST_CANARY_BATCH_07_CALCULATED_SCORES_VERSION =
  "1.0-assessment-production-post-canary-batch-07-adjudicated-calculated-scores";
export const POST_CANARY_BATCH_07_SCORE_STABILITY_THRESHOLDS =
  ACTIVE_SCORE_STABILITY_THRESHOLDS;
...
export function evaluatePostCanaryBatch07ScoreStability(
  debates,
  thresholds = POST_CANARY_BATCH_07_SCORE_STABILITY_THRESHOLDS
) {
  assertV4(
    Array.isArray(debates) && debates.length === 10,
    "ten scored post-canary Batch 7 debates required"
  );
  assertV4(
    canonicalJson(thresholds) ===
      canonicalJson(POST_CANARY_BATCH_07_SCORE_STABILITY_THRESHOLDS),
    "post-canary Batch 7 active thresholds changed"
  );
...
export function derivePostCanaryBatch07Scores(
  ledger,
  debateInputs,
  ...
  const debates = debateInputs.map((input) => {
    ...
    assertV4(
      finalLedgerDebate,
      `${debateNumber}: Batch 7 final-ledger debate missing`
    );
  ...
  const stability = evaluatePostCanaryBatch07ScoreStability(debates);
  ...
  return {
    schemaVersion: POST_CANARY_BATCH_07_CALCULATED_SCORES_VERSION,
    ...
    status: stability.acceptancePassed
      ...
      : "post-canary-batch-07-single-score-pass-stability-gate-failed",
...
export function validatePostCanaryBatch07Scores(
  scores,
  ledger,
  debateInputs,
  productionReferences,
  ...
  const expected = derivePostCanaryBatch07Scores(
    ledger,
    debateInputs,
    productionReferences,
    hashes
  ...
  assertV4(
    ...
    "post-canary Batch 7 calculated scores differ from deterministic replay"
  );
  return {
    status: "passed",
