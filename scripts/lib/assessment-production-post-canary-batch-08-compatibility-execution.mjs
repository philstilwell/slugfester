...
export const POST_CANARY_BATCH_08_COMPATIBILITY_ACTIVATION_STATUS =
...
export function buildPostCanaryBatch08ValidatorSource(baselineSource) {
  let source = baselineSource;

  const batch07Import = `import {
  ...
  source = replaceExactOnce(
    ...
    "Batch 8 adapter-version route"
  );

  const routeFunctions = `export function validatePostCanaryBatch08LedgerAdapterRouteLocks({
  debate,
  ledgerText,
  packetPath,
  packetText,
...
`;
  source = replaceExactOnce(
    ...
    "Batch 8 route functions"
  );

  const batch08Branch = `  if (
...
`;
  source = replaceExactOnce(
    source,
    "  let calculated;\n",
    `${batch08Branch}  let calculated;\n`,
    "Batch 8 ledger branch"
  );

  validatePostCanaryBatch08ValidatorSource(source);
  return source;
}

export function validatePostCanaryBatch08ValidatorSource(source) {
  const required = [
    "POST_CANARY_BATCH_08_COMPATIBILITY_ROOT",
    "POST_CANARY_BATCH_08_SITE_LEDGER_ADAPTER_VERSION",
    "validatePostCanaryBatch08SiteLedgerAdapter",
    ...
    "validatePostCanaryBatch08LedgerAdapterRoute",
    POST_CANARY_BATCH_08_COMPATIBILITY_ACTIVATION_STATUS,
    "Batch 8 production assessments require an AI Extension",
    "Batch 8 ledger validation failed"
  ];
  for (const text of required) {
    if (!source.includes(text)) {
      throw new Error(`proposed Batch 8 validator is missing ${text}`);
    }
  }
  if (
    ...
    throw new Error("proposed Batch 8 validator weakens an existing route");
  }
  return {
    status: "passed",
