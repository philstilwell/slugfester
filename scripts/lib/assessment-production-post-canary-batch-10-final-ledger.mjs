...
import { extractAssessmentProductionPostCanaryBatch10Disagreements } from "./assessment-production-post-canary-batch-10-disagreement.mjs";
import {
  buildPostCanaryBatch10DisputeAdjudicationPacket,
  validatePostCanaryBatch10DisputeAdjudicationOutput
} from "./assessment-production-post-canary-batch-10-dispute-adjudication.mjs";

export const POST_CANARY_BATCH_10_FINAL_LEDGER_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-10/final-ledger";
export const POST_CANARY_BATCH_10_FINAL_LEDGER_PROTOCOL_ID =
  "assessment-production-post-canary-batch-10-adjudicated-consensus";
export const POST_CANARY_BATCH_10_FINAL_LEDGER_VERSION =
  "1.0-assessment-production-post-canary-batch-10-adjudicated-raw-ledger";
...
function validateInputReplay(input) {
  ...
  const replayedDisagreements =
    extractAssessmentProductionPostCanaryBatch10Disagreements(
      primaryA,
      primaryB,
      lockedInventory
    );
  assertV4(
    ...
    `${primaryA.debateNumber}: stored disagreements differ from deterministic Batch 10 replay`
  );
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication packet differs from deterministic Batch 10 replay`
  );
  assertV4(
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication provenance differs from deterministic Batch 10 replay`
  );
  validatePostCanaryBatch10DisputeAdjudicationOutput(
    adjudicationOutput,
    adjudicationPacket
...
export function compilePostCanaryBatch10FinalLedgerDebate(input) {
  const {
    primaryA,
    primaryB,
    adjudicationPacket,
...
export function buildPostCanaryBatch10FinalLedger(debateInputs, sourceHashes) {
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === EXPECTED_DEBATES.length,
    "ten post-canary Batch 10 debate inputs required"
  );
  const debates = debateInputs.map(compilePostCanaryBatch10FinalLedgerDebate);
  assertV4(
    debates.map((debate) => debate.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "post-canary Batch 10 final-ledger debate order invalid"
  );
  ...
  assertV4(aggregate.finalMoves === 182, "Batch 10 final-move population mismatch");
  assertV4(aggregate.disputedMoves === 177, "Batch 10 disputed-move population mismatch");
  assertV4(candidateSelections === 483, "Batch 10 candidate-selection population mismatch");
  assertV4(
    aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed === 381,
    "Batch 10 rounded-mean population mismatch"
  );
  assertV4(aggregate.audioVerifiedMoves === 9, "Batch 10 audio-verification population mismatch");
  return {
    schemaVersion: POST_CANARY_BATCH_10_FINAL_LEDGER_VERSION,
    protocolId: POST_CANARY_BATCH_10_FINAL_LEDGER_PROTOCOL_ID,
    status: "passed-post-canary-batch-10-deterministic-final-ledger-assembly",
    ...
    AIOnly: true,
    scoringInputSchema:
      "v4.2.20-source-span-primary-with-post-canary-batch-10-adjudicated-provenance",
    ...
    nextAuthorizedAction:
      "standing-authorization-permits-batch-10-single-deterministic-score-pass-preparation"
  };
}

export function validatePostCanaryBatch10FinalLedger(
  ledger,
  debateInputs,
  sourceHashes
) {
  const expected = buildPostCanaryBatch10FinalLedger(debateInputs, sourceHashes);
  assertV4(
    canonicalJson(ledger) === canonicalJson(expected),
    "post-canary Batch 10 final ledger differs from deterministic replay"
  );
  assertV4(
    !containsProhibitedCalculatedField(
      ledger.debates.map((debate) => debate.finalJudgment)
    ),
    "post-canary Batch 10 final ledger contains a prohibited calculated field"
  );
  return {
    status: "passed",
...
export async function loadPostCanaryBatch10FinalLedgerInputs() {
  ...
  assertV4(
    adjudicationAnalysis.status ===
      ...
      adjudicationAnalysis.gate.correctedDebates === 1 &&
      adjudicationAnalysis.gate.disputedMovesDecided === 177 &&
      adjudicationAnalysis.gate.candidateSelections === 483 &&
      ...
      preparation.acceptedSourceBoundary.allNineAudioMovesVerified === true &&
      preparation.totals.audioVerifiedMoves === 9,
    "accepted post-canary Batch 10 adjudication gate unavailable"
  );
  const sourcePaths = [
  ...
  for (const context of preparation.contexts) {
    ...
    debateInputs.push({
      ...
      adjudicationOutputAcceptedWithoutCorrection:
        result.route !== "retained-passed-shard-01-plus-correction-2-merge",
      adjudicationOutputPath,
  ...
  assertV4(
    debateInputs.map((input) => input.primaryA.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "loaded post-canary Batch 10 debate order invalid"
  );
