...
import { compileSidePartitionedSelectionMapInventory } from "./assessment-production-score-stability-v2-inventory-side-partitioned-selection-map.mjs";

export const V2_INVENTORY = Object.freeze({
  validationRoot:
    "docs/assessment-production/score-stability-v2-validation-cohort",
  root: "docs/assessment-production/score-stability-v2-validation-cohort/inventory-side-partitioned-selection-map-successor",
  preparation:
  ...
  manifest:
    "docs/assessment-production/score-stability-v2-validation-cohort/inventory-side-partitioned-selection-map-successor/execution-manifest.json",
  execution:
    "docs/assessment-production/score-stability-v2-validation-cohort/inventory-side-partitioned-selection-map-successor/model-execution.json",
  analysis:
    "docs/assessment-production/score-stability-v2-validation-cohort/inventory-side-partitioned-selection-map-successor/analysis.json",
  manifestStatus:
  ...
  executionPassedStatus:
    "ten-fresh-side-partitioned-selection-map-v2-validation-score-blind-inventory-successor-contexts-passed",
  executionFailedStatus:
    "v2-validation-score-blind-inventory-side-partitioned-selection-map-successor-complete-with-failure",
  analysisStatus:
    "ten-fresh-side-partitioned-selection-map-v2-validation-score-blind-inventory-successor-contexts-passed-independent-judgment-packet-preparation-authorized",
  contexts: 10,
  candidates: 406,
  proCandidates: 203,
  conCandidates: 203,
...
const PREDECESSOR_FAILED_EXECUTION =
  "docs/assessment-production/score-stability-v2-validation-cohort/inventory/model-execution.json";
const PREDECESSOR_TIMEOUT_DIAGNOSIS =
  "docs/assessment-production/score-stability-v2-validation-cohort/inventory-columnar-recovery/timeout-diagnosis.json";
const RECOVERY_FAILED_EXECUTION =
  "docs/assessment-production/score-stability-v2-validation-cohort/inventory-columnar-recovery/model-execution.json";
const RECOVERY_FAILURE_DIAGNOSIS =
  "docs/assessment-production/score-stability-v2-validation-cohort/inventory-columnar-recovery/failure-diagnosis.json";
const UNIQUE_SELECTION_MAP_FAILED_EXECUTION =
  "docs/assessment-production/score-stability-v2-validation-cohort/inventory-unique-selection-map-successor/model-execution.json";
const UNIQUE_SELECTION_MAP_FAILURE_DIAGNOSIS =
  "docs/assessment-production/score-stability-v2-validation-cohort/inventory-unique-selection-map-successor/failure-diagnosis.json";
const DEVELOPMENT_ANALYSIS =
  "docs/assessment-production/score-stability-v2-validation-cohort/inventory-side-partitioned-selection-map-development/development-analysis.json";
const SIDE_PARTITIONED_SELECTION_MAP_LIBRARY_PATH =
...
const PREREGISTER_PATH =
  "scripts/preregister-assessment-production-score-stability-v2-inventory-side-partitioned-selection-map-successor.mjs";
const RUNNER_PATH =
  "scripts/run-assessment-production-score-stability-v2-inventory-side-partitioned-selection-map-successor.mjs";
const ANALYZER_PATH =
  "scripts/analyze-assessment-production-score-stability-v2-inventory-side-partitioned-selection-map-successor.mjs";
const VALIDATOR_PATH =
  "scripts/validate-assessment-production-score-stability-v2-inventory-side-partitioned-selection-map-successor.mjs";
const TEST_PATH =
  "scripts/test-assessment-production-score-stability-v2-inventory-side-partitioned-selection-map-successor-gate.mjs";
const VALIDATOR_TEST_PATH =
  "scripts/test-assessment-production-score-stability-v2-inventory-side-partitioned-selection-map-successor-validator.mjs";
...
async function readPreparation() {
  ...
  assertV4(
    preparation.status ===
      ...
      preparation.selectionTopology?.everyCandidateKeyRequired === true &&
      preparation.selectionTopology?.candidateIdentityStructurallyUnique === true &&
      preparation.selectionTopology?.duplicateCandidateSelectionRepresentable ===
        false &&
      preparation.selectionTopology?.repositorySideStructurallyPartitioned ===
        true &&
      preparation.selectionTopology?.wrongSideCandidateKeyRepresentable ===
        false &&
      preparation.selectionTopology?.orderWithinSideModelAuthored === false &&
      preparation.selectionTopology
        ?.orderWithinSideRepositoryDerivedFromChronology === true &&
      preparation.selectionTopology?.positionCollisionRepresentable === false &&
      preparation.selectionTopology
        ?.sectionSideCardinalityDeterministicallyValidated === true &&
      preparation.selectionTopology?.unsupportedUniqueItemsUsed === false &&
      preparation.selectionTopology?.failedDebate31DuplicateRejected === true &&
      preparation.selectionTopology
        ?.failedDebate31CardinalityRejectedAfterOrderRemoval === true &&
      preparation.selectionTopology?.preservedRegressionArtifacts === 13 &&
      preparation.selectionTopology?.preservedLockedInventoriesIdentical === 13 &&
      ...
      preparation.failedGateDisposition?.columnarRecoveryGatePreservedFailed ===
        true &&
      preparation.failedGateDisposition
      ...
      preparation.isolation?.fullTenContextFreshExecutionRequired === true &&
      preparation.isolation?.allPredecessorOutputsUnavailable === true &&
...
export async function preregisterV2InventorySidePartitionedSelectionMapSuccessor({ shouldWrite, frozenAt }) {
  assertV4(
  ...
  const [
    ...
    retiredExecution,
    retiredAnalysis,
    predecessorFailedExecution,
    predecessorTimeoutDiagnosis,
    recoveryFailedExecution,
    recoveryFailureDiagnosis,
    uniqueSelectionMapFailedExecution,
    uniqueSelectionMapFailureDiagnosis,
    developmentAnalysis,
  ] = await Promise.all([
    readPreparation(),
    readFile(RETIRED_EXECUTION, "utf8").then(JSON.parse),
    readFile(RETIRED_ANALYSIS, "utf8").then(JSON.parse),
    readFile(PREDECESSOR_FAILED_EXECUTION, "utf8").then(JSON.parse),
    readFile(PREDECESSOR_TIMEOUT_DIAGNOSIS, "utf8").then(JSON.parse),
    readFile(RECOVERY_FAILED_EXECUTION, "utf8").then(JSON.parse),
    readFile(RECOVERY_FAILURE_DIAGNOSIS, "utf8").then(JSON.parse),
    readFile(UNIQUE_SELECTION_MAP_FAILED_EXECUTION, "utf8").then(JSON.parse),
    readFile(UNIQUE_SELECTION_MAP_FAILURE_DIAGNOSIS, "utf8").then(JSON.parse),
    readFile(DEVELOPMENT_ANALYSIS, "utf8").then(JSON.parse),
  ]);
  assertV4(
  ...
  assertV4(
    predecessorFailedExecution.status ===
        "v2-validation-score-blind-inventory-complete-with-failure" &&
      predecessorFailedExecution.validContexts === 9 &&
      predecessorFailedExecution.invalidContexts === 1 &&
      predecessorFailedExecution.retries === 0 &&
      predecessorFailedExecution.currentCanaryReclassified === false &&
      predecessorFailedExecution.proposedPolicyPromoted === false &&
      predecessorTimeoutDiagnosis.status ===
        "failed-inventory-gate-preserved-columnar-full-cohort-successor-preparation-authorized" &&
      predecessorTimeoutDiagnosis.requiredSuccessorDesign
        ?.fullTenContextFreshExecution ===
        true &&
      predecessorTimeoutDiagnosis.requiredSuccessorDesign
        ?.priorNineValidOutputsReusableForSuccessorAcceptance === false &&
      predecessorTimeoutDiagnosis.requiredSuccessorDesign
        ?.samePerContextTimeoutMs ===
        600000 &&
      predecessorTimeoutDiagnosis.authorization?.recoveryModelExecution ===
        false &&
      recoveryFailedExecution.status ===
        "v2-validation-score-blind-inventory-columnar-recovery-complete-with-failure" &&
      recoveryFailedExecution.contextsAttempted === 3 &&
      recoveryFailedExecution.validContexts === 2 &&
      recoveryFailedExecution.invalidContexts === 1 &&
      recoveryFailedExecution.retries === 0 &&
      recoveryFailureDiagnosis.status ===
        "recovery-inventory-gate-failed-cross-section-duplicate-confirmed-no-further-action-authorized" &&
      recoveryFailureDiagnosis.authorization?.retry === false &&
      recoveryFailureDiagnosis.authorization?.successorModelExecution === false &&
      uniqueSelectionMapFailedExecution.status ===
        "v2-validation-score-blind-inventory-unique-selection-map-successor-complete-with-failure" &&
      uniqueSelectionMapFailedExecution.contextsAttempted === 3 &&
      uniqueSelectionMapFailedExecution.validContexts === 2 &&
      uniqueSelectionMapFailedExecution.invalidContexts === 1 &&
      uniqueSelectionMapFailedExecution.retries === 0 &&
      uniqueSelectionMapFailureDiagnosis.status ===
        "unique-selection-map-successor-gate-failed-section-side-cardinality-confirmed-no-further-action-authorized" &&
      uniqueSelectionMapFailureDiagnosis.gateDisposition
        ?.uniqueSelectionSuccessorGatePreservedFailed === true &&
      uniqueSelectionMapFailureDiagnosis.authorization?.retry === false &&
      uniqueSelectionMapFailureDiagnosis.authorization?.successorModelExecution ===
        false &&
      developmentAnalysis.status ===
        "side-partitioned-order-free-map-retired-regression-passed-successor-preparation-authorized" &&
      developmentAnalysis.authorization?.successorPreparation === true &&
      developmentAnalysis.authorization?.successorModelExecution === false,
    "failed gates, diagnosis, or model-free successor development drifted"
  );
  for (const context of preparation.contexts) {
    const [transportBytes, schemaBytes, legacySchemaBytes] = await Promise.all([
    ...
    assertV4(
      transportBytes.equals(
        ...
        legacySchemaBytes.equals(
          Buffer.from(`${JSON.stringify(JSON.parse(legacySchemaBytes))}\n`)
        ) &&
        context.candidateSelectionProperties === context.candidates,
  ...
  const sourceFiles = [
    ...
    PREDECESSOR_FAILED_EXECUTION,
    PREDECESSOR_TIMEOUT_DIAGNOSIS,
    RECOVERY_FAILED_EXECUTION,
    RECOVERY_FAILURE_DIAGNOSIS,
    UNIQUE_SELECTION_MAP_FAILED_EXECUTION,
    UNIQUE_SELECTION_MAP_FAILURE_DIAGNOSIS,
    DEVELOPMENT_ANALYSIS,
    preparation.inputs.manual,
    preparation.inputs.columnarTransportGuide,
    preparation.inputs.sidePartitionedSelectionMapGuide,
    RETIRED_EXECUTION,
    RETIRED_ANALYSIS,
    ...
    SIDE_PARTITIONED_SELECTION_MAP_LIBRARY_PATH,
    LIBRARY_PATH,
    VALIDATOR_PATH,
    PREREGISTER_PATH,
    RUNNER_PATH,
  ...
  const manifest = {
    schemaVersion:
      "1.0-score-stability-v2-fresh-validation-side-partitioned-selection-map-score-blind-inventory-successor-execution-manifest",
    ...
    costEstimate: {
      ...
      expectedAggregateModelMinutes: [15, 35],
      expectedAggregateComputeHours: [0.25, 0.59],
      absoluteGateTimeoutMinutes: 60,
      estimateBasis:
        "The original v2 gate attempted all ten contexts in 17.84 wall-minutes and 24.31 aggregate model-minutes. This successor preserves the 1-to-2 ramp and frozen timeouts, uses the same lossless columnar evidence, and requires one nullable property for every candidate under a repository-owned side partition; the maximum copied input is 90,227 bytes.",
    },
    ...
    modelInputs: {
      ...
      sidePartitionedSelectionMapGuide: preparation.inputs.sidePartitionedSelectionMapGuide,
    },
    ...
    priorFailedGateEvidence: {
      predecessorTimeoutGate: {
        execution: PREDECESSOR_FAILED_EXECUTION,
        diagnosis: PREDECESSOR_TIMEOUT_DIAGNOSIS,
        status: predecessorFailedExecution.status,
        validContexts: predecessorFailedExecution.validContexts,
        invalidContexts: predecessorFailedExecution.invalidContexts,
        retries: predecessorFailedExecution.retries,
        preservedAsFailed: true,
      },
      columnarRecoveryGate: {
        execution: RECOVERY_FAILED_EXECUTION,
        diagnosis: RECOVERY_FAILURE_DIAGNOSIS,
        status: recoveryFailedExecution.status,
        validContexts: recoveryFailedExecution.validContexts,
        invalidContexts: recoveryFailedExecution.invalidContexts,
        retries: recoveryFailedExecution.retries,
        preservedAsFailed: true,
      },
      uniqueSelectionMapSuccessorGate: {
        execution: UNIQUE_SELECTION_MAP_FAILED_EXECUTION,
        diagnosis: UNIQUE_SELECTION_MAP_FAILURE_DIAGNOSIS,
        status: uniqueSelectionMapFailedExecution.status,
        validContexts: uniqueSelectionMapFailedExecution.validContexts,
        invalidContexts: uniqueSelectionMapFailedExecution.invalidContexts,
        retries: uniqueSelectionMapFailedExecution.retries,
        preservedAsFailed: true,
      },
      allThreeGatesPreservedAsFailed: true,
      priorValidOutputsReusableForSuccessorAcceptance: false,
      priorFailedOutputsReusableForSuccessorAcceptance: false,
    },
    isolation: {
    ...
    selectionTopology: structuredClone(preparation.selectionTopology),
    ...
    acceptance: {
      ...
      structurallyUniqueCandidateSelectionRequired: true,
...
export async function validateV2InventorySidePartitionedSelectionMapSuccessorProposal({
  proposalPath,
  preparationPath,
  debateNumber,
  shouldWrite,
  ...
  const summary = {
    schemaVersion:
      "1.0-score-stability-v2-fresh-validation-side-partitioned-selection-map-score-blind-inventory-successor-validation",
    protocolId: preparation.protocolId,
    status: "passed",
    developmentValidationOnly: true,
    productionCanary: false,
    ...
    sidePartitionedSelectionMapUsed: true,
    everyCandidateKeyRequired: true,
    ...
    positionCollisionRepresentable: false,
    sectionSideCardinalityDeterministicallyValidated: true,
    selectedCandidatesProjectedDeterministically: true,
    projectedProposalSha256: sha256(jsonBytes(compiled.projectedProposal)),
...
async function readAuthorizedManifest() {
  ...
  assertV4(
    manifest.status === V2_INVENTORY.manifestStatus &&
      ...
      manifest.authorization?.successorModelContexts === true &&
      ...
      manifest.isolation?.allPredecessorOutputsUnavailable === true &&
      manifest.isolation?.allPredecessorExecutionMetadataUnavailable === true &&
      manifest.modelInputs?.columnarTransportGuide ===
        "docs/assessment-production/score-stability-v2-validation-cohort/inventory-columnar-recovery/columnar-transport-guide.md" &&
      manifest.modelInputs?.sidePartitionedSelectionMapGuide ===
        "docs/assessment-production/score-stability-v2-validation-cohort/inventory-side-partitioned-selection-map-development/side-partitioned-selection-map-guide.md" &&
      manifest.selectionTopology?.everyCandidateKeyRequired === true &&
      manifest.selectionTopology?.duplicateCandidateSelectionRepresentable ===
        false &&
      manifest.selectionTopology?.repositorySideStructurallyPartitioned ===
        true &&
      manifest.selectionTopology?.wrongSideCandidateKeyRepresentable === false &&
      manifest.selectionTopology?.orderWithinSideModelAuthored === false &&
      manifest.selectionTopology
        ?.orderWithinSideRepositoryDerivedFromChronology === true &&
      manifest.selectionTopology?.positionCollisionRepresentable === false &&
      manifest.selectionTopology
        ?.sectionSideCardinalityDeterministicallyValidated === true &&
...
export async function runV2InventorySidePartitionedSelectionMapSuccessor() {
  ...
  async function executeContext(context, contextIndex) {
    ...
    const temporary = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-v2-validation-inventory-side-partitioned-selection-map-successor-${context.debateNumber}-`
      )
    );
    const isolatedCodexHome = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-v2-validation-inventory-side-partitioned-selection-map-successor-home-${context.debateNumber}-`
      )
    );
    ...
    try {
      for (const [source, target] of [
        ...
        [manifest.modelInputs.sidePartitionedSelectionMapGuide, "side-partitioned-selection-map-guide.md"],
      ...
      const prompt = `Read manual.md, columnar-transport-guide.md, side-partitioned-selection-map-guide.md, packet.json, candidate-evidence-bundle.json, and schema.json completely; read nothing else. Act only as the isolated score-blind inventory curator for fresh development-validation successor Debate ${context.debateNumber}. The candidate evidence uses the lossless columnar representation defined in columnar-transport-guide.md, and the response must use candidateSelectionsBySide.pro and candidateSelectionsBySide.con exactly as defined in side-partitioned-selection-map-guide.md. All ${context.candidates} candidate keys are required exactly once under their repository-owned side map: use null for every unselected candidate, and for every selected candidate provide only sectionId, moveId, moveKind, and a source-faithful proposition. Do not author side, orderWithinSide, or any position field; the enclosing map fixes side, and repository chronology fixes within-side order. Select 8 to 24 candidates total. Produce four to six weighted issue sections totaling exactly 100 percent, with one or two pro and one or two con selections in every section. Define one route per side and its burden bridges. A reply must have an earlier selected opposing move in source chronology, but do not name targets. Ratings, response topology, burden contact, adjustments, scores, winners, tags, Overall Commentary, AI Extension, score-policy analysis, prior execution information, and publication prose are prohibited. Return exactly one schema-conforming JSON object.`;
      process.stdout.write(
        `[v2-validation-inventory-side-partitioned-selection-map-successor] starting ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber}\n`
      );
    ...
    process.stdout.write(
      `[v2-validation-inventory-side-partitioned-selection-map-successor] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
    );
    return record;
  }
  ...
  const execution = {
    schemaVersion:
      "1.0-score-stability-v2-fresh-validation-side-partitioned-selection-map-score-blind-inventory-successor-model-execution",
    ...
    scoresDerived: 0,
    authorization: {
      deterministicPassingAnalysis:
        validContexts === manifest.contexts.length,
      failureDiagnosis: validContexts !== manifest.contexts.length,
      retry: false,
      semanticCorrection: false,
...
export async function analyzeV2InventorySidePartitionedSelectionMapSuccessor({ shouldWrite }) {
  const manifest = await readAuthorizedManifest();
  ...
  assertV4(
    execution.status === V2_INVENTORY.executionPassedStatus &&
      ...
      execution.authorization?.deterministicPassingAnalysis === true &&
      execution.authorization?.failureDiagnosis === false,
  ...
  for (const result of execution.results) {
    ...
    debates.push({
      ...
      sidePartitionedSelectionMapUsed: validation.sidePartitionedSelectionMapUsed,
      everyCandidateKeyRequired: validation.everyCandidateKeyRequired,
      candidateIdentityStructurallyUnique:
        validation.candidateIdentityStructurallyUnique,
      duplicateCandidateSelectionRepresentable:
        validation.duplicateCandidateSelectionRepresentable,
      repositorySideStructurallyPartitioned:
        validation.repositorySideStructurallyPartitioned,
      wrongSideCandidateKeyRepresentable:
        validation.wrongSideCandidateKeyRepresentable,
      modelAuthoredOrderAbsent: validation.modelAuthoredOrderAbsent,
      orderWithinSideRepositoryDerivedFromChronology:
        validation.orderWithinSideRepositoryDerivedFromChronology,
      positionCollisionRepresentable:
        validation.positionCollisionRepresentable,
      sectionSideCardinalityDeterministicallyValidated:
        validation.sectionSideCardinalityDeterministicallyValidated,
      selectedCandidatesProjectedDeterministically:
        validation.selectedCandidatesProjectedDeterministically,
  ...
  assertV4(
    debates.every(
      (debate) =>
        ...
        debate.sidePartitionedSelectionMapUsed &&
        debate.everyCandidateKeyRequired &&
        debate.candidateIdentityStructurallyUnique &&
        debate.duplicateCandidateSelectionRepresentable === false &&
        debate.repositorySideStructurallyPartitioned &&
        debate.wrongSideCandidateKeyRepresentable === false &&
        debate.modelAuthoredOrderAbsent &&
        debate.orderWithinSideRepositoryDerivedFromChronology &&
        debate.positionCollisionRepresentable === false &&
        debate.sectionSideCardinalityDeterministicallyValidated &&
        debate.selectedCandidatesProjectedDeterministically
    ),
  ...
  const analysis = {
    schemaVersion:
      "1.0-score-stability-v2-fresh-validation-side-partitioned-selection-map-score-blind-inventory-successor-analysis",
    protocolId: manifest.protocolId,
...
export async function testV2InventorySidePartitionedSelectionMapSuccessorGate() {
  ...
  assertV4(
    manifest.acceptance?.everyCandidateAvailableDuringSelection === true &&
      ...
      manifest.acceptance?.everyCandidateKeyRequired === true &&
      manifest.acceptance?.duplicateCandidateSelectionRepresentable === false &&
      manifest.acceptance?.structurallyUniqueCandidateSelectionRequired ===
        true &&
      manifest.acceptance?.repositorySideStructurallyPartitioned === true &&
      manifest.acceptance?.wrongSideCandidateKeyRepresentable === false &&
      manifest.acceptance?.modelAuthoredOrderAbsent === true &&
      manifest.acceptance
        ?.orderWithinSideRepositoryDerivedFromChronology === true &&
      manifest.acceptance?.positionCollisionRepresentable === false &&
      manifest.acceptance
        ?.sectionSideCardinalityDeterministicallyValidated === true &&
      manifest.transport?.parsedRoundTripIdentityVerified === true &&
      manifest.transport?.maximumCopiedInputBytes === 90227 &&
  ...
  assertV4(
    manifest.priorFailedGateEvidence?.allThreeGatesPreservedAsFailed === true &&
      manifest.priorFailedGateEvidence?.predecessorTimeoutGate
        ?.preservedAsFailed === true &&
      manifest.priorFailedGateEvidence?.columnarRecoveryGate
        ?.preservedAsFailed === true &&
      manifest.priorFailedGateEvidence?.uniqueSelectionMapSuccessorGate
        ?.preservedAsFailed === true &&
      manifest.priorFailedGateEvidence
        ?.priorValidOutputsReusableForSuccessorAcceptance === false &&
      manifest.priorFailedGateEvidence
      ...
      manifest.contexts.every(
        (context) =>
          context.modelCandidateTransport.startsWith(
            `${V2_INVENTORY.validationRoot}/inventory-columnar-recovery/candidate-transport/`
          ) &&
          context.schema.startsWith(
            `${V2_INVENTORY.validationRoot}/inventory-side-partitioned-selection-map-development/schemas/`
          ) &&
    ...
    "successor freshness, structural selection, or namespace isolation drifted"
  );
  for (const key of [
    "independentJudgmentPacketPreparation",
  ...
  if (!(await exists(manifest.artifacts.execution))) {
    for (const future of manifest.futureOutputPathsExcludedFromSourceHashes) {
      assertV4(
        !(await exists(future)) &&
          !Object.hasOwn(manifest.sourceHashes, future),
  ...
  assertV4(
    execution.contextsAttempted === V2_INVENTORY.contexts &&
      ...
      execution.authorization?.deterministicPassingAnalysis === true &&
      execution.authorization?.failureDiagnosis === false &&
      execution.authorization?.independentJudgmentPacketPreparation === true &&
      execution.rampPhases.length === 3 &&
