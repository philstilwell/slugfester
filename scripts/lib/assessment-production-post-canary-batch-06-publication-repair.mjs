...
} from "./assessment-production-post-canary-batch-06-publication-validation.mjs";
import { assertV4, canonicalJson } from "./v4-lean-production.mjs";

export const POST_CANARY_BATCH_06_PUBLICATION_REPAIR_ROOT =
  `${POST_CANARY_BATCH_06_PUBLICATION_ROOT}/repair-1`;
export const POST_CANARY_BATCH_06_PUBLICATION_REPAIR_PROTOCOL_ID =
  "assessment-production-post-canary-batch-06-publication-repair-1";
export const POST_CANARY_BATCH_06_PUBLICATION_REPAIR_PACKET_VERSION =
  "1.0-assessment-production-post-canary-batch-06-publication-repair-packet";
export const POST_CANARY_BATCH_06_PUBLICATION_REPAIR_OUTPUT_VERSION =
  "1.0-assessment-production-post-canary-batch-06-publication-repair-output";
...
export function parseBatch06PublicationRepairField(field) {
  ...
  throw new Error(`invalid Batch 6 publication repair field: ${field}`);
}

export function buildBatch06PublicationRepairSchema(packet) {
  return {
    $schema: "https://json-schema.org/draft/2020-12/schema",
    $id: `slugfester-batch-06-publication-repair-${packet.contextIndex}`,
    title: `Batch 6 publication repair ${packet.packetId}`,
    type: "object",
    ...
    properties: {
      schemaVersion: { type: "string", const: POST_CANARY_BATCH_06_PUBLICATION_REPAIR_OUTPUT_VERSION },
      protocolId: { type: "string", const: POST_CANARY_BATCH_06_PUBLICATION_REPAIR_PROTOCOL_ID },
      ...
      assessmentModel: { type: "string", const: POST_CANARY_BATCH_06_PUBLICATION_MODEL.label },
...
export function validateBatch06PublicationRepairOutput(output, packet) {
  exactKeys(output, ["schemaVersion", "protocolId", "contextIndex", "packetId",
    "debateNumber", "debateId", "assessmentModel", "completedAt", "correctedFields"],
  "repair output");
  assertV4(output.schemaVersion === POST_CANARY_BATCH_06_PUBLICATION_REPAIR_OUTPUT_VERSION &&
    output.protocolId === POST_CANARY_BATCH_06_PUBLICATION_REPAIR_PROTOCOL_ID &&
    ...
    output.assessmentModel === POST_CANARY_BATCH_06_PUBLICATION_MODEL.label &&
...
function setField(output, field, value) {
  const parsed = parseBatch06PublicationRepairField(field);
...
export function mergeAndValidateBatch06PublicationRepairs({ baseOutput, repairOutputs,
  ...
  for (let index = 0; index < repairPackets.length; index += 1) {
    ...
    validateBatch06PublicationRepairOutput(output, packet);
    for (const correction of packet.corrections) {
      const parsed = parseBatch06PublicationRepairField(correction.field);
  ...
  const fullValidation = validatePostCanaryBatch06PublicationOutput(merged, publicationPacket);
