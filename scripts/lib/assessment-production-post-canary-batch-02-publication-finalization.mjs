...
import {
  ...
  validatePostCanaryBatch02CompiledStagingRecord
} from "./assessment-production-post-canary-batch-02-publication-compilation.mjs";
import { assertV4, canonicalJson } from "./v4-lean-production.mjs";

export const POST_CANARY_BATCH_02_PUBLICATION_FINALIZATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-02/publication-finalization";
export const POST_CANARY_BATCH_02_PUBLICATION_FINALIZATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-02-publication-finalization";
export const POST_CANARY_BATCH_02_PUBLICATION_FINALIZATION_ORDER =
  POST_CANARY_BATCH_02_PUBLICATION_COMPILATION_ORDER;
...
export function buildPostCanaryBatch02PublicationFinalization({
  compiled,
  compiledPath,
  compiledSha256,
  output,
  ...
  assertV4(
    compiled.stagingAudit?.protocolId ===
      ...
      compiled.stagingAudit.batchNumber === 2 &&
    ...
    `${compiled.number}: Batch 2 compiled staging audit changed`
  );
  const candidate = stripStagingAudit(compiled);
  const provenance = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-02-publication-finalization-provenance",
    protocolId: POST_CANARY_BATCH_02_PUBLICATION_FINALIZATION_PROTOCOL_ID,
    status: "staging-only-batch-02-final-candidate-provenance",
    ...
    model: {
      label: POST_CANARY_BATCH_02_PUBLICATION_MODEL.label,
      slug: POST_CANARY_BATCH_02_PUBLICATION_MODEL.slug,
      reasoningEffort: POST_CANARY_BATCH_02_PUBLICATION_MODEL.reasoningEffort,
      authentication: POST_CANARY_BATCH_02_PUBLICATION_MODEL.authentication,
...
export function validatePostCanaryBatch02PublicationFinalCandidate({
  candidate,
  provenance,
  compiled,
  output,
  ...
  const expected = buildPostCanaryBatch02PublicationFinalization({
    compiled,
  ...
  assertV4(
    ...
    `${packet.debateNumber}: Batch 2 publication finalization differs from deterministic replay`
  );
  ...
  assertV4(
    ...
    `${packet.debateNumber}: Batch 2 final candidate invariants failed`
  );
  return {
    status: "passed",
...
export function buildPostCanaryBatch02PublicationStagingPreviewHtml() {
...
<html lang="en">
  <head>
    ...
    <title>Slugfester post-canary Batch 2 publication staging preview</title>
  ...
  <body>
    ...
    <script type="module">
      ...
      const debateNumber = params.get("debate") || "103";
      if (!allowedHosts.has(window.location.hostname)) {
        ...
        document.querySelector("#app").innerHTML = "<main><h1>Unknown Batch 2 publication-staging debate</h1></main>";
      } else {
        ...
        if (banner) banner.innerHTML = "<strong>Publication staging preview:</strong> validated post-canary Batch 2 candidate only. This scorecard remains excluded from production data and rankings pending rendering and mutation authorization.";
      }
    </script>
  </body>
</html>
