...
import { validatePostCanaryBatch17PublicationOutput } from
  "./assessment-production-post-canary-batch-17-publication-validation.mjs";
import { assertV4, canonicalJson } from "./v4-lean-production.mjs";

export const POST_CANARY_BATCH_17_PUBLICATION_COMPILATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-17/deterministic-publication-compilation";
export const POST_CANARY_BATCH_17_PUBLICATION_COMPILATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-17-deterministic-publication-compilation";
export const POST_CANARY_BATCH_17_PUBLICATION_COMPILATION_ORDER = Object.freeze(
  [...POST_CANARY_BATCH_17_PUBLICATION_DEBATES]);

export function compilePostCanaryBatch17PublicationStagingRecord({ output, packet, identity }) {
  validatePostCanaryBatch17PublicationOutput(output, packet);
  ...
  compiled.stagingAudit.protocolId = POST_CANARY_BATCH_17_PUBLICATION_COMPILATION_PROTOCOL_ID;
  compiled.stagingAudit.productionCanary = false;
  compiled.stagingAudit.batchNumber = 17;
  return compiled;
}

export function validatePostCanaryBatch17CompiledStagingRecord({ compiled, output, packet, identity }) {
  const expected = compilePostCanaryBatch17PublicationStagingRecord({ output, packet, identity });
