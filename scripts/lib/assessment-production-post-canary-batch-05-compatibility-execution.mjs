...
export const POST_CANARY_BATCH_05_COMPATIBILITY_ACTIVATION_STATUS =
...
export function buildPostCanaryBatch05ValidatorSource(baselineSource) {
  let source = baselineSource;

  const batch04Import = `import {
  ...
  source = replaceExactOnce(
    ...
    "Batch 5 adapter-version route"
  );

  const routeFunctions = `export function validatePostCanaryBatch05LedgerAdapterRouteLocks({
  debate,
  ledgerText,
  packetPath,
  packetText,
...
`;
  source = replaceExactOnce(
    ...
    "Batch 5 route functions"
  );

  const batch05Branch = `  if (
...
`;
  source = replaceExactOnce(
    source,
    "  let calculated;\n",
    `${batch05Branch}  let calculated;\n`,
    "Batch 5 ledger branch"
  );

  validatePostCanaryBatch05ValidatorSource(source);
  return source;
}

export function validatePostCanaryBatch05ValidatorSource(source) {
  const required = [
    "POST_CANARY_BATCH_05_COMPATIBILITY_ROOT",
    "POST_CANARY_BATCH_05_SITE_LEDGER_ADAPTER_VERSION",
    "validatePostCanaryBatch05SiteLedgerAdapter",
    ...
    "validatePostCanaryBatch05LedgerAdapterRoute",
    POST_CANARY_BATCH_05_COMPATIBILITY_ACTIVATION_STATUS,
    "Batch 5 production assessments require an AI Extension",
    "Batch 5 ledger validation failed"
  ];
  for (const text of required) {
    if (!source.includes(text)) {
      throw new Error(`proposed Batch 5 validator is missing ${text}`);
    }
  }
  if (
    ...
    throw new Error("proposed Batch 5 validator weakens an existing route");
  }
  return {
    status: "passed",
