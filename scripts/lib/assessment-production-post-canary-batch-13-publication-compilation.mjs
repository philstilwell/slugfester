...
import { validatePostCanaryBatch13PublicationOutput } from
  "./assessment-production-post-canary-batch-13-publication-validation.mjs";
import { assertV4, canonicalJson } from "./v4-lean-production.mjs";

export const POST_CANARY_BATCH_13_PUBLICATION_COMPILATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-13/deterministic-publication-compilation";
export const POST_CANARY_BATCH_13_PUBLICATION_COMPILATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-13-deterministic-publication-compilation";
export const POST_CANARY_BATCH_13_PUBLICATION_COMPILATION_ORDER = Object.freeze(
  [...POST_CANARY_BATCH_13_PUBLICATION_DEBATES]);

export function compilePostCanaryBatch13PublicationStagingRecord({ output, packet, identity }) {
  validatePostCanaryBatch13PublicationOutput(output, packet);
  ...
  compiled.stagingAudit.protocolId = POST_CANARY_BATCH_13_PUBLICATION_COMPILATION_PROTOCOL_ID;
  compiled.stagingAudit.productionCanary = false;
  compiled.stagingAudit.batchNumber = 12;
  return compiled;
}

export function validatePostCanaryBatch13CompiledStagingRecord({ compiled, output, packet, identity }) {
  const expected = compilePostCanaryBatch13PublicationStagingRecord({ output, packet, identity });
