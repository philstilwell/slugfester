...
import { makeV376DSchema, assert, canonicalJson } from "./v376d-burden-contact.mjs";

export const V376D_EXECUTION_MANIFEST = "docs/calibration/v3.7.6/case-disjoint-retired-burden-contact-test/execution-manifest.json";

export async function readV376DJson(root, file) {
  return JSON.parse(await readFile(path.resolve(root, file), "utf8"));
}

export function mappedV376DOption(mapping, reviewerPass, bundleId, optionId) {
...
export function adjudicationV376DOption(mapping, debateNumber, bundleId, optionId) {
...
export function compareV376DOutputs(mapping, outputA, outputB) {
  ...
  return outputA.bundles.map((choiceA) => {
    ...
    const passA = mappedV376DOption(mapping, "pass-a", choiceA.bundleId, choiceA.optionId).semanticTuple;
    const passB = mappedV376DOption(mapping, "pass-b", choiceB.bundleId, choiceB.optionId).semanticTuple;
...
export function makeV376DAdjudicationArtifacts(debateNumber, passAPacket, comparisons, mapping, rotationSeed = 0) {
  const sourceById = new Map(passAPacket.bundles.map((item) => [item.bundleId, item]));
  const map = { schemaVersion: "3.7.6-disjoint-adjudication-option-map", debateNumber, bundles: [] };
  const bundles = comparisons.filter((item) => !item.agreed).map((comparison, disputeIndex) => {
    ...
    const shift = (rotationSeed + disputeIndex + 1) % universe.length, ordered = [...universe.slice(shift), ...universe.slice(0, shift)];
    map.bundles.push({ bundleId: comparison.bundleId, options: ordered.map((item, index) => ({ optionId: `option-${index + 1}`, semanticTuple: item.semanticTuple, matchesProvisionalReference: item.matchesProvisionalReference })) });
    return { ...source, candidates: ordered.map((item, index) => ({ optionId: `option-${index + 1}`, values: item.values })) };
  });
  const packet = { schemaVersion: "3.7.6-disjoint-burden-contact-packet", debateNumber, reviewerPass: "pass-c", allSpeakerAttributionConfidenceHigh: passAPacket.allSpeakerAttributionConfidenceHigh, bundles };
  return { packet, schema: makeV376DSchema(packet), map };
}

export function v376dSemanticWinner(votes) {
  const counts = [];
  for (const value of votes.filter((item) => item !== null && item !== undefined)) {
    ...
    if (found) found.votes += 1; else counts.push({ key, value, votes: 1 });
  }
  counts.sort((left, right) => right.votes - left.votes || left.key.localeCompare(right.key));
  return counts[0]?.votes >= 2 ? counts[0] : null;
}

export function matchesV376DProvisionalReference(mapping, bundleId, semanticTuple) {
  ...
  return option.matchesProvisionalReference;
}
