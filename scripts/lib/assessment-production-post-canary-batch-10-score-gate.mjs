...
export const POST_CANARY_BATCH_10_SCORE_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-10/score-pass";
export const POST_CANARY_BATCH_10_CALCULATED_SCORES_VERSION =
  "1.0-assessment-production-post-canary-batch-10-adjudicated-calculated-scores";
export const POST_CANARY_BATCH_10_SCORE_STABILITY_THRESHOLDS =
  ACTIVE_SCORE_STABILITY_THRESHOLDS;
...
export function evaluatePostCanaryBatch10ScoreStability(
  debates,
  thresholds = POST_CANARY_BATCH_10_SCORE_STABILITY_THRESHOLDS
) {
  assertV4(
    Array.isArray(debates) && debates.length === 10,
    "ten scored post-canary Batch 10 debates required"
  );
  assertV4(
    canonicalJson(thresholds) ===
      canonicalJson(POST_CANARY_BATCH_10_SCORE_STABILITY_THRESHOLDS),
    "post-canary Batch 10 active thresholds changed"
  );
...
export function derivePostCanaryBatch10Scores(
  ledger,
  debateInputs,
  ...
  assertV4(
    ...
    "validated score-free post-canary Batch 10 final ledger required for scoring"
  );
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === 10,
    "ten post-canary Batch 10 score inputs required"
  );
  ...
  const debates = debateInputs.map((input) => {
    ...
    assertV4(
      finalLedgerDebate,
      `${debateNumber}: Batch 10 final-ledger debate missing`
    );
  ...
  const stability = evaluatePostCanaryBatch10ScoreStability(debates);
  ...
  return {
    schemaVersion: POST_CANARY_BATCH_10_CALCULATED_SCORES_VERSION,
    ...
    status: stability.acceptancePassed
      ...
      : "post-canary-batch-10-single-score-pass-stability-gate-failed",
...
export function validatePostCanaryBatch10Scores(
  scores,
  ledger,
  debateInputs,
  productionReferences,
  ...
  const expected = derivePostCanaryBatch10Scores(
    ledger,
    debateInputs,
    productionReferences,
    hashes
  ...
  assertV4(
    ...
    "post-canary Batch 10 calculated scores differ from deterministic replay"
  );
  return {
    status: "passed",
