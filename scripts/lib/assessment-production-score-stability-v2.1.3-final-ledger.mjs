...
import { extractAssessmentProductionScoreStabilityV213Disagreements } from "./assessment-production-score-stability-v2.1.3-disagreement.mjs";
import {
  buildV213DisputeAdjudicationPacket,
  validateV213DisputeAdjudicationOutput,
} from "./assessment-production-score-stability-v2.1.3-dispute-adjudication.mjs";

export const V213_FINAL_LEDGER_ROOT =
  "docs/assessment-production/score-stability-v2.1.3-validation-cohort/final-ledger";
export const V213_FINAL_LEDGER_PROTOCOL_ID =
  "assessment-production-score-stability-v2.1.3-adjudicated-consensus";
export const V213_FINAL_LEDGER_VERSION =
  "1.0-score-stability-v2.1.3-adjudicated-raw-ledger";
...
function validateInputReplay(input) {
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: stored disagreements differ from deterministic v2.1.3 replay`
  );
  const rebuiltAdjudication = buildV213DisputeAdjudicationPacket(
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication packet differs from deterministic v2.1.3 replay`
  );
  assertV4(
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication provenance differs from deterministic v2.1.3 replay`
  );
  validateV213DisputeAdjudicationOutput(
    adjudicationOutput,
    adjudicationPacket
...
export function compileV213FinalLedgerDebate(input) {
  const {
    primaryA,
    primaryB,
    adjudicationPacket,
...
export function buildV213FinalLedger(debateInputs, sourceHashes) {
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === EXPECTED_DEBATES.length,
    "ten v2.1.3 final-ledger debate inputs required"
  );
  const debates = debateInputs.map(compileV213FinalLedgerDebate);
  assertV4(
    debates.map((debate) => debate.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "v2.1.3 final-ledger debate order invalid"
  );
  ...
  assertV4(
    aggregate.disputedMoves === 176,
    "v2.1.3 disputed-move population mismatch"
  );
  assertV4(
    candidateSelections === 543,
    "v2.1.3 candidate-selection population mismatch"
  );
  assertV4(
    aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed === 396,
    "v2.1.3 rounded-mean population mismatch"
  );
  assertV4(
    aggregate.audioVerifiedMoves === 5,
    "v2.1.3 audio-verification population mismatch"
  );
  return {
    schemaVersion: V213_FINAL_LEDGER_VERSION,
    protocolId: V213_FINAL_LEDGER_PROTOCOL_ID,
    status: "passed-v2.1.3-deterministic-final-ledger-assembly",
    productionCanary: false,
    stagingOnly: true,
    developmentValidationOnly: true,
    AIOnly: true,
    scoringInputSchema:
      "v4.2.20-source-span-primary-with-v2.1.3-adjudicated-provenance",
...
export function validateV213FinalLedger(ledger, debateInputs, sourceHashes) {
  const expected = buildV213FinalLedger(debateInputs, sourceHashes);
  assertV4(
    canonicalJson(ledger) === canonicalJson(expected),
    "v2.1.3 final ledger differs from deterministic replay"
  );
  assertV4(
    !containsProhibitedCalculatedField(
      ledger.debates.map((debate) => debate.finalJudgment)
    ),
    "v2.1.3 final ledger contains a prohibited calculated field"
  );
  return {
    status: "passed",
...
function buildCombinedAudioAudit(audioAudit, audioAttributionAnalysis) {
  ...
  const originalUnresolved = combined.debates
    ...
    .find(
      (move) =>
        move.debateNumber === "78" && move.moveId === decision.moveId
    );
  assertV4(
    decision.status === "verified" &&
      decision.moveId === "con-uncertain-single-catholic-lineage" &&
      originalUnresolved?.status === "unresolved",
    "v2.1.3 combined audio resolution unavailable"
  );
  const debate = combined.debates.find(
    (item) => item.debateNumber === "78"
  );
  ...
  combined.status = "passed-all-five-v2.1.3-audio-moves-verified";
  combined.totals.verified = 5;
  combined.totals.unresolved = 0;
  return combined;
}

export async function loadV213FinalLedgerInputs() {
  ...
  assertV4(
    analysis.status === "v2.1.3-dispute-only-adjudication-gate-passed" &&
      analysis.authorization.finalLedgerAssembly &&
      !analysis.authorization.scoreDerivation,
    "v2.1.3 final-ledger assembly is not authorized"
  );
  assertV4(
    execution.status ===
      "ten-isolated-v2.1.3-dispute-only-adjudication-contexts-passed" &&
      execution.validContexts === 10 &&
      execution.retries === 0 &&
      execution.corrections === 0 &&
      execution.scoresDerived === 0,
    "accepted v2.1.3 adjudication execution unavailable"
  );
  assertV4(
    audioAnalysis.status === "v2.1.3-audio-verification-unresolved" &&
  ...
  assertV4(
    debateInputs.map((input) => input.primaryA.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "loaded v2.1.3 debate order invalid"
  );
