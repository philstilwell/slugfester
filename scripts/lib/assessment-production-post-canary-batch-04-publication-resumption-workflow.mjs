...
import {
  buildPostCanaryBatch04PublicationSchema,
...
import {
  validatePostCanaryBatch04PublicationOutput
} from "./assessment-production-post-canary-batch-04-publication-validation.mjs";
import {
  POST_CANARY_BATCH_04_PUBLICATION_RESUMPTION_DEBATES,
  POST_CANARY_BATCH_04_PUBLICATION_RESUMPTION_PROTOCOL_ID,
  POST_CANARY_BATCH_04_PUBLICATION_RESUMPTION_ROOT
} from "./assessment-production-post-canary-batch-04-publication-resumption.mjs";
import {
  POST_CANARY_BATCH_04_STANDING_AUTHORIZATION,
  loadAndValidatePostCanaryBatch04StandingAuthorization
} from "./assessment-production-post-canary-batch-04-standing-authorization.mjs";
import { assertV4, canonicalJson } from "./v4-lean-production.mjs";

const ROOT = POST_CANARY_BATCH_04_PUBLICATION_ROOT;
const RESUMPTION_ROOT = POST_CANARY_BATCH_04_PUBLICATION_RESUMPTION_ROOT;
...
const REPAIRED_DEBATE_127 = `${ROOT}/repair-1/merged/debate-127.json`;
const DEBATE_127_PACKET = `${ROOT}/packets/debate-127.json`;
...
const staticSources = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-04-publication-resumption.mjs",
  "scripts/lib/assessment-production-post-canary-batch-04-publication-resumption-workflow.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-04-publication-resumption.mjs",
  "scripts/test-assessment-production-post-canary-batch-04-publication-resumption-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-04-publication-resumption.mjs",
  "scripts/run-assessment-production-post-canary-batch-04-publication-resumption.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-04-publication-resumption.mjs"
];
...
function assertOriginalAndRepairBoundary(value) {
  ...
  assertV4(
    originalPreparation.status ===
      "frozen-ten-post-canary-batch-04-score-locked-publication-contexts-prepared-not-activated" &&
    ...
    "the original Batch 4 publication preparation changed"
  );
  assertV4(
    originalExecution.status ===
      ...
      originalAnalysis.status ===
        "post-canary-batch-04-publication-output-gate-failed" &&
      diagnosis.status ===
      ...
      canonicalJson(diagnosis.rampDisposition.unattemptedDebates) ===
        canonicalJson(POST_CANARY_BATCH_04_PUBLICATION_RESUMPTION_DEBATES),
    "the preserved nine-context Batch 4 publication boundary changed"
  );
  assertV4(
    repairPreparation.status ===
      "frozen-two-isolated-three-field-batch-04-debate-127-publication-repair-contexts-prepared-under-standing-authorization" &&
      repairActivation.status ===
        "frozen-two-isolated-three-field-batch-04-debate-127-publication-repair-contexts-authorized-under-standing-authorization" &&
      repairExecution.status ===
        "batch-04-debate-127-two-context-publication-repair-gate-passed" &&
      repairExecution.contextsAttempted === 2 &&
      repairExecution.validContexts === 2 &&
      repairExecution.attempts === 2 &&
      repairExecution.retries === 0 &&
      repairAnalysis.status ===
        "batch-04-debate-127-bounded-repair-and-complete-publication-validation-passed" &&
      ...
      repairValidation.validationSummary?.moves === 23 &&
      repairValidation.authorizedFieldsChanged === 3 &&
      repairValidation.immutableFieldsChanged === 0 &&
      repairValidation.lockedScoresUnchanged === true &&
      repairMergeAudit.status === "passed" &&
      repairMergeAudit.authorizedFieldsChanged === 3 &&
      repairMergeAudit.immutableFieldsChanged === 0,
    "the accepted Debate 127 repair boundary changed"
  );
}

export async function prepareBatch04PublicationResumption({ frozenAt, write }) {
  ...
  const originalPreparation = value(ORIGINAL_PREPARATION);
  const repaired = value(REPAIRED_DEBATE_127);
  const repairedPacket = value(DEBATE_127_PACKET);
  const repairedValidation = value(REPAIR_VALIDATION);
  assertV4(
    sha256(loaded[REPAIRED_DEBATE_127].bytes) ===
      repairedValidation.mergedOutputSha256 &&
      validatePostCanaryBatch04PublicationOutput(repaired, repairedPacket).status ===
        "passed",
    "the accepted Debate 127 output failed replay"
  );
  ...
  const contexts = [];
  for (let index = 0; index < POST_CANARY_BATCH_04_PUBLICATION_RESUMPTION_DEBATES.length; index += 1) {
    const debateNumber = POST_CANARY_BATCH_04_PUBLICATION_RESUMPTION_DEBATES[index];
    const source = originalPreparation.contexts.find(
      (context) => context.debateNumber === debateNumber
    );
    assertV4(source?.contextIndex === index + 1,
  ...
  assertV4(
    contexts.length === 9 && resumptionMoves === 180 &&
      resumptionSections === 47 && resumptionAudioVerifiedMoves === 4,
    "the Batch 4 nine-context coverage changed"
  );
  const futureOutputs = [
  ...
  for (const file of [
    ...
    REPAIR_VALIDATION, REPAIR_MERGE_AUDIT, REPAIRED_DEBATE_127,
    DEBATE_127_PACKET, POST_CANARY_BATCH_04_STANDING_AUTHORIZATION,
    ...staticSources
  ]) {
  ...
  const rampPhases = [
    ...
    { phase: "resumption-steady-two", maximumParallelContexts: 2,
      contextIndexes: [3, 4, 5, 6, 7, 8], expansionRequiresAllValid: false }
  ];
  const stopRules = Object.fromEntries([
    "acceptedDebate127RepairFailureBlocks", "sourceHashMismatchBlocks",
  ...
  const manifest = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-04-publication-resumption-execution-preparation-manifest",
    protocolId: POST_CANARY_BATCH_04_PUBLICATION_RESUMPTION_PROTOCOL_ID,
    status: "frozen-nine-untouched-post-canary-batch-04-publication-resumption-contexts-prepared-under-standing-authorization",
    frozenAt,
    ...
    inputs: {
      ...
      repairedDebate127: REPAIRED_DEBATE_127,
      debate127Packet: DEBATE_127_PACKET,
      standingAuthorization: POST_CANARY_BATCH_04_STANDING_AUTHORIZATION
    },
    ...
    acceptedDebate127: {
      ...
      packet: DEBATE_127_PACKET,
      output: REPAIRED_DEBATE_127,
    ...
    isolation: {
      ...
      repairedDebate127OutputUnavailableToResumptionModels: true,
      failedOriginalDebate127OutputUnavailable: true,
    ...
    deterministicValidation: {
      ...
      acceptedDebate127RepairReplayedAtFreeze: true,
    ...
    acceptanceContract: {
      ...
      resumptionMovesRequired: 180,
      cohortMovesRequired: 203,
      resumptionCritiquesRequired: 180,
      cohortCritiquesRequired: 203,
    ...
    nextAuthorizedAction:
      "activate-and-execute-exactly-nine-frozen-batch-04-publication-resumption-contexts-under-standing-authorization"
  };
  if (write) {
...
export async function activateBatch04PublicationResumption({ activatedAt, write }) {
  ...
  assertV4(
    preparation.protocolId === POST_CANARY_BATCH_04_PUBLICATION_RESUMPTION_PROTOCOL_ID &&
      preparation.status ===
        "frozen-nine-untouched-post-canary-batch-04-publication-resumption-contexts-prepared-under-standing-authorization" &&
      preparation.batchNumber === 4 && preparation.contexts?.length === 9 &&
      preparation.totals?.resumptionMoves === 180 && preparation.totals?.cohortMoves === 203 &&
    ...
    "the Batch 4 publication resumption is not prepared"
  );
  ...
  const activation = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-04-publication-resumption-execution-activation",
    protocolId: preparation.protocolId,
    status: "frozen-nine-untouched-post-canary-batch-04-publication-resumption-contexts-authorized-under-standing-authorization",
    activatedAt,
    checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(),
    productionCanary: false,
    batchNumber: 4,
    ...
    acceptedDebate127: structuredClone(preparation.acceptedDebate127),
    ...
    nextRequiredAction: "execute-the-nine-frozen-batch-04-publication-resumption-contexts-once"
  };
...
export async function runBatch04PublicationResumption() {
  ...
  assertV4(
    activation.status ===
      ...
      activation.batchNumber === 4 && activation.contexts?.length === 9 &&
    ...
    "the Batch 4 publication resumption is not authorized"
  );
  ...
  async function runContext(context) {
    const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-04-publication-resumption-${context.debateNumber}-`));
    const codexHome = await mkdtemp(path.join(os.tmpdir(), `batch-04-publication-resumption-home-${context.debateNumber}-`));
    ...
    try {
      ...
      process.stdout.write(`[batch-04-publication-resumption] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber}\n`);
      ...
      if (invocation.error || invocation.timedOut || invocation.code !== 0 || invocation.signal !== null || !resultExists) {
        ...
        try {
          validationSummary = validatePostCanaryBatch04PublicationOutput(
        ...
        const validationRecord = {
          schemaVersion: "1.0-assessment-production-post-canary-batch-04-publication-resumption-validation",
        ...
        const provenanceRecord = {
          schemaVersion: "1.0-assessment-production-post-canary-batch-04-publication-resumption-provenance",
    ...
    process.stdout.write(`[batch-04-publication-resumption] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
    return record;
  }
  ...
  const execution = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-04-publication-resumption-model-execution",
    protocolId: activation.protocolId,
    status: passed ? "nine-post-canary-batch-04-publication-resumption-contexts-passed" :
      "post-canary-batch-04-publication-resumption-complete-with-failure",
    gateStartedAt, gateCompletedAt: new Date().toISOString(), contextsPlanned: 9,
    ...
    ownDebateScoresImmutable: true, acceptedDebate127RepairContexts: 2,
    acceptedDebate127RepairedFields: 3, meteredApiCostUsd: 0,
...
export async function analyzeBatch04PublicationResumption({ write }) {
  ...
  assertV4(
    activation.status ===
      ...
      execution.contextsAttempted <= 9 && execution.attempts === execution.contextsAttempted &&
      ...
      activation.userAuthorization?.standingAuthorizationSha256 === standing.sha256,
    "the Batch 4 publication resumption analysis boundary changed"
  );
  for (const [file, digest] of Object.entries(activation.sourceHashes)) {
    assertV4(sha256(await readFile(path.resolve(file))) === digest,
      `resumption analysis source hash mismatch: ${file}`);
  }
  const [acceptedOutputBytes, acceptedPacketBytes, acceptedRecord] = await Promise.all([
    readFile(path.resolve(activation.acceptedDebate127.output)),
    readFile(path.resolve(activation.acceptedDebate127.packet)),
    readFile(path.resolve(activation.acceptedDebate127.validation), "utf8").then(JSON.parse)
  ]);
  assertV4(
    sha256(acceptedOutputBytes) === acceptedRecord.mergedOutputSha256 &&
      acceptedRecord.status === "passed" && acceptedRecord.authorizedFieldsChanged === 3 &&
      acceptedRecord.immutableFieldsChanged === 0 && acceptedRecord.lockedScoresUnchanged === true,
    "the accepted Debate 127 repair record changed"
  );
  const acceptedValidation = validatePostCanaryBatch04PublicationOutput(
    JSON.parse(acceptedOutputBytes), JSON.parse(acceptedPacketBytes)
  );
  const replayed = [];
  for (const result of execution.results) {
    ...
    const validation = validatePostCanaryBatch04PublicationOutput(JSON.parse(outputBytes), JSON.parse(packetBytes));
  ...
  const resumptionSemanticPass =
    execution.status === "nine-post-canary-batch-04-publication-resumption-contexts-passed" &&
    execution.contextsAttempted === 9 && execution.contextsUnattempted === 0 &&
    execution.validContexts === 9 && execution.invalidContexts === 0 && valid.length === 9 &&
  ...
  const cohort = [{ debateNumber: "127", validation: acceptedValidation }, ...valid];
  ...
  const analysis = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-04-publication-resumption-analysis",
    protocolId: activation.protocolId,
    status: passed ? "post-canary-batch-04-publication-resumption-output-gate-passed" :
      cohortSemanticPass ? "post-canary-batch-04-publication-resumption-failed-timing" :
    ...
    sources: { preparation: MANIFEST, preparationSha256: sha256(preparationBytes),
      ...
      acceptedDebate127: activation.acceptedDebate127.output },
    acceptedDebate127: { status: "passed-repaired-and-complete-publication-validation",
      validation: acceptedValidation, repairContexts: 2, repairedFields: 3,
      immutableFieldsChanged: 0, lockedScoresUnchanged: true },
    ...
    gate: { resumptionSemanticPass, cohortSemanticPass, timingPass,
      ...
      modelAuthoredScores: 0, scorePassesExecutedThisStage: 0 },
    totals: { acceptedRepairedDebates: 1, acceptedRepairContexts: 2,
      repairedFields: 3, resumptionModelContexts: execution.contextsAttempted,
    ...
    integrity: { participantJudgmentWasScoreBlind: true, scoresRemainedImmutable: true,
      acceptedDebate127RepairReplayed: true,
...
export async function testBatch04PublicationResumptionPreparation() {
  const manifest = JSON.parse(await readFile(MANIFEST, "utf8"));
  assert.equal(manifest.status,
    "frozen-nine-untouched-post-canary-batch-04-publication-resumption-contexts-prepared-under-standing-authorization");
  assert.equal(manifest.protocolId, POST_CANARY_BATCH_04_PUBLICATION_RESUMPTION_PROTOCOL_ID);
  ...
  assert.deepEqual(manifest.contexts.map((context) => context.debateNumber),
    POST_CANARY_BATCH_04_PUBLICATION_RESUMPTION_DEBATES);
  assert.deepEqual(manifest.contexts.map((context) => context.contextIndex), [0,1,2,3,4,5,6,7,8]);
  assert.deepEqual(manifest.contexts.map((context) => context.originalContextIndex), [1,2,3,4,5,6,7,8,9]);
  ...
  assert.equal(manifest.totals.acceptedMoves, 23);
  assert.equal(manifest.totals.resumptionMoves, 180);
  assert.equal(manifest.totals.resumptionSections, 47);
  assert.equal(manifest.totals.resumptionAudioVerifiedMoves, 4);
  ...
  for (const context of manifest.contexts) {
    ...
    assert.deepEqual(JSON.parse(schemaBytes), buildPostCanaryBatch04PublicationSchema(packet));
  ...
  assert.equal(moves, 180); assert.equal(sections, 47); assert.equal(audioVerifiedMoves, 4);
  const standing = await loadAndValidatePostCanaryBatch04StandingAuthorization();
  ...
  const [repairedBytes, packetBytes, record] = await Promise.all([
    readFile(manifest.acceptedDebate127.output), readFile(manifest.acceptedDebate127.packet),
    readFile(manifest.acceptedDebate127.validation, "utf8").then(JSON.parse)
  ]);
  const validation = validatePostCanaryBatch04PublicationOutput(JSON.parse(repairedBytes), JSON.parse(packetBytes));
  assert.equal(validation.status, "passed"); assert.equal(validation.moves, 23);
  assert.equal(validation.lockedScoresUnchanged, true);
  assert.equal(sha256(repairedBytes), record.mergedOutputSha256);
  return { status: "passed", acceptedDebates: 1, resumptionContexts: 9,
    resumptionMoves: 180, cohortDebates: 10, cohortMoves: 203,
    existingPacketsReused: 9, packetsGenerated: 0, modelContextsExecuted: 0,
