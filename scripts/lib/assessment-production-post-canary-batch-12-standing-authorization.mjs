...
export const POST_CANARY_BATCH_12_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-12";
export const POST_CANARY_BATCH_12_STANDING_AUTHORIZATION =
  `${POST_CANARY_BATCH_12_ROOT}/standing-authorization.json`;
export const POST_CANARY_BATCH_12_STANDING_AUTHORIZATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-12-standing-authorization";
export const POST_CANARY_BATCH_12_STANDING_AUTHORIZATION_STATUS =
  "frozen-active-batch-12-complete-remaining-workflow-standing-authorization";
export const POST_CANARY_BATCH_12_STANDING_AUTHORIZATION_INSTRUCTION_PATH =
  POST_CANARY_BATCH_12_ROOT + "/standing-authorization-instruction.txt";
export const POST_CANARY_BATCH_12_STANDING_AUTHORIZATION_INSTRUCTION = readFileSync(
  path.resolve(POST_CANARY_BATCH_12_STANDING_AUTHORIZATION_INSTRUCTION_PATH),
  "utf8"
).trimEnd();

const sha256 = (value) => createHash("sha256").update(value).digest("hex");

export function validatePostCanaryBatch12StandingAuthorization(record) {
  assertV4(
    record?.schemaVersion ===
      ...
      record.status === POST_CANARY_BATCH_12_STANDING_AUTHORIZATION_STATUS &&
      record.productionCanary === false &&
      record.batchNumber === 12 &&
      record.sourcePacketCommit === "217343109d51642a35516bef7911f3487e867899" &&
      ...
      record.userAuthorization?.resolvedAntecedent ===
        "The attached Batch 12 request authorizes deterministic selection and the complete reassessment, recovery, publication, validation, commit, and push workflow." &&
      record.userAuthorization?.scopeInterpretation ===
        "Create and use a complete-workflow Batch 12 standing authorization for the ten frozen selections, beginning with the separately frozen score-blind source packets, including bounded diagnosed recovery and production publication, while preserving the established campaign controls and stopping before Batch 13 selection." &&
    ...
    "Batch 12 standing-authorization identity changed"
  );
  assertV4(
    canonicalJson(record.selectedDebates) ===
      canonicalJson(["152", "28", "119", "116", "131", "07", "33", "15", "29", "11"]),
    "Batch 12 standing-authorization debate boundary changed"
  );
  assertV4(
    ...
    "Batch 12 standing-authorization model changed"
  );
  assertV4(
    ...
    "Batch 12 standing-authorization execution controls changed"
  );
  assertV4(
  ...
  assertV4(
    ...
    "Batch 12 standing-authorization cost boundary changed"
  );
  assertV4(
    ...
    "Batch 12 standing-authorization source-recovery controls changed"
  );
  assertV4(
    Object.values(record.stopRules ?? {}).every(Boolean),
    "Batch 12 standing-authorization stop rule disabled"
  );
  assertV4(
    ...
    "Batch 12 standing-authorization recovery controls changed"
  );
  assertV4(
    ...
    "Batch 12 standing-authorization Git controls changed"
  );
  return record;
}

export async function loadAndValidatePostCanaryBatch12StandingAuthorization() {
  const bytes = await readFile(
    path.resolve(POST_CANARY_BATCH_12_STANDING_AUTHORIZATION)
  );
  const record = validatePostCanaryBatch12StandingAuthorization(JSON.parse(bytes));
