...
import { extractAssessmentProductionPostCanaryBatch03Disagreements } from "./assessment-production-post-canary-batch-03-disagreement.mjs";
import {
  buildPostCanaryBatch03DisputeAdjudicationPacket,
  validatePostCanaryBatch03DisputeAdjudicationOutput
} from "./assessment-production-post-canary-batch-03-dispute-adjudication.mjs";

export const POST_CANARY_BATCH_03_FINAL_LEDGER_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-03/final-ledger";
export const POST_CANARY_BATCH_03_FINAL_LEDGER_PROTOCOL_ID =
  "assessment-production-post-canary-batch-03-adjudicated-consensus";
export const POST_CANARY_BATCH_03_FINAL_LEDGER_VERSION =
  "1.0-assessment-production-post-canary-batch-03-adjudicated-raw-ledger";
...
function validateInputReplay(input) {
  ...
  const replayedDisagreements =
    extractAssessmentProductionPostCanaryBatch03Disagreements(
      primaryA,
      primaryB,
      lockedInventory
    );
  assertV4(
    ...
    `${primaryA.debateNumber}: stored disagreements differ from deterministic Batch 3 replay`
  );
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication packet differs from deterministic Batch 3 replay`
  );
  assertV4(
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication provenance differs from deterministic Batch 3 replay`
  );
  validatePostCanaryBatch03DisputeAdjudicationOutput(
    adjudicationOutput,
    adjudicationPacket
...
export function compilePostCanaryBatch03FinalLedgerDebate(input) {
  const {
    primaryA,
    primaryB,
    adjudicationPacket,
...
export function buildPostCanaryBatch03FinalLedger(debateInputs, sourceHashes) {
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === EXPECTED_DEBATES.length,
    "ten post-canary Batch 3 debate inputs required"
  );
  const debates = debateInputs.map(compilePostCanaryBatch03FinalLedgerDebate);
  assertV4(
    debates.map((debate) => debate.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "post-canary Batch 3 final-ledger debate order invalid"
  );
  ...
  assertV4(aggregate.finalMoves === 200, "Batch 3 final-move population mismatch");
  assertV4(aggregate.disputedMoves === 190, "Batch 3 disputed-move population mismatch");
  assertV4(candidateSelections === 586, "Batch 3 candidate-selection population mismatch");
  assertV4(
    aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed === 419,
    "Batch 3 rounded-mean population mismatch"
  );
  assertV4(aggregate.audioVerifiedMoves === 8, "Batch 3 audio-verification population mismatch");
  return {
    schemaVersion: POST_CANARY_BATCH_03_FINAL_LEDGER_VERSION,
    protocolId: POST_CANARY_BATCH_03_FINAL_LEDGER_PROTOCOL_ID,
    status: "passed-post-canary-batch-03-deterministic-final-ledger-assembly",
    ...
    AIOnly: true,
    scoringInputSchema:
      "v4.2.20-source-span-primary-with-post-canary-batch-03-adjudicated-provenance",
    ...
    nextAuthorizedAction:
      "standing-authorization-permits-batch-03-single-deterministic-score-pass-preparation"
  };
}

export function validatePostCanaryBatch03FinalLedger(
  ledger,
  debateInputs,
  sourceHashes
) {
  const expected = buildPostCanaryBatch03FinalLedger(debateInputs, sourceHashes);
  assertV4(
    canonicalJson(ledger) === canonicalJson(expected),
    "post-canary Batch 3 final ledger differs from deterministic replay"
  );
  assertV4(
    !containsProhibitedCalculatedField(
      ledger.debates.map((debate) => debate.finalJudgment)
    ),
    "post-canary Batch 3 final ledger contains a prohibited calculated field"
  );
  return {
    status: "passed",
...
export async function loadPostCanaryBatch03FinalLedgerInputs() {
  const adjudicationAnalysisPath =
    `${ADJUDICATION_ROOT}/failure-recovery/cohort-analysis.json`;
  const preparationPath = `${ADJUDICATION_ROOT}/preparation-manifest.json`;
  const audioAnalysisPath = `${AUDIO_ROOT}/correction-analysis.json`;
  const audioAuditPath = `${AUDIO_ROOT}/correction-audio-verification.json`;
  const [
  ...
  assertV4(
    adjudicationAnalysis.status ===
        "passed-complete-batch-03-adjudication-cohort-after-bounded-first-corrections-standing-authorization-active-for-final-ledger-assembly" &&
      adjudicationAnalysis.gate.passed === true &&
      adjudicationAnalysis.gate.debateOutputsAccepted === 10 &&
      adjudicationAnalysis.gate.disputedMovesDecided === 190 &&
      adjudicationAnalysis.gate.candidateSelections === 586 &&
      adjudicationAnalysis.gate.calculatedScores === 0 &&
      adjudicationAnalysis.authorization.finalLedgerAssembly === true &&
      adjudicationAnalysis.authorization.scoreDerivation === false,
    "accepted post-canary Batch 3 adjudication gate unavailable"
  );
  assertV4(
    audioAnalysis.status ===
        "passed-all-eight-batch-03-audio-attributions-and-cost-overlay-after-correction" &&
      audioAnalysis.gate.verified === 8 &&
      audioAnalysis.gate.unresolved === 0 &&
      audioAudit.totals.verified === 8 &&
      audioAudit.totals.unresolved === 0,
    "complete Batch 3 audio verification unavailable"
  );
  ...
  for (const context of preparation.contexts) {
    ...
    debateInputs.push({
      ...
      sourceLedgerBytes,
      audioAudit,
      adjudicationOutputAcceptedWithoutCorrection: result.route === "resumption",
      adjudicationOutputPath,
  ...
  assertV4(
    debateInputs.map((input) => input.primaryA.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "loaded post-canary Batch 3 debate order invalid"
  );
