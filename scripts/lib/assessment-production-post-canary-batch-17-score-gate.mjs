...
export const POST_CANARY_BATCH_17_SCORE_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-17/score-pass";
export const POST_CANARY_BATCH_17_CALCULATED_SCORES_VERSION =
  "1.0-assessment-production-post-canary-batch-17-adjudicated-calculated-scores";
export const POST_CANARY_BATCH_17_SCORE_STABILITY_THRESHOLDS =
  ACTIVE_SCORE_STABILITY_THRESHOLDS;
...
export function evaluatePostCanaryBatch17ScoreStability(
  debates,
  thresholds = POST_CANARY_BATCH_17_SCORE_STABILITY_THRESHOLDS
) {
  assertV4(
    Array.isArray(debates) && debates.length === 4,
    "four scored post-canary Batch 17 debates required"
  );
  assertV4(
    canonicalJson(thresholds) ===
      canonicalJson(POST_CANARY_BATCH_17_SCORE_STABILITY_THRESHOLDS),
    "post-canary Batch 17 active thresholds changed"
  );
...
export function derivePostCanaryBatch17Scores(
  ledger,
  debateInputs,
  ...
  assertV4(
    ledger?.status ===
      ...
      ledger.batchNumber === 17 &&
      ledger.audit.finalRawJudgments === 4 &&
      ledger.audit.calculatedScores === 0 &&
      ledger.authorization.scoreDerivation === false,
    "validated score-free post-canary Batch 17 final ledger required for scoring"
  );
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === 4,
    "four post-canary Batch 17 score inputs required"
  );
  ...
  const debates = debateInputs.map((input) => {
    ...
    assertV4(
      finalLedgerDebate,
      `${debateNumber}: Batch 17 final-ledger debate missing`
    );
  ...
  const stability = evaluatePostCanaryBatch17ScoreStability(debates);
  ...
  return {
    schemaVersion: POST_CANARY_BATCH_17_CALCULATED_SCORES_VERSION,
    ...
    status: stability.acceptancePassed
      ...
      : "post-canary-batch-17-single-score-pass-stability-gate-failed",
    ...
    formulaBoundary: {
      scoringPasses: 1,
      rawJudgmentsScoredInPass: 12,
      finalJudgmentsEvaluatedFromPass: 4,
...
export function validatePostCanaryBatch17Scores(
  scores,
  ledger,
  debateInputs,
  productionReferences,
  ...
  const expected = derivePostCanaryBatch17Scores(
    ledger,
    debateInputs,
    productionReferences,
    hashes
  ...
  assertV4(
    ...
    "post-canary Batch 17 calculated scores differ from deterministic replay"
  );
  return {
    status: "passed",
