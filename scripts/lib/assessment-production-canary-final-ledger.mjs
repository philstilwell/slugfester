...
import { extractAssessmentProductionCanaryDisagreements } from "./assessment-production-canary-disagreement.mjs";
import {
  buildProductionCanaryDisputeAdjudicationPacket,
  validateProductionCanaryDisputeAdjudicationOutput,
} from "./assessment-production-canary-dispute-adjudication.mjs";

export const PRODUCTION_CANARY_FINAL_LEDGER_ROOT =
  "docs/assessment-production/canary-v1-final-ledger";
export const PRODUCTION_CANARY_FINAL_LEDGER_PROTOCOL_ID =
  "assessment-production-canary-v1-adjudicated-consensus";
export const PRODUCTION_CANARY_FINAL_LEDGER_VERSION =
  "1.0-production-canary-adjudicated-raw-ledger";
...
function validateInputReplay(input) {
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: stored disagreements differ from deterministic production-canary replay`
  );
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication packet differs from deterministic production-canary replay`
  );
  assertV4(
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication provenance differs from deterministic production-canary replay`
  );
  validateProductionCanaryDisputeAdjudicationOutput(
    adjudicationOutput,
    adjudicationPacket
...
export function compileProductionCanaryFinalLedgerDebate(input) {
  const {
    primaryA,
    primaryB,
    adjudicationPacket,
...
export function buildProductionCanaryFinalLedger(debateInputs, sourceHashes) {
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === EXPECTED_DEBATES.length,
    "ten production-canary debate inputs required"
  );
  const debates = debateInputs.map(compileProductionCanaryFinalLedgerDebate);
  assertV4(
    debates.map((debate) => debate.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "production-canary final-ledger debate order invalid"
  );
  ...
  assertV4(
    ...
    "production-canary disputed-move population mismatch"
  );
  assertV4(
    candidateSelections === 504,
    "production-canary candidate-selection population mismatch"
  );
  assertV4(
    aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed === 392,
    "production-canary rounded-mean population mismatch"
  );
  assertV4(
    aggregate.audioVerifiedMoves === 4,
    "production-canary audio-verification population mismatch"
  );
  return {
    schemaVersion: PRODUCTION_CANARY_FINAL_LEDGER_VERSION,
    protocolId: PRODUCTION_CANARY_FINAL_LEDGER_PROTOCOL_ID,
    status: "passed-production-canary-deterministic-final-ledger-assembly",
    productionCanary: true,
    stagingOnly: true,
    AIOnly: true,
    scoringInputSchema:
      "v4.2.20-source-span-primary-with-production-canary-adjudicated-provenance",
...
export function validateProductionCanaryFinalLedger(
  ledger,
  debateInputs,
  sourceHashes
) {
  const expected = buildProductionCanaryFinalLedger(
    debateInputs,
    sourceHashes
  );
  assertV4(
    canonicalJson(ledger) === canonicalJson(expected),
    "production-canary final ledger differs from deterministic replay"
  );
  assertV4(
    !containsProhibitedCalculatedField(
      ledger.debates.map((debate) => debate.finalJudgment)
    ),
    "production-canary final ledger contains a prohibited calculated field"
  );
  return {
    status: "passed",
...
function buildCombinedAudioAudit(audioAudit, audioAttributionAnalysis) {
  ...
  const originalUnresolved = combined.debates
    ...
    .find(
      (move) =>
        move.debateNumber === "05" && move.moveId === decision.moveId
    );
  assertV4(
    decision.status === "verified" &&
      decision.moveId === "pro-move-07" &&
      originalUnresolved?.status === "unresolved",
    "production-canary combined audio resolution unavailable"
  );
  const debate = combined.debates.find(
    (item) => item.debateNumber === "05"
  );
  ...
  combined.status = "passed-all-four-production-canary-audio-moves-verified";
  combined.totals.verified = 4;
  combined.totals.unresolved = 0;
  return combined;
}

export async function loadProductionCanaryFinalLedgerInputs() {
  const adjudicationRoot =
    "docs/assessment-production/canary-v1-dispute-only-adjudication";
  const judgmentRoot =
    "docs/assessment-production/canary-v1-independent-judgments";
  const audioRoot =
    "docs/assessment-production/canary-v1-audio-verification";
  const audioAttributionRoot =
  ...
  assertV4(
    ...
    "production-canary final-ledger assembly is not authorized"
  );
  assertV4(
    execution.status ===
    ...
    "accepted production-canary adjudication execution unavailable"
  );
  assertV4(
    audioAnalysis.status === "production-canary-audio-verification-unresolved" &&
    ...
    "complete production-canary audio verification chain unavailable"
  );
  ...
  assertV4(
    debateInputs.map((input) => input.primaryA.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "loaded production-canary debate order invalid"
  );
