...
import {
  validatePostCanaryBatch08RenderingViewportEvidence
} from "./assessment-production-post-canary-batch-08-rendering-verification.mjs";
...
function createHybridBrowser({ pointerBrowser, keyboardBrowser, viewport }) {
  ...
  return {
    adapter: {
      capabilities: {
        async get(name) {
          assertV4(name === "viewport", "Batch 8 viewport capability required");
          return {
            async set() {
              viewportIndex += 1;
              if (viewportIndex === 1) {
                const controller =
                  await pointerBrowser.capabilities.get("viewport");
                await controller.set(viewport.pointerControllerInput);
                return;
              }
              assertV4(viewportIndex === 2, "Batch 8 viewport phase overflow");
              const controller =
                await keyboardBrowser.capabilities.get("viewport");
      ...
      tabs: {
        async new() {
          ...
          assertV4(tabIndex === 2, "Batch 8 tab phase overflow");
    ...
    snapshot() {
      assertV4(
        viewportIndex === 2 && tabIndex === 2,
        "Batch 8 rendering phases incomplete"
      );
...
export async function runPostCanaryBatch08RenderingViewport({
  pointerBrowser,
  keyboardBrowser,
  packet,
  viewportName,
  ...
  assertV4(viewport, `${packet.debateNumber}: unknown Batch 8 viewport`);
  assertV4(options.timeoutMs === 15000, "frozen timeout must be 15000ms");
  ...
  const dimensionsPass = ["collapsed", "open"].every((name) => {
    ...
    return (
      image.pixelWidth === viewport.expectedScreenshotPixels.width &&
  ...
  try {
    ...
    if (options.failureDiagnosticPath) {
      await writeFile(
        path.join(options.rootAbs, options.failureDiagnosticPath),
        `${JSON.stringify(
          {
            schemaVersion:
              "1.0-assessment-production-post-canary-batch-08-rendering-rejected-evidence",
            protocolId: packet.protocolId,
            status: "preserved-rejected-rendering-evidence",
