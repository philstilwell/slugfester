...
export const V213_SCORE_ROOT =
  "docs/assessment-production/score-stability-v2.1.3-validation-cohort/score-pass";
export const V213_CALCULATED_SCORES_VERSION =
  "1.0-score-stability-v2.1.3-adjudicated-calculated-scores";
export const V213_SCORE_STABILITY_THRESHOLDS =
  PROPOSED_V21_SCORE_STABILITY_THRESHOLDS;
...
function deriveRawScores(raw, eventsDocument) {
  ...
  return {
    ...scores,
    scoreProtocolId:
      "assessment-production-score-stability-v2.1.3-single-deterministic-score-pass",
  };
}
...
export function evaluateV213ScoreStability(
  debates,
  thresholds = V213_SCORE_STABILITY_THRESHOLDS
) {
  assertV4(
    Array.isArray(debates) && debates.length === 10,
    "ten scored v2.1.3 debates required"
  );
  assertV4(
    canonicalJson(thresholds) ===
      canonicalJson(V213_SCORE_STABILITY_THRESHOLDS),
    "v2.1.3 prospective thresholds changed"
  );
...
export function deriveV213Scores(
  ledger,
  debateInputs,
  ...
  assertV4(
    ...
    "validated v2.1.3 adjudicated final ledger required for scoring"
  );
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === 10,
    "ten v2.1.3 score inputs required"
  );
  ...
  const debates = debateInputs.map((input) => {
    ...
    assertV4(
      finalLedgerDebate,
      `${debateNumber}: v2.1.3 final ledger debate missing`
    );
  ...
  const stability = evaluateV213ScoreStability(debates);
  ...
  return {
    schemaVersion: V213_CALCULATED_SCORES_VERSION,
    protocolId: ledger.protocolId,
    status: stability.acceptancePassed
      ? "v2.1.3-single-score-pass-stability-gate-passed"
      : "v2.1.3-single-score-pass-stability-gate-failed",
    ...
    acceptanceRule: {
      ...
      unroundedDirectionDiagnosticOnly: true,
      ...structuredClone(V213_SCORE_STABILITY_THRESHOLDS),
...
export function validateV213Scores(
  scores,
  ledger,
  debateInputs,
  productionReferences,
  ...
  const expected = deriveV213Scores(
    ledger,
    debateInputs,
    productionReferences,
    hashes
  ...
  assertV4(
    ...
    "v2.1.3 calculated scores differ from deterministic replay"
  );
  return {
    status: "passed",
