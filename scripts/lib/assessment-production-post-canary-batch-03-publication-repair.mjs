...
export const POST_CANARY_BATCH_03_DEBATE_124_REPAIR_ROOT =
  `${POST_CANARY_BATCH_03_PUBLICATION_ROOT}/repair-1`;
export const POST_CANARY_BATCH_03_DEBATE_124_REPAIR_PROTOCOL_ID =
  "assessment-production-post-canary-batch-03-debate-124-publication-repair-1";
export const POST_CANARY_BATCH_03_DEBATE_124_REPAIR_PACKET_VERSION =
  "1.0-assessment-production-post-canary-batch-03-debate-124-publication-repair-packet";
export const POST_CANARY_BATCH_03_DEBATE_124_REPAIR_OUTPUT_VERSION =
  "1.0-assessment-production-post-canary-batch-03-debate-124-publication-repair-output";
export const POST_CANARY_BATCH_03_DEBATE_124_REPAIR_PARTITIONS = Object.freeze([
  Object.freeze([
    "moveProse.pro-narrative-moral-limiting-cases.critique",
    "moveProse.pro-rational-instruction-behavioral-limit.critique"
  ]),
  Object.freeze([
    "moveProse.pro-evolved-implicit-guidance.critique"
  ])
]);
export const POST_CANARY_BATCH_03_DEBATE_124_REPAIR_FIELDS = Object.freeze(
  POST_CANARY_BATCH_03_DEBATE_124_REPAIR_PARTITIONS.flat()
);
export const POST_CANARY_BATCH_03_DEBATE_124_BASE_OUTPUT =
  `${POST_CANARY_BATCH_03_PUBLICATION_ROOT}/outputs/debate-124.json`;
export const POST_CANARY_BATCH_03_DEBATE_124_PUBLICATION_PACKET =
  `${POST_CANARY_BATCH_03_PUBLICATION_ROOT}/packets/debate-124.json`;
...
export function debate124RepairMoveId(field) {
  const match = /^moveProse\.([^.]+)\.critique$/.exec(field);
  assertV4(match, `invalid repair field: ${field}`);
  return match[1];
}

export function buildDebate124RepairSchema(packet) {
  ...
  return {
    ...
    $id:
      `slugfester-post-canary-batch-03-debate-124-publication-repair-${packet.packetIndex}`,
    title:
      `Slugfester post-canary Batch 3 Debate 124 bounded publication repair packet ${packet.packetIndex}`,
    type: "object",
    ...
    properties: {
      schemaVersion: {
        type: "string",
        const: POST_CANARY_BATCH_03_DEBATE_124_REPAIR_OUTPUT_VERSION
      },
      protocolId: {
        type: "string",
        const: POST_CANARY_BATCH_03_DEBATE_124_REPAIR_PROTOCOL_ID
      },
      packetIndex: { type: "integer", const: packet.packetIndex },
      debateNumber: { type: "string", const: "124" },
...
export function validateDebate124RepairOutput(repair, packet) {
  const expectedTop = [
    "schemaVersion",
    "protocolId",
    "packetIndex",
  ...
  assertV4(
    repair.schemaVersion ===
      POST_CANARY_BATCH_03_DEBATE_124_REPAIR_OUTPUT_VERSION &&
      repair.protocolId === POST_CANARY_BATCH_03_DEBATE_124_REPAIR_PROTOCOL_ID &&
      repair.packetIndex === packet.packetIndex &&
      repair.debateNumber === "124" &&
...
function withRepairMarkers(output) {
  ...
  for (const field of POST_CANARY_BATCH_03_DEBATE_124_REPAIR_FIELDS) {
    copy.moveProse[debate124RepairMoveId(field)].critique =
      "__AUTHORIZED_REPAIR_FIELD__";
  }
  return copy;
}

export function mergeAndValidateDebate124Repairs({
  baseOutput,
  repairs,
  repairPackets,
  publicationPacket
  ...
  assertV4(
    ...
    "both Debate 124 repair packets are required for merge"
  );
  ...
  for (let index = 0; index < 2; index += 1) {
    ...
    validateDebate124RepairOutput(repair, packet);
    for (const correction of packet.corrections) {
