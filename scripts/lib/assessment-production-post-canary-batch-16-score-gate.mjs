...
export const POST_CANARY_BATCH_16_SCORE_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-16/score-pass";
export const POST_CANARY_BATCH_16_CALCULATED_SCORES_VERSION =
  "1.0-assessment-production-post-canary-batch-16-adjudicated-calculated-scores";
export const POST_CANARY_BATCH_16_SCORE_STABILITY_THRESHOLDS =
  ACTIVE_SCORE_STABILITY_THRESHOLDS;
...
export function evaluatePostCanaryBatch16ScoreStability(
  debates,
  thresholds = POST_CANARY_BATCH_16_SCORE_STABILITY_THRESHOLDS
) {
  assertV4(
    Array.isArray(debates) && debates.length === 10,
    "ten scored post-canary Batch 16 debates required"
  );
  assertV4(
    canonicalJson(thresholds) ===
      canonicalJson(POST_CANARY_BATCH_16_SCORE_STABILITY_THRESHOLDS),
    "post-canary Batch 16 active thresholds changed"
  );
...
export function derivePostCanaryBatch16Scores(
  ledger,
  debateInputs,
  ...
  const debates = debateInputs.map((input) => {
    ...
    assertV4(
      finalLedgerDebate,
      `${debateNumber}: Batch 16 final-ledger debate missing`
    );
  ...
  const stability = evaluatePostCanaryBatch16ScoreStability(debates);
  ...
  return {
    schemaVersion: POST_CANARY_BATCH_16_CALCULATED_SCORES_VERSION,
    protocolId: ledger.protocolId,
    status: stability.acceptancePassed
      ? "post-canary-batch-16-single-score-pass-stability-gate-passed"
...
export function validatePostCanaryBatch16Scores(
  scores,
  ledger,
  debateInputs,
  productionReferences,
  ...
  const expected = derivePostCanaryBatch16Scores(
    ledger,
    debateInputs,
    productionReferences,
    hashes
  ...
  assertV4(
    ...
    "post-canary Batch 16 calculated scores differ from deterministic replay"
  );
  return {
    status: "passed",
