...
export function extractCheckpointV22RenderingRemedyV6SignatureHex(
  bytes,
  contract
) {
  if (
    !Buffer.isBuffer(bytes) ||
    !Number.isInteger(contract?.signatureBytes) ||
    contract.signatureBytes <= 0 ||
    !Number.isInteger(contract?.signatureHexCharacters) ||
    contract.signatureHexCharacters !== contract.signatureBytes * 2 ||
    typeof contract?.signatureHex !== "string" ||
    contract.signatureHex.length !== contract.signatureHexCharacters ||
    !/^[a-f0-9]+$/.test(contract.signatureHex) ||
    bytes.length < contract.signatureBytes
  ) {
    throw new Error("Frozen JPEG signature extraction contract is invalid");
  }
  return bytes.subarray(0, contract.signatureBytes).toString("hex");
}

async function analyzeJpeg(bytes, analyzerPath, contract) {
  const temporary = await mkdtemp(path.join(tmpdir(), "slugfester-v6-image-"));
  ...
  try {
    ...
    return {
      ...
      signatureHex: extractCheckpointV22RenderingRemedyV6SignatureHex(
        bytes,
        contract
      ),
...
function imageChecks(analysis, viewport, target, ratio, contract) {
  return {
    jpeg: analysis.format === contract.format &&
      analysis.signatureBytesInspected === contract.signatureBytes &&
...
export async function runCheckpointV22RenderingRemedyV6Viewport({
  browser,
  packet,
  viewportName,
