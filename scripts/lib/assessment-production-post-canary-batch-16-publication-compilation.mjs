...
import { validatePostCanaryBatch16PublicationOutput } from
  "./assessment-production-post-canary-batch-16-publication-validation.mjs";
import { assertV4, canonicalJson } from "./v4-lean-production.mjs";

export const POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-16/deterministic-publication-compilation";
export const POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-16-deterministic-publication-compilation";
export const POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_ORDER = Object.freeze(
  [...POST_CANARY_BATCH_16_PUBLICATION_DEBATES]);

export function compilePostCanaryBatch16PublicationStagingRecord({ output, packet, identity }) {
  validatePostCanaryBatch16PublicationOutput(output, packet);
  ...
  compiled.stagingAudit.protocolId = POST_CANARY_BATCH_16_PUBLICATION_COMPILATION_PROTOCOL_ID;
  compiled.stagingAudit.productionCanary = false;
  compiled.stagingAudit.batchNumber = 16;
  return compiled;
}

export function validatePostCanaryBatch16CompiledStagingRecord({ compiled, output, packet, identity }) {
  const expected = compilePostCanaryBatch16PublicationStagingRecord({ output, packet, identity });
