...
import { extractAssessmentProductionPostCanaryBatch12Disagreements } from "./assessment-production-post-canary-batch-12-disagreement.mjs";
import {
  buildPostCanaryBatch12DisputeAdjudicationPacket,
  validatePostCanaryBatch12DisputeAdjudicationOutput
} from "./assessment-production-post-canary-batch-12-dispute-adjudication.mjs";

export const POST_CANARY_BATCH_12_FINAL_LEDGER_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-12/final-ledger";
export const POST_CANARY_BATCH_12_FINAL_LEDGER_PROTOCOL_ID =
  "assessment-production-post-canary-batch-12-adjudicated-consensus";
export const POST_CANARY_BATCH_12_FINAL_LEDGER_VERSION =
  "1.0-assessment-production-post-canary-batch-12-adjudicated-raw-ledger";
...
function validateInputReplay(input) {
  ...
  const replayedDisagreements =
    extractAssessmentProductionPostCanaryBatch12Disagreements(
      primaryA,
      primaryB,
      lockedInventory
    );
  assertV4(
    ...
    `${primaryA.debateNumber}: stored disagreements differ from deterministic Batch 12 replay`
  );
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication packet differs from deterministic Batch 12 replay`
  );
  assertV4(
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication provenance differs from deterministic Batch 12 replay`
  );
  validatePostCanaryBatch12DisputeAdjudicationOutput(
    adjudicationOutput,
    adjudicationPacket
...
export function compilePostCanaryBatch12FinalLedgerDebate(input) {
  const {
    primaryA,
    primaryB,
    adjudicationPacket,
...
export function buildPostCanaryBatch12FinalLedger(debateInputs, sourceHashes) {
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === EXPECTED_DEBATES.length,
    "ten post-canary Batch 12 debate inputs required"
  );
  const debates = debateInputs.map(compilePostCanaryBatch12FinalLedgerDebate);
  assertV4(
    debates.map((debate) => debate.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "post-canary Batch 12 final-ledger debate order invalid"
  );
  ...
  assertV4(aggregate.finalMoves === 204, "Batch 12 final-move population mismatch");
  assertV4(aggregate.disputedMoves === 196, "Batch 12 disputed-move population mismatch");
  assertV4(candidateSelections === 528, "Batch 12 candidate-selection population mismatch");
  assertV4(
    aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed === 442,
    `Batch 12 rounded-mean population mismatch: ${aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed}`
  );
  assertV4(aggregate.audioVerifiedMoves === 3, "Batch 12 audio-verification population mismatch");
  return {
    schemaVersion: POST_CANARY_BATCH_12_FINAL_LEDGER_VERSION,
    protocolId: POST_CANARY_BATCH_12_FINAL_LEDGER_PROTOCOL_ID,
    status: "passed-post-canary-batch-12-deterministic-final-ledger-assembly",
    ...
    AIOnly: true,
    scoringInputSchema:
      "v4.2.20-source-span-primary-with-post-canary-batch-12-adjudicated-provenance",
    ...
    nextAuthorizedAction:
      "standing-authorization-permits-batch-12-single-deterministic-score-pass-preparation"
  };
}

export function validatePostCanaryBatch12FinalLedger(
  ledger,
  debateInputs,
  sourceHashes
) {
  const expected = buildPostCanaryBatch12FinalLedger(debateInputs, sourceHashes);
  assertV4(
    canonicalJson(ledger) === canonicalJson(expected),
    "post-canary Batch 12 final ledger differs from deterministic replay"
  );
  assertV4(
    !containsProhibitedCalculatedField(
      ledger.debates.map((debate) => debate.finalJudgment)
    ),
    "post-canary Batch 12 final ledger contains a prohibited calculated field"
  );
  return {
    status: "passed",
...
export async function loadPostCanaryBatch12FinalLedgerInputs() {
  ...
  assertV4(
    adjudicationAnalysis.status ===
      ...
      adjudicationAnalysis.gate.candidateSelections === 528 &&
    ...
    "accepted post-canary Batch 12 adjudication gate unavailable"
  );
  const sourcePaths = [
  ...
  assertV4(
    debateInputs.map((input) => input.primaryA.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "loaded post-canary Batch 12 debate order invalid"
  );
