import { POST_CANARY_BATCH_09_PUBLICATION_BYLINE,
  POST_CANARY_BATCH_09_PUBLICATION_MODEL } from
  "./assessment-production-post-canary-batch-09-publication.mjs";
import { POST_CANARY_BATCH_09_PUBLICATION_COMPILATION_ORDER,
  POST_CANARY_BATCH_09_PUBLICATION_COMPILATION_PROTOCOL_ID,
  validatePostCanaryBatch09CompiledStagingRecord } from
  "./assessment-production-post-canary-batch-09-publication-compilation.mjs";
import { assertV4, canonicalJson } from "./v4-lean-production.mjs";

export const POST_CANARY_BATCH_09_PUBLICATION_FINALIZATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-09/publication-finalization";
export const POST_CANARY_BATCH_09_PUBLICATION_FINALIZATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-09-publication-finalization";
export const POST_CANARY_BATCH_09_PUBLICATION_FINALIZATION_ORDER =
  POST_CANARY_BATCH_09_PUBLICATION_COMPILATION_ORDER;

export function buildPostCanaryBatch09PublicationFinalization({ compiled, compiledPath,
  compiledSha256, output, packet, identity }) {
  const compilationValidation = validatePostCanaryBatch09CompiledStagingRecord({
    compiled, output, packet, identity });
  assertV4(compiled.stagingAudit?.protocolId === POST_CANARY_BATCH_09_PUBLICATION_COMPILATION_PROTOCOL_ID &&
  ...
  const provenance = { schemaVersion: "1.0-assessment-production-post-canary-batch-09-publication-finalization-provenance",
    protocolId: POST_CANARY_BATCH_09_PUBLICATION_FINALIZATION_PROTOCOL_ID,
    status: "staging-only-batch-09-final-candidate-provenance",
    productionCanary: false, batchNumber: 9, debateNumber: compiled.number,
...
export function validatePostCanaryBatch09PublicationFinalCandidate({ candidate, provenance,
  compiled, output, packet, identity }) {
  const expected = buildPostCanaryBatch09PublicationFinalization({ compiled,
...
export function buildPostCanaryBatch09PublicationStagingPreviewHtml() {
  const allowed = JSON.stringify(POST_CANARY_BATCH_09_PUBLICATION_FINALIZATION_ORDER);
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8">
    ...
    <title>Slugfester Batch 9 publication staging preview</title>
  ...
  <body>
    ...
    <script type="module">
      ...
      const debateNumber = params.get("debate") || "170";
      if (!allowedHosts.has(window.location.hostname)) {
        ...
        document.querySelector("#app").innerHTML = "<main><h1>Unknown Batch 9 publication-staging debate</h1></main>";
      } else {
