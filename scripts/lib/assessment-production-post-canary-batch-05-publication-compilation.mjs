...
export const POST_CANARY_BATCH_05_PUBLICATION_COMPILATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-05/deterministic-publication-compilation";
export const POST_CANARY_BATCH_05_PUBLICATION_COMPILATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-05-deterministic-publication-compilation";
export const POST_CANARY_BATCH_05_PUBLICATION_COMPILATION_ORDER = Object.freeze(
  [...POST_CANARY_BATCH_05_PUBLICATION_DEBATES]
);
...
function referenceUrl(tag) {
  ...
  assertV4(
    ...
    `unknown Batch 5 publication reference ${tag.type}:${tag.slug}`
  );
...
export function compilePostCanaryBatch05PublicationStagingRecord({
  output,
  packet,
  identity
}) {
  validatePostCanaryBatch05PublicationOutput(output, packet);
  assertV4(
    identity &&
...
export function validatePostCanaryBatch05CompiledStagingRecord({
  compiled,
  output,
  packet,
  identity
}) {
  const expected = compilePostCanaryBatch05PublicationStagingRecord({
    output,
    packet,
    identity
  });
