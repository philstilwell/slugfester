...
export const POST_CANARY_BATCH_01_SCORE_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-01/score-pass";
export const POST_CANARY_BATCH_01_CALCULATED_SCORES_VERSION =
  "1.0-assessment-production-post-canary-batch-01-adjudicated-calculated-scores";
export const POST_CANARY_BATCH_01_SCORE_STABILITY_THRESHOLDS =
  ACTIVE_SCORE_STABILITY_THRESHOLDS;
...
export function evaluatePostCanaryBatch01ScoreStability(
  debates,
  thresholds = POST_CANARY_BATCH_01_SCORE_STABILITY_THRESHOLDS
) {
  assertV4(
    Array.isArray(debates) && debates.length === 10,
    "ten scored post-canary Batch 1 debates required"
  );
  assertV4(
    canonicalJson(thresholds) ===
      canonicalJson(POST_CANARY_BATCH_01_SCORE_STABILITY_THRESHOLDS),
    "post-canary Batch 1 active thresholds changed"
  );
...
export function derivePostCanaryBatch01Scores(
  ledger,
  debateInputs,
  ...
  const debates = debateInputs.map((input) => {
    ...
    assertV4(
      finalLedgerDebate,
      `${debateNumber}: Batch 1 final-ledger debate missing`
    );
  ...
  const stability = evaluatePostCanaryBatch01ScoreStability(debates);
  ...
  return {
    schemaVersion: POST_CANARY_BATCH_01_CALCULATED_SCORES_VERSION,
    ...
    status: stability.acceptancePassed
      ...
      : "post-canary-batch-01-single-score-pass-stability-gate-failed",
    ...
    acceptanceRule: {
      ...
      unroundedDirectionDiagnosticOnly: true,
      ...structuredClone(POST_CANARY_BATCH_01_SCORE_STABILITY_THRESHOLDS),
...
export function validatePostCanaryBatch01Scores(
  scores,
  ledger,
  debateInputs,
  productionReferences,
  ...
  const expected = derivePostCanaryBatch01Scores(
    ledger,
    debateInputs,
    productionReferences,
    hashes
  ...
  assertV4(
    ...
    "post-canary Batch 1 calculated scores differ from deterministic replay"
  );
  return {
    status: "passed",
