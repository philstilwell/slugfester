...
export const ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-12/publication-reconstruction/failure-recovery/original-unattempted-context-resumption-2/field-disjoint-repair-2-continuation";
export const PROTOCOL_ID =
  "assessment-production-post-canary-batch-12-publication-field-disjoint-repair-2-continuation";
export const OUTPUT_VERSION =
  "1.0-assessment-production-post-canary-batch-12-publication-field-disjoint-repair-output";
...
export const DEBATE_PLANS = Object.freeze([
  Object.freeze({
    debateNumber: "131",
    critiqueShardSizes: Object.freeze([2, 2, 2, 2, 2, 2, 2, 2, 2, 2])
  }),
  Object.freeze({ debateNumber: "07", critiqueShardSizes: Object.freeze([0]) }),
  Object.freeze({ debateNumber: "15", critiqueShardSizes: Object.freeze([2, 2, 2, 2]) })
]);
...
export function buildFieldDisjointRepairSchema(packet) {
  return {
    ...
    $id: `slugfester-batch-12-field-disjoint-repair-${packet.contextIndex}`,
    title: `Batch 12 Debate ${packet.debateNumber} field-disjoint repair ${packet.shardId}`,
    type: "object",
...
export function validateFieldDisjointRepairOutput(output, packet) {
  ...
  for (const target of packet.targets) {
    ...
    if (target.kind === "novelty") {
      const words = wordCount(correction);
      assertV4(words >= 8,
        `${target.field}: novelty explanation shorter than eight words`);
      fieldValidations.push({ fieldKey: target.fieldKey, field: target.field,
        kind: target.kind, words, characters: correction.length });
      continue;
    }
    const words = wordCount(correction);
