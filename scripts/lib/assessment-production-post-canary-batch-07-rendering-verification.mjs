import {
  POST_CANARY_BATCH_07_PUBLICATION_FINALIZATION_ORDER
} from "./assessment-production-post-canary-batch-07-publication-finalization.mjs";
import {
...
export const POST_CANARY_BATCH_07_RENDERING_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-07/rendering-verification";
export const POST_CANARY_BATCH_07_RENDERING_PROTOCOL_ID =
  "assessment-production-post-canary-batch-07-rendering-verification";
export const POST_CANARY_BATCH_07_RENDERING_PORT = 4203;
export const POST_CANARY_BATCH_07_RENDERING_ORDER =
  POST_CANARY_BATCH_07_PUBLICATION_FINALIZATION_ORDER;
export const POST_CANARY_BATCH_07_RENDERING_MODEL =
  POST_CANARY_BATCH_07_PUBLICATION_MODEL;
export const POST_CANARY_BATCH_07_RENDERING_BYLINE =
  POST_CANARY_BATCH_07_PUBLICATION_BYLINE;
export const POST_CANARY_BATCH_07_RENDERING_IMAGE_CONTRACT =
  CHECKPOINT_V22_RENDERING_REMEDY_V9_IMAGE_CONTRACT;
export const POST_CANARY_BATCH_07_RENDERING_REQUIRED_BOOLEAN_CHECKS =
  CHECKPOINT_V22_RENDERING_REMEDY_V9_REQUIRED_BOOLEAN_CHECKS;
export const POST_CANARY_BATCH_07_RENDERING_VIEWPORTS =
  CHECKPOINT_V22_RENDERING_REMEDY_V9_VIEWPORTS;

function evidencePaths(debateNumber, viewportName) {
  const root = `${POST_CANARY_BATCH_07_RENDERING_ROOT}/evidence/debate-${debateNumber}`;
  return {
...
export function buildPostCanaryBatch07RenderingPacket({
  auditRow,
  candidate,
  provenance,
  previewPath,
  ...
  assertV4(
    auditRow &&
      ...
      provenance.model.label === POST_CANARY_BATCH_07_RENDERING_MODEL.label &&
      provenance.model.reasoningEffort ===
        POST_CANARY_BATCH_07_RENDERING_MODEL.reasoningEffort &&
      provenance.model.authentication ===
        POST_CANARY_BATCH_07_RENDERING_MODEL.authentication &&
      ...
      provenance.displayContract.byline === POST_CANARY_BATCH_07_RENDERING_BYLINE &&
      provenance.displayContract.defaultCollapsed === true,
    `${auditRow?.debateNumber ?? "unknown"}: Batch 7 finalization provenance changed`
  );
  const newArguments = {
  ...
  const packet = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-07-rendering-verification-packet",
    protocolId: POST_CANARY_BATCH_07_RENDERING_PROTOCOL_ID,
    status: "frozen-batch-07-rendering-verification-packet",
    ...
    preview: {
      ...
      url:
        `http://127.0.0.1:${POST_CANARY_BATCH_07_RENDERING_PORT}/` +
        `${previewPath}?debate=${auditRow.debateNumber}`,
      localOnly: true,
      noindex: true
    },
    expectedDisplay: {
      byline: POST_CANARY_BATCH_07_RENDERING_BYLINE,
      stagingBannerPrefix: "Publication staging preview:",
      stagingBannerRequiredText: "validated post-canary Batch 7 candidate only",
    ...
    viewports: Object.fromEntries(
      Object.entries(POST_CANARY_BATCH_07_RENDERING_VIEWPORTS).map(
        ([name, viewport]) => [
          name,
          {
    ...
    runnerPolicy: {
      ...
      imageAnalyzerPath: POST_CANARY_BATCH_07_RENDERING_IMAGE_CONTRACT.analyzerPath,
      imageAnalyzerSha256:
        POST_CANARY_BATCH_07_RENDERING_IMAGE_CONTRACT.analyzerSha256,
      imageContract: structuredClone(
        POST_CANARY_BATCH_07_RENDERING_IMAGE_CONTRACT
      ),
  ...
  return validatePostCanaryBatch07RenderingPacket(packet);
}

export function validatePostCanaryBatch07RenderingPacket(packet) {
  assertV4(
    packet?.schemaVersion ===
        "1.0-assessment-production-post-canary-batch-07-rendering-verification-packet" &&
      packet.protocolId === POST_CANARY_BATCH_07_RENDERING_PROTOCOL_ID &&
      packet.status === "frozen-batch-07-rendering-verification-packet" &&
      packet.productionCanary === false &&
      packet.batchNumber === 7 &&
      packet.stagingOnly === true &&
      POST_CANARY_BATCH_07_RENDERING_ORDER.includes(packet.debateNumber) &&
      packet.preview.url.startsWith(
        `http://127.0.0.1:${POST_CANARY_BATCH_07_RENDERING_PORT}/`
      ) &&
      canonicalJson(packet.requiredBooleanChecks) ===
        canonicalJson(POST_CANARY_BATCH_07_RENDERING_REQUIRED_BOOLEAN_CHECKS) &&
      packet.expectedDisplay.byline === POST_CANARY_BATCH_07_RENDERING_BYLINE &&
      packet.expectedDisplay.defaultCollapsed === true &&
      packet.provenance.assessmentModel ===
        POST_CANARY_BATCH_07_RENDERING_MODEL.label &&
    ...
    `${packet?.debateNumber ?? "unknown"}: invalid Batch 7 rendering packet`
  );
  for (const [name, expected] of Object.entries(
    POST_CANARY_BATCH_07_RENDERING_VIEWPORTS
  )) {
    ...
    assertV4(
      canonicalJson(viewport.targetCssViewport) ===
        ...
        viewport.evidence.result ===
          `${POST_CANARY_BATCH_07_RENDERING_ROOT}/evidence/debate-${packet.debateNumber}/${name}.json`,
      `${packet.debateNumber}: invalid ${name} Batch 7 viewport`
    );
  }
  return packet;
}
...
function imagePasses(image, viewport) {
  ...
  const contract = POST_CANARY_BATCH_07_RENDERING_IMAGE_CONTRACT;
  return (
...
export function validatePostCanaryBatch07RenderingViewportEvidence({
  packet,
  viewportName,
  activationNavigationToken,
  evidence
}) {
  validatePostCanaryBatch07RenderingPacket(packet);
  ...
  assertV4(
    viewport &&
      evidence?.schemaVersion ===
        "1.0-assessment-production-post-canary-batch-07-rendering-viewport-evidence" &&
    ...
    `${packet?.debateNumber ?? "unknown"}: invalid ${viewportName} Batch 7 rendering evidence`
  );
  return evidence;
}
