...
export const POST_CANARY_BATCH_16_COMPATIBILITY_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-16/production-compatibility";
export const POST_CANARY_BATCH_16_COMPATIBILITY_PROTOCOL_ID =
  "assessment-production-post-canary-batch-16-production-compatibility";
export const POST_CANARY_BATCH_16_SITE_LEDGER_ADAPTER_VERSION =
  "1.0-assessment-production-post-canary-batch-16-site-ledger-adapter";
export const POST_CANARY_BATCH_16_COMPATIBILITY_ORDER = Object.freeze([
  "16",
  "108",
  "164",
  "144",
...
export function postCanaryBatch16SectionTitleMatches({
...
export function validatePostCanaryBatch16CandidateAgainstScores(
  candidate,
  finalScores
) {
  assertV4(
    ...
    `${finalScores.debateNumber}: Batch 16 publication candidate identity mismatch`
  );
  assertV4(
    ...
    `${finalScores.debateNumber}: Batch 16 publication candidate overall scores changed`
  );
  assertV4(
    candidate.sections?.length === finalScores.sections.length,
    `${finalScores.debateNumber}: Batch 16 publication candidate section count changed`
  );
  let moveCount = 0;
  finalScores.sections.forEach((scoredSection, sectionIndex) => {
    ...
    assertV4(
      candidateSection?.sectionId === scoredSection.sectionId &&
        postCanaryBatch16SectionTitleMatches({
      ...
      `${finalScores.debateNumber}: Batch 16 section ${sectionIndex} identity or score changed`
    );
    for (const side of ["pro", "con"]) {
      ...
      assertV4(
        ...
        `${finalScores.debateNumber}: Batch 16 section ${sectionIndex} ${side} move IDs, order, or scores changed`
      );
...
export function buildPostCanaryBatch16SiteLedgerAdapter({
  finalLedgerDebate,
  scoreDebate,
  candidate,
  eventsDocument,
  ...
  assertV4(
    ...
    "Batch 16 final-ledger and score identities differ"
  );
  ...
  assertV4(
    ...
    `${scoreDebate.debateNumber}: locked Batch 16 score is not an exact repository replay`
  );
  const candidateValidation =
    validatePostCanaryBatch16CandidateAgainstScores(candidate, {
      ...scoreDebate.final,
  ...
  return {
    ...
    status: "frozen-post-canary-batch-16-site-ledger-adapter",
...
export function validatePostCanaryBatch16SiteLedgerAdapter({
  adapter,
  candidate,
  expectedSourceLocks
}) {
  assertV4(
    adapter?.schemaVersion ===
      ...
      adapter.protocolId === POST_CANARY_BATCH_16_COMPATIBILITY_PROTOCOL_ID &&
      adapter.status ===
        "frozen-post-canary-batch-16-site-ledger-adapter" &&
      adapter.productionCanary === false &&
      adapter.batchNumber === 16 &&
      adapter.model === "5.6 Sol" &&
      adapter.rubric === "Slugfester Reassessment Rubric v2" &&
      adapter.scoreProtocolId === SCORE_PROTOCOL_ID,
    `${adapter?.debateNumber ?? "unknown"}: Batch 16 site-ledger adapter identity mismatch`
  );
  assertV4(
    canonicalJson(adapter.sourceLocks) === canonicalJson(expectedSourceLocks),
    `${adapter.debateNumber}: Batch 16 site-ledger source locks changed`
  );
  ...
  assertV4(
    ...
    `${adapter.debateNumber}: Batch 16 site-ledger scoring identity changed`
  );
  assertV4(
    canonicalJson(scoredSnapshot(replayed)) === canonicalJson(adapter.calculated),
    `${adapter.debateNumber}: Batch 16 site-ledger score replay changed`
  );
  const candidateValidation =
    validatePostCanaryBatch16CandidateAgainstScores(candidate, {
      ...replayed,
  ...
  assertV4(
    ...
    `${adapter.debateNumber}: Batch 16 site-ledger adapter audit changed`
  );
  return {
    ...candidateValidation,
