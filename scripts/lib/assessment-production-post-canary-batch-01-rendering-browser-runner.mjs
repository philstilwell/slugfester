...
} from "./assessment-production-post-canary-batch-01-rendering-verification.mjs";
...
function createHybridBrowser({ pointerBrowser, keyboardBrowser, viewport }) {
  ...
  return {
    adapter: {
      capabilities: {
        async get(name) {
          assertV4(name === "viewport", "Batch 1 viewport capability required");
          return {
            async set() {
              viewportIndex += 1;
              if (viewportIndex === 1) {
              ...
              assertV4(viewportIndex === 2, "Batch 1 viewport phase overflow");
              const controller =
      ...
      tabs: {
        async new() {
          ...
          assertV4(tabIndex === 2, "Batch 1 tab phase overflow");
    ...
    snapshot() {
      assertV4(
        ...
        "Batch 1 rendering phases incomplete"
      );
...
export async function runPostCanaryBatch01RenderingViewport({
  pointerBrowser,
  keyboardBrowser,
  packet,
  viewportName,
  ...
  assertV4(viewport, `${packet.debateNumber}: unknown Batch 1 viewport`);
