...
export const POST_CANARY_BATCH_04_DEBATE_127_REPAIR_ROOT =
  `${POST_CANARY_BATCH_04_PUBLICATION_ROOT}/repair-1`;
export const POST_CANARY_BATCH_04_DEBATE_127_REPAIR_PROTOCOL_ID =
  "assessment-production-post-canary-batch-04-debate-127-publication-repair-1";
export const POST_CANARY_BATCH_04_DEBATE_127_REPAIR_PACKET_VERSION =
  "1.0-assessment-production-post-canary-batch-04-debate-127-publication-repair-packet";
export const POST_CANARY_BATCH_04_DEBATE_127_REPAIR_OUTPUT_VERSION =
  "1.0-assessment-production-post-canary-batch-04-debate-127-publication-repair-output";
export const POST_CANARY_BATCH_04_DEBATE_127_REPAIR_PARTITIONS = Object.freeze([
  Object.freeze([
    "moveProse.con-circular-obedience-reason.critique",
    "moveProse.pro-perfect-consequence-knowledge.critique"
  ]),
  Object.freeze([
    "moveProse.pro-recipient-preference-variability.critique"
  ])
]);
export const POST_CANARY_BATCH_04_DEBATE_127_REPAIR_FIELDS = Object.freeze(
  POST_CANARY_BATCH_04_DEBATE_127_REPAIR_PARTITIONS.flat()
);
export const POST_CANARY_BATCH_04_DEBATE_127_BASE_OUTPUT =
  `${POST_CANARY_BATCH_04_PUBLICATION_ROOT}/outputs/debate-127.json`;
export const POST_CANARY_BATCH_04_DEBATE_127_PUBLICATION_PACKET =
  `${POST_CANARY_BATCH_04_PUBLICATION_ROOT}/packets/debate-127.json`;
...
export function debate127RepairMoveId(field) {
  const match = /^moveProse\.([^.]+)\.critique$/.exec(field);
  assertV4(match, `invalid repair field: ${field}`);
  return match[1];
}

export function buildDebate127RepairSchema(packet) {
  ...
  return {
    ...
    $id:
      `slugfester-post-canary-batch-04-debate-127-publication-repair-${packet.packetIndex}`,
    title:
      `Slugfester post-canary Batch 4 Debate 127 bounded publication repair packet ${packet.packetIndex}`,
    type: "object",
    ...
    properties: {
      schemaVersion: {
        type: "string",
        const: POST_CANARY_BATCH_04_DEBATE_127_REPAIR_OUTPUT_VERSION
      },
      protocolId: {
        type: "string",
        const: POST_CANARY_BATCH_04_DEBATE_127_REPAIR_PROTOCOL_ID
      },
      packetIndex: { type: "integer", const: packet.packetIndex },
      debateNumber: { type: "string", const: "127" },
...
export function validateDebate127RepairOutput(repair, packet) {
  const expectedTop = [
    "schemaVersion",
    "protocolId",
    "packetIndex",
  ...
  assertV4(
    repair.schemaVersion ===
      POST_CANARY_BATCH_04_DEBATE_127_REPAIR_OUTPUT_VERSION &&
      repair.protocolId === POST_CANARY_BATCH_04_DEBATE_127_REPAIR_PROTOCOL_ID &&
      repair.packetIndex === packet.packetIndex &&
      repair.debateNumber === "127" &&
...
function withRepairMarkers(output) {
  ...
  for (const field of POST_CANARY_BATCH_04_DEBATE_127_REPAIR_FIELDS) {
    copy.moveProse[debate127RepairMoveId(field)].critique =
      "__AUTHORIZED_REPAIR_FIELD__";
  }
  return copy;
}

export function mergeAndValidateDebate127Repairs({
  baseOutput,
  repairs,
  repairPackets,
  publicationPacket
  ...
  assertV4(
    repairs.length === 2 && repairPackets.length === 2,
    "both Debate 127 repair packets are required for merge"
  );
  ...
  for (let index = 0; index < 2; index += 1) {
    ...
    validateDebate127RepairOutput(repair, packet);
    for (const correction of packet.corrections) {
  ...
  const fullValidation = validatePostCanaryBatch04PublicationOutput(
    merged,
    publicationPacket
  );
