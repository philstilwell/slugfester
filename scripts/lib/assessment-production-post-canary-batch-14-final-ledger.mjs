...
import { extractAssessmentProductionPostCanaryBatch14Disagreements } from "./assessment-production-post-canary-batch-14-disagreement.mjs";
import {
  buildPostCanaryBatch14DisputeAdjudicationPacket,
  validatePostCanaryBatch14DisputeAdjudicationOutput
} from "./assessment-production-post-canary-batch-14-dispute-adjudication.mjs";

export const POST_CANARY_BATCH_14_FINAL_LEDGER_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-14/final-ledger";
export const POST_CANARY_BATCH_14_FINAL_LEDGER_PROTOCOL_ID =
  "assessment-production-post-canary-batch-14-adjudicated-consensus";
export const POST_CANARY_BATCH_14_FINAL_LEDGER_VERSION =
  "1.0-assessment-production-post-canary-batch-14-adjudicated-raw-ledger";
...
function validateInputReplay(input) {
  ...
  const replayedDisagreements =
    extractAssessmentProductionPostCanaryBatch14Disagreements(
      primaryA,
      primaryB,
      lockedInventory
    );
  assertV4(
    ...
    `${primaryA.debateNumber}: stored disagreements differ from deterministic Batch 14 replay`
  );
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication packet differs from deterministic Batch 14 replay`
  );
  assertV4(
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication provenance differs from deterministic Batch 14 replay`
  );
  validatePostCanaryBatch14DisputeAdjudicationOutput(
    adjudicationOutput,
    adjudicationPacket
...
export function compilePostCanaryBatch14FinalLedgerDebate(input) {
  const {
    primaryA,
    primaryB,
    adjudicationPacket,
...
export function buildPostCanaryBatch14FinalLedger(debateInputs, sourceHashes) {
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === EXPECTED_DEBATES.length,
    "ten post-canary Batch 14 debate inputs required"
  );
  const debates = debateInputs.map(compilePostCanaryBatch14FinalLedgerDebate);
  assertV4(
    debates.map((debate) => debate.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "post-canary Batch 14 final-ledger debate order invalid"
  );
  ...
  assertV4(aggregate.finalMoves === 190, "Batch 14 final-move population mismatch");
  assertV4(aggregate.disputedMoves === 181, "Batch 14 disputed-move population mismatch");
  assertV4(candidateSelections === 539, "Batch 14 candidate-selection population mismatch");
  assertV4(
    aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed === 394,
    `Batch 14 rounded-mean population mismatch: ${aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed}`
  );
  assertV4(aggregate.audioVerifiedMoves === 12, "Batch 14 audio-verification population mismatch");
  return {
    schemaVersion: POST_CANARY_BATCH_14_FINAL_LEDGER_VERSION,
    protocolId: POST_CANARY_BATCH_14_FINAL_LEDGER_PROTOCOL_ID,
    status: "passed-post-canary-batch-14-deterministic-final-ledger-assembly",
    ...
    AIOnly: true,
    scoringInputSchema:
      "v4.2.20-source-span-primary-with-post-canary-batch-14-adjudicated-provenance",
    ...
    nextAuthorizedAction:
      "standing-authorization-permits-batch-14-single-deterministic-score-pass-preparation"
  };
}

export function validatePostCanaryBatch14FinalLedger(
  ledger,
  debateInputs,
  sourceHashes
) {
  const expected = buildPostCanaryBatch14FinalLedger(debateInputs, sourceHashes);
  assertV4(
    canonicalJson(ledger) === canonicalJson(expected),
    "post-canary Batch 14 final ledger differs from deterministic replay"
  );
  assertV4(
    !containsProhibitedCalculatedField(
      ledger.debates.map((debate) => debate.finalJudgment)
    ),
    "post-canary Batch 14 final ledger contains a prohibited calculated field"
  );
  return {
    status: "passed",
...
export async function loadPostCanaryBatch14FinalLedgerInputs() {
  ...
  assertV4(
    adjudicationAnalysis.status ===
        "post-canary-batch-14-dispute-only-adjudication-gate-passed-standing-authorization-active-for-final-ledger-assembly" &&
      ...
      adjudicationAnalysis.gate.disputedMovesDecided === 181 &&
      adjudicationAnalysis.gate.candidateSelections === 539 &&
      adjudicationAnalysis.gate.scoresDerived === 0 &&
      adjudicationAnalysis.nextAuthorizedAction ===
        "standing-authorization-permits-batch-14-deterministic-final-ledger-assembly" &&
      adjudicationAnalysis.authorization.scoreDerivation === false &&
      adjudicationExecution.status ===
        "ten-post-canary-batch-14-dispute-only-adjudication-contexts-passed" &&
      ...
      preparation.acceptedSourceBoundary.allTwelveAudioMovesVerified === true &&
      preparation.totals.audioVerifiedMoves === 12,
    "accepted post-canary Batch 14 adjudication gate unavailable"
  );
  const sourcePaths = [
  ...
  assertV4(
    debateInputs.map((input) => input.primaryA.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "loaded post-canary Batch 14 debate order invalid"
  );
