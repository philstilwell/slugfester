...
export const POST_CANARY_BATCH_08_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-08";
export const POST_CANARY_BATCH_08_STANDING_AUTHORIZATION =
  `${POST_CANARY_BATCH_08_ROOT}/standing-authorization.json`;
export const POST_CANARY_BATCH_08_STANDING_AUTHORIZATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-08-standing-authorization";
export const POST_CANARY_BATCH_08_STANDING_AUTHORIZATION_STATUS =
  "frozen-active-batch-08-complete-remaining-workflow-standing-authorization";
export const POST_CANARY_BATCH_08_STANDING_AUTHORIZATION_INSTRUCTION_PATH =
  POST_CANARY_BATCH_08_ROOT + "/standing-authorization-instruction.txt";
export const POST_CANARY_BATCH_08_STANDING_AUTHORIZATION_INSTRUCTION = readFileSync(
  path.resolve(POST_CANARY_BATCH_08_STANDING_AUTHORIZATION_INSTRUCTION_PATH),
  "utf8"
).trimEnd();

const sha256 = (value) => createHash("sha256").update(value).digest("hex");

export function validatePostCanaryBatch08StandingAuthorization(record) {
  assertV4(
    record?.schemaVersion ===
      ...
      record.status === POST_CANARY_BATCH_08_STANDING_AUTHORIZATION_STATUS &&
      record.productionCanary === false &&
      record.batchNumber === 8 &&
      record.sourcePacketCommit === "8add15b63fc813d3d9b74565143dfcd136f2b18f" &&
      ...
      record.userAuthorization?.resolvedAntecedent ===
        "The next approval needed is for the Batch 8 standing authorization and score-blind source-packet preparation." &&
      record.userAuthorization?.scopeInterpretation ===
        "Create and use a complete-workflow Batch 8 standing authorization for the ten frozen selections, beginning with the separately frozen score-blind source packets, while preserving the established campaign controls and stopping before Batch 9 selection." &&
    ...
    "Batch 8 standing-authorization identity changed"
  );
  assertV4(
    canonicalJson(record.selectedDebates) ===
      canonicalJson(["88", "194", "137", "08", "65", "140", "156", "120", "118", "145"]),
    "Batch 8 standing-authorization debate boundary changed"
  );
  assertV4(
    ...
    "Batch 8 standing-authorization model changed"
  );
  assertV4(
    ...
    "Batch 8 standing-authorization execution controls changed"
  );
  assertV4(
  ...
  assertV4(
    ...
    "Batch 8 standing-authorization cost boundary changed"
  );
  assertV4(
    Object.values(record.stopRules ?? {}).every(Boolean),
    "Batch 8 standing-authorization stop rule disabled"
  );
  assertV4(
    ...
    "Batch 8 standing-authorization recovery controls changed"
  );
  return record;
}

export async function loadAndValidatePostCanaryBatch08StandingAuthorization() {
  const bytes = await readFile(
    path.resolve(POST_CANARY_BATCH_08_STANDING_AUTHORIZATION)
  );
  const record = validatePostCanaryBatch08StandingAuthorization(JSON.parse(bytes));
