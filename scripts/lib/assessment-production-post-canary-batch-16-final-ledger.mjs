...
import { extractAssessmentProductionPostCanaryBatch16Disagreements } from "./assessment-production-post-canary-batch-16-disagreement.mjs";
import {
  buildPostCanaryBatch16DisputeAdjudicationPacket,
  validatePostCanaryBatch16DisputeAdjudicationOutput
} from "./assessment-production-post-canary-batch-16-dispute-adjudication.mjs";

export const POST_CANARY_BATCH_16_FINAL_LEDGER_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-16/final-ledger";
export const POST_CANARY_BATCH_16_FINAL_LEDGER_PROTOCOL_ID =
  "assessment-production-post-canary-batch-16-adjudicated-consensus";
export const POST_CANARY_BATCH_16_FINAL_LEDGER_VERSION =
  "1.0-assessment-production-post-canary-batch-16-adjudicated-raw-ledger";
...
function validateInputReplay(input) {
  ...
  const replayedDisagreements =
    extractAssessmentProductionPostCanaryBatch16Disagreements(
      primaryA,
      primaryB,
      lockedInventory
    );
  assertV4(
    ...
    `${primaryA.debateNumber}: stored disagreements differ from deterministic Batch 16 replay`
  );
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication packet differs from deterministic Batch 16 replay`
  );
  assertV4(
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication provenance differs from deterministic Batch 16 replay`
  );
  validatePostCanaryBatch16DisputeAdjudicationOutput(
    adjudicationOutput,
    adjudicationPacket
...
export function compilePostCanaryBatch16FinalLedgerDebate(input) {
  const {
    primaryA,
    primaryB,
    adjudicationPacket,
...
export function buildPostCanaryBatch16FinalLedger(debateInputs, sourceHashes) {
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === EXPECTED_DEBATES.length,
    "ten post-canary Batch 16 debate inputs required"
  );
  const debates = debateInputs.map(compilePostCanaryBatch16FinalLedgerDebate);
  assertV4(
    debates.map((debate) => debate.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "post-canary Batch 16 final-ledger debate order invalid"
  );
  ...
  assertV4(aggregate.finalMoves === 194, "Batch 16 final-move population mismatch");
  assertV4(aggregate.disputedMoves === 185, "Batch 16 disputed-move population mismatch");
  assertV4(candidateSelections === 576, "Batch 16 candidate-selection population mismatch");
  assertV4(
    aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed === 417,
    `Batch 16 rounded-mean population mismatch: ${aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed}`
  );
  assertV4(aggregate.audioVerifiedMoves === 10, "Batch 16 audio-verification population mismatch");
  return {
    schemaVersion: POST_CANARY_BATCH_16_FINAL_LEDGER_VERSION,
    protocolId: POST_CANARY_BATCH_16_FINAL_LEDGER_PROTOCOL_ID,
    status: "passed-post-canary-batch-16-deterministic-final-ledger-assembly",
    ...
    AIOnly: true,
    scoringInputSchema:
      "v4.2.20-source-span-primary-with-post-canary-batch-16-adjudicated-provenance",
    ...
    nextAuthorizedAction:
      "standing-authorization-permits-batch-16-single-deterministic-score-pass-preparation"
  };
}

export function validatePostCanaryBatch16FinalLedger(
  ledger,
  debateInputs,
  sourceHashes
) {
  const expected = buildPostCanaryBatch16FinalLedger(debateInputs, sourceHashes);
  assertV4(
    canonicalJson(ledger) === canonicalJson(expected),
    "post-canary Batch 16 final ledger differs from deterministic replay"
  );
  assertV4(
    !containsProhibitedCalculatedField(
      ledger.debates.map((debate) => debate.finalJudgment)
    ),
    "post-canary Batch 16 final ledger contains a prohibited calculated field"
  );
  return {
    status: "passed",
...
export async function loadPostCanaryBatch16FinalLedgerInputs() {
  ...
  assertV4(
    adjudicationAnalysis.status ===
        "post-canary-batch-16-dispute-only-adjudication-gate-passed-standing-authorization-active-for-final-ledger-assembly" &&
      ...
      adjudicationAnalysis.gate.disputedMovesDecided === 185 &&
      adjudicationAnalysis.gate.candidateSelections === 576 &&
      adjudicationAnalysis.gate.scoresDerived === 0 &&
      adjudicationAnalysis.nextAuthorizedAction ===
        "standing-authorization-permits-batch-16-deterministic-final-ledger-assembly" &&
      adjudicationAnalysis.authorization.scoreDerivation === false &&
      adjudicationExecution.status ===
        "ten-post-canary-batch-16-dispute-only-adjudication-contexts-passed" &&
      ...
      standingAuthorization.status ===
        "frozen-active-batch-16-complete-remaining-workflow-standing-authorization" &&
      standingAuthorization.executionControls.scorePassesMaximum === 1 &&
      standingAuthorization.authorization.finalLedgerAssembly === true &&
      preparation.acceptedSourceBoundary.allTenAudioMovesVerified === true &&
      preparation.totals.audioVerifiedMoves === 10,
    "accepted post-canary Batch 16 adjudication gate unavailable"
  );
  const sourcePaths = [
  ...
  assertV4(
    debateInputs.map((input) => input.primaryA.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "loaded post-canary Batch 16 debate order invalid"
  );
