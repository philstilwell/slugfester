...
export const CHECKPOINT_V22_SCORE_ROOT =
  "docs/assessment-production/production-checkpoint-v2.2-1/score-pass";
export const CHECKPOINT_V22_CALCULATED_SCORES_VERSION =
  "1.0-production-checkpoint-v2.2-adjudicated-calculated-scores";
export const CHECKPOINT_V22_SCORE_STABILITY_THRESHOLDS =
  ACTIVE_SCORE_STABILITY_THRESHOLDS;
...
export function evaluateCheckpointV22ScoreStability(
  debates,
  thresholds = CHECKPOINT_V22_SCORE_STABILITY_THRESHOLDS
) {
  assertV4(
    Array.isArray(debates) && debates.length === 10,
    "ten scored production-checkpoint v2.2 debates required"
  );
  assertV4(
    canonicalJson(thresholds) ===
      canonicalJson(CHECKPOINT_V22_SCORE_STABILITY_THRESHOLDS),
    "production-checkpoint v2.2 active thresholds changed"
  );
...
export function deriveCheckpointV22Scores(
  ledger,
  debateInputs,
  ...
  assertV4(
    ledger?.status ===
        "passed-production-checkpoint-v2.2-deterministic-final-ledger-assembly" &&
      ledger.authorization.scoreDerivation &&
      ledger.authorization.scorePassesMaximum === 1 &&
      ledger.audit.calculatedScores === 0,
    "validated production-checkpoint v2.2 adjudicated final ledger required for scoring"
  );
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === 10,
    "ten production-checkpoint v2.2 score inputs required"
  );
  ...
  const debates = debateInputs.map((input) => {
    ...
    assertV4(
      finalLedgerDebate,
      `${debateNumber}: production-checkpoint v2.2 final ledger debate missing`
    );
  ...
  const stability = evaluateCheckpointV22ScoreStability(debates);
  ...
  return {
    schemaVersion: CHECKPOINT_V22_CALCULATED_SCORES_VERSION,
    ...
    status: stability.acceptancePassed
      ...
      : "production-checkpoint-v2.2-single-score-pass-stability-gate-failed",
...
export function validateCheckpointV22Scores(
  scores,
  ledger,
  debateInputs,
  productionReferences,
  ...
  const expected = deriveCheckpointV22Scores(
    ledger,
    debateInputs,
    productionReferences,
    hashes
  ...
  assertV4(
    ...
    "production-checkpoint v2.2 calculated scores differ from deterministic replay"
  );
  return {
    status: "passed",
