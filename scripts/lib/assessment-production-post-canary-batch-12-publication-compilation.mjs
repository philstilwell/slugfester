...
import { validatePostCanaryBatch12PublicationOutput } from
  "./assessment-production-post-canary-batch-12-publication-validation.mjs";
import { assertV4, canonicalJson } from "./v4-lean-production.mjs";

export const POST_CANARY_BATCH_12_PUBLICATION_COMPILATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-12/deterministic-publication-compilation";
export const POST_CANARY_BATCH_12_PUBLICATION_COMPILATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-12-deterministic-publication-compilation";
export const POST_CANARY_BATCH_12_PUBLICATION_COMPILATION_ORDER = Object.freeze(
  [...POST_CANARY_BATCH_12_PUBLICATION_DEBATES]);

export function compilePostCanaryBatch12PublicationStagingRecord({ output, packet, identity }) {
  validatePostCanaryBatch12PublicationOutput(output, packet);
  ...
  compiled.stagingAudit.protocolId = POST_CANARY_BATCH_12_PUBLICATION_COMPILATION_PROTOCOL_ID;
  compiled.stagingAudit.productionCanary = false;
  compiled.stagingAudit.batchNumber = 12;
  return compiled;
}

export function validatePostCanaryBatch12CompiledStagingRecord({ compiled, output, packet, identity }) {
  const expected = compilePostCanaryBatch12PublicationStagingRecord({ output, packet, identity });
