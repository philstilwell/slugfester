...
import { POST_CANARY_BATCH_07_PUBLICATION_RESUMPTION_ROOT } from
  "./assessment-production-post-canary-batch-07-publication-resumption.mjs";
import { assertV4, canonicalJson } from "./v4-lean-production.mjs";

export const POST_CANARY_BATCH_07_DEBATE_80_REPAIR_ROOT =
  `${POST_CANARY_BATCH_07_PUBLICATION_RESUMPTION_ROOT}/repair-1`;
export const POST_CANARY_BATCH_07_DEBATE_80_REPAIR_PROTOCOL_ID =
  "assessment-production-post-canary-batch-07-debate-80-publication-resumption-repair-1";
export const POST_CANARY_BATCH_07_DEBATE_80_REPAIR_PACKET_VERSION =
  "1.0-assessment-production-post-canary-batch-07-debate-80-publication-repair-packet";
export const POST_CANARY_BATCH_07_DEBATE_80_REPAIR_OUTPUT_VERSION =
  "1.0-assessment-production-post-canary-batch-07-debate-80-publication-repair-output";
...
export function debate80RepairMoveId(field) {
  const match = /^moveProse\.([^.]+)\.critique$/.exec(field);
  assertV4(match, `invalid Debate 80 repair field: ${field}`); return match[1];
}
export function buildDebate80RepairSchema(packet) {
  const properties = Object.fromEntries(packet.corrections.map(({ moveId }) =>
    [moveId, { type: "string", minLength: 880 }]));
  return { $schema: "https://json-schema.org/draft/2020-12/schema",
    $id: `slugfester-batch-07-debate-80-publication-repair-${packet.packetIndex}`,
    title: `Batch 7 Debate 80 publication repair packet ${packet.packetIndex}`,
    type: "object", additionalProperties: false,
    required: ["schemaVersion", "protocolId", "packetIndex", "debateNumber",
      "debateId", "assessmentModel", "completedAt", "correctedCritiques"],
    properties: { schemaVersion: { type: "string", const: POST_CANARY_BATCH_07_DEBATE_80_REPAIR_OUTPUT_VERSION },
      protocolId: { type: "string", const: POST_CANARY_BATCH_07_DEBATE_80_REPAIR_PROTOCOL_ID },
      packetIndex: { type: "integer", const: packet.packetIndex },
      debateNumber: { type: "string", const: "80" },
...
export function validateDebate80RepairOutput(output, packet) {
  ...
  assertV4(output.schemaVersion === POST_CANARY_BATCH_07_DEBATE_80_REPAIR_OUTPUT_VERSION &&
    output.protocolId === POST_CANARY_BATCH_07_DEBATE_80_REPAIR_PROTOCOL_ID &&
    output.packetIndex === packet.packetIndex && output.debateNumber === "80" &&
  ...
  return { status: "passed", debateNumber: "80", packetIndex: packet.packetIndex,
    correctedFields, modelAuthoredScores: 0 };
}
function withMarkers(output, fields) {
  const copy = structuredClone(output);
  for (const field of fields) copy.moveProse[debate80RepairMoveId(field)].critique = "__AUTHORIZED_REPAIR_FIELD__";
  return copy;
}
export function mergeAndValidateDebate80Repair({ baseOutput, repairOutputs, repairPackets, publicationPacket }) {
  assertV4(repairOutputs.length === 3 && repairPackets.length === 3,
    "three Debate 80 repair packets are required");
  const merged = structuredClone(baseOutput);
  const fields = repairPackets.flatMap((packet) => packet.corrections.map(({ field }) => field));
  assertV4(fields.length === 5 && new Set(fields).size === 5, "the five repair fields changed");
  const transformations = [];
  for (let index = 0; index < 3; index += 1) {
    const repair = repairOutputs[index]; const packet = repairPackets[index];
    validateDebate80RepairOutput(repair, packet);
    for (const correction of packet.corrections) {
  ...
  assertV4(canonicalJson(withMarkers(merged, fields)) === canonicalJson(withMarkers(baseOutput, fields)),
    "repair merge changed a field outside the five-field authorization");
