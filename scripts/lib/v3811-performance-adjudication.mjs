...
import { V3811_PERFORMANCE_ROOT, V3811_RATING_KEYS, assertV3811, canonicalJson, containsProhibitedDerivedField } from "./v3811-performance-judgment.mjs";

export const V3811_ADJUDICATION_ROOT = `${V3811_PERFORMANCE_ROOT}/adjudication`;
...
export function makeV3811AdjudicationSchema() {
  return {
    ...
    properties: {
      schemaVersion: { type: "string", enum: ["3.8.11-performance-adjudication-output"] },
      protocolId: { type: "string", enum: ["v3.8.11-performance-judgment-consensus"] },
          ...
          properties: {
            ...
            ratingChoices: {
              ...
              items: {
                ...
                properties: {
                  ratingKey: { type: "string", enum: V3811_RATING_KEYS },
...
function objectShape(value, keys, label) {
  assertV3811(value && typeof value === "object" && !Array.isArray(value), `${label} must be an object`);
  assertV3811(canonicalJson(Object.keys(value).sort()) === canonicalJson([...keys].sort()), `${label} keys mismatch`);
}

function validateChoice(value, required, label) {
  if (required) assertV3811(value === 1 || value === 2, `${label} must select candidate 1 or 2`);
  else assertV3811(value === null, `${label} must be null for a nondisputed field`);
}

export function validateV3811AdjudicationOutput(output, packet) {
  const schema = makeV3811AdjudicationSchema();
  objectShape(output, schema.required, "output");
  assertV3811(output.schemaVersion === "3.8.11-performance-adjudication-output" && output.protocolId === "v3.8.11-performance-judgment-consensus", "adjudication identity mismatch");
  assertV3811(output.debateNumber === packet.debateNumber && output.debateId === packet.debateId, "debate identity mismatch");
  assertV3811(output.reviewerRole === "performance-adjudicator" && output.assessmentModel === "5.6 Sol" && output.calibrationOnly, "adjudicator role mismatch");
  assertV3811(!containsProhibitedDerivedField(output), "adjudication output contains prohibited score or publication prose fields");
  objectShape(output.isolation, Object.keys(schema.properties.isolation.properties), "isolation");
  for (const [key, definition] of Object.entries(schema.properties.isolation.properties)) assertV3811(output.isolation[key] === definition.enum[0], `isolation.${key} mismatch`);
  assertV3811(Array.isArray(output.moveDecisions) && output.moveDecisions.length === packet.disputedMoves.length, "move decision count mismatch");
  assertV3811(new Set(output.moveDecisions.map((item) => item.moveId)).size === output.moveDecisions.length, "move decisions must be unique");
  ...
  for (let index = 0; index < packet.disputedMoves.length; index += 1) {
    ...
    assertV3811(decision.moveId === required.moveId, `${label}: move order or identity mismatch`);
    ...
    assertV3811(Array.isArray(decision.ratingChoices), `${label}.ratingChoices must be an array`);
    ...
    assertV3811(canonicalJson(actualKeys) === canonicalJson(expectedKeys) && new Set(actualKeys).size === actualKeys.length, `${label}: rating choice keys mismatch`);
    for (const item of decision.ratingChoices) {
      objectShape(item, ["ratingKey", "choice"], `${label}.ratingChoices.${item.ratingKey}`);
      assertV3811(item.choice === 1 || item.choice === 2, `${label}.${item.ratingKey}: invalid candidate choice`);
    }
    assertV3811(typeof decision.rationale === "string" && decision.rationale.trim().length >= 40, `${label}.rationale too short`);
  }

  assertV3811(Array.isArray(output.burdenAdjustmentDecisions) && output.burdenAdjustmentDecisions.length === packet.burdenAdjustmentDisputes.length, "burden adjustment decision count mismatch");
  assertV3811(new Set(output.burdenAdjustmentDecisions.map((item) => item.side)).size === output.burdenAdjustmentDecisions.length, "burden adjustment decisions must be unique");
  for (let index = 0; index < packet.burdenAdjustmentDisputes.length; index += 1) {
    ...
    assertV3811(decision.side === required.side && (decision.choice === 1 || decision.choice === 2), `burdenAdjustmentDecisions[${index}] mismatch`);
    assertV3811(typeof decision.rationale === "string" && decision.rationale.trim().length >= 40, `burdenAdjustmentDecisions[${index}].rationale too short`);
  }

  objectShape(output.audit, Object.keys(schema.properties.audit.properties), "audit");
  assertV3811(Object.values(output.audit).every((value) => value === true), "adjudication audit flags must all be true");
  return {
    status: "passed",
...
export async function readV3811AdjudicationJson(relativePath, cwd = process.cwd()) {
