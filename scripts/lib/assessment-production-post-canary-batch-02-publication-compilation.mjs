...
export const POST_CANARY_BATCH_02_PUBLICATION_COMPILATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-02/deterministic-publication-compilation";
export const POST_CANARY_BATCH_02_PUBLICATION_COMPILATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-02-deterministic-publication-compilation";
export const POST_CANARY_BATCH_02_PUBLICATION_COMPILATION_ORDER = Object.freeze(
  [...POST_CANARY_BATCH_02_PUBLICATION_DEBATES]
);
...
function referenceUrl(tag) {
  ...
  assertV4(
    ...
    `unknown Batch 2 publication reference ${tag.type}:${tag.slug}`
  );
...
export function compilePostCanaryBatch02PublicationStagingRecord({
  output,
  packet,
  identity
}) {
  ...
  const record = {
    ...
    assessmentModel: POST_CANARY_BATCH_02_PUBLICATION_MODEL.label,
...
export function validatePostCanaryBatch02CompiledStagingRecord({
  compiled,
  output,
  packet,
  identity
}) {
  const expected = compilePostCanaryBatch02PublicationStagingRecord({
    output,
    packet,
    identity
  });
