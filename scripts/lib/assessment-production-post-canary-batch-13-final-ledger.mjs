...
import { extractAssessmentProductionPostCanaryBatch13Disagreements } from "./assessment-production-post-canary-batch-13-disagreement.mjs";
import {
  buildPostCanaryBatch13DisputeAdjudicationPacket,
  validatePostCanaryBatch13DisputeAdjudicationOutput
} from "./assessment-production-post-canary-batch-13-dispute-adjudication.mjs";

export const POST_CANARY_BATCH_13_FINAL_LEDGER_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-13/final-ledger";
export const POST_CANARY_BATCH_13_FINAL_LEDGER_PROTOCOL_ID =
  "assessment-production-post-canary-batch-13-adjudicated-consensus";
export const POST_CANARY_BATCH_13_FINAL_LEDGER_VERSION =
  "1.0-assessment-production-post-canary-batch-13-adjudicated-raw-ledger";
...
function validateInputReplay(input) {
  ...
  const replayedDisagreements =
    extractAssessmentProductionPostCanaryBatch13Disagreements(
      primaryA,
      primaryB,
      lockedInventory
    );
  assertV4(
    ...
    `${primaryA.debateNumber}: stored disagreements differ from deterministic Batch 13 replay`
  );
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication packet differs from deterministic Batch 13 replay`
  );
  assertV4(
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication provenance differs from deterministic Batch 13 replay`
  );
  validatePostCanaryBatch13DisputeAdjudicationOutput(
    adjudicationOutput,
    adjudicationPacket
...
export function compilePostCanaryBatch13FinalLedgerDebate(input) {
  const {
    primaryA,
    primaryB,
    adjudicationPacket,
...
export function buildPostCanaryBatch13FinalLedger(debateInputs, sourceHashes) {
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === EXPECTED_DEBATES.length,
    "ten post-canary Batch 13 debate inputs required"
  );
  const debates = debateInputs.map(compilePostCanaryBatch13FinalLedgerDebate);
  assertV4(
    debates.map((debate) => debate.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "post-canary Batch 13 final-ledger debate order invalid"
  );
  ...
  assertV4(aggregate.finalMoves === 199, "Batch 13 final-move population mismatch");
  assertV4(aggregate.disputedMoves === 191, "Batch 13 disputed-move population mismatch");
  assertV4(candidateSelections === 541, "Batch 13 candidate-selection population mismatch");
  assertV4(
    aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed === 440,
    `Batch 13 rounded-mean population mismatch: ${aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed}`
  );
  assertV4(aggregate.audioVerifiedMoves === 8, "Batch 13 audio-verification population mismatch");
  return {
    schemaVersion: POST_CANARY_BATCH_13_FINAL_LEDGER_VERSION,
    protocolId: POST_CANARY_BATCH_13_FINAL_LEDGER_PROTOCOL_ID,
    status: "passed-post-canary-batch-13-deterministic-final-ledger-assembly",
    ...
    AIOnly: true,
    scoringInputSchema:
      "v4.2.20-source-span-primary-with-post-canary-batch-13-adjudicated-provenance",
...
export function validatePostCanaryBatch13FinalLedger(
  ledger,
  debateInputs,
  sourceHashes
) {
  const expected = buildPostCanaryBatch13FinalLedger(debateInputs, sourceHashes);
  assertV4(
    canonicalJson(ledger) === canonicalJson(expected),
    "post-canary Batch 13 final ledger differs from deterministic replay"
  );
  assertV4(
    !containsProhibitedCalculatedField(
      ledger.debates.map((debate) => debate.finalJudgment)
    ),
    "post-canary Batch 13 final ledger contains a prohibited calculated field"
  );
  return {
    status: "passed",
...
export async function loadPostCanaryBatch13FinalLedgerInputs() {
  ...
  assertV4(
    adjudicationAnalysis.status ===
        "post-canary-batch-13-dispute-only-adjudication-gate-passed-standing-authorization-active-for-final-ledger-assembly" &&
      ...
      adjudicationAnalysis.gate.disputedMovesDecided === 191 &&
      adjudicationAnalysis.gate.candidateSelections === 541 &&
      adjudicationAnalysis.gate.scoresDerived === 0 &&
      adjudicationAnalysis.nextAuthorizedAction ===
        "standing-authorization-permits-batch-13-deterministic-final-ledger-assembly" &&
      adjudicationAnalysis.authorization.scoreDerivation === false &&
      adjudicationExecution.status ===
        "ten-post-canary-batch-13-dispute-only-adjudication-contexts-passed" &&
      ...
      preparation.acceptedSourceBoundary.allEightAudioMovesVerified === true &&
      preparation.totals.audioVerifiedMoves === 8,
    "accepted post-canary Batch 13 adjudication gate unavailable"
  );
  const sourcePaths = [
  ...
  assertV4(
    debateInputs.map((input) => input.primaryA.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "loaded post-canary Batch 13 debate order invalid"
  );
