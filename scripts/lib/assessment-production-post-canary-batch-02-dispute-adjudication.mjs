...
export const POST_CANARY_BATCH_02_DISPUTE_ADJ_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-02/dispute-only-adjudication";
export const POST_CANARY_BATCH_02_DISPUTE_ADJ_PROTOCOL_ID =
  "assessment-production-post-canary-batch-02-dispute-only-adjudication";
export const POST_CANARY_BATCH_02_DISPUTE_ADJ_PACKET_VERSION =
  "1.0-assessment-production-post-canary-batch-02-dispute-only-adjudication-packet";
export const POST_CANARY_BATCH_02_DISPUTE_ADJ_OUTPUT_VERSION =
  "1.0-assessment-production-post-canary-batch-02-dispute-only-adjudication-output";

const clone = (value) => structuredClone(value);

export function buildPostCanaryBatch02DisputeAdjudicationPacket(
  disagreements,
  lockedInventory,
  events,
  ...
  built.packet.schemaVersion = POST_CANARY_BATCH_02_DISPUTE_ADJ_PACKET_VERSION;
  built.packet.protocolId = POST_CANARY_BATCH_02_DISPUTE_ADJ_PROTOCOL_ID;
  built.packet.productionCanary = false;
  built.packet.batchNumber = 2;
  built.packet.stagingOnly = true;
  built.packet.developmentValidationOnly = false;
  return built;
}

export function makePostCanaryBatch02DisputeAdjudicationSchema() {
  const schema = makeV42211728AdjudicationSchema();
  schema.$id =
    "slugfester-assessment-production-post-canary-batch-02-dispute-only-adjudication";
  schema.title = "Slugfester post-canary Batch 2 dispute-only adjudication";
  ...
  schema.properties.batchNumber = { type: "integer", const: 2 };
...
export function validatePostCanaryBatch02DisputeAdjudicationOutput(
  output,
  packet
) {
...
export {
  V42211728_ADJUDICATION_ISOLATION as POST_CANARY_BATCH_02_DISPUTE_ADJ_ISOLATION
};
