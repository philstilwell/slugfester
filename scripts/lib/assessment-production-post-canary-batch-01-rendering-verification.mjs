...
} from "./assessment-production-post-canary-batch-01-publication-finalization.mjs";
import {
  POST_CANARY_BATCH_01_PUBLICATION_BYLINE,
...
export const POST_CANARY_BATCH_01_RENDERING_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-01/rendering-verification";
export const POST_CANARY_BATCH_01_RENDERING_PROTOCOL_ID =
  "assessment-production-post-canary-batch-01-rendering-verification";
export const POST_CANARY_BATCH_01_RENDERING_PORT = 4198;
export const POST_CANARY_BATCH_01_RENDERING_ORDER =
  POST_CANARY_BATCH_01_PUBLICATION_FINALIZATION_ORDER;
export const POST_CANARY_BATCH_01_RENDERING_MODEL =
  POST_CANARY_BATCH_01_PUBLICATION_MODEL;
export const POST_CANARY_BATCH_01_RENDERING_BYLINE =
  POST_CANARY_BATCH_01_PUBLICATION_BYLINE;
export const POST_CANARY_BATCH_01_RENDERING_IMAGE_CONTRACT =
  CHECKPOINT_V22_RENDERING_REMEDY_V9_IMAGE_CONTRACT;
export const POST_CANARY_BATCH_01_RENDERING_REQUIRED_BOOLEAN_CHECKS =
  CHECKPOINT_V22_RENDERING_REMEDY_V9_REQUIRED_BOOLEAN_CHECKS;
export const POST_CANARY_BATCH_01_RENDERING_VIEWPORTS =
  CHECKPOINT_V22_RENDERING_REMEDY_V9_VIEWPORTS;

function evidencePaths(debateNumber, viewportName) {
  const root = `${POST_CANARY_BATCH_01_RENDERING_ROOT}/evidence/debate-${debateNumber}`;
  return {
...
export function buildPostCanaryBatch01RenderingPacket({
  auditRow,
  candidate,
  provenance,
  previewPath,
  ...
  assertV4(
    auditRow &&
      ...
      provenance.model.label === POST_CANARY_BATCH_01_RENDERING_MODEL.label &&
      provenance.model.reasoningEffort ===
        POST_CANARY_BATCH_01_RENDERING_MODEL.reasoningEffort &&
      provenance.model.authentication ===
        POST_CANARY_BATCH_01_RENDERING_MODEL.authentication &&
      ...
      provenance.displayContract.byline === POST_CANARY_BATCH_01_RENDERING_BYLINE &&
      provenance.displayContract.defaultCollapsed === true,
    `${auditRow?.debateNumber ?? "unknown"}: Batch 1 finalization provenance changed`
  );
  const newArguments = {
  ...
  const packet = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-01-rendering-verification-packet",
    protocolId: POST_CANARY_BATCH_01_RENDERING_PROTOCOL_ID,
    status: "frozen-batch-01-rendering-verification-packet",
    ...
    preview: {
      ...
      url:
        `http://127.0.0.1:${POST_CANARY_BATCH_01_RENDERING_PORT}/` +
        `${previewPath}?debate=${auditRow.debateNumber}`,
      localOnly: true,
      noindex: true
    },
    expectedDisplay: {
      byline: POST_CANARY_BATCH_01_RENDERING_BYLINE,
      stagingBannerPrefix: "Publication staging preview:",
      stagingBannerRequiredText: "validated post-canary Batch 1 candidate only",
    ...
    viewports: Object.fromEntries(
      Object.entries(POST_CANARY_BATCH_01_RENDERING_VIEWPORTS).map(
        ([name, viewport]) => [
          name,
          {
    ...
    runnerPolicy: {
      ...
      imageAnalyzerPath: POST_CANARY_BATCH_01_RENDERING_IMAGE_CONTRACT.analyzerPath,
      imageAnalyzerSha256:
  ...
  return validatePostCanaryBatch01RenderingPacket(packet);
}

export function validatePostCanaryBatch01RenderingPacket(packet) {
  assertV4(
    packet?.schemaVersion ===
        "1.0-assessment-production-post-canary-batch-01-rendering-verification-packet" &&
      packet.protocolId === POST_CANARY_BATCH_01_RENDERING_PROTOCOL_ID &&
      packet.status === "frozen-batch-01-rendering-verification-packet" &&
      packet.productionCanary === false &&
      packet.batchNumber === 1 &&
      packet.stagingOnly === true &&
      POST_CANARY_BATCH_01_RENDERING_ORDER.includes(packet.debateNumber) &&
      ...
      packet.expectedDisplay.byline === POST_CANARY_BATCH_01_RENDERING_BYLINE &&
      packet.expectedDisplay.defaultCollapsed === true &&
      packet.provenance.assessmentModel ===
        POST_CANARY_BATCH_01_RENDERING_MODEL.label &&
    ...
    `${packet?.debateNumber ?? "unknown"}: invalid Batch 1 rendering packet`
  );
    ...
    assertV4(
      canonicalJson(viewport.targetCssViewport) ===
        ...
        viewport.evidence.result ===
          `${POST_CANARY_BATCH_01_RENDERING_ROOT}/evidence/debate-${packet.debateNumber}/${name}.json`,
      `${packet.debateNumber}: invalid ${name} Batch 1 viewport`
    );
  }
  return packet;
}
...
function imagePasses(image, viewport) {
  ...
  const contract = POST_CANARY_BATCH_01_RENDERING_IMAGE_CONTRACT;
  return (
...
export function validatePostCanaryBatch01RenderingViewportEvidence({
  packet,
  viewportName,
  activationNavigationToken,
  evidence
}) {
  validatePostCanaryBatch01RenderingPacket(packet);
  ...
  assertV4(
    viewport &&
      evidence?.schemaVersion ===
        "1.0-assessment-production-post-canary-batch-01-rendering-viewport-evidence" &&
    ...
    `${packet?.debateNumber ?? "unknown"}: invalid ${viewportName} Batch 1 rendering evidence`
  );
  return evidence;
}
