import { POST_CANARY_BATCH_17_PUBLICATION_BYLINE,
  POST_CANARY_BATCH_17_PUBLICATION_MODEL } from
  "./assessment-production-post-canary-batch-17-publication.mjs";
import { POST_CANARY_BATCH_17_PUBLICATION_COMPILATION_ORDER,
  POST_CANARY_BATCH_17_PUBLICATION_COMPILATION_PROTOCOL_ID,
  validatePostCanaryBatch17CompiledStagingRecord } from
  "./assessment-production-post-canary-batch-17-publication-compilation.mjs";
import { assertV4, canonicalJson } from "./v4-lean-production.mjs";

export const POST_CANARY_BATCH_17_PUBLICATION_FINALIZATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-17/publication-finalization";
export const POST_CANARY_BATCH_17_PUBLICATION_FINALIZATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-17-publication-finalization";
export const POST_CANARY_BATCH_17_PUBLICATION_FINALIZATION_ORDER =
  POST_CANARY_BATCH_17_PUBLICATION_COMPILATION_ORDER;

export function buildPostCanaryBatch17PublicationFinalization({ compiled, compiledPath,
  compiledSha256, output, packet, identity }) {
  const compilationValidation = validatePostCanaryBatch17CompiledStagingRecord({
    compiled, output, packet, identity });
  assertV4(compiled.stagingAudit?.protocolId === POST_CANARY_BATCH_17_PUBLICATION_COMPILATION_PROTOCOL_ID &&
    compiled.stagingAudit.productionCanary === false && compiled.stagingAudit.batchNumber === 17 &&
    compiled.stagingAudit.stagingOnly === true && compiled.stagingAudit.productionMutationPerformed === false &&
    compiled.stagingAudit.displayContract.byline === POST_CANARY_BATCH_17_PUBLICATION_BYLINE &&
  ...
  const provenance = { schemaVersion: "1.0-assessment-production-post-canary-batch-17-publication-finalization-provenance",
    protocolId: POST_CANARY_BATCH_17_PUBLICATION_FINALIZATION_PROTOCOL_ID,
    status: "staging-only-batch-17-final-candidate-provenance",
    productionCanary: false, batchNumber: 17, debateNumber: compiled.number,
    ...
    model: { ...POST_CANARY_BATCH_17_PUBLICATION_MODEL,
...
export function validatePostCanaryBatch17PublicationFinalCandidate({ candidate, provenance,
  compiled, output, packet, identity }) {
  const expected = buildPostCanaryBatch17PublicationFinalization({ compiled,
...
export function buildPostCanaryBatch17PublicationStagingPreviewHtml() {
  const allowed = JSON.stringify(POST_CANARY_BATCH_17_PUBLICATION_FINALIZATION_ORDER);
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8">
    ...
    <title>Slugfester Batch 17 publication staging preview</title>
  ...
  <body>
    ...
    <script type="module">
      ...
      const debateNumber = params.get("debate") || "77";
      if (!allowedHosts.has(window.location.hostname)) {
        ...
        document.querySelector("#app").innerHTML = "<main><h1>Unknown Batch 17 publication-staging debate</h1></main>";
      } else {
