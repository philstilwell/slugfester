...
export const POST_CANARY_BATCH_04_COMPATIBILITY_ACTIVATION_STATUS =
...
export function buildPostCanaryBatch04ValidatorSource(baselineSource) {
  ...
  source = replaceExactOnce(
    ...
    "Batch 4 adapter-version route"
  );

  const routeFunctions = `export function validatePostCanaryBatch04LedgerAdapterRouteLocks({
  debate,
  ledgerText,
  packetPath,
  packetText,
...
`;
  source = replaceExactOnce(
    ...
    "Batch 4 route functions"
  );

  const batch04Branch = `  if (
...
`;
  source = replaceExactOnce(
    source,
    "  let calculated;\n",
    `${batch04Branch}  let calculated;\n`,
    "Batch 4 ledger branch"
  );

  validatePostCanaryBatch04ValidatorSource(source);
  return source;
}

export function validatePostCanaryBatch04ValidatorSource(source) {
  const required = [
    "POST_CANARY_BATCH_04_COMPATIBILITY_ROOT",
    "POST_CANARY_BATCH_04_SITE_LEDGER_ADAPTER_VERSION",
    "validatePostCanaryBatch04SiteLedgerAdapter",
    ...
    "validatePostCanaryBatch04LedgerAdapterRoute",
    POST_CANARY_BATCH_04_COMPATIBILITY_ACTIVATION_STATUS,
    "Batch 4 production assessments require an AI Extension",
    "Batch 4 ledger validation failed"
  ];
  for (const text of required) {
    if (!source.includes(text)) {
      throw new Error(`proposed Batch 4 validator is missing ${text}`);
    }
  }
  if (
    ...
    throw new Error("proposed Batch 4 validator weakens an existing route");
  }
  return {
    status: "passed",
