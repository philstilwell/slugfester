...
import {
  POST_CANARY_BATCH_11_PUBLICATION_FINALIZATION_ORDER
} from "./assessment-production-post-canary-batch-11-publication-finalization.mjs";
import {
  POST_CANARY_BATCH_11_PUBLICATION_BYLINE,
  POST_CANARY_BATCH_11_PUBLICATION_MODEL
} from "./assessment-production-post-canary-batch-11-publication.mjs";
import { assertV4, canonicalJson } from "./v4-lean-production.mjs";

export const POST_CANARY_BATCH_11_RENDERING_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-11/rendering-verification";
export const POST_CANARY_BATCH_11_RENDERING_PROTOCOL_ID =
  "assessment-production-post-canary-batch-11-rendering-verification";
export const POST_CANARY_BATCH_11_RENDERING_PORT = 4207;
export const POST_CANARY_BATCH_11_RENDERING_ORDER =
  POST_CANARY_BATCH_11_PUBLICATION_FINALIZATION_ORDER;
export const POST_CANARY_BATCH_11_RENDERING_BYLINE =
  POST_CANARY_BATCH_11_PUBLICATION_BYLINE;
export const POST_CANARY_BATCH_11_RENDERING_MODEL =
  POST_CANARY_BATCH_11_PUBLICATION_MODEL;
export const POST_CANARY_BATCH_11_RENDERING_VIEWPORTS = Object.freeze({
  desktop: Object.freeze({ width: 1440, height: 1000 }),
  mobile: Object.freeze({ width: 390, height: 844 })
});
export const POST_CANARY_BATCH_11_RENDERING_REQUIRED_CHECKS = Object.freeze([
  "exactViewport",
...
export function renderingEvidencePaths(debateNumber, viewportName) {
  const screenshots = `output/playwright/batch-11-rendering/debate-${debateNumber}`;
  return {
    result: `${POST_CANARY_BATCH_11_RENDERING_ROOT}/evidence/debate-${debateNumber}/${viewportName}.json`,
    collapsedScreenshot: `${screenshots}/${viewportName}-collapsed.jpg`,
    openScreenshot: `${screenshots}/${viewportName}-open.jpg`
  };
}

export function buildPostCanaryBatch11RenderingPacket({
  auditRow,
  candidate,
  provenance,
  previewPath,
  ...
  assertV4(candidate.number === auditRow.debateNumber &&
    ...
    provenance.model.label === POST_CANARY_BATCH_11_RENDERING_MODEL.label &&
    provenance.model.reasoningEffort === POST_CANARY_BATCH_11_RENDERING_MODEL.reasoningEffort &&
    provenance.displayContract.byline === POST_CANARY_BATCH_11_RENDERING_BYLINE &&
    provenance.displayContract.defaultCollapsed === true,
  `${auditRow.debateNumber}: finalization provenance changed`);
  const packet = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-11-rendering-packet",
    protocolId: POST_CANARY_BATCH_11_RENDERING_PROTOCOL_ID,
    status: "frozen-batch-11-rendering-verification-packet",
    ...
    preview: {
      ...
      url: `http://127.0.0.1:${POST_CANARY_BATCH_11_RENDERING_PORT}/${previewPath}?debate=${auditRow.debateNumber}`,
      localOnly: true,
      noindex: true
    },
    expectedDisplay: {
      byline: POST_CANARY_BATCH_11_RENDERING_BYLINE,
      stagingBanner: "Publication staging preview: validated post-canary Batch 11 candidate only. This scorecard remains excluded from production data and rankings pending rendering and mutation authorization.",
    ...
    viewports: Object.fromEntries(Object.entries(POST_CANARY_BATCH_11_RENDERING_VIEWPORTS)
  ...
  return validatePostCanaryBatch11RenderingPacket(packet);
}

export function validatePostCanaryBatch11RenderingPacket(packet) {
  assertV4(packet?.schemaVersion ===
    "1.0-assessment-production-post-canary-batch-11-rendering-packet" &&
    packet.protocolId === POST_CANARY_BATCH_11_RENDERING_PROTOCOL_ID &&
    packet.status === "frozen-batch-11-rendering-verification-packet" &&
    packet.batchNumber === 11 && packet.productionCanary === false &&
    packet.stagingOnly === true &&
    POST_CANARY_BATCH_11_RENDERING_ORDER.includes(packet.debateNumber) &&
    packet.preview.url.startsWith(`http://127.0.0.1:${POST_CANARY_BATCH_11_RENDERING_PORT}/`) &&
    packet.expectedDisplay.byline === POST_CANARY_BATCH_11_RENDERING_BYLINE &&
    ...
    canonicalJson(packet.requiredChecks) ===
      canonicalJson(POST_CANARY_BATCH_11_RENDERING_REQUIRED_CHECKS) &&
  ...
  `${packet?.debateNumber ?? "unknown"}: invalid Batch 11 rendering packet`);
  for (const [name, viewport] of Object.entries(POST_CANARY_BATCH_11_RENDERING_VIEWPORTS)) {
...
export function validatePostCanaryBatch11RenderingEvidence({ packet, viewportName, token, evidence }) {
  validatePostCanaryBatch11RenderingPacket(packet);
  const viewport = packet.viewports[viewportName];
  assertV4(evidence?.schemaVersion ===
    "1.0-assessment-production-post-canary-batch-11-rendering-evidence" &&
