...
export const POST_CANARY_BATCH_06_SCORE_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-06/score-pass";
export const POST_CANARY_BATCH_06_CALCULATED_SCORES_VERSION =
  "1.0-assessment-production-post-canary-batch-06-adjudicated-calculated-scores";
export const POST_CANARY_BATCH_06_SCORE_STABILITY_THRESHOLDS =
  ACTIVE_SCORE_STABILITY_THRESHOLDS;
...
export function evaluatePostCanaryBatch06ScoreStability(
  debates,
  thresholds = POST_CANARY_BATCH_06_SCORE_STABILITY_THRESHOLDS
) {
  assertV4(
    Array.isArray(debates) && debates.length === 10,
    "ten scored post-canary Batch 6 debates required"
  );
  assertV4(
    canonicalJson(thresholds) ===
      canonicalJson(POST_CANARY_BATCH_06_SCORE_STABILITY_THRESHOLDS),
    "post-canary Batch 6 active thresholds changed"
  );
...
export function derivePostCanaryBatch06Scores(
  ledger,
  debateInputs,
  ...
  const debates = debateInputs.map((input) => {
    ...
    assertV4(
      finalLedgerDebate,
      `${debateNumber}: Batch 6 final-ledger debate missing`
    );
  ...
  const stability = evaluatePostCanaryBatch06ScoreStability(debates);
  ...
  return {
    schemaVersion: POST_CANARY_BATCH_06_CALCULATED_SCORES_VERSION,
    ...
    status: stability.acceptancePassed
      ...
      : "post-canary-batch-06-single-score-pass-stability-gate-failed",
...
export function validatePostCanaryBatch06Scores(
  scores,
  ledger,
  debateInputs,
  productionReferences,
  ...
  const expected = derivePostCanaryBatch06Scores(
    ledger,
    debateInputs,
    productionReferences,
    hashes
  ...
  assertV4(
    ...
    "post-canary Batch 6 calculated scores differ from deterministic replay"
  );
  return {
    status: "passed",
