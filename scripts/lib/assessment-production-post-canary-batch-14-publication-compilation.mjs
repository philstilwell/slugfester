...
import { validatePostCanaryBatch14PublicationOutput } from
  "./assessment-production-post-canary-batch-14-publication-validation.mjs";
import { assertV4, canonicalJson } from "./v4-lean-production.mjs";

export const POST_CANARY_BATCH_14_PUBLICATION_COMPILATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-14/deterministic-publication-compilation";
export const POST_CANARY_BATCH_14_PUBLICATION_COMPILATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-14-deterministic-publication-compilation";
export const POST_CANARY_BATCH_14_PUBLICATION_COMPILATION_ORDER = Object.freeze(
  [...POST_CANARY_BATCH_14_PUBLICATION_DEBATES]);

export function compilePostCanaryBatch14PublicationStagingRecord({ output, packet, identity }) {
  validatePostCanaryBatch14PublicationOutput(output, packet);
  ...
  compiled.stagingAudit.protocolId = POST_CANARY_BATCH_14_PUBLICATION_COMPILATION_PROTOCOL_ID;
  compiled.stagingAudit.productionCanary = false;
  compiled.stagingAudit.batchNumber = 14;
  return compiled;
}

export function validatePostCanaryBatch14CompiledStagingRecord({ compiled, output, packet, identity }) {
  const expected = compilePostCanaryBatch14PublicationStagingRecord({ output, packet, identity });
