...
export const POST_CANARY_BATCH_06_COMPATIBILITY_ACTIVATION_STATUS =
...
export function buildPostCanaryBatch06ValidatorSource(baselineSource) {
  ...
  source = replaceExactOnce(
    ...
    "Batch 6 adapter-version route"
  );

  const routeFunctions = `export function validatePostCanaryBatch06LedgerAdapterRouteLocks({
  debate,
  ledgerText,
  packetPath,
  packetText,
...
`;
  source = replaceExactOnce(
    ...
    "Batch 6 route functions"
  );

  const batch06Branch = `  if (
...
`;
  source = replaceExactOnce(
    source,
    "  let calculated;\n",
    `${batch06Branch}  let calculated;\n`,
    "Batch 6 ledger branch"
  );

  validatePostCanaryBatch06ValidatorSource(source);
  return source;
}

export function validatePostCanaryBatch06ValidatorSource(source) {
  const required = [
    "POST_CANARY_BATCH_06_COMPATIBILITY_ROOT",
    "POST_CANARY_BATCH_06_SITE_LEDGER_ADAPTER_VERSION",
    "validatePostCanaryBatch06SiteLedgerAdapter",
    ...
    "validatePostCanaryBatch06LedgerAdapterRoute",
    POST_CANARY_BATCH_06_COMPATIBILITY_ACTIVATION_STATUS,
    "Batch 6 production assessments require an AI Extension",
    "Batch 6 ledger validation failed"
  ];
  for (const text of required) {
    if (!source.includes(text)) {
      throw new Error(`proposed Batch 6 validator is missing ${text}`);
    }
  }
  if (
    ...
    throw new Error("proposed Batch 6 validator weakens an existing route");
  }
  return {
    status: "passed",
