...
export const POST_CANARY_BATCH_15_COMPATIBILITY_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-15/production-compatibility";
export const POST_CANARY_BATCH_15_COMPATIBILITY_PROTOCOL_ID =
  "assessment-production-post-canary-batch-15-production-compatibility";
export const POST_CANARY_BATCH_15_SITE_LEDGER_ADAPTER_VERSION =
  "1.0-assessment-production-post-canary-batch-15-site-ledger-adapter";
export const POST_CANARY_BATCH_15_COMPATIBILITY_ORDER = Object.freeze([
  "39",
  "48",
  "23",
  "162",
...
export function postCanaryBatch15SectionTitleMatches({
...
export function validatePostCanaryBatch15CandidateAgainstScores(
  candidate,
  finalScores
) {
  assertV4(
    ...
    `${finalScores.debateNumber}: Batch 15 publication candidate identity mismatch`
  );
  assertV4(
    ...
    `${finalScores.debateNumber}: Batch 15 publication candidate overall scores changed`
  );
  assertV4(
    candidate.sections?.length === finalScores.sections.length,
    `${finalScores.debateNumber}: Batch 15 publication candidate section count changed`
  );
  let moveCount = 0;
  finalScores.sections.forEach((scoredSection, sectionIndex) => {
    ...
    assertV4(
      candidateSection?.sectionId === scoredSection.sectionId &&
        postCanaryBatch15SectionTitleMatches({
      ...
      `${finalScores.debateNumber}: Batch 15 section ${sectionIndex} identity or score changed`
    );
    for (const side of ["pro", "con"]) {
      ...
      assertV4(
        ...
        `${finalScores.debateNumber}: Batch 15 section ${sectionIndex} ${side} move IDs, order, or scores changed`
      );
...
export function buildPostCanaryBatch15SiteLedgerAdapter({
  finalLedgerDebate,
  scoreDebate,
  candidate,
  eventsDocument,
  ...
  assertV4(
    ...
    "Batch 15 final-ledger and score identities differ"
  );
  ...
  assertV4(
    ...
    `${scoreDebate.debateNumber}: locked Batch 15 score is not an exact repository replay`
  );
  const candidateValidation =
    validatePostCanaryBatch15CandidateAgainstScores(candidate, {
      ...scoreDebate.final,
  ...
  return {
    ...
    status: "frozen-post-canary-batch-15-site-ledger-adapter",
...
export function validatePostCanaryBatch15SiteLedgerAdapter({
  adapter,
  candidate,
  expectedSourceLocks
}) {
  assertV4(
    adapter?.schemaVersion ===
      ...
      adapter.protocolId === POST_CANARY_BATCH_15_COMPATIBILITY_PROTOCOL_ID &&
      adapter.status ===
        "frozen-post-canary-batch-15-site-ledger-adapter" &&
      adapter.productionCanary === false &&
      adapter.batchNumber === 15 &&
      adapter.model === "5.6 Sol" &&
      adapter.rubric === "Slugfester Reassessment Rubric v2" &&
      adapter.scoreProtocolId === SCORE_PROTOCOL_ID,
    `${adapter?.debateNumber ?? "unknown"}: Batch 15 site-ledger adapter identity mismatch`
  );
  assertV4(
    canonicalJson(adapter.sourceLocks) === canonicalJson(expectedSourceLocks),
    `${adapter.debateNumber}: Batch 15 site-ledger source locks changed`
  );
  ...
  assertV4(
    ...
    `${adapter.debateNumber}: Batch 15 site-ledger scoring identity changed`
  );
  assertV4(
    canonicalJson(scoredSnapshot(replayed)) === canonicalJson(adapter.calculated),
    `${adapter.debateNumber}: Batch 15 site-ledger score replay changed`
  );
  const candidateValidation =
    validatePostCanaryBatch15CandidateAgainstScores(candidate, {
      ...replayed,
  ...
  assertV4(
    ...
    `${adapter.debateNumber}: Batch 15 site-ledger adapter audit changed`
  );
  return {
    ...candidateValidation,
