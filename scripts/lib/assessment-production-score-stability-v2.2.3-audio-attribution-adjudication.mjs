...
export const V223_AUDIO_ADJ_ROOT =
  "docs/assessment-production/score-stability-v2.2.3-validation-cohort/audio-attribution-adjudication";
export const V223_AUDIO_ADJ_PROTOCOL_ID =
  "assessment-production-score-stability-v2.2.3-audio-attribution-adjudication";
export const V223_AUDIO_ADJ_PACKET_VERSION =
  "1.0-score-stability-v2.2.3-audio-attribution-adjudication-packet";
export const V223_AUDIO_ADJ_OUTPUT_VERSION =
  "1.0-score-stability-v2.2.3-audio-attribution-adjudication-output";
...
export function makeV223AudioAttributionAdjudicationSchema() {
  const item = exactObject({
    moveId: {
      type: "string",
      const: "pro-cumulative-moral-christian-case"
    },
    expectedSpeaker: { type: "string", const: "Francis Collins" },
  ...
  return {
    ...
    $id: "slugfester-score-stability-v2-2-3-audio-attribution-adjudication",
    title:
      "Slugfester score-stability v2.2.3 isolated audio-attribution adjudication",
    type: "object",
    additionalProperties: false,
    required: [
      "schemaVersion",
    ...
    properties: {
      schemaVersion: { type: "string", const: V223_AUDIO_ADJ_OUTPUT_VERSION },
      protocolId: { type: "string", const: V223_AUDIO_ADJ_PROTOCOL_ID },
      debateNumber: { type: "string", const: "17" },
      debateId: {
        type: "string",
        const: "collins-oconnor-god-existence-2024"
      },
      reviewerRole: {
        type: "string",
...
export async function validateV223AudioAttributionAdjudicationOutput(
  output,
  packet
) {
  ...
  assertV4(
    ...
    "v2.2.3 audio-attribution adjudication output keys invalid"
  );
  assertV4(
    output.schemaVersion === V223_AUDIO_ADJ_OUTPUT_VERSION &&
      output.protocolId === V223_AUDIO_ADJ_PROTOCOL_ID,
    "v2.2.3 audio-attribution adjudication output identity mismatch"
  );
  assertV4(
    output.debateNumber === packet.debateNumber &&
      output.debateId === packet.debateId,
    "v2.2.3 audio-attribution adjudication debate identity mismatch"
  );
  assertV4(
    ...
    "v2.2.3 audio-attribution adjudication reviewer boundary invalid"
  );
  assertV4(
    canonicalJson(output.isolation) === canonicalJson(ISOLATION),
    "v2.2.3 audio-attribution adjudication isolation invalid"
  );
  assertV4(
    canonicalJson(output.audit) === canonicalJson(AUDIT),
    "v2.2.3 audio-attribution adjudication audit invalid"
  );
  assertV4(
    !containsProhibitedCalculatedField(output),
    "v2.2.3 audio-attribution adjudication emitted calculated fields"
  );
  assertV4(
    ...
    "v2.2.3 audio-attribution adjudication move order invalid"
  );
...
export {
  AUDIT as V223_AUDIO_ADJ_AUDIT,
  ISOLATION as V223_AUDIO_ADJ_ISOLATION
};
