...
import { extractV4221173Disagreements } from "./v4221173-decomposed-disagreement.mjs";
import { buildV4221175AdjudicationPacket, validateV4221175AdjudicationOutput } from "./v4221175-decomposed-adjudication.mjs";

export const V4221176_ROOT = "docs/calibration/v4.2.21.17.6/final-ledger";
export const V4221176_PROTOCOL_ID = "v4.2.21.17.6-decomposed-consensus";
export const V4221176_LEDGER_VERSION = "4.2.21.17.6-adjudicated-raw-ledger";

const EXPECTED_DEBATES = Object.freeze(["133", "178", "182"]);
...
function validateInputReplay(input) {
  ...
  const replayedDisagreements = extractV4221173Disagreements(primaryA, primaryB, lockedInventory);
  assertV4(canonicalJson(replayedDisagreements) === canonicalJson(disagreements), `${primaryA.debateNumber}: stored disagreements differ from deterministic replay`);
  const rebuiltAdjudication = buildV4221175AdjudicationPacket(replayedDisagreements, lockedInventory, eventsDocument, audioMap(audioAudit));
  assertV4(canonicalJson(rebuiltAdjudication.packet) === canonicalJson(adjudicationPacket), `${primaryA.debateNumber}: adjudication packet differs from deterministic replay`);
  assertV4(provenance.modelInput === false, `${primaryA.debateNumber}: provenance must not be model input`);
  assertV4(canonicalJson(rebuiltAdjudication.provenance) === canonicalJson(provenance.mappings), `${primaryA.debateNumber}: adjudication provenance differs from deterministic replay`);
  validateV4221175AdjudicationOutput(adjudicationOutput, adjudicationPacket);
  return replayedDisagreements;
}

export function compileV4221176Debate(input) {
...
export function buildV4221176FinalLedger(debateInputs, sourceHashes) {
  assertV4(Array.isArray(debateInputs) && debateInputs.length === 3, "three debate inputs required");
  const debates = debateInputs.map(compileV4221176Debate);
  assertV4(debates.map((debate) => debate.debateNumber).join(",") === EXPECTED_DEBATES.join(","), "final-ledger debate order invalid");
  ...
  assertV4(aggregate.disputedMoves === 53, "disputed-move population mismatch");
  assertV4(candidateSelections === 172, "candidate-selection population mismatch");
  assertV4(aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed === 106, "rounded-mean population mismatch");
  assertV4(aggregate.audioVerifiedMoves === 2, "audio-verification population mismatch");
  return {
    schemaVersion: V4221176_LEDGER_VERSION,
    protocolId: V4221176_PROTOCOL_ID,
    status: "passed-deterministic-final-ledger-assembly",
    calibrationOnly: true,
    AIOnly: true,
    scoringInputSchema: "v4.2.20-source-span-primary-with-v4.2.21.17.6-adjudicated-provenance",
    sources: clone(sourceHashes),
    debates,
    audit: { ...aggregate, candidateSelections, finalRawJudgments: debates.length, deterministicDisagreementReplay: true, anonymizedCandidateProvenanceReplay: true, fullSourceChainValidation: true, singleScoringPassSchema: true, scoresDerivedOnlyAfterAdjudicatedLedgerLock: true, calculatedScores: 0 },
...
export function validateV4221176FinalLedger(ledger, debateInputs, sourceHashes) {
  const expected = buildV4221176FinalLedger(debateInputs, sourceHashes);
  assertV4(canonicalJson(ledger) === canonicalJson(expected), "final ledger differs from deterministic replay");
  assertV4(!containsProhibitedCalculatedField(ledger.debates.map((debate) => debate.finalJudgment)), "final ledger contains a prohibited calculated field");
...
export async function loadV4221176FinalLedgerInputs() {
  const adjudicationRoot = "docs/calibration/v4.2.21.17.5/dispute-only-adjudication";
  ...
  const audioAnalysisPath = "docs/calibration/v4.2.21.17.4/medium-confidence-audio-verification/analysis.json";
  const audioAuditPath = "docs/calibration/v4.2.21.17.4/medium-confidence-audio-verification/audio-verification.json";
  ...
  assertV4(analysis.status === "partition-dispute-only-adjudication-gate-passed" && analysis.authorization.finalLedgerAssembly && !analysis.authorization.scoreDerivation, "final-ledger assembly is not authorized");
  assertV4(execution.status === "three-isolated-dispute-only-adjudication-contexts-passed" && execution.validContexts === 3 && execution.retries === 0, "accepted adjudication execution unavailable");
  assertV4(audioAnalysis.status === "passed-all-two-medium-assessment-moves-audio-verified" && audioAudit.totals.verified === 2 && audioAudit.totals.unresolved === 0, "required audio verification unavailable");
  ...
  for (const context of preparation.contexts) {
    ...
    const [eventsFileBytes, sourceLedgerBytes] = await Promise.all([readFile(path.resolve(eventsPath)), readFile(path.resolve(sourceLedgerPath))]);
  ...
  assertV4(debateInputs.map((input) => input.primaryA.debateNumber).join(",") === EXPECTED_DEBATES.join(","), "loaded debate order invalid");
