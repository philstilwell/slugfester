...
import { POST_CANARY_BATCH_03_PUBLICATION_MODEL, POST_CANARY_BATCH_03_PUBLICATION_ROOT } from "./assessment-production-post-canary-batch-03-publication.mjs";
import { POST_CANARY_BATCH_03_PUBLICATION_RESUMPTION_ROOT } from "./assessment-production-post-canary-batch-03-publication-resumption.mjs";
...
export const POST_CANARY_BATCH_03_RESUMPTION_REPAIR_ROOT =
  `${POST_CANARY_BATCH_03_PUBLICATION_RESUMPTION_ROOT}/repair-1`;
export const POST_CANARY_BATCH_03_RESUMPTION_REPAIR_PROTOCOL_ID =
  "assessment-production-post-canary-batch-03-publication-resumption-1-repair-1";
export const POST_CANARY_BATCH_03_RESUMPTION_REPAIR_PACKET_VERSION =
  "1.0-assessment-production-post-canary-batch-03-publication-resumption-repair-packet";
export const POST_CANARY_BATCH_03_RESUMPTION_REPAIR_OUTPUT_VERSION =
  "1.0-assessment-production-post-canary-batch-03-publication-resumption-repair-output";
export const POST_CANARY_BATCH_03_RESUMPTION_REPAIR_PARTITIONS = Object.freeze([
  Object.freeze({
    debateNumber: "58",
  ...
  Object.freeze({
    debateNumber: "150",
    writableFields: Object.freeze([
      "moveProse.con-divine-agent-requires-prior-support.critique",
      "moveProse.pro-causation-against-methodological-exclusion.critique"
    ])
  }),
  Object.freeze({
    debateNumber: "150",
    writableFields: Object.freeze([
      "moveProse.con-test-before-supernatural-attribution.critique",
      "moveProse.con-paul-and-reports-insufficient.critique"
    ])
  })
]);
export const POST_CANARY_BATCH_03_RESUMPTION_REPAIR_FIELDS = Object.freeze(
  POST_CANARY_BATCH_03_RESUMPTION_REPAIR_PARTITIONS.flatMap(
    ({ writableFields }) => writableFields
  )
);
export const POST_CANARY_BATCH_03_RESUMPTION_REPAIR_BASE_OUTPUTS =
  Object.freeze({
    "58": `${POST_CANARY_BATCH_03_PUBLICATION_RESUMPTION_ROOT}/outputs/debate-58.json`,
    "150": `${POST_CANARY_BATCH_03_PUBLICATION_RESUMPTION_ROOT}/outputs/debate-150.json`
  });
export const POST_CANARY_BATCH_03_RESUMPTION_REPAIR_PUBLICATION_PACKETS =
  Object.freeze({
    "58": `${POST_CANARY_BATCH_03_PUBLICATION_ROOT}/packets/debate-58.json`,
    "150": `${POST_CANARY_BATCH_03_PUBLICATION_ROOT}/packets/debate-150.json`
  });
...
export function parseResumptionRepairField(field) {
  const novelty =
    /^aiExtension\.(pro|con)\.premises\[id=([^\]]+)\]\.novelty\.explanation$/.exec(
      field
    );
  if (novelty) return { type: "novelty-explanation", side: novelty[1], itemId: novelty[2] };
...
export function buildResumptionRepairSchema(packet) {
  ...
  return {
    ...
    $id:
      `slugfester-post-canary-batch-03-publication-resumption-repair-${packet.packetIndex}`,
    title:
      `Slugfester post-canary Batch 3 bounded publication-resumption repair packet ${packet.packetIndex}`,
    type: "object",
    ...
    properties: {
      schemaVersion: {
        type: "string",
        const: POST_CANARY_BATCH_03_RESUMPTION_REPAIR_OUTPUT_VERSION
      },
      protocolId: {
        type: "string",
        const: POST_CANARY_BATCH_03_RESUMPTION_REPAIR_PROTOCOL_ID
      },
...
export function validateResumptionRepairOutput(repair, packet) {
  ...
  assertV4(
    repair.schemaVersion ===
      ...
      repair.protocolId === POST_CANARY_BATCH_03_RESUMPTION_REPAIR_PROTOCOL_ID &&
  ...
  for (const correction of packet.corrections) {
    ...
    if (correction.repairType === "novelty-explanation") {
      ...
      assertV4(
        words >= 8 && words <= 35,
        `${correction.field}: repaired novelty explanation outside 8–35 words`
      );
      assertV4(
        displayedLanguagePasses(value),
        `${correction.field}: repaired novelty explanation has prohibited language`
      );
      assertV4(
        unexpectedCharactersAbsent(value),
        `${correction.field}: repaired novelty explanation has an unexpected character`
      );
...
function withRepairMarkers(output, debateNumber) {
  ...
  for (const partition of POST_CANARY_BATCH_03_RESUMPTION_REPAIR_PARTITIONS) {
    ...
    for (const field of partition.writableFields) {
      ...
      if (parsed.type === "novelty-explanation") {
        const item = copy.aiExtension[parsed.side].premises.find(
          ({ id }) => id === parsed.itemId
        );
        item.novelty.explanation = "__AUTHORIZED_REPAIR_FIELD__";
      } else {
...
export function mergeAndValidateResumptionRepairs({
  ...
  const mergedOutputs = {
    "58": structuredClone(baseOutputs["58"]),
    "150": structuredClone(baseOutputs["150"])
  };
  ...
  for (let index = 0; index < 3; index += 1) {
    ...
    for (const correction of packet.corrections) {
      ...
      let before;
      if (parsed.type === "novelty-explanation") {
        const item = target.aiExtension[parsed.side].premises.find(
          ({ id }) => id === parsed.itemId
        );
        before = item.novelty.explanation;
        item.novelty.explanation = repair.correctedFields[correction.field];
      } else {
  ...
  assertV4(
    ...
    "the publication-resumption merge must change exactly five fields"
  );
