...
import { extractAssessmentProductionPostCanaryBatch17Disagreements } from "./assessment-production-post-canary-batch-17-disagreement.mjs";
import {
  buildPostCanaryBatch17DisputeAdjudicationPacket,
  validatePostCanaryBatch17DisputeAdjudicationOutput
} from "./assessment-production-post-canary-batch-17-dispute-adjudication.mjs";

export const POST_CANARY_BATCH_17_FINAL_LEDGER_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-17/final-ledger";
export const POST_CANARY_BATCH_17_FINAL_LEDGER_PROTOCOL_ID =
  "assessment-production-post-canary-batch-17-adjudicated-consensus";
export const POST_CANARY_BATCH_17_FINAL_LEDGER_VERSION =
  "1.0-assessment-production-post-canary-batch-17-adjudicated-raw-ledger";
...
function validateInputReplay(input) {
  ...
  const replayedDisagreements =
    extractAssessmentProductionPostCanaryBatch17Disagreements(
      primaryA,
      primaryB,
      lockedInventory
    );
  assertV4(
    ...
    `${primaryA.debateNumber}: stored disagreements differ from deterministic Batch 17 replay`
  );
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication packet differs from deterministic Batch 17 replay`
  );
  assertV4(
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication provenance differs from deterministic Batch 17 replay`
  );
  validatePostCanaryBatch17DisputeAdjudicationOutput(
    adjudicationOutput,
    adjudicationPacket
...
export function compilePostCanaryBatch17FinalLedgerDebate(input) {
  const {
    primaryA,
    primaryB,
    adjudicationPacket,
...
export function buildPostCanaryBatch17FinalLedger(debateInputs, sourceHashes) {
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === EXPECTED_DEBATES.length,
    "four post-canary Batch 17 debate inputs required"
  );
  const debates = debateInputs.map(compilePostCanaryBatch17FinalLedgerDebate);
  assertV4(
    debates.map((debate) => debate.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "post-canary Batch 17 final-ledger debate order invalid"
  );
  ...
  assertV4(aggregate.finalMoves === 79, "Batch 17 final-move population mismatch");
  assertV4(aggregate.disputedMoves === 77, "Batch 17 disputed-move population mismatch");
  assertV4(candidateSelections === 230, "Batch 17 candidate-selection population mismatch");
  assertV4(
    aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed === 158,
    `Batch 17 rounded-mean population mismatch: ${aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed}`
  );
  assertV4(aggregate.audioVerifiedMoves === 1, "Batch 17 audio-verification population mismatch");
  return {
    schemaVersion: POST_CANARY_BATCH_17_FINAL_LEDGER_VERSION,
    protocolId: POST_CANARY_BATCH_17_FINAL_LEDGER_PROTOCOL_ID,
    status: "passed-post-canary-batch-17-deterministic-final-ledger-assembly",
    ...
    AIOnly: true,
    scoringInputSchema:
      "v4.2.20-source-span-primary-with-post-canary-batch-17-adjudicated-provenance",
    ...
    nextAuthorizedAction:
      "standing-authorization-permits-batch-17-single-deterministic-score-pass-preparation"
  };
}

export function validatePostCanaryBatch17FinalLedger(
  ledger,
  debateInputs,
  sourceHashes
) {
  const expected = buildPostCanaryBatch17FinalLedger(debateInputs, sourceHashes);
  assertV4(
    canonicalJson(ledger) === canonicalJson(expected),
    "post-canary Batch 17 final ledger differs from deterministic replay"
  );
  assertV4(
    !containsProhibitedCalculatedField(
      ledger.debates.map((debate) => debate.finalJudgment)
    ),
    "post-canary Batch 17 final ledger contains a prohibited calculated field"
  );
  return {
    status: "passed",
...
export async function loadPostCanaryBatch17FinalLedgerInputs() {
  ...
  assertV4(
    adjudicationAnalysis.status ===
        "post-canary-batch-17-dispute-only-adjudication-gate-passed-standing-authorization-active-for-final-ledger-assembly" &&
      ...
      adjudicationAnalysis.gate.validContexts === 4 &&
      adjudicationAnalysis.gate.disputedMovesDecided === 77 &&
      adjudicationAnalysis.gate.candidateSelections === 230 &&
      adjudicationAnalysis.gate.scoresDerived === 0 &&
      adjudicationAnalysis.nextAuthorizedAction ===
        "standing-authorization-permits-batch-17-deterministic-final-ledger-assembly" &&
      adjudicationAnalysis.authorization.scoreDerivation === false &&
      adjudicationExecution.status ===
        "four-post-canary-batch-17-dispute-only-adjudication-contexts-passed" &&
      adjudicationExecution.validContexts === 4 &&
      ...
      preparation.acceptedSourceBoundary.allRequiredAudioMovesVerified === true &&
      preparation.totals.audioVerifiedMoves === 1,
    "accepted post-canary Batch 17 adjudication gate unavailable"
  );
  const sourcePaths = [
  ...
  assertV4(
    debateInputs.map((input) => input.primaryA.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "loaded post-canary Batch 17 debate order invalid"
  );
