import { POST_CANARY_BATCH_12_PUBLICATION_BYLINE,
  POST_CANARY_BATCH_12_PUBLICATION_MODEL } from
  "./assessment-production-post-canary-batch-12-publication.mjs";
import { POST_CANARY_BATCH_12_PUBLICATION_COMPILATION_ORDER,
  POST_CANARY_BATCH_12_PUBLICATION_COMPILATION_PROTOCOL_ID,
  validatePostCanaryBatch12CompiledStagingRecord } from
  "./assessment-production-post-canary-batch-12-publication-compilation.mjs";
import { assertV4, canonicalJson } from "./v4-lean-production.mjs";

export const POST_CANARY_BATCH_12_PUBLICATION_FINALIZATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-12/publication-finalization";
export const POST_CANARY_BATCH_12_PUBLICATION_FINALIZATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-12-publication-finalization";
export const POST_CANARY_BATCH_12_PUBLICATION_FINALIZATION_ORDER =
  POST_CANARY_BATCH_12_PUBLICATION_COMPILATION_ORDER;

export function buildPostCanaryBatch12PublicationFinalization({ compiled, compiledPath,
  ...
  const provenance = { schemaVersion: "1.0-assessment-production-post-canary-batch-12-publication-finalization-provenance",
    protocolId: POST_CANARY_BATCH_12_PUBLICATION_FINALIZATION_PROTOCOL_ID,
    status: "staging-only-batch-12-final-candidate-provenance",
    ...
    model: { ...POST_CANARY_BATCH_12_PUBLICATION_MODEL,
...
export function validatePostCanaryBatch12PublicationFinalCandidate({ candidate, provenance,
  compiled, output, packet, identity }) {
  const expected = buildPostCanaryBatch12PublicationFinalization({ compiled,
...
export function buildPostCanaryBatch12PublicationStagingPreviewHtml() {
  const allowed = JSON.stringify(POST_CANARY_BATCH_12_PUBLICATION_FINALIZATION_ORDER);
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8">
    ...
    <title>Slugfester Batch 12 publication staging preview</title>
  ...
  <body>
    ...
    <script type="module">
      ...
      const debateNumber = params.get("debate") || "152";
      if (!allowedHosts.has(window.location.hostname)) {
        ...
        document.querySelector("#app").innerHTML = "<main><h1>Unknown Batch 12 publication-staging debate</h1></main>";
      } else {
