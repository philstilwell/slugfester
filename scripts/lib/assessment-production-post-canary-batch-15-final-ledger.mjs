...
import { extractAssessmentProductionPostCanaryBatch15Disagreements } from "./assessment-production-post-canary-batch-15-disagreement.mjs";
import {
  buildPostCanaryBatch15DisputeAdjudicationPacket,
  validatePostCanaryBatch15DisputeAdjudicationOutput
} from "./assessment-production-post-canary-batch-15-dispute-adjudication.mjs";

export const POST_CANARY_BATCH_15_FINAL_LEDGER_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-15/final-ledger";
export const POST_CANARY_BATCH_15_FINAL_LEDGER_PROTOCOL_ID =
  "assessment-production-post-canary-batch-15-adjudicated-consensus";
export const POST_CANARY_BATCH_15_FINAL_LEDGER_VERSION =
  "1.0-assessment-production-post-canary-batch-15-adjudicated-raw-ledger";
...
function validateInputReplay(input) {
  ...
  const replayedDisagreements =
    extractAssessmentProductionPostCanaryBatch15Disagreements(
      primaryA,
      primaryB,
      lockedInventory
    );
  assertV4(
    ...
    `${primaryA.debateNumber}: stored disagreements differ from deterministic Batch 15 replay`
  );
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication packet differs from deterministic Batch 15 replay`
  );
  assertV4(
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication provenance differs from deterministic Batch 15 replay`
  );
  validatePostCanaryBatch15DisputeAdjudicationOutput(
    adjudicationOutput,
    adjudicationPacket
...
export function compilePostCanaryBatch15FinalLedgerDebate(input) {
  const {
    primaryA,
    primaryB,
    adjudicationPacket,
...
export function buildPostCanaryBatch15FinalLedger(debateInputs, sourceHashes) {
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === EXPECTED_DEBATES.length,
    "ten post-canary Batch 15 debate inputs required"
  );
  const debates = debateInputs.map(compilePostCanaryBatch15FinalLedgerDebate);
  assertV4(
    debates.map((debate) => debate.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "post-canary Batch 15 final-ledger debate order invalid"
  );
  ...
  assertV4(aggregate.finalMoves === 191, "Batch 15 final-move population mismatch");
  assertV4(aggregate.disputedMoves === 186, "Batch 15 disputed-move population mismatch");
  assertV4(candidateSelections === 561, "Batch 15 candidate-selection population mismatch");
  assertV4(
    aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed === 409,
    `Batch 15 rounded-mean population mismatch: ${aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed}`
  );
  assertV4(aggregate.audioVerifiedMoves === 3, "Batch 15 audio-verification population mismatch");
  return {
    schemaVersion: POST_CANARY_BATCH_15_FINAL_LEDGER_VERSION,
    protocolId: POST_CANARY_BATCH_15_FINAL_LEDGER_PROTOCOL_ID,
    status: "passed-post-canary-batch-15-deterministic-final-ledger-assembly",
    ...
    AIOnly: true,
    scoringInputSchema:
      "v4.2.20-source-span-primary-with-post-canary-batch-15-adjudicated-provenance",
    ...
    nextAuthorizedAction:
      "standing-authorization-permits-batch-15-single-deterministic-score-pass-preparation"
  };
}

export function validatePostCanaryBatch15FinalLedger(
  ledger,
  debateInputs,
  sourceHashes
) {
  const expected = buildPostCanaryBatch15FinalLedger(debateInputs, sourceHashes);
  assertV4(
    canonicalJson(ledger) === canonicalJson(expected),
    "post-canary Batch 15 final ledger differs from deterministic replay"
  );
  assertV4(
    !containsProhibitedCalculatedField(
      ledger.debates.map((debate) => debate.finalJudgment)
    ),
    "post-canary Batch 15 final ledger contains a prohibited calculated field"
  );
  return {
    status: "passed",
...
export async function loadPostCanaryBatch15FinalLedgerInputs() {
  ...
  assertV4(
    adjudicationAnalysis.status ===
        "post-canary-batch-15-dispute-only-adjudication-gate-passed-standing-authorization-active-for-final-ledger-assembly" &&
      ...
      adjudicationAnalysis.gate.disputedMovesDecided === 186 &&
      adjudicationAnalysis.gate.candidateSelections === 561 &&
      adjudicationAnalysis.gate.scoresDerived === 0 &&
      adjudicationAnalysis.nextAuthorizedAction ===
        "standing-authorization-permits-batch-15-deterministic-final-ledger-assembly" &&
      adjudicationAnalysis.authorization.scoreDerivation === false &&
      adjudicationExecution.status ===
        "ten-post-canary-batch-15-dispute-only-adjudication-contexts-passed" &&
      ...
      preparation.acceptedSourceBoundary.allThreeAudioMovesVerified === true &&
      preparation.totals.audioVerifiedMoves === 3,
    "accepted post-canary Batch 15 adjudication gate unavailable"
  );
  const sourcePaths = [
  ...
  assertV4(
    debateInputs.map((input) => input.primaryA.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "loaded post-canary Batch 15 debate order invalid"
  );
