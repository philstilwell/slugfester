...
import { compileUniqueSelectionMapInventory } from "./assessment-production-score-stability-v2-inventory-unique-selection-map.mjs";

export const V2_INVENTORY = Object.freeze({
  validationRoot:
    "docs/assessment-production/score-stability-v2-validation-cohort",
  root: "docs/assessment-production/score-stability-v2-validation-cohort/inventory-unique-selection-map-successor",
  preparation:
  ...
  manifest:
    "docs/assessment-production/score-stability-v2-validation-cohort/inventory-unique-selection-map-successor/execution-manifest.json",
  execution:
  ...
  analysis:
    "docs/assessment-production/score-stability-v2-validation-cohort/inventory-unique-selection-map-successor/analysis.json",
  manifestStatus:
  ...
  executionPassedStatus:
    "ten-fresh-unique-selection-map-v2-validation-score-blind-inventory-successor-contexts-passed",
  executionFailedStatus:
    "v2-validation-score-blind-inventory-unique-selection-map-successor-complete-with-failure",
  analysisStatus:
    "ten-fresh-unique-selection-map-v2-validation-score-blind-inventory-successor-contexts-passed-independent-judgment-packet-preparation-authorized",
  contexts: 10,
  candidates: 406,
  proCandidates: 203,
  conCandidates: 203,
...
const DEVELOPMENT_ANALYSIS =
  "docs/assessment-production/score-stability-v2-validation-cohort/inventory-unique-selection-map-development/development-analysis.json";
const UNIQUE_SELECTION_MAP_LIBRARY_PATH =
...
const PREREGISTER_PATH =
  "scripts/preregister-assessment-production-score-stability-v2-inventory-unique-selection-map-successor.mjs";
const RUNNER_PATH =
  "scripts/run-assessment-production-score-stability-v2-inventory-unique-selection-map-successor.mjs";
const ANALYZER_PATH =
  "scripts/analyze-assessment-production-score-stability-v2-inventory-unique-selection-map-successor.mjs";
const VALIDATOR_PATH =
  "scripts/validate-assessment-production-score-stability-v2-inventory-unique-selection-map-successor.mjs";
const TEST_PATH =
  "scripts/test-assessment-production-score-stability-v2-inventory-unique-selection-map-successor-gate.mjs";
const VALIDATOR_TEST_PATH =
  "scripts/test-assessment-production-score-stability-v2-inventory-unique-selection-map-successor-validator.mjs";
...
export async function preregisterV2InventoryUniqueSelectionMapSuccessor({ shouldWrite, frozenAt }) {
  assertV4(
  ...
  const sourceFiles = [
    ...
    preparation.inputs.uniqueSelectionMapGuide,
    RETIRED_EXECUTION,
    RETIRED_ANALYSIS,
    ...
    UNIQUE_SELECTION_MAP_LIBRARY_PATH,
    LIBRARY_PATH,
    VALIDATOR_PATH,
    PREREGISTER_PATH,
    RUNNER_PATH,
  ...
  const manifest = {
    schemaVersion:
      "1.0-score-stability-v2-fresh-validation-unique-selection-map-score-blind-inventory-successor-execution-manifest",
    ...
    costEstimate: {
      ...
      estimateBasis:
        "The predecessor attempted all ten contexts in 17.84 wall-minutes and 24.31 aggregate model-minutes. This successor preserves the 1-to-2 ramp and frozen timeouts, uses the same lossless columnar evidence, and adds a required nullable property for every candidate; the maximum copied input is 89,937 bytes.",
    },
    ...
    modelInputs: {
      ...
      uniqueSelectionMapGuide: preparation.inputs.uniqueSelectionMapGuide,
    },
    ...
    priorFailedGateEvidence: {
      ...
      bothGatesPreservedAsFailed: true,
      priorValidOutputsReusableForSuccessorAcceptance: false,
      priorFailedOutputsReusableForSuccessorAcceptance: false,
    },
    isolation: {
...
export async function validateV2InventoryUniqueSelectionMapSuccessorProposal({
  proposalPath,
  preparationPath,
  debateNumber,
  shouldWrite,
  ...
  const summary = {
    schemaVersion:
      "1.0-score-stability-v2-fresh-validation-unique-selection-map-score-blind-inventory-successor-validation",
    protocolId: preparation.protocolId,
    status: "passed",
    developmentValidationOnly: true,
    productionCanary: false,
...
async function readAuthorizedManifest() {
  ...
  assertV4(
    manifest.status === V2_INVENTORY.manifestStatus &&
      ...
      manifest.modelInputs?.uniqueSelectionMapGuide ===
        "docs/assessment-production/score-stability-v2-validation-cohort/inventory-unique-selection-map-development/unique-selection-map-guide.md" &&
...
export async function runV2InventoryUniqueSelectionMapSuccessor() {
  ...
  async function executeContext(context, contextIndex) {
    ...
    const temporary = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-v2-validation-inventory-unique-selection-map-successor-${context.debateNumber}-`
      )
    );
    const isolatedCodexHome = await mkdtemp(
      path.join(
        os.tmpdir(),
        `slugfester-v2-validation-inventory-unique-selection-map-successor-home-${context.debateNumber}-`
      )
    );
    ...
    try {
      for (const [source, target] of [
        ...
        [manifest.modelInputs.uniqueSelectionMapGuide, "unique-selection-map-guide.md"],
      ...
      const prompt = `Read manual.md, columnar-transport-guide.md, unique-selection-map-guide.md, packet.json, candidate-evidence-bundle.json, and schema.json completely; read nothing else. Act only as the isolated score-blind inventory curator for fresh development-validation successor Debate ${context.debateNumber}. The candidate evidence uses the lossless columnar representation defined in columnar-transport-guide.md, and the response must use the structurally unique selection map defined in unique-selection-map-guide.md. All ${context.candidates} candidate keys are required exactly once in candidateSelections: use null for every unselected candidate, and for every selected candidate provide only sectionId, orderWithinSide, moveId, moveKind, and a source-faithful proposition. Select 8 to 24 candidates total. Produce four to six weighted issue sections totaling exactly 100 percent, with one or two pro and one or two con selections in every section. A candidate key can be selected at most once. Define one route per side and its burden bridges. A reply must have an earlier selected opposing move in source chronology, but do not name targets. Ratings, response topology, burden contact, adjustments, scores, winners, tags, Overall Commentary, AI Extension, score-policy analysis, prior execution information, and publication prose are prohibited. Return exactly one schema-conforming JSON object.`;
      process.stdout.write(
        `[v2-validation-inventory-unique-selection-map-successor] starting ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber}\n`
      );
    ...
    process.stdout.write(
      `[v2-validation-inventory-unique-selection-map-successor] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
    );
    return record;
  }
  ...
  const execution = {
    schemaVersion:
      "1.0-score-stability-v2-fresh-validation-unique-selection-map-score-blind-inventory-successor-model-execution",
...
export async function analyzeV2InventoryUniqueSelectionMapSuccessor({ shouldWrite }) {
  const manifest = await readAuthorizedManifest();
  ...
  for (const result of execution.results) {
    ...
    debates.push({
      ...
      uniqueSelectionMapUsed: validation.uniqueSelectionMapUsed,
  ...
  const analysis = {
    schemaVersion:
      "1.0-score-stability-v2-fresh-validation-unique-selection-map-score-blind-inventory-successor-analysis",
    protocolId: manifest.protocolId,
...
export async function testV2InventoryUniqueSelectionMapSuccessorGate() {
  ...
  assertV4(
    manifest.acceptance?.everyCandidateAvailableDuringSelection === true &&
      ...
      manifest.transport?.maximumCopiedInputBytes === 89937 &&
      ...
      manifest.contexts.every(
        (context) =>
          ...
          context.schema.startsWith(
            `${V2_INVENTORY.validationRoot}/inventory-unique-selection-map-development/schemas/`
          ) &&
