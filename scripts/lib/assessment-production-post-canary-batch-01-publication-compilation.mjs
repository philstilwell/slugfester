...
export const POST_CANARY_BATCH_01_PUBLICATION_COMPILATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-01/deterministic-publication-compilation";
export const POST_CANARY_BATCH_01_PUBLICATION_COMPILATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-01-deterministic-publication-compilation";
export const POST_CANARY_BATCH_01_PUBLICATION_COMPILATION_ORDER = Object.freeze(
  [...POST_CANARY_BATCH_01_PUBLICATION_DEBATES]
);
...
function referenceUrl(tag) {
  ...
  assertV4(
    ...
    `unknown Batch 1 publication reference ${tag.type}:${tag.slug}`
  );
...
export function compilePostCanaryBatch01PublicationStagingRecord({
  output,
  packet,
  identity
}) {
  ...
  const record = {
    ...
    assessmentModel: POST_CANARY_BATCH_01_PUBLICATION_MODEL.label,
...
export function validatePostCanaryBatch01CompiledStagingRecord({
  compiled,
  output,
  packet,
  identity
}) {
  const expected = compilePostCanaryBatch01PublicationStagingRecord({
    output,
    packet,
    identity
  });
