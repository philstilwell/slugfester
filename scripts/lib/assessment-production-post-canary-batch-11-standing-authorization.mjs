...
export const POST_CANARY_BATCH_11_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-11";
export const POST_CANARY_BATCH_11_STANDING_AUTHORIZATION =
  `${POST_CANARY_BATCH_11_ROOT}/standing-authorization.json`;
export const POST_CANARY_BATCH_11_STANDING_AUTHORIZATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-11-standing-authorization";
export const POST_CANARY_BATCH_11_STANDING_AUTHORIZATION_STATUS =
  "frozen-active-batch-11-complete-remaining-workflow-standing-authorization";
export const POST_CANARY_BATCH_11_STANDING_AUTHORIZATION_INSTRUCTION_PATH =
  POST_CANARY_BATCH_11_ROOT + "/standing-authorization-instruction.txt";
export const POST_CANARY_BATCH_11_STANDING_AUTHORIZATION_INSTRUCTION = readFileSync(
  path.resolve(POST_CANARY_BATCH_11_STANDING_AUTHORIZATION_INSTRUCTION_PATH),
  "utf8"
).trimEnd();

const sha256 = (value) => createHash("sha256").update(value).digest("hex");

export function validatePostCanaryBatch11StandingAuthorization(record) {
  assertV4(
    record?.schemaVersion ===
      ...
      record.status === POST_CANARY_BATCH_11_STANDING_AUTHORIZATION_STATUS &&
      record.productionCanary === false &&
      record.batchNumber === 11 &&
      record.sourcePacketCommit === "d446c055aec2a0e4170209501d63ddcc20683f3f" &&
      ...
      record.userAuthorization?.resolvedAntecedent ===
        "The attached Batch 11 request authorizes deterministic selection and the complete reassessment, recovery, publication, validation, commit, and push workflow." &&
      record.userAuthorization?.scopeInterpretation ===
        "Create and use a complete-workflow Batch 11 standing authorization for the ten frozen selections, beginning with the separately frozen score-blind source packets, including bounded diagnosed recovery and production publication, while preserving the established campaign controls and stopping before Batch 12 selection." &&
    ...
    "Batch 11 standing-authorization identity changed"
  );
  assertV4(
    canonicalJson(record.selectedDebates) ===
      canonicalJson(["54", "01", "82", "191", "151", "188", "60", "79", "43", "24"]),
    "Batch 11 standing-authorization debate boundary changed"
  );
  assertV4(
    ...
    "Batch 11 standing-authorization model changed"
  );
  assertV4(
    ...
    "Batch 11 standing-authorization execution controls changed"
  );
  assertV4(
  ...
  assertV4(
    ...
    "Batch 11 standing-authorization cost boundary changed"
  );
  assertV4(
    ...
    "Batch 11 standing-authorization source-recovery controls changed"
  );
  assertV4(
    Object.values(record.stopRules ?? {}).every(Boolean),
    "Batch 11 standing-authorization stop rule disabled"
  );
  assertV4(
    ...
    "Batch 11 standing-authorization recovery controls changed"
  );
  assertV4(
    ...
    "Batch 11 standing-authorization Git controls changed"
  );
  return record;
}

export async function loadAndValidatePostCanaryBatch11StandingAuthorization() {
  const bytes = await readFile(
    path.resolve(POST_CANARY_BATCH_11_STANDING_AUTHORIZATION)
  );
  const record = validatePostCanaryBatch11StandingAuthorization(JSON.parse(bytes));
