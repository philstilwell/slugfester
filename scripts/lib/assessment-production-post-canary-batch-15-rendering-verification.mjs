...
import {
  POST_CANARY_BATCH_15_PUBLICATION_FINALIZATION_ORDER
} from "./assessment-production-post-canary-batch-15-publication-finalization.mjs";
import {
  POST_CANARY_BATCH_15_PUBLICATION_BYLINE,
  POST_CANARY_BATCH_15_PUBLICATION_MODEL
} from "./assessment-production-post-canary-batch-15-publication.mjs";
import { assertV4, canonicalJson } from "./v4-lean-production.mjs";

export const POST_CANARY_BATCH_15_RENDERING_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-15/rendering-verification";
export const POST_CANARY_BATCH_15_RENDERING_PROTOCOL_ID =
  "assessment-production-post-canary-batch-15-rendering-verification";
export const POST_CANARY_BATCH_15_RENDERING_PORT = 4211;
export const POST_CANARY_BATCH_15_RENDERING_ORDER =
  POST_CANARY_BATCH_15_PUBLICATION_FINALIZATION_ORDER;
export const POST_CANARY_BATCH_15_RENDERING_BYLINE =
  POST_CANARY_BATCH_15_PUBLICATION_BYLINE;
export const POST_CANARY_BATCH_15_RENDERING_MODEL =
  POST_CANARY_BATCH_15_PUBLICATION_MODEL;
export const POST_CANARY_BATCH_15_RENDERING_VIEWPORTS = Object.freeze({
  desktop: Object.freeze({ width: 1440, height: 1000 }),
  mobile: Object.freeze({ width: 390, height: 844 })
});
export const POST_CANARY_BATCH_15_RENDERING_REQUIRED_CHECKS = Object.freeze([
  "exactViewport",
...
export function renderingEvidencePaths(debateNumber, viewportName) {
  const screenshots = `output/playwright/batch-15-rendering/debate-${debateNumber}`;
  return {
    result: `${POST_CANARY_BATCH_15_RENDERING_ROOT}/evidence/debate-${debateNumber}/${viewportName}.json`,
    collapsedScreenshot: `${screenshots}/${viewportName}-collapsed.jpg`,
    openScreenshot: `${screenshots}/${viewportName}-open.jpg`
  };
}

export function buildPostCanaryBatch15RenderingPacket({
  auditRow,
  candidate,
  provenance,
  previewPath,
  ...
  assertV4(candidate.number === auditRow.debateNumber &&
    ...
    provenance.model.label === POST_CANARY_BATCH_15_RENDERING_MODEL.label &&
    provenance.model.reasoningEffort === POST_CANARY_BATCH_15_RENDERING_MODEL.reasoningEffort &&
    provenance.displayContract.byline === POST_CANARY_BATCH_15_RENDERING_BYLINE &&
    provenance.displayContract.defaultCollapsed === true,
  `${auditRow.debateNumber}: finalization provenance changed`);
  const packet = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-15-rendering-packet",
    ...
    preview: {
      ...
      url: `http://127.0.0.1:${POST_CANARY_BATCH_15_RENDERING_PORT}/${previewPath}?debate=${auditRow.debateNumber}`,
      localOnly: true,
      noindex: true
    },
    expectedDisplay: {
      byline: POST_CANARY_BATCH_15_RENDERING_BYLINE,
      stagingBanner: "Publication staging preview: validated post-canary Batch 15 candidate only. This scorecard remains excluded from production data and rankings pending rendering and mutation authorization.",
    ...
    viewports: Object.fromEntries(Object.entries(POST_CANARY_BATCH_15_RENDERING_VIEWPORTS)
  ...
  return validatePostCanaryBatch15RenderingPacket(packet);
}

export function validatePostCanaryBatch15RenderingPacket(packet) {
  assertV4(packet?.schemaVersion ===
    ...
    packet.expectedDisplay.byline === POST_CANARY_BATCH_15_RENDERING_BYLINE &&
    ...
    canonicalJson(packet.requiredChecks) ===
      canonicalJson(POST_CANARY_BATCH_15_RENDERING_REQUIRED_CHECKS) &&
  ...
  `${packet?.debateNumber ?? "unknown"}: invalid Batch 15 rendering packet`);
  for (const [name, viewport] of Object.entries(POST_CANARY_BATCH_15_RENDERING_VIEWPORTS)) {
...
export function validatePostCanaryBatch15RenderingEvidence({ packet, viewportName, token, evidence }) {
  validatePostCanaryBatch15RenderingPacket(packet);
  const viewport = packet.viewports[viewportName];
  assertV4(evidence?.schemaVersion ===
    "1.0-assessment-production-post-canary-batch-15-rendering-evidence" &&
