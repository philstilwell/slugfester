...
export const POST_CANARY_BATCH_02_COMPATIBILITY_ACTIVATION_STATUS =
...
export function buildPostCanaryBatch02ValidatorSource(baselineSource) {
  ...
  source = replaceExactOnce(
    ...
    "Batch 2 adapter-version route"
  );

  const routeFunctions = `export function validatePostCanaryBatch02LedgerAdapterRouteLocks({
  debate,
  ledgerText,
  packetPath,
  packetText,
...
`;
  source = replaceExactOnce(
    ...
    "Batch 2 route functions"
  );

  const batch02Branch = `  if (
...
`;
  source = replaceExactOnce(
    source,
    "  let calculated;\n",
    `${batch02Branch}  let calculated;\n`,
    "Batch 2 ledger branch"
  );

  validatePostCanaryBatch02ValidatorSource(source);
  return source;
}

export function validatePostCanaryBatch02ValidatorSource(source) {
  const required = [
    "POST_CANARY_BATCH_02_COMPATIBILITY_ROOT",
    "POST_CANARY_BATCH_02_SITE_LEDGER_ADAPTER_VERSION",
    "validatePostCanaryBatch02SiteLedgerAdapter",
    ...
    "validatePostCanaryBatch02LedgerAdapterRoute",
    POST_CANARY_BATCH_02_COMPATIBILITY_ACTIVATION_STATUS,
    "Batch 2 production assessments require an AI Extension",
    "Batch 2 ledger validation failed"
  ];
  for (const text of required) {
    if (!source.includes(text)) {
      throw new Error(`proposed Batch 2 validator is missing ${text}`);
    }
  }
  if (
    ...
    throw new Error("proposed Batch 2 validator weakens an existing route");
  }
  return {
    status: "passed",
