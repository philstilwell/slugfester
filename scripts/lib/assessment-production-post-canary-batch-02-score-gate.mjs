...
export const POST_CANARY_BATCH_02_SCORE_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-02/score-pass";
export const POST_CANARY_BATCH_02_CALCULATED_SCORES_VERSION =
  "1.0-assessment-production-post-canary-batch-02-adjudicated-calculated-scores";
export const POST_CANARY_BATCH_02_SCORE_STABILITY_THRESHOLDS =
  ACTIVE_SCORE_STABILITY_THRESHOLDS;
...
export function evaluatePostCanaryBatch02ScoreStability(
  debates,
  thresholds = POST_CANARY_BATCH_02_SCORE_STABILITY_THRESHOLDS
) {
  assertV4(
    Array.isArray(debates) && debates.length === 10,
    "ten scored post-canary Batch 2 debates required"
  );
  assertV4(
    canonicalJson(thresholds) ===
      canonicalJson(POST_CANARY_BATCH_02_SCORE_STABILITY_THRESHOLDS),
    "post-canary Batch 2 active thresholds changed"
  );
...
export function derivePostCanaryBatch02Scores(
  ledger,
  debateInputs,
  ...
  const debates = debateInputs.map((input) => {
    ...
    assertV4(
      finalLedgerDebate,
      `${debateNumber}: Batch 2 final-ledger debate missing`
    );
  ...
  const stability = evaluatePostCanaryBatch02ScoreStability(debates);
  ...
  return {
    schemaVersion: POST_CANARY_BATCH_02_CALCULATED_SCORES_VERSION,
    ...
    status: stability.acceptancePassed
      ...
      : "post-canary-batch-02-single-score-pass-stability-gate-failed",
...
export function validatePostCanaryBatch02Scores(
  scores,
  ledger,
  debateInputs,
  productionReferences,
  ...
  const expected = derivePostCanaryBatch02Scores(
    ledger,
    debateInputs,
    productionReferences,
    hashes
  ...
  assertV4(
    ...
    "post-canary Batch 2 calculated scores differ from deterministic replay"
  );
  return {
    status: "passed",
