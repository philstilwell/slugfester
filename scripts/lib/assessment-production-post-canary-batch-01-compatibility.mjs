...
export const POST_CANARY_BATCH_01_COMPATIBILITY_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-01/production-compatibility";
export const POST_CANARY_BATCH_01_COMPATIBILITY_PROTOCOL_ID =
  "assessment-production-post-canary-batch-01-production-compatibility";
export const POST_CANARY_BATCH_01_SITE_LEDGER_ADAPTER_VERSION =
  "1.0-assessment-production-post-canary-batch-01-site-ledger-adapter";
export const POST_CANARY_BATCH_01_COMPATIBILITY_ORDER = Object.freeze([
  "31",
  "94",
  "52",
  "146",
...
export function validatePostCanaryBatch01CandidateAgainstScores(
  candidate,
  finalScores
) {
  assertV4(
    ...
    `${finalScores.debateNumber}: Batch 1 publication candidate identity mismatch`
  );
  assertV4(
    ...
    `${finalScores.debateNumber}: Batch 1 publication candidate overall scores changed`
  );
  assertV4(
    candidate.sections?.length === finalScores.sections.length,
    `${finalScores.debateNumber}: Batch 1 publication candidate section count changed`
  );
  let moveCount = 0;
  finalScores.sections.forEach((scoredSection, sectionIndex) => {
    ...
    assertV4(
      ...
      `${finalScores.debateNumber}: Batch 1 section ${sectionIndex} identity or score changed`
    );
    for (const side of ["pro", "con"]) {
      ...
      assertV4(
        ...
        `${finalScores.debateNumber}: Batch 1 section ${sectionIndex} ${side} move IDs, order, or scores changed`
      );
...
export function buildPostCanaryBatch01SiteLedgerAdapter({
  finalLedgerDebate,
  scoreDebate,
  candidate,
  eventsDocument,
  ...
  assertV4(
    ...
    "Batch 1 final-ledger and score identities differ"
  );
  ...
  assertV4(
    ...
    `${scoreDebate.debateNumber}: locked Batch 1 score is not an exact repository replay`
  );
  const candidateValidation =
    validatePostCanaryBatch01CandidateAgainstScores(candidate, {
      ...scoreDebate.final,
  ...
  return {
    ...
    status: "frozen-post-canary-batch-01-site-ledger-adapter",
...
export function validatePostCanaryBatch01SiteLedgerAdapter({
  adapter,
  candidate,
  expectedSourceLocks
}) {
  assertV4(
    adapter?.schemaVersion ===
      ...
      adapter.protocolId === POST_CANARY_BATCH_01_COMPATIBILITY_PROTOCOL_ID &&
      adapter.status ===
        "frozen-post-canary-batch-01-site-ledger-adapter" &&
      adapter.productionCanary === false &&
      adapter.batchNumber === 1 &&
      adapter.model === "5.6 Sol" &&
      adapter.rubric === "Slugfester Reassessment Rubric v2" &&
      adapter.scoreProtocolId === SCORE_PROTOCOL_ID,
    `${adapter?.debateNumber ?? "unknown"}: Batch 1 site-ledger adapter identity mismatch`
  );
  assertV4(
    canonicalJson(adapter.sourceLocks) === canonicalJson(expectedSourceLocks),
    `${adapter.debateNumber}: Batch 1 site-ledger source locks changed`
  );
  ...
  assertV4(
    ...
    `${adapter.debateNumber}: Batch 1 site-ledger scoring identity changed`
  );
  assertV4(
    canonicalJson(scoredSnapshot(replayed)) === canonicalJson(adapter.calculated),
    `${adapter.debateNumber}: Batch 1 site-ledger score replay changed`
  );
  const candidateValidation =
    validatePostCanaryBatch01CandidateAgainstScores(candidate, {
      ...replayed,
  ...
  assertV4(
    ...
    `${adapter.debateNumber}: Batch 1 site-ledger adapter audit changed`
  );
  return {
    ...candidateValidation,
