...
import { extractAssessmentProductionPostCanaryBatch04Disagreements } from "./assessment-production-post-canary-batch-04-disagreement.mjs";
import {
  buildPostCanaryBatch04DisputeAdjudicationPacket,
  validatePostCanaryBatch04DisputeAdjudicationOutput
} from "./assessment-production-post-canary-batch-04-dispute-adjudication.mjs";

export const POST_CANARY_BATCH_04_FINAL_LEDGER_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-04/final-ledger";
export const POST_CANARY_BATCH_04_FINAL_LEDGER_PROTOCOL_ID =
  "assessment-production-post-canary-batch-04-adjudicated-consensus";
export const POST_CANARY_BATCH_04_FINAL_LEDGER_VERSION =
  "1.0-assessment-production-post-canary-batch-04-adjudicated-raw-ledger";
...
function validateInputReplay(input) {
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: stored disagreements differ from deterministic Batch 4 replay`
  );
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication packet differs from deterministic Batch 4 replay`
  );
  assertV4(
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication provenance differs from deterministic Batch 4 replay`
  );
  validatePostCanaryBatch04DisputeAdjudicationOutput(
    adjudicationOutput,
    adjudicationPacket
...
export function compilePostCanaryBatch04FinalLedgerDebate(input) {
  const {
    primaryA,
    primaryB,
    adjudicationPacket,
...
export function buildPostCanaryBatch04FinalLedger(debateInputs, sourceHashes) {
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === EXPECTED_DEBATES.length,
    "ten post-canary Batch 4 debate inputs required"
  );
  const debates = debateInputs.map(compilePostCanaryBatch04FinalLedgerDebate);
  assertV4(
    debates.map((debate) => debate.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "post-canary Batch 4 final-ledger debate order invalid"
  );
  ...
  assertV4(aggregate.finalMoves === 203, "Batch 4 final-move population mismatch");
  assertV4(aggregate.disputedMoves === 196, "Batch 4 disputed-move population mismatch");
  assertV4(candidateSelections === 582, "Batch 4 candidate-selection population mismatch");
  assertV4(
    aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed === 416,
    "Batch 4 rounded-mean population mismatch"
  );
  assertV4(aggregate.audioVerifiedMoves === 4, "Batch 4 audio-verification population mismatch");
  return {
    schemaVersion: POST_CANARY_BATCH_04_FINAL_LEDGER_VERSION,
    protocolId: POST_CANARY_BATCH_04_FINAL_LEDGER_PROTOCOL_ID,
    status: "passed-post-canary-batch-04-deterministic-final-ledger-assembly",
    ...
    AIOnly: true,
    scoringInputSchema:
      "v4.2.20-source-span-primary-with-post-canary-batch-04-adjudicated-provenance",
...
export function validatePostCanaryBatch04FinalLedger(
  ledger,
  debateInputs,
  sourceHashes
) {
  const expected = buildPostCanaryBatch04FinalLedger(debateInputs, sourceHashes);
  assertV4(
    canonicalJson(ledger) === canonicalJson(expected),
    "post-canary Batch 4 final ledger differs from deterministic replay"
  );
  assertV4(
    !containsProhibitedCalculatedField(
      ledger.debates.map((debate) => debate.finalJudgment)
    ),
    "post-canary Batch 4 final ledger contains a prohibited calculated field"
  );
  return {
    status: "passed",
...
export async function loadPostCanaryBatch04FinalLedgerInputs() {
  ...
  assertV4(
    adjudicationAnalysis.status ===
        "passed-complete-batch-04-adjudication-cohort-after-bounded-first-correction-standing-authorization-active-for-final-ledger-assembly" &&
      adjudicationAnalysis.gate.passed === true &&
      adjudicationAnalysis.gate.debateOutputsAccepted === 10 &&
      adjudicationAnalysis.gate.disputedMovesDecided === 196 &&
      adjudicationAnalysis.gate.candidateSelections === 582 &&
  ...
  for (const context of preparation.contexts) {
    ...
    debateInputs.push({
      ...
      audioAudit,
      adjudicationOutputAcceptedWithoutCorrection:
        result.route === "original-execution",
      adjudicationOutputPath,
  ...
  assertV4(
    debateInputs.map((input) => input.primaryA.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "loaded post-canary Batch 4 debate order invalid"
  );
