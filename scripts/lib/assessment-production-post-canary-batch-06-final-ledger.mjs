...
import { extractAssessmentProductionPostCanaryBatch06Disagreements } from "./assessment-production-post-canary-batch-06-disagreement.mjs";
import {
  buildPostCanaryBatch06DisputeAdjudicationPacket,
  validatePostCanaryBatch06DisputeAdjudicationOutput
} from "./assessment-production-post-canary-batch-06-dispute-adjudication.mjs";

export const POST_CANARY_BATCH_06_FINAL_LEDGER_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-06/final-ledger";
export const POST_CANARY_BATCH_06_FINAL_LEDGER_PROTOCOL_ID =
  "assessment-production-post-canary-batch-06-adjudicated-consensus";
export const POST_CANARY_BATCH_06_FINAL_LEDGER_VERSION =
  "1.0-assessment-production-post-canary-batch-06-adjudicated-raw-ledger";
...
function validateInputReplay(input) {
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: stored disagreements differ from deterministic Batch 6 replay`
  );
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication packet differs from deterministic Batch 6 replay`
  );
  assertV4(
  ...
  assertV4(
    ...
    `${primaryA.debateNumber}: adjudication provenance differs from deterministic Batch 6 replay`
  );
  validatePostCanaryBatch06DisputeAdjudicationOutput(
    adjudicationOutput,
    adjudicationPacket
...
export function compilePostCanaryBatch06FinalLedgerDebate(input) {
  const {
    primaryA,
    primaryB,
    adjudicationPacket,
...
export function buildPostCanaryBatch06FinalLedger(debateInputs, sourceHashes) {
  assertV4(
    Array.isArray(debateInputs) && debateInputs.length === EXPECTED_DEBATES.length,
    "ten post-canary Batch 6 debate inputs required"
  );
  const debates = debateInputs.map(compilePostCanaryBatch06FinalLedgerDebate);
  assertV4(
    debates.map((debate) => debate.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "post-canary Batch 6 final-ledger debate order invalid"
  );
  ...
  assertV4(aggregate.finalMoves === 200, "Batch 6 final-move population mismatch");
  assertV4(aggregate.disputedMoves === 197, "Batch 6 disputed-move population mismatch");
  assertV4(candidateSelections === 609, "Batch 6 candidate-selection population mismatch");
  assertV4(
    aggregate.meanMerges + aggregate.dependencyMeanMergesSuppressed === 402,
    "Batch 6 rounded-mean population mismatch"
  );
  assertV4(aggregate.audioVerifiedMoves === 2, "Batch 6 audio-verification population mismatch");
  return {
    schemaVersion: POST_CANARY_BATCH_06_FINAL_LEDGER_VERSION,
    protocolId: POST_CANARY_BATCH_06_FINAL_LEDGER_PROTOCOL_ID,
    status: "passed-post-canary-batch-06-deterministic-final-ledger-assembly",
    ...
    AIOnly: true,
    scoringInputSchema:
      "v4.2.20-source-span-primary-with-post-canary-batch-06-adjudicated-provenance",
    ...
    nextAuthorizedAction:
      "standing-authorization-permits-batch-06-single-deterministic-score-pass-preparation"
  };
}

export function validatePostCanaryBatch06FinalLedger(
  ledger,
  debateInputs,
  sourceHashes
) {
  const expected = buildPostCanaryBatch06FinalLedger(debateInputs, sourceHashes);
  assertV4(
    canonicalJson(ledger) === canonicalJson(expected),
    "post-canary Batch 6 final ledger differs from deterministic replay"
  );
  assertV4(
    !containsProhibitedCalculatedField(
      ledger.debates.map((debate) => debate.finalJudgment)
    ),
    "post-canary Batch 6 final ledger contains a prohibited calculated field"
  );
  return {
    status: "passed",
...
export async function loadPostCanaryBatch06FinalLedgerInputs() {
  ...
  assertV4(
    adjudicationAnalysis.status ===
        "post-canary-batch-06-dispute-only-adjudication-gate-passed-standing-authorization-active-for-final-ledger-assembly" &&
      adjudicationAnalysis.gate.semanticPass === true &&
      adjudicationAnalysis.gate.validContexts === 10 &&
      adjudicationAnalysis.gate.disputedMovesDecided === 197 &&
      adjudicationAnalysis.gate.candidateSelections === 609 &&
      adjudicationAnalysis.gate.scoresDerived === 0 &&
      adjudicationExecution.status ===
        "ten-post-canary-batch-06-dispute-only-adjudication-contexts-passed" &&
    ...
    "accepted post-canary Batch 6 adjudication gate unavailable"
  );
  assertV4(
    ...
    "complete Batch 6 audio verification unavailable"
  );
  ...
  assertV4(
    debateInputs.map((input) => input.primaryA.debateNumber).join(",") ===
      EXPECTED_DEBATES.join(","),
    "loaded post-canary Batch 6 debate order invalid"
  );
