...
export const POST_CANARY_BATCH_04_SCORE_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-04/score-pass";
export const POST_CANARY_BATCH_04_CALCULATED_SCORES_VERSION =
  "1.0-assessment-production-post-canary-batch-04-adjudicated-calculated-scores";
export const POST_CANARY_BATCH_04_SCORE_STABILITY_THRESHOLDS =
  ACTIVE_SCORE_STABILITY_THRESHOLDS;
...
export function evaluatePostCanaryBatch04ScoreStability(
  debates,
  thresholds = POST_CANARY_BATCH_04_SCORE_STABILITY_THRESHOLDS
) {
  assertV4(
    Array.isArray(debates) && debates.length === 10,
    "ten scored post-canary Batch 4 debates required"
  );
  assertV4(
    canonicalJson(thresholds) ===
      canonicalJson(POST_CANARY_BATCH_04_SCORE_STABILITY_THRESHOLDS),
    "post-canary Batch 4 active thresholds changed"
  );
...
export function derivePostCanaryBatch04Scores(
  ledger,
  debateInputs,
  ...
  const debates = debateInputs.map((input) => {
    ...
    assertV4(
      finalLedgerDebate,
      `${debateNumber}: Batch 4 final-ledger debate missing`
    );
  ...
  const stability = evaluatePostCanaryBatch04ScoreStability(debates);
  ...
  return {
    schemaVersion: POST_CANARY_BATCH_04_CALCULATED_SCORES_VERSION,
    ...
    status: stability.acceptancePassed
      ...
      : "post-canary-batch-04-single-score-pass-stability-gate-failed",
...
export function validatePostCanaryBatch04Scores(
  scores,
  ledger,
  debateInputs,
  productionReferences,
  ...
  const expected = derivePostCanaryBatch04Scores(
    ledger,
    debateInputs,
    productionReferences,
    hashes
  ...
  assertV4(
    ...
    "post-canary Batch 4 calculated scores differ from deterministic replay"
  );
  return {
    status: "passed",
