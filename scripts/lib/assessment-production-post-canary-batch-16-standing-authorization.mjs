...
export const POST_CANARY_BATCH_16_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-16";
export const POST_CANARY_BATCH_16_STANDING_AUTHORIZATION =
  `${POST_CANARY_BATCH_16_ROOT}/standing-authorization.json`;
export const POST_CANARY_BATCH_16_STANDING_AUTHORIZATION_PROTOCOL_ID =
  "assessment-production-post-canary-batch-16-standing-authorization";
export const POST_CANARY_BATCH_16_STANDING_AUTHORIZATION_STATUS =
  "frozen-active-batch-16-complete-remaining-workflow-standing-authorization";
export const POST_CANARY_BATCH_16_STANDING_AUTHORIZATION_INSTRUCTION_PATH =
  POST_CANARY_BATCH_16_ROOT + "/standing-authorization-instruction.txt";
export const POST_CANARY_BATCH_16_STANDING_AUTHORIZATION_INSTRUCTION = readFileSync(
  path.resolve(POST_CANARY_BATCH_16_STANDING_AUTHORIZATION_INSTRUCTION_PATH),
  "utf8"
).trimEnd();

const sha256 = (value) => createHash("sha256").update(value).digest("hex");

export function validatePostCanaryBatch16StandingAuthorization(record) {
  assertV4(
    record?.schemaVersion ===
        "1.0-assessment-production-post-canary-batch-16-standing-authorization" &&
      record.protocolId ===
        POST_CANARY_BATCH_16_STANDING_AUTHORIZATION_PROTOCOL_ID &&
      record.status === POST_CANARY_BATCH_16_STANDING_AUTHORIZATION_STATUS &&
      record.productionCanary === false &&
      record.batchNumber === 16 &&
      record.sourcePacketCommit === "055ef8bcb28264a4c22a167a0eeaa83f47a47ab4" &&
      record.stagingOnly === true &&
      !Number.isNaN(Date.parse(record.authorizedAt)) &&
      record.userAuthorization?.instruction ===
        POST_CANARY_BATCH_16_STANDING_AUTHORIZATION_INSTRUCTION &&
      record.userAuthorization?.resolvedAntecedent ===
        "The user explicitly authorized the complete Batch 16 workflow, automatic continuation across passing gates, bounded diagnosed recovery, the standing exceptional third recovery level for atomic companion-field loss, deterministic derived-file correction, production publication, validation, commits, and pushes." &&
      record.userAuthorization?.scopeInterpretation ===
        "Create and use a complete-workflow Batch 16 standing authorization for the ten frozen selections, beginning with the separately frozen score-blind source packets, including bounded diagnosed recovery and production publication, while preserving the established campaign controls and stopping before Batch 17 selection." &&
    ...
    "Batch 16 standing-authorization identity changed"
  );
  assertV4(
    canonicalJson(record.selectedDebates) ===
      canonicalJson(["16", "108", "164", "144", "76", "41", "92", "139", "163", "161"]),
    "Batch 16 standing-authorization debate boundary changed"
  );
  assertV4(
    ...
    "Batch 16 standing-authorization model changed"
  );
  assertV4(
    ...
    "Batch 16 standing-authorization execution controls changed"
  );
  assertV4(
  ...
  assertV4(
    ...
    "Batch 16 standing-authorization cost boundary changed"
  );
  assertV4(
    ...
    "Batch 16 standing-authorization source-recovery controls changed"
  );
  assertV4(
    Object.values(record.stopRules ?? {}).every(Boolean),
    "Batch 16 standing-authorization stop rule disabled"
  );
  assertV4(
    ...
    "Batch 16 standing-authorization recovery controls changed"
  );
  assertV4(
    ...
    "Batch 16 atomic-shard preservation-gap standing authorization changed"
  );
  assertV4(
    ...
    "Batch 16 deterministic correction or rendering controls changed"
  );
  assertV4(
    ...
    "Batch 16 standing-authorization Git controls changed"
  );
  assertV4(
    Object.values(record.cleanupControls ?? {}).every(Boolean),
    "Batch 16 cleanup controls changed"
  );
  return record;
}
export async function loadAndValidatePostCanaryBatch16StandingAuthorization() {
  const bytes = await readFile(
    path.resolve(POST_CANARY_BATCH_16_STANDING_AUTHORIZATION)
  );
  const record = validatePostCanaryBatch16StandingAuthorization(JSON.parse(bytes));
