...
export const V3811_PERFORMANCE_ROOT = "docs/calibration/v3.8.11/performance-judgment-consensus";
export const V3811_PERFORMANCE_DEBATES = Object.freeze(["55", "103", "161"]);
export const V3811_PERFORMANCE_PASSES = Object.freeze(["A", "B"]);
export const V3811_RATING_KEYS = Object.freeze([
  "logicalCoherence",
  "evidenceWarrant",
  "responsiveness",
  "relevanceBurden",
...
export const V3811_RESPONSE_CLASSES = Object.freeze([
...
export const V3811_RESPONSE_RANGES = Object.freeze({
...
export const V3811_BURDEN_RANGES = Object.freeze({
  motion: Object.freeze([90, 100]),
  central: Object.freeze([75, 89]),
  subsidiary: Object.freeze([55, 74]),
  none: Object.freeze([0, 54])
});
export const V3811_SCALAR_DISPUTE_THRESHOLD = 5;
export const V3811_DIAGNOSTIC_MOVE_DELTA_THRESHOLD = 4;

export function assertV3811(condition, message) {
...
export function makeV3811PerformanceSchema() {
  const rating = {
    type: "object",
  ...
  return {
    ...
    $id: "slugfester-v3811-performance-judgment-pass",
    title: "Slugfester v3.8.11 shared performance judgment pass",
    type: "object",
    additionalProperties: false,
    ...
    properties: {
      schemaVersion: { type: "string", const: "3.8.11-performance-judgment-output" },
      protocolId: { type: "string", const: "v3.8.11-performance-judgment-consensus" },
        ...
        items: {
          ...
          properties: {
            ...
            response: {
              ...
              properties: {
                class: { type: "string", enum: V3811_RESPONSE_CLASSES },
            ...
            ratings: {
              type: "object",
              additionalProperties: false,
              required: V3811_RATING_KEYS,
              properties: Object.fromEntries(V3811_RATING_KEYS.map((key) => [key, rating]))
            },
...
function assertObjectShape(object, allowedKeys, label) {
  assertV3811(object && typeof object === "object" && !Array.isArray(object), `${label}: expected object`);
  const unexpected = Object.keys(object).filter((key) => !allowedKeys.includes(key));
  assertV3811(unexpected.length === 0, `${label}: unexpected keys ${unexpected.join(", ")}`);
}

function assertString(value, minimum, label) {
  assertV3811(typeof value === "string" && value.trim().length >= minimum, `${label}: string shorter than ${minimum}`);
}

function assertRating(rating, label) {
  assertObjectShape(rating, ["value", "rationale"], label);
  assertV3811(Number.isInteger(rating.value) && rating.value >= 0 && rating.value <= 100, `${label}: value outside 0..100`);
...
export function validateV3811PerformanceOutput(output, packet, expectedPass) {
  const schema = makeV3811PerformanceSchema();
  assertObjectShape(output, schema.required, "output");
  assertV3811(output.schemaVersion === "3.8.11-performance-judgment-output", "schemaVersion mismatch");
  assertV3811(output.protocolId === "v3.8.11-performance-judgment-consensus", "protocolId mismatch");
  assertV3811(output.debateNumber === packet.debateNumber && output.debateId === packet.debateId, "debate identity mismatch");
  assertV3811(output.pass === expectedPass && V3811_PERFORMANCE_PASSES.includes(output.pass), "pass mismatch");
  assertV3811(output.reviewerRole === "performance-judge" && output.assessmentModel === "5.6 Sol" && output.calibrationOnly === true, "reviewer boundary mismatch");
  assertV3811(!containsProhibitedDerivedField(output), "model output contains a prohibited derived score or prose field");
  assertObjectShape(output.isolation, Object.keys(schema.properties.isolation.properties), "isolation");
  for (const [key, definition] of Object.entries(schema.properties.isolation.properties)) assertV3811(output.isolation[key] === definition.const, `isolation.${key} mismatch`);
  assertV3811(Array.isArray(output.moveJudgments) && output.moveJudgments.length === packet.moves.length, "move count mismatch");
  ...
  for (let index = 0; index < packet.moves.length; index += 1) {
    ...
    for (const key of ["moveId", "sectionId", "side", "speaker"]) assertV3811(judgment[key] === locked[key], `${label}.${key} differs from lock`);
    assertV3811(canonicalJson(judgment.sourceSpan) === canonicalJson(locked.sourceSpan), `${label}.sourceSpan differs from lock`);
    assertV3811(canonicalJson(judgment.lockedBurdenContact) === canonicalJson(locked.lockedBurdenContact), `${label}.lockedBurdenContact differs from lock`);
    ...
    assertV3811(V3811_RESPONSE_CLASSES.includes(response.class), `${label}: unknown response class`);
    assertV3811(Array.isArray(response.decisiveTargetIds) && new Set(response.decisiveTargetIds).size === response.decisiveTargetIds.length, `${label}: target IDs must be unique`);
    assertV3811(response.decisiveTargetIds.every((id) => locked.allowedResponseTargetIds.includes(id)), `${label}: target outside locked response links`);
    assertV3811(Number.isInteger(response.contactedComponents) && Number.isInteger(response.totalComponents), `${label}: response component counts must be integers`);
    assertV3811(response.contactedComponents >= 0 && response.totalComponents >= 0 && response.contactedComponents <= response.totalComponents, `${label}: invalid response component counts`);
    assertString(response.rationale, 60, `${label}.response.rationale`);
    const constructive = response.class === "constructive-opening";
    if (constructive) {
      assertV3811(locked.moveKind === "constructive", `${label}: only a constructive move may be constructive-opening`);
      assertV3811(response.decisiveTargetIds.length === 0 && response.contactedComponents === 0 && response.totalComponents === 0, `${label}: constructive-opening must have an empty response tuple`);
    } else {
      assertV3811(locked.moveKind !== "constructive", `${label}: constructive move assigned a responsive class`);
      assertV3811(response.decisiveTargetIds.length >= 1 && response.totalComponents >= 1, `${label}: responsive class requires a locked target and at least one component`);
    }
    if (response.class === "full-answer") {
      assertV3811(response.contactedComponents === response.totalComponents, `${label}: full answer must contact every indispensable component`);
    }
    if (response.class === "partial-answer") {
      assertV3811(response.totalComponents >= 2 && response.contactedComponents >= 1 && response.contactedComponents < response.totalComponents, `${label}: partial answer must contact some but not all of at least two components`);
    }
    if (["relevant-nonanswer", "nonanswer"].includes(response.class)) {
      assertV3811(response.contactedComponents === 0, `${label}: nonanswer class cannot contact an indispensable component`);
    }
    if (["diagnostic-defeat", "justified-reframe"].includes(response.class)) {
      assertV3811(response.contactedComponents >= 1, `${label}: diagnostic defeat or justified reframe must identify contact`);
    }
    assertObjectShape(judgment.ratings, V3811_RATING_KEYS, `${label}.ratings`);
    for (const key of V3811_RATING_KEYS) assertRating(judgment.ratings[key], `${label}.ratings.${key}`);
    const responseValue = judgment.ratings.responsiveness.value;
    const responseRange = V3811_RESPONSE_RANGES[response.class];
    assertV3811(responseValue >= responseRange[0] && responseValue <= responseRange[1], `${label}: responsiveness outside class band`);
    const burdenTier = judgment.lockedBurdenContact?.tier ?? "none";
    const burdenRange = V3811_BURDEN_RANGES[burdenTier];
    const burdenValue = judgment.ratings.relevanceBurden.value;
    assertV3811(burdenValue >= burdenRange[0] && burdenValue <= burdenRange[1], `${label}: relevance/burden outside locked tier band`);
    assertV3811(typeof judgment.charityTested === "boolean", `${label}: charityTested must be boolean`);
    if (!judgment.charityTested) {
      assertV3811(judgment.ratings.representationalCharity.value === 75, `${label}: untested charity must equal 75`);
    }
    assertString(judgment.evidenceBasis, 40, `${label}.evidenceBasis`);
    assertV3811(["high", "medium", "low"].includes(judgment.assessmentConfidence), `${label}: invalid assessment confidence`);
  }
  assertV3811(new Set(output.moveJudgments.map((move) => move.moveId)).size === packet.moves.length, "move judgments must be unique");
  ...
  assertV3811(output.audit.moveCount === packet.moves.length, "audit moveCount mismatch");
  for (const [key, definition] of Object.entries(schema.properties.audit.properties)) if (key !== "moveCount") assertV3811(output.audit[key] === definition.const, `audit.${key} mismatch`);
...
function validateAdjustment(adjustment, side, moveIds, bridgeIds) {
  ...
  assertV3811(Number.isInteger(adjustment.value) && adjustment.value >= -5 && adjustment.value <= 5, `${label}: value outside -5..5`);
  ...
  for (const key of ["distinctDebateWideConsequence", "affectsBurdenCompletion", "notAlreadyScored"]) assertV3811(typeof eligibility[key] === "boolean", `${label}.${key} must be boolean`);
  for (const key of ["affectedBurdenIds", "relatedMoveIds", "alreadyCapturedBy"]) assertV3811(Array.isArray(eligibility[key]) && new Set(eligibility[key]).size === eligibility[key].length, `${label}.${key} must be a unique array`);
  assertV3811(eligibility.affectedBurdenIds.every((id) => bridgeIds.has(id)), `${label}: unknown affected burden ID`);
  assertV3811(eligibility.relatedMoveIds.every((id) => moveIds.has(id)), `${label}: unknown related move ID`);
  for (const key of ["completionCriterion", "distinctConsequence", "counterfactual"]) assertString(eligibility[key], 1, `${label}.${key}`);
  if (eligibility.alreadyCapturedBy.length > 0 || !eligibility.notAlreadyScored) assertV3811(adjustment.value === 0, `${label}: duplicate capture forces zero`);
  if (adjustment.value !== 0) {
    assertV3811(eligibility.distinctDebateWideConsequence && eligibility.affectsBurdenCompletion && eligibility.notAlreadyScored, `${label}: nonzero value fails all-three eligibility test`);
    assertV3811(eligibility.affectedBurdenIds.length > 0 && eligibility.relatedMoveIds.length > 0 && eligibility.alreadyCapturedBy.length === 0, `${label}: nonzero value lacks named burden/move evidence or contains duplicate capture`);
...
function flatRatings(judgment) {
  return Object.fromEntries(V3811_RATING_KEYS.map((key) => [key, judgment.ratings[key].value]));
}
...
export function diagnosticMoveScore(judgment) {
  ...
  return scoreDimensions({ logicalCoherence: ratings.logicalCoherence, evidenceWarrant: ratings.evidenceWarrant, responsiveness: ratings.responsiveness, relevanceBurden: ratings.relevanceBurden, precisionClarity: ratings.precisionClarity, calibrationCharity }, "v3.8.11 diagnostic raw judgments");
}

export function extractV3811MoveDisagreement(judgmentA, judgmentB) {
  ...
  const materialRatingKeys = unequalRatingKeys.filter((key) => Math.abs(judgmentA.ratings[key].value - judgmentB.ratings[key].value) > V3811_SCALAR_DISPUTE_THRESHOLD);
  const diagnosticDelta = Math.abs(diagnosticMoveScore(judgmentA) - diagnosticMoveScore(judgmentB));
  const diagnosticTrigger = diagnosticDelta > V3811_DIAGNOSTIC_MOVE_DELTA_THRESHOLD;
  return {
    responseMismatch,
