...
export const POST_CANARY_BATCH_11_SCORE_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-11/score-pass";
export const POST_CANARY_BATCH_11_CALCULATED_SCORES_VERSION =
  "1.0-assessment-production-post-canary-batch-11-adjudicated-calculated-scores";
export const POST_CANARY_BATCH_11_SCORE_STABILITY_THRESHOLDS =
  ACTIVE_SCORE_STABILITY_THRESHOLDS;
...
export function evaluatePostCanaryBatch11ScoreStability(
  debates,
  thresholds = POST_CANARY_BATCH_11_SCORE_STABILITY_THRESHOLDS
) {
  assertV4(
    Array.isArray(debates) && debates.length === 10,
    "ten scored post-canary Batch 11 debates required"
  );
  assertV4(
    canonicalJson(thresholds) ===
      canonicalJson(POST_CANARY_BATCH_11_SCORE_STABILITY_THRESHOLDS),
    "post-canary Batch 11 active thresholds changed"
  );
...
export function derivePostCanaryBatch11Scores(
  ledger,
  debateInputs,
  ...
  const debates = debateInputs.map((input) => {
    ...
    assertV4(
      finalLedgerDebate,
      `${debateNumber}: Batch 11 final-ledger debate missing`
    );
  ...
  const stability = evaluatePostCanaryBatch11ScoreStability(debates);
  ...
  return {
    schemaVersion: POST_CANARY_BATCH_11_CALCULATED_SCORES_VERSION,
    ...
    status: stability.acceptancePassed
      ...
      : "post-canary-batch-11-single-score-pass-stability-gate-failed",
...
export function validatePostCanaryBatch11Scores(
  scores,
  ledger,
  debateInputs,
  productionReferences,
  ...
  const expected = derivePostCanaryBatch11Scores(
    ledger,
    debateInputs,
    productionReferences,
    hashes
  ...
  assertV4(
    ...
    "post-canary Batch 11 calculated scores differ from deterministic replay"
  );
  return {
    status: "passed",
