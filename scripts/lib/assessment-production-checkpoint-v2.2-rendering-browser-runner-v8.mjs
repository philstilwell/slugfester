...
import {
  validateCheckpointV22RenderingRemedyV8ViewportEvidence
...
function createHybridBrowser({ pointerBrowser, keyboardBrowser, viewport }) {
  ...
  return {
    adapter: {
      capabilities: {
        async get(name) {
          assertV4(name === "viewport", "v8 viewport capability required");
          return {
            async set() {
              viewportIndex += 1;
              if (viewportIndex === 1) {
              ...
              assertV4(viewportIndex === 2, "v8 viewport phase overflow");
      ...
      tabs: {
        async new() {
          ...
          assertV4(tabIndex === 2, "v8 tab phase overflow");
          return wrapTab(await keyboardBrowser.tabs.new(), "keyboard", stats);
        }
      }
    },
    snapshot() {
      assertV4(viewportIndex === 2 && tabIndex === 2, "v8 phases incomplete");
      return structuredClone(stats);
    }
  };
}

export async function runCheckpointV22RenderingRemedyV8Viewport({
  pointerBrowser,
  keyboardBrowser,
  packet,
  viewportName,
  ...
  assertV4(viewport, `${packet.debateNumber}: unknown v8 viewport`);
  const hybrid = createHybridBrowser({ pointerBrowser, keyboardBrowser, viewport });
  ...
  const evidence = await runCheckpointV22RenderingRemedyV6Viewport({
    ...
    viewportName,
    browserName: "split-Chromium-rendering-and-keyboard-transport"
  });
