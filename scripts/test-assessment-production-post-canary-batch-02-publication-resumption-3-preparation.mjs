...
const MANIFEST =
  `${POST_CANARY_BATCH_02_PUBLICATION_RESUMPTION_3_ROOT}/execution-preparation-manifest.json`;
...
assert.equal(manifest.userAuthorization.contextsPrepared, 5);
assert.equal(manifest.userAuthorization.existingPacketsReused, 5);
...
assert.equal(manifest.totals.acceptedDebates, 5);
assert.equal(manifest.totals.acceptedMoves, 93);
assert.equal(manifest.totals.resumptionContexts, 5);
assert.equal(manifest.totals.resumptionMoves, 97);
assert.equal(manifest.totals.resumptionSections, 26);
assert.equal(manifest.totals.resumptionAudioVerifiedMoves, 8);
...
assert.equal(moves, 97);
assert.equal(sections, 26);
