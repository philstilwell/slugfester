...
assertV4(!(await exists(activationPath)), "Debate 27 correction activation already exists");
...
assertV4(prep.status ===
  ...
  prep.contexts.length === 1 && prep.contexts[0].candidateSelections === 70 &&
  ...
  "Debate 27 correction not prepared");
for (const [file, digest] of Object.entries(prep.sourceHashes))
  assertV4(sha256(await readFile(file)) === digest, `source drift: ${file}`);
const activation = {
  ...prep,
  schemaVersion: "1.0-assessment-production-post-canary-batch-03-debate-27-adjudication-correction-activation",
  status: "frozen-one-batch-03-debate-27-full-packet-adjudication-correction-context-authorized",
  authorizedAt,
  ...
  nextAuthorizedAction: "execute-one-fresh-debate-27-full-packet-adjudication-correction-context"
};
...
console.log(JSON.stringify({ status: shouldWrite ? "frozen-authorized" : "preview",
  contexts: 1, candidateSelections: 70, attemptsPerContext: 1,
