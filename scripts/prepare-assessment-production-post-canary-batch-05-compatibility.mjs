...
import {
  POST_CANARY_BATCH_05_COMPATIBILITY_ORDER,
  POST_CANARY_BATCH_05_COMPATIBILITY_PROTOCOL_ID,
  POST_CANARY_BATCH_05_COMPATIBILITY_ROOT,
  POST_CANARY_BATCH_05_SITE_LEDGER_ADAPTER_VERSION,
  buildPostCanaryBatch05SiteLedgerAdapter,
  serializedJson,
  sha256,
...
const paths = {
  preparation: `${POST_CANARY_BATCH_05_COMPATIBILITY_ROOT}/preparation-manifest.json`,
  analysis: `${POST_CANARY_BATCH_05_COMPATIBILITY_ROOT}/analysis.json`,
  activation: `${POST_CANARY_BATCH_05_COMPATIBILITY_ROOT}/execution-activation.json`,
  execution: `${POST_CANARY_BATCH_05_COMPATIBILITY_ROOT}/execution.json`,
  stagedLedgerRoot:
    `${POST_CANARY_BATCH_05_COMPATIBILITY_ROOT}/output-bundle/staged-ledgers`,
  compatibility:
    "docs/assessment-production/post-canary-continuation-v1/batch-05/publication-finalization/compatibility-analysis.json",
  finalizationAudit:
    "docs/assessment-production/post-canary-continuation-v1/batch-05/publication-finalization/output-bundle/finalization-audit.json",
  finalLedger:
  ...
  renderingAnalysis:
    "docs/assessment-production/post-canary-continuation-v1/batch-05/rendering-verification/resumption-1/analysis.json",
  renderingAudit:
    "docs/assessment-production/post-canary-continuation-v1/batch-05/rendering-verification/resumption-1/rendering-audit.json",
  ...
  batchFinalLedgerLibrary:
    "scripts/lib/assessment-production-post-canary-batch-05-final-ledger.mjs",
  ...
  preparationScript:
    "scripts/prepare-assessment-production-post-canary-batch-05-compatibility.mjs",
  preparationTest:
    "scripts/test-assessment-production-post-canary-batch-05-compatibility-preparation.mjs",
...
assertV4(
  ...
  "a stable --frozen-at ISO timestamp is required for the first Batch 5 compatibility preparation write"
);
...
assertV4(
  renderingAnalysis.status ===
      "ten-debate-batch-05-rendering-resumption-1-passed" &&
    ...
    renderingAnalysis.compatibilityBoundary?.productionMutationBlocked ===
      true &&
    renderingAnalysis.authorization?.compatibilityPlanPreparation === true &&
  ...
  "Batch 5 rendering gate does not authorize compatibility planning only"
);
assertV4(
  renderingAudit.status ===
      "passed-ten-debate-batch-05-rendering-resumption-1" &&
...
assertV4(
  ...
  "frozen Batch 5 publication compatibility blocker required"
);
assertV4(
  finalizationAudit.status ===
      "passed-ten-debate-batch-05-publication-finalization" &&
  ...
  "passed one-pass Batch 5 staging-only finalization audit required"
);
assertV4(
  scores.status ===
      "post-canary-batch-05-single-score-pass-stability-gate-passed" &&
  ...
  "accepted one-pass Batch 5 score ledger required"
);
validatePostCanaryBatch05FinalLedger(
  finalLedger,
...
const packetRecords = [];
for (const debateNumber of POST_CANARY_BATCH_05_COMPATIBILITY_ORDER) {
  ...
  const packetPath =
    `${POST_CANARY_BATCH_05_COMPATIBILITY_ROOT}/packets/debate-${debateNumber}.json`;
  ...
  const adapter = buildPostCanaryBatch05SiteLedgerAdapter({
    finalLedgerDebate,
    scoreDebate,
    candidate,
  ...
  const packet = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-05-compatibility-plan-packet",
    protocolId: POST_CANARY_BATCH_05_COMPATIBILITY_PROTOCOL_ID,
    status: "frozen-post-canary-batch-05-compatibility-plan-packet",
...
assertV4(
  totals.debates === 10 &&
    ...
    totals.overallBlunders === 64 &&
    totals.emptyReferenceLinks === 59 &&
    totals.suppliedReferenceLinks === 5,
  "Batch 5 compatibility population changed"
);

const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-05-compatibility-preparation-manifest",
  protocolId: POST_CANARY_BATCH_05_COMPATIBILITY_PROTOCOL_ID,
  status: "post-canary-batch-05-compatibility-plan-prepared-and-frozen",
  frozenAt,
  productionCanary: false,
  batchNumber: 5,
  planningOnly: true,
  ...
  userAuthorization: {
    instruction:
      "Use the frozen Batch 5 standing authorization for deterministic compatibility staging.",
    scopeInterpretation:
      "Prepare, validate, freeze, commit, and push the Batch 5 production-compatibility plan; later activation and one staging pass are already covered by the standing authorization. Do not publish production ledgers, mutate production, run models, use paid services, or select Batch 6."
  },
  scope: {
    finding: "batch-05-site-ledger-adapter-and-validator-route",
    description:
      "Freeze the exact Batch 5 ledger adapters and schema-specific validator route without applying them to the active validator or production data."
  },
  invariants: {
    ...
    scoreProtocolId:
      "assessment-production-post-canary-batch-05-single-deterministic-score-pass",
  ...
  proposedValidatorRoute: {
    ...
    newSchemaVersion: POST_CANARY_BATCH_05_SITE_LEDGER_ADAPTER_VERSION,
    existingCheckpointSchemaVersion:
      "1.0-production-checkpoint-v2.2-site-ledger-adapter",
    exactRoute:
      "When a production ledger uses the Batch 5 adapter schema, load the Batch 5 packet and activation from the frozen Batch 5 compatibility root, verify their hashes, and replay the adapter with the Batch 5 validator. Keep the checkpoint and Batch 1–4 adapters on their existing roots, credentials, and validators, and keep legacy Rubric v2 and v2.1 ledgers on their existing calculators.",
    oneSidedDisplayRows:
      "Treat all six authenticated adjudicated-consensus adapter schemas as requiring ledgerMoveId and permitting a section exchange with only one side populated.",
    activationRequirements: [
      "activation status is post-canary-batch-05-compatibility-execution-authorized-and-frozen",
  ...
  stagedExecutionPlan: [
    ...
    },
    {
      ...
      allowedWrites: [
        paths.activeValidator,
        `${POST_CANARY_BATCH_05_COMPATIBILITY_ROOT}/output-bundle/**`,
        paths.execution,
        paths.analysis
      ],
      actions: [
        "wire the schema-specific Batch 5 adapter route",
  ...
  mandatoryExecutionTests: {
    positive: [
      "all ten Batch 5 adapters reproduce all 187 locked move scores, 49 section scores per side, and 20 overall scores",
      "all ten final candidates match ledgerMoveId, section order, model, rubric, and score protocol",
      "all 7 one-sided display rows pass through the authenticated Batch 5 adapter route",
      "all 59 empty Overall Commentary reference arrays pass without synthesized links",
      "all 5 supplied reference links retain full validation",
    ...
    negative: [
      ...
      "a Batch 5 adapter routed through the checkpoint or Batch 1–4 roots fails",
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-05-compatibility-preparation-analysis",
  protocolId: POST_CANARY_BATCH_05_COMPATIBILITY_PROTOCOL_ID,
  status: "post-canary-batch-05-compatibility-plan-freeze-passed",
  productionCanary: false,
  batchNumber: 5,
  planningOnly: true,
  preparation: {
...
if (write) {
  assertV4(
    ...
    "Batch 5 compatibility execution artifact already exists; planning write blocked"
  );
  await mkdir(
    resolve(`${POST_CANARY_BATCH_05_COMPATIBILITY_ROOT}/packets`),
    { recursive: true }
  );
  ...
  assertV4(
    ...
    "stored Batch 5 compatibility preparation differs from deterministic replay"
  );
  const storedAnalysis = await readJson(paths.analysis);
  assertV4(
    canonicalJson(storedAnalysis) === canonicalJson(analysis),
    "stored Batch 5 compatibility analysis differs from deterministic replay"
  );
  for (const record of packetRecords) {
    const storedPacket = await readFile(resolve(record.packetPath), "utf8");
    assertV4(
      storedPacket === record.packetBytes,
      `${record.debateNumber}: stored Batch 5 compatibility packet differs from deterministic replay`
    );
  }
}
