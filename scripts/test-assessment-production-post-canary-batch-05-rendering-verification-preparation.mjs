...
assert.equal(preparation.protocolId, POST_CANARY_BATCH_05_RENDERING_PROTOCOL_ID);
assert.equal(
  preparation.status,
...
assert.deepEqual(preparation.explicitOrder, [...POST_CANARY_BATCH_05_RENDERING_ORDER]);
...
assert.equal(
  ...
  canonicalJson(POST_CANARY_BATCH_05_RENDERING_VIEWPORTS)
);
assert.equal(
...
assert.equal(preparation.browserPlan.localServer.port, POST_CANARY_BATCH_05_RENDERING_PORT);
...
assert.equal(
  preparation.toolHashes[POST_CANARY_BATCH_05_RENDERING_IMAGE_CONTRACT.analyzerPath],
...
for (const row of preparation.packets) {
  ...
  assert.equal(packet.preview.url.includes(`127.0.0.1:${POST_CANARY_BATCH_05_RENDERING_PORT}`), true);
...
assert.equal(sectionTotal, 49);
assert.equal(moveTotal, 187);

const previewHtml = await readFile(path.resolve(preparation.inputs.localPreview), "utf8");
assert.match(previewHtml, /validated post-canary Batch 5 candidate only/);
...
console.log(
  JSON.stringify(
    {
      status: "post-canary-batch-05-rendering-preparation-passed",
