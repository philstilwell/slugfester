...
assertV4(
  activation.authorization.analysis === true &&
    ...
    execution.repositoryMaterializedSourceWindows === true &&
    execution.modelAuthoredEndEvents === false,
...
for (const debate of sourcePreparation.contexts) {
  ...
  for (const chunk of debate.chunks) {
    ...
    const validation = validateV211Discovery(output, {
      packet,
      chunk,
      plan,
      eventsDocument,
    ...
    materializedWindows += validation.materializedWindows.length;
    outputs.push(output);
  }
  const bundle = compileV211CandidateBundle({
    packet,
    plan,
    outputs,
    eventsDocument,
  ...
  assertV4(
    bundle.protocolId === V211_DISCOVERY_PROTOCOL_ID &&
      bundle.completeSourceDiscovery.repositoryMaterializedSourceWindows ===
        true &&
      bundle.completeSourceDiscovery.modelAuthoredEndEvents === false &&
      bundle.candidateCount === materializedWindows,
    `${debate.debateNumber}: successor bundle materialization drifted`
  );
  ...
  debates.push({
    ...
    repositoryMaterializedSourceWindows: materializedWindows,
...
const analysis = {
  schemaVersion:
    "1.0-score-stability-v2.1.1-fresh-validation-discovery-analysis",
  ...
  audit: {
    ...
    repositoryMaterializedEndEvent:
      activation.schemaHardening.repositoryMaterializedEndEvent,
    modelAuthoredEndEventProhibited:
      activation.schemaHardening.modelAuthoredEndEventProhibited,
    minimumRequestedLexicalTokens:
      activation.schemaHardening.minimumRequestedLexicalTokens,
  ...
  totals: {
    ...
    repositoryMaterializedSourceWindows: debates.reduce(
      (sum, debate) => sum + debate.repositoryMaterializedSourceWindows,
      0
    ),
    modelAuthoredEndEvents: 0,
  ...
  nextAuthorizedAction:
    "prepare-v2.1.1-candidate-sharded-inventory-packets-model-free-only",
};
if (shouldWrite) {
