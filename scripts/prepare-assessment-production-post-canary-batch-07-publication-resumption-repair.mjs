...
import { POST_CANARY_BATCH_07_PUBLICATION_MODEL,
  POST_CANARY_BATCH_07_PUBLICATION_ROOT } from
...
import { POST_CANARY_BATCH_07_DEBATE_80_REPAIR_OUTPUT_VERSION,
  POST_CANARY_BATCH_07_DEBATE_80_REPAIR_PACKET_VERSION,
  POST_CANARY_BATCH_07_DEBATE_80_REPAIR_PROTOCOL_ID,
  POST_CANARY_BATCH_07_DEBATE_80_REPAIR_ROOT,
  buildDebate80RepairSchema, debate80RepairMoveId,
  mergeAndValidateDebate80Repair, validateDebate80RepairOutput } from
...
const BASE_OUTPUT = `${RESUMPTION_ROOT}/outputs/debate-80.json`;
const PUBLICATION_PACKET = `${POST_CANARY_BATCH_07_PUBLICATION_ROOT}/packets/debate-80.json`;
...
const STATIC = [DIAGNOSIS, BASE_OUTPUT, PUBLICATION_PACKET, PRODUCTION_WORKFLOW,
  READINESS_WORKFLOW, OUTPUT_CONTRACT, MANUAL, POST_CANARY_BATCH_07_STANDING_AUTHORIZATION,
  ...
  "scripts/lib/assessment-production-post-canary-batch-07-publication-resumption-repair.mjs",
  "scripts/lib/assessment-production-post-canary-batch-07-standing-authorization.mjs",
  "scripts/diagnose-assessment-production-post-canary-batch-07-publication-resumption-debate-80.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-07-publication-resumption-repair.mjs",
  "scripts/test-assessment-production-post-canary-batch-07-publication-resumption-repair-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-07-publication-resumption-repair.mjs",
  "scripts/run-assessment-production-post-canary-batch-07-publication-resumption-repair.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-07-publication-resumption-repair.mjs"];
...
assertV4(diagnosis.status === "frozen-diagnosed-batch-07-debate-80-five-critique-word-overruns" &&
  diagnosis.failedFieldCount === 5 && diagnosis.excessWordsTotal === 7 &&
  diagnosis.diagnosticReplay?.result?.status === "passed" &&
  diagnosis.minimumBoundedRepair?.packetCount === 3 &&
  diagnosis.preservedFields?.eightContextsUnattempted === true,
"the frozen Debate 80 diagnosis changed");
...
for (let packetIndex = 0; packetIndex < 3; packetIndex += 1) {
  ...
  const corrections = writableFields.map((field) => {
    const moveId = debate80RepairMoveId(field);
  ...
  const packet = { schemaVersion: POST_CANARY_BATCH_07_DEBATE_80_REPAIR_PACKET_VERSION,
    protocolId: POST_CANARY_BATCH_07_DEBATE_80_REPAIR_PROTOCOL_ID,
    contextIndex: packetIndex, packetIndex, productionCanary: false, batchNumber: 7,
    stagingOnly: true, debateNumber: "80", debateId: publicationPacket.debateId,
  ...
  const schema = buildDebate80RepairSchema(packet);
  ...
  contexts.push({ contextIndex: packetIndex, packetIndex, debateNumber: "80",
  ...
  const outputRecord = { schemaVersion: POST_CANARY_BATCH_07_DEBATE_80_REPAIR_OUTPUT_VERSION,
    protocolId: POST_CANARY_BATCH_07_DEBATE_80_REPAIR_PROTOCOL_ID,
    packetIndex, debateNumber: "80", debateId: packet.debateId,
    assessmentModel: POST_CANARY_BATCH_07_PUBLICATION_MODEL.label,
    completedAt: frozenAt, correctedCritiques };
  validateDebate80RepairOutput(outputRecord, packet); synthetic.push(outputRecord);
}
assertV4(contexts.length === 3 && new Set(contexts.flatMap((row) => row.writableFields)).size === 5,
  "the three-packet repair boundary changed");
const syntheticMerge = mergeAndValidateDebate80Repair({ baseOutput,
  repairOutputs: synthetic, repairPackets: packets, publicationPacket });
assertV4(syntheticMerge.fullValidation.status === "passed" &&
  syntheticMerge.fullValidation.moves === 19, "synthetic complete Debate 80 replay failed");
const futureOutputs = [`${ROOT}/execution-activation.json`, `${ROOT}/model-execution.json`,
  ...
  `${ROOT}/merged/debate-80.json`, `${ROOT}/complete-debate-validation.json`, `${ROOT}/merge-audit.json`];
...
const manifest = { schemaVersion: "1.0-assessment-production-post-canary-batch-07-debate-80-repair-preparation",
  protocolId: POST_CANARY_BATCH_07_DEBATE_80_REPAIR_PROTOCOL_ID,
  status: "frozen-three-context-batch-07-debate-80-publication-repair-prepared-not-activated",
  ...
  costEstimate: { authentication: "ChatGPT subscription", contexts: 3,
  ...
  isolation: { freshTemporaryWorkingDirectoryPerContext: true,
    ...
    APIKeysRemoved: true },
  repairContract: { contexts: 3, writableFields: 5, maximumWritableFieldsPerPacket: 2,
  ...
  executionPolicy: { contexts: 3, attemptsPerContext: 1, retriesMaximum: 0,
    ...
    rampPhases: [{ phase: "repair-first", maximumParallelContexts: 1,
      contextIndexes: [0], expansionRequiresAllValid: true },
    { phase: "repair-second-and-third", maximumParallelContexts: 2,
  ...
  authorization: { executionActivationPreparation: true, repairModelExecution: false,
    deterministicMergeAndValidation: false, eightContextResumptionPreparation: false,
    paidServices: false, productionMutation: false, nextBatchSelection: false },
  nextAuthorizedAction: "activate-and-execute-exactly-three-frozen-debate-80-repair-contexts" };
...
console.log(JSON.stringify({ status: manifest.status, contextsPrepared: 3,
  writableFieldsPrepared: 5, completeSyntheticReplay: "passed",
