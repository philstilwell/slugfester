...
const TOOL_SOURCES = [
  "scripts/extract-assessment-production-post-canary-batch-05-disagreements.mjs",
  ...
  "scripts/test-assessment-production-post-canary-batch-05-disagreements.mjs"
];
...
assertV4(
  ...
  "Batch 5 accepted-judgment gate or prior stop rule changed"
);
assertV4(
  JSON.stringify(judgmentAnalysis.pairs.map((pair) => pair.debateNumber)) ===
    JSON.stringify(EXPECTED_DEBATES),
  "Batch 5 independent-judgment population changed"
);
assertV4(
  ...
  "Batch 5 does not contain ten accepted, independent A/B judgment pairs"
);
assertV4(
  ...
  "Batch 5 twenty-context accepted execution contract changed"
);
assertV4(
  ...
  "Batch 5 model, authentication, score-blindness, or tie contract changed"
);
assertV4(
  executionActivation.executionPolicy.attemptsPerContext === 1 &&
    ...
    JSON.stringify(executionActivation.executionPolicy.schedulerRamp) ===
      JSON.stringify([1, 2]),
  "Batch 5 one-attempt or frozen 1-to-2 scheduler stop rule changed"
);
assertV4(
...
assertV4(
  ...
  "Batch 5 accepted execution evidence changed"
);
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const disagreementPath =
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_05_DISAGREEMENT_ROOT}/disagreements/debate-${debateNumber}.json`;
...
assertV4(totalMoves === 187, "Batch 5 extraction must compare 187 moves");
assertV4(
  JSON.stringify(extractedAudioQueue.map(queueKey).sort()) ===
    JSON.stringify(independentGateQueue.map(queueKey).sort()),
  "Batch 5 frozen audio queue was not reproduced"
);

const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-05-disagreement-extraction",
  protocolId:
    ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_05_DISAGREEMENT_PROTOCOL_ID,
  status:
...
if (shouldWrite) {
  await mkdir(ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_05_DISAGREEMENT_ROOT, {
    recursive: true
  });
  await writeFile(
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_05_DISAGREEMENT_ROOT}/analysis.json`,
