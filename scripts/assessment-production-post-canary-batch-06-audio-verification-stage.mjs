...
const stageRoot = "docs/assessment-production/post-canary-continuation-v1/batch-06/audio-verification";
const paths = {
...
const expectedDebates = ["97"];
const expectedMoves = [
  "con-classical-suppression-and-absorption",
  "pro-secularism-theological-descent"
];
const toolPath = "scripts/assessment-production-post-canary-batch-06-audio-verification-stage.mjs";
...
const standingAuthorization = await loadAndValidatePostCanaryBatch06StandingAuthorization();

async function validatePreparation(preparation) {
  assert.equal(
    preparation.status,
    "prepared-two-post-canary-batch-06-paid-known-speaker-diarizations-standing-authorization-conditional-activation-ready"
  );
  ...
  assert.equal(preparation.costEstimate.primaryExpectedFutureExecutionCostUsd, 0.1114675);
...
async function activate() {
  ...
  const activation = {
    ...preparation,
    schemaVersion: "1.0-assessment-production-post-canary-batch-06-audio-verification-execution-manifest",
    status: "frozen-two-post-canary-batch-06-paid-known-speaker-diarizations-authorized-under-standing-authorization",
    activatedAt,
    ...
    userExecutionAuthorization: {
      ...
      frozenUsageDerivedEstimateUsd: 0.1114675,
      verificationCallsAuthorized: 2,
    ...
    canonicalSourceGate: {
      ...
      transcriptHashesVerified: 1,
      eventHashesVerified: 1,
      manifestHashesVerified: 1,
      dyadicDebatesVerified: 1
    },
    executionToolHashes,
    costEstimate: {
    ...
    nextAuthorizedAction: "execute-exactly-two-post-canary-batch-06-paid-audio-verification-calls-once-sequentially"
  };
  ...
  console.log(JSON.stringify({
    ...
    activatedAt,
    calls: 2,
    expectedCostUsd: 0.1114675,
    maximumAuthorizedCostUsd: 1,
    canonicalSourceHashesVerified: 3,
    sequentialExecution: true,
    attemptsPerCall: 1,
    retriesMaximum: 0,
...
async function validateActivation(manifest) {
  ...
  assert.equal(manifest.costEstimate.primaryExpectedFutureExecutionCostUsd, 0.1114675);
...
async function run() {
  ...
  for (const [callIndex, call] of manifest.calls.entries()) {
    ...
    process.stdout.write(`[batch-06-audio] ${callIndex + 1}/2 starting ${call.debateNumber}:${call.moveId}\n`);
    ...
    results.push(result);
    process.stdout.write(`[batch-06-audio] ${callIndex + 1}/2 ${result.status}; cumulative $${result.cumulativeUsageDerivedEstimatedCostUsd.toFixed(7)}\n`);
  }
  ...
  const execution = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-06-audio-verification-model-execution",
    protocolId: manifest.protocolId,
    status: completed.length === 2
      ? "two-post-canary-batch-06-paid-known-speaker-diarizations-completed"
      : "post-canary-batch-06-paid-diarization-incomplete",
    provider: "OpenAI Transcription API",
...
async function analyze() {
  ...
  const executionComplete = execution.callsCompleted === 2;
  const passed = executionComplete && verified === 2;
  const status = passed
    ? "passed-all-two-post-canary-batch-06-confidence-moves-audio-verified"
    : executionComplete
      ? "post-canary-batch-06-audio-verification-unresolved"
      : "post-canary-batch-06-audio-verification-incomplete";
  const nextAuthorizedAction = execution.costCapReachedOrExceeded || !execution.directIncrementalCostCapControlPassed
    ? "user-review-required-before-any-batch-06-downstream-work-after-audio-cost-cap-event"
    : passed
      ? "prepare-freeze-and-push-batch-06-dispute-only-adjudication-packets-under-standing-authorization"
      : "standing-authorization-stop-new-approval-required-before-batch-06-audio-verification-failure-diagnosis";
  const authorization = {
  ...
  const audit = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-06-audio-verification-audit",
    protocolId: manifest.protocolId,
    status,
  ...
  const analysis = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-06-audio-verification-analysis",
    protocolId: manifest.protocolId,
    status,
    productionCanary: false,
    batchNumber: 5,
...
async function cost() {
  ...
  const result = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-06-audio-cost-control-analysis",
    ...
    costControl: {
      ...
      usageDerivedPlanningEstimateUsd: 0.1114675,
      approvedMaximumCostUsd: 1,
      usageDerivedEstimatedCostUsd: totals.usageDerivedEstimatedCostUsd,
      estimateDifferenceUsd: totals.usageDerivedEstimatedCostUsd - 0.1114675,
...
async function test() {
  ...
  if (!(await exists(paths.activation))) {
    console.log(JSON.stringify({ status: "passed-preactivation", calls: 2, paidCalls: 0 }, null, 2));
    return;
  }
  ...
  if (!(await exists(paths.execution))) {
    ...
    console.log(JSON.stringify({ status: "passed-activated", calls: 2, retries: 0, maximumAuthorizedCostUsd: 1 }, null, 2));
    return;
  }
  ...
  assert(execution.callsAttempted <= 2);
  assert.equal(execution.retries, 0);
  ...
  if (!execution.requestFailure && !execution.costCapReachedOrExceeded) {
    assert.equal(execution.callsAttempted, 2);
    assert.equal(execution.callsCompleted, 2);
  ...
  assert.equal(audit.totals.verified + audit.totals.unresolved, 2);
  assert.equal(audit.totals.retries, 0);
  assert.equal(analysis.gate.requiredMoves, 2);
  assert.equal(analysis.gate.verified + analysis.gate.unresolved, 2);
  ...
  assert.equal(costResult.costControl.usageDerivedPlanningEstimateUsd, 0.1114675);
