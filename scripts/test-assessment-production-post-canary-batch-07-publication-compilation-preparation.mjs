...
import { POST_CANARY_BATCH_07_PUBLICATION_COMPILATION_ORDER,
  ...
  compilePostCanaryBatch07PublicationStagingRecord } from
...
assertV4(m.status === "frozen-post-canary-batch-07-deterministic-publication-compilation-prepared" &&
  canonicalJson(m.explicitOrder) === canonicalJson(POST_CANARY_BATCH_07_PUBLICATION_COMPILATION_ORDER) &&
  m.contexts?.length === 10 && m.contexts.reduce((sum, row) => sum + row.expectedMoves, 0) === 187 &&
...
"Batch 7 compilation preparation changed");
let moves = 0;
for (const context of m.contexts) {
  ...
  const compiled = compilePostCanaryBatch07PublicationStagingRecord({ output, packet, identity: row });
  const validation = validatePostCanaryBatch07CompiledStagingRecord({ compiled, output, packet, identity: row });
  moves += validation.moves;
}
assertV4(moves === 187, "compiled move replay changed");
