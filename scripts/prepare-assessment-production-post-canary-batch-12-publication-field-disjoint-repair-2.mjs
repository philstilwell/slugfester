...
} from "./lib/assessment-production-post-canary-batch-12-publication-field-disjoint-repair-2.mjs";
...
const RESUMPTION_ROOT = `${PUBLICATION_ROOT}/failure-recovery/original-unattempted-context-resumption-1`;
...
assertV4(
  originalExecution.status === "nine-context-publication-resumption-stopped-with-failure" &&
    originalExecution.contextsAttempted === 1 && originalExecution.contextsUnattempted === 8 &&
    ...
    originalAnalysis.status === "nine-context-publication-resumption-failed",
...
const debate28 = allArtifacts.get("28");
const structuralStandIn = Object.values(debate28.outputJson.moveProse)
  .map((entry) => entry.critique)
  .find((critique) => {
    const words = wordCount(critique);
    return words >= 105 && words <= 130 && critique.length >= 880;
  });
assertV4(structuralStandIn, "Debate 28 has no validation-clean diagnostic stand-in");

const expectedCritiqueCounts = { "28": 1 };
const diagnostics = [];
...
assertV4(diagnostics.reduce((sum, item) => sum + item.invalidCritiques.length, 0) === 1 &&
  diagnostics.reduce((sum, item) => sum + item.invalidQuotes.length, 0) === 0,
"diagnosed one-field repair scope changed");
...
assertV4(contexts.length === 1 &&
  ...
  contexts.reduce((sum, context) => sum + context.writableFieldCount, 0) === 1 &&
  new Set(contexts.flatMap((context) => context.writableFields)).size === 1,
"one-shard, one-field coverage changed");

const sourceScripts = [
  "scripts/lib/assessment-production-post-canary-batch-12-publication-field-disjoint-repair-2.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-12-publication-field-disjoint-repair-2.mjs",
  "scripts/activate-assessment-production-post-canary-batch-12-publication-field-disjoint-repair-2.mjs",
  "scripts/run-assessment-production-post-canary-batch-12-publication-field-disjoint-repair-2.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-12-publication-field-disjoint-repair-2.mjs",
...
const manifest = {
  ...
  protocolId: PROTOCOL_ID,
  status: "frozen-one-context-one-field-batch-12-publication-repair-level-2-prepared-not-activated",
  frozenAt,
  ...
  userAuthorization: {
    ...
    resolvedScope: "one final level-2 field-disjoint shard correcting exactly one rejected Debate 28 critique while retaining every validation-clean field",
  ...
  diagnosis: {
    status: "deterministically-confirmed-one-critique-level-2-failure",
    debates: diagnostics,
    totals: { debates: 1, invalidCritiques: 1, invalidQuotes: 0, writableFields: 1 },
    everyNonTargetFieldStructurallyValidated: true,
    temporaryStandInsWrittenOrRetained: false
  },
  diagnosticStandIn: {
    source: "validation-clean-critique-from-the-same-preserved-Debate-28-output",
  ...
  retentionContract: {
    ...
    completeTenDebateCohortReplayDeferredUntilEightUnattemptedContextsComplete: true
  },
  ...
  totals: {
    debates: 1, contexts: 1, critiqueFields: 1, quoteFields: 0, writableFields: 1,
  ...
  nextAuthorizedAction: "commit-and-push-frozen-final-level-2-shard-then-activate"
};
