...
import { V388_ADJUDICATION_ROOT, makeV388AdjudicationSchema } from "./lib/v388-performance-adjudication.mjs";

const root = process.cwd();
const recoveryRoot = `${V388_PERFORMANCE_ROOT}/validated-recovery`;
const disagreementsPath = `${V388_PERFORMANCE_ROOT}/initial-disagreements.json`;
const audioAuditPath = `${V388_PERFORMANCE_ROOT}/audio-verification.json`;
const schemaPath = `${V388_ADJUDICATION_ROOT}/adjudication-schema.json`;
...
assertV388(!(await exists(V388_ADJUDICATION_ROOT)), `${V388_ADJUDICATION_ROOT} already exists`);
const [disagreements, audioAudit] = await Promise.all([readJson(disagreementsPath), readJson(audioAuditPath)]);
assertV388(disagreements.status === "passed-deterministic-disagreement-extraction" && disagreements.authorization.prepareDisputeOnlyAdjudicationPackets && !disagreements.authorization.adjudicationModelExecution && !disagreements.authorization.scoreDerivation, "disagreement artifact does not authorize preparation");
assertV388(audioAudit.status === "passed-all-medium-confidence-moves-audio-verified" && audioAudit.verifiedMoves === 17 && audioAudit.authorization.prepareDisputeOnlyAdjudicationPackets && !audioAudit.authorization.adjudicationModelExecution, "audio verification does not authorize preparation");
...
await mkdir(path.resolve(root, `${V388_ADJUDICATION_ROOT}/packets`), { recursive: true });
...
for (const debateDisagreements of disagreements.debates) {
  ...
  for (const dispute of debateDisagreements.moveDisputes) {
    ...
    assertV388(Number.isInteger(index), `${debateNumber}:${dispute.moveId}: locked move missing`);
    ...
    if (charityMismatch) {
      ...
      assertV388(charityPair.candidate1.charityTested !== charityPair.candidate2.charityTested, `${dispute.moveId}: charity pair lacks a flag dispute`);
      for (const candidate of [charityPair.candidate1, charityPair.candidate2]) if (!candidate.charityTested) assertV388(candidate.representationalCharity === 75, `${dispute.moveId}: untested charity candidate is not 75`);
    }

    const audioVerification = audioByMoveId.get(dispute.moveId) ?? null;
    if (audioVerification) {
      assertV388(audioVerification.verificationStatus === "passed-audio-derived-transcript-matches-locked-excerpt", `${dispute.moveId}: invalid audio verification`);
  ...
  const packet = {
    schemaVersion: "3.8.8-performance-adjudication-packet",
...
assertV388(disputedMoves === disagreements.summary.disputedMoves, "adjudication packets changed disputed move population");
assertV388(responseTupleChoices === disagreements.summary.responseTupleDisputes && charityPairChoices === disagreements.summary.charityTestedDisputes, "adjudication compound choice counts mismatch");
assertV388(dependencyAddedCharityRatings === 2, "expected exactly two dependency-added charity rating fields");
assertV388(ratingChoices + 4 === disagreements.summary.ratingFieldDisputes, "four exposed charity ratings must be absorbed into compound pairs");
assertV388(burdenAdjustmentChoices === disagreements.summary.burdenAdjustmentDisputes, "burden adjustment dispute count mismatch");

const preparation = {
  schemaVersion: "3.8.8-performance-adjudication-preparation-audit",
  protocolId: "v3.8.8-performance-judgment-consensus",
  ...
  dependencyClosure: {
    ...
    exposedCharityRatingsAbsorbedIntoCompoundPairs: 4,
