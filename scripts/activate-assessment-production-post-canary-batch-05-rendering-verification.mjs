...
assertV4(
  ...
  "frozen Batch 5 rendering preparation required"
);
execFileSync(
  process.execPath,
  [
...
for (const [file, digest] of Object.entries(preparation.sourceHashes)) {
  assertV4(
    ...
    `source hash mismatch before Batch 5 rendering activation: ${file}`
  );
}
for (const [file, digest] of Object.entries(preparation.toolHashes)) {
  assertV4(
    sha256(await readFile(path.resolve(file))) === digest,
    `tool hash mismatch before Batch 5 rendering activation: ${file}`
  );
}
for (const row of preparation.packets) {
  ...
  assertV4(
    ...
    `${row.debateNumber}: Batch 5 rendering packet hash mismatch`
  );
...
assertV4(
  ...
  "Batch 5 rendering evidence already exists"
);
...
const activation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-05-rendering-verification-execution-activation",
  protocolId: POST_CANARY_BATCH_05_RENDERING_PROTOCOL_ID,
  status: "frozen-post-canary-batch-05-rendering-verification-authorized",
  authorizedAt,
  activationCommit,
  ...
  nextAuthorizedAction:
    "single-batch-05-rendering-verification-execution-under-standing-authorization-and-frozen-stop-rules"
};

if (shouldWrite) {
  assertV4(!(await exists(ACTIVATION)), "Batch 5 rendering activation already exists");
