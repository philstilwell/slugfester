...
import { POST_CANARY_BATCH_03_DISPUTE_ADJ_ROOT } from "./lib/assessment-production-post-canary-batch-03-dispute-adjudication.mjs";
...
const preparationPath = `${POST_CANARY_BATCH_03_DISPUTE_ADJ_ROOT}/preparation-manifest.json`;
const manifestPath = `${POST_CANARY_BATCH_03_DISPUTE_ADJ_ROOT}/execution-preparation-manifest.json`;
const activationPath = `${POST_CANARY_BATCH_03_DISPUTE_ADJ_ROOT}/execution-activation.json`;
const executionPath = `${POST_CANARY_BATCH_03_DISPUTE_ADJ_ROOT}/model-execution.json`;
const analysisPath = `${POST_CANARY_BATCH_03_DISPUTE_ADJ_ROOT}/analysis.json`;
...
assertV4(
  preparation.status ===
    ...
    preparation.totals.disputedMoves === 190 &&
    preparation.totals.candidateSelections === 586 &&
  ...
  "Batch 3 adjudication packets are not ready for execution preparation"
);
assertV4(
  ...
  "Batch 3 accepted source or cost boundary changed"
);
assertV4(
  ...
  "Batch 3 adjudication model or score-blind boundary changed"
);
assertV4(
  preparation.totals.maximumCopiedInputBytes <= 360000,
  "Batch 3 context exceeds the frozen transport ceiling"
);
assertV4(
