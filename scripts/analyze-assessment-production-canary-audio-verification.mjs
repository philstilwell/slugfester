...
assertV4(
  manifest.status === "frozen-four-paid-known-speaker-diarizations-authorized" &&
    execution.retries === 0 &&
    execution.scoresDerived === 0,
  "production-canary audio execution unavailable or crossed its boundary"
);
for (const [file, digest] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    sha256(await readFile(file)) === digest,
    `source hash mismatch during production-canary audio analysis: ${file}`
  );
}
...
const debateNumbers = [...new Set(moves.map((move) => move.debateNumber))];
const debates = debateNumbers.map((debateNumber) => ({
  debateNumber,
...
const audit = {
  schemaVersion: "1.0-production-canary-audio-verification-audit",
...
const analysis = {
  schemaVersion: "1.0-production-canary-audio-verification-analysis",
  protocolId: manifest.protocolId,
  status: audit.status,
  productionCanary: true,
  stagingOnly: true,
  ...
  JSON.stringify(
    {
      ...
      excerptRecalls: moves.map((move) => ({
        ...
        expectedSpeakerRecall:
          move.deterministicEvidence?.expectedSpeakerExcerptRecall ?? null,
      ...
      nextAuthorized: passed
        ? "adjudication-packet-preparation"
        : "audio-failure-diagnosis-only"
    },
    null,
    2
  )
