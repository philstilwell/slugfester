...
import {
  POST_CANARY_BATCH_04_DISPUTE_ADJ_ISOLATION,
...
const EXPECTED_DEBATES = ["127", "67", "85", "49", "186", "81", "148", "47", "03", "185"];
...
const preparation = JSON.parse(
  await readFile(`${POST_CANARY_BATCH_04_DISPUTE_ADJ_ROOT}/preparation-manifest.json`, "utf8")
);
...
assert.equal(preparation.totals.candidateSelections, 582);
...
for (const context of preparation.contexts) {
  ...
  const output = {
    schemaVersion: POST_CANARY_BATCH_04_DISPUTE_ADJ_OUTPUT_VERSION,
    ...
    isolation: structuredClone(POST_CANARY_BATCH_04_DISPUTE_ADJ_ISOLATION),
  ...
  const validation = validatePostCanaryBatch04DisputeAdjudicationOutput(output, packet);
...
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_04_DISPUTE_ADJ_ROOT}/packets`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
assert.deepEqual(
  (await readdir(`${POST_CANARY_BATCH_04_DISPUTE_ADJ_ROOT}/provenance`)).sort(),
  EXPECTED_DEBATES.map((number) => `debate-${number}.json`).sort()
);
