...
const MANIFEST =
  `${POST_CANARY_BATCH_15_PUBLICATION_ROOT}/execution-preparation-manifest.json`;
...
assert.equal(manifest.protocolId, POST_CANARY_BATCH_15_PUBLICATION_PROTOCOL_ID);
...
assert.equal(manifest.totals.moves, 191);
...
for (const context of manifest.contexts) {
  ...
  assert.equal(packet.protocolId, POST_CANARY_BATCH_15_PUBLICATION_PROTOCOL_ID);
  ...
  assert.deepEqual(schema, buildPostCanaryBatch15PublicationSchema(packet));
  assert.equal(schema.properties.schemaVersion.const, POST_CANARY_BATCH_15_PUBLICATION_OUTPUT_VERSION);
  assert.equal(schema.properties.protocolId.const, POST_CANARY_BATCH_15_PUBLICATION_PROTOCOL_ID);
