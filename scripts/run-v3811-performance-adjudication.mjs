...
import { assertV3811 } from "./lib/v3811-performance-judgment.mjs";
...
assertV3811(manifest.status === "frozen-three-context-adjudication-authorized" && manifest.authorization.adjudicationModelExecution && !manifest.authorization.furtherAutomaticRetry && !manifest.authorization.scoreDerivation, "adjudication execution unauthorized");
...
for (const context of manifest.contexts) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-v3811-adjudication-${context.debateNumber}-`));
  const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), `slugfester-v3811-home-adjudication-${context.debateNumber}-`));
  ...
  try {
    ...
    process.stdout.write(`\n[v3.8.11-adjudication] starting ${manifest.model.label} Debate ${context.debateNumber}\n`);
    ...
    if (invocation.timedOut || invocation.code !== 0 || invocation.signal !== null) record = { ...base, status: invocation.timedOut ? "timed-out" : "transport-failed", outputWritten: false, deterministicValidationPassed: false, gateAcceptancePassed: false };
      ...
      const validation = await run(process.execPath, ["scripts/validate-v3811-performance-adjudication-output.mjs", context.output, context.packet], { cwd: root, env: process.env });
...
const execution = { schemaVersion: "3.8.11-performance-adjudication-model-execution", protocolId: manifest.protocolId, stage: manifest.stage, startedAt: results[0]?.startedAt ?? null, completedAt: new Date().toISOString(), contextsPlanned: manifest.population.contexts, validContexts: results.filter((item) => item.gateAcceptancePassed).length, disputedMovesDecided: results.reduce((sum, item) => sum + (item.gateAcceptancePassed ? item.disputedMoves : 0), 0), attempts: results.length, retries: 0, meteredApiCostUsd: 0, transcriptionApiCalls: 0, transcriptionCostUsd: 0, results };
await writeFile(path.resolve(root, manifest.artifacts.execution), `${JSON.stringify(execution, null, 2)}\n`);
assertV3811(execution.validContexts === manifest.population.contexts && execution.disputedMovesDecided === manifest.population.disputedMoves, "adjudication execution failed; final ledger and scores remain blocked");
console.log(JSON.stringify({ status: "v3.8.11-performance-adjudication-passed", validContexts: execution.validContexts, disputedMovesDecided: execution.disputedMovesDecided, retries: 0, meteredApiCostUsd: 0, finalLedgerAssemblyAuthorized: true, scoreDerivationAuthorized: false }, null, 2));
