...
assert.equal(manifest.aggregateExpectations.moves, 190);
...
assert.equal(manifest.preservedControls.debate172IntegerRoundedTiePreserved, true);
...
assert.equal(replayedMoves, 190);
...
const preview = buildPostCanaryBatch02PublicationStagingPreviewHtml();
assert.match(preview, /noindex,nofollow/);
assert.match(preview, /validated post-canary Batch 2 candidate only/);
