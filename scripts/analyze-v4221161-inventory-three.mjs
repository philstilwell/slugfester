...
const analysis = {
  ...
  interpretation: passed ? { scoreBlindCurationOperational: true, partitionTranscriptCoveragePreserved: true, monolithicCrossFieldFailuresEliminatedAtInventoryStage: true, sameLockedInventoryCanNowBePreparedForTwoIndependentJudgments: true } : { scoreBlindCurationOperationalForReturnedContexts: debates.filter((debate) => debate.accepted).length === 2, validLockedInventoriesReusableInTransportRecovery: debates.filter((debate) => debate.accepted).map((debate) => debate.debateNumber), failureLocalizedToLargestContext: execution.results.find((result) => result.debateNumber === "182")?.timedOut === true && execution.results.find((result) => result.debateNumber === "182")?.proposalWritten === false, timeoutEnforcementDriftDetected: execution.results.find((result) => result.debateNumber === "182")?.elapsedMs > manifest.executionPolicy.timeoutMs + 30000, inventorySpecificProjectionRecommended: true, everyCandidateMustRemainAvailable: true, semanticCandidateDownselectionRecommended: false, independentJudgmentPreparationRecommended: false },
  recoveryRecommendation: passed ? null : { protocolSuccessorRequired: true, executeDebates: ["182"], reuseAcceptedLockedInventories: ["133", "178"], removeOnlyValidatorOwnedCandidateMetadataFromModelTransport: ["attributionConfidence", "attributionBasis", "candidateConfidence", "candidate evidence rendering diagnostics"], retainForEveryCandidate: ["candidate ID", "side", "speaker", "source chronology", "discovery move-kind advisory", "proposed proposition", "load-bearing level and reason", "response intent", "context summary", "source-exact excerpt"], semanticCandidateDownselection: false, retry: false, sameProtocolRerun: false },
  authorization: { inventoryTransportRecoveryDesign: !passed, independentJudgmentPreparation: passed, independentJudgmentExecutionManifest: false, independentJudgmentModelExecution: false, disagreementExtraction: false, audioVerification: false, adjudication: false, scoreDerivation: false, productionMutation: false, all195Debates: false }
};
