...
const PREPARATION_CORRECTION = `${ROOT}/preparation-correction-1/analysis.json`;
const PREPARATION_CORRECTION_DIAGNOSIS =
  `${ROOT}/preparation-correction-1/diagnosis.json`;
...
const SCRIPT =
  "scripts/preregister-assessment-production-post-canary-batch-04-independent-judgments.mjs";
const TEST =
  "scripts/test-assessment-production-post-canary-batch-04-independent-judgment-manifest.mjs";
const CORRECTED_PREPARATION_TEST =
  "scripts/test-assessment-production-post-canary-batch-04-independent-judgment-preparation-correction-1.mjs";
const PREPARATION_CORRECTION_VALIDATOR =
...
const [
  ...
  await Promise.all([
    ...
    readFile(PREPARATION_CORRECTION, "utf8").then(JSON.parse),
...
assertV4(
  preparation.status ===
    ...
    preparation.totals.uniqueMoves === 203 &&
    preparation.totals.movesJudgedAcrossPasses === 406 &&
  ...
  "Batch 4 preparation does not authorize an execution-preparation manifest"
);
assertV4(
  preparationCorrection.status ===
      "bounded-deterministic-preparation-validation-overlay-passed" &&
    preparationCorrection.preparation === PREPARATION &&
    preparationCorrection.preparationSha256 ===
      sha256(await readFile(PREPARATION)) &&
    preparationCorrection.exactCorrectedExpectations
      .pendingAttributionVerificationMoves === 0 &&
    preparationCorrection.exactCorrectedExpectations.uniqueMoves === 203 &&
    preparationCorrection.exactCorrectedExpectations.movesJudgedAcrossPasses ===
      406 &&
    preparationCorrection.packetsChanged === false &&
    preparationCorrection.schemasChanged === false &&
    preparationCorrection.sourcesChanged === false &&
    preparationCorrection.inventoriesChanged === false &&
    preparationCorrection.modelSettingsChanged === false &&
    preparationCorrection.modelContextsExecuted === 0 &&
    preparationCorrection.directIncrementalCostUsd === 0,
  "Batch 4 independent-judgment preparation validation overlay is unavailable"
);
assertV4(
...
assertV4(
  ...
  "the Batch 4 gate disposition or score-blind inventory contract drifted"
);
assertV4(
  ...
  "the Batch 4 exact-source compatibility boundary drifted"
);
assertV4(
...
const sourceFiles = [
  ...
  PREPARATION_CORRECTION_DIAGNOSIS,
  CORRECTED_PREPARATION_TEST,
  PREPARATION_CORRECTION_VALIDATOR,
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-independent-judgment-execution-preparation-manifest",
  protocolId: preparation.protocolId,
  status:
    "frozen-twenty-post-canary-batch-04-independent-judgment-contexts-prepared-not-authorized",
  frozenAt,
  ...
  nextAuthorizedAction:
    "freeze-and-activate-batch-04-independent-judgment-execution-under-standing-authorization",
};
