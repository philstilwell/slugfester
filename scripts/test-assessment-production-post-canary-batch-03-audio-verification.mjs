...
const expectedMoves = [
  "con-heuristics-fail-under-change",
  "pro-rational-instruction-behavioral-limit",
  "pro-unconstrained-bubble-causal-gap",
  "pro-logic-presupposition-suffices",
  "con-event-acceptance-not-causal-verdict",
  "con-reason-incarnation-access-gap",
  "pro-divine-doubt-self-knowledge",
  "pro-infer-divine-self-knowledge"
];
...
assert.equal(preparation.costEstimate.primaryExpectedFutureExecutionCostUsd, 0.2435025);
...
if (!(await exists(paths.activation))) {
  console.log(JSON.stringify({ status: "passed-preactivation", calls: 8, paidCalls: 0 }, null, 2));
  process.exit(0);
}

const activation = JSON.parse(await readFile(paths.activation, "utf8"));
assert.equal(activation.status, "frozen-eight-post-canary-batch-03-paid-known-speaker-diarizations-authorized-under-standing-authorization");
assert.equal(activation.calls.length, 8);
...
assert.equal(activation.canonicalSourceGate.transcriptHashesVerified, 5);
assert.equal(activation.canonicalSourceGate.eventHashesVerified, 5);
assert.equal(activation.canonicalSourceGate.manifestHashesVerified, 5);
assert.equal(activation.canonicalSourceGate.dyadicDebatesVerified, 5);
...
if (!(await exists(paths.execution))) {
  ...
  console.log(JSON.stringify({ status: "passed-activated", calls: 8, retries: 0, maximumAuthorizedCostUsd: 1 }, null, 2));
  process.exit(0);
}

const execution = JSON.parse(await readFile(paths.execution, "utf8"));
assert.equal(execution.callsPlanned, 8);
assert.equal(execution.results.length, 8);
...
assert.ok(execution.callsAttempted <= 8);
assert.equal(execution.callsCompleted + execution.callsSkipped + execution.results.filter((result) => result.status === "request-failed").length, 8);
...
if (!execution.requestFailure && !execution.costCapReachedOrExceeded) {
  assert.equal(execution.callsAttempted, 8);
  assert.equal(execution.callsCompleted, 8);
...
assert.equal(audit.totals.requiredMoves, 8);
assert.equal(audit.totals.verified + audit.totals.unresolved, 8);
...
assert.equal(analysis.gate.requiredMoves, 8);
assert.equal(analysis.gate.verified + analysis.gate.unresolved, 8);
...
if (analysis.gate.passed) {
  assert.equal(analysis.status, "passed-all-eight-post-canary-batch-03-confidence-moves-audio-verified");
  assert.equal(analysis.gate.verified, 8);
  assert.equal(analysis.gate.unresolved, 0);
} else {
  assert.ok(["post-canary-batch-03-audio-verification-unresolved", "post-canary-batch-03-audio-verification-incomplete"].includes(analysis.status));
...
assert.equal(cost.costControl.usageDerivedPlanningEstimateUsd, 0.2435025);
