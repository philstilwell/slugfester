...
import {
  ...
  hashFile,
  sha256,
  validatePostCanaryBatch11RenderingEvidence,
  validatePostCanaryBatch11RenderingPacket
...
const ROOT = POST_CANARY_BATCH_11_RENDERING_ROOT;
...
assertV4(activation.status ===
  "frozen-post-canary-batch-11-rendering-verification-authorized" &&
...
"Batch 11 rendering activation changed");
...
for (const debateNumber of POST_CANARY_BATCH_11_RENDERING_ORDER) {
  ...
  const packet = validatePostCanaryBatch11RenderingPacket(JSON.parse(packetBytes));
  for (const viewportName of ["desktop", "mobile"]) {
    ...
    validatePostCanaryBatch11RenderingEvidence({ packet, viewportName,
...
"complete passing Batch 11 rendering evidence required");
...
const audit = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-11-rendering-audit",
  protocolId: activation.protocolId,
  status: "passed-ten-debate-batch-11-rendering-verification",
...
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-11-rendering-execution",
  protocolId: activation.protocolId,
  status: "ten-debate-batch-11-rendering-verification-passed",
  completedAt,
  attempts: 20,
  retries: 0,
  timeoutExtensions: 0,
...
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-11-rendering-analysis",
  protocolId: activation.protocolId,
  ...
  nextAuthorizedAction: "prepare-batch-11-production-compatibility-staging"
};
...
console.log(JSON.stringify({ status: analysis.status, debates: 10,
  sections: 50, moves: 190, viewportResults: 20,
