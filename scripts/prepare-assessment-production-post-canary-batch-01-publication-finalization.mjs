...
import {
  POST_CANARY_BATCH_01_PUBLICATION_FINALIZATION_ORDER,
  POST_CANARY_BATCH_01_PUBLICATION_FINALIZATION_PROTOCOL_ID,
  POST_CANARY_BATCH_01_PUBLICATION_FINALIZATION_ROOT,
  buildPostCanaryBatch01PublicationFinalization,
  validatePostCanaryBatch01PublicationFinalCandidate
} from "./lib/assessment-production-post-canary-batch-01-publication-finalization.mjs";
...
const COMPILATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-01/deterministic-publication-compilation";
...
const ROOT = POST_CANARY_BATCH_01_PUBLICATION_FINALIZATION_ROOT;
...
assertV4(
  compilationAnalysis.status ===
      "ten-debate-batch-01-deterministic-publication-compilation-passed" &&
    ...
    compilationAudit.status ===
      "passed-ten-debate-batch-01-deterministic-publication-compilation" &&
    canonicalJson(compilationAudit.explicitOrder) ===
      canonicalJson(POST_CANARY_BATCH_01_PUBLICATION_FINALIZATION_ORDER) &&
    compilationAudit.rows.length === 10 &&
    compilationAudit.totals.sections === 50 &&
    compilationAudit.totals.moves === 177 &&
  ...
  "passing Batch 1 deterministic compilation evidence required"
);

const contexts = [];
const compatibilityRows = [];
const sourceChainFiles = [];
for (const debateNumber of POST_CANARY_BATCH_01_PUBLICATION_FINALIZATION_ORDER) {
  ...
  assertV4(
    ...
    `${debateNumber}: Batch 1 finalization source context missing`
  );
  ...
  assertV4(
    ...
    `${debateNumber}: Batch 1 compiled input hash changed`
  );
  const compiled = JSON.parse(compiledBytes);
  const built = buildPostCanaryBatch01PublicationFinalization({
    compiled,
  ...
  const validation = validatePostCanaryBatch01PublicationFinalCandidate({
...
const compatibility = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-01-publication-finalization-compatibility-analysis",
  protocolId: POST_CANARY_BATCH_01_PUBLICATION_FINALIZATION_PROTOCOL_ID,
  status: "batch-01-production-compatibility-boundary-recorded",
  analyzedAt: frozenAt,
  ...
  findings: [
    {
      id: "batch-01-site-ledger-adapter-and-validator-route",
      description:
        "The live site validator recognizes adjudicated-consensus ledgers only through the earlier checkpoint-specific adapter schema, activation, and packet root; it cannot authenticate or replay Batch 1 ledgers without a separately frozen Batch 1 adapter route.",
      evidence: {
        debates: 10,
        sections: 50,
        moves: 177,
      ...
      requiredRemedy:
        "A separately authorized production-compatibility plan must define, hash-lock, and test a Batch 1 site-ledger adapter and validator route without changing scores, prose, attribution, or the accepted optional-reference behavior."
    }
  ],
  authorization: {
...
const sourceFiles = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-01-publication-finalization.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-01-publication-finalization.mjs",
  "scripts/activate-assessment-production-post-canary-batch-01-publication-finalization.mjs",
  "scripts/run-assessment-production-post-canary-batch-01-publication-finalization.mjs",
  "scripts/test-assessment-production-post-canary-batch-01-publication-finalization-preparation.mjs",
...
for (const file of futureOutputPaths) {
  assertV4(!(await exists(file)), `future Batch 1 finalization output exists: ${file}`);
}

const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-01-publication-finalization-preparation",
  protocolId: POST_CANARY_BATCH_01_PUBLICATION_FINALIZATION_PROTOCOL_ID,
  status: "frozen-post-canary-batch-01-publication-finalization-prepared-not-authorized",
  frozenAt,
  ...
  stagingOnly: true,
  userAuthorization: {
    instruction: "I approve Batch 1 publication-finalization preparation only.",
    scopeInterpretation:
      "Prepare, validate, freeze, commit, and push the Batch 1 publication-finalization plan without activating or executing finalization.",
  ...
  preservedControls: {
    ...
    participantJudgmentWasScoreBlind: true,
    integerRoundedScoreTiesPermitted: true,
    debate91IntegerRoundedTiePreserved: true,
  ...
  explicitOrder: POST_CANARY_BATCH_01_PUBLICATION_FINALIZATION_ORDER,
  contexts,
  executionPolicy: {
  ...
  finalizationPolicy: {
    ...
    previewUsesPostCanaryBatch01StagingBanner: true,
    previewNoindex: true,
  ...
  nextAuthorizedAction:
    "user-approval-required-before-activation-and-execution-of-one-frozen-batch-01-deterministic-publication-finalization-pass"
};
  ...
  JSON.stringify(
    {
      status: shouldWrite
        ? manifest.status
        : "post-canary-batch-01-publication-finalization-preparation-preview",
