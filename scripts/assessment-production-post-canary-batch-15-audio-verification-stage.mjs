...
const stageRoot = "docs/assessment-production/post-canary-continuation-v1/batch-15/audio-verification";
const paths = {
...
const expectedDebates = ["39", "159", "98"];
const expectedMoves = [
  "pro-exodus-minimal-migration",
  "con-dispute-determinism-rationality-link",
  "con-possibility-feasibility-gap"
];
const expectedCalls = 3;
const durationOnlyPlanningEstimateUsd = 0.027076;
const usageDerivedPlanningEstimateUsd = 0.0988975;
const toolPath = "scripts/assessment-production-post-canary-batch-15-audio-verification-stage.mjs";
...
const standingAuthorization = await loadAndValidatePostCanaryBatch15StandingAuthorization();
...
async function validatePreparation(preparation) {
  assert.equal(
    preparation.status,
    "prepared-three-post-canary-batch-15-paid-known-speaker-diarizations-standing-authorization-conditional-activation-ready"
  );
...
async function activate() {
  ...
  const activation = {
    ...preparation,
    schemaVersion: "1.0-assessment-production-post-canary-batch-15-audio-verification-execution-manifest",
    status: "frozen-three-post-canary-batch-15-paid-known-speaker-diarizations-authorized-under-standing-authorization",
    activatedAt,
    ...
    nextAuthorizedAction: "execute-exactly-three-post-canary-batch-15-paid-audio-verification-calls-once-sequentially"
  };
...
async function validateActivation(manifest) {
  assert.equal(
    manifest.status,
    "frozen-three-post-canary-batch-15-paid-known-speaker-diarizations-authorized-under-standing-authorization"
  );
...
async function run() {
  ...
  for (const [callIndex, call] of manifest.calls.entries()) {
    ...
    process.stdout.write(`[batch-15-audio] ${callIndex + 1}/${expectedCalls} starting ${call.debateNumber}:${call.moveId}\n`);
    ...
    results.push(result);
    process.stdout.write(`[batch-15-audio] ${callIndex + 1}/${expectedCalls} ${result.status}; cumulative $${result.cumulativeUsageDerivedEstimatedCostUsd.toFixed(7)}\n`);
  }
  ...
  const execution = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-15-audio-verification-model-execution",
    protocolId: manifest.protocolId,
    status: completed.length === expectedCalls
      ? "three-post-canary-batch-15-paid-known-speaker-diarizations-completed"
      : "post-canary-batch-15-paid-diarization-incomplete",
    provider: "OpenAI Transcription API",
...
async function analyze() {
  ...
  const status = passed
    ? "passed-all-three-post-canary-batch-15-confidence-moves-audio-verified"
    : executionComplete
      ? "post-canary-batch-15-audio-verification-unresolved"
      : "post-canary-batch-15-audio-verification-incomplete";
  const nextAuthorizedAction = execution.costCapReachedOrExceeded || !execution.directIncrementalCostCapControlPassed
    ? "user-review-required-before-any-batch-15-downstream-work-after-audio-cost-cap-event"
    : passed
      ? "prepare-freeze-and-push-batch-15-dispute-only-adjudication-packets-under-standing-authorization"
      : "standing-authorization-stop-new-approval-required-before-batch-15-audio-verification-failure-diagnosis";
  const authorization = {
  ...
  const audit = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-15-audio-verification-audit",
    protocolId: manifest.protocolId,
    status,
  ...
  const analysis = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-15-audio-verification-analysis",
    protocolId: manifest.protocolId,
    status,
    productionCanary: false,
    batchNumber: 15,
...
async function cost() {
  ...
  const result = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-15-audio-cost-control-analysis",
