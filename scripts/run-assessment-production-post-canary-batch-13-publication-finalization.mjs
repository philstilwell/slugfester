...
const identity = JSON.parse(await readFile(path.resolve(
  "docs/assessment-production/post-canary-continuation-v1/batch-13/deterministic-publication-compilation/production-identity-snapshot.json"), "utf8"));
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const exists = (file) => access(path.resolve(file)).then(() => true, () => false);
assertV4(a.status === "frozen-post-canary-batch-13-publication-finalization-authorized" &&
  ...
  canonicalJson(a.explicitOrder) === canonicalJson(POST_CANARY_BATCH_13_PUBLICATION_FINALIZATION_ORDER),
"Batch 13 finalization is not authorized");
...
try {
  ...
  assertV4(candidates.length === 10 && candidates.reduce((sum, row) => sum + row.validation.moves, 0) === 199,
...
if (passed) {
  ...
  const preview = Buffer.from(buildPostCanaryBatch13PublicationStagingPreviewHtml());
  ...
  await writeFile(path.resolve(a.artifacts.audit), `${JSON.stringify({
    schemaVersion: "1.0-assessment-production-post-canary-batch-13-publication-finalization-audit",
    protocolId: a.protocolId, status: "passed", rows,
    totals: { debates: 10, moves: 199, deterministicFinalizationPasses: 1,
...
const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-13-publication-finalization-execution",
  protocolId: a.protocolId, status: passed ? "ten-debate-batch-13-publication-finalization-passed" :
    "ten-debate-batch-13-publication-finalization-failed", startedAt,
...
const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-13-publication-finalization-analysis",
  ...
  nextAuthorizedAction: passed ? "prepare-batch-13-rendering-verification" : "diagnose-finalization-failure" };
