...
const resumptionRoot =
  `${publicationRoot}/failure-recovery/original-unattempted-context-resumption-2`;
const root = `${resumptionRoot}/critique-repair`;
const protocolId = "assessment-production-post-canary-batch-14-publication-resumption-2-debate-12-critique-field-disjoint-recovery";
...
const files = {
  diagnosis: `${resumptionRoot}/debate-12-failure-diagnosis.json`, originalExecution: `${resumptionRoot}/model-execution.json`, packet: `${publicationRoot}/packets/debate-12.json`, fullSchema: `${publicationRoot}/schemas/debate-12.schema.json`, failedOutput: `${publicationRoot}/outputs/debate-12.json`, originalValidation: `${publicationRoot}/validations/debate-12.json`, originalProvenance: `${publicationRoot}/provenance/debate-12.json`, workflow: "docs/assessment-production-workflow.md", outputContract: "/Users/philstilwell/.codex/skills/reassess-slugfester-debates/references/output-contract.md", manual: `${root}/manual.md`, preparation: `${root}/execution-preparation-manifest.json`, activation: `${root}/execution-activation.json`, execution: `${root}/model-execution.json`, analysis: `${root}/analysis.json`, mergedOutput: `${publicationRoot}/outputs/debate-12.json`, mergedValidation: `${publicationRoot}/validations/debate-12.json`, mergedProvenance: `${publicationRoot}/provenance/debate-12.json`, preservedOutput: `${root}/preserved-original-output.json`, preservedValidation: `${root}/preserved-original-validation.json`, preservedProvenance: `${root}/preserved-original-provenance.json`,
};
...
if (mode === "--prepare") {
  const sourcePaths = [files.diagnosis, files.originalExecution, files.packet, files.fullSchema, files.failedOutput, files.originalValidation, files.originalProvenance, files.workflow, files.outputContract, files.manual, "scripts/assessment-production-post-canary-batch-14-publication-resumption-2-debate-12-critique-recovery.mjs", "scripts/lib/assessment-production-post-canary-batch-14-publication-validation.mjs"];
  const sourceBytes = Object.fromEntries(await Promise.all(sourcePaths.map(async (file) => [file, await readFile(path.resolve(file))])));
  const [diagnosis, originalExecution, packet, failedOutput] = [files.diagnosis, files.originalExecution, files.packet, files.failedOutput].map((file) => JSON.parse(sourceBytes[file]));
  assert.equal(diagnosis.status, "debate-12-seventeen-critique-fields-failed-all-other-publication-fields-valid");
  assert.equal(diagnosis.diagnosis.invalidCritiqueCount, 17);
  assert.equal(diagnosis.diagnosis.allOtherFieldsStructurallyValid, true);
  assert.equal(originalExecution.status, "six-context-publication-resumption-stopped-with-failure");
  ...
  assert.equal(invalidIds.length, 17);
  const contexts = [];
  for (let offset = 0; offset < invalidIds.length; offset += 2) {
    ...
    const repairPacket = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-resumption-2-debate-12-critique-recovery-packet", protocolId, contextIndex, debateNumber: "12", debateId: packet.debateId, assessmentModel: model.label, writableFields: moveIds.map((moveId) => `moveProse.${moveId}.critique`), writableFieldCount: moveIds.length, targets, allOtherFieldsUnavailableAndImmutable: true, participantJudgmentClosed: true, publicationScoreLocked: true };
    ...
    const properties = { schemaVersion: { type: "string", const: "1.0-assessment-production-post-canary-batch-14-publication-resumption-2-debate-12-critique-recovery-output" }, protocolId: { type: "string", const: protocolId }, contextIndex: { type: "integer", const: contextIndex }, debateNumber: { type: "string", const: "12" }, debateId: { type: "string", const: packet.debateId }, assessmentModel: { type: "string", const: model.label }, completedAt: { type: "string", minLength: 1 }, corrections: { type: "array", minItems: moveIds.length, maxItems: moveIds.length, items: { anyOf: itemSchemas } } };
    const schema = { $schema: "https://json-schema.org/draft/2020-12/schema", $id: `slugfester-b14-debate12-critique-repair-${contextIndex}`, type: "object", additionalProperties: false, required: Object.keys(properties), properties };
    ...
    contexts.push({ contextIndex, debateNumber: "12", moveIds, writableFields: repairPacket.writableFields, writableFieldCount: moveIds.length, packet: packetPath, packetSha256: sha256(packetText), schema: schemaPath, schemaSha256: sha256(schemaText), output: outputPath });
  ...
  assert.equal(contexts.length, 9); assert.equal(contexts.reduce((sum, item) => sum + item.writableFieldCount, 0), 17); assert(contexts.every((item) => item.writableFieldCount <= 2));
  ...
  const manifest = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-critique-recovery-preparation", protocolId, status: "frozen-nine-context-seventeen-critique-batch-14-publication-recovery-level-1-prepared-not-activated", frozenAt: at, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(), productionCanary: false, batchNumber: 14, stagingOnly: true, recoveryLevel: 1, recoveryLevelsMaximum: 2, model, contexts, executionPolicy: { contexts: 9, maximumParallelContexts: 2, attemptsPerContext: 1, retriesMaximum: 0, timeoutExtensionsMaximum: 0, timeoutMsPerContext: 600000, writableFieldsMaximumPerPacket: 2, directIncrementalCostUsdMaximum: 0, paidServices: false, separateActivationRequired: true, APIKeysRemoved: true, removedEnvironmentVariables: ["OPENAI_API_KEY", "OPENAI_ORG_ID", "OPENAI_PROJECT_ID", "OPENAI_BASE_URL", "AZURE_OPENAI_API_KEY", "CODEX_API_KEY"] }, diagnosis: { path: files.diagnosis, sha256: sourceHashes[files.diagnosis], invalidCritiques: 17, allOtherFieldsStructurallyValid: true }, sourceHashes, futureOutputs, authorization: { preparation: true, activation: true, modelExecution: false, deterministicMergeAndValidation: false, retries: false, furtherRecoveryLevel: false, scorePass: false, productionMutation: false, nextBatchSelection: false }, nextAuthorizedAction: "activate-nine-field-disjoint-debate-12-critique-recovery-contexts" };
  if (shouldWrite) await writeFile(files.preparation, pretty(manifest));
  console.log(pretty({ status: shouldWrite ? manifest.status : "preview", contexts: 9, writableCritiques: 17, maximumWritableFieldsPerContext: 2, attemptsMaximum: 9, retriesMaximum: 0, recoveryLevel: 1, recoveryLevelsMaximum: 2, directIncrementalCostUsdMaximum: 0, modelExecutionAuthorized: false })); process.exit(0);
}

if (mode === "--activate") {
  const bytes = await readFile(files.preparation); const preparation = JSON.parse(bytes);
  assert.equal(preparation.status, "frozen-nine-context-seventeen-critique-batch-14-publication-recovery-level-1-prepared-not-activated"); assert(!(await exists(files.activation)), "activation exists");
  ...
  const activation = { ...preparation, schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-critique-recovery-activation", status: "frozen-nine-context-seventeen-critique-batch-14-publication-recovery-level-1-authorized", activatedAt: at, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(), preparation: { path: files.preparation, sha256: sha256(bytes) }, authorization: { ...preparation.authorization, modelExecution: true, deterministicMergeAndValidation: true }, nextAuthorizedAction: "execute-nine-field-disjoint-debate-12-critique-recovery-contexts-once" };
  if (shouldWrite) await writeFile(files.activation, pretty(activation));
  console.log(pretty({ status: shouldWrite ? activation.status : "preview", contexts: 9, writableCritiques: 17, recoveryLevel: 1, attemptsMaximum: 9, retriesMaximum: 0, directIncrementalCostUsdMaximum: 0, modelExecutionAuthorized: shouldWrite })); process.exit(0);
}

if (mode === "--run") {
  const activation = await readJson(files.activation); assert.equal(activation.status, "frozen-nine-context-seventeen-critique-batch-14-publication-recovery-level-1-authorized");
  ...
  async function execute(context) {
    const temp = await mkdtemp(path.join(os.tmpdir(), `slugfester-b14-pub12-c-${context.contextIndex}-`)); const home = await mkdtemp(path.join(os.tmpdir(), `slugfester-b14-pub12-ch-${context.contextIndex}-`)); const startedAt = new Date().toISOString(); const started = Date.now();
    try {
      ...
      const prompt = `Read workflow.md, output-contract.md, manual.md, packet.json, and schema.json completely and no other files. Act only as the isolated 5.6 Sol/low Debate 12 critique-recovery editor for context ${context.contextIndex}. Author exactly the ${context.moveIds.length} required critique string${context.moveIds.length === 1 ? "" : "s"} in packet order. Return exactly one schema-conforming JSON object and no commentary.`;
  ...
  const passed = results.every((item) => item.gateAcceptancePassed); const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-critique-recovery-execution", protocolId, status: passed ? "nine-context-seventeen-critique-batch-14-publication-recovery-level-1-execution-passed" : "batch-14-publication-critique-recovery-level-1-execution-failed", startedAt, completedAt: new Date().toISOString(), elapsedMs: Date.now() - started, contextsAttempted: 9, validContexts: results.filter((item) => item.gateAcceptancePassed).length, invalidContexts: results.filter((item) => !item.gateAcceptancePassed).length, attempts: 9, retries: 0, timeoutExtensions: 0, directIncrementalCostUsd: 0, paidServiceCalls: 0, modelAuthoredScores: 0, results, authorization: { analysis: passed, retries: false, furtherRecoveryLevel: !passed, paidServices: false, productionMutation: false, nextBatchSelection: false } };
  await writeFile(files.execution, pretty(execution)); console.log(pretty({ status: execution.status, contextsAttempted: 9, validContexts: execution.validContexts, invalidContexts: execution.invalidContexts, elapsedMinutes: Number((execution.elapsedMs / 60000).toFixed(2)), attempts: 9, retries: 0, recoveryLevel: 1, directIncrementalCostUsd: 0 })); process.exit(0);
}
...
if (mode === "--analyze") {
  ...
  assert.equal(execution.status, "nine-context-seventeen-critique-batch-14-publication-recovery-level-1-execution-passed"); const outputs = await Promise.all(activation.contexts.map((item) => readJson(item.output))); const corrections = outputs.flatMap((output) => output.corrections); assert.equal(corrections.length, 17); assert.equal(new Set(corrections.map((item) => item.moveId)).size, 17);
  ...
  const validationRecord = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-validation", protocolId: merged.protocolId, status: "passed", debateNumber: "12", debateId: packet.debateId, outputSha256: sha256(mergedText), validationSummary: validation, validationMessage: null, modelAuthoredScores: 0, lockedScoresUnchanged: true };
  const provenance = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-debate-12-recovery-provenance", protocolId, debateNumber: "12", originalFailure: { status: "output-validation-failed", originalOutputPreserved: true }, recoveryLevel1: { contexts: 9, acceptedCritiques: 17, acceptedCompanionFieldsReused: true, eachTargetFieldAcceptedExactlyOnce: true }, merge: { acceptedFieldsChanged: false, scoresChanged: false, scorePassRerun: false, modelAuthoredScores: 0, completeValidationPassed: true } };
  const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-resumption-2-debate-12-critique-recovery-analysis", protocolId, status: "debate-12-level-1-field-disjoint-publication-recovery-passed-awaiting-five-context-resumption", recoveryLevel: 1, recoveryLevelsMaximum: 2, debateNumber: "12", moves: 18, critiques: 18, contextsThisLevel: 9, attemptsThisLevel: 9, retries: 0, timeoutExtensions: 0, directIncrementalCostUsd: 0, paidServiceCalls: 0, modelAuthoredScores: 0, validation, output: { path: files.mergedOutput, sha256: sha256(mergedText) }, authorization: { fiveOriginalUnattemptedContextResumption: true, furtherRecoveryLevel: false, deterministicCompilation: false, productionMutation: false, nextBatchSelection: false }, nextAuthorizedAction: "resume-five-original-unattempted-publication-contexts" };
  ...
  console.log(pretty({ status: analysis.status, debateNumber: "12", moves: validation.moves, critiques: validation.critiques, minimumCritiqueCharacters: validation.minimumCritiqueCharacters, exactSourceQuotes: validation.quoteExactSourceMatches, contextsThisLevel: 9, attemptsThisLevel: 9, retries: 0, directIncrementalCostUsd: 0, nextAuthorizedAction: analysis.nextAuthorizedAction }));
}
