...
const executionPath =
  `${POST_CANARY_BATCH_14_DISPUTE_ADJ_ROOT}/model-execution.json`;
...
assertV4(
  ...
  "Batch 14 adjudication execution is unavailable or crossed its boundary"
);
for (const [file, digest] of Object.entries(activation.sourceHashes)) {
  assertV4(
    sha256(await readFile(file)) === digest,
    `source hash mismatch during Batch 14 adjudication analysis: ${file}`
  );
}
...
for (const context of activation.contexts) {
  ...
  if (result.gateAcceptancePassed) {
    ...
    replay = validatePostCanaryBatch14DisputeAdjudicationOutput(
...
const semanticPass =
  ...
  valid.reduce((sum, context) => sum + context.candidateSelections, 0) ===
    539 &&
  valid.reduce((sum, context) => sum + context.disputedMoves, 0) === 181;
const timingPass =
  semanticPass &&
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-14-dispute-only-adjudication-analysis",
  ...
  status: passed
    ...
    : semanticPass
      ? timingPass
        ? scoreBlindPass
          ? "post-canary-batch-14-dispute-only-adjudication-gate-failed-isolation"
          : "post-canary-batch-14-dispute-only-adjudication-gate-failed-scoreblindness"
        : "post-canary-batch-14-dispute-only-adjudication-gate-failed-timing"
      : "post-canary-batch-14-dispute-only-adjudication-gate-failed-validation",
  ...
  nextAuthorizedAction: passed
    ...
    : "new-user-approval-required-after-batch-14-adjudication-failure-before-downstream-work"
};
if (shouldWrite) {
