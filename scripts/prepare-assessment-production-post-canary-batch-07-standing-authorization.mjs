...
import {
  ...
  POST_CANARY_BATCH_07_STANDING_AUTHORIZATION_INSTRUCTION_PATH,
  POST_CANARY_BATCH_07_STANDING_AUTHORIZATION_PROTOCOL_ID,
  POST_CANARY_BATCH_07_STANDING_AUTHORIZATION_STATUS,
  validatePostCanaryBatch07StandingAuthorization
...
const SELECTION_FREEZE_COMMIT = "ca5c2e61ac22a96d82846305002dbf7dac688fe6";
...
const SOURCE_FILES = [
  ...
  "scripts/prepare-assessment-production-post-canary-batch-07-standing-authorization.mjs",
  "scripts/test-assessment-production-post-canary-batch-07-standing-authorization.mjs"
];
const REQUIRED_ORDER = ["193", "80", "121", "100", "78", "113", "180", "02", "182", "56"];
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const exists = (file) => access(path.resolve(file)).then(() => true, () => false);

assertV4(
  !(await exists(POST_CANARY_BATCH_07_STANDING_AUTHORIZATION)),
  `${POST_CANARY_BATCH_07_STANDING_AUTHORIZATION} already exists`
);
...
assertV4(
  selection.status ===
    ...
    selection.batchNumber === 7 &&
    selection.checkpointCommit ===
      "6dde76ca52ece31adf4d02c29c937ff2cc53b287" &&
    ...
    analysis.status ===
      "seventh-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization" &&
    ...
    sourcePreparation.totals.discoveryContexts === 35 &&
    ...
    sourceValidation.totals.discoveryContexts === 35 &&
    sourceValidation.totals.modelContextsExecuted === 0 &&
    sourceValidation.directIncrementalCostUsd === 0,
  "the frozen Batch 7 selection or source-packet boundary changed"
);
assertV4(
  execFileSync("git", ["show", `${SELECTION_FREEZE_COMMIT}:${SELECTION}`]).equals(selectionBytes),
  "Batch 7 selection differs from the user-authorized selection commit"
);
...
assertV4(
  ...
  "Batch 7 standing authorization must freeze at the pushed source-packet commit"
);
...
const record = {
  ...
  protocolId: POST_CANARY_BATCH_07_STANDING_AUTHORIZATION_PROTOCOL_ID,
  status: POST_CANARY_BATCH_07_STANDING_AUTHORIZATION_STATUS,
  authorizedAt,
  ...
  userAuthorization: {
    instruction: POST_CANARY_BATCH_07_STANDING_AUTHORIZATION_INSTRUCTION,
    resolvedAntecedent: "The next step requires your Batch 7 standing authorization.",
    scopeInterpretation: "Create and use a complete-workflow Batch 7 standing authorization for the ten frozen selections, beginning with the separately frozen score-blind source packets, while preserving the established campaign controls and stopping before Batch 8 selection.",
  ...
  authorizedSequence: [
    "thirty-five-frozen-score-blind-discovery-contexts",
  ...
  stopRules: {
    ...
    outsideBatchSevenDebateBlocks: true,
    ...
    batchEightSelectionBlocks: true,
  ...
  nextAuthorizedAction:
    "prepare-validate-freeze-commit-and-push-batch-07-discovery-execution-manifest"
};
validatePostCanaryBatch07StandingAuthorization(record);
if (shouldWrite) {
  await writeFile(
