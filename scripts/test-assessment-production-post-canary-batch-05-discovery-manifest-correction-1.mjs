...
assert.equal(manifest.status, "frozen-forty-one-post-canary-batch-05-discovery-contexts-prepared-not-authorized");
...
assert.equal(manifest.executionPolicy.copiedInputBytesMaximum, 66284);
assert.equal(preparation.totals.maximumCopiedInputBytes, 66284);
...
assert.equal(manifest.copiedInputBoundary.frozenObservedCopiedInputBytesMaximum, 66284);
assert.equal(manifest.copiedInputBoundary.executionCopiedInputBytesMaximum, 66284);
assert.deepEqual(manifest.copiedInputBoundary.maximumContext, { debateNumber: "109", chunkId: "chunk-004" });
...
for (const context of manifest.contexts) {
  ...
  assert(buildBatch05TokenCountedChunkLedger(chunkBytes).equals(tokenBytes));
...
assert.equal(manifest.nextAuthorizedAction, "prepare-and-freeze-batch-05-discovery-execution-activation-under-standing-authorization");

const validation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-discovery-execution-preparation-validation",
  protocolId: manifest.protocolId,
  status: "batch-05-discovery-execution-manifest-validation-passed-frozen-standing-authorization-active",
