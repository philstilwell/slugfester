...
import {
  validatePostCanaryBatch17Scores
} from "./lib/assessment-production-post-canary-batch-17-score-gate.mjs";
import {
  buildPostCanaryBatch17PublicationPacket,
  buildPostCanaryBatch17PublicationSchema,
  postCanaryBatch17ReferenceCatalog,
  POST_CANARY_BATCH_17_PUBLICATION_DEBATES,
  POST_CANARY_BATCH_17_PUBLICATION_MODEL,
  POST_CANARY_BATCH_17_PUBLICATION_PROTOCOL_ID,
...
const SELECTION =
  "docs/assessment-production/post-canary-continuation-v1/batch-17/selection.json";
const FINAL_LEDGER =
...
const FINAL_LEDGER_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-17/final-ledger/final-ledger-manifest.json";
const FINAL_LEDGER_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-17/final-ledger/analysis.json";
const SCORES =
  "docs/assessment-production/post-canary-continuation-v1/batch-17/score-pass/calculated-scores.json";
const SCORE_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-17/score-pass/score-pass-manifest.json";
const SCORE_PREPARATION =
  "docs/assessment-production/post-canary-continuation-v1/batch-17/score-pass/score-pass-preparation-manifest.json";
const SCORE_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-17/score-pass/analysis.json";
const SOURCE_PACKET_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-17/independent-judgments/source-packets";
...
const STATIC_SOURCE_FILES = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-17-score-gate.mjs",
  ...
  "scripts/prepare-assessment-production-post-canary-batch-17-publication.mjs",
  "scripts/test-assessment-production-post-canary-batch-17-publication-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-17-publication.mjs",
  "scripts/run-assessment-production-post-canary-batch-17-publication.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-17-publication.mjs"
];
...
assertV4(
  scoreAnalysis.status === "post-canary-batch-17-score-stability-gate-passed" &&
    scoreAnalysis.productionCanary === false &&
    scoreAnalysis.batchNumber === 17 &&
    ...
    scoreAnalysis.nextAuthorizedAction ===
      "standing-authorization-permits-batch-17-publication-packet-preparation",
  "the accepted Batch 17 score gate is unavailable or its stop boundary changed"
);
assertV4(
  scores.status === "post-canary-batch-17-single-score-pass-stability-gate-passed" &&
    scores.totals?.debates === 4 &&
  ...
  "the locked Batch 17 score artifact is unavailable"
);
assertV4(
  ...
  "the frozen Batch 17 single-pass score controls changed"
);
assertV4(
  ledger.status ===
      "passed-post-canary-batch-17-deterministic-final-ledger-assembly" &&
    ledger.debates?.length === 4 &&
    ledger.audit?.finalMoves === 79 &&
    ledger.audit?.audioVerifiedMoves === 1 &&
    ledger.authorization?.publicationReconstruction === false &&
    ledger.authorization?.productionMutation === false &&
    ledgerManifest.status ===
      "frozen-four-debate-post-canary-batch-17-deterministic-final-ledger-assembly" &&
    ledgerAnalysis.status ===
      "post-canary-batch-17-deterministic-final-ledger-gate-passed",
  "the locked Batch 17 final ledger changed"
);
assertV4(
  productionManifest.model?.label === POST_CANARY_BATCH_17_PUBLICATION_MODEL.label &&
    productionManifest.model?.slug === POST_CANARY_BATCH_17_PUBLICATION_MODEL.slug &&
    productionManifest.model?.reasoningEffort ===
      POST_CANARY_BATCH_17_PUBLICATION_MODEL.reasoningEffort &&
    productionManifest.model?.authentication ===
      POST_CANARY_BATCH_17_PUBLICATION_MODEL.authentication,
  "the frozen production model or authentication changed"
);
assertV4(
  selection.selected?.map((item) => item.debateNumber).join(",") ===
      POST_CANARY_BATCH_17_PUBLICATION_DEBATES.join(",") &&
    selection.selected.every((item) => item.speakerCount === 2) &&
    selection.authorization?.productionMutation === false,
  "the frozen Batch 17 selection changed"
);
assertV4(
  ledger.debates.map((debate) => debate.debateNumber).join(",") ===
    ...
    scores.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_17_PUBLICATION_DEBATES.join(","),
  "the Batch 17 ledger or score order changed"
);
assertV4(
...
const finalLedgerInputs = await loadPostCanaryBatch17FinalLedgerInputs();
validatePostCanaryBatch17FinalLedger(
  ledger,
...
const catalog = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-17-local-reference-catalog",
  references: postCanaryBatch17ReferenceCatalog()
};
...
for (const debateNumber of POST_CANARY_BATCH_17_PUBLICATION_DEBATES) {
  ...
  const packet = buildPostCanaryBatch17PublicationPacket({
    ledgerDebate,
    scoreDebate,
    sourcePacket,
  ...
  const schema = buildPostCanaryBatch17PublicationSchema(packet);
...
assertV4(contexts.length === 4, "exactly four publication contexts required");
assertV4(
  contexts.reduce((sum, context) => sum + context.moves, 0) === 79,
  "Batch 17 publication move coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.sections, 0) === 21,
  "Batch 17 publication section coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.audioVerifiedMoves, 0) === 1,
  "Batch 17 audio-verification coverage changed"
);
assertV4(
  contexts.every(
    (context) =>
  ...
  "Batch 17 publication section or quote eligibility is invalid"
);
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-17-publication-execution-preparation-manifest",
  protocolId: POST_CANARY_BATCH_17_PUBLICATION_PROTOCOL_ID,
  status:
  ...
  userAuthorization: {
    instruction:
      "The frozen Batch 17 standing authorization permits preparation, activation, and execution of exactly four score-locked publication-reconstruction contexts while every gate passes, with direct incremental cost capped at $0.",
  ...
  model: structuredClone(POST_CANARY_BATCH_17_PUBLICATION_MODEL),
  costEstimate: {
    ...
    contexts: 4,
    expectedParallelWallMinutes: [10, 20],
    expectedAggregateModelMinutes: [17, 28],
    expectedAggregateComputeHours: [0.28, 0.47],
    ...
    estimateBasis: {
      ...
      scalingRule: "four-fifths-of-five-context-plan"
    }
  },
  ...
  acceptanceContract: {
    validContextsRequired: 4,
    movesAuthoredRequired: 79,
    critiquesRequired: 79,
    exactSourceQuotesRequired: 8,
    overallCommentarySidesRequired: 8,
    aiExtensionSidesRequired: 8,
