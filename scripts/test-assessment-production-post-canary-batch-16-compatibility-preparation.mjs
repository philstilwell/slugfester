...
const preparationPath =
  `${POST_CANARY_BATCH_16_COMPATIBILITY_ROOT}/preparation-manifest.json`;
const analysisPath =
  `${POST_CANARY_BATCH_16_COMPATIBILITY_ROOT}/analysis.json`;
...
assert.equal(
  ...
  "batch-16-site-ledger-adapter-and-validator-route"
);
assert.equal(
...
assert.equal(preparation.totals.sections, 53);
assert.equal(preparation.totals.moves, 194);
assert.equal(preparation.totals.oneSidedDisplayRows, 12);
assert.equal(preparation.totals.overallBlunders, 51);
assert.equal(preparation.totals.emptyReferenceLinks, 49);
assert.equal(preparation.totals.suppliedReferenceLinks, 2);
...
assert.equal(
  await exists(`${POST_CANARY_BATCH_16_COMPATIBILITY_ROOT}/execution.json`),
  false
);
assert.equal(
  await exists(
...
assert.deepEqual(packetNumbers, POST_CANARY_BATCH_16_COMPATIBILITY_ORDER);
for (const packetRecord of preparation.artifacts.packets) {
  ...
  const positive = validatePostCanaryBatch16SiteLedgerAdapter({
