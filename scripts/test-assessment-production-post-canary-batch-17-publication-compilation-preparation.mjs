...
import { POST_CANARY_BATCH_17_PUBLICATION_COMPILATION_ORDER,
  ...
  compilePostCanaryBatch17PublicationStagingRecord } from
...
assertV4(m.status === "frozen-post-canary-batch-17-deterministic-publication-compilation-prepared" &&
  canonicalJson(m.explicitOrder) === canonicalJson(POST_CANARY_BATCH_17_PUBLICATION_COMPILATION_ORDER) &&
  m.contexts?.length === 4 && m.contexts.reduce((sum, row) => sum + row.expectedMoves, 0) === 79 &&
  ...
  identity.rows?.length === 4 && identity.legacyScoresIncluded === false,
"Batch 17 compilation preparation changed");
let moves = 0;
for (const context of m.contexts) {
  ...
  const compiled = compilePostCanaryBatch17PublicationStagingRecord({ output, packet, identity: row });
  const validation = validatePostCanaryBatch17CompiledStagingRecord({ compiled, output, packet, identity: row });
  moves += validation.moves;
}
assertV4(moves === 79, "compiled move replay changed");
...
console.log(JSON.stringify({ status: "passed", debates: 4, moves, sourceHashes: Object.keys(m.sourceHashes).length,
