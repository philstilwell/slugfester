...
import {
  POST_CANARY_BATCH_09_DEBATE_170_BASE_OUTPUT,
  POST_CANARY_BATCH_09_DEBATE_170_PUBLICATION_PACKET,
  POST_CANARY_BATCH_09_DEBATE_170_REPAIR_FIELDS,
  POST_CANARY_BATCH_09_DEBATE_170_REPAIR_PACKET_VERSION,
  POST_CANARY_BATCH_09_DEBATE_170_REPAIR_PARTITIONS,
  POST_CANARY_BATCH_09_DEBATE_170_REPAIR_PROTOCOL_ID,
  POST_CANARY_BATCH_09_DEBATE_170_REPAIR_ROOT,
  buildDebate170RepairSchema,
  debate170RepairMoveId
...
const DIAGNOSIS = `${POST_CANARY_BATCH_09_PUBLICATION_ROOT}/failure-diagnosis.json`;
const FAILED_EXECUTION = `${POST_CANARY_BATCH_09_PUBLICATION_ROOT}/model-execution.json`;
const FAILED_ANALYSIS = `${POST_CANARY_BATCH_09_PUBLICATION_ROOT}/analysis.json`;
...
const STATIC_SOURCE_FILES = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-09-publication-repair.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-09-publication-repair.mjs",
  "scripts/test-assessment-production-post-canary-batch-09-publication-repair-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-09-publication-repair.mjs",
  "scripts/run-assessment-production-post-canary-batch-09-publication-repair.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-09-publication-repair.mjs"
];
...
const standingAuthorization = parsed(STANDING_AUTHORIZATION);
const baseOutput = parsed(POST_CANARY_BATCH_09_DEBATE_170_BASE_OUTPUT);
const publicationPacket = parsed(POST_CANARY_BATCH_09_DEBATE_170_PUBLICATION_PACKET);

assertV4(
  standingAuthorization.status ===
      "frozen-active-batch-09-complete-remaining-workflow-standing-authorization" &&
    standingAuthorization.batchNumber === 9 &&
    standingAuthorization.authorization?.boundedPublicationRepairs === true &&
    standingAuthorization.authorization?.publicationPreparationAndModelExecution ===
      true &&
    standingAuthorization.authorization?.commitAndPushPassingCheckpoints === true &&
    standingAuthorization.executionControls?.repairWritableFieldsMaximumPerPacket ===
      2 &&
    standingAuthorization.executionControls?.attemptsPerContextOrPass === 1 &&
    standingAuthorization.executionControls?.retriesMaximum === 0 &&
    standingAuthorization.model?.label === "5.6 Sol" &&
    standingAuthorization.model?.reasoningEffort === "low" &&
    standingAuthorization.model?.authentication === "ChatGPT subscription",
  "the active Batch 9 standing authorization changed"
);
assertV4(
  diagnosis.status ===
      "diagnosed-batch-09-operational-canary-fourteen-critique-word-overruns" &&
    diagnosis.failedContext?.debateNumber === "170" &&
    diagnosis.failedContext?.debateId === publicationPacket.debateId &&
    diagnosis.failureBoundary?.failedFieldCount === 14 &&
    diagnosis.failureBoundary?.excessWordsTotal === 59 &&
    diagnosis.prospectiveRecoveryOnly?.currentlyAuthorized === true &&
  ...
  "the frozen Debate 170 failure diagnosis changed"
);
assertV4(
  execution.contextsPlanned === 10 &&
    ...
    execution.results?.[0]?.debateNumber === "170" &&
    execution.results?.[0]?.gateAcceptancePassed === false &&
    analysis.status === "post-canary-batch-09-publication-output-gate-failed",
  "the preserved failed publication gate changed"
);
assertV4(
  sha256(bytesByFile[POST_CANARY_BATCH_09_DEBATE_170_BASE_OUTPUT]) ===
      diagnosis.failedContext.outputSha256 &&
    sha256(bytesByFile[POST_CANARY_BATCH_09_DEBATE_170_BASE_OUTPUT]) ===
      diagnosis.artifacts.output.sha256 &&
    sha256(bytesByFile[POST_CANARY_BATCH_09_DEBATE_170_PUBLICATION_PACKET]) ===
  ...
  "a diagnosed Debate 170 source artifact drifted"
);
assertV4(
  canonicalJson(diagnosis.prospectiveRecoveryOnly.proposedRepairPartition) ===
      canonicalJson(POST_CANARY_BATCH_09_DEBATE_170_REPAIR_PARTITIONS) &&
    canonicalJson(diagnosis.diagnosticReplay.hypotheticalWritableFields) ===
      canonicalJson(POST_CANARY_BATCH_09_DEBATE_170_REPAIR_FIELDS),
...
for (let packetIndex = 0; packetIndex < 7; packetIndex += 1) {
  const writableFields = POST_CANARY_BATCH_09_DEBATE_170_REPAIR_PARTITIONS[packetIndex];
  const corrections = writableFields.map((field) => {
    const moveId = debate170RepairMoveId(field);
  ...
  const packet = {
    schemaVersion: POST_CANARY_BATCH_09_DEBATE_170_REPAIR_PACKET_VERSION,
    protocolId: POST_CANARY_BATCH_09_DEBATE_170_REPAIR_PROTOCOL_ID,
    packetIndex,
    ...
    immutableBaseOutput: POST_CANARY_BATCH_09_DEBATE_170_BASE_OUTPUT,
    publicationPacket: POST_CANARY_BATCH_09_DEBATE_170_PUBLICATION_PACKET,
  ...
  const schema = buildDebate170RepairSchema(packet);
...
const MERGED_OUTPUT = `${ROOT}/merged/debate-170.json`;
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-09-debate-170-publication-repair-execution-preparation-manifest",
  protocolId: POST_CANARY_BATCH_09_DEBATE_170_REPAIR_PROTOCOL_ID,
  status:
  ...
  userAuthorization: {
    instruction:
      "Use the active Batch 9 complete-workflow standing authorization and the user's instruction to continue as far as possible without another routine approval.",
    resolvedScope:
      "prepare, validate, freeze, activate, and execute exactly seven field-disjoint two-field Debate 170 publication-repair contexts, merge accepted repairs, validate Debate 170, and continue to the nine untouched publication contexts after successful gates",
  ...
  repairContract: {
    repairType: "critique-word-boundary",
    writableFields: POST_CANARY_BATCH_09_DEBATE_170_REPAIR_FIELDS,
  ...
  artifacts: {
    ...
    execution: EXECUTION,
    analysis: ANALYSIS,
    originalFailedOutput: POST_CANARY_BATCH_09_DEBATE_170_BASE_OUTPUT,
