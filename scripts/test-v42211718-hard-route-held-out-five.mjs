...
assert.equal(sample.audit.direct, 0);
assert.equal(sample.audit.partition, 5);
assert.equal(sample.audit.failedFiveOverlap, 0);
assert(sample.audit.distinctTopicFamilies >= 5);
...
if (await access(screeningPath).then(() => true, () => false)) {
  ...
  assert.equal(screening.status, "hard-route-held-out-five-screened-source-preparation-authorized");
