...
import {
  ...
  buildPostCanaryBatch03PublicationStagingPreviewHtml,
...
assertV4(
  ...
  "Batch 3 publication finalization is not authorized"
);
for (const [file, digest] of Object.entries(activation.sourceHashes)) {
  assertV4(
    sha256(await readFile(path.resolve(file))) === digest,
    `Batch 3 finalization activation source hash mismatch: ${file}`
  );
}
...
try {
  ...
  for (const debateNumber of POST_CANARY_BATCH_03_PUBLICATION_FINALIZATION_ORDER) {
    ...
    assertV4(context && identity, `${debateNumber}: Batch 3 finalization context missing`);
    ...
    assertV4(
      ...
      `${debateNumber}: frozen Batch 3 finalization input hash changed`
    );
    ...
    assertV4(
      ...
      `${debateNumber}: prepared Batch 3 finalization output changed`
    );
    candidates.push({
      context,
      candidateBytes,
  ...
  assertV4(
    candidates.length === 10 &&
      candidates.reduce((sum, item) => sum + item.validation.moves, 0) === 200 &&
      candidates.every(
        (item) =>
    ...
    "complete ten-debate Batch 3 publication finalization validation failed"
  );
  ...
  const audit = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-03-publication-finalization-audit",
    protocolId: activation.protocolId,
    status: "passed-ten-debate-batch-03-publication-finalization",
    ...
    preview: {
      ...
      localOnly: true,
      noindex: true,
      postCanaryBatch03StagingBanner: true,
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-publication-finalization-execution",
  protocolId: activation.protocolId,
  status:
    status === "passed"
      ? "ten-debate-batch-03-publication-finalization-passed"
      : "ten-debate-batch-03-publication-finalization-failed",
  startedAt,
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-publication-finalization-analysis",
  protocolId: activation.protocolId,
  ...
  nextAuthorizedAction:
    status === "passed"
      ? "prepare-batch-03-rendering-verification-under-standing-authorization"
