...
const finalLedgerPath =
  `${POST_CANARY_BATCH_10_FINAL_LEDGER_ROOT}/final-ledger.json`;
...
assertV4(
  ...
  "post-canary Batch 10 final-ledger manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen Batch 10 final-ledger source hash mismatch`
  );
}
const inputs = await loadPostCanaryBatch10FinalLedgerInputs();
const ledger = buildPostCanaryBatch10FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
