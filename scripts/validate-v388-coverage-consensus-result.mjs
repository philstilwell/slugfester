...
const [analysis, inventory, primaryManifest, primaryExecution, conditionalManifest, conditionalExecution, coherenceManifest, coherenceExecution, audio] = await Promise.all([
  readJson(`${V388_CONSENSUS_ROOT}/coverage-consensus-analysis.json`),
  readJson(`${V388_CONSENSUS_ROOT}/final-coverage-inventory.json`),
  readJson(`${V388_CONSENSUS_ROOT}/adjudication/execution-manifest.json`),
  ...
  readJson(`${V388_CONSENSUS_ROOT}/coherence-adjudication/execution-manifest.json`),
  readJson(`${V388_CONSENSUS_ROOT}/coherence-adjudication/model-execution.json`),
  readJson(`${V388_CONSENSUS_ROOT}/audio-verification.json`)
]);
for (const [file, digest] of Object.entries(primaryManifest.sourceHashes)) assert(sha256(await readBytes(file)) === digest, `primary source hash mismatch: ${file}`);
...
for (const [file, digest] of Object.entries(coherenceManifest.sourceHashes)) assert(sha256(await readBytes(file)) === digest, `coherence source hash mismatch: ${file}`);
...
assert(coherenceExecution.validOutputContexts === 1 && coherenceExecution.totalRetries === 0 && coherenceExecution.results.every((item) => item.gateAcceptancePassed && item.transportClassification !== "invalid"), "coherence adjudication execution invalid");
...
assert(analysis.totals.primaryAdjudicationContexts === 3 && analysis.totals.conditionalAdjudicationContexts === 1 && analysis.totals.coherenceAdjudicationContexts === 1 && analysis.totals.crossFieldCoherenceViolationsResolved === 1 && analysis.totals.scoringFields === 0 && analysis.totals.meteredApiCostUsd === 0 && analysis.totals.transcriptionCostUsd === 0, "execution or cost totals invalid");
