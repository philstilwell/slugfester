...
const protocolId = "assessment-production-post-canary-batch-14-publication-resumption-1-critique-recovery-level-2";
...
const files = {
  ...
  levelOneOutputs: [0, 1, 2, 4, 5, 6, 7, 9].map((index) => `${levelOneRoot}/outputs/context-${index}.json`),
...
if (mode === "--prepare") {
  ...
  const levelOneExecution = await readJson(files.levelOneExecution);
  ...
  assert.equal(levelOneExecution.status, "batch-14-publication-resumption-1-critique-recovery-level-1-execution-failed");
  assert.equal(levelOneExecution.validContexts, 8); assert.equal(levelOneExecution.invalidContexts, 2);
  const sourcePaths = [files.diagnosis, files.resumptionExecution, files.levelOneExecution, ...files.levelOneOutputs, files.workflow, files.outputContract, files.manual, "scripts/assessment-production-post-canary-batch-14-publication-resumption-1-critique-recovery-level-2.mjs", "scripts/lib/assessment-production-post-canary-batch-14-publication-validation.mjs", ...debateNumbers.flatMap((debateNumber) => Object.values(debateFiles[debateNumber]).slice(0, 5))];
  ...
  const contexts = [];
  const failedTargets = levelOneExecution.results.filter((item) => !item.gateAcceptancePassed).flatMap((item) => item.moveIds.map((moveId) => ({ debateNumber: item.debateNumber, moveId })));
  assert.deepEqual(failedTargets, [
    { debateNumber: "133", moveId: "pro-sourcehood-without-branching-future" },
    { debateNumber: "133", moveId: "con-modal-scope-fixed-past-distinction" },
    { debateNumber: "133", moveId: "con-absolute-regulative-or-real" },
    { debateNumber: "133", moveId: "pro-normative-inquiry-real-absolute" }
  ]);
  for (const target of failedTargets) {
    const debateNumber = target.debateNumber;
    ...
    const moveIds = [target.moveId]; const contextIndex = contexts.length;
...
if (mode === "--activate") {
  ...
  const activation = { ...preparation, schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-resumption-1-critique-recovery-level-2-activation", status: "frozen-four-atomic-critique-batch-14-publication-resumption-1-recovery-level-2-authorized", activatedAt: at, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(), preparation: { path: files.preparation, sha256: sha256(bytes) }, authorization: { ...preparation.authorization, modelExecution: true, deterministicMergeAndValidation: true }, nextAuthorizedAction: "execute-four-atomic-debate-133-critique-recovery-level-2-contexts-once" };
  if (shouldWrite) await writeFile(files.activation, pretty(activation));
  console.log(pretty({ status: shouldWrite ? activation.status : "preview", contexts: 4, writableCritiques: 4, recoveryLevel: 2, attemptsMaximum: 4, retriesMaximum: 0, directIncrementalCostUsdMaximum: 0 })); process.exit(0);
}

if (mode === "--run") {
  const activation = await readJson(files.activation); assert.equal(activation.status, "frozen-four-atomic-critique-batch-14-publication-resumption-1-recovery-level-2-authorized");
...
if (mode === "--analyze") {
  ...
  assert.equal(execution.status, "four-atomic-critique-batch-14-publication-resumption-1-recovery-level-2-execution-passed"); const [levelOneOutputs, levelTwoOutputs] = await Promise.all([Promise.all(files.levelOneOutputs.map(readJson)), Promise.all(activation.contexts.map((item) => readJson(item.output)))]); const outputs = [...levelOneOutputs, ...levelTwoOutputs]; const corrections = outputs.flatMap((output) => output.corrections); assert.equal(corrections.length, 19); assert.equal(new Set(corrections.map((item) => `${item.moveId}`)).size, 19);
  const validations = [];
  for (const debateNumber of debateNumbers) {
    ...
    const validation = validatePostCanaryBatch14PublicationOutput(merged, packet); const mergedText = pretty(merged); const validationRecord = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-validation", protocolId: merged.protocolId, status: "passed", debateNumber, debateId: packet.debateId, outputSha256: sha256(mergedText), validationSummary: validation, validationMessage: null, modelAuthoredScores: 0, lockedScoresUnchanged: true }; const provenance = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-recovery-provenance", protocolId, debateNumber, originalFailure: { status: "output-validation-failed", originalOutputPreserved: true }, recoveryLevel1: { contexts: levelOneOutputs.filter((item) => item.debateNumber === debateNumber).length, acceptedCritiques: levelOneOutputs.filter((item) => item.debateNumber === debateNumber).flatMap((item) => item.corrections).length, failedAtomicContextIndexes: debateNumber === "133" ? [3, 8] : [] }, recoveryLevel2: { contexts: levelTwoOutputs.filter((item) => item.debateNumber === debateNumber).length, acceptedCritiques: levelTwoOutputs.filter((item) => item.debateNumber === debateNumber).flatMap((item) => item.corrections).length, eachUnavailableFieldAcceptedExactlyOnce: true }, merge: { acceptedCompanionFieldsChanged: false, scoresChanged: false, scorePassRerun: false, modelAuthoredScores: 0, completeValidationPassed: true } };
  ...
  const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-resumption-1-critique-recovery-analysis", protocolId, status: "debates-110-and-133-two-level-field-disjoint-publication-recovery-passed-awaiting-six-context-resumption", recoveryLevel: 2, recoveryLevelsMaximum: 2, debates: debateNumbers, critiques: 19, contextsThisLevel: 4, attemptsThisLevel: 4, attemptsAcrossRecoveryLevels: 14, retries: 0, timeoutExtensions: 0, directIncrementalCostUsd: 0, paidServiceCalls: 0, modelAuthoredScores: 0, validations, authorization: { sixOriginalUnattemptedContextResumption: true, furtherRecoveryLevel: false, deterministicCompilation: false, productionMutation: false, nextBatchSelection: false }, nextAuthorizedAction: "resume-six-original-unattempted-publication-contexts" };
  if (shouldWrite) await writeFile(files.analysis, pretty(analysis)); console.log(pretty({ status: analysis.status, debates: debateNumbers, critiques: 19, contextsThisLevel: 4, attemptsAcrossRecoveryLevels: 14, retries: 0, directIncrementalCostUsd: 0, nextAuthorizedAction: analysis.nextAuthorizedAction }));
}
