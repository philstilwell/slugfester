...
const finalLedgerPath =
  `${POST_CANARY_BATCH_03_FINAL_LEDGER_ROOT}/final-ledger.json`;
...
assertV4(
  ...
  "post-canary Batch 3 final-ledger manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen Batch 3 final-ledger source hash mismatch`
  );
}
const inputs = await loadPostCanaryBatch03FinalLedgerInputs();
const ledger = buildPostCanaryBatch03FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
