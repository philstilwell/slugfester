...
assertV4(
  ...
  "production checkpoint v2.2 audio execution unavailable or crossed its boundary"
);
...
const audit = {
  schemaVersion: "1.0-production-checkpoint-v2.2-audio-verification-audit",
...
const analysis = {
  schemaVersion: "1.0-production-checkpoint-v2.2-audio-verification-analysis",
  protocolId: manifest.protocolId,
  status,
  productionCanary: true,
  stagingOnly: true,
  ...
  nextAuthorizedAction: passed
    ? "prepare-production-checkpoint-v2.2-dispute-only-adjudication-packets"
    : "diagnose-production-checkpoint-v2.2-audio-verification-failure-only"
};
if (shouldWrite) {
  ...
  JSON.stringify(
    {
      ...
      excerptRecalls: moves.map((move) => ({
        ...
        moveId: move.moveId,
        fullClipRecall:
          move.deterministicEvidence?.fullClipExcerptRecall ?? null,
        expectedSpeakerRecall:
          move.deterministicEvidence?.expectedSpeakerExcerptRecall ?? null,
        margin:
          move.deterministicEvidence?.expectedSpeakerRecallMargin ?? null
      })),
      paidDiarizationCallsAttempted: execution.attempts,
      retries: 0,
      estimatedProcessingExposureUsd: execution.estimatedProcessingExposureUsd,
