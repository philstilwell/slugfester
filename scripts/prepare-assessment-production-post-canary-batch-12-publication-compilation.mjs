...
import {
  POST_CANARY_BATCH_12_PUBLICATION_COMPILATION_ORDER,
  POST_CANARY_BATCH_12_PUBLICATION_COMPILATION_PROTOCOL_ID,
  POST_CANARY_BATCH_12_PUBLICATION_COMPILATION_ROOT,
  compilePostCanaryBatch12PublicationStagingRecord,
  validatePostCanaryBatch12CompiledStagingRecord
} from "./lib/assessment-production-post-canary-batch-12-publication-compilation.mjs";
import { POST_CANARY_BATCH_12_PUBLICATION_ROOT } from
  "./lib/assessment-production-post-canary-batch-12-publication.mjs";
...
import { POST_CANARY_BATCH_12_STANDING_AUTHORIZATION,
  loadAndValidatePostCanaryBatch12StandingAuthorization } from
  "./lib/assessment-production-post-canary-batch-12-standing-authorization.mjs";
...
const ROOT = POST_CANARY_BATCH_12_PUBLICATION_COMPILATION_ROOT;
const COHORT = `${POST_CANARY_BATCH_12_PUBLICATION_ROOT}/failure-recovery/original-unattempted-context-resumption-2/field-disjoint-repair-2-continuation/complete-cohort-analysis.json`;
...
const ACCEPTED_OUTPUTS = Object.freeze(Object.fromEntries(
  POST_CANARY_BATCH_12_PUBLICATION_COMPILATION_ORDER.map((debateNumber) => [
    debateNumber,
    `${POST_CANARY_BATCH_12_PUBLICATION_ROOT}/outputs/debate-${debateNumber}.json`
  ])
));
const acceptedOutput = (debateNumber) => ACCEPTED_OUTPUTS[debateNumber];
const packetPath = (debateNumber) =>
  `${POST_CANARY_BATCH_12_PUBLICATION_ROOT}/packets/debate-${debateNumber}.json`;
...
assertV4(
  cohort.status ===
    "post-canary-batch-12-complete-ten-debate-publication-output-gate-passed-after-two-recovery-levels" &&
  cohort.totals?.debates === 10 && cohort.totals?.lockedMoves === 204 &&
  cohort.totals?.critiques === 204 && cohort.totals?.exactSourceQuotes === 20 &&
  cohort.integrity?.scoresRemainedImmutable === true &&
  cohort.integrity?.everyAcceptedOutputReplayedDeterministically === true,
"the accepted Batch 12 publication cohort changed");

const productionByNumber = new Map(debates.map((debate) => [debate.number, debate]));
const identities = [];
const contexts = [];
for (const debateNumber of POST_CANARY_BATCH_12_PUBLICATION_COMPILATION_ORDER) {
  ...
  const validation = validatePostCanaryBatch12PublicationOutput(output, packet);
  ...
  const compiled = compilePostCanaryBatch12PublicationStagingRecord({ output, packet, identity });
  const compiledValidation = validatePostCanaryBatch12CompiledStagingRecord({ compiled, output, packet, identity });
...
assertV4(contexts.length === 10 && contexts.reduce((sum, row) => sum + row.expectedMoves, 0) === 204,
"the Batch 12 compilation cohort changed");
const identitySnapshot = { schemaVersion: "1.0-assessment-production-post-canary-batch-12-publication-compilation-identity-snapshot",
  protocolId: POST_CANARY_BATCH_12_PUBLICATION_COMPILATION_PROTOCOL_ID,
...
const sources = [COHORT, POST_CANARY_BATCH_12_STANDING_AUTHORIZATION,
  ...
  "scripts/lib/assessment-production-post-canary-batch-12-publication-compilation.mjs",
  "scripts/lib/assessment-production-post-canary-batch-12-standing-authorization.mjs",
  "scripts/lib/v4-lean-production.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-12-publication-compilation.mjs",
  "scripts/test-assessment-production-post-canary-batch-12-publication-compilation-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-12-publication-compilation.mjs",
  "scripts/run-assessment-production-post-canary-batch-12-publication-compilation.mjs",
...
const manifest = { schemaVersion: "1.0-assessment-production-post-canary-batch-12-deterministic-publication-compilation-preparation",
  protocolId: POST_CANARY_BATCH_12_PUBLICATION_COMPILATION_PROTOCOL_ID,
  status: "frozen-post-canary-batch-12-deterministic-publication-compilation-prepared",
  ...
  userAuthorization: { standingAuthorization: POST_CANARY_BATCH_12_STANDING_AUTHORIZATION,
  ...
  explicitOrder: POST_CANARY_BATCH_12_PUBLICATION_COMPILATION_ORDER, contexts,
  ...
  nextAuthorizedAction: "activate-one-batch-12-deterministic-publication-compilation-pass" };
...
console.log(JSON.stringify({ status: manifest.status, debates: 10, moves: 204,
