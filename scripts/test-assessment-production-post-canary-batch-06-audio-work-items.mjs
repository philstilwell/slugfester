...
assert.equal(work.batchNumber, 5);
assert.equal(work.stagingOnly, true);
assert.equal(preparation.userAuthorization.workItemsAuthorized, 2);
assert.equal(
...
assert.equal(preparation.totals.debates, 1);
assert.equal(preparation.totals.sourceVideoIds, 1);
assert.equal(preparation.totals.moves, 2);
assert.equal(work.moves.length, 2);
...
assert.equal(preparation.validation.exactAuthorizedMoveCount, 2);
assert.equal(
  preparation.validation.canonicalLocalTextAndMetadataChainsVerified,
  1
);
assert.equal(preparation.validation.repositoryRenderedLockedExcerpts, 2);
assert.equal(preparation.validation.repositoryRenderedTimestampWindows, 2);
assert.equal(preparation.validation.expectedSpeakersLocked, 2);
assert.equal(
...
assert.equal(preparation.sourceCompatibility.occurrences[0].debateNumber, "141");
