...
assert(manifest.status === "frozen-before-model-execution" && manifest.correctionSmokeExecutionAuthorized && manifest.exposedCorrectionCases, "execution manifest identity invalid");
...
assert(sha256(dryText) === manifest.executionDryFixture.sha256 && dry.passed && dry.modelContextsExecuted === 0 && dry.syntheticAdjudicationContexts === 3 && dry.twoVoteResolutions === 3, "execution dry fixture invalid");
execFileSync(process.execPath, ["scripts/test-v375-correction-execution-tooling.mjs"], { cwd: root, stdio: "ignore" });
...
assert(manifest.executionPolicy.modelOutputRetriesMaximum === 0 && manifest.executionPolicy.sameRequestStreamRecoveriesMaximum === 0 && manifest.executionPolicy.meteredApiCostUsdMaximum === 0 && manifest.executionPolicy.transcriptionCostUsdMaximum === 0, "retry or cost lock invalid");
assert(manifest.thresholds.atomicBundles === 8 && manifest.thresholds.initialAtomicBundleAgreementsRequired === 8 && manifest.thresholds.finalTwoVoteBundlesRequired === 8 && manifest.thresholds.scoringFieldsMaximum === 0, "perfect semantic threshold invalid");
for (const [reviewerPass, debates] of Object.entries(manifest.initialContexts)) for (const [debateNumber, context] of Object.entries(debates)) {
  ...
  assert(packet.reviewerPass === reviewerPass && packet.debateNumber === debateNumber && !canonicalJson(packet).includes("matchesDevelopmentReference") && !canonicalJson(packet).includes("semanticTuple"), `${reviewerPass}.${debateNumber}: model packet leak or identity invalid`);
}
assert(manifest.prohibitions.correctedBenchmarkKey && manifest.prohibitions.largerModelBatch && manifest.prohibitions.heldOutAccess && manifest.prohibitions.numericalParticipantScoring && manifest.prohibitions.assessmentProse && manifest.prohibitions.productionMutation, "prohibitions invalid");
