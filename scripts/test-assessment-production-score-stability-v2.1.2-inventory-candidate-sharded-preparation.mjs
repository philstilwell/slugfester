...
assert.equal(
  ...
  V212_CANDIDATE_SHARDED_INVENTORY.protocolId
);
assert.equal(
  preparation.status,
...
assert.equal(
  preparation.stageDesign.maximumPlanSideSchemaPrototypesFrozen,
  20
);
...
for (const context of preparation.contexts) {
  ...
  assert.equal(
    sha256(discoveryCandidateBundleBytes),
    context.discoveryCandidateBundleSha256
  );
  assert.equal(
    sha256(discoverySparseContextBytes),
    context.discoverySparseContextSha256
  );
  assert.equal(
  ...
  const sourcePreparationPacket = {
    ...structuredClone(inventorySourcePacket),
    schemaVersion:
  ...
  sourcePreparationPacket.modelInputBoundary = {
  ...
  assert.deepEqual(
    buildV212InventorySourcePacket(sourcePreparationPacket),
    inventorySourcePacket
  );
  ...
  for (const asset of context.sideAssets) {
    ...
    assert.equal(
      ...
      asset.maximumPlanSchemaPrototypeSha256
    );
  ...
  assert.equal(planPacket.copiedInputs.length, 5);
  ...
  assert.equal(
    context.proCandidates + context.conCandidates,
    context.candidates
  );
...
assert.equal(
  preparation.transport.semanticCandidateCorrectionPerformed,
  false
);
  ...
  JSON.stringify(
    {
      ...
      exactPlannerPacketsFrozen:
        preparation.totals.exactPlannerPacketsFrozen,
      completeSideTransportsFrozen:
        preparation.stageDesign.completeSideTransportsFrozen,
      exactSidePacketsFrozen: preparation.totals.exactSidePacketsFrozen,
      modelContextsExecuted: preparation.totals.modelContextsExecuted,
      scoresDerived: preparation.totals.scoresDerived,
