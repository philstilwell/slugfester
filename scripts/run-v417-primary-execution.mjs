...
const manifest = JSON.parse(await readFile(path.resolve(root, `${V417_ROOT}/primary-execution-manifest.json`), "utf8"));
const codex = "/Applications/ChatGPT.app/Contents/Resources/codex";
const authSource = path.join(os.homedir(), ".codex", "auth.json");
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
assertV4(manifest.status === "frozen-six-context-primary-authorized" && manifest.authorization.primaryModelExecution && !manifest.authorization.passBModelExecution, "primary execution unauthorized");
...
for (const context of manifest.contexts) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-v417-primary-${context.debateNumber}-`));
  const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), `slugfester-v417-home-primary-${context.debateNumber}-`));
  ...
  try {
    ...
    const prompt = `Read ${inputNames} completely and no other files. The v4.1.7 workflow and manual govern conflicts. Act only as the fresh isolated bounded primary judge for Debate ${context.debateNumber}. Build four to six genuinely contested sections and the minimum eight-to-twenty-four moves needed to preserve every load-bearing route and decisive exchange, with one or two actual moves per side per section. Compression must never raise a rating: include a distinct weaker move only when it is independently necessary to represent central burden completion, and never select or omit material because of its likely score. Do not supply sequence numbers; chronology is repository-derived. Supply every required route tier. Silently run the response, burden-reference, rating-band, charity, and duplicate-exclusion consistency checklist before submission. Do not calculate totals, identify a winner, write publication prose, or supply precision/clarity or epistemic-calibration scalar values. Control selection, legacy assessments, prior scores, winners, other debates, and high-effort references are unavailable. Return exactly one schema-conforming JSON object and no commentary.`;
    process.stdout.write(`[v4.1.7-primary] starting ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber}\n`);
    ...
    if (invocation.timedOut || invocation.code !== 0 || invocation.signal !== null) record = { ...base, status: invocation.timedOut ? "timed-out" : "transport-failed", outputWritten: false, deterministicValidationPassed: false, gateAcceptancePassed: false };
      ...
      const validation = await run(process.execPath, ["scripts/validate-v417-primary-output.mjs", context.output, context.packet], { cwd: root, env: process.env }, 120000);
  ...
  results.push(record);
  process.stdout.write(`[v4.1.7-primary] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
