...
const activation = JSON.parse(
  await readFile(`${ROOT}/resumption-1/execution-activation.json`, "utf8")
);
...
assertV4(
  ...
  "Batch 15 independent-judgment execution is unauthorized"
);
assertV4(
...
async function executeContext(context, localContextIndex) {
  const contextIndex = context.originalContextIndex;
  const remainingGateMs = gateDeadline - Date.now();
  assertV4(
    remainingGateMs > 0,
    "absolute Batch 15 independent-judgment gate timeout reached before launch"
  );
  ...
  const temporary = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-batch-15-judgment-${context.debateNumber}-${context.reviewerPass.toLowerCase()}-`
    )
  );
  const isolatedCodexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-batch-15-judgment-home-${context.debateNumber}-${context.reviewerPass.toLowerCase()}-`
    )
  );
  ...
  process.stdout.write(
    `[batch-15-judgment-resumption-1] Debate ${context.debateNumber} Pass ${context.reviewerPass} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const unattemptedContextIndexes = activation.contexts
  .filter((_, index) => !resultsByIndex[index])
  .map((context) => context.originalContextIndex);
const passed =
  results.length === activation.contexts.length &&
  validContexts === activation.contexts.length;
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-15-independent-judgment-model-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "seventeen-original-unattempted-batch-15-independent-judgment-contexts-passed"
    : "batch-15-independent-judgment-resumption-1-complete-with-failure",
