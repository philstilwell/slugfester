...
assertV4(!(await exists(ACTIVATION)), "five-context resumption activation already exists");
const preparationBytes = await readFile(path.resolve(PREPARATION));
const preparation = JSON.parse(preparationBytes);
assertV4(
  preparation.protocolId === PROTOCOL_ID &&
    preparation.status === "frozen-five-original-unattempted-batch-13-publication-contexts-prepared-not-activated" &&
    preparation.contexts?.length === 5 && preparation.totals?.moves === 102 &&
  ...
  "five original unattempted contexts are not prepared"
);
assertV4(
  ...
  "five-context model or execution controls changed"
);
assertV4(
...
const activation = {
  ...
  status: "frozen-five-original-unattempted-batch-13-publication-contexts-activated",
  activatedAt,
  ...
  nextAuthorizedAction: "execute-only-five-frozen-original-unattempted-publication-contexts"
};
