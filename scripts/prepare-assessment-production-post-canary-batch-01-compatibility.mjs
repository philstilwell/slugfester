...
import {
  POST_CANARY_BATCH_01_COMPATIBILITY_ORDER,
  POST_CANARY_BATCH_01_COMPATIBILITY_PROTOCOL_ID,
  POST_CANARY_BATCH_01_COMPATIBILITY_ROOT,
  POST_CANARY_BATCH_01_SITE_LEDGER_ADAPTER_VERSION,
  buildPostCanaryBatch01SiteLedgerAdapter,
  serializedJson,
  sha256,
...
const paths = {
  preparation: `${POST_CANARY_BATCH_01_COMPATIBILITY_ROOT}/preparation-manifest.json`,
  analysis: `${POST_CANARY_BATCH_01_COMPATIBILITY_ROOT}/analysis.json`,
  activation: `${POST_CANARY_BATCH_01_COMPATIBILITY_ROOT}/execution-activation.json`,
  execution: `${POST_CANARY_BATCH_01_COMPATIBILITY_ROOT}/execution.json`,
  stagedLedgerRoot:
    `${POST_CANARY_BATCH_01_COMPATIBILITY_ROOT}/output-bundle/staged-ledgers`,
  compatibility:
    "docs/assessment-production/post-canary-continuation-v1/batch-01/publication-finalization/compatibility-analysis.json",
  finalizationAudit:
    "docs/assessment-production/post-canary-continuation-v1/batch-01/publication-finalization/output-bundle/finalization-audit.json",
  finalLedger:
  ...
  renderingAnalysis:
    "docs/assessment-production/post-canary-continuation-v1/batch-01/rendering-verification/analysis.json",
  renderingAudit:
    "docs/assessment-production/post-canary-continuation-v1/batch-01/rendering-verification/rendering-audit.json",
  ...
  preparationScript:
    "scripts/prepare-assessment-production-post-canary-batch-01-compatibility.mjs",
  preparationTest:
    "scripts/test-assessment-production-post-canary-batch-01-compatibility-preparation.mjs",
...
assertV4(
  ...
  "a stable --frozen-at ISO timestamp is required for the first Batch 1 compatibility preparation write"
);
...
assertV4(
  ...
  "passed Batch 1 rendering evidence audit required"
);
assertV4(
  compatibility.status ===
      "batch-01-production-compatibility-boundary-recorded" &&
    canonicalJson(compatibility.findings.map((finding) => finding.id)) ===
      canonicalJson(["batch-01-site-ledger-adapter-and-validator-route"]) &&
  ...
  "frozen Batch 1 publication compatibility blocker required"
);
assertV4(
  finalizationAudit.status ===
      "passed-ten-debate-batch-01-publication-finalization" &&
  ...
  "passed one-pass Batch 1 staging-only finalization audit required"
);
assertV4(
  scores.status ===
      "post-canary-batch-01-single-score-pass-stability-gate-passed" &&
  ...
  "accepted one-pass Batch 1 score ledger required"
);
validatePostCanaryBatch01FinalLedger(
  finalLedger,
...
assertV4(
  finalLedger.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_01_COMPATIBILITY_ORDER.join(",") &&
    scores.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_01_COMPATIBILITY_ORDER.join(",") &&
    renderingAudit.explicitOrder.join(",") ===
      POST_CANARY_BATCH_01_COMPATIBILITY_ORDER.join(","),
  "Batch 1 compatibility debate order changed"
);
...
const packetRecords = [];
for (const debateNumber of POST_CANARY_BATCH_01_COMPATIBILITY_ORDER) {
  ...
  const packetPath =
    `${POST_CANARY_BATCH_01_COMPATIBILITY_ROOT}/packets/debate-${debateNumber}.json`;
  ...
  const adapter = buildPostCanaryBatch01SiteLedgerAdapter({
    finalLedgerDebate,
    scoreDebate,
    candidate,
  ...
  const packet = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-01-compatibility-plan-packet",
    protocolId: POST_CANARY_BATCH_01_COMPATIBILITY_PROTOCOL_ID,
    status: "frozen-post-canary-batch-01-compatibility-plan-packet",
...
assertV4(
  totals.debates === 10 &&
    totals.sections === 50 &&
    totals.moves === 177 &&
    totals.oneSidedDisplayRows === 19 &&
    totals.overallBlunders === 53 &&
    totals.emptyReferenceLinks === 52 &&
    totals.suppliedReferenceLinks === 1,
  "Batch 1 compatibility population changed"
);

const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-01-compatibility-preparation-manifest",
  protocolId: POST_CANARY_BATCH_01_COMPATIBILITY_PROTOCOL_ID,
  status: "post-canary-batch-01-compatibility-plan-prepared-and-frozen",
  frozenAt,
  productionCanary: false,
  batchNumber: 1,
  planningOnly: true,
  ...
  scope: {
    finding: "batch-01-site-ledger-adapter-and-validator-route",
    description:
      "Freeze the exact Batch 1 ledger adapters and schema-specific validator route without applying them to the active validator or production data."
  },
  invariants: {
    ...
    scoreProtocolId:
      "assessment-production-post-canary-batch-01-single-deterministic-score-pass",
  ...
  proposedValidatorRoute: {
    ...
    newSchemaVersion: POST_CANARY_BATCH_01_SITE_LEDGER_ADAPTER_VERSION,
    existingCheckpointSchemaVersion:
      "1.0-production-checkpoint-v2.2-site-ledger-adapter",
    exactRoute:
      "When a production ledger uses the Batch 1 adapter schema, load the Batch 1 packet and activation from the frozen Batch 1 compatibility root, verify their hashes, and replay the adapter with the Batch 1 validator. Keep the checkpoint adapter on its existing root and validator and keep legacy Rubric v2 and v2.1 ledgers on their existing calculators.",
    oneSidedDisplayRows:
      "Treat both authenticated adjudicated-consensus adapter schemas as requiring ledgerMoveId and permitting a section exchange with only one side populated.",
    activationRequirements: [
      "activation status is post-canary-batch-01-compatibility-execution-authorized-and-frozen",
  ...
  stagedExecutionPlan: [
    ...
    },
    {
      ...
      allowedWrites: [
        paths.activeValidator,
        `${POST_CANARY_BATCH_01_COMPATIBILITY_ROOT}/output-bundle/**`,
        paths.execution,
        paths.analysis
      ],
      actions: [
        "wire the schema-specific Batch 1 adapter route",
  ...
  mandatoryExecutionTests: {
    positive: [
      "all ten Batch 1 adapters reproduce all 177 locked move scores, 50 section scores per side, and 20 overall scores",
      "all ten final candidates match ledgerMoveId, section order, model, rubric, and score protocol",
      "all 19 one-sided display rows pass through the authenticated Batch 1 adapter route",
      "all 52 empty Overall Commentary reference arrays pass without synthesized links",
      "the one supplied reference link retains full validation",
    ...
    negative: [
      ...
      "a Batch 1 adapter routed through the checkpoint root fails",
  ...
  nextAuthorizedAction:
    "user-approval-required-before-batch-01-production-compatibility-execution-activation"
};
const preparationBytes = serializedJson(preparation);
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-01-compatibility-preparation-analysis",
  protocolId: POST_CANARY_BATCH_01_COMPATIBILITY_PROTOCOL_ID,
  status: "post-canary-batch-01-compatibility-plan-freeze-passed",
  productionCanary: false,
  batchNumber: 1,
  planningOnly: true,
  preparation: {
...
if (write) {
  assertV4(
    ...
    "Batch 1 compatibility execution artifact already exists; planning write blocked"
  );
  await mkdir(
    resolve(`${POST_CANARY_BATCH_01_COMPATIBILITY_ROOT}/packets`),
    { recursive: true }
  );
  ...
  assertV4(
    ...
    "stored Batch 1 compatibility preparation differs from deterministic replay"
  );
  const storedAnalysis = await readJson(paths.analysis);
  assertV4(
    canonicalJson(storedAnalysis) === canonicalJson(analysis),
    "stored Batch 1 compatibility analysis differs from deterministic replay"
  );
  for (const record of packetRecords) {
    const storedPacket = await readFile(resolve(record.packetPath), "utf8");
    assertV4(
      storedPacket === record.packetBytes,
      `${record.debateNumber}: stored Batch 1 compatibility packet differs from deterministic replay`
    );
  }
}
