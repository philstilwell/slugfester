...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-12/discovery";
...
assert.equal(manifest.status, "frozen-forty-four-post-canary-batch-12-discovery-contexts-prepared-not-authorized");
...
assert.equal(manifest.contexts.length, 44);
assert.deepEqual(manifest.contexts.map((context) => context.contextIndex), Array.from({ length: 44 }, (_, index) => index));
...
assert.equal(manifest.executionPolicy.contexts, 44);
...
assert.equal(manifest.executionPolicy.copiedInputBytesMaximum, 65991);
assert.equal(preparation.totals.maximumCopiedInputBytes, 65991);
...
assert.equal(manifest.copiedInputBoundary.frozenObservedCopiedInputBytesMaximum, 65991);
assert.equal(manifest.copiedInputBoundary.executionCopiedInputBytesMaximum, 65991);
assert.deepEqual(manifest.copiedInputBoundary.maximumContext, { debateNumber: "07", chunkId: "chunk-004" });
...
assert.equal(manifest.costEstimate.contexts, 44);
...
assert.deepEqual(manifest.costEstimate.expectedParallelWallMinutes, [14, 32]);
assert.deepEqual(manifest.costEstimate.expectedAggregateModelMinutes, [43, 85]);
...
assert.equal(preparedByContext.size, 44);
for (const context of manifest.contexts) {
  ...
  assert(buildBatch12TokenCountedChunkLedger(chunkBytes).equals(tokenBytes));
...
assert.equal(manifest.nextAuthorizedAction, "prepare-and-freeze-batch-12-discovery-execution-activation-under-standing-authorization");

const validation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-discovery-execution-preparation-validation",
  protocolId: manifest.protocolId,
  status: "batch-12-discovery-execution-manifest-validation-passed-frozen-standing-authorization-active",
