...
const ACCEPTED_DEBATES = Object.freeze(["26", "190", "87"]);
...
const [originalPreparationBytes, originalActivationBytes, originalExecutionBytes,
  originalAnalysisBytes, debate87AnalysisBytes] =
  await Promise.all([
    ORIGINAL_PREPARATION, ORIGINAL_ACTIVATION, ORIGINAL_EXECUTION,
    ORIGINAL_ANALYSIS, DEBATE_87_ANALYSIS
...
assertV4(
  ...
  "original seven-context unattempted boundary changed"
);
...
assertV4(
  contexts.reduce((sum, context) => sum + context.moves, 0) === 141 &&
    contexts.reduce((sum, context) => sum + context.sections, 0) === 38 &&
    contexts.reduce((sum, context) => sum + context.audioVerifiedMoves, 0) === 8,
  "seven-context publication coverage changed"
);
...
const sourceScripts = [
  "scripts/lib/assessment-production-post-canary-batch-13-publication-resumption-1.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-13-publication-resumption-1.mjs",
  "scripts/activate-assessment-production-post-canary-batch-13-publication-resumption-1.mjs",
  "scripts/run-assessment-production-post-canary-batch-13-publication-resumption-1.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-13-publication-resumption-1.mjs",
...
const rampPhases = [
  { phase: "operational-one", maximumParallelContexts: 1,
    originalContextIndexes: [3], expansionRequiresAllValid: true },
  { phase: "ramp-two", maximumParallelContexts: 2,
    originalContextIndexes: [4, 5], expansionRequiresAllValid: true },
  { phase: "steady-two", maximumParallelContexts: 2,
    originalContextIndexes: [6, 7, 8, 9], expansionRequiresAllValid: false }
];
const manifest = {
  ...
  status: "frozen-seven-original-unattempted-batch-13-publication-contexts-prepared-not-activated",
  frozenAt,
  ...
  userAuthorization: {
    ...
    resolvedScope: "resume only the seven original publication contexts that remained unattempted after Debates 26 and 190 passed and Debate 87 timed out, then passed the authorized bounded and exceptional field-disjoint recovery sequence",
  ...
  acceptedPriorDebates: acceptedOutputs.map((item) => ({
    ...
    source: item.debateNumber === "87" ? "exceptional-third-level-atomic-shard-recovery" : "original-publication-first-attempt",
  ...
  nextAuthorizedAction: "commit-and-push-frozen-seven-context-resumption-then-activate"
};
if (shouldWrite) {
