...
const executionPath =
  `${POST_CANARY_BATCH_13_DISPUTE_ADJ_ROOT}/model-execution.json`;
...
assertV4(
  ...
  "Batch 13 adjudication execution is unavailable or crossed its boundary"
);
for (const [file, digest] of Object.entries(activation.sourceHashes)) {
  assertV4(
    sha256(await readFile(file)) === digest,
    `source hash mismatch during Batch 13 adjudication analysis: ${file}`
  );
}
...
for (const context of activation.contexts) {
  ...
  if (result.gateAcceptancePassed) {
    ...
    replay = validatePostCanaryBatch13DisputeAdjudicationOutput(
...
const semanticPass =
  ...
  valid.reduce((sum, context) => sum + context.candidateSelections, 0) ===
    541 &&
  valid.reduce((sum, context) => sum + context.disputedMoves, 0) === 191;
const timingPass =
  semanticPass &&
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-13-dispute-only-adjudication-analysis",
  ...
  status: passed
    ...
    : semanticPass
      ? timingPass
        ? scoreBlindPass
          ? "post-canary-batch-13-dispute-only-adjudication-gate-failed-isolation"
          : "post-canary-batch-13-dispute-only-adjudication-gate-failed-scoreblindness"
        : "post-canary-batch-13-dispute-only-adjudication-gate-failed-timing"
      : "post-canary-batch-13-dispute-only-adjudication-gate-failed-validation",
  ...
  nextAuthorizedAction: passed
    ...
    : "new-user-approval-required-after-batch-13-adjudication-failure-before-downstream-work"
};
if (shouldWrite) {
