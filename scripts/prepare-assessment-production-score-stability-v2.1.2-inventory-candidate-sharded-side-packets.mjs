...
const SCRIPT =
  "scripts/prepare-assessment-production-score-stability-v2.1.2-inventory-candidate-sharded-side-packets.mjs";
const TEST =
  "scripts/test-assessment-production-score-stability-v2.1.2-inventory-candidate-sharded-side-packets.mjs";
...
function makeSidePacket({
  ...
  return {
    ...
    stage: `${side}-candidate-evidence-selection`,
...
assertV4(
  planActivation.status ===
    ...
    planAnalysis.status ===
      "v2.1.2-candidate-census-plan-gate-passed-exact-side-packet-preparation-authorized" &&
    ...
    planAnalysis.nextAuthorizedAction ===
      "prepare-and-freeze-twenty-exact-v2.1.2-side-selector-packets-model-free-only",
...
for (const prepared of sourcePreparation.contexts) {
  ...
  for (const side of ["pro", "con"]) {
    ...
    const schema = buildCandidateShardedSideSelectionSchema({
      side,
      legacySchema,
      candidateTransport: fullTransport,
