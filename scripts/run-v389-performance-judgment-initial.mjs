...
const manifest = JSON.parse(await readFile(path.resolve(root, `${V389_PERFORMANCE_ROOT}/initial-execution-manifest.json`), "utf8"));
const codex = "/Applications/ChatGPT.app/Contents/Resources/codex";
const authSource = path.join(os.homedir(), ".codex", "auth.json");
const bytes = (relativePath) => readFile(path.resolve(root, relativePath));
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
assertV389(manifest.status === "frozen-six-context-authorized" && manifest.authorization.initialPerformanceModelExecution && !manifest.authorization.scoreDerivation && !manifest.stopRules.furtherAutomaticRetryAuthorized, "initial performance execution unauthorized");
...
for (const context of manifest.contexts) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-v389-performance-${context.debateNumber}-${context.pass}-`));
  const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), `slugfester-v389-home-performance-${context.debateNumber}-${context.pass}-`));
  ...
  try {
    ...
    process.stdout.write(`\n[v3.8.9-performance] starting ${manifest.model.label} Debate ${context.debateNumber} Pass ${context.pass}\n`);
    ...
    if (invocation.timedOut || invocation.code !== 0 || invocation.signal !== null) record = { ...base, status: invocation.timedOut ? "timed-out" : "transport-failed", outputWritten: false, deterministicValidationPassed: false, gateAcceptancePassed: false };
      ...
      const validation = await run(process.execPath, ["scripts/validate-v389-performance-judgment-output.mjs", context.output, context.packet, context.pass], { cwd: root, env: process.env });
