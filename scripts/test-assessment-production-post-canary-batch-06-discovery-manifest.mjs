...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-06/discovery";
...
assert.equal(manifest.status, "frozen-thirty-nine-post-canary-batch-06-discovery-contexts-prepared-not-authorized");
...
assert.equal(manifest.executionPolicy.copiedInputBytesMaximum, 65804);
assert.equal(preparation.totals.maximumCopiedInputBytes, 65804);
...
assert.equal(manifest.copiedInputBoundary.frozenObservedCopiedInputBytesMaximum, 65804);
assert.equal(manifest.copiedInputBoundary.executionCopiedInputBytesMaximum, 65804);
assert.deepEqual(manifest.copiedInputBoundary.maximumContext, { debateNumber: "97", chunkId: "chunk-002" });
...
for (const context of manifest.contexts) {
  ...
  assert(buildBatch06TokenCountedChunkLedger(chunkBytes).equals(tokenBytes));
...
assert.equal(manifest.nextAuthorizedAction, "prepare-and-freeze-batch-06-discovery-execution-activation-under-standing-authorization");

const validation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-discovery-execution-preparation-validation",
  protocolId: manifest.protocolId,
  status: "batch-06-discovery-execution-manifest-validation-passed-frozen-standing-authorization-active",
