...
const manifestPath = `${V384_COVERAGE_ROOT}/proposal-correction-01/execution-manifest.json`;
...
assert(manifest.status === "frozen-single-context-correction-authorized" && manifest.authorization.coverageProposalCorrectionModelExecution && manifest.authorization.freshCoverageProposalContexts === 1, "coverage correction is not authorized");
assert(!manifest.authorization.coverageReviewModelExecution && !manifest.authorization.numericalParticipantScoring && !manifest.authorization.assessmentProse, "downstream authorization boundary invalid");
...
for (const output of manifest.futureOutputPathsExcludedFromSourceHashes) {
  try { await access(path.resolve(root, output)); throw new Error(`correction future output already exists: ${output}`); }
...
if (dryRun) {
  console.log(JSON.stringify({
    ...
    debateNumber: manifest.proposalContext.debateNumber,
    sourceHashesValidated: Object.keys(manifest.sourceHashes).length,
    contextFiles: ["workflow.md", "rubric.md", "manual.md", "packet.json", "schema.json", "transcript.txt", "events.json"],
    priorProposalFilesInContext: 0,
    validOtherDebateOutputsInContext: 0,
    subscriptionAuthenticationAvailable: true,
    APIKeysWillBeRemoved: true,
    meteredApiCostUsd: 0,
...
try {
  ...
  process.stdout.write(`\n[v3.8.4-coverage-correction] starting ${manifest.model.label} Debate 161\n`);
  ...
  const base = {
    ...
    streamRecoveryLimit: manifest.executionPolicy.sameRequestStreamRecoveriesMaximum,
    spawnError: invocation.spawnError
  };
  if (invocation.timedOut || invocation.code !== 0) {
    result = { ...base, status: invocation.timedOut ? "timed-out" : base.preInferenceSchemaRejected ? "pre-inference-schema-rejected" : "transport-failed", outputWritten: false, deterministicValidationPassed: false };
  } else {
    ...
    const streamValid = recoveries <= manifest.executionPolicy.sameRequestStreamRecoveriesMaximum;
...
const execution = {
  schemaVersion: "3.8.4-full-coverage-proposal-correction-01-model-execution",
  protocolId: manifest.protocolId,
  stage: manifest.stage,
  startedAt,
  completedAt: new Date().toISOString(),
  contextsPlanned: 1,
  validOutputContexts: result.status === "completed-valid" ? 1 : 0,
  totalAttempts: 1,
  totalRetries: 0,
  sameRequestStreamRecoveries: result.sameRequestStreamRecoveries ?? 0,
  meteredApiCostUsd: 0,
  transcriptionCostUsd: 0,
  result
};
...
assert(execution.validOutputContexts === 1, "coverage correction did not produce one valid output; review packet construction remains blocked");
console.log(JSON.stringify({
  status: "v3.8.4-full-coverage-proposal-correction-recorded",
  debateNumber: "161",
  validOutputContexts: 1,
  totalAttempts: 1,
  totalRetries: 0,
