...
"valid Batch 17 rendering activation required");
...
for (const debateNumber of POST_CANARY_BATCH_17_RENDERING_ORDER) {
  ...
  packetRows.push({ row, packet: validatePostCanaryBatch17RenderingPacket(
...
assertV4(plans.length === 8, "eight Batch 17 rendering viewport plans required");
...
try {
  ...
  throw new Error(`Playwright CLI Batch 17 rendering execution failed: ${error.stdout || error.message}`);
}
let browserResult = JSON.parse(raw.trim());
if (typeof browserResult === "string") browserResult = JSON.parse(browserResult);
if (browserResult.status !== "passed" || browserResult.results?.length !== 8) {
  await Promise.all(plans.flatMap((plan) => [plan.collapsedScreenshot, plan.openScreenshot])
    .map((file) => rm(file, { force: true })));
  throw new Error(`Batch 17 rendering stopped on its first failing viewport: ${JSON.stringify(browserResult).slice(-5000)}`);
}
...
try {
  for (const result of browserResult.results) {
    ...
    const evidence = {
      schemaVersion: "1.0-assessment-production-post-canary-batch-17-rendering-evidence",
    ...
    validatePostCanaryBatch17RenderingEvidence({ packet,
      viewportName: result.viewportName,
...
console.log(JSON.stringify({ status: "four-debate-batch-17-rendering-browser-pass-complete",
  debates: 4, viewportResults: 8, screenshots: 16,
  requiredBooleanChecks: 176, runtimeFailures: 0,
  attempts: 8, retries: 0, timeoutExtensions: 0,
  directIncrementalCostUsd: 0,
  nextAuthorizedAction: "validate-and-finalize-batch-17-rendering-verification" }, null, 2));
