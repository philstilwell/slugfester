...
assert.equal(work.batchNumber, 10);
assert.equal(work.stagingOnly, true);
assert.equal(preparation.userAuthorization.workItemsAuthorized, 9);
assert.equal(
...
assert.equal(preparation.sources.length, 5);
assert.equal(preparation.totals.debates, 5);
assert.equal(preparation.totals.sourceVideoIds, 5);
assert.equal(preparation.totals.moves, 9);
assert.equal(work.moves.length, 9);
...
assert.equal(preparation.validation.exactAuthorizedMoveCount, 9);
assert.equal(
  preparation.validation.canonicalLocalTextAndMetadataChainsVerified,
  5
);
assert.equal(preparation.validation.repositoryRenderedLockedExcerpts, 9);
assert.equal(preparation.validation.repositoryRenderedTimestampWindows, 9);
assert.equal(preparation.validation.expectedSpeakersLocked, 9);
assert.equal(
