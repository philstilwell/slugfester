...
import {
  validatePostCanaryBatch13Scores
} from "./lib/assessment-production-post-canary-batch-13-score-gate.mjs";
import {
  buildPostCanaryBatch13PublicationPacket,
  buildPostCanaryBatch13PublicationSchema,
  postCanaryBatch13ReferenceCatalog,
  POST_CANARY_BATCH_13_PUBLICATION_DEBATES,
  POST_CANARY_BATCH_13_PUBLICATION_MODEL,
  POST_CANARY_BATCH_13_PUBLICATION_PROTOCOL_ID,
  POST_CANARY_BATCH_13_PUBLICATION_ROOT
} from "./lib/assessment-production-post-canary-batch-13-publication.mjs";
...
const ROOT = POST_CANARY_BATCH_13_PUBLICATION_ROOT;
...
const SELECTION =
  "docs/assessment-production/post-canary-continuation-v1/batch-13/selection.json";
const FINAL_LEDGER =
...
const FINAL_LEDGER_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-13/final-ledger/final-ledger-manifest.json";
const FINAL_LEDGER_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-13/final-ledger/analysis.json";
const SCORES =
  "docs/assessment-production/post-canary-continuation-v1/batch-13/score-pass/calculated-scores.json";
const SCORE_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-13/score-pass/score-pass-manifest.json";
const SCORE_PREPARATION =
  "docs/assessment-production/post-canary-continuation-v1/batch-13/score-pass/score-pass-preparation-manifest.json";
const SCORE_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-13/score-pass/analysis.json";
const SOURCE_PACKET_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-13/independent-judgments/source-packets";
...
const STATIC_SOURCE_FILES = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-13-score-gate.mjs",
  "scripts/lib/assessment-production-post-canary-batch-13-publication.mjs",
  "scripts/lib/assessment-production-post-canary-batch-13-publication-validation.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-13-publication.mjs",
  "scripts/test-assessment-production-post-canary-batch-13-publication-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-13-publication.mjs",
  "scripts/run-assessment-production-post-canary-batch-13-publication.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-13-publication.mjs"
];
...
assertV4(
  scoreAnalysis.status === "post-canary-batch-13-score-stability-gate-passed" &&
    scoreAnalysis.productionCanary === false &&
    scoreAnalysis.batchNumber === 13 &&
    ...
    scoreAnalysis.nextAuthorizedAction ===
      "standing-authorization-permits-batch-13-publication-packet-preparation",
  "the accepted Batch 13 score gate is unavailable or its stop boundary changed"
);
assertV4(
  scores.status === "post-canary-batch-13-single-score-pass-stability-gate-passed" &&
  ...
  "the locked Batch 13 score artifact is unavailable"
);
assertV4(
  scoreManifest.status ===
      "frozen-post-canary-batch-13-single-deterministic-score-pass-authorized" &&
    ...
    scorePreparation.status ===
      "frozen-post-canary-batch-13-single-deterministic-score-pass-prepared-not-activated",
  "the frozen Batch 13 single-pass score controls changed"
);
assertV4(
  ledger.status ===
      "passed-post-canary-batch-13-deterministic-final-ledger-assembly" &&
    ledger.debates?.length === 10 &&
    ledger.audit?.finalMoves === 199 &&
    ledger.audit?.audioVerifiedMoves === 8 &&
  ...
  "the locked Batch 13 final ledger changed"
);
assertV4(
  productionManifest.model?.label === POST_CANARY_BATCH_13_PUBLICATION_MODEL.label &&
    productionManifest.model?.slug === POST_CANARY_BATCH_13_PUBLICATION_MODEL.slug &&
    productionManifest.model?.reasoningEffort ===
      POST_CANARY_BATCH_13_PUBLICATION_MODEL.reasoningEffort &&
    productionManifest.model?.authentication ===
      POST_CANARY_BATCH_13_PUBLICATION_MODEL.authentication,
  "the frozen production model or authentication changed"
);
assertV4(
  selection.selected?.map((item) => item.debateNumber).join(",") ===
      POST_CANARY_BATCH_13_PUBLICATION_DEBATES.join(",") &&
    selection.selected.every((item) => item.speakerCount === 2) &&
    selection.authorization?.productionMutation === false,
  "the frozen Batch 13 selection changed"
);
assertV4(
  ledger.debates.map((debate) => debate.debateNumber).join(",") ===
    ...
    scores.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_13_PUBLICATION_DEBATES.join(","),
  "the Batch 13 ledger or score order changed"
);
assertV4(
...
const finalLedgerInputs = await loadPostCanaryBatch13FinalLedgerInputs();
validatePostCanaryBatch13FinalLedger(
  ledger,
...
const catalog = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-13-local-reference-catalog",
  references: postCanaryBatch13ReferenceCatalog()
};
...
for (const debateNumber of POST_CANARY_BATCH_13_PUBLICATION_DEBATES) {
  ...
  const packet = buildPostCanaryBatch13PublicationPacket({
    ledgerDebate,
    scoreDebate,
    sourcePacket,
  ...
  const schema = buildPostCanaryBatch13PublicationSchema(packet);
...
assertV4(
  contexts.reduce((sum, context) => sum + context.moves, 0) === 199,
  "Batch 13 publication move coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.sections, 0) === 53,
  "Batch 13 publication section coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.audioVerifiedMoves, 0) === 8,
  "Batch 13 audio-verification coverage changed"
);
assertV4(
  contexts.every(
    (context) =>
  ...
  "Batch 13 publication section or quote eligibility is invalid"
);
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-13-publication-execution-preparation-manifest",
  protocolId: POST_CANARY_BATCH_13_PUBLICATION_PROTOCOL_ID,
  status:
    "frozen-ten-post-canary-batch-13-score-locked-publication-contexts-prepared-not-activated",
  frozenAt,
  ...
  userAuthorization: {
    instruction:
      "The frozen Batch 13 standing authorization permits preparation, activation, and execution of exactly ten score-locked publication-reconstruction contexts while every gate passes, with direct incremental cost capped at $0.",
  ...
  model: structuredClone(POST_CANARY_BATCH_13_PUBLICATION_MODEL),
  costEstimate: {
  ...
  acceptanceContract: {
    ...
    movesAuthoredRequired: 199,
    critiquesRequired: 199,
  ...
  nextAuthorizedAction:
    "standing-authorization-permits-activation-and-execution-of-the-ten-frozen-batch-13-publication-contexts"
};
