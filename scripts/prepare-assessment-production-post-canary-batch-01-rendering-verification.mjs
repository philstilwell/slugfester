...
import {
  ...
  POST_CANARY_BATCH_01_RENDERING_MODEL,
  ...
  buildPostCanaryBatch01RenderingPacket
...
const preparationPath = `${POST_CANARY_BATCH_01_RENDERING_ROOT}/preparation-manifest.json`;
...
const FINALIZATION_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-01/publication-finalization";
...
const PACKETS_ROOT = `${POST_CANARY_BATCH_01_RENDERING_ROOT}/packets`;
const EVIDENCE_ROOT = `${POST_CANARY_BATCH_01_RENDERING_ROOT}/evidence`;
const ACTIVATION = `${POST_CANARY_BATCH_01_RENDERING_ROOT}/execution-activation.json`;
const EXECUTION = `${POST_CANARY_BATCH_01_RENDERING_ROOT}/execution.json`;
const ANALYSIS = `${POST_CANARY_BATCH_01_RENDERING_ROOT}/analysis.json`;
const RENDERING_AUDIT = `${POST_CANARY_BATCH_01_RENDERING_ROOT}/rendering-audit.json`;
const IMAGE_ANALYZER = POST_CANARY_BATCH_01_RENDERING_IMAGE_CONTRACT.analyzerPath;
...
if (shouldWrite) {
  assertV4(
    ...
    "Batch 1 rendering preparation already exists; freeze is immutable"
  );
}
...
assertV4(
  finalizationPreparation.status ===
      "frozen-post-canary-batch-01-publication-finalization-prepared-not-authorized" &&
    finalizationActivation.status ===
      "frozen-post-canary-batch-01-publication-finalization-authorized" &&
    finalizationExecution.status ===
      "ten-debate-batch-01-publication-finalization-passed" &&
    ...
    compatibility.status === "batch-01-production-compatibility-boundary-recorded" &&
    ...
    canonicalJson(finalizationAudit.explicitOrder) ===
      canonicalJson(POST_CANARY_BATCH_01_RENDERING_ORDER) &&
    finalizationAudit.rows.length === 10 &&
    finalizationAudit.totals.debates === 10 &&
    finalizationAudit.totals.sections === 50 &&
    finalizationAudit.totals.moves === 177 &&
  ...
  "passing Batch 1 publication finalization required"
);
assertV4(
...
for (const debateNumber of POST_CANARY_BATCH_01_RENDERING_ORDER) {
  ...
  const packet = buildPostCanaryBatch01RenderingPacket({
    auditRow,
...
const directInputPaths = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-01-rendering-verification.mjs",
  "scripts/lib/assessment-production-post-canary-batch-01-rendering-browser-runner.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-01-rendering-verification.mjs",
  "scripts/activate-assessment-production-post-canary-batch-01-rendering-verification.mjs",
  "scripts/finalize-assessment-production-post-canary-batch-01-rendering-verification.mjs",
  "scripts/test-assessment-production-post-canary-batch-01-rendering-verification-preparation.mjs",
  "scripts/validate-assessment-production-post-canary-batch-01-rendering-evidence.mjs",
...
for (const file of [...futureOutputPathsExcludedFromSourceHashes, EVIDENCE_ROOT]) {
  assertV4(!(await exists(file)), `future Batch 1 rendering output exists: ${file}`);
}

const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-01-rendering-verification-preparation",
  protocolId: POST_CANARY_BATCH_01_RENDERING_PROTOCOL_ID,
  status: "frozen-post-canary-batch-01-rendering-verification-prepared-not-authorized",
  frozenAt,
  checkpointCommit,
  ...
  stagingOnly: true,
  userAuthorization: {
    instruction: "I approve: Batch 1 rendering-verification preparation",
    scopeInterpretation:
      "Prepare, validate, freeze, commit, and push the Batch 1 rendering-verification plan without opening a candidate page or taking screenshots.",
  ...
  explicitOrder: [...POST_CANARY_BATCH_01_RENDERING_ORDER],
  viewports: structuredClone(POST_CANARY_BATCH_01_RENDERING_VIEWPORTS),
  ...
  browserPlan: {
    ...
    localServer: {
      command: [
        ...
        "-m",
        "http.server",
        String(POST_CANARY_BATCH_01_RENDERING_PORT),
        "--bind",
        "127.0.0.1"
      ],
      workingDirectory: ".",
      port: POST_CANARY_BATCH_01_RENDERING_PORT,
      baseUrl: `http://127.0.0.1:${POST_CANARY_BATCH_01_RENDERING_PORT}`,
    ...
    imageContract: structuredClone(POST_CANARY_BATCH_01_RENDERING_IMAGE_CONTRACT),
