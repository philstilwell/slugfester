...
assert.equal(activation.batchNumber, 1);
...
assert.equal(activation.transport.maximumCopiedInputBytes, 106385);
...
if (
  execution.status ===
  "twenty-post-canary-batch-01-independent-judgment-contexts-passed"
) {
  ...
  assert.equal(
    execution.status,
    "post-canary-batch-01-independent-judgment-gate-complete-with-failure"
  );
  assert(
...
if (analysis.acceptance.passed) {
  ...
  assert.equal(analysis.totals.uniqueMoves, 177);
  assert.equal(analysis.totals.movesJudgedAcrossPasses, 354);
  assert.equal(
  ...
  assert.equal(
    analysis.status,
    "post-canary-batch-01-independent-judgment-gate-failed-analysis-only"
  );
}
console.log(
  JSON.stringify(
