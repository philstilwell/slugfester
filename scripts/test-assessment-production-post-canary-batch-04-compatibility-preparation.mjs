...
const preparationPath =
  `${POST_CANARY_BATCH_04_COMPATIBILITY_ROOT}/preparation-manifest.json`;
const analysisPath =
  `${POST_CANARY_BATCH_04_COMPATIBILITY_ROOT}/analysis.json`;
...
assert.equal(
  ...
  "batch-04-site-ledger-adapter-and-validator-route"
);
assert.equal(
...
assert.equal(preparation.totals.moves, 203);
assert.equal(preparation.totals.oneSidedDisplayRows, 7);
...
assert.equal(
  await exists(`${POST_CANARY_BATCH_04_COMPATIBILITY_ROOT}/execution.json`),
  false
);
assert.equal(
  await exists(
...
assert.deepEqual(packetNumbers, POST_CANARY_BATCH_04_COMPATIBILITY_ORDER);
for (const packetRecord of preparation.artifacts.packets) {
  ...
  assert.equal(packet.batchNumber, 4);
  ...
  const positive = validatePostCanaryBatch04SiteLedgerAdapter({
