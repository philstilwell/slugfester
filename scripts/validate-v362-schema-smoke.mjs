...
const root = process.cwd(), gateRoot = "docs/calibration/v3.6.2/schema-smoke", read = (file) => readFile(path.resolve(root, file), "utf8");
const manifestText = await read(`${gateRoot}/gate-manifest.json`), manifest = JSON.parse(manifestText);
assert(manifest.schemaVersion === "3.6.2-schema-smoke-manifest" && manifest.status === "frozen-before-remote-smoke", "manifest identity invalid");
assert(manifest.calibrationOnly && !manifest.independentModelAccuracyTest, "smoke scope invalid");
...
assert(sha256(await read(manifest.priorV361.manifestPath)) === manifest.priorV361.manifestSha256, "v3.6.1 manifest hash mismatch");
assert(sha256(await read(manifest.priorV361.analysisPath)) === manifest.priorV361.analysisSha256, "v3.6.1 analysis hash mismatch");
const dryText = await read(manifest.dryFixture.path), dry = JSON.parse(dryText);
assert(sha256(dryText) === manifest.dryFixture.sha256 && dry.passed && dry.modelContextsExecuted === 0, "dry fixture invalid");

const executionText = await read(manifest.executionResultPath), execution = JSON.parse(executionText);
assert(execution.schemaVersion === "3.6.2-schema-smoke-execution" && execution.gateId === manifest.gateId, "execution identity invalid");
assert(execution.authentication.includes("ChatGPT subscription") && execution.authentication.includes("API keys removed"), "execution authentication invalid");
assert(execution.contextsPlanned === 4 && execution.totalAttempts === 4 && execution.totalRetries === 0 && execution.meteredApiCostUsd === 0, "execution count, retry, or cost invalid");
assert(execution.results.length === 4 && new Set(execution.results.map((item) => item.family)).size === 4, "execution family coverage invalid");
for (const result of execution.results) {
  ...
  if (result.status === "completed-valid") {
    ...
    execFileSync(process.execPath, ["scripts/validate-v362-smoke-output.mjs", manifest.outputs[result.family], manifest.families[result.family].packet, result.family], { cwd: root, stdio: "ignore" });
  }
}
assert(execution.contextsCompleted === execution.results.filter((item) => item.commandExitCode === 0).length, "contextsCompleted count invalid");
...
assert(execution.scoringFieldCount === execution.results.reduce((sum, item) => sum + item.scoringFieldCount, 0), "scoring field count invalid");

const analysisText = await read(manifest.analysisPath), analysis = JSON.parse(analysisText);
assert(analysis.schemaVersion === "3.6.2-schema-smoke-analysis" && analysis.sources.manifestSha256 === sha256(manifestText) && analysis.sources.executionSha256 === sha256(executionText), "analysis identity or provenance invalid");
for (const [family, digest] of Object.entries(analysis.sources.outputSha256)) {
  if (digest !== null) assert(digest === sha256(await read(manifest.outputs[family])), `${family}: analysis output hash mismatch`);
}
const expectedGates = {
...
assert(analysis.results.semanticMonitoringAssertions === analysis.semanticMonitoring.length && analysis.results.semanticMonitoringMatches === analysis.semanticMonitoring.filter((item) => item.matched).length, "semantic monitoring counts invalid");
assert(analysis.decision.retiredSemanticCardTestPreregistrationAuthorized === analysis.passed, "next-step authorization mismatch");
