...
assertV4(
  activation.status ===
      "frozen-twenty-v2.2.2-side-selector-contexts-authorized" &&
  ...
  "v2.2.2 side-selector execution is unauthorized"
);
...
async function executeContext(context, contextIndex) {
  ...
  const sideAsset = source?.sideAssets.find(
  ...
  assertV4(
    prepared &&
      ...
      context.output.endsWith(
        `/side-selections/debate-${context.debateNumber}-${context.side}.json`
      ),
  ...
  const sourceDirectory = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-v222-side-${context.debateNumber}-${context.side}-`
    )
  );
  const isolatedCodexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-v222-side-home-${context.debateNumber}-${context.side}-`
    )
  );
  ...
  try {
    const roleTarget = (role) => {
      ...
      if (role === "chronology-fallback-inventory-guide") {
        return "chronology-fallback-inventory-guide.md";
      }
    ...
    const prompt = `Read inventory-source-packet.json, immutable-plan.json, side-candidate-evidence.json, chronology-fallback-inventory-guide.md, inventory-manual.md, and side-schema.json completely; read nothing else. Act only as the isolated score-blind ${context.side}-side candidate-evidence selector for fresh v2.2.2 staging-validation Debate ${context.debateNumber}. Review every candidate key in the delivered ${context.side}-only transport and map each key to either null or exactly one schema-conforming nomination. Preserve the immutable routes and sections. Every nomination must author both the preferred move kind and a source-grounded constructive orphan fallback with a substantive rationale; the repository applies that fallback only if a retained preferred reply has no earlier retained opposing move. Nominate at least one candidate for every section, using essential, strong, or supporting priority; the repository will retain at most two per section by frozen deterministic reduction. The other side, its output, failed v2.2.1 plan outputs, planner execution metadata, ratings, response topology, scores, winners, legacy assessments, other debates, other judgments, tags, Overall Commentary, AI Extension, and publication prose are unavailable and prohibited. Return exactly one side-schema-conforming JSON object.`;
    process.stdout.write(
      `[v2.2.2-side-selector] starting ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber} ${context.side}\n`
    );
    ...
    if (
      ...
      try {
        const [plan, legacySchema, candidateTransport, sideTransport, candidateCensus] =
          await Promise.all([
            readFile(prepared.composedPlanOutput, "utf8").then(JSON.parse),
  ...
  process.stdout.write(
    `[v2.2.2-side-selector] Debate ${context.debateNumber} ${context.side} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-score-stability-v2.2.2-side-selector-model-execution",
  protocolId: activation.protocolId,
  status:
    validContexts === preparation.contexts.length
      ? "twenty-v2.2.2-side-selector-contexts-passed"
      : "v2.2.2-side-selector-gate-complete-with-failure",
  ...
  nextRequiredAction:
    validContexts === preparation.contexts.length
      ? "analyze-compile-and-lock-v2.2.2-inventories-model-free-only"
      : "stop-preserve-failed-v2.2.2-side-selector-gate",
};
