...
assertV4(
  ...
  "eight-context resumption execution boundary changed"
);
...
const eightValid = replayed.filter((item) => item.gateAcceptancePassed);
const resumptionPassed =
  execution.status === "eight-original-unattempted-batch-12-publication-contexts-passed" &&
  execution.validContexts === 8 && execution.invalidContexts === 0 &&
  ...
  eightValid.length === 8;
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-12-publication-eight-original-unattempted-context-resumption-analysis",
  protocolId: activation.protocolId,
  status: resumptionPassed
    ? "eight-original-unattempted-batch-12-publication-contexts-passed"
    : "eight-context-publication-resumption-failed",
  ...
  totals: {
    debates: 8,
    lockedMoves: 157,
    critiques: eightValid.reduce((sum, item) => sum + item.validation.critiques, 0),
    exactSourceQuotes: eightValid.reduce((sum, item) => sum + item.validation.quoteExactSourceMatches, 0),
    overallCommentarySides: eightValid.reduce((sum, item) => sum + item.validation.overallCommentarySides, 0),
    aiExtensionSides: eightValid.reduce((sum, item) => sum + item.validation.aiExtensionSides, 0),
...
if (resumptionPassed) {
  const allDebates = ["152", "28", ...DEBATES];
  ...
  for (const debateNumber of allDebates) {
    ...
    const source = debateNumber === "152"
      ? "accepted-two-shard-three-field-level-1-repair"
      : debateNumber === "28"
        ? "accepted-one-shard-one-field-level-2-repair"
        : "original-first-attempt-from-eight-context-resumption";
    completeReplay.push({
      debateNumber,
      output: outputPath,
  ...
  completeCohort = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-12-complete-publication-cohort-analysis-after-all-recoveries",
    protocolId: activation.protocolId,
    status: "post-canary-batch-12-complete-ten-debate-publication-output-gate-passed-after-all-recoveries",
    productionCanary: false,
    batchNumber: 12,
    stagingOnly: true,
    debates: allDebates,
    ...
    totals: {
      ...
      debate28FieldDisjointRepairContexts: 1,
      debate28CorrectedFields: 1,
      totalCodexInvocations: 13,
      invocationsProducingNoModelOutput: 0,
      retries: 0,
      timeoutExtensions: 0,
      modelAuthoredScores: 0,
    ...
    integrity: {
      ...
      eightResumedContextsWereOriginalFirstAttempts: true,
      rejectedDebate152CritiquesRetained: false,
      rejectedDebate28CritiqueRetained: false,
