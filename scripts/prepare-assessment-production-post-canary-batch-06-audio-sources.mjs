...
const localRoot =
  "output/transcribe/assessment-production-post-canary-batch-06-audio-verification";
...
const TOOL_SOURCES = [
  ...
  "scripts/prepare-assessment-production-post-canary-batch-06-audio-work-items.mjs",
  "scripts/test-assessment-production-post-canary-batch-06-audio-work-items.mjs",
  "scripts/lib/assessment-production-post-canary-batch-06-audio-work-items.mjs",
...
assertV4(
  workPreparation.status ===
    "prepared-and-frozen-two-post-canary-batch-06-local-audio-source-work-items-standing-authorization-active-for-audio-preparation" &&
    workPreparation.authorization.localAudioSourcePreparation &&
    workPreparation.nextAuthorizedAction ===
      "prepare-local-batch-06-source-audio-and-two-frozen-clips-under-standing-authorization",
  "Batch 6 local audio-source preparation is not authorized"
);
assertV4(
  work.status ===
      "prepared-two-post-canary-batch-06-local-audio-source-work-items-standing-authorization-active-for-audio-preparation" &&
    work.moves.length === 2 &&
    work.authorization.localAudioSourcePreparation,
  "Batch 6 audio work-item population changed"
);
assertV4(
  sha256(workBytes) === workPreparation.workArtifact.sha256,
  "Batch 6 audio work-item hash changed"
);
assertV4(
  JSON.stringify(
    work.moves.map((move) => `${move.debateNumber}:${move.moveId}`).sort()
  ) === JSON.stringify([...EXPECTED_AUDIO].sort()),
  "Batch 6 exact two-clip population changed"
);
...
assertV4(
  grouped.size === 1,
  "expected exactly one Batch 6 audio source"
);
...
if (!shouldWrite) {
  console.log(
    JSON.stringify(
      {
        status: "preview-one-post-canary-batch-06-local-audio-source-two-clips",
...
const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-06-audio-source-preparation",
