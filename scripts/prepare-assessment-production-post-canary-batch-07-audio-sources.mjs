...
const localRoot =
  "output/transcribe/assessment-production-post-canary-batch-07-audio-verification";
...
const WORK_ITEM_TEST_PREIMAGE_RULE =
  `${planRoot}/audio-work-item-preparation-recovery-2/preimage-reconstruction-rule.json`;
const TOOL_SOURCES = [
...
assertV4(
  ...
  "Batch 7 local audio-source preparation is not authorized"
);
assertV4(
  work.status ===
...
assertV4(
  grouped.size === 3,
  "expected exactly three Batch 7 audio sources"
);
...
if (!shouldWrite) {
  console.log(
    JSON.stringify(
      {
        status: "preview-three-post-canary-batch-07-local-audio-sources-five-clips",
...
const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-07-audio-source-preparation",
