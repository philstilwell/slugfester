...
if (shouldWrite) {
  ...
  const synthetic = { ...oldSynthetic, schemaVersion: V41_PACKET_VERSION, protocolId: V41_PROTOCOL_ID, debateId: "v415-bounded-schema-preflight", modelInputBoundary: { ...oldSynthetic.modelInputBoundary, highEffortReferenceOutputsUnavailable: true, boundedMoveMinimum: 8, boundedMoveMaximum: 24 } };
  await writeFile(path.resolve(root, `${V41_LEAN_ROOT}/schema-preflight/packet.json`), `${JSON.stringify(synthetic, null, 2)}\n`);
}

const preparation = {
    schemaVersion: "4.1.5-bounded-retired-preparation",
  protocolId: V41_PROTOCOL_ID,
