...
assertV4(
  activation.status ===
    ...
    activation.discoveryProtocolId === V211_DISCOVERY_PROTOCOL_ID &&
  ...
  "v2.1.1 discovery execution is unauthorized"
);
...
assertV4(
  preparationManifest.contexts.length === 42 &&
...
async function executeContext(context, contextIndex) {
  ...
  assertV4(
    buildV211TokenCountedChunkLedger(validationChunkBytes).equals(
      modelLedgerBytes
    ) &&
  ...
  const sourceDirectory = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-v211-discovery-${context.debateNumber}-${context.chunkId}-`
    )
  );
  const isolatedCodexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-v211-discovery-home-${context.debateNumber}-${context.chunkId}-`
    )
  );
  ...
  try {
    ...
    const prompt = `Read manual.md, packet.json, schema.json, and every line of token-counted-ledger.jsonl; read nothing else. Act only as the isolated repository-materialized score-blind source-discovery reviewer for development-validation Debate ${context.debateNumber}, ${context.chunkId}. The owned core is events ${context.coreStartEvent} through ${context.coreEndEvent}; boundary context is events ${context.contextStartEvent} through ${context.contextEndEvent}. Review the entire context. Follow the predecessor-chunk ownership rule. Emit zero to ten chronological load-bearing candidates whose start event lies inside the core. Emit sourceWindow.startEvent and sourceWindow.requestedLexicalTokens only; never emit an end event, target ID, moveKind, evidence text, rating, score, section, winner, tag, Overall Commentary, AI Extension, policy analysis, or assessment/publication prose. Return exactly one schema-conforming JSON object.`;
    process.stdout.write(
      `[v2.1.1-discovery] starting ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber} ${context.chunkId}\n`
    );
    ...
    if (
      ...
      const validation = await runChild(
        process.execPath,
        [
          "scripts/validate-v211-discovery.mjs",
          context.rawOutput,
      ...
      if (valid) {
        assertV4(
          validationSummary.repositoryMaterializedSourceWindows === true &&
            validationSummary.modelAuthoredEndEvents === false,
  ...
  process.stdout.write(
    `[v2.1.1-discovery] Debate ${context.debateNumber} ${context.chunkId} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-score-stability-v2.1.1-fresh-validation-discovery-model-execution",
  ...
  authentication: "ChatGPT subscription",
  scoreBlind: true,
  repositoryMaterializedSourceWindows: results.every(
    (result) =>
      !result.accepted ||
      result.validationSummary?.repositoryMaterializedSourceWindows === true
  ),
  ...
  JSON.stringify(
    {
      ...
      maximumParallelContextsObserved,
      repositoryMaterializedSourceWindows:
        execution.repositoryMaterializedSourceWindows,
      modelAuthoredEndEvents: false,
      retries: 0,
      timeoutExtensions: 0,
