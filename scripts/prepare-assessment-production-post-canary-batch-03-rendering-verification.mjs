...
import {
  POST_CANARY_BATCH_03_RENDERING_IMAGE_CONTRACT,
  POST_CANARY_BATCH_03_RENDERING_MODEL,
  POST_CANARY_BATCH_03_RENDERING_ORDER,
  POST_CANARY_BATCH_03_RENDERING_PORT,
  POST_CANARY_BATCH_03_RENDERING_PROTOCOL_ID,
  POST_CANARY_BATCH_03_RENDERING_REQUIRED_BOOLEAN_CHECKS,
  POST_CANARY_BATCH_03_RENDERING_ROOT,
  POST_CANARY_BATCH_03_RENDERING_VIEWPORTS,
  buildPostCanaryBatch03RenderingPacket
...
const preparationPath = `${POST_CANARY_BATCH_03_RENDERING_ROOT}/preparation-manifest.json`;
...
const PACKETS_ROOT = `${POST_CANARY_BATCH_03_RENDERING_ROOT}/packets`;
const EVIDENCE_ROOT = `${POST_CANARY_BATCH_03_RENDERING_ROOT}/evidence`;
const ACTIVATION = `${POST_CANARY_BATCH_03_RENDERING_ROOT}/execution-activation.json`;
const EXECUTION = `${POST_CANARY_BATCH_03_RENDERING_ROOT}/execution.json`;
const ANALYSIS = `${POST_CANARY_BATCH_03_RENDERING_ROOT}/analysis.json`;
const RENDERING_AUDIT = `${POST_CANARY_BATCH_03_RENDERING_ROOT}/rendering-audit.json`;
const IMAGE_ANALYZER = POST_CANARY_BATCH_03_RENDERING_IMAGE_CONTRACT.analyzerPath;
...
if (shouldWrite) {
  assertV4(
    ...
    "Batch 3 rendering preparation already exists; freeze is immutable"
  );
}
...
assertV4(
  finalizationPreparation.status ===
      "frozen-post-canary-batch-03-publication-finalization-prepared-not-authorized" &&
    finalizationActivation.status ===
      "frozen-post-canary-batch-03-publication-finalization-authorized" &&
    finalizationExecution.status ===
      "ten-debate-batch-03-publication-finalization-passed" &&
    ...
    compatibility.status === "batch-03-production-compatibility-boundary-recorded" &&
    ...
    canonicalJson(finalizationAudit.explicitOrder) ===
      canonicalJson(POST_CANARY_BATCH_03_RENDERING_ORDER) &&
    finalizationAudit.rows.length === 10 &&
    finalizationAudit.totals.debates === 10 &&
    finalizationAudit.totals.sections === 53 &&
    finalizationAudit.totals.moves === 200 &&
  ...
  "passing Batch 3 publication finalization required"
);
assertV4(
...
for (const debateNumber of POST_CANARY_BATCH_03_RENDERING_ORDER) {
  ...
  const packet = buildPostCanaryBatch03RenderingPacket({
    auditRow,
...
const directInputPaths = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-03-publication-finalization.mjs",
  "scripts/lib/assessment-production-post-canary-batch-03-standing-authorization.mjs",
  "scripts/lib/assessment-production-post-canary-batch-03-rendering-verification.mjs",
  "scripts/lib/assessment-production-post-canary-batch-03-rendering-browser-runner.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-03-rendering-verification.mjs",
  "scripts/activate-assessment-production-post-canary-batch-03-rendering-verification.mjs",
  "scripts/finalize-assessment-production-post-canary-batch-03-rendering-verification.mjs",
  "scripts/test-assessment-production-post-canary-batch-03-rendering-verification-preparation.mjs",
  "scripts/validate-assessment-production-post-canary-batch-03-rendering-evidence.mjs",
...
assertV4(
  sourceHashes[IMAGE_ANALYZER] === undefined ||
    sourceHashes[IMAGE_ANALYZER] ===
      POST_CANARY_BATCH_03_RENDERING_IMAGE_CONTRACT.analyzerSha256,
...
for (const file of [...futureOutputPathsExcludedFromSourceHashes, EVIDENCE_ROOT]) {
  assertV4(!(await exists(file)), `future Batch 3 rendering output exists: ${file}`);
}

const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-rendering-verification-preparation",
  protocolId: POST_CANARY_BATCH_03_RENDERING_PROTOCOL_ID,
  status: "frozen-post-canary-batch-03-rendering-verification-prepared-not-authorized",
  frozenAt,
  checkpointCommit,
  ...
  userAuthorization: {
    ...
    scopeInterpretation:
      "Prepare, validate, freeze, commit, and push the Batch 3 rendering-verification plan without opening a candidate page or taking screenshots.",
  ...
  explicitOrder: [...POST_CANARY_BATCH_03_RENDERING_ORDER],
  viewports: structuredClone(POST_CANARY_BATCH_03_RENDERING_VIEWPORTS),
  ...
  browserPlan: {
    ...
    localServer: {
      command: [
        ...
        "-m",
        "http.server",
        String(POST_CANARY_BATCH_03_RENDERING_PORT),
        "--bind",
        "127.0.0.1"
      ],
      workingDirectory: ".",
      port: POST_CANARY_BATCH_03_RENDERING_PORT,
      baseUrl: `http://127.0.0.1:${POST_CANARY_BATCH_03_RENDERING_PORT}`,
    ...
    imageContract: structuredClone(POST_CANARY_BATCH_03_RENDERING_IMAGE_CONTRACT),
