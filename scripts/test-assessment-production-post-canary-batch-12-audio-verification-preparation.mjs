...
assert.deepEqual(preparation.scope.debates, ["152", "07", "15"]);
...
assert.equal(preparation.costEstimate.clipSeconds, 470.241);
assert.equal(preparation.costEstimate.clipMinutes, 7.8374);
assert.equal(preparation.costEstimate.durationOnlyPlanningEstimateUsd, 0.0470241);
assert.equal(preparation.costEstimate.durationOnlyEstimateAuthorizedByUser, true);
assert.equal(preparation.costEstimate.usageDerivedPlanningEstimateUsd, 0.171415);
assert.equal(preparation.costEstimate.primaryExpectedFutureExecutionCostUsd, 0.171415);
...
assert.deepEqual(preparation.costEstimate.projectedUsage, {
  audioInputTokens: 8762,
  textInputTokens: 456,
  outputTokens: 14837,
  totalInputTokens: 9218
});
...
assert.equal(
  ...
  "activate-and-execute-four-batch-12-paid-audio-verification-calls-under-standing-authorization"
);
