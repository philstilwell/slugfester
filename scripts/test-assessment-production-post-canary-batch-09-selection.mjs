...
assert.equal(selection.status, "ninth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed");
assert.equal(analysis.status, "ninth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization");
assert.equal(analysis.selection.sha256, sha256(selectionBytes));
assert.equal(selection.batchNumber, 9);
assert.equal(selection.productionSequenceOrdinal, 10);
...
assert.equal(selection.eligibility.previouslyPublishedAndRemovedCount, 80);
assert.equal(selection.eligibility.remainingBeforeBatchCount, 84);
assert.equal(selection.eligibility.selectedCount, 10);
assert.equal(selection.eligibility.remainingUnselectedCount, 74);
...
assert.equal(priorCompletionExecution.status, "passed-complete-batch-08-production-transaction-after-dependent-pilot-analysis-correction");
...
assert.equal(priorCompletionExecution.totals.recursiveCorrections, 0);
...
assert.equal(priorCompletionAnalysis.status, "batch-08-production-publication-accepted-after-dependent-pilot-analysis-correction");
...
assert.equal(sha256(serializedJson(ranked.slice(80))), selection.eligibility.remainingBeforeBatchRankedSha256);
assert.deepEqual(priorSelection.selected.map((item) => item.debateNumber), ranked.slice(70, 80).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.debateNumber), ranked.slice(80, 90).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.rankOrdinal), [81, 82, 83, 84, 85, 86, 87, 88, 89, 90]);
assert.deepEqual(selection.selected.map((item) => item.rankSha256), ranked.slice(80, 90).map((item) => item.rankSha256));
...
assert.deepEqual(selection.deterministicOrdering.selectedRankOrdinals, [81, 90]);
...
assert.equal(selection.nextAuthorizedAction, "prepare-batch-09-standing-authorization-and-source-packets-under-user-authorization");

console.log(JSON.stringify({ status: "passed", selectedDebates: selection.selected.map((item) => item.debateNumber), selectedRankOrdinals: [81, 90], sourceFiles: 30, replacements: 0, sourcePacketsPrepared: 0, modelContexts: 0, paidServiceCalls: 0, directCostUsd: 0, nextAuthorizedAction: selection.nextAuthorizedAction }, null, 2));
