...
import { POST_CANARY_BATCH_07_RESUMPTION_2_REPAIR_PROTOCOL_ID,
  POST_CANARY_BATCH_07_RESUMPTION_2_REPAIR_ROOT } from
...
assertV4(p.protocolId === POST_CANARY_BATCH_07_RESUMPTION_2_REPAIR_PROTOCOL_ID &&
  p.status === "frozen-twelve-context-batch-07-publication-resumption-2-repair-prepared-not-activated" &&
  p.contexts?.length === 12 &&
  canonicalJson(p.contexts.map((row) => row.debateNumber)) === canonicalJson(
    ["100", "100", "100", "100", "78", "78", "78", "78", "78", "78", "78", "78"]) &&
  canonicalJson(p.contexts.map((row) => row.writableFieldCount)) === canonicalJson(
...
"the Batch 7 Debate 100/78 repair is not prepared");
...
const activation = { schemaVersion:
    "1.0-assessment-production-post-canary-batch-07-publication-resumption-2-repair-activation",
  protocolId: p.protocolId,
  status: "frozen-twelve-context-batch-07-publication-resumption-2-repair-authorized",
  ...
  userAuthorization: p.userAuthorization, standingAuthorization: p.standingAuthorization,
  ...
  authorization: { repairModelExecution: true, deterministicOutputValidation: true,
    deterministicMergeAndValidation: true, fiveContextResumption: true,
  ...
  nextAuthorizedAction: "execute-exactly-twelve-frozen-batch-07-publication-repair-contexts" };
if (shouldWrite) await writeFile(path.resolve(ACTIVATION), `${JSON.stringify(activation, null, 2)}\n`);
console.log(JSON.stringify({ status: activation.status, contextsAuthorized: 12,
  debatesAuthorized: { "100": 4, "78": 8 }, writableFieldsAuthorized: 22,
