...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), `batch-03-publication-resumption-${context.debateNumber}-`)
  );
  const codexHome = await mkdtemp(
    path.join(os.tmpdir(), `batch-03-publication-resumption-home-${context.debateNumber}-`)
  );
  ...
  try {
    ...
    process.stdout.write(
      `[batch-03-publication-resumption] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
    ...
    if (
      ...
      const validationRecord = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-03-publication-resumption-validation",
      ...
      const provenanceRecord = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-03-publication-resumption-provenance",
        protocolId: activation.protocolId,
        contextIndex: context.contextIndex,
        originalContextIndex: context.originalContextIndex,
        debateNumber: context.debateNumber,
  ...
  process.stdout.write(
    `[batch-03-publication-resumption] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-publication-resumption-model-execution",
  protocolId: activation.protocolId,
  status: passed
  ...
  participantJudgmentWasScoreBlind: true,
  ownDebateScoresImmutable: true,
  acceptedDebate124RepairContexts: 2,
  acceptedDebate124RepairedFields: 3,
  meteredApiCostUsd: 0,
  paidServiceCallsThisStage: 0,
  modelAuthoredScores: 0,
  scorePassesExecutedThisStage: 0,
