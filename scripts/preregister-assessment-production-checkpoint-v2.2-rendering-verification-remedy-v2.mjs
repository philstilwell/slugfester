...
assertV4(
  preparation.protocolId === CHECKPOINT_V22_RENDERING_REMEDY_V2_PROTOCOL_ID &&
    preparation.status ===
      "second-replacement-rendering-verification-plan-prepared-and-frozen" &&
    preparation.authorization.remedyV2PlanPreparation === true &&
  ...
  "frozen remedy-v2 preparation required"
);
execFileSync(
  process.execPath,
  [
...
for (const [file, digest] of Object.entries(preparation.sourceHashes)) {
  assertV4(
    ...
    `source hash mismatch before remedy-v2 activation: ${file}`
  );
}
for (const row of preparation.packets) {
  ...
  assertV4(
    ...
    `${row.debateNumber}: remedy-v2 packet hash mismatch`
  );
  validateCheckpointV22RenderingRemedyV2Packet(JSON.parse(bytes));
}
...
const activation = {
  schemaVersion:
    "1.0-production-checkpoint-v2.2-rendering-remedy-v2-execution-activation",
  ...
  authorizedAt,
  activationCommit: execFileSync("git", ["rev-parse", "HEAD"], {
    encoding: "utf8"
  }).trim(),
  ...
  nextAuthorizedAction:
    "single-remedy-v2-rendering-execution-under-frozen-stop-rules"
};

if (shouldWrite) {
  assertV4(!(await exists(activationPath)), "remedy-v2 activation already exists");
