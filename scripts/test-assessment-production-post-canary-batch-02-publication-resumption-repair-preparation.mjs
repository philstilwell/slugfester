...
const MANIFEST = `${POST_CANARY_BATCH_02_DEBATE_172_REPAIR_ROOT}/execution-preparation-manifest.json`;
...
assert.equal(manifest.protocolId, POST_CANARY_BATCH_02_DEBATE_172_REPAIR_PROTOCOL_ID);
...
assert.equal(manifest.totals.contexts, 1);
assert.equal(manifest.totals.writableFields, 2);
...
for (const context of manifest.contexts) {
  ...
  assert.equal(packet.debateNumber, "172");
  ...
  assert.deepEqual(schema, buildDebate172RepairSchema(packet));
  ...
  assert.equal(validateDebate172RepairOutput(repair, packet).status, "passed");
...
const merged = mergeAndValidateDebate172Repairs({
  baseOutput,
  repairs: syntheticRepairs,
  repairPackets,
  publicationPacket
});
assert.equal(merged.transformations.length, 2);
assert.equal(merged.fullValidation.status, "passed");
assert.equal(merged.fullValidation.moves, 19);
