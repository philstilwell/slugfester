...
import {
  POST_CANARY_BATCH_11_RENDERING_ORDER,
  POST_CANARY_BATCH_11_RENDERING_PORT,
  POST_CANARY_BATCH_11_RENDERING_PROTOCOL_ID,
  POST_CANARY_BATCH_11_RENDERING_REQUIRED_CHECKS,
  POST_CANARY_BATCH_11_RENDERING_ROOT,
  POST_CANARY_BATCH_11_RENDERING_VIEWPORTS,
  buildPostCanaryBatch11RenderingPacket,
  hashFile,
  sha256
} from "./lib/assessment-production-post-canary-batch-11-rendering-verification.mjs";
import {
...
const FINALIZATION =
  "docs/assessment-production/post-canary-continuation-v1/batch-11/publication-finalization";
...
const PREPARATION = `${POST_CANARY_BATCH_11_RENDERING_ROOT}/preparation-manifest.json`;
const ACTIVATION = `${POST_CANARY_BATCH_11_RENDERING_ROOT}/execution-activation.json`;
const EXECUTION = `${POST_CANARY_BATCH_11_RENDERING_ROOT}/execution.json`;
const ANALYSIS = `${POST_CANARY_BATCH_11_RENDERING_ROOT}/analysis.json`;
const AUDIT = `${POST_CANARY_BATCH_11_RENDERING_ROOT}/rendering-audit.json`;
const PACKETS = `${POST_CANARY_BATCH_11_RENDERING_ROOT}/packets`;
...
const standing = await loadAndValidatePostCanaryBatch11StandingAuthorization();
...
"Batch 11 standing rendering authorization required");
...
assertV4(finalPreparation.status ===
  "frozen-post-canary-batch-11-publication-finalization-prepared" &&
  finalActivation.status ===
  "frozen-post-canary-batch-11-publication-finalization-authorized" &&
  finalExecution.status === "ten-debate-batch-11-publication-finalization-passed" &&
  finalAnalysis.status === "ten-debate-batch-11-publication-finalization-passed" &&
  finalAudit.status === "passed" && finalAudit.rows?.length === 10 &&
  finalAudit.totals?.moves === 190,
"passing Batch 11 finalization required");

let previewText = originalPreviewBytes.toString("utf8");
previewText = previewText.replace(
  "renderPublicationStagingDebate(await response.json());",
  `renderPublicationStagingDebate(await response.json());\n        const banner = document.querySelector("[data-publication-staging-preview] .calibration-preview-note");\n        if (banner) banner.innerHTML = "<strong>Publication staging preview:</strong> validated post-canary Batch 11 candidate only. This scorecard remains excluded from production data and rankings pending rendering and mutation authorization.";`
);
assertV4(previewText !== originalPreviewBytes.toString("utf8"),
  "Batch 11 rendering preview adaptation failed");
const previewBytes = Buffer.from(previewText);

const compatibility = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-11-publication-finalization-compatibility-analysis",
  protocolId: "assessment-production-post-canary-batch-11-publication-finalization",
  status: "batch-11-production-compatibility-boundary-recorded",
  analyzedAt: frozenAt,
  productionCanary: false,
  batchNumber: 11,
  stagingOnly: true,
  findings: [{
    id: "batch-11-site-ledger-adapter-and-validator-route",
    description: "The live validator requires a separately frozen authenticated Batch 11 adapter route before production mutation.",
...
const packets = [];
for (const debateNumber of POST_CANARY_BATCH_11_RENDERING_ORDER) {
  ...
  const packet = buildPostCanaryBatch11RenderingPacket({ auditRow: row,
...
const sources = [FINALIZATION_PREPARATION, FINALIZATION_ACTIVATION,
  ...
  ORIGINAL_PREVIEW, POST_CANARY_BATCH_11_STANDING_AUTHORIZATION,
  "src/app.js", "src/styles.css",
  "scripts/lib/assessment-production-post-canary-batch-11-rendering-verification.mjs",
  "scripts/lib/assessment-production-post-canary-batch-11-publication-finalization.mjs",
  "scripts/lib/assessment-production-post-canary-batch-11-standing-authorization.mjs",
  "scripts/lib/v4-lean-production.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-11-rendering-verification.mjs",
  "scripts/test-assessment-production-post-canary-batch-11-rendering-verification-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-11-rendering-verification.mjs",
  "scripts/run-assessment-production-post-canary-batch-11-rendering-verification.mjs",
  "scripts/finalize-assessment-production-post-canary-batch-11-rendering-verification.mjs",
...
const manifest = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-11-rendering-preparation",
  protocolId: POST_CANARY_BATCH_11_RENDERING_PROTOCOL_ID,
  status: "frozen-post-canary-batch-11-rendering-verification-prepared",
  frozenAt,
  ...
  explicitOrder: POST_CANARY_BATCH_11_RENDERING_ORDER,
  viewports: POST_CANARY_BATCH_11_RENDERING_VIEWPORTS,
  requiredChecks: POST_CANARY_BATCH_11_RENDERING_REQUIRED_CHECKS,
  browserPlan: {
    controller: "playwright-cli",
    browserFamily: "Chromium",
    sessionName: "batch11-rendering",
    port: POST_CANARY_BATCH_11_RENDERING_PORT,
  ...
  artifacts: {
    ...
    evidenceRoot: `${POST_CANARY_BATCH_11_RENDERING_ROOT}/evidence`,
    screenshotRoot: "output/playwright/batch-11-rendering",
  ...
  nextAuthorizedAction: "activate-batch-11-rendering-verification"
};
...
console.log(JSON.stringify({ status: manifest.status, debates: 10,
  sections: 50, moves: 190, viewportResultsPlanned: 20,
