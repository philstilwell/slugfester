...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(
      os.tmpdir(),
      `batch-03-publication-resumption-repair-${context.contextIndex}-`
    )
  );
  const codexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `batch-03-publication-resumption-repair-home-${context.contextIndex}-`
    )
  );
  ...
  try {
    ...
    const prompt = [
      ...
      `Act only as isolated Batch 3 publication-resumption repair editor ${context.contextIndex} for Debate ${context.debateNumber}.`,
      "Return exactly the correctedFields entries named by the schema and no other field.",
      context.repairType === "novelty-explanation-word-boundary"
        ? "Repair only the named novelty explanation: preserve its item text, novelty classification, and source-move mapping; target 8–18 words and remain within 8–35 words."
    ...
    process.stdout.write(
      `[batch-03-publication-resumption-repair] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
    ...
    if (
      ...
      const validation = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-03-publication-resumption-repair-validation",
      ...
      const provenance = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-03-publication-resumption-repair-provenance",
        protocolId: activation.protocolId,
        debateNumber: context.debateNumber,
        packetIndex: context.packetIndex,
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-publication-resumption-repair-model-execution",
  protocolId: activation.protocolId,
  status:
    validContexts === 3
      ? "batch-03-publication-resumption-three-context-repair-gate-passed"
      : "batch-03-publication-resumption-repair-gate-complete-with-failure",
  gateStartedAt,
