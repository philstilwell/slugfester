...
assertV4(
  preparation.protocolId === POST_CANARY_BATCH_01_RENDERING_PROTOCOL_ID &&
    preparation.status ===
  ...
  "frozen Batch 1 rendering preparation required"
);
execFileSync(
  process.execPath,
  [
...
for (const [file, digest] of Object.entries(preparation.sourceHashes)) {
  assertV4(
    ...
    `source hash mismatch before Batch 1 rendering activation: ${file}`
  );
}
for (const [file, digest] of Object.entries(preparation.toolHashes)) {
  assertV4(
    sha256(await readFile(path.resolve(file))) === digest,
    `tool hash mismatch before Batch 1 rendering activation: ${file}`
  );
}
for (const row of preparation.packets) {
  ...
  assertV4(
    ...
    `${row.debateNumber}: Batch 1 rendering packet hash mismatch`
  );
  validatePostCanaryBatch01RenderingPacket(JSON.parse(bytes));
}
...
assertV4(
  ...
  "Batch 1 rendering evidence already exists"
);
...
const activation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-01-rendering-verification-execution-activation",
  protocolId: POST_CANARY_BATCH_01_RENDERING_PROTOCOL_ID,
  status: "frozen-post-canary-batch-01-rendering-verification-authorized",
  authorizedAt,
  activationCommit,
  ...
  stagingOnly: true,
  userAuthorization: {
    anticipatedInstruction:
      "I approve activation and execution of the frozen Batch 1 rendering-verification gate, with a direct incremental cost cap of $0. Use the frozen explicit debate and viewport order, the proven split-Chromium v9 transport, one attempt per viewport, no retries, and no timeout extensions. Stop after deterministic evidence validation, analysis, committing, and pushing. Do not execute models, use paid services, repair rendering, publish production ledgers, mutate production, or select the next batch.",
  ...
  nextAuthorizedAction:
    "single-batch-01-rendering-verification-execution-under-frozen-stop-rules"
};

if (shouldWrite) {
  assertV4(!(await exists(ACTIVATION)), "Batch 1 rendering activation already exists");
