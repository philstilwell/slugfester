...
import { POST_CANARY_BATCH_08_DEBATE_120_REPAIR_ROOT, validateDebate120RepairOutput } from
...
assertV4(activation.status === "frozen-two-context-batch-08-debate-120-publication-repair-authorized" &&
  activation.contexts?.length === 2 && activation.authorization?.repairModelExecution === true &&
...
"the Debate 120 repair execution is not authorized");
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-08-debate-120-repair-${context.contextIndex}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `batch-08-debate-120-repair-home-${context.contextIndex}-`));
  ...
  try { const copies = [[activation.modelInputs.productionWorkflow, "production-workflow.md"],
    ...
    const prompt = ["Read production-workflow.md, readiness-workflow.md, output-contract.md, manual.md, packet.json, and schema.json completely and no other files.",
      `Act only as isolated Debate 120 bounded repair editor ${context.packetIndex}.`,
      "Rewrite exactly the correctedFields named by the packet and schema, preserving adjudicated substance and every immutable field.",
      "For a critique field, write exactly four ordered labeled sentences, target 112–118 words, remain within 105–130 words and at least 880 characters, and end every sentence with terminal punctuation. For a novelty-explanation field, preserve its classification, move mapping, and item meaning, target 10–18 words, and remain within 8–35 words. Count words before returning.",
    ...
    process.stdout.write(`[batch-08-debate-120-repair] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort}\n`);
    ...
    const base = { contextIndex: context.contextIndex, packetIndex: context.packetIndex,
      debateNumber: "120", debateId: context.debateId, model: activation.model.label,
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 || invocation.signal !== null || !resultExists) {
      ...
      try { validationSummary = validateDebate120RepairOutput(JSON.parse(outputBytes),
      ...
      const validation = { schemaVersion: "1.0-assessment-production-post-canary-batch-08-debate-120-repair-validation",
        ...
        debateNumber: "120", outputSha256: sha256(outputBytes), validationSummary,
        validationMessage, modelAuthoredScores: 0 };
      const provenance = { schemaVersion: "1.0-assessment-production-post-canary-batch-08-debate-120-repair-provenance",
        protocolId: activation.protocolId, contextIndex: context.contextIndex,
        packetIndex: context.packetIndex, debateNumber: "120", model: activation.model,
    ...
    debateNumber: "120", attemptCount: 1, retryCount: 0, timeoutExtensionCount: 0,
  ...
  process.stdout.write(`[batch-08-debate-120-repair] index ${context.contextIndex} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`); return record;
}
...
const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-08-debate-120-repair-execution",
  protocolId: activation.protocolId, status: validContexts === 2
    ? "batch-08-debate-120-two-context-repair-gate-passed"
    : "batch-08-debate-120-repair-gate-complete-with-failure",
  gateStartedAt, gateCompletedAt: new Date().toISOString(), contextsPlanned: 2,
  contextsAttempted: results.length, contextsUnattempted: 2 - results.length,
