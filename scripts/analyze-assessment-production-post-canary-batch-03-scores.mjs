...
import {
  POST_CANARY_BATCH_03_CALCULATED_SCORES_VERSION,
...
const manifest = JSON.parse(
  await readFile(
    path.resolve(`${POST_CANARY_BATCH_03_SCORE_ROOT}/score-pass-manifest.json`),
    "utf8"
  )
);
...
const validation = validatePostCanaryBatch03Scores(
  scores,
  ledger,
  inputs.debateInputs,
...
assertV4(
  scores.schemaVersion === POST_CANARY_BATCH_03_CALCULATED_SCORES_VERSION &&
    !scores.productionCanary &&
    scores.batchNumber === 3 &&
    scores.stagingOnly &&
    !scores.developmentValidationOnly &&
    scores.formulaBoundary.scoringPasses === 1 &&
    !scores.formulaBoundary.modelCalculatedScores,
  "post-canary Batch 3 score artifact identity mismatch"
);
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-score-analysis",
  protocolId: scores.protocolId,
  status: passed
    ? "post-canary-batch-03-score-stability-gate-passed"
    : "post-canary-batch-03-score-stability-gate-failed",
  ...
  nextAuthorizedAction: passed
    ...
    : "failure-recovery-standing-authorization-permits-batch-03-score-stability-failure-diagnosis"
};
if (shouldWrite) {
  await writeFile(
