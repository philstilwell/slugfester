...
import { validateV223DisputeAdjudicationOutput } from "./lib/assessment-production-score-stability-v2.2.3-dispute-adjudication.mjs";
...
assertV4(
  ...
  "v2.2.3 dispute adjudication execution unauthorized"
);
...
async function runContext(context, contextIndex) {
  const temporary = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-v223-adjudication-${context.debateNumber}-`
    )
  );
  const codexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-v223-adjudication-home-${context.debateNumber}-`
    )
  );
  ...
  try {
    ...
    }; read nothing else. Act only as the isolated disputed-fields-only v2.2.3 adjudicator for Debate ${
    ...
    process.stdout.write(
      `[v2.2.3-adjudication] starting index ${contextIndex} ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
    ...
    if (
      ...
      try {
        validation = validateV223DisputeAdjudicationOutput(
  ...
  process.stdout.write(
    `[v2.2.3-adjudication] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-score-stability-v2.2.3-dispute-only-adjudication-model-execution",
