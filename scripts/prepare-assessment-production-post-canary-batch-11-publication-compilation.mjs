...
import {
  POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_ORDER,
  POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_PROTOCOL_ID,
  POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_ROOT,
  compilePostCanaryBatch11PublicationStagingRecord,
  validatePostCanaryBatch11CompiledStagingRecord
} from "./lib/assessment-production-post-canary-batch-11-publication-compilation.mjs";
import { POST_CANARY_BATCH_11_PUBLICATION_ROOT } from
  "./lib/assessment-production-post-canary-batch-11-publication.mjs";
import { validatePostCanaryBatch11PublicationOutput } from
  "./lib/assessment-production-post-canary-batch-11-publication-validation.mjs";
import { POST_CANARY_BATCH_11_STANDING_AUTHORIZATION,
  loadAndValidatePostCanaryBatch11StandingAuthorization } from
  "./lib/assessment-production-post-canary-batch-11-standing-authorization.mjs";
...
const ROOT = POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_ROOT;
const COHORT = `${POST_CANARY_BATCH_11_PUBLICATION_ROOT}/failure-recovery/field-disjoint-repair-1/complete-cohort-analysis.json`;
...
const ACCEPTED_OUTPUTS = Object.freeze(Object.fromEntries(
  POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_ORDER.map((debateNumber) => [
    debateNumber,
    `${POST_CANARY_BATCH_11_PUBLICATION_ROOT}/outputs/debate-${debateNumber}.json`
  ])
));
const acceptedOutput = (debateNumber) => ACCEPTED_OUTPUTS[debateNumber];
const packetPath = (debateNumber) =>
  `${POST_CANARY_BATCH_11_PUBLICATION_ROOT}/packets/debate-${debateNumber}.json`;
...
assertV4(
  cohort.status ===
    "post-canary-batch-11-complete-ten-debate-publication-output-gate-passed-after-field-disjoint-repair" &&
  cohort.totals?.debates === 10 && cohort.totals?.lockedMoves === 190 &&
  cohort.totals?.critiques === 190 && cohort.totals?.exactSourceQuotes === 20 &&
  cohort.integrity?.scoresRemainedImmutable === true &&
  cohort.integrity?.everyAcceptedOutputReplayedDeterministically === true,
"the accepted Batch 11 publication cohort changed");

const productionByNumber = new Map(debates.map((debate) => [debate.number, debate]));
const identities = [];
const contexts = [];
for (const debateNumber of POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_ORDER) {
  ...
  const validation = validatePostCanaryBatch11PublicationOutput(output, packet);
  ...
  const compiled = compilePostCanaryBatch11PublicationStagingRecord({ output, packet, identity });
  const compiledValidation = validatePostCanaryBatch11CompiledStagingRecord({ compiled, output, packet, identity });
...
assertV4(contexts.length === 10 && contexts.reduce((sum, row) => sum + row.expectedMoves, 0) === 190,
"the Batch 11 compilation cohort changed");
const identitySnapshot = { schemaVersion: "1.0-assessment-production-post-canary-batch-11-publication-compilation-identity-snapshot",
  protocolId: POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_PROTOCOL_ID,
...
const sources = [COHORT, POST_CANARY_BATCH_11_STANDING_AUTHORIZATION,
  ...
  "scripts/lib/assessment-production-post-canary-batch-11-publication-compilation.mjs",
  "scripts/lib/assessment-production-post-canary-batch-11-standing-authorization.mjs",
  "scripts/lib/v4-lean-production.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-11-publication-compilation.mjs",
  "scripts/test-assessment-production-post-canary-batch-11-publication-compilation-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-11-publication-compilation.mjs",
  "scripts/run-assessment-production-post-canary-batch-11-publication-compilation.mjs",
...
const manifest = { schemaVersion: "1.0-assessment-production-post-canary-batch-11-deterministic-publication-compilation-preparation",
  protocolId: POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_PROTOCOL_ID,
  status: "frozen-post-canary-batch-11-deterministic-publication-compilation-prepared",
  frozenAt, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(),
  productionCanary: false, batchNumber: 11, stagingOnly: true,
  userAuthorization: { standingAuthorization: POST_CANARY_BATCH_11_STANDING_AUTHORIZATION,
  ...
  explicitOrder: POST_CANARY_BATCH_11_PUBLICATION_COMPILATION_ORDER, contexts,
  ...
  nextAuthorizedAction: "activate-one-batch-11-deterministic-publication-compilation-pass" };
