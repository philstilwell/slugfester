...
assertV4(
  activation.status === "frozen-six-original-unattempted-batch-13-publication-contexts-activated" &&
    activation.authorization?.modelExecution === true &&
    activation.contexts?.length === 6 &&
    canonicalJson(activation.contexts.map((context) => context.originalContextIndex)) ===
      canonicalJson(ORIGINAL_CONTEXT_INDEXES),
  "six-context resumption execution is not activated"
);
assertV4(
  ...
  "six-context execution policy changed"
);
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-13-publication-resumption-${context.debateNumber}-`));
  const isolatedCodexHome = await mkdtemp(path.join(os.tmpdir(), `batch-13-publication-resumption-home-${context.debateNumber}-`));
  ...
  try {
    ...
    const prompt = [
      ...
      "Never infer, emit, recalculate, or suggest changing a score; never change identity, structure, move selection, or source evidence; never consult legacy assessment material, any accepted prior Batch 13 output, other debates, rankings, winners, or another model context; never attribute AI material to a participant.",
    ...
    process.stdout.write(`[batch-13-publication-resumption] starting original index ${context.originalContextIndex} Debate ${context.debateNumber}\n`);
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 || invocation.signal !== null || !resultExists) {
      ...
      try {
        validationSummary = validatePostCanaryBatch13PublicationOutput(
      ...
      const validation = {
        schemaVersion: "1.0-assessment-production-post-canary-batch-13-publication-resumption-validation",
      ...
      const provenance = {
        schemaVersion: "1.0-assessment-production-post-canary-batch-13-publication-resumption-provenance",
        protocolId: activation.protocolId,
        originalContextIndex: context.originalContextIndex,
        debateNumber: context.debateNumber,
        debateId: context.debateId,
  ...
  process.stdout.write(`[batch-13-publication-resumption] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
...
const passed = results.length === 6 && validContexts === 6;
const execution = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-13-publication-original-unattempted-context-resumption-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "six-original-unattempted-batch-13-publication-contexts-passed"
    : "six-context-publication-resumption-stopped-with-failure",
  gateStartedAt,
  ...
  nextRequiredAction: passed
    ? "deterministically-replay-six-outputs-and-complete-ten-debate-publication-cohort"
