...
import {
  ...
  evaluatePostCanaryBatch04ScoreStability
...
const accepted = evaluatePostCanaryBatch04ScoreStability(passing);
...
assert.equal(
  evaluatePostCanaryBatch04ScoreStability(winnerMutation).acceptancePassed,
  false
);
const distanceMutation = structuredClone(passing);
distanceMutation[2].final = scores(95, 80);
assert.equal(
  evaluatePostCanaryBatch04ScoreStability(distanceMutation).acceptancePassed,
  false
);
assert.throws(() =>
  evaluatePostCanaryBatch04ScoreStability(passing, {
    ...POST_CANARY_BATCH_04_SCORE_STABILITY_THRESHOLDS,
