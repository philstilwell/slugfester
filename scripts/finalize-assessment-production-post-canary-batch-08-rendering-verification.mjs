...
import { POST_CANARY_BATCH_08_RENDERING_ORDER, POST_CANARY_BATCH_08_RENDERING_ROOT,
  validatePostCanaryBatch08RenderingPacket,
  validatePostCanaryBatch08RenderingViewportEvidence } from
  "./lib/assessment-production-post-canary-batch-08-rendering-verification.mjs";
import { assertV4, canonicalJson } from "./lib/v4-lean-production.mjs";
const ROOT = POST_CANARY_BATCH_08_RENDERING_ROOT;
...
assertV4(a.status === "frozen-post-canary-batch-08-rendering-verification-authorized" &&
...
"Batch 8 rendering activation changed");
const results = [];
for (const debateNumber of POST_CANARY_BATCH_08_RENDERING_ORDER) {
  ...
  const packet = validatePostCanaryBatch08RenderingPacket(JSON.parse(packetBytes));
  for (const viewportName of ["desktop", "mobile"]) {
    const evidence = JSON.parse(await readFile(path.resolve(packet.viewports[viewportName].evidence.result), "utf8"));
    validatePostCanaryBatch08RenderingViewportEvidence({ packet, viewportName,
...
assertV4(results.length === 20 && results.every((row) => row.evidence.status === "passed-rendering-viewport"),
  "complete Batch 8 rendering evidence failed");
const totals = { debates: 10, sections: 51, moves: 182, viewportResults: 20,
...
const audit = { schemaVersion: "1.0-assessment-production-post-canary-batch-08-rendering-audit",
  protocolId: a.protocolId, status: "passed-ten-debate-batch-08-rendering-verification",
...
const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-08-rendering-execution",
  protocolId: a.protocolId, status: "ten-debate-batch-08-rendering-verification-passed",
...
const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-08-rendering-analysis",
  ...
  nextAuthorizedAction: "prepare-batch-08-production-compatibility-staging" };
...
console.log(JSON.stringify({ status: analysis.status, debates: 10, sections: 51, moves: 182,
