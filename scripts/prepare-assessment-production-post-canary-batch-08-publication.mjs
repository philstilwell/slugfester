...
import {
  validatePostCanaryBatch08Scores
} from "./lib/assessment-production-post-canary-batch-08-score-gate.mjs";
import {
  buildPostCanaryBatch08PublicationPacket,
  buildPostCanaryBatch08PublicationSchema,
  postCanaryBatch08ReferenceCatalog,
  POST_CANARY_BATCH_08_PUBLICATION_DEBATES,
  POST_CANARY_BATCH_08_PUBLICATION_MODEL,
  POST_CANARY_BATCH_08_PUBLICATION_PROTOCOL_ID,
...
const SELECTION =
  "docs/assessment-production/post-canary-continuation-v1/batch-08/selection.json";
const FINAL_LEDGER =
...
const FINAL_LEDGER_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-08/final-ledger/final-ledger-manifest.json";
const FINAL_LEDGER_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-08/final-ledger/analysis.json";
const SCORES =
  "docs/assessment-production/post-canary-continuation-v1/batch-08/score-pass/calculated-scores.json";
const SCORE_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-08/score-pass/score-pass-manifest.json";
const SCORE_PREPARATION =
  "docs/assessment-production/post-canary-continuation-v1/batch-08/score-pass/score-pass-preparation-manifest.json";
const SCORE_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-08/score-pass/analysis.json";
const AUDIO_AUDIT =
  "docs/assessment-production/post-canary-continuation-v1/batch-08/audio-verification/resolution-execution/audio-verification.json";
const SOURCE_PACKET_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-08/independent-judgments/source-packets";
...
const STATIC_SOURCE_FILES = [
  ...
  "docs/assessment-production/post-canary-continuation-v1/batch-08/continuation-standing-authorization.json",
  "src/data/debates.js",
  ...
  "scripts/lib/assessment-production-post-canary-batch-08-score-gate.mjs",
  ...
  "scripts/prepare-assessment-production-post-canary-batch-08-publication.mjs",
  "scripts/test-assessment-production-post-canary-batch-08-publication-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-08-publication.mjs",
  "scripts/run-assessment-production-post-canary-batch-08-publication.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-08-publication.mjs"
];
...
assertV4(
  scoreAnalysis.status === "post-canary-batch-08-score-stability-gate-passed" &&
    scoreAnalysis.productionCanary === false &&
    scoreAnalysis.batchNumber === 8 &&
    ...
    scoreAnalysis.nextAuthorizedAction ===
      "standing-authorization-permits-batch-08-publication-packet-preparation",
  "the accepted Batch 8 score gate is unavailable or its stop boundary changed"
);
assertV4(
  scores.status === "post-canary-batch-08-single-score-pass-stability-gate-passed" &&
  ...
  "the locked Batch 8 score artifact is unavailable"
);
assertV4(
  scoreManifest.status ===
      "frozen-post-canary-batch-08-single-deterministic-score-pass-authorized" &&
    ...
    scorePreparation.status ===
      "frozen-post-canary-batch-08-single-deterministic-score-pass-prepared-not-activated",
  "the frozen Batch 8 single-pass score controls changed"
);
assertV4(
  ledger.status ===
      "passed-post-canary-batch-08-deterministic-final-ledger-assembly" &&
    ledger.debates?.length === 10 &&
    ledger.audit?.finalMoves === 182 &&
    ledger.audit?.audioVerifiedMoves === 6 &&
    ...
    ledgerAnalysis.status ===
      "post-canary-batch-08-deterministic-final-ledger-gate-passed",
  "the locked Batch 8 final ledger changed"
);
assertV4(
  audioAudit.status ===
      "passed-all-six-batch-08-audio-attributions-after-transient-reference-overlays" &&
    audioAudit.totals?.requiredMoves === 6 &&
    audioAudit.totals?.verified === 6 &&
    audioAudit.totals?.unresolved === 0,
  "the five required Batch 8 audio checks are incomplete"
);
assertV4(
  productionManifest.model?.label === POST_CANARY_BATCH_08_PUBLICATION_MODEL.label &&
    productionManifest.model?.slug === POST_CANARY_BATCH_08_PUBLICATION_MODEL.slug &&
    productionManifest.model?.reasoningEffort ===
      POST_CANARY_BATCH_08_PUBLICATION_MODEL.reasoningEffort &&
    productionManifest.model?.authentication ===
      POST_CANARY_BATCH_08_PUBLICATION_MODEL.authentication,
  "the frozen production model or authentication changed"
);
assertV4(
  selection.selected?.map((item) => item.debateNumber).join(",") ===
      POST_CANARY_BATCH_08_PUBLICATION_DEBATES.join(",") &&
    selection.selected.every((item) => item.speakerCount === 2) &&
    selection.authorization?.productionMutation === false,
  "the frozen Batch 8 selection changed"
);
assertV4(
  ledger.debates.map((debate) => debate.debateNumber).join(",") ===
    ...
    scores.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_08_PUBLICATION_DEBATES.join(","),
  "the Batch 8 ledger or score order changed"
);
assertV4(
...
const finalLedgerInputs = await loadPostCanaryBatch08FinalLedgerInputs();
validatePostCanaryBatch08FinalLedger(
  ledger,
...
const catalog = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-08-local-reference-catalog",
  references: postCanaryBatch08ReferenceCatalog()
};
...
for (const debateNumber of POST_CANARY_BATCH_08_PUBLICATION_DEBATES) {
  ...
  const packet = buildPostCanaryBatch08PublicationPacket({
    ledgerDebate,
    scoreDebate,
    sourcePacket,
  ...
  const schema = buildPostCanaryBatch08PublicationSchema(packet);
...
assertV4(
  ...
  "Batch 8 publication move coverage changed"
);
assertV4(
...
assertV4(
  ...
  "Batch 8 publication section or quote eligibility is invalid"
);
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-08-publication-execution-preparation-manifest",
  protocolId: POST_CANARY_BATCH_08_PUBLICATION_PROTOCOL_ID,
  status:
  ...
  userAuthorization: {
    instruction:
      "The frozen Batch 8 standing authorization permits preparation, activation, and execution of exactly ten score-locked publication-reconstruction contexts while every gate passes, with direct incremental cost capped at $0.",
