...
const finalLedgerPath =
  `${POST_CANARY_BATCH_13_FINAL_LEDGER_ROOT}/final-ledger.json`;
...
assertV4(
  ...
  "post-canary Batch 13 final-ledger manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen Batch 13 final-ledger source hash mismatch`
  );
}
const inputs = await loadPostCanaryBatch13FinalLedgerInputs();
const ledger = buildPostCanaryBatch13FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
