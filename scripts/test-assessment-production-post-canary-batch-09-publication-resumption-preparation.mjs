...
const MANIFEST =
  `${POST_CANARY_BATCH_09_PUBLICATION_RESUMPTION_ROOT}/execution-preparation-manifest.json`;
...
assert.equal(manifest.totals.resumptionMoves, 160);
...
assert.equal(manifest.totals.cohortMoves, 180);
...
assert.equal(moves, 160);
assert.equal(sections, 46);
assert.equal(audioVerifiedMoves, 3);

const [repairedBytes, debate170PacketBytes, completeValidation] =
  await Promise.all([
    readFile(manifest.acceptedDebate170.output),
    readFile(manifest.acceptedDebate170.packet),
    readFile(manifest.acceptedDebate170.validation, "utf8").then(JSON.parse)
  ]);
const repairedValidation = validatePostCanaryBatch09PublicationOutput(
  JSON.parse(repairedBytes),
  JSON.parse(debate170PacketBytes)
);
...
assert.equal(completeValidation.authorizedFieldsChanged, 14);
