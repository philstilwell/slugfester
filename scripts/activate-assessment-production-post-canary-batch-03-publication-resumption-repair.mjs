...
assertV4(
  preparation.protocolId === POST_CANARY_BATCH_03_RESUMPTION_REPAIR_PROTOCOL_ID &&
    preparation.status ===
    ...
    JSON.stringify(
      ...
      JSON.stringify([
        { debateNumber: "58", writableFieldCount: 1 },
        { debateNumber: "150", writableFieldCount: 2 },
        { debateNumber: "150", writableFieldCount: 2 }
      ]) &&
...
const activation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-publication-resumption-repair-execution-activation",
  ...
  userAuthorization: {
    instruction:
      "Batch 3 failure-recovery standing authorization permits activation and one execution attempt for each newly frozen bounded correction context, followed by deterministic merge and affected-cohort replay.",
  ...
  standingAuthorization: structuredClone(preparation.standingAuthorization),
  failureRecoveryStandingAuthorization: structuredClone(
    preparation.failureRecoveryStandingAuthorization
  ),
  ...
  nextRequiredAction:
    "execute-the-three-frozen-batch-03-publication-resumption-repair-contexts-once"
};
