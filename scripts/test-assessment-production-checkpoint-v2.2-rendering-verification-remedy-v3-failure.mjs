...
assert.equal(execution.transientDomPassingViewportResults, 13);
assert.equal(execution.finalizedViewportResults, 0);
assert.equal(execution.failedViewportResults, 1);
assert.equal(execution.unattemptedViewportResults, 6);
assert.equal(execution.transientScreenshotsCaptured, 26);
...
assert.equal(execution.attemptedFailure.debateNumber, "22");
assert.equal(execution.attemptedFailure.viewport, "mobile");
assert.equal(execution.attemptedFailure.measuredCandidateHttpStatus, 200);
assert.equal(execution.attemptedFailure.screenshotCaptured, false);
assert.match(execution.attemptedFailure.error, /deadline before command dispatch/);
assert.equal(execution.screenshotCaptureAudit.desktop.colors, 1);
assert.equal(execution.screenshotCaptureAudit.desktop.entropy, 0);
assert.equal(execution.screenshotCaptureAudit.mobile.colors, 1);
assert.equal(execution.screenshotCaptureAudit.mobile.entropy, 0);
assert.equal(execution.screenshotCaptureAudit.validRenderingScreenshots, 0);
assert.equal(execution.cleanup.evidenceRootRemoved, true);
...
assert.equal(analysis.gate.transientDomViewportResultsPassed, 13);
...
assert.equal(analysis.authorization.renderingVerificationRemedyV4PlanPreparation, false);
assert.equal(analysis.authorization.browserControl, false);
assert.equal(analysis.authorization.screenshotCapture, false);
assert.equal(analysis.authorization.modelExecution, false);
...
console.log(JSON.stringify({
  status: "remedy-v3-rendering-failure-audit-passed",
  transientDomViewportResults: execution.transientDomPassingViewportResults,
  finalizedViewportResults: execution.finalizedViewportResults,
  rejectedBlankScreenshots: execution.transientScreenshotsCaptured,
