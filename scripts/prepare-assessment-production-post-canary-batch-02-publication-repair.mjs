...
import {
  POST_CANARY_BATCH_02_DEBATE_103_BASE_OUTPUT,
  POST_CANARY_BATCH_02_DEBATE_103_PUBLICATION_PACKET,
  POST_CANARY_BATCH_02_DEBATE_103_REPAIR_FIELDS,
  POST_CANARY_BATCH_02_DEBATE_103_REPAIR_PACKET_VERSION,
...
const ROOT = POST_CANARY_BATCH_02_DEBATE_103_REPAIR_ROOT;
const MANIFEST = `${ROOT}/execution-preparation-manifest.json`;
const DIAGNOSIS = `${POST_CANARY_BATCH_02_PUBLICATION_ROOT}/failure-diagnosis.json`;
const FAILED_EXECUTION = `${POST_CANARY_BATCH_02_PUBLICATION_ROOT}/model-execution.json`;
const FAILED_ANALYSIS = `${POST_CANARY_BATCH_02_PUBLICATION_ROOT}/analysis.json`;
...
const STATIC_SOURCE_FILES = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-02-publication-repair.mjs",
  "scripts/lib/assessment-production-post-canary-batch-02-standing-authorization.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-02-publication-repair.mjs",
  "scripts/test-assessment-production-post-canary-batch-02-publication-repair-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-02-publication-repair.mjs",
  "scripts/run-assessment-production-post-canary-batch-02-publication-repair.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-02-publication-repair.mjs"
];
...
const baseOutput = parsed(POST_CANARY_BATCH_02_DEBATE_103_BASE_OUTPUT);
const publicationPacket = parsed(POST_CANARY_BATCH_02_DEBATE_103_PUBLICATION_PACKET);
...
assertV4(
  diagnosis.status ===
    ...
    diagnosis.failedContext?.debateNumber === "103" &&
    diagnosis.failedContext?.debateId === publicationPacket.debateId &&
    diagnosis.failureBoundary?.failedFieldCount === 17 &&
    diagnosis.failureBoundary?.excessWordsTotal === 83 &&
  ...
  "the frozen Debate 103 failure diagnosis changed"
);
assertV4(
...
assertV4(
  execution.contextsPlanned === 10 &&
    ...
    execution.results?.[0]?.debateNumber === "103" &&
    execution.results?.[0]?.gateAcceptancePassed === false &&
    analysis.status === "post-canary-batch-02-publication-output-gate-failed",
  "the preserved failed publication gate changed"
);
assertV4(
  sha256(bytesByFile[POST_CANARY_BATCH_02_DEBATE_103_BASE_OUTPUT]) ===
      diagnosis.failedContext.outputSha256 &&
    sha256(bytesByFile[POST_CANARY_BATCH_02_DEBATE_103_BASE_OUTPUT]) ===
      diagnosis.artifacts.output.sha256 &&
    sha256(bytesByFile[POST_CANARY_BATCH_02_DEBATE_103_PUBLICATION_PACKET]) ===
  ...
  "a diagnosed Debate 103 source artifact drifted"
);
assertV4(
  canonicalJson(diagnosis.prospectiveRecoveryOnly.proposedRepairPartition) ===
      canonicalJson(POST_CANARY_BATCH_02_DEBATE_103_REPAIR_PARTITIONS) &&
    canonicalJson(diagnosis.diagnosticReplay.hypotheticalWritableFields) ===
      canonicalJson(POST_CANARY_BATCH_02_DEBATE_103_REPAIR_FIELDS),
...
const contexts = [];
const generated = [];
for (let packetIndex = 0; packetIndex < 9; packetIndex += 1) {
  const writableFields = POST_CANARY_BATCH_02_DEBATE_103_REPAIR_PARTITIONS[packetIndex];
  const corrections = writableFields.map((field) => {
    const moveId = debate103RepairMoveId(field);
  ...
  const packet = {
    schemaVersion: POST_CANARY_BATCH_02_DEBATE_103_REPAIR_PACKET_VERSION,
    ...
    immutableBaseOutput: POST_CANARY_BATCH_02_DEBATE_103_BASE_OUTPUT,
    publicationPacket: POST_CANARY_BATCH_02_DEBATE_103_PUBLICATION_PACKET,
  ...
  const schema = buildDebate103RepairSchema(packet);
...
assertV4(
  ...
  "exactly nine disjoint one-or-two-field repair contexts are required"
);

const ACTIVATION = `${ROOT}/execution-activation.json`;
const EXECUTION = `${ROOT}/model-execution.json`;
const ANALYSIS = `${ROOT}/analysis.json`;
const MERGED_OUTPUT = `${ROOT}/merged/debate-103.json`;
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-debate-103-publication-repair-execution-preparation-manifest",
  protocolId: POST_CANARY_BATCH_02_DEBATE_103_REPAIR_PROTOCOL_ID,
  status:
  ...
  userAuthorization: {
    ...
    resolvedScope:
      "prepare, validate, freeze, activate, and execute exactly nine bounded Debate 103 publication-repair packets exposing seventeen critique fields",
    standingAuthorizationApplied: true,
    standingAuthorizationSha256: standingAuthorization.sha256,
    directIncrementalCostUsdMaximum: 0,
    contextsPrepared: 9,
    writableFieldsPrepared: 17,
  ...
  costEstimate: {
    ...
    contexts: 9,
    expectedParallelWallMinutes: [6, 15],
    expectedAggregateModelMinutes: [9, 21],
    absoluteGateTimeoutMinutes: 72,
    estimateBasis: {
      source:
      ...
      scalingRule: "nine-sevenths-of-observed-seven-context-repair-with-contingency"
    }
  },
  ...
  repairContract: {
    repairType: "critique-word-boundary",
    writableFields: POST_CANARY_BATCH_02_DEBATE_103_REPAIR_FIELDS,
  ...
  deterministicValidation: {
    ...
    nineBoundedFieldSchemasReproducedAtFreeze: true,
  ...
  artifacts: {
    ...
    execution: EXECUTION,
    analysis: ANALYSIS,
    originalFailedOutput: POST_CANARY_BATCH_02_DEBATE_103_BASE_OUTPUT,
  ...
  nextAuthorizedAction:
    "activate-and-execute-exactly-nine-frozen-debate-103-publication-repair-contexts-under-standing-authorization"
};
