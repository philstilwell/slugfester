...
import {
  buildBatch08TokenCountedChunkLedger,
  findBatch08ZeroLexicalTokenRows,
} from "./lib/assessment-production-post-canary-batch-08-source-preparation.mjs";
import {
...
const PROTOCOL_ID = "assessment-production-post-canary-batch-08-source-preparation";
const CACHE_VERSION = "post-canary-continuation-v1-batch-08";
...
const SCRIPT = "scripts/prepare-assessment-production-post-canary-batch-08-source-packets.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-08-source-packets.mjs";
...
assertV4(
  selection.status === "eighth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    selectionAnalysis.status === "eighth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization" &&
    selectionAnalysis.selection.sha256 === sha256(selectionBytes) &&
    selection.nextAuthorizedAction === "prepare-batch-08-standing-authorization-and-source-packets-under-user-authorization" &&
    JSON.stringify(selection.selected.map((item) => item.debateNumber)) === JSON.stringify(REQUIRED_ORDER),
  "Batch 8 frozen selection is not ready for its separately authorized source-packet step"
);
assertV4(
  ...
  "Batch 8 model boundary or stop rules drifted"
);
assertV4(
...
for (const selected of selection.selected) {
  ...
  packet.schemaVersion = "1.0-assessment-production-post-canary-batch-08-score-blind-source-packet";
  ...
  for (const chunk of plan.chunks) {
    ...
    const tokenLedgerBytes = buildBatch08TokenCountedChunkLedger(chunkBytes);
    const zeroLexicalTokenRows = findBatch08ZeroLexicalTokenRows(chunkBytes);
...
const preparation = {
  ...
  status: shouldWrite ? "post-canary-batch-08-ten-complete-score-blind-source-packets-prepared-awaiting-validation" : "preview",
  ...
  AIOnly: true,
  userAuthorization: {
    scope: "prepare, validate, freeze, commit, and push score-blind source packets for Batch 8 under the complete-workflow standing authorization",
  ...
  nextAuthorizedAction: "run-batch-08-deterministic-source-packet-validation-model-free-only",
};
