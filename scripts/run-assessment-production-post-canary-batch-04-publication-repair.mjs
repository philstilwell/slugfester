...
assertV4(
  ...
  "the Debate 127 repair execution is not authorized"
);
assertV4(
  ...
  "the Debate 127 repair execution controls changed"
);
...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(os.tmpdir(), `batch-04-debate-127-repair-${context.contextIndex}-`)
  );
  const codexHome = await mkdtemp(
    path.join(os.tmpdir(), `batch-04-debate-127-repair-home-${context.contextIndex}-`)
  );
  ...
  try {
    ...
    const prompt = [
      ...
      `Act only as isolated Debate 127 bounded publication repair editor ${context.contextIndex}.`,
    ...
    process.stdout.write(
      `[batch-04-debate-127-publication-repair] starting index ${context.contextIndex} ${activation.model.label}/${activation.model.reasoningEffort}\n`
    );
    ...
    if (
      ...
      try {
        validationSummary = validateDebate127RepairOutput(
      ...
      const validation = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-04-debate-127-publication-repair-validation",
      ...
      const provenance = {
        schemaVersion:
          "1.0-assessment-production-post-canary-batch-04-debate-127-publication-repair-provenance",
        protocolId: activation.protocolId,
        debateNumber: "127",
        packetIndex: context.packetIndex,
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-debate-127-publication-repair-model-execution",
  protocolId: activation.protocolId,
  status:
    validContexts === 2
      ? "batch-04-debate-127-two-context-publication-repair-gate-passed"
      : "batch-04-debate-127-publication-repair-gate-complete-with-failure",
  gateStartedAt,
  ...
  authorization: {
    ...
    deterministicMergeAndCompleteValidation: validContexts === 2,
    retry: false,
