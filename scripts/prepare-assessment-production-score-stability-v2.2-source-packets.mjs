...
const CACHE_VERSION =
  "assessment-production-score-stability-v2.2-fresh-validation";
...
const SCRIPT =
  "scripts/prepare-assessment-production-score-stability-v2.2-source-packets.mjs";
const TEST =
  "scripts/test-assessment-production-score-stability-v2.2-source-packets.mjs";
const SOURCE_FILES = [
  SELECTION,
...
assertV4(
  ...
  "v2.2 selection does not authorize model-free source preparation"
);
assertV4(
  ...
  "frozen v2.2 model, isolation, or stop-rule boundary drifted"
);
assertV4(
  selection.policy.version === "v2.2-proposal" &&
    selection.policy.agreedWinningSideMayCollapseToIntegerRoundedTie === true &&
    selection.policy.agreedInitialTieImposesNoDirectionConstraint === true &&
    selection.policy.numericalThresholdsChanged === false &&
  ...
  "v2.2 proposal or inherited workflow boundary drifted"
);
assertV4(
...
const preparation = {
  schemaVersion:
    "1.0-score-stability-v2.2-fresh-validation-source-preparation",
  ...
  inheritedInventorySuccessorContract: structuredClone(
  ...
  nextAuthorizedAction:
    "freeze-v2.2-discovery-execution-manifest-model-free-only",
};
  ...
  JSON.stringify(
    {
      ...
      debates: contexts.map((context) => ({
        ...
        maximumCopiedInputKilobytes: Math.round(
          Math.max(...context.chunks.map((chunk) => chunk.copiedInputBytes)) / 1000
        ),
      })),
