...
import { POST_CANARY_BATCH_01_DISPUTE_ADJ_ROOT } from "./lib/assessment-production-post-canary-batch-01-dispute-adjudication.mjs";
...
const activationPath =
  `${POST_CANARY_BATCH_01_DISPUTE_ADJ_ROOT}/execution-activation.json`;
const executionPath =
  `${POST_CANARY_BATCH_01_DISPUTE_ADJ_ROOT}/model-execution.json`;
const analysisPath = `${POST_CANARY_BATCH_01_DISPUTE_ADJ_ROOT}/analysis.json`;
...
assertV4(
  preparation.status ===
    ...
    preparation.totals.disputedMoves === 169 &&
    preparation.totals.candidateSelections === 461 &&
  ...
  "Batch 1 adjudication packets are not ready for execution preparation"
);
assertV4(
  preparation.userAuthorization.audioCostEstimateAcknowledgedUsd ===
      0.1190425 &&
    ...
    preparation.priorCostControl.acknowledged === true &&
    preparation.priorCostControl.additionalPaidCallsThisStage === 0,
  "Batch 1 audio cost acknowledgement changed"
);
assertV4(
  ...
  "Batch 1 adjudication model or score-blind boundary changed"
);
assertV4(
  preparation.totals.maximumCopiedInputBytes <= 350000,
  "Batch 1 adjudication context exceeds the frozen transport ceiling"
);
assertV4(
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-01-dispute-only-adjudication-execution-preparation-manifest",
  protocolId: preparation.protocolId,
  status:
    "frozen-ten-post-canary-batch-01-dispute-only-adjudication-contexts-prepared-not-authorized",
  frozenAt,
  activatedAt: null,
  ...
  priorCostControl: structuredClone(preparation.priorCostControl),
  ...
  nextAuthorizedAction:
    "user-approval-required-before-batch-01-dispute-only-adjudication-activation-or-any-model-execution"
};
