...
import { POST_CANARY_BATCH_08_DEBATE_08_REPAIR_PROTOCOL_ID,
  POST_CANARY_BATCH_08_DEBATE_08_REPAIR_ROOT } from
...
assertV4(p.protocolId === POST_CANARY_BATCH_08_DEBATE_08_REPAIR_PROTOCOL_ID &&
  p.status === "frozen-one-context-batch-08-debate-08-publication-repair-prepared-not-activated" &&
  p.contexts?.length === 1 && canonicalJson(p.contexts.map((row) => row.writableFieldCount)) === canonicalJson([2]) &&
...
"the Debate 08 repair is not prepared");
...
const activation = { schemaVersion: "1.0-assessment-production-post-canary-batch-08-debate-08-repair-activation",
  protocolId: p.protocolId, status: "frozen-one-context-batch-08-debate-08-publication-repair-authorized",
  ...
  authorization: { repairModelExecution: true, deterministicOutputValidation: true,
    deterministicMergeAndValidation: true, sixContextResumptionPreparation: true,
  ...
  nextAuthorizedAction: "execute-exactly-one-frozen-debate-08-repair-context" };
...
console.log(JSON.stringify({ status: activation.status, contextsAuthorized: 1,
  writableFieldsAuthorized: 2, model: activation.model, attemptsPerContext: 1,
