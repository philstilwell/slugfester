...
import {
  POST_CANARY_BATCH_11_STANDING_AUTHORIZATION,
  loadAndValidatePostCanaryBatch11StandingAuthorization,
} from "./lib/assessment-production-post-canary-batch-11-standing-authorization.mjs";

const ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-11/independent-judgments";
const RECOVERY = `${ROOT}/recovery-1/debate-60-pass-b`;
...
const ORIGINAL_PACKET = `${ROOT}/judgment-packets/pass-b/debate-60.json`;
const ORIGINAL_SCHEMA = `${ROOT}/schemas/pass-b/debate-60.schema.json`;
const MERGED_JUDGMENT = `${ROOT}/judgments/pass-b/debate-60.json`;
const MERGED_RAW = `${ROOT}/raw-outputs/pass-b/debate-60.json`;
const MERGED_VALIDATION = `${ROOT}/validations/pass-b/debate-60.json`;
const MERGED_PROVENANCE = `${ROOT}/provenance/pass-b/debate-60.json`;
...
const SCRIPT =
  "scripts/recover-assessment-production-post-canary-batch-11-debate-60-judgment-pass-b.mjs";
const RECOVERED_ANALYZER =
  "scripts/analyze-assessment-production-post-canary-batch-11-independent-judgments-recovered.mjs";
const VALIDATOR =
  "scripts/validate-assessment-production-post-canary-batch-11-independent-judgment.mjs";
const STANDING_LIBRARY =
  "scripts/lib/assessment-production-post-canary-batch-11-standing-authorization.mjs";
const DEBATE_NUMBER = "60";
const REVIEWER_PASS = "B";
const FAILED_CONTEXT_INDEX = 13;
const SHARD_IDS = ["moves-1", "moves-2-burden"];
const PREPARATION_STATUS =
  "two-field-disjoint-debate-60-pass-b-judgment-recovery-shards-prepared-not-authorized";
const ACTIVATION_STATUS =
  "two-field-disjoint-debate-60-pass-b-judgment-recovery-shards-authorized";
const EXECUTION_STATUS =
  "two-field-disjoint-debate-60-pass-b-judgment-recovery-shards-passed-merged-and-validated";
...
function buildShardSchema(fullSchema, moveIds, includeBurdenAdjustment, shardId) {
  ...
  return {
    ...
    title: `SLUGFESTER Batch 11 Debate 60 Pass B ${shardId} recovery shard`,
    type: "object",
    additionalProperties: false,
    required,
    properties,
...
async function buildPreparation({ frozenAt, checkpointCommit }) {
  const standingAuthorization =
    await loadAndValidatePostCanaryBatch11StandingAuthorization();
  const [
  ...
  assertV4(
    originalPreparation.status ===
      "twenty-post-canary-batch-11-independent-judgment-contexts-prepared-and-frozen" &&
      originalActivation.status ===
        "frozen-twenty-post-canary-batch-11-independent-judgment-contexts-authorized" &&
      originalExecution.status ===
        "post-canary-batch-11-independent-judgment-gate-complete-with-failure" &&
      originalExecution.contextsAttempted === 20 &&
      originalExecution.validContexts === 19 &&
      ...
      !(await exists(STANDARD_ANALYSIS)),
    "preserved Debate 60 Pass B timeout boundary drifted"
  );
  assertV4(
    ...
    "Batch 11 bounded recovery authorization drifted"
  );
  ...
  assertV4(moveIds.length === 20, "Debate 60 Pass B move count drifted");
  const shardDefinitions = [
    {
      shardId: SHARD_IDS[0],
      moveIds: moveIds.slice(0, 10),
      includeBurdenAdjustment: false,
    },
    {
      shardId: SHARD_IDS[1],
      moveIds: moveIds.slice(10),
      includeBurdenAdjustment: true,
    },
  ];
  assertV4(
    new Set(shardDefinitions.flatMap((shard) => shard.moveIds)).size === 20 &&
  ...
  for (const [contextIndex, shard] of shardDefinitions.entries()) {
    ...
    const packet = {
      schemaVersion:
        "1.0-assessment-production-post-canary-batch-11-debate-60-pass-b-judgment-recovery-shard-packet",
      recoveryId: "batch-11-debate-60-pass-b-judgment-recovery-1",
      recoveryLevel: 1,
  ...
  assertV4(
    ...
    "recovery copied-input boundary failed"
  );
  ...
  const mergeRule = {
    ruleId: "batch-11-debate-60-pass-b-judgment-recovery-1-field-disjoint-merge-v1",
  ...
  const diagnosis = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-11-debate-60-pass-b-judgment-timeout-diagnosis",
    status:
      "preserved-debate-60-pass-b-timeout-diagnosed-output-size-time-failure-no-partial-output-accepted",
    frozenAt,
    checkpointCommit,
    debateNumber: DEBATE_NUMBER,
    reviewerPass: REVIEWER_PASS,
    ...
    deterministicDiagnosis: {
      ...
      rationale:
        "The full 20-move judgment emitted substantial generation evidence but did not produce a result file within 15 minutes. Two fresh 10-move shards are the minimum output-size reduction; the debate-wide burden adjustment is assigned only to the second shard.",
    },
    schedulerAudit: {
      ...
      implementationDefect:
        "The steady-phase worker loop evaluated failure only after the entire phase, so contexts 18 and 19 began after context 13 failed.",
      postFailureOutputsQuarantinedPendingRecovery: true,
      isolationAllowsByteIdenticalPreservationAfterCompleteCohortReplay: true,
    },
    preservedAcceptedCohort: {
      acceptedJudgments: 19,
      protectedOutputHashes,
      acceptedOutputsMustRemainByteIdentical: true,
    },
    directIncrementalCostUsd: 0,
    nextAuthorizedAction:
      "freeze-two-minimum-field-disjoint-debate-60-pass-b-recovery-contexts",
  };
  ...
  const correction = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-11-debate-60-pass-b-judgment-recovery-correction-plan",
  ...
  const preparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-11-debate-60-pass-b-judgment-recovery-preparation",
    ...
    reviewerPass: REVIEWER_PASS,
    recoveryLevel: 1,
    standingAuthorization: POST_CANARY_BATCH_11_STANDING_AUTHORIZATION,
    ...
    directIncrementalCostUsdMaximum: 0,
    nextAuthorizedAction:
      "activate-two-batch-11-debate-60-pass-b-field-disjoint-recovery-shards",
  };
...
async function prepare() {
  ...
  console.log(JSON.stringify({
    ...
    originalAcceptedJudgmentsPreserved: 19,
    failedPartialOutputReusable: false,
    attemptsPerShard: 1,
    retriesMaximum: 0,
    timeoutExtensionsMaximum: 0,
  ...
  assertV4(
    preparation.status === PREPARATION_STATUS &&
      ...
      preparation.contexts.map((context) => context.shardId).join(",") ===
        SHARD_IDS.join(",") &&
      preparation.contexts.every((context) => context.assignedMoveIds.length === 10) &&
      preparation.contexts.filter((context) => context.burdenCompletionAdjustmentAssigned).length === 1 &&
      preparation.standingAuthorization === POST_CANARY_BATCH_11_STANDING_AUTHORIZATION &&
...
async function validate() {
  ...
  console.log(JSON.stringify({
    status: "passed-frozen-batch-11-debate-60-pass-b-judgment-recovery-preparation",
...
async function activate() {
  ...
  const activation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-11-debate-60-pass-b-judgment-recovery-activation",
    ...
    directIncrementalCostUsdMaximum: 0,
    nextRequiredAction:
      "execute-two-batch-11-debate-60-pass-b-field-disjoint-recovery-shards-once",
  };
...
async function executeRecovery() {
  ...
  for (const future of [EXECUTION, ANALYSIS, COHORT_OVERLAY, MERGED_JUDGMENT, MERGED_RAW, MERGED_VALIDATION, MERGED_PROVENANCE, STANDARD_ANALYSIS, ...activation.contexts.map((context) => context.output)]) {
    assertV4(!(await exists(future)), `future output already exists: ${future}`);
  }
  ...
  for (const context of activation.contexts) {
    ...
    const temporary = await mkdtemp(
      path.join(os.tmpdir(), `slugfester-batch-11-debate-60-pass-b-${context.shardId}-`)
    );
    const isolatedCodexHome = await mkdtemp(
      path.join(os.tmpdir(), `slugfester-batch-11-debate-60-pass-b-home-${context.shardId}-`)
    );
    ...
    results.push(record);
    process.stdout.write(`[batch-11-judgment-recovery] ${context.shardId} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  }
  ...
  const execution = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-11-debate-60-pass-b-judgment-recovery-execution",
    status: passed ? EXECUTION_STATUS : "debate-60-pass-b-judgment-recovery-failed",
    activation: ACTIVATION,
  ...
  const overlay = {
    ...originalExecution,
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-11-independent-judgment-recovered-cohort-execution-overlay",
    status:
      "twenty-post-canary-batch-11-independent-judgment-contexts-passed-after-bounded-field-disjoint-recovery",
    ...
    recovery: {
      ...
      originalAcceptedJudgmentsPreservedByteIdentical: 19,
      failedPartialOutputUsed: false,
      schedulerDefectRecorded: true,
      postFailureContextsReplayedAndPreserved: [18, 19],
  ...
  const analysis = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-11-debate-60-pass-b-judgment-recovery-analysis",
    status:
      "debate-60-pass-b-bounded-field-disjoint-recovery-passed-complete-cohort-replay-required",
    recoveryLevel: 1,
    originalExecution: ORIGINAL_EXECUTION,
    ...
    moveFieldsAcceptedExactlyOnce: 20,
    ...
    scoresDerived: 0,
    nextAuthorizedAction:
      "replay-and-freeze-complete-batch-11-independent-judgment-analysis",
  };
  ...
  console.log(JSON.stringify({
    ...
    burdenCompletionAdjustmentAcceptedExactlyOnce: true,
    originalFailedPartialOutputUsed: false,
    protectedAcceptedJudgmentsByteIdentical: 19,
    completeMergedJudgmentValidated: true,
    totalModelContextsExecuted: 22,
    retries: 0,
    timeoutExtensions: 0,
...
if (command === "prepare") await prepare();
  ...
  throw new Error(
    "usage: recover-assessment-production-post-canary-batch-11-debate-60-judgment-pass-b.mjs <prepare|validate|activate|run>"
  );
}
