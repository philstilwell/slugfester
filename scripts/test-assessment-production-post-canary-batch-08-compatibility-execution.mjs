...
const root = POST_CANARY_BATCH_08_COMPATIBILITY_ROOT;
...
const validator = await import(
  `./validate-debates.mjs?batch08Compatibility=${activation.validator.proposedSha256}`
);
...
for (const lock of activation.packetHashes) {
  ...
  const routeLock =
    validator.validatePostCanaryBatch08LedgerAdapterRouteLocks({
      debate: candidate,
      ledgerText,
  ...
  const route = validator.validatePostCanaryBatch08LedgerAdapterRoute({
    debate: candidate,
    ledger,
    ledgerText
  });
  assert.equal(route.repositoryScoreReplayPassed, true);
  const adapter = validatePostCanaryBatch08SiteLedgerAdapter({
    adapter: ledger,
    candidate,
...
assert.equal(oneSidedDisplayRows, 14);
assert.equal(emptyReferenceLinks, 55);
assert.equal(suppliedReferenceLinks, 4);
assert.equal(
  ...
  JSON.stringify(
    {
      ...
      positiveRoutes,
      negativeControls,
      existingAdapterRoutesPreserved: 6,
      modelContexts: 0,
      paidServiceCalls: 0,
