...
const MANIFEST =
  "docs/assessment-production/score-stability-v2.1.3-validation-cohort/independent-judgments/execution-preparation-manifest.json";
...
const sourcePreparation = JSON.parse(
  await readFile(
    "docs/assessment-production/score-stability-v2.1.3-validation-cohort/source-preparation/preparation-manifest.json",
    "utf8"
  )
);
...
assert.equal(
  manifest.failedGateDisposition.v212FailedOutputsUsedForSuccessorAcceptance,
  false
);
assert.equal(
...
assert.equal(preparation.totals.maximumCopiedInputBytes, 109180);
