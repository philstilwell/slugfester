...
const root = POST_CANARY_BATCH_14_COMPATIBILITY_ROOT;
...
const validator = await import(
  `./validate-debates.mjs?batch14Compatibility=${activation.validator.proposedSha256}`
);
...
for (const lock of activation.packetHashes) {
  ...
  const routeLock =
    validator.validatePostCanaryBatch14LedgerAdapterRouteLocks({
      debate: candidate,
      ledgerText,
  ...
  const route = validator.validatePostCanaryBatch14LedgerAdapterRoute({
    debate: candidate,
    ledger,
    ledgerText
  });
  assert.equal(route.repositoryScoreReplayPassed, true);
  const adapter = validatePostCanaryBatch14SiteLedgerAdapter({
    adapter: ledger,
    candidate,
...
assert.equal(emptyReferenceLinks, 45);
assert.equal(suppliedReferenceLinks, 10);
assert.equal(
  ...
  JSON.stringify(
    {
      ...
      positiveRoutes,
      negativeControls,
      existingAdapterRoutesPreserved: 14,
      modelContexts: 0,
      paidServiceCalls: 0,
