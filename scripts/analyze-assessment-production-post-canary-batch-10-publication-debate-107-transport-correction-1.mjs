...
assertV4(
  execution.attemptCount === 1 && execution.retryCount === 0 &&
    ...
    execution.modelAuthoredScores === 0 && execution.newContextNotAutomaticRetry === true &&
    execution.priorTransportOutputReused === false,
  "transport correction execution record changed"
);
...
if (execution.status === "completed-valid" && execution.gateAcceptancePassed) {
  try {
    ...
    assertV4(sha256(correctionBytes) === execution.outputSha256,
      "transport correction output hash changed");
    assertV4(
    ...
    correctionValidation = validateTransportCorrectionOutput(correction, packet);
    ...
    for (const target of TARGETS) {
      ...
      const replacement = correction.corrections[target.itemId];
...
const analysis = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-debate-107-transport-correction-analysis",
  protocolId: activation.protocolId,
  status: passed
    ? "batch-10-debate-107-schema-corrected-three-field-publication-correction-passed-awaiting-six-context-resumption"
    : "batch-10-debate-107-schema-corrected-three-field-publication-correction-failed",
  ...
  gate: {
    ...
    schemaCorrectionApplied: true,
    priorTransportAttemptProducedModelOutput: false,
...
if (shouldWrite) {
  assertV4(!(await exists(activation.artifacts.analysis)), "transport correction analysis already exists");
  if (passed) {
    ...
    const acceptedValidation = {
      schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-validation-after-transport-corrected-field-correction",
      protocolId: activation.protocolId,
      status: "passed",
      debateNumber: "107",
    ...
    const acceptedProvenance = {
      schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-provenance-after-transport-corrected-field-correction",
      protocolId: activation.protocolId,
      debateNumber: "107",
      debateId: corrected.debateId,
      ...
      failedTransportCorrection: {
        sourceExecution: activation.priorTransportFailure.execution,
        sourceExecutionSha256: activation.priorTransportFailure.executionSha256,
        modelOutputGenerated: false,
        outputReused: false
      },
      acceptedTransportCorrection: {
    ...
    await writeFile(path.resolve(activation.artifacts.completeValidation),
      `${JSON.stringify({
        schemaVersion: "1.0-assessment-production-post-canary-batch-10-debate-107-transport-correction-complete-validation",
        protocolId: activation.protocolId,
        status: "passed",
        debateNumber: "107",
        correctedFields: TARGETS.map((target) => target.field),
    ...
    await writeFile(path.resolve(activation.artifacts.mergeAudit),
      `${JSON.stringify({
        schemaVersion: "1.0-assessment-production-post-canary-batch-10-debate-107-transport-correction-merge-audit",
        protocolId: activation.protocolId,
        status: "passed",
        debateNumber: "107",
        sourceFailedOutput: {
        ...
        failedTransportCorrection: activation.priorTransportFailure,
        acceptedCorrection: {
          path: activation.context.output,
          sha256: execution.outputSha256,
