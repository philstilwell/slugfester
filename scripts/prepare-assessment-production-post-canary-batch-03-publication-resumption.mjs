...
const RESUMPTION_ROOT = POST_CANARY_BATCH_03_PUBLICATION_RESUMPTION_ROOT;
...
const REPAIRED_DEBATE_124 = `${ROOT}/repair-1/merged/debate-124.json`;
const DEBATE_124_PACKET = `${ROOT}/packets/debate-124.json`;
...
const repairedDebate124 = value(REPAIRED_DEBATE_124);
const debate124Packet = value(DEBATE_124_PACKET);
...
assertV4(
  originalExecution.status ===
    ...
    canonicalJson(diagnosis.rampDisposition.unattemptedDebates) ===
      canonicalJson(POST_CANARY_BATCH_03_PUBLICATION_RESUMPTION_DEBATES),
...
assertV4(
  repairPreparation.status ===
    ...
    repairExecution.status ===
      "batch-03-debate-124-two-context-publication-repair-gate-passed" &&
    ...
    repairAnalysis.status ===
      "batch-03-debate-124-bounded-repair-and-complete-publication-validation-passed" &&
    ...
    repairValidation.validationSummary?.moves === 24 &&
  ...
  "the accepted Debate 124 repair boundary changed"
);
assertV4(
  sha256(loaded[REPAIRED_DEBATE_124].bytes) ===
      repairValidation.mergedOutputSha256 &&
    validatePostCanaryBatch03PublicationOutput(
      repairedDebate124,
      debate124Packet
    ).status === "passed",
  "the repaired Debate 124 staging output failed replay"
);
...
for (
  ...
  resumptionIndex < POST_CANARY_BATCH_03_PUBLICATION_RESUMPTION_DEBATES.length;
  resumptionIndex += 1
) {
  const debateNumber =
    POST_CANARY_BATCH_03_PUBLICATION_RESUMPTION_DEBATES[resumptionIndex];
...
const newStaticSources = [
  ...
  "scripts/prepare-assessment-production-post-canary-batch-03-publication-resumption.mjs",
  "scripts/test-assessment-production-post-canary-batch-03-publication-resumption-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-03-publication-resumption.mjs",
  "scripts/run-assessment-production-post-canary-batch-03-publication-resumption.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-03-publication-resumption.mjs"
];
...
const stopRules = {
  acceptedDebate124RepairFailureBlocks: true,
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-publication-resumption-execution-preparation-manifest",
  protocolId: POST_CANARY_BATCH_03_PUBLICATION_RESUMPTION_PROTOCOL_ID,
  status:
  ...
  inputs: {
    ...
    repairedDebate124: REPAIRED_DEBATE_124,
    debate124Packet: DEBATE_124_PACKET,
    standingAuthorization: POST_CANARY_BATCH_03_STANDING_AUTHORIZATION,
    failureRecoveryStandingAuthorization: RECOVERY_AUTHORIZATION
  },
  ...
  acceptedDebate124: {
    ...
    packet: DEBATE_124_PACKET,
    output: REPAIRED_DEBATE_124,
  ...
  isolation: {
    ...
    repairedDebate124OutputUnavailableToResumptionModels: true,
    failedOriginalDebate124OutputUnavailable: true,
  ...
  deterministicValidation: {
    ...
    acceptedDebate124RepairReplayedAtFreeze: true,
  ...
  acceptanceContract: {
    ...
    resumptionMovesRequired: 176,
    cohortMovesRequired: 200,
    resumptionCritiquesRequired: 176,
  ...
  nextAuthorizedAction:
    "activate-and-execute-exactly-nine-frozen-batch-03-publication-resumption-contexts-under-failure-recovery-standing-authorization"
};
