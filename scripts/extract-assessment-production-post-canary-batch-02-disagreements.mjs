...
const USER_AUTHORIZATION = Object.freeze({
  instruction:
    "I approve deterministic extraction, validation, freezing, analysis, committing, and pushing of the Batch 2 disagreements from the twenty accepted independent-judgment outputs only, with a direct incremental cost cap of $0. Do not execute models, paid services, audio work, adjudication, score derivation, publication reconstruction, production mutation, or next-batch selection.",
  directIncrementalCostUsdMaximum: 0,
  acceptedIndependentJudgmentOutputsOnly: true,
  modelExecutionAuthorized: false,
  paidServicesAuthorized: false,
  audioWorkAuthorized: false,
...
const TOOL_SOURCES = [
  "scripts/extract-assessment-production-post-canary-batch-02-disagreements.mjs",
  ...
  "scripts/test-assessment-production-post-canary-batch-02-disagreements.mjs"
];
...
assertV4(
  judgmentAnalysis.status ===
    ...
    judgmentAnalysis.nextAuthorizedAction ===
      "user-approval-required-before-batch-02-deterministic-disagreement-extraction",
  "Batch 2 accepted-judgment gate or prior stop rule changed"
);
assertV4(
  JSON.stringify(judgmentAnalysis.pairs.map((pair) => pair.debateNumber)) ===
    JSON.stringify(EXPECTED_DEBATES),
  "Batch 2 independent-judgment population changed"
);
assertV4(
  ...
  "Batch 2 does not contain ten accepted, independent A/B judgment pairs"
);
assertV4(
  ...
  "Batch 2 twenty-context accepted execution contract changed"
);
assertV4(
  ...
  "Batch 2 model, authentication, score-blindness, or tie contract changed"
);
assertV4(
  executionActivation.executionPolicy.attemptsPerContext === 1 &&
    ...
    JSON.stringify(executionActivation.executionPolicy.schedulerRamp) ===
      JSON.stringify([1, 2]),
  "Batch 2 one-attempt or frozen 1-to-2 scheduler stop rule changed"
);
assertV4(
...
assertV4(
  judgmentAnalysis.sourceCompatibility?.status ===
    ...
    judgmentAnalysis.sourceCompatibility.occurrences[0]?.debateNumber === "99",
...
assertV4(
  ...
  "Batch 2 accepted execution evidence changed"
);
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const disagreementPath =
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_02_DISAGREEMENT_ROOT}/disagreements/debate-${debateNumber}.json`;
...
assertV4(totalMoves === 190, "Batch 2 extraction must compare 190 moves");
assertV4(
  JSON.stringify(extractedAudioQueue.map(queueKey).sort()) ===
    JSON.stringify(independentGateQueue.map(queueKey).sort()),
  "Batch 2 ten-move audio queue was not reproduced"
);

const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-disagreement-extraction",
  protocolId:
    ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_02_DISAGREEMENT_PROTOCOL_ID,
  status:
...
if (shouldWrite) {
  await mkdir(ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_02_DISAGREEMENT_ROOT, {
    recursive: true
  });
  await writeFile(
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_02_DISAGREEMENT_ROOT}/analysis.json`,
