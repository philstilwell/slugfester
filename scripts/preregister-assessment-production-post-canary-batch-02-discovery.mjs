...
const PROTOCOL_ID = "assessment-production-post-canary-batch-02-discovery";
const WORKFLOW = "docs/assessment-production-canary-discovery-workflow.md";
const SCRIPT = "scripts/preregister-assessment-production-post-canary-batch-02-discovery.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-02-discovery-manifest.mjs";
...
assertV4(
  preparation.status === "post-canary-batch-02-ten-complete-score-blind-source-packets-prepared-awaiting-validation" &&
    preparationValidation.status === "post-canary-batch-02-score-blind-source-packet-validation-passed-frozen-awaiting-user-approval-before-any-model-execution" &&
    ...
    preparation.totals.maximumCopiedInputBytes === 68003 &&
  ...
  "validated Batch 2 source preparation is not frozen at the required authorization boundary"
);
assertV4(
  ...
  for (const chunk of debate.chunks) {
    ...
    assertV4(
      sha256(chunkBytes) === chunk.chunkLedgerSha256 &&
        ...
        buildBatch02TokenCountedChunkLedger(chunkBytes).equals(tokenBytes) &&
...
assertV4(contexts.length === 36, "Batch 2 discovery must contain exactly 36 contexts");
...
assertV4(maximumContext.debateNumber === "136" && maximumContext.chunkId === "chunk-002", "maximum copied-input context drifted");
...
const manifest = {
  ...
  status: shouldWrite ? "frozen-thirty-six-post-canary-batch-02-discovery-contexts-prepared-not-authorized" : "preview",
  ...
  AIOnly: true,
  userAuthorization: {
    scope: "prepare, validate, freeze, commit, and push the Batch 2 discovery-execution manifest only",
  ...
  costEstimate: {
    ...
    estimateBasis: "The promoted production-canary estimate covers the same 36-context scheduler shape; Batch 2's largest exact copied input remains below the historical 70,000-byte boundary. Subscription use has no direct incremental API charge.",
  },
  ...
  stageBoundary: {
    ...
    discoveryExecutionActivationPreparation: "not-authorized",
  ...
  nextAuthorizedAction: "user-approval-required-before-batch-02-discovery-execution-activation-or-any-model-execution",
};
