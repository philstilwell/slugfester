...
const RECOVERY_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-04/dispute-only-adjudication/failure-recovery/debate-185-correction";
...
assertV4(
  ...
  "Batch 4 dispute-only adjudication execution is unauthorized"
);
...
async function runContext(context) {
  const temporary = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-post-canary-batch-04-debate-185-adjudication-correction-${context.debateNumber}-`
    )
  );
  const codexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-post-canary-batch-04-debate-185-adjudication-correction-home-${context.debateNumber}-`
    )
  );
  ...
  try {
    ...
    }; read nothing else. Act only as the isolated, score-blind, disputed-fields-only adjudicator for post-canary Batch 4 Debate ${
      context.debateNumber
    }. This is a fresh full-packet correction; do not use or infer any failed prior response. Decide every required anonymous candidate pair and scoring field exactly once from the locked evidence. Return exactly 18 moveDecisions in packet order and exactly 2 burdenAdjustmentDecisions in packet order, first pro and then con. Candidate ordering may reverse independently for every field. Select only candidate 1 or candidate 2. Never mix, average, interpolate, repair, rewrite, or invent a candidate. Never infer either initial pass identity or rationale. Never calculate a move, section, side, or debate score. Never produce a winner, legacy assessment, Overall Commentary, AI Extension, or publication prose. Return exactly one schema-conforming JSON object.`;
    process.stdout.write(
      `[batch-04-debate-185-correction] starting index ${context.contextIndex} ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber}\n`
    );
    ...
    if (
      ...
      try {
        validation = validatePostCanaryBatch04DisputeAdjudicationOutput(
  ...
  process.stdout.write(
    `[batch-04-debate-185-correction] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-04-dispute-only-adjudication-model-execution",
