...
const SCRIPT =
  "scripts/prepare-assessment-production-post-canary-batch-03-inventory.mjs";
const PROTOCOL_ID =
  "assessment-production-post-canary-batch-03-candidate-sharded-inventory";
const SOURCE_PACKET_VERSION =
  "1.0-assessment-production-post-canary-batch-03-candidate-sharded-inventory-source-packet";
...
const EXPECTED = {
  ...
  proCandidates: 176,
  conCandidates: 184,
  belowHighAttributionCandidates: 4,
};
...
function buildInventorySourcePacket(sourcePacket) {
  assertV4(
    sourcePacket?.schemaVersion ===
      "1.0-assessment-production-post-canary-batch-03-score-blind-source-packet" &&
      sourcePacket.protocolId ===
        "assessment-production-post-canary-batch-03-source-preparation" &&
    ...
    "Batch 3 source packet is unavailable or not dyadic"
  );
  return {
...
function makePlanPacket({
  ...
  return {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-03-candidate-census-plan-packet",
    protocolId: PROTOCOL_ID,
...
async function buildFrozenArtifacts(frozenAt) {
  ...
  assertV4(
    ...
    "Batch 3 standing authorization does not cover exact inventory preparation and execution"
  );

  assertV4(
    discoveryAnalysis.status ===
      "post-canary-batch-03-discovery-passed-standing-authorization-active-for-inventory-preparation" &&
      ...
      discoveryAnalysis.audit?.validContexts === 38 &&
      ...
      discoveryAnalysis.totals?.modelContextsExecuted === 38 &&
      ...
      discoveryAnalysis.nextAuthorizedAction ===
        "prepare-freeze-and-activate-batch-03-candidate-census-planner-contexts-under-standing-authorization",
    "passed Batch 3 discovery boundary drifted"
  );
  assertV4(
    discoveryExecution.status ===
      "thirty-eight-post-canary-batch-03-discovery-contexts-passed" &&
      discoveryExecution.contextsAttempted === 38 &&
      discoveryExecution.validContexts === 38 &&
    ...
    "Batch 3 discovery execution drifted"
  );
  assertV4(
      sourcePreparation.status ===
      "post-canary-batch-03-ten-complete-score-blind-source-packets-prepared-awaiting-validation" &&
    ...
    "Batch 3 source preparation or model boundary drifted"
  );
  assertV4(
    sourcePreparationValidation.status ===
      "post-canary-batch-03-score-blind-source-packet-validation-passed-frozen-standing-authorization-active" &&
    ...
    "Batch 3 source validation drifted"
  );
  assertV4(
    selection.status ===
    ...
    "Batch 3 frozen selection drifted"
  );
  assertV4(
  ...
  for (const discovered of discoveryAnalysis.debates) {
    ...
    assertV4(
      ...
      `${discovered.debateNumber}: Batch 3 identity drifted`
    );
    const [
  ...
  assertV4(
    ...
    "Batch 3 totals or packet state drifted"
  );
  ...
  const preparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-03-candidate-sharded-inventory-preparation",
    protocolId: PROTOCOL_ID,
    sideSelectionProtocolId: CHRONOLOGY_FALLBACK_INVENTORY.protocolId,
    status:
      "post-canary-batch-03-candidate-sharded-source-assets-and-ten-planner-packets-frozen",
    preparedAt: frozenAt,
    ...
    userAuthorization: {
      scope:
        "prepare, freeze, activate, and execute the Batch 3 candidate-census and candidate-sharded side-selection workflow under the frozen standing authorization",
    ...
    nextAuthorizedAction:
      "freeze-and-activate-batch-03-candidate-census-plan-execution-under-standing-authorization",
  };
  ...
  const executionPreparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-03-candidate-census-plan-execution-preparation-manifest",
    protocolId: PROTOCOL_ID,
    status:
      "frozen-ten-post-canary-batch-03-candidate-census-plan-contexts-prepared-not-authorized",
    frozenAt,
  ...
  const validation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-03-candidate-sharded-inventory-preparation-validation",
    protocolId: PROTOCOL_ID,
    status:
      "batch-03-candidate-sharded-inventory-packets-and-plan-execution-manifest-validation-passed-frozen-standing-authorization-active",
...
async function validateFrozen() {
  ...
  assertV4(
    ...
    "frozen Batch 3 inventory preparation is invalid"
  );
...
if (command === "freeze") await freeze();
  ...
  throw new Error(
    "usage: prepare-assessment-production-post-canary-batch-03-inventory.mjs <freeze|validate>"
  );
}
