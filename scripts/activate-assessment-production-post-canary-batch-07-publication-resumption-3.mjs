...
import { POST_CANARY_BATCH_07_PUBLICATION_RESUMPTION_3_PROTOCOL_ID,
...
assertV4(p.protocolId === POST_CANARY_BATCH_07_PUBLICATION_RESUMPTION_3_PROTOCOL_ID &&
  p.status === "frozen-five-untouched-post-canary-batch-07-publication-resumption-3-contexts-prepared-not-authorized" &&
  ...
  p.executionPolicy?.contexts === 5 && p.executionPolicy?.attemptsPerContext === 1 &&
...
"the five-context Batch 7 publication resumption is not prepared");
...
const activation = { schemaVersion:
    "1.0-assessment-production-post-canary-batch-07-publication-resumption-3-execution-activation",
  protocolId: p.protocolId,
  status: "frozen-five-untouched-post-canary-batch-07-publication-resumption-3-contexts-authorized",
  ...
  nextAuthorizedAction: "execute-exactly-five-frozen-batch-07-publication-resumption-3-contexts-once" };
...
console.log(JSON.stringify({ status: activation.status,
  debates: activation.contexts.map((row) => row.debateNumber), contextsAuthorized: 5,
