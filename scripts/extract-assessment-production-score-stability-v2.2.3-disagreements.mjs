...
const TOOL_SOURCES = [
  "scripts/extract-assessment-production-score-stability-v2.2.3-disagreements.mjs",
  ...
  "scripts/test-assessment-production-score-stability-v2.2.3-disagreements.mjs"
];
...
assertV4(
  ...
  "v2.2.3 model, authentication, or score-blindness contract changed"
);
assertV4(
  executionActivation.executionPolicy.retriesMaximum === 0 &&
    executionActivation.executionPolicy.timeoutExtensionsMaximum === 0,
  "v2.2.3 retry or timeout-extension stop rule changed"
);
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const disagreementPath =
    `${ASSESSMENT_PRODUCTION_SCORE_STABILITY_V223_DISAGREEMENT_ROOT}/disagreements/debate-${debateNumber}.json`;
  if (shouldWrite) {
...
assertV4(totalMoves === 197, "v2.2.3 extraction must compare 197 moves");
assertV4(
...
const analysis = {
  schemaVersion: "1.0-score-stability-v2.2.3-disagreement-extraction",
  protocolId:
    ASSESSMENT_PRODUCTION_SCORE_STABILITY_V223_DISAGREEMENT_PROTOCOL_ID,
  status:
...
if (shouldWrite) {
  await mkdir(ASSESSMENT_PRODUCTION_SCORE_STABILITY_V223_DISAGREEMENT_ROOT, {
    recursive: true
  });
  await writeFile(
    `${ASSESSMENT_PRODUCTION_SCORE_STABILITY_V223_DISAGREEMENT_ROOT}/analysis.json`,
