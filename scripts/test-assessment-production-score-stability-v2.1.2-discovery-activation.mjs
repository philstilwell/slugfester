...
assert.equal(
  activation.failedGateDisposition.retiredOutputsAcceptedForSuccessorEvidence,
  false
);
assert.equal(activation.failedGateDisposition.currentCanaryReclassified, false);
assert.equal(activation.proposedPolicy.version, "v2.1-proposal");
...
assert.equal(activation.successorContract.thresholdRelaxed, false);
assert.equal(activation.successorContract.silentCandidateDeletion, false);
assert.equal(activation.successorContract.automaticTruncation, false);
assert.equal(activation.successorContract.automaticSemanticRepair, false);
assert.deepEqual(activation.successorContract.sourceSelectionShape, [
  "startEvent",
  "endEvent",
]);
assert.equal(activation.successorContract.modelAuthoredEndEvent, true);
assert.equal(
  activation.successorContract
    .modelAuthoredEndEventStructurallyBoundedByLockedContext,
  true
);
assert.equal(activation.successorContract.modelAuthoredLexicalTokenCount, false);
assert.equal(activation.successorContract.repositoryDerivedLexicalTokenCount, true);
assert.equal(activation.successorContract.requestedLexicalTokensRemoved, true);
...
assert.equal(activation.executionPolicy.contexts, 33);
...
assert.equal(activation.schemaHardening.minimumLexicalTokensSchemaEnforced, false);
...
assert.equal(execution.contextsPlanned, 33);
assert(execution.contextsAttempted >= 1 && execution.contextsAttempted <= 33);
assert.equal(execution.contextsUnattempted, 33 - execution.contextsAttempted);
...
for (const result of execution.results) {
  ...
  if (result.accepted) {
    assert.equal(
      result.validationSummary.repositoryDerivedLexicalTokenCounts,
      true
    );
    assert.equal(result.validationSummary.modelAuthoredLexicalTokenCounts, false);
    assert.equal(result.validationSummary.modelAuthoredBoundedEndEvents, true);
  }
}
...
assert.equal(execution.contextsAttempted, 33);
assert.equal(execution.contextsUnattempted, 0);
assert.equal(execution.validContexts, 33);
...
assert.equal(analysis.audit.validContexts, 33);
...
assert.equal(analysis.audit.modelAuthoredEndEventRequired, true);
assert.equal(analysis.audit.modelAuthoredEndEventLockedContextBounds, true);
assert.equal(analysis.audit.repositoryDerivedLexicalTokenCount, true);
assert.equal(analysis.audit.minimumLexicalTokens, 12);
assert.equal(
  analysis.audit.minimumLexicalTokensDeterministicallyEnforced,
  true
);
assert.equal(analysis.audit.minimumLexicalTokensSchemaEnforced, false);
...
assert.equal(analysis.audit.v1CanaryReclassified, false);
assert.equal(analysis.audit.v2ValidationReclassified, false);
assert.equal(analysis.audit.v21DiscoveryReclassified, false);
assert.equal(analysis.audit.v211DiscoveryReclassified, false);
...
for (const debate of analysis.debates) {
  ...
  assert(debate.con >= 4);
  assert.equal(
    debate.repositoryDerivedLexicalTokenCountWindows,
    debate.candidates
  );
  assert.equal(debate.modelAuthoredLexicalTokenCounts, false);
  assert.equal(debate.modelAuthoredBoundedEndEvents, debate.candidates);
...
assert.equal(analysis.totals.modelContextsExecuted, 33);
assert.equal(
  analysis.totals.repositoryDerivedLexicalTokenCountWindows,
  analysis.totals.candidates
);
assert.equal(analysis.totals.modelAuthoredLexicalTokenCounts, 0);
assert.equal(
  analysis.totals.modelAuthoredBoundedEndEvents,
  analysis.totals.candidates
);
