...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-11/discovery";
...
assert.equal(manifest.status, "frozen-thirty-nine-post-canary-batch-11-discovery-contexts-prepared-not-authorized");
...
assert.equal(manifest.executionPolicy.copiedInputBytesMaximum, 69824);
assert.equal(preparation.totals.maximumCopiedInputBytes, 69824);
...
assert.equal(manifest.copiedInputBoundary.frozenObservedCopiedInputBytesMaximum, 69824);
assert.equal(manifest.copiedInputBoundary.executionCopiedInputBytesMaximum, 69824);
assert.deepEqual(manifest.copiedInputBoundary.maximumContext, { debateNumber: "191", chunkId: "chunk-002" });
...
assert.deepEqual(manifest.costEstimate.expectedParallelWallMinutes, [12, 28]);
assert.deepEqual(manifest.costEstimate.expectedAggregateModelMinutes, [38, 75]);
...
for (const context of manifest.contexts) {
  ...
  assert(buildBatch11TokenCountedChunkLedger(chunkBytes).equals(tokenBytes));
...
assert.equal(manifest.nextAuthorizedAction, "prepare-and-freeze-batch-11-discovery-execution-activation-under-standing-authorization");

const validation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-discovery-execution-preparation-validation",
  protocolId: manifest.protocolId,
  status: "batch-11-discovery-execution-manifest-validation-passed-frozen-standing-authorization-active",
