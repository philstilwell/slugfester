...
assertV4(
  preparation.protocolId === PROTOCOL_ID &&
    preparation.status === "frozen-five-context-nine-field-batch-13-debate-34-publication-repair-prepared-not-activated" &&
    preparation.contexts?.length === 5 && preparation.totals?.writableFields === 9 &&
    preparation.diagnosis?.totals?.invalidCritiques === 9 &&
    preparation.diagnosis?.totals?.invalidQuotes === 0 &&
    preparation.contexts.every((context) => context.writableFieldCount <= 2) &&
    new Set(preparation.contexts.flatMap((context) => context.writableFields)).size === 9,
  "five-shard Debate 34 repair is not prepared"
);
assertV4(
  ...
  "five-shard model or execution controls changed"
);
...
const activation = {
  ...
  status: "frozen-five-context-nine-field-batch-13-debate-34-publication-repair-activated",
  activatedAt,
  ...
  nextAuthorizedAction: "execute-only-five-frozen-debate-34-field-disjoint-repair-contexts"
};
