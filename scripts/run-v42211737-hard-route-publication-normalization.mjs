...
import { V42211737_ROOT, normalizeV42211737PublicationOutput } from "./lib/v42211737-hard-route-publication-normalization.mjs";

const manifest = JSON.parse(await readFile(path.resolve(`${V42211737_ROOT}/execution-manifest.json`), "utf8"));
...
async function runContext(context, contextIndex) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `v42211737-publication-${context.debateNumber}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `v42211737-publication-home-${context.debateNumber}-`));
  ...
  try {
    ...
    const prompt = `Read workflow.md, output-contract.md, manual.md, reference-catalog.json, packet.json, and schema.json completely and no other files. Act only as the isolated publication editor for Debate ${context.debateNumber}. Participant judgment and every score are closed and repository-owned. Author exactly the schema fields: source-exact representative quotes, prose for every locked move, Overall Commentary, post-scoring references, and a balanced separately disclosed AI Extension with complete novelty mappings. Select each representative quote as an exact 6–14-word source substring. The top-level summary must contain 18–28 words. Write every critique in exactly four complete labeled sentences, 112–122 words, at least 880 characters, and terminal punctuation; never truncate to fit a character maximum or emit unexpected CJK or Hangul script. Use only exact local references. Never infer or emit a score, change identity or structure, consult legacy assessment material, or attribute AI material to a participant. Return exactly one schema-conforming JSON object and nothing else.`;
    process.stdout.write(`[v4.2.21.17.37-publication-normalization] starting index ${contextIndex} ${manifest.model.label}/${manifest.model.reasoningEffort} Debate ${context.debateNumber}\n`);
    ...
    if (invocation.timedOut || invocation.code !== 0 || invocation.signal !== null || !resultExists) {
      ...
      await copyFile(path.join(temporary, "result.json"), path.resolve(context.rawOutput));
      let validation = null, validationMessage = null, transformations = [], normalizedOutputWritten = false;
      try {
        const rawOutput = JSON.parse(await readFile(path.resolve(context.rawOutput), "utf8"));
        ...
        await writeFile(path.resolve(context.output), `${JSON.stringify(normalized.output)}\n`);
        normalizedOutputWritten = true;
        validation = validateV42211736PublicationOutput(normalized.output, packet);
      }
      catch (error) { validationMessage = error.stack ?? error.message; }
      record = { ...base, status: validation?.status === "passed" ? "completed-valid" : "output-validation-failed", gateAcceptancePassed: validation?.status === "passed", rawOutputWritten: true, rawOutputSha256: sha256(await readFile(path.resolve(context.rawOutput))), outputWritten: normalizedOutputWritten, outputSha256: normalizedOutputWritten ? sha256(await readFile(path.resolve(context.output))) : null, deterministicQuoteNormalizations: transformations, validationSummary: validation, validationMessage: validationMessage?.slice(-10000) ?? null };
    }
  } catch (error) {
  ...
  process.stdout.write(`[v4.2.21.17.37-publication-normalization] Debate ${context.debateNumber} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`);
  return record;
}
