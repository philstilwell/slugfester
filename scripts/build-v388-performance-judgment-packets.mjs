...
const workflowPath = "docs/assessment-workflow-v3.8.4.md";
const rubricPath = "docs/reassessment-rubric-v3.8.4.md";
const manualPath = `${V388_PERFORMANCE_ROOT}/manual.md`;
const preregistrationPath = `${V388_PERFORMANCE_ROOT}/preregistration.md`;
...
assertV388(coverage.schemaVersion === "3.8.8-final-coverage-inventory" && coverage.selectedMoveCount === 81, "coverage lock invalid");
assertV388(sectionLock.schemaVersion === "3.8.8-locked-section-weight-plans" && sectionLock.assignedMoveCount === 81, "section lock invalid");
assertV388(contactLock.schemaVersion === "3.8.8-locked-burden-contact-ledger" && contactLock.moveCount === 81 && contactLock.unresolvedMoves === 0, "burden-contact lock invalid");
assertV388(contactAnalysis.passed === true && contactAnalysis.decision.performanceJudgmentPreregistrationAuthorized === true, "burden-contact checkpoint did not authorize performance preregistration");
assertV388(contactAnalysis.decision.numericalParticipantScoringAuthorized === false, "scores unexpectedly authorized");
assertV388(sourceAudit.totals.pendingAudioVerifications === 0, "medium-confidence source move remains unverified");
...
for (const debateNumber of V388_PERFORMANCE_DEBATES) {
  ...
  assertV388(coverageDebate && sectionDebate && contactDebate && sourceChain, `${debateNumber}: incomplete locked source set`);
  assertV388(coverageDebate.debateId === sectionDebate.debateId && coverageDebate.debateId === contactDebate.debateId, `${debateNumber}: debate identity mismatch`);
  ...
  assertV388(assignments.size === coverageDebate.moves.length && contactByMove.size === coverageDebate.moves.length, `${debateNumber}: move lock cardinality mismatch`);
  ...
  const moves = coverageDebate.moves.map((move) => {
    ...
    assertV388(assignment && contact, `${move.moveId}: missing section or contact lock`);
    assertV388(canonicalJson(move.sourceSpan) === canonicalJson(contact.sourceSpan), `${move.moveId}: source span changed between locks`);
    assertV388(move.attributionConfidence === "high" && move.audioVerification === null, `${move.moveId}: audio verification boundary not satisfied`);
    const responseTargets = move.respondsToRefs.map((targetId) => {
      const target = coverageByMove.get(targetId);
      assertV388(target, `${move.moveId}: unavailable response target ${targetId}`);
      return {
  ...
  assertV388(sha256(transcriptBytes) === sourceChain.transcriptSha256, `${debateNumber}: transcript hash mismatch`);
  assertV388(sha256(eventBytes) === sourceChain.eventsSha256, `${debateNumber}: event hash mismatch`);
  assertV388(sha256(manifestBytes) === sourceChain.localManifestSha256, `${debateNumber}: caption manifest hash mismatch`);
  const packet = {
    schemaVersion: "3.8.8-performance-judgment-packet",
    ...
    modelInputBoundary: {
      ...
      sharedSchemaPath: `${V388_PERFORMANCE_ROOT}/performance-judgment-schema.json`,
  ...
  packets.push(packet);
  if (shouldWrite) {
    const outputPath = path.resolve(root, `${V388_PERFORMANCE_ROOT}/packets/debate-${debateNumber}.json`);
...
const manifest = {
  schemaVersion: "3.8.8-performance-judgment-preparation",
  protocolId: "v3.8.8-performance-judgment-consensus",
  ...
  packets: packets.map((packet) => ({ debateNumber: packet.debateNumber, debateId: packet.debateId, path: `${V388_PERFORMANCE_ROOT}/packets/debate-${packet.debateNumber}.json`, moveCount: packet.moves.length, highConfidenceAttributions: packet.moves.filter((move) => move.attributionConfidence === "high").length, pendingAudioVerifications: 0 })),
  consensus: {
    initialContexts: 6,
...
if (shouldWrite) {
  ...
  await writeFile(path.resolve(root, `${V388_PERFORMANCE_ROOT}/performance-judgment-schema.json`), `${JSON.stringify(schema, null, 2)}\n`);
  await writeFile(path.resolve(root, `${V388_PERFORMANCE_ROOT}/preparation-manifest.json`), `${JSON.stringify(manifest, null, 2)}\n`);
}
