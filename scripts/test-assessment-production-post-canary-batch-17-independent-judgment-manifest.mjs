...
const MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-17/independent-judgments/execution-preparation-manifest.json";
...
assert.equal(manifest.contexts.length, 8);
assert.deepEqual(
...
assert.equal(preparation.totals.uniqueMoves, 79);
assert.equal(preparation.totals.movesJudgedAcrossPasses, 158);
...
assert.equal(manifest.costEstimate.contexts, 8);
...
assert.deepEqual(manifest.costEstimate.expectedParallelWallMinutes, [20, 40]);
...
assert.equal(manifest.acceptanceContract.validContextsRequired, 8);
