...
const finalLedgerPath =
  `${POST_CANARY_BATCH_04_FINAL_LEDGER_ROOT}/final-ledger.json`;
...
assertV4(
  ...
  "post-canary Batch 4 final-ledger manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen Batch 4 final-ledger source hash mismatch`
  );
}
const inputs = await loadPostCanaryBatch04FinalLedgerInputs();
const ledger = buildPostCanaryBatch04FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
