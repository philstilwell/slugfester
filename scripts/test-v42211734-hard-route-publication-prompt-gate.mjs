...
const manifestPath = `${V42211734_ROOT}/execution-manifest.json`, executionPath = `${V42211734_ROOT}/model-execution.json`, analysisPath = `${V42211734_ROOT}/analysis.json`;
...
assert.equal(preparation.status, "prepared-five-isolated-hard-route-publication-prompt-contexts");
...
assert.equal(preparation.repair.counterfactualOnlySummaryReplacementPassed, true);
assert.deepEqual(preparation.repair.modelSummaryTargetWords, [18, 28]);
assert.deepEqual(preparation.repair.repositorySummaryAcceptanceWords, [8, 35]);
assert.equal(preparation.repair.structuredOutputRepairRetained, true);
assert.equal(preparation.repair.repositoryUniquenessValidationRetained, true);
assert.equal(preparation.repair.priorGateTreatedAsRetry, false);
assert.match(await readFile(path.resolve(preparation.inputs.workflow), "utf8"), /summary must contain \*\*18–28 words\*\*/);
assert.match(await readFile(path.resolve("scripts/run-v42211734-hard-route-publication-prompt.mjs"), "utf8"), /top-level summary must contain 18–28 words/);
