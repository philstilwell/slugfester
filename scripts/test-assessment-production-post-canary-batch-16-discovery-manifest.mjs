...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-16/discovery";
...
assert.equal(manifest.status, "frozen-thirty-six-post-canary-batch-16-discovery-contexts-prepared-not-authorized");
...
assert.equal(manifest.executionPolicy.copiedInputBytesMaximum, 66002);
assert.equal(preparation.totals.maximumCopiedInputBytes, 66002);
...
assert.equal(manifest.copiedInputBoundary.frozenObservedCopiedInputBytesMaximum, 66002);
assert.equal(manifest.copiedInputBoundary.executionCopiedInputBytesMaximum, 66002);
assert.deepEqual(manifest.copiedInputBoundary.maximumContext, { debateNumber: "92", chunkId: "chunk-002" });
...
for (const context of manifest.contexts) {
  ...
  assert(buildBatch16TokenCountedChunkLedger(chunkBytes).equals(tokenBytes));
...
assert.equal(manifest.nextAuthorizedAction, "prepare-and-freeze-batch-16-discovery-execution-activation-under-standing-authorization");

const validation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-discovery-execution-preparation-validation",
  protocolId: manifest.protocolId,
  status: "batch-16-discovery-execution-manifest-validation-passed-frozen-standing-authorization-active",
