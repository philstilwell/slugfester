...
assertV4(p.protocolId === POST_CANARY_BATCH_06_PUBLICATION_REPAIR_PROTOCOL_ID &&
  p.status === "frozen-twenty-five-context-batch-06-publication-repair-prepared-and-authorized" &&
  p.contexts?.length === 25 && p.contexts.every((row) => row.writableFieldCount <= 2) &&
  new Set(p.contexts.flatMap((row) => row.writableFields)).size === 49 &&
...
"the Batch 6 publication repair is not prepared");
...
const activation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-06-publication-repair-activation",
  protocolId: p.protocolId,
  status: "frozen-twenty-five-context-batch-06-publication-repair-authorized",
  ...
  nextAuthorizedAction: "execute-exactly-twenty-five-frozen-batch-06-publication-repair-contexts"
};
if (shouldWrite) await writeFile(path.resolve(ACTIVATION), `${JSON.stringify(activation, null, 2)}\n`);
console.log(JSON.stringify({ status: activation.status, contextsAuthorized: 25,
  writableFieldsAuthorized: 49, model: activation.model, schedulerRamp: [1, 2],
