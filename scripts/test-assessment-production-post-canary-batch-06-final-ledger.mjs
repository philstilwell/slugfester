...
const scoreArtifactCandidates = [
  `${POST_CANARY_BATCH_06_FINAL_LEDGER_ROOT}/calculated-scores.json`,
  `${POST_CANARY_BATCH_06_FINAL_LEDGER_ROOT}/score-analysis.json`,
  `${POST_CANARY_BATCH_06_FINAL_LEDGER_ROOT}/score-pass-manifest.json`
];
...
const rebuiltLedger = buildPostCanaryBatch06FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
...
assert.deepEqual(
  ...
  ["73", "36", "38", "97", "141", "06", "168", "135", "143", "169"]
);
assert.equal(storedLedger.audit.finalMoves, 200);
assert.equal(storedLedger.audit.disputedMoves, 197);
assert.equal(storedLedger.audit.pairSelections, 226);
assert.equal(storedLedger.audit.scoringFieldSelections, 367);
assert.equal(storedLedger.audit.burdenAdjustmentSelections, 16);
assert.equal(storedLedger.audit.candidateSelections, 609);
assert.equal(storedLedger.audit.meanMerges, 402);
...
assert.throws(() =>
  buildPostCanaryBatch06FinalLedger(mutatedInputs, inputs.sourceHashes)
);
...
assert.equal(analysis.totals.finalMoves, 200);
assert.equal(analysis.totals.disputedMoves, 197);
assert.equal(analysis.totals.candidateSelections, 609);
