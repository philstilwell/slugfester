...
const PRIOR_SELECTION = "docs/assessment-production/post-canary-continuation-v1/batch-07/selection.json";
const PRIOR_SELECTION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-07/selection-analysis.json";
const PRIOR_MUTATION_MANIFEST = "docs/assessment-production/post-canary-continuation-v1/batch-07/production-publication/mutation-manifest.json";
const PRIOR_COMPLETION_EXECUTION = "docs/assessment-production/post-canary-continuation-v1/batch-07/production-publication/generated-seo-correction/execution.json";
const PRIOR_COMPLETION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-07/production-publication/generated-seo-correction/analysis.json";
const PRIOR_COMPLETION_COMMIT = "a3ace6ac65d7d9f0390e79fd808b32a9e09f3209";
const SCRIPT = "scripts/select-assessment-production-post-canary-batch-08.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-08-selection.mjs";
...
assertV4(!(await exists(FUTURE_SOURCE_PREPARATION)), "source preparation already exists before Batch 8 selection");
...
assertV4(
  ...
  "Batch 8 selection is not compatible with the frozen continuation policy"
);
...
assertV4(
  priorSelection.status === "seventh-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    priorSelection.protocolId === "assessment-production-post-canary-batch-07" &&
    priorSelection.productionSequenceOrdinal === 8 &&
    priorSelection.selected.length === 10 &&
    priorSelectionAnalysis.status === "seventh-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization" &&
    priorSelectionAnalysis.selection.sha256 === sha256(controlBytes[PRIOR_SELECTION]) &&
    priorMutationManifest.status === "frozen-batch-07-production-publication-mutation-manifest-prepared" &&
    JSON.stringify(priorMutationManifest.explicitOrder) === JSON.stringify(priorSelectedNumbers) &&
    priorCompletionExecution.status === "passed-complete-batch-07-transaction-ready-for-atomic-commit" &&
    ...
    priorCompletionExecution.totals.automaticRepairs === 0 &&
    priorCompletionExecution.totals.rollbacks === 0 &&
    ...
    priorCompletionAnalysis.status === "batch-07-production-publication-accepted-for-atomic-commit-after-generated-seo-correction" &&
    ...
    priorCompletionAnalysis.decision.batch06ProductionPublicationGatePassed === true &&
    ...
    priorCompletionAnalysis.decision.automaticRepairPerformed === false &&
    priorCompletionAnalysis.decision.productionMutationRerunPerformed === false &&
    priorCompletionAnalysis.decision.rollbackPerformed === false &&
  ...
  "Batch 7 has not satisfied the frozen successful-publication prerequisite"
);
assertV4(git("rev-parse", "--abbrev-ref", "HEAD") === "main", "Batch 8 selection must run on main");
assertV4(git("rev-parse", "HEAD") === PRIOR_COMPLETION_COMMIT, "Batch 8 selection checkpoint is not the exact Batch 7 publication commit");
assertV4(git("rev-parse", "origin/main") === PRIOR_COMPLETION_COMMIT, "Batch 7 publication commit is not the pushed origin/main checkpoint");
...
assertV4(
  JSON.stringify(priorSelectedNumbers) === JSON.stringify(ranked.slice(60, 70).map((item) => item.debateNumber)),
  "Batch 7 is not the immutable seventh rank slice"
);

const selected = [];
let totalEvents = 0;
let totalDurationMs = 0;
for (const [offset, rankedItem] of ranked.slice(70, 80).entries()) {
...
assertV4(selected.length === 10 && new Set(selected.map((item) => item.debateNumber)).size === 10, "Batch 8 did not select ten unique debates");

const checkpointCommit = git("rev-parse", "HEAD");
const selection = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-selection",
  protocolId: "assessment-production-post-canary-batch-08",
  status: "eighth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed",
  frozenAt,
  checkpointCommit,
  branch: "main",
  ...
  stagingOnly: true,
  batchNumber: 8,
  productionSequenceOrdinal: 9,
  userAuthorization: {
    instruction: "I authorize that.",
    scopeInterpretation: "Materialize and source-validate only the deterministic Batch 8 selection under the frozen continuation policy after authenticating successful Batch 7 publication. This selection pass prepares no source packets, creates no Batch 8 standing authorization, executes no models, derives no scores, and performs no production mutation.",
  ...
  eligibility: {
    ...
    previouslyPublishedAndRemovedCount: 70,
    remainingBeforeBatchCount: ranked.length - 70,
    remainingBeforeBatchRankedSha256: sha256(serializedJson(ranked.slice(70))),
    selectedCount: selected.length,
    remainingUnselectedCount: ranked.length - 80,
    dyadicOnly: true,
  ...
  nextAuthorizedAction: "prepare-batch-08-standing-authorization-and-source-packets-under-user-authorization"
};
...
const analysis = {
  ...
  status: "eighth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization",
  analyzedAt: frozenAt,
  ...
  finding: "Successful Batch 7 production publication was authenticated at the pushed atomic commit. The immutable campaign rank then selected ordinals 71 through 80 as ten unique pending dyadic debates, and all thirty effective source files and ten canonical event chains passed without replacement. The prior Batch 7 completion analysis retains its inherited batch06ProductionPublicationGatePassed key and was authenticated byte-for-byte without rewriting prior evidence.",
  decision: {
