...
const scoreArtifactCandidates = [
  `${POST_CANARY_BATCH_11_FINAL_LEDGER_ROOT}/calculated-scores.json`,
  `${POST_CANARY_BATCH_11_FINAL_LEDGER_ROOT}/score-analysis.json`,
  `${POST_CANARY_BATCH_11_FINAL_LEDGER_ROOT}/score-pass-manifest.json`
];
...
const rebuiltLedger = buildPostCanaryBatch11FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
...
assert.deepEqual(
  ...
  ["54", "01", "82", "191", "151", "188", "60", "79", "43", "24"]
);
assert.equal(storedLedger.audit.finalMoves, 190);
assert.equal(storedLedger.audit.disputedMoves, 184);
assert.equal(storedLedger.audit.pairSelections, 199);
assert.equal(storedLedger.audit.scoringFieldSelections, 288);
assert.equal(storedLedger.audit.burdenAdjustmentSelections, 20);
assert.equal(storedLedger.audit.candidateSelections, 507);
assert.equal(storedLedger.audit.meanMerges, 438);
assert.equal(storedLedger.audit.dependencyMeanMergesSuppressed, 0);
assert.equal(storedLedger.audit.audioVerifiedMoves, 2);
...
assert.throws(() =>
  buildPostCanaryBatch11FinalLedger(mutatedInputs, inputs.sourceHashes)
);
...
assert.equal(analysis.totals.disputedMoves, 184);
assert.equal(analysis.totals.candidateSelections, 507);
