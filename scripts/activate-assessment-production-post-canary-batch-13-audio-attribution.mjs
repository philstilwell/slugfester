...
import { MODEL, PROTOCOL_ID, ROOT, sha256 } from "./lib/assessment-production-post-canary-batch-13-audio-attribution.mjs";
...
const authorizationPath = "docs/assessment-production/post-canary-continuation-v1/batch-13/audio-verification/audio-attribution-successor-authorization.json";
...
assert.equal(preparation.status, "prepared-two-batch-13-audio-attribution-recovery-contexts-not-active");
assert.equal(authorization.status, "frozen-active-batch-13-audio-attribution-recovery-level-2-successor-authorization");
assert.equal(preparation.authorization.sha256, sha256(authorizationBytes));
assert.deepEqual(preparation.contexts.map((item) => [item.debateNumber, item.moveIds.length]), [["37", 2], ["111", 4]]);
...
const sourceFiles = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-13-audio-attribution.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-13-audio-attribution.mjs",
  "scripts/test-assessment-production-post-canary-batch-13-audio-attribution.mjs",
  "scripts/activate-assessment-production-post-canary-batch-13-audio-attribution.mjs",
  "scripts/run-assessment-production-post-canary-batch-13-audio-attribution.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-13-audio-attribution.mjs",
];
const sourceHashes = {};
...
const manifest = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-13-audio-attribution-execution-manifest",
  protocolId: PROTOCOL_ID,
  status: "frozen-two-batch-13-audio-attribution-recovery-contexts-active",
  activatedAt,
...
console.log(JSON.stringify({ status: shouldWrite ? manifest.status : "preview", contexts: 2, decisions: 6, copiedInputBytes: contexts.map((item) => ({ debateNumber: item.debateNumber, bytes: item.copiedInputBytes })), model: "5.6 Sol/low", authentication: MODEL.authentication, attemptsMaximum: 2, retriesMaximum: 0, paidTranscriptionCalls: 0, directIncrementalCostUsdMaximum: 0, scoresDerived: 0 }, null, 2));
