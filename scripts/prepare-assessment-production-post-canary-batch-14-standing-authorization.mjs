...
import {
  ...
  POST_CANARY_BATCH_14_STANDING_AUTHORIZATION_INSTRUCTION,
  POST_CANARY_BATCH_14_STANDING_AUTHORIZATION_INSTRUCTION_PATH,
  POST_CANARY_BATCH_14_STANDING_AUTHORIZATION_PROTOCOL_ID,
  POST_CANARY_BATCH_14_STANDING_AUTHORIZATION_STATUS,
  validatePostCanaryBatch14StandingAuthorization
...
const SELECTION_FREEZE_COMMIT = "d7d7f4f2642fb5fb1d02b9b787647437d609ab11";
const SOURCE_PACKET_COMMIT = "308977929ec1f96d09508c23fdf85f3c19257930";
...
const SOURCE_FILES = [
  ...
  "scripts/prepare-assessment-production-post-canary-batch-14-standing-authorization.mjs",
  "scripts/test-assessment-production-post-canary-batch-14-standing-authorization.mjs"
];
const REQUIRED_ORDER = ["53", "69", "110", "133", "12", "187", "160", "55", "106", "57"];
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const exists = (file) => access(path.resolve(file)).then(() => true, () => false);

assertV4(
  !(await exists(POST_CANARY_BATCH_14_STANDING_AUTHORIZATION)),
  `${POST_CANARY_BATCH_14_STANDING_AUTHORIZATION} already exists`
);
...
assertV4(
  selection.status ===
    ...
    selection.batchNumber === 14 &&
    selection.checkpointCommit ===
      "59dfc4f036ed22499783da758f23f4c66b298a0f" &&
    ...
    analysis.status ===
      "fourteenth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization" &&
    ...
    sourcePreparation.totals.discoveryContexts === 33 &&
    ...
    sourceValidation.totals.discoveryContexts === 33 &&
    sourceValidation.totals.modelContextsExecuted === 0 &&
    sourceValidation.directIncrementalCostUsd === 0,
  "the frozen Batch 14 selection or source-packet boundary changed"
);
assertV4(
  execFileSync("git", ["show", `${SELECTION_FREEZE_COMMIT}:${SELECTION}`]).equals(selectionBytes),
  "Batch 14 selection differs from the user-authorized selection commit"
);
...
assertV4(
  ...
  "Batch 14 standing authorization must freeze at the pushed source-packet commit"
);
...
const record = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-14-standing-authorization",
  protocolId: POST_CANARY_BATCH_14_STANDING_AUTHORIZATION_PROTOCOL_ID,
  status: POST_CANARY_BATCH_14_STANDING_AUTHORIZATION_STATUS,
  authorizedAt,
  ...
  userAuthorization: {
    instruction: POST_CANARY_BATCH_14_STANDING_AUTHORIZATION_INSTRUCTION,
    resolvedAntecedent: "The user's instruction to proceed carries forward the established standing authorization for the complete reassessment, bounded recovery, publication, validation, commit, and push workflow under the current frozen campaign controls.",
    scopeInterpretation: "Create and use a complete-workflow Batch 14 standing authorization for the ten frozen selections, beginning with the separately frozen score-blind source packets, including bounded diagnosed recovery and production publication, while preserving the established campaign controls and stopping before Batch 15 selection.",
  ...
  authorizedSequence: [
    "thirty-three-frozen-score-blind-discovery-contexts",
  ...
  stopRules: {
    ...
    outsideBatchFourteenDebateBlocks: true,
    ...
    batchFifteenSelectionBlocks: true,
  ...
  nextAuthorizedAction:
    "prepare-validate-freeze-commit-and-push-batch-14-discovery-execution-manifest"
};
validatePostCanaryBatch14StandingAuthorization(record);
if (shouldWrite) {
  await writeFile(
    path.resolve(POST_CANARY_BATCH_14_STANDING_AUTHORIZATION),
