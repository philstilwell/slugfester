...
const SOURCE_PACKET_VERSION =
  "1.0-assessment-production-post-canary-batch-02-candidate-sharded-inventory-source-packet";
...
function buildInventorySourcePacket(sourcePacket) {
  assertV4(
    sourcePacket?.schemaVersion ===
      "1.0-assessment-production-post-canary-batch-02-score-blind-source-packet" &&
      sourcePacket.protocolId ===
        "assessment-production-post-canary-batch-02-source-preparation" &&
    ...
    "Batch 2 source packet is unavailable or not dyadic"
  );
  return {
...
function makePlanPacket({
  ...
  return {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-02-candidate-census-plan-packet",
    protocolId: PROTOCOL_ID,
...
async function buildFrozenArtifacts(frozenAt) {
  ...
  assertV4(
    discoveryAnalysis.status ===
      "post-canary-batch-02-discovery-passed-awaiting-separate-inventory-authorization" &&
      ...
      discoveryAnalysis.totals?.modelContextsExecuted === 36 &&
      ...
      discoveryAnalysis.nextAuthorizedAction ===
        "user-approval-required-before-batch-02-inventory-packet-preparation-or-any-inventory-model-execution",
    "passed Batch 2 discovery boundary drifted"
  );
  assertV4(
    discoveryExecution.status ===
      "thirty-six-post-canary-batch-02-discovery-contexts-passed" &&
      discoveryExecution.contextsAttempted === 36 &&
      discoveryExecution.validContexts === 36 &&
    ...
    "Batch 2 discovery execution drifted"
  );
  assertV4(
      sourcePreparation.status ===
      "post-canary-batch-02-ten-complete-score-blind-source-packets-prepared-awaiting-validation" &&
    ...
    "Batch 2 source preparation or model boundary drifted"
  );
  assertV4(
    sourcePreparationValidation.status ===
      "post-canary-batch-02-score-blind-source-packet-validation-passed-frozen-awaiting-user-approval-before-any-model-execution" &&
      ...
      sourcePreparationValidation.checks
        ?.exactZeroLexicalTokenRowPreservedWithZeroCount === true &&
      sourcePreparationValidation.checks
    ...
    "Batch 2 source validation drifted"
  );
  assertV4(
    selection.status ===
      "second-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    ...
    "Batch 2 frozen selection drifted"
  );
  assertV4(
  ...
  for (const discovered of discoveryAnalysis.debates) {
    ...
    assertV4(
      ...
      `${discovered.debateNumber}: Batch 2 identity drifted`
    );
    const [
  ...
  assertV4(
    ...
    "Batch 2 totals or packet state drifted"
  );
  ...
  const preparation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-02-candidate-sharded-inventory-preparation",
    protocolId: PROTOCOL_ID,
    ...
    userAuthorization: {
      ...
      approvedScope:
        "Prepare, validate, freeze, commit, and push the Batch 2 candidate-sharded inventory packets and execution-preparation manifest only, with a direct incremental cost cap of $0. Do not execute inventory models, judgment models, paid services, audio work, score derivation, publication reconstruction, or production mutation.",
    ...
    discoveryDisposition: {
      batch02DiscoveryPassed: true,
      batch02DiscoveryRetried: false,
      batch02DiscoveryCandidatesChangedDuringTransport: false,
    ...
    nextAuthorizedAction:
      "user-approval-required-before-batch-02-candidate-census-plan-execution-activation-or-any-inventory-model-execution",
  };
  ...
  const executionPreparation = {
    ...
    costEstimate: {
      ...
      estimateBasis:
        `The promoted ten-context candidate-census planner used this contract at concurrency two. Batch 2 freezes the same ten-context shape with an observed maximum of ${maximumCopiedInputBytes.toLocaleString("en-US")} copied bytes, below the proven ${V212_CANDIDATE_SHARDED_INVENTORY.maximumCopiedInputBytes.toLocaleString("en-US")}-byte ceiling. ChatGPT-subscription execution has no direct incremental API charge.`,
    },
  ...
  const validation = {
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-02-candidate-sharded-inventory-preparation-validation",
    protocolId: PROTOCOL_ID,
    status:
      "batch-02-candidate-sharded-inventory-packets-and-plan-execution-manifest-validation-passed-frozen-not-authorized",
    ...
    checks: {
      ...
      noBelowHighAttributionCandidateRecordedAtDiscovery: true,
...
async function validateFrozen() {
  ...
  assertV4(
    preparation.status ===
      ...
      preparation.audioPolicy.discoveryBelowHighCandidates === 0 &&
    ...
    "frozen Batch 2 inventory preparation is invalid"
  );
...
if (command === "freeze") await freeze();
  ...
  throw new Error(
    "usage: prepare-assessment-production-post-canary-batch-02-inventory.mjs <freeze|validate>"
  );
}
