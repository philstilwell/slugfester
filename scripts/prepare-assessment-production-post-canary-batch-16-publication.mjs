...
import {
  validatePostCanaryBatch16Scores
} from "./lib/assessment-production-post-canary-batch-16-score-gate.mjs";
import {
  buildPostCanaryBatch16PublicationPacket,
  buildPostCanaryBatch16PublicationSchema,
  postCanaryBatch16ReferenceCatalog,
  POST_CANARY_BATCH_16_PUBLICATION_DEBATES,
  POST_CANARY_BATCH_16_PUBLICATION_MODEL,
  POST_CANARY_BATCH_16_PUBLICATION_PROTOCOL_ID,
  POST_CANARY_BATCH_16_PUBLICATION_ROOT
} from "./lib/assessment-production-post-canary-batch-16-publication.mjs";
...
const ROOT = POST_CANARY_BATCH_16_PUBLICATION_ROOT;
...
const SELECTION =
  "docs/assessment-production/post-canary-continuation-v1/batch-16/selection.json";
const FINAL_LEDGER =
...
const FINAL_LEDGER_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-16/final-ledger/final-ledger-manifest.json";
const FINAL_LEDGER_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-16/final-ledger/analysis.json";
const SCORES =
  "docs/assessment-production/post-canary-continuation-v1/batch-16/score-pass/calculated-scores.json";
const SCORE_MANIFEST =
  "docs/assessment-production/post-canary-continuation-v1/batch-16/score-pass/score-pass-manifest.json";
const SCORE_PREPARATION =
  "docs/assessment-production/post-canary-continuation-v1/batch-16/score-pass/score-pass-preparation-manifest.json";
const SCORE_ANALYSIS =
  "docs/assessment-production/post-canary-continuation-v1/batch-16/score-pass/analysis.json";
const SCORE_EXCEPTION =
...
const SOURCE_PACKET_ROOT =
  "docs/assessment-production/post-canary-continuation-v1/batch-16/independent-judgments/source-packets";
...
const STATIC_SOURCE_FILES = [
  ...
  "scripts/lib/assessment-production-post-canary-batch-16-score-gate.mjs",
  "scripts/lib/assessment-production-post-canary-batch-16-publication.mjs",
  "scripts/lib/assessment-production-post-canary-batch-16-publication-validation.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-16-publication.mjs",
  "scripts/test-assessment-production-post-canary-batch-16-publication-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-16-publication.mjs",
  "scripts/run-assessment-production-post-canary-batch-16-publication.mjs",
  "scripts/analyze-assessment-production-post-canary-batch-16-publication.mjs"
];
...
const scoreException = json(SCORE_EXCEPTION);
...
assertV4(
  scoreAnalysis.status === "post-canary-batch-16-score-stability-gate-failed" &&
    scoreAnalysis.productionCanary === false &&
    scoreAnalysis.batchNumber === 16 &&
    scoreAnalysis.stagingOnly === true &&
    scoreAnalysis.validation?.numericPassed === true &&
    scoreAnalysis.validation?.winnerStabilityPassed === false &&
    scoreAnalysis.validation?.acceptancePassed === false &&
    scoreAnalysis.validation?.publishedOppositeSideReversals?.join(",") === "144" &&
    ...
    scoreAnalysis.nextAuthorizedAction ===
      "standing-authorization-permits-batch-16-score-stability-failure-diagnosis",
  "the preserved failed Batch 16 score gate changed"
);
assertV4(
  scores.status === "post-canary-batch-16-single-score-pass-stability-gate-failed" &&
    scores.totals?.debates === 10 &&
    scores.totals?.scoringPasses === 1 &&
    scores.totals?.acceptancePassed === false &&
    scores.stability?.numericPassed === true &&
    scores.stability?.winnerStability?.passed === false &&
    scores.stability?.winnerStability?.publishedOppositeSideReversals?.join(",") === "144" &&
    scores.authorization?.scoreRerun === false &&
    scores.authorization?.publicationPacketPreparation === false,
  "the locked failed Batch 16 score artifact is unavailable"
);
assertV4(
  scoreException.status ===
      "frozen-active-batch-16-debate-144-score-stability-publication-exception" &&
    scoreException.batchNumber === 16 &&
    scoreException.preservedFailure?.debateNumber === "144" &&
    scoreException.preservedFailure?.numericDistanceGatePassed === true &&
    scoreException.preservedFailure?.winnerStabilityGatePassed === false &&
    scoreException.preservedFailure?.erasedOrReclassified === false &&
    scoreException.immutableBoundaries?.scorePassesCompleted === 1 &&
    scoreException.immutableBoundaries?.scoreRerunAllowed === false &&
    scoreException.immutableBoundaries?.manualScoreChangesAllowed === false &&
    scoreException.immutableBoundaries?.acceptedAdjudicationFieldChangesAllowed === false &&
    scoreException.immutableBoundaries?.finalLedgerChangesAllowed === false &&
    scoreException.immutableBoundaries?.calculatedScoresChangesAllowed === false &&
    scoreException.exception?.scope === "publication-readiness-gate-only" &&
    scoreException.exception?.debates?.join(",") === "144" &&
    scoreException.exception?.existingScoresMayProceedToPublicationReconstruction === true,
  "the narrow Batch 16 Debate 144 publication exception is unavailable or changed"
);
for (const [file, digest] of Object.entries(scoreException.sourceHashes)) {
  assertV4(
    sha256(await readFile(path.resolve(file))) === digest,
    `${file}: exceptional publication authorization source drifted`
  );
}
assertV4(
  scoreManifest.status ===
      "frozen-post-canary-batch-16-single-deterministic-score-pass-authorized" &&
    ...
    scorePreparation.status ===
      "frozen-post-canary-batch-16-single-deterministic-score-pass-prepared-not-activated",
  "the frozen Batch 16 single-pass score controls changed"
);
assertV4(
  ledger.status ===
      "passed-post-canary-batch-16-deterministic-final-ledger-assembly" &&
    ledger.debates?.length === 10 &&
    ledger.audit?.finalMoves === 194 &&
    ledger.audit?.audioVerifiedMoves === 10 &&
    ledger.authorization?.publicationReconstruction === false &&
    ledger.authorization?.productionMutation === false &&
    ledgerManifest.status ===
      "frozen-ten-debate-post-canary-batch-16-deterministic-final-ledger-assembly" &&
    ledgerAnalysis.status ===
      "post-canary-batch-16-deterministic-final-ledger-gate-passed",
  "the locked Batch 16 final ledger changed"
);
assertV4(
  productionManifest.model?.label === POST_CANARY_BATCH_16_PUBLICATION_MODEL.label &&
    productionManifest.model?.slug === POST_CANARY_BATCH_16_PUBLICATION_MODEL.slug &&
    productionManifest.model?.reasoningEffort ===
      POST_CANARY_BATCH_16_PUBLICATION_MODEL.reasoningEffort &&
    productionManifest.model?.authentication ===
      POST_CANARY_BATCH_16_PUBLICATION_MODEL.authentication,
  "the frozen production model or authentication changed"
);
assertV4(
  selection.selected?.map((item) => item.debateNumber).join(",") ===
      POST_CANARY_BATCH_16_PUBLICATION_DEBATES.join(",") &&
    selection.selected.every((item) => item.speakerCount === 2) &&
    selection.authorization?.productionMutation === false,
  "the frozen Batch 16 selection changed"
);
assertV4(
  ledger.debates.map((debate) => debate.debateNumber).join(",") ===
    ...
    scores.debates.map((debate) => debate.debateNumber).join(",") ===
      POST_CANARY_BATCH_16_PUBLICATION_DEBATES.join(","),
  "the Batch 16 ledger or score order changed"
);
assertV4(
...
const finalLedgerInputs = await loadPostCanaryBatch16FinalLedgerInputs();
validatePostCanaryBatch16FinalLedger(
  ledger,
...
const catalog = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-16-local-reference-catalog",
  references: postCanaryBatch16ReferenceCatalog()
};
...
const audioVerifiedMoveIds = new Set(
  finalLedgerInputs.debateInputs.flatMap((input) =>
    input.adjudicationPacket.disputedMoves
      .filter((move) => move.evidence.audioVerification?.status === "verified")
...
for (const debateNumber of POST_CANARY_BATCH_16_PUBLICATION_DEBATES) {
  ...
  const packet = buildPostCanaryBatch16PublicationPacket({
    ledgerDebate,
    scoreDebate,
    sourcePacket,
  ...
  const schema = buildPostCanaryBatch16PublicationSchema(packet);
...
assertV4(
  contexts.reduce((sum, context) => sum + context.moves, 0) === 194,
  "Batch 16 publication move coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.sections, 0) === 53,
  "Batch 16 publication section coverage changed"
);
assertV4(
  contexts.reduce((sum, context) => sum + context.audioVerifiedMoves, 0) === 10,
  "Batch 16 audio-verification coverage changed"
);
assertV4(
  contexts.every(
    (context) =>
  ...
  "Batch 16 publication section or quote eligibility is invalid"
);
...
const manifest = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-16-publication-execution-preparation-manifest",
  protocolId: POST_CANARY_BATCH_16_PUBLICATION_PROTOCOL_ID,
  status:
    "frozen-ten-post-canary-batch-16-score-locked-publication-contexts-prepared-not-activated",
  frozenAt,
  ...
  userAuthorization: {
    instruction:
      "The frozen Batch 16 standing authorization permits preparation, activation, and execution of exactly ten score-locked publication-reconstruction contexts while every gate passes, with direct incremental cost capped at $0.",
  ...
  model: structuredClone(POST_CANARY_BATCH_16_PUBLICATION_MODEL),
  costEstimate: {
  ...
  inputs: {
    ...
    scoreException: SCORE_EXCEPTION,
  ...
  acceptanceContract: {
    ...
    movesAuthoredRequired: 194,
    critiquesRequired: 194,
  ...
  nextAuthorizedAction:
    "standing-authorization-permits-activation-and-execution-of-the-ten-frozen-batch-16-publication-contexts"
};
