...
const preparationPath =
  `${POST_CANARY_BATCH_09_COMPATIBILITY_ROOT}/preparation-manifest.json`;
const analysisPath =
  `${POST_CANARY_BATCH_09_COMPATIBILITY_ROOT}/analysis.json`;
...
assert.equal(preparation.batchNumber, 9);
...
assert.equal(
  ...
  "batch-09-site-ledger-adapter-and-validator-route"
);
assert.equal(
...
assert.equal(
  await exists(`${POST_CANARY_BATCH_09_COMPATIBILITY_ROOT}/execution.json`),
  false
);
assert.equal(
  await exists(
...
assert.deepEqual(packetNumbers, POST_CANARY_BATCH_09_COMPATIBILITY_ORDER);
for (const packetRecord of preparation.artifacts.packets) {
  ...
  assert.equal(packet.batchNumber, 9);
  ...
  const positive = validatePostCanaryBatch09SiteLedgerAdapter({
