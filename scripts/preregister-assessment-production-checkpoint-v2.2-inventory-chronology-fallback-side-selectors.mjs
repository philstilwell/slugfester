...
const SCRIPT =
  "scripts/preregister-assessment-production-checkpoint-v2.2-inventory-chronology-fallback-side-selectors.mjs";
const TEST =
  "scripts/test-assessment-production-checkpoint-v2.2-inventory-chronology-fallback-side-selector-manifest.mjs";
...
assertV4(
  sidePreparation.status ===
      "twenty-exact-production-checkpoint-v2.2-side-selector-packets-frozen-not-authorized" &&
    sidePreparation.developmentValidationOnly === false &&
    sidePreparation.productionCanary === true &&
    ...
    sidePreparation.totals?.candidatesTransported === 332 &&
    sidePreparation.totals?.modelContextsExecuted === 0 &&
    sidePreparation.gateDisposition
      ?.failedProductionCanaryV1PreservedFailed === true &&
    sidePreparation.gateDisposition
      ?.failedProductionCanaryV1OutputsUsedAsModelInput === false &&
    sidePreparation.validatedInventoryContract?.planAndSideIsolationPreserved ===
      true &&
    sidePreparation.validatedInventoryContract
      ?.fallbackAppliedOnlyToRetainedOrphanReply === true &&
    sidePreparation.validatedInventoryContract?.scoreFieldsAvailable ===
      false &&
    sidePreparation.activePolicy?.version === "v2.2" &&
    sidePreparation.activePolicy?.agreedWinningSideMayCollapseToIntegerRoundedTie === true &&
    sidePreparation.activePolicy?.scorePassesMaximum === 1 &&
    ...
    sidePreparation.nextAuthorizedAction ===
      "prepare-production-checkpoint-v2.2-side-selector-execution-manifest-model-free-only",
...
for (const prepared of sidePreparation.contexts) {
  ...
  assertV4(
    packet.schemaVersion ===
        "1.0-production-checkpoint-v2.2-chronology-fallback-side-selector-packet" &&
      ...
      packet.isolation?.failedProductionCanaryOutputsUnavailable === true &&
      packet.isolation?.validationCohortOutputsUnavailable === true &&
      packet.isolation?.preferredMoveKindRequired === true &&
      packet.isolation?.constructiveOrphanFallbackRequired === true &&
      packet.isolation?.fallbackConditionRepositoryOwned === true &&
...
const manifest = {
  schemaVersion:
    "1.0-production-checkpoint-v2.2-side-selector-execution-preparation-manifest",
  protocolId: sidePreparation.protocolId,
  sideSelectionProtocolId: sidePreparation.sideSelectionProtocolId,
  status:
    "frozen-twenty-production-checkpoint-v2.2-side-selector-contexts-prepared-not-authorized",
  frozenAt,
  ...
  stagingOnly: true,
  AIOnly: true,
  gateDisposition: structuredClone(
    sidePreparation.gateDisposition
  ),
  ...
  costEstimate: {
    ...
    estimateBasis:
      "The prior twenty-context side-selector gate completed in 10.80 wall minutes and 19.74 aggregate model minutes at concurrency two. These chronology-fallback contexts are similar at roughly 37-59 KB but require two additional model-authored fields per retained nomination. Subscription use has no direct incremental API charge.",
  },
  ...
  nextAuthorizedAction:
    "prepare-separate-production-checkpoint-v2.2-side-selector-execution-activation-only",
};
