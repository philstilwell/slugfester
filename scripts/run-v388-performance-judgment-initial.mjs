...
const manifest = JSON.parse(await readFile(path.resolve(root, `${V388_PERFORMANCE_ROOT}/initial-execution-manifest.json`), "utf8"));
const codex = "/Applications/ChatGPT.app/Contents/Resources/codex";
const authSource = path.join(os.homedir(), ".codex", "auth.json");
const bytes = (relativePath) => readFile(path.resolve(root, relativePath));
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
assertV388(manifest.status === "frozen-six-context-authorized" && manifest.authorization.initialPerformanceModelExecution && !manifest.authorization.scoreDerivation && !manifest.stopRules.furtherAutomaticRetryAuthorized, "initial performance execution unauthorized");
...
for (const context of manifest.contexts) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-v388-performance-${context.debateNumber}-${context.pass}-`));
  const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), `slugfester-v388-home-performance-${context.debateNumber}-${context.pass}-`));
  ...
  try {
    ...
    process.stdout.write(`\n[v3.8.8-performance] starting ${manifest.model.label} Debate ${context.debateNumber} Pass ${context.pass}\n`);
