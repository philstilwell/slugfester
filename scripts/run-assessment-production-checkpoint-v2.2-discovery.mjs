...
assertV4(
  activation.status ===
    ...
    activation.executionPolicy.contexts === 36 &&
    ...
    activation.executionPolicy.copiedInputBytesMaximum === 82270 &&
    ...
    Object.values(activation.stopRules).every(Boolean),
  "production checkpoint v2.2 discovery execution is unauthorized"
);
...
assertV4(
  preparationManifest.contexts.length === 36 &&
...
async function executeContext(context, contextIndex) {
  ...
  assertV4(
    context.copiedInputBytes <=
      activation.executionPolicy.copiedInputBytesMaximum,
  ...
  const sourceDirectory = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-production-v22-discovery-${context.debateNumber}-${context.chunkId}-`
    )
  );
  const isolatedCodexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-production-v22-discovery-home-${context.debateNumber}-${context.chunkId}-`
    )
  );
  ...
  process.stdout.write(
    `[production-v2.2-discovery] Debate ${context.debateNumber} ${context.chunkId} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-production-checkpoint-v2.2-discovery-model-execution",
  ...
  status:
    validContexts === preparationManifest.contexts.length
      ...
      : "production-checkpoint-v2.2-discovery-complete-with-failure",
