...
import {
  ...
  POST_CANARY_BATCH_14_RENDERING_PORT,
  POST_CANARY_BATCH_14_RENDERING_PROTOCOL_ID,
  POST_CANARY_BATCH_14_RENDERING_REQUIRED_CHECKS,
  POST_CANARY_BATCH_14_RENDERING_ROOT,
  POST_CANARY_BATCH_14_RENDERING_VIEWPORTS,
  buildPostCanaryBatch14RenderingPacket,
  hashFile,
  sha256
...
const FINALIZATION =
  "docs/assessment-production/post-canary-continuation-v1/batch-14/publication-finalization";
...
const PREPARATION = `${POST_CANARY_BATCH_14_RENDERING_ROOT}/preparation-manifest.json`;
const ACTIVATION = `${POST_CANARY_BATCH_14_RENDERING_ROOT}/execution-activation.json`;
const EXECUTION = `${POST_CANARY_BATCH_14_RENDERING_ROOT}/execution.json`;
const ANALYSIS = `${POST_CANARY_BATCH_14_RENDERING_ROOT}/analysis.json`;
const AUDIT = `${POST_CANARY_BATCH_14_RENDERING_ROOT}/rendering-audit.json`;
const PACKETS = `${POST_CANARY_BATCH_14_RENDERING_ROOT}/packets`;
...
"Batch 14 standing rendering authorization required");
...
assertV4(finalPreparation.status ===
  "frozen-post-canary-batch-14-publication-finalization-prepared" &&
  finalActivation.status ===
  "frozen-post-canary-batch-14-publication-finalization-authorized" &&
  finalExecution.status === "ten-debate-batch-14-publication-finalization-passed" &&
  finalAnalysis.status === "ten-debate-batch-14-publication-finalization-passed" &&
  finalAudit.status === "passed" && finalAudit.rows?.length === 10 &&
  finalAudit.totals?.moves === 190,
"passing Batch 14 finalization required");

let previewText = originalPreviewBytes.toString("utf8");
previewText = previewText.replace(
  "renderPublicationStagingDebate(await response.json());",
  `renderPublicationStagingDebate(await response.json());\n        const banner = document.querySelector("[data-publication-staging-preview] .calibration-preview-note");\n        if (banner) banner.innerHTML = "<strong>Publication staging preview:</strong> validated post-canary Batch 14 candidate only. This scorecard remains excluded from production data and rankings pending rendering and mutation authorization.";`
);
assertV4(previewText !== originalPreviewBytes.toString("utf8"),
  "Batch 14 rendering preview adaptation failed");
const previewBytes = Buffer.from(previewText);

const compatibility = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-14-publication-finalization-compatibility-analysis",
  protocolId: "assessment-production-post-canary-batch-14-publication-finalization",
  status: "batch-14-production-compatibility-boundary-recorded",
  analyzedAt: frozenAt,
  productionCanary: false,
  batchNumber: 14,
  stagingOnly: true,
  findings: [{
    id: "batch-14-site-ledger-adapter-and-validator-route",
    description: "The live validator requires a separately frozen authenticated Batch 14 adapter route before production mutation.",
...
for (const debateNumber of POST_CANARY_BATCH_14_RENDERING_ORDER) {
  ...
  const packet = buildPostCanaryBatch14RenderingPacket({ auditRow: row,
...
const sources = [FINALIZATION_PREPARATION, FINALIZATION_ACTIVATION,
  ...
  ORIGINAL_PREVIEW, POST_CANARY_BATCH_14_STANDING_AUTHORIZATION,
  "src/app.js", "src/styles.css",
  "scripts/lib/assessment-production-post-canary-batch-14-rendering-verification.mjs",
  ...
  "scripts/prepare-assessment-production-post-canary-batch-14-rendering-verification.mjs",
  "scripts/test-assessment-production-post-canary-batch-14-rendering-verification-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-14-rendering-verification.mjs",
  "scripts/run-assessment-production-post-canary-batch-14-rendering-verification.mjs",
  "scripts/finalize-assessment-production-post-canary-batch-14-rendering-verification.mjs",
...
const manifest = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-14-rendering-preparation",
  protocolId: POST_CANARY_BATCH_14_RENDERING_PROTOCOL_ID,
  status: "frozen-post-canary-batch-14-rendering-verification-prepared",
  frozenAt,
  ...
  explicitOrder: POST_CANARY_BATCH_14_RENDERING_ORDER,
  viewports: POST_CANARY_BATCH_14_RENDERING_VIEWPORTS,
  requiredChecks: POST_CANARY_BATCH_14_RENDERING_REQUIRED_CHECKS,
  browserPlan: {
    controller: "playwright-cli",
    browserFamily: "Chromium",
    sessionName: "batch14-rendering",
    port: POST_CANARY_BATCH_14_RENDERING_PORT,
  ...
  artifacts: {
    ...
    evidenceRoot: `${POST_CANARY_BATCH_14_RENDERING_ROOT}/evidence`,
    screenshotRoot: "output/playwright/batch-14-rendering",
  ...
  nextAuthorizedAction: "activate-batch-14-rendering-verification"
};
...
console.log(JSON.stringify({ status: manifest.status, debates: 10,
  sections: 52, moves: 190, viewportResultsPlanned: 20,
