...
assertV4(
  ...
  "v2.1 discovery execution is unauthorized"
);
...
assertV4(
  preparationManifest.contexts.length === 40 &&
...
async function executeContext(context, contextIndex) {
  ...
  const sourceDirectory = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-v21-discovery-${context.debateNumber}-${context.chunkId}-`
    )
  );
  const isolatedCodexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-v21-discovery-home-${context.debateNumber}-${context.chunkId}-`
    )
  );
  ...
  try {
    ...
    process.stdout.write(
      `[v2.1-discovery] starting ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber} ${context.chunkId}\n`
    );
  ...
  process.stdout.write(
    `[v2.1-discovery] Debate ${context.debateNumber} ${context.chunkId} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-score-stability-v2.1-fresh-validation-discovery-model-execution",
  protocolId: activation.protocolId,
  status:
    validContexts === preparationManifest.contexts.length
      ? "forty-v2.1-validation-discovery-contexts-passed"
      : "v2.1-validation-discovery-complete-with-failure",
