...
const expectedDebates = ["193", "78", "02"];
const expectedMoves = [
  "con-authentic-eschatological-judge-sayings",
  "pro-gospel-patristic-source-convergence",
  "con-selective-hostile-source-survival",
  "con-jewish-sect-and-syrian-divergence",
...
async function validatePreparation(preparation) {
  ...
  assert.equal(preparation.calls.length, 5);
...
async function activate() {
  ...
  const activation = {
    ...preparation,
    schemaVersion: "1.0-assessment-production-post-canary-batch-07-audio-verification-execution-manifest",
    status: "frozen-five-post-canary-batch-07-paid-known-speaker-diarizations-authorized-under-standing-authorization",
    activatedAt,
    ...
    userExecutionAuthorization: {
      ...
      frozenUsageDerivedEstimateUsd: 0.1654,
      verificationCallsAuthorized: 5,
    ...
    nextAuthorizedAction: "execute-exactly-five-post-canary-batch-07-paid-audio-verification-calls-once-sequentially"
  };
...
async function validateActivation(manifest) {
  ...
  assert.equal(manifest.calls.length, 5);
...
async function run() {
  ...
  for (const [callIndex, call] of manifest.calls.entries()) {
    ...
    process.stdout.write(`[batch-07-audio] ${callIndex + 1}/5 starting ${call.debateNumber}:${call.moveId}\n`);
    ...
    results.push(result);
    process.stdout.write(`[batch-07-audio] ${callIndex + 1}/5 ${result.status}; cumulative $${result.cumulativeUsageDerivedEstimatedCostUsd.toFixed(7)}\n`);
  }
  ...
  const execution = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-07-audio-verification-model-execution",
    protocolId: manifest.protocolId,
    status: completed.length === 5
      ? "five-post-canary-batch-07-paid-known-speaker-diarizations-completed"
      : "post-canary-batch-07-paid-diarization-incomplete",
    provider: "OpenAI Transcription API",
...
async function analyze() {
  ...
  const executionComplete = execution.callsCompleted === 5;
  const passed = executionComplete && verified === 5;
  const status = passed
    ? "passed-all-five-post-canary-batch-07-confidence-moves-audio-verified"
    : executionComplete
      ? "post-canary-batch-07-audio-verification-unresolved"
      : "post-canary-batch-07-audio-verification-incomplete";
  const nextAuthorizedAction = execution.costCapReachedOrExceeded || !execution.directIncrementalCostCapControlPassed
    ? "user-review-required-before-any-batch-07-downstream-work-after-audio-cost-cap-event"
    : passed
      ? "prepare-freeze-and-push-batch-07-dispute-only-adjudication-packets-under-standing-authorization"
      : "standing-authorization-stop-new-approval-required-before-batch-07-audio-verification-failure-diagnosis";
  const authorization = {
  ...
  const audit = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-07-audio-verification-audit",
    protocolId: manifest.protocolId,
    status,
  ...
  const analysis = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-07-audio-verification-analysis",
    protocolId: manifest.protocolId,
    status,
    productionCanary: false,
    batchNumber: 7,
...
async function cost() {
  ...
  const result = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-07-audio-cost-control-analysis",
    ...
    costControl: {
      ...
      usageDerivedPlanningEstimateUsd: 0.1654,
      approvedMaximumCostUsd: 1,
      usageDerivedEstimatedCostUsd: totals.usageDerivedEstimatedCostUsd,
      estimateDifferenceUsd: totals.usageDerivedEstimatedCostUsd - 0.1654,
...
async function test() {
  ...
  if (!(await exists(paths.activation))) {
    console.log(JSON.stringify({ status: "passed-preactivation", calls: 5, paidCalls: 0 }, null, 2));
    return;
  }
  ...
  if (!(await exists(paths.execution))) {
    ...
    console.log(JSON.stringify({ status: "passed-activated", calls: 5, retries: 0, maximumAuthorizedCostUsd: 1 }, null, 2));
    return;
  }
  const execution = await readJson(paths.execution);
  assert.equal(execution.callsPlanned, 5);
  ...
  assert(execution.callsAttempted <= 5);
  assert.equal(execution.retries, 0);
  ...
  if (!execution.requestFailure && !execution.costCapReachedOrExceeded) {
    assert.equal(execution.callsAttempted, 5);
    assert.equal(execution.callsCompleted, 5);
  ...
  assert.equal(audit.totals.requiredMoves, 5);
  assert.equal(audit.totals.verified + audit.totals.unresolved, 5);
  assert.equal(audit.totals.retries, 0);
  assert.equal(analysis.gate.requiredMoves, 5);
  assert.equal(analysis.gate.verified + analysis.gate.unresolved, 5);
  ...
  assert.equal(costResult.costControl.usageDerivedPlanningEstimateUsd, 0.1654);
