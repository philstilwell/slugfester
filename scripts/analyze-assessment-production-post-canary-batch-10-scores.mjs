...
import {
  POST_CANARY_BATCH_10_CALCULATED_SCORES_VERSION,
...
const manifest = JSON.parse(
  await readFile(
    path.resolve(`${POST_CANARY_BATCH_10_SCORE_ROOT}/score-pass-manifest.json`),
    "utf8"
  )
);
...
const validation = validatePostCanaryBatch10Scores(
  scores,
  ledger,
  inputs.debateInputs,
...
assertV4(
  ...
  "post-canary Batch 10 score artifact identity mismatch"
);
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-10-score-analysis",
  protocolId: scores.protocolId,
  status: passed
    ? "post-canary-batch-10-score-stability-gate-passed"
    : "post-canary-batch-10-score-stability-gate-failed",
  ...
  nextAuthorizedAction: passed
    ...
    : "standing-authorization-permits-batch-10-score-stability-failure-diagnosis"
};
if (shouldWrite) {
  await writeFile(
