...
const preparationPath =
  `${POST_CANARY_BATCH_14_COMPATIBILITY_ROOT}/preparation-manifest.json`;
const analysisPath =
  `${POST_CANARY_BATCH_14_COMPATIBILITY_ROOT}/analysis.json`;
...
assert.equal(
  ...
  "batch-14-site-ledger-adapter-and-validator-route"
);
assert.equal(
...
assert.equal(preparation.totals.sections, 52);
assert.equal(preparation.totals.moves, 190);
assert.equal(preparation.totals.oneSidedDisplayRows, 14);
assert.equal(preparation.totals.overallBlunders, 55);
assert.equal(preparation.totals.emptyReferenceLinks, 45);
assert.equal(preparation.totals.suppliedReferenceLinks, 10);
...
assert.equal(
  await exists(`${POST_CANARY_BATCH_14_COMPATIBILITY_ROOT}/execution.json`),
  false
);
assert.equal(
  await exists(
...
assert.deepEqual(packetNumbers, POST_CANARY_BATCH_14_COMPATIBILITY_ORDER);
for (const packetRecord of preparation.artifacts.packets) {
  ...
  assert.equal(packet.batchNumber, 14);
  ...
  const positive = validatePostCanaryBatch14SiteLedgerAdapter({
