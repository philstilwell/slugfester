...
import { POST_CANARY_BATCH_07_RESUMPTION_2_REPAIR_ROOT, validateResumption2RepairOutput } from
...
assertV4(activation.status === "frozen-twelve-context-batch-07-publication-resumption-2-repair-authorized" &&
  activation.contexts?.length === 12 && activation.authorization?.repairModelExecution === true &&
...
"the Batch 7 resumption-2 repair execution is not authorized");
...
async function runContext(context) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-07-resumption-2-repair-${context.contextIndex}-`));
  const codexHome = await mkdtemp(path.join(os.tmpdir(), `batch-07-resumption-2-repair-home-${context.contextIndex}-`));
  ...
  try { const copies = [[activation.modelInputs.productionWorkflow, "production-workflow.md"],
    ...
    const prompt = ["Read production-workflow.md, readiness-workflow.md, output-contract.md, manual.md, packet.json, and schema.json completely and no other files.",
      `Act only as isolated Batch 7 resumption-2 bounded repair editor ${context.contextIndex} for Debate ${context.debateNumber}.`,
    ...
    process.stdout.write(`[batch-07-resumption-2-repair] starting index ${context.contextIndex} Debate ${context.debateNumber} ${activation.model.label}/${activation.model.reasoningEffort}\n`);
    ...
    const base = { contextIndex: context.contextIndex, packetIndex: context.packetIndex,
      debateNumber: context.debateNumber, debateId: context.debateId, model: activation.model.label,
    ...
    if (invocation.error || invocation.timedOut || invocation.code !== 0 || invocation.signal !== null || !resultExists) {
      ...
      try { validationSummary = validateResumption2RepairOutput(JSON.parse(outputBytes),
      ...
      const validation = { schemaVersion: "1.0-assessment-production-post-canary-batch-07-publication-resumption-2-repair-validation",
        ...
        debateNumber: context.debateNumber, outputSha256: sha256(outputBytes), validationSummary,
        validationMessage, modelAuthoredScores: 0 };
      const provenance = { schemaVersion: "1.0-assessment-production-post-canary-batch-07-publication-resumption-2-repair-provenance",
        protocolId: activation.protocolId, contextIndex: context.contextIndex,
        packetIndex: context.packetIndex, debateNumber: context.debateNumber, model: activation.model,
    ...
    debateNumber: context.debateNumber, attemptCount: 1, retryCount: 0, timeoutExtensionCount: 0,
  ...
  process.stdout.write(`[batch-07-resumption-2-repair] index ${context.contextIndex} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`); return record;
}
async function runBounded(indexes, maximumParallelContexts) {
  const phaseResults = []; let cursor = 0; let failureObserved = false;
  ...
  return phaseResults.sort((left, right) => left.contextIndex - right.contextIndex);
}
...
const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-07-publication-resumption-2-repair-execution",
  protocolId: activation.protocolId, status: validContexts === 12
    ? "batch-07-publication-resumption-2-twelve-context-repair-gate-passed"
    : "batch-07-publication-resumption-2-repair-gate-complete-with-failure",
  gateStartedAt, gateCompletedAt: new Date().toISOString(), contextsPlanned: 12,
  contextsAttempted: results.length, contextsUnattempted: 12 - results.length,
  validContexts, invalidContexts: results.length - validContexts, attempts: results.length,
  retries: 0, timeoutExtensions: 0, recursiveCorrections: 0, schedulerRamp: [1, 2],
