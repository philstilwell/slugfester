...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-13/discovery";
...
assert.equal(manifest.status, "frozen-forty-one-post-canary-batch-13-discovery-contexts-prepared-not-authorized");
...
assert.equal(manifest.contexts.length, 41);
assert.deepEqual(manifest.contexts.map((context) => context.contextIndex), Array.from({ length: 41 }, (_, index) => index));
...
assert.equal(manifest.executionPolicy.contexts, 41);
...
assert.equal(manifest.executionPolicy.copiedInputBytesMaximum, 65893);
assert.equal(preparation.totals.maximumCopiedInputBytes, 65893);
...
assert.equal(manifest.copiedInputBoundary.frozenObservedCopiedInputBytesMaximum, 65893);
assert.equal(manifest.copiedInputBoundary.executionCopiedInputBytesMaximum, 65893);
assert.deepEqual(manifest.copiedInputBoundary.maximumContext, { debateNumber: "117", chunkId: "chunk-002" });
...
assert.equal(manifest.costEstimate.contexts, 41);
...
assert.equal(preparedByContext.size, 41);
for (const context of manifest.contexts) {
  ...
  assert(buildBatch13TokenCountedChunkLedger(chunkBytes).equals(tokenBytes));
...
assert.equal(manifest.nextAuthorizedAction, "prepare-and-freeze-batch-13-discovery-execution-activation-under-standing-authorization");

const validation = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-discovery-execution-preparation-validation",
  protocolId: manifest.protocolId,
  status: "batch-13-discovery-execution-manifest-validation-passed-frozen-standing-authorization-active",
