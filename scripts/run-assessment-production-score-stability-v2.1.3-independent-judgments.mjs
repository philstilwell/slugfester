...
const ROOT =
  "docs/assessment-production/score-stability-v2.1.3-validation-cohort/independent-judgments";
...
assertV4(
  activation.status ===
      "frozen-twenty-v2.1.3-independent-judgment-contexts-authorized" &&
  ...
  "v2.1.3 independent-judgment execution is unauthorized"
);
assertV4(
...
async function executeContext(context, contextIndex) {
  ...
  assertV4(
    remainingGateMs > 0,
    "absolute v2.1.3 independent-judgment gate timeout reached before launch"
  );
  ...
  const temporary = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-v213-judgment-${context.debateNumber}-${context.reviewerPass.toLowerCase()}-`
    )
  );
  const isolatedCodexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-v213-judgment-home-${context.debateNumber}-${context.reviewerPass.toLowerCase()}-`
    )
  );
  ...
  try {
    ...
    if (
      ...
      const validation = await run(
        process.execPath,
        [
          "scripts/validate-assessment-production-score-stability-v2.1.3-independent-judgment.mjs",
  ...
  process.stdout.write(
    `[v2.1.3-judgment] Debate ${context.debateNumber} Pass ${context.reviewerPass} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-score-stability-v2.1.3-independent-judgment-model-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "twenty-v2.1.3-independent-judgment-contexts-passed"
    : "v2.1.3-independent-judgment-gate-complete-with-failure",
