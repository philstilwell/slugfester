...
const chronologyDiagnosticPath = "docs/calibration/v4.2.2/chronology-first-smoke/analysis.json";
const workflowPath = "docs/assessment-workflow-v4.2.3.md";
const scriptPath = "scripts/select-v423-fresh-six.mjs";
const outputPath = "docs/calibration/v4.2.3/chronology-first-fresh-six-gate/source-only-sample.json";
const salt = "slugfester-v4.2.3-chronology-first-fresh-six-source-only";
...
const [poolText, chronologyText, ...priorSampleTexts] = await Promise.all([poolPath, chronologyDiagnosticPath, ...priorSamplePaths].map((file) => readFile(path.resolve(root, file), "utf8")));
const pool = JSON.parse(poolText);
const chronologyDiagnostic = JSON.parse(chronologyText);
...
if (chronologyDiagnostic.status !== "chronology-smoke-passed-new-fresh-gate-preparation-authorized" || !chronologyDiagnostic.authorization.newDisjointFreshGatePreparation) throw new Error("v4.2.2 chronology diagnostic did not authorize preparation");
...
const priorVersionNames = ["v417", "v418", "v419", "v421"];
...
if (new Set(selected.map((candidate) => candidate.debateId)).size !== 6 || Object.values(bandCounts).some((count) => count < 1) || bandCounts["long-over-120"] > 3) throw new Error("v4.2.3 sample invariants failed");
if (selected.some((candidate) => priorFreshIds.includes(candidate.debateId))) throw new Error("v4.2.3 sample overlaps a prior fresh gate");

const sourceHashes = { [poolPath]: sha256(poolText), [chronologyDiagnosticPath]: sha256(chronologyText), [workflowPath]: sha256(await readFile(path.resolve(root, workflowPath))), [scriptPath]: sha256(await readFile(path.resolve(root, scriptPath))) };
...
const artifact = {
  ...
  authorization: { compactChronologySourcePacketPreparation: true, primaryModelExecution: false, legacyComparison: false, paidTranscription: false, productionMutation: false, heldOutGate: false, all195Debates: false },
  sourceHashes
};
if (shouldWrite) {
