...
const analysisPath = `${POST_CANARY_BATCH_17_PRODUCTION_PUBLICATION_ROOT}/preparation-analysis.json`;
const [manifest, analysis] = await Promise.all([readJson(manifestPath), readJson(analysisPath)]);

assert.equal(manifest.protocolId, POST_CANARY_BATCH_17_PRODUCTION_PUBLICATION_PROTOCOL_ID);
assert.equal(manifest.status, "frozen-batch-17-production-publication-mutation-manifest-prepared");
...
assert.deepEqual(manifest.explicitOrder, POST_CANARY_BATCH_17_PRODUCTION_PUBLICATION_ORDER);
...
assert.equal(manifest.productionBaseline.existingProductionLedgers.count, 170);
assert.equal(manifest.productionBaseline.batchProductionLedgersAbsent, 4);
assert.equal(manifest.productionBaseline.batchProductionLedgersPresent, 0);
assert.equal(manifest.debates.length, 4);
...
assert.equal(manifest.executionContract.exactProductionWrites.length, 5);
...
assert.equal(analysis.status, "batch-17-production-publication-mutation-manifest-freeze-passed");
...
assert.equal(analysis.totals.productionWritesPrepared, 5);
assert.equal(await exists(`${POST_CANARY_BATCH_17_PRODUCTION_PUBLICATION_ROOT}/execution-activation.json`), false);
assert.equal(await exists(`${POST_CANARY_BATCH_17_PRODUCTION_PUBLICATION_ROOT}/execution.json`), false);

console.log(serializedJson({ status: "passed", debates: manifest.debates.length, lockedProductionRecords: 4, productionMutationPerformed: false }));
