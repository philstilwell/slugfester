...
const root = process.cwd(), manifestText = await readFile(path.resolve(root, V376_EXECUTION_MANIFEST), "utf8"), manifest = JSON.parse(manifestText), codex = "/Applications/ChatGPT.app/Contents/Resources/codex", concurrency = Number(process.env.SLUGFESTER_MODEL_CONCURRENCY ?? 2);
...
async function modelTask(context) {
  const { debateNumber, reviewerPass, packet, schema, output, bundleCount } = context, temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-v376-${reviewerPass}-${debateNumber}-`)), temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), `slugfester-v376-home-${reviewerPass}-${debateNumber}-`)), startedAt = new Date().toISOString();
  try {
    ...
    process.stdout.write(`\n[v3.7.6] starting ${manifest.model.label} Debate ${debateNumber} ${reviewerPass}\n`);
    ...
    try {
      ...
      const outputText = await readFile(outputPath, "utf8"), parsed = JSON.parse(outputText), validation = await run(process.execPath, ["scripts/validate-v376-burden-contact-output.mjs", output, packet, schema], { cwd: root, env: process.env }), valid = validation.code === 0;
      process.stdout.write(`[v3.7.6] Debate ${debateNumber} ${reviewerPass} ${valid ? "completed-valid" : "output-validation-failed"}\n`);
...
if (!disagreement.allInitialValid) adjudicationExecution = executionRecord("3.7.6-adjudication-burden-contact-execution", new Date().toISOString(), [], []);
else { const adjudicationStartedAt = new Date().toISOString(), adjudicationResults = await pool(disagreement.adjudicationContexts.map((context) => () => modelTask(context))); adjudicationExecution = executionRecord("3.7.6-adjudication-burden-contact-execution", adjudicationStartedAt, disagreement.adjudicationContexts, adjudicationResults); }
await writeFile(path.resolve(root, manifest.artifacts.adjudicationExecution), `${JSON.stringify(adjudicationExecution, null, 2)}\n`);
const analysisRun = await run(process.execPath, ["scripts/analyze-v376-burden-contact-smoke.mjs", "--write"], { cwd: root, env: process.env }); assert(analysisRun.code === 0, `analysis failed: ${analysisRun.stderr}`);
const analysis = JSON.parse(await readFile(path.resolve(root, manifest.artifacts.analysis), "utf8"));
console.log(JSON.stringify({ status: "burden-contact-smoke-execution-recorded", passed: analysis.passed, initialValidContexts: initialExecution.validOutputContexts, initialAgreements: disagreement.counts.agreements, initialDisagreements: disagreement.counts.disagreements, adjudicationContexts: adjudicationExecution.contextsPlanned, adjudicationValidContexts: adjudicationExecution.validOutputContexts, retries: initialExecution.totalRetries + adjudicationExecution.totalRetries, meteredApiCostUsd: 0 }, null, 2));
