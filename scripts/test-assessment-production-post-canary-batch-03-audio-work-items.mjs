...
assert.equal(work.batchNumber, 3);
assert.equal(work.stagingOnly, true);
assert.equal(preparation.userAuthorization.workItemsAuthorized, 8);
assert.equal(
...
assert.equal(preparation.totals.moves, 8);
assert.equal(work.moves.length, 8);
...
assert.equal(preparation.validation.exactAuthorizedMoveCount, 8);
assert.equal(
  preparation.validation.canonicalLocalTextAndMetadataChainsVerified,
  5
);
assert.equal(preparation.validation.repositoryRenderedLockedExcerpts, 8);
assert.equal(preparation.validation.repositoryRenderedTimestampWindows, 8);
assert.equal(preparation.validation.expectedSpeakersLocked, 8);
assert.equal(
...
assert.deepEqual(preparation.sourceCompatibility.occurrences, []);
