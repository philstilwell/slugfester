...
import {
  ...
  validateCheckpointV22RenderingRemedyV1ViewportEvidence
...
const activationPath = `${CHECKPOINT_V22_RENDERING_REMEDY_V1_ROOT}/execution-activation.json`;
const [activation, execution, analysis, audit] = await Promise.all([
  parse(activationPath),
  parse(`${CHECKPOINT_V22_RENDERING_REMEDY_V1_ROOT}/execution.json`),
  parse(`${CHECKPOINT_V22_RENDERING_REMEDY_V1_ROOT}/analysis.json`),
  parse(`${CHECKPOINT_V22_RENDERING_REMEDY_V1_ROOT}/rendering-audit.json`)
]);
...
assert.equal(activation.authorization.replacementRenderingVerification, true);
...
assert.equal(execution.status, "ten-debate-replacement-rendering-verification-passed");
...
assert.equal(analysis.status, "ten-debate-replacement-rendering-verification-passed");
...
assert.equal(
  audit.schemaVersion,
  "1.0-production-checkpoint-v2.2-rendering-remedy-v1-audit"
);
assert.equal(audit.protocolId, CHECKPOINT_V22_RENDERING_REMEDY_V1_PROTOCOL_ID);
assert.equal(audit.status, "passed-ten-debate-replacement-rendering-verification");
assert.deepEqual(audit.explicitOrder, CHECKPOINT_V22_RENDERING_REMEDY_V1_ORDER);
...
for (const debateNumber of CHECKPOINT_V22_RENDERING_REMEDY_V1_ORDER) {
  const packetRow = activation.packets.find((item) => item.debateNumber === debateNumber);
  const auditRow = audit.rows.find((item) => item.debateNumber === debateNumber);
  assert.ok(packetRow && auditRow, `${debateNumber}: replacement rendering row missing`);
