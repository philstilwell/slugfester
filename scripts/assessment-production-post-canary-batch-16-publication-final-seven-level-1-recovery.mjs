...
const recoveryRoot = `${publicationRoot}/final-seven-failure-recovery`;
const root = `${recoveryRoot}/level-1`;
const protocolId = "assessment-production-post-canary-batch-16-publication-final-seven-level-1-one-field-recovery";
...
const files = {
  ...
  continuationActivation: `${publicationRoot}/final-seven-execution-activation.json`,
  continuationExecution: `${publicationRoot}/model-execution-final-seven.json`,
  continuationAnalysis: `${publicationRoot}/final-seven-analysis.json`,
...
const validateCorrection = (text, failure, packet) => {
  ...
  if (failure.kind === "representative-quote-context") {
    const words = wordCount(value);
    assert(words >= 12 && words <= 55, `${failure.field}: quote context word count ${words}`);
    return { kind: failure.kind, words };
  }
...
if (mode === "--prepare") {
  ...
  assert.equal(diagnosis.status, "batch-16-publication-final-seven-debate-92-nine-field-failure-diagnosed");
  assert.equal(diagnosis.totals.invalidFields, 9);
  assert.equal(continuationExecution.contextsAttempted, 7);
  ...
  const contexts = diagnosis.failures.map((failure, contextIndex) => {
    ...
    const target = failure.kind === "critique"
      ...
      : { ...failure, lockedMove: source.packet.moves.find((move) => move.moveId === failure.sourceMoveId), immutableCompanionFields: { sourceMoveId: source.output.representativeQuotes[failure.side].sourceMoveId, text: source.output.representativeQuotes[failure.side].text }, rejectedPriorStringAvailable: false };
    const repairPacket = { schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-final-seven-one-field-repair-packet", protocolId, contextIndex, shardId, debateNumber: failure.debateNumber, debateId: source.packet.debateId, assessmentModel: model.label, recoveryLevel: 1, writableField: failure.field, target, allOtherFieldsUnavailableAndImmutable: true, participantJudgmentClosed: true, publicationScoreLocked: true };
    const properties = {
      schemaVersion: { type: "string", const: "1.0-assessment-production-post-canary-batch-16-publication-final-seven-one-field-repair-output" },
...
if (mode === "--activate") {
  ...
  assert.equal(preparation.status, "frozen-nine-one-field-batch-16-publication-final-seven-recovery-level-1-prepared-not-activated");
  ...
  const activation = { ...preparation, schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-final-seven-level-1-recovery-activation", status: "frozen-nine-one-field-batch-16-publication-final-seven-recovery-level-1-authorized", activatedAt: at, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(), preparation: { path: files.preparation, sha256: sha256(preparationBytes) }, authorization: { ...preparation.authorization, modelExecution: true, deterministicMergeAndValidation: true }, nextAuthorizedAction: "execute-nine-fresh-one-field-batch-16-publication-final-seven-recovery-level-1-contexts-once" };
...
if (mode === "--run") {
  ...
  assert.equal(activation.status, "frozen-nine-one-field-batch-16-publication-final-seven-recovery-level-1-authorized");
  ...
  const execute = async (context) => {
    const temporary = await mkdtemp(path.join(os.tmpdir(), `slugfester-b16-pub-cont-r1-${context.contextIndex}-`));
    const temporaryHome = await mkdtemp(path.join(os.tmpdir(), `slugfester-b16-pub-cont-r1-home-${context.contextIndex}-`));
    ...
    try {
      ...
      const prompt = "Read workflow.md, output-contract.md, manual.md, packet.json, and schema.json completely and no other files. Act only as the fresh isolated 5.6 Sol/low Batch 16 one-field publication recovery editor. Author exactly the single writable field. The rejected prior string is unavailable. Return exactly one schema-conforming JSON object and no commentary.";
  ...
  for (let offset = 0; offset < activation.contexts.length; offset += 2) {
    const pair = activation.contexts.slice(offset, offset + 2); console.error(`[batch-16-publication-final-seven-recovery-1] starting ${pair.map((item) => `${item.debateNumber}:${item.field}`).join(", ")}`);
    const pairResults = await Promise.all(pair.map(execute)); results.push(...pairResults); for (const result of pairResults) console.error(`[batch-16-publication-final-seven-recovery-1] ${result.debateNumber}:${result.field} ${result.status} ${(result.elapsedMs / 60000).toFixed(2)}m`);
  }
  const passed = results.every((item) => item.gateAcceptancePassed);
  const execution = { schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-final-seven-level-1-recovery-execution", protocolId, status: passed ? "nine-one-field-batch-16-publication-final-seven-recovery-level-1-execution-passed" : "batch-16-publication-final-seven-recovery-level-1-execution-failed", startedAt, completedAt: new Date().toISOString(), elapsedMs: Date.now() - started, contextsAttempted: 9, validContexts: results.filter((item) => item.gateAcceptancePassed).length, invalidContexts: results.filter((item) => !item.gateAcceptancePassed).length, attempts: 9, retries: 0, timeoutExtensions: 0, directIncrementalCostUsd: 0, paidServiceCalls: 0, modelAuthoredScores: 0, results, authorization: { analysis: true, retries: false, furtherRecoveryLevel: !passed, paidServices: false, scorePass: false, productionMutation: false, nextBatchSelection: false } };
  await writeFile(path.resolve(files.execution), pretty(execution));
  console.log(pretty({ status: execution.status, contextsAttempted: 9, validContexts: execution.validContexts, invalidContexts: execution.invalidContexts, elapsedMinutes: Number((execution.elapsedMs / 60000).toFixed(2)), attempts: 9, retries: 0, recoveryLevel: 1, directIncrementalCostUsd: 0 })); process.exit(0);
}
...
if (mode === "--analyze") {
  ...
  if (invalid.length) {
    const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-final-seven-level-1-recovery-analysis", protocolId, status: "batch-16-publication-final-seven-recovery-level-1-gate-failed", recoveryLevel: 1, recoveryLevelsMaximum: 2, originalFailuresPreserved: true, contexts: 9, attempts: 9, retries: 0, validContexts: execution.validContexts, invalidContexts: invalid.length, failedFields: invalid.map((item) => `${item.debateNumber}:${item.field}`), rejectedRawOutputsPreserved: invalid.every((item) => item.outputWritten), directIncrementalCostUsd: 0, authorization: { recoveryLevel2Preparation: true, scorePass: false, productionMutation: false, nextBatchSelection: false }, nextAuthorizedAction: "prepare-minimum-fresh-field-disjoint-batch-16-publication-final-seven-recovery-level-2-shards" };
    if (shouldWrite) await writeFile(path.resolve(files.analysis), pretty(analysis)); console.log(pretty(analysis)); process.exit(0);
  }
  assert.equal(execution.status, "nine-one-field-batch-16-publication-final-seven-recovery-level-1-execution-passed");
  ...
  for (const debateNumber of diagnosis.failedDebates) {
    ...
    for (const context of contexts) {
      ...
      if (context.kind === "critique") merged.moveProse[context.failure.moveId].critique = correction.text;
      else if (context.kind === "representative-quote-context") merged.representativeQuotes[context.failure.side].context = correction.text;
      else merged.representativeQuotes[context.failure.side].text = correction.text;
    }
    const replay = structuredClone(merged);
    for (const context of contexts) {
      if (context.kind === "critique") replay.moveProse[context.failure.moveId].critique = failedOutput.moveProse[context.failure.moveId].critique;
      else if (context.kind === "representative-quote-context") replay.representativeQuotes[context.failure.side].context = failedOutput.representativeQuotes[context.failure.side].context;
      else replay.representativeQuotes[context.failure.side].text = failedOutput.representativeQuotes[context.failure.side].text;
    }
    assert.deepEqual(replay, failedOutput, `${debateNumber}: non-target companion field changed`);
    const validation = validatePostCanaryBatch16PublicationOutput(merged, packet); const mergedBytes = Buffer.from(pretty(merged));
  ...
  const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-final-seven-level-1-recovery-analysis", protocolId, status: "batch-16-publication-final-seven-level-1-one-field-recovery-passed", recoveryLevel: 1, recoveryLevelsMaximum: 2, originalFailuresPreserved: true, contexts: 9, attempts: 9, retries: 0, repairedValidationFields: 9, recoveredDebates: diagnosis.failedDebates, validationResults, directIncrementalCostUsd: 0, paidServiceCalls: 0, modelAuthoredScores: 0, authorization: { completeCohortFinalization: true, furtherRecoveryLevel: false, scorePass: false, productionMutation: false, nextBatchSelection: false }, nextAuthorizedAction: "finalize-and-validate-the-complete-ten-debate-batch-16-publication-cohort" };
  if (shouldWrite) await writeFile(path.resolve(files.analysis), pretty(analysis)); console.log(pretty({ status: analysis.status, recoveredDebates: analysis.recoveredDebates, contexts: 9, attempts: 9, retries: 0, repairedValidationFields: 9, acceptedCompanionFieldsChanged: false, directIncrementalCostUsd: 0, nextAuthorizedAction: analysis.nextAuthorizedAction }));
}
