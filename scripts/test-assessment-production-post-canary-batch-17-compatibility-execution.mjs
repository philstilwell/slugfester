...
const root = POST_CANARY_BATCH_17_COMPATIBILITY_ROOT;
...
const validator = await import(
  `./validate-debates.mjs?batch17Compatibility=${activation.validator.proposedSha256}`
);
...
for (const lock of activation.packetHashes) {
  ...
  const routeLock =
    validator.validatePostCanaryBatch17LedgerAdapterRouteLocks({
      debate: candidate,
      ledgerText,
  ...
  const route = validator.validatePostCanaryBatch17LedgerAdapterRoute({
    debate: candidate,
    ledger,
    ledgerText
  });
  assert.equal(route.repositoryScoreReplayPassed, true);
  const adapter = validatePostCanaryBatch17SiteLedgerAdapter({
    adapter: ledger,
    candidate,
...
assert.equal(positiveRoutes, 4);
assert.equal(negativeControls, 16);
assert.equal(sections, 21);
assert.equal(moves, 79);
assert.equal(oneSidedDisplayRows, 5);
assert.equal(emptyReferenceLinks, 14);
  ...
  JSON.stringify(
    {
      ...
      positiveRoutes,
      negativeControls,
      existingAdapterRoutesPreserved: 17,
      modelContexts: 0,
      paidServiceCalls: 0,
