...
assert.equal(preparation.calls.length, 1);
assert.deepEqual(preparation.calls.map((call) => `${call.debateNumber}:${call.moveId}`), expectedMoves);
assert.deepEqual(preparation.scope.debates, ["77"]);
assert.equal(preparation.scope.frozenTargetClips, 1);
assert.equal(preparation.scope.verificationCalls, 1);
assert.equal(preparation.scope.sameDebateSpeakerSupportReferences, 2);
assert.deepEqual(preparation.scope.sourceCompatibility.occurrences, []);
assert.equal(preparation.referenceContract.references.length, 2);
...
assert.equal(preparation.costEstimate.clipSeconds, 166.76);
assert.equal(preparation.costEstimate.clipMinutes, 2.7793);
assert.equal(preparation.costEstimate.durationOnlyPlanningEstimateUsd, 0.016676);
assert.equal(preparation.costEstimate.durationOnlyEstimateAuthorizedByUser, true);
assert(preparation.costEstimate.usageDerivedPlanningEstimateUsd > 0);
assert.equal(
  preparation.costEstimate.primaryExpectedFutureExecutionCostUsd,
  preparation.costEstimate.usageDerivedPlanningEstimateUsd
);
assert.equal(preparation.costEstimate.maximumConditionallyAuthorizedCostUsd, 1);
assert(preparation.costEstimate.projectedUsage.audioInputTokens > 0);
assert(preparation.costEstimate.projectedUsage.textInputTokens > 0);
assert(preparation.costEstimate.projectedUsage.outputTokens > 0);
assert.equal(
  preparation.costEstimate.projectedUsage.totalInputTokens,
  preparation.costEstimate.projectedUsage.audioInputTokens +
    preparation.costEstimate.projectedUsage.textInputTokens
);
...
assert.equal(preparation.executionPolicy.callsMaximum, 1);
...
assert.equal(
  ...
  "activate-and-execute-one-batch-17-paid-audio-verification-call-under-standing-authorization"
);
