...
import { POST_CANARY_BATCH_08_PUBLICATION_RESUMPTION_3_PROTOCOL_ID,
...
assertV4(p.protocolId === POST_CANARY_BATCH_08_PUBLICATION_RESUMPTION_3_PROTOCOL_ID &&
  p.status === "frozen-one-untouched-post-canary-batch-08-publication-resumption-3-context-prepared-not-authorized" &&
...
"the one-context Batch 8 publication resumption is not prepared");
...
const activation = { schemaVersion:
    "1.0-assessment-production-post-canary-batch-08-publication-resumption-3-execution-activation",
  protocolId: p.protocolId,
  status: "frozen-one-untouched-post-canary-batch-08-publication-resumption-3-context-authorized",
  ...
  nextAuthorizedAction: "execute-only-frozen-debate-145-publication-resumption-context-once" };
...
console.log(JSON.stringify({ status: activation.status,
  debates: activation.contexts.map((row) => row.debateNumber), contextsAuthorized: 1,
