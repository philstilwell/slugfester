...
import { extractAssessmentProductionPostCanaryBatch01Disagreements } from "./lib/assessment-production-post-canary-batch-01-disagreement.mjs";
...
assert.equal(analysis.batchNumber, 1);
...
assert.equal(analysis.adjudicationWorkload.totalMoves, 177);
assert.equal(
...
assert.equal(analysis.audioWorkload.moves, 3);
