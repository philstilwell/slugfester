...
const expectedDebates = ["70", "37", "111"];
const expectedMoves = [
  "con-shared-liability-neutrality",
  "con-impartial-source-standards",
  "pro-paul-hostile-vision-challenge",
  "pro-visions-cross-religious-distinction",
  "con-functional-mixed-linkage-rna",
  "pro-first-ribozyme-and-replication-gap",
  "con-joyce-self-replicating-ribozyme",
  "pro-surface-stability-reaction-incompatibility"
];
const expectedCalls = 8;
const durationOnlyPlanningEstimateUsd = 0.0553217;
const usageDerivedPlanningEstimateUsd = 0.2026;
const toolPath = "scripts/assessment-production-post-canary-batch-13-audio-verification-stage.mjs";
...
const standingAuthorization = await loadAndValidatePostCanaryBatch13StandingAuthorization();
...
async function activate() {
  ...
  const activation = {
    ...preparation,
    schemaVersion: "1.0-assessment-production-post-canary-batch-13-audio-verification-execution-manifest",
    status: "frozen-eight-post-canary-batch-13-paid-known-speaker-diarizations-authorized-under-standing-authorization",
    activatedAt,
    ...
    nextAuthorizedAction: "execute-exactly-eight-post-canary-batch-13-paid-audio-verification-calls-once-sequentially"
  };
...
async function validateActivation(manifest) {
  assert.equal(
    manifest.status,
    "frozen-eight-post-canary-batch-13-paid-known-speaker-diarizations-authorized-under-standing-authorization"
  );
...
async function run() {
  ...
  for (const [callIndex, call] of manifest.calls.entries()) {
    ...
    process.stdout.write(`[batch-13-audio] ${callIndex + 1}/${expectedCalls} starting ${call.debateNumber}:${call.moveId}\n`);
    ...
    results.push(result);
    process.stdout.write(`[batch-13-audio] ${callIndex + 1}/${expectedCalls} ${result.status}; cumulative $${result.cumulativeUsageDerivedEstimatedCostUsd.toFixed(7)}\n`);
  }
  ...
  const execution = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-13-audio-verification-model-execution",
    protocolId: manifest.protocolId,
    status: completed.length === expectedCalls
      ? "eight-post-canary-batch-13-paid-known-speaker-diarizations-completed"
      : "post-canary-batch-13-paid-diarization-incomplete",
    provider: "OpenAI Transcription API",
...
async function analyze() {
  ...
  const status = passed
    ? "passed-all-eight-post-canary-batch-13-confidence-moves-audio-verified"
    : executionComplete
      ? "post-canary-batch-13-audio-verification-unresolved"
      : "post-canary-batch-13-audio-verification-incomplete";
  const nextAuthorizedAction = execution.costCapReachedOrExceeded || !execution.directIncrementalCostCapControlPassed
    ? "user-review-required-before-any-batch-13-downstream-work-after-audio-cost-cap-event"
    : passed
      ? "prepare-freeze-and-push-batch-13-dispute-only-adjudication-packets-under-standing-authorization"
      : "standing-authorization-stop-new-approval-required-before-batch-13-audio-verification-failure-diagnosis";
  const authorization = {
  ...
  const audit = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-13-audio-verification-audit",
    protocolId: manifest.protocolId,
    status,
  ...
  const analysis = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-13-audio-verification-analysis",
    protocolId: manifest.protocolId,
    status,
    productionCanary: false,
    batchNumber: 13,
...
async function cost() {
  ...
  const result = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-13-audio-cost-control-analysis",
