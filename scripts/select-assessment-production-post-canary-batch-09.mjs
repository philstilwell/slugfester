...
const ROOT = "docs/assessment-production/post-canary-continuation-v1/batch-09";
...
const PRIOR_SELECTION = "docs/assessment-production/post-canary-continuation-v1/batch-08/selection.json";
const PRIOR_SELECTION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-08/selection-analysis.json";
const PRIOR_MUTATION_MANIFEST = "docs/assessment-production/post-canary-continuation-v1/batch-08/production-publication/mutation-manifest.json";
const PRIOR_COMPLETION_EXECUTION = "docs/assessment-production/post-canary-continuation-v1/batch-08/production-publication/generated-seo-correction/dependent-pilot-analysis-correction/execution.json";
const PRIOR_COMPLETION_ANALYSIS = "docs/assessment-production/post-canary-continuation-v1/batch-08/production-publication/generated-seo-correction/dependent-pilot-analysis-correction/analysis.json";
const PRIOR_COMPLETION_COMMIT = "596d08afe685c85b2356e1e13d5fede990c7f05c";
const SCRIPT = "scripts/select-assessment-production-post-canary-batch-09.mjs";
const TEST = "scripts/test-assessment-production-post-canary-batch-09-selection.mjs";
...
assertV4(!(await exists(FUTURE_SOURCE_PREPARATION)), "source preparation already exists before Batch 9 selection");
...
assertV4(
  ...
  "Batch 9 selection is not compatible with the frozen continuation policy"
);
...
assertV4(
  priorSelection.status === "eighth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed" &&
    priorSelection.protocolId === "assessment-production-post-canary-batch-08" &&
    priorSelection.productionSequenceOrdinal === 9 &&
    priorSelection.selected.length === 10 &&
    priorSelectionAnalysis.status === "eighth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization" &&
    priorSelectionAnalysis.selection.sha256 === sha256(controlBytes[PRIOR_SELECTION]) &&
    priorMutationManifest.status === "frozen-batch-08-production-publication-mutation-manifest-prepared" &&
    JSON.stringify(priorMutationManifest.explicitOrder) === JSON.stringify(priorSelectedNumbers) &&
    priorCompletionExecution.status === "passed-complete-batch-08-production-transaction-after-dependent-pilot-analysis-correction" &&
    ...
    priorCompletionExecution.totals.recursiveCorrections === 0 &&
    ...
    priorCompletionAnalysis.status === "batch-08-production-publication-accepted-after-dependent-pilot-analysis-correction" &&
  ...
  "Batch 8 has not satisfied the frozen successful-publication prerequisite"
);
assertV4(git("rev-parse", "--abbrev-ref", "HEAD") === "main", "Batch 9 selection must run on main");
assertV4(git("rev-parse", "HEAD") === PRIOR_COMPLETION_COMMIT, "Batch 9 selection checkpoint is not the exact Batch 8 publication commit");
assertV4(git("rev-parse", "origin/main") === PRIOR_COMPLETION_COMMIT, "Batch 8 publication commit is not the pushed origin/main checkpoint");
...
assertV4(
  JSON.stringify(priorSelectedNumbers) === JSON.stringify(ranked.slice(70, 80).map((item) => item.debateNumber)),
  "Batch 8 is not the immutable eighth rank slice"
);

const selected = [];
let totalEvents = 0;
let totalDurationMs = 0;
for (const [offset, rankedItem] of ranked.slice(80, 90).entries()) {
...
assertV4(selected.length === 10 && new Set(selected.map((item) => item.debateNumber)).size === 10, "Batch 9 did not select ten unique debates");

const checkpointCommit = git("rev-parse", "HEAD");
const selection = {
  schemaVersion: "1.0-assessment-production-post-canary-batch-selection",
  protocolId: "assessment-production-post-canary-batch-09",
  status: "ninth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed",
  frozenAt,
  checkpointCommit,
  branch: "main",
  ...
  stagingOnly: true,
  batchNumber: 9,
  productionSequenceOrdinal: 10,
  userAuthorization: {
    instruction: "I authorize Batch 9.",
    scopeInterpretation: "Materialize and source-validate only the deterministic Batch 9 selection under the frozen continuation policy after authenticating successful Batch 8 publication. This selection pass prepares no source packets, creates no Batch 9 standing authorization, executes no models, derives no scores, and performs no production mutation.",
  ...
  eligibility: {
    ...
    previouslyPublishedAndRemovedCount: 80,
    remainingBeforeBatchCount: ranked.length - 80,
    remainingBeforeBatchRankedSha256: sha256(serializedJson(ranked.slice(80))),
    selectedCount: selected.length,
    remainingUnselectedCount: ranked.length - 90,
    dyadicOnly: true,
  ...
  nextAuthorizedAction: "prepare-batch-09-standing-authorization-and-source-packets-under-user-authorization"
};
...
const analysis = {
  ...
  status: "ninth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization",
  analyzedAt: frozenAt,
  ...
  finding: "Successful Batch 8 production publication was authenticated at the pushed atomic commit. The immutable campaign rank then selected ordinals 81 through 90 as ten unique pending dyadic debates, and all thirty effective source files and ten canonical event chains passed without replacement.",
  decision: {
