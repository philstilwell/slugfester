...
assertV4(
  ...
  "the Batch 3 publication finalization is not prepared"
);
for (const [file, digest] of Object.entries(preparation.sourceHashes)) {
  assertV4(
    sha256(await readFile(path.resolve(file))) === digest,
    `Batch 3 finalization preparation source hash mismatch: ${file}`
  );
}
...
const activation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-03-publication-finalization-execution-activation",
  protocolId: POST_CANARY_BATCH_03_PUBLICATION_FINALIZATION_PROTOCOL_ID,
  status: "frozen-post-canary-batch-03-publication-finalization-authorized",
  activatedAt,
  ...
  nextRequiredAction:
    "execute-one-frozen-batch-03-deterministic-publication-finalization-pass"
};
