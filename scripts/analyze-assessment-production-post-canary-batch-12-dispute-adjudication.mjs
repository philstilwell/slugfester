...
const executionPath =
  `${POST_CANARY_BATCH_12_DISPUTE_ADJ_ROOT}/model-execution.json`;
...
assertV4(
  ...
  "Batch 12 adjudication execution is unavailable or crossed its boundary"
);
for (const [file, digest] of Object.entries(activation.sourceHashes)) {
  assertV4(
    sha256(await readFile(file)) === digest,
    `source hash mismatch during Batch 12 adjudication analysis: ${file}`
  );
}
...
for (const context of activation.contexts) {
  ...
  if (result.gateAcceptancePassed) {
    ...
    replay = validatePostCanaryBatch12DisputeAdjudicationOutput(
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-12-dispute-only-adjudication-analysis",
  ...
  status: passed
    ...
    : semanticPass
      ? timingPass
        ? scoreBlindPass
          ? "post-canary-batch-12-dispute-only-adjudication-gate-failed-isolation"
          : "post-canary-batch-12-dispute-only-adjudication-gate-failed-scoreblindness"
        : "post-canary-batch-12-dispute-only-adjudication-gate-failed-timing"
      : "post-canary-batch-12-dispute-only-adjudication-gate-failed-validation",
  ...
  gate: {
    ...
    requiredCandidateSelections: 528,
