...
const samplePath = `${V419_ROOT}/source-only-sample.json`;
const sample = JSON.parse(await readFile(path.resolve(root, samplePath), "utf8"));
assertV4(sample.status === "frozen-before-legacy-score-access" && sample.debates.length === 6 && !sample.selectionBoundary.legacyAssessmentContentAccessed && sample.audit.v417Overlap === 0 && sample.audit.v418Overlap === 0, "v4.1.9 fresh-six sample invalid");
...
const controlIds = new Set(selectV419ControlDebates(sample.debates.map((debate) => debate.debateId)));
...
for (const debate of sample.debates) {
  ...
  const packet = {
    schemaVersion: V419_PACKET_VERSION,
    ...
    modelInputBoundary: {
      ...
      excerptMaximumTokens: 100,
      excerptMaximumCharacters: 600,
...
if (shouldWrite) {
  await mkdir(path.resolve(root, `${V419_ROOT}/packets`), { recursive: true });
  await mkdir(path.resolve(root, `${V419_ROOT}/schemas`), { recursive: true });
  for (const item of packets) await writeFile(path.resolve(root, `${V419_ROOT}/packets/debate-${item.debate.number}.json`), `${JSON.stringify(item.packet, null, 2)}\n`);
  await writeFile(path.resolve(root, `${V419_ROOT}/schemas/primary.schema.json`), `${JSON.stringify(makeV419PrimarySchema(), null, 2)}\n`);
}
...
const inputs = {
  ...
  workflowSchemaBounded: "docs/assessment-workflow-v4.1.9.md",
  ...
  manual: `${V419_ROOT}/manual.md`,
  schema: `${V419_ROOT}/schemas/primary.schema.json`
};
const preparation = {
  schemaVersion: "4.1.9-schema-bounded-fresh-six-preparation",
  protocolId: V419_PROTOCOL_ID,
  status: shouldWrite ? "prepared-source-only-no-model-execution" : "preview",
  calibrationOnly: true,
  AIOnly: true,
  dyadicOnly: true,
  model: { label: V419_MODEL.label, slug: V419_MODEL.slug, primaryReasoningEffort: V419_MODEL.primaryReasoningEffort, reviewReasoningEffort: V419_MODEL.reviewReasoningEffort, authentication: "ChatGPT subscription", meteredApiCostUsdMaximum: 0 },
  sample: samplePath,
  debates: packets.map((item) => ({ debateNumber: item.debate.number, debateId: item.debate.debateId, family: item.debate.family, durationSeconds: item.debate.durationSeconds, controlSampleSelected: item.controlSampleSelected, packet: `${V419_ROOT}/packets/debate-${item.debate.number}.json`, rawOutput: `${V419_ROOT}/primary-outputs/debate-${item.debate.number}.json`, compiledOutput: `${V419_ROOT}/primary-compiled/debate-${item.debate.number}.json` })),
  inputs,
  endpointSchemaInheritance: { from: "v4.1.8-source-integrity-primary", structuralFieldsChanged: false, newSchemaKeywords: ["sourceSpan.excerpt.maxLength"], maxLengthPreviouslyAcceptedInRepositoryEndpointSchemas: true, deterministicTranslationFixtureRequired: true, newEndpointPreflightRequired: false },
  sourceIntegrityPolicy: { completeEventFileHashRequired: true, repositoryOwnedSourceTimes: true, excerptTokens: [12, 100], excerptMaximumCharacters: 600, minimumLexicalRecall: 0.8, minimumOrderedCoverage: 0.8, invalidContextRetryMaximum: 0, outputNormalizationAuthorized: false },
...
if (shouldWrite) await writeFile(path.resolve(root, `${V419_ROOT}/preparation-manifest.json`), `${JSON.stringify(preparation, null, 2)}\n`);
console.log(JSON.stringify({ status: preparation.status, outputSchemaVersion: V419_OUTPUT_VERSION, debates: 6, controls: preparation.controlPolicy.selectedNumbers, totalDurationHours: Number((sample.debates.reduce((sum, debate) => sum + debate.durationSeconds, 0) / 3600).toFixed(2)), primaryReasoningEffort: V419_MODEL.primaryReasoningEffort, excerptMaximumCharacters: 600, excerptMaximumTokens: 100, repositoryOwnedSourceTimes: true, meteredApiCostUsd: 0, transcriptionCostUsd: 0 }, null, 2));
