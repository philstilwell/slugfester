...
const [execution, output, packet, audioAudit] = await Promise.all(
  ["model-execution.json", "output.json", "packet.json"]
    .map((file) =>
      readFile(`${V223_AUDIO_ADJ_ROOT}/${file}`, "utf8").then(JSON.parse)
    )
    .concat([
      readFile(
        "docs/assessment-production/score-stability-v2.2.3-validation-cohort/audio-verification/audio-verification.json",
        "utf8"
      ).then(JSON.parse)
    ])
);
assertV4(
  ...
  "valid v2.2.3 audio-attribution execution unavailable"
);
const validation = await validateV223AudioAttributionAdjudicationOutput(
  output,
  packet
);
...
const analysis = {
  schemaVersion:
    "1.0-score-stability-v2.2.3-audio-attribution-adjudication-analysis",
  protocolId: V223_AUDIO_ADJ_PROTOCOL_ID,
  status: passed
    ? "v2.2.3-audio-attribution-adjudication-passed"
    : "v2.2.3-audio-attribution-adjudication-unresolved",
  ...
  nextAuthorizedAction: passed
    ? "prepare-v2.2.3-dispute-only-adjudication-packets"
    : "stop-v2.2.3-audio-attribution-unresolved"
};
if (shouldWrite) {
  await writeFile(
    `${V223_AUDIO_ADJ_ROOT}/analysis.json`,
