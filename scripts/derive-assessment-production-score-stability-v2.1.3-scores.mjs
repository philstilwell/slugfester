...
assertV4(
  manifest.status === "frozen-v2.1.3-single-deterministic-score-pass" &&
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen v2.1.3 score source hash mismatch`
  );
}
...
const calculated = deriveV213Scores(
  ledger,
  inputs.debateInputs,
