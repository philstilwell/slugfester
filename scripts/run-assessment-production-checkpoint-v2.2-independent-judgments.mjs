...
assertV4(
  ...
  "production checkpoint v2.2 independent-judgment execution is unauthorized"
);
assertV4(
...
async function executeContext(context, contextIndex) {
  ...
  assertV4(
    remainingGateMs > 0,
    "absolute production checkpoint v2.2 independent-judgment gate timeout reached before launch"
  );
  ...
  const temporary = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-pcv22-judgment-${context.debateNumber}-${context.reviewerPass.toLowerCase()}-`
    )
  );
  const isolatedCodexHome = await mkdtemp(
    path.join(
      os.tmpdir(),
      `slugfester-pcv22-judgment-home-${context.debateNumber}-${context.reviewerPass.toLowerCase()}-`
    )
  );
  ...
  try {
    ...
    const prompt = `Read manual.md, source-packet.json, judgment-packet.json, and schema.json completely; read nothing else. Act only as isolated independent performance Judge ${context.reviewerPass} for production checkpoint v2.2 Debate ${context.debateNumber}. Judge every locked move exactly once. The score-blind inventory, chronology, source evidence, routes, sections, weights, propositions, and attribution are immutable. Use only legal earlier-opposing targets exposed by the schema. Apply the response-component, partial-answer, burden-relevance, logical-coherence, evidence-warrant, precision, calibration, charity, confidence, and strict burden-residual anchors literally. Do not calculate scores, name a winner, claim audio review, alter candidate selection, or write Overall Commentary, AI Extension, or publication prose. The other independent judgment, all other debates, failed production-canary outputs, validation-cohort outputs, and all legacy assessment material are unavailable. Return exactly one schema-conforming JSON object and no commentary.`;
    process.stdout.write(
      `[production-checkpoint-v2.2-judgment] starting index ${contextIndex} ${activation.model.label}/${activation.model.reasoningEffort} Debate ${context.debateNumber} Pass ${context.reviewerPass}\n`
    );
    ...
    if (resultExists) {
      ...
      preservedJudgmentSha256 = sha256(
        await readFile(context.judgmentOutput)
      );
    }
    if (
      invocation.timedOut ||
  ...
  process.stdout.write(
    `[production-checkpoint-v2.2-judgment] Debate ${context.debateNumber} Pass ${context.reviewerPass} ${record.status} in ${(record.elapsedMs / 60000).toFixed(2)}m\n`
  );
  return record;
}
...
const execution = {
  schemaVersion:
    "1.0-production-checkpoint-v2.2-independent-judgment-model-execution",
  protocolId: activation.protocolId,
  status: passed
    ? "twenty-production-checkpoint-v2.2-independent-judgment-contexts-passed"
    : "production-checkpoint-v2.2-independent-judgment-gate-complete-with-failure",
