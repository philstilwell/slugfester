...
import {
  POST_CANARY_BATCH_09_DISPUTE_ADJ_PROTOCOL_ID,
  POST_CANARY_BATCH_09_DISPUTE_ADJ_ROOT,
  buildPostCanaryBatch09DisputeAdjudicationPacket,
  makePostCanaryBatch09DisputeAdjudicationSchema
} from "./lib/assessment-production-post-canary-batch-09-dispute-adjudication.mjs";
...
const USER_AUTHORIZATION = Object.freeze({
  instruction:
    "The frozen Batch 9 standing authorization permits automatic preparation, validation, freezing, activation, execution, analysis, committing, and pushing of score-blind dispute-only adjudication checkpoints for exactly the ten selected debates while every frozen gate passes, with subscription-backed and local direct incremental cost capped at $0.",
...
const inputPaths = {
  ...
  schema: `${POST_CANARY_BATCH_09_DISPUTE_ADJ_ROOT}/adjudication.schema.json`
};
const TOOL_SOURCES = [
  "scripts/lib/v4-lean-production.mjs",
  "scripts/lib/v4221175-decomposed-adjudication.mjs",
  "scripts/lib/v42211728-hard-route-adjudication.mjs",
  "scripts/lib/assessment-production-post-canary-batch-09-dispute-adjudication.mjs",
  "scripts/build-assessment-production-post-canary-batch-09-dispute-adjudication-packets.mjs",
  "scripts/test-assessment-production-post-canary-batch-09-dispute-adjudication-packets.mjs",
  "scripts/validate-assessment-production-post-canary-batch-09-dispute-adjudication-output.mjs",
  "scripts/preregister-assessment-production-post-canary-batch-09-dispute-adjudication.mjs",
  "scripts/test-assessment-production-post-canary-batch-09-dispute-adjudication-manifest.mjs"
];
...
const partialAudioRoot = `${COHORT_ROOT}/audio-verification-partial-18`;
const debate183AudioRoot = `${COHORT_ROOT}/audio-verification-debate-183-21`;
const audioCorrectionRoot =
  `${debate183AudioRoot}/evidence-boundary-correction-1`;
const partialAudioAuditPath = `${partialAudioRoot}/audio-verification.json`;
const partialAudioCostPath = `${partialAudioRoot}/cost-control-analysis.json`;
const partialAudioExecutionManifestPath = `${partialAudioRoot}/execution-manifest.json`;
const debate183AudioAuditPath = `${debate183AudioRoot}/audio-verification.json`;
const debate183AudioCostPath = `${debate183AudioRoot}/cost-control-analysis.json`;
const debate183AudioExecutionManifestPath =
  `${debate183AudioRoot}/execution-manifest.json`;
const audioCorrectionExecutionPath = `${audioCorrectionRoot}/execution.json`;
const audioCohortReplayPath = `${audioCorrectionRoot}/cohort-replay.json`;
const audioCorrectionAnalysisPath = `${audioCorrectionRoot}/analysis.json`;
...
const sourcePaths = [
  sourceAnalysisPath,
  partialAudioAuditPath,
  partialAudioCostPath,
  partialAudioExecutionManifestPath,
  debate183AudioAuditPath,
  debate183AudioCostPath,
  debate183AudioExecutionManifestPath,
  audioCorrectionExecutionPath,
  audioCohortReplayPath,
  audioCorrectionAnalysisPath,
...
const [
  ...
  partialAudioAudit,
  partialAudioCost,
  partialAudioExecutionManifest,
  debate183AudioAudit,
  debate183AudioCost,
  debate183AudioExecutionManifest,
  audioCorrectionExecution,
  audioCohortReplay,
  audioCorrectionAnalysis,
  judgmentPreparation,
  judgmentExecution,
  judgmentAnalysis,
...
assertV4(
  sourceAnalysis.status ===
      "post-canary-batch-09-deterministic-disagreements-extracted-standing-authorization-active-for-audio-work" &&
    JSON.stringify(sourceAnalysis.debates.map((item) => item.debateNumber)) ===
      JSON.stringify(EXPECTED_DEBATES) &&
    sourceAnalysis.adjudicationWorkload.disputedMoves === 173 &&
    sourceAnalysis.adjudicationWorkload.candidateSelections === 459 &&
    sourceAnalysis.adjudicationWorkload.packetsPrepared === false &&
    sourceAnalysis.adjudicationWorkload.modelContextsExecuted === 0,
  "Batch 9 deterministic disagreement workload changed"
);
assertV4(
  judgmentExecution.status ===
    ...
    judgmentAnalysis.status ===
      "twenty-post-canary-batch-09-independent-judgments-passed-standing-authorization-active-for-disagreement-extraction" &&
    judgmentAnalysis.totals.contexts === 20,
  "Batch 9 accepted independent-judgment boundary changed"
);
assertV4(
  ...
  "Batch 9 independent-judgment configuration changed"
);
...
assertV4(
  ...
  "Batch 9 standing authorization changed"
);
assertV4(
  partialAudioAudit.status ===
      "verified-two-available-batch-09-audio-work-items-debate-183-remains-blocked" &&
    partialAudioAudit.results.length === 2 &&
    partialAudioAudit.results.every((item) => item.status === "verified") &&
    partialAudioCost.status ===
      "batch-09-partial-audio-verification-cost-within-standing-cap" &&
    partialAudioExecutionManifest.calls.length === 2 &&
    debate183AudioAudit.status ===
      "batch-09-debate-183-audio-verification-has-unresolved-work-item" &&
    debate183AudioAudit.results.length === 2 &&
    debate183AudioCost.status ===
      "batch-09-complete-audio-verification-cost-within-standing-cap" &&
    debate183AudioCost.cumulativeBatch9UsageDerivedCostUsd === 0.224845 &&
    debate183AudioCost.maximumAuthorizedCumulativeBatch9CostUsd === 1 &&
    debate183AudioCost.withinAuthorizedCap === true &&
    debate183AudioExecutionManifest.calls.length === 2 &&
    audioCorrectionExecution.status ===
      "completed-one-batch-09-debate-183-deterministic-audio-evidence-boundary-correction" &&
    audioCorrectionExecution.correction.correctedVerification.status === "verified" &&
    audioCohortReplay.status ===
      "batch-09-complete-four-work-item-audio-verification-cohort-passed" &&
    audioCohortReplay.requiredItems === 4 &&
    audioCohortReplay.verifiedItems === 4 &&
    audioCohortReplay.unresolvedItems === 0 &&
    audioCorrectionAnalysis.status ===
      "batch-09-audio-verification-complete-after-bounded-evidence-boundary-correction",
  "Batch 9 accepted audio-verification boundary changed"
);
assertV4(
  ...
  "Batch 9 packet-preparation authorization changed"
);

const frozenAudioCalls = [
  ...partialAudioExecutionManifest.calls,
  ...debate183AudioExecutionManifest.calls
];
const acceptedAudioResults = [
  ...partialAudioAudit.results,
  ...debate183AudioAudit.results.map((item) =>
    item.moveId === audioCorrectionExecution.correction.moveId
      ? {
          ...item,
          status: "verified",
          verification: audioCorrectionExecution.correction.correctedVerification
        }
      : item
  )
];
...
for (const move of acceptedAudioResults) {
    ...
    assertV4(
      move.status === "verified" &&
        move.verification.status === "verified" &&
        Object.values(move.verification.checks).every(Boolean),
      `${move.debateNumber}:${move.moveId}: verified audio evidence unavailable`
    );
    const transcriptBytes = await readFile(move.transcriptPath);
    assertV4(
      sha256(transcriptBytes) === move.transcriptSha256,
    ...
    audioByDebateMove.set(`${move.debateNumber}:${move.moveId}`, {
      ...
      status: "verified",
      deterministicEvidence: move.verification,
      ...
      transcript: {
        path: move.transcriptPath,
        sha256: move.transcriptSha256,
...
assertV4(audioByDebateMove.size === 4, "Batch 9 verified audio population changed");

const schema = makePostCanaryBatch09DisputeAdjudicationSchema();
...
if (shouldWrite) {
  const immutableOutputs = [
    inputPaths.schema,
    `${POST_CANARY_BATCH_09_DISPUTE_ADJ_ROOT}/preparation-manifest.json`,
    `${POST_CANARY_BATCH_09_DISPUTE_ADJ_ROOT}/execution-preparation-manifest.json`,
    ...EXPECTED_DEBATES.flatMap((debateNumber) => [
      `${POST_CANARY_BATCH_09_DISPUTE_ADJ_ROOT}/packets/debate-${debateNumber}.json`,
      `${POST_CANARY_BATCH_09_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`
    ])
  ];
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const built = buildPostCanaryBatch09DisputeAdjudicationPacket(
  ...
  const provenancePath =
    `${POST_CANARY_BATCH_09_DISPUTE_ADJ_ROOT}/provenance/debate-${debateNumber}.json`;
  const outputPath =
    `${POST_CANARY_BATCH_09_DISPUTE_ADJ_ROOT}/outputs/debate-${debateNumber}.json`;
  const packetBytes = pretty(built.packet);
  const provenanceBytes = pretty({
    schemaVersion:
      "1.0-assessment-production-post-canary-batch-09-adjudication-candidate-provenance",
...
const preparation = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-09-dispute-only-adjudication-preparation",
  protocolId: POST_CANARY_BATCH_09_DISPUTE_ADJ_PROTOCOL_ID,
  status: shouldWrite
    ? "prepared-ten-isolated-post-canary-batch-09-dispute-only-adjudication-contexts"
    : "preview",
  frozenAt,
  ...
  acceptedSourceBoundary: {
    ...
    audioVerification: audioCohortReplayPath,
    allTwentyJudgmentsAccepted: true,
    allFourAudioMovesVerified: true,
    audioValidationOverlaysPreserved: 1,
    audioCorrectionCallsPreserved: 0,
    audioUsageDerivedEstimatedCostUsd: 0.224845,
  ...
  nextAuthorizedAction:
    "freeze-post-canary-batch-09-dispute-only-adjudication-execution-preparation-manifest-model-free-only"
};
assertV4(
  contexts.length === 10 &&
    disputedMoves === 173 &&
    disputedMoves === sourceAnalysis.adjudicationWorkload.disputedMoves &&
    candidateSelections === 459 &&
    candidateSelections === sourceAnalysis.adjudicationWorkload.candidateSelections &&
    audioVerifiedMoves === 4,
  "Batch 9 adjudication packet workload differs from frozen inputs"
);
assertV4(
  preparation.totals.maximumCopiedInputBytes <= 400000,
  `Batch 9 context ${preparation.totals.maximumCopiedInputBytes} bytes exceeds the frozen 400 KB correction ceiling`
);
if (shouldWrite) {
  await mkdir(POST_CANARY_BATCH_09_DISPUTE_ADJ_ROOT, { recursive: true });
