...
const workflowPath = "docs/assessment-workflow-v4.2.1.md";
const scriptPath = "scripts/select-v421-fresh-six.mjs";
const outputPath = "docs/calibration/v4.2.1/compact-fresh-six-gate/source-only-sample.json";
const salt = "slugfester-v4.2.1-compact-fresh-six-source-only";
...
const [poolText, ...priorSampleTexts] = await Promise.all([poolPath, ...priorSamplePaths].map((file) => readFile(path.resolve(root, file), "utf8")));
...
const priorIdsByVersion = Object.fromEntries(["v417", "v418", "v419"].map((version, index) => [version, priorSamples[index].debates.map((debate) => debate.debateId)]));
const exclusions = new Set([...pool.retiredDebateIds, ...v416DevelopmentIds, ...Object.values(priorIdsByVersion).flat()]);
...
for (const family of families) {
  ...
  for (const state of states.values()) {
    for (const [rank, candidate] of byFamily.get(family).entries()) {
...
const eligibleStates = [...states.entries()].filter(([key]) => {
  const [shortCount, mediumCount, longCount] = key.split(",").map(Number);
  return shortCount >= 1 && mediumCount >= 1 && longCount >= 1 && longCount <= 3;
}).map(([, state]) => state);
...
if (new Set(selected.map((candidate) => candidate.debateId)).size !== 6 || Object.values(bandCounts).some((count) => count < 1) || bandCounts["long-over-120"] > 3) throw new Error("v4.2.1 sample invariants failed");
if (selected.some((candidate) => Object.values(priorIdsByVersion).flat().includes(candidate.debateId))) throw new Error("v4.2.1 sample overlaps a prior fresh gate");
...
const artifact = {
  schemaVersion: "4.2.1-compact-fresh-six-sample",
  ...
  exclusions: { priorCalibrationIdsFromPool: pool.retiredDebateIds.length, v416DevelopmentDebates: ["55", "103", "161"], v417DiagnosticDebates: priorSamples[0].debates.map((debate) => debate.number), v418DiagnosticDebates: priorSamples[1].debates.map((debate) => debate.number), v419DiagnosticDebates: priorSamples[2].debates.map((debate) => debate.number), excludedDebateIds: [...exclusions].sort() },
  ...
  audit: { debates: selected.length, distinctTopicFamilies: new Set(selected.map((debate) => debate.family)).size, distinctDebateIds: new Set(selected.map((debate) => debate.debateId)).size, priorFreshGateOverlap: selected.filter((debate) => Object.values(priorIdsByVersion).flat().includes(debate.debateId)).length, durationBandCounts: bandCounts, minimumDurationSeconds: Math.min(...selected.map((debate) => debate.durationSeconds)), maximumDurationSeconds: Math.max(...selected.map((debate) => debate.durationSeconds)), localTranscriptChainsPresent: selected.filter((debate) => debate.transcriptChainPresentAtSelection).length, legacyAssessmentFields: 0 },
  authorization: { compactSourcePacketPreparation: true, primaryModelExecution: false, legacyComparison: false, paidTranscription: false, productionMutation: false, heldOutGate: false, all195Debates: false },
  sourceHashes
};
if (shouldWrite) {
