...
const finalLedgerPath = `${V223_FINAL_LEDGER_ROOT}/final-ledger.json`;
const manifest = JSON.parse(
  await readFile(path.resolve(manifestPath), "utf8")
);
assertV4(
  manifest.status ===
    "frozen-ten-debate-v2.2.3-deterministic-final-ledger-assembly" &&
  ...
  "v2.2.3 final-ledger manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen v2.2.3 source hash mismatch`
  );
}
