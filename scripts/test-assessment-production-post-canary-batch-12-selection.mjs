...
assert.equal(selection.status, "twelfth-post-canary-ten-debate-batch-selection-frozen-source-gate-passed");
assert.equal(analysis.status, "twelfth-post-canary-batch-selection-analysis-passed-awaiting-standing-authorization");
assert.equal(analysis.selection.sha256, sha256(selectionBytes));
assert.equal(selection.batchNumber, 12);
assert.equal(selection.productionSequenceOrdinal, 13);
...
assert.equal(selection.eligibility.previouslyPublishedAndRemovedCount, 110);
assert.equal(selection.eligibility.remainingBeforeBatchCount, 54);
assert.equal(selection.eligibility.selectedCount, 10);
assert.equal(selection.eligibility.remainingUnselectedCount, 44);
...
assert.equal(priorCompletionExecution.status, "passed-complete-batch-11-transaction-ready-for-atomic-commit");
assert.equal(priorCompletionExecution.totals.isolatedGeneratorRuns, 1);
assert.equal(priorCompletionExecution.totals.repositoryValidationRuns, 2);
assert.equal(priorCompletionExecution.totals.generatedDerivativeWrites, 12);
assert.equal(priorCompletionExecution.totals.dependentCorrectionWrites, 4);
...
assert.equal(priorCompletionExecution.productionTransaction.debatesPublished, 10);
assert.equal(priorCompletionExecution.productionTransaction.productionLedgerFiles, 10);
assert.equal(priorCompletionExecution.productionTransaction.referencesByteIdentical, true);
assert.equal(priorCompletionExecution.dependentRecovery.scoringFixtureFiles, 2);
assert.equal(priorCompletionExecution.dependentRecovery.calibrationAnalysisFiles, 2);
assert.equal(priorCompletionExecution.dependentRecovery.scoreChanges, 0);
assert.equal(priorCompletionExecution.dependentRecovery.ledgerChanges, 0);
assert.equal(priorCompletionAnalysis.status, "batch-11-production-publication-accepted-for-atomic-commit-after-generated-seo-correction");
assert.equal(priorCompletionAnalysis.decision.generatedSeoCorrectionGatePassed, true);
assert.equal(priorCompletionAnalysis.decision.batch11ProductionPublicationGatePassed, true);
assert.equal(priorCompletionAnalysis.decision.boundedRepositoryDependentCorrectionPassed, true);
...
assert.equal(sha256(serializedJson(ranked.slice(110))), selection.eligibility.remainingBeforeBatchRankedSha256);
assert.deepEqual(priorSelection.selected.map((item) => item.debateNumber), ranked.slice(100, 110).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.debateNumber), ranked.slice(110, 120).map((item) => item.debateNumber));
assert.deepEqual(selection.selected.map((item) => item.rankOrdinal), [111, 112, 113, 114, 115, 116, 117, 118, 119, 120]);
assert.deepEqual(selection.selected.map((item) => item.rankSha256), ranked.slice(110, 120).map((item) => item.rankSha256));
...
assert.deepEqual(selection.deterministicOrdering.selectedRankOrdinals, [111, 120]);
...
assert.equal(selection.nextAuthorizedAction, "prepare-batch-12-standing-authorization-and-source-packets-under-user-authorization");

console.log(JSON.stringify({ status: "passed", selectedDebates: selection.selected.map((item) => item.debateNumber), selectedRankOrdinals: [111, 120], sourceFiles: 30, replacements: 0, sourcePacketsPrepared: 0, modelContexts: 0, paidServiceCalls: 0, directCostUsd: 0, nextAuthorizedAction: selection.nextAuthorizedAction }, null, 2));
