...
import {
  POST_CANARY_BATCH_02_CALCULATED_SCORES_VERSION,
...
const manifest = JSON.parse(
  await readFile(
    path.resolve(`${POST_CANARY_BATCH_02_SCORE_ROOT}/score-pass-manifest.json`),
    "utf8"
  )
);
...
const validation = validatePostCanaryBatch02Scores(
  scores,
  ledger,
  inputs.debateInputs,
...
assertV4(
  scores.schemaVersion === POST_CANARY_BATCH_02_CALCULATED_SCORES_VERSION &&
    !scores.productionCanary &&
    scores.batchNumber === 2 &&
    scores.stagingOnly &&
    !scores.developmentValidationOnly &&
    scores.formulaBoundary.scoringPasses === 1 &&
    !scores.formulaBoundary.modelCalculatedScores,
  "post-canary Batch 2 score artifact identity mismatch"
);
...
const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-02-score-analysis",
  protocolId: scores.protocolId,
  status: passed
    ? "post-canary-batch-02-score-stability-gate-passed"
    : "post-canary-batch-02-score-stability-gate-failed",
  ...
  nextAuthorizedAction: passed
    ...
    : "user-approval-required-before-batch-02-score-stability-failure-diagnosis-only"
};
if (shouldWrite) {
  await writeFile(
