...
const continuationRecoveryRoot = `${root}/continuation-failure-recovery/level-1`;
const protocolId = "assessment-production-post-canary-batch-16-publication-final-seven";
...
const files = {
  ...
  continuationExecution: `${root}/model-execution-continuation.json`,
  continuationAnalysis: `${root}/continuation-analysis.json`,
  continuationRecoveryAnalysis: `${continuationRecoveryRoot}/analysis.json`,
  recoveredRampValidation108: `${root}/validations/debate-108.json`,
  recoveredRampValidation164: `${root}/validations/debate-164.json`,
  preparation: `${root}/final-seven-execution-preparation-manifest.json`,
  activation: `${root}/final-seven-execution-activation.json`,
...
if (mode === "--prepare") {
  const [originalActivation, originalExecution, recoveryAnalysis, recoveredValidation, continuationExecution, continuationAnalysis, continuationRecoveryAnalysis, recoveredRampValidation108, recoveredRampValidation164] = await Promise.all([
    files.originalActivation, files.originalExecution, files.recoveryAnalysis, files.recoveredValidation,
    files.continuationExecution, files.continuationAnalysis, files.continuationRecoveryAnalysis,
    files.recoveredRampValidation108, files.recoveredRampValidation164
  ].map(readJson));
  ...
  assert.equal(continuationExecution.status, "batch-16-publication-continuation-complete-with-failure");
  assert.deepEqual(continuationExecution.unattemptedContextIndexes, [3, 4, 5, 6, 7, 8, 9]);
  assert.equal(continuationAnalysis.status, "batch-16-publication-continuation-gate-failed");
  assert.equal(continuationRecoveryAnalysis.status, "batch-16-publication-continuation-level-1-one-field-recovery-passed");
  assert.equal(recoveredRampValidation108.status, "passed");
  assert.equal(recoveredRampValidation164.status, "passed");
  const contexts = originalActivation.contexts.slice(3);
  assert.deepEqual(contexts.map((item) => item.contextIndex), [3, 4, 5, 6, 7, 8, 9]);
  ...
  const sourceFiles = [
    ...
    files.continuationExecution, files.continuationAnalysis,
    files.continuationRecoveryAnalysis, files.recoveredRampValidation108,
    files.recoveredRampValidation164,
    `${root}/outputs/debate-108.json`, `${root}/outputs/debate-164.json`,
    `${root}/provenance/debate-108.json`, `${root}/provenance/debate-164.json`,
    ...Object.keys(originalActivation.sourceHashes),
    "scripts/assessment-production-post-canary-batch-16-publication-final-seven.mjs"
  ];
  const sourceHashes = {};
  ...
  const manifest = {
    schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-final-seven-preparation",
    protocolId,
    status: "frozen-seven-unattempted-batch-16-publication-contexts-prepared-not-activated",
    frozenAt: at,
    checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(),
    productionCanary: false,
    batchNumber: 16,
    stagingOnly: true,
    recoveredPriorContexts: { debates: ["16", "108", "164"], completeValidationPassed: true },
    ...
    executionPolicy: {
      ...
      schedulerRamp: [2],
      rampPhases: [
        { phase: "final-steady-two", maximumParallelContexts: 2, contextIndexes: [3, 4, 5, 6, 7, 8, 9], expansionRequiresAllValid: false }
      ],
    ...
    nextAuthorizedAction: "activate-seven-previously-unattempted-batch-16-publication-contexts"
  };
  if (shouldWrite) await writeFile(path.resolve(files.preparation), pretty(manifest));
  console.log(pretty({ status: shouldWrite ? manifest.status : "preview", debates: contexts.map((item) => item.debateNumber), contexts: 7, attemptsMaximum: 7, retriesMaximum: 0, maximumParallelContexts: 2, directIncrementalCostUsdMaximum: 0, modelExecutionAuthorized: false }));
  process.exit(0);
}
...
if (mode === "--activate") {
  ...
  assert.equal(preparation.status, "frozen-seven-unattempted-batch-16-publication-contexts-prepared-not-activated");
  ...
  const activation = { ...preparation, schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-final-seven-activation", status: "frozen-seven-unattempted-batch-16-publication-contexts-authorized", activatedAt: at, checkpointCommit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(), preparation: { path: files.preparation, sha256: sha256(preparationBytes) }, authorization: { ...preparation.authorization, modelExecution: true, deterministicAnalysis: true }, nextAuthorizedAction: "execute-seven-previously-unattempted-batch-16-publication-contexts-once" };
  if (shouldWrite) await writeFile(path.resolve(files.activation), pretty(activation));
  console.log(pretty({ status: shouldWrite ? activation.status : "preview", debates: activation.contexts.map((item) => item.debateNumber), contexts: 7, attemptsMaximum: 7, retriesMaximum: 0, directIncrementalCostUsdMaximum: 0, modelExecutionAuthorized: shouldWrite }));
  process.exit(0);
}

if (mode === "--run") {
  const activation = await readJson(files.activation);
  assert.equal(activation.status, "frozen-seven-unattempted-batch-16-publication-contexts-authorized");
  ...
  const execute = async (context) => {
    const temporary = await mkdtemp(path.join(os.tmpdir(), `batch-16-publication-final-seven-${context.debateNumber}-`));
    const temporaryHome = await mkdtemp(path.join(os.tmpdir(), `batch-16-publication-final-seven-home-${context.debateNumber}-`));
    ...
    try {
      ...
      console.error(`[batch-16-publication-final-seven] starting index ${context.contextIndex} Debate ${context.debateNumber}`);
...
if (mode === "--analyze") {
  ...
  const passed = execution.status === "seven-unattempted-batch-16-publication-contexts-passed" && valid.length === 7;
  const analysis = { schemaVersion: "1.0-assessment-production-post-canary-batch-16-publication-final-seven-analysis", protocolId, status: passed ? "batch-16-publication-final-seven-gate-passed" : "batch-16-publication-final-seven-gate-failed", batchNumber: 16, productionCanary: false, stagingOnly: true, execution: { contextsPlanned: 7, contextsAttempted: execution.contextsAttempted, contextsUnattempted: execution.contextsUnattempted, validContexts: execution.validContexts, invalidContexts: execution.invalidContexts, attempts: execution.attempts, retries: 0, timeoutExtensions: 0, maximumObservedConcurrency: execution.maximumObservedConcurrency }, validationReplay, totals: { debates: 7, critiques: valid.reduce((sum, item) => sum + item.validation.critiques, 0), exactSourceQuotes: valid.reduce((sum, item) => sum + item.validation.quoteExactSourceMatches, 0), overallCommentarySides: valid.reduce((sum, item) => sum + item.validation.overallCommentarySides, 0), aiExtensionSides: valid.reduce((sum, item) => sum + item.validation.aiExtensionSides, 0), modelContexts: execution.contextsAttempted, modelAuthoredScores: 0, scorePassesExecutedThisStage: 0, paidServiceCalls: 0, directIncrementalCostUsd: 0 }, integrity: { recoveredPriorContextsRetained: true, scoresRemainedImmutable: true, everyAcceptedOutputReplayedDeterministically: valid.every((item) => item.validationReplayed), retriesPerformed: false, timeoutExtensionsPerformed: false, productionMutated: false }, authorization: { failureDiagnosis: !passed, recoveryPacketPreparation: !passed, deterministicCohortFinalization: passed, scorePass: false, publicationFinalization: false, productionMutation: false, nextBatchSelection: false }, nextAuthorizedAction: passed ? "finalize-and-validate-the-complete-ten-debate-batch-16-publication-cohort" : "diagnose-and-recover-only-the-failed-publication-fields" };
