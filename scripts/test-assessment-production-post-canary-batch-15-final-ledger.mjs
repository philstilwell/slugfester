...
const scoreArtifactCandidates = [
  `${POST_CANARY_BATCH_15_FINAL_LEDGER_ROOT}/calculated-scores.json`,
  `${POST_CANARY_BATCH_15_FINAL_LEDGER_ROOT}/score-analysis.json`,
  `${POST_CANARY_BATCH_15_FINAL_LEDGER_ROOT}/score-pass-manifest.json`
];
...
const rebuiltLedger = buildPostCanaryBatch15FinalLedger(
  inputs.debateInputs,
  inputs.sourceHashes
);
...
assert.deepEqual(
  ...
  ["39", "48", "23", "162", "86", "159", "128", "98", "155", "178"]
);
assert.equal(storedLedger.audit.finalMoves, 191);
assert.equal(storedLedger.audit.disputedMoves, 186);
assert.equal(storedLedger.audit.pairSelections, 221);
assert.equal(storedLedger.audit.scoringFieldSelections, 320);
assert.equal(storedLedger.audit.burdenAdjustmentSelections, 20);
assert.equal(storedLedger.audit.candidateSelections, 561);
assert.equal(storedLedger.audit.meanMerges, 409);
...
assert.throws(() =>
  buildPostCanaryBatch15FinalLedger(mutatedInputs, inputs.sourceHashes)
);
...
assert.equal(analysis.totals.finalMoves, 191);
assert.equal(analysis.totals.disputedMoves, 186);
assert.equal(analysis.totals.candidateSelections, 561);
