...
import {
  ...
  ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_16_DISAGREEMENT_ROOT,
  extractAssessmentProductionPostCanaryBatch16Disagreements
} from "./lib/assessment-production-post-canary-batch-16-disagreement.mjs";
import {
...
const TOOL_SOURCES = [
  "scripts/extract-assessment-production-post-canary-batch-16-disagreements.mjs",
  "scripts/lib/assessment-production-post-canary-batch-16-disagreement.mjs",
  ...
  "scripts/test-assessment-production-post-canary-batch-16-disagreements.mjs"
];
...
assertV4(
  judgmentAnalysis.status ===
    "twenty-post-canary-batch-16-independent-judgments-passed-standing-authorization-active-for-disagreement-extraction" &&
    judgmentAnalysis.authorization.disagreementExtraction === false &&
    judgmentAnalysis.nextAuthorizedAction ===
      "extract-freeze-and-analyze-batch-16-disagreements-under-standing-authorization",
  "Batch 16 accepted-judgment gate or prior stop rule changed"
);
assertV4(
  JSON.stringify(judgmentAnalysis.pairs.map((pair) => pair.debateNumber)) ===
    JSON.stringify(EXPECTED_DEBATES),
  "Batch 16 independent-judgment population changed"
);
assertV4(
  ...
  "Batch 16 does not contain ten accepted, independent A/B judgment pairs"
);
assertV4(
  ...
  "Batch 16 twenty-context accepted execution contract changed"
);
assertV4(
  ...
  "Batch 16 model, authentication, score-blindness, or tie contract changed"
);
assertV4(
  executionActivation.executionPolicy.attemptsPerContext === 1 &&
    ...
    JSON.stringify(executionActivation.executionPolicy.schedulerRamp) ===
      JSON.stringify([1, 2]),
  "Batch 16 one-attempt or frozen 1-to-2 scheduler stop rule changed"
);
assertV4(
...
assertV4(
  ...
  "Batch 16 source-compatibility evidence changed"
);
assertV4(
  ...
  "Batch 16 accepted execution evidence changed"
);
...
for (const debateNumber of EXPECTED_DEBATES) {
  ...
  const disagreementPath =
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_16_DISAGREEMENT_ROOT}/disagreements/debate-${debateNumber}.json`;
...
assertV4(totalMoves === 194, "Batch 16 extraction must compare 194 moves");
assertV4(
  JSON.stringify(extractedAudioQueue.map(queueKey).sort()) ===
    JSON.stringify(independentGateQueue.map(queueKey).sort()),
  "Batch 16 frozen audio queue was not reproduced"
);

const analysis = {
  schemaVersion:
    "1.0-assessment-production-post-canary-batch-16-disagreement-extraction",
  protocolId:
    ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_16_DISAGREEMENT_PROTOCOL_ID,
  status:
    "post-canary-batch-16-deterministic-disagreements-extracted-standing-authorization-active-for-audio-work",
  ...
  standingAuthorization: {
    path: POST_CANARY_BATCH_16_STANDING_AUTHORIZATION,
  ...
  nextAuthorizedAction:
    "prepare-freeze-and-analyze-batch-16-local-audio-source-work-items-under-standing-authorization"
};

if (shouldWrite) {
  await mkdir(ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_16_DISAGREEMENT_ROOT, {
    recursive: true
  });
  await writeFile(
    `${ASSESSMENT_PRODUCTION_POST_CANARY_BATCH_16_DISAGREEMENT_ROOT}/analysis.json`,
