...
assert.equal(activation.batchNumber, 2);
assert.equal(activation.stagingOnly, true);
assert.equal(
  activation.userAuthorization.instruction,
  "I approve."
);
assert.match(activation.userAuthorization.approvedScope, /exactly the twenty frozen Batch 2/);
assert.equal(
...
assert.equal(activation.sourceCompatibility.occurrences.length, 1);
assert.equal(activation.sourceCompatibility.occurrences[0].debateNumber, "99");
assert.equal(activation.transport.maximumCopiedInputBytes, 100023);
...
if (analysis.acceptance.passed) {
  ...
  assert.equal(analysis.totals.uniqueMoves, 190);
  assert.equal(analysis.totals.movesJudgedAcrossPasses, 380);
  assert.equal(
  ...
  assert.equal(
    analysis.status,
    "post-canary-batch-02-independent-judgment-gate-failed-analysis-only"
  );
}
console.log(
  JSON.stringify(
