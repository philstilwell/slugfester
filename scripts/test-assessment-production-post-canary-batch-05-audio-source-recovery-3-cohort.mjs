...
assert.equal(execution.routeDiscoverySha256, sha256(discoveryBytes));
...
assert.equal(
  execution.status,
  "completed-one-shot-official-broadcaster-recovery-3-and-audio-cohort"
);
assert.equal(
  analysis.status,
  "accepted-complete-three-source-six-clip-cohort-after-official-broadcaster-recovery"
);
...
assert.deepEqual(execution.state, {
  attempts: 1,
  debate189OfficialDownloadCliInvocations: 1,
...
assert.equal(preparation.schemaVersion.includes("transport-recovery-3"), true);
...
assert.equal(preparation.recovery.routeDiscovery.sha256, sha256(discoveryBytes));
...
assert.equal(preparation.recovery.canonicalSourceVideoIdPreserved, "3DHvNRK452c");
assert.equal(preparation.recovery.debate158ProtectedHashesPreserved, 4);
assert.equal(preparation.recovery.debate189TransportRecoveryAttempts, 3);
assert.equal(preparation.recovery.debate189OfficialBroadcasterAttempts, 1);
...
assert.equal(preparation.recovery.privateCookieReads, 0);
...
assert.equal(preparation.acquisitionPolicy.debate189AuthorizedTransportRecoveryAttemptsMaximum, 3);
assert.equal(preparation.acquisitionPolicy.completeCohortPublicSourceAttemptsMaximum, 6);
assert.equal(preparation.acquisitionPolicy.officialBroadcasterSameEpisodeDeliveryPermittedForDebate189, true);
assert.equal(preparation.acquisitionPolicy.canonicalEvidenceSourceVideoIdChanged, false);
assert.equal(preparation.acquisitionPolicy.officialDownloadExpectedBytes, 51845141);
...
assert.deepEqual(
  ...
  [
    ...
    },
    {
      ...
      recoveryAttempts: 3,
      maximum: 4,
      outcome: "success-via-official-broadcaster-route"
    },
    {
      debateNumber: "05",
      attempts: 1,
...
assert.equal(preparation.publicSourceAttemptAudit.reduce((sum, item) => sum + item.attempts, 0), 6);
assert.deepEqual(preparation.executionBoundary, {
...
for (const directory of [
  ...
  assert(
    (await readdir(directory)).every(
      (entry) => !entry.includes("source.recovery-3.") &&
...
assert.equal(analysis.result.officialBroadcasterRouteValidated, true);
assert.equal(analysis.result.canonicalSourceVideoIdPreserved, true);
assert.equal(analysis.result.sourcesValidated, 3);
assert.equal(analysis.result.clipsValidated, 6);
assert.equal(analysis.result.remainingClipsCreated, 3);
assert.equal(analysis.result.failedPartialOutputsReused, 0);
assert.equal(analysis.result.privateCookieReads, 0);
