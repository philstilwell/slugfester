...
assertV388(manifest.status === "frozen-one-synthetic-context-authorized" && manifest.authorization.syntheticSchemaPreflightModelExecution && !manifest.authorization.debatePerformanceModelExecution, "schema preflight unauthorized");
...
const temporary = await mkdtemp(path.join(os.tmpdir(), "slugfester-v388-schema-preflight-"));
const temporaryCodexHome = await mkdtemp(path.join(os.tmpdir(), "slugfester-v388-home-schema-preflight-"));
...
try {
  ...
  if (invocation.code !== 0 || invocation.signal !== null || invocation.timedOut) result = { ...base, status: "endpoint-preflight-failed", outputWritten: false, packetAwareValidationPassed: false };
    ...
    const validation = await run(process.execPath, ["scripts/validate-v388-performance-judgment-output.mjs", manifest.artifacts.output, manifest.input.packet, "A"], { cwd: root, env: process.env });
