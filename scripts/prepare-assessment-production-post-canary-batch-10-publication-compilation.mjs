...
import {
  ...
  POST_CANARY_BATCH_10_PUBLICATION_COMPILATION_ROOT,
...
import { POST_CANARY_BATCH_10_PUBLICATION_ROOT } from
  "./lib/assessment-production-post-canary-batch-10-publication.mjs";
...
import { POST_CANARY_BATCH_10_STANDING_AUTHORIZATION,
  loadAndValidatePostCanaryBatch10StandingAuthorization } from
...
const ROOT = POST_CANARY_BATCH_10_PUBLICATION_COMPILATION_ROOT;
const COHORT = `${POST_CANARY_BATCH_10_PUBLICATION_ROOT}/failure-recovery/original-unattempted-context-resumption-2/field-disjoint-repair-1/complete-cohort-analysis.json`;
...
const ACCEPTED_OUTPUTS = Object.freeze(Object.fromEntries(
  POST_CANARY_BATCH_10_PUBLICATION_COMPILATION_ORDER.map((debateNumber) => [
    debateNumber,
    `${POST_CANARY_BATCH_10_PUBLICATION_ROOT}/outputs/debate-${debateNumber}.json`
  ])
));
const acceptedOutput = (debateNumber) => ACCEPTED_OUTPUTS[debateNumber];
const packetPath = (debateNumber) =>
  `${POST_CANARY_BATCH_10_PUBLICATION_ROOT}/packets/debate-${debateNumber}.json`;
...
assertV4(
  cohort.status ===
    "post-canary-batch-10-complete-ten-debate-publication-output-gate-passed-after-field-disjoint-repair" &&
  cohort.totals?.debates === 10 && cohort.totals?.lockedMoves === 182 &&
  cohort.totals?.critiques === 182 && cohort.totals?.exactSourceQuotes === 20 &&
  cohort.integrity?.scoresRemainedImmutable === true &&
  cohort.integrity?.everyAcceptedOutputReplayedDeterministically === true,
"the accepted Batch 10 publication cohort changed");
...
for (const debateNumber of POST_CANARY_BATCH_10_PUBLICATION_COMPILATION_ORDER) {
  ...
  const validation = validatePostCanaryBatch10PublicationOutput(output, packet);
  ...
  const compiled = compilePostCanaryBatch10PublicationStagingRecord({ output, packet, identity });
  const compiledValidation = validatePostCanaryBatch10CompiledStagingRecord({ compiled, output, packet, identity });
...
"the Batch 10 compilation cohort changed");
const identitySnapshot = { schemaVersion: "1.0-assessment-production-post-canary-batch-10-publication-compilation-identity-snapshot",
  protocolId: POST_CANARY_BATCH_10_PUBLICATION_COMPILATION_PROTOCOL_ID,
...
const sources = [COHORT, POST_CANARY_BATCH_10_STANDING_AUTHORIZATION,
  ...
  "scripts/lib/assessment-production-post-canary-batch-10-publication-compilation.mjs",
  "scripts/lib/assessment-production-post-canary-batch-10-standing-authorization.mjs",
  "scripts/lib/v4-lean-production.mjs",
  "scripts/prepare-assessment-production-post-canary-batch-10-publication-compilation.mjs",
  "scripts/test-assessment-production-post-canary-batch-10-publication-compilation-preparation.mjs",
  "scripts/activate-assessment-production-post-canary-batch-10-publication-compilation.mjs",
  "scripts/run-assessment-production-post-canary-batch-10-publication-compilation.mjs",
...
const manifest = { schemaVersion: "1.0-assessment-production-post-canary-batch-10-deterministic-publication-compilation-preparation",
  protocolId: POST_CANARY_BATCH_10_PUBLICATION_COMPILATION_PROTOCOL_ID,
  status: "frozen-post-canary-batch-10-deterministic-publication-compilation-prepared",
  ...
  userAuthorization: { standingAuthorization: POST_CANARY_BATCH_10_STANDING_AUTHORIZATION,
  ...
  explicitOrder: POST_CANARY_BATCH_10_PUBLICATION_COMPILATION_ORDER, contexts,
  ...
  nextAuthorizedAction: "activate-one-batch-10-deterministic-publication-compilation-pass" };
