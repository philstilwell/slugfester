...
import {
  ...
  derivePostCanaryBatch01Scores
} from "./lib/assessment-production-post-canary-batch-01-score-gate.mjs";
import { assertV4 } from "./lib/v4-lean-production.mjs";

const manifestPath = `${POST_CANARY_BATCH_01_SCORE_ROOT}/score-pass-manifest.json`;
...
assertV4(
  ...
  "post-canary Batch 1 score manifest invalid"
);
...
for (const [file, expected] of Object.entries(manifest.sourceHashes)) {
  assertV4(
    ...
    `${file}: frozen Batch 1 score source hash mismatch`
  );
}
...
const calculated = derivePostCanaryBatch01Scores(
  ledger,
  inputs.debateInputs,
