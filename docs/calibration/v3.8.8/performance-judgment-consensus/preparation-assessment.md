...
The corrected synthetic output used relevance/burden 50 and satisfied every substantive invariant, but the historical charity check required the exact contiguous phrase “not tested.” The output instead said “no charity test occurred.” The deterministic validator now accepts those equivalent formulations while still requiring `charityTested: false` and exactly 75. Revalidating the unchanged model output passes. The exact shared schema has therefore passed the live endpoint and the full packet-aware contract without another model call.
