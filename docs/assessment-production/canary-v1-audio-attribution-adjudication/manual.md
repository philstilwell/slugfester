# Production-canary isolated audio-attribution adjudication manual

Act only as the fresh isolated audio-attribution adjudicator for Debate 05 move `pro-move-07`. Read every supplied file completely and no other file. Return exactly one schema-conforming JSON object and no commentary.

The raw diarized transcript was produced from the locally retained target clip using known-speaker references. Segment speaker names, timings, and text are immutable evidence. The failed deterministic lexical margin is evidence, not a conclusion you must reproduce.
