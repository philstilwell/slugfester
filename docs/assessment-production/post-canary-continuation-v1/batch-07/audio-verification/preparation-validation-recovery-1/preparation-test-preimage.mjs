...
assert.deepEqual(preparation.scope.debates, ["193", "78", "02"]);
assert.equal(preparation.scope.frozenTargetClips, 5);
assert.equal(preparation.scope.verificationCalls, 5);
...
assert.deepEqual(preparation.costEstimate.projectedUsage, {
  audioInputTokens: 8438,
  textInputTokens: 570,
  outputTokens: 14288,
  totalInputTokens: 9008
});
...
assert.equal(preparation.executionPolicy.callsMaximum, 5);
...
assert.equal(
  ...
  "activate-and-execute-five-batch-07-paid-audio-verification-calls-under-standing-authorization"
);
