# Post-canary Batch 2 candidate-sharded inventory contract

## Scope

This contract governs the score-blind inventory stage for Debates 103, 172, 04, 136, 83, 66, 126, 99, 93, and 101 under the active v2.2 score-stability policy. It prepares a three-context route for each debate without executing an inventory model, deriving a score, preparing an independent judgment, performing audio work, reconstructing publication prose, or changing production.

The passed Batch 2 discovery bundles are the only candidate source. Every discovered candidate remains present. The repository may project candidates into lossless transports, but it may not silently deduplicate, semantically repair, rewrite, truncate, relocate, or delete a candidate. Debate 99’s exact redaction-only source row remains preserved with a repository lexical-token count of zero, and the unchanged twelve-token minimum still governs every candidate window.
...
Earlier failed canary outputs and validation-cohort outputs remain historical evidence only. They are unavailable to every fresh Batch 2 model context.

## Audio and score boundaries

Discovery found no below-high-confidence attribution candidates in Batch 2, so this preparation records no current audio trigger. If a later stage independently creates an audio trigger under the frozen rules, verification remains mandatory before judgment or scoring. This preparation performs no audio work.
