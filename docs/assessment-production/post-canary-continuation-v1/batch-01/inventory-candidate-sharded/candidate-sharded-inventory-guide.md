# Post-canary Batch 1 candidate-sharded inventory contract

## Scope

This contract governs the score-blind inventory stage for Debates 31, 94, 52, 146, 91, 175, 75, 72, 13, and 195 under the active v2.2 score-stability policy. It prepares a three-context route for each debate without executing an inventory model, deriving a score, preparing an independent judgment, performing audio work, reconstructing publication prose, or changing production.

The passed Batch 1 discovery bundles are the only candidate source. Every discovered candidate remains present. The repository may project candidates into lossless transports, but it may not silently deduplicate, semantically repair, rewrite, truncate, relocate, or delete a candidate.
...
Earlier failed canary outputs and validation-cohort outputs remain historical evidence only. They are unavailable to every fresh Batch 1 model context.

## Audio and score boundaries

Discovery found one medium-confidence attribution candidate in Debate 146. If that candidate is later selected into the locked inventory, audio verification is mandatory before judgment or scoring. This preparation performs no audio work.
