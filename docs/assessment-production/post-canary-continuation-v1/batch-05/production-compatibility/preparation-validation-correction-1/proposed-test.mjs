...
const preparationPath =
  `${POST_CANARY_BATCH_05_COMPATIBILITY_ROOT}/preparation-manifest.json`;
const analysisPath =
  `${POST_CANARY_BATCH_05_COMPATIBILITY_ROOT}/analysis.json`;
...
assert.equal(preparation.totals.sections, 49);
assert.equal(preparation.totals.moves, 187);
assert.equal(preparation.totals.oneSidedDisplayRows, 7);
assert.equal(preparation.totals.overallBlunders, 64);
...
assert.equal(
  await exists(`${POST_CANARY_BATCH_05_COMPATIBILITY_ROOT}/execution.json`),
  false
);
assert.equal(
  await exists(
...
assert.deepEqual(packetNumbers, POST_CANARY_BATCH_05_COMPATIBILITY_ORDER);
for (const packetRecord of preparation.artifacts.packets) {
  ...
  const positive = validatePostCanaryBatch05SiteLedgerAdapter({
