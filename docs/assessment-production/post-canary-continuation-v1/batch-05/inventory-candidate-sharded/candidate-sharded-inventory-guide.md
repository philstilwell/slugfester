# Post-canary Batch 5 candidate-sharded inventory contract

## Scope

This contract governs the score-blind inventory stage for Debates 158, 46, 64, 132, 189, 109, 179, 05, 42, and 59 under the active v2.2 score-stability policy. It prepares a three-context route for each debate without deriving a score, preparing an independent judgment, performing audio work, reconstructing publication prose, or changing production.

The passed Batch 5 discovery bundles are the only candidate source. Every discovered candidate remains present. The repository may project candidates into lossless transports, but it may not silently deduplicate, semantically repair, rewrite, truncate, relocate, or delete a candidate. No nonempty Batch 5 source row has a zero repository lexical-token count; every exact source row remains preserved, and the unchanged twelve-token minimum still governs every candidate window.
...
Earlier failed canary outputs and validation-cohort outputs remain historical evidence only. They are unavailable to every fresh Batch 5 model context.

## Audio and score boundaries

Discovery found one below-high-confidence attribution candidate in Batch 5. A candidate creates an audio trigger only if a later side selector retains it; every such retained trigger requires verification before judgment or scoring. This preparation performs no audio work.
