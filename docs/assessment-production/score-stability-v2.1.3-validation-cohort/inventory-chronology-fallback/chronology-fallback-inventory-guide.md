# v2.1.3 chronology-fallback inventory contract

## Scope

This contract governs the score-blind inventory stage for the fresh v2.1.3 validation cohort. It prepares a three-context route for each debate without executing a model, deriving a score, promoting the proposed score-stability policy, reclassifying the failed v2.1.2 gate, or authorizing production mutation.

The complete passing v2.1.3 discovery bundle is the only candidate source. Every discovered candidate remains present. The repository may project that bundle into lossless transports, but it may not silently deduplicate, semantically repair, rewrite, truncate, relocate, or delete a candidate.
...
The failed v2.1.2 selector outputs are development evidence only. They are not accepted for the current gate and are unavailable to every fresh v2.1.3 model context.

## Audio and score boundaries

The five below-high attribution discovery candidates are not audio-verified during preparation. If any is retained in a locked inventory, the existing rule requiring later audio verification remains mandatory.
...
The user accepts genuine ties after integer rounding. That instruction remains recorded as the unpromoted v2.1 proposal; it does not alter or retroactively pass any failed gate.
