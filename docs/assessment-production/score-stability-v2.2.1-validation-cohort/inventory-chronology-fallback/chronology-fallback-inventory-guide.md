# v2.2.1 chronology-fallback inventory contract

## Scope

This contract governs the score-blind inventory stage for the fresh v2.2.1 validation cohort. It prepares a three-context route for each debate without executing a model, deriving a score, promoting the proposed score-stability policy, reclassifying the failed v2.2 discovery gate or the failed v2.1.3 score gate, or authorizing production mutation.

The complete mechanically validated v2.2.1 recovery bundle is the only candidate source. It was compiled from the frozen v2.2 raw outputs after repository-owned canonical ordering; the v2.2 execution remains failed and was not retried. Every discovered candidate remains present. The repository may project that bundle into lossless transports, but it may not silently deduplicate, semantically repair, rewrite, truncate, relocate, or delete a candidate.
...
Every deferred nomination remains in the reduction audit. The repository then merges the two side maps in exact source chronology. It preserves each retained `preferredMoveKind` unless that move is a reply with no earlier selected opposing move. Only in that case does it apply the selector-authored constructive fallback. The repository records every applied fallback, restores validator-only evidence from the hash-locked full evidence bundle, and replays the unchanged legacy inventory compiler and all uniqueness, source, chronology, section-coverage, and schema checks.

The failed v2.1.2 selector outputs are development evidence only. They are not accepted for the current gate and are unavailable to every fresh v2.2.1 model context. The failed v2.2 execution metadata is also unavailable to those contexts; only the mechanically validated candidate content may be transported.

## Audio and score boundaries

The one below-high attribution discovery candidate is not audio-verified during preparation. If it is retained in a locked inventory, the existing rule requiring later audio verification remains mandatory.
...
The user accepts genuine ties after integer rounding. That instruction remains recorded in the unpromoted v2.2 proposal; it does not alter or retroactively pass any failed gate.
