...
const allowedOutputCalls = calls.filter(x => /tools\.apply_patch\(/.test(x.payload.input));
...
assert(finalWrite.payload.input.includes(path.dirname(plan.allowedOutputs[0]) + '/'));
assert(finalWrite.payload.input.includes('model-output.json') && finalWrite.payload.input.includes('reading-authentication.json'));
