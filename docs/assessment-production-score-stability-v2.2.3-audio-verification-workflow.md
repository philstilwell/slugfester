# Score-stability v2.2.3 audio-verification workflow

This validation-only stage audio-verifies the four below-high-confidence moves frozen by the v2.2.3 independent-judgment gate. It uses the two locally normalized source files and four immutable clips recorded in `audio-source-preparation.json`.
...
This stage makes no 5.6 Sol judgment call and derives no score. Audio verification must pass for all four moves before adjudication packet preparation can be authorized. Adjudication execution, ledger assembly, score derivation, policy promotion, publication, production mutation, and remaining production batches remain unauthorized.
