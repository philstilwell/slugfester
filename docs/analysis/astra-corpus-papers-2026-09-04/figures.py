...
CONTRACTS=[]
DIM=list(R['p1']['dimensions']); DL=['Coherence','Evidence / warrant','Responsiveness','Relevance / burden','Precision / clarity','Confidence / fairness']
...
def save(fig,key,title,scope,unit,source):
    ...
    CONTRACTS.append(dict(id=key,title=title,scope=scope,unit=unit,source=source,
      design='Two semantic hues (teal, rust) plus neutral ink. Direct labels. Zero on all bars. Intervals are 95% resampling ranges, not measurement-error bounds.'))
...
def margin_hist(key,title,rows,field,scope):
    ...
    bins=np.arange(-20,29,2);cnt,edges=np.histogram(vals,bins=bins)
    ax.bar(edges[:-1]+1,cnt,width=1.8,color=[RUST if x<0 else TEAL for x in edges[:-1]])
...
ax.set_xticks(range(6),['Coherence','Evidence','Reply','Burden','Clarity','Confidence'],fontsize=9);ax.set_yticks(range(8),[TL[t['topic']] for t in ts]);ax.tick_params(length=0)
for i in range(8):
...
ax.scatter([t['pro'] for t in cross],[t['con'] for t in cross],s=[15+t['n']*3 for t in cross],color=TEAL,alpha=.75,edgecolors='white')
...
fig,axs=plt.subplots(1,2,figsize=(7.3,4.0));labels=['Logic','Evidence','Reply','Burden','Clarity','Confidence']
...
fig.subplots_adjust(wspace=.43,bottom=.24,top=.89,right=.92);cax=fig.add_axes([.94,.27,.015,.5]);fig.colorbar(im,cax=cax,label='Correlation')
...
for i,r in enumerate(top):
    ax.plot(r['model_rank_ci'],[i+.12]*2,color=RUST,lw=2.4)
    ax.plot(r['empirical_rank_ci'],[i-.12]*2,color=TEAL,lw=2.4)
    ax.scatter(r['rank'],i,c=NAVY,s=20,zorder=4)
ax.set_yticks(range(15),[f"{r['speaker']} ({r['n']})" for r in top],fontsize=9);ax.invert_yaxis();ax.set_xlim(.5,32);ax.set_xlabel('Rank within the fixed 50-speaker field (1 = highest)');ax.set_title('Current leaders and their plausible rank ranges',loc='left');ax.plot([],[],c=TEAL,lw=2,label='Observed-score resampling');ax.plot([],[],c=RUST,lw=2,label='Pooled-variation model');ax.legend(frameon=False,fontsize=9,loc='upper center',bbox_to_anchor=(.5,-.11),ncol=2);ax.grid(axis='x',color=LIGHT,lw=.5);ax.set_axisbelow(True);fig.tight_layout();save(fig,'p7-ranges','Current leaders and their plausible rank ranges','Top 15 current means; 95% ranges from different uncertainty models; dots show display order','Rank positions',top)
...
bars('p7-top10','Top-ten membership in the pooled-variation model',[r['speaker'] for r in ranks[:12]],[100*r['model_top10'] for r in ranks[:12]],'20,000 draws of latent means; fixed 50-speaker field; model-conditional probabilities','Share of model draws in top ten (%)',ranks[:12],percent=True)
