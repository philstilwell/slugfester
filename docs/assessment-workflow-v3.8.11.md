# Slugfester End-to-End Consensus Workflow v3.8.11

Workflow v3.8.11 is a calibration-only clean-execution rerun on retired Debates 55, 103, and 161 after the failed v3.8.9 lexical validator and the failed v3.8.10 packet-blind synthetic preflight. It inherits only the hash-locked source, coverage, section, burden-contact, and audio-verification records from v3.8.8; no v3.8.8, v3.8.9, or v3.8.10 performance judgment, score, reconstruction prose, or audit conclusion may enter a model context. It tests first-attempt validity, reply-complete reconstruction, and AI-novelty discipline before another held-out gate. A pass may authorize a separately preregistered ten-debate gate. It cannot authorize the 195-debate rollout by itself.

## v3.8.11 clean-gate amendments
...
- The isolated preflight receives both the exact schema and the frozen synthetic packet; packet-aware identity fields may never be reconstructed from prompt prose.
...
1. **Freeze the gate.** Commit the sample, source hashes, inherited classifications, coverage rules, schemas, thresholds, costs, and stop rules before any v3.8.11 model call.
...
8. **Run Scoring Pass A and Pass B.** Use separate fresh 5.6 Sol contexts with the identical locked packet and the single v3.8.11 scoring-judgment schema. Neither context may access the other pass, legacy scores or prose, calculated totals, a winner, Overall Commentary, or AI Extension material.
