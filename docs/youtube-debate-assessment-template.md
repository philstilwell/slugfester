...
- The header includes a `Rankings` link to `/rankings/`, which ranks interlocutors with at least three debate appearances by their average overall score; a multi-person side assigns its published side score to each listed participant. Each ranking row labels the speaker's average as `First name's avg.` and separately labels the opponents' average score. Readers can filter by topic, set a higher appearance threshold, re-rank by the average published scores of opponents faced, change sorting, and expand a speaker's performance details.
