...
- The header includes a `Rankings` link to `/rankings/`, whose page title is `Rankings & Flags`. It ranks interlocutors with at least three debate appearances by their average overall score; a multi-person side assigns its published side score to each listed participant. Each ranking row labels the speaker's average as `First name's avg.` and separately labels `Opponents' Avg.`; the speaker's most common topic appears beneath their debate count. Readers can filter by topic, set a higher appearance threshold, re-rank by `Opponents' Avg.`, and change sorting.
