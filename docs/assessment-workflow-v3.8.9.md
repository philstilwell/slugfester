# Slugfester End-to-End Consensus Workflow v3.8.9

Workflow v3.8.9 is a calibration-only clean-execution rerun on retired Debates 55, 103, and 161. It inherits only the hash-locked source, coverage, section, burden-contact, and audio-verification records from v3.8.8; no v3.8.8 performance judgment, score, reconstruction prose, or adversarial-audit conclusion may enter a model context. It tests first-attempt validity, reply-complete reconstruction, and AI-novelty discipline before another held-out gate. A pass may authorize a separately preregistered ten-debate gate. It cannot authorize the 195-debate rollout by itself.

## v3.8.9 clean-gate amendments
...
1. **Freeze the gate.** Commit the sample, source hashes, inherited classifications, coverage rules, schemas, thresholds, costs, and stop rules before any v3.8.9 model call.
...
8. **Run Scoring Pass A and Pass B.** Use separate fresh 5.6 Sol contexts with the identical locked packet and the single v3.8.9 scoring-judgment schema. Neither context may access the other pass, legacy scores or prose, calculated totals, a winner, Overall Commentary, or AI Extension material.
