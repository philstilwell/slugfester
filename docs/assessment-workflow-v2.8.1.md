# Slugfester Reassessment Workflow v2.8.1
...
The original v2.8 development manifest, input, key, passes, and analysis are immutable failed attempt-1 artifacts. v2.8.1 uses a new attempt-2 directory, schema version, selection ledger, input, independently readjudicated key, manifest, pass files, and analysis. No attempt-1 result is recomputed.
...
Attempt 2 uses retired material only. Its selection ledger excludes cases whose attempt-1 disagreements exposed unresolved semantic ambiguity and adds clear retired rare-feature fixtures as needed. The hidden key is newly readjudicated under v2.8.1 without access to either attempt-1 pass.
